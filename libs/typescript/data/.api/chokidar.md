...
- `ssh2` / `webdav` / `basic-ftp` (`libs/typescript/.api/ssh2.md`, `.api/webdav.md`, `.api/basic-ftp.md`): the remote halves of the watch strategy row — exec-push where the host carries a notify tool, `Schedule`-driven poll diffs elsewhere; the strategy row dispatches on origin kind, and this package is only ever the `file:` arm.
