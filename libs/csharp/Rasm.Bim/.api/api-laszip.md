...
| [INDEX] | [SYMBOL]            | [TYPE_FAMILY]   | [RAIL]                                                                  |
| :-----: | :------------------ | :-------------- | :--------------------------------------------------------------------- |
|  [01]   | `laszip_point`      | point record    | `X`/`Y`/`Z` (raw ints), `intensity`, `classification`/`extended_classification`, `gps_time`, `rgb[4]` (RGB + NIR), `wave_packet[29]`, `extra_bytes`, the return/scan/flag bit-fields |
...
[INTEGRATION_STACK]:
- with `Themis.Las` (`api-themis-las`): the `Exchange/reconstruct#LAS_INGEST` fold composes the TWO as one ingest front discriminated by compression — `Themis.Las.LasReader` decodes the `.las` uncompressed leg producing the `LasPoint` whose `Position` is already a `MathNet.Numerics.LinearAlgebra.Vector<double>`, and `LASzip.Net.laszip` decodes the `.laz` compressed leg producing a raw `laszip_point` the fold lifts into the SAME `MathNet` vector through `get_coordinates` + `Vector<double>.Build.Dense(coords)`; one `LasCloud` carrier, two decode engines, the kernel registration agnostic to which decoded — the `reconstruct#LAS_INGEST` `LasIngest.Decode` switches on the `.las`/`.laz` extension (or the `open_reader` `is_compressed` flag), never two parallel ingest models
- MathNet seam: the decoded `get_coordinates` `double[]` lifts to the `MathNet.Numerics.LinearAlgebra.Vector<double>` the kernel `csharp:Rasm/Vectors#ALIGN` cloud-ICP registration and the `csharp:Rasm.Compute/Tensor/blas#DENSE_ALGEBRA` covariance/PCA normal estimation consume — the LAZ point batch enters the Compute dense-LA substrate through the same vector the `Themis.Las` uncompressed point already is, no second point model
- content identity: the source-cloud `.laz`/`.las` bytes are content-keyed through the `csharp:Compute/Runtime/codecs#CONTENT_ADDRESSING` `InterchangeIdentity.Key` (the `reconstruct#RECONSTRUCTION` `ReconstructionLineage` `[ValueObject<UInt128>]`), never a second hashing scheme over the LAS/LAZ bytes
- classification seed: the `laszip_point.classification`/`extended_classification` ASPRS byte seeds the `reconstruct#RECONSTRUCTION` `ElementClassifier` `(PrimitiveShape, IfcDomain, orientation)` discipline-hint table exactly as the `Themis.Las` `LasPoint.Classification` does — the LAZ leg feeds the same classifier, never a per-codec classification branch
- georeference: the `header.vlrs` CRS WKT (record_id 2112) and the GeoTIFF geokeys feed the `csharp:Rasm.Bim/Semantics/georeference#GEOREFERENCE` `GeoReference` projection through the `ProjNET` datum leg — the same georeferencing seam the `Themis.Las` VLR reads, never a laszip-local reprojection
...
- the decoded point lifts through `get_coordinates` into the `MathNet.Numerics` vector the kernel consumes (the same vector `Themis.Las.LasPoint.Position` is), and the raw `laszip_point`/`laszip_header` types never leak past the `LasIngest` fold — internal code holds the canonical `LasCloud`/`SegmentedCloud` per the boundary-mapping law
...
- Accept: the `Exchange/reconstruct#LAS_INGEST` LAZ-capable decode leg producing the raw point set lifted into the `MathNet.Numerics` vector the kernel registration consumes, selective decode via `decompress_selective`, windowed reads via the `.lax` `inside_rectangle` path, and the CRS WKT VLR threaded onto `Semantics/georeference#GEOREFERENCE`
- Reject: a re-minted point-cloud scan/segmentation/registration engine (the kernel owns it); a second point model beside the `Themis.Las` `LasPoint`/`LasCloud`; a hand-rolled LAZ arithmetic decoder or a native libLASzip binding; a second hashing scheme over the LAS/LAZ bytes beside `InterchangeIdentity.Key`; a laszip-local CRS reprojection beside `ProjNET`; decoding unneeded channels where `decompress_selective` masks them; ILMerging the LGPL-2.1 assembly into a Rasm assembly
