...
`coloraide` supplies the pure-Python color-engine surface for the `graphic/color/derive#DERIVE` palette source and the `graphic/color/managed#MANAGED` egress provenance: a single `Color` object parses CSS, named, and coordinate input; converts across registered color spaces; maps gamut; applies CVD and W3C effects; measures difference and contrast; builds interpolations; composites color stacks; derives harmonies; and reads CCT and chromaticity. In `derive`, `everything.ColorAll` is the all-plugins engine the closed `ColorOp` family folds onto the closed `Derivation` result family; each case owns its notation, path, measures, or `GamutFit` evidence, and `coords` is the common projection. `Color.fit`/`in_gamut` own the gamut leg, `Color.filter` the effect leg, `Color.steps`/`discrete`/`interpolate` the palette leg, `Color.layer`/`weighted_mix` the composition leg, `Color.harmony`/`closest`/`average` the wheel and seed leg, `Color.blackbody`/`cct` the Planckian leg, and `Color.delta_e` the perceptual difference rows `colour-science` lacks. LUT, plate, and swatch terminals mint `ArtifactReceipt.Color` on `managed`; derive mints no receipt.
...
- evidence: each `Derivation` case captures only its operation's evidence: resolved space, channel coordinates, CSS notation, path, a `Metric`-keyed `frozendict[Metric, float]`, or the `GamutFit` witness. `Derivation.coords` projects the coordinate array common to every case.
...
- `coloraide` -> `graphic/color/managed#MANAGED` (egress provenance, not pixel transform): the resolved color-space id (`space()`) and the `GamutFit` method the gamut/CSS leg records on `Derivation` feed the `managed` ICC/LUT raster-egress leg as the color-space provenance it consumes; tone-curve/CCTF provenance is `managed`'s own `GradeStep.cctf` axis, and the device-to-device ICC pixel transform is pyvips `icc_transform` on the `managed` side (Pillow `ImageCms` limited to `buildProofTransform` soft proofing and profile-header reads), never inside `coloraide`.
- `coloraide` -> figures/visuals/document owners: the channel `Vector` (`coords`/`get`/`split_chromaticity`/`Y`), the `Interpolator` sampled per stop, and the serialized `to_string` CSS strings feed `visualization/chart/spec#CHART`, `scene/render#SCENE`, `visualization/table#TABLE`, `graphic/marks/encode#MARK`, and the document output directly as numeric data + CSS, never via a render call inside this package.
- universal-rail stacking (`libs/python/.api`, layered ON TOP of this folder catalog): a resolved `Color` projects into one case of `derive#DERIVE`'s `Derivation` family, never an `NDArray` wire owner or erased `object`. `beartype.vale.Is`-guarded `Annotated` contracts refine scalar bounds, vector rank, cardinality, dtype, and finiteness; field-spanning factories return `Result[ColorOp, DeriveFault]` for source-model, route, payload, and arity failures. `expression` threads the result through `RuntimeRail[Derivation]`, `LanePolicy.offload` supplies the interpreter-local CPU crossing, and `numpy` materializes `coords` as the case's array payload.
