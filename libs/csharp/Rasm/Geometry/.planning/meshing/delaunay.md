...
- Packages: `Rasm`/Vectors (`Point3d`/`Vector3d`/`MeshSpace` — composed for vertex geometry and the result projection), `Rasm.Geometry.Numerics` (`Predicate` `InCircle`/`InSphere`/`Orient2D`/`Orient3D` direct and `OrientLPI`/`OrientTPI` implicit-point predicates with `Lpi`/`Tpi` exact constructions — the exact-sign floor, now landed, composed never re-minted), `Rasm.Geometry` (`GeometryKeyPolicy` string-key comparer — composed, never re-minted), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox.
