...
- Packages: `hash-wasm` `createXXHash128(0, 0)` `IHasher` resolved once at the worker composition root and threaded as the production binding; `effect` `Schema.decodeSync(ContentKey)` to lift the frozen literal into the branded slot (the settled `Codec/frame.md` `h128` idiom).
...
- Cases: the snapshot-header fingerprint crosses through the `Codec/codec.md` `decodeSnapshotHeader` `getBigUint64(0, false)` big-endian read (the `littleEndian: false` flag load-bearing); the op-log `logical`/`sequence` bigints cross through the reused `snapshotDecoder` `Decoder({ useBigInt64: true })` mapping the MessagePack 64-bit integers to bigint. The round-trip asserts the decoded bigint equals the canonical two-half composition `(physicalHigh << 32n) | logicalLow` order, so a half-swap that reads the logical half into the physical slot is caught before it corrupts the convergence order.
...
- Growth: the frozen `HLC_TWO_HALF` reference stamp lands as one `HLC_REFERENCE` row the binding asserts against the moment `csharp:Rasm.AppHost/Runtime/ports#HLC_FANIN` pins it; the `assertsReference` leg is one `bytesEqual` assertion over the big-endian-written 8-byte buffer against that frozen row — zero algebra change, the `compose`/`roundTrip` algebra already settled; zero second HLC encoding notion.
