...
- `ICurrentEnvironment` — `ForBackground` / `ForReflectionAndRefraction` / `ForLighting -> RenderEnvironment` (get/set); the current-environment usage triple `RhinoDoc.CurrentEnvironment` returns, and that accessor is `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-document.md`'s.
...
- `RenderContentCollection.GetForcedVaries() -> bool` / `SetForcedVaries(bool)` / `GetSearchPattern() -> string` / `SetSearchPattern(string)` / `ContentNeedsPreviewThumbnail(RenderContent c, bool includeChildren) -> bool` — editor selection-set state the content-editor interface reads; the thumbnail probe takes TWO parameters, so a single-argument call does not compile.
...
- Content authoring and configuration are this catalog; live texture evaluation (`CreateEvaluator`, `SimulateTexture`) and post-effect execution are `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-render.md` — one graph configures, the other evaluates, and the two never merge.

[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Transform`/`Interval` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): content `bool` and index outcomes project to `Fin<Unit>`/`Fin<int>`; nullable content reads (`FromId`, `Find`, `FindChild`) lift to `Option<RenderContent>`; table and collection rosters land as `Seq<A>`; a `RenderContent`/`SimulatedEnvironment`/`TextureMapping` `IDisposable` rides a `using` bounded by `Eff`, and a failed create or refused assign crosses as `Fin<A>`.
...
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-document.md`: `RhinoDoc.RenderMaterials`/`RenderEnvironments`/`RenderTextures` are the table accessors, and `RenderMaterial.ToMaterial`/`ConvertToPhysicallyBased` bridge to the `Rhino.DocObjects.Material`/`PhysicallyBasedMaterial` document surface the document catalog owns.
...
- `FieldDictionary` is the sole parameter owner; the render-hash policy is `RenderHashExclude` over bounded flags; a re-derived hash or a stringly parameter map beside the field dictionary is rejected.
