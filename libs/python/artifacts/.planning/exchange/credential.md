...
class Provenance:
    ...
    def emit(self, lane: LanePolicy, /) -> ArtifactWork[tuple[ContentKey, bytes, CredentialEvidence]]:
    ...
    async def close(self, lane: LanePolicy, /) -> RuntimeResult[tuple[ContentKey, bytes, CredentialEvidence]]:
        outcome = await lane.offload(Kernel.of(self._run, KernelTrait.RELEASING))
        return outcome.map(lambda pair: (ContentIdentity.key(f"credential.{self.tag}", pair[0]), pair[0], pair[1]))

    async def _emit(self, lane: LanePolicy, /) -> RuntimeResult[tuple[ContentKey, bytes, CredentialEvidence]]:
        settled = await self.close(lane)
        match settled:
            case Result(tag="ok", ok=(key, payload, evidence)):
                Metrics.record({BYTE_VOLUME: float(len(payload))}, domain=DOMAIN, kind="credential", scope=lane.scope)
                return Ok((key, payload, evidence))
            case refused:
