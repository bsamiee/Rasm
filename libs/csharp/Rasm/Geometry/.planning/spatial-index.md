...
Rasm geometry-domain spatial-acceleration lane: ONE polymorphic `SpatialIndex` owner that closes the entire broad-phase acceleration concern over a `[Union]` of two first-principles author-kernels — a SAH-partitioned bounding-volume hierarchy (`Bvh`) and a Morton-order linear octree (`LinearOctree`) — built over ONE flat `NodeStore` value layout so a query, a refit, and a Compute-seam projection read the same struct-of-arrays node memory regardless of which kernel produced it. The page owns the `SpatialKind` discriminant, the `SpatialKey` ordinal key-policy accessor, the `NodeStore` struct-of-arrays node memory, the `SpatialIndex` `[Union]` with its `Build`/`Refit` rail and its `Query` fold, the `SpatialQuery` `[Union]` query algebra (`Nearest`/`Range`/`Ray`/`Overlap`) with the typed `QueryResult` carrier, and the `ToAcceleration` projection that hands the flat node store to the Compute `solver-and-optimization#CLASH_AND_TWIN` `AccelerationStructure` consumer at the in-process seam without re-minting a second acceleration structure. It composes RhinoCommon `BoundingBox`/`Point3d`/`Vector3d`/`Ray3d`/`Sphere`/`Line` through the `Rasm`/Vectors substrate as SETTLED vocabulary — read public shapes, compose, NEVER re-mint — and operates on raw primitive coordinates because the index is a geometric-coordinate structure, not a unit-bearing quantity surface.

Wire posture: this page is HOST-LOCAL and carries no TS_PROJECTION cluster — the index is interior acceleration substance that crosses no browser or peer wire. The ONLY cross-package egress is the `ToAcceleration` projection that returns the Compute `solver-and-optimization#CLASH_AND_TWIN` `AccelerationStructure` `[Union]` value DIRECTLY (`Bvh(ReadOnlyMemory<float> Bounds, ReadOnlyMemory<long> Nodes, int LeafSize)` / `Octree(ReadOnlyMemory<float> Origin, double Extent, ReadOnlyMemory<long> Cells, int MaxDepth)`) — the geometry domain mints NO parallel `Acceleration` record duplicating that union's field shape, because a domain-local acceleration-structure record IS the rejected double-owner form the Boundary paragraph deletes. The immutable `NodeStore` is a hash-friendly record the Persistence blob lane content-addresses by reference — the geometry domain computes no hash and mints no second store.
