...
- Boundary: `ParseError` from output decodes maps into this family at the decode seam; the severity fold over accumulated faults rides `DeployFault.bySeverity`.
...
- Law: `Effect.tryPromise` supplies the fiber-bound `AbortSignal`, and the caller's `onEvent` callback remains the direct engine observation seam.
...
- Law: `Automation.Options` derives from the run-option seam with `signal` reserved for `Effect.tryPromise`; the option arrays keep the engine's mutable-array spelling at the boundary.
