    ...
    public static Option<IndexedColour> Of(float[] lane, long[] corners) {
        ...
        for (int t = 0; t < corners.Length / 3; t++) {
            ...
            Option<(byte R, byte G, byte B)> encoded = AppearanceProjection
                .Bytes(lane[at], lane[at + 1], lane[at + 2], 1.0)
...
public static class AppearanceProjection {
    ...
    public static Fin<StyledAppearance> Project(IfcStyledItem styledItem, double tolerance) =>
    ...
    static Fin<AppearanceSummary> SummaryOf(IfcSurfaceStyle surface) {
    ...
    static Fin<AppearanceSummary> Neutral(
        ...
        Option<IfcSurfaceStyleRefraction> refraction, (double R, double G, double B) surfaceBase) {
    ...
    public static IfcStyledItem Author(DatabaseIfc db, AppearanceSummary summary, Seq<SurfaceTexture> textures, Option<SurfaceLighting> lighting, bool doubleSided) {
        ...
        IfcSurfaceStyle style = new(
            rendering,
            HostEdge.Slot(lighting.Map(row => row.Author(db))),
    ...
    static readonly Lazy<FrozenDictionary<EncodingChannel, Func<IfcStyledItem, IfcTriangulatedFaceSet, float[], long[], long, Fin<Option<IfcPresentationItem>>>>> Binders =
        new(static () => new Dictionary<EncodingChannel, Func<IfcStyledItem, IfcTriangulatedFaceSet, float[], long[], long, Fin<Option<IfcPresentationItem>>>> {
            ...
            [EncodingChannel.ColorRgba] = static (_, faceSet, lane, corners, reach, key) => Coloured(faceSet, lane, corners, reach).Map(static map => map.Map(static item => (IfcPresentationItem)item)),
        }.ToFrozenDictionary());
