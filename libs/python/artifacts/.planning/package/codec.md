...
def _zstd_frame(frame: bytes, /) -> tuple[bytes, bytes]:
    ...
    # small-declared frame with a huge window-log would force an oversized window allocation before decode; the window
    # bound refuses it up front (zstandard .api bomb axis), the two gates together contain the frame.
    obj = zstandard.ZstdDecompressor(max_window_size=_DECOMPRESS_WINDOW).decompressobj(read_across_frames=False)
