...
- Packages: `Rasm.Geometry.Intersection` (sibling file — `Intersection.Apply`/`IntersectOp.PlaneMesh`/`IntersectResult.Chains`/`Chain`/`IntersectPolicy`, composed never re-founded), `Rasm.Geometry.Numerics` (`Predicate.Orient2D(in Implicit, in Implicit, in Implicit, Axis)` + `Predicate.Compare` + `Sign`/`Axis` — the exact nesting signs), `Rasm.Geometry.Meshing` (`MeshEdit.Of` — the one soup adapter feeding the frame pass), `Rasm.Geometry` (`GeometryFault`), `Rasm.Domain` (`Op`, `Kind`, `ValidityClaim`/`IValidityEvidence`), `Rasm`/Vectors (`Point3d`/`Vector3d`/`Plane`/`Polyline`/`BoundingBox`/`MeshSpace`), QuikGraph (`BidirectionalGraph<int, SEdge<int>>`, `AddVertexRange`/`AddVerticesAndEdge`, `IsDirectedAcyclicGraph`, `ComputeTransitiveReduction`, `Roots` — in-computation only, per the bounded-lane law), CommunityToolkit.HighPerformance (`MemoryOwner<T>` pooled result slots, `ArrayPoolBufferWriter<T>` channel emit, `ParallelHelper.For` + `IAction` struct partition), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox (`Array.BinarySearch`).
...
// normal, a binned steepest-slope table (|n·d| per elevation bin), and the sorted overhang-start
// elevations feeding the support-interface law. Vertical = the nesting projection plane.
public sealed record SliceFrame(Plane Datum, Axis Vertical, double Lo, double Hi, double[] MaxSlope, double[] OverhangStarts) {
    public static SliceFrame Of(MeshSpace mesh, Plane datum, SlicePolicy policy) {
        ...
        for (int f = 0; f < soup.FaceCount; f++) {
            ...
            if (cos < 0.0) { overhang.Add(fl); }
        }
        return new SliceFrame(datum, DominantOf(d), lo, hi, slope, [.. overhang.Order()]);
    }
    ...
    public bool NearInterface(double z, double band) {
        int at = Array.BinarySearch(OverhangStarts, z);
        int next = at >= 0 ? at : ~at;
        return (next < OverhangStarts.Length && OverhangStarts[next] - z <= band)
            || (next > 0 && z - OverhangStarts[next - 1] <= band);
    }
...
public abstract partial record LayerPlan {
    ...
    public Fin<Arr<double>> Elevations(SliceFrame frame, SlicePolicy policy) =>
        Admit().Bind(_ => Switch(
            ...
            supportInterface: static (s, i) => March(s.Frame, s.Policy, z => s.Frame.NearInterface(z, i.InterfaceLayers * i.InterfaceHeight) ? i.InterfaceHeight : i.BaseHeight),
            atElevations:     static (s, x) => x.Elevations.ForAll(e => e > s.Frame.Lo && e < s.Frame.Hi)
                && toSeq(x.Elevations).Zip(toSeq(x.Elevations).Skip(1)).ForAll(static pair => pair.Left < pair.Right)
...
// The five-channel SoA forest wire (the FAB:48 / Compute-circulation decoder schema) + typed
...
public static class Slicing {
    ...
    // reduced in-degree <= 1 (laminar), Roots the layer's outermost boundaries.
    static Fin<Unit> Nest(SliceFrame frame, Seq<Chain> closed, int baseOrdinal, List<int> parent, int layer, double elevation, int openCount) {
        ...
        Axis u = Axis.Get(frame.Vertical.U);
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                switch (Parity(anchors[i], closed[j].Points, frame.Vertical, u, v)) {
    ...
    static bool? Parity(Point3d anchor, Polyline ring, Axis plane, Axis uAxis, Axis vAxis) {
        bool inside = false;
        for (int i = 0; i < ring.Count - 1; i++) {
            (Point3d s, Point3d t) = (ring[i], ring[i + 1]);
            Sign sv = Predicate.Compare(s, anchor, vAxis);
            Sign tv = Predicate.Compare(t, anchor, vAxis);
            ...
            Sign side = Predicate.Orient2D(s, t, anchor, plane);
...
flowchart LR
    ...
    SliceStack -->|five-channel SoA wire| Decoders["FAB:48 toolpath · Compute circulation"]
...
|  [1b]   | Run facts       | `SliceFrame` | derived record — extent, binned steepest slope, overhang starts, nesting axis (one soup pass)      | value (read by the height laws)            |    —    |
