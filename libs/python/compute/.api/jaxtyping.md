...
- concern: array shape/dtype contracts
...
|  [08]   | `TypeCheckError`                          | error type     | annotation breach the `boundary` fence converts to its result  |
...
[TOPOLOGY]:
- Annotations stay inert type metadata until `jaxtyped` weaves a typechecker over the call; shared symbolic axis names then bind ranks and shapes across every argument in one call scope, so a mismatch faults at entry rather than mid-solve.

[STACKING]:
- `beartype`(`../../.api/beartype.md`): `jaxtyped(typechecker=beartype(conf=FAULT_CONF))` folds the shape/dtype check into the one `FAULT_CONF` boundary fence, so a breach faults on the same typed failure as the finiteness refinement.
