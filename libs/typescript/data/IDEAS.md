...
[GENERATION_RECOVERY_CONTRACT]-[COMPLETE]: landed at `lane/capability#CONTRACT` as `Backend.Objective`/`Window`/`Reading`/`Observation`, `_window` deriving off the observation's own stamps, the `_WINDOWS` axis table carrying the opposite absence polarity as a row column, and both axes seated as independent `_Check` rows inside the one admission stage. Wire shapes untouched, so no peer decode ripples.
[HOST_OPLOG_CRDT_CONSUMER]-[COMPLETE]: decode lands on `journal/append#ATOMIC_PUBLISH`, admitting through the same compiled family the windowed read runs, so a producer-side schema move rides the one generation. `state/causal` already separated the dot from the content digest and needed only its encode sorted by replica.
