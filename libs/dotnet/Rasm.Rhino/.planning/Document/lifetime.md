...
- Law: kernel `Custody.Rollback` is the failure-arm custody rail and `Custody.Settled` its BOTH-ARMS sibling. Cleanup refusals append to the primary; release never rides `ignore` or a success-only `.Map`.
