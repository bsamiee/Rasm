...
and scoped through a disposable. This catalog GUIDES the `Planning/cost#ESTIMATE` rebuild to RETIRE the
hand-rolled `MonetaryAmount`/`CostValue` carriers (`(double Amount, string Currency)` + the
manual `Add`/`Scale`/`Rate` fold) in favor of `Money` + `Currency` + `ExchangeRate`: a
`CostItem` value becomes `Money`, the `CostSchedule.Rollup` becomes a `Money` sum, the
per-line `Rate` becomes `Money / decimal`, and the cross-currency rollup composes
`ExchangeRate.Convert`. `Money` meets the admitted `UnitsNet` (`api-unitsnet`) at the
`CostItem.ValueOf` quantity x rate join — `Money * quantity.As(SiUnit)` — the dimensioned
quantity supplying the multiplier and `Money` supplying the priced scalar.
...
- dependency: the `net10.0` dependency group is EMPTY — the `System.Text.Encodings.Web`/`System.Text.Json` 4.7.2 deps the nuspec declares apply ONLY to the `netstandard2.0`/`netstandard2.1` groups (those facades are in-box under net10), so the net10 bind pulls no transitive package
...
|  [07]   | `NodaMoney.Context.CashDenominationRounding` | rounding strategy   | `record(decimal decimals)` — cash rounding to a denomination (e.g. CHF 0.05), the "smallest coin" rule |
|  [08]   | `Transaction`                              | multi-currency bag   | `class` aggregating mixed-currency `Money` (a wallet/ledger of per-currency totals) |
...
|  [04]   | `ExchangeRate.Parse` / `TryParse`                          | FX parse         | parse a `"USD/EUR 1.2345"`-style rate string      |
...
[INTEGRATION_STACK]:
- with `UnitsNet` (`api-unitsnet`): the `Planning/cost#ESTIMATE` `CostItem.ValueOf` join is the canonical stack — `UnitsNet` owns the dimensioned takeoff (a `Volume`/`Area`/`Length` from `QuantitySet.Derive`, read in its SI base through `.As(SiUnit)`), `Money` owns the priced scalar, and the line value is `unitRate * (decimal)quantity.As(SiUnit)` (`Money * decimal → Money`) — one rail from dimensioned quantity and priced rate to a `Money` line, never a `double` cross-multiply; `Money / decimal` applies the `UnitBasis` per-basis denominator
...
- with `LanguageExt.Core`: an unknown ISO 4217 code degrades through a `Currency.FromCode` guard (catch `InvalidCurrencyException`) lowered onto `Model/faults#FAULT_BAND` `BimFault.CodecReject`/`Fin<T>` via `.ToError()` — never a thrown currency exception in domain code; the `CostSchedule.Rollup` per-category total is `lines.Fold(new Money(0m, currency), (acc, line) => acc + line)` over the `Money` operator (`Money.Zero` is the no-currency additive anchor), never a manual `double` accumulation, and a cross-currency rollup folds each line through `ExchangeRate.Convert` into the schedule's reporting currency first
...
[LOCAL_ADMISSION]:
- the `Planning/cost#ESTIMATE` priced scalar is `Money`; the hand-rolled `MonetaryAmount` `(double Amount, string Currency)` record with its manual `Add`/`Scale` and the `CostValue.Rate` `Amount / UnitBasis` divide is the RETIRED form — `Money + Money`, `Money * decimal`, and `Money / decimal` own that algebra and `Money.Currency` owns the currency identity
...
[RAIL_LAW]:
- Package: `NodaMoney` (2.7.0, Apache-2.0, pure-managed `lib/net10.0` AnyCPU IL, empty net10 dependency group)
...
- Accept: a `Planning/cost#ESTIMATE` `CostItem` value carried as `Money`, the `CostSchedule.Rollup` as a `Money` fold, the unit rate as `Money / decimal`, the quantity x rate join as `Money * (decimal)quantity.As(SiUnit)`, lossless multi-share allocation via `MoneyExtensions.Split`, and cross-currency rollup via `ExchangeRate.Convert`
- Reject: a hand-rolled `MonetaryAmount` `(double Amount, string Currency)` carrier beside `Money`; a `double` cost-arithmetic helper where the `Money` operators discriminate; a naive `total / n` allocation losing remainder pennies where `Split` is lossless; a stringly-typed currency field where `Currency.FromCode` resolves the ISO 4217 registry; a thrown `InvalidCurrencyException` in domain code instead of `.ToError()` onto `BimFault`; a second rounding policy threaded as a `MidpointRounding` argument where one `MoneyContext` scope governs
