...
- Entry: `public static Fin<MaterialComposition> LayerSet(Seq<(MaterialId Material, double ThicknessMm, string Name)> layers, Op key)` · `ProfileSet(MaterialId material, ProfileId profile, Op key)` · `ConstituentSet(Seq<(MaterialId Material, string Category, double Fraction)> constituents, Op key)` · `Single(MaterialId material)` — four smart-builders discriminating the composition shape, each `Fin<T>` aborting on an empty set, a non-positive layer thickness, or a non-normalizing constituent-fraction set (`ConstructionFault`), each composing the seam `MeasureValue.Of(value, UnitsNet.Units.LengthUnit.Millimeter, key)` for a thickness and `ProfileRef.Of(profileId.Value)` for the `[M7]` one-hop section handle (the seam `LayerSet`/`ProfileSet`/`ConstituentSet` constructors take `Op` and return `Fin`, owning the empty/thickness/fraction admission); `public static MaterialUsage UsageOf(MaterialComposition composition)` derives the `[C7]` occurrence usage the seam `Associate` edge carries from the composition shape.
...
public static class CompositionAuthor {
    public static MaterialComposition Single(MaterialId material) => MaterialComposition.Single(material);
    ...
    public static Fin<MaterialComposition> LayerSet(Seq<(MaterialId Material, double ThicknessMm, string Name)> layers, Op key) =>
        layers.IsEmpty || layers.Exists(static l => !(double.IsFinite(l.ThicknessMm) && l.ThicknessMm > 0.0))
            ...
            : layers.Traverse(l => MeasureValue.Of(l.ThicknessMm, UnitsNet.Units.LengthUnit.Millimeter, key).Map(t => new MaterialLayer(l.Material, t, l.Name)))
                .Bind(specs => MaterialComposition.LayerSet(specs, key));
    ...
    public static Fin<MaterialComposition> ProfileSet(MaterialId material, ProfileId profile, Op key) =>
        MaterialComposition.ProfileSet(material, ProfileRef.Of(profile.Value), key);

    // The seam ConstituentSet owns the fraction-sum normalization; this author keeps only the empty-set ConstructionFault.
    public static Fin<MaterialComposition> ConstituentSet(Seq<(MaterialId Material, string Category, double Fraction)> constituents, Op key) =>
        constituents.IsEmpty
            ...
            : MaterialComposition.ConstituentSet(constituents.Map(static c => new MaterialConstituent(c.Material, c.Category, c.Fraction)), key);
