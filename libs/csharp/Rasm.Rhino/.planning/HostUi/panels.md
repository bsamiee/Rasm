...
public static class PanelHost {
    ...
    public static ShellSubscription Watch(Action<PanelFact> observer) {
        long id = Interlocked.Increment(location: ref watchSeq);
        _ = Watchers.Swap(held => held.Add((Id: id, Observer: observer)));
        return new ShellSubscription(Halt: () => ignore(Watchers.Swap(held => held.Filter(row => row.Id != id))));
    }
