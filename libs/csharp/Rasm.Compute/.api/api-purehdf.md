...
[STACKING]:
- `Runtime/codecs#HDF_ARCHIVE`: THE branch archive owner — `HdfArchive.Mount/Open/Begin`, `HdfSource`, `HdfHandle` (concrete `NativeDataset`/`NativeGroup` resolve), `HdfArchivePolicy` (`DeflateGrade` four-value closed vocabulary), and the cursor-guarded `HdfWriter` — every other row on this list composes it and none opens an `H5File` of its own.
- `Runtime/codecs#FIELD_RESULT_CODEC`: `FieldCodec.Hdf5Decode`/`Hdf5Encode` read and emit the station×component chunk model as an HDF5 1.10 container — ONE dataset with the trailing COMPONENT axis (a dataset per component is the refuted sibling layout, forking the chunk address a consumer computes) — and `FieldCodec.Grid` is the one station-outermost `(FileDims, Chunks)` derivation every producer composes; `InterchangeIo.ImportWaveforms` reads `[samples, channels]` SHM and reference-bank corpora into `WaveformCorpus` under the frame/hop hyperslab law (hop at or above frame rides one strided selection, overlap walks per-frame slabs), `Stats/signal` the storing-nothing consumer.
- `Solver/contract#SOLVE_CONTRACT`: `SolveArchive` and its three container classes — `SolveHistory` `[steps, dofs]` one chunk per accepted step, `SolveModes` `[pairs, dofs]` mode-outermost with eigenvalue/participation/condensation attributes, and one create-only `SolveCheckpoint` container per committed arc step.
- `Solver/discretization#DISCRETIZATION_MESH`: `DiscreteMesh.Archive` writes the mesh container through `Begin` under the same chunk-aligned law.
...
- `Model/inference#TILED_INFERENCE`: the INTERNAL `TileSpill` rolling band (stride-aligned, seal after tile row r+1) and the `Chunked` index-keyed `WindowFill` provider — PureHDF on no Compute signature at either.
- `Symbolic/lowering#ENCLOSURE_AND_COLUMNS`: the blocked `ColumnProgram.Evaluate` overload — RentCeiling as the block bound, per-block hyperslab column fills, block-ascending chunk writes.
...
- `Analysis/structural#DESIGN_CHECK`: the shared demands/modal artifact — `/demands` member rows for both routes, the seismic `/modes` `[modes, dofs]` chunked mode-outermost.
