...
def _tuned(policy: TransformPolicy) -> None:  # Exemption: the vegafusion runtime singleton exposes its caps as properties whose setter resets the pool only on a changed value — an idempotent per-worker tune, never a fresh runtime per transform
    ...
    if policy.reclaim_cache:
        vegafusion.runtime.clear_cache()  # reclaim stale cache entries while KEEPING the pool — the reset()-free counterpart to the cap setters above


def _occupancy() -> tuple[int, int]:
    # worker-pool occupancy read AFTER the pre-pass: (cache entry count, resident cache bytes); both int|None before init, coalesced to 0
...
def _columns(usage: object) -> frozendict[str, tuple[str, ...]]:
    # get_column_usage(spec) -> {root_dataset: [col, ...] | None}; a None entry (not statically determinable) folds to () so the caller reads a total map
...
- [RESOURCE_POLICY] [RESOLVED]: `runtime.worker_threads`/`cache_capacity`/`memory_limit` are read/write properties on the lazily-initialized multi-threaded worker-pool singleton whose SETTERS `reset()` the pool only on a changed value (`.api/vegafusion.md` entry [07]), so `_tuned` assigns the `TransformPolicy` caps idempotently once per worker process rather than constructing a fresh runtime per transform, and calls `runtime.clear_cache()` (entry [06], reclaim WITHOUT the pool drop `reset` forces) when `TransformPolicy.reclaim_cache` is set — the pool-preserving reclaim distinct from the cap setters' reset. `runtime.size`/`total_memory` (entry [08], `int | None` before pool init) are the worker-pool occupancy `_evidence` reads at mint time onto `PrePassEvidence.cache_entries`/`resident_bytes`, so the pre-pass reports its cache footprint as observability rather than dropping it. `build_pre_transform_spec_plan(spec, preserve_interactivity=, keep_signals=, keep_datasets=) -> PreTransformSpecPlan {client_spec, server_spec, comm_plan, warnings}` (entry [06]) is the diagnostic that returns the SAME planned split `pre_transform_spec` would produce WITHOUT executing — `planned` reads its `server_spec.data` cardinality as the cost gate a caller checks (`server_datasets == 0` skips the pre-pass), the `comm_plan.server_to_client`/`client_to_server` `Variable` lists (type [03]) as the pre-execution `client_vars`/`server_vars` interactivity cardinality the executed `State` arm would otherwise be the sole source of, `get_column_usage(spec)` (entry [07], the top-level `__all__` export, `{root: [col] | None}`) as the referenced-column INPUT-prune map onto `column_usage`, and its `warnings` as the pre-execution data-loss prediction. The GIL-releasing DataFusion `apply`/`planned` kernels cross the `visualization/chart/export#EXPORT` `anyio.to_process` subprocess seam under one `CapacityLimiter`, recovered by that owner's bounded `stamina` retry on a `BrokenWorkerProcess` death; the `PrePassEvidence`/`PrePass`/`Result` return pickles back across the seam for the export owner to fold onto its charts receipt under one `structlog` event inside an `opentelemetry` span.
</content>
