...
[ACTIVE_SURFACE]:
- Compiler floor: `tsgo` and `tsc` under one flag set — a construct is legal when both accept it, and `tsc` diagnostic codes are the ones doctrine cites
...
- Law: `tsc` is the conformance authority — when `tsgo` and `tsc` disagree, `tsc` decides which behavior was specified and its diagnostic codes are the ones doctrine cites; the same rejection may surface under sibling codes across the floor, and the repair lands at the shape until both compilers accept it.
...
declare namespace Lens {             // type-only companion merged on the owner; ambient members export implicitly
  type Spread = {
    readonly lead: Gauge
...
const spread = (labels: ReadonlyArray<string>, hold: Duration.Duration, frame: Frame): Option.Option<Lens.Spread> => // bare type positions still resolve the global: ReadonlyArray stays built-in under the effect Array binding
  pipe(
    labels,
...
const _spans = (bytes: Uint8Array, mark: number): ReadonlyArray<readonly [number, number]> => { // BOUNDARY ADAPTER: measured scan kernel — mutation dies at the return
