...
[HATCH_PATTERN]:
- `new Rhino.DocObjects.HatchPattern()` / `new HatchPattern(HatchPattern other)` — fresh and copy construction; `Name`, `Description`, `FillType`, `PatternUnitSystem`, `AlwaysModelDistances`, and `Index` are settable, so a custom pattern composes fully detached before its table `Add`/`Modify`.
- `new Rhino.DocObjects.HatchLine()` / `new HatchLine(HatchLine other)` — one generator line with settable `Angle`, `BasePoint`, and `Offset`.
...
[HATCH_TABLE]:
- `Rhino.DocObjects.Tables.HatchPatternTable.FindName(string name) : HatchPattern` — resolves by name; `FindIndex(int index) : HatchPattern` and `FindNameHash(NameHash nameHash) : HatchPattern` add the index and hash routes, and `Find(string, bool)` is `[Obsolete]`.
...
- `Rhino.DocObjects.Linetype.LineCapStyle : LineCapStyle` / `LineJoinStyle : LineJoinStyle` / `Width : double` / `WidthUnits : UnitSystem` / `AlwaysModelDistances : bool` / `IsPatternLocked : bool` — stroke config; `Default() : void`, `DuplicateLinetype() : Linetype` (the copy clears name and id, so an amend re-stamps the name before `Modify`), `CommitChanges() : bool` maintain the definition.
