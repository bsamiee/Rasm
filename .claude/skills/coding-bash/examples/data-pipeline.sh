...
_mono_now() {
    local -n _o=$1
    ((_BASH_V >= 503)) && _o="${BASH_MONOSECONDS}" || _o="${EPOCHREALTIME}"
}
_perf_log() {
    local -r label="$1" start="$2"
    ((_BASH_V >= 503)) &&
        {
            _info "${label}: $((BASH_MONOSECONDS - start))ms"
            return
        }
...
_discover_files() {
    ...
    command -v fd >/dev/null 2>&1 &&
        {
            readarray -d '' -t _out < <(fd -e json --type f --print0 . "${src_dir}")
            return
        }
...
_extract_page() {
    local -r file="$1" offset="$2" limit="$3"
    local -n _page=$4
...
_process_file() {
    ...
    local -n _out=$4
    local page_data=""
    _extract_page "${file}" "${offset}" "${limit}" page_data || {
        _out="null"
        return 1
    }
...
_write_atomic() {
    ...
    saved_umask="$(umask)"
    umask 077
    local -r tmp="$(mktemp "${dest}.tmp.XXXXXX")"
    umask "${saved_umask}"
...
_self_test() {
    ...
    local agg=""
    _aggregate_metrics "${test_data}" agg
...
_main() {
    ...
    for file in "${log_files[@]}"; do
        ...
        _process_file "${file}" "${PAGE_OFFSET}" "${PAGE_SIZE}" result &&
            {
                _accumulate file_results "${file##*/}" "${result}"
                ((ok++))
            } ||
            {
                _debug "Skipped: ${file##*/}"
