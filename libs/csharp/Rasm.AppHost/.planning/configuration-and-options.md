...
- [SOURCE_ROUTES]: secrets-store keychain provider route behind `ConfigLayer.SecretsSource` — Security framework P/Invoke versus `/usr/bin/security` child process; host-document transition-reload route over the static in-memory mount — re-mount through the host-attach trigger versus a custom reload-token source.
