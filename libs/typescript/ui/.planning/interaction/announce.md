...
- [1]-[ACCESSIBILITY_BROADCAST]: the `@react-aria/live-announcer` live-region announce path keyed by the `_Roles` politeness column, and the `react-stately` `ToastQueue` external-store toast queue.
