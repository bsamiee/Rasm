...
- Cases: `AgentJournal` is the durable agent ledger keyed by session — `session_start`/`tool_call`/`checkpoint`/`session_complete` rows with `running`/`completed`/`failed`/`interrupted` status — persisted through the one `SqlBoundary` (`persistence/store#STORE_BOUNDARY`). `Resilience` carries the circuit-breaker (open/half-open/closed over a failure-rate window), the retry/backoff `Schedule`, and the `rfc6902` JSON-patch diff every sync and outbound path composes; `Resilience.breaker` rejects with a typed `DurableFault.CircuitOpen` (`engine#ENGINE`), never an inline anonymous `_tag` shape.
