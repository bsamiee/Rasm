...
- Law: the armed roster is policy, not a fan — `profilers` names which native samplers run, so a deployment carrying wall attribution without heap retention pressure arms one row and pays one sampler's overhead. The roster is SELECTIVE rather than wider: the package's aggregate `start`/`stop` arms and drains both native samplers together and can express no subset, which is the whole reason a per-row roster exists.
...
- Law: this bracket publishes whether the engine is SEATED, because the label surface `[04]` bands with resolves the singleton profiler and throws where `init` never ran — an unarmed deployment composes nothing here, so without the seat a banded kernel dies on a lane the deployment deliberately declined. The seat is written where `init` runs and cleared where the scope closes, so it can never outlive the engine it reports.
...
// The sink is PROCESS-GLOBAL and closes over this runtime, so it BRACKETS: an unbracketed install outlives its
...
// The one bit `[04]`'s band reads before it touches the engine. Every label op resolves the singleton profiler and
          ...
          Effect.isEffect(work)
            ...
            : !_seated
              ...
              : Effect.sync(() => {
                ...
                // The engine restores the prior label set after the callback returns and outside any `finally`, so a
