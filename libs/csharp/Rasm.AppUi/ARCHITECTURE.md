...
The host-mount axis precedes the shell, the shell precedes the screens it routes, and the vocabulary owners (theme, typography, assets, motion) precede every consumer that traces a literal to them. The viewport, drafting, notebook, animation, realitycapture, and coordination folders are projections over the settled receipt spine, the visuals codec, the GPU render-target factory, and the Compute geometry/field/point/splat receipts. The `realitycapture/` (a Viewport reality-capture sibling) and `coordination/` (an openBIM issue board composing the viewport, notebook, and chart owners over the `Rasm.Bim` BCF topic contract) sub-domains carry design pages. The `shading/`, `immersive/`, and `presentation/` sub-domains deepen the GPU viewport frontier: `shading/` owns the per-backend shader-source and pipeline-state cache the render-graph passes bind below the `RenderTargetFactory` column, `immersive/` projects the one `RenderGraph` into an OpenXR stereo session over the shared `Wgpu` device with a controller-navigated `Viewpoint`, and `presentation/` sequences saved `Viewpoint`s into a narrated review tour over the animation playhead — each composing the settled viewport, viewpoint, animation, and coordination owners rather than minting a parallel scene model.
...
├── shading/            # GPU shader-asset owner: the WGSL/SKSL shader-source vocabulary keyed by content hash, the per-backend pipeline-state cache the render passes bind, the shader-module compile receipt, and the hot-reload shader watch under the dev loop
├── immersive/          # OpenXR stereo design-review: the XR session over the shared Wgpu device, the predicted-display-time stereo frame loop, the action-set controller navigation, the LiDAR/snap measurement and BCF-Viewpoint markup in headset, environment-blend passthrough mixed reality
├── presentation/       # Saved-viewpoint review tour: an ordered Viewpoint sequence with per-stop dwell and transition, a narrated annotation track, the offline walkthrough render over the animation playhead, and the BCF-topic-to-tour projection
...
├── realitycapture/     # Gaussian-splat and point-cloud reality-capture visualization projected as viewport render-pass cases over the backend target factory, with a LiDAR-anchored measurable overlay and time-based capture playback
└── coordination/       # openBIM issue board composing the Viewpoint view-state, the Rasm.Bim BCF topic/component contract, a CRDT comment thread, and a snapshot tile — the board projection, never a second BCF semantic model
```
