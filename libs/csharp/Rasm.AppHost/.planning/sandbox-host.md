...
| [INDEX] | [CLUSTER]      | [OWNS]                                                                   |
| :-----: | :------------- | :----------------------------------------------------------------------- |
|   [1]   | ISOLATION_AXIS | WASM-component and process isolation rows; no-ambient-authority load law |
|   [2]   | GRANT_HANDLE   | Capability-brokered grant handle; per-call authority mediation           |
|   [3]   | QUOTA_CONTROL  | CPU/memory/wall/egress quota cell; kill and quarantine rail              |
|   [4]   | SUPPLY_CHAIN   | Signature, SLSA-attestation, semver admission before load                |
...
- [WASM_RUNTIME]: the WebAssembly component-model host runtime (the linear-memory boundary, the import-table linkage scoped to the granted descriptors, and the fuel/memory counters the quota cell reads) confirms against the admitted WASM runtime package at the integrated host; the component instantiation with a scope-derived import set is the open distinction the live runtime resolves.
