...
- Boundary intake/emit: `json_format.ParseDict` at intake from JSON config/payloads into a typed message; `MessageToDict(preserving_proto_field_name=True)` at emit to keep snake_case keys. Well-known types are the canonical carriers — `Timestamp` at the time boundary (`FromDatetime`/`ToDatetime` truncate to microseconds, so a 100-ns-fidelity clock renders through the `json_format` RFC 3339 mapping and reconstructs from its own carrier slots), `Struct.update` for dynamic JSON-ish bodies, `Any.Pack`/`Unpack` for heterogeneous typed envelopes.
...
- msgspec envelope seam: the runtime msgspec `Struct` envelope holds the op id/actor/lamport and the protobuf frame is the opaque `bytes` value `proto.serialize(message, deterministic=True)`/`proto.parse(message_class, payload)` produce and consume, `message_factory.GetMessageClass(descriptor)` supplying descriptor-driven dispatch — msgspec owns the envelope schema, protobuf owns the inner op wire, never two parallel schemas for one op; `Any` `@type`-keyed envelopes decode as the shapes-vocabulary `Packed` mappings dispatched on the `@type` discriminant, never an `Any.Unpack` re-materialization beside the vocabulary.
- grpc seam: generated message classes are the request/response types of the `grpc.aio` stubs with `proto.serialize`/`proto.parse` as the channel serializer/deserializer, and the `opentelemetry-instrumentation-grpc` `request_hook`/`response_hook` receive these message instances for span enrichment.
- descriptor drift-gate: the runtime `transport/shapes#aligned` root pass resolves `DescriptorPool.FindMessageTypeByName(message.DESCRIPTOR.full_name)` per vocabulary row — a pool read, never an `AddSerializedFile` re-registration — and cross-checks the compiled `Descriptor` against the struct's `msgspec.inspect` field set for absent slots, phantom slots, and the `TYPE_*64` floor; the gate proves structure, never byte parity, which stays with the seed-reproduction corpus.

[LOCAL_ADMISSION]:
- Generated `_pb2.py` files are the only source of concrete message classes; never instantiate `Message` directly. For runtime-resolved schemas use `message_factory.GetMessageClass` against the pool; the `google.protobuf.internal.builder` path stays inside the generated modules alone.
