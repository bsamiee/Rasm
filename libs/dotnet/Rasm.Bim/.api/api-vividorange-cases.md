...
- with `LanguageExt.Core`: an uncovered `NationalAnnex` faults at the boundary — the EN `ψ`/`γ` singletons throw `NotImplementedException` and `IStandard.Title` can throw `MissingNationalAnnexException`; `Try.lift` preserves either exact exceptional error, while explicit unsupported-policy admissions mint `BimFault.Refused` under the model/capability axes
