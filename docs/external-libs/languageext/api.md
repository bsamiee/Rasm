...
Typical workspace imports: `LanguageExt`, `LanguageExt.Common`, `LanguageExt.Traits`, `LanguageExt.Effects`, `LanguageExt.Pretty`, `LanguageExt.Traits.Domain`, and static `LanguageExt.Prelude` when global usings are enabled.
...
|   [9]   | `Atom<T>`, `Ref<T>`          | Managed reactive state.                    | UI/session state; react through host paint or polling surfaces. |
