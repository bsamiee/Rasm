    ...
    public static IO<Fin<Unit>> Define(NpgsqlDataSource source, CypherEnablement gate, GraphDdl ddl) =>
        gate is CypherEnablement.SelfHosted
            ? HostEdge.CapturedIO(async _ => {
              ...
              }).Map(outcome => outcome.MapFail(static error => error.Exception.Case is NpgsqlException
...
- Law: every result on this page is `Fin`-shaped end to end; `Try.lift` captures provider raises before the two lane gates lower their documented operation contexts onto `CypherFault`.
...
public static class GraphLane {
    ...
    static IO<Fin<T>> Captured<T>(Func<System.Threading.Tasks.Task<Fin<T>>> crossing, Func<Error, CypherFault> arm) =>
        HostEdge.CapturedIO(async _ => await crossing().ConfigureAwait(false)).Map(outcome => outcome
