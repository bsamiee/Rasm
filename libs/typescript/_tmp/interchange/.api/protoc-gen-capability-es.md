...
- the `EffectClassKey`/`IdempotencyKey`/`CostUnitKey` literal vocabularies and the `txn` union literals are transcribed VERBATIM from the C# enum source under the `Contract/inventory#WIRE_LAW` cross-language drift law — a new C# value folds an unknown to `Ingress/quarantine.md` `Additive`, never a silent branch-side drift
