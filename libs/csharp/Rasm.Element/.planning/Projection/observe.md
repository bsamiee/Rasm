...
- Packages: BCL `System.Diagnostics.Metrics` and `System.Diagnostics` (the folder catalog `.api/api-diagnostics-activity.md` overlay), `Rasm` (the kernel instrument mechanism), Thinktecture.Runtime.Extensions (the generated fact `Switch`), LanguageExt.Core.
