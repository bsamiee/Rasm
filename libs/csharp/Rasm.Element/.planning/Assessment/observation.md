...
public readonly partial record struct ObservationChunk(Interval Window, BlobKey SeriesKey, int SampleCount) {
 ...
 public Fin<Seq<(Instant At, double Si, ObservationGrade Grade)>> Decode(ReadOnlyMemory<byte> blob, Op key) =>
  BlobKey.Of(blob.Span) == SeriesKey
   ? Read(blob, key)
   : new ElementFault.AddressUnstable(key, $"<observation-chunk-key-mismatch:{SeriesKey.Value:x32}>");
 ...
 private static Fin<(ObservationChunk Chunk, ReadOnlyMemory<byte> Bytes)> Written(
  ...
  CanonicalWriter.Retaining(0d)
   ...
   .ToBytes(key)
   .Map(bytes => (new ObservationChunk(
    new Interval(samples[0].At, samples[samples.Count - 1].At),
    BlobKey.Of(bytes.Span), samples.Count), bytes));
