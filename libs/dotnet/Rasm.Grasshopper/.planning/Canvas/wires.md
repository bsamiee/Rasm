...
- Packages: Grasshopper2 (`WireShape`, `WireShapeDefault.CreateSpline`, `IParameterAttributes`, `WireEnds`), LanguageExt.Core (`Validation`, `Partition`), `Rasm.Domain` (`Cell`, `FaultCell`, `Lease<T>`).
...
- Packages: Grasshopper2 (`WireSkin`, `EdgeDescription`, `Canvas.ZuiWireDetailingState`), `Rasm.Interaction` (`PaintColor`, `HostEdge.Nullable`), `Rasm.Numerics` (`PerceptualColor`), `Canvas/paint.md` (`GhPlan`, `GhMark.WireCase`), LanguageExt.Core, `Rasm.Domain`.
