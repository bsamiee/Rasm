...
- issuer-side presigning: the `Store/blobstore#OBJECT_STORE` `ObjectLeg.Issue` leg mints TTL-boxed grants against the store's own residence, one signing entry per provider row; the Minio row's presign family (`PresignedGetObjectArgs`/`PresignedPutObjectArgs`) is catalogued at `api-minio`.

[PRESIGN_ISSUER]:
- `AmazonS3Client.GetPreSignedURL(GetPreSignedUrlRequest) -> string` signs a V4 URL from the client's own credential; `GetPreSignedUrlRequest` carries `BucketName`, `Key`, `Verb` (`HttpVerb.GET`/`PUT`/`DELETE`), and `Expires` (`DateTime`) — the absolute expiry the caller derives from the injected frame instant plus the TTL.
- `BlobClient.GenerateSasUri(BlobSasPermissions permissions, DateTimeOffset expiresOn) -> Uri` mints a service SAS; issuance requires the client dialed with a `StorageSharedKeyCredential` (an AAD-dialed client cannot sign — `CanGenerateSasUri` is the probe), so SAS capability is a deployment fact of the host-dialed container.
- `UrlSigner.FromCredential(GoogleCredential) -> UrlSigner` and `UrlSigner.SignAsync(string bucket, string objectName, TimeSpan duration, HttpMethod? httpMethod = null) -> Task<string>` own GCS V4 signed URLs — the signer is a credential-bound handle SEPARATE from `StorageClient`, so the `ObjectClient.Gcs` row carries both.
