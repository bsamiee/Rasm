...
- [02]-[SCHEDULE_SOURCE]: `ScheduleOp` the one parse/serialize/probe family over `ScheduleSpec`, the seven-member `ScheduleFormat` egress axis, the `DependencyKind`/`ConstraintKind` seam vocabularies, the always-`ReadAll` container law, the row-boundary fault fold on both legs, and the typed fact stream.
- [03]-[DURABLE_NETWORK]: `ScheduleProject` persisted activity/relation/calendar/resource rows at full MPXJ axis depth, the unit-tagged `Option<ScheduleSpan>` crossing, the `ProjectRows.Of`/`Synthesis.Fold` round-trip seam, and the `ScheduleRows.Reconcile` baseline/update variance owner.
