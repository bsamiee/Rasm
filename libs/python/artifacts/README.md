...
- [03]-[EGRESS](.planning/documents/egress.md): `DocumentEgress` security-and-navigation finishing layer over an emitted PDF — AES encryption with a permission policy, outline/bookmark-tree authoring, overlay/underlay watermarking and stamping, embedded-attachment binding, and n-up/booklet imposition over `pikepdf`, `pypdf`, and `weasyprint`; finishes an emitted PDF, never authors one.
...
- [05]-[CHART](.planning/figures/chart.md): 2D chart union over `altair`/`plotly`/`matplotlib` with host-free static export (`vl-convert-python` primary, `kaleido` gated) and server-side Vega transform via `vegafusion`.
...
- [08]-[PREVIEW](.planning/figures/preview.md): Raster/preview owner over `pillow`, `scikit-image`, `segno`, and `python-magic`; the `MARK` encoded-mark arm carries a `Symbology` sub-axis (`segno` QR/Micro-QR settled, `python-barcode` linear 1D symbologies RESEARCH-pending the catalogue sync).
- [09]-[COMPOSE](.planning/figures/compose.md): `Figure` composition owner turning emitted graphics into placed, annotated, color-correct figures — SVG scale-to-fit, n-up tiling, crop, and bounds query over `svgelements`, plus draw/text/filter annotation and EXIF/XMP metadata over `pillow`.
