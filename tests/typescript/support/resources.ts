...
import { Array, Context, Data, Effect, HashMap, Layer, Mailbox, Option, type Order, type ParseResult, Ref, Schema, type Scope } from 'effect';
...
declare namespace TestDatabase {
    type Service = {
        ...
        readonly rows: (statement: string, params?: ReadonlyArray<unknown>) => Effect.Effect<ReadonlyArray<unknown>, TestResourceError>;
        readonly decoded: <A, I>(
            schema: Schema.Schema<A, I, never>,
        ) => (statement: string, params?: ReadonlyArray<unknown>) => Effect.Effect<ReadonlyArray<A>, TestResourceError | ParseResult.ParseError>;
...
declare namespace ObjectStore {
    type Service = {
        ...
        readonly list: (prefix: string) => Effect.Effect<ReadonlyArray<string>, TestResourceError>;
    ...
    static readonly operation =
        ...
        (cause: unknown): TestResourceError =>
            new TestResourceError({
                resource,
                reason: 'operation',
                code: cause instanceof Error ? Option.some(cause.name) : Option.none(),
                detail: String(cause),
            });
}
...
class ObjectStore extends Context.Tag('rasm-test-support/ObjectStore')<ObjectStore, ObjectStore.Service>() {}

class TestDatabase extends Context.Tag('rasm-test-support/TestDatabase')<TestDatabase, TestDatabase.Service>() {}

// --- [OPERATIONS] ----------------------------------------------------------------------

const _fromPromise = <A>(resource: TestResourceError['resource'], run: () => Promise<A>): Effect.Effect<A, TestResourceError> =>
    Effect.tryPromise({
        try: run,
        ...
        Effect.gen(function* () {
            ...
            yield* Effect.acquireRelease(
                _fromPromise('database', () =>
                    db.listen(channel, (payload) => void (payload.startsWith(_CONTROL_NOTIFICATION_PREFIX) || mailbox.unsafeOffer(payload))),
                ),
...
const TestDatabases = {
    pglite: (options?: string | TestDatabase.PgliteOptions): Layer.Layer<TestDatabase, TestResourceError> => {
        const configuration: TestDatabase.PgliteOptions = typeof options === 'string' ? { seed: options } : (options ?? {});
        return Layer.scoped(
            TestDatabase,
            Effect.gen(function* () {
                const db = yield* Effect.acquireRelease(
                    _fromPromise('database', () =>
                        PGlite.create({
                            ...
                            ...(configuration.extensions === undefined ? {} : { extensions: configuration.extensions }),
                        }),
                    ),
                    (live) => Effect.promise(() => live.close()),
                );
                const seed = configuration.seed;
                yield* seed === undefined ? Effect.void : _fromPromise('database', () => db.exec(seed));
                const exec: TestDatabase.Service['exec'] = (statements) => Effect.asVoid(_fromPromise('database', () => db.exec(statements)));
                return _database(
                    exec,
                    (statement, params) =>
                        Effect.map(
                            _fromPromise('database', () => db.query(statement, params === undefined ? undefined : [...params])),
                            (result) => result.rows,
                        ),
                    _pgliteListen(db),
...
export { Loopback, LoopbackServers, ObjectStore, ObjectStoreDoubles, TestDatabase, TestDatabases, TestResourceError };
