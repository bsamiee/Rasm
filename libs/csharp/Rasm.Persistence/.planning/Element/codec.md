...
- [03]-[CONTENT_ADDRESS]: the kernel-composed content address, the canonical-byte projection, the precomputed-digest wrap, the seam `GraphMembers` incremental accumulator, and the streaming digest leg.
...
public partial class ElementJson : JsonSerializerContext {
    public static readonly JsonSerializerOptions Options =
        new JsonSerializerOptions(JsonSerializerOptions.Strict) {
            ...
            // The kernel carrier factory (`Rasm/Domain/rails#CARRIER_CODEC`) is what lets the graph's
            // `Seq`/`Option` members decode at all; this mint stays explicit-null (no `OmitAbsent`
            // modifier, default ignore condition), so a `None` writes `null` and every `Option<T>`
            // constructor parameter correctly carries no default.
