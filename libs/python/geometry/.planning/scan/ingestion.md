...
- Boundary: the inbound LAS/LAZ/COPC decode and the `pyarrow.Table` bridge are `data/spatial/mesh#POINTCLOUD`'s (`laspy` full decode and the COPC octree subset live there), so ingestion never re-reads LAS nor crosses a `pdal` `Pipeline` at the data seam; the E57 path is legitimately ingestion's because `pye57` is absent from the data branch, but the E57 write path (`write_scan_raw`) is declined — scan egress is the data seam's. Registration is `scan/registration#REGISTRATION`'s; ingestion never registers, deviates, reconstructs, tessellates, stores, or mutates a Rhino/GH document.
