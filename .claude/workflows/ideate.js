...
const LAW = [
  ...
  'WRITE-FULLY MANDATE: every card you author/refine/repair you MUST write NOW via Edit/Write directly into the folder IDEAS.md / TASKLOG.md ' +
    ...
    'pool is already world-class, return verdict=clean — never invent cards to look busy.',
].join('\n')
...
const surveyPrompt = (folder) => [LAW, '', 'TASK: SURVEY ' + folder + ' (read-only). Read its design pages (' + folder + '/.planning/**), ' +
  'ARCHITECTURE.md, README.md, and any existing ' + folder + '/IDEAS.md + TASKLOG.md. Produce a structured map: (realized) the capability the ' +
  'pages ALREADY deliver — these must NOT be carded; (gaps) genuinely-deferred capability the REAL domain needs beyond what is realized — research ' +
  'the domain, do not guess; (cross_folder) seams where a card should Ripple to a sibling owner; (existing_cards) the current card pool and its ' +
  'state. Return the structured survey.'].join('\n')
const ideatePrompt = (folder, ctx) => [LAW, '', 'SURVEY of ' + folder + ':\n' + ctx, '', 'TASK: author/rebuild the IDEAS + TASKS pool in ' + folder + '/IDEAS.md ' +
  'and ' + folder + '/TASKLOG.md, grounded in the survey above. Author IDEAS for the conceptual/multi-step gaps and TASKS for the ' +
  'concrete/deferred ones; each card dense at the 7-point rubric, genuinely-deferred ONLY (never a realized page from the survey), correctly ' +
  'anchored, with Ripple refs to existing cross-folder counterparts where the seam exists. Match the exact card schema/voice from ' +
  'campaign-method.md. Fix-in-place (write the files, create if absent). Return the card-log + residual — each a {files: [both folders ' +
  'IDEAS.md/TASKLOG.md the Ripple pairs], claim} object for any CROSS-FOLDER Ripple counterpart you cannot author from this folder (NO severity; ' +
  'the reconcile phase fixes all of them).'].join('\n')
const critiquePrompt = (folder) => [LAW, '', 'TASK: CONSTRUCTIVE CRITIQUE + FIX IN PLACE of the cards in ' + folder + '/IDEAS.md + TASKLOG.md. ' +
  'Pull the pool UP: denser/sharper theses, fuller domain-completeness (is a genuinely-deferred capability still missing?), better anchors, richer ' +
  'and correct Ripple refs, polymorphic/AOP framing in Capability/Shape. EDIT the card files. Return the card-log + residual — each a {files: ' +
  '[both folders IDEAS.md/TASKLOG.md the Ripple pairs], claim} object for any CROSS-FOLDER Ripple asymmetry you cannot fix from this folder (NO ' +
  'severity; the reconcile phase fixes all of them).'].join('\n')
const redteamPrompt = (folder) => [LAW, '', 'TASK: ADVERSARIAL RED-TEAM + FIX IN PLACE of the cards in ' + folder + '/IDEAS.md + TASKLOG.md. You ' +
  'are a HOSTILE reviewer whose goal is to REJECT this pool — assume it is redundant, under-dense, or under-ideated until it proves otherwise. ' +
  'ATTACK and repair: any card that duplicates a realized design page (mis-carded in-pass work — delete/disposition); any ' +
  ...
  'weak/under-dense cards failing the 7-point rubric; non-deferred work wrongly carded. ALSO the FORWARD lens: genuinely-deferred domain ' +
  'capability MISSING from the pool (under-ideation), a pool that understates the real domain — author what is missing. Run COUNTERFACTUALS (is ' +
  'this the strongest set of deferred bets, or is there a denser/higher-leverage framing?). Repair every defect in place. Return the card-log + ' +
  'residual — each item a {files: [both folders IDEAS.md/TASKLOG.md the Ripple pairs], claim} object (genuine cross-folder items only; everything ' +
  'within this folder you fix yourself).'].join('\n')
...
const inv = await agent('List the package/area folders under ' + SWEEP + ' that own a design corpus (immediate child directories containing a ' +
  ...
  'Use find/ls; do not cd.', { label: 'discover', phase: 'Survey', schema: DISCOVERY_SCHEMA, model: 'sonnet', effort: 'low' })
...
if (clusters.length) {
  ...
  reconciled = (await pipeline(
    ...
    (fix, cl, i) => fix ? agent([LAW, '', 'TASK: ADVERSARIAL COMPLETENESS VERIFY. A reconcile agent claims to have repaired the cross-folder ' +
      'Ripple residuals below. Read the named card files from disk and prove, per claim, whether the pairing is ACTUALLY bidirectional and ' +
      'collision-free now — status "fixed" (resolved), "invalid" (the claimed asymmetry is factually wrong — cite why), or "open" (still ' +
      'dangling/asymmetric). Default "open" on any doubt; "invalid" only when provably wrong. One verdict per claim plus overall. Claims:\n' + JSON.stringify(cl, null, 1) + '\nCard ' +
