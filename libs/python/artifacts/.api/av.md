...
|  [12]   | `VideoStream.encode_lazy`      | `encode_lazy(frame=None) -> Iterator[Packet]`                                                                                            | encode a frame lazily as a packet iterator |
...
|  [06]   | `VideoStream.framerate`         | attribute -> `Fraction`                 | encode frame rate (`average_rate`/`base_rate` reads) |
...
- encode axis: `stream.encode(frame)` -> `container.mux(packets)` is the one encode-then-mux row; the flush is `stream.encode(None)` muxed at end-of-stream, never a separate teardown surface; `encode_lazy` is the streaming-iterator row for backpressure-sensitive consumers; `AudioFifo` rebuffers to a fixed `frame_size` for encoders (AAC) that require it.
