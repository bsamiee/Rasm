...
const wrapPrompt = (report, law, task, medium, minHeads) =>
    ...
    REPO +
    '/.claude/skills/delegate-codex/scripts/codex.sh --task ' +
    report +
    ...
    'S=0; until [ -f ' +
    report +
    '.delegate/receipt.json ]; do sleep 5; S=$((S+5)); [ $S -ge 570 ] && break; done; [ -f ' +
    report +
    '.delegate/receipt.json ] && cat ' +
    report +
    '.delegate/receipt.json || echo PENDING — PENDING is a delegate still producing: run the IDENTICAL call again, as many rounds as ' +
