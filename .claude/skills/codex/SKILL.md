...
- A tier DOWNSHIFT is the first overrun control: dropping effort reduces exploration depth and verification looping more cheaply than any prompt clamp, so a lane that overruns gets `medium` before it gets more instructions. The upshift mirror: xhigh earns its latency on review-shaped legs with demonstrated benefit, never as a blanket review default.
