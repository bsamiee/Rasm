...
class FrameCovenant(Struct, frozen=True):
    ...
    def _lower(self, members: "tuple[CovenantMember, ...]") -> "RuntimeRail[dict[str, pl.LazyFrame]]":
        return traversed(
            tuple(self.interop.translate(m.frame, Backend.POLARS).map(lambda lowered, _name=m.name: (_name, lowered.lazy())) for m in members),
            accumulate=False,
        ).map(dict)
