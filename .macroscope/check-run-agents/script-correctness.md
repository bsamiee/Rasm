...
Settled corpus law outranks intuition: a shape the owning doctrine or the script's own standing mechanism already discharges is never a finding — a timeout-bounded one-shot owes no streaming drain, a cheap probe no retry rail, a body whose one held gate already makes every ownership transition atomic owes no lifecycle cell — so a machinery finding proves that mechanism absent or insufficient first, and finding nothing after a genuine pass is a first-class verdict, stated plainly.
