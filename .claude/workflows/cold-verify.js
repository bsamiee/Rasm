...
const FINDINGS = { type: 'object', additionalProperties: false, required: ['findings', 'coverage', 'summary'], properties: {
...
const codexPrompt = (label, task, schema, writes) => {
  ...
  '(4) READY: do NOT relay the report body through your output — build the MECHANICAL headline with jq (never your own ' +
    'judgment): entries=$(jq \'.findings | length\' ' + base + '-report.json); classes=$(jq -r \'[.findings[].class] | group_by(.) | map(.[0] + "x" + (length|tostring)) | join(",")\' ' + base + '-report.json); top=$(jq -r \'[.findings[].files[0]] | group_by(.) | max_by(length) | .[0] // "none"\' ' + base + '-report.json). ' +
    'Return the RECEIPT: ok=true, report=' + base + '-report.json, entries=that count, headline="<entries> findings | <classes> | top: <top>", failure empty. ' +
...
const lanes = await parallel(CAMPS.map((c) => async () => {
  ...
  log(tag + ': ' + total + ' finding(s) across ' + verified.length + '/' + roster.length + ' lanes' +
    (verified.length < roster.length ? ' — FAILED: ' + roster.filter((r) => !r.ok).map((r) => r.lane).join(', ') : ''))
  const fix = await agent(CTX(c) + '\n\n' + HUNT + '\n\nTASK: TERMINAL FINALIZE (WRITER — full authority over ' + c.root +
    ...
    'CONSUMPTION PROTOCOL, in order: (a) read ' + c.doc + ' IN FULL — it is the ruling; (b) UNMAPPED scope below is ' +
  ...
  return { campaign: c.root, lanes: roster.length, failedLanes: roster.filter((r) => !r.ok).map((r) => r.lane),
    findings: total, resolved: (fix && fix.resolved && fix.resolved.length) || 0,
