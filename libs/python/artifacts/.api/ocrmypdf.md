...
[STACKING]:
- `expression`(`.api/expression.md`): `ExitCode.ok` admits the sidecar text; every other `ExitCode.name` lands in the recovered structure node, while `ExitCodeException` converts to `BoundaryFault` at `async_boundary`.
