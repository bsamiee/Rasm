...
- `Gdal.UseExceptions()` (with the `Ogr`/`Osr` peers) follows the bootstrap so a failed open or transform throws into `Try.lift`; the exact exceptional `Error` survives, while documented null/status refusals map at their owning site and domain code never branches on a raw `CPLErr`.
