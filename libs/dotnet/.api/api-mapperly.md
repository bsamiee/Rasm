...
- `Rasm.Compute` (`Runtime/wire`, `Runtime/receipts`, `Runtime/progress`, `Analysis/lifecycle`): `SplatMapper`, `FaultDetailMapper`, `AllocationMapper`, `ProgressWireMap`, and `Ec3Wire`+`EpdCodec` are the boundary-mapper estate; no transaction mirror remains.
