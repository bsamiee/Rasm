...
declare namespace Containers {
    ...
    type Row = {
        ...
        readonly net?: {
            ...
            readonly aliases: Array.NonEmptyReadonlyArray<string>;
        };
        readonly resources?: {
            readonly cpu?: number;
...
const _ARM = {
    pauseMs: 25,
    turns: 120,
    prefix: '<rasm-testkit-armed:',
} as const;

// --- [MODELS] --------------------------------------------------------------------------

const _Pins = Schema.Record({
    key: Schema.String,
...
class _Rollback<A> extends Data.TaggedError('rasm-testkit/Rollback')<{
    readonly verdict: A;
}> {}
...
class PinsPath extends Context.Reference<PinsPath>()('rasm-testkit/PinsPath', {
    defaultValue: (): string =>
        fileURLToPath(new URL('../../containers.json', import.meta.url)),
}) {}
...
const _breath = Effect.promise<void>(
    () => new Promise((wake) => setTimeout(wake, _ARM.pauseMs)),
);

const _decodePins = Schema.decodeUnknown(Schema.parseJson(_Pins), {
    errors: 'all',
});
...
const _clientSandbox =
    ...
    <A, E, R>(work: Effect.Effect<A, E, R>) =>
        pipe(
            pg.withTransaction(
                Effect.flatMap(Effect.either(work), (verdict) =>
                    Effect.fail(new _Rollback({ verdict })),
                ),
            ),
            Effect.catchAll((fault) =>
                fault instanceof _Rollback
                    ? Effect.succeed(fault.verdict)
                    : Effect.fail(HarnessFault.engine('pg')(fault)),
            ),
            Effect.flatten,
        );
...
const Containers = {
    ...
    exec: (
        ...
        command: Array.NonEmptyReadonlyArray<string>,
    ): Effect.Effect<Containers.Exec, HarnessFault> =>
        Effect.map(
    ...
    pin: (
        name: string,
    ): Effect.Effect<string, HarnessFault, FileSystem.FileSystem> =>
        Effect.gen(function* () {
            ...
            return yield* Effect.mapError(
                ...
                () =>
                    new HarnessFault({
                        ...
                        reason: 'unsupported',
                        code: Option.none(),
                        detail: `no image pin named ${name}`,
                    }),
            );
        }),
    start: (
        row: Containers.Row,
    ): Effect.Effect<StartedTestContainer, HarnessFault, Scope.Scope> =>
                ...
                pipe(
                    ...
                    (built) =>
                        row.command === undefined
                            ? built
                            : built.withCommand([...row.command]),
                    (built) =>
                        row.copy === undefined
                            ? built
                            : built.withCopyContentToContainer([...row.copy]),
                    (built) =>
                        row.labels === undefined
                            ? built
                            : built.withLabels(row.labels),
                    (built) =>
                        row.net === undefined
                            ? built
                            : built
                                  .withNetwork(row.net.network)
                                  .withNetworkAliases(...row.net.aliases),
                    (built) =>
                        row.resources === undefined
                            ? built
                            : built.withResourcesQuota(row.resources),
                ).start(),
            ),
            (started) => Effect.ignore(Effect.promise(() => started.stop())),
        ),
        ...
        Effect.gen(function* () {
            ...
            yield* Effect.iterate(0, {
                ...
                body: (turn) =>
                    pipe(
                        Effect.mapError(
                            pg.unsafe('SELECT pg_notify($1, $2)', [
                                channel,
                                sentinel,
                            ]),
                ...
                Effect.filterOrFail(
                    ...
                    () =>
                        new HarnessFault({
                            ...
                            reason: 'engine',
                            code: Option.none(),
                            detail: `listen never armed on ${channel}`,
                        }),
                ),
            );
            return box;
...
const _pgFromClient = (
    seed?: string,
): Layer.Layer<PgLane, HarnessFault, PgClient.PgClient> =>
    Layer.effect(
        PgLane,
        Effect.gen(function* () {
            ...
            const fault = (raw: SqlError): HarnessFault =>
                new HarnessFault({
                    lane: 'pg',
                    reason: 'engine',
                    code: Option.some(raw._tag),
                    detail: raw.message,
                });
            const lane = _lane(
                (statements) =>
                    Effect.asVoid(
                        Effect.mapError(pg.unsafe(statements), fault),
                    ),
                (statement, params) =>
                    Effect.mapError(
                        pg.unsafe(
...
const PgLanes = {
    ...
    container: (
        options: PgLane.ContainerOptions,
    ): Layer.Layer<PgLane, HarnessFault> =>
        Layer.provide(
            _pgFromClient(options.seed),
            Layer.unwrapScoped(
                Effect.map(
                    Containers.start(Containers.pg(options)),
                    (started) =>
                        Layer.mapError(
                            PgClient.layer({
            ...
            url: () =>
                Effect.fail(
                    new HarnessFault({
                        ...
                        reason: 'unsupported',
                        code: Option.none(),
                        detail: 'presign requires the s3 lane',
                    }),
                ),
        })),
    ),
    s3: (
        options: ObjectStore.S3Options,
    ): Layer.Layer<ObjectStore, HarnessFault> =>
        Layer.scoped(
            ObjectStore,
            Effect.gen(function* () {
                ...
                yield* Effect.catchIf(
                    ...
                    (fault) =>
                        Option.exists(fault.code, (name) =>
                            name.startsWith('BucketAlready'),
                        ),
                    () => Effect.void,
                );
                return {
                    ...
                    get: (key) =>
                        _guarded('store', () =>
                            ...
                            Effect.catchIf(
                                (fault) =>
                                    Option.contains(fault.code, 'NoSuchKey'),
                                    ...
                                    body: (state) =>
                                        Effect.map(
                                            ...
                                            (reply) => ({
                                                ...
                                                token: Option.fromNullable(
                                                    reply.NextContinuationToken,
                                                ),
                                                open:
