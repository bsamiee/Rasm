...
- sequence axis: `make_sequence` takes a REQUIRED `symbol_count` bounded 1..16 (bare call raises; 17 raises `ValueError`) and answers exactly that many members, so member count is a declared admission fact; each member saves through its own `QRCode.save(kind="svg")` and stamps `INDEX`/`COUNT` position facts beside its own scan evidence.
...
- evidence: each symbol stamps `designator`/`version`/`error`/`mask`/`mode`/`symbol_size` onto the shared `RasterFact.score` `frozendict` keyed by the `MarkFact` vocabulary; every arm measures its real `symbol_size` extent, each sequence member projecting its own `core/receipt#RECEIPT` `ArtifactReceipt.Preview` keyed over that member's bytes.
