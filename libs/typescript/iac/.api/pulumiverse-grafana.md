...
[DATA_SOURCE_TRAPS]: both `jsonData` halves cross as SERIALIZED JSON STRINGS, never typed structs — `jsonDataEncoded` and `secureJsonDataEncoded` take whatever camelCased key set the target plugin's own config declares, so the plugin's config record is the contract a fence transcribes and this resource type checks none of it; `url` is generic and a plugin dialing its own host/port coordinates ignores it outright, so an address alone provisions a door nothing connects through. `orgId` takes `Input<string>` while `oss.Organization.orgId` yields `Output<number>`, so an org-scoped row renders the realized number through `.apply(String)`.
[CUSTOM_HEADERS]: Grafana CORE owns the per-datasource request header, never the target plugin — it crosses as an INDEXED pair SPLIT across the two documents, `httpHeaderName<N>` in `jsonDataEncoded` beside `httpHeaderValue<N>` in `secureJsonDataEncoded`, one-indexed and dense, so the value never renders into readable state and a name landed without its paired value provisions a header the request omits. `httpHeaders` states the same capability as a typed arg at the resource surface, so a row already folding its whole settings body spells the indexed pair instead and one seam owns both documents. `operate/observe#BOARD_APPLY`'s `_headed` is that fold, and the scope header an `org`-tenancy store row demands is its one current consumer.

[VERSIONED_SPLIT]: the namespace root carries a deprecated flat alias per versioned resource, each a `@deprecated` re-home rather than a second capability — `AlertRuleV0Alpha1` -> `alerting.v0alpha1.AlertRule`, `RecordingRuleV0Alpha1` -> `alerting.v0alpha1.RecordingRule`, `AlertEnrichment` -> `alerting.v1beta1.AlertEnrichment`; the versioned owner is the one spelling a fence takes, and the flat alias never appears.
...
- Accept: one Doppler-sourced `Provider` per stack; resources fed LGTM `Output<T>` URLs; every `oss.DataSource` row answering both `jsonDataEncoded` and `secureJsonDataEncoded` so a plugin dialing its own coordinates connects and a row needing neither encodes an empty document; construction inside the Automation-API inline program with `{ provider }` on every resource; `storeDashboardSha256` for content-hash drift; `oss.ServiceAccountRotatingToken` as the durable automation credential over an Editor account scoped by one `oss.FolderPermissionItem`; `orgId` (`Input<string>`) tenant scoping from the realized `oss.Organization.orgId` (`Output<number>`); explicit org-scoped `uid` on every `oss.DataSource` so one compiled dashboard JSON binds identically in every org; the versioned owner under `alerting.v0alpha1`/`alerting.v1beta1` for every resource the namespace root also aliases; routing identity carried as alert LABELS so matchers, `groupBies`, and silences all read one plane
- Reject: an inline `auth` token; authored `Pulumi.yaml` or dashboard-JSON files on disk; importing outside `iac`; a `get*` data source where a managed resource with `Output` wiring belongs; a deprecated flat alias (`AlertRuleV0Alpha1`, `RecordingRuleV0Alpha1`, `AlertEnrichment`) where the versioned owner stands; `alerting.v1beta1.AlertEnrichment`, a Grafana Cloud preview surface with no self-hosted OSS target; a rule whose write target the store row's own ingest posture does not admit
