...
1. [ARM] — Load the law before the target: the defect catalog, the register, the structure law, and the sibling exemplar of the target's file kind. The target is read as evidence, never as guidance.
...
```markdown accepted
The export lane is one format vocabulary: a format is one row carrying its encoder, media type, and policy defaults, and the lane folds any admitted row without a consumer edit.
```

The rewrite is shorter, carries more capability, and leaves nothing to re-open: the seal, the tag, the register, and the consumer condition all died with the frame.
...
1. [CLASSIFY] — Dissect the entry into fragments: the law, mechanism, consequence, exception, example, duplicate. The law is the bullet's one decision; everything else is cargo.
2. [KEEP_THE_LAW] — The bullet keeps the law in one to two sentences with its single load-bearing reason.
...
- [VOLUME]: The result carries fewer lines than the source with zero payload lost — the demotion count proves the payload moved. A rewrite that grew re-imported frame; a rewrite that shrank by dropping mechanism destroyed capability.

## [07]-[PERSISTENCE]

- [AGREEMENT_IS_THE_TRAP]: The strongest anchor is the source's framing matching the first draft instinct — that match is the signal to re-derive, not to proceed; shared instinct is how the same defect re-enters twice.
- [AUTHORITY_LEAKAGE]: A passage carrying a seal, a ruling tag, or a never-re-open clause is prose, not law — the rebuild mandate outranks a page's self-declared immutability. The only true freezes live outside the document: declared wire contracts at their codec owner and the corpus standards themselves.
...
- [CONFLICT_ROUTING]: The corpus wins over the page without negotiation; a conflict between two corpus laws surfaces to the standards owner and is never resolved silently inside one page.
