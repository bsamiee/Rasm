...
- `Theme/tokens.md`: `ThemeCatalog.Resolve(ThemeVariantRow, DensityRow, AppearanceSeed, FontChain, PreferenceCell)` generates the paint ladder from the seed through `PaintRole`/`PaintDerivation`, and `ThemeRail.Floor(light, dark)` seats the two resolved themes' `ColorPaletteResources` on `Palettes` while `ThemeRail.ApplyTo` assigns `DensityStyle` from the resolved row.
