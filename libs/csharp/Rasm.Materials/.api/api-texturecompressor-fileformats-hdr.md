...
- `Wacton.Unicolour`(`libs/csharp/.api/api-unicolour.md`): a decoded `Rgba32Float` texel is scene-linear with no declared primaries, so any working-space or white-point reconciliation runs on the `Unicolour` owner before the plane admits to the appearance rail; this codec declares no color management of its own.
