...
- Law: `Convention.scope(pkg)` is the one instrumentation-scope spelling — `@rasm/<pkg>`, the emitting package id — consumed at the one raw-OTel seam a package reaches for a scoped meter or tracer: `runtime/otel/emit` `Hooks.meter(pkg)` names its `getMeter` call with this value, while Effect's own spans, metrics, and logs ride the app `Resource` under one facade scope; a backend slices third-party instrument series by package uniformly, and a free-string scope at that seam is the same defect as a free-string metric name.
- Law: `Convention.wire` is the estate wire constant pair — `schemaUrl` pins one semconv schema on every `Resource` the runtime export lanes emit (`runtime/otel/emit` folds it into `resourceFromAttributes`; the path segment tracks the manifest's semconv pin and bumps with it), and `translation` names the Prometheus OTLP-receiver strategy (`NoUTF8EscapingWithSuffixes`) that preserves dotted names while appending type and unit suffixes — `iac/operate/observe` reads it into the metrics store's `otlp.translation_strategy`, so neither wire member is spelled free-string at any store or export seam and the three runtimes' series stay byte-identical.
