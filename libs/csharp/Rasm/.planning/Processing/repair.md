...
- [WELD_IDEMPOTENCE] — `DuplicateWeld` delegates `Meshing/edit.md` `Kernels.WeldDuplicates`: union-find over a tolerance grid at `ArenaPolicy.WeldTolerance`, in-place SoA compaction, class centroids, union-by-smallest-index order stability. The law-matrix asserts idempotence (`Repair(Repair(m)) == Repair(m)` at one band) and enumeration-order stability at the arena owner; this page adds only the composition and SCOPES the heal-level corollary: a `Standard` re-run is receipt-stable exactly on sessions whose `ManifoldRepair` split nothing — a vertex-copy split leaves coincident copies INSIDE the weld band, so a re-run's weld re-merges them and its manifold pass re-splits (the same healed shape re-converges; the receipt chain is not empty). The fixpoint law is stated on the split-free class, never as an unqualified idempotence claim.
