...
- within-lib retrieval: numpy embeddings → `create_index(col, "IVF_HNSW_PQ", metric="cosine")` (or `"IVF_RQ"` for RaBitQ compression) → `scanner(nearest={"column":, "q":np_vec, "k":})` → `pa.Table`, with `prefilter` gating a scalar predicate before the ANN stage; the FTS domain stacks `create_scalar_index(col, "INVERTED")` → `full_text_query=lance.query.BoostQuery([MatchQuery(...), PhraseQuery(...)])` for one-scan hybrid lexical+vector retrieval.
