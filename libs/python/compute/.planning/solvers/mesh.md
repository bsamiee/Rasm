...
class ElementRow(Struct, frozen=True):
    ...
    def built(self, skfem: Any) -> Any:
        ...
        # several, each inner element itself built by its own row. Names resolve through `getattr(skfem, ...)` behind
        # the deferred worker import, so no spelling is duplicated per family.
...
# row resolves both spellings behind the deferred skfem import — module-load lightness, not cycle evasion —
