# [PLATFORM_FEATURE_FLAGS_CONFIG]

One page owns the browser feature-flag and remote-config read-side concern — `RemoteConfig`, the browser READ-side companion to the `services` `persistence` `FeatureFlags` owner: a `RuntimeConfig`-fed remote-config fetch decoded once through `Schema.decodeUnknown` into one `FlagSet` `SubscriptionRef`, with flag evaluation as a total `Match` dispatch over a `Schema.Literal` flag-key axis whose percentage-bucket and variant VOCABULARY is consumed as settled from `services` and whose read-side resolution is a local deterministic bucket over the decoded `FlagSet`. The flag-bucket/variant axis is declared once in `services` and referenced here, never re-authored — the anti-spam cluster law. `RemoteConfig` authors no wire shape; the `FlagSet` decode shape is the only platform-local `Schema`. The page composes `RuntimeConfig` and the browser `HttpClient`, holds no flag vocabulary, and surfaces a flag through the `ui` `AtomBinding`.

## [1]-[INDEX]

| [INDEX] | [CLUSTER]            | [OWNS]                                                              |
| :-----: | :------------------- | :----------------------------------------------------------------- |
|   [1]   | FEATURE_FLAGS_CONFIG | the remote-config fetch, the flag-key dispatch, and the refresh    |

## [2]-[FEATURE_FLAGS_CONFIG]
...
- Growth: a new flag lands as one literal on the `FlagKey` axis referencing the `services` `FeatureFlags` vocabulary; a new evaluation modality lands as one arm on the `FlagEvaluation` `Match` dispatch; a new refresh trigger lands as one row on the refresh `Schedule`, never a parallel config surface and never a re-authored bucket axis.
...
const evaluateFlag = (value: FlagValue, subjectKey: string): FlagEvaluation => {
  const bucket = (key: string): number => {
    const hash = Array.from(key).reduce((acc, ch) => (acc * 31 + ch.charCodeAt(0)) >>> 0, 0);
    return hash % 100;
  };
