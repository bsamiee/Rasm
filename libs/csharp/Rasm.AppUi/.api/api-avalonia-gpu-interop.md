...
[STACKING]:
- `api-silk-webgpu`(`libs/csharp/.api/api-silk-webgpu.md`) / `api-silk-webgpu-wgpu`(`libs/csharp/.api/api-silk-webgpu-wgpu.md`): the `Wgpu` backend renders through `WebGPU.GetApi()` -> instance/adapter/device with the adapter matched to this interop's `DeviceLuid`/`DeviceUuid`; the rendered `Texture` exports as the platform handle `ImportImage` binds, and its export-format synchronization primitive selects the matching `UpdateWith*Async` member.
