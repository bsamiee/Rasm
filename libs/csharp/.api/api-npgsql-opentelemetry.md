...
- `Npgsql`(`libs/csharp/.api/api-npgsql.md`): `NpgsqlDataSourceBuilder.ConfigureTracing` folds every span filter, name provider, and enrichment callback into one per-data-source pass, and `Name` keys the pool dimension every instrument tags — one builder shapes both signals.
