...
- [1]-[PATCH_RAIL]: the six-arm `PatchOpWire` union, the `applyPatch` fold, the `createTests` guard, the `fieldMaskLower`.
- [2]-[TS_PROJECTION]: the C# `JsonPatchDocument` operation wire and the proto `FieldMask` shapes the rail decodes.
