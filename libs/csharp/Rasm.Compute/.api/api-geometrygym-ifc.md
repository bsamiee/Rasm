...
| [INDEX] | [SURFACE]                       | [CALL_SHAPE]              | [CAPABILITY]                                       |
| :-----: | :------------------------------ | :----------------------- | :------------------------------------------------ |
|   [1]   | `DatabaseIfc.Project`           | `IfcContext` property     | the root context as `IfcProject` when applicable  |
|   [2]   | `DatabaseIfc.Context`           | `IfcContext` property     | the active context (project or library)           |
|   [3]   | `DatabaseIfc.Factory`           | `FactoryIfc` property     | per-database entity factory                        |
|   [4]   | `DatabaseIfc.Release`           | `ReleaseVersion` property | active schema version                             |
|   [5]   | `DatabaseIfc.ModelView`         | `ModelView` property      | active model view                                 |
|   [6]   | `DatabaseIfc.Tolerance`         | `double` property         | geometric tolerance                              |
|   [7]   | `DatabaseIfc.ToleranceAngleRadians`| `double` property      | angular tolerance in radians                      |
|   [8]   | `DatabaseIfc.ScaleSI`           | `double` property         | SI length scale                                  |
|   [9]   | `DatabaseIfc.ScaleAngle`        | `()` → `double`           | active angle scale factor                         |
|  [10]   | `DatabaseIfc.this[int stepId]`  | indexer → `BaseClassIfc`  | entity by STEP record id                          |
|  [11]   | `DatabaseIfc.this[string globalID]` | indexer → `BaseClassIfc` | entity by IFC GlobalId                          |
|  [12]   | `DatabaseIfc` enumeration       | `IEnumerable<BaseClassIfc>` (via `DatabaseSTEP<T>`) | iterates all entities       |
|  [13]   | `DatabaseSTEP<T>.NextObjectRecord` | `int` property         | next STEP record id                              |
|  [14]   | `DatabaseSTEP<T>.OriginatingFileInformation` | `STEPFileInformation` field | source-file header metadata          |
...
| [INDEX] | [SURFACE]                    | [CALL_SHAPE]                          | [CAPABILITY]                                          |
| :-----: | :--------------------------- | :------------------------------------ | :--------------------------------------------------- |
|   [1]   | `BaseClassIfc.Extract<T>`    | `()` → `List<T>` where `T : IBaseClassIfc` | collects all reachable entities of a type     |
|   [2]   | `BaseClassIfc.Database`      | `DatabaseIfc` property                  | owning database                                     |
|   [3]   | `BaseClassIfc.GetType`       | `(string classNameIfc)` → `Type` (static) | resolves a CLR `Type` from an IFC class name   |
|   [4]   | `BaseClassIfc.Construct`     | `(string ifcClassName)` → `BaseClassIfc` (static) | constructs an entity by IFC class name  |
|   [5]   | `BaseClassIfc.StringSTEP`    | `(ReleaseVersion)` → `string`           | serializes one entity to a STEP record line         |
|   [6]   | `BaseClassIfc.getJson`       | `(BaseClassIfc host, SetJsonOptions)` → `JsonObject` | serializes one entity to IFC-JSON      |
|   [7]   | `IfcObjectDefinition.FindPropertySet` | `(string name)` → `IfcPropertySetDefinition` | resolves a property set by name           |
|   [8]   | `IfcObjectDefinition.FindProperty` | `(string name)` → `IfcProperty`     | resolves a single property by name                 |
|   [9]   | `IfcObjectDefinition.AddAggregated` | `(IfcObjectDefinition)`             | adds a part to a `IfcRelAggregates` decomposition  |
|  [10]   | `IfcProduct.AddElement`      | `(IfcProduct)`                         | adds a contained product into the spatial structure |
|  [11]   | `IfcElement.SetMaterial`     | `(IfcMaterialSelect)`                  | associates a material with an element               |
