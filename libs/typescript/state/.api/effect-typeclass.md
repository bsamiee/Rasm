# [@effect/typeclass] — the lawful merge-law substrate: a CRDT merge is a `Semigroup`/`Monoid` instance, the semilattice is `min`/`max` over an `Order`, never an ad hoc function

[PACKAGE_SURFACE]:
- package: `@effect/typeclass` · version `0.40.0` · license `MIT` · © Effectful Technologies
...
- plane: `plane:runtime` (W1); folder-local to `state`, catalogued here — the merge-law owner `crdt/merge` and the `proof` law combinators share.
- rail: algebraic-structure / convergence-law.

`@effect/typeclass` is the standalone, complete higher-kinded typeclass hierarchy `effect` core does not fully expose: the full `Semigroup`/`Monoid`/`Bounded` algebra plus the `Covariant`→`Applicative`→`Monad`→`Foldable`→`Traversable` lattice over `TypeLambda`. In `state` it is the ONE thing that makes `crdt/merge` lawful: a CRDT merge is a `Semigroup<Op>` (an associative `combine`), a bounded merge is a `Monoid<Op>` (associative + identity), and the convergence-legal subset is the idempotent-commutative semilattice — `Semigroup.min(Order)` / `Semigroup.max(Order)` for join/meet, `Record.getSemigroupUnion` for grow-only maps. `crdt/merge` declares merges as these instances; `crdt/converge` asserts associativity/commutativity/idempotence as the typeclass laws over `proof/corpus`; `fold/replay` applies the same instance through the d2ts `reduce` — the merge law and the reducer law are one law. The instance is composed structurally (`struct`/`tuple`/`array`), lifted through any functor (`SemiApplicative.getSemigroup`), and folded over any container (`Foldable.combineMap`), so growth is a row on the algebra, never a new hand-written merge.
...
| [INDEX] | [SYMBOL]            | [CONTRACT]                                                    | [MERGE ROLE / BOUNDARY]                                        |
| :-----: | :------------------ | :----------------------------------------------------------- | :------------------------------------------------------------ |
...
| [INDEX] | [SURFACE]                                                     | [PRODUCES]              | [MERGE SEMANTICS]                                             |
| :-----: | :------------------------------------------------------------ | :---------------------- | :----------------------------------------------------------- |
...
The higher hierarchy is the `proof` law-combinator surface: it LIFTS a `Semigroup`/`Monoid` through any functor `F` and FOLDS it over any `Foldable`, so the same merge instance proves its laws inside `Option`, `Either`, `Array`, or a decoded op container without a bespoke harness. This is the "shared with the proof law combinators" the merge design names.

| [INDEX] | [SURFACE]                                              | [LIFTS / FOLDS]                                              | [PROOF ROLE]                                                  |
| :-----: | :----------------------------------------------------- | :---------------------------------------------------------- | :----------------------------------------------------------- |
...
|  [06]   | `Covariant.mapComposition` / `Covariant.imap`          | map through nested functors; invariant bidirectional map     | the functor laws `crdt/converge` checks alongside merge laws  |
