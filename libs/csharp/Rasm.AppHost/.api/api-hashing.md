...
- kernel identity entry (`[V18]`): content digests compose `ContentHash.Of` → `UInt128`; `UInt128` is the identity currency and hex/two-lane-`ulong` encodings are boundary projections at the consuming seam, never a re-mint. This catalog's `XxHash128` row is the algorithm the kernel entry wraps, not a direct AppHost call site.
