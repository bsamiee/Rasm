...
- [02]-[ASSERTION_HOOK]: both JWT builders mint `{ jti, aud: as.issuer, exp: now + 60, iat, nbf, iss: client_id, sub: client_id }` and hand header and payload to `options[modifyAssertion]` before signing — the one hook an issuer demanding a divergent claim (Apple's team-scoped `iss`) takes without a hand-built assertion.
