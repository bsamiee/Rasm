...
- highlight return semantics — `Highlight(bool)` and `HighlightSubObject(ComponentIndex, bool)` each answer the RESULTING state ("true if the object is now highlighted"), never a changed flag, so a request that was already satisfied answers `true` and only a refused write answers `false`; the selection family's `int` counts are the asymmetric sibling, and a guard reading one shape onto the other inverts. `IsHighlighted(bool checkSubObjects)` answers the closed grade roster `0` not highlighted, `1` entire object, `3` proper sub-objects.
...
- `Rhino.DocObjects.Custom.CustomObjectGrips.RegisterGripsEnabler(TurnOnGripsEventHandler enabler, Type customGripsType) -> void` (static) — seats the enabler the host calls when grips turn on for that grip type.
- grips-enabler custody — registration keys on `customGripsType.GUID` into a process-static map and a second registration under the same identity REPLACES the first; NO unregister member exists, so the only removal the host admits is re-registering an inert enabler under the same `[Guid]`. The member throws `ArgumentNullException` on a null enabler and `ArgumentException` on a type not derived from `CustomObjectGrips`.
...
- `Rhino.DocObjects.RhinoObject.IsMeshable(MeshType meshType) -> bool` / `GetRenderMeshParameters() -> MeshingParameters` / `GetRenderMeshParameters(bool returnDocumentParametersIfUnset) -> MeshingParameters` / `SetRenderMeshParameters(MeshingParameters mp) -> bool` — meshability and per-object meshing policy.
- meshing-parameter custody splits by ARGUMENT, not by member name — `GetRenderMeshParameters()` is defined as `GetRenderMeshParameters(true)` and mints a fresh caller-owned disposable filled from the object or the document, while `GetRenderMeshParameters(false)` returns `new MeshingParameters(RHC_RhinoObjectGetMeshParameters(ConstPointer()))`, a NON-OWNING wrapper over the object's stored parameters, or `null` when none are set. `MeshingParameters.Dispose` frees the pointer it holds unconditionally, so the `false` overload is never bracketed and a `using` over it corrupts the object's own state.
...
- `ObjectAttributes` re-declares the user-string roster `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns; the attribute store is independent of the geometry store on the same object.
...
- `Rhino.DocObjects.ObjectAttributes.ObjectFrame() -> Plane` / `SetObjectFrame(Transform xform) -> void` / `SetObjectFrame(Plane plane) -> void` — the object-frame write home; both `RhinoObject.SetObjectFrame(Transform)`/`(Plane)` carry `[Obsolete("Use Attributes.SetObjectFrame instead")]` and forward to these before `CommitChanges`, so the attribute-set pair plus an explicit commit is the whole write.
- `Rhino.DocObjects.ObjectAttributes.Decals -> Decals` / `MaterialRefs -> MaterialRefs` / `File3dmMeshModifiers -> File3dmMeshModifiers` — readable render-content carriers on the attribute set.
- `Rhino.DocObjects.ObjectAttributes.RenderMaterial -> RenderMaterial` — write-only accessor. The setter resolves `value.DocumentOwner` and throws `ArgumentException` when the content is not document-attached, projects through `ToMaterial`, adds or modifies the document material at `MaterialIndex`, and sets `MaterialSource = MaterialFromObject`. Render-material IDENTITY reads back through `MaterialIndex` and the document material table; a read of this member is a compile error.
- `Rhino.DocObjects.ObjectAttributes.CustomMeshingParameters -> MeshingParameters` / `EnableCustomMeshingParameters -> bool` — per-object meshing-parameter override. The getter hands back a wrapper over the attribute set's own stored parameters (`GC.KeepAlive(this)` marks the aliasing) or `null` when unset, and `MeshingParameters.Dispose` frees that pointer unconditionally, so the read is a BORROW and never rides a `using`; the setter copies from the value's const pointer, so the caller keeps custody of what it assigns.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Transform`/`BoundingBox` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
