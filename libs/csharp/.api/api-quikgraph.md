...
[STACK_INTEGRATION]:
- `Rasm` (kernel) `Spatial/neighbors#NEIGHBORHOOD_FOLDS` (Hoppe-DeRose normal orientation): `NeighborKernel.OrientNormals` folds the `GraphOf` kNN rows into a transient `UndirectedGraph<int, SEdge<int>>` (`allowParallelEdges: false`; `AddVertexRange(Enumerable.Range(0, n))` admitting every point so a disconnected cluster still orients as its own forest root; `AddEdge(new SEdge<int>(i, j))` per neighbor row), takes `MinimumSpanningTreePrim(edgeWeights: e => 1.0 − Math.Abs(normals[e.Source] * normals[e.Target]))`, and propagates normal sign by BFS over the MST adjacency flipping any child whose dot against its parent is negative — QuikGraph owns the MST, the sign propagation and the `Fin`-railed receipt stay the domain fold. The vertex is the dense point INDEX (no domain key — the cloud row position IS the identity), and the graph is transient per `OrientNormals` call, never a stored field.
