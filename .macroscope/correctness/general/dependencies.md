...
- A sanctioned pin carries an inline comment stating exactly what breaks without it; a pin missing that mechanism comment is a finding, and a pin carrying one is never flagged — `python_version` gates in `pyproject.toml` are evidence-backed rulings, re-tested only when the named blocker moves. Demand-side law matches: a demanded new floor or pin justified by a hypothetical older release is void where the installed pinned-newest surface constructs the member — a pin is earned only by a reproduced incompatibility.
...
- A manifest reference breaching the strata edges the owning branch's `ARCHITECTURE.md` table declares is a strata defect; resolve that table live for the branch the manifest serves, since each branch names its own strata and a transcribed roster reads stale the moment the table grows.
...
- A typed settings map in a workspace manifest (`allowBuilds` and kin) carries only values of its declared type, sorted by key; a scaffold-hint placeholder committed as a value parses as neither state and the package manager re-warns on every invocation.
