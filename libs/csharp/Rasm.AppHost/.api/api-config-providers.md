...
`Microsoft.Extensions.Configuration.Json`, `EnvironmentVariables`, `CommandLine`, and `UserSecrets` supply the four sibling configuration provider sources that mount JSON files, process environment, argument vectors, and developer secret stores onto one configuration builder chain; `Microsoft.Extensions.Options.ConfigurationExtensions` binds a configuration section onto an `OptionsBuilder<T>` so options validate-on-start re-binds from configuration without an interior `IConfiguration` read.
