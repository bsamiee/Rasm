...
- package: `Thinktecture.Runtime.Extensions.MessagePack` `10.4.0`
...
- dependencies: `Thinktecture.Runtime.Extensions` `10.4.0`; `MessagePack` resolved through central/transitive package graph
...
- `GetFormatter<T>()` returns `null` for types that are not Thinktecture-generated owners or that should be skipped because an explicit `[MessagePackFormatter]` owns them.
...
- A type carrying an explicit `[MessagePackFormatter]` can be skipped by resolver policy so the explicit formatter remains the owner.
...
- Reject: hand-written MessagePack formatters for generated owners, conflating this bridge with the `MessagePack` engine package, or registering this resolver after a fallback that would shadow it.
