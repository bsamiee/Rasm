...
- Cases: `IssueOp.Ingest(IssueSpec, Seq<IssueTopic>)` admits a handed row set and yields correlated rows; `IssueOp.Egress(IssueSpec, Seq<IssueTopic>)` releases held rows and yields the count. `IssueFault` is `SourceReject | TopicRejected`; independent failures accumulate through `Validation<Error, …>`.
...
public abstract partial record IssueFault : Fault {
    ...
    private IssueFault() { }
    [FaultCase(0)]
    public sealed partial record SourceReject(string Detail) : IssueFault();
    [FaultCase(1)]
    public sealed partial record TopicRejected(Guid Topic, string Detail) : IssueFault();


    public override string Message => Switch(
        sourceReject:    static c => $"<issue-source:{c.Detail}>",
