...
- Cases: `SubdivisionScheme` rows 2 (the next scheme is a ROW — Doo-Sabin and √3 are recorded growth, each one `static readonly` item with delegates, never a sibling subdivider); `SubdivideOp` cases `Refine` · `Limit` (2); `SubdivisionResult` cases `Refined` · `LimitField` (2).
...
// --- [TYPES] ------------------------------------------------------------------------------------
// Scheme IS the row: arity + stencil weights + limit masks + eigenbasis seat as delegate
...
public static class Subdivision {
    ...
    // → stencil triplets (interior rows off the scheme columns; boundary rows read BoundaryMask —
    // VertexEnd/VertexSelf/VertexEnd vertex mask, EdgeEnd·2 spline midpoint — crease/corner masks
    ...
    // 2449 witness column carries the refinement level — the per-concern measure of this stage.
