...
[ENTRYPOINT_SCOPE]: the registry record — `PlugInInfo` is read-only with an internal constructor, so `GetPlugInInfo` is its only ingress
- `PlugInInfo.Id -> Guid` / `Name` / `Description` / `FileName` / `RegistryPath` / `Version -> string` — record identity and location
- `PlugInInfo.Organization` / `Address` / `Country` / `Email` / `Phone` / `Fax` / `WebSite` / `UpdateUrl -> string` — the contact block `PlugInDescriptionAttribute` populates
- `PlugInInfo.PlugInType -> PlugInType` / `PlugInLoadTime -> PlugInLoadTime` — each reads a native `int` and answers the default (`None`, `Disabled`) when the value is not a defined enum member, so an unrecognized record reads as untyped and never-loading rather than faulting
- `PlugInInfo.IsLoaded` / `ShipsWithRhino` / `IsDotNet -> bool` — load and provenance discriminants
- `PlugInInfo.CommandNames` / `FileTypeDescriptions` / `FileTypeExtensions -> string[]` — the registered command and file-type rosters, each a fresh array off a native string array
- `PlugInInfo.Icon(Size size) -> Bitmap` / `IsLoadProtected(out bool loadSilently) -> bool` — sized icon and the load-protection probe
- `Version` is not a plain record read: an empty native string falls back to `AssemblyName.GetAssemblyName(FileName).Version` for a managed plug-in and `FileVersionInfo` otherwise, so the read touches disk and answers `null` when `FileName` names no existing file
...
- `protected virtual CreateCommands() -> void` / `RegisterCommand(Command command) -> bool`; `GetCommands() -> Command[]`; `HostUtils.RegisterDynamicCommand` composes the runtime command family of `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-commands.md`
- `public virtual GetPlugInObject() -> object` / `public virtual DisplayHelp(nint windowHandle) -> bool` — the two PUBLIC overridable hooks, both returning the base's own `null`/`false` unless overridden
- `public Icon(Size size) -> Bitmap` is NOT virtual — it forwards to `GetPlugInInfo(Id)?.Icon(size)`, so no icon override exists, the plug-in icon is a registry read, and a null answer is an absent icon
...
- `protected virtual OptionsDialogPages(List<OptionsDialogPage> pages) -> void` / `DocumentPropertiesDialogPages(RhinoDoc doc, List<OptionsDialogPage> pages) -> void` / `ObjectPropertiesPages(ObjectPropertiesPageCollection collection) -> void`; the sibling `ObjectPropertiesPages(List<ObjectPropertiesPage> pages)` overload carries `[Obsolete]` naming the collection form as its replacement, so only the collection overload is an admission route

[ENTRYPOINT_SCOPE]: plug-in settings
- `PlugIn.GetPluginSettings(Guid plugInId, bool load) -> PersistentSettings` / `SavePluginSettings(Guid plugInId) -> void` / `SaveSettings() -> void` / `CommandSettings(string name) -> PersistentSettings` — the settings tree, whose typed writer surface `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns
- `PlugIn.SettingsSaved -> EventHandler<PersistentSettingsSavedEventArgs>` — fires the change notification `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns; `RaiseOnPlugInSettingsSavedEvent()` / `FlushSettingsSavedQueue()` raise and drain it
...
- `PreviewNotification.NotifyIntermediateUpdate(RenderWindow rw) -> void` — the sole member of the sealed host-minted notifier a preview body receives; its constructor is internal, so a long-running preview publishes partial frames through the instance the host hands it and never mints one
...
- `LicenseUtils.GetLicenseStatus() -> LicenseStatus[]` / `GetOneLicenseStatus(Guid productid) -> LicenseStatus` / `CheckOutLicense(Guid productId) -> bool` / `CheckInLicense(Guid productId) -> bool` / `ReturnLicense(Guid productId) -> bool` / `ConvertLicense(Guid productId) -> bool` / `DeleteLicense(Guid productId) -> bool` / `GetLicenseType(Guid productId) -> int` / `IsCheckOutEnabled() -> bool`
- `LicenseUtils.GetLicenseCapabilities(int filter) -> LicenseCapabilities` maps the filter bit-for-bit onto the SAME `LicenseCapabilities` ordinals and THROWS `ArgumentException` for any filter above `0x1FF`, so a raw persisted bitfield admits onto a typed capability owner rather than through this member
...
- `LicenseUtils.Initialize() -> bool` / `Echo(string message) -> string` (null on blank input) / `ShowRhinoExpiredMessage(Rhino.Runtime.Mode mode, ref int result) -> bool` / `LicenseOptionsHandler(Guid pluginId, Guid licenseId, string productTitle, bool standAlone) -> bool` — the handler reads its CALLING assembly name and answers false unless that assembly is `LicenseOptions`, so it is unreachable from package code
...
[STACKING]:
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-runtime.md`: the license family splits at the caller — this catalog owns the `PlugIn`-facing entitlement rail (`PlugIn.GetLicense`, `LicenseUtils`, the license data/status/lease carriers, and the two change payloads), and the runtime catalog owns the Zoo and Rhino Accounts state beneath it (`ZooClientParameters`, `IZooClientUtilities`, `LicenseTypes`, `LicenseStateChangedEventArgs`, and the accounts token rail)
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Point3d` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
...
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-fileio.md`: `FileWriteOptions`/`WriteFileResult` write-policy detail and the direct format engines compose there; this catalog owns only their dispatch-contract role
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md`: `PersistentSettings` node and `SettingsSaved` event compose there; `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendercontent.md` and the render catalogs own the `RenderPlugIn` content-registration and change-queue surfaces
...
- license acquisition enters through `PlugIn.GetLicense`/`LicenseUtils`, and settings custody routes to `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md`; direct `File3dm` and engine invocation stay the package-driven path of `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-fileio.md`
