...
- Auto: `Compare` reads the capture graph as an occurrence set (`ByKind(ObjectKind.Occurrence)`) and splits it once — the `ByProperty` restriction over the `Pset_Reconstruction` `NeedsReview` row (the flag the `Exchange/reconstruct#RECONSTRUCTION` fold stamps when a fit falls under its `ConfidenceFloor`) partitions the below-floor occurrences off through the same-graph `ElementSet.Except`, so the verified set and the flagged set are two projections of one selection and no occurrence lands in both; each task's `Observed` is `MatchedElements / ExpectedElements` over the VERIFIED ids alone while the flagged intersection rides `Uncertain`, so a low-confidence fit never inflates a completion fraction and never disappears from the receipt either; `Expected` is the clamped elapsed fraction of the task's scheduled `Interval` at `captureAt` — the schedule's planned window, never the actual one, because the comparison the report exists to make is measured-against-PLAN; the `CaptureKey` folds the ordinal-sorted DISTINCT `SourceCloud` lineage texts the capture occurrences publish through the seam `Rasm.Element/Projection/address#CONTENT_ADDRESS` `ContentAddress` over the `Rasm.Element/Projection/address#CANONICAL_WRITER` `CanonicalWriter` — length-prefixed per row under its `Ordinal` count — so a single-scan verification keys off that one lineage, a federated multi-scan verification off its whole set, and a dashboard re-reads a report only when the capture behind it genuinely moved; the `Unmatched` residue is the capture's `GlobalIds` minus the union of every assignment set, the as-built evidence a coordination read treats as unplanned work or as an unmodelled existing condition.
- Receipt: the `ProgressReport` is the typed physical-progress evidence — `Planning/cost#EARNED_VALUE` reads the PRESENT per-task `Observed` fractions as the `Map<string, double> observed` the `CostSchedule.EarnedValue` actual-percent election consumes at its HIGHEST tier (evidence outranking the authored `IfcTaskTime.Completion`, an absent measurement falling through to the next tier rather than entering as a zero), and the `Rasm.AppUi/Charts` progress dashboard reads the row set by the `CaptureKey` so a re-render costs nothing on an unchanged capture; `Variance` is the per-task finding a 4D report ranks where a measurement exists, `Uncertain` the data-coverage verdict a reviewer reads before trusting any fraction, and `Unmatched` the as-built residue the coordination board turns into an issue — each carried on the one report, never a second progress store.
...
using IdSet = LanguageExt.HashSet<string>;                  // the GlobalId membership set — equality-keyed, aliased so
                                                            // the bare name never collides with the global-using
                                                            // System.Collections.Generic.HashSet.

namespace Rasm.Bim.Planning;

// --- [MODELS] -----------------------------------------------------------------------------
// One task's measured-against-planned row. Observed divides the assigned elements the capture VERIFIES by the
// assigned elements the as-designed graph declares, so it carries physical evidence and never the authored
// IfcTaskTime.Completion the schedule owns; Expected reads the same instant's planned fraction; Variance signs
// whichever gap a 4D report ranks on, negative reading behind physical plan. Uncertain counts assigned elements
// whose capture occurrence the reconstruction flagged below its ConfidenceFloor — held OFF the fraction and ON
// its own column, because folding a low-confidence fit into Observed reads an unverified element as built while
// dropping it erases the coverage question a reviewer asks before trusting any number here.
// Observed is an OPTION because a task assigning no geometry measures NOTHING: a capture cannot confirm an
// activity that claims no element, and a zero there is a false measurement — it reads as "built nothing" and
// ranks a fully-planned activity as the worst variance on the board. Variance propagates that absence rather
// than coercing it to a number, so a dashboard renders "unmeasured" where no measurement exists.
...
public static class ProgressVerification {
    // Two standing selections, both in the ONE predicate algebra, both keyed off the Exchange/reconstruct
    // #RECONSTRUCTION ReconstructionRows statics the producer already publishes — NeedsReview flags a
    // below-ConfidenceFloor occurrence, SourceCloud carries the CaptureLineage hex the LAS/LAZ decode minted over
    // the source bytes (the ONE address a re-fetch of that capture closes on). A page-local const restating those
    // spellings is the deleted form: the producer renames a row and this consumer silently selects nothing.
    ...
    public static Fin<ProgressReport> Compare(ScheduleNetwork network, ElementGraph asDesigned, ElementGraph observed, Instant captureAt, Op key) =>
        network.BindAssignments(asDesigned, key).Map(bound => {
            ElementSet capture = ElementSet.Query(observed, new ElementPredicate.ByKind(ObjectKind.Occurrence));
            ElementSet uncertain = capture.Where(Flagged);
    ...
    // Each assigned set denominates, the VERIFIED capture ids among it numerate, and the flagged ids fill the
    // Uncertain column. A task assigning nothing measures NOTHING — the fraction is absent, not zero and not a
    // vacuous 1.0: no capture confirms an activity that claims no geometry, and either number would rank an
    // unmeasurable activity against measured ones on the same board.
    ...
    // Capture identity folds the ordinal-sorted DISTINCT SourceCloud lineage texts the occurrences publish,
    // minting the TYPED seam ContentAddress over the CanonicalWriter preimage — length-prefixed strings
    // under their Ordinal count, so no concatenation of two lineage sets can forge an equality with a third.
    // Re-minting a CaptureLineage over the SET is the deleted form: that value object addresses ONE capture's
    // source bytes, and the reconstruction owner already closed the one-type-two-key-spaces defect.
    static ContentAddress CaptureKeyOf(ElementGraph observed, ElementSet capture) {
        Seq<string> lineages = toSeq(capture.Objects
            .Bind(obj => ElementSet.ValuesOf(observed, obj, Lineage))
            .Map(static value => value.Render())
            .Distinct()
            .OrderBy(static text => text, StringComparer.Ordinal));
        return ContentAddress.Of(lineages
            .Fold(new CanonicalWriter(0.0).Ordinal(lineages.Count), static (writer, text) => writer.String(text))
            .ToBytes().Span);
    }
}
```
