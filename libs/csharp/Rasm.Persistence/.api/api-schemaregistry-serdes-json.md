...
|  [03]   | `IAsyncSerializer<T>` / `IAsyncDeserializer<T>` | codec slot      | `Confluent.Kafka` — `SetValueSerializer`/`SetValueDeserializer` slot |
...
Every ctor takes an optional trailing `jsonSchemaGeneratorSettings?` the table omits.

| [INDEX] | [SURFACE]                                                       | [ENTRY_FAMILY] | [RAIL]                                           |
| :-----: | :-------------------------------------------------------------- | :------------- | :----------------------------------------------- |
|  [01]   | `new JsonSerializer<T>(client, config?, ruleRegistry?)`         | ctor           | generates the schema from `T` via NJsonSchema    |
|  [02]   | `new JsonSerializer<T>(client, schema, config?, ruleRegistry?)` | ctor           | uses an explicit registered `Schema`             |
|  [03]   | `new JsonDeserializer<T>(client, config, ruleRegistry?)`        | ctor           | registry + typed config                          |
|  [04]   | `new JsonDeserializer<T>(client, schema, config?)`              | ctor           | explicit reader `Schema`                         |
|  [05]   | `new JsonDeserializer<T>(config?)`                              | ctor           | registry-less decode (id framing only, no fetch) |
...
| [INDEX] | [SURFACE]                                                | [ENTRY_FAMILY] | [RAIL]                                                     |
| :-----: | :------------------------------------------------------- | :------------- | :--------------------------------------------------------- |
|  [01]   | `SerializeAsync(value, context)` -> `Task<byte[]>`       | encode         | generates schema, validates doc, frames id, writes JSON    |
|  [02]   | `DeserializeAsync(data, isNull, context)` -> `Task<T>`   | decode         | reads id, validates against writer schema, parses into `T` |
...
|  [06]   | `Validate` (`bool?`)                                   | both           | validate the JSON document against the schema            |
...
|  [10]   | `SchemaIdStrategy` (`SchemaIdSerializerStrategy?`)     | serializer     | `Prefix` (magic byte) vs. `Header` id framing            |
|  [11]   | `SchemaIdStrategy` (`SchemaIdDeserializerStrategy?`)   | deserializer   | `Prefix` vs. `Dual` id reading                           |
|  [12]   | `UseLatestWithMetadata` (`IDictionary<string,string>`) | both           | pin the schema version whose `Metadata` matches          |
