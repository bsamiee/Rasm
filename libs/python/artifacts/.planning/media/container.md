...
from rasm.runtime.faults import TRANSIENT, BoundaryFault, Catch, FaultRow, RuntimeResult, async_boundary, rostered
...
class Media(Struct, frozen=True):
    ...
    async def _emit(self) -> RuntimeResult[Produced]:
        held = await async_boundary(MEDIA_MUX, self._folded, catch=MEDIA_RESIDUE)
        settled = held.bind(lambda res: res.map_error(lambda fault: BoundaryFault(domain=(MEDIA_MUX.subject, fault))))
        match settled:
