...
public static class LiveWire {
    ...
    static ScheduleEntry PollEntry(LiveWireRuntime runtime, BindingSpec spec, TransportRow row, CancelScope scope) =>
        new($"live-wire-{spec.BindingId}", spec.Cadence, row.Attempt, None,
            () => TransportRows.Row(spec.Transport) is var transportRow
                ? IO.liftAsync(async () => unit)
                : IO.pure(unit));
}
```
...
- Entry: `Contribute(Seq<BindingHandle> bindings)` returns `HealthContributorRow` — projects the aggregate binding state into one health-contributor row tagged `remote` so a faulted critical binding degrades the host through the existing degradation rail; `Transition(BindingHandle handle, BindingState next, Instant at)` folds one state transition over the binding's atom.
...
- [TRANSPORT_CLIENTS]: the OPC-UA, Modbus, MQTT, and serial client surfaces the transport adapters compose ride admitted industrial-protocol packages catalogued at app-root creation; the read and write member shapes per transport confirm against the admitted client packages, and a missing protocol is one `ExternalTransport` row plus its admitted client, never a transport-page client reimplementation.
- [UNIT_COERCION]: the `QuantityFamily.Admit(double value, string unit, UnitPolicy policy, Guid correlation)` and `QuantityFamily.Render(double canonicalValue, UnitPolicy policy, Option<Enum> target)` overloads the edge coercion and write rendering read resolve against the finalized Compute units-boundary#QUANTITY_TABLE surface, so the binding's coercion is the suite's single unit truth and carries no unit math of its own.
