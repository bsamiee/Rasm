export const meta = {
  name: 'stack-ts',
  whenToUse: 'Ground-up rebuild of the docs/stacks/typescript code doctrine to the Effect-TS bar.',
  description: 'GROUND-UP, extreme-aggression rebuild of the docs/stacks/typescript code doctrine. The existing TS doc set is treated as JUNIOR-LEVEL TRASH: weak, loose, no cohesive/unified shape system; every page is reconsidered from zero and only what genuinely survives scrutiny is reconstituted. Keeps the SAME framing/standards/goal as the python doctrine rebuild (page-craft, adversarial stance, opinionated/zero-table-stakes, agnostic snippets, style-guide) but swaps the language doctrine to bleeding-edge TypeScript: Effect-TS rails, Schema-first boundaries (parse-not-validate), branded/nominal types, exhaustive discriminated unions, total dispatch with assertNever, ZERO any/throw/enum, and ONE canonical declaration form per value (NEVER a const + type + typeof triple for one concept). The PYTHON doctrine docs/stacks/python/ is the HIGHER read-only reference (the most rigorous dense stack; page-craft + ~450 soft cap + the shared density laws + extreme ADT/AOP/parameterization), and docs/stacks/csharp/ is the FLOOR; coding-ts is the route-owned TS law. Phases: Inventory (the current trash + salvage candidates) -> Reconsider (research-heavy adversarial judge panel deciding the settled file set BEFORE rebuild) -> Rebuild (1 agent/file, 3-step ADVERSARIAL rebuild(max) -> critique(xhigh) -> redteam(max, EXTRA-HARSH), CAP=12) -> Sweep (sequential atlas-order integration, no duplicated snippets) -> Reconcile (union-find cross-file residuals -> fix -> verify). Every edit is scoped to docs/stacks/typescript. Takes no args.',
  phases: [
    { title: 'Inventory', detail: 'list the current (trash) TS doc pages in atlas order; note any salvage-worthy fragment, and the region ledger seed' },
    { title: 'Reconsider', detail: 'research-heavy adversarial judge panel (treat existing as trash): 3 drafts -> synthesize -> redteam-gate decides the settled TS file set BEFORE rebuild' },
    { title: 'Rebuild', detail: 'per file (1 agent/file): rebuild(max, ground-up) -> critique(xhigh) -> redteam(max, EXTRA-HARSH), every stage ADVERSARIAL, pooled at CAP=12' },
    { title: 'Sweep', detail: 'sequential atlas-order pass: each file integrates finalized priors implicitly, unifies the shape system, no duplicated snippet demonstrations' },
...
const processPage = async (page, arch) => {
  ...
  for (const st of STAGES) {
    const r = await agent(st.build(page, arch), { label: st.key + ':' + nameOf(page), phase: 'Rebuild', schema: FIXLOG_SCHEMA, effort: st.effort, stallMs: STALL })
    if (r === null) break
    logs[st.key] = r
  }
...
log('Rebuild+Sweep done; reconcile ' + uniq.length + ' residuals -> ' + clusters.length + ' clusters; work [' + clusters.map(clusterWork).join(', ') + '] (2*files+claims)')
let reconciled = []
if (clusters.length) {
  phase('Reconcile')
...
return { workflow: 'stack-ts', root: ROOT, settled: arch, ordered: ordered, rebuilt: done.filter((r) => r.ok).length, incomplete: done.filter((r) => !r.ok).length, total: ordered.length, region_seed: ledger, clusters: clusters.length, hard_residual: hard_residual }
