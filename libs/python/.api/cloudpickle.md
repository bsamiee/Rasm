...
- `execution/workers`(`runtime/.planning/execution/workers.md`): `Kernel.of` calls `cloudpickle.dumps(fn)` to fill the `payload` bytes field the instant a callable's qualname carries `<lambda>` or `<locals>`, and the worker-floor `shipped` gate calls `cloudpickle.loads(kernel.payload)` far-side — the two direct call sites, keyed off the `Shipping` `REFERENCE|VALUE` axis where `REFERENCE` re-imports by qualified name and `VALUE` ships cloudpickle bytes, so a lambda crosses the process and subinterpreter seams by value instead of degrading to a thread.
...
- `loky`(`.api/loky.md`): cloudpickle is loky's default payload codec (`get_loky_pickler_name()` resolves `cloudpickle`), so the `PROCESS`/`COOPERATIVE` warm reusable pool ships `shipped` submissions carrying closures by value at zero per-call opt-in; loky owns the crash-respawning pool transport, cloudpickle the serialization inside it.
- `pebble`(`.api/pebble.md`): the `PROCESS`/`TERMINAL` pool and `InterpreterPoolExecutor` pickle through STDLIB pickle, not cloudpickle — the by-value kernel still crosses because `shipped` is a module-level importable and `kernel.payload` is already cloudpickle bytes, so `schedule(shipped, args=(kernel, *args))` and `.submit(shipped, kernel, *args)` both carry a cloudpickle payload through a stdlib pickler that never sees a raw closure.
- `anyio`(`.api/anyio.md`): `to_process.run_sync`/`to_interpreter.run_sync` are the `execution/lanes` `_ISOLATION` arms' pickle boundary — anyio owns the crossing mechanism, cloudpickle the picklability of the payload it carries, so an ordinary closure crosses only because `Kernel.of` pre-reduced it.
