...
[STACKING]:
- `ACadSharp`(`libs/csharp/.api/api-acadsharp.md`): the 2D complement — `ACadSharp` admits 2D profiles (polylines, arcs into `Loop`), this admits 3D solids (STEP/IGES into B-rep into mesh); a 3D STEP solid tessellates then planar-sections to 2D loops, a 2D DXF profile goes straight to `Loop`, neither duplicating the other.
