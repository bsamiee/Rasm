...
One cluster: `[2]-[HANGER_FAMILY]` owns the `HangerType` framing-connector discriminant, the `SteelGauge` sheet-steel gauge axis, the `HangerSection` cold-formed capacity receipt (carried-member size / allowable download-uplift-lateral / gauge and base thickness), and the `ConnectionCatalogue.BuildHangerRows` framing-connector row table.
