...
| :-----: | :------------- | :----------- | :--------------------- | :-------------------------- |
|  [01]   | `Vector`       | `vector`     | `float` / `float[]`    | full-precision dense vector |
|  [02]   | `HalfVector`   | `halfvec`    | `Half` / `Half[]`      | half-precision dense vector |
|  [03]   | `SparseVector` | `sparsevec`  | `float` indices+values | sparse vector               |

[VECTOR_MEMBERS]: `Pgvector.Vector`

| [INDEX] | [MEMBER]                          | [RETURN]                | [CAPABILITY]                    |
| :-----: | :-------------------------------- | :---------------------- | :------------------------------ |
|  [01]   | `Vector(ReadOnlyMemory<float> v)` | ctor                    | wraps existing memory           |
...
|  [05]   | `Equals(Vector?)`, `==`, `!=`     | `bool`                  | element-wise equality           |

[HALFVECTOR_MEMBERS]: `Pgvector.HalfVector`

| [INDEX] | [MEMBER]                             | [RETURN]               | [CAPABILITY]                    |
| :-----: | :----------------------------------- | :--------------------- | :------------------------------ |
|  [01]   | `HalfVector(ReadOnlyMemory<Half> v)` | ctor                   | wraps existing memory           |
...
|  [05]   | `Equals(HalfVector?)`, `==`, `!=`    | `bool`                 | element-wise equality           |

[SPARSEVECTOR_MEMBERS]: `Pgvector.SparseVector`

| [INDEX] | [MEMBER]                            | [SHAPE]                     | [RETURN]                | [CAPABILITY]                      |
| :-----: | :---------------------------------- | :-------------------------- | :---------------------- | :-------------------------------- |
|  [01]   | `SparseVector`                      | dimensions, indices, values | ctor                    | from pre-sorted sparse components |
|  [02]   | `SparseVector`                      | dense memory                | ctor                    | dense-to-sparse conversion        |
|  [03]   | `SparseVector`                      | dictionary plus dimensions  | ctor                    | from index-to-value dictionary    |
|  [04]   | `SparseVector`                      | wire string                 | ctor                    | parses `{i:v,...}/d` wire format  |
|  [05]   | `Dimensions`                        | property                    | `int`                   | total dimension count             |
|  [06]   | `Indices`                           | property                    | `ReadOnlyMemory<int>`   | non-zero index positions          |
|  [07]   | `Values`                            | property                    | `ReadOnlyMemory<float>` | non-zero element values           |
|  [08]   | `ToArray()`                         | no args                     | `float[]`               | materialises dense float array    |
|  [09]   | `Equals(SparseVector?)`, `==`, `!=` | comparison                  | `bool`                  | element-wise equality             |
...
| :-----: | :---------------------------------------- | :----------------- | :---------------------------------------- |
|  [01]   | `VectorDbContextOptionsBuilderExtensions` | builder extension  | admits plugin via `UseVector`             |
|  [02]   | `VectorDbContextOptionsExtension`         | options extension  | carries plugin policy                     |
|  [03]   | `VectorTypeMapping`                       | type mapping       | maps `Vector`/`HalfVector`/`SparseVector` |
|  [04]   | `VectorTypeMappingSourcePlugin`           | mapping plugin     | resolves `vector`/`halfvec`/`sparsevec`   |
|  [05]   | `VectorDataSourceConfigurationPlugin`     | data source plugin | enables vector wire type resolution       |
|  [06]   | `VectorDbFunctionsExtensions`             | function surface   | projects distance SQL operators           |
|  [07]   | `VectorDbFunctionsTranslatorPlugin`       | translator plugin  | translates distance calls to operators    |
|  [08]   | `VectorCodeGeneratorPlugin`               | scaffolding plugin | emits `UseVector` in scaffolded code      |
|  [09]   | `VectorDesignTimeServices`                | design services    | admits design tooling                     |
...
| :-----: | :---------------------------------------------- | :--------------- | :-------------------------------------------------------- |
|  [01]   | `VectorExtensions.UseVector(INpgsqlTypeMapper)` | extension method | registers `VectorTypeInfoResolverFactory` for wire decode |
...
| [INDEX] | [SURFACE]                                                      | [CALL_SHAPE]                              | [CAPABILITY]                    |
| :-----: | :------------------------------------------------------------- | :---------------------------------------- | :------------------------------ |
|  [01]   | `UseVector`                                                    | `NpgsqlDbContextOptionsBuilder` extension | admits EF plugin + wire mapping |
|  [02]   | `VectorTypeMapping.Default`                                    | `static VectorTypeMapping`                | maps `Vector` CLR to `vector`   |
|  [03]   | `VectorTypeMapping(string storeType, Type clrType, int? size)` | constructor                               | sizes vector columns            |
...
|  [02]   | `MaxInnerProduct` | `<#>`          | `vector`, `halfvec`              | negative inner product |
|  [03]   | `CosineDistance`  | `<=>`          | `vector`, `halfvec`, `sparsevec` | cosine distance        |
|  [04]   | `L1Distance`      | `<+>`          | `vector`, `halfvec`, `sparsevec` | taxicab distance       |
...
| [INDEX] | [SURFACE]             | [CALL_SHAPE]                                        | [CAPABILITY]                            |
| :-----: | :-------------------- | :-------------------------------------------------- | :-------------------------------------- |
|  [01]   | `HasMethod`           | `IndexBuilder.HasMethod(string? method)`            | selects `hnsw` or `ivfflat`             |
|  [02]   | `HasOperators`        | `IndexBuilder.HasOperators(params string[]?)`       | sets ops class e.g. `vector_cosine_ops` |
|  [03]   | `HasStorageParameter` | `IndexBuilder.HasStorageParameter(string, object?)` | sets `m`, `ef_construction`, `lists`    |
