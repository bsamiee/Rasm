...
public static class EgressPump {
    public static readonly Seq<StoreSlot> Slots = Seq(
        StoreSlot.Create("store.egress.drain"), StoreSlot.Create("store.egress.deadletter"), StoreSlot.Create("store.egress.replay"));
...
public static class Egress {
    ...
    // `Envelope` is the ONE rail: id = content key, source = the oplog URI, type = family.kind, partitionkey =
    // EntityKey RAW (the routing identity — redacting it collapses every partition to one value and destroys
    // per-entity order), sequence = the changefeed sequence. PAYLOAD bytes redact BEFORE the envelope leaves the
    // trust boundary, the `redacted` attribute records that masking fired, and the trace slot continues as traceparent.
