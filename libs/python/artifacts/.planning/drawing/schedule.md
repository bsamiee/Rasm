...
from artifacts.drawing.standard import Discipline, HatchMaterial, HatchSpec, LayerName, LineType, Standard
from artifacts.visualization.chart.spec import Palette, hex_ramp
from artifacts.visualization.table import FmtKind, TableFormat, TableOp, TablePlan, Theme
...
# the AEC schedule publication identity — a clean bordered grid, all-caps headers, centered title.
_AEC_THEME: Theme = Theme(style=1, color="gray", all_caps=True, header_align="center", outline=("solid", "1px", "#333333"))
...
class ScheduleTemplate(Struct, frozen=True):
    # the ordered AEC schedule layout — the title, the mark/stub key column, the ColumnSpec set, the SpannerSpec
    # groups, the default sort key, the optional data-colored severity column, and the standards note.
    title: str
    key: str
    ...
    totals: tuple[str, ...] = ()  # numeric columns the grand-summary SUMs beside the key count (the QTO takeoff totals)
    note: str = ""
...
def _schedule_frame(frame: pl.DataFrame, template: ScheduleTemplate) -> pl.DataFrame:
    # boundary shaping only — select the template columns the settled frame actually carries, in template order,
    # and sort by the key; the frame arrives authored over the data/tabular wire, never re-authored here.
    ...
    return shaped.sort(template.sort) if template.sort and template.sort in shaped.columns else shaped


def _schedule_ops(template: ScheduleTemplate, palette: Palette, present: frozenset[str]) -> tuple[TableOp, ...]:
    # DERIVE the whole TableOp sequence from the ONE template row, filtered to the columns the shaped frame
    # actually carries so a subset QTO frame never references an absent column in a Spanner/Fmt/Color/summary.
    ...
    # the grand-summary counts the key column AND SUMs each present `totals` column (the QTO takeoff totals),
    # so a quantity/cost schedule closes on a real total row rather than a bare item count — one derived op.
    sums = {column: pl.col(column).sum() for column in template.totals if column in present}
    summary = (TableOp.GrandSummary({template.key: pl.col(template.key).count(), **sums}, missing_text=""),) if template.key in present else ()
    noted = (TableOp.SourceNote(template.note),) if template.note else ()
    return (TableOp.Header(template.title), *stub, TableOp.Label(labels), *fmts, *aligns, *spanners, *colored, TableOp.SubMissing(text="—"), *summary, *noted)
...
def _hatch_swatch(spec: HatchSpec) -> Swatch:
    # the ISO 128-50 section indicator — a bordered box crosshatched at the material's real HatchSpec.angle; the
    # legend indicator of the characteristic angle, NOT the full ezdxf set_pattern_fill render the section owns.
    canvas = drawsvg.Drawing(28, 14, origin=(0, 0))
...
_TEMPLATE: frozendict[ScheduleKind, ScheduleTemplate] = frozendict({
    ...
    # the canonical Rasm.Bim QTO consumer: the takeoff frame arriving over data/tabular is styled to the
    # classification-grouped measure/cost layout and the grand-summary SUMs each measure + the cost via `totals`.
    ScheduleKind.QUANTITY: ScheduleTemplate(
        "QUANTITY TAKEOFF", "mark",
        (
            ColumnSpec("mark", "MARK", align="center"), ColumnSpec("description", "DESCRIPTION"),
            ColumnSpec("classification", "CODE", align="center"), ColumnSpec("material", "MATERIAL"),
            ...
            ColumnSpec("weight", "WEIGHT", FmtKind.NUMBER, "kg", "right", 1), ColumnSpec("unit", "UNIT", align="center"),
        ...
        sort="classification", totals=("count", "length", "area", "volume", "weight", "total_cost"),
        note="Quantities BIM-derived and approximate; verify before pricing. Rates exclude overheads and profit.",
    ),
})
