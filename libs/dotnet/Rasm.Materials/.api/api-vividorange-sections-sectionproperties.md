...
- `UnitsNet`(`libs/dotnet/.api/api-unitsnet.md`): every property is a `UnitsNet` quantity, so a computed section property and a measured material property are the same quantity type, and section-modulus `Volume` and inertia `AreaMomentOfInertia` carry their dimension into any downstream structural check.
