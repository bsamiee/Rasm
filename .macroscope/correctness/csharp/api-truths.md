...
- An optional parameter cannot default to a `[ValueObject]`/`[SmartEnum]` value or `default` (CS1736/TTRESG047); `Option<T> x = default` plus `IfNone` is the fix and the one optional-parameter form on every rail surface — never a smell, while the nullable-defaulting-null parameter is the defect.
...
- GH2 truths hold: `ObjectList.AllWires` includes dangling entries by design — filtering them is the consumer's job, not a library bug; `Canvas.ScheduleRedraw` with the before-background paint phase is the canonical motion tick, and `KeyedValues.Set` is typed-overload-only with no generic `Set<T>`.
