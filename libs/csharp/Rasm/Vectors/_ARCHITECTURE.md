...
1. `[implemented]` Matrix solve ownership is in `Matrix.cs`: dense LU/QR/Cholesky, sparse BiCGStab with MathNet fallback, CSparse Cholesky, dense/sparse/generalized eigen paths, and QR least-squares receipts exist. Dense, sparse, and CSparse solve paths share finite RHS/output admission through the existing solve rails; eigen receipts now reject nonfinite known payload shapes, sparse generalized eigen validates both matrix inputs, and LOBPCG can materialize finite max-iteration exhaustion receipts. Remaining matrix work is broader sparse/Hermitian result hardening and ensuring consumers treat only `ResidualConverged` as usable eigenvectors.
...
3. `[implemented]` Cloud, alignment, and transport diagnostics are real: `CloudCorrespondenceSet` feeds ICP and Sinkhorn, `SinkhornReceipt` reports computed residual, tolerance, cutoff, and retained coupling facts, weighted mass reaches metrics/alignment/transport, convex hulls route through `Mesh.CreateConvexHull3D`, footprint hulls route through `PolylineCurve.CreateConvexHull2d`, `CloudKernel.MassOf` is the mass admission rail for cloud/sample/alignment/transport consumers, confidence remains empty until a kernel computes it, and true GICP remains explicit unsupported instead of approximated.
...
- `[partial]` `Cloud.cs`: rings, polylines, clusters, weighted clusters, PCA/covariance, oriented normals, proof-backed local curvature receipts, convex/footprint hull receipts, winding, Sinkhorn, and correspondence summaries are wired. Mass validation routes through `CloudKernel.MassOf`; kNN and radius neighborhoods resolve native point-cloud indices through the owning `PointCloud.PointAt(index)`; RhinoWIP bridge proof covers native hull facets, native neighborhood lookup, and plane/sphere/saddle curvature ranges; and concave/alpha hulls plus correspondence confidence stay explicit unsupported/empty until real kernels land.
...
- `[implemented]` `Cloud.cs`: `SinkhornReceipt` reports coupling summaries, source/target residual semantics, numeric status, convergence tolerance, positive coupling cutoff, debiased self-costs, and retained-correspondence summaries from computed facts. Transport internals consume normalized mass facts admitted by `CloudKernel.MassOf`.
