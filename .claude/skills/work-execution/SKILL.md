...
- Forks hold the dispatch tool under a default that tells them to execute directly, and a brief that needs a fork to dispatch says so in plain words
- Every brief opens with the edit rule and pastes in the standards the work meets: placement, language rules, checkers at zero warnings
- Spawn `prose-editor` with the Agent tool over every file touched since the last checkpoint
...
- The main agent rewrites its memory of the plan's intent and execution shape at the close from the run
