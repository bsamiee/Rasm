...
- Entry:
  - `CausalFrame.decode(carrier)` returns `RuntimeRail[CausalFrame]`: it reads the `physical`/`logical`/`tenant` header names off `SLOTS` out of the `dict[str, str]` gRPC metadata and lifts the slot strings through `msgspec.convert(mapping, CausalFrame, strict=False)` inside the one `reliability/faults#FAULT` `boundary("wire", ...)` fence. `strict=False` coerces the numeric-string halves to `int` AND runs each half's `Meta` bound in the C core — `physical_ticks` the `I63` `Meta(ge=0, lt=2**63)` producer mint domain (the C# physical half is a non-negative NodaTime Int64, so a value at or past `2**63` is unmintable by the sole producer AND would overflow the OTLP signed-int64 `halves` export the projection emits as native `int`), `logical` the `U64` `Meta(ge=0, lt=2**64)` `ulong` wire domain — so a non-numeric, negative, or out-of-domain half is a typed `msgspec.ValidationError` landing as `Error(BoundaryFault(boundary=("wire", "ValidationError")))`, never the `Hlc(...)` `__init__` path that runs no `Meta` check and would silently admit a half the `packed` shift truncates or the `halves` export corrupts. A genuinely absent slot is the `.get(...)` `"0"`/`"default"` floor — the only branch the default owns, distinct from a present-but-malformed value the bound rejects. The decode and the domain check are one railed surface, not an inline `int()` plus a hand `try` per servicer.
...
type I63 = Annotated[int, Meta(ge=0, lt=2**63)]  # the C# physical mint domain: non-negative NodaTime Int64 ticks — also the OTLP signed-int64 ceiling
...
class CausalFrame(Struct, frozen=True):
    ...
    def decode(cls, carrier: dict[str, str]) -> RuntimeRail[Self]:
        ...
        # I63/U64 half domains are enforced here; a direct `__init__` would admit a half the
        # `packed` shift truncates. An absent slot is the `.get(...)` floor; a present-but-malformed
        # or out-of-domain half is a `ValidationError`.
        return boundary(
            "wire",
            lambda: convert(
                {
