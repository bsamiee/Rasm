...
- [03]-[MESH]: `Meshes` mesh inspection — the banded `MeshSampleGroup` census over its `CensusSource` capture column, `MeshMetric` visible-polygon measurement over the `Spatial/cloud` ring, and the `MeshSample`/`MeshMetricSample`/`MeshFaceShape` evidence carriers.
