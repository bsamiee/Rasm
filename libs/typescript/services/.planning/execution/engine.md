...
- Boundary: the successor `@effect/cluster` plus standalone `@effect/workflow` split owns this concern, never the merged predecessor; `DurableUnit` is the live dispatch vocabulary `durableKernel` projects, never a dead type; the SQL boundary is reached only through `ClusterEngine` over `persistence/store#STORE_BOUNDARY`, never a second client; this page imports `interchange`, `projection`, and the persistence owner, never the browser domain.
