...
- Auto: `Optimize` dispatches the search by the `OptimizerKind` row — the population-based rows (`nsga2`/`cma-es`/`pso`/`simulated-annealing`/`robust-minimax`) drive the `GeneticEngine` `GeneticSharp.GeneticAlgorithm` over a `DesignVariable`-mapped `FloatingPointChromosome` genome, a `FuncFitness` bound to the `evaluate` oracle, and a row-selected operator set (the `nsga2` row a `(rank, crowding)`-style binary `TournamentSelection`, the `cma-es` row a `UniformCrossover`, the `simulated-annealing` row a `FlipBitMutation`), evaluated in parallel through a deadline-bounded `ParallelTaskExecutor` on the bounded compute lanes — the GA engine, operator catalog, and executor are the admitted package's while the fast-non-dominated dominance/crowding comparator and the `ParetoFront` accumulation stay the optimizer page's; Bayesian-GP fits a Gaussian-process surrogate over the evaluated points and proposes the next point by the `AcquisitionFunction` row (expected-improvement over the GP posterior mean/variance) ranking the `GeneticEngine.Candidates` genome-decoded population, gradient-adjoint drives a steepest-descent/L-BFGS step from the `SolveLane` adjoint sensitivity under the `StepControl` row (Armijo backtrack or trust-region radius update), and topology-SIMP updates the density field by the optimality-criteria bisection under a volume constraint; the constraint handling is the `ConstraintHandling` row (the feasibility-rules dominance comparator, the static/death penalty addition, or the augmented-Lagrangian multiplier update); the surrogate duality is a policy column — when `Surrogate` is supplied the search evaluates the cheap ROM/GP and the error-bound receipt gates whether a candidate re-evaluates on the full solve; the Pareto front accumulates every non-dominated point with its crowding distance so the result is a queryable artifact, not a single optimum.
...
public sealed record OptimizerPolicy(
    ...
    public static readonly OptimizerPolicy CanonicalNsga = new(
        ...
        IntegerStep: 1.0, SolveSeconds: 30.0, MultiStartInner: OptimizerKind.CmaEs, Restarts: 8, Scenarios: Seq<double[]>());
...
public static class Optimizer {
    static readonly FrozenDictionary<string, Func<DesignProblem, OptimizerPolicy, Func<DesignPoint, Fin<Seq<double>>>, ParetoFront, Fin<ParetoFront>>> Steps =
        new Dictionary<string, Func<DesignProblem, OptimizerPolicy, Func<DesignPoint, Fin<Seq<double>>>, ParetoFront, Fin<ParetoFront>>>(StringComparer.Ordinal) {
            [OptimizerKind.Nsga2.Key] = static (problem, policy, evaluate, front) => GeneticEngine.Evolve(problem, policy, evaluate, front),
            ...
            [OptimizerKind.SimulatedAnnealing.Key] = static (problem, policy, evaluate, front) => GeneticEngine.Evolve(problem, policy, evaluate, front),
            [OptimizerKind.CmaEs.Key] = static (problem, policy, evaluate, front) => GeneticEngine.Evolve(problem, policy, evaluate, front),
            [OptimizerKind.Pso.Key] = static (problem, policy, evaluate, front) => GeneticEngine.Evolve(problem, policy, evaluate, front),
            [OptimizerKind.CpSat.Key] = static (problem, policy, evaluate, front) => SolveCpSat(problem, policy, evaluate, front),
            [OptimizerKind.Milp.Key] = static (problem, policy, evaluate, front) => SolveMilp(problem, policy, evaluate, front),
            [OptimizerKind.MultiStartGlobal.Key] = static (problem, policy, evaluate, front) => MultiStart(problem, policy, evaluate, front),
            [OptimizerKind.RobustMinimax.Key] = static (problem, policy, evaluate, front) => RobustMinimax(problem, policy, evaluate, front),
    ...
    static Fin<ParetoFront> AcquireNext(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) {
        ...
        var candidates = GeneticEngine.Candidates(problem, front, policy.Population, 0x9E3779B9 ^ front.Points.Count);
    ...
    // CP-SAT: lower the typed Integer/Categorical/Continuous(discretized)/Linked DesignVariable and the
    // ConstraintHandling rows to an OR-Tools CpModel through the typed model-builder API (never a string-
    // parsed model), solve to a classified CpSolverStatus, and harvest the assignment into the ParetoFront —
    // the already-declared Integer/Categorical cases finally get an exact solver instead of metaheuristic
    // rounding. CP-SAT models integer/boolean natively and maps continuous only through its linear backend,
    // so the row declares it solves Integer/Categorical exactly and discretizes Continuous.
    static Fin<ParetoFront> SolveCpSat(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) {
        using var model = new CpModel();
        var vars = problem.Variables.Map(v => v switch {
            DesignVariable.Integer i => model.NewIntVar(i.Lower, i.Upper, i.Name),
            DesignVariable.Categorical c => model.NewIntVar(0, c.Choices.Count - 1, c.Name),
            DesignVariable.Continuous co => model.NewIntVar((long)(co.Lower / policy.IntegerStep), (long)(co.Upper / policy.IntegerStep), co.Name),
            _ => model.NewConstant(0),
        }).ToArray();
        return HarvestExact(problem, policy, evaluate, front, _ =>
            new CpSolver { StringParameters = $"max_time_in_seconds:{policy.SolveSeconds}" } is var solver
            && solver.Solve(model) is CpSolverStatus.Optimal or CpSolverStatus.Feasible
                ? Some(problem.Variables.Map((v, axis) => DiscreteValue(v, solver.Value(vars[axis]), policy.IntegerStep)).ToImmutableArray())
                : None);
    }

    // MILP: build the OR-Tools LinearSolver (SCIP for mixed-integer, GLOP for the continuous relaxation),
    // declare the typed variables and bounded linear constraints, and solve to a Solver.ResultStatus —
    // a mixed continuous+integer problem routes the integer part exactly and the continuous part through
    // the linear backend rather than discretizing.
    static Fin<ParetoFront> SolveMilp(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) {
        using var solver = Google.OrTools.LinearSolver.Solver.CreateSolver("SCIP");
        if (solver is null) { return Fin.Fail<ParetoFront>(ComputeFault.Create("<milp-backend-unavailable:SCIP>")); }
        var vars = problem.Variables.Map(v => v switch {
            ...
            _ => solver.MakeNumVar(0, 0, v.VariableName),
        }).ToArray();
        return HarvestExact(problem, policy, evaluate, front, _ =>
            solver.Solve() is Google.OrTools.LinearSolver.Solver.ResultStatus.OPTIMAL or Google.OrTools.LinearSolver.Solver.ResultStatus.FEASIBLE
                ? Some(problem.Variables.Map((v, axis) => v.Clamp(vars[axis].SolutionValue())).ToImmutableArray())
                : None);
    }

    // Multi-start global: wrap any local row with a LowDiscrepancy-seeded basin restart so the continuous
    // rows escape local minima; each restart runs the inner row from a fresh space-filling start and the
    // best basin enters the one ParetoFront.
    static Fin<ParetoFront> MultiStart(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) {
        OptimizerKind inner = policy.MultiStartInner;
        return toSeq(Enumerable.Range(0, Math.Max(1, policy.Restarts)))
            .Fold(Fin.Succ(front), (acc, restart) => acc.Bind(current => {
                double[] start = Uniform(policy.Generations * 101 + restart, problem.Variables.Count);
                var seeded = current.Insert(new DesignPoint([.. problem.Variables.Map((v, axis) => v.Clamp(v.Lower + start[axis] * v.Width))], [], []));
                return Steps[inner.Key](problem, policy with { Kind = inner }, evaluate, seeded);
            }));
    }

    // Robust min-max: the worst-case-over-uncertainty objective transform reading the FILL_UNCERTAINTY_UQ
    // RandomVariable scenario set — each candidate is scored over the UQ scenario set and the objective is
    // the worst-case value, so robust optimization and forward UQ share inputs. The inner reliability rides
    // the cheap Surrogate.Predict where the error bound holds, never a full inner MC per outer candidate.
    static Fin<ParetoFront> RobustMinimax(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) =>
        GeneticEngine.Evolve(problem, policy, point => policy.Scenarios.Fold(Fin.Succ(Seq<double>()), (acc, scenario) =>
            acc.Bind(worst => evaluate(new DesignPoint([.. point.Coordinates.Select((c, axis) => c + (axis < scenario.Length ? scenario[axis] : 0.0))], [], []))
                .Map(values => worst.IsEmpty ? values : worst.Zip(values).Map(static p => Math.Max(p.Item1, p.Item2))))), front);

    static Fin<ParetoFront> HarvestExact(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front, Func<Unit, Option<ImmutableArray<double>>> solve) =>
        solve(unit).Match(
            Some: coords => evaluate(new DesignPoint(problem.Resolve(coords), [], [])).Map(objectives => front.Insert(new DesignPoint(problem.Resolve(coords), [.. objectives], []))),
            None: () => Fin.Fail<ParetoFront>(ComputeFault.Create($"<exact-infeasible:{policy.Kind.Key}>")));
...
// The metaheuristic proposal kernel: GeneticSharp.GeneticAlgorithm drives every population-based
// OptimizerKind row (Nsga2/CmaEs/Pso/SimulatedAnnealing/RobustMinimax) over a DesignVariable-mapped genome,
// a FuncFitness bound to the one evaluate oracle, a row-selected operator set, and a ParallelTaskExecutor on
// the bounded compute lanes — the GA machinery is the admitted package's, never hand-rolled tournament /
// recombination / swarm math. The dominance/crowding comparator and the ParetoFront stay the optimizer
// page's: GeneticSharp supplies the population/operator/executor engine, this fold harvests its generation
// into the existing front. RandomizationProvider is seeded from the policy so a run is replayable.
public static class GeneticEngine {
    public static Fin<ParetoFront> Evolve(DesignProblem problem, OptimizerPolicy policy, Func<DesignPoint, Fin<Seq<double>>> evaluate, ParetoFront front) {
        ...
        // ParallelTaskExecutor evaluates FuncFitness across threads, so the front accumulates through an
        // Atom CAS Swap — a captured `Fin<ParetoFront>` read-modify-write is a data race the Atom replaces.
        var harvest = Atom(front);
        var fitness = new FuncFitness(chromosome =>
            evaluate(Decode(problem, chromosome)).Match(
                Succ: objectives => { harvest.Swap(f => f.Insert(Decode(problem, chromosome) with { Objectives = [.. objectives] })); return -objectives.Sum(); },
                Fail: _ => double.MinValue));
        var algorithm = new GeneticAlgorithm(population, fitness, Selection(policy), Crossover(policy), Mutation(policy)) {
        ...
        algorithm.Start();
        return Fin.Succ(harvest.Value);
    }

    // The genome-decoded space-filling population the Bayesian acquisition ranks: each draw clones the
    // adam-prototype through the same GeneticSharp encoding so the acquisition and the GA search share one
    // genome contract, never a parallel sampler.
    public static Seq<ImmutableArray<double>> Candidates(DesignProblem problem, ParetoFront front, int count, int seed) {
        RandomizationProvider.Current = new FastRandomRandomization();
        IChromosome adam = Genome(problem);
        return toSeq(Enumerable.Range(0, Math.Max(1, count))).Map(_ => Decode(problem, adam.CreateNew()).Coordinates);
    }
    ...
    static DesignPoint Decode(DesignProblem problem, IChromosome chromosome) {
        ...
        return new DesignPoint(problem.Resolve([.. full]), [], []);
    }
    ...
    static ICrossover Crossover(OptimizerPolicy policy) =>
        policy.Kind == OptimizerKind.CmaEs ? new UniformCrossover(0.5f) : new TwoPointCrossover();

    static IMutation Mutation(OptimizerPolicy policy) =>
        policy.Kind == OptimizerKind.SimulatedAnnealing ? new FlipBitMutation() : new UniformMutation(true);
}
