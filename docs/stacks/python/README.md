...
The doctrine's compiled form is the lint, type, and contract gate: `Ruff` (`select = ["ALL"]`, preview), strict `ty`, strict `mypy`, and `beartype` boundary contracts are the enforcement surfaces, and the manifest owns every rule pin and message. The loop is one-directional — a doctrine page legislates, a gate enforces; a rule never introduces law of its own.
