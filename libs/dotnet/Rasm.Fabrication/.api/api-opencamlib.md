...
- Shim functions mirror the lifecycle: `ocl_op_create(kind)` → `ocl_op_set_surface`/`ocl_op_set_cutter` → `ocl_op_set_sampling`/`ocl_op_set_threads` → `ocl_op_run` → `ocl_op_get_clpoints(double* out, int* n)` → `ocl_op_destroy`, with one ctor per `CutterForm` row (`ocl_cutter_cyl`/`ocl_cutter_ball`/`ocl_cutter_bull`/`ocl_cutter_cone` and composites).
