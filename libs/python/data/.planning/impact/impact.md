...
# epdx spells materials-for-recycling `mrf`, openepd spells exported energy `ee`/`eh`.
EPDX_FIELD: Final[Map[Indicator, str]] = Map.of_seq([(ind, {"mfr": "mrf"}.get(ind.value, ind.value)) for ind in Indicator])
OPENEPD_FIELD: Final[Map[Indicator, str]] = Map.of_seq([(ind, {"eee": "ee", "eet": "eh"}.get(ind.value, ind.value)) for ind in Indicator])
...
def _from_score(solve: LcaSolve, source: str, identity: bytes) -> MaterialImpact:
    ...
    if solve.iterations > 0:
        ...
        samples = tuple(lca.score for _ in zip(range(solve.iterations), lca, strict=False))
