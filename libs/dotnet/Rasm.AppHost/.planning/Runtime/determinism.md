...
- Packages: Rasm.Contracts (project — generated `Benchmark.HostFingerprintWire` and `LabelPair`), Google.Protobuf, Rasm (kernel `CanonicalWriter`/`ContentHash` identity capsule, `Deterministic` with `IDrawLane<TSelf>`), Generator.Equals, Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox
...
using Rasm.Domain;
using Thinktecture;
using Host = Rasm.Contracts.Benchmark;
