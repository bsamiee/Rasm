...
- `usd-core`(`.api/usd-core.md`): the resident build ships the `vtkIOExport` scene exporters (`vtkGLTFExporter`/`vtkOBJExporter`/`vtkVRMLExporter`/`vtkX3DExporter`) but no `vtkmodules.vtkIOUSD` — VTK USD I/O needs that module enabled against OpenUSD, so USD/USDZ authoring belongs to `usd-core` (`pxr`) across the numpy buffer boundary.
