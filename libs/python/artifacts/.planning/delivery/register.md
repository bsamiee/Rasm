...
class Register(Struct, frozen=True):
    ...
    def admit(
        ...
        *payloads: ContainerPayload,
        ...
        # `parents` = the caller's typed upstream sheet keys (DATA edges) — the only lineage ingress, because
        # the raw `asset_key` string band is boundary evidence and never re-parses into a `ContentKey`.
    ...
    def of_sheets(
        ...
        *,
        ...
        # admits documented codes from the merged client register. `parents` threads the sheet content keys
        # the aggregating caller already holds, one per entry, so `emit()` carries real DATA edges.
    ...
    def emit(self, /) -> ArtifactWork:
        # parents = the constituent sheet/container content keys where the caller threads them (DATA edges) —
        # the raw `asset_key` string band stays boundary evidence; a re-issued set re-renders only changed members.
    ...
    def _key(self) -> ContentKey:
        # key-over-INPUT: canonical (op ⊕ containers ⊕ meta) minted PRE-RUN — never a key over the rendered index bytes.
    ...
    async def _rendered(self) -> ArtifactReceipt:
        # the _composed render crosses the runtime thread lane so the GIL-releasing great-tables/lxml/xlsxwriter
        # render never blocks the loop; the terminal receipt threads the PRE-RUN key (receipt.slot == node.key).
