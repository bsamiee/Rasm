...
[STACKING]:
- `LanguageExt.Core`(`.api/api-languageext.md`): the `itemDropped` delegate and a rejected `TryWrite` fold into one `Atom<A>.Swap` loss counter, and a `ReadAllAsync` drain body lands on `Fin<A>`/`Eff<A>` so a consumer fault is a typed row.
...
- `Rasm.AppUi` `Collab/presence` (bounded lane per `TransportLane` row — `Wait` on the collab topic, `DropOldest` on presence — with the `itemDropped` observer parking every eviction as a counted fault), `Document/media` (raised-command lane, `DropOldest`+`SingleReader`), `Document/notebook` (recompute lane), `Render/immersive` (`XrObservation` fan-out off the compositor thread), `Render/capture` and `Charts/basemap` (pyramid + health lanes): every lane is `CreateBounded`, drained through `ReadAllAsync`, with the Rx-vs-channel discriminant stated at each boundary.
