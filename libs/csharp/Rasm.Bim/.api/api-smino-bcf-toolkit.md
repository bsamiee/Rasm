...
- COORDINATION owner: `Coordination.Propose`/`Run` mints a domain `BcfTopic` per `Model/systems#INTERFERENCE` clash and `BcfArchive.Write` carries the topic set out through `BcfBuilder.AddMarkup(...)` → `Build()` → `Worker.ToBcf(bcf, Bcf30)`, so the toolkit builder is reached at the codec alone and never inside the clash fold; the BCF `Extensions` status/type/priority vocabularies feed the `SignOff` `[SmartEnum]` state machine and the `IssueBoard` priority rank; the topic GUID joins a `BcfTopic` to its `ElementSet` through `IComponent` IFC GUIDs on the shared `Model/query#ELEMENT_SET` `ElementPredicate` algebra at read time; any `Worker` throw lowers BARE onto `Fin<T>` as `Model/faults#FAULT_BAND` `BimFault.ModelRejected` at the `Review/issues#BCF_ARCHIVE` codec boundary — the branch's one `.bcfzip` custodian.
