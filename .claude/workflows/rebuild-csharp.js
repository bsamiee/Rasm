...
const LAW = [
  'Rasm monorepo, libs/csharp planning corpus (markdown specs of intended C# package designs). CLAUDE.md manifest + WORKSPACE_LAW strata govern (KERNEL -> AEC-DOMAIN -> APP-PLATFORM -> HOST-BOUNDARY -> APP; depend strictly upward; a host-neutral owner only where a non-Rhino runtime consumes the contract).',
...
const ULTRA = [
  ...
  'STACK CAPABILITY: compose the host APIs and admitted NuGet packages into single dense owners woven as ONE rail (source-generated owners, fold algebra, data tables), NOT flat one-shot per-API uses. Capture host/platform surfaces into focused local owners; verify members with `uv run python -m tools.assay api` (host DLLs, NuGet). Use the MOST powerful members; reject surface-level subsets.',
