...
- `CloudNative.CloudEvents.SystemTextJson`(`libs/dotnet/.api/api-cloudevents.md`): its `JsonEventFormatter` frames the CloudEvents message envelope on `Message<TKey,TValue>` headers/body while this serde codes the payload body `Data`; `SchemaIdStrategy = Header` writes the id to `SchemaId.VALUE_SCHEMA_ID_HEADER` (`__value_schema_id`) disjoint from the `ce_`-prefixed message-envelope headers, and `SchemaIdDeserializerStrategy.Dual` reads a `Header`- or `Prefix`-framed id so both producers interoperate.
