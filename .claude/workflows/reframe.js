...
const fixerPrompt = (langs, rows, backlog, orphans, folders, round) =>
    [
        ...
        'TASK: TERMINAL DRAIN (WRITER — you are the run`s LAST agent, nothing follows you; full write authority over the ' +
            ...
            'seamsTouched rows were never folded forward — read each IN FULL from disk and drain those rows under the same law): ' +
...
const doctrinePrompt = (rows) =>
    'TASK: DOCTRINE LANDER — the durable-learning terminal of a hostile reframe run. Load the `docgen` skill AND the ' +
    ...
    'admission evidence — recurrence across independent units, or a structural singleton whose blast radius spans ' +
    ...
    'ROUTING: stacks -> the owning `docs/stacks/<lang>/` page, refactored in place, never append-beside; documentation ' +
    'craft -> `docs/standards/` at the owning page; reviewer -> `.greptile/rules.md` + `.coderabbit.yaml` (machine-law ' +
    'boundary: a rule a formatter, analyzer, or gate can enforce routes to that gate owner and is REJECTED here with that ' +
    'reason, never landed as reviewer prose); constitution -> `CLAUDE.md` or `AGENTS.md` at the reader that ACTS on the ' +
    'fact, cross-referenced never duplicated; planning -> the owning `libs/.planning/` or `libs/<lang>/.planning/` index ' +
    'doc or system page; laws -> `docs/laws/` per its admission table; readme -> the named README.\n' +
...
const doctrine = HARVEST_ROWS.length
    ? await slot(() =>
          agent(doctrinePrompt(HARVEST_ROWS), {
              label: 'doctrine',
              phase: 'Close',
              model: 'fable',
              effort: 'high',
