...
async def _via_document(pipeline: "TemplatePipeline", target: TemplateTarget, /) -> RuntimeRail[Keyed]:
    ...
    # the fused `DELEGATES` row resolves the `DocumentMode` AND its `EmitSpec` builder together — one row,
    # so the mode and the spec can never drift the way two parallel target-keyed maps would.
