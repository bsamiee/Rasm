...
|  [06]   | `DescMessage` / `DescEnum` / `DescField` / `DescOneof`  | descriptor union  | the reflected schema graph the drift gate walks       |
...
- [VALID_TYPES]: `valid_types` takes `legacy_required` and `protovalidate_required`, `+`-combined; upstream ships the option on its unstable track, so a tree generated under it re-verifies on every plugin bump.
...
- [BRANCH_OPTIONS]: three options answer the root `tsconfig.json`. `target=ts` binds today's emission, since the default publishes a `.js`+`.d.ts` pair to a branch whose gate is `tsc --noEmit` over sources. `erasable_syntax=true` and `import_extension=ts` bind no byte of the current corpus — no `rasm.<family>.v1` source declares an enum or imports a peer source — and hold the emission the day one does: `erasableSyntaxOnly` makes a generated TS `enum` a compile error, and `allowImportingTsExtensions` is the spelling every branch module already uses.
...
- `@bufbuild/protoc-gen-es`(same `catalog`): emits the `_pb.ts` `GenMessage` consts from the corpus-homed `.proto`; the generated file is build output the `codec/*` pages import, and root `buf.yaml` holds lint, breaking, and formatting while the sibling root `buf.gen.yaml` holds every generation row, this branch's among them. That row is the template's `protoc-gen-es` entry under `nx run workspace-foundation:proto`, which runs `buf generate` from the repo root, discovers the template there, and caches the `out` tree as the target's `outputs` so `typecheck` reads a regenerated tree. Its input is the corpus module as `directory: tests/contracts` and its Render is one `_pb.ts` per SOURCE at the MODULE-relative path — `rasm/<family>/v1/<family>.proto` landing at `core/src/interchange/gen/rasm/<family>/v1/<family>_pb.ts`, since the plugin bundles nothing and no barrel stands between. `clean: true` sweeps from that tree every file a run did not write, so a retired family strands no orphan module and the `out` path holds generated output alone. The option set and what each one answers seat at `[02]-[BRANCH_OPTIONS]`. `interchange/format`'s `Proto.registry` admits that estate generation alone. The VENDORED publisher source at `tests/contracts/io/cloudevents/v1/cloudevents.proto` has NO producer row: root `buf.yaml` excludes `tests/contracts/io` from buf's one module, so a `proto_file` or `paths` entry naming it targets zero files while the excluded directory itself refuses as contained by the module — no input form reaches it. Its `CloudEvent` message would in any case collide with the message-envelope class every consumer imports, so that generation emits BESIDE the estate tree and never into it, and `interchange/format`'s protobuf event row takes the descriptor through its `Core` seat rather than through this registry; `tests/contracts/.api/bufbuild-buf.md` `[04]-[TOPOLOGY]` owns what closes the gap.
