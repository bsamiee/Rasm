...
Plugins form one global registry per user with per-scope enablement flags: marketplace registration state lives in `~/.claude/plugins/known_marketplaces.json` regardless of which scope registered it, and a marketplace name is unique per user — a second registration under an existing name silently replaces the first. Registration and enablement are decoupled surfaces: a project may enable a plugin whose marketplace is known only at user scope.
...
- Version resolves first-match: `version` in the plugin's `plugin.json`, `version` in the marketplace entry, the git commit SHA of the plugin source, else `unknown`. Resolved version is the cache key — an unchanged version skips the update, so editing master files without moving the version ships nothing.
...
- LSP manager reads configs once at session start and spawns a server on the first matching file; servers live for the process, and each process subtree spawns its own instances — heavy servers multiply under agent fleets.
...
- `settings` and `workspaceFolder` fields never reach the server; per-server configuration travels through `args`, `initializationOptions`, and `env`.
- `LSP` tool is unreachable from subagents; LSP-first navigation belongs to the main loop. A first LSP call in a fresh session can race server startup — retry, never conclude absence.
