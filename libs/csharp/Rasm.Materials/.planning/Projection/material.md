...
public sealed class MaterialProjector : IElementProjection {
    ...
    // The neutral seam SectionProperties lifted from the Profiles ComputedSection (PositiveMagnitude → SI double),
    // so the seam graph carries the baked scalars and Rasm.Compute reads them without admitting VividOrange.
    static SectionProperties SeamSection(ComputedSection c) =>
        new(c.AreaMm2.Value, c.IxMm4.Value, c.IyMm4.Value, c.SxMm3.Value, c.SyMm3.Value, c.RxMm.Value, c.RyMm.Value);
