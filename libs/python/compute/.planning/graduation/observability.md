...
- Growth: a new module leaf is one `SCOPE_DOMAIN` row; a new domain is one `Domain` literal member whose pair derives; a new measure is one instrument-name constant, one `_measures` arm, and one runtime `INSTRUMENTS` row; a new admission fact is one point row and one fire arm on the owning rail.
