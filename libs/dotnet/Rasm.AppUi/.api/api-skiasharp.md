...
- `SKRuntimeEffectUniforms.Add`/`SKRuntimeEffectChildren.Add` THROW `ArgumentOutOfRangeException` on a name the program never declared, and the uniform overload validates the value's own `SKRuntimeEffectUniform.DataType` against the declaration's type, count, and array flag and throws on a mismatch — the implicit conversions widen the CALL, never the check, so a typed rail resolves the name and the declared shape ahead of the write rather than trapping the throw.
...
- Capture rail: `SKSurface.Create(SKImageInfo)` or a GPU surface from `GRRecordingContext` draws, `Snapshot()` returns an `SKImage`, and `Encode(SKEncodedImageFormat.Png, ...)` returns the diffable `SKData` buffer; `SKColorSpace.CreateSrgb`/`CreateIcc` + `SKImageInfo.WithColorSpace` make it color-managed, and animated evidence decodes through `SKCodec.FrameCount`/`GetFrameInfo` per frame.
