...
[RANK_AND_TOLERANCE]:
- Law: every threshold is one of two scale-derivation forms closed in the `Scale` `[SmartEnum]` the `[01]` route column carries — `OperatorRhs` is `ε·‖A‖_F·max(‖b‖∞, 1)` for a square solve, `SingularDim` is `ε·σ_max·max(m, n)` for the least-squares rank floor reading `σ_max` off the held `Svd` — each a `[UseDelegateFromConstructor]` `Derive(Matrix<double>, Vector<double>) -> double` column, so the derivation is the route's own policy axis projected once at the witness terminus and travelled as the gated tolerance on the result, never a per-route table whose square and definite rows duplicate one body.
- Reject: a bare per-module absolute literal in `1e-4..1e-8`; it is unreplayable and uncomparable across operators, and a fresh literal per call site is the same defect spelled inline; a `double.Epsilon` band is exact equality wearing a tolerance — below every representable rounding error, it admits only bit-identical values — so intended identity spells the exact-equality form on site.
- Law: a threshold or tolerance derivation never pays the complexity class of the operation it gates — a policy record whose constructor hides the gated kernel's own decomposition is a second solve the result never shows, so derivations read cheap norms and refine only off an already-held handle.
...
- Law: record `Symmetricity` as the case supplied (the caller's verified precondition), not the library's inferred result; the result `IsSymmetric` bool merges `Symmetric` and `Hermitian` and loses the distinction.
- Reject: a `QRMethod.Thin` result where full-column-space reconstruction is required.
- Reject: a `computeVectors: false` result read for vectors; it leaves `U`/`VT` as identity-shaped stubs.
...
[ONLINE_STATISTICS]:
- Law: the accumulator is `RunningStatistics` for the whole stream and `MovingStatistics` for a fixed window — distinct semantics recorded as the result's accumulator fact, never one knob, since the moving form re-weights as samples leave the window where the running form never forgets.
...
- Boundary: count the skipped evaluation in the result, never silently as coverage.
...
- Law: freeze adaptive control as one policy row — safety factor, step-ratio clamps, error-norm choice, and reject budget travel together — and read the next step off the result.
