...
First, retry the command outside the sandbox. In Codex, request escalation for `heptabase` commands where the tool permits it.

If it still fails, ask the user to confirm the desktop app has CLI enabled at `Settings > AI Features`.

For a persistent `workspace-write` setup, ask the user to add this to `~/.codex/config.toml`:
