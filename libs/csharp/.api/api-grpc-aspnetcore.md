...
`MapGrpcHealthChecksService`). The member in-process test surface Compute consumes is
...
- in-process surface types in §2/§3 are member against `Grpc.Net.Client`,
 `Grpc.Core.Api`, `Microsoft.AspNetCore.TestHost`, and `Grpc.Net.Client.Web`; the server-hosting,
 grpc-web-middleware, and health-service members are member against the consumer-bound
...
- symmetric compression axis: the SAME `ICompressionProvider` rows (`gzip`/`deflate`, owned by
 `Grpc.Net.Common`, catalogued in `libs/csharp/Rasm.Compute/.api/api-grpc-common.md`) register on BOTH the client
