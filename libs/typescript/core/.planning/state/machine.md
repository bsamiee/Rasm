...
const _drive = <Id extends string, S extends string, V extends string, X>(
  ...
  const seeded: Either.Either<
    ...
    _Spent
  > = Either.right([origin, []])
  return Array.reduce(signals, seeded, (acc, signal) =>
    Either.flatMap(acc, ([config, macros]) =>
