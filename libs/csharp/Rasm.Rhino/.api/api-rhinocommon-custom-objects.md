...
This catalog is the sole member-depth home for the whole `Rhino.UI` in-viewport widget family; `libs/csharp/Rasm.Rhino/.api/api-rhino-ui.md` registers it and tables none of it.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Line`/`Transform`/`BoundingBox`/`Interval` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
