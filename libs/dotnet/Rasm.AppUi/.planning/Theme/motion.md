...
- Packages: Rasm (project — `SpringShape`/`SpringState`/`SettleBand` the spring algebra, `FaultBand`/`[FaultCase]`/`Fault` the fault floor, `Easing` the easing vocabulary, `UnitInterval` the progress admission every kernel read carries), Thinktecture.Runtime.Extensions, NodaTime, LanguageExt.Core, BCL inbox
...
- Packages: Rasm (project — `SpringShape`/`SpringState`/`DecayShape`/`SettleBand` the physics owners, `UnitInterval`), Thinktecture.Runtime.Extensions, NodaTime, LanguageExt.Core
