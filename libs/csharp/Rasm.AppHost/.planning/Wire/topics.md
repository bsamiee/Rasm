...
public sealed record DomainEvent(
    ...
    // route differently per deployment. `Wire/outbox#OUTBOX_FABRIC` holds the one projection between them, so
    // that durable envelope carries the type while this bus carries the channel, with no third spelling.
    //
    ...
    public static Fin<DomainEvent> Of(
        ...
        SignalGovernance.Rostered(type).Map(admitted =>
            new DomainEvent(topic.Key, admitted, source, idempotencyKey, payload, classification, logical, physical, Offset: 0));
