...
// Dimension law is boundary-aware (2g + max(0, b−1)) — the same law the harmonic basis and DEC receipt state; the
// mature closed-surface-only gate contradicted the basis its own Build produced on bounded surfaces.
...
public readonly partial record struct HodgeDecompositionReceipt(
    ...
    private bool ValidityGate() {
        ...
        return ExpectedDimension == (2 * ExpectedGenus) + Math.Max(0, ExpectedBoundaryComponents - 1)
            && ExpectedDimension >= 1 && BasisCount == ExpectedDimension
