...
def _solve_bind(
    ...
    for name, leaf in parameters.items():
        leaf.value = np.asarray(bind.get(name, leaf.value), dtype=float)
...
def _evidence(constraints: list[object], cp: object) -> ConvexEvidence:
    ...
    return ConvexEvidence(
        ...
        primal_residual=max((kkt.primal(expr) for kkt, _, expr in solved), default=0.0),
