...
class Transmittal:  # Closed issue vocabulary; each owned/composed seam faults through `BoundaryFault`
    ...
    async def _emit(self) -> RuntimeRail[ArtifactReceipt]:
        ...
        # Soft terminal notice: the CloudEvents envelope seals from the settled receipt under the live trace context;
        # its rail is deliberately dropped here because a notice fault is receipt-stream evidence, never a close break.
        projected.map2(railed, lambda receipt, pair: TransmittalNotice.issued(receipt, self._deliverable.issued_register, extensions=_extended(pair[1])))
        return projected
