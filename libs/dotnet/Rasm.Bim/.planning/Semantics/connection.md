...
public static class ConnectionProjection {
    ...
    public static Fin<Seq<(Node Bag, Relationship Edge)>> All(
        ...
        (project.Extract<IfcElement>().AsIterable().ToSeq()
            ...
            .As()
            .Map(static details => details.Choose(identity))).ToFin();
    ...
    static Fin<Option<(PropertyName Name, PropertyValue Value)>> Measured(PropertyName name, Dimension dim, double native, UnitScheme scale) =>
        double.IsFinite(native)
            ? MeasureValue.OfSi(dim, scale.Coerce(native, QuantityType.OfDimension(dim), dim))
                .Map(value => Some((name, (PropertyValue)new PropertyValue.Measure(value))))
            : Fin.Succ(Option<(PropertyName, PropertyValue)>.None);

    static Fin<PropertyBag> Rows(Seq<Fin<Option<(PropertyName Name, PropertyValue Value)>>> rows) =>
    ...
    static Seq<(PropertyName Name, Dimension Dimension, Func<IfcReinforcingMesh, double> Read)> Family(
        ...
        toSeq(axes).Bind(axis => Seq<(PropertyName, Dimension, Func<IfcReinforcingMesh, double>)>(
            (PropertyCategory.Neutral.Row($"{axis.Axis}BarNominalDiameter"), Dimension.LengthDim, m => axis.Read(m).Diameter),
            (PropertyCategory.Neutral.Row($"{axis.Axis}BarSpacing"), Dimension.LengthDim, m => axis.Read(m).Spacing),
            (PropertyCategory.Neutral.Row($"{axis.Axis}BarCrossSectionArea"), Dimension.AreaDim, m => axis.Read(m).Area)));

    // --- [READER_ROWS]
    static readonly PropertyName BearingType = PropertyCategory.Neutral.Row("BearingType");
    static readonly PropertyName IsolatorType = PropertyCategory.Neutral.Row("IsolatorType");
    static readonly PropertyName TendonType = PropertyCategory.Neutral.Row("TendonType");
    static readonly PropertyName BarSurface = PropertyCategory.Neutral.Row("BarSurface");
    static readonly PropertyName SteelGrade = PropertyCategory.Neutral.Row("SteelGrade");
    static readonly PropertyName BarLength = PropertyCategory.Neutral.Row("BarLength");
    static readonly PropertyName MeshType = PropertyCategory.Neutral.Row("MeshType");
    static readonly PropertyName MeshLength = PropertyCategory.Neutral.Row("MeshLength");
    static readonly PropertyName MeshWidth = PropertyCategory.Neutral.Row("MeshWidth");
    static readonly PropertyName ConduitType = PropertyCategory.Neutral.Row("ConduitType");
    static readonly PropertyName AssemblyType = PropertyCategory.Neutral.Row("AssemblyType");
    static readonly PropertyName AssemblyPlace = PropertyCategory.Neutral.Row("AssemblyPlace");
}
```
