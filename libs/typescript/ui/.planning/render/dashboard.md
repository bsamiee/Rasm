...
- [1]-[LIVE_BINDING_DASHBOARD]: the `@tanstack/react-table` binding-status row model virtualized through `@tanstack/react-virtual`, the `CoercedValueWire` coercion column, and the `WriteReceiptWire` write-disposition projection.
