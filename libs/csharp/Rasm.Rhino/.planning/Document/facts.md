...
- Packages: `Thinktecture.Runtime.Extensions` (`libs/csharp/.api/api-thinktecture-runtime-extensions.md` — `[SmartEnum]` slot vocabularies, `[Union]` bodies, `[ValueObject]` `UndoSerial`); `LanguageExt.Core` (`libs/csharp/.api/api-languageext.md` — `Seq` fact runs, monoid append); kernel `Domain/rails` (`Op`, `Fin`).
