...
- Run `dotnet build libs/csharp/Rasm.Rhino/Rasm.Rhino.csproj --no-restore` for local proof.
- Run `pnpm check:cs` before completion after C# edits.
- Run `dotnet format Workspace.slnx --verify-no-changes --severity warn --no-restore` and `git diff --check` for final hygiene when the change touches public surface or formatting-sensitive code.
- Do not run Rhino runtime checks unless explicitly requested. Static build, RhinoWIP XML, and decompile evidence are default proof for this library.
