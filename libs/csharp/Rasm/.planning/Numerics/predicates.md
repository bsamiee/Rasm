...
- Owner: `Sign` `[SmartEnum<int>]` the closed ternary verdict (`Negative`/`Zero`/`Positive`, key `-1`/`0`/`+1`) every predicate returns, with the `Flip`/`Times` verdict algebra the homogeneous denominator flips ride; `Axis` `[SmartEnum<int>]` the closed coordinate vocabulary (`X`/`Y`/`Z`, key = coordinate ordinal) carrying the projected-plane ordinal columns `U`/`V` (the plane NORMAL to the axis: `X → (y,z)`, `Y → (z,x)`, `Z → (x,y)`) — the ONE generator over which every axis-projected member spans its three planes, never three enumerated member names per plane, and carrying `DominantOf` — the one admission of geometry into the vocabulary returning `Fin<Axis>` (|max-component| axis of a vector · supporting-plane axis of a triangle · Newell-normal axis of a quad cycle; one name, operand arity discriminates; the vector arity is the ONE degeneracy gate — finite AND non-zero — the triangle and quad arities compute their normal and delegate to, a non-finite or zero normal routing the `Op`-keyed `Fault.InvalidInput` refusal over the caller's rail rather than the silent fallback axis a bare max-component pick returns for a `NaN` component; a float heuristic selecting a projection plane for the exact members, its normals never an exact-carrier operand — the former Meshing-local `DominantOf`/`DominantAxis` five-page copies and the untyped fallback-axis pick are the deleted forms); `Implicit` the closed `[Union<Point3d, Ssi, Lpi, Tpi>]` ad-hoc point carrier — `Explicit` (an ordinary coordinate point), `Ssi` (segment-segment crossing, FOUR defining points + the construction plane), `Lpi` (line-plane crossing, FIVE defining points), `Tpi` (three-plane crossing, NINE defining points) — implicit conversions make every `Point3d` call site lift without ceremony, and the carrier stores DEFINING POINTS ONLY: the exact homogeneous coordinates (`X`/`Y`/`Z` numerators + `Lambda` denominator) derive on demand through `Homogeneous<T>()` in whichever `IExact<T>` carrier the tier needs, so no rounded coordinate and no rounded denominator ever sits inside the carrier; `Predicate` the ONE static surface owning the direct ladders and the implicit folds.
...
- Entry: `public static Sign Orient2D(double ax, double ay, double bx, double by, double cx, double cy)` (with the `Point3d` adapter over it) the exact sign of the `(b-a)×(c-a)` 2D cross-product determinant — positive for a counter-clockwise turn; `public static Sign Orient3D(Point3d a, Point3d b, Point3d c, Point3d d)` the exact sign of the `3×3` determinant placing `d` relative to the oriented plane `abc`; `public static Sign InCircle(Point3d a, Point3d b, Point3d c, Point3d d)` the exact sign of the `4×4` lifted determinant testing `d` inside the circumcircle of `abc`; `public static Sign InSphere(Point3d a, Point3d b, Point3d c, Point3d d, Point3d e)` the exact `5×5` lifted in-sphere determinant; `public static Sign Orient2D(in Implicit a, in Implicit b, in Implicit c, Axis axis)` the exact orientation of three possibly-constructed points projected on the axis plane; `public static Sign Compare(in Implicit a, in Implicit b, Axis axis)` the exact three-way comparison of one coordinate (`Negative` = `a < b`); `public static Sign InCircle(Point3d, Point3d, Point3d, in Implicit, Axis)` / `public static Sign InSphere(Point3d, Point3d, Point3d, Point3d, in Implicit)` the exact in-circum verdicts for a constructed query point — every member total, pure, exact, no rail; a degenerate construction (`lambda = 0`: parallel segments, a line parallel to its plane, near-parallel plane triples) yields `Sign.Zero` through the `Times` flip algebra, the degeneracy witness the consumer's recovery reads.
...
// DominantOf is the ONE admission of geometry into the vocabulary — the |max-component| axis of a
// vector, the supporting-plane axis of a triangle, the Newell-normal axis of a quad cycle; one
// name, the operand arity discriminates. The choice is a float HEURISTIC selecting a projection
// plane for the exact members below — its cross/Newell normals never enter an exact carrier.
// Admission returns Fin<Axis>: a degenerate normal has NO dominant axis, so the vector arity is the
// ONE gate — finite AND non-zero — the triangle and quad arities compute their normal and delegate
// to, and a non-finite or zero input (a collinear triangle, a slit quad, a zero-length direction)
// routes the Op-keyed Fault.InvalidInput refusal the caller's rail already carries, never the
// silent fallback axis a bare max-component pick returns for a NaN component. Op? key = null is the
// public-surface optional-key form: the caller threads its own Op, OrDefault names this operation.
[SmartEnum<int>]
public sealed partial class Axis {
    ...
    // The ONE degeneracy gate: a finite non-zero direction always has a dominant axis; a non-finite
    // or zero vector has none and refuses. The triangle/quad arities delegate here, so one gate
    // decides the whole family and no rounded normal ever reaches a bare max-component pick.
