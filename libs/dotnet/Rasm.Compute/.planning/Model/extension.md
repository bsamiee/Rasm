...
- Entry: `Register` accumulates asset probes before `Try.lift` admits native registration; `Egress` captures native metadata reads and proves the projected value against its declared slot.
- Result: `Register` returns the same `SessionOptions` after every configured library verifies and registers; a missing asset or native registration fault remains on the typed result.
