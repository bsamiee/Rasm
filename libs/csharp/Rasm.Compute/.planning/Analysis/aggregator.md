...
// One resolved window field the EN ISO 10077-1 fold weights — a glazed field (pane area, center-of-glass Ug, edge length,
// edge-seal Ψg) or a frame field (profile area, Uf, no edge bridge). Glazed iff EdgeLengthM > 0 — the spacer Ψg rides the
// glazing/frame interface perimeter, so a frame field zeroes its lg·Ψg. The thermal runner assembles these; the kernel folds them.
...
public static class AssemblyAggregator {
    ...
    // mass-derived fields — never aborting the U a thermal runner reads. Only a missing material NODE rails; a missing property is typed not-applicable.
    ...
    // property. The series fields (U/heat-capacity/Sd) and thickness-less STC are double.NaN/0 (no series structure). Only a missing NODE rails.
    ...
    // The per-ply geometric volume each composition case distributes: a Single/ProfileSet the element volume, a layer its
    ...
    // The worst-ply fire envelope reads the EN 13501-2 R (load-bearing) criterion off the seam FireResistance — the assembly's
    ...
    // The per-module Stage accumulation: a FRESH array each ply (never mutated in place) so the carbon fold stays immutable.
    // The seam StageGwp is the DERIVED GwpTotal-per-stage row (Environmental.StageGwp = IndicatorAt(GwpTotal, stage) over the
    ...
    // --- [FOLD_STATE] ---------------------------------------------------------------------
    // The typed per-discipline fold accumulators (algorithm state co-located with the kernel): each Absorbs one ply and
