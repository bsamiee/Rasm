...
- [05]-[TOKEN]: `({ accountId, tunnelId }) → { token }` — the `TUNNEL_TOKEN` the in-cluster cloudflared Deployment runs with (cross a manifest only `pulumi.secret`-wrapped).
...
- spelling law: author the catalog-bound current class, never the deprecated alias — `DnsRecord` not `Record`, `WorkersScript` not `WorkerScript`, `ZeroTrustAccessApplication` not `AccessApplication`, `ZeroTrustTunnelCloudflared`/`…Config`/`…Route` not `Tunnel`/`TunnelConfig`/`TunnelRoute`; the `@deprecated` JSDoc names the replacement.
...
- canonical law: `getXOutput` for graph-threaded reads; the catalog-bound current class over every deprecated alias; one shared `Provider` per arm.
