...
|  [5a]   | saga compensation                 | `SagaOwner`/`SagaStep`                      | fold over StepOutcome   | forward/compensating pairs, `Workflow.withCompensation` reverse-order, `sagaFold` Committed/RolledBack/Aborted, proxy over the ONE RpcGroup | FINALIZED |
...
Durable saga composition (compensating transactions across activity chains) is landed as the `SagaOwner`/`SagaStep` fold over the closed `StepOutcome` union with engine-owned `Workflow.withCompensation` reverse-order rollback, derived over the ONE `WorkflowProxy` `RpcGroup`; the next deepening drives saga parallel-branch fan-out (concurrent forward arms with per-branch compensation barriers), hybrid-search re-ranking (learned-rank fusion over the four signals), and the IaC lifecycle (drift detection and policy-as-code expansion). The GraphFork CRDT op vocabulary the durable hub peer would fold waits on the C# `sync-collaboration#MERGE_LAW` op-log amendment. Closed by the bar: every provider is one literal, every entity is one `Model.Class`, every export format is one codec row, every runner topology is one protocol row, and the deploy mode is one dispatch row — the domain admits no parallel surface.
