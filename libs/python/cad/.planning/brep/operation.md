...
- Entry: `built` composes from `brep/placement#ADMISSION`, which seats the result below every arm this fold imports.
...
- Law: `sourced` lives at `exchange/step#CODEC` and threads that path unchanged, so a decode refusal keeps its `exchange` leg and no resolution twin grows here.
...
type Arm = Callable[[Oneof, frozendict[bytes, Path]], CadResult[Outcome]]


# --- [OPERATIONS] -----------------------------------------------------------------------


def _narrowed[P: Message](payload: type[P], held: Oneof, /) -> CadResult[P]:
...
def sewn(shape: TopoDS_Shape, tolerance_m: float, /) -> CadResult[TopoDS_Shape]:
...
def _rostered(held: Oneof, _sources: frozendict[bytes, Path], /) -> CadResult[Outcome]:
    return primitive(held).map(Outcome.of)


def _algebra(field: str, held: Oneof, sources: frozendict[bytes, Path], /) -> CadResult[Outcome]:
    return _narrowed(BooleanInputs, held).bind(lambda payload: boolean(field, payload, sources))


def _shaped[P: Message](payload: type[P], run: Callable[[P], CadResult[TopoDS_Shape]], /) -> Arm:
...
def _rewritten[P: Message](
    ...
    run: Callable[[TopoDS_Shape, P], CadResult[TopoDS_Shape]],
    /,
) -> Arm:
    def arm(held: Oneof, sources: frozendict[bytes, Path], /) -> CadResult[Outcome]:
...
- Law: `measured` and `sealed` both return `CadResult`, so the seal's exception-to-exception restamp is gone and each refusal keeps its own producing leg.
...
def _dispatched(elected: Oneof, sources: frozendict[bytes, Path], /) -> CadResult[Outcome]:
    return (
...
def _evidence(outcome: Outcome, output: Path, /) -> CadResult[BrepEvidence]:
...
def execute(request: ExecuteRequest, sources: frozendict[bytes, Path], output: Path, /) -> CadResult[BrepEvidence]:
