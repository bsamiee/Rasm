...
This catalog owns the annotation model boundary: `DimensionStyle` drafting-config algebra with its field-override inheritance and the `DimStyleTable` transaction; `AnnotationBase` rich-text run editing and property-override projection shared by every annotation; `TextEntity` construction plus the text-to-curve/surface/polysurface/extrusion outlining family; `Leader` construction; the six-kind `Dimension` geometry family over one per-instance override surface; and `TextFields` formula evaluators feeding `TextFormula`. Drawn pixels cross through `api-rhinocommon-display.md`; hatch, linetype, font, and section resources those styles reference live in `api-rhinocommon-drafting-resources.md`; the block-attribute `TextFields` slice stays in `api-rhinocommon-blocks.md`; `TextDot` derives straight from `GeometryBase` and carries no `DimensionStyle`, so it stays outside this style-bound boundary; every live document-bound annotation resolves inside the owning session before detached values cross the boundary.
...
|  [04]   | `TextFields`     | static surface  | text-field formula evaluators feeding `TextFormula`               |
...
[STYLE_FIELD_ALGEBRA]:
- `Rhino.DocObjects.DimensionStyle.SetFieldOverride(Field field) : void` — marks one field overridden so its value comes from this child, not the parent.
...
- `Rhino.Geometry.AnnotationBase.Font : Font` / `FirstCharFont : Font` / `FontIndex : int` — the annotation font and its first-run font.
...
- `Rhino.Geometry.AnnotationBase.FirstCharProperties(string rtfStr, ref bool bold, ref bool italic, ref bool underline, ref string facename) : bool` [static] — reads the first character's run formatting.
...
- `Rhino.Runtime.TextFields.PageNumber() : int` / `NumPages() : int` / `PageName([string id]) : string` / `PaperName() : string` / `PageWidth() : double` / `PageHeight() : double` / `DetailScale(string detailId, string scaleFormat) : string` / `LayoutUserText([string layoutId,] string key) : string` — layout fields.
...
[ANNOTATION_TOPOLOGY]:
- `DimensionStyle` is the shared drafting config; a `Field` is the atom the override algebra addresses. Parent/child inheritance is per-field: a child holds a value only for fields marked overridden via `SetFieldOverride`, every other field inheriting the parent through `ParentId`. Per-object customization mints a child style, marks the differing fields, and points the annotation's `DimensionStyleId` at it — `SetOverrideDimStyle` on `AnnotationBase` performs the mint, `GetDimensionStyle(parent)` resolves the effective merge.
- annotation text carries three coupled forms — `RichText` (RTF source), `PlainText` (flattened), and `TextFormula` (`TextFields` expression) — edited through run operations (`RunReplace`, `SetBold`/`SetItalic`/`SetUnderline`/`SetFacename`) and re-flowed by `WrapText`; a formula is resolved by `TextFields.TryFormat` against the document, never string-concatenated.
