...
- package: `GShark`
- version: `2.3.1`
- license: MIT (GShark / Cesar Pinto et al.; `github.com/GSharker/G-Shark`; nuspec `<license type="expression">MIT</license>`)
...
|  [03]   | `NurbsSurface`                 | rational B-spline surface            | `DegreeU`/`DegreeV`/`KnotsU`/`KnotsV`/`ControlPoints` + `Weights`/`ControlPointLocations` (dehomogenized view — the full canonical-bytes READ surface); the `From*`/`Ruled`/`Revolved` factory family; `PointAt(u,v)`/`EvaluateAt`/`IsoCurve`/`SplitAt`/`ClosestPoint` |
...
|  [08]   | `Divide(int numberOfSegments)` / `Divide(double maxSegmentLength, bool equalSegmentLengths=false)` / `DivideByChordLength(double)` | instance | equal-count / max-length subdivision → `(Points, Parameters)`; `DivideByChordLength` returns `List<double>` (parameters ONLY, no point list) |
...
- internal-vs-public boundary: `GShark.Sampling`/`GShark.Analyze`/`GShark.Modify` are dominantly `internal static` algorithm kernels — the public surface is the `NurbsBase`/`NurbsSurface` INSTANCE methods plus `Fitting.Curve`, `Intersection.Intersect`, `Evaluate.Evaluate`, and the named public statics `Sampling.Curve.AdaptiveSample`/`AdaptiveSampleRange` (curvature-adaptive tessellation) and `KnotVector.Refine` (arbitrary knot refinement, serving BOTH carriers). The README/manifest framing of "Sampling/Analyze/Modify/Optimization" as composable namespaces is INACCURATE for the public API; design pages compose the instance algebra.
