...
- [GWN_CLASSIFICATION] — `SpatialQuery.Winding(probes, otherSoup, BetaSquared)` batches every patch probe per operand; `InteriorOffset` nudges centroids off the surface, and `WindingThreshold` admits the returned field scalars. `CellComplex` freezes the full per-operand classification as `CellSet`; boolean emission consumes the same bits before welding.
