...
- `Query/datasets#SERIES_ROSTER`: one `SeriesKind` row derives the whole ordered provisioning set through `SeriesLane.Provision`, and `SeriesLane.Jobs` joins `jobs` against `job_stats` into one `SeriesJobHealth` row per policy. `timescaledb.segmentby` takes the schema's BOUNDED text keys alone while `timescaledb.orderby` takes its identity keys before the time column — a content-key segment mints one compressed batch per row — and the rollup's `GROUP BY` stays the whole key so each stream keeps its own buckets.
