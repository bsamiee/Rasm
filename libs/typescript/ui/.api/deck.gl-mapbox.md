...
- [02]-[MAP]: `addControl`/`removeControl`/`hasControl`, camera getters `getCenter`/`getZoom`/`getBearing`/`getPitch`/`getPadding`, optional `getProjection?`/`getTerrain?`/`getFreeCameraOptions?` (`getFreeCameraOptions` mapbox-only, absent on maplibre → deck falls back to `transform.elevation` for altitude), `addLayer`/`removeLayer`/`moveLayer`, `getCanvas`/`getContainer`, `triggerRepaint`, `on`/`off`/`once`.
