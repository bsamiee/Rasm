...
[ENTRYPOINT_SCOPE]: the deep-pixel image codecs carrying planes above 8-bit
- rail: compression
...
|  [09]   | `tiff_decode(data, /, index=0, *, asrgb=False)`                                  | `index` takes an `int`, a sequence, or a `slice`     |
|  [10]   | `jpegxl_encode(data, /, level=None, *, lossless, distance, transfer, …)`         | JXL at uint8, uint16, float16, float32               |
...
|  [19]   | `qoi_encode` and `qoi_decode`                                                    | QOI 8-bit lossless, the fast intermediate            |
...
- `pyvips`(`.api/pyvips.md`): resampling, thumbnailing, and the fused decode path stay with libvips; this surface owns the codecs libvips lacks at depth — EXR, `rgbe`, JXL at float, 12-bit AVIF, LERC. Planes resample through `pyvips` in the float lane and encode here, never the inverse.
