...
- `libs/dotnet/.api/api-languageext.md`: every host call that can fail or return null crosses through `Op.Catch`; nullable results bind `Optional(...).ToFin(Fail: op.InvalidResult())`, so a captured bitmap, resolved mode, or bound conduit crosses into domain code as `Fin<A>`, never as a nullable host handle.
