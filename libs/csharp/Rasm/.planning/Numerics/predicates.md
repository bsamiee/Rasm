...
public static class RationalOracle {
    ...
    // `ToFraction().Sign`. `WithBlankFlags()` proves the accumulation raised no `FlagInexact`, so the binary
    // verdict is self-certifying exact rather than assumed; the `EContext` carries the exactness proof, never a
    // bare `Sqrt`/`Pow` inside an adjudication (those are inexact and confined to readout).
    public static Sign BinaryOf(Expansion e) {
        ...
        return exact.HasFlags && (exact.Flags & EContext.FlagInexact) != 0 ? Sign.Zero : Sign.Of(acc.Sign);
    }
}
```
...
- Owner: `Expansion` the `readonly struct` nonoverlapping floating-point expansion (a small per-fold-sized component buffer, most significant component last) with the `TwoSum`/`TwoProduct`/`Grow`/`ScaleExpansion`/`ExpansionSum` construction folds, the `Estimate`/`SignOf` projections, and the `ToFraction` exact rational lift bridging to the topmost tier — the sign-exact arithmetic the predicates' exact branch rides; `ErrorBound` the `record` per-tier static-permanence filter-row table (`Orient2D`/`Orient3D`/`InCircle`/`InSphere` rows, each carrying its `double` forward-error `Coefficient` and its 106-bit `RefineCoefficient` derived once from `Epsilon`/`DoubleDoubleEpsilon`) with the `Of` → `Stage` filter projection and the `Refine` → `Sign?` 106-bit verdict; `RationalOracle` the static exact adjudicator (`Orient2D`/`Orient3D` direct + `InCircum` implicit over the PRIMARY `ExtendedNumerics` `Fraction` oracle, plus `BinaryOf` the INDEPENDENT `PeterO.Numbers` `EFloat`-under-`EContext.Unlimited` second-source binary adjudicator whose `EInteger` `uint[]` bignum shares no type with `Fraction`'s `BigInteger`) the implicit predicates escalate to and the law-matrix's four-way `Expansion`/`EFloat`/`Fraction`/`ERational` differential proves against; `NumericsPolicy` the static owner documenting the filter-then-refine-then-exact contract and the error-bound constants — the named, scoped sanction for raw interior `double` arithmetic.
...
- Packages: Thinktecture.Runtime.Extensions (`[SmartEnum<int>]`), TYoshimura.DoubleDouble (`ddouble` 106-bit error-free-transform refine, `AdjustScale`/`Sign`/`Abs` — the middle tier sharing the `Expansion` `TwoProduct`/`TwoSum` rounding model), ExtendedNumerics.BigRational (`Fraction`/`Fraction.Sign`/`Add`/`Subtract`/`Multiply`, `(Fraction)double` lossless decomposition — the PRIMARY exact-rational top tier over `System.Numerics.BigInteger`), PeterO.Numbers (`EFloat.FromDouble`/`Multiply`/`Subtract`/`Add`/`Sign` under `EContext.Unlimited` — the INDEPENDENT arbitrary-precision BINARY exact adjudicator between the fixed-106-bit `ddouble` and the rational oracle, plus `ERational.FromDouble`/`CompareToBinary`/`Sign` the second independent exact-rational oracle whose self-contained `EInteger` `uint[]` bignum shares NO representation with `Fraction`'s `BigInteger`, so a sign confirmed by BOTH oracles is verified across two unrelated implementations; the dyadic-lossless `FromDouble` lift and the `EContext.WithBlankFlags`/`FlagInexact` exactness self-proof — composed never re-minted), BCL inbox (`System.Math.FusedMultiplyAdd`, `double.Epsilon`/`BitIncrement`); no external geometry dependency
...
public readonly struct Expansion {
    ...
    // The nonoverlapping components in magnitude order — the exact bridge the INDEPENDENT `RationalOracle.BinaryOf`
    // `EFloat`/`ERational` second-source adjudicator lifts losslessly (every component is exactly dyadic), so the
    // determinant sign is verifiable in a representation unrelated to `Fraction`'s `BigInteger`.
...
- [NUMERIC_DETERMINISM] — the sign-exactness law-matrix the spec rail asserts. The harness is `PredicateLaws` (a CsCheck property suite under `testing-cs`): it generates near-degenerate input by perturbing collinear/cocircular/cospherical configurations at the `Epsilon` scale and asserts (1) every `Predicate` member returns the EXACT sign of the rational-arithmetic determinant the `RationalOracle` `Fraction.Sign` computes over the lossless `(Fraction)double` decomposition of the same ordinates — the PRIMARY exact-rational oracle the kernel is BOTH proved against and adjudicates the highest-degree implicit construction through at runtime, the admitted `ExtendedNumerics.BigRational` carrier the floor composes; (1b) the FOUR-WAY differential agreement — the `Expansion.SignOf` exact branch, the `RationalOracle.BinaryOf` `PeterO.Numbers` `EFloat` binary adjudicator, the `Fraction.Sign` `ExtendedNumerics` rational oracle, and the `ERational.Sign` `PeterO.Numbers` second rational oracle all return the SAME determinant sign across FOUR unrelated arbitrary-precision implementations (`Expansion`'s in-house Shewchuk expansion, `EFloat`'s `EInteger` `uint[]` binary bignum, `Fraction`'s `System.Numerics.BigInteger` rational, and `ERational`'s independent `EInteger` rational — no two share a representation), so a shared-implementation blind spot cannot pass; (1c) `EContext.WithBlankFlags()` proves the `EFloat`/`ERational` accumulation raised no `FlagInexact`, so the binary/rational cross-check is self-certifying exact rather than assumed; (2) the antisymmetry law `Orient2D(a,b,c) == Orient2D(b,a,c).Flip` and its 3D/incircle/insphere analogues hold under coordinate permutation parity; (3) every ladder tier agrees with the oracle on the set it resolves — the `double` filter where `Stage.Filtered`, the `ddouble` 106-bit `Refine` where `Some`, and the `Expansion` branch elsewhere all return the `Fraction.Sign` verdict, so the ladder is monotone (a tier never reverses a coarser tier's determinate sign); (4) the predicates are translation-invariant (a uniform `Point3d` offset never flips a verdict). The harness needs NO live-host probe — `Fraction`/`ddouble`/`EFloat`/`ERational`/`Math.FusedMultiplyAdd`/`Point3d` are stable; the kernel is total by construction. The one residual the harness watches is the FMA-availability assumption: `Math.FusedMultiplyAdd` lowers to a hardware FMA on osx-arm64; a future FMA-free RID routes `TwoProduct` through the named Dekker-split fallback (`NumericsPolicy.Splitter`, the `2^27+1` constant already seated) — confirming that fallback's bit-exactness against the FMA path on a non-FMA target is the only deferred numeric probe, and it gates a fallback row, never the FMA-path predicates.
...
- [PRECISION_LADDER] — the `PrecisionTier` four-tier adaptive ladder is the floor's robustness/throughput contract: `Double` (the IEEE-754 forward-error filter, `ErrorBound.Of`), `DoubleDouble` (the `TYoshimura.DoubleDouble` `ddouble` 106-bit error-free-transform refine, `ErrorBound.Refine`), `Expansion` (the in-house sign-exact nonoverlapping expansion), and `Rational` (the `ExtendedNumerics.BigRational` `Fraction` `BigInteger` oracle, with `PeterO.Numbers` `EFloat`/`ERational` the INDEPENDENT second-source binary+rational adjudicator at the SAME exact-adjudication altitude in an unrelated representation). each direct predicate walks the first three inline — the cheap `double` filter clears the bulk, the `ddouble` recompute resolves the near-degenerate residue without a heap allocation (each per-vertex difference vector normalized to one binary exponent through `ddouble.AdjustScale` before the EFT determinant), and only the sub-106-bit-degenerate set reaches the `Expansion` exact branch — while the implicit in-circum predicates escalate their homogeneous determinant straight to the `Rational` tier through `RationalOracle.InCircum`. The `PeterO.Numbers` `EFloat` arbitrary-precision BINARY tier slots between the fixed-106-bit `ddouble` and the unbounded `Fraction` rational: under `EContext.Unlimited` the polynomial orient/incircle/insphere determinant over dyadic-double coordinates is EXACT in a binary `mantissa*2^exp` representation INDEPENDENT of `BigRational`, so `RationalOracle.BinaryOf` cross-validates the `Expansion` sign and the `Fraction` sign across two unrelated bignums, and `ERational` adds a third independent exact-rational oracle whose `EInteger` `uint[]` shares no type with `Fraction`'s `BigInteger`. The ladder is MONOTONE: because `ddouble` arithmetic uses the SAME FMA `TwoProduct` and Knuth `TwoSum` error-free transforms as the `Expansion` kernel, the 106-bit stage and the exact branch share one rounding model and never disagree on a sign both resolve, and the `Fraction`/`EFloat`/`ERational` tiers are the infinite-precision ceiling every faster tier is differentially fuzzed against. The cost profile is the design: the rational/binary arbitrary-precision adjudication and the `Expansion` fold are paid only on the measure-zero degenerate set, never the bulk, and the `ddouble` refine is a fixed two-`double` recompute with no allocation — so the floor stays fast on the common path while remaining exact on every input. The `EFloat`/`ERational` cross-check is a differential VERIFIER, not a throughput tier — the runtime ladder resolves on `double`→`ddouble`→`Expansion`→`Fraction`, and the independent `PeterO` oracles confirm the `Expansion`/`Fraction` verdict in the law-matrix and on the highest-degree implicit residue where a shared-implementation blind spot would be most dangerous. This is the `PRECISION_LADDER` anchor the `api-doubledouble`, `api-bigrational`, and `api-peteronumbers` catalogs reference as the home of their middle, primary-top, and independent-adjudicator tiers.
