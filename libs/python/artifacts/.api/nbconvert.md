...
[CROSS_TIER_STACKING]:
- runtime lane: `from_notebook_node` is blocking native/subprocess work, so `ReportPlan.lane.offload(_exported, spec, executed, modality=Modality.THREAD, retry=RetryClass.OCCT)` bounds every exporter through the runtime owner rather than an inline await or folder-local limiter.
