...
const _taggedReuse = Metric.tagged(
    Metric.counter('kit_spec_tagged_reuse'),
    'tenant',
    '<tenant-a>',
);
...
describe('metric drift', () => {
    it.effect(
        'a stepped counter lands as one drift row carrying before and after',
        () =>
            Effect.gen(function* () {
                ...
                const row = yield* _driftOf(
                    observed.drift,
                    'kit_spec_token_reuse',
                );
    ...
    it.effect(
        'an untouched metric never enters the drift — the gauge can stay silent',
        () =>
            Effect.gen(function* () {
                yield* Metric.increment(_inert);
                const observed = yield* Telemetry.observed(Effect.void);
                expect(
                    Option.isNone(_driftOf(observed.drift, 'kit_spec_inert')),
                ).toBe(true);
            }),
    );
    ...
    it.effect('a frequency fans one reading per occurrence word', () =>
        Effect.gen(function* () {
            const observed = yield* Telemetry.observed(
                Effect.zipRight(
                    Metric.update(_refusals, '<reason-a>'),
                    Metric.update(_refusals, '<reason-a>'),
                ),
            );
    ...
    it.effect(
        'a failing flow still yields its drift — the failure-path counter is the audit material',
        () =>
            Effect.gen(function* () {
                const observed = yield* Telemetry.observed(
                    Effect.zipRight(
                ...
                expect(
                    Option.isSome(
                        _driftOf(observed.drift, 'kit_spec_token_reuse'),
                    ),
                ).toBe(true);
            }),
    );
...
describe('span capture', () => {
    ...
    it.effect(
        'a failing span records the failure outcome — the capture can refute a green claim',
        () =>
            Effect.gen(function* () {
                const observed = yield* Telemetry.observed(
                    Effect.withSpan(Effect.fail('refused' as const), 'mint'),
                );
                const span = yield* Array.findFirst(
                    observed.spans,
                    (row) => row.name === 'mint',
    ...
    it.effect(
        'a nested span carries its parent name and a span event lands on its own record',
        () =>
            Effect.gen(function* () {
                const observed = yield* Telemetry.observed(
                    Effect.withSpan(
...
describe('board projection', () => {
    it.effect('the board serves the live reading for a registered metric', () =>
        Effect.gen(function* () {
            ...
            const row = yield* Array.findFirst(
                board,
                (reading) => reading.name === 'kit_spec_inert',
            );
