...
- [CLT_IN_PLANE_BENDING]-[BLOCKED]: which verification model governs cross-laminated in-plane (minor-axis) bending, so the `TimberDesign` panel arm replaces its gated `BendingMinorNmm: 0.0` with a real `M_Rd,z`; route: the EN 1995-1-1:2025 in-plane clause on publication, cross-read against the EN 16351 in-plane rules.
