...
- Law: bytes never enter the draft — `FileTrigger` opens the platform picker and `DropZone` admits drops through the `isFileDropItem` refinement (text and directory items decline at the seam; `isTextDropItem` stays the collection-reorder refinement `system/primitive` names); the admitted `File` hands straight to `[06]`'s upload lane and the DRAFT field holds only the finished object's identity the wire returns, so an abandoned session restores a reference, never a stranded blob.
