...
from rasm.data.tabular.lakehouse import Lakehouse
...
class DerivedSnapshot(Struct, frozen=True):
    ...
    def _materialize(self, source: Lakehouse, start: int, end: int | None, prior: tuple[PartitionBundle, ...]) -> tuple[PartitionBundle, ...]:
        ...
        recomputed = {str(value): self._recompute(cdf.filter(pc.equal(cdf.column(key_col), value)), str(value)) for value in changed}
