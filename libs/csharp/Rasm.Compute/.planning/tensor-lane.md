...
public static class TensorVocabulary {
    ...
    // closing the [INFERENCE_LANE] "size from GetTensorTypeAndShape().ElementCount, never re-multiplied dimensions" law.
...
- [OPERATOR_BACKLOG]: `Normalize` has no `TensorPrimitives` member and never becomes a single-call row — vector normalization composes `Norm` then `Divide` against the reduced magnitude. `ConvertToInteger`/`ConvertToIntegerNative` are catalogued conversion rows whose `ConvertKernel<TFrom, TTo>` instantiation is the integer-destination `ConvertKernels<TFrom, int>`/`<TFrom, long>` row, reached only behind a `TensorDtype.Quantized` admission, never a bare float-to-int loop.
- [KERNEL_LOWERING]: the matrix and structural rows resolve through the live `numeric-lane#KERNEL_LOWERING` GEMM, `Im2Col` patch-projection, and strided-window fold against the MathNet provider; the `Backward.MatMul` reverse-mode transpose product routes the same `KernelLowering.Lower` matmul entrypoint with operand B transposed (no phantom member); the residual is the fingerprint-matched `BenchmarkClaim` that gates a partition route over the lowered GEMM on the live host (the in-page partition fence reads the threaded claim's `Route` column; only the live-host fingerprint resolution stays tier-3).
- [ORT_BRIDGE]: the `TensorBridge` C-data residency lattice and the `BoundFlow` `OrtIoBinding` steady-state loop are tier-2 decompile-closed against the MAXIMAL `api-onnxruntime.md` surface — `CreateTensorValueFromSystemNumericsTensorObject`, `CreateTensorValueFromMemory`, `CreateTensorValueWithData`, `CreateAllocatedTensorValue`, `GetTensorDataAsTensorSpan`/`GetTensorMutableDataAsTensorSpan`, `GetTensorDataAsSpan`/`GetTensorMutableRawData`, `GetTensorSizeInBytes`, `GetTensorTypeAndShape`, `GetTensorMemoryInfo`, `OrtIoBinding.ClearBoundInputs`/`ClearBoundOutputs`/`GetOutputValues`/`BindOutputToDevice`, and `OrtMemoryInfo.DefaultInstance` carry decompile-1.26.0 evidence and need no probe; the `ManagedSpan`/`MemoryBacked`/`OutputValue`/`SpanView` residency rows are CPU-proved and the `SynchronizeBoundInputs`/`SynchronizeBoundOutputs` device-stream fence is a CPU no-op here, and the CPU egress lattice (IEEE-754 TensorSpan and quantized raw-byte polarities) is FINALIZED-able now. The residual is the `DeviceResident` row's `OrtEnv.CreateSharedAllocator` device-memory allocation and the bound-loop stream synchronization latency on the live CoreML/GPU rows — a model-lane#INFERENCE_LANE tier-3 host probe (the `model-lane` `BoundLoop` shares this fence), not this lane's, because the tensor↔OrtValue C-data projection this lane owns is fully proved on the CPU residency gates.
- [DDG_ADJOINT]: the elementwise differentiable-operator rows (Tanh, Sigmoid, Exp, Log, ReLU) carry their reverse-mode VJP and forward-mode JVP as transcription-complete diagonal-Jacobian folds renting `MemoryOwner` scratch, and the `Chain` tape fold replays each op's adjoint against its own recorded primal — these need no probe; the residual cross-folder leaves are the non-diagonal `Backward.MatMul`/`Backward.SoftMax` exact operand-transpose/Jacobian-minus-outer spellings against the live `KernelLowering.Lower` matmul geometry and the differentiable-geometry operator adjoints — the reverse-mode VJP of the DDG Laplacian, heat-flow, and spectral operators and the remeshing-step adjoint — which compose the `Rasm`/Vectors operator kernel's forward primitives and ground against the `Rasm`/Vectors operator member surface at cross-folder alignment (the `Rasm`/Vectors operator kernel owns the forward operator and adjoint-coefficient derivation, this lane owns the reverse-mode tape chain the `solver-and-optimization#OPTIMIZER_LANE` gradient-adjoint row consumes for shape optimization and inverse design); a non-diagonal operator carries no forward-mode JVP until its forward Jacobian map is grounded, so `AdjointMode.Forward` on `MatMul`/`SoftMax` faults rather than fabricating a gradient. The DDG geometry-operator rows are designed-but-unfenced (no `DifferentiableOp` row yet carries a Laplacian/heat-flow/spectral/remeshing adjoint); the coverage closes by one `DifferentiableOp` row per operator once the `Rasm`/Vectors adjoint-coefficient member surface is grounded.
