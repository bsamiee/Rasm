...
[ENTRYPOINT_SCOPE]: node children — the positional record members every tree fold binds, each of type `Entity` unless its slot spells another

| [INDEX] | [SURFACE]                                                                | [SHAPE]  | [CAPABILITY]                                   |
| :-----: | :----------------------------------------------------------------------- | :------- | :--------------------------------------------- |
|  [01]   | `Entity.Sumf(Augend, Addend)`                                            | record   | addition children in declaration order         |
|  [02]   | `Entity.Minusf(Subtrahend, Minuend)`                                     | record   | children; names INVERT the math sense          |
|  [03]   | `Entity.Mulf(Multiplier, Multiplicand)`                                  | record   | product children                               |
|  [04]   | `Entity.Divf(Dividend, Divisor)`                                         | record   | quotient children                              |
|  [05]   | `Entity.Powf(Base, Exponent)`                                            | record   | power base and exponent                        |
|  [06]   | `Entity.Logf(Base, Antilogarithm)`                                       | record   | logarithm base and argument                    |
|  [07]   | `Entity.Absf(Argument)` / `Entity.Signumf(Argument)`                     | record   | unary `Argument` beside `IUnaryNode.NodeChild` |
|  [08]   | `Entity.CalculusOperator(Expression, Var)`                               | abstract | base of the three deferred-analytic residues   |
|  [09]   | `Entity.Derivativef(Expression, Var, int Iterations)`                    | record   | deferred derivative with its order             |
|  [10]   | `Entity.Integralf(Expression, Var, (Entity from, Entity to)? Range)`     | record   | deferred integral with its optional bounds     |
|  [11]   | `Entity.Limitf(Expression, Var, Destination, ApproachFrom ApproachFrom)` | record   | deferred limit with direction                  |
|  [12]   | `Entity.Providedf(Expression, Predicate)`                                | record   | one regime case                                |
