...
lazy from simple_idml import idml  # [RESEARCH] manifest/catalogue candidate; the proxy reifies on first `_mutate` use in the to_process worker

if TYPE_CHECKING:
    from simple_idml.idml import IDMLPackage  # [RESEARCH] worker-side package type; `_resolved` reads `xml_structure` off it
