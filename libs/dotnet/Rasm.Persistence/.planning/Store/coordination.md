...
|  [08]   | signal row      | fenced `(workflow, channel)` upsert           | `SignalPut`/`SignalLoad` decode `StepStateSeam.SignalPut`/`SignalOf` |
|  [09]   | throw crossing  | one `CoordinationFault.Lift` per rail leg     | banded case on every `Fin`; a bare `Error` is the deleted form       |
