...
[STACKING]:
- with `VividOrange.Cases` (`.api/api-vividorange-cases`): `NationalAnnex` keys every `ENLoadCaseFactory`/`ENCombinationFactory` entry and every `ITableA1_1`/`ITableA1_2.GetProperties(NationalAnnex)`, so a national deviation is a table row; the `Cases` EN `ψ`/`γ` singletons throw `NotImplementedException` for an uncovered annex, and Bim preserves that or `MissingNationalAnnexException` exactly through `Try.lift`
- with `VividOrange.Countries` (`.api/api-vividorange-countries`): `Country` (249 ISO nations) is the broad national axis and `NationalAnnex` the narrow Eurocode-parameter axis; both meet on `Model/eurocode#EUROCODE_ALGEBRA` `AnnexRegime` roster, whose `Of(ICountry)` keys on `CountryCode` and lands `RecommendedValues` off-roster — no compiled `Country`→`NationalAnnex` member exists and name equality is unsound, both stated there; the abbreviation table is internal to `VividOrange.Standards`
...
- with `StructuralAnalysisFormat` (`.api/api-structural-analysis-format`): all 37 `NationalAnnex` members hold exactly one `ExcelNationalCode` `EC_*` member (`RecommendedValues`→`EC_Standard_EN`, `Belarus`→`EC_TKP_EN`), carried as the `AnnexRegime` row's own KEY so `Exchange/saf#SAF_EXCHANGE` parses `ExcelModelInformation.NationalCode` off the roster; the abbreviation does NOT stem that token (`Ткп`/`БДС`/`ΕΛΟΤ` non-Latin, `ILNAS` vs `EC_LU_EN`, Singapore/Sweden colliding on `SS`)
- with `Thinktecture.Runtime.Extensions` (`libs/dotnet/.api/api-thinktecture-json.md`): the Bim layer owns a `[SmartEnum]`/`[ValueObject]` keyed by standard identity, and `NationalAnnex`/`StandardBody` are the boundary vocabulary mapped onto it at ingest, never re-exported as the canonical shape
- with `LanguageExt.Core`: `MissingNationalAnnexException` and the `Cases` `NotImplementedException` are captured by `Try.lift` as exact exceptional errors, so the Eurocode dispatch never throws across the Bim fold or remints a message

[LOCAL_ADMISSION]:
- `NationalAnnex` is the canonical Eurocode dispatch key the Bim design layer carries on the `Model/eurocode#EUROCODE_ALGEBRA` `AnnexRegime` row; `RecommendedValues` is the EN fallback when no national annex applies, the `AnnexRegime.Recommended` landing for a non-CEN nation and never a fault
