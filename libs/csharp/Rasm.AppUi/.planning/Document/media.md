...
- Owner: `MediaSurface` the `[Union]` codec-row family; `PlaybackPolicy` the playback operating envelope with `LoopMode` its repeat vocabulary; `MediaLease` the control-plus-native lifetime capsule; `MediaReceipt` the materialization evidence.
