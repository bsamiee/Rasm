...
public sealed class ExtractMesh : Component {
    public ExtractMesh() : base(self: typeof(ExtractMesh)) { }
    ...
    [Output] private IOutputGroup NakedEdges => Output.Query(input: Geometry, port: Port.List<Polyline>(name: "Naked Edges", code: "NE", info: "Connected boundary polylines via Mesh.GetNakedEdges(); empty when the mesh is closed."), aspect: Rasm.Analysis.Boundaries.Naked);
    [Output] private IOutputGroup Pieces => Output.Query(input: Geometry, port: Port.List<Mesh>(name: "Pieces", code: "P", info: "Disjoint mesh components via Mesh.SplitDisjointPieces(); a single-element list for a connected mesh."), aspect: static _ => Rasm.Analysis.Query.Components<object, Mesh>());
    [Output]
    ...
    private IOutputGroup IndexedFace => Output.Details<MeshFaceProjection>(
        input: Geometry,
        aspect: runtime => shape => Rasm.Analysis.Query.Meshes<object, MeshFaceProjection>(aspect: Meshes.AtFace(index: runtime.Read(port: FaceIndex).ToNullable())).Apply(geometry: shape.Inner),
        emptyUnsupported: false,
