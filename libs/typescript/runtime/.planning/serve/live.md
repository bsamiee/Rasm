...
- Law: neither lane resolves a tenant, so `tenancy` rides this lead rather than a column — `Edge.admission`'s one credential lift binds `TenantScope` around the dispatch and `[06]`'s roster read filters on that bound scope, whatever consumption row the root supplied.
...
- Boundary: which feeds exist and who attaches is `[06]`'s admission; the inbound SSE parser is `net/channel#FEED_SESSION`'s — this endpoint only emits.
...
- Law: `serve/route#LAYER_ROUTES` composes this same member for the tus server's node dispatchers, so the adapter has one spelling and `Router.IngestMount` keeps the Layer assembly — prefix routing and groom scheduling — that no lift decides.
...
- Law: the Connect row continues NO context of its own — `Edge.guard` already ran `Current.traced` over the same HTTP headers this adapter reads, above the catch-all and before the handler, so the inbound W3C hop crossed once already; a Connect-side `Carrier.extract` interceptor mints a SECOND continuation of one hop, which is the two-trace law's forbidden fold rather than its admitted pair. Egress print is `core:interchange/invoke#DIAL_AXIS`'s, so no interceptor pair rides this row in either direction.
...
- Law: `contextValues` is the EARLIER boundary and the one a principal crosses — it runs against the raw node request before any handler dispatch, so the credential and tenancy `Edge.admission` already bound reach `HandlerContext` there rather than through an interceptor that runs a decode later.
