...
Each axis crosses under the one admission gate, and no `planned` receipt is emitted for a crossing that did not clear its ceiling. C# crossing stays outward-only: compute graduates `→` `dotnet:Rasm.Compute`; C# never imports back.
