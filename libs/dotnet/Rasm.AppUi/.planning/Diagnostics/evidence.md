...
- Law: the domain union SURVIVES beside the generated message on a named discriminant — total in-process dispatch. `Lower` is the union's generated total `Switch`, so a twenty-first domain case breaks the build at the boundary; `Admit` is exhaustive over the generated `KindOneofCase`, refusing `None` (an unset arm, the only value a parse of a newer producer's unknown arm can yield) and an undefined ordinal on the result; `Probe` proves the seat roster, the union, and the descriptor bijective at boot, so a twenty-first WIRE arm the corpus grows fails the composition that forgot its case.
...
public static class EvidenceOps {
    ...
    public static Option<JsonSerializerOptions> Wire { get; set; }
}
...
public static class AppUiTelemetry {
    ...
    static readonly Seq<Func<Fin<Unit>>> Rosters =
        [QualityTier.Proof, TypographyRole.Proof, MeasureRole.Proof, PhaseMotion.Covered];
    ...
    public static Fin<InstrumentSet> Mount(
        ...
        from _rosters in Rosters.Traverse(static proof => proof().ToValidation()).As().ToFin()
...
flowchart LR
    ...
    AppUiFact -->|"HookSet.Fire"| SurfaceRuntime
