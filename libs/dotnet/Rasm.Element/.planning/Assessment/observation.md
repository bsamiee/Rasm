...
public readonly partial record struct ObservationChunk(Interval Window, ArtifactContent Series, int SampleCount) {
 ...
 private static Fin<Seq<(Instant At, double Si, ObservationGrade Grade)>> Read(ReadOnlyMemory<byte> blob) {
  ...
  if (span.Length < sizeof(int)) { return Truncated("count-prefix"); }
  int declared = BinaryPrimitives.ReadInt32LittleEndian(span);
  int cursor = sizeof(int);
  if (declared < 0 || declared > (span.Length - sizeof(int)) / RecordStride) { return Truncated("count-claim"); }
  (Instant, double, ObservationGrade)[] run = new (Instant, double, ObservationGrade)[declared];
  for (int index = 0; index < declared; index++) {
   if (cursor + RecordStride > span.Length) { return Truncated("record"); }
   ...
   cursor += RecordStride;
   if (length < 0 || cursor + length > span.Length) { return Truncated("token"); }
   ...
   if (!ObservationGrade.TryGet(token, out ObservationGrade? grade) || grade is not { } admitted) {
    return new KernelFault.InvalidValue("observation-grade", $"resolve {token}");
   }
   run[index] = (Instant.FromUnixTimeTicks(ticks), si, admitted);
  }
  return cursor == span.Length ? Fin.Succ(toSeq(run)) : Truncated("tail");
 }

 private static Fin<Seq<(Instant At, double Si, ObservationGrade Grade)>> Truncated(string stage) =>
  new KernelFault.InvalidValue("observation-chunk", $"contain a complete {stage}");
}
...
public sealed partial record ObservationSeries {
 ...
 public Fin<MeasureValue> Value(double si) =>
 ...
 // --- [ADMISSION_GATES] ----------------------------------------------------------------
 private static Validation<Error, Unit> Named(QuantitySignature quantity) =>
  Gate(quantity.CanonicalUnit.Exists(static unit => !string.IsNullOrWhiteSpace(unit)), "observation-unit",
   static (label, op) => (Error)new KernelFault.InvalidValue(label, "carry a non-blank canonical unit"));

 private static Validation<Error, Unit> Cadenced(Option<Duration> cadence) =>
  cadence.Traverse(span => In(span.TotalSeconds, Band.Positive, "observation-cadence-seconds")).As().Map(static _ => unit);
...
- Entry: `SeriesStatistics.From(run, sampling, quantity)` derives the whole summary off one decoded sample run through ONE weighted `MeasureStat.Of` fold — the ONE owner of the register-span rule and of the monotone-run refusal that rule presupposes; `Of(census, span, figures, total)` takes an already-composed column set RESULT-RETURNING on the same `IsCoherent` law the series transitions re-impose; `Fold(left, right)` merges two ADJACENT summaries — `Figures` through the kernel `Stat.Merge` Pebay join, `Total` additively, the census key-wise, the span as covered duration — the ONE merge every rollup composes; `Minimum`/`Maximum`/`Mean` are one-hop reads over `Figures`, so every pre-kernel consumer column survives verbatim; `Observed` sums the census and `Consumable` folds it over the `GradeCapability.Consumable` axis; `Completeness(expected)` reads the consumable share against a cadence-derived denominator (`None` where the denominator is unanswerable); `Representative(sampling)` reads the ONE comparable figure the sampling row designates — the mean for an averaged or instantaneous stream, the total for a totalled or cumulative one, the extremum for a min or max one.
...
public readonly record struct SeriesStatistics(
 ...
 public static Fin<SeriesStatistics> Of(
  ...
  new SeriesStatistics(census, span, figures, total) is { IsCoherent: true } admitted
   ? Fin.Succ(admitted)
   : new ElementFault.ValueRejected("<observation-statistics-incoherent>");

 public static Fin<SeriesStatistics> From(
  Seq<(Instant At, double Si, ObservationGrade Grade)> run, SamplingKind sampling, QuantitySignature quantity) {
  ...
  if (sampling.Capabilities.Admits(SamplingCapability.Monotone) && run.Zip(run.Tail).Exists(static pair => pair.Item2.Si < pair.Item1.Si)) {
   return new ElementFault.ValueRejected("<observation-register-decreasing>");
  }
  ...
  return MeasureStat.Of(quantity, run.Map(static sample => sample.Si),
 ...
 private static Fin<MeasureValue> Figure(Option<MeasureValue> value, string column) =>
  value.ToFin(new ElementFault.ValueRejected($"<observation-representative-absent:{column}>"));
}
```
