...
- `polars`(`.api/polars.md`) / `pyarrow`(`.api/pyarrow.md`): `to_pyarrow()`/`to_pyarrow_batches()`/`to_polars()` hand a result to the Arrow columnar layer directly, and `Schema.from_polars`/`from_pyarrow`/`from_pandas` cross a foreign schema in with `to_*` back — the single typed boundary, never rebuilt from raw dtype strings
- within-lib: `tabular/query#QUERY` is the one member-level consumer — the `QuerySpec.Ir` arm carries an `IbisTable` frontend whose `_ir` cell opens `ibis.connect(uri)` or `ibis.duckdb.connect()`, binds through `ibis.parse_sql(text, catalog, dialect)`, emits `ibis.to_sql(expr, dialect)` for both the DuckDB Substrait `CALL` and the query result's column-level lineage edges, and releases through `backend.disconnect()`; `ibis.IbisError` is one of the disjoint exception roots its broad envelope catch exists for. `tabular/interop#INTEROP` composes the CONTRACT alone — `Backend.IBIS` is a narwhals lazy-axis row over `to_native()`, never an ibis member — and `tabular/profile#PROFILE` reaches the same backend transitively through pointblank's narwhals engine.
...
[LOCAL_ADMISSION]:
- import `ibis` at boundary scope; the branch admits it as the portable relational expression IR and multi-backend query frontend over the shared columnar layer
