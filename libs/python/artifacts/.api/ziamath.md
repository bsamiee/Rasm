...
`drawon` is the composition boundary: a diagram or document owner threading its own `xml.etree` tree gets the equation drawn in place and the inserted `<g>` back, no string concatenation or temp file; `svg()` is the durable string artifact the content-key records and downstream rasterizers consume.
...
`config.numbering.autonumber` drives a global counter: every `Math`/`Latex` takes the next number formatted by `format` (or a `format_func` callback for roman or section-relative labels), right-aligned to `columnwidth`. `\tag{label}` or the `number=` constructor argument overrides one equation; `reset_numbering` resets the counter at each numbered-section boundary.
...
- config is global: one `Config` singleton owns house math/text style, numbering, debug overlays, and `svg2`/`precision` egress, inherited wherever a constructor argument is `None`; `config.svg2` matches the embedded equation to the host `drawsvg`/PDF SVG profile via `ziafont.config.svg2`. Because it is process-global, a per-render profile is a bracketed set-and-restore under one reentrant lock, never a set-once at module init: two concurrent renders wanting different profiles otherwise race, and the loser ships an equation the host cannot embed.
- `declareoperator` REBINDS `latex2mathml.commands.FUNCTIONS` — measured, `FUNCTIONS = FUNCTIONS + (name,)` over a process-global TUPLE, never an in-place append. Per-render operator vocabularies are therefore a snapshot of that binding and a `finally` rebind; the same discipline over a list aliases the mutated object and restores nothing, leaking one formula's vocabulary into every sibling parse and poisoning the content-addressed cache, so the tuple identity is the load-bearing fact. Each render captures whatever ziamath's own import-time `declareoperator` calls left as the baseline, not the bare latex2mathml roster, reading it per render rather than freezing once.
...
- offload: the GIL-bound lxml-free kernel — `xml.etree` parse, MATH-table font walk, global `config` read/mutate, SVG serialization — runs through the runtime `to_thread.run_sync(..., limiter=...)` boundary, the same arm the `drawsvg` emitter and GIL-releasing `rustworkx` layout take, never a `to_interpreter` subinterpreter arm (the global mutable `config` and C-extension `ziafont`/`numpy` neighbors are subinterpreter-hostile).
...
[LOCAL_ADMISSION]:
- `zm.Latex(formula).svg()` renders a formula, or `zm.Math(...).drawon(tree, x, y, halign, valign)` when sharing one `ET.Element` tree — never a hand-built MathML->SVG layout where the render trio owns it.
- mixed text-and-math is `zm.Text(...)` and a single expression is `zm.Latex(...)`; font, style, and numbering ride the constructor where the render trio exposes them, and the two process-global surfaces the trio does not — `config.svg2` and the `declareoperator` vocabulary — are set and restored per render inside one lock, never registered once at module init where a second caller inherits the first's profile and operator set.
