...
- `Band.GetBlockSize` maps to the seam `OverviewLevel.Block` optional pair on every level including the run's base head; a zero or strip extent is ABSENCE there, never a declared tile size of nothing. `GetOverview(int)`/`GetOverviewCount` supply the decreasing-resolution level set.
