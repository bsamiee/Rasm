...
public sealed partial class BasemapSurface {
    ...
    static Fin<Map> Candidate(Seq<BasemapLayerRow> rows, ResolvedTheme theme) {
        Map candidate = new();
        return rows.Fold(Fin.Succ(candidate), static (acc, row) => acc.Bind(map => Mount(map, row)))
    ...
    Fin<Map> Landed(MapSeat settled, Map candidate, ChartInk ink) {
        ...
        return Instruments.Write(Layers, 1d).Map(_ => candidate);
    }
    ...
    public async IAsyncEnumerable<TileHealth> Watch([EnumeratorCancellation] CancellationToken token = default) {
        ...
        try {
            ...
            await foreach (TileHealth health in pump.Reader.ReadAllAsync(token)) {
                ignore(Instruments.Write(Health, 1d, InstrumentSet.Tags(
                    (AppUiTelemetry.OutcomeSlot, health.State.Key),
                    (AppUiTelemetry.CauseSlot, health.State.Recovery.Key))));
                yield return health;
            }
        }
        finally {
...
public readonly record struct BasemapHit(
    string FeatureId, Option<string> Layer, double WorldX, double WorldY, Option<double> Distance);

public readonly record struct BasemapHover(BasemapHit Top, int Ambiguity);

// --- [OPERATIONS] ----------------------------------------------------------------------
public sealed partial class BasemapSurface {
    public static readonly ArtifactKind Artifact = ArtifactKind.Create("basemap");

    public Fin<Seq<BasemapHit>> Pick(ScreenPosition screen, PickPolicy policy) =>
        Optional(Control.GetMapInfo(screen, Control.Map.Layers))
            ...
            .Match(
                ...
                None: static () => Fin.Succ(Seq<BasemapHit>()));
    ...
    static Seq<BasemapHit> Nearest(MapInfo info, MPoint world, PickPolicy policy) {
        ...
        Seq<BasemapHit> hits = toSeq(info.MapInfoRecords)
            ...
            .Strict();
        (Seq<BasemapHit> Measured, Seq<BasemapHit> Unmeasured) split =
...
- Law: `MercatorFilter.Inverse` is admitted on its own domain (folder RULINGS `[02]`) — the cylindrical axis WRAPS at the antimeridian because the antimeridian names a place, and the mercator clamp REFUSES because the asymptote names none.
...
public readonly record struct RedlineCommit(
    EditIntent Intent, RevertCursor Next, GaugedSpan<RedlineLane> Span);
...
public sealed class RedlineSurface(
    ...
    public IO<Fin<Option<RedlineCommit>>> Drive(
        RedlineVerb verb, RevertCursor cursor, MonotonicTimeline line) =>
        Observe(verb).Match(
            Succ: _ => Opened(verb).Match(
                Succ: session => verb.Switch(
                    ...
                    delete: static (scope, command) => scope.Surface.Durable(
                        ...
                        command.DocKey, command.TargetId, scope.Session.Before, cursor),
                    commit: static (scope, commit) => scope.Surface.Durable(
                        RedlineLane.Commit, line,
                        () => scope.Surface.Sealed(commit, scope.Session),
                        commit.DocKey, commit.TargetId, scope.Session.Before, cursor),
                ...
                Succ: intent => History
                    .Record(
                        new RevertibleOp(
                            ...
                            Stamp()),
                        cursor, Hooks)
                    .Map(recorded => recorded.Map(next =>
                        Some(new RedlineCommit(intent, next, gauged.Span)))),
                Fail: error => IO.pure(Fin.Fail<Option<RedlineCommit>>(error))),
            Fail: error => IO.pure(Fin.Fail<Option<RedlineCommit>>(error)));
