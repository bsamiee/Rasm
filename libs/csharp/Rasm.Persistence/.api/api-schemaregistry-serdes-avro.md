...
- Field-level encryption rides the shared `RuleRegistry`: a `DomainRule` of `RuleMode.WriteRead` naming a field-encryption `IRuleExecutor` (Avro `bytes`/`string` fields tagged in the schema `Metadata` sensitive-field set) wraps/unwraps per-field DEKs against the `Element/identity#KEY_ENVELOPE` `EnvelopeKeyring` KMS clients (the `KmsProvider` axis owned at `Element/identity#AUTHORITY`) during `SerializeAsync`/`DeserializeAsync` — the same rule engine the Protobuf and Json serdes use, never an Avro-specific crypto pass.
