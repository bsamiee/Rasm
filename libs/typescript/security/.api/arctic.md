...
- package: `arctic` (3.7.0, MIT, © pilcrowOnPaper)
...
| :-----: | :-------------------------------------------------------------------------------------------------- | :-------------- | :--------------------------------------------------------------- |
|  [01]   | `new OAuth2Client(clientId, clientPassword \| null, redirectURI \| null)` / `new <Provider>(clientId, clientSecret, redirectURI)` | construct | `authn/oauth` — provider ctor is the row; `OAuth2Client` is the fallback for an unlisted issuer |
