...
_SCAN_READER: Final[frozendict[str, str]] = frozendict({
    "csv": "scan_csv",
...
if TYPE_CHECKING:
    # `polars` is on `banned-module-level-imports`, so the lazy arms bind `pl` function-local;
    # the type-only import keeps `LazyFrame`/`Expr`/the module handle precise without a runtime load.
    import polars as pl
...
class ExcelSpec(Struct, frozen=True):
    ...
    dtypes: frozendict[str | int, ExcelDType] | None = None
...
def _run(plan: ScanPlan, dataset: DatasetRef) -> pa.Table:
    ...
    match plan:
        ...
        case ScanPlan(tag="duckdb", duckdb=(sql, projection)):
            ...
            # `to_arrow_table` has eagerly materialized the relation inside the bracket.
...
- Packages: `deltalake` (the `load_cdf` change feed returning an `arro3.core.RecordBatchReader` with the `_change_type`/`_commit_version` CDF columns, composed through `lakehouse`), `duckdb` (the partition-delta recompute, composed through `query`), `arro3-core` (the `RecordBatchReader.read_all` drain to an `arro3.core.Table` the `pa.table(...)` re-import lifts zero-copy), `pyarrow` (`table(...)` the PyCapsule re-import, `Table.group_by(keys).aggregate([])` the distinct partition-tuple rows, `compute.field`/the `Expression` `==`/`&` operators the `_key_mask` conjunction folds, `Table.filter` the per-partition isolation, `combine_chunks` the `_arrow_bytes` whole-table `combine_chunks().to_batches()[0].serialize()` content-key source shared with the scan owner), `expression` (`Block.of_seq` the per-partition rail collection), stdlib `operator`/`functools` (`reduce(operator.and_, ...)` the composite-key predicate conjunction), `anyio` (`create_task_group` the failure boundary the independent per-partition recomputes run under and `create_memory_object_stream` the bounded inbox their rails stream into, never `asyncio.gather`), runtime (`ContentIdentity`/`ContentKey`/`RuntimeRail`/`BoundaryFault`/`async_boundary`/`traversed`/`Disposition`/`ReceiptContributor`, the awaitable `async_boundary` fault fence the materialization rides and the `traversed(by=Disposition.ABORT)` fold collecting the per-partition `RuntimeRail[PartitionBundle]` rails).
...
class DerivedSnapshot(Struct, frozen=True):
    ...
    async def _gathered(self, deltas: tuple[tuple[pa.Table, str], ...]) -> "Block[RuntimeRail[PartitionBundle]]":
        ...
        # shared mutable list; `traversed(ABORT)` downstream is order-independent (`_merge` keys by partition).
...
- [CORPUS_WIRE]: SETTLED — the `Corpus` arm is the data-side endpoint of the `tabular ← python:artifacts/documents [WIRE]` seam (`data/ARCHITECTURE.md` L40), consuming the artifacts-owned flat-record projection at the wire — a sequence of `Mapping[str, Any]` rows — and lifting it through the catalogue-confirmed `pyarrow` `Table.from_pylist` row [03][06], then carrying it zero-copy through the settled `interop#INTEROP` `ArrowCStream` PyCapsule carrier. The exact column shape is now confirmed against the authored producing endpoint: the `python:artifacts/documents` `model#NODE` owner exports `to_corpus_record(node) -> dict[str, object]` (the `msgspec.to_builtins` lowering of its typed `CorpusRow` Struct), whose seven keys `key`/`kind`/`role`/`page`/`text`/`alt`/`alt_status` and native scalar dtypes fix the `from_pylist` Arrow schema as `key`/`kind`/`role`/`text`/`alt`/`alt_status` Arrow `string` and `page` Arrow `int64` (the `kind`/`alt_status` `StrEnum` members lowering to their string values) — reflection-confirmed that `pa.Table.from_pylist` over the raw `CorpusRow` `msgspec.Struct` instances raises `AttributeError` (a `Struct` exposes no `.keys()`), so the producer owns the flat-`dict` projection and the `Corpus` arm consumes `to_corpus_record` mappings rather than the typed Struct, the column shape agreed at the seam with no consumer re-derivation. The `pa.Table.from_pylist` lift and the `ArrowCStream` carrier are settled fence code; the `Corpus` arm never re-mints the document recovery, and the content-key consumption is a separate runtime ripple, not conflated with this WIRE seam.
- [CDF_PARTITION_GROUP]: SETTLED — the `deltalake` `DeltaTable.load_cdf(starting_version=, ending_version=)` reader and its `_change_type`/`_commit_version` CDF columns the `_materialize` partition-grouping reads are catalogue-confirmed against the folder `deltalake` `.api`, where `load_cdf` returns an `arro3.core.RecordBatchReader`; `_materialize` drains that reader through `.read_all()` to an `arro3.core.Table` and zero-copy re-imports it into a real `pa.Table` through `pa.table(...)` over the Arrow PyCapsule C-stream — confirmed live against `arro3.core` `0.8.1`/`pyarrow` `23.0.1` that `arro3.core.Table` exports `__arrow_c_stream__` and `pa.table(arro3.Table)` succeeds, the load-bearing hop the prior direct-`pc`-on-arro3 form skipped (the arro3 reader exposes no `pyarrow.compute` surface). A non-Delta source returns `Error(BoundaryFault(boundary=(...)))` directly (the `lakehouse#LAKEHOUSE` `_PORTABLE` row proves `changefeed` is Delta-exclusive), never a `raise ValueError` into the fence re-key that discards the typed key. The arm filters the re-imported CDF through `cdf.filter(pc.field("_change_type") != "delete")` so a deleted partition never re-keys, and groups the touched frame by the full `partition_by` tuple through `Table.group_by(partition_by).aggregate([])` (the empty-aggregation distinct-key rows), each delta isolated by the `_key_mask` `reduce(operator.and_, pc.field(col) == value)` conjunction over every partition column — never the first column alone, which would collapse a composite partition into one bundle. `Table.group_by(keys).aggregate([])`, the `pc.field(col) == value` `Expression` operators, `Table.filter(Expression)`, and `RecordBatch.serialize` are confirmed live. The railed `ContentIdentity.of` flows correctly: the `async` `_recompute` returns `RuntimeRail[PartitionBundle]` keyed off `_arrow_bytes`, the per-partition rails gathered concurrently through `_gathered` (one `anyio.create_task_group()` plus a bounded memory-object-stream, order-independent because `_merge` keys by partition) and folded through `traversed(..., by=Disposition.ABORT)`, and `snapshot_key` returns `RuntimeRail[ContentKey]` over the Merkle child tuple — none collapses a `RuntimeRail[ContentKey]` into a bare field. The `query#QUERY` `QueryEngine.run` is a coroutine (it offloads every blocking leg to the `anyio` worker pool), so `_recompute` `await`s it and the whole `refresh`/`_materialize` chain is `async` fenced by `async_boundary` — the prior synchronous `run(...).default_value(...)` call on the coroutine object is the deleted form; the `(await QueryEngine.of(...).run(...)).default_value(delta)` unwrap is the intentional reuse-on-fault policy the refresh boundary owns: a query fault reuses the prior delta rather than propagating. The `DerivedSnapshot` composes the `Lakehouse` owner for the `table_uri` identity and reads the CDF through the one `load_cdf` surface the `lakehouse` `ChangeFeed` op binds, never a second CDF differ.
