...
`VividOrange.IStandards` (contract) is the Eurocode standards-identity taxonomy and, above all,
the owner of the `NationalAnnex` dispatch key the rest of the VividOrange structural family reads.
...
enum (37 members: `RecommendedValues` — the EN recommended fallback — plus 36 national annexes) that
...
[LOCAL_ADMISSION]:
- `NationalAnnex` is the canonical Eurocode dispatch key the Bim design layer carries; `RecommendedValues` is the EN recommended fallback when no national annex applies (the default the `Countries` bridge resolves to for a non-CEN nation)
