...
One cluster: `FORM_BINDING` owns the schema-driven form-validity fold and the per-field ARIA error projection.
...
- Entry: a form composes one `FormBinding<A>` over its `Schema.Struct`; the field controls compose the `role-behavior.md` `inputs` rows; the submit decodes once and either renders the projected error map or crosses the `interchange` `CommandGateway`; the form holds no domain state and reads any prefill default through the `binding/atom-binding.md` `AtomBinding`.
