...
`pyvips` owns the fused streaming-raster engine the `graphic/raster/io#IO` `RasterEngine.LIBVIPS` arm and the `graphic/color/managed#MANAGED` `_icc_apply` arm compose: one lazily-evaluated `Image` whose factory family loads a source, whose generated libvips-operation methods transform it demand-driven, and whose `write_to_*`/`numpy` egress computes the whole pass exactly once. Every arm crosses the `runtime/execution/lanes#LANE` process seam through its `lane.offload(Kernel.of(_worker, KernelTrait.HOSTILE), ...)` worker body — pyvips admits BARE as a source API-mode build against the Forge `vips` (`no-binary-package` pins the sdist), so API mode compiles the full operation surface the machine libvips links — HEVC/HEIF, JXL, EXR, the float lanes — and it rides `WORKER_BAND` like the other host-native packages — captures `pyvips.Error` into `RasterFault.engine` and the dlopen `OSError` of an unprovisioned `libvips` into `RasterFault.provision`, folds one `msgspec` `RasterFact`/`ManagedFact`, and projects `core/receipt#RECEIPT` `ArtifactReceipt.Preview`. It is the fused-pipeline / large-image / managed-ICC-egress owner where `pillow` (`.api/pillow.md`) is the in-process pure-Python working surface.
...
- [03]-[MINT]: `new_temp_file(format)`; `copy_memory()`; `black(width, height, bands)`; `text(text, *, font, dpi, width, wrap)`; `sdf(width, height, shape, *, r, a, b, corners) -> Image` — `shape` is POSITIONAL (a keyword-only call raises `sdf needs 3 arguments`).
...
- [01]-[THUMBNAIL]: `thumbnail_source(source, width, *, height, size, crop, no_rotate, input_profile, output_profile, intent, linear, fail_on, option_string) -> Image`; `thumbnail_image(width, *, height, size, crop) -> Image` — the profile kwargs spell `input_profile`/`output_profile` on the live binding.
- [02]-[RESAMPLE]: `resize(scale, *, vscale, kernel, gap) -> Image`.
- [03]-[REDUCE]: `shrink(hshrink, vshrink)`; `reduce(hshrink, vshrink, *, kernel) -> Image` — BOTH take float factors and land the requested extent at any ratio (measured: `reduce` is byte-identical to `resize` per ratio); an integer box `shrink` matches a box-integral at float32 epsilon while every other libvips reduction diverges from a hand integral by percents, so engine selection is a declared request axis, never an equivalence assumption.
...
- [16]-[JXL_COLOUR]: `jxlload`/`jxlload_buffer` publishes `icc-profile-data` on EVERY JPEG XL source, whether or not the file embeds a profile — libjxl parses the codestream's colour-encoding bundle and SYNTHESIZES an ICC v4.4 profile from it, tagged `['desc', 'cprt', 'wtpt', 'chad', 'cicp', 'rXYZ', 'gXYZ', 'bXYZ', 'A2B0', 'B2A0']`. The `cicp` tag carries the codestream's own H.273 transfer and primaries codes, and `ProfileDescription` names the encoding (`Rec2100PQ`). Measured across `linear`, `srgb`, `pq`, and `hlg` at uint16 and float16: the tag is present at every one, so this is the ONE surface that opens a `jxlc` codestream's colour declaration — a box walker over the same file reads `ftyp`/`jxll` and returns nothing colour-shaped. The blob is opaque here; parsing the `cicp` tag inside it is `pyexiftool`(`.api/pyexiftool.md`).
...
`pyvips.base` module-level functions tune the one shared libvips runtime the whole process binds — boundary-init calls read once at worker startup, never per-op knobs. `VIPS_CONCURRENCY` bounds libvips's own internal intra-op thread pool, which the worker owner sets DOWN so the outer `WORKER_BAND` process fan and the inner libvips pool do not oversubscribe the host — it is an init-time ENV read, so a worker sets it before its first `pyvips` import and never per op; `cache_set_max*` and the untrusted-loader block (`block_untrusted_set`/`operation_block_set`) are the throughput and security controls, and `get_suffixes`/`type_find`/`at_least_libvips`/`version` the build-capability probes.

| [INDEX] | [SURFACE]                                  | [CAPABILITY]                                                                        |
| :-----: | :----------------------------------------- | :---------------------------------------------------------------------------------- |
|  [01]   | `VIPS_CONCURRENCY` env var                 | bound libvips's internal intra-op thread pool; set BEFORE the first `pyvips` import |
...
- [01]-[CONCURRENCY]: `os.environ["VIPS_CONCURRENCY"] = "1"` before the first `import pyvips` — libvips reads it at library init; the 2.x `pyvips.concurrency_set`/`concurrency_get` pair does NOT exist on this binding and a call raises `AttributeError`.
...
- egress: `write_to_buffer(format_string, **kwargs)` keys the encoder by `format_string` (`".png"`/`".jpg"`/`".webp"`/`".tif"`), and that ONE suffix literal is simultaneously the call spelling and its own `get_suffixes()` key — the `graphic/raster/io#IO` `CODEC` row's libvips writer column carries it, and the `graphic/color/managed#MANAGED` egress resolves through that same row rather than a second option table. Encoder options (`Q`/`compression`/`lossless`/`effort`/`subsample_mode`/`bitdepth`, `keep=ForeignKeep` for EXIF/ICC/XMP) ride `**kwargs` per the row's own `(quality, effort)` option builder. Saver capability reads off `get_suffixes()` per build, never a remembered roster — the API-mode build registers `.bmp` and `.jxl` savers (`.qoi` none), yet `jxlsave` refuses 2- and 4-band images MID-WRITE while passing a 1×1 3-band trial, so a writer column probes by memoized trial call over its real band shape and degrades to the `imagecodecs` array writer on refusal.
- saver capability: `get_suffixes()` reports REGISTRATION, never encode capability. `heifsave` serves the whole delegating HEIF family — `.heic`, `.heif`, and `.avif` all route through libheif under `ForeignHeifCompression` — so the suffixes appear on a build whose libheif links no encoder for that compression and `write_to_buffer(".heic")` then raises `pyvips.Error: heifsave: Unsupported compression` MID-WRITE, where the lazy pipeline has already paid the whole decode and a guard mapping `pyvips.Error → RasterFault.engine` records a capability gap as a content fault. Every delegating saver therefore probes by one-shot trial write of a 1×1 sRGB image, memoized per suffix per process; `.avif` passing that trial never proves `.heic` does, because each compression row links its own backend. `write_to_target` streams to a `Target`/`TargetCustom`; `dzsave`/`dzsave_buffer` emits a DeepZoom pyramid; `numpy(dtype=)`/`write_to_memory` extract raw pixels. Egress computes the pipeline exactly once, so the whole fused decode+orient+ICC+flatten+encode runs in one streamed pass.
...
- runtime: `VIPS_CONCURRENCY` bounds libvips's own internal thread pool DOWN, read at library init, so a worker exports it BEFORE its first `pyvips` import and does not oversubscribe the host against the outer `WORKER_BAND` `to_process` fan (the two-pool guard the `graphic/raster/io#IO` page names); `cache_set_max_mem`/`cache_set_max` bound the operation cache; `block_untrusted_set(True)`/`operation_block_set(...)` harden the loader set for hostile uploads; `set_progress`/`set_kill` give a long evaluation a cancellation hook.
...
- `graphic/texture/plane#CODEC`: composes the JXL colour leg ALONE — `new_from_buffer(payload, "")`, the `get_typeof("icc-profile-data")` presence gate, and `get` for the synthesized profile bytes it hands to `pyexiftool`. No pixel crosses that seam: the deep-pixel resample lane stays `graphic/texture/derive#DERIVE`'s, and an unprovisioned `libvips` surfaces there as the dlopen `OSError` that resolves the readback to the row's declared tag rather than to `RasterFault.provision`, because that page's carrier holds no provision case and an unreadable declaration is not a pixel fault.
