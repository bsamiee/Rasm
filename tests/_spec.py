...
def de_eq(*, delta: float | None = None) -> Callable[[float, float], bool]:
    """Build an approximate-equality `eq` callable via `dirty_equals.IsApprox` for oracle `eq` kwargs.

    Args:
        delta: Absolute tolerance; None uses IsApprox's relative default.

    Returns:
        Two-arg predicate `left == IsApprox(right, delta=delta)`. IsApprox.__eq__ swallows
