...
- [02]-[TABLE]: the discovered item collection encoded as a `stac-geoparquet` columnar Arrow `RecordBatchReader` the `tabular/columnar` scan and `tabular/query` engine consume, with the GeoParquet/NDJSON/Delta sink rows split across `stac_table_egress` (table-in writes) and `stac_table_direct` (one-call source-to-disk fast-paths), and the RE-HOMED `StacGeoClaim`/`StacIngest` claim wrapper — the STAC-NDJSON interchange claim folding `ToArrow`/`ToDelta`/`Rehydrate` over the co-located `stac_table*` entrypoints in one `QueryReceipt`/span/`apply_remote` retry rail (the former geospatial-tier claim, homed with the encode it composes so the catalog⇄geospatial cycle is dead).
- [03]-[ASSETS]: the one awaitable `AssetFold` over the signed `ItemCollection` discriminating asset role by `MediaType` — the `tabular/egress` `ObjectEgress.GetRange` byte-window read, the `gridded/virtual` `VirtualReference.apply(VersionOp(aggregate=(...)))` virtual-chunk registration over the `gridded/virtual#MANIFEST` `FieldVirtual` manifest, the `odc-stac` `odc.stac.load` COG datacube under the `guarded(RetryClass.HTTP, ...)` envelope driven by the `pystac.extensions`-derived `RasterGeoClaim`/`stac_cfg`/`patch_url`, the one railed `StacDiscovery` re-key.
...
class StacGeoClaim(Struct, frozen=True):
    # the RE-HOMED STAC-NDJSON interchange claim: it binds THIS module's `stac_table*` entrypoints
    # directly — the former flat-module-path self-import and the
    # geospatial-side lazy `# noqa: PLC0415` cycle dodge are both dead by ownership.
    ...
    async def apply_remote(self, op: StacIngest) -> "RuntimeRail[StacResult]":
        with _TRACER.start_as_current_span(f"stac.claim.{op.tag}", attributes={"rasm.geo.remote": True, "rasm.geo.op": op.tag}):
            acquired = await guarded(RetryClass.HTTP, anyio.to_thread.run_sync, lambda: self._stac(op), subject=f"stac.claim.{op.tag}")
