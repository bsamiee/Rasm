...
const _VIEWS: readonly string[] = [
    ...
    "create view turn_cost as with stop as (select session_id, ts, json_extract(payload, '$.usage.cost.usd') as usd, max(ts) filter (where event = 'SessionStart') over (partition by session_id order by ts) as process_ts, event from (select session_id, ts, 'SessionStart' as event, null as payload from processes union all select session_id, ts, event, payload from observation where event = 'Stop')), priced as (select session_id, ts, usd, process_ts, lag(usd) over (partition by session_id, process_ts order by ts) as previous_usd, lead(ts) over (partition by session_id order by ts) as next_ts from stop where event = 'Stop') select p.session_id, p.prompt_id, p.turn_id, p.started_ts, json_extract(c.payload, '$.usage.model') as model, json_extract(c.payload, '$.usage.input_tokens') as input_tokens, json_extract(c.payload, '$.usage.output_tokens') as output_tokens, json_extract(c.payload, '$.usage.cache_read_input_tokens') as cache_read_input_tokens, json_extract(c.payload, '$.usage.cache_creation_input_tokens') as cache_creation_input_tokens, (select count(1) from observation s where s.event = 'turn.step' and json_extract(s.payload, '$.turnId') = p.turn_id) as steps, json_extract(c.payload, '$.durationMs') as duration_ms, json_extract(c.payload, '$.reason') as reason, (select json_extract(x.payload, '$.last_assistant_message') from observation x where x.event = 'StopFailure' and x.session_id = p.session_id and x.agent_id is null and x.ts between p.started_ts and c.ts order by x.ts desc limit 1) as cause, iif(k.process_ts is not null, k.usd - ifnull(k.previous_usd, 0), null) as usd from (select t.session_id, json_extract(t.payload, '$.turnId') as turn_id, t.ts as started_ts, (select u.prompt_id from observation u where u.session_id = t.session_id and u.event = 'UserPromptSubmit' and u.ts <= t.ts order by u.ts desc limit 1) as prompt_id from observation t where t.event = 'turn.start') p left join observation c on c.event = 'turn.complete' and json_extract(c.payload, '$.turnId') = p.turn_id left join priced k on k.session_id = p.session_id and k.ts between p.started_ts and c.ts and (k.next_ts is null or k.next_ts > c.ts);",
    "create view agent_cost as select l.session_id, l.prompt_id, l.agent_id, l.agent_type, l.stopped_ts - l.started_ts as span_ms, count(o.rowid) as tool_uses, sum(json_extract(o.payload, '$.duration_ms')) as tool_duration_ms, coalesce(l.total_tokens, t.tokens) as total_tokens, coalesce(l.total_duration_ms, t.duration_ms) as total_duration_ms, (select json_extract(x.payload, '$.last_assistant_message') from observation x where x.event = 'StopFailure' and x.agent_id = l.agent_id order by x.ts desc limit 1) as cause from lineage l left join observation o on o.event = 'PostToolUse' and o.agent_id = l.agent_id left join (select c.agent_id, sum(json_extract(c.payload, '$.usage.input_tokens') + json_extract(c.payload, '$.usage.output_tokens') + json_extract(c.payload, '$.usage.cache_read_input_tokens') + json_extract(c.payload, '$.usage.cache_creation_input_tokens')) as tokens, sum(json_extract(c.payload, '$.durationMs')) as duration_ms from observation c where c.event = 'turn.complete' and c.agent_id is not null group by c.agent_id) t on t.agent_id = l.agent_id group by l.agent_id;",
    ...
    "create view session_audit as with cost as (select session_id, ts, event, usd, max(ts) filter (where event = 'SessionStart') over (partition by session_id order by ts) as process_ts from (select session_id, ts, 'SessionStart' as event, null as usd from processes union all select session_id, ts, event, json_extract(payload, '$.usage.cost.usd') from observation where event in ('Stop', 'SessionEnd'))), spent as (select session_id, sum(usd) as usd from (select session_id, max(usd) as usd from cost where event <> 'SessionStart' and process_ts is not null group by session_id, process_ts) group by session_id), base as (select session_id, min(ts) as first_ts, max(ts) as last_ts, count(1) filter (where event = 'UserPromptSubmit') as prompts, count(1) filter (where event = 'SubagentStart') as agents, count(1) filter (where event = 'SubagentStop' and json_extract(payload, '$.agent_type') = '') as forks, count(1) filter (where event = 'PostCompact') as compactions, sum(length(cast(json_extract(payload, '$.compact_summary') as blob))) filter (where event = 'PostCompact') as compact_summary_bytes, min(ts) = min(ts) filter (where event = 'SessionStart') as recorded from observation group by session_id) select b.session_id, b.first_ts, b.last_ts, ifnull(n.processes, 0) as processes, b.prompts, b.agents, b.forks, b.compactions, b.compact_summary_bytes, (select json_extract(e.payload, '$.reason') from observation e where e.session_id = b.session_id and e.event = 'SessionEnd' order by e.ts desc limit 1) as end_reason, (select json_array_length(json_extract(p.payload, '$.background_tasks')) from observation p where p.session_id = b.session_id and p.event = 'Stop' order by p.ts desc limit 1) as background_tasks_at_end, iif(b.recorded, x.usd, null) as usd from base b left join (select session_id, count(1) as processes from processes group by session_id) n on n.session_id = b.session_id left join spent x on x.session_id = b.session_id;",
    ...
    "create view running_agents as with parented as materialized (select json_extract(payload, '$.tool_response.agentId') as agent_id from observation where event = 'PostToolUse' and tool = 'Agent') select s.session_id, s.prompt_id, s.agent_id, json_extract(s.payload, '$.agent_type') as agent_type, json_extract(s.payload, '$.cwd') as cwd, s.ts as started_ts from observation s where s.event = 'SubagentStart' and (select max(m.ts) from observation m where m.session_id = s.session_id) > unixepoch('subsec') * 1000 - 172800000 and not exists (select 1 from observation x where x.session_id = s.session_id and x.ts > s.ts and (x.event = 'SessionEnd' or (x.event = 'SessionStart' and json_extract(x.payload, '$.source') <> 'compact') or (x.event = 'turn.complete' and x.agent_id = s.agent_id) or (x.event in ('Stop', 'SubagentStop') and s.agent_id in (select agent_id from parented) and not exists (select 1 from json_each(x.payload, '$.background_tasks') t where json_extract(t.value, '$.id') = s.agent_id))));",
    "create view placed_rules as select c.session_id, c.agent_id, c.ts, json_extract(c.payload, '$.placed') as placed, (select min(d.lineage_key) from finding_delivery d where d.agent_id = c.agent_id and d.channel = 'report') as lineage_key, (select min(f.category) from finding_delivery d join finding f on f.finding_id = d.finding_id where d.agent_id = c.agent_id and d.channel = 'report') as category, (select json_group_array(distinct d.lineage_key) from finding_delivery d where d.agent_id = c.agent_id and d.channel = 'additionalContext') as told_on from observation c where c.event = 'turn.complete' and json_array_length(json_extract(c.payload, '$.placed')) > 0;",
    "create view finding_state as select f.finding_id, f.category, coalesce(t.path, f.path) as path, f.text, f.ntext, f.text_hash, f.occurrence, coalesce(t.start_line, f.start_line) as start_line, coalesce(t.start_column, f.start_column) as start_column, coalesce(t.end_line, f.end_line) as end_line, coalesce(t.end_column, f.end_column) as end_column, coalesce(t.byte_start, f.byte_start) as byte_start, coalesce(t.byte_end, f.byte_end) as byte_end, f.severity, f.message, f.replacement, f.source, f.session_id, f.prompt_id, f.agent_id, f.tool_use_id, f.observed_at, j.state, t.subject_hash, t.at, t.by, j.evidence, j.verdict, (select max(c.at) from finding_transition c where c.finding_id = f.finding_id and c.state in ('fixed', 'vanished')) as last_closed_at from finding f join finding_transition t on t.rowid = (select u.rowid from finding_transition u where u.finding_id = f.finding_id order by u.at desc, u.rowid desc limit 1) join finding_transition j on j.rowid = (select u.rowid from finding_transition u where u.finding_id = f.finding_id and u.state <> 'moved' order by u.at desc, u.rowid desc limit 1);",
    "create view confirmed_findings as select finding_id, category, path, text, ntext, occurrence, start_line, start_column, end_line, end_column, message, replacement, source, session_id, prompt_id, agent_id, subject_hash, at as confirmed_at, evidence, verdict, last_closed_at from finding_state s where state = 'confirmed' and source like 'agent:%' and not exists (select 1 from finding_transition w where w.finding_id = s.finding_id and w.state = 'wrong' and w.subject_hash = s.subject_hash);",
    ...
    "create view unjudged_edits as select e.session_id, e.prompt_id, e.agent_id, e.ts, e.tool, e.tool_use_id, e.file_path, e.cwd from edited_files e left join judged_edits x on x.tool_use_id = e.tool_use_id where substr(e.file_path, 1, length(e.cwd) + 1) = e.cwd || '/' and x.tool_use_id is null;",
