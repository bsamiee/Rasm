...
- BLOCK ENCODE IS 8-BIT-INPUT-ONLY (measured, both legs): `compress_basis` raises `KtxError(INVALID_OPERATION)` and `compress_astc` raises `KtxError(UNSUPPORTED_FEATURE)` on a u16/f16/f32 store, and `KtxPackAstcEncoderMode.HDR` is accepted-and-inert at 8-bit input — the enum member proves a parameter, never a code path. Float/deep planes ship as uncompressed deep KTX2; ASTC composes only at LDR.
