...
- Unlocks true-parallel CPU geometry on free-threaded and subinterpreter-capable runtimes without a process-pool serialization tax on large meshes, keeping one lane spine for I/O-bound and CPU-bound work alike.
...
- Compose `identity/content-identity` with `concurrency/lanes` so a unit of work admitted as a `(ContentKey, Work[T])` pair short-circuits when the key — folding input bytes and `IdentityPolicy` — already carries a result, the `DrainReceipt` distinguishing a cache hit from a fresh completion and the cache itself a `frozendict[ContentKey, T]` the lane threads immutably across one session.
...
- [INCREMENTAL_CONTENT_IDENTITY]: closed — `identity/content-identity` carries `ContentIdentity.stream` over the `xxh3_128` class and `ContentIdentity.fold` over child `ContentKey` values under the one settings-folded seed; the streaming and Merkle-bundle design is complete, transcription owned by `[TRANSCRIBE_OWNERS]`.
...
- [XXHASH_ADMISSION]: closed — `xxhash` is admitted in the root manifest and `identity/content-identity` consumes `xxh3_128_intdigest`/`xxh3_64_intdigest` directly; the seed-derivation spelling is settled.
