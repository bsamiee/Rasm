...
class MaterialImpact(Struct, frozen=True, gc=False):
    ...
    def frame(self) -> "RuntimeRail[pa.Table]":
        # method × indicator × stage flatten for tabular/contract + tabular/profile and the seam
        # Discipline.Environmental Assessment / MaterialPropertySet.Environmental wire the C# AEC
        # domain (Rasm.Compute assessment runner, Rasm.Materials material-node projection) content-keys.
...
def _lower(impact: MaterialImpact) -> "pa.Table":
    ...
    # and tabular/profile (pointblank quality), and is the seam Discipline.Environmental Assessment /
    # MaterialPropertySet.Environmental cross-language wire the C# AEC domain content-keys (Rasm.Compute
    # assessment runner, Rasm.Materials material-node projection), never a re-minted impact shape.
...
flowchart TD
    ...
    tbl -->|EN 15804 as Discipline.Environmental Assessment, content-keyed| mat["csharp:Rasm.Compute / Rasm.Materials"]
