...
[PUBLIC_TYPE_SCOPE]: `buf.yaml` v2 — the gate axis every verb reads from its `libs/contracts` input
...
|  [16]   | `modules[].name`                         | struct        | BSR identity; only `libs/contracts/proto` names `buf.build/rasm/contracts`  |
...
[PUBLIC_TYPE_SCOPE]: `buf.gen.yaml` v2 — the generation axis `--template libs/contracts/buf.gen.yaml` names from the repo root, its `out:` rows repo-root relative
...
|  [06]   | `inputs: [{ directory }]`                       | struct        | workspace or module root; ONE `directory: libs/contracts` carries all |
...
[TOPOLOGY]:
- `libs/contracts/buf.yaml` names the estate module at `proto` as `buf.build/rasm/contracts`; publisher modules stay unnamed.
...
- Publisher modules clearing `STANDARD` whole carve nothing; `libs/contracts/vendor/cloudevents/proto` carries no rule block at all.
...
- `buf format --diff --exit-code libs/contracts/proto` names the estate module alone, so no publisher byte enters the format path.
- `libs/contracts/buf.gen.yaml` feeds ONE workspace input (`directory: libs/contracts`) to every plugin row; exact FQN filters resolve on one image.
...
- `libs/contracts/README.md` (within-corpus edge): README routes, `ARCHITECTURE.md` owns registry and generation law, this catalog commands and keys.
