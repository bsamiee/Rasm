...
- `Rasm` (kernel): the `Numerics/matrix` transform band REFUSES the `Tensor<T>` plane on four structural grounds — array-only static entrypoints at the mint, `ref struct` span views that cannot cross the kernel `Fin` boundary, an allocating `PermuteDimensions` on every transpose, and `CellLattice`/`ReadOnlySpan2D` already owning grid addressing under a one-linearization law — so kernel spectra stay on the MathNet arena pair and only `TensorPrimitives` folds enter kernel fences.
