...
- catalog-verdict: KEEP as SDK-bridge protobuf peer completing the JSON/protobuf log pair and the three-signal proto exporter set beside the trace-proto and metrics-proto rows; edge-ledger fences `@opentelemetry/*` to `scope:runtime`; `[R3]`-collapse member (native `OtlpLogger` supersedes)
...
- prefer the native `OtlpLogger` lane; reach for this SDK exporter only for protobuf-wire SDK-only log-pipeline capability, and record it as an `[R3]` non-collapsed dependency.
