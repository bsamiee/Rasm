...
[LOCAL_ADMISSION]:
- build a profile as `Polyline<double>` carrying bulge directly from the `Polygon/import` `ACadSharp` arc/`Spline` entities (an arc DXF entity → one `PlineVertex` pair with the bulge computed from its sweep), never densifying the arc to a line fan at ingest
