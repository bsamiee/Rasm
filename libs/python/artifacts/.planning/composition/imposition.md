...
_PDFIMPOSE_SCHEMAS: Final[frozendict[Scheme, PdfImposeSchema]] = frozendict({
    Scheme.BOOKLET: PdfImposeSchema(pdf_saddle.impose, creep=True, group=False),
    Scheme.SIGNATURE: PdfImposeSchema(pdf_saddle.impose, creep=True, group=True),
    Scheme.CUT_AND_STACK: PdfImposeSchema(pdf_cutstackfold.impose, creep=True, group=True),
    Scheme.COME_AND_GO: PdfImposeSchema(pdf_copycutfold.impose, creep=True, group=True),
})
...
def _composed(op: ImposeOp) -> Composed:  # the one pure render fold both `of` and `contribute` read
    match op:
        case ImposeOp(tag="impose", impose=(source, scheme, geometry, marks)):
            if geometry.engine is ImpositionEngine.PDFIMPOSE:
...
def _pdfimpose_kwargs(schema: PdfImposeSchema, scheme: Scheme, geometry: Geometry, marks: Marks, /) -> dict[str, object]:
    ...
    if schema.bind:
        kwargs["bind"] = geometry.binding
    if schema.creep:
        kwargs["creep"] = lambda sheets: geometry.creep * max(sheets - 1, 0)
    if schema.group:
        kwargs["group"] = geometry.leaves if scheme is not Scheme.BOOKLET else 1
    return kwargs
...
- [PDFIMPOSE_ENGINE_SETTLED]: `pdfimpose` is admitted as a provider-contained engine row, not a second imposition owner. The package's current schema wrappers share the stable public shape `impose(files: Sequence[str | pathlib.Path | io.BytesIO], output: str | pathlib.Path | io.BytesIO, *, ...)`; `saddle.impose`, `cutstackfold.impose`, and `copycutfold.impose` are the schemas this owner maps from local `Scheme` values. `Geometry.engine = ImpositionEngine.PDFIMPOSE` switches the `Impose` arm to `_pdfimposed`, which wraps the source bytes in `BytesIO`, calls the schema with `signature=(geometry.across, geometry.down)`, `imargin=geometry.gutter`, `bind=geometry.binding`, crop marks derived from `Marks`, and group/creep rows derived from `Geometry.leaves`/`Geometry.creep`, then reads only imposed PDF bytes back into `Composed`. Provider objects, `Matrix`, `Page`, and schema names never cross the owner boundary; local `ImposedPlan`, `Placement`, `Layer`, and `ArtifactReceipt` remain the only downstream facts. `PERFECT_BIND` stays on the local placement engine because the provider exposes no `perfect.impose` schema; unsupported schemes under `PDFIMPOSE` raise the `_PDFIMPOSE_SCHEMAS` `KeyError` that the boundary catches, which is preferable to silently falling back to local placement and hiding the requested provider.
