...
import { Array, Boolean, Either, FastCheck, Schema } from 'effect';
import { distinctArray, missingClassifications, optionalFields } from './arbitraries.ts';
...
const _BIT = { min: 0, max: 1 } as const;

const _decode = Schema.decodeUnknownEither(OptionalFieldsRecord);

// --- [OPERATIONS] ----------------------------------------------------------------------

const _parity = (value: number): Parity => Boolean.match(value % 2 === 0, { onFalse: () => 'odd', onTrue: () => 'even' });

describe('optional field generation', () => {
    it('varies optional-key presence across the encoded samples', () => {
        const samples = FastCheck.sample(optionalFields(OptionalFieldsRecord), _SEED);
    ...
    it('explicit undefined is generated only for encoded fields that accept it', () => {
        const samples = FastCheck.sample(optionalFields(OptionalFieldsRecord), { ..._SEED, numRuns: 256 });
    ...
    it('empty optional-key lists preserve every generated field', () => {
        ...
        const samples = FastCheck.sample(optionalFields(base, []), _SEED);
        expect(Array.every(samples, (sample) => Object.hasOwn(sample, 'version'))).toBe(true);
    });

    it('the record model varies optional keys and always keeps required keys', () => {
        const samples = FastCheck.sample(optionalFields({ version: FastCheck.integer(), label: FastCheck.string() }, ['version']), _SEED);
        expect(Array.every(samples, (sample) => Object.hasOwn(sample, 'version'))).toBe(true);
...
describe('distinct arrays', () => {
    it('generated arrays are pairwise distinct over a two-value domain', () => {
        const samples = FastCheck.sample(
            distinctArray(FastCheck.integer(_BIT), 2, (a, b) => a === b),
            _SEED,
        );
    ...
    it('unconstrained pair generators can produce equal values', () => {
        const samples = FastCheck.sample(FastCheck.tuple(FastCheck.integer(_BIT), FastCheck.integer(_BIT)), _SEED);
...
describe('classification coverage', () => {
    it('full-range generators cover every parity label', () => {
        expect(missingClassifications(FastCheck.integer(), _parity, ['even', 'odd'])).toEqual([]);
    });

    it('over-biased generators report labels they never produce', () => {
        expect(missingClassifications(FastCheck.integer({ min: 0, max: 0 }), _parity, ['even', 'odd'])).toEqual(['odd']);
    });

    it('multi-label classification records every label produced by one sample', () => {
        const missing = missingClassifications(FastCheck.integer({ min: 1, max: 1 }), () => ['positive', 'small'], ['positive', 'small', 'huge']);
