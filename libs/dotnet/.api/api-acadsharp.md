...
- Resilient read: `CadReaderConfiguration.Failsafe` defaults `true`, so recoverable corruption routes to `NotificationEventArgs(NotificationType, Message, Exception)` and the read completes; a hard reader throw enters `Op.Catch` once at the consuming boundary and retains its exception evidence.
