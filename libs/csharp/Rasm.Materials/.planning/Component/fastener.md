...
using Rasm.Materials.Component;      // ComponentFamily/ComponentId/Component/ComponentSection/ComponentFault (the parent COMPONENT_OWNER)
using Thinktecture;
...
// frozen IfcClass valid-set (§4-RT C6), never the authority — this owner carries only the portable string.
[SmartEnum<string>]
...
public static class ComponentCatalogue {
    ...
    static Fin<(ComponentId Id, Component Item)> FastenerOf(FastenerRow r, Context context, Op key) =>
        ...
        from item in Component.Of(ComponentFamily.Fastener, r.Designation, new ComponentSection.Fastener(section), section.CapacityKey, section.AppearanceId, key)
