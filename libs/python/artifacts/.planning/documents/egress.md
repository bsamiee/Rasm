...
- [1]-[FINISH]: `DocumentEgress` + `EgressStep` (ENCRYPT/OUTLINE/WATERMARK/ATTACH/IMPOSE) policy-table dispatch over pikepdf/pypdf/weasyprint; `Permissions` the row-policy value object the ENCRYPT step carries; the OUTLINE step folds the `DocumentNode` heading tree and the IMPOSE step the `pypdf.Transformation` n-up algebra.
