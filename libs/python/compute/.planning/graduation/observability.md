...
def registered(composition: ScopeKey = DEFAULT_SCOPE) -> RuntimeRail[ComputeInstall]:
    ...
    return traversed(COMPUTE_POINTS.map(lambda point: Hooks.register(point, scope=composition)), by=Disposition.ACCUMULATE).map(
