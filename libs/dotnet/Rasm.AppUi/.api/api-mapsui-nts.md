...
|  [02]   | `IndexedMemoryProvider`               | provider      | bounding-envelope-indexed feature set |
...
- `GeometryFeature` : `BaseFeature`, `IFeature`, `ICloneable`; its `Geometry?` member is the settable NTS payload and `Extent` projects the geometry's bounding envelope as an `MRect?`.
...
- `ObservableCollectionProvider<T>` constrains `T : IFeatureProvider`, so a bound `ObservableCollection<T>` is the live source; `ShapeFile` implements `IDisposable` and owns a rebuildable spatial index.
...
|  [05]   | `MRectExtensions`      | static class  | `MRect` to bounding envelope, polygon |
|  [06]   | `EnvelopeExtensions`   | static class  | bounding envelope to `MRect`          |
...
|  [05]   | `new IndexedMemoryProvider(IEnumerable<IFeature>)`                              | ctor     | bounding-envelope-indexed feature source |
...
|  [14]   | `Envelope.ToMRect() -> MRect?`                                             | static  | NTS bounding envelope to world rectangle |
...
- Bim geodesy boundary: features arriving from `Rasm.Bim` carry their `GeoReference` lineage and reproject at that boundary; this package holds no CRS authority beyond the `CRS` string every `IProvider` carries, so a datum transform inside a provider decorator is unrepresentable.
