export const meta = {
    name: 'implement-cs',
    whenToUse:
        'Realize open IDEAS and TASKLOG cards into design-page code fences across the C# target folders (default: AppHost, Compute, AppUi, Persistence).',
    description:
    ...
    phases: [
        ...
        {
            title: 'Pins',
            detail: 'one terminal single-writer applies every reported central Directory.Packages.props pin row at the symbol anchor and every reported ARCHITECTURE.md [02]-[SEAMS] row serially; runs only when rows were reported',
            model: 'sonnet',
        },
    ],
};
...
const FIXLOG_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['folder', 'verdict', 'ripples', 'pins', 'seams', 'summary', 'realized', 'deferred', 'collapsed'],
    properties: {
        folder: { type: 'string' },
...
const REDTEAM_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['folder', 'verdict', 'ripples', 'pins', 'seams', 'closed', 'reopened', 'summary', 'realized', 'deferred', 'collapsed'],
    properties: {
        folder: { type: 'string' },
        verdict: { type: 'string', enum: ['realized', 'refined', 'clean'] },
        realized: { type: 'array', items: { type: 'string' } },
...
const PIN_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['applied', 'seam_rows_applied', 'rejected', 'summary'],
    properties: {
...
const codexPrompt = (label, task, schema, writes) => {
    ...
    return [
        ...
        '(4) READY: do NOT relay the report body through your output — build the MECHANICAL receipt with jq (never your own ' +
            "judgment): cards=$(jq '(.tasks | length) + (.ideas | length)' " +
            base +
            '-report.json); parts=$(jq -r \'"\\(.tasks|length)t+\\(.ideas|length)i cards | ripples:\\(.ripples|length) | gates:\\(.gates|length) | map:\\(.map|length)"\' ' +
            base +
            "-report.json); skeleton=$(jq '{folder, order, tasks: [.tasks[] | {slug, status, atomic, pages}], ideas: [.ideas[] | {slug, status, pages}], ripples, gates, malformed_ripples}' " +
            base +
            '-report.json). ' +
            'Return: ok=true, report=' +
            base +
            "-report.json, entries=that cards count, headline=that parts string, failure empty, and every skeleton field copied VERBATIM from the jq output — a mechanical transcription (the orchestrator's no-op/fan/ripple control flow runs on these rows), never retyped, filtered, reworded, or summarized. " +
...
const implementPrompt = (folder, seq, report, note) =>
    [
        DOCTRINE,
        '',
        'TASK: IMPLEMENT — realize the open cards of `' +
            ...
            'counts) + ripples + pins + seams + summary.' +
            (note ? '\n' + note : ''),
    ].join('\n');
...
const critiquePrompt = (folder, seq, report) =>
    [
        ...
        'Also enforce the docs/stacks/csharp file-organization + section-order law, cross-package convention consistency, and prose + comment hygiene. ' +
            ...
            '+ collapsed + ripples + pins + seams + summary.',
    ].join('\n');

const redteamPrompt = (folder, seq, report) =>
    [
        DOCTRINE,
        '',
...
const pinPrompt = (pins, seams) =>
    [
        ...
        PROSE,
        '',
        "TASK: TERMINAL SINGLE-WRITER — you are the run's SOLE writer for the repo-root `" +
            CENTRAL +
            ...
            'version via the `nuget` MCP or `uv run python -m tools.assay api`' +
            FB +
            ' before applying; confirm the owning `<pkg>.csproj` carries the ' +
...
const runFolder = async (target) => {
    ...
    try {
        ...
        if (groups) {
            ...
            impls = (
                await parallel(
                    groups.map((g, gi) => async () => {
                        await stagger();
                        return agent(implementPrompt(target, groupSeq(t, g), report, GROUPNOTE), {
            ...
            const one = await agent(implementPrompt(target, seq, report, ''), {
                label: 'implement:' + tag,
                phase: 'Realize',
                schema: FIXLOG_SCHEMA,
                effort: 'high',
        ...
        const crit = await agent(critiquePrompt(target, seq, report), {
            label: 'critique:' + tag,
            phase: 'Realize',
            schema: FIXLOG_SCHEMA,
            effort: 'high',
        ...
        const red = await agent(redteamPrompt(target, seq, report), {
            label: 'redteam:' + tag,
            phase: 'Realize',
