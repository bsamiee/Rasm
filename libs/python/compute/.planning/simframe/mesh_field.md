...
class AssembledSystem(Struct, frozen=True):
    ...
    def contribute(self) -> Receipt:
        ...
        return Receipt.of("emitted", "compute.simframe", self.element.value, facts)
...
class MeshField(Struct, frozen=True):
    ...
    def assemble(self, form: FemForm, /) -> "RuntimeRail[AssembledSystem]":
        return boundary("simframe.assemble", lambda: self._assemble(form))

    def _assemble(self, form: FemForm) -> AssembledSystem:
        import skfem

        mesh = getattr(skfem, _MESH_CTOR[self.element])(
            np.ascontiguousarray(self.points.T), np.ascontiguousarray(self.cells.T)
        )
...
class MeshExchange:
    ...
    def read(path: Path, element: ElementKind, /) -> "RuntimeRail[MeshField]":
        return boundary("simframe.read", lambda: MeshExchange._read(path, element))

    @staticmethod
    def write(field: "MeshField", path: Path, /) -> "RuntimeRail[None]":
        return boundary("simframe.write", lambda: MeshExchange._write(field, path))
