...
# 14. Remove policy-free curve construction aliases

`select.md:135-142,148-149` — fixed edge selectors and pass-through index factories on `Curves`.
...
```csharp
// Curves.All DELETED
// Curves.Boundary DELETED
// Curves.NakedOuter DELETED
// Curves.NakedInner DELETED
// Curves.Interior DELETED
// Curves.NonManifold DELETED
// Curves.OuterLoop DELETED
// Curves.InnerLoop DELETED
// Curves.At DELETED
// Curves.Form DELETED
```
...
The feature value or optional index is already the case payload. These aliases add a second vocabulary and construction surface without admission, normalization, or shared policy.

Change

Construct `EdgesCase`, `AtCase`, and `FormCase` directly. Retain `Segments`, `Iso`, `Silhouette`, and `Draft` because they establish defaults or coupled policy.

Delta

-10 LOC and -10 module-level members; type count neutral.

Ripples

At `select.md:579`, replace `Curves.All` with `new Curves.EdgesCase(Kind: Option<EdgeFeature>.None)`. Update the curve owner and growth prose to name public cases as the construction surface.

# 15. Remove policy-free face construction aliases

`select.md:377,380` — direct `Faces` construction aliases.
...
Both members only call a public case constructor and the optional index passes through unchanged.

Change

Construct `AllCase` and `AtCase` directly. Retain `Top` and `Bottom` because they canonicalize the absent axis and bind it to a direction.

Delta

-2 LOC and -2 module-level members; type count neutral.

Ripples

Update the face owner and growth prose to name public cases as the construction surface. The current planning corpus has no direct consumer of either alias.

# 16. Remove point construction forwarders

`select.md:541-546` — the complete `Points` factory façade.
...
```csharp
// Points.Quadrants DELETED
// Points.Extrema DELETED
// Points.EdgeMidpoints DELETED
// Points.Vertices DELETED
// Points.ControlPoints DELETED
// Points.Spread DELETED
```

Why

Every member forwards directly to a public union case. `Quadrants` is only `ExtremaCase` with `None`; that distinction belongs in the typed payload, not a parallel coined entrypoint.

Change

Construct the six point-query shapes through their public cases and delete the façade.

Delta

-6 LOC and -6 module-level members; type count neutral.

Ripples

Update the point owner, growth prose, and the Task 13 ripple example to use `new Points.SpreadCase(Aspect: SpreadAspect.Coplanar)`. The current planning corpus has no direct external consumer of these aliases.
