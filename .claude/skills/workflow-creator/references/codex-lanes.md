...
The wrapper never performs, edits, re-judges, softens, or relays the work itself. On any call error the wrapper runs the codex skill's blocking-caller recovery ladder ([09]) — disk product first, `codex-reply` on the captured threadId second, one fresh identical dispatch last, each rung once, never looped — and a fully failed ladder returns `ok: false` with the error text in `failure` and any threadId preserved in `thread`. Lane law rides the tool's `developer-instructions` parameter with a task-only `prompt` (the codex skill's prompt-contract law — battery-validated as the strongest architecture); the wrapper passes both verbatim, composing neither.
