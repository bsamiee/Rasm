...
[STACK]:
- The identity rail composes `XxHash3.HashToUInt64` as the canonical `ContentHash` seed (`Geometry/Spatial/naming`, `reconciliation` consume `ContentHash`); the `ulong` value flows into the `SnapshotId`/`ContentHash` value object with no intermediate byte allocation
