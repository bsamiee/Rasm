...
[LOCAL_ADMISSION]:
- `GamutMap` is accepted only by `MapToRgbGamut`; `MapToPointerGamut` and `MapToMacAdamLimits` take no argument, and the four gamut predicates gate mapping — the kernel `GamutPolicy` row is the solution's admitted spelling for the pair.
