...
- [1]-[VERIFIER]: owns the `AuthCommand` request axis, the `CredentialKind` vocabulary, the `Authn` verifier service, the replay-defeating counter/time-step advance, and the `AuthFault` policy-projected fault.
