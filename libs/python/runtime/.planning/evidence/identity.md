...
- Auto: `ContentIdentity.seed` folds the format key, deflection, tolerance, and angle tolerance into a 64-bit seed so a coarse and a fine tessellation of one input key distinctly and a re-import of identical bytes at identical settings keys identically; the children fold serializes each child's 128-bit value LITTLE-endian before re-hashing so the parent key is order-sensitive over its parts and reproduces the C# `BinaryPrimitives.WriteUInt128LittleEndian` canonical span the `Rasm.Persistence/Version/commits#COMMIT_DAG` `CommitGraph.Of`/`MerkleRange.Of` and `#CRDT_WIRE` `CrdtWire.ContentKey` fold over before `XxHash128.HashToUInt128`.
