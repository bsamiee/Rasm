...
- `Rasm.Persistence/Ingest/pointcloud#SCAN_SOURCE`: this codec is the SOLE engine for both `.las` and `.laz` on the storage leg — `open_reader_stream`'s `is_compressed` makes the `ScanFormat.Las`/`Laz` rows one decode path, the forward `read_point`/`get_coordinates` loop feeds the chunk-and-cell fold, `decompress_selective` masks to `CHANNEL_RETURNS_XY | Z | CLASSIFICATION | RGB` because a durable region row reads position, class, and colour alone, and the `.lax` `has_spatial_index` → `inside_rectangle` → `exploit_spatial_index` → `read_inside_point` path serves the cell-windowed read where an index exists
...
- A nonzero `int` status maps once to the consuming owner's codec refusal using `get_error()` as provider evidence — the reconstruct leg's Bim-owned refusal and `ScanFault.CodecReject` on the storage leg. `get_warning()` appends to the decode result diagnostics and never becomes a terminal fault.
