...
- Output: `Measured` carries the live evaluation until `StudyRun` retains its design, responses, wallclock, speedup, sensitivity indices, discrepancy, seed, and content key; `surrogate` reads the cross-validated `R^2` while `polynomial` reads the in-sample univariate diagnostic. `StudyRun.benched` projects the held wallclock onto the runtime bench fabric under the run's content-keyed subject — `Benchmark.of` consumes the measurement the run already paid for, a SPEEDUP run recovers its serial baseline as the sibling `.serial` duration series, `RESULT`'s zero elapsed suppresses the contribution, and distinct objectives never merge into one method-only benchmark series.
...
class StudyRun(Struct, frozen=True):
    method: str
    ...
    @property
    def design_cells(self) -> int:
        return int(self.design.shape[0])

    @property
    def response_width(self) -> Option[int]:
        return Some(int(self.responses.shape[1]) if self.responses.ndim > 1 else 1) if len(self.responses) else Nothing
    ...
    def attributes(self) -> dict[str, str | int | float]:
        return {
            ...
            "band": ";".join(self.band),
            **{f"S[{name}]": value for name, value in self.indices.items()},
        }

    def _noted(self) -> "StudyRun":
    ...
    def benched(self, subject: Option[str] = Nothing) -> tuple[Benchmark, ...]:
        keyed = subject.default_value(self.subject)
        serial = self.speedup.map(lambda ratio: (Benchmark.of(f"{keyed}.serial", BenchMode.LATENCY, 0, (self.elapsed * ratio * 1000.0,)),))
        return () if self.elapsed <= 0.0 else (Benchmark.of(keyed, BenchMode.LATENCY, 0, (self.elapsed * 1000.0,)), *serial.default_value(()))
...
class Study(Struct, frozen=True):
    ...
    def _execute(self, objective: Objective, seed: int, mark: StageTap) -> "RuntimeRail[StudyRun]":
        ...
        def graded(key: ContentKey) -> StudyRun:
            staged.beat(StudyStage.KEYED, cells)
            run = StudyRun.graded(self, design, measured, key, seed)
