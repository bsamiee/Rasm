...
- [02]-[CRITICAL_PATH]: the `WorkCalendar` working-time fold over `IfcWorkCalendar`/`IfcWorkTime`/`IfcRecurrencePattern`, the `CriticalPath` value record per task, and the `ScheduleNetwork.Schedule` forward/backward-pass CPM fold over the `SequenceRel` DAG by the `QuikGraph` `SourceFirstTopologicalSort` Kahn order.
...
public static class ScheduleCpm {
    ...
    // The QuikGraph SourceFirstTopologicalSort over the transient AdjacencyGraph folded from the
    // SequenceRel edges (vertices = task GlobalId, value SEdge<string> = predecessor->successor): the
    // Kahn source-first order the forward CPM pass folds in, replacing the hand-rolled in-degree drain.
    // Every task vertex (including a no-dependency isolate) is added so an unsequenced task still orders;
    // the IsDirectedAcyclicGraph pre-gate rejects a residual cycle onto BimFault.ModelRejected before the
    // sort throws. The graph is the algorithm input only — folded once, projected back, never a domain field.
    static Fin<Seq<string>> Order(ScheduleNetwork network) {
        ...
        return graph.IsDirectedAcyclicGraph()
            ...
            : FinFail<Seq<string>>(new BimFault.ModelRejected("schedule-cyclic-dependency").ToError());
    }
}
```
...
- [CPM_TOPOLOGICAL_PASS]: the `ScheduleNetwork.Schedule` forward/backward-pass CPM fold orders the `SequenceRel` predecessor→successor DAG through the `QuikGraph` `[GRAPH_ALGORITHM]` owner — the edges fold into a transient `AdjacencyGraph<string, SEdge<string>>` (the task `GlobalId` the vertex directly, no built-in vertex identity, `AddVertexRange` adding every task so an unsequenced isolate still orders, the value `SEdge<string>` allocating nothing per edge) and `graph.SourceFirstTopologicalSort()` is the Kahn source-first order (`api-quikgraph` topological-order row 2) the forward pass folds in, the `graph.IsDirectedAcyclicGraph()` predicate (row 4) the cheap acyclicity pre-gate that lowers a residual cycle onto `BimFault.ModelRejected` before the sort would throw `NonAcyclicGraphException` — verified against the live `QuikGraph` 2.5.0 decompile (`public static IEnumerable<TVertex> SourceFirstTopologicalSort<TVertex, TEdge>(this IVertexAndEdgeListGraph<TVertex, TEdge>)`, `public static bool IsDirectedAcyclicGraph<TVertex, TEdge>(this IVertexListGraph<TVertex, TEdge>)`, `AddVerticesAndEdge`/`AddVertexRange` on `AdjacencyGraph`, the netstandard2.0 single managed assembly with an EMPTY dependency group binding forward under net10); the forward pass folds each task in that order deriving `EarlyStart` as the maximum over predecessors of the modality-shifted predecessor finish plus the edge `Period` lag (the `SequenceRel.Switch` discriminating `FinishToStart`/`StartToStart`/`FinishToFinish` so the float algebra never assumes finish-to-start) and `EarlyFinish` through `WorkCalendar.Advance`, the backward pass folds in reverse order deriving `LateFinish`/`LateStart` through `WorkCalendar.Recede`, and `TotalFloat = LateStart − EarlyStart`, `FreeFloat` against the earliest successor `EarlyStart`, `IsCritical` on zero total float — `QuikGraph` owns the ORDER, the domain owns the calendar/float arithmetic and the typed `CriticalPath` receipt, the transient graph the algorithm input only and never a domain field; the topological order makes a single pass over each task sufficient (a predecessor's `CriticalPath` is computed before its successor), and the `WorkCalendar` working-time `Advance`/`Recede` resolve the duration-driven finish past the non-working days so a five-working-day task spanning a weekend reads its calendar-true finish, recovering the under-utilized NodaTime `ZonedDateTime`/`LocalTime`/`IsoDayOfWeek` surface beyond `Instant`.
