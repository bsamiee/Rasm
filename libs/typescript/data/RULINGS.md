...
- One `[name, token]` column declaration mints Arrow fields, builders, ordinals, and each dialect's DDL — `Olap.events` seats it, `iac` derives.
...
- Reach results publish the hop distance their walk computed — membership derives from it, and a membership-only row re-walks a level per consumer.
