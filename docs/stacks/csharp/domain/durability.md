...
- Law: first-opener-materializes — the register check and the artifact build share one IMMEDIATE transaction, losers blocked on the lock observe the bumped register on acquisition and no-op, and a register ahead of the compiled expectation is a typed rejection; correctness needs no leader election.
- Law: a register behind the compiled expectation routes on where the store's truth lives — a store whose truth lives elsewhere materializes a fresh file at the compiled generation through this page's own seven-step choreography, a store holding its own truth rebuilds by replaying its op-log into that fresh file inside the same choreography, and a class holding truth no replay reaches declares an operator restore as its route.
...
- Exemption: the ritual's command kernel is the platform-forced ADO statement body.
...
- Exemption: the segment reader loop is the platform-forced stream statement body.
...
- Exemption: the seal kernel — the rented-buffer lease, the stream writes, and the catch arm — is the platform-forced stream statement body.
...
- Exemption: the choreography's step kernels — the pool fence, file materialization, and register bump — are the platform-forced ADO and stream statement body.
...
[ADJUDICATION_LAW]:
- Law: one IMMEDIATE transaction holds the whole effect set — a local write holds register mutation plus op insert, a remote batch holds dedup inserts, adjudication, and cursor advance — so crash recovery is re-delivery and read-your-writes holds by construction; local and remote writes are one path, because a local fast path bypassing adjudication lets a stale local write overwrite newer synced state.
...
- Exemption: the apply transaction's command kernel is the platform-forced ADO statement body.
...
public static class OpLog {
    ...
    static Seq<long> Keys(SqliteConnection store, SqliteTransaction apply, string sql, ReadOnlySpan<(string Name, object Value)> binds) {
        ...
        while (rows.Read()) { ids.Add(rows.GetInt64(0)); }                        // Exemption: ADO read loop fills a boundary-local list frozen once by toSeq; repeated Seq.Add forcing is the rejected form
        return toSeq(ids);
    }
...
- Law: every removed artifact emits (class, identity, deciding rule, policy stamp, bytes) and the run summary proves inventory = kept + held + evicted; unrecorded deletion anywhere is a typed rejection — operator deletion routes through an administrative verdict kind — and the fact stream is itself a count-and-age-bounded class, closing meta-retention at depth one.
