...
- Evidence: `MediaSurfaces.Observed` fires `AppUiFact.Media` directly through the runtime `HookRail`; fault absence means ready and fault presence means failed. The mounted and failed instrument rows contribute inward through `MediaSurfaces.TelemetryRow`.
