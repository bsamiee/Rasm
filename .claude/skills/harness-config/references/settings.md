...
- [PROMPT_ECONOMY]: recurring prompts are a config smell — each session's repeated approvals convert into scoped allow rows, and destructive classes into deny rows, so unattended lanes (workflows, background workers, `-p` calls) run without a human at the prompt.
...
|  [10]   | `workflowSizeGuideline`, `disableWorkflows`          | Advisory agent-count target for generated workflows; the off switch              |
