...
- Unlocks joined and grouped incremental live queries over the changefeed; precondition is the catalog admission of `d2ts`, and a join or distinct over the decoded op-log must hold the Z-set retraction law the current `signTable` arithmetic already encodes.
