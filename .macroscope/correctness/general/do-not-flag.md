...
- A completeness or restoration demand against a registry or research row deliberately BLOCKED on an upstream producer is void — the blocked row is the correct fail-closed form, and the demand routes to the producer owner the row names. The class inverts identically: a demand re-minting a retired row over a producer that has since landed is void at the same bar, because a resolved row deletes whole with no tombstone and `(none)` is the correct roster, never evidence of a missing producer.
- Bounds answer the scope their owner declares and no second one: a directory-, queue-, or run-wide cap is not a per-item ceiling, and a ceiling stamped as evidence on a receipt is not a second input to the grade that receipt reports — reuse across that boundary proves the second scope at the bound's own owner first.
...
- A machinery demand the file's own standing mechanism already discharges is void — a streaming drain on a timeout-bounded one-shot, retry rails on a cheap probe, a per-scope lifecycle cell where one held gate already makes every ownership transition atomic — so the demand proves that mechanism absent or insufficient before naming a new cell, and a template header stating its fill-and-replace contract in the bundle's sibling-consistent schema is schema, never narration to delete.
