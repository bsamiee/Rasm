...
| :-----: | :---------------------------------- | :------------------------------------------------------ | :------------------------------ |
|   [1]   | `cron.schedule`                     | `cron.schedule('name', '* * * * *', 'SQL')`             | register a recurring SQL job    |
|   [2]   | `cron.schedule_in_database`         | `cron.schedule_in_database('name', sched, 'SQL', 'db')` | job scoped to a target database |
|   [3]   | `cron.unschedule`                   | `cron.unschedule('name')`                               | remove a job by name            |
|   [4]   | `cron.job` / `cron.job_run_details` | views                                                   | configured jobs and run history |
...
| :-----: | :------------------------ | :------------------------------------------------------------------------------------ | :--------------------------------- |
|   [1]   | `partman.create_parent`   | `partman.create_parent(p_parent_table => 't', p_control => 'col', p_interval => 'i')` | declare a partition set            |
|   [2]   | `partman.run_maintenance` | `partman.run_maintenance(p_parent_table => 't')`                                      | create future / drop expired parts |
|   [3]   | `partman.part_config`     | table                                                                                 | per-parent retention and premake   |
...
| [INDEX] | [SURFACE]               | [SIGNATURE]                                | [SEMANTICS]                         |
| :-----: | :---------------------- | :----------------------------------------- | :---------------------------------- |
|   [1]   | `squeeze.squeeze_table` | `squeeze.squeeze_table('schema', 'table')` | online rewrite to reclaim bloat     |
|   [2]   | `squeeze.start_worker`  | `squeeze.start_worker()`                   | start the scheduling bgworker       |
|   [3]   | `squeeze.tables`        | table                                      | scheduled-table registry and policy |

## [5]-[PG_JSONSCHEMA_PGAUDIT]

Server-side JSON Schema validation and session/object audit logging.

| [INDEX] | [SURFACE]                   | [SIGNATURE]                                   | [SEMANTICS]                          |
| :-----: | :-------------------------- | :-------------------------------------------- | :----------------------------------- |
|   [1]   | `jsonb_matches_schema`      | `jsonb_matches_schema('<schema>'::json, doc)` | validate a `jsonb` document; `CHECK` |
|   [2]   | `json_matches_schema`       | `json_matches_schema('<schema>'::json, doc)`  | validate a `json` document; `CHECK`  |
|   [3]   | `pgaudit.log` (GUC)         | `SET pgaudit.log = 'read, write, ddl, role'`  | session-audit class selection        |
|   [4]   | `pgaudit.log_catalog` (GUC) | `SET pgaudit.log_catalog = off`               | exclude catalog statements           |
|   [5]   | `pg_settings`               | view                                          | read-only GUC verification probe     |
