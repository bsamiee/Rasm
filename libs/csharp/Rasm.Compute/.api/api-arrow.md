...
[TYPE_SEAM]:
- `Field.Builder.DataType` takes an `IArrowType`; the `.Default` singletons (`DoubleType.Default`, `BooleanType.Default`, `StringType.Default`) type the scalar columns, and `TimestampType.Default` (`TimeUnit.Millisecond`, UTC) types the `Instant At` column so the Arrow wire carries the same NodaTime clock seam (`libs/csharp/.api/api-nodatime.md`) the receipt stream uses — a bare `DateTime` column is the rejected form, symmetric to the Persistence egress rule.
...
- NodaTime clock seam (`libs/csharp/.api/api-nodatime.md`) crosses at the `TimestampArray` column: an `Instant` projects to the epoch column under `TimestampType.Default` at the builder edge, the same instant seam the receipt stream and the Persistence relational store share, so the Arrow wire, the receipt fold, and the store agree on one clock.
