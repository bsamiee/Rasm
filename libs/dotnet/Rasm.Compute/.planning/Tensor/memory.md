...
- [02]-[ALLOCATION_AXIS]: staging axis with its stack cap and per-lane pool columns; `Admits` accumulating gate; `Grant`/`Rent` result with the typed allocation fault; `StagingEventKind` taxonomy carrying its own manager attachment; `AllocationEvidence` event payload; bit-mask/grow/tokenize views.
...
- [05]-[RENT_CLEARING]: `AllocationMode` is a request column — a fully-overwritten payload rents `Default` and a partially-written or trust-boundary payload rents `Clear`, because the unwritten tail of a partial write carries the pool's prior content whatever the payload's secrecy class; `ZeroOutBuffer` stays a `Diagnostic`-row policy for the manager's own blocks
