...
- `NodaTime`(`libs/dotnet/.api/api-nodatime.md`): the mapped CLR vocabulary is the `api-nodatime` clock model, and `api-nodatime-stj`(`libs/dotnet/.api/api-nodatime-stj.md`) carries the same values across the System.Text.Json wire, so a `timestamptz` column and a serialized snapshot share one clock and never diverge.
