...
// --- [ERRORS] ---------------------------------------------------------------------------
[Union]
internal abstract partial record BridgeFault : Error, ICategorized {
    private BridgeFault() { }
...
public static class Bridge {
    ...
    internal static Unit Write<TOut>(IDataAccess access, int slot, string name, Access targetAccess, Seq<Sourced<TOut>> values) {
        ...
        return (targetAccess, values.Count) switch {
            ...
            (Access.Tree, _) => Effect(action: () => {
                ITree tree = Garden.TreeFromPears(pears: values.AsIterable().Select(ToPear));
