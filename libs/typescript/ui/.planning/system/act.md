...
- [07]-[DOCUMENT_LADDER]: three-tier document-transition ladder, the `<Activity>` row, the degrade chain; `Transition`.
...
- Law: hover carries intent — `useHover` suppresses touch-emulated hover and pairs with floating-ui's `safePolygon` only at the overlay boundary (`view/overlay`); `useInteractOutside` owns outside-press dismissal where a full overlay stack is not mounted.
...
- Law: every hold leads with `motion-reduce:animate-none` under the same construction law as the transition rows, and that guard resolves because ONE merge group at `system/token#CLASS_MERGE` owns every `animate-*` trigger — enter, exit, every platform-sustained animation, the named component animations, and the `animate-none` guard together; splitting any trigger into the default group leaves `motion-reduce:animate-none` in a foreign group where it silently loses.
- Law: the row strings participate in `cn` conflict resolution — the motion class groups are taught to the one merge instance at `system/token#CLASS_MERGE`, so a caller override of `delay-*` or `fade-in-*` wins deterministically.
