...
Purchase requires an `item_id` from the billing catalog (`billing_getCatalogItemListV1`), a payment method (default used when omitted), and WHOIS contacts (TLD defaults used when omitted). WHOIS profiles for the target TLD exist before registration, some TLDs demand `additional_details`.
...
`301` preserves SEO for permanent moves, `302` serves temporary campaigns. Remove forwarding before the domain points at hosting.
...
Profiles are per-TLD contact bundles reused across domains, `entity_type` is `individual` or `organization`, and requirements differ by TLD. Check usage before deletion, a profile in use by active domains refuses to delete.
...
Portfolio-wide security audit runs as a list call and a per-domain detail loop: `GET /api/domains/v1/portfolio`, then check `domain_lock` and `privacy_protection` on each, enabling both wherever absent. The same loop serves any portfolio-wide mutation.
