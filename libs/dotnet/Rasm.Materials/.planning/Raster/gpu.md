...
- Entry: `public static Fin<Unit> Prove(PressDevice device, WgslKernel kernel, Op key)` runs the op-code totality gate, then dispatches EVERY fixture on the row's own `Oracle` sequence and compares each reduced output's leading `Expected.Length` elements against the fixture's tolerance; `Oracle.All` DERIVES from `WgslKernel.Items` — the `Projection/benchmarks` parity workload and the proof estate iterate one public projection, and no hand-maintained second roster exists to drift.
