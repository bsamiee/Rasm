...
Proposed correction: split the README folder layout into `Standards bodies` and `Instruction and report overlays`, or replace the sentence with `Active standards bodies are the standards files in this layout; AGENTS.md and .reports/** are instruction or source-material overlays, not standards bodies.` Keep AGENTS.md in read-order rules, but not in active-standards counts.
Active owner file: docs/standards/README.md.
Ripple files: docs/standards/AGENTS.md; .reports/AGENTS.md.
...
Path and line/heading: .reports/AGENTS.md:11-17, .reports/standards-structure-notation-060626/README.md.
Axis: evidence.
Evidence snippet/source id: .reports/AGENTS.md says a named `.reports/**` task reads the session manifest first and treats a missing manifest as an archive-maintenance gap. The session folder `.reports/standards-structure-notation-060626/` exists, but `README.md` is absent.
...
Proposed correction: create the session README in a separate archive-maintenance change, using .reports/AGENTS.md section [4][SESSION_MANIFEST]. Do not create it in this pass because the user required exactly one report file.
Active owner file: .reports/AGENTS.md.
Ripple files: .reports/standards-structure-notation-060626/README.md.
Decision recommendation: HOLD.
