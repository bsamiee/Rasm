...
`RenderSettings.PostEffects` is the settings-side effect roster; `api-rhinocommon-render.md` owns the `PostEffect` authoring, execution, and gating family this container configures.
...
- The post-effect container is the settings side of that split: `PostEffectCollection` orders, selects, and parameterizes the roster while `api-rhinocommon-render.md`'s `PostEffect`/`PostEffectPipeline` executes it, so a configuration write never reaches an execute body and an execute body never edits the roster.
