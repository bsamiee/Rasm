...
| [INDEX] | [SYMBOL]                                          | [FAMILY]         | [CAPABILITY / BOUNDARY]                                              |
| :-----: | :------------------------------------------------ | :--------------- | :------------------------------------------------------------------- |
...
[STACK: `vitest` + `fast-check`] — property law. `it.prop`/`it.effect.prop` (`@effect/vitest`) accept `fast-check` `Arbitrary`s beside `Schema`s; `proof/law/property.ts` folds them into the three reusable law combinators (see `fast-check.md`). The `fastCheck` option forwards `FC.Parameters` (seed, `numRuns`) into the vitest test.
