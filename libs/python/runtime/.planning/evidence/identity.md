...
`ContentIdentity` is the single content-addressing owner the whole branch consumes, the module `rasm.runtime.identity`. It derives one XxHash128 key over canonical bytes, reproducing the C# `System.IO.Hashing.XxHash128` seed with format and the consumer-folded policy bytes carried into the key, so a re-tessellation at identical settings is a cache hit by reference. This collapses the former parallel content owners — data `ExchangeBundle`, artifacts `ContentDigest`/`ArtifactBundle`, the companion GLB key — into one owner data, geometry, compute, and artifacts consume without re-minting.
...
class IdentityPolicy(Struct, frozen=True, gc=False):
    # the GENERIC content-seed carrier: a domain knob (geometry's tessellation deflection/angle rides
    # the geometry-owned TessellationPolicy folded into the canonical seed bytes by its consumer) is
    # never a new IdentityPolicy field per domain — tolerance is the one generic slot.
