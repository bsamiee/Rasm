...
from typing import ClassVar, Final, Literal, Protocol, assert_never, overload
...
from rasm.runtime.faults import SCOPES, BoundaryFault, FaultTag, RuntimeRail, Scope, latched, scoped
...
# observable-callback state alone, and every field is a LEVEL the export cycle reads: cached process reading with its
# sample window, and live occupancy probes bounded owners registered. No per-event count lands here — a delta ADDS at
# its call, where a snapshot held for the export thread reports the last event forever.
class MetricState(Struct, frozen=True):
    process: psutil.Process
    reading: ProcessReading | None
    sampled_at: float = 0.0
    occupancy: Map[str, Block[Occupancy]] = Map.empty()
...
def _banded(band: str, probes: Block[Occupancy]) -> tuple[Observation, ...]:
...
def _inflight(state: MetricState) -> Iterable[Observation]:
    ...
    return [reading for band, probes in state.occupancy.items() for reading in _banded(band, probes)]
...
def _refreshed(state: MetricState, now: float) -> MetricState:
    # one process sample per collection window, taken on the EXPORT thread. Sampling at a producer call instead
    # freezes every `rasm.process.*` gauge in a composition that records nothing, and the window stamps even when the
    # sample fails, so a vanished process re-probes once per cycle rather than once per gauge.
    return (
        state
        if now - state.sampled_at < READING_TTL_S
        else replace(state, reading=ProcessReading.sample(state.process), sampled_at=now)
    )
...
class Metrics:
    # two-tier custody: `_state`/`_receipts` key per-composition snapshots and evidence by ScopeKey, while
    ...
    # composition owns its own state entry. `_tenants`/`_budget` are the process-wide cardinality guard: admitted
    # tenant values and their ceiling. Carrier and tenant set read WITHOUT the gate — atomic-reference reads are
    # lock-free by definition, and gating them puts a process-wide acquisition on every measurement a free-threaded
    # publisher takes. Gate custody covers map read-modify-write alone.
    _state: ClassVar[Map[ScopeKey, MetricState]] = Map.of_seq([(DEFAULT_SCOPE, MetricState(psutil.Process(), None))])
    ...
    def install(cls, scope: ScopeKey = DEFAULT_SCOPE, budget: int = TENANT_BUDGET) -> MeterReceipt:
        ...
        match standing:
            ...
            case _:
                ...
                with cls._gate:
                    cls._state = cls._seeded(scope)
                    cls._receipts = cls._receipts.add(scope, receipt)
                return receipt

    @classmethod
    def _seeded(cls, scope: ScopeKey) -> Map[ScopeKey, MetricState]:
        # gate-held state seed: a scope reaching any custody surface first owns its entry, so `install` and `occupied`
        # arrive in either order and neither overwrites the other's registrations.
        return cls._state if cls._state.contains_key(scope) else cls._state.add(scope, MetricState(psutil.Process(), None))
    ...
    def occupied(cls, probe: Occupancy, *, band: str, scope: ScopeKey = DEFAULT_SCOPE) -> Iterator[None]:
        ...
        def rebound(fold: Callable[[Block[Occupancy]], Block[Occupancy]]) -> None:
            with cls._gate:
                ...
                cls._state = held.add(scope, replace(held[scope], occupancy=occupancy))
    ...
    def observe(cls, drain: DrainReceipt[object], *, scope: ScopeKey = DEFAULT_SCOPE) -> None:
        ...
        # wire's DELTA temporality reports each window's own movement. Held instead as observable state the callback
        # would republish the last drain's snapshot every cycle — a sawtooth a cumulative reader compounds into a
        # monotonic total nobody produced. This fold walks the TERMINAL partition alone — `accepted` names the
        # admitted total those columns exactly partition, so carrying it as one more `outcome` value makes every sum
        # across that dimension read twice the work the lane actually drained. Occupancy is NOT this call's to report:
    ...
    def _callback(cls, spec: InstrumentSpec) -> ObservableCallback:
        ...
        # Process readings refresh HERE, under the collection window rather than at a producer call, so a composition
        # that records nothing still exports live gauges and `READING_TTL_S` keeps one cycle's process rows on one
        # `oneshot`; the roster snapshots under the gate, the sample runs off it, and the readings republish per key.
        def observed(_: CallbackOptions) -> Iterable[Observation]:
            now = perf_counter()
            with cls._gate:
                held = tuple(cls._state.items())
            # `ProcessReading.sample` is a syscall window — `memory_full_info` walks the kernel's own maps — so it
            # runs OUTSIDE the gate every `occupied` registration and first-seen tenant queues behind. The gate
            # snapshots, the sample runs unheld, and the refreshed halves publish back per key.
            sampled = Block.of_seq((key, _refreshed(state, now)) for key, state in held)
            with cls._gate:
                cls._state = sampled.fold(
                    lambda live, row: live.try_find(row[0]).map(lambda seat: live.add(row[0], _resampled(seat, row[1]))).default_value(live),
                    cls._state,
                )
                states = tuple(cls._state.items())
            match spec.project:
                case None:
                    return ()
                case project:
                    return [
                        # Re-wrapping carries `obs.context` forward: a project fold holding the producing span's
                        # context is the only way a pulled series earns an exemplar, and rebuilding the
                        # observation without that slot drops it for non-default compositions alone.
                        obs if scope == DEFAULT_SCOPE
                        else Observation(obs.value, {**dict(obs.attributes or {}), **_composed(scope)}, obs.context)
...
- Cases: the DBAPI rows (`PsycopgInstrumentor`, `SQLite3Instrumentor`) patch the drivers the data query surfaces ride; `HTTPXClientInstrumentor` spans the transport client legs; `Jinja2Instrumentor` spans the artifacts template render/compile/load legs — renders happen at artifacts altitude, activation stays here; `ThreadingInstrumentor` and `AsyncioInstrumentor` propagate context across the thread and coroutine hops the worker crossing drives; `SystemMetricsInstrumentor` runs the `_SYSTEM_SLICE` — the `system.*` and `cpython.gc.*` families alone, because the `rasm.process.*` gauges own the process family off the cached `MetricState` fold and one fact keeps one owner.
- Entry: `install(scope=)` latches per composition over the one `latched`-guarded train activation and takes no profile argument — the gate is the installed provider, the same law the instrument fold holds — so a PACKAGE/TEST process patches against the no-op providers at zero export cost, a later composition's receipt truthfully records zero newly activated rows, and activation happens once at the composition root, never at library altitude. Each row's own `wraps` presence gate runs before its thunk reifies, so a deployment resolving fewer drivers than the roster admits activates the rows it can and records exactly those, where one absent driver otherwise raises out of the composition root and takes every later row with it. DBAPI wrapping likewise activates at the composition root alone: the data-side consumer hands its own admitted driver module in (duckdb, ADBC DBAPI), so this folder imports and patches nothing it does not admit.
...
# each row names the driver it wraps beside the thunk minting its instrumentor, and `wraps` is the presence gate the
# fold reads FIRST. A contrib instrumentor imports its driver at ITS OWN module scope, so reifying a thunk whose
...
# skips silently, never enters the receipt, and activates unchanged the moment that driver resolves.
...
def _present(row: TrainRow) -> bool:
    # gate then mount, in that order and inside one expression: the probe imports nothing, and only a resolved
    # driver reifies its lazy instrumentor proxy.
    return find_spec(row.wraps) is not None and row.mount().instrument() is None
...
class Instrumentation:
    ...
    def _activated(cls) -> TrainReceipt:
        def enroll(names: tuple[str, ...], row: TrainRow) -> tuple[str, ...]:
            return (*names, row.name) if _present(row) else names
