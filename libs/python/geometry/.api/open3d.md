...
- tensor axis: the `t.pipelines.registration` backend mirrors ICP with `icp`/`multi_scale_icp` over a `t.geometry.PointCloud`, each estimator taking a `robust_kernel.RobustKernel(RobustKernelMethod.TukeyLoss, scale)` and `multi_scale_icp` carrying the coarse-to-fine voxel/iteration/correspondence schedule. Both backends return `RegistrationResult.transformation`/`.fitness`/`.inlier_rmse`, and both take their initial pose on `init_source_to_target`, so pre-transforming a source cloud to seed a solve is the deleted form — it pays a whole-cloud copy the provider does not need and returns a residual the caller must then compose.
