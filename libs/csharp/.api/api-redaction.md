...
- `Rasm.Element` (`Graph/wire#EGRESS_REDACTION`): the CONTRACT assembly alone — `DataClassification(taxonomy, value)` minting the `rasm.element` keys and `DataClassificationSet` whole-set `Union` containment selecting `ClassifiedColumn` rows; no `Redactor` resolves on the seam, presence clearing is the whole mechanism.
