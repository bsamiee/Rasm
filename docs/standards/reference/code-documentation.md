...
| :-----: | :-------- | :-------------------------- | :---------------------------------- | :----------------- |
|  [01]   | Pure      | total values/facts          | purpose, obligations                | absent             |
|  [02]   | Rail      | result/effect/status        | success, faults, runtime, resources | typed data/status  |
|  [03]   | Throwing  | native exception boundary   | exception contract, cause           | thrown types       |
|  [04]   | Script    | Bash command surfaces       | stdout, stderr, state, traps        | exit status        |
|  [05]   | Catalog   | PostgreSQL catalog surfaces | meaning, security, planner, route   | exposed `SQLSTATE` |
...
| [INDEX] | [STATE]      | [MEANING]                                                                                                           |
| :-----: | :----------- | :------------------------------------------------------------------------------------------------------------------ |
|  [01]   | `supported`  | current public contract; no lifecycle warning                                                                       |
|  [02]   | `preview`    | external support contract is intentionally provisional and consumed by generated reference or support policy        |
|  [03]   | `deprecated` | supported with warning, replacement path, behavior delta, and migration condition                                   |
|  [04]   | `removed`    | public contract no longer exists; historical fact routes to support or migration documentation, not source comments |
|  [05]   | `internal`   | excluded from public generated reference; not a preservation device for greenfield stale surfaces                   |
