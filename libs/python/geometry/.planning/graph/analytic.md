...
- Owner: `AnalyticValue` is the one carrier for every graph-analytic reducer return; each projection closes with `assert_never`, so a new return shape breaks every census at type-check. `tabled` is the columnar third projection — the shape the graduation `EvidenceFrame` port admits — so an analytic board crosses the geometry-to-data boundary through the producing page's frame row while this substrate stays graduation-free.
