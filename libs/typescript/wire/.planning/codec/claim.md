...
- Entry: `Claim.admit(octets, identity)` — decode plus gate in one rail: the claim decodes through the proto engine, the embedded fingerprint's canonical `print` compares against the kernel `AppIdentity.host` (`HostPrint`, the boot-minted dimension), and a divergence refuses with `WireFault` reason `parity` carrying both prints as evidence.
- Receipt: an admitted claim is displayable evidence — suite, metric rows, the mint instant, and the proven-local fingerprint with its machine/arch/cores/runtime dimensions; the probe UI renders it with provenance intact.
- Growth: a new measurement axis is one `_Metric` row field; a new fingerprint dimension is a field on `HostFingerprint` mirroring the C# emit — display evidence, never a second gate input, because the gate reads the canonical print alone.
- Law: the gate is identity, not trust — the C# writer mints the canonical `print` over its own dimensions and the kernel host boot mints the same spelling into `AppIdentity.host`, so the gate is one branded-string equality; TS never re-derives a fingerprint from the dimensions, and a claim without a print cannot decode because the field is total.
...
class Claim extends Schema.Class<Claim>("Claim")({
  ...
  static readonly admit = (octets: Uint8Array, identity: AppIdentity): Effect.Effect<Claim, ParseResult.ParseError | WireFault> =>
    Effect.gen(function* () {
      ...
      return claim.host.print === identity.host
        ? claim
        : yield* new WireFault({
            family: "BenchmarkClaimWire",
            reason: "parity",
            detail: "<foreign-host-claim>",
            evidence: Option.some({ actual: claim.host.print, expected: identity.host }),
          })
    })
}
