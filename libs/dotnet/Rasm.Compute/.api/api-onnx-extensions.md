...
- `Model/extension#EXTENSION_OPS`: `RegisterOrtExtensions()` is the `CustomOpLibrary.Bundled` row's registration arm, sequenced beside the leak-free `Asset` row's `RegisterCustomOpLibrary(path)`; the asset probe accumulates so a refusal names every absent or replaced library, and its `File.Exists` + content-hash guard faults `ExtensionAssetMissing` at the managed layer ahead of the native `DllNotFoundException`. The bundle row carries no file to re-hash, so its registration IS its probe and a runtime shipping without the native asset refuses there. This page also owns the `StringSlots` bound-output allocator (`CreateTensorWithEmptyStrings`) and the polymorphic `Egress(SlotShape)` reader over the same `OrtValue` carrier: `Egress` discriminates on `OnnxType`, reading a `String`-tensor output through `GetStringTensorAsArray` + `GetTensorTypeAndShape().Shape` and a `ZipMap` sequence/map through `GetValueCount`/`GetValue`, then proves the projection against the slot `Model/identity#MODEL_IDENTITY` declared — so the `String` dtype stays a model-boundary row and never enters the interior tensor vocabulary.
