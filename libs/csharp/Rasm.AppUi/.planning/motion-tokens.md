...
- Boundary: a second easing or duration vocabulary anywhere in the package is the deleted pattern — charts, dialogs, toasts, behaviors, and pan-zoom canvases all read these rows; GH2 canvas motion and Rhino viewport overlay motion stay host-owned, and cross-surface consistency is carried as values — spring rows hold numeric parity with the `SpringPreset` Snappy and Relaxed rows in Rasm.Rhino and Rasm.Grasshopper, a spring row's duration equals its response envelope, and the stiffness and damping derivations mirror the host `SpringConfig` tuning math; color tweens interpolate in OKLab in parity with the host interpolation kernel — a channel-space sRGB lerp is the named defect.
...
- Boundary: every projection receives the post-degrade token — callers resolve through the degrade switch or the phase map first, so projections stay pure and reduction applies exactly once; motion clocks read `ClockPolicy.Time`, and a direct wall-clock or stopwatch call site in motion code is the named defect; the Avalonia transition member spellings ride the RESEARCH row while the plan and anchor values bind spelling-independent; `Throttle` and `Debounce` are the pacing anchors the screen runtime, live-data pipelines, and behavior interval rows consume — a duration literal at any of those call sites is the deleted pattern.
...
| :-----: | :-------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------ | :----------------- |
|   [1]   | Avalonia 12 transition, animation, page-transition, and spring-easing member spellings binding token rows | `uv run python -m tools.assay api query avalonia Animatable`                    | MOTION_APPLICATION |
|   [2]   | macOS reduce-motion preference probe spelling for embedded and standalone rows                 | libs/csharp/Rasm.AppUi/scenarios/reduced-motion-probe.verify.csx under live RhinoWIP | REDUCED_MOTION     |
