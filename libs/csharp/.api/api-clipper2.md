...
- `Clipper.Triangulate`/`TriangulateResult` are public surface but stay OUT of every consumer: the `2.0.0` triangulation module is author-flagged buggy (open infinite-loop defects in the internal `Delaunay` kernel); route triangulation through the kernel triangulation owner instead.
