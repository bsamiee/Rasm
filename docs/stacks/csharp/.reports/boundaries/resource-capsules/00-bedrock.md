...
// IO<A>.Local(): fully isolated child scope — new Resources AND new CancellationToken
// Resources registered inside Local() are released when Local() scope exits.
static IO<TResult> IsolatedScope<TResult>(IO<TResult> work) =>
    work.Local();    // both resource registry and cancellation token are isolated
```
