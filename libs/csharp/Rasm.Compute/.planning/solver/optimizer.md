...
Rasm.Compute solver optimizer: one `Optimizer` design-space-search axis over typed design-var/constraint/objective with NSGA-II crowded tournament, Bayesian-GP expected-improvement, gradient-adjoint line-search/trust-region, topology-SIMP optimality-criteria, CMA-ES rank-µ covariance adaptation, simulated-annealing, and PSO rows under a closed `ConstraintHandling` feasibility axis, and one `Surrogate` reduced-order duality column making every solve an optional error-bounded learned/ROM/GP evaluation behind the same contract — the Gaussian-process covariance Cholesky and marginal-likelihood weight riding the `numeric#DENSE_ALGEBRA` `Cholesky<double>` surface and the reduced basis riding the `Orthogonalization` SVD/QR rows. The page owns the `OptimizerKind`/`DesignVariable`/`ActivationRule`/`ObjectiveSense`/`ConstraintHandling`/`StepControl`/`AcquisitionFunction`/`SurrogateKind`/`Orthogonalization` vocabulary, the `DesignProblem`/`DesignPoint`/`ParetoFront`/`OptimizerPolicy`/`OptimizationResult` carriers, the `Surrogate`/`RomBasis`/`GpModel`/`RbfModel` reduced-order models, and the `Optimizer` search fold; the `evaluate` oracle composes the `solve-contract#SOLVE_CONTRACT` `SolveLane.Solve` or a `Surrogate.Predict` evaluation behind one signature, the gradient-adjoint row reads the `tensors#EQUIVALENCE_INTEROP` reverse-mode tape, the GP-covariance fit and reduced-basis SVD ride `numeric#DENSE_ALGEBRA`/`numeric#OWNED_BUILDS`, and the `ComputeReceipt` rail, `WorkLane`/`Substrate`/`AllocationClass`, `CorrelationId`, `ClockPolicy`, and the `SolverKeyPolicy` ordinal accessor from `discretization#DISCRETIZATION_MESH` arrive settled. The `ParetoFront` crosses to Persistence as a content-keyed vector-index artifact and the `Surrogate` crosses to `clash#CLASH_AND_TWIN` as the digital-twin baseline.
...
public sealed record DesignProblem(
    ...
    static readonly HashMap<Type, TensorOpFamily> OperatorRows = HashMap<Type, TensorOpFamily>();

    static Seq<TensorOpFamily> Lower(Seq<DesignVariable> variables) =>
        variables.Filter(static v => v.Free).Bind(v => OperatorRows.Find(v.GetType()).ToSeq());
...
- [GRADIENT_ADJOINT]: the gradient-adjoint row reads the `tensors#EQUIVALENCE_INTEROP` reverse-mode tape (`SensitivityLaw.Chain` over the `DesignProblem.AdjointTape` `TensorOpFamily` rows). The `DesignProblem.OperatorRows` registry that lowers a free design-variable type to its `TensorOpFamily` adjoint row is empty until the `tensors#DDG_ADJOINT` DDG-operator `DifferentiableOp` rows (Laplacian, heat-flow, spectral, gradient) land on the closed `vocabulary#OPERATION_TABLE` `TensorOpFamily` axis — until then `Lower` yields the empty tape and `Chain` returns the seed gradient unchanged (a total degenerate descent that moves no axis), never a fabricated `Laplacian`/`Gradient`/`Identity` member the vocabulary table does not declare. The open leaf is each DDG operator's `TensorOpFamily` row plus its `OperatorRows` binding, grounded once the adjoint-coefficient member surface grounds against the `Rasm`/Vectors operator kernel at cross-folder alignment; the NSGA-II, Bayesian-EI, SIMP, simulated-annealing, CMA-ES, PSO, and trust-region descent kernels are authored in-package folds.
