...
# the native-offload bounded slot the `of` path threads: the `_composed` MuPDF fold is a GIL-releasing
# native render, so it rides one `CapacityLimiter`-bounded `to_thread` band off the event loop instead
# of the per-loop `current_default_thread_limiter()` 40-token default the concurrency owner rejects.
...
class Imposition(Struct, frozen=True):
    ...
    async def _keyed(self) -> ContentKey:
        ...
        return ContentIdentity.of(f"impose-{self.op.tag}", composed.data)
