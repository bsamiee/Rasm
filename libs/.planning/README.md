...
- Branch `.api/` (`libs/<lang>/.api/`, Python and TypeScript only): one catalogue per language-wide cross-cutting package, authored once and never duplicated into a folder `.api/`. A folder consuming a cross-cutting package reads the branch catalogue and lists the package in its README `[CROSS_CUTTING]` section. C# carries no branch `.api/`: its universal kernels (`LanguageExt.Core`, `Thinktecture.Runtime.Extensions` core) are excluded from catalogues entirely, and every other C# catalogue stays per folder.
...
- Cross-cutting (Python and TypeScript folders only): a `[CROSS_CUTTING]` section after the main package list names the language-wide packages the folder consumes whose catalogue lives at the branch `libs/<lang>/.api/`. The main list carries folder-specific packages; the cross-cutting list carries the shared-infrastructure packages. C# folder READMEs carry no such section.
...
A package that is cross-cutting language-wide infrastructure — the typing, concurrency, observability, numeric-core, and wire/codegen tiers a branch's `.planning/` cross-cutting registry names — carries one catalogue at the branch `libs/<lang>/.api/` (Python and TypeScript only; see [1]), never duplicated into a folder `.api/`, and each consuming folder lists it in its README `[CROSS_CUTTING]` section. A package shared across folders only as a domain seam (mesh-file interchange, object-store transport, companion native cores) keeps a per-folder catalogue in each consuming folder rather than a branch catalogue, because the seam resource is folder-local. Per-language packages that are not cross-cutting stay folder-local in the folder README and `.api/`, and are never duplicated into the cross-`libs/` core.
