...
- Permitted-edge ledger parses live from its owning page — the strata flowchart in `libs/typescript/.planning/ARCHITECTURE.md` is the single source: `subgraph S<n>` clusters carry the stratum marks, `[IMPORT]`-labeled edges carry the permitted crossings, and a reshaped or vanished fence fails the gauge loudly. Acyclicity, stratum direction, and the runtime/plane tag law prove against the parsed rows and folder tag triples.
...
- Root `package.json` scripts name the runner entrypoints — `test`, `coverage`, `bench`, `mutation`, and `e2e` each own their runner; `coverage` is the committed arming of the per-file thresholds, `bench` the committed feeder of the sustained-regression ledger. Type and lint gates invoke their binaries direct: `tsc --noEmit -p tsconfig.base.json` is the one compiler gate, and Biome `check` runs the sole lint rail with the promoted GritQL rules.
