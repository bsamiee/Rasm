# [ssh2] — the in-process SSHv2 root: client lifecycle, exec/shell channels, the SFTP subsystem, sock-chained jump hosts
...
- runtime target: node only; runtime deps `asn1` + `bcrypt-pbkdf`; optional `cpu-features` + `nan` tune the default cipher list only — a pure-JS crypto fallback exists, so there is no hard native gate
- rail: substrate — the SSH root under every remote exec, SFTP transfer, and remote-watch origin row; catalogued once at the branch tier
...
|  [02]   | `Client` forwarding (`forwardOut`, `forwardIn`, `unforwardIn`, `openssh_forwardOutStreamLocal`, `openssh_forwardInStreamLocal`, `openssh_unforwardInStreamLocal`, `openssh_noMoreSessions`) | forwarding | TCP/unix-socket tunnels; `forwardOut` mints the jump-host duplex |
...
|  [06]   | config trust + tuning (`hostVerifier`, `hostHash`, `algorithms`, `readyTimeout`, `keepaliveInterval`, `keepaliveCountMax`, `localAddress`, `localPort`, `forceIPv4`, `forceIPv6`, `strictVendor`, `debug`) | connect config | known-hosts verification, cipher policy, liveness budget |
...
|  [02]   | `stat` remote size → `open(path, 'a' \| 'r+')` → positioned `read`/`write`           | resume: offset | the no-rsync fallback row — byte-offset arithmetic, a resume-capable `Sink` |
...
- `webdav` / `basic-ftp` (`data/.api/webdav.md`, `data/.api/basic-ftp.md`): sibling remote-origin rows on the `object/file` plane — one origin-addressed surface dispatches on scheme + capability flags, and a missing server-side capability degrades by row, never by fork.
- `chokidar` (`data/.api/chokidar.md`): the local half of the watch strategy row; the remote halves are the exec-push and poll rows above.
