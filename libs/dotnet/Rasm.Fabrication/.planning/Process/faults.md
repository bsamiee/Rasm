...
- [05]-[ADMISSION]: `Admission`, the one generated-owner bridge onto the `Fin` result beside the `Rasm.Element` `AdmissionSlots` band-blind `Gate` lift and `Accumulate` fold every accumulating policy admission composes.
