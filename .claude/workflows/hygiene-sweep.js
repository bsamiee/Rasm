...
const FIXLOG_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['files', 'applied', 'verdict', 'manifest_rows', 'summary'],
    properties: {
        files: { type: 'array', items: { type: 'string' } },
        applied: { type: 'array', items: { type: 'string' } },
        verdict: { type: 'string', enum: ['fixed', 'clean', 'fail'] },
        manifest_rows: {
            type: 'array',
            items: {
                type: 'object',
...
const inv = await agent(
    'SCOPE DISCOVERY under ' +
        SWEEP +
        ', resolved against REAL disk state: run find/ls NOW and enumerate the package/area ' +
        ...
        'full-read their folders themselves; it never licenses a skim.',
    { label: 'discover', phase: 'Hygiene-Discover', schema: DISCOVERY_SCHEMA, model: 'sonnet', effort: 'low' },
);
...
// --- [H1_FOLDER]
phase('H1-Folder');
const h1Raw = await pool(FOLDERS, CAP, (f) =>
    agent(
        [
            LAW,
            '',
            BLOCKER,
            '',
            'TASK (folder hygiene, primary folder ' +
                f +
                '): full-read the folder at large FIRST — README, ARCHITECTURE, cards, .api entries, every ' +
                '.planning page — full files, never skims. (1) CONSISTENCY — every dependency in ' +
                f +
                '/README.md exists in the central dependency ' +
                ...
                'record it on the owning card surface. README fresh vs the current manifest. (2) SEAMS — audit ' +
                f +
                '/ARCHITECTURE.md LINE BY LINE: every ' +
...
log(
    'H1 folder hygiene done across ' +
        h1.length +
        ' folders; ' +
        ROWS.length +
        ' manifest row(s) reported' +
...
const h2Prompt = [
    ...
    'TASK (global, scope ' +
        SWEEP +
        " — you are the run's ONLY central-manifest writer): (1) MANIFEST ROWS — apply every reported row below to " +
        ...
        'ROWS: ' +
        JSON.stringify(ROWS) +
        '. (2) GLOBAL MANIFEST CONSISTENCY — diff the central dependency manifest against ALL folder README ' +
...
log(
    h2
        ? 'H2 global consistency + manifest rows + branch/lib refinement done'
...
phase('H3-ColdVerify');
const h3Raw = await pool(FOLDERS, CAP, (f) =>
    agent(
        [
            LAW,
            '',
            ...
            'TASK (COLD VERIFY of ' +
                f +
                ' — adversarial and WRITING, no prior verdicts exist for you): re-derive the folder hygiene state from fresh ' +
...
return {
    ...
    h1: h1.map((r) => ({
        ...
        manifest_rows: (r.manifest_rows || []).length,
    })),
