...
- `Rasm.Persistence` alone owns columnar lake custody — a producer joins by handing its typed record-batch schema, never by folder-local storage code.
...
- Typed rails a consumer seam cannot carry outward park on the composing app's evidence cell, never `ignore`d — a `void` delegate licenses no discard.
...
- Hook censuses fold into ONE frozen registry at `HookRail.Mount` — a peer reaching `HookRegistry.Mount` splits the audit into partial tables.
...
- Each host boundary types plugin identity ONCE — `HookScope` and `PluginKey` are those keys, so a raw-string plugin parameter forks the key spaces.
