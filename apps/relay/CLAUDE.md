...
Relay shows Claude and OpenAI subscription usage in the menu bar, switches the machine's working account, and opens a 5-hour window on any account without switching to it.
...
- Sign-in runs `claude auth login --claudeai` into the account's private store, 5 minutes at most, the browser login waits in a sheet
- Shared store is the directory `CLAUDE_SECURESTORAGE_CONFIG_DIR` names, else `CLAUDE_CONFIG_DIR`, else `~/.claude`
- Shared item holds the one live grant, every other account's grant sits in its private item
- Switch copies the shared grant into the outgoing private item and the incoming private item into the shared item
- Switch deletes the incoming private item after the copy
- Switch writes the incoming `oauthAccount` into `.claude.json`, runs no child process, and makes no network request inside the locks
...
- Switch interrupted by a crash leaves its record in `claude-selection.json`, the next launch deletes the private copy the shared item already holds
- Running CLI keeps the token it holds until its own refresh is due, then adopts the shared item, new invocations follow at once
- Working account is the shared store's `oauthAccount`, read at launch, wake, panel open, and every refresh
- Watch on `.claude.json` through kqueue rereads the working account on each change
- Login the CLI made outside Relay to an unknown identity adds an account row
- Displaced account's last-read grant is parked into its private item, a grant that rotated since reads "Sign in to this Claude account again."
- Parked grant is refreshed by `claude -p` in its private store when its access token or refresh token nears expiry
- Relay posts nothing to the token endpoint
- Tombstoned grant (`refreshToken` empty) reads as signed out
...
- 429 keeps `Retry-After`, a missing or zero value keeps the usage cache
...
- `CLAUDE_CONFIG_DIR`, `CLAUDE_SECURESTORAGE_CONFIG_DIR`, and `CODEX_HOME` come from `$SHELL -lc`, no `-i`, once after launch, 5 s at most
...
- Sign-out runs `claude auth logout` in the account's store, the working account's sign-out signs the CLI out
- Remove signs out, then deletes the private store, its Keychain item, and the account directory
- Account directories with no record are deleted at launch
...
- `~/.codex/auth.json` is the one live sign-in, `Accounts/<id>/Codex/auth.json` the parked copies, identity is the file's `id_token` claims
- Switch stages each file beside its destination at mode 0600 and replaces it through `FileManager.replaceItemAt`
- Switch parks the live file in the outgoing account, installs the incoming copy live, then removes the copy
...
- Running `codex` keeps its account and the desktop app rereads no `auth.json`, a switch quits and reopens the desktop app when it is running
...
- Working account is the live `auth.json`, read at launch, wake, panel open, every refresh, and on each change through a kqueue watch
...
- Sign-out runs `account/logout` on the account's server, remove deletes the account's `Codex` directory
...
- Start Session sends the greeting while the account is ready
- Start reads usage first and returns it unless the account is ready
- Claude greeting is `claude -p` in stream-json against the account's own store, tools, hooks, and memory off, one turn, then `hi`, 2 minutes at most
...
- OpenAI greeting takes 2 minutes at most
...
- Email row selects the account, an accessory-bar button reading "In use" with a checkmark on the working account, a spinner over it while switching
- Provider mark is a static glyph beside the gauges
- Gauges Session, Weekly, and one per model window sit beside the mark, label leading, `percent · reset` trailing
...
- Sidebar is General, then Accounts rows with mark, email, and checkmark, drag to reorder, the same context menu
- `+` as a provider menu and `−` on the selection sit in a control group at the sidebar's bottom
- Account detail is a grouped form: Working account (Use Account or In use), Session start picker, Sign Out or Sign In…, issue as footer
- Working account section of an OpenAI account shows the switch precondition refusal as its footer
...
- Cancel stays enabled and reads "Cancelling…" while the login child stops
...
|  [04]   | `Accounts/<id>/Codex`   | `CODEX_HOME` with the parked `auth.json`                  |
...
- Claude Code's lock directories are created, stamped, and removed through `FileManager`, inode and modification date decide a compromised lock
...
- Login shell is `SHELL` as a path, a bare name resolves on the launch `PATH`, a missing or unresolved name skips the read for launchd's variables
...
- `Providers/` holds one client per provider, the process runner, and the keychain tool
...
- `swift-subprocess` from `https://github.com/swiftlang/swift-subprocess` is the one package, `Package.resolved` under the workspace pins it
- `Relay:build` passes `-disableAutomaticPackageResolution`, a stale `Package.resolved` fails the build instead of moving the pin
- Derived data holds the whole build tree, `-derivedDataPath` puts `Relay:build`'s under `.cache/xcode` and Xcode's own entry points keep theirs
...
- Project-level `SYMROOT` turns on legacy build locations, which refuse packages outright
