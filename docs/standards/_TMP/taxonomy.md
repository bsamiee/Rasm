---
description: Structural vocabulary and marker semantics for documentation
---

# [H1][TAXONOMY]

[IMPORTANT] Structure encodes semantics—each element type carries meaning.

[CRITICAL] Formatting signals intent for agent execution.

---
## [1][LEXICON]

| [INDEX] | [TERM]    | [DEFINITION]                                                            |
| :-----: | :-------- | :---------------------------------------------------------------------- |
|   [1]   | Index     | Table row identifier using [#].                                         |
|   [2]   | Sigil     | Bracket container: [...].                                               |
|   [3]   | Marker    | Content inside sigil.                                                   |
|   [4]   | Divider   | Code separator: // --- [LABEL] --- padded to column 90[^1]              |
|   [5]   | Strata    | Tripartite section pattern: Preamble → Corpus → Terminus.               |
|   [6]   | Preamble  | First marker(s) at section entry—directs agent behavior.                |
|   [7]   | Corpus    | Primary section content—prose, lists, tables, blocks.                   |
|   [8]   | Terminus  | Final marker(s) in section—reinforces critical information at exit.     |
|   [9]   | Gate      | Terminus sub-type. Verification checkpoint enforcing quality criteria.  |
|  [10]   | Qualifier | Inline semantic tag [KEYWORD].                                          |
|  [11]   | Directive | Block instruction marker [IMPORTANT]: or [CRITICAL]:. Introduces lists. |
|  [12]   | Modifier  | Per-item keyword prefix [ALWAYS] or [NEVER]. Reinforces each list item. |
|  [13]   | Reference | Cross-document link marker [REFERENCE]. Signals external dependency.    |

[^1]: Comment delimiter is language-dependent (//, #, --, etc.)

[IMPORTANT]:
- [ALWAYS] **Preamble:** Limit to 0–2 markers.
- [ALWAYS] **Terminus:** Limit to 0–3 markers.
- [ALWAYS] **Gate:** Enforce verification before section exit.
- [ALWAYS] **Item Order:** `[ALWAYS]` items before `[NEVER]` items within same directive.
- [ALWAYS] **Section Order:** Positive directive list before negative directive list.

[REFERENCE] Valid keywords: [→voice.md§5[KEYWORDS]](voice.md#5keywords)

---
## [2][MARKERS]

### [2.1][STATI]

| [INDEX] | [GLYPH] | [GLYPH_SEMANTIC]        | [STASIS]             | [STASIS_SEMANTIC]         |
| :-----: | :-----: | :---------------------- | :------------------- | :------------------------ |
|   [1]   |   [o]   | Pass—affirmed, valid.   | [OK], [PASSED]       | Explicit pass state.      |
|   [2]   |   [x]   | Fail—rejected, invalid. | [FAILED], [ERROR]    | Explicit fail state.      |
|   [3]   |   [!]   | Alert—attention needed. | [WARNING], [CAUTION] | Explicit warning state.   |
|   [4]   |   [?]   | Unknown—indeterminate.  | [PENDING], [UNKNOWN] | Explicit uncertain state. |
|   [5]   |   [+]   | Added—new, appended.    | [ADDED], [NEW]       | Explicit addition.        |
|   [6]   |   [-]   | Removed—deleted, gone.  | [REMOVED], [DELETED] | Explicit subtraction.     |
|   [7]   |   [=]   | Unchanged—same, static. | [UNCHANGED], [SAME]  | Explicit no-change.       |
|   [8]   |   [/]   | Skip—not applicable.    | [NULL], [SKIP]       | Explicit exclusion.       |
|   [9]   |   [~]   | Partial—approximate.    | [PARTIAL], [APPROX]  | Explicit incompleteness.  |
|  [10]   |   [$]   | Cached—frozen, cost.    | [CACHED], [SAVED]    | Explicit cache state.     |

[IMPORTANT] Glyphs for density. Stasis for clarity.

---
## [3][REFERENCES]

| [INDEX] | [TYPE]           | [SYNTAX]                       | [EXAMPLE]                                             |
| :-----: | :--------------- | :----------------------------- | :---------------------------------------------------- |
|   [1]   | Internal Section | [§N.M](#anchor)                | [§1.2](#12label) — links to H3 anchor.                |
|   [2]   | External File    | [→path/file.md](path/file.md)  | [→voice.md](voice.md) — relative link.                |
|   [3]   | External Section | [→file.md§N.M](file.md#anchor) | [→voice.md§2.1](voice.md#21punctuation) — cross-file. |
|   [4]   | Symbol           | backticks in prose             | createRetry(), B.config                               |

**Anchor Format:** Headers ### [N.M][LABEL] generate anchors as #nmlabel (lowercase, no brackets, no dots).

[IMPORTANT]:
- [ALWAYS] **Section:** Wrap §N.M in markdown link to anchor.
- [ALWAYS] **File:** Wrap →path in markdown link to relative path.
- [ALWAYS] **Symbol:** Wrap code identifiers in backticks.

[CRITICAL]:
- [NEVER] Unlinked § or → notations—link to valid anchors.
- [NEVER] Absolute paths—use relative paths from current file location.

---
## [4][EXAMPLE]

```text
[CHECKLIST]                          ← Validation results
- [o] Schema valid.
- [x] Types missing.
- [~] Coverage partial.

[DELTA]                              ← Change tracking
- [+] Auth module.
- [-] Legacy adapter.
- [=] Core utils.

[STATUS]                             ← Process state
- [?] Review pending.
- [!] Attention required.
- [/] E2E skipped.
- [$] Build cached.

[INLINE] Task [o], review [?].       ← Glyph: compact density
[REPORT] Build: [PASSED]             ← Stasis: explicit state
```
