...
Local scan: `rg -n "typedoc|api-extractor|api-documenter|@microsoft/tsdoc|tsdoc|docModel|dtsRollup|@public|@beta|@alpha|@internal" package.json pnpm-workspace.yaml pnpm-lock.yaml tsconfig*.json apps libs tools docs -g '!docs/standards/_reports/**' -g '!node_modules/**'` found only active standards wording, not configured tooling.
