...
public static class DevLoop {
    ...
    // Hud runs never stack duplicate render callbacks on one sample feed. The Rx callback is the named
    // terminal edge — recovery composes before the one Run, so no failure is discarded.
    public static IO<IDisposable> Hud(IObservable<HudSample> samples, Func<HudSample, IO<Unit>> render, Func<Error, IO<Unit>> faults) =>
        IO.lift(() => samples.Subscribe(sample =>
            ignore((render(sample) | @catch<IO, Unit>(static _ => true, error => faults(error))).As().Run())));
