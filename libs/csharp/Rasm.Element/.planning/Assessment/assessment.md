...
- [01]-[ASSESSMENT_NODE]: the `AssessmentPayload` generic receipt keyed by `Discipline`+`AnalysisRoute`+`InputKey`, the `AnalysisRoute` opaque route token, the `AssessmentOutcome` `Usable`/`Terminal` lifecycle, the typed `Results` bag with the `Result`/`ResultMeasure` flat reads, the failure `Detail`, the `ResultBlob` heavy-artifact reference, and the `Provenance`.
...
- Receipt: an `Assessment` node is the analysis evidence a `Bake`-derived `Element` carries flat — `element.Assessments.Filter(a => a.Discipline == Discipline.Energy && a.Outcome.Usable)` reads every usable energy result, `assessment.ResultMeasure(name)` reads a dimensioned output as a `MeasureValue` directly (and `Result(name)` the raw `PropertyValue` for a non-measure output), `assessment.ResultBlob` fetches the heavy artifact by content key, `assessment.Detail` reads a failure diagnostic; the `Rasm.Compute` analysis route writes the `Computed` node back keyed on `(InputKey, Route)`, and the seam carries the receipt without owning the solver — the discipline-specific input/result shapes, the FEA/EnergyPlus/EC3 runners, and the multi-ply `AssemblyAggregator` all live in Compute.
- Packages: Thinktecture.Runtime.Extensions (`[SmartEnum<string>]`/`[ValueObject<string>]`), Generator.Equals (`[Equatable]` the payload's member-level diff + `[UnorderedEquality]` the order-insensitive `Results` bag, so the `Graph/element#NODE_MODEL` `Node.Assessment` drill descends to `Nodes[id].Payload.Results[name]`), LanguageExt.Core (`Map`/`Option`/`Fin`), NodaTime (`Instant` provenance), `Projection/address#CANONICAL_WRITER` (`CanonicalWriter` the `CanonicalBytes` projection writes through), `Rasm` (the kernel `Op` op-key + the content-key seed the `InputKey`/`ResultBlob` share).
...
// --- [MODELS] -----------------------------------------------------------------------------
// [Equatable] (the [GRAPH_FAMILY] / Graph/element#NODE_MODEL [STRUCTURAL_EQUALITY] discipline) is LOAD-BEARING, not
// decorative: the Node.Assessment case wraps THIS payload, and Generator.Equals DRILLS into a node member only when the
// member type is itself [Equatable] — so a re-solve under the SAME (Discipline, Route, InputKey) (the node id keys on that
// triple, NOT the results/outcome, so a re-solve mutates the SAME node IN PLACE) surfaces as a Nodes[id].Payload.Results[name]
// path AND an AsStale() Outcome flip as Nodes[id].Payload.Outcome in ElementGraph.EqualityComparer.Default.Inequalities (the
// member granularity the Rasm.Persistence 3-way StructuralMerge keys its RFC 6902 patch egress on), never a whole-payload
// replacement. A plain (un-[Equatable]) record would be an opaque equality leaf to the Node comparer's DefaultEqualityComparer
// (correct VALUE equality via the record's own Equals, but the diff stops at Nodes[id].Payload), collapsing every assessment
// re-solve to whole-payload replacement — the deleted form for a node the merge must reconcile at member granularity. The
// order-insensitive Results bag carries [UnorderedEquality] (the dictionary-aware multiset comparer) so a Result key-set delta
// nests into the graph diff; the remaining members (Discipline/Route/InputKey/Outcome/Detail/ResultBlob/Provenance) are
// generated-equality-default — a scalar or single-key value-object the Node comparer compares directly. NEVER stacked on a
// record-root [Union] (this is a single-shape product record, not a union, so [Equatable] is the sole equality owner here).
[Equatable]
public sealed partial record AssessmentPayload {
 ...
 [UnorderedEquality] public Map<PropertyName, PropertyValue> Results { get; init; }
 ...
 // call site (the deleted per-call `Result(name) is { Case: PropertyValue.Measure m }` form). A non-Measure result
 // (a Text/Boolean diagnostic carried in the bag) reads None, so the typed read is total over the bag and honestly
 // absent for a non-measure entry; it derives from the one Result(name) read, never a parallel result store.
 ...
 // The canonical content contribution co-located on the payload owner (the Composition/material#MATERIAL_COMPOSITION
 // MaterialComposition.CanonicalBytes / Geospatial/coverage#COVERAGE_NODE CoverageGrid.CanonicalBytes shape): the
 // Graph/element#NODE_MODEL Node.ToCanonicalBytes assessment arm writes its case ordinal then DELEGATES to this
 // method rather than re-spelling the projection per case (the SHARED_CANONICAL_CODEC ownership the Projection/address
 // CanonicalWriter establishes — each complex payload owns its own CanonicalBytes), so the node-id mint and the diff
 // share ONE projection. ONLY the (Discipline, Route, InputKey) cache triple is written — the SAME identity the node id
 // self-hashes on (NodeId.Content(node.ToCanonicalBytes(tolerance))) and the Projection/address#CONTENT_ADDRESS Verify
 // dual recomputes — so the mutable lifecycle/result fields (Outcome/Results/Detail/ResultBlob) and Provenance are
 // EXCLUDED: a re-solve or an AsStale flip must NOT fork the node id (the cache-hit invariant), and Provenance is a
 // separate additive axis the content key never folds (the graph-altitude OwnerHistory/StepHeader exclusion). The triple
 // is element.md's assessment-arm contribution, owned here so the projection is never re-spelled off the seam.
