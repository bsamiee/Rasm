...
`Microsoft.IdentityModel.JsonWebTokens` supplies the modern allocation-conscious JWT engine: `JsonWebTokenHandler` creates, signs, encrypts (JWE), and validates JWS/JWE compact tokens, and `JsonWebToken` is the parsed token with typed, span-friendly header/payload accessors. It is the token leg that peers with the `OpenIddict.Client` acquisition rail: where the client obtains tokens, this handler reads and validates them against the `Microsoft.IdentityModel.Tokens` validation contract (`TokenValidationParameters`/`TokenValidationResult` and the modern `ValidationParameters`/`ValidationResult<,>`), keyed by the signing material that `Microsoft.IdentityModel.Protocols.OpenIdConnect` discovery feeds into `TokenValidationParameters.ConfigurationManager`. It is the successor to the legacy `System.IdentityModel.Tokens.Jwt` `JwtSecurityTokenHandler`; this package is the only JWT handler the host admits.
...
| :-----: | :------------------------------------------------------------------------------------------------- | :---------------- | :------------------------------------ |
|  [01]   | `CreateToken(SecurityTokenDescriptor tokenDescriptor)`                                             | descriptor create | canonical signed/encrypted token      |
|  [02]   | `CreateToken(string payload, SigningCredentials signingCredentials)`                              | JWS create        | signed token from raw payload         |
|  [03]   | `CreateToken(string payload, EncryptingCredentials encryptingCredentials)`                        | JWE create        | encrypted token from raw payload      |
|  [04]   | `CreateToken(string payload, SigningCredentials, EncryptingCredentials)`                          | nested create     | signed-then-encrypted token           |
|  [05]   | `CreateToken(string payload, SigningCredentials, EncryptingCredentials, string compressionAlgo)`  | nested+zip create | nested token with DEFLATE compression |
|  [06]   | `CreateToken(string payload, …, IDictionary<string,object> additionalHeaderClaims)`              | header-extended   | adds custom JOSE header claims        |
|  [07]   | `EncryptToken(string innerJwt, EncryptingCredentials encryptingCredentials)`                      | JWE wrap          | encrypts an existing signed JWT        |
|  [08]   | `EncryptToken(string innerJwt, EncryptingCredentials, string algorithm)`                          | JWE wrap          | encrypt with explicit alg + header     |
...
| [INDEX] | [SURFACE]                                                                       | [ENTRY_FAMILY]    | [RAIL]                                       |
| :-----: | :------------------------------------------------------------------------------ | :---------------- | :------------------------------------------- |
|  [01]   | `bool CanReadToken(string token)`                                               | structural probe  | cheap JWS/JWE format check                   |
|  [02]   | `JsonWebToken ReadJsonWebToken(string token)`                                   | parse             | parses without validation                    |
|  [03]   | `JsonWebToken ReadJsonWebToken(ReadOnlyMemory<char> token)`                     | parse (span)      | zero-copy parse over rented memory           |
|  [04]   | `SecurityToken ReadToken(string token)`                                         | parse (base)      | `TokenHandler` override returning base type  |
|  [05]   | `Task<TokenValidationResult> ValidateTokenAsync(string token, TVP)`             | validate          | full async validate (signature/claims)       |
|  [06]   | `Task<TokenValidationResult> ValidateTokenAsync(SecurityToken token, TVP)`      | validate (parsed) | revalidate an already-parsed token           |
|  [07]   | `TokenValidationResult ValidateToken(string token, TVP)`                        | validate (sync)   | synchronous validate                         |
|  [08]   | `string DecryptToken(JsonWebToken jwtToken, TVP)`                               | JWE decrypt       | decrypts to inner JWS string                 |
|  [09]   | `Task<string> DecryptTokenWithConfigurationAsync(JsonWebToken, TVP, CT)`        | JWE decrypt+disco | decrypt using `ConfigurationManager` keys    |
