...
class Idml(Struct, frozen=True):
    ...
    async def emit(self) -> RuntimeRail[ArtifactReceipt]:
        # the ONE content-keyed production entry a `core/plan#PLAN` `ArtifactWork` node wraps (`work=idml.emit`),
        # mirroring `delivery/register#REGISTER` `Register.emit` / `export/layered#LAYERED` `LayeredExport.emit` —
        # the returned `ArtifactReceipt` IS the `ReceiptContributor` the runtime lane harvests, no module-level batch entry.
        return await async_boundary("export.indesign", self._emit)
...
- [ADMISSION_GATE] [RESOLVED]: the untrusted `.idml` template is admitted exactly once at `Idml.of` through the closed `IndesignPayload` `TypedDict` and the module-level `_PAYLOAD = TypeAdapter`, mapping a `ValidationError` to `IdmlFault.payload` — the lifecycle-spine fix the prior shape lacked entirely (the raw bytes flowed straight into `IdmlSource.data` with only non-empty checks), bringing IDML in line with the `document/emit#EMIT` `EmitPayload` and `export/layered#LAYERED` `ExportPayload` admission. The base prefix is now REFINED at the `TypeAdapter` itself: `IndesignPayload.prefix` carries `Annotated[str, StringConstraints(pattern=r"\A\w+\Z")]`, so a non-alphanumeric ingress base-prefix is an `IdmlFault.payload` at admission (the refined-admission parity the sibling `derive`/`layered` factories carry) rather than a worker-time `IDMLPackage.prefix` `BaseException` — the two-tier refinement the imperative `_PREFIX.match(src.prefix)` check in `of` then extends over the trusted-producer STEP sources (the `IdmlSource` prefixes inside `Insert`/`AddPages` cases, which carry an indexed `bad_prefix` fault the untrusted-payload `TypeAdapter` cannot reach). The `empty_ref` arm rejects an empty layer/story/content id before a layer-or-story mutation reaches the worker, so the worker fold stays total across the full fourteen-verb family. `_mutate` then guards the base-prefix APPLICATION with `is_prefixed`: an already-namespaced template short-circuits the `@use_working_copy` `prefix` round-trip and the double-namespacing it would otherwise inflict, the idempotence guard the prior shape's unconditional `prefix(...)` lacked.
- [EXPORT_EGRESS_SCOPE] [RESOLVED]: SimpleIDML's `export_xml(from_tag, encoding) -> str` / `export_as_tree() -> dict` tagged-content EGRESS (the round-trip inverse of `import_xml`) is scoped to `document/lens#LENS` (the recovered-tree read the introspection rail owns), NOT re-authored as an `IdmlStep` case or an `IdmlFact` field — a structured content extract is a lens/recovery concern, and folding it onto the picklable `IdmlFact` would bloat the worker's cross-back evidence with the whole tagged-content tree the `Office` byte-count receipt never carries. A future need for the extract lands as one `document/lens#LENS` arm over the round-tripped `.idml`, never a growth on this mutation owner.
