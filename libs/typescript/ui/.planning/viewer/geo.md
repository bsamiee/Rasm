...
- Law: events fold into atoms — `map.on(...)` returns `Subscription`s registered as scope finalizers; React owns only mount/unmount and the imperative map lifecycle never leaks into render. Module-level worker policy (`prewarm`, `addProtocol` for authed tile transport) is app-composition material set before the first `Map`; `transformRequest` routes tile URLs through the app's auth boundary.
...
const _rasterTiles = (
  ...
  new TileLayer<ImageBitmap>({
    id,
    getTileData: ({ index, signal }) => fetched({ zoom: index.z, x: index.x, y: index.y }, signal),
    renderSubLayers: (props) =>
      new BitmapLayer({
        id: `${props.id}/frame`,
        image: props.data,
        bounds: [
          props.tile.boundingBox[0][0] ?? 0,
          props.tile.boundingBox[0][1] ?? 0,
          props.tile.boundingBox[1][0] ?? 0,
          props.tile.boundingBox[1][1] ?? 0,
        ],
      }),
  })
