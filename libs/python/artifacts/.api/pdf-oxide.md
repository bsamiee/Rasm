...
- [02]: `Pdf` — build a PDF from text/HTML/Markdown/image(s)/merge via `from_*` static factories
...
|  [02]   | `Pdf`                                      | creation root     |
|  [03]   | `DocumentBuilder`                          | fluent builder    |
|  [04]   | `FluentPageBuilder`                        | page builder      |
|  [05]   | `PdfPage`                                  | page edit view    |
|  [06]   | `Page`                                     | page extract view |
|  [07]   | `AsyncPdfDocument`                         | async root        |
|  [08]   | `AsyncPdf`                                 | async creation    |
|  [09]   | `OfficeConverter` / `AsyncOfficeConverter` | office gate       |
...
Members by index:
- [01]: `TextChar` — one char: `char`/`bbox`/`font_name`/`font_size`/`font_weight`/`is_italic`/`is_monospace`/`color`/`rotation_degrees`/`origin_x`/`origin_y`/`advance_width`/`mcid`
...
- [09]: `LayoutParams` — computed per-page `column_count`/`word_gap_threshold`/`line_gap_threshold`/`median_font_size`/`median_line_spacing`/`median_char_width`
...
|  [02]   | `TextWord`                               | word            |
|  [03]   | `TextLine`                               | line            |
|  [04]   | `TextSpan`                               | styled span     |
|  [05]   | `FormField`                              | AcroForm field  |
|  [06]   | `PdfPageRegion`                          | region view     |
|  [07]   | `PdfElement`                             | structured node |
|  [08]   | `PdfText` / `PdfImage` / `PdfAnnotation` | element leaves  |
|  [09]   | `LayoutParams`                           | layout metrics  |
...
Members by index:
- [01]: `RenderedPixmap` — `data: bytes` (row-major, top-left origin, 4 B premultiplied RGBA/pixel, `len == width*height*4`) + `width`/`height`
- [02]: `SeparationPlate` — one grayscale ink separation: `data` (0–255 == tint %)`/`width`/`height`/`ink_name`; prepress ink-coverage + ML/QC source

| [INDEX] | [SYMBOL]          | [PACKAGE_ROLE] |
| :-----: | :---------------- | :------------- |
|  [01]   | `RenderedPixmap`  | raster buffer  |
|  [02]   | `SeparationPlate` | ink plate      |
...
Members by index:
- [01]: `Color` — `Color(r, g, b)` 0..1 floats; `from_hex(str)` + `black`/`white`/`red`/`green`/`blue` presets; `.r`/`.g`/`.b`
- [02]: `LinearGradient` — `start`/`end` + `add_stop(offset, color)`; `horizontal(width, start, end)`/`vertical(height, start, end)` helpers
- [03]: `RadialGradient` — `inner_circle`/`outer_circle` + `add_stop`; `centered(x, y, radius)` helper
- [04]: `PatternPresets` — `checkerboard`/`crosshatch`/`diagonal_lines`/`dots`/`horizontal_stripes`/`vertical_stripes` tiling-pattern factories
- [05]: `BlendMode` — `NORMAL`/`MULTIPLY`/`SCREEN`/`OVERLAY`/`DARKEN`/`LIGHTEN`/`COLOR_DODGE`/`COLOR_BURN`/`HARD_LIGHT`/`SOFT_LIGHT`/`DIFFERENCE`/`EXCLUSION` (PDF blend modes)
- [06]: `ExtGState` — `fill_alpha`/`stroke_alpha`/`alpha`/`blend_mode(mode)` + `semi_transparent()` transparency state
- [07]: `LineCap` / `LineJoin` — `BUTT`/`ROUND`/`SQUARE` caps; `MITER`/`ROUND`/`BEVEL` joins
- [08]: `EmbeddedFont` — one-shot TTF/OTF handle: `from_file(path)`/`from_bytes(data, name=None)` + `.name`; moved into the builder by `register_embedded_font` (registering twice raises)
- [09]: `Table` / `Column` / `Align` — `Column(header, width=100.0, align=Align.LEFT)` + `Table` buffered grid for `FluentPageBuilder.table`; `Align.{LEFT,CENTER,RIGHT}`
- [10]: `StreamingTable` — row-by-row `push_row`/`push_row_span`/`flush`/`finish` for large grids; `batch_count`/`column_count`/`pending_row_count`
- [11]: `PageTemplate` / `Header` / `Footer` / `Artifact` / `ArtifactStyle` — running header/footer + tagged artifact templates (`Header.center`/`Footer.center`/`ArtifactStyle.bold`/`.font`) for `Pdf.from_markdown_with_template`
- [12]: `ExtractionProfile` — pre-tuned profile: `conservative`/`aggressive`/`balanced`/`academic`/`policy`/`form`/`government`/`scanned_ocr`/`adaptive`; `available()` lists; tuning props (`word_margin_ratio`/`space_threshold_em_ratio`/…)
- [13]: `OcrEngine` / `OcrConfig` — OCR engine + config handles passed to `extract_text_ocr(engine=)` (opaque constructors; built via module OCR model functions)

| [INDEX] | [SYMBOL]                                                            | [PACKAGE_ROLE]  |
| :-----: | :------------------------------------------------------------------ | :-------------- |
|  [01]   | `Color`                                                             | rgb value       |
|  [02]   | `LinearGradient`                                                    | linear fill     |
|  [03]   | `RadialGradient`                                                    | radial fill     |
|  [04]   | `PatternPresets`                                                    | tiling patterns |
|  [05]   | `BlendMode`                                                         | blend enum      |
|  [06]   | `ExtGState`                                                         | graphics state  |
|  [07]   | `LineCap` / `LineJoin`                                              | stroke style    |
|  [08]   | `EmbeddedFont`                                                      | font handle     |
|  [09]   | `Table` / `Column` / `Align`                                        | buffered table  |
|  [10]   | `StreamingTable`                                                    | streaming table |
|  [11]   | `PageTemplate` / `Header` / `Footer` / `Artifact` / `ArtifactStyle` | running content |
|  [12]   | `ExtractionProfile`                                                 | extract tuning  |
|  [13]   | `OcrEngine` / `OcrConfig`                                           | ocr config      |
...
Members by index:
- [01]: `Certificate` — `load(der)`/`load_pem(cert_pem, key_pem)`/`load_pkcs12(data, password)`; `subject`/`issuer`/`serial`/`validity: (int,int)`/`is_valid`
- [02]: `Signature` — one embedded `/Sig`: `signer_name`/`reason`/`location`/`contact_info`/`signing_time`/`covers_whole_document`/`pades_level`; `verify()`/`verify_detached(pdf_data)`
- [03]: `PadesLevel` — frozen `B_B`/`B_T`/`B_LT`/`B_LTA` (+ `BB`/`BT`/`BLt`/`BLta` aliases); `int` → 0..3 ABI code
- [04]: `TsaClient` — `TsaClient(url, username=None, password=None, timeout_seconds=30, hash_algorithm=2, use_nonce=True, cert_req=True)`; `request_timestamp(data)`/`request_timestamp_hash(hash, hash_algorithm)` → `Timestamp`
- [05]: `Timestamp` — `parse(der)`; `time`/`serial`/`policy_oid`/`tsa_name`/`hash_algorithm`/`message_imprint`/`verify()`
- [06]: `RevocationMaterial` — `RevocationMaterial(certs=, crls=, ocsps=)` offline B-LT validation material (DER certs/CRLs/OCSP responses)
- [07]: `Dss` — parsed `/DSS` (ISO 32000-2 §12.8.4.3): `certs`/`crls`/`ocsps` DER blobs + `vri` per-signature VRI keys

| [INDEX] | [SYMBOL]             | [PACKAGE_ROLE]  |
| :-----: | :------------------- | :-------------- |
|  [01]   | `Certificate`        | x509 + signer   |
|  [02]   | `Signature`          | existing sig    |
|  [03]   | `PadesLevel`         | level enum      |
|  [04]   | `TsaClient`          | RFC-3161 client |
|  [05]   | `Timestamp`          | timestamp token |
|  [06]   | `RevocationMaterial` | LTV material    |
|  [07]   | `Dss`                | security store  |
...
- [02]: `PdfDocument.from_bytes` — `from_bytes(data: bytes, password: str \| None = None) -> PdfDocument`
- [03]: `PdfDocument.authenticate` — `authenticate(password) -> ...`
- [04]: `PdfDocument.page` / `pages` / `page_count` — `page(index) -> PdfPage` (edit view) / `pages -> PdfDocumentIter` yielding `Page` (extract view) / `page_count -> int`
- [05]: `PdfDocument.save` — `save(path, compress=True, garbage_collect=True, linearize=False)`
...
|  [04]   | `PdfDocument.page` / `pages` / `page_count`         | per-page edit view, extract-view iterator, page count |
...
|  [09]   | `PdfDocument.permissions` / `version` / `save_page` | read permission flags, PDF version, single-page bytes |
...
Call shapes by index:
- [01]: `PdfDocument.extract_text` — `extract_text(page, region=None, exclude_layers=None, exclude_inks=None, extract_tables=True) -> str`
- [02]: `PdfDocument.extract_words` — `extract_words(page, *, include_artifacts=True, region=None, word_gap_threshold=None, profile: ExtractionProfile \| None = None) -> list[TextWord]`
- [03]: `PdfDocument.extract_text_lines` — `extract_text_lines(page, *, include_artifacts=True, region=None, word_gap_threshold=None, line_gap_threshold=None, profile=None) -> list[TextLine]`
- [04]: `PdfDocument.extract_chars` — `extract_chars(page, region=None, exclude_layers=None, exclude_inks=None) -> list[TextChar]`
- [05]: `PdfDocument.extract_spans` — `extract_spans(page, region=None, reading_order=None) -> list[TextSpan]`
- [06]: `PdfDocument.extract_tables` — `extract_tables(page, region=None, table_settings: dict \| None = None) -> Any`
- [07]: `PdfDocument.extract_structured` — `extract_structured(page) -> str` (+ `extract_text_auto`/`extract_page_auto(options_json=)`)
- [08]: `PdfDocument.extract_lines` / `extract_rects` / `extract_paths` — `extract_lines(page, region=)` / `extract_rects(...)` / `extract_paths(...)`
- [09]: `PdfDocument.extract_text_ocr` — `extract_text_ocr(page, engine: OcrEngine \| None = None) -> str`
- [10]: `PdfDocument.to_markdown` / `to_markdown_all` — `to_markdown(page, preserve_layout=False, detect_headings=True, include_images=False, image_output_dir=None, embed_images=True, include_form_fields=True) -> str`
...
|  [04]   | `PdfDocument.extract_chars`                                     | per-glyph extraction with full font/color/origin metrics |
|  [05]   | `PdfDocument.extract_spans`                                     | styled-run extraction → `RunNode` styled spans           |
|  [06]   | `PdfDocument.extract_tables`                                    | native ruled+text table grid → `TableNode`               |
|  [07]   | `PdfDocument.extract_structured`                                | structured/auto-profile extraction (JSON-serialized)     |
|  [08]   | `PdfDocument.extract_lines` / `extract_rects` / `extract_paths` | vector line/rect/path geometry harvest                   |
|  [09]   | `PdfDocument.extract_text_ocr`                                  | OCR a scanned page in-process (Rust OCR engine)          |
...
|  [14]   | `PdfDocument.classify_document` / `classify_page`               | document/page type classification (e.g. form/scanned)    |
...
Call shapes by index:
- [01]: `PdfDocument.render_page` — `render_page(page, dpi=None, format=None, background=None, transparent=False, render_annotations=None, jpeg_quality=None, excluded_layers=None) -> bytes`
- [02]: `PdfDocument.render_page_fit` — `render_page_fit(page, width, height, *, format=None, background=None, transparent=False, render_annotations=None, jpeg_quality=None, excluded_layers=None) -> bytes`
- [03]: `PdfDocument.render_pixmap` — `render_pixmap(page, dpi=None) -> RenderedPixmap`
- [04]: `PdfDocument.render_separation` — `render_separation(page, ink_name, dpi=None) -> SeparationPlate`
- [05]: `PdfDocument.render_separations` — `render_separations(page, dpi=None) -> list[SeparationPlate]`
- [06]: `PdfDocument.flatten_to_images` — `flatten_to_images(dpi=150) -> ...`
...
|  [06]   | `PdfDocument.flatten_to_images`  | rasterize all pages (flatten interactive layers to raster) |
...
Call shapes by index:
- [01]: `PdfDocument.add_redaction` — `add_redaction(page, rect, fill=None)`
- [02]: `PdfDocument.apply_page_redactions` / `apply_all_redactions` — `apply_page_redactions(page)` / `apply_all_redactions()`
- [03]: `PdfDocument.apply_redactions_destructive` — `apply_redactions_destructive(scrub_metadata=True, remove_javascript=True, remove_embedded_files=True, fill=...)`
...
Call shapes by index:
- [01]: `PdfDocument.merge_from` — `merge_from(source: PdfDocument)`
- [02]: `PdfDocument.select_pages` / `move_page` / `delete_page` — `select_pages(pages)` / `move_page(from_index, to_index)` / `delete_page(index)`
- [03]: `PdfDocument.extract_pages` / `extract_pages_to_bytes` / `extract_page_ranges_to_bytes` — `extract_pages(pages, output)` / `extract_pages_to_bytes(pages) -> bytes` / `extract_page_ranges_to_bytes(ranges)`
- [04]: `PdfDocument.rotate_page` / `rotate_all_pages` / `set_page_rotation` — `rotate_page(page, degrees)` / `rotate_all_pages(degrees)` / `set_page_rotation(page, degrees)`; `page_rotation(page)` reads
- [05]: `PdfDocument.crop_margins` / `set_page_crop_box` / `set_page_media_box` — `crop_margins(left, right, top, bottom)` / `set_page_crop_box(page, llx, lly, urx, ury)` / `set_page_media_box(...)`; `page_crop_box`/`page_media_box` read
- [06]: `PdfDocument.page_images` / `extract_images` / `extract_image_bytes` — `page_images(page)` / `extract_images(page, region=None)` / `extract_image_bytes(page)`
- [07]: `PdfDocument.reposition_image` / `resize_image` / `set_image_bounds` — `reposition_image(page, image_name, x, y)` / `resize_image(page, image_name, width, height)` / `set_image_bounds(page, image_name, x, y, width, height)`; `has_image_modifications`/`clear_image_modifications` track
- [08]: `PdfDocument.within` — `within(page, bbox) -> PdfPageRegion`
...
|  [03]   | `PdfDocument.extract_pages` / `extract_pages_to_bytes` / `extract_page_ranges_to_bytes` | split selected pages/ranges to a file or bytes |
...
Call shapes by index:
- [01]: `PdfDocument.get_form_fields` — `get_form_fields() -> list[FormField]`
- [02]: `PdfDocument.get_form_field_value` / `set_form_field_value` — `get_form_field_value(name)` / `set_form_field_value(name, value)`
- [03]: `PdfDocument.export_form_data` — `export_form_data(path, format='fdf')`
- [04]: `PdfDocument.flatten_forms` / `flatten_forms_on_page` — `flatten_forms()` / `flatten_forms_on_page(page)`
- [05]: `PdfDocument.get_annotations` — `get_annotations(page) -> Any` (list of `PdfAnnotation`)
- [06]: `PdfDocument.flatten_all_annotations` / `flatten_page_annotations` — `flatten_all_annotations()` / `flatten_page_annotations(page)`
- [07]: `PdfDocument.get_outline` — `get_outline() -> Any \| None`
- [08]: `PdfDocument.get_layers` / `get_page_inks` / `get_page_inks_deep` — `get_layers() -> list[str]` / `get_page_inks(page)` / `get_page_inks_deep(page)`
- [09]: `PdfDocument.page_labels` — `page_labels() -> Any`
- [10]: `PdfDocument.set_title` / `set_author` / `set_subject` / `set_keywords` — `set_title(title)` / `set_author(author)` / `set_subject(subject)` / `set_keywords(keywords)`
- [11]: `PdfDocument.xmp_metadata` — `xmp_metadata() -> Any`
- [12]: `PdfDocument.embed_file` — `embed_file(name, data)`
- [13]: `PdfDocument.edit_header` / `edit_footer` — `edit_header(page)` / `edit_footer(page)`
- [14]: `PdfPage` (from `page(index)`) — `add_text(text, x, y, font_size=12.0)` / `add_highlight(x, y, width, height, color)` / `add_link(x, y, width, height, url)` / `add_note(x, y, text)` / `set_text(text_id, new_text)` / `find_text_containing(needle)` / `find_images()` / `get_element(id)` / `children()` / `annotations()` / `remove_annotation(index)` / `remove_element(id)`
...
|  [07]   | `PdfDocument.get_outline`                                               | recover the native bookmark outline tree → `StructureNode` |
...
|  [03]   | `PdfDocument.validate_pdf_a`                                    | PDF/A conformance validation (e.g. `"1b"`/`"2b"`/`"3b"`) |
...
Call shapes by index:
- [01]: `PdfDocument.signatures` — `signatures() -> list[Signature]`
- [02]: `PdfDocument.signature_count` — `signature_count() -> int`
- [03]: `PdfDocument.dss` — `dss() -> Dss \| None`

| [INDEX] | [SURFACE]                     | [CAPABILITY]                                                              |
| :-----: | :---------------------------- | :------------------------------------------------------------------------ |
|  [01]   | `PdfDocument.signatures`      | recover every embedded `/Sig` (then `.verify()`/`.verify_detached(data)`) |
|  [02]   | `PdfDocument.signature_count` | embedded-signature count                                                  |
|  [03]   | `PdfDocument.dss`             | recover the `/DSS` document-security store (certs/CRLs/OCSPs/VRI)         |
...
Call shapes by index:
- [01]: `OfficeConverter.from_docx` / `from_pptx` / `from_xlsx` — `from_docx(path) -> Pdf` / `from_pptx(path)` / `from_xlsx(path)` (+ `_bytes(data)` + `convert(path)`)
- [02]: `PdfDocument.to_docx` / `to_xlsx` / `to_pptx` — `to_docx(path)` / `to_xlsx(path)` / `to_pptx(path)` (+ `_bytes() -> bytes` variants)
...
Call shapes by index:
- [01]: `Pdf.from_markdown` — `from_markdown(content: str, title=None, author=None) -> Pdf`
- [02]: `Pdf.from_html` — `from_html(content: str, title=None, author=None) -> Pdf`
- [03]: `Pdf.from_text` — `from_text(content: str, title=None, author=None) -> Pdf`
- [04]: `Pdf.from_html_css` / `from_html_css_with_fonts` — `from_html_css(html, css, font_bytes: bytes) -> Pdf` / `from_html_css_with_fonts(html, css, fonts: list[tuple[str, bytes]])`
- [05]: `Pdf.from_markdown_with_template` — `from_markdown_with_template(content, template: PageTemplate, title=None, author=None) -> Pdf`
- [06]: `Pdf.from_image` / `from_images` / `from_image_bytes` — `from_image(path) -> Pdf` / `from_images(paths: list[str])` / `from_image_bytes(data)`
- [07]: `Pdf.merge` — `merge(paths: list[str]) -> Pdf`
- [08]: `Pdf.from_bytes` / `save` / `to_bytes` — `from_bytes(data) -> Pdf` / `save(path)` / `to_bytes() -> bytes` (+ `len(pdf)` page count)
...
Call shapes by index:
- [01]: `DocumentBuilder` — `DocumentBuilder()` → `.title`/`.author`/`.subject`/`.keywords`/`.creator`/`.language`/`.on_open(script)`
- [02]: `DocumentBuilder.register_embedded_font` — `register_embedded_font(name: str, font: EmbeddedFont) -> DocumentBuilder`
- [03]: `DocumentBuilder.tagged_pdf_ua1` / `role_map` — `tagged_pdf_ua1() -> DocumentBuilder` / `role_map(custom, standard)`
- [04]: `DocumentBuilder.a4_page` / `letter_page` / `page` — `a4_page() -> FluentPageBuilder` / `letter_page()` / `page(width, height)`
...
- [12]: `FluentPageBuilder` links + barcodes — `link_url(url)` / `link_page(page)` / `link_named(destination)` / `barcode_1d(barcode_type, data, x, y, w, h)` / `barcode_qr(data, x, y, size)`
- [13]: `FluentPageBuilder.done` / `new_page_same_size` — `done() -> DocumentBuilder` / `new_page_same_size() -> FluentPageBuilder`
...
|  [03]   | `DocumentBuilder.tagged_pdf_ua1` / `role_map`                              | emit tagged PDF/UA-1 + custom→standard structure role map |
...
|  [13]   | `FluentPageBuilder.done` / `new_page_same_size`                            | seal the page (single-use) / open another same-size page  |
...
Call shapes by index:
- [01]: `sign_pdf_bytes` — `sign_pdf_bytes(pdf_data: bytes, cert: Certificate, reason=None, location=None) -> bytes`
- [02]: `sign_pdf_bytes_pades` — `sign_pdf_bytes_pades(pdf_data: bytes, cert: Certificate, level: PadesLevel, tsa_url=None, reason=None, location=None, revocation: RevocationMaterial \| None = None) -> bytes`
- [03]: `has_document_timestamp` — `has_document_timestamp(pdf_data: bytes) -> bool`
- [04]: `crypto_active_provider` / `crypto_available_providers` — `crypto_active_provider() -> str` / `crypto_available_providers() -> list[str]`
- [05]: `crypto_use_fips` / `crypto_policy` / `crypto_set_policy` — `crypto_use_fips()` / `crypto_policy() -> str` / `crypto_set_policy(spec: str)`
- [06]: `crypto_inventory` / `crypto_cbom` — `crypto_inventory() -> list[str]` / `crypto_cbom() -> str`
- [07]: `generate_barcode_svg` / `generate_qr_svg` — `generate_barcode_svg(barcode_type: int, data: str) -> str` / `generate_qr_svg(data: str, error_correction: int, size: int) -> str`
- [08]: `plan_split_by_bookmarks` / `split_by_bookmarks` — `split_by_bookmarks(src_bytes: bytes, title_prefix=None, ignore_case=False, level=1, include_front_matter=True) -> list` (+ `plan_*` dry-run)
- [09]: `pdf_oxide.pdf_oxide.prefetch_models` / `model_manifest` / `prefetch_available` — `prefetch_models(languages: list[str]) -> str` / `model_manifest() -> str` / `prefetch_available() -> bool` (inner-module access — not top-level re-exported)
- [10]: `pdf_oxide.pdf_oxide.set_max_ops_per_stream` / `set_preserve_unmapped_glyphs` — `set_max_ops_per_stream(limit: int \| None) -> int \| None` / `set_preserve_unmapped_glyphs(preserve: bool) -> bool` (inner-module access — not top-level re-exported)
- [11]: `setup_logging` / `set_log_level` / `get_log_level` / `disable_logging` — `setup_logging()` / `set_log_level(level: str)` / `get_log_level() -> str` / `disable_logging()`
...
|  [06]   | `crypto_inventory` / `crypto_cbom`                                              | cryptographic inventory + CBOM (CycloneDX crypto BOM) |
...
|  [11]   | `setup_logging` / `set_log_level` / `get_log_level` / `disable_logging`         | route the Rust core's logs (bridge into `structlog`)  |
...
Call shapes by index:
- [01]: `AsyncPdfDocument.open` / `from_bytes` / `close` — `await AsyncPdfDocument.open(path, password=None)` / `await AsyncPdfDocument.from_bytes(data, password=None)` / `await doc.close()`
- [02]: `AsyncPdfDocument.<op>` — every `PdfDocument` op as a coroutine (`await doc.extract_words(...)`, `await doc.render_page(...)`, `await doc.apply_all_redactions()`, …)
- [03]: `AsyncPdf.from_*` / `save` / `to_bytes` — `await AsyncPdf.from_markdown(...)` / `await pdf.save(path)` / `await pdf.to_bytes()` (+ all `Pdf.from_*` mirrored)
- [04]: `AsyncOfficeConverter.from_docx` / `from_pptx` / `from_xlsx` / `convert` — `await AsyncOfficeConverter.from_docx(path) -> AsyncPdf` (+ `_bytes` + `convert`)
...
- async axis: `pdf_oxide` owns its async offload — `AsyncPdfDocument`/`AsyncPdf`/`AsyncOfficeConverter` run every op on a built-in single-worker pool, so a consumer already on the loop awaits them directly. For the *sync* `PdfDocument`/`Pdf` root, the rail crosses the runtime `LanePolicy.offload` thread arm (`self.lane.offload(Kernel.of(..., KernelTrait.RELEASING))` — the GIL-releasing Rust render/extract runs off the scheduler under the runtime-owned band, never a folder-minted limiter) — never inline on the event loop, mirroring how `document/egress#FINISH` and `exchange/conformance#CONFORMANCE` cross their native arms. Do not double-offload the async mirror through another thread.
