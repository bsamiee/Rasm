...
The whole materialize-clean-gate-buffer-identify body is one `railed` `expression.effect.result` chain inside the runtime `boundary("array.admit", ...)` exception-to-fault fence: `FiniteGate` short-circuits a non-finite violation to `Error(BoundaryFault(boundary=...))` on the rail through one masked reduction over the `is_lazy_array` lazy/eager fork, a backend-transfer/coordinate-build/lazy-reduction fault converts to a `BoundaryFault` exactly once, and the railed `ContentIdentity.of` `ContentKey` threads through the same chain so a payload from any backend or any `ArraySource` keys identically to its numpy floor. `NamedAxis` carries the labelled-coordinate cell; the `Labelled` source arm admits `xarray.Dataset`/`DataArray` carriers from any producer, extracting `.data` plus coords into `NamedAxis` rows at the boundary — a package-level co-consumption of the branch-tier `xarray` catalog, never a re-owned data interior. The admitted payload graduates outward on the `array_layout` `HandoffAxis` case once its content key reproduces bit-identically across backends, the proof riding the runtime reproduction `ParityReceipt` against the corpus-admitted `array-layout` fixture.

## [01]-[INDEX]

- [01]-[PAYLOAD]: namespace-dispatched array admission over the one `ArraySource` operand-source axis (live, sparse pair, labelled `xarray` carrier) and the one `AdmitMode` output-shape axis, named axes, the `FiniteGate` masked-admission policy carrying the lazy/eager reduction fork, the `SparseLayout` pydata-sparse construction-and-evidence axis, layout, railed content identity, and the `array_layout` graduation producer over the reproduction `ParityReceipt`, on one `ArrayPayload` owner stacking `array-api-compat` resolution under `array-api-extra` extension ops as one `railed` ROP chain.
...
- Source axis: `ArraySource` is the ONE operand-source request — `Live(array)`, `Sparsify(dense, layout, fill_value)`, `SparseFrom(coords, data, shape, layout, fill_value)`, `Labelled(carrier, name)` — and `ArraySource.operand` is the one total `match` that materializes the backend `Array` each case stands for: `Live` is the operand verbatim; both sparse arms share one `COO`-build-then-`SparseLayout.reformat` shape — `Sparsify` lowers a dense source through `sparse.COO.from_numpy(dense, fill_value=fill_value)` (`sparse.asarray` carries no `fill_value`; `from_numpy` is the densify-to-sparse path that sets the implicit dense value), `SparseFrom` builds `sparse.COO(coords, data, shape=shape, fill_value=fill_value)` from the coordinate triple, each reshaping through `reformat` only when the layout is not COO, never a dense intermediate; `Labelled` is the labelled-array arm — `labelled_target` is the one total carrier resolution: an `xarray.DataArray` passes through, an `xarray.Dataset` yields its named variable (or its sole data variable when nameless — a multi-variable nameless `Dataset` raises inside the admission fence as the one `BoundaryFault`, never a phantom `.data` read), and `axes_of` projects the resolved variable's dims/sizes into `NamedAxis` rows the admission gate checks against the materialized buffer — structural attribute reads, no runtime `xarray` import, carriers admitted from any producer. `ArrayPayload.admit` consumes any `ArraySource` and never grows a per-source classmethod; a fifth operand source is one `ArraySource` case plus its `operand` arm.
- Graduation: `ArrayPayload.graduates(parity)` is the self-wired `array_layout` producer — the cross-backend bit-identity proof rides the runtime reproduction `ParityReceipt` graded against the corpus-admitted `array-layout` `CorpusFixture` (the design-pin the runtime corpus carries beside `material-layer-golden`/`glb-by-key`), and the verification folds to the `parity_delta` residual the hub clears against the `_LAYOUT_CEILING` family policy row (bit-identity admits zero delta; a caller's tighter row overrides at the hub). The producer imports the hub downward on the `array_layout` case; the identity seam is runtime EVIDENCE — array decodes and keys, never re-mints.
...
if TYPE_CHECKING:
    ...
    # `jax`/`sparse`/`dask`/`xarray` packages are companion-gated and never import at runtime, so the
    # union is a `TYPE_CHECKING`-only alias whose every arm structurally carries the `shape`/`dtype`/
    # `device` members `array_namespace` admits, read directly off the operand rather than a second Protocol.
...
# the `array_layout` family's DEFAULT graduation ceiling — the governed policy row beside this
# owner's tables per the hub ceiling law; bit-identity admits zero parity delta.
...
class ArraySource:
    ...
    def Labelled(cls, carrier: "LabelledCarrier", name: str | None = None) -> Self:
        # a `DataArray` passes `name=None`; a `Dataset` names the variable to extract (mandatory when
        # it carries more than one) — one case, the carrier shape discriminated by `labelled_target`,
        # never a per-carrier sibling case.
        return cls(labelled=(carrier, name))

    def labelled_target(self) -> "xr.DataArray":
        # the ONE total labelled-carrier resolution both `operand` and `axes_of` read: a `DataArray`
        # passes through, a `Dataset` selects the named variable, and a nameless `Dataset` admits only
        # its sole data variable — a multi-variable nameless `Dataset` raises HERE inside the
        # `boundary("array.admit", ...)` fence (converted to the one `BoundaryFault`), never passing
        # the carrier type to fail later on a phantom `.data`. Structural `data_vars` read, no runtime
        # `xarray` import.
    ...
    def operand(self) -> "Array":
        match self:
            ...
            case ArraySource(tag="labelled"):
                # the labelled-array arm: structural `.data` read off the resolved DataArray — the
                # wrapped backend array rides the same namespace resolution every other source rides.
                return self.labelled_target().data
            case _ as unreachable:
                assert_never(unreachable)

    def axes_of(self) -> tuple[NamedAxis, ...]:
        # coords-to-axes projection for the labelled arm: dims/sizes off the resolved DataArray become
        # the `NamedAxis` rows the admission gate checks against the materialized buffer; every other
        # source is unlabelled and yields the empty tuple.
        match self:
            case ArraySource(tag="labelled"):
...
class ArrayPayload(Struct, frozen=True):
    ...
    def graduates(self, parity: ParityReceipt) -> "RuntimeRail[GraduationReceipt]":
        # the self-wired `array_layout` producer: the cross-backend bit-identity proof rides the
        # runtime reproduction `ParityReceipt` graded against the corpus-admitted `array-layout`
        # fixture; verification folds to the `parity_delta` residual the hub clears against the
        # family ceiling row — bit-identity admits zero delta, a caller's tighter row overrides.
