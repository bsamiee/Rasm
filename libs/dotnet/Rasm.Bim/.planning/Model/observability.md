...
Composition-scoped observability for the BIM-and-exchange engine: `BimPoint` closes the `rasm.bim.<domain>.<point>` roster onto the kernel `IHookRoster` floor and `BimHooks.Live` mints the ONE kernel `HookRail` over it, `BimTelemetry` projects typed facts onto `rasm.bim.<domain>.<measure>` instruments as a rail subscriber — domain code fires facts and observability projects them, zero emit calls inside a projector or codec arm — and `BimBenchClaims` fixes typed, corpus-bound claim identity for every measured BIM lane.
