...
public static class MaterialsBench {
    public static Seq<BenchWorkload> Corpus(Func<BenchInput, UInt128> contentKey) =>
        Seq<(BenchKernel Kernel, BenchInput Input)>(
            ...
            // TileSynth pins BOTH strategy cost classes at the default 32-texel overlap — the graph-cut solve runs
            // Edmonds-Karp per axis over a (2*Overlap+1) x extent vertex grid while the histogram blend is a per-texel
            // field fold, so one averaged row would describe neither.
            (BenchKernel.TileSynth, new BenchInput.Extent(4096, 4096, "graphCut")),
            (BenchKernel.TileSynth, new BenchInput.Extent(4096, 4096, "histogramBlend")),
            ...
            (BenchKernel.HeightSolve, new BenchInput.Extent(2048, 2048, "poisson")),
            (BenchKernel.HeightSolve, new BenchInput.Extent(4096, 4096, "poisson")),
