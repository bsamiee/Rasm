...
`Rasm` owns the RhinoCommon- and Eto-aware geometry, numeric, and interaction kernel below the .NET app strata: the exact-arithmetic floor and the operational geometry plane above it, one body. Its robust floor is necessary, never sufficient: the operational plane composes it into the parametric forms, toolpaths, GPU residency payloads, and persistence naming every upper stratum consumes. Where no admissible package carries the robustness guarantee the kernel authors from first principles; where the ecosystem owns the concern it composes the categorical-best engine, never both.

`Rasm` is the one C# geometry owner and the one owner of host-boundary interaction machinery; the host-free peers meet it only at the content-identity and tessellation wire, while `Rasm.Rhino` and `Rasm.Grasshopper` compose the `Interaction/` plane directly with no adapter of their own. Pure-numeric floor stays host-neutral-shaped without minting a host-free assembly; `Context.Of(RhinoDoc)` and `Analyze.From(RhinoDoc)` remain the only DOCUMENT-coupled entries, and every Eto-shaped owner is a on the interaction plane.

## [01]-[ROUTER]

[DOMAIN]:
- [01]-[RESULTS](.planning/Domain/results.md): Kernel ROP substrate — result union, fault-band registry, re-drive, lease, and transition types.
...
- [06]-[EVALUATION](.planning/Domain/evaluation.md): Closest-point evaluation family — one request union over frames, sampling, and signed distance.
...
- [08]-[HOOKS](.planning/Domain/hooks.md): Extension mechanism — hook seats, the per-composition capsule, and the mount roster frozen at composition.
...
- [26]-[CLOUD](.planning/Spatial/cloud.md): Point-cloud union under one dedup admission — metric, hull, and 3D Voronoi cell arms.
...
- [31]-[CURVE](.planning/Parametric/curve.md): Host-neutral curve op API — one `ParametricOp` union folded over the vendored curve form.
- [32]-[SURFACE](.planning/Parametric/surface.md): Host-neutral surface API — `SurfaceOp` union with UV-provenance tessellation and pullback.
...
- [34]-[DEVELOP](.planning/Parametric/develop.md): Isometric developable strips — geodesic edges, rigid unroll, and per-strip isometry witness.
...
- [37]-[PROJECTIONS](.planning/Parametric/projections.md): Parameter-addressed projection selectors beside the motion, easing, and timing owners.
...
- [41]-[INTERSECT](.planning/Meshing/intersect.md): Predicate-exact intersection table — one `IntersectOp` over an exact-sign crossing carrier.
...
- [46]-[EDIT](.planning/Meshing/edit.md): Mutable-arena owner — the single-writer `MeshEdit` SoA build arena and the publish-by-freeze boundary.
- [47]-[DEC](.planning/Meshing/dec.md): Mesh-bound DEC assembly owner — connection heat, holonomy, and the Hodge decomposition family.
- [48]-[RECONSTRUCT](.planning/Meshing/reconstruct.md): Implicit-reconstruction owner — `Reconstruct` over signed-heat, mesh-SDF, and iso arms.
...
- `UnitsNet` — Dynamic unit conversion behind the `ModelUnit` unit-bridge adapter; geometry measures still leave the kernel as bare `double`.
