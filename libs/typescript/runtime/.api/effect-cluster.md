...
- `SqlMessageStorage.layer` is durable storage over the store `SqlClient`; `ShardingConfig.layerFromEnv` reads runner topology from env/`Config` (the `program/spec` binding); `Sharding.layer` composes the routing service.
...
- Topology is config, discovery is a Layer: `ShardingConfig.layerFromEnv` reads runner address/weight, shard groups, lock intervals, and entity mailbox/idle/timeout budgets from env/`Config` — the `program/spec` `StackOutputs → ShardingConfig` `[SHAPE]` edge. Runner discovery is `RunnerHealth.layerK8s` over `K8sHttpClient`, and the runner entrypoint is selected via `proc/exec` at the app root, so `work` imports no `platform-node/bun` binding.
...
- `program/spec` (`iac`): `ShardingConfig.layerFromEnv` reads deployment topology — the sole `iac ↔ work` meeting, a `[SHAPE]` edge, never an `iac` import.
...
- Route faults through the `ClusterError` `catchTag` ladder, and select the runner entrypoint at the app root via `proc/exec`, `K8sHttpClient` reserved for discovery.
