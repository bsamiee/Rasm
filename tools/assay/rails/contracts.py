...
from tools.assay.rails.contracts_generation import changes, compose_image, freshness_rows, GenerationImage, render
...
_PY_PACKAGE_ROOT: Final = PurePosixPath("libs/python/contracts/src/rasm/contracts")
...
class _Plugin(_Row, frozen=True, kw_only=True):
    ...
    def kinds(self) -> frozenset[_Kind]:
        """Descriptor kinds the selected emitter writes; service rows never leak from a message-only plugin."""
        ...
        if self.protoc_builtin in {"csharp", "python", "pyi"} or name in {"protoc-gen-py", "protoc-gen-es"}:
            return frozenset(("message", "enum", "service")) if name == "protoc-gen-es" else frozenset(("message", "enum"))
...
def _emitted(file: _File, kinds: frozenset[_Kind]) -> _File:
    ...
    return msgspec.structs.replace(
        ...
        methods=file.methods if "service" in kinds else (),
    )
...
def _roster_block(roster: _Roster, files: tuple[_File, ...], roots: tuple[str, ...], distributions: tuple[Distribution, ...] = ()) -> str:
    ...
    tables = tuple(
        ...
        + _table(
            ...
            tuple(
                dict.fromkeys(
                    (f"`{roster.name(kind, dotted)}`", kind, "public-root" if fqn in public else "support-closure", f"`{fqn}`")
                    for file in files
        ...
        match package:
            case Result(
                ...
                export_ok = (
                    ...
                    and frozenset(("dist", "LICENSE", "README.md")).issubset(files)
                    and exports.get(_TS_EXPORT) == _TS_SOURCE
                    and published.get(_TS_EXPORT) == _TS_TARGET
                    and scripts.get("build") == "tsc --build tsconfig.build.json --clean && tsc --build tsconfig.build.json"
...
def _rule_selectors(corpus: _Corpus) -> Iterable[_Finding]:
    ...
    yield from (
        ...
        if not any(
            ...
            and ((kind in {"service", "method"} and "service" in plugin.kinds) or (kind == "message" and "message" in plugin.kinds))
            and fqn in roots[id(plugin)]
            for plugin in plugins
        )
    )
...
def _lane_check(lane: _Lane, scratch: Path, baseline: str = "") -> Check:
    args = ToolArgs(input=baseline) if lane.name == "buf-breaking" else ToolArgs(output=str(scratch / lane.output)) if lane.output else ToolArgs()
    ...
    *,
    ...
    def gate_phase(seed: tuple[Completed, Completed, Completed, str]) -> Result[_Lanes, Fault]:
        ...
        def seated(chk: Check) -> Completed | None:
            ...
            if chk.tool.name == "buf-breaking" and not baseline:
