...
- Law: `TrackInterp` is the AppUi stratum PEER of the kernel one-slerp law, not a copy of it, and the two owners partition by TYPE DOMAIN. `TrackInterp.Pose`/`Rigid` slerp `System.Numerics.Quaternion` over the AppUi `ViewCamera` union and `ElementPose` on the per-keyframe hot path — the scalar wire shapes the viewport, the drafting projection, and the walkthrough encode all already carry — so a sample costs one quaternion blend and no marshal. `[NOT]` the kernel `MotionInterpolation.Rotate`, whose "a second double-reflection implementation here is the deleted form" is scoped to the KERNEL INTERIOR and operates on `Plane`, `Direction`, and `Rhino.Geometry.Quaternion` under a `Context`/`Op` admission rail: routing a camera keyframe through it would marshal `ViewCamera` to `Plane` and back on every sample of every frame of every walkthrough, and would put a host geometry type on a track the standalone shell must interpolate with no host loaded. The kernel law binds where kernel poses interpolate; this owner binds where AppUi camera and element poses do, and a THIRD pose-interpolation site in either stratum is the deleted form on both.
