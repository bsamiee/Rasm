...
| [INDEX] | [SURFACE]                                                                               | [ENTRY_FAMILY]  |
| :-----: | :-------------------------------------------------------------------------------------- | :-------------- |
|  [01]   | `useEffect(fn, deps?)` / `useLayoutEffect(fn, deps?)` / `useInsertionEffect(fn, deps?)` | effect hook     |
...
- [01]-[EFFECT_HOOK]: subscription lifetime, DOM measurement (`floating-ui` anchor read), style insertion; cleanup is the returned fn.
...
- state is layered by lifetime: `react` owns *ephemeral interaction-local* state (`useState`/`useReducer` — the uncommitted, this-frame value), domain/app state is the `@effect-atom` fold, and `useSyncExternalStore` is their only boundary — `@effect-atom/atom-react` is built on it, so an atom's value reaches a component through that subscription and nowhere else and a component never holds a second copy of domain state.
...
- Express async with `use`/`Suspense`/`lazy` + `useTransition`/`useDeferredValue`; never a manual `isLoading` boolean ladder, and never run an `Effect` inside a component body — the atom binding owns execution and the one `runFork` edge is at boot.
- Fire `<ViewTransition>` only from a transition-wrapped update with `addTransitionType` selecting the arm, keep the native `startViewTransition` path (and motion `animateView`) for document-level swaps per the `act/transition` tier law, and admit the canary types with one `react/canary` reference; never author `react/experimental` surfaces, and never ship `cache`/`cacheSignal` as client capability.
