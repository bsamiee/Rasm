...
|   [2]   | source-generated binder interception coverage for `Get<T>` with `BinderOptions.ErrorOnUnknownConfiguration` under EnableConfigurationBindingGenerator | dotnet build of a probe project with the generator property set, then binlog inspection of the emitted interceptors for the `Get<T>` call shape | TYPED_BINDING |
|   [3]   | host-document transition-reload route over the static in-memory mount (re-mount through the host-attach trigger versus a custom reload-token source)  | dotnet run over a scratch ConfigurationManager probe re-adding the host-document snapshot and asserting one options-monitor change invocation   | SOURCE_AXIS   |
