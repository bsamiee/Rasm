...
| [05] | `SqlResolver.void(tag, { Request, execute })` | batch resolver | batched write with no result decode — windowed touch/complete statements |
| [06] | `resolver.execute(input)` / `.cachePopulate(id, result)` / `.cacheInvalidate(id)` | dispatch/cache | run a batched request; seed or evict the per-resolver cache on write |
