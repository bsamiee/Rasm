...
|  [06]   | `clickhouse.users`                | `[{ name, hostIP, grants, … }]` — scoped users; an absent mask renders `0.0.0.0/0`        |
...
[clickhouse.defaultUser]: `password` `password_secret_name` (a Secret keyed `password`) `allowExternalAccess` `hostIP` — `hostIP` defaults to `127.0.0.1/32` and renders as the CR's `default/networks/ip`, so the shipped default user reaches the server from its OWN container and nothing else; `allowExternalAccess` is the chart's only widening switch for that user and it overrides the mask to `0.0.0.0/0` outright, on a user the same template also grants `access_management` and the named-collection secret grants.
[clickhouse.users]: a LIST, not a map — each row takes `name` REQUIRED beside `hostIP` (a string or a STRING ARRAY, both rendering `<name>/networks/ip`), `password_secret_name` (a Secret keyed `password`) or `password_sha256_hex`, `grants` (an array of statements onto `<name>/grants/query`), `profile`, `accessManagement`, and a `settings` map fanned one key per row. This block INVERTS the default user's reach: an absent `hostIP` renders `0.0.0.0/0`, so stating the key on a scoped user NARROWS while omitting it opens — the opposite of what the default-user block trains a reader to expect.
[REMOTE_REACH]: a caller in another pod — a collector exporter, a Grafana driver — cannot use the default user at all, and this chart answers that in exactly two ways: widen the access-managing user to every source address, or mint a scoped `clickhouse.users` row carrying its own mask and grants. Cluster allocation owns the pod CIDR and no chart-time constant names it, so a scoped row masks to the private space a CNI allocates inside while the Kubernetes network fence carries the real edge.
