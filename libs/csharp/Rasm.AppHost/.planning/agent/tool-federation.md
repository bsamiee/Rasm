...
The MCP-client federation surface for the runtime spine: the official `ModelContextProtocol` SDK owns the client session — JSON-RPC framing, the `IClientTransport` connection, the initialize handshake, and `McpClientTool : AIFunction` adoption — and this page inverts `agent/mcp-projection#METHOD_AXIS`, folding each attached external server's tools, resources, and prompts inward as brokered `CapabilityDescriptor` rows under a `federated.{server}.{tool}` surface key. Where the server projection flows descriptor outward to one `Microsoft.Extensions.AI` `AIFunction`, federation flows a peer `McpClientTool` inward to one descriptor, wrapping the same `CommandAIFunction` the projection mints (one tool-adoption seam, never a second `AIFunction` subclass) so a federated call routes through `capability/registry#COMMAND_ALGEBRA` `CommandAlgebra.Run`, the `capability/registry#GRANT_BROKER` admission, the dry-run cost preview, and the `determinism/determinism-and-replay#EVENT_LOG` chain exactly as a native op — the broker gates it, the algebra wraps it in commit-or-rollback, and the event log chains its content hash. The external server is reached over `outbound/outbound-resilience#HOP_AXIS` `OutboundHop` so its bytes ride the existing retry, breaker, and deadline; the peer-tool JSON Schema becomes the descriptor's argument schema content-keyed for the `capability/registry#SDK_CODEGEN` identity gate. The page owns the transport-kind axis, the server admission row and trust scope, the tool/resource/prompt projection inversion, the federated dispatch through the command algebra, and the peer resource-update subscription; it consumes `CapabilityDescriptor`/`DescriptorSurface.Describe`, `CapabilityRegistry`, `CommandAlgebra`/`CommandRuntime`/`GrantBroker`, `ComputeIntent`/`IntentAdmission`, `ToolResult` (reused from `agent/mcp-projection#TOOL_DISPATCH`), `CommandAIFunction` (reused from `agent/mcp-projection#METHOD_AXIS`), `SubscriptionLane`/`ExternalValue` (reused from `live-wire/live-wire#TRANSPORT_BINDING`), `OutboundHop`/`OutboundSurface`, `TenantContext`, `ReceiptSinkPort`, and `CancelScope` as settled vocabulary and mints no eighth port.

## [1]-[INDEX]

| [INDEX] | [CLUSTER]            | [OWNS]                                                                          |
| :-----: | :------------------- | :----------------------------------------------------------------------------- |
|   [1]   | FEDERATION_AXIS      | Transport-kind taxonomy; external-server admission row, trust scope, fault band |
...
|   [4]   | RESOURCE_PROMPT_FOLD | Peer resource/prompt/template projection; resource-update subscription drain    |
...
- Receipt: `FederatedServer` is its own value-object evidence carrying the server key, the transport kind key, the endpoint, and the trust-scope hash; the admission transition logs through one `SpineLog` event in the 1000-1999 band — no parallel admission receipt.
...
public abstract partial record FederationFault : Expected, IValidationError<FederationFault> {
    ...
    public sealed record Text : FederationFault { public Text(string detail) : base(detail, 4800) { } }
    public sealed record TransportRejected : FederationFault { public TransportRejected(string detail) : base(detail, 4801) { } }
    public sealed record HandshakeFailed : FederationFault { public HandshakeFailed(string detail) : base(detail, 4802) { } }
    public sealed record PeerUnavailable : FederationFault { public PeerUnavailable(string detail) : base(detail, 4803) { } }
    public sealed record ToolCallFaulted : FederationFault { public ToolCallFaulted(string detail) : base(detail, 4804) { } }
    public sealed record UntrustedScope : FederationFault { public UntrustedScope(string detail) : base(detail, 4805) { } }
}
```
...
- Boundary: the projection is the only peer-tool-to-descriptor owner — a per-server descriptor table, a hand-mirrored federated tool list, and a second tool catalog are the deleted forms, so every federated tool is a real registry descriptor adopted as the one `CommandAIFunction`; the inversion is the exact mirror of `agent/mcp-projection#METHOD_AXIS` — that page folds `DiscoveryResult` outward to an `McpTool` adopted as `AIFunction`, this page folds `McpClientTool` inward to a `CapabilityDescriptor`, so one tool-adoption seam serves both front doors and a federated call and a native call share one `CommandAIFunction` invoker; the peer-tool `JsonSchema` crosses verbatim as the descriptor's argument schema so the content-addressed codegen identity gate at `capability/registry#SDK_CODEGEN` sees the peer's published schema, never a host-fabricated one; the `McpClient` session is the SDK's — the federation never re-implements the JSON-RPC client, the initialize handshake, or the tool enumeration, it composes `ListToolsAsync` and holds the session; the `TrustScope` ceiling is the trust boundary so a federated descriptor's effect class is the lesser of the peer's declared class and the server's trust, the broker reading the inherited `PermissionShape` with no federation-specific permission path.
...
public sealed record FederationRuntime(
    ...
    Func<McpClientTool, JsonNode> SchemaOf,
    McpRuntime Mcp,
    ClockPolicy Clocks,
...
public static class FederationProjection {
    ...
    static CapabilityDescriptor Descriptor(FederationRuntime runtime, FederatedServer server, McpClientTool tool) {
        ...
        return CapabilityDescriptor.Of(
            ...
            compile: args => FederatedDispatch.Compile(server, tool.Name, args, runtime.SchemaOf(tool)));
    }
...
- Cases: a federated descriptor's `Compile` projects to one `FederatedCall` carrying the server, the tool name, and the payload; the dispatch resolves the peer session, sends `CallToolAsync` over the server's `OutboundHop`, and projects the peer result onto the reused `agent/mcp-projection#TOOL_DISPATCH` `ToolResult` — never a branch-side `ToolResultWire` mint.
- Entry: `Compile(FederatedServer server, string tool, CommandArguments arguments, JsonNode schema)` returns `Fin<ComputeIntent>` — projects the federated tool call into a `ComputeIntent` whose dispatch body is the brokered peer call, so the command algebra admits and dispatches it exactly as any intent; `Call(FederationRuntime runtime, FederatedCall call)` returns `IO<ToolResult>` — resolves the peer session, invokes `McpClient.CallToolAsync(name, args, progress: null, options, ct)` over the `OutboundHop`, and projects the peer `CallToolResult` content blocks onto the reused `ToolResult`.
- Auto: the federated descriptor routes through `CommandAlgebra.Run` so the grant brokerage at `capability/registry#GRANT_BROKER` runs before the peer call — a denied federated call never reaches the peer and never charges the broker's `CostUnit.Calls`/`CostUnit.BytesEgress` ceiling, the dry-run cost preview the `agent/mcp-projection#TOOL_DISPATCH` `McpDispatch.Preview` exposes pricing the federated call against the same standing grant a native call prices against; the `ComputeIntent` the descriptor compiles carries the federated-call body so the Compute dispatch rail invokes the peer through `runtime.Dispatch`, and the dispatch sends `McpClient.CallToolAsync(name, args, progress: null, options, ct)` over the server's `OutboundHop` so the peer call inherits the hop's retry, breaker, and deadline — a flapping peer breaks on the same circuit breaker an HTTP API breaks on, never a per-server retry loop; the peer's `CallToolResult` content blocks and `IsError` flag project onto the reused `ToolResult` (`Tool`/`Content`/`IsError`/`Correlation`) so the federated result rides the existing structured-result wire the agent transport already decodes; a peer-call fault projects to `FederationFault.ToolCallFaulted` whose 4800-band code the command algebra carries as a `CommandFault.ExecutionFaulted`, so a faulted federated call returns a typed transaction disposition, never a thrown exception; the federated call's `CommandReceipt` chains into `determinism/determinism-and-replay#EVENT_LOG` through the same `ReceiptSinkPort.Send` fan a native command's receipt rides, so a federated tool call is content-addressed and replayable exactly as a native op.
- Receipt: the federated call mints one `CommandReceipt` through the command algebra carrying the transaction disposition, the charged cost vector, and the correlation id; the peer result is the reused `ToolResult`, never a parallel federation receipt; the determinism chain seat is the command algebra's `ReceiptSinkPort` fan, so federation owns no direct `EventLog.Append`.
...
- Boundary: the federated dispatch is the only federated-call owner — a direct `McpClient.CallToolAsync` outside the command algebra, a per-server call helper, and an unbrokered peer call are the deleted forms, so every federated call routes through `CommandAlgebra.Run` exactly as `ARCHITECTURE.md#[4]-[PROHIBITIONS]` mandates; the dispatch never executes the peer call directly — it routes through the Compute dispatch rail the command algebra threads, so the transaction, grant, and cost semantics are the command algebra's and the federation is the projection over the SDK client; the peer call rides the server's `OutboundHop` so the external server's bytes inherit the existing resilience and the federation owns no transport retry; the `ToolResult` is the reused `agent/mcp-projection#TOOL_DISPATCH` record ridden as the `ReceiptEnvelopeWire` `TPayload` at `TS_PROJECTION` — a branch-side `ToolResultWire` mint is the named drift defect both this page and the server projection delete; the federated `EffectClass.External` forces the command algebra onto the saga path because no rollback restores a peer's side effect, so a federated write declares a compensation descriptor on the runtime or admits as a single-shot non-compensatable op, never a phantom undo of a remote side effect.
...
public static class FederatedDispatch {
    public static Fin<ComputeIntent> Compile(FederatedServer server, string tool, CommandArguments arguments, JsonNode schema) =>
        Fin.Succ(ComputeIntent.Federated(
            new FederatedCall(server.Value, tool, arguments.Payload, arguments.Correlation),
            schema));
...
public static partial class FederationProjection {
    ...
    static CapabilityDescriptor ResourceDescriptor(FederatedServer server, string uri) =>
        CapabilityDescriptor.Of(
            ...
            compile: args => FederatedDispatch.Compile(server, uri, args, JsonNode.Parse("{}")!));
    ...
    static CapabilityDescriptor PromptDescriptor(FederatedServer server, string name) =>
        CapabilityDescriptor.Of(
            ...
            compile: args => FederatedDispatch.Compile(server, name, args, JsonNode.Parse("{}")!));
    ...
    static CapabilityDescriptor TemplateDescriptor(FederatedServer server, string template) =>
        CapabilityDescriptor.Of(
            ...
            cost: new CostModel(new CostVector(HashMap((CostUnit.Calls, 1L))), static _ => CostVector.Zero),
            permission: server.Trust.Floor(EffectClass.Read),
            compile: args => FederatedDispatch.Compile(server, template, args, JsonNode.Parse("{}")!));
}
...
// keys in the one capability catalog (capability/registry#TS_PROJECTION); the federated tool result
// rides the existing ReceiptEnvelopeWire<ToolResultWire> from agent/mcp-projection#TS_PROJECTION.
...
- [FEDERATED_INTENT]: the `ComputeIntent.Federated(FederatedCall, JsonNode)` case the federated descriptor's `Compile` projects to is the federation's compile target on the Compute intent rail — the case carries the brokered peer call as the intent body so the command algebra dispatches a federated call exactly as a native intent, the Compute dispatch rail invoking the peer through the `FederatedDispatch.Call` body the runtime threads. The `ComputeIntent` federated-case arity confirms against the finalized `Rasm.Compute` intent-and-selection#INTENT_FAMILY surface; a federated tool's side effect is non-rollback so an `EffectClass.External` federated descriptor declares its compensation on the `CommandRuntime` or admits single-shot, the same saga-path the command algebra carries for any irreversible op.
