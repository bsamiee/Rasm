...
- Impressive is measurable, never a mood: the defect class dissolves at its root so it cannot respell anywhere, the owning surface ends denser and more general than a correct-but-local patch leaves it, and capability the page's charter already implies lands as rows, cases, or policy values on existing owners — never as parallel surfaces, and never past what verified API or doctrine grounds.
- Capability mandate: each fixer arms with its folders' `.api/` catalogs through 1-2 read-only mining delegates and, on surfaces already under edit, lands justified capability — verified API stacking, parameterization of hardcoded shapes, polymorphic collapse of flat or repeated arms; every added host member passes the truth rail, correctness outranks addition, and an extension that cannot be grounded lands as a routed idea row instead of code.
...
- Each reviewer surface receives the fact in its OWN idiom, one owner per fact per surface; `docs/` receives a rule only when refute-first proves no reviewer surface owns it — most lessons are already legislated, and the gap is reviewer-config knowledge. A universal pattern proven by fix-wave work may additionally strengthen the owning `docs/stacks/<language>/` fence — snippet-first under that stack's `README.md` page craft, existing snippets as the floor; a pattern no fence owns may land as a new snippet on the card that lacks one, agnostic and non-anchoring, never forced.
- Accumulating sibling rulings consolidate: when several guards or rules share one root, one richer, better-defined rule REPLACES them all — sharpening over adding is the standing direction, and conflicting or overlapping directives are the defect the consolidation removes. Reviewer prose obeys the trim law on every pass: identical guidance in fewer words, ordering repaired, ambiguity dissolved — signal per token rises, payload never drops.
- A recurring refuted class hardens the existing owner's wording in place — never a duplicate owner, never a swollen blob; format gates (prose gate, YAML/JSON parse) run before landing, and the repo's reviewer surfaces stay mirrored at equal depth.
- Each surface grows at its own grain: a CodeRabbit fact is one clause inside its owning `path_instructions` block, never a per-fact path row; a Greptile lesson is one sentence in its owning `rules.md` section or one id-bearing, severity-scoped `config.json` row a child can disable — an outgrown row splits into sibling ids; a Macroscope file owns one topic, and an outgrown topic becomes a sibling file.
- Distillation commits and pushes to origin immediately, on its own lane: reviewer-config and docs hardening never waits on, mixes with, or rides the branch carrying the reviewed work — hosted reviewers read the indexed default branch, so an unpushed guard protects nothing. The lane's commit scope is the reviewer configs, `docs/`, and `tools/`; every other surface stays uncommitted so iterative rounds keep reviewing it.
