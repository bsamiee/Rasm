export const meta = {
  name: 'rebuild',
  whenToUse: 'The standing hostile rebuild pass for any libs/ planning corpus: pass targets (file / sub-folder / package root, any number); it plans, discovers, hostile-implements, critiques, and red-teams every page batch CONCURRENTLY at the owning-language doctrine bar, then a read-only finder fan and one terminal fixer close the run.',
  description: 'Language-agnostic rebuild engine over libs/{csharp,python,typescript} planning corpora. args = a target path, an array of paths, or {targets}; empty = no-op; language derives from the target root and selects the doctrine, both .api tiers, casing, and the member-verification rail. Plan (1 sonnet) expands targets to pages in dependency order under the owning-package charter (full expansion, charter-demanded new pages, settled-page skip). Build runs ALL batches across all package lanes concurrently under one global cap: per batch, Discover (opus, read-only, 1 per batch of 4 pages) deep-reads pages + folder + BOTH .api tiers + the doctrine, emits per-page reading maps, and writes a grounding dossier of verified primary extracts; then implement, critique, and redteam (all fable) chain behind their own batch\'s prior stage only — each stage re-reads the CURRENT on-disk state of its pages and every sibling page they compose, so concurrently landed sibling work is composed as found, never assumed from the dossier snapshot. A dead implement skips its batch\'s reviews; failed pages are reported and the run continues. Close: a read-only opus finder fan (one per ~6 landed pages plus one governance finder over the owning packages\' index docs) reads the landed corpus fresh from disk, hunts missing/wrong/faked/naive/drift/phantom work with typed anchored findings, and discards anything already resolved on current disk; ONE terminal fable fixer applies every reported index row (it is the run\'s sole writer for package README/ARCHITECTURE and central manifests), re-verifies findings as signals, fixes everything in place plus its own hunt past the signal list, and returns the final fixlog. Nothing follows the fixer.',
  phases: [
    { title: 'Plan', detail: 'one thin agent expands the targets into the dependency-ordered page list under the owning-package charter: every existing page enters as rebuild, a charter-demanded absent page as new, a charter-settled page is skipped', model: 'sonnet' },
    { title: 'Build', detail: 'all batches across all lanes concurrent under the one global cap: per batch discover (opus) then implement then critique then redteam (fable), each stage chained only behind its own batch; the current-state law composes concurrently landed sibling work' },
    { title: 'Close', detail: 'read-only opus finder fan over the landed corpus plus one governance finder, stale residuals discarded; ONE terminal fable fixer applies the index rows, resolves findings as signals, hunts past them, returns the final fixlog' },
  ],
}
...
const STALL = 300000
const IMPL_BATCH = 4 // pages per batch; one discover + one build pipeline per batch
const FINDER_PAGES = 6 // landed pages per close-phase finder
...
const PLAN_SCHEMA = { type: 'object', additionalProperties: false, required: ['packages', 'pages'], properties: {
  ...
  pages: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['page', 'kind'], properties: {
    page: { type: 'string' }, kind: { type: 'string', enum: ['new', 'rebuild'] } } } }, // ARRAY ORDER IS DEPENDENCY ORDER — the engine never re-sorts
...
const WORK = { type: 'array', items: { type: 'object', additionalProperties: false,
  ...
  page: { type: 'string' }, kind: { type: 'string', enum: ['new', 'rebuild'] },
...
const FIXER_SCHEMA = { type: 'object', additionalProperties: false,
  required: ['files', 'indexApplied', 'resolved', 'beyond', 'rejected', 'summary'], properties: {
...
const dossierPath = (pkg, i) => SCRATCH + '/' + pkg.split('/').pop() + '-b' + i + '.md'
// Preserves plan emission order (dependency order); dedupe by page, first wins.
...
// MAP_BY_PAGE — the per-page reading map Discover surfaced, filled per batch before its build stages.
const MAP_BY_PAGE = new Map()
const mapFor = (p) => MAP_BY_PAGE.get(p.page || p) || { page: p.page || p, kind: p.kind || 'rebuild',
...
const BUILD_LAW = 'BUILD LAW — buildout over removal, always. Removal authority is reserved to ONE case: a PHANTOM — a cited ' +
  'member that does not exist. An underutilized catalog, an orphan-looking admission, or a weak fence is an INTEGRATION ' +
  'target: the capability lands as ' + L.ownerGrammar + ' — inside the existing owner, reshaped as if always carried — or is ' +
  'wired into its owning sibling page in the same pass. Never a parallel type, a new file, a sibling shape, or flat appended ' +
  'code; never extract a file to cut LOC; never regress existing capability. Structural collapse and CAPABILITY completeness ' +
  'are orthogonal — a fully collapsed owner can still model a naive slice; close both. Every extension cites exactly one gap ' +
  'source. PACKAGE — a member the admitted surface exposes that the concept admits but the page ignores (' + L.gapPkg + '). ' +
  'DOMAIN — an attribute, metric, sub-kind, relationship, state, or operation the real concept demands (' + L.gapDomain + '). ' +
  'CONSUMER — a contract a sibling or downstream owner will require. Byte-count is a weak proxy: assess every owner against ' +
  'its full domain and both-tier package surface independently of size. Anticipate the FIVE-TIMES demand: model each owner ' +
  'for five times today\'s cases, fields, and consumers — a thin slice built "for now" is the COVERAGE defect by definition. ' +
...
const WRITE_FULLY = 'WRITE FULLY — every fix you identify you make NOW via Edit/Write; the fix-log is a report of edits ' +
  ...
  'report the exact rows you need in `indexRows`; the run\'s terminal fixer applies them once.'
const CURRENT_STATE = 'CURRENT STATE — sibling batches land work concurrently with yours. Before any edit, re-read the ' +
  ...
  'picked up as found, never assumed from the dossier snapshot — the dossier grounds verified `.api` extracts, never ' +
...
const HUNT = 'HUNT CLASSES: missing (an owner, case, field, seam counterpart, or capability the charter or the landed design ' +
  ...
  'that does not exist). Every finding carries a file anchor. Verify cited external members against the .api catalogs; never ' +
  'trust page prose about itself.'
const preamble = (batch, dossier) => [CONTEXT, readFirst(pkgOf(batch[0].page), dossier), STANCE, BUILD_LAW, BODY, VERIFY,
  WRITE_FULLY, CURRENT_STATE, PROSE_COMMENTS].concat(L.mechanics ? [L.mechanics] : []).concat([readingMap(mapsFor(batch))])
...
const planPrompt = () => [CONTEXT,
  ...
  'EMIT `pages` IN DEPENDENCY ORDER — grouped by sub-folder, foundations before their consumers, alphabetical within a ' +
  'group; the engine batches in your emitted order and never re-sorts, so early rows tend to land early.',
].join('\n\n')
const discoverPrompt = (batch, dossier) => [CONTEXT, readFirst(pkgOf(batch[0].page), ''), STANCE, VERIFY,
  ...
  'read its concept in the owning-package charter (ARCHITECTURE.md + README.md + IDEAS.md) plus its nearest sibling pages. ' +
  'Read the folder at large — the sibling pages each composes and the owning-package index docs — as full-file reads. ' +
  'DISBELIEVE the page: prose claiming a package is composed is verified against the fence body; attack every admitted ' +
  'catalog (both tiers) for the members, combinators, generated surfaces, and native pipelines the concept ADMITS but no ' +
  'fence exploits, and DIFF the complete admitted inventory against the whole folder — a capability no page exploits is a ' +
  'named integration gap ROUTED to EVERY page whose concept admits it, never one "best" owner alone. SINGLE-CONSUMER ' +
  'EXPANSION: a package with a catalog at ANY tier (central manifest, shared substrate, folder) consumed by only ONE page — ' +
  'even deliberately narrowly — is expansion pressure on its siblings: name the package, its unexploited members in exact ' +
  'spellings, and each candidate page where deeper integration would extend the capability. Discovery has ZERO removal ' +
  'authority: an underutilized catalog, orphan-looking admission, or weak fence is always framed as a buildout target ' +
  '(which owner grows which case/row/field/operation), never as removal evidence. ANTI-ANCHOR LAW: your maps and dossier ' +
  'carry FINDINGS, never designs — quality defects graded against the doctrine read at source (name the law and the ' +
  L.stack + ' pattern whose application would most deeply transform the page — the collapse, the owner form, the rail — ' +
  'never the resulting code) and capability inventory in catalog-anchored spellings; a fence sketch, a prescribed shape, or ' +
  'a pre-ruled design in the dossier ANCHORS and WEAKENS the rebuild and is your defect — the implement agent rules every ' +
  'design.\n' +
  ...
  'member the maps cite, the branch ARCHITECTURE.md [02]-[SEAMS] rows covering these pages, and folder-context anchors. ' +
  'FORBIDDEN in the dossier: any doctrine-law digest, summary, or paraphrase; any removal framing; any claim without a ' +
  '`file:line` anchor; any prescriptive design, fence sketch, or code-shape ruling — findings name the defect, the law, ' +
  'and the capability, never the resulting code. ' +
  'Downstream agents spot-verify your anchors — a fake anchor is your defect, surfaced loudly. Return worklist + dossier ' +
  '(the path).',
].join('\n\n')
const implementPrompt = (batch, dossier) => preamble(batch, dossier).concat([
  ...
  'entry/receipt seam owners where the folder has them. kind=`rebuild`: HOSTILE GROUND-UP REBUILD in place. Construct in ' +
  'LIFECYCLE order — admit raw once, canonical owner by OWNER_CHOOSER, stacked rail/aspect over a thin pure core, ' +
  'projection, egress, BOTH ingress and egress parameterized; collapse parallel shapes into ' + L.collapseInto + '; one ' +
  'polymorphic entrypoint per modality. COMPOSE the map\'s `apiUsed` at full operator depth, STACK every ' +
  '`apiUnderutilized` into the owner, and CONFIRM no other admitted catalog is missing. CLOSE the concept capability gaps ' +
  'per BUILD LAW. ' + L.modern + '; ' + L.fileOrg + '; high-signal all-backticked prose. Return the fix-log.',
]).join('\n\n')
const critiquePrompt = (batch, dossier) => preamble(batch, dossier).concat([
  ...
  batch.map((p) => p.page).join(', ') + '. Audit every fence against the doctrine you read at source, never a summary, and ' +
  'against the CURRENT on-disk state of every file it composes per CURRENT STATE; repair every hit now — a fix, never a ' +
  'ledger note; a cross-file hit is yours per WRITE FULLY.\n' +
...
const redteamPrompt = (batch, dossier, crit) => preamble(batch, dossier).concat([
  ...
  'author and critique missed things.\n' +
  ...
  L.fileOrg + ', both-tier .api maximization, prose + comment hygiene — each judged against the CURRENT disk state per ' +
  'CURRENT STATE. VERIFY the critique\'s `seamsTouched` landed on BOTH ends of every seam; make any missed repair ' +
  'yourself. CRITIQUE RESULT: ' + JSON.stringify(crit || {}) + '. Return the batched fix-log.',
]).join('\n\n')
const finderPrompt = (pages, i) => [CONTEXT, HUNT,
  ...
  'slice. STALE DISCARD: judge only CURRENT disk — a defect already resolved on disk, at either end of a seam, is DROPPED, ' +
  'never reported. Findings are INFORMATION for the terminal fixer, never prescriptions: name the defect, the law or ' +
  'catalog member it violates, and the exact file anchor — never the resulting code, a fence sketch, or a ruled design. ' +
  'Return typed anchored findings.',
].join('\n\n')
const govFinderPrompt = (pkgs, pages, rows) => [CONTEXT, HUNT,
...
const fixerPrompt = (findings, rows, failed, pages) => [CONTEXT, HUNT, VERIFY, PROSE_COMMENTS,
  'TASK: TERMINAL FIX (WRITER — you are the run\'s LAST agent, nothing follows you; full write authority over the landed ' +
  'corpus, and you are the run\'s SOLE writer for the owning-package index docs (ARCHITECTURE.md + README.md) and the ' +
  'central manifests). Landed pages: ' + JSON.stringify(pages) + '.\n' +
  ...
  'hand-edits the grouped manifest at the SYMBOL anchor (never a line number), preserving label-group order: ' +
  JSON.stringify(rows) + '.\n' +
  '(2) FINDINGS are SIGNALS, not law: re-verify each on disk; never re-litigate a correct finding — implement the ' +
  ...
  'a dead anchor or already resolved on disk is rejected with reason: ' + JSON.stringify(findings) + '.\n' +
  '(3) OWN HUNT: hunt PAST the signal list on your own authority — the hunt classes above over the landed pages and the ' +
  ...
  'manifest rows, .api anchors; wire-canonical names stay frozen. FAILED PAGES (reported, not landed — never author them ' +
  'here; correct any index or sibling claim that pretends they landed): ' + JSON.stringify(failed) + '. ' +
  L.modern + '; ' + L.fileOrg + '. Return the final fixlog.',
].join('\n\n')
...
log('Plan[' + LANG_KEY + ']: ' + PAGES.length + ' pages (' + PAGES.filter((p) => p.kind === 'new').length + ' new, ' +
  PAGES.filter((p) => p.kind === 'rebuild').length + ' rebuild); CAP=' + CAP)
if (!PAGES.length) {
  log('No pages resolved under the targets')
  return { targets: TARGETS, language: LANG_KEY, total: 0 }
}

phase('Build')
// One flat batch set: lanes keep sub-folder grouping + plan dependency order for batch composition
// (foundations tend to launch early), but nothing serializes on it — the pool is the only governor.
const lanes = [...new Set(PAGES.map((p) => pkgOf(p.page)))].map((pkg) => ({ pkg, pages: PAGES.filter((p) => pkgOf(p.page) === pkg) }))
const BATCHES = lanes.flatMap((lane) => chunk(lane.pages, IMPL_BATCH).map((pages, i) => ({ pkg: lane.pkg, i, pages })))
const built = (await pool(BATCHES, CAP, async (b) => {
  const tag = b.pkg.split('/').pop() + ':b' + b.i
  const dossier = dossierPath(b.pkg, b.i)
  const disc = await agent(discoverPrompt(b.pages, dossier),
    { label: 'discover:' + tag, phase: 'Build', model: 'opus', effort: 'high', schema: DISCOVER_SCHEMA, stallMs: STALL })
  ;((disc && disc.worklist) || []).forEach((m) => { if (m && m.page) MAP_BY_PAGE.set(m.page, m) })
  const fix = await agent(implementPrompt(b.pages, dossier),
  ...
  const crit = await agent(critiquePrompt(b.pages, dossier),
    { label: 'crit:' + tag, phase: 'Build', model: 'fable', effort: 'high', schema: REVIEW_SCHEMA, stallMs: STALL })
  const rt = await agent(redteamPrompt(b.pages, dossier, crit),
...
if (!LANDED.length) {
  ...
  return { targets: TARGETS, language: LANG_KEY, batches: BATCHES.length, landed: 0, failed: FAILED }
}

phase('Close')
const PKGS = [...new Set(LANDED.map(pkgOf))]
const finderTasks = chunk(LANDED, FINDER_PAGES).map((pages, i) => ({ gov: false, pages, i })).concat([{ gov: true }])
const found = (await pool(finderTasks, CAP, (t) => t.gov
  ? agent(govFinderPrompt(PKGS, LANDED, ROWS), { label: 'finder:gov', phase: 'Close', model: 'opus', effort: 'high', schema: FINDINGS_SCHEMA, stallMs: STALL })
  : agent(finderPrompt(t.pages, t.i), { label: 'finder:s' + t.i, phase: 'Close', model: 'opus', effort: 'high', schema: FINDINGS_SCHEMA, stallMs: STALL })
)).filter(Boolean)
const FOUND = found.flatMap((f) => f.findings || [])
log('Close: ' + FOUND.length + ' finding(s) from ' + found.length + ' finder(s); ' + ROWS.length + ' index row(s) pending')
const fixer = await agent(fixerPrompt(FOUND, ROWS, FAILED, LANDED),
  { label: 'fixer', phase: 'Close', model: 'fable', effort: 'high', schema: FIXER_SCHEMA, stallMs: STALL })
return { targets: TARGETS, language: LANG_KEY, batches: BATCHES.length, landed: LANDED.length, failed: FAILED,
  findings: FOUND.length, fixer: fixer && { files: (fixer.files || []).length, indexApplied: (fixer.indexApplied || []).length,
  resolved: (fixer.resolved || []).length, beyond: (fixer.beyond || []).length, rejected: (fixer.rejected || []).length,
