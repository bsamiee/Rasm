...
- [05]-[CRASH](.planning/signal/crash.md): crash capture enriched through the kernel fault-enricher contract (`FaultCapture` round-trip), with session-replay redaction-at-capture.
...
- `@effect/platform` — the `HttpClient.HttpClient` requirement the native `Otlp` lane rides; the root satisfies it with the shared net-client policy.
