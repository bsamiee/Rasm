...
- Entry: `public Fin<ProfileUnit> ToUnit(Context context, Op key)` on `TimberSection` — the section→`ProfileUnit` projection (`WidthMm` = section width, `HeightMm`/`CourseHeightMm` = section depth, `LengthMm` = the lamella module a glulam/CLT layup repeats, or the section depth for sawn) so a timber member flows through the SAME `Construction/layout#ASSEMBLY_FOLD` `Resolve` fold; `public int LamellaCount()` the `[BoundaryAdapter]` ply/lamella count the layup seam reads, and `public Fin<ComputedSection> Section(Op key)` the elastic section properties through the shared `profile#PROFILE_OWNER` `ParametricSection.Rectangle` solver (strong-AND-weak-axis modulus/inertia/radius), the design seam's section the same owner steel/cmu compute through.
- Packages: Rasm (project — `PositiveMagnitude` for every fractional-millimeter section column), VividOrange.Sections.SectionProperties (via the shared `profile#PROFILE_OWNER` `ParametricSection.Rectangle` bridge; `.api/api-vividorange-sections-sectionproperties.md`), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox (`FrozenDictionary`).
...
public readonly record struct TimberSection(
    ...
    // and the SAME owner steel/cmu compute through. A CLT panel's cross-ply effective stiffness is the gamma-method
    // refinement over this gross-rectangle base, read from the lamella columns (TIMBER_LAYUP_GEOMETRY).
...
- [TIMBER_LAYUP_GEOMETRY]: REALIZED — the rectangular section modulus/inertia is the `TimberSection.Section` projection over the shared `profile#PROFILE_OWNER` `ParametricSection.Rectangle` solver, not a hand-rolled `W·D²/6` literal, so a sawn/glulam member computes its strong-AND-weak-axis properties through the SAME polygon integral steel/cmu run. A CLT panel's cross-ply layup (alternating longitudinal/transverse plies) drives the effective-section composite stiffness the design seam computes by the gamma-method refinement OVER that gross-rectangle base — the per-ply orientation and the rolling-shear modulus are `TimberSection` column growth, never a parallel section owner, the lamella thickness/count and the `TimberForm.CrossPly` discriminant already carrying the layup receipt the `IfcRectangleProfileDef` rectangle wire and the gamma-method read. The CLT panel crosses the IFC wire as the rectangle outer profile, the layup carried in the material-profile-set layers.
