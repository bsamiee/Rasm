...
`Rasm` rulings settle package-scoped decisions.

## [01]-[PACKAGES]

Every reopen predicate rides the license gate.
...
- `NodeVerdict` is the ONE pruning vocabulary both `NodeWalk` arms read — a graph arm re-colours the whole store per query item a descent skips.
- `Analysis/query` owns the ONE analysis entry — `AnalysisQuery` and its verb roster front one `Analyze` class, peers publishing their own builders.
- `AnalysisVerb` is the ONE analysis roster and its generic operation build stays on the union — a delegate column carries no caller-chosen generic.
- Every `[SmartEnum]` roster proof rides an accessor the domain already reads — a `Lazy` behind a readerless `static` member never runs.
- Every cross-product `[SmartEnum]` mints through its `Of(<coordinates>)` fold alone — a consumer storing a coordinate beside the row forks it.
...
- `Domain/telemetry` splits by dependency direction — `TelemetryContributorPort` seats at the top page and every remaining edge points down.
