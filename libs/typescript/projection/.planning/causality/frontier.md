...
- [1]-[STABILITY_FRONTIER]: Owns `StabilityFrontier`, `cursorPositions`, `cursorOrder`, `meetCursors`, `frontierMerge`, the `stabilityFrontier` per-cursor-set fold, and the `stableBelow`/`stableMeet` read predicates the GC, the as-of `Stable` coordinate, and the Merkle prefix consume.
- [2]-[FRONTIER_FACE]: Owns `frontierProjection`, the `Projection<VersionVectorWire>` `Subscribable` read face that lifts the meet store into the one atom-bridge surface `retention#CAUSAL_FINALIZE` and the `query/asof#AS_OF_QUERY` `Stable` coordinate subscribe to without reaching the raw `SubscriptionRef` interior.
