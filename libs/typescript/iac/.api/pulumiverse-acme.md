...
- One `Registration` per directory per deployment binds `serverUrl` as provider data: a staging directory for proof stacks, production for live ones, switched by provider construction, never resource edits; `revokeCertificateOnDestroy: true` where a torn-down stack must not leave a live cert.
