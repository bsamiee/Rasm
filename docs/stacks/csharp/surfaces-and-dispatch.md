...
- Reject: a request-union-shaped success-or-failure carrier; carriers own outcome transport.
...
- Boundary: input validates once at the factory and behavior is reached once through dispatch; positional cases stay constructible with already-admitted payloads, and construction scopes fully to one owner only in the nested form — access runs one way, so any member the enclosing type reads back off the nested record is `internal`, never left implicitly private (CS0122).
...
- Use: `Argument<TResult>` only on `MapPartially`'s per-case slots and on bespoke factory overloads; the generated `ValidateFactoryArguments` takes `ref T` and `Validate` takes raw `TValue?`, so `Argument<T>` in either generated hook is a compile-time mismatch.
...
- Law: the error vocabulary stays owner-chosen because `TError` carries its own static abstract `Create`; bounding `TError` by the shared fault base keeps every owner's precise faults liftable with no translation hop.
- Law: a generic bound binds only the signature that spells it, so every interior core a bounded entrypoint forwards to repeats the bound — a constraint on the forwarding method alone leaves the unconstrained core a compile-valid bypass for its direct callers.
...
[OPEN_FLOOR_DISPATCH]:
- Law: when a foreign assembly supplies the implementation, the boundary inverts again — an instance-interface floor (`IProjection<TIn,TOut>`, one member returning the result) is the open extension point, a minting factory selects the concrete the consumer never names, and one polymorphic operation dispatches over the floor — distinct from the closed family the owner exhaustively `Switch`es and never publishes.
- Law: the discriminant between the two forms is direction of extension — an open point others plug into (a constraint, a projection, a store) is an instance-interface floor returning `Fin<T>` or `Validation<Error,T>`, a closed family the owner controls is a `[Union]`/`[SmartEnum]` + total `Switch`; an interface over the family the owner closes forfeits `Switch` totality, and a `[Union]` a foreign assembly must extend forfeits the seal.
- Law: the open constraint set and the owner's closed `Switch` co-exist on one owner — the shape switches internally, an `IConstraint<T>` floor folds independent violations at the edge — so a foreign-supplied constraint is the accumulating-`Validation` floor and never a case the owner closes; the fold mechanic and its monoid are the carrier page's, composed here as the floor's consequence.
...
- Reject: rejection logic in the constructor hook or null/blank guards in the factory hook — the weave order fixes absence below both, so either placement is dead code, and a throwing hook breaks the carrier bridge above the boundary.
...
- Law: every transformer satisfies one shape — `Func<IO<Report>, IO<Report>>` — so the rank-ordered fold keeps the carrier and the result intact through an arbitrary-depth stack, and the rank lives on the vocabulary item rather than a bent comparer so the order is recoverable from the declaration alone.
