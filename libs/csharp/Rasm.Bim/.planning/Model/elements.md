...
- [OVERLAY_SEEDS]-[OPEN]: which classes need a `ClassIntroductions` floor row, which schema-abstract concretes need an `AbstractSupertypes` row, which retired subtypes carry which deprecation-versus-removal `Retirements` window, and which GROWN enum members beyond `IfcWall.PARAPET` need a dotted token pin; run `IfcVocabularyEmitter.Emit` over the pinned `GeometryGymIFC_Core` assembly with empty overlays and read the `introduction-miss`/`domain-root-miss` failures and the emitted-versus-schema token-span diff, then seed each overlay from that output.
