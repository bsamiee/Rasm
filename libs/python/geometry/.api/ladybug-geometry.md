...
- `trimesh`/`manifold3d`(`.api/trimesh.md`, `geometry/.api/manifold3d.md`): `Polyface3D`/`Mesh3D` serialize via `to_obj` into `trimesh`/`manifold3d` for robust watertight CSG and repair, and `Polygon2D` converts into `shapely` for industrial-scale planar ops.
