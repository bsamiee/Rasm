...
[STACKING]:
- `Microsoft.Extensions.AI.Abstractions`(`libs/dotnet/.api/api-extensions-ai.md`): the abstractions tier owns `IChatClient`, `IEmbeddingGenerator`, `AIFunction`, `DelegatingChatClient`, `IChatReducer`, and `AIJsonUtilities`; every decorator subclasses `DelegatingChatClient`, `Build` emits an `IChatClient`, and structured output reads its schema from `AIJsonUtilities`.
- `OpenTelemetry`(`libs/dotnet/.api/api-opentelemetry.md`): `UseOpenTelemetry(source)` emits the `gen_ai.*` GenAI span and metric conventions on the `ActivitySource`/`Meter` the OTel composition root admits through `AddSource`/`AddMeter`.
- `api-hybrid-cache.md`(`libs/dotnet/.api/api-hybrid-cache.md`): `UseDistributedCache` binds the resources-lane `HybridCache` surfaced as `IDistributedCache`, so the model response cache and the suite content cache share one store.
