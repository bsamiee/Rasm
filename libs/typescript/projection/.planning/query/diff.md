...
- [1]-[AS_OF_DIFF]: Owns `DiffEntry`, `DiffSummary`, `valueEquivalence`, `diffEntry`, `summarize`, and `asOfDiff` — the closed verdict family, the one fact-stream count carrier, and the two-coordinate diff fold over the materialized snapshots.
- [2]-[DIFF_PROJECTION]: Owns `AsOfDiffView`, `diffSince`, and `asOfDiffStore` — the live `Projection`-faced diff the conflict inspector binds through the `fold/projection#PROJECTION` `Atom.subscribable` seam, re-materializing the diff whenever either coordinate's store advances.
