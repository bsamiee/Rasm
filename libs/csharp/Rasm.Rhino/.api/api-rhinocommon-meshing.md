...
- edit additions — `Weld(double angleToleranceRadians, bool preserveSurfaceParameters)`, instance `MatchEdges(double distance, bool rachet) : bool`, `ExplodeAtUnweldedEdges() : Mesh[]`, `CreateUnweldedMesh(Mesh) : Mesh`, `CreateFromPatchSingleFace(Mesh, IEnumerable<ComponentIndex>) : Mesh`, `CreateFromFilteredFaceList(Mesh, IEnumerable<bool>) : Mesh`, `WithShutLining(bool faceted, double tolerance, IEnumerable<ShutLiningCurveInfo>) : Mesh`, `MeshFaceList.ExtractFaces(IEnumerable<int>) : Mesh`.
