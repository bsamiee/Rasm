...
from rasm.artifacts.delivery.notice import TransmittalNotice
    ...
    async def _emit(self) -> RuntimeRail[ArtifactReceipt]:
        ...
        match settled:
            case Result(tag="ok", ok=(evidence, receipt)):
                # the branch's ONE `Actor.USER` leg: an issue for construction is a NAMED party's act under an
                ...
                # Soft terminal notice: the CloudEvents envelope seals from the settled receipt under the live trace
                # context; its rail drops deliberately, because a notice fault is receipt-stream evidence rather than
                # a close break, and it fires off the SETTLED close whichever way the durable plane answered — the
                # issue happened, and letting a plane fault silence its announcement inverts which of the two is soft.
                TransmittalNotice.issued(receipt, self._deliverable.issued_register, extensions=_extended(evidence))
...
def _extended(evidence: TransmittalEvidence, /) -> frozendict[str, str | int]:
    # Notice extension attributes obey the CloudEvents lowercase-alphanumeric grammar; each value is a settled
    # evidence scalar an ingesting system routes on, never a re-derived fact.
    return frozendict({
        "purpose": evidence.purpose,
        "revision": evidence.revision,
        "issuedat": evidence.issued_at,
        "recordstate": evidence.record_state,
        "padeslevel": evidence.pades_level,
