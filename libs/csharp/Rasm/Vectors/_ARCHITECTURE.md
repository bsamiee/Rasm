...
1. `[implemented]` Matrix solve ownership is in `Matrix.cs`: dense LU/QR/Cholesky, sparse BiCGStab with MathNet fallback, CSparse Cholesky, dense/sparse/generalized eigen paths, and QR least-squares receipts exist. Dense, sparse, and CSparse solve paths share finite RHS/output admission through the existing solve rails; solve/eigen stops own usable status; eigen receipts reject nonfinite known payload shapes; sparse generalized eigen validates both matrix inputs and reports factor nonzeros; and LOBPCG solves the normalized symmetric matrix while preserving finite max-iteration exhaustion receipts as diagnostics. Remaining matrix work is broader sparse/Hermitian result hardening and ensuring consumers treat only usable receipt stops as consumable eigenvectors.
...
3. `[implemented]` Cloud, alignment, and transport diagnostics are real: `CloudCorrespondenceSet` feeds ICP and Sinkhorn, `SinkhornReceipt` reports computed residual, canonical `CloudTransportPolicy`, tolerance, cutoff, and retained coupling facts, Sinkhorn correspondences compute retained-coupling confidence, weighted mass reaches metrics/alignment/transport, convex hulls route through `Mesh.CreateConvexHull3D`, footprint hulls route through `PolylineCurve.CreateConvexHull2d`, `CloudKernel.MassOf` is the mass admission rail for cloud/sample/alignment/transport consumers, and true GICP remains explicit unsupported instead of approximated.
4. `[implemented]` Mesh and spectral receipts exist for topology, features, flatten, remesh, descriptors, scalar isolines, SDF, and segmentation. `MeshSegmentation` now executes scalar threshold components, scalar band components, seeded scalar region growing, and descriptor-scalar clusters with face/vertex assignments and typed receipts. `LaplacianCache` exposes caller-keyed success-only cache facts for spectral bases/factors, cotangent/IDT assemblies report skipped degenerate faces, SDF signed-heat receipts reuse topology/source facts and carry boundary-source heat plus final Poisson solve facts, closed SignedHeat executes through a cached regular `VolumeGrid` domain with topology, source, operator, interpolation, and solve receipts, and robust tufted Laplacian, signpost transport, true watershed/normalized-cut segmentation, and true tangent log-map remain unsupported until they land truthfully.
...
6. `[partial]` Field, kernels, and SDFs include derivative-aware kernels, anisotropic tensor metrics, RBF receipts, oriented MLS receipts, half-space/slab/capped-cone primitives, closed planar single-region profile-extrusion SDFs, Lipschitz bounds, mesh-backed SDF receipts, and native iso-surface receipts. `FieldNabla` owns shared kernel/periodic/reconstruction/profile/iso-surface/plane validation, extraction preserves valid mesh output as well as invalid iso-surface receipt/result facts, native iso-surface receipts carry bridge-proven parallel callback and fixed `0.001` tolerance labels plus a local RhinoWIP-shaped `IsoSurfaceGrid`, mesh-backed SDF receipts reuse topology facts, and bridge scenario source covers primitive, mesh-backed, boundary SignedHeat, closed VolumeGrid SignedHeat, flipped sign convention, support containment, evaluator failure, and native callback concurrency; live bridge proof remains pending unless the configured RhinoWIP bridge verify command passes. `ScalarField` now owns one recursive scalar-payload admission fold over scalar, vector, tensor, falloff, RBF, MLS, and mesh-backed SDF payloads, reusing `FieldNabla`, `MeshKernel`, `CloudKernel`, and matrix solve receipt facts. Watertight preflight is now admission for executable closed `VolumeGrid` SignedHeat; product-strength native extraction matrices and executable `VolumeTet` remain partial.
...
- `[partial]` `Cloud.cs`: rings, polylines, clusters, weighted clusters, PCA/covariance, oriented normals, proof-backed local curvature receipts, convex/footprint hull receipts, winding, Sinkhorn, and correspondence summaries are wired. Mass validation routes through `CloudKernel.MassOf`; transport knobs route through canonical `CloudTransportPolicy`; Sinkhorn confidence means retained coupling mass over retained row/column mass, not geometric probability; kNN and radius neighborhoods resolve native point-cloud indices through the owning `PointCloud.PointAt(index)`; RhinoWIP bridge proof covers native hull facets, native footprint hull routing, native neighborhood lookup, duplicate-coordinate index facts, and plane/sphere/saddle curvature ranges; and concave/alpha hulls stay explicit unsupported until real kernels land.
...
- `[implemented]` `Mesh.cs`: topology, feature basics, flattening, remesh, descriptors, segmentation assignments, isolines, heat/geodesic paths, Hodge/vector-heat/cross-field constraints, generalized winding SDF, boundary-source signed heat, and closed `VolumeGrid` SignedHeat are wired. Segmentation execution returns `MeshSegmentationResult` / `MeshSegmentationReceipt`; cache-hit, skipped-degenerate, caller-keyed cache-failure, topology-backed SDF, boundary-source admission, heat solve, residual-gated final signed-heat Poisson solve facts, closed/solid/watertight preflight, regular-grid source/operator/interpolation/solve facts, and SDF domain/status facts now surface from existing cache/solver owners. Feature receipts use native topology-edge unwelded/ngon-interior classification; emit `NativeRejected` where receipts define it and classify ridge/valley facts truthfully before strengthening feature claims.
- `[partial]` `Spectral.cs`: DEC assembly, spectral basis, filters, descriptor receipts, descriptor normalization/ranking, Crouzeix-Raviart connection Laplacian receipts, FEM heat scaffold, and CDS holonomy distribution exist and feed mesh paths. Public DEC projection returns `DiscreteCalculus` / kinded `SpectralAssemblyReceipt`; boundary SignedHeat carries edge-connection `SpectralAssemblyReceipt` facts; genus-positive Hodge/CDS paths fail unsupported with harmonic dimension facts; and robust/flipped/harmonic-basis gaps remain explicit unsupported.
- `[partial]` `Field.cs`: vector/scalar/tensor field algebra, kernel profiles, anisotropic falloff, RBF reconstruction, context-admitted oriented MLS reconstruction receipts, SDF primitives, closed planar single-region profile extrusion, mesh-backed fields, and iso-surface receipts exist. Public constructor bypasses for invariant-bearing field/falloff cases are closed through internal case constructors, shared field admission routes through `FieldNabla`, native iso-surface attempt receipts preserve callback/tolerance/evaluator-failure facts, `IsoSurfaceGrid` reports local decompile-shaped cell and initial sample counts, and public mesh projection remains valid-only. Recursive scalar-payload preflight now covers Density, Morse, Mollifier, vector-backed scalar cases, Potential/Coulomb charge payloads, falloffs, RBF/MLS receipt coherence, and mesh-backed SDF prewarm; closed SignedHeat is executable through `SdfMeshPolicy.ClosedSignedHeat(...)` and cached `VolumeGrid` prewarm.
...
- `[partial]` Descriptor projection: spectral descriptors return values and receipt/status through the detailed rail. `SpectralDescriptorPolicy` owns basis-time zero-mode, scale, energy, and eigenmode crop normalization; post-hoc normalization is value-only energy normalization; `SpectralRankingPolicy` owns deterministic full-vector descriptor ranking; and `SpectralFilter.Wave` remains raw log-Gaussian weighting until a full WKS receipt rail lands.
...
- `[implemented]` Field admission: invariant-bearing field/falloff/integrator cases construct through validated factories, direct external case construction is closed where Thinktecture regular unions still require public case types, `FieldNabla` owns finite scalar/vector/point, plane, kernel, periodic, reconstruction-sample, and iso-surface input checks, and `ScalarField` owns the recursive iso-surface scalar-payload fold.
...
- `[implemented]` SDF outputs: `ScalarField.LipschitzBound()` exists, mesh-backed signing routes through admitted `SdfMeshPolicy` values for generalized winding, boundary-source SignedHeat, and closed `VolumeGrid` SignedHeat, and SDF/iso-surface receipts expose approximate/native facts. Mesh SDF receipts carry the shared `TopologyReceipt` and an `SdfMeshDomain` (`SurfaceMesh`, `BoundarySource`, `VolumeGrid`, `VolumeTet`) instead of duplicate booleans; boundary-source SignedHeat requires non-null naked-edge polylines while closed SignedHeat requires oriented closed/solid/watertight meshes with no boundary/nonmanifold edges, nonzero native solid orientation, and at least one strict-inside grid node for sign calibration. `ClosedSurfaceSignedHeat` maps to executable `VolumeGrid`, uses a regular padded Cartesian lattice, triangle-source vector heat oriented from `Mesh.SolidOrientation()`, gauge-pinned finite-difference Poisson solve, residual-tolerance enforcement, finite-domain trilinear interpolation, source zero-level calibration, sign convention after interpolation, and explicit `VolumeGridReceipt` facts. This is approximate volumetric Signed Heat Method infrastructure, not an exact Euclidean SDF or tet FEM claim. `VolumeTet` remains researched and deferred because current .NET-compatible options are stale/unconstrained, prerelease, AGPL/GPL/commercial, or native-packaging mismatches for the `net10.0` macOS RhinoWIP baseline.

### Mesh And Spectral Operators

- `[partial]` `Mesh.cs`: per-vertex scalar isolines exist as local PL mesh contours with payload-length admission, finite-level validation, Rhino quad triangulation, edge interpolation, exact-edge dedupe, plateau rejection, branch-safe stitching, stitched candidate counts, branch-node counts, and max incident segment degree. Add richer plateau semantics and bridge proof before productizing them.
...
- `[implemented]` Mesh diagnostics: `LaplacianCache` exposes spectral cache-hit facts, cotangent/IDT Laplacians count skipped degenerate faces, descriptor receipts carry truncated eigenpair/cache/skip facts, CR edge-connection factors carry kinded assembly receipts into boundary SignedHeat, sparse fallback solve status is distinct from residual convergence, failed memoized solves are re-materialized under the caller key through success-only memoization, boundary-source signed heat surfaces topology/source counts plus heat and final Poisson solve receipts, closed SignedHeat surfaces `VolumeGrid` counts/source/operator/interpolation/solve facts without overloading boundary-source counts, and SDF receipts reuse topology/domain facts. Robust tufted Laplacian, flipped signpost vector heat, runtime scalar-isoline geometry proof, executable `VolumeTet`, and true tangent log-map remain unsupported.
- `[partial]` Spectral descriptors: detailed descriptors expose raw/normalized status, pairwise/source metadata, basis-time zero-mode/eigenmode-crop facts, value-only post-hoc energy normalization, comparison readiness, deterministic full-vector ranking, skipped-degenerate/factor facts from mesh owners, and genus-positive harmonic unsupported facts. Full harmonic bases and paper-complete WKS normalization remain unsupported.
...
- `[implemented]` `Cloud.cs`: `SinkhornReceipt` reports coupling summaries, source/target residual semantics, numeric status, canonical `CloudTransportPolicy`, convergence tolerance, positive coupling cutoff, debiased self-costs, and retained-correspondence summaries from computed facts. Transport internals consume normalized mass facts admitted by `CloudKernel.MassOf`, and correspondence confidence is retained-coupling coverage rather than geometric probability.
- `[partial]` `Cloud.cs`: weighted clusters feed centroid/covariance, principal axes/frame, shape/spread, density, transport mass, alignment row weights, oriented normals, and local curvature receipts through `CloudKernel.MassOf` plus one neighborhood/PCA policy rail. Curvature proof covers accepted/rejected samples, rank/residual rejection, eigen-gap and fit-residual tolerances, finite principal directions, scalar derived curvedness/shape-index outputs, and bridge-proven plane/sphere/saddle ranges; broader mass/correspondence receipt propagation can still expand.
- `[partial]` `Cloud.cs`: 3D convex and 2D convex/footprint hull results carry tolerance, angle tolerance, native facet count, planarity/coplanar rejection, input/output counts, and containment proof counts. Convex 3D uses Rhino native hull facets; footprint hulls use Rhino native 2D hull indices plus local containment proof. Concave outline and alpha-style requests return explicit unsupported receipts until a real kernel lands.
...
- `[partial]` `Transport`: coupling, distance, receipt, transported cloud, retained-mass correspondence summaries, canonical Sinkhorn policy admission, and row-mass payload transfer exist. Product IDs and module attributes stay outside this library.
...
- `[implemented]` `Matrix.cs`: `SolveReceipt` / `EigenSolveReceipt` cover dense LU/QR/Cholesky, sparse iterative/MathNet fallback, CSparse Cholesky, dense/sparse/generalized eigen paths, and QR least-squares is owned by `Matrix.cs`. Sparse direct fallback uses a distinct solve stop instead of pretending to be residual convergence, LOBPCG normalizes symmetric sparse storage before solving, and max-iteration exhaustion can return finite diagnostic pairs/residuals under `MaxIterationsExhausted` without becoming usable. Remaining matrix work is not new surface; harden dense/sparse admission and downstream cache/factor receipt propagation, with consumers continuing to reject unusable eigen receipts.
- `[partial]` Receipts and failures: several paths still use raw values or sentinel-style fallback. Nonconvergence, unsupported topology, invalid factorization, missing native capability, lossy fallback, and approximate output should continue to surface through `Fin<T>` failures or typed statuses; materialized diagnostic statuses are not permission for downstream consumers to use invalid payloads.
