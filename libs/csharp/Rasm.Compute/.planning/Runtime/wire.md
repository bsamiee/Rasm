...
- [02]-[PROGRESS]: phase enum mirrors the nine phase keys 1:1, keyed by correlation
- [03]-[CAPABILITIES]: substrate rows, EP rows, model inventory, payload caps, contract metadata — Compute capability rows only
- [04]-[CAPABILITIES]: verb inventory and document scope
- [05]-[DOCUMENTEVENTS]: watch-fact stream feeding the live-data spine
- [06]-[EXECUTETRANSACTION]: flagship parity: idempotency key; server dedup window equals the DeadlineClass.HopTotal allotment — the one retry owner's horizon; the forwarder folds Bounded+budget+Classify+receipt; the receipt mirrors the DocumentTransaction typed receipt field-for-field through `WireDocument.Receipt` and the in-band conflict decodes through `WireDocument.Conflict`
- [07]-[QUERY]: read verb with FieldMask projection via `WireServices.Mask`
- [08]-[CAPTUREEVENTS]: per-frame HLC idempotency keys; Http2 and UnixDomainSocket rows only
- [09]-[CAPTURESUPPORT]: projects the SupportManifest receipt
- [10]-[SETDEGRADATION]: level key lands on the one override rail — the OperatorOverride consequence
- [11]-[RELOADOPTIONS]: projects the ReloadReceipt
- [12]-[SYNC]: frame law below; FieldMask partials; Any artifact envelopes
- [13]-[CHECK]: maps from the HealthChecks registry via the WireHealthRow tag predicate; substrate predicate and node selection read it
- [14]-[WATCH]: compiled verbatim from the well-known proto
- [15]-[SOLVE]: carries the numeric-lane dense or sparse decomposition field-for-field; faults ride FaultDetail; the row-block shard sub-solve dials this rpc
- [16]-[GENERATE]: remote token streaming rides the Progress-class server-stream pattern, keyed by correlation; faults ride FaultDetail
