...
The `Redaction` model, `REDACTION_KEY`, and the deterministic `ENCODE` arrive settled from `observability/observe#OBSERVE`; the `scoped` instrumentation stamp and the `Scope`/`SCOPES` row it binds arrive from `reliability/faults#FAULT`, the one tier below every emitting owner. `LogShip` is the policy value `observability/telemetry#TELEMETRY` threads into its install, so one value drives both halves of the egress — provider registration there, chain arm here. Chain-resident `redact` applies whatever `Redaction` the producer bound on its logger under `REDACTION_KEY`, and a line with no bound policy folds through the keep-all `OPEN`.
...
from rasm.runtime.observe import DEFAULT_SCOPE, ENCODE, OPEN, REDACTION_KEY, Facts, Redaction, ScopeKey
...
type Uncaught = Callable[..., tuple[BaseException | None, Facts]]
...
LEVEL_NUMBER: Final[Map[LogLevel, int]] = Map.of_seq([("debug", 10), ("info", 20), ("warning", 30), ("error", 40), ("critical", 50)])

IGNORED_FRAMES: Final[list[str]] = ["rasm.runtime.faults", "rasm.runtime.logging", "rasm.runtime.observe"]
...
class LogInstall(Struct, frozen=True):
    outcome: LogOutcome
    scope: ScopeKey
    floor: LogLevel
    ship: LogShip
...
def _severity(event: ChainEvent) -> tuple[SeverityNumber, str]:
    match event.get("_record"):
        ...
        case _:
            numeric, text = LEVEL_NUMBER[(level := event["level"])], level.upper()
...
def _reported(door: str, level: LogLevel, fold: tuple[BaseException | None, Facts]) -> None:
    getattr(structlog.get_logger(), level)(door, exc_info=fold[0], **fold[1])
...
def _posture(installs: Map[ScopeKey, LogInstall]) -> tuple[LogLevel, bool, LogLimits]:
    rows = Block.of_seq(installs.values())
    return (
        rows.fold(lambda held, row: row.floor if LEVEL_NUMBER[row.floor] < LEVEL_NUMBER[held] else held, rows[0].floor),
...
class LogPipeline:
    _installs: ClassVar[Map[ScopeKey, LogInstall]] = Map.empty()
    _process: ClassVar[LogInstall | None] = None
    ...
    def configure(
        ...
        *,
        ...
        with cls._gate:
            ...
            root.setLevel(LEVEL_NUMBER[effective])
            ...
            cls._installs = stamped
            cls._process = configured
            return configured

    @classmethod
    def installed(cls) -> Option[LogInstall]:
