...
- [POINTBLANK_AGGREGATE_SIBLINGS]: the live `pb.Validate` surface exposes the column-aggregate family `col_avg_gt(columns, value=None, tol=0, thresholds=, brief=, actions=, active=)` over the cross-product `(avg/sum/sd) × (gt/ge/lt/le/eq)` — the five comparators only, since `col_avg_ne`/`col_sum_ne`/`col_sd_ne` do NOT exist on the class (the aggregate family carries no not-equal arm, unlike the per-cell `col_vals_ne`). The `ProbeTables.aggregate` comprehension therefore resolves `getattr(pb.Validate, f"col_{stat}_{op}")` over the `_STATS × _AGG_OPS` product (`AggOp` the five-member `gt`/`ge`/`lt`/`le`/`eq` vocabulary, distinct from the six-member `Operator`), so the bind never reaches a non-existent `col_{stat}_ne` and the fifteen members collapse to one `(stat, op)`-keyed lookup, never a fifteen-method family. The `.api` step algebra row [14] names the family generically as `(avg/sum/sd) × comparison`; the live class confirms the comparison subset is the five non-`ne` operators. Settled fence code.
