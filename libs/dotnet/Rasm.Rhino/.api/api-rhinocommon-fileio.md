...
[STACKING]:
- `LanguageExt`(`libs/dotnet/.api/api-languageext.md`): nullable archive/bitmap/style/byte returns and engine `bool`/`WriteFileResult` outcomes fold to `Option<T>`/`Fin<T>`, while nonempty `ReadWithLog`/`WriteWithLog`/`IsValidWithLog` diagnostics remain typed evidence or fault detail
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-runtime-extensions.md`): the format-engine roster wraps as a `Union` format vocabulary, `TableTypeFilter`/`ObjectTypeFilter` wrap as flag `SmartEnum` read policy, and the archive write version wraps as a `ValueObject`
- `Hashing`(`libs/dotnet/.api/api-hashing.md`): `File3dm.ToByteArray` feeds the `XxHash128` content key the persistence artifact index dedupes archives on
