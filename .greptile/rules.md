...
- Every table-, law-, or row-driven surface refuses empty input loudly; a fold that silently succeeds on zero rows is a vacuous fold, and a pass verdict quantified over a required evidence stream gates non-emptiness first, because universal quantification over an empty sequence passes vacuously.
...
- Retry geometry reads the schedule-owning vocabulary's gate-modal compile (`Budget.schedule`); a consumer-local `Schedule.exponential` duration chain beside that declared owner is policy leakage even when its values match a ledger row — a genuinely novel envelope is a new row on the owner, never a per-site literal.
- A total classification fold over `unknown` answers identity on its own vocabulary — `classify(kind) === kind` — because call sites hand it raw carriers and already-classified values alike (`retryable(fault.class)`); a residue fold that sends its own literals to the terminal class is a live defect at every such call site.
- Literal-typed semantic-convention keys prove key spelling, never value-domain conformance: typed evidence enters an attribute band at its native scalar — a boolean fact as `boolean`, a count as `number` — and a stringified boolean or numeric value under a well-known key is the defect.
...
- An external member appears as settled fence code only when the folder's `.api/` catalogue verifies its spelling, public accessibility, and deprecation status — existence alone never admits, a catalogued-but-internal type is a phantom for every consumer, a member-level `@deprecated` bans the member even where its family survives, and a delegate-typed parameter binds only at its decompiled signature because a name-only row admits an invented arity; otherwise it is a marked RESEARCH item, never fence code, and a fence contradicting a sibling RESEARCH item fails.
- Fences never compile, so compiler-fatal shapes are review duty swept mechanically across a folder's sibling pages, never spot-judged: every non-package callee resolves to a declaration on the page or a cited sibling anchor, every INDEX row resolves to a same-page section (a phantom index row hides a missing owner), and a union base member sharing a case positional parameter's name suppresses property synthesis and silently discards the constructor argument — shared columns thread through base positionals.
- Declared capability is consumed capability: every capability noun in card prose anchors to a fence member or a cited owning page, every receipt type names a constructing member or consuming evidence column on its page, and every declared policy row, state carrier, and constructor field names its consuming transition or body — an unconsumed declaration is decorative density, and a receipt nothing mints is illusory evidence.
