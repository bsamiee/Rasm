...
public readonly record struct LocalVector(double X, double Y, double Z) {
    ...
    public LocalVector RotateZ(double radians) =>
        radians == 0.0 ? this : Math.SinCos(radians) switch { var (s, c) => new LocalVector((X * c) - (Y * s), (X * s) + (Y * c), Z) };
...
public readonly partial struct RgbSpectrum {
    ...
    internal static readonly Chromaticities Ap1 = RgbProfile.Acescg.Geometry switch {
        var g => new Chromaticities(
            redX: (float)g.Red.X, redY: (float)g.Red.Y, greenX: (float)g.Green.X, greenY: (float)g.Green.Y,
            blueX: (float)g.Blue.X, blueY: (float)g.Blue.Y, whiteX: (float)g.White.X, whiteY: (float)g.White.Y),
    };
...
public abstract partial record BsdfLobe {
    ...
    public sealed record Clearcoat(double Weight, double Roughness, RgbSpectrum Tint) : BsdfLobe {
    ...
    // ONE Alpha: the coat interface is isotropic here, and an AlphaX/AlphaY pair that can never differ is a shape
    // announcing anisotropy it does not deliver — coat grain is the IDEAS [COAT_ANISOTROPY] widening, which lands
    // an anisotropy pair and a rotation on this case exactly as Conductor carries them.
    public sealed record ThinFilm(double Thickness, double FilmIor, double Roughness, ComplexIor BaseIor) : BsdfLobe {
        public static double Alpha(ThinFilm f) => Microfacet.AlphaOf(f.Roughness);
    }
    ...
    public double Pdf(LocalVector wo, LocalVector wi) => Switch(
        ...
        conductor:  static (s, c) => MicrofacetReflectPdf(s.Wo.RotateZ(-c.Rotation), s.Wi.RotateZ(-c.Rotation), c.AlphaX, c.AlphaY),
        ...
        thinFilm:   static (s, f) => MicrofacetReflectPdf(s.Wo, s.Wi, ThinFilm.Alpha(f), ThinFilm.Alpha(f)));
    ...
    public Fin<LobeSample> Sample(LocalVector wo, double uc, double u0, double u1, Op key) => Switch(
        ...
        conductor:  static (s, c) => MicrofacetReflectSample(s.Wo, c.Rotation, c.AlphaX, c.AlphaY, s.U0, s.U1, s.Owner, s.Key),
        ...
        clearcoat:  static (s, c) => MicrofacetReflectSample(s.Wo, rotation: 0.0, Clearcoat.Alpha(c), Clearcoat.Alpha(c), s.U0, s.U1, s.Owner, s.Key),
        subsurface: static (s, _) => CosineSample(s.Wo, s.U0, s.U1, s.Owner, s.Key),
        thinFilm:   static (s, f) => MicrofacetReflectSample(s.Wo, rotation: 0.0, ThinFilm.Alpha(f), ThinFilm.Alpha(f), s.U0, s.U1, s.Owner, s.Key));
    ...
    // Rotation enters ONCE, here: wo and wi turn by −Rotation about local Z so the half-vector, the Smith
    ...
    private static RgbSpectrum EvalConductor(Conductor c, LocalVector wo, LocalVector wi) =>
        MicrofacetReflect(c, wo.RotateZ(-c.Rotation), wi.RotateZ(-c.Rotation), c.AlphaX, c.AlphaY,
    ...
    private static RgbSpectrum EvalDielectricReflect(Dielectric g, LocalVector wo, LocalVector wi) =>
        MicrofacetReflect(g, wo.RotateZ(-g.Rotation), wi.RotateZ(-g.Rotation), g.AlphaX, g.AlphaY,
            fresnel: static (own, cosH) => own.SpecularTint.Scale(Microfacet.FresnelDielectric(cosH, own.Ior)),
    ...
    private static RgbSpectrum EvalDielectricTransmit(Dielectric g, LocalVector local, LocalVector localWi) {
        (LocalVector wo, LocalVector wi) = (local.RotateZ(-g.Rotation), localWi.RotateZ(-g.Rotation));
    ...
    // negligible at Clearcoat.Alpha and the base substrate recovers its own energy).
    ...
    private static RgbSpectrum EvalThinFilm(ThinFilm f, LocalVector wo, LocalVector wi) =>
        MicrofacetReflect(f, wo, wi, ThinFilm.Alpha(f), ThinFilm.Alpha(f),
    ...
    private static Fin<LobeSample> MicrofacetReflectSample(LocalVector wo, double rotation, double ax, double ay, double u0, double u1, BsdfLobe owner, Op key) {
        LocalVector local = wo.RotateZ(-rotation);
        LocalVector h = Microfacet.SampleVisibleNormal(local.CosTheta < 0.0 ? local.Scale(-1.0) : local, ax, ay, u0, u1);
        LocalVector reflected = h.Scale(2.0 * local.Dot(h)).Add(local.Scale(-1.0));
        if (!local.SameHemisphere(reflected)) { return Fin.Fail<LobeSample>(MaterialFault.Graph(key, "<vndf-below-horizon>")); }
        double pdf = Microfacet.VisibleNormalPdf(local, h, ax, ay) / (4.0 * Math.Abs(local.Dot(h)));
        LocalVector wi = reflected.RotateZ(rotation);
    ...
    private static Fin<LobeSample> DielectricSample(Dielectric g, LocalVector outgoing, double uc, double u0, double u1, Op key) {
        LocalVector wo = outgoing.RotateZ(-g.Rotation);
        LocalVector h = Microfacet.SampleVisibleNormal(wo.CosTheta < 0.0 ? wo.Scale(-1.0) : wo, g.AlphaX, g.AlphaY, u0, u1);
        double f = Microfacet.FresnelDielectric(wo.Dot(h), g.Ior);
        if (uc < f) {
            LocalVector reflected = h.Scale(2.0 * wo.Dot(h)).Add(wo.Scale(-1.0));
            LocalVector wiReflect = reflected.RotateZ(g.Rotation);
            double pdf = f * Microfacet.VisibleNormalPdf(wo, h, g.AlphaX, g.AlphaY) / (4.0 * Math.Abs(wo.Dot(h)));
            return wo.SameHemisphere(reflected) && pdf > 0.0 ? Fin.Succ(new LobeSample(wiReflect, EvalDielectricReflect(g, outgoing, wiReflect), pdf, Transmission: false)) : Fin.Fail<LobeSample>(MaterialFault.Graph(key, "<dielectric-reflect-degenerate>"));
        }
        ...
        LocalVector refracted = wo.Scale(-eta).Add(n.Scale(eta * cosI - Math.Sqrt(k)));
        ...
        double pdf = (1.0 - f) * Microfacet.VisibleNormalPdf(wo, h, g.AlphaX, g.AlphaY) * Math.Abs(etaH * etaH * refracted.Dot(h) / (sqrtDenom * sqrtDenom));
        LocalVector wi = refracted.RotateZ(g.Rotation);
        return pdf > 0.0 ? Fin.Succ(new LobeSample(wi, EvalDielectricTransmit(g, outgoing, wi), pdf, Transmission: true)) : Fin.Fail<LobeSample>(MaterialFault.Graph(key, "<dielectric-refract-degenerate>"));
    }
    private static double DielectricPdf(Dielectric g, LocalVector outgoing, LocalVector incident) {
        (LocalVector wo, LocalVector wi) = (outgoing.RotateZ(-g.Rotation), incident.RotateZ(-g.Rotation));
...
public static class MultiScatter {
    ...
    // INTERNAL rather than private: [07]-[BSDF_GOLDEN] asserts the furnace against THIS quadrature's own sum, so a
    // fixture re-deriving the eight nodes beside it would drift the expectation from the term it proves.
...
- Packages: Rasm (project — `Deterministic.Hammersley`/`RadicalInverse`, the kernel's equidistributed draw family), LanguageExt.Core, Thinktecture.Runtime.Extensions.
...
- Exemption: the three `Measure` folds are fixed-trip accumulations over the closed-form kernels — the same bounded-quadrature carve `MultiScatter.HemisphericalAlbedo` names.
...
public static class Energy {
    ...
    // FurnaceRow expects the kernel's OWN 8-node hemispherical albedo at this alpha, and Residue records the gap that
    // quadrature leaves against the analytic unit — asserting 1.0 here fails a correct lobe at every roughness.
    internal static BsdfGolden FurnaceRow(double roughness) =>
        Microfacet.AlphaOf(roughness) switch {
    ...
    internal static double Furnace(BsdfLobe lobe, int taps) {
        ...
        for (int i = 0; i < taps; i++) {
            ...
            acc += lobe.Sample(LocalVector.Normal, Deterministic.RadicalInverse(index: (uint)i, radix: 3), u0, u1, Probe)
    ...
    internal static double Reciprocity(BsdfLobe lobe, int taps) {
        ...
        for (int i = 0; i < taps; i++) {
            (double u0, double u1) = Deterministic.Hammersley(index: i, count: taps);
            LocalVector wo = Hemisphere(u0, u1), wi = Hemisphere(u1, Deterministic.RadicalInverse(index: (uint)i, radix: 5));
    ...
    internal static double NdfNormal(BsdfLobe lobe, int taps) {
        ...
        for (int i = 0; i < taps; i++) {
            (double u0, double u1) = Deterministic.Hammersley(index: i, count: taps);
            LocalVector h = Hemisphere(u0, u1);
            acc += Microfacet.Ndf(h, ax, ay) * h.CosTheta;
        }
        return acc * (2.0 * Math.PI) / taps;
    }

    private static LocalVector Hemisphere(double u0, double u1) {
...
- [POSITION_FREE_MULTI_SCATTER]-[OPEN]: does a path-trace per-bounce budget admit an unbiased position-free random walk over the `SlabStack` as the high-fidelity multi-scatter path beside the closed-form `MultiScatter.KullaConty` fast path; measure the walk against the closed form at the `Rasm.AppUi/Render/pathtrace#PATH_TRACE` integrator seam that owns the per-bounce cost.
- [AP1_CHROMATICITY_CONSTRUCTION]-[OPEN]: which parameter names and order does the `TinyEXR.V3.Chromaticities` eight-float constructor declare, so the `[04]-[LOBE_FAMILY]` AP1 mint binds by name rather than position; decompile the constructor through `uv run python -m tools.assay api --key tinyexr --type TinyEXR.V3.Chromaticities`.
