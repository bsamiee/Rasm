...
def _affirm(outcome: Result[Completed, Fault]) -> Result[None, Fault]:
    ...
    A ``Completed`` at or below ``OK`` severity is a clean run (``None``); a higher severity
    (``FAILED``/``TIMEOUT``/``BUSY``) is promoted to an ``Error(Fault)`` carrying the argv so the
    prelude rail short-circuits. A rail-level ``Fault`` (spawn/timeout) passes through unchanged.
    """
    match outcome:
        case Result(tag="ok", ok=done) if done.status.severity <= RailStatus.OK.severity:
            return Ok(None)
        case Result(tag="ok", ok=done):
            return Error(Fault(done.argv, done.status, (done.stderr or done.stdout or b"")[:1024].decode(errors="replace")))
        case Result(error=fault):
            return Error(fault)
...
def _bridge_tool_census() -> tuple[Tool, ...]:
    """The bridge ``Tool`` rows this rail constructs plus the catalog's ``Claim.BRIDGE`` slice.

    Affirms the module-owned client/build rows coexist with the catalog ``select(Claim.BRIDGE,
    Language.CSHARP)`` slice (the ``rasm-bridge`` verify row + ``parse_verify``). The catalog import
    is kept live so a renamed catalog row surfaces as an import-time error, never a silent drift.
    """
    return (_CLIENT_TOOL, _BUILD_TOOL, *select(Claim.BRIDGE, Language.CSHARP))
