...
[GEOSPATIAL_TERRAIN]-[BLOCKED]-[F]: Add the raster-ANALYTICS axis the cluster wholly lacks (current RasterOp is IO/reshape only) — a Terrain DEM-analytics owner over `xarray-spatial` on the worker-capable geospatial lane.
...
- Ripple: `python:compute` `[DATA_STUDY_INPUT]` — the compute numerics array seam aligns with this Terrain owner's `xrspatial` worker lane; the edge stays blocked on the data side behind the Terrain owner and the `CATALOG_COVERAGE_ODCSTAC` `[COVERAGE]` DataArray dependency, matching the compute counterpart.
...
[TENSORSTORE_ADMIT]-[COMPLETE]: admitted `tensorstore` as the companion async zarr-v3 `TensorBackend.TENSORSTORE` row on `store.md#STORE` opening the identical Zarr v3 chunk grid through `tensorstore.open` over a native `KvStore`, with `oindex`/`vindex` read-selection and `Transaction(atomic=True)` multi-region writes.
