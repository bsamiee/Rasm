...
- `ERounding.Floor`/`Ceiling` with `RoundToExponentExact`/`RoundToPrecision` bracket a value into a directed-rounded interval — the software-rounded endpoints of an interval-arithmetic sign filter (a cheaper-than-exact second stage available to the predicate, no current consumer composes it).
