...
- Receipt: the kernel `EncodedGeometry.Witness` is the lossless-round-trip proof keyed by the `Spatial/reconciliation#ONE_WIRE_FIXTURE_CORPUS` content hash; `Of` admits only a lossless payload, so the residency wrap carries no second witness and mints no second content key.
