...
- `Numerics/calculus#SOLAR_EPHEMERIS`: `SunPosition.OfDirection` admits a survey-frame `Vector3d` through `IsValid`, `Length`, and `operator /(Vector3d, double)`, so the host sentinel screens where a finiteness probe passes it.
