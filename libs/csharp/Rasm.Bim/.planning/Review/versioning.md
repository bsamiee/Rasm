...
- Events: a sealed commit mints the `Exchange/events#EVENTS` `BimEvent.CommitLanded` row at the composing rail's edge — `CommitKey`, the sorted-distinct `ParentKeys`, the advanced branch name, and the fingerprint count off the `Commit`/`CommitMerge` result, subject the `CommitKey` hex — the unchanged-model no-op minting nothing; envelope seal and transport are the events owner's, never a second emit path here.
...
public sealed record BimCommit(
    ...
    public static readonly BimCommit Empty = new(UInt128.Zero, Seq<UInt128>(), Map<string, ElementFingerprint>(), "", "", Instant.MinValue, None);
    ...
    public static Fin<BimCommit> Of(ElementGraph graph, Seq<UInt128> parents, string author, string message, Instant at, Op key, Option<FidelityReceipt> fidelity = default) {
    ...
    public static BimCommit Sealed(Seq<UInt128> parents, Map<string, ElementFingerprint> fingerprints, string author, string message, Instant at, Option<FidelityReceipt> fidelity = default) {
...
public sealed record BimRepository(Map<UInt128, BimCommit> Commits, Map<string, BimBranch> Branches) {
    ...
    public Fin<(BimRepository Repository, BimCommit Commit)> Commit(ElementGraph graph, string branch, string author, string message, Instant at, Op key, Option<FidelityReceipt> fidelity = default) =>
        BimCommit.Of(graph, Branches.Find(branch).Map(static b => Seq(b.Head)).IfNone(Seq<UInt128>()), author, message, at, key, fidelity)
