...
| :-----: | :----------------------------- | :-------------- | :------------------------------------------------------------------------ |
|  [01]   | `onnxruntime.InferenceSession` | session root    | loads and runs a model over selected execution providers                  |
|  [02]   | `onnxruntime.SessionOptions`   | session policy  | graph-optimization level, thread counts, execution mode, profiling        |
|  [03]   | `onnxruntime.RunOptions`       | run policy      | per-run log level, termination flag, active LoRA adapter                  |
|  [04]   | `onnxruntime.NodeArg`          | io descriptor   | named input/output exposing `name`, `type`, `shape`                       |
|  [05]   | `onnxruntime.OrtValue`         | value carrier   | native typed tensor with `numpy`, `shape`, `element_type`, `device_name`  |
|  [06]   | `onnxruntime.IOBinding`        | io binder       | binds pre-allocated device inputs/outputs to a session                    |
|  [07]   | `onnxruntime.ModelMetadata`    | metadata record | `producer_name`, `domain`, `version`, `graph_name`, `custom_metadata_map` |
|  [08]   | `onnxruntime.ModelCompiler`    | aot compiler    | compiles a model to an EP-specific optimized artifact                     |
|  [09]   | `onnxruntime.SparseTensor`     | sparse value    | COO/CSR/block-sparse tensor value                                         |
...
|  [06]   | `get_available_providers()` / `get_all_providers()`                                   | provider query | lists installed and known execution providers     |
|  [07]   | `get_device()`                                                                        | device query   | returns the build's primary device string         |
|  [08]   | `set_default_logger_severity(level)`                                                  | logging        | sets the global runtime log severity              |
|  [09]   | `ModelCompiler(sess_options, input_model_path_or_bytes, ...).compile_to_file(...)`    | aot compile    | compiles a model to an optimized on-disk artifact |
