...
[CLOUDEVENTS_ENVELOPE]-[COMPLETE]: HTTP routes by `Format.event`, crosses every decoded member through `Event.admit` and `Event.rasm.read`, and webhook egress seals generated extension vocabulary before signing exact transport octets.
