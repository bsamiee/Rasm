...
- [02]-[FORMAT_LAW]: `Export` seats the format row table, the parcel owner, the content mint delegate, and the fault family; `Export`, `ExportFault`.
- [03]-[SERIALIZER_MATRIX]: `Export` correlates the source family against its per-source admitted-format matrix, with the raster-encode port; `Export`, `Raster`.
- [04]-[EGRESS_PORT]: `Egress` declares the delivery capability Tag, the route vocabulary, and the streaming lane; `Egress`.

## [02]-[FORMAT_LAW]

[FORMAT_LAW]:
- Owner: `Export` — one owner whose members are the octet fold, the parcel mint, and the delivery dispatch; a format is a `Export.format` row carrying `mime`, `extension`, and `binary`, so a new format is one row and the filename, the media type, and the text-versus-octets encode step all derive from it. Spelling a `mime` literal beside a call site, or assembling a filename by concatenation, restates a column the row already carries.
...
- Law: octets are the one currency — every serializer answers `Uint8Array`, text formats crossing through the encoder at the boundary, so the parcel, the digest, and the port all speak one shape and no arm carries a `string | Uint8Array` union downstream.
...
declare namespace Export {
  ...
  type _Rows<T extends Record<Formats[number], FormatRow> = typeof _formatRows> = T
  type _Keys<K extends Formats[number] = Format> = K
}
...
const _Format = Schema.Literal(..._formats)
...
class Parcel extends Schema.Class<Parcel>("Parcel")({
  ...
  get filename(): string {
    return `${this.name}.${this.row.extension}`
  }
}
...
[SERIALIZER_MATRIX]:
- Owner: `Export.Source` — the closed source family, one case per surface that can leave the browser: `Pivot` carries the engine handle and its view config, `Figure` the detached Plot element, `Canvas` the uplot backing canvas, `Rows` the Arrow table a grid or a selection projects, `Scene` the `<model-viewer>` element, `Readback` the renderer pixel band with its extent, `Draft` a form payload beside the kernel Schema that admitted it, `Document` an editor document beside its fused codec and its plain-text projection, and `Lines` the evidence rows a probe board renders.
- Law: legality is the matrix's own shape, never a runtime refusal — `Export.Admits` names the formats each case admits and the serializer record is `{ readonly [T in Kind]: { readonly [F in Admits[T]]: … } }`, so `Export.octets` is one correlated generic indexed call and an illegal pair — a scene as csv, a draft as png — fails at the call site rather than answering a fault at runtime. Adding a `legal(source, format)` predicate beside this matrix re-derives at runtime what the mapped contract already proves.
...
- Law: a document export never re-renders its embeds — the `Document` case carries the compiled roster codec (`view/content#COMMIT_BOUNDARY`'s `Content.field`, parseJson-fused exactly as `Draft`'s) and its plain-text projection, so the `json` arm ships every embed row's WIRE form inside the one document payload and the `text` arm ships the projection the owning page already computed; an embed wanting a rendered format (a chart as svg, a graph as png) exports through its OWN source case, and a cell here that reaches into an embed to render it forks the serializer its owner holds.
...
declare namespace Export {
  type Source = Data.TaggedEnum<{
    ...
    Document: { readonly document: unknown; readonly codec: Schema.Schema<unknown, string>; readonly text: string }
  ...
  type _Admits<A extends Record<Export.Kind, Export.Format> = { [T in Export.Kind]: Export.Admits[T] }> = A
}
...
const _snapshot = <A>(row: Export.Case<"Pivot">, read: (view: View) => Promise<A>) =>
  Effect.mapError(
    Chart.snapshot(row.pivot, row.feed, row.config, read),
...
const _MATRIX: Export.Matrix = {
  ...
  Canvas: { png: (row) => _raster("series", row.chart.ctx.canvas) },
  ...
  Draft: {
    json: (row) =>
      Effect.mapError(
        Effect.map(Schema.encode(row.schema)(row.draft), _encoded),
  ...
  Document: {
    ...
    text: (row) => Effect.succeed(_encoded(row.text)),
  },
...
[EGRESS_PORT]:
- Owner: `Egress` — the folder-declared delivery capability Tag: `deliver(parcel, route)` writes a whole parcel and `open(parcel)` yields a `WritableStream` for a payload no buffer holds, both on the typed fault channel, declared HERE and satisfied at the browser composition root exactly as `system/primitive#CLIPBOARD_PORT` is. This folder never imports the platform package, so the capability travels the requirement channel and a proof substitutes a Layer.
- Law: the native surfaces stay outside this folder — the save picker is not declared in the platform lib at all, and `navigator.storage` is confined to its own runtime owner, so the port's contract is the whole ui-side truth and the composition root spells whichever native call its host carries. Reaching for a picker, an anchor click, or a storage handle from a ui page is the ungated-native-call defect, and a host carrying none of them satisfies the port with a Layer whose every route answers `egress-absent`.
...
- Law: routes are a closed vocabulary with a handler row each — `Export.deliver(parcel, route)` dispatches `save` and `share` onto the `Egress` port and `clipboard` onto the `Clipboard` port `system/primitive` already declares, so text that belongs in the paste buffer never grows a second port and a binary parcel routed to the clipboard refuses at the row rather than at the host. Every new route lands as one row and one handler; a `saveParcel`/`shareParcel` sibling pair is the arity twin this entrypoint deletes.
...
declare namespace Export {
  ...
  type Shape = Types.Simplify<{
    ...
    readonly format: typeof _formatRows
    ...
    Match.when("clipboard", () =>
      parcel.row.binary
        ...
        : Effect.flatMap(Clipboard, (board) =>
          Effect.mapError(
            board.copy(new TextDecoder().decode(parcel.octets)),
