...
// registered-row mint) — co-located on the data shape so the contracted projection#MATERIAL_SUBGRAPH spelling is
...
public sealed record MaterialWire(
    ...
    // The CONTRACTED seam lowering projection#MATERIAL_SUBGRAPH composes: a library row → the neutral seam
    // AppearanceSummary the MaterialProjector content-keys a Node.Appearance on. The base color lowers to scene-linear
