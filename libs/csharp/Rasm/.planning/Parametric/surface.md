...
public static class Surfaces {
    ...
    // G5 refit → inter-sample deviation probes → densify breaching rows/columns — the SAME bounded
    // RefinePolicy fold curve.md's Offset runs; the terminal receipt carries the achieved deviation.
    static Fin<SurfaceResult> NormalOffsetOf(SurfaceOp.NormalOffset op, Op? key) =>
        Range(0, op.Refine.MaxRounds).Fold(
            OffsetFit(op, GrevilleGrid(op.Surface), round: 0, key),
            (state, round) => state.Bind(s => s.Breaching.Count == 0 ? Fin.Succ(s) : OffsetFit(op, Densified(s.Grid, s.Breaching), round + 1, key)))
        ...
        .Bind(final => final.Breaching.Count > 0 && final.Deviation > op.Refine.DeviationTolerance * 8.0
            ? Fault<SurfaceResult>(ParametricStage.Construction, nameof(NurbsForm.Surface), $"normal offset unconverged at deviation {final.Deviation}")
            : Fin.Succ<SurfaceResult>(new SurfaceResult.Offsets(
                final.Fit, new RefineReceipt(op.Refine.DeviationTolerance, final.Deviation, final.Round, final.Grid.Count))));

    internal readonly record struct SurfaceRound(NurbsForm.Surface Fit, Arr<Point2d> Grid, Arr<Point2d> Breaching, double Deviation, int Round);

    static Arr<Point2d> GrevilleGrid(NurbsForm.Surface surface);                                          // γ rows off KnotsU × KnotsV
    static Fin<SurfaceRound> OffsetFit(SurfaceOp.NormalOffset op, Arr<Point2d> grid, int round, Op? key); // NormalAt displacement (degenerate normal → Evaluation fault) → Nurbs.Of(SurfaceThrough) → probes
