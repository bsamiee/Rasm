...
- Receipt: each evaluation contributes an emitted-phase `Receipt.of` row through `ReceiptContributor` and produces a geometry `GraduationReceipt` subject keyed by case (`network-graph`, `form-finding`, `numerical-primitive`, `mesh-algebra`).
...
def _dispatch(algebra: ComputationalGeometry) -> AlgebraResult:
    match algebra:
        ...
        case ComputationalGeometry(tag="numerical", numerical=(points, op)):
            return AlgebraResult("numerical", (json_dumps(NUMERICAL[op]([list(p) for p in points])),), "numerical-primitive")
        ...
        case ComputationalGeometry(tag="mesh", mesh=(mesh, op)):
            datastructure = Mesh.from_json(mesh)
            result = datastructure.subdivide() if op == "subdivide" else datastructure.dual()
            return AlgebraResult("mesh", (json_dumps(result),), "mesh-algebra")
        case unreachable:
