...
// and placement keys (the ONE Projection/address#CONTENT_ADDRESS codec — never a second hasher). Two fingerprints
...
public sealed record ModelDiff(Seq<ElementChange> Changes, int UnchangedCount) {
    ...
    // The geometry/placement key: the Object's content-hashed representation map plus the analytical coordinate
    // geometry (BoundaryPolygon surface, Axis line) the structural/energy disciplines read — a relocation moves this
    // key while the semantic content key stays stable, so a pure move reads as Moved, never Modified. The projection
    // rides the ONE CanonicalWriter codec (the geometry subset of Graph/element#NODE_MODEL Node.ToCanonicalBytes).
    static ContentAddress PlacementKey(Node.Object node, double tolerance) {
        ...
        writer.Ordinal(node.BoundaryPolygon.Count);
        foreach (Vector3 point in node.BoundaryPolygon) { writer.Double(point.X).Double(point.Y).Double(point.Z); }
        writer.Bool(node.Axis.IsSome);
        node.Axis.IfSome(axis => writer
