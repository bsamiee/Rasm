...
- material: a PBR material is `UsdShade.Material.Define` + `UsdShade.Shader.Define` (the `UsdPreviewSurface` node) wired through `ConnectableAPI`, then attached through `UsdShade.MaterialBindingAPI.Apply(prim).Bind(material)` — the bind is distinct from the define; a per-face region is a `UsdGeom.Subset` the binding targets, so one mesh carries N material regions without N meshes. The displacement terminal wires `material.CreateDisplacementOutput().ConnectToSource(surface.CreateOutput("displacement", Sdf.ValueTypeNames.Token))` — the shader's `token outputs:displacement` (renderType `terminal displacement`) is DISTINCT from its `float inputs:displacement` scalar, and `Material.CreateDisplacementAttr`/`GetDisplacementAttr` is the legacy schema-declared attribute no `Compute*Source` resolves — a decoy reaching nothing.
