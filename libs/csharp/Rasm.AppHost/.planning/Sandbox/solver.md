...
- Owner: `SolverKind` `[SmartEnum<string>]` the seven extension-category axis under the `ComparerAccessors.StringOrdinal` accessor; `EncodingKind` `[SmartEnum<string>]` the six-row representation axis signature-locked 1:1 on the kernel `Rasm/Drawing/pack#ENCODING` keys (`point-cloud`/`mesh-patch`/`voxel-grid`/`brep-patch`/`field`/`toolpath`), the case axis Compute residency mirrors; `KindContract` per-kind contract-shape record; `KindContracts` the frozen row set with the total dispatch; `SolverFault` `[Union]` fault family deriving its codes through `FaultBand.Solver`.
