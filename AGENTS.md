...
- `docs/laws/` is the repo-wide maintenance-law corpus — coupling topology, cross-branch pattern rows, and the scar ledger; read it at source in substantive passes (it stays small by law), and land a diff touching a `topology.md` `[SURFACE]` together with its obligated counterparts. `docs/laws/README.md` owns the admission law; the twin routing note lives in `CLAUDE.md` `[02]`.
