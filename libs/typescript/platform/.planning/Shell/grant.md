...
- [1]-[PERMISSION_GRANT_FOLD]: the closed `PermissionState` vocabulary, the native-state-and-persist fold, the `PermissionStatus.change` live-edge stream, the `CapabilityFault` family, and the `capability-rank` projection.
