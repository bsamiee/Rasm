...
`matplotlib` owns the artifacts visuals domain's imperative publication 2D plotting and offscreen raster/vector export through the Agg/PDF/SVG/PS/PGF backend canvases. Every `ChartSpec.Matplotlib(figure, palette)` case renders through `_matplotlib_savefig` on the `anyio.to_process` subprocess boundary, so matplotlib never resolves in the runtime process and re-implements neither the rendering backends nor the colormap registry it already owns. It holds the imperative-plotting band alone; declarative Vega/grammar charts, 3D scenes, display tables, and color-science models route to their own owners.
...
[STACKING]:
- `numpy` (`libs/python/.api/numpy.md`): every `Axes` plot method consumes `numpy` arrays with zero marshalling — the `graphic/color/derive#DERIVE` `derived color coordinates` palette feeds `ListedColormap(palette)` and `set_prop_cycle(color=hex_ramp(palette))` with no per-row loop, and `FigureCanvasAgg.buffer_rgba()` yields a numpy-addressable RGBA buffer so a render becomes a zero-copy raster frame for `graphic/raster/process#PROCESS` without a file round-trip.
- `anyio` (`libs/python/.api/anyio.md`): `_matplotlib_savefig` rides `anyio.to_process.run_sync(_matplotlib_savefig, figure, palette, fmt, ppi, limiter=_RENDER_LIMITER)` under one shared `CapacityLimiter`, so the GIL-heavy Agg render crosses the subprocess boundary off the event loop while the vega/lets-plot bands ride `to_thread`; the `Figure` and its `Axes` construct on the worker, matching the `ChartSpec.Matplotlib(figure, palette)` case carry.
...
- downstream handoff: flat-SVG/raster bytes hand to the `composition/compose#COMPOSE` placement owner, which re-renders nothing; a multi-page `PdfPages` sink feeds `package/archive`; a vector SVG/PDF `savefig` feeds `export/dxf` and the imposition domain.
