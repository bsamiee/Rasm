...
[STACKING]:
- `dotbim`(`.api/api-dotbim`), `SharpGLTF`(`libs/csharp/.api/api-sharpgltf`), `AssimpNetter`(`.api/api-assimpnetter`): every display `Mesh` fans to the shared canonical triangle carrier these codecs decode into, so a received Speckle model re-exports through any of them.
