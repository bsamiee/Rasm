...
- `AttachNative`/`DetachNative` are the interface a Rhino-hosted panel uses to dock Eto content under a host-owned native parent, and they pair — a detach without its attach leaks the parent link.

[STACKING]:
- `libs/dotnet/.api/api-eto-platform.md`: the registered handler root this boundary sits beneath — identity, capability gating, handler resolution, the style registry, and native hosting all read there, and this boundary re-tables none of it.
- `libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md`: `EtoExtensions` native styling and the Rhino native windowing surface dock and style the Eto content the handler places, and the host bridge owns the document-owned window this boundary attaches into.
...
- `libs/dotnet/Rasm.Rhino/.api/api-macos-native.md`: on the macOS backend the registered native host bridges to an AppKit view, where native pacing composes rather than in this layer.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): `Eff<A>` scopes the attach and detach pair for deterministic release, and a host theme flip feeds a `Fin<A>`-returning re-style.
...
- The theme boundary stays behind the Rasm.Rhino UI owner; `Eto.*` platform types never leak past it.
