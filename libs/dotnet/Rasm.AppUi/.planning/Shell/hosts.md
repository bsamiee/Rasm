...
- Entry: `SurfaceRuntime.Open(line, eventCapacity, contract, clock, send, count, span, faults, key)` — the app-root construction edge seating the AppUi-owned observe tap directly in `HookRail.Of`; `SurfaceRuntime.Evidence()` — the current bounded live source `RunQueueSeams.Evidence` binds; `SurfaceRuntime.Release()` and `Dispose()` — idempotent typed release and lifetime projection; `Fin<SurfaceSession> Mount(ConsumptionProfile profile, SurfaceMount mount, SurfaceSeam seam, Control content, SurfaceRuntime runtime)` — `Fin` aborts on unadmitted mount, absent host, an undeclared runtime identifier, rejected mount, missing handle, and thread-affinity violation; `Fin<Unit> Boot(…, SurfaceRuntime runtime, Func<AppBuilder> entry)` — the claim-commit boot edge.
...
- Packages: Avalonia, Avalonia.Desktop, Avalonia.Headless, Avalonia.Skia, Irihi.Ursa, Irihi.Ursa.ReactiveUIExtension, ReactiveUI.Avalonia, System.Reactive, Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, Rasm (kernel rails, capability, transition, custody, timeline), Rasm.Contracts (project — `Event.Extensions`), Rasm.AppHost (project)
...
public sealed class SurfaceRuntime : IDisposable {
    ...
    private SurfaceRuntime(
        ...
        (Line, Rail, this.events, Count) = (line, rail, events, count);

    public MonotonicTimeline Line { get; }
    public HookRail<AppUiPoint, AppUiFact, TelemetrySource> Rail { get; }
    ...
    public static Fin<SurfaceRuntime> Open(
        ...
        return HookRail<AppUiPoint, AppUiFact, TelemetrySource>
            .Of(key: key, taps: Seq(tap), span: span, cell: Some(faults))
            .Map(rail => new SurfaceRuntime(line, rail, events, count));
    }
    ...
    public Fin<Unit> Release() =>
        Cell.Seat(released, static () => unit) is Transition<Option<Unit>>.Committed
            ? Rail.Release(AppUiTelemetry.Source, Op.Of(name: nameof(Release)))
            : Fin.Succ(unit);

    public void Dispose() =>
        Release().IfFail(error => ignore(Rail.Faults.Park(AppUiPoint.Surface.Id, error)));
}
...
public static class Surfaces {
    ...
    public static Fin<SurfaceSession> Mount(
        ConsumptionProfile profile, SurfaceMount mount, SurfaceSeam seam, Control content, SurfaceRuntime runtime) =>
        from row in Row(profile, mount, seam)
    ...
    private static Fin<SurfaceSession> Seated(
        ...
        from fired in runtime.Rail.Fire(
            AppUiPoint.Surface,
    ...
    private static Fin<AssetCensus> Identified(SurfaceRuntime runtime) =>
        NativeAssets.Current
            ...
            .Bind(census => census.Present
                .Traverse(fact => runtime.Rail.Fire(
