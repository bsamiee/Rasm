...
- Growth: a new presence dimension lands as one `PresenceRowWire` field, never a parallel store; the TTL horizon unifies with the `query/watermark#WATERMARK` frontier through the `retention#SOURCE_WIRING` `presenceReclaimable` projection — each presence row projects its `expiresAt` parsed to nanos into the one merged reclaimable feed, so an expired row is evicted by the unified frontier rule rather than a per-row `expiresAt` scan in this fold.
- Boundary: the row reads `expiresAt` as a decode-admitted instant, never re-validated; presence keys on the `actor`-on-`entityKey` row identity and never on the `ContentKey` convergence identity; both folds pipe through `policy#STREAM_POLICY`.
