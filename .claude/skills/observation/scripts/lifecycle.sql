...
-- A removed line holding the text is fixed, an edit or `git mv` since the transition placing it elsewhere is moved, text gone otherwise is vanished, a confirmed row present at a new hash is reconfirmed, every other live row keeps its state
