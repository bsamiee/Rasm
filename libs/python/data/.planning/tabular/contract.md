...
- Entry: `DataQuality.of` folds a tuple of `QualityRule` into one `DataFrameSchema`; `DataQuality.validate` binds the railed `ContentIdentity.of("schema", repr(schema).encode())` key through `.bind` into one `boundary("quality.validate", ...)` running `schema.validate(frame.collect(), lazy=lazy, sample=sample, random_state=seed)`, returning a `RuntimeRail[ContractClaim]` — the schema fold is bound once and threaded as both the railed key source and the `_validate` argument, never reconstructed per branch and never re-railed inside the boundary thunk. The `pandera.polars` backend defers a `LazyFrame` validation into the scan and returns the frame unraised, so the error rail never fires on a lazy input; the gate collects the lazy plan to a `pl.DataFrame` exactly once at the boundary so `lazy=True` raises `SchemaErrors` with the full `failure_cases` frame and `lazy=False` raises the first `SchemaError`. The optional `sample` row-count bound plus its `seed` (`random_state`) restrict validation to a deterministic row subset of the collected frame for a large input, the pandera large-frame sampling knob the gate mines rather than checking every row unconditionally. Both outcomes land in one `ContractClaim`; the rail is `Ok` even on validation failure because the claim records but does not enforce, and the rail is `Error` only when the collect or the railed content-key derivation faults.
...
_CMP: Final[Map[str, Callable[[float], Check]]] = Map.of_seq([
...
_TEXT: Final[Map[str, Callable[[str | Pattern[str]], Check]]] = Map.of_seq([
...
class ContractClaim(Struct, frozen=True):
    ...
    def contribute(self) -> tuple[Receipt, ...]:
        ...
        facts = {"status": str(self.status), "breaches": str(len(self.breaches)), "key": self.content_key.hex}
...
class DataQuality(Struct, frozen=True):
    ...
    def validate(self, frame: pl.LazyFrame, *, lazy: bool = True, sample: int | None = None, seed: int | None = None) -> "RuntimeRail[ContractClaim]":
        ...
        return ContentIdentity.of("schema", repr(schema).encode()).bind(
            lambda key: boundary("quality.validate", lambda: self._validate(schema, frame, lazy, sample, seed, key))
        )

    def _validate(self, schema: pap.DataFrameSchema, frame: pl.LazyFrame, lazy: bool, sample: int | None, seed: int | None, key: ContentKey) -> ContractClaim:
        ...
        try:
            schema.validate(frame.collect(), lazy=lazy, sample=sample, random_state=seed)
...
class FrameCovenant(Struct, frozen=True):
    ...
    def _dispatch(self, op: CovenantOp, pairs: "Block[tuple[str, pl.LazyFrame]]", key: ContentKey) -> CovenantOutcome:
        import dataframely as dy  # noqa: PLC0415
        ...
        with dy.Config(max_failure_examples=self.failure_examples) if self.failure_examples is not None else nullcontext():
            match op:
                ...
                case CovenantOp(tag="consistent", consistent=members):
                    return self._collection(members).is_valid(dict(pairs), cast=self.cast)
...
- [DATAFRAMELY_FILTER_RESULT]: `Collection.filter(data, /, *, cast=, eager=True)` returns a `CollectionFilterResult[Self]` whose public surface is `{collect_all, count, failure, index, result}`; the per-member `FailureInfo` map is the `result.failure` attribute (singular, a `dict[str, FailureInfo]` keyed by member name), never a `result.failures` plural attribute or an `invalid()`/`failure_info()` method — the `_failures` seam reads `result.failure`. Signature-confirmed against the live `2.10.1` distribution; settled fence code.
