...
- **LOC:** baseline Vectors inventory was 8,091 lines; current inventory before this architecture edit was 8,910 lines by `fd -t f . libs/csharp/Rasm/Vectors -X wc -l`. This tier is complete for correctness/centralization, but the promised LOC win did not materialize after receipt-backed flatten distortion, Tier 3 receipts, and Tier 4 executable rails were added; treat additional source folding as follow-up, not as unresolved drift.
...
- **Tier 4 — Algorithm rails:** partial. Executable rails exist for mesh-candidate Bridson-style active-list selection, scalar-field variable-density candidate priority, capacity-limited Lloyd residuals, thresholded mesh-spectrum receipts, signed-dihedral/curvature-proxy ridge/valley/region-boundary features, watershed/normalized-cut segmentation, and vector-heat-backed tangent log-map approximation. The current code does not yet truthfully implement paper-faithful Dwork sampling, true transport-backed OT/CCVT assignment, continuous power diagrams, curvature-derivative ridge extraction, exact exponential maps, or mesh runtime proofs for every rail.
...
- **Tier 6 — Native bridge proofs (RhinoWIP runtime, gated on Tier 3 receipts):** landed for cloud hull/neighborhood/curvature receipts in `vectors-cloud.verify.csx`, WKS receipt facts in `vectors-spectral.verify.csx`, and native iso-surface callback/projection facts in `vectors-fields.verify.csx`. Still pending for scalar isoline runtime geometry, mesh feature/segmentation/sampling/log-map runtime proof, and a successful closed `VolumeGrid` SignedHeat sample.
...
- `[partial]` `Field.cs`: scalar/vector/tensor field algebra, kernel profiles + anisotropic falloff, RBF + MLS detailed receipts, SDF primitives + closed planar profile extrusion, mesh-backed fields including tangent log-map, iso-surface receipts; `FieldNabla` owns shared validation; `ScalarField` owns the recursive 30-case scalar-payload fold. Native iso-surface bridge proof is landed in `vectors-fields.verify.csx`; paper-faithful MLS, successful closed `VolumeGrid` runtime proof, kernel spatial Hessian, exact exponential maps, and tet FEM remain future work.
...
- `[implemented]` Descriptor projection: `SpectralDescriptorPolicy` owns basis-time zero-mode, scale, energy, and eigenmode-crop normalization; `SpectralRankingPolicy` owns deterministic full-vector ranking; `SpectralFilter.Wave` normalizes the cropped nonzero WKS-style log-Gaussian kernel and emits `SpectralWaveReceipt`. `BandwidthNormalized` tracks policy scale normalization; WKS kernel normalization is recorded on `SpectralWaveReceipt`.
