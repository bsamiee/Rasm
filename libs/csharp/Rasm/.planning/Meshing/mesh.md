...
public sealed partial class MeshLaplacian {
    public static readonly MeshLaplacian Cotangent        = new(key: 0, select: static (cache, key) => cache.Cotangent(key));
    public static readonly MeshLaplacian IntrinsicDelaunay = new(key: 1, select: static (cache, key) => cache.IntrinsicDelaunay(key));
    public static readonly MeshLaplacian TuftedIntrinsic   = new(key: 2, select: static (cache, key) => cache.TuftedIntrinsic(key));
...
public readonly record struct SignpostPolicy(
    ...
    int TraceMaxIters, PositiveMagnitude VertexAngleRescaleFloor, SignpostGauge ReferenceDirectionGauge,
    ...
    public static readonly SignpostPolicy Default = new(
        ...
        DegenerateAreaFloor: PositiveMagnitude.Create(value: RhinoMath.SqrtEpsilon), TraceMaxIters: 0,
    ...
    internal int TraceCapFor(int edgeCount) => TraceMaxIters > 0 ? TraceMaxIters : Math.Max(1, edgeCount) * 16;
...
public readonly partial record struct SparseLaplacian(
    ...
    private bool ValidityGate() =>
        ...
        && Stiffness.Rows.Value == MassConsistent.Rows.Value && Stiffness.Cols.Value == MassConsistent.Cols.Value
        && MassLumped.Count == Stiffness.Rows.Value
        && Tufted.Map(static receipt => receipt.IsValid).IfNone(noneValue: true);
}

// The tufted-cover construction witness; the fold owns every finite/nonnegative conjunct, the hook only the cover law.
[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct TuftedLaplacianReceipt(
    ...
    private bool ValidityGate() =>
        OriginalFaces >= IntrinsicFaces + DroppedNonTriangleFaces && EnergyScaleApplied > 0.0
        && (!CoverAware || (CoverFaces == 2 * IntrinsicFaces && GluingMapIsBijection && GluingSymmetryViolations == 0
...
[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct TuftedBaseFaces(Mesh Triangulated, int TriangleCount, int DroppedNonTriangleFaces) : IValidityEvidence {
    private bool ValidityGate() => Triangulated is { IsValid: true };
...
public readonly partial record struct SignpostTransportReceipt(
...
[ValidityEvidence, StructLayout(LayoutKind.Auto)]
public readonly partial record struct CommonSubdivision(
    ...
    private bool ValidityGate() =>
        ...
        && RowSumResidualA <= RhinoMath.SqrtEpsilon && RowSumResidualB <= RhinoMath.SqrtEpsilon;
}
...
[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct PowerFacet(int SiteI, int SiteJ, double Length, Option<double> OffsetI, Point3d Centroid) : IValidityEvidence {
    private bool ValidityGate() => SiteI != SiteJ;
}

[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct PowerCell(
    int Site, int FragmentCount, double Area, double Mass, Point3d Barycenter, double TransportCost, bool Empty) : IValidityEvidence {
    private bool ValidityGate() => Empty == (Mass <= 0.0) && (Empty || Barycenter.IsValid);
}

[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct RestrictedPowerReceipt(
    ...
    private bool ValidityGate() =>
        SiteCount > 0 && KNearest >= 1 && FragmentAreaMax >= FragmentAreaMin
        && FirstMomentFiniteCount <= FragmentCount && EmptyCellCount <= SiteCount && BoundarySiteCount <= SiteCount;
}

[ValidityEvidence, BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly partial record struct RestrictedPowerDiagram(Arr<PowerCell> Cells, Arr<PowerFacet> Facets, RestrictedPowerReceipt Receipt) : IValidityEvidence {
    private bool ValidityGate() =>
        Cells.Count == Receipt.SiteCount
...
internal sealed class LaplacianCache {
    ...
    private readonly Memo<HodgeKey, HodgeBundle> hodgeCache = new();
    ...
    internal Fin<HodgeBundle> Hodge(HodgeKey probe, Func<Fin<HodgeBundle>> compute) => hodgeCache.Of(probe, compute);
...
internal static class MeshKernel {
    ...
    internal static Fin<SparseLaplacian> LaplacianOf(MeshSpace space, MeshLaplacian kind, Op key) =>
        ...
        from _ in active.Equals(MeshLaplacian.Cotangent)
            ? AspectRatioGuard(mesh: space.Native, ceiling: MeshAssemblyPolicy.Default.AspectRatioCeiling.Value, key: key)
            : Fin.Succ(unit)
    ...
    // Cotangent kind preserves the input triangulation (intrinsic angles consistent with extrinsic positions);
    // Delaunay/tufted kinds run the IDT flip.
    ...
    // Deterministic edge queue, FlipCapPerEdge budget, IsDelaunay cos-sum gate; total re-check proves the invariant.
    ...
    // table. INTERNAL to Vectors — the public cross-package handle is MeshAdjointSnapshot.
    ...
    private static int[][] PowerSiteNeighbours(Point3d[] sites, int kNearest);       // Spatial/neighbors kNN over the site set, self removed
...
|  [02]   | Laplacian selection   | `MeshLaplacian`                        | `[SmartEnum<int>]` + `[UseDelegateFromConstructor]` cache routing   | `Select → Fin<SparseLaplacian>`                      |    3    |
|  [03]   | Memoization           | `LaplacianCache`                       | `ConditionalWeakTable` service, `Atom<HashMap>` success-only memos  | `Memo.Of → Fin<T>`                                   |   19    |
...
- `Meshing/reconstruct` reaches the `SignedHeatDetailed`/`ClosedSignedHeatDetailed`/`EdgeConnectionCholeskyDetailed`/`ScalarHeatCholesky` memo slots through `MeshSpace.Cache`; the signed-heat policies, kernels, and receipts are that page's owners — this page carries only the memo slots and the `SpdMassShift` scale law.
- `Processing/geodesics`, `Processing/segment`, and `Spatial/fields` mesh-aware cases reach every solver artifact through the cache memo accessors (`Geodesic`/`Mcf`/`CrossField`/`Hodge`/`VectorHeat`) — key records (`GeodesicKey`, `McfKey`, `CrossFieldKey`, `HodgeKey`, `VectorHeatKey`, `BoundarySignedHeatKey`, `ClosedSignedHeatKey`) and solution carriers are declared beside their owning kernels on those pages; the cache stays the ONE memo service and mints no second entry.
- `Processing/sample` `PowerCcvtRun` is the sole `RestrictedPowerDiagram` consumer; settled `Processing/receipts` routes `MeshKernel.TopologyDetailed`; settled `Processing/repair` owns `MeshEdit` — this page mints no edit surface.
