...
const _SLUMPED_LATEST = JSON.stringify({
    files: [
        {
            filepath: '/abs/kit.bench.ts',
            groups: [
                {
                    fullName: 'kit hot paths',
                    benchmarks: [{ name: 'fold', hz: 58, rme: 1 }],
                },
            ],
        },
    ],
...
const _run = (
    ...
    Array.map(
        ...
        (hz, index) =>
            new BenchRow({
                at: `2026-01-0${index + 1}T00:00:00Z`,
                name,
                hz,
                rme,
            }),
...
describe('sustained-regression fold', () => {
    it('a window of consecutive slow runs against the baseline is a breach', () => {
        const report = Bench.fold(
            _run('fold', [100, 101, 99, 100, 100, 60, 58, 61]),
        );
    ...
    it('a drift inside tolerance never trips the gate', () => {
        const report = Bench.fold(
            _run('drift', [100, 101, 99, 100, 100, 95, 94, 96]),
        );
        expect(report.verdict).toBe('pass');
    });
...
layer(NodeContext.layer)('gate verb', (it) => {
    it.effect(
        'the gate FAILS typed when the harvested run sustains a slump',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
                    const home = yield* _seededHome(
                        _run(
                            'kit hot paths::fold',
                    ...
                    const fault = yield* Effect.flip(
                        Effect.provideService(Bench.gate(), BenchHome, home),
                    );
    ...
    it.effect(
        'the gate passes a healthy ledger and reports every benchmark verdict',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
                    const home = yield* _seededHome(
                        _run('kit hot paths::fold', [60, 59, 61, 60, 58, 61]),
                    );
                    ...
                    ).toEqual(['kit hot paths::fold']);
                }),
            ),
    );

    it.effect(
        'a second gate over one autosave harvests nothing: duplicated rows cannot forge the slump evidence',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
                    const fs = yield* FileSystem.FileSystem;
                    const path = yield* Path.Path;
                    const home = yield* _seededHome(
                        _run('kit hot paths::fold', [60, 61]),
                    );
                    const gate = Effect.provideService(
                        Bench.gate(),
                        BenchHome,
                        home,
                    );
    ...
    it.effect(
        'a missing autosave is a typed fault: the gate never runs without a harvest',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
                    const fs = yield* FileSystem.FileSystem;
    ...
    it.effect(
        'a corrupted history line is a typed malformed fault, never a die',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
                    const fs = yield* FileSystem.FileSystem;
