...
- [08]-[PREVIEW](.planning/figures/preview.md): Raster/preview owner over `pillow`, `scikit-image`, `segno`, `python-barcode`, `zxing-cpp`, and `python-magic`; the `MARK` encoded-mark arm carries a `Symbology` sub-axis over `segno` QR/Micro-QR/sequence, the `python-barcode` linear 1D registry, and the `zxing-cpp` 2D-matrix DataMatrix/PDF417/Aztec/MaxiCode rows, plus a `DECODE` round-trip op over `zxing-cpp` `read_barcodes` — the former separate-2D-matrix-owner routing note is resolved in this one owner.
