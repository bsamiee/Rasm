...
# one frame roster both the callsite adder and the stdlib logger-name deduction skip, so a line names its producing
# owner instead of the emit machinery that carried it.
IGNORED_FRAMES: Final[list[str]] = ["rasm.runtime.receipts", "rasm.runtime.logging"]
...
def faulted(_: object, __: str, event: ChainEvent) -> ChainEvent:
    ...
    # `exc_info` spellings — a truthy flag, a bare exception, an `(type, value, traceback)` triple — down to the one
    # live object, so the console's structured frames and the wire's semantic triple report one exception rather than
    # two independent reads of a consumed slot. Writing the resolved object back leaves the renderer its own input shape.
...
def _wire(value: object, length: int, items: int, depth: int) -> object:
    ...
    # nesting. `str` and `bytes` are themselves Sequences, so their arms precede the collection folds; the depth guard
...
def _attributed(event: ChainEvent, count: int) -> dict[str, object]:
    # chain insertion order decides which fields survive the count cap, so the shared rows every line carries rank
    # ahead of a producer's tail rather than dropping by hash order. Values arrive already bounded from the chain row.
    return dict(islice(((key, value) for key, value in event.items() if key not in RECORD_SLOTS), count))
...
def _severity(event: ChainEvent) -> SeverityNumber:
    ...
    # folding onto that closed literal before the row is read.
    match event.get("_record"):
        case logging.LogRecord() as record:
            numeric = record.levelno
        case _:
            numeric = LEVEL_METHOD[event["level"]][0]
    return SeverityNumber(max(numeric, 0) // 10 * 4 + 1) if numeric < 50 else SeverityNumber.FATAL
...
def _uncaught(door: str, project: Uncaught, prior: Callable[..., object], *payload: object) -> None:
    ...
    # are both producer-reachable code, so the fence is what makes that predecessor unconditional: a raise escaping
    # this door replaces the interpreter's own report with itself, and this door is the surface that just failed.
    # The rail is discarded by law — the one sink that would carry it is the plane this fence caught refusing.
...
DOORS: Final[Block[tuple[ModuleType, str, Uncaught]]] = Block.of_seq([
    ...
    (threading, "excepthook", lambda args: (args.exc_value, {"origin": args.thread.name if args.thread else ""})),
])
        ...
        with cls._gate:
            ...
            handler.setFormatter(
                structlog.stdlib.ProcessorFormatter(
                    # ExtraAdder rides the foreign path alone — native events never populate stdlib `extra` — and precedes
                    # the shared rows, so correlation, the payload bound, redaction, and the wire projection each govern
                    # the fields it merges rather than admitting a foreign record's whole `extra` mapping unread.
