...
- Growth: a new kernel route is one `TensorOpFamily` row with one `EquivalencePolicy` row; convolution lands as one matrix-kind row lowered through `numeric#KERNEL_LOWERING` im2col and pooling as one structural-kind row lowered to the strided-window route; a new differentiable operator is one `DifferentiableOp` row binding its vector-Jacobian-product, so the six DDG geometry rows (`Gradient`/`Divergence`/`Curl`/`CotangentLaplacian`/`HeatFlow`/`Spectral`) each gain reverse-mode adjoint coverage by one `DifferentiableOp` row routing to the `Backward.Operator` apply, and a new geometry operator (remeshing-step, connection-Laplacian) lands as one `vocabulary#OPERATION_TABLE` geometry row plus one `DifferentiableOp` row, never a parallel autodiff surface; zero new surface.
