...
- [02]-[GRAPH](.planning/Graph/element.md): the `NodeId` `[ValueObject<string>]` identity owner (rooted Guid-v7 / non-rooted `XxHash128` content hash), the `Node` `[Union]` seven-case property-graph vocabulary, the `ToCanonicalBytes` shared canonical projection, the `Header`, the frozen `ElementGraph` with the built-once incidence index and `QuikGraph` topology view, and the memoized `Bake` derived fold.
...
- [07]-[QUANTITY](.planning/Properties/quantity.md): the `Dimension` `[ComplexValueObject]` seven-SI-exponent quantity-type discriminator and the `MeasureValue` SI-coerced scalar carrier with its dimension-checked QTO accessors over the `UnitsNet` registry.
...
- [09]-[ACOUSTIC](.planning/Composition/acoustic.md): the `AcousticBand` octave-centre vocabulary, the banded `Acoustic` carrier, the `Nrc`/`Saa`/`StcWeighted` pure projection folds, and the shared `StcContourFit` single-number kernel.
- [10]-[ASSESSMENT](.planning/Assessment/assessment.md): the generic `AssessmentPayload` analysis receipt keyed by `Discipline`+route+`InputKey`, the `AssessmentOutcome` lifecycle, the typed `Results` bag, and the content-keyed `ResultBlob`/`Provenance`.
