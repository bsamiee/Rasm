...
`pyktx` owns the IN-PROCESS KTX2 container — the GPU-texture file the estate's texture egress ships to a web or desktop consumer. It authors a `KtxTexture2` from a create-info record, fills each mip level, array layer, and cubemap face from raw bytes, encodes the payload to UASTC or ETC1S through Basis Universal, supercompresses with Zstd or ZLIB, transcodes a Basis payload down to BC1/BC3/BC4/BC5/BC7, ETC2, ASTC, PVRTC, or an uncompressed row, and writes to a file or to `bytes`. It is a `cffi` binding over the SAME `libktx` the provisioned `ktx` CLI ships, so the two legs agree byte-for-byte on the container.
...
- capability: KTX2 and KTX1 create, read, and write; per-level, per-layer, and per-face image placement; Basis UASTC and ETC1S encode; Zstd and ZLIB supercompression; ASTC encode across 24 block dimensions; Basis transcode to twenty-one target formats; key-value metadata; and the offset and size geometry of every image in the payload
...
|  [07]   | `data`, `data_size`, `data_size_uncompressed`, `row_pitch`       | the raw store and its measures                             |
...
|  [04]   | `kv_data` and `kv_data_raw`                                 | the key-value metadata block                                  |
...
[RAIL_LAW]:
- Package: `pyktx`
- Owns: the in-process KTX2 container — create-info authoring, per-level, per-layer, and per-face image placement, Basis UASTC and ETC1S encode, ASTC encode across every block dimension, Zstd and ZLIB supercompression, Basis transcode to the twenty-one target formats, key-value metadata, and the payload offset and size geometry
- Accept: one create-info record sized for the whole pyramid with `ALLOC`; caller-built mip levels placed one call each; encode policy as a stored `KtxBasisParams` or `KtxAstcParams` row selected by channel role; `needs_transcoding` as the transcode predicate; a file round-trip between `deflate_zstd` and `transcode_basis`; images passed as contiguous `bytes` at the declared `VkFormat`; provisioning evidence asserting presence and the subcommand roster
- Reject: `transcode_basis` on a texture still holding its Zstd supercompression, which raises `TRANSCODE_FAILED`; a reader branching on `vk_format` where `needs_transcoding` is the predicate; `generate_mipmaps` read as a CPU mip fold; a bare `int` where the parameter record carries the policy; a strided or wrong-dtype buffer reaching `set_image_from_memory`, which no boundary check catches; any pixel-format encode or image decode claimed here, which belongs to `imagecodecs` and `pyvips`; a `--version` assertion against a `ktx` binary; KTX1 authoring; an import-time `libktx` version check, which this binding never performs
