...
`python-bidi` (import `bidi`) owns the UAX#9 Unicode Bidirectional Algorithm reorder for the artifacts text-shaping rail: `get_display` resolves a logical-order `str`/`bytes` into visual display order over the Rust `unicode-bidi` core, and `get_base_level` probes the first paragraph's base direction (`0` LTR / `1` RTL). Both public names re-export from `bidi.wrapper` onto the native `bidi.bidi` PyO3 extension; the pure-Python `bidi.algorithm` ships the full UAX#9 stage pipeline over a mutable storage dict and the `bidi.mirror.MIRRORED` pairing table as the reference/diagnostic path. `typography/shape#SHAPE` drives it as the `ShapeOp.BIDI` pre-pass splitting a mixed-direction logical run into directional level runs BEFORE `uharfbuzz` shapes it, gated onto the `runtime` `to_process` subprocess seam because the native binding has no in-process gated package.
...
- build-floor: native cp315 wheel, not cp-gated — the operative bidi path on this interpreter (`pyicu` is cp-gated absent)
...
`bidi.algorithm` is the pure-Python reference implementation of the same public surface (its `get_display(str_or_bytes, encoding='utf-8', upper_is_rtl=False, base_dir=None, debug=False)` adds the `upper_is_rtl` debug knob) exposing the UAX#9 stages as separately-callable functions over a `get_empty_storage()` dict — the diagnostic/test path, never the gated production binding. Stages run in fixed UAX#9 order, each taking `(storage, debug=False)` (`calc_level_runs` takes `(storage)`); `get_embedding_levels` drives them and fills the storage `chars`/`runs`, `debug_storage` dumps it. Owner code reaches this module only to surface the embedding-level array / level-run partition as receipt evidence or to QA the native reorder against the reference; `[SURFACE]` names drop the `bidi.algorithm.` prefix.
...
[TOPOLOGY]:
- ordering: bidi resolves BEFORE `uharfbuzz` shaping, and the shaping feed is the directional-run decomposition, never the whole-paragraph `get_display` string — the owner splits the logical paragraph into same-level runs, shapes each run's LOGICAL-order text through its own `Buffer.add_str`+`shape` with the run-level-derived direction (HarfBuzz reverses glyph order inside an RTL run, so the cluster->source mapping survives per run), then orders the shaped runs into visual order per UAX#9 L2. `get_display` is the whole-paragraph reorder only where reordered TEXT itself is the deliverable; on a pure-LTR run the arm is a no-op the owner elides (`get_display('hello') == 'hello'`; `get_display('אבג abc') == 'abc גבא'`).
...
- universal-tier transport (`.api/`): the gated reorder rides the `anyio` (`.api/anyio.md`) `to_process.run_sync(func, *args, cancellable=False, limiter=...)` subprocess seam under the shaping-wide `CapacityLimiter` — the SUBPROCESS lane, not the `to_thread` lane `uharfbuzz`/`blackrenderer` use, because the PyO3 binding is not the in-process gated package — with worker death surfacing as `anyio.BrokenWorkerProcess`; every `BIDI` arm returns the `expression` (`.api/expression.md`) `Result`/`RuntimeRail[ContentKey]` rail the rest of `ShapeOp` returns, a Rust panic folding into `runtime` `RuntimeRail` via `async_boundary`.
- universal-tier substrate: `beartype` (`.api/beartype.md`) validates the `base_dir`/`text` boundary inputs, the reorder params ride a frozen `msgspec.Struct` (`.api/msgspec.md`) encoded through one module-level `msgspec.msgpack.Encoder` into the content key, the strong-RTL / base-level / override span records through `structlog` + OpenTelemetry (`.api/structlog.md`, `.api/opentelemetry-api.md`) on the worker hop, and a surfaced pure-Python level array lands as a `numpy` (`.api/numpy.md`) `int8` embedding-level array keyed by character index.
