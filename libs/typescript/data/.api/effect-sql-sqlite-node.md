...
- `updateValues: never` is the type-level degradation signal — sqlite lacks multi-row `UPDATE … FROM`, so a row needing it branches through `sql.onDialect` (per-row on sqlite, set-based on pg). `lane/sqlite` + `lane/capability` own the broader degradation table (no RLS → file-per-app tenancy, no `pg_ivm` → in-process `state` folds), parameterized over the dialect; this driver contributes the sqlite row.
...
- `SqlError.cause` is `better-sqlite3`'s `SqliteError`, carrying `message` and an own `code` whose value is the extended result-code NAME — `SQLITE_CONSTRAINT_UNIQUE` and its siblings, the string constants the binding registers — so a fault classifier discriminates on that one field. No constraint-name field exists, and SQLite's message names the violated index's COLUMNS, so a rule keyed on a constraint identifier has nothing to read and parses prose instead.
- `loadExtension` admits a native sqlite capability (vector search, phonetic) — the fail-closed analog to the pg extension matrix; FTS5 and JSON1 are compiled in, and `lane/capability` probes both dialects behind one closed vocabulary.
- DDL rides `withTransaction` atomically on this driver — a `CREATE TABLE` and the `sqlite_sequence` row its first insert mints roll back with the transaction, `ALTER TABLE … RENAME` carries the `sqlite_sequence` row and the autoindex under the new name, and `CREATE TABLE … AS SELECT` yields affinity-named columns with no key, constraint, default, or `NOT NULL` — so `journal/evolve`'s cutover mints its shadow from the relation's own DDL through `sql.unsafe` and seeds the counter with one `INSERT INTO sqlite_sequence … SELECT` before the copy.
...
- `@effect/experimental`(`.api/effect-experimental.md`): `make` requires `Reactivity`, so `sql.reactive` read-your-writes works on the lane, and `SqlPersistedQueue.layerStore` gives `runtime/work` a durable job store on the file lane; EventLog's node journal backs onto this lane via `SqlEventJournal.layer` `[SQL_OVERLAY_BACKING]`.
- within `data`: `SqliteClient.export`/`backup` feed the content-addressed `object` plane (a snapshot blob is content-keyed via the core `ContentKey` mint), `loadExtension` extends `lane/capability` for the sqlite dialect, and `runtime/work` and the `security` state ports see only the neutral `SqlClient` the lane Layer satisfies.
