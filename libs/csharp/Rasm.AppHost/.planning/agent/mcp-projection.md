...
The settled-fence members the cards compose verify against the folder `.api/api-mcp.md` (`ModelContextProtocol`/`ModelContextProtocol.Core`/`ModelContextProtocol.AspNetCore`) and `.api/api-extensions-ai.md` (`Microsoft.Extensions.AI.Abstractions`) catalogues — `McpServerTool.Create(AIFunction, McpServerToolCreateOptions?)`, `McpServerToolCreateOptions`, `ProgressNotificationValue`, `WithTools(IEnumerable<McpServerTool>)`, `AddMcpServer`/`WithHttpTransport`/`MapMcp`, the `StdioServerTransport`/`StreamServerTransport` transport types, `ApprovalRequiredAIFunction`, and the `AIFunction : AIFunctionDeclaration : AITool` chain (`JsonSchema`/`ReturnJsonSchema`/`InvokeAsync`). The server long-running surface (`McpServer.SampleAsync`/`AsSamplingChatClient`/`ElicitAsync<T>`/`GetTaskAsync`/`WaitForTaskResultAsync<T>`/`PollTaskUntilCompleteAsync`, `McpServerOptions.TaskStore`/`SendTaskStatusNotifications`, `RequestContext.EnablePollingAsync`/`RequestContext<T>.Server`) and the `WithStdioServerTransport()` builder extension are NOT in the folder `.api/api-mcp.md` catalogue — its `McpServerOptions` property scope omits `TaskStore`/`SendTaskStatusNotifications`, no `McpServer`/`RequestContext` member table carries the sampling/elicitation/task verbs, and the entrypoint scope catalogues only `WithHttpTransport`/`MapMcp`/`AddMcp` transport extensions — so these members are the `[SDK_LONG_RUNNING]` source-map RESEARCH card below, authored from the official SDK surface and the open verification before transcription, never asserted as catalogue-verified. Each settled card below states the projection's binding shape over the catalogued members.

- [SDK_HOSTING]: the app-root server-builder fold is `services.AddMcpServer()` (`IMcpServerBuilder`, catalogued) then a transport extension — `WithHttpTransport(o => o.Stateless = false)` + `app.MapMcp()` (`ModelContextProtocol.AspNetCore`, both catalogued) for the SSE/HTTP transport, and a stdio transport for the stdout/stdin host — and the enumerable registration `.WithTools(IEnumerable<McpServerTool>)` (catalogued) over the projected catalog, never the generic attribute-discovery `.WithTools<T>()` or `.WithToolsFromAssembly()`: `ToolProjection.Adopt` constructs `McpServerTool` instances programmatically with no `[McpServerToolType]`/`[McpServerTool]` attribute surface for the generic/assembly overloads to scan, so the projected surface reaches the server as one deliberate registry. The stdio builder extension `WithStdioServerTransport()` is uncatalogued and rides the `[SDK_LONG_RUNNING]` source-map card; the catalogued seam is the `StdioServerTransport` type the app root constructs and mounts directly, so the stdio host carries no fabricated builder member. `McpServerToolCreateOptions` carries settable `Name`/`Title`/`Description`/`ReadOnly`/`Idempotent`/`Destructive`/`OpenWorld`/`UseStructuredContent`/`OutputSchema`/`SerializerOptions`/`SchemaCreateOptions`/`Metadata`/`Services`, with `ReadOnly`/`Destructive`/`Idempotent` typed `bool?`; the projection sets `ReadOnly`/`Destructive`/`Idempotent` from `EffectClass`/`Idempotency`, `UseStructuredContent = true`, and `OutputSchema` explicitly from the `CommandReceipt` schema, because an unset `OutputSchema` defaults to the return-type shape rather than the receipt shape. The MCP HTTP transport is the app-root pin named in `ARCHITECTURE.md`, never an interior dependency of this package and never a hand-rolled JSON-RPC transport beside the SDK.
...
- [SDK_PROGRESS]: the progress fan binds by declaring an `IProgress<ProgressNotificationValue>` parameter on the tool method — the SDK auto-binds it from the request's `_meta.progressToken`, which never news up host-side; `ProgressNotificationValue` (catalogued: required `float Progress`, optional `float? Total`/`string? Message`) is the wire value. The SSE `Last-Event-ID` resumption, the task primitive's status/poll/result surface, the `notifications/cancelled`-to-`CancelScope` binding, and the server-initiated sampling (`IChatClient`, catalogued in `api-extensions-ai.md`) and elicitation server→client legs that force `HttpServerTransportOptions.Stateless = false` are the SDK's long-running-request transport — the `TaskStore`/`SendTaskStatusNotifications`/`EnablePollingAsync`/`GetTaskAsync`/`WaitForTaskResultAsync<T>`/`PollTaskUntilCompleteAsync`/`SampleAsync`/`AsSamplingChatClient`/`ElicitAsync<T>` member spellings ride the `[SDK_LONG_RUNNING]` source-map card and are the open verification before transcription, not catalogue-settled members.
- [SDK_LONG_RUNNING]: the server long-running and stdio-builder members the streaming and sampling cards compose are NOT in the folder `.api/api-mcp.md` catalogue — `McpServerOptions.TaskStore`/`SendTaskStatusNotifications` (the catalogued `McpServerOptions` property scope stops at `ToolCollection`/`PromptCollection`/`ResourceCollection`/`ServerInfo`/`Capabilities`/`ProtocolVersion`/`InitializationTimeout`/`ServerInstructions`), the `RequestContext`/`RequestContext<T>.Server`/`EnablePollingAsync` request-context surface, the `McpServer.GetTaskAsync`/`WaitForTaskResultAsync<T>`/`PollTaskUntilCompleteAsync`/`SampleAsync`/`AsSamplingChatClient`/`ElicitAsync<T>` session verbs, and the `WithStdioServerTransport()` builder extension are authored from the official `ModelContextProtocol`/`ModelContextProtocol.Core` SDK surface and the `IMcpTaskStore`/`InMemoryMcpTaskStore` catalogued task-store contract; their exact spellings, the `RequestContext` type table, and the `McpServerOptions` task-surface properties are the one open verification before transcription, mirroring the `[HEALTH_SERVICE]` transitive source-map at companion#RESEARCH — the assay binder registers these once it indexes the full `ModelContextProtocol.Core` server assembly, and until then the page never asserts them as catalogue-verified.
...
// stdio transport mounts the catalogued StdioServerTransport type at the app root;
// the WithStdioServerTransport() builder shortcut rides [SDK_LONG_RUNNING] until catalogued.
