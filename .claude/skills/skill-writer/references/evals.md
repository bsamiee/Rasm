...
- [RATIONALIZATIONS]: The baseline transcript is mined for the model's own justifications at the moment it deviates — recorded verbatim, each becoming the body rule or gate that forecloses it. Pressure re-runs iterate until a run yields no new rationalization; a rule authored against an imagined excuse rather than a recorded one is speculation wearing law.
