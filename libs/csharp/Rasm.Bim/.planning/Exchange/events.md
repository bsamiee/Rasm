...
`Model/observability#HOOK_RAIL` owns the in-process tap and this page the durable outbox payload. `BimEvent` composes each owning rail's key grammar as settled vocabulary; every owning rail carries its own `- Events:` mint row naming the case it projects.
