...
[ENTRYPOINT_SCOPE]: `EContext` threads into every rounding operation; `Unlimited` holds an accumulation exact and `WithBlankFlags()` arms condition recording, where an absent `FlagInexact` certifies exact output.
...
[TOPOLOGY]:
- Representation independence: `EInteger` shares no substrate with `System.Numerics.BigInteger`, so a `PeterO` reading and a BCL bignum reading of one value are evidence from unrelated implementations.
...
- Predicate filter: ordinates lift through `EFloat.FromDouble`, fold under the directed `Floor`/`Ceiling` contexts into the `Interval` bracket, and `EFloat.Sign`/`IsZero` read the bracket's verdict before the value is discarded.
...
[STACKING]:
- `TYoshimura.DoubleDouble`(`Rasm/.api/api-doubledouble.md`): `ddouble` is the fixed 106-bit refine tier of the direct ladder, its `TwoProduct`/`TwoSum` transforms matching the `Expansion` kernel; the `EFloat` `Interval` bracket opens the constructed-point ladder instead, where no rounded-coordinate `double` filter exists, and both ladders end at `Expansion`.
...
- Kernel consumer anchor: `ERounding.Floor`/`Ceiling` with `ForPrecisionAndRounding(53, …).WithPrecisionInBits(true)` bracket the software-rounded interval endpoints of the constructed-point sign filter — the form `Numerics/predicates.md` `Interval` composes at its `Down`/`Up` statics, and the SAME finite bounded context `NextPlus`/`NextMinus` require to answer at all. The in-house adaptive Shewchuk `Expansion` kernel is the exact tier that filter escalates to and the verdict owner; a differential test recomputes from original ordinates in the test assembly rather than re-reading the terminal expansion through a second carrier.
...
[LOCAL_ADMISSION]:
- `EFloat` under the directed `Floor`/`Ceiling` contexts is the kernel's interval filter for constructed points, below the exact `Expansion` tier where the runtime ladder ends; no kernel predicate member reads `EContext.Unlimited`. `Unlimited` carries an unlimited exponent range, so the adjacency neighbours and every analytic operation take a finite bounded context, and an unlimited context there returns NaN or raises `FlagInvalid` rather than refusing.
- `EDecimal` carries General Decimal Arithmetic and IEEE-754 decimal semantics for exact human-readable I/O and banker's rounding; geometry filtering uses `EFloat`.
