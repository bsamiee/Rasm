...
- One `Config` compiles once and its `Processor` cache is internal, so a plane fold acquires the processor outside the loop and applies inside it. `Processor.getCacheID()` supplies the stable identity `ConfigSource.identity` returns without serializing the graph.
