...
- Boundary: waveform corpora — long SHM records, python-fitted filter banks, reference spectra — enter through `Runtime/codecs#FIELD_RESULT_CODEC` `InterchangeIo.ImportWaveforms` as `WaveformCorpus` under declared frame/hop selections, and this lane composes the carrier's frame-major planes into its own framing, windowing, and PSD folds; the lane stores nothing and opens no `H5File` — the archive one-owner ruling stands, and the estimator (Arrow) and monitor (receipt-stream) storage declines stay closed.
