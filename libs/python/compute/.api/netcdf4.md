# [PY_COMPUTE_API_NETCDF4]

`netCDF4` supplies Python bindings to the Unidata netCDF-4 C library — hierarchical group/dimension/variable containers, CF-compliant time conversion, multi-file dataset aggregation, compression filters, and chunking controls — for the compute climate/geophysical data-loading rail.
...
- compression: `zlib`, `szip` (`szip_coding`, `szip_pixels_per_block`), `blosc` (`blosc_shuffle`), `bzip2`, `zstd` — detected via `has_blosc_filter()`, `has_bzip2_filter()`, `has_szip_filter()`, `has_zstd_filter()`
