...
public abstract partial class MaterialPropertySet {
 ...
 static K<Validation<Error>, double> Guarded(bool valid, double value, string name, Op key) =>
  valid ? value : ElementFault.ValueRejected(key, $"<{name}:{value:R}>");

 static K<Validation<Error>, MeasureValue> Positive(Fin<MeasureValue> column, string name, Op key) =>
  column.Bind(m => m.Si > 0.0 ? Fin.Succ(m) : ElementFault.ValueRejected(key, $"<{name}-non-positive:{m.Si:R}>")).ToValidation();

 static K<Validation<Error>, MeasureValue> Positive(MeasureValue column, string name, Op key) =>
  Positive(Fin.Succ(column), name, key);

 static K<Validation<Error>, MeasureValue> PositiveSi(double value, QuantityType type, Dimension dimension, string name, Op key) =>
  double.IsFinite(value) && value > 0.0
   ? MeasureValue.OfSi(type, dimension, value)
   : ElementFault.ValueRejected(key, $"<{name}-non-positive:{value:R}>");

 static K<Validation<Error>, Option<double>> OptionalPositive(Option<double> value, string name, Op key) =>
  value.Exists(static v => !double.IsFinite(v) || v <= 0.0)
   ? ElementFault.ValueRejected(key, $"<{name}-non-positive>")
   : value;

 static K<Validation<Error>, Option<(double AlphaPerS, double BetaS)>> Rayleigh(Option<(double AlphaPerS, double BetaS)> pair, Op key) =>
 ...
 static K<Validation<Error>, ImmutableArray<double>> Matrix(ImmutableArray<double> impacts, Op key) =>
