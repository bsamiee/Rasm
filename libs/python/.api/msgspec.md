...
- `protobuf-py`(`.api/protobuf-py.md`): generated `rasm.contracts` classes ARE the wire vocabulary, so no `Struct` mirrors one; where a domain `Struct` admits from a message, `message_to_json_value` hands the ProtoJSON mapping to `convert(mapping, StructType)` and `to_builtins` inverts it into `message_from_json_value` — the `transport/wire#MIRROR_TRANSCODE` MessagePack mirrors stay the one decode-only `convert(strict=False)` leg, positional arrays zipped onto their producer's key roster.
- transport/wire: `CrdtOpDecode` caches one `msgpack.Decoder` per op family, and the `Raw`-typed `OpLogEntry.payload` defers the inner op decode until the lane column routes it, so a foreign-lane entry crosses the reader without ever being decoded. `ext_hook` and `Raw.copy()` stand as the declared growth seam that page names for an `Ext`-typed producer slot and a detached buffer view; no producer publishes either today.
