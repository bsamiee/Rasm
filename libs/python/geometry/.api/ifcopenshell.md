...
[LOCAL_ADMISSION]:
- `geometry:ifc` owner admits `ifcopenshell.open`, the query/authoring/tessellation surface, and `util` analysis as the ifc domain: a path opens the model, the usecase vocabulary authors it, and `geom` meshes it under a `geometry_library` kernel.
