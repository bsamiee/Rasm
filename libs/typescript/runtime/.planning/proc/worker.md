...
- [04]-[RUNNER_BOOT]: the handler record, the worker boot module, the spawn boundary; runner Layer.
...
- Law: `Transferable.ImageData` stays unrostered HERE and only here — the row crosses a DOM type this pool's node and bun members never construct, so `browser/fetch#WIRE_PROTOCOL`'s `Imprint` request owns the pixel crossing whole, taking the plane and answering encoded octets off an `OffscreenCanvas` no node or bun member holds; a row minted on this boundary would be unspellable on the platform it ships to, so the omission is a decided split, never a gap in the `Transferable` roster this page otherwise carries whole.
- Law: request statics carry the call surface — each class owns its call static composing the pool Tag (`Render.rendered`, `Drop.announced`), so a consumer imports the request and reaches the whole interface; the stated union on the static is the marshal truth: domain fault, wire decode, worker transport.
...
- Boundary: browser worker spawn and the `BrowserWorker` binding are `browser/fetch#BINDING_ROWS`'s rows under `browser/boot`'s root; this page owns the protocol and the node/bun binding.
...
The worker entry module — `runtime/src/proc/worker.main.ts`, the exports-nothing boot entry the spawn factory names:
