...
- import: `import virtualizarr as vz`; import `ObjectStoreRegistry` from `obspec_utils.registry` (the `virtualizarr` re-export is deprecation-flagged). `virtualizarr` is not on the manifest banned-module-level-imports list, so the module-top `import virtualizarr as vz` is admitted (the banned set is `dask`/`numpy`/`pandas`/`polars`/`scipy`/`sympy`/`xarray`).
