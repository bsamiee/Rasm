...
[COMPUTE_HOOK_RAIL]-[COMPLETE]: landed as `COMPUTE_POINTS`/`SCOPE_DOMAIN` with the `registered`/`tapped` legs and the `_measures` projection on `libs/python/compute/.planning/graduation/observability.md`; the `rasm.compute.*` instrument rows remain a runtime `runtime/observability/metrics#METRIC` ripple.
