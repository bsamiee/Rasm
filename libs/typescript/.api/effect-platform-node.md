...
|  [06]   | `NodeHttpServerRequest.toIncomingMessage` / `.toServerResponse`  | node interop   | `edge` — raw `node:http` (stream/upgrade)        |

[ENTRYPOINT_SCOPE]: sockets, workers, and stream bridges
- rail: system-apis
- `NodeSocket`/`NodeSocketServer` bind `ws`-backed WebSocket behind `Socket`/`SocketServer` (`runtime/src/net/channel.ts`); `NodeStream`/`NodeSink` bridge Node `Readable`/`Duplex`/`Writable` ⇄ Effect `Stream` and process stdio; `NodeWorker.layer(spawn)` binds the worker-thread pool for `Worker.makePoolSerialized`.
...
- `@effect/opentelemetry`: `NodeSdk.layer` rides beside `NodeContext.layer` to bind the `Tracer`/`MetricRegistry`; `runtime/src/otel/emit.ts` owns the `NodeSdk` row.
