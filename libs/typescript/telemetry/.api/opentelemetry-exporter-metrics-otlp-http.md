...
- consumed-by: `telemetry/otlp/export` SDK-bridge metric leg via the facade's `NodeSdk`/`WebSdk` `Configuration.metricReader`; the wire under `signal/meter`'s usage/cost counters
- catalog-verdict: KEEP as SDK-bridge peer; edge-ledger fences `@opentelemetry/*` to `scope:telemetry`; `[R3]`-collapse member (native `OtlpMetrics` supersedes)
...
- Stack with `.api/effect-platform.md` posture (the divergence to record): like the trace exporter, this carries its own HTTP transport and does NOT inherit the `host/net/client` `HttpClient` retry/proxy policy — a concrete reason `otlp/export` prefers the native `OtlpMetrics` lane and marks this row `[R3]`.

[LOCAL_ADMISSION]:
- `@opentelemetry/*` is admitted ONLY inside `scope:telemetry` (edge-ledger ban); the exporter is constructed at the composition root only. Instrumentation code uses Effect's native `Metric` and never imports this package.
...
- the browser exporter is the RUM metric egress leg (`signal/vital`); apply the export-boundary redaction policy rows before the metric leaves the browser.
