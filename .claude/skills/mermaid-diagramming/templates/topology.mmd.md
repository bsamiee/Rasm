...
Refill by renaming zones and services to the real deployment, keep a port pair on every edge, and re-derive the `align` grid whenever a service lands — every row and column of the intended grid is declared, because the constraint set is the layout. Pink polygon arrows, Selection icon plates, and Lavender dashed group walls are fixed law — a refill renames the fleet, never strips the fidelity surface.
