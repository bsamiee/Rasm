...
[STACKING]:
- base-abstraction boundary: the driver IS the concrete `AdbcDriver` for the `Apache.Arrow.Adbc` abstraction (`api-arrow-egress.md`); the Persistence Query-federation owner selects it by protocol, opens it with a parameter map, and reads the base `QueryResult.Stream` `IArrowArrayStream` — one egress shape across every warehouse backend, distinct from the in-process DuckDB path (`api-duckdb.md`)
- arrow-result boundary: every statement returns Arrow `RecordBatch`, so a Spark/Hive/Impala result and an in-process batch are the SAME `Apache.Arrow` type (`libs/dotnet/.api/api-arrow.md`), folding directly to Parquet (`api-parquetsharp.md`) or a Delta table (`api-deltalake.md`) with zero re-materialization
- substrait boundary: the base `AdbcStatement.SubstraitPlan` executes a `FlowtideDotNet.Substrait` plan (`api-flowtide-substrait.md`) against the Thrift warehouse where the backend implements it, so one portable relational-algebra IR drives both the federation pipeline and the remote SQL endpoint
- transport-security boundary: the `HttpTlsOptions`/`StandardTlsOptions`/`HttpProxyOptions` key sets compose with the deployment mTLS/proxy posture, populated from the same redacted-credential source the KMS/KeyVault owners (`api-aws-kms.md`, `api-azure-keyvault.md`) feed — a private-VPC self-signed cert sets `HttpTlsOptions.AllowSelfSigned`, never `DisableServerCertificateValidation`
...
- `HiveServer2Exception` carries every statement fault; the boundary lifts its `SqlState`/`NativeError` onto the canonical `Fin`/typed-failure result, and no `AdbcException` crosses into domain logic
