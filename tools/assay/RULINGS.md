...
`tools/assay` rulings settle tool-scoped decisions.

## [01]-[PACKAGES]

- Phase-journalled swaps bind each target to identity and full image digest; rollback restores whole and committed recovery cleans a proved image.
...
- `Report.counts` is a census keyed on `RailStatus`; `RailStatus.band` owns the proved/refused/unproved partition and every total derives at read.
- Only a lane that ran and answered bands as a verdict — a skip, an unsupported scope, a timeout, and a faulted tool alike prove nothing.
