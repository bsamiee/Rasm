...
- Add every core package to root `Directory.Packages.props` at newest viable versions; project references stay versionless; no version numbers in `.csproj`. Core packages: `Microsoft.EntityFrameworkCore.Sqlite`, `Microsoft.Data.Sqlite`, `Microsoft.EntityFrameworkCore.Design` (`PrivateAssets=all`), `SQLitePCLRaw.bundle_e_sqlite3`, `SourceGear.sqlite3`, `NodaTime`, `NodaTime.Serialization.SystemTextJson`, `FluentValidation`, `MessagePack`, `MessagePack.Generator`, `EFCore.NamingConventions`, `EFCore.BulkExtensions`, `K4os.Compression.LZ4`, `Microsoft.Extensions.Compliance.Redaction`. `LanguageExt.Core`, `System.Reactive`, and `DynamicData` are already in central management via the App UI group.
