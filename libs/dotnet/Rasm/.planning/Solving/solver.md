...
- [02]-[LM_FUNCTOR]: `Lm.Minimize` the one damped Gauss-Newton iterate — a `Next` loop-or-done fold over the `ILmModel` residual+Jacobian floor and `SolvePolicy` ladder — with `Dual<T>`/`DualModel` deriving exact Jacobians from residual code alone, and `ObjectiveSense` the branch objective-direction vocabulary the maximizing consumers fold through.
- [03]-[CONSTRAINT_SOLVER]: `Constraint` algebra, union-find island decomposition, structural and witness `Determinacy` verdicts, and `ConstraintSystem.Solve` returning `Solution`.

## [02]-[LM_FUNCTOR]

- Owner: `ILmModel` mints the residual+Jacobian floor — `Dof`, `Seed`, the 106-bit `Norm`, and packed-upper `Linearize` — the open instance-interface contract every residual-row system implements; `IDualResidual` is the residual-only floor beside it and `DualModel` the adapter conforming one to the other; `Dual<T>` the forward-mode scalar those two differentiate through, satisfying `INumber`/`IRootFunctions`/`IPowerFunctions`/`IExponentialFunctions`/`ILogarithmicFunctions`/`ITrigonometricFunctions` at exactly the constraint set it demands of `T`; `SolvePolicy` the λ-ladder policy record whose `Of(Context)` factory derives every threshold off its own lane and whose `LambdaUp`/`LambdaDown`/`LambdaFloor` operands the ladder transitions read in place; `LmState` the internal trial carrier; `LmResult` the typed outcome registering `IValidityEvidence`; `Lm` the static functor surface.
- Cases: `SolveStatus` closes over the three ways an iterate ends — `Converged` on the residual tolerance, `Stationary` on a step floor the residual never cleared, `Exhausted` on a spent budget with a live descent direction — and its KEY ordinal IS the severity the island fold maxes over, so no bool accumulator re-states that ordering. LanguageExt `Next<LmState, (LmState State, SolveStatus Status)>` is the loop-or-done carrier the driver folds and never interprets — a terminal status exists only on the `Done` arm, so a live trial state and a settled status never coexist. The singular outcome rides the `Fin` failure channel as `GeometryFault.SingularSystem`, kept off the status vocabulary. `ObjectiveSense` `Minimize`/`Maximize` is the branch objective-direction vocabulary seated beside the status roster: the kernel iterate itself always minimizes, and a consumer whose objective points the other way folds `Sign * objective` through the row rather than a call-site sign literal — Compute's optimizer descent rows and Fabrication's orientation scoring compose these two cases, so direction spells once for the branch.
- Entry: `Lm.Minimize(ILmModel, SolvePolicy)` is the one nonlinear least-squares entrypoint. It routes `GeometryFault.SingularSystem(rank, dof)` when the damped normal matrix stays rank-deficient past `LambdaCeiling` — rank read as the `JᵀJ` eigen-rank through `SymmetricMatrix.DecomposeEigenDetailed`, counted spectral-radius-relative against `EpsilonPolicy.SqrtEpsilon`, a functor-computable witness needing no dense `J`; every other outcome is a success-carrier `LmResult` whose `Status` separates the converged fixpoint from the stationary one and both from the spent budget — a caller reads the row before consuming the vector, and a stationary or exhausted run wearing `Converged` is the killed shape.
- Auto: every foreign model member executes inside the `Try.lift` funnel — policy and model admission reject a non-finite, non-monotone, or budget-uncrossable policy and a mis-shaped or non-finite seed, and no member runs outside the boundary. The iterate is ONE loop-or-done fold: `Pass` returns `Next.Loop` or `Next.Done`, `IO.FoldUntil` under `Schedule.recurs(MaxIterations)` threads the carrier as its own accumulator, and a fold still on `Loop` when the schedule stops IS the typed exhaustion — the accept ladder and the reject ladder are the same pass on one budget, and a rejected trial keeps its linearization because raising `λ` moves the damped diagonal alone. Convergence is `‖r‖₂ < ResidualTolerance`; a `‖δ‖₂ < StepFloor` step settles `Stationary` unless the residual it reached also clears the tolerance. A zero-diagonal column damps on the bare `λ` floor because multiplicative damping never regularizes an exact zero, holding that coordinate at the seed — the under-constrained manifold behavior the entry promises. One MathNet path `SymmetricMatrix.Of → DecomposeCholesky → SolveDetailed` mints `LinearSolution` gating each step all-finite, so an indefinite non-throwing factor fails the mint and the ladder climbs rather than accept a NaN step. The budget has ONE authority — the schedule counts every pass, accepted and rejected alike — so no ladder climb can outlive it, and objective comparison stays `ddouble` until the one result mint admits a `double` readout.
...
public static class Lm {
    ...
    static Next<LmState, (LmState State, SolveStatus Status)> Descend(
        SolvePolicy policy, LmState state, double[] trial, ddouble trialNorm, double stepNorm) =>
        state with { Parameters = trial, Norm = trialNorm, Iterations = state.Iterations + 1, Normal = None } switch {
            LmState moved => stepNorm < policy.StepFloor
                ? Next.Done<LmState, (LmState State, SolveStatus Status)>((moved,
                    trialNorm < policy.ResidualTolerance.Value ? SolveStatus.Converged : SolveStatus.Stationary))
                : Next.Loop<LmState, (LmState State, SolveStatus Status)>(moved with {
                    Lambda = double.Max(state.Lambda / policy.LambdaDown, policy.LambdaFloor),
                }),
        };

    static Next<LmState, (LmState State, SolveStatus Status)> Reject(
        SolvePolicy policy, LmState state, (double[] Packed, double[] Gradient) normal) =>
    ...
    public (double[] PackedNormal, double[] Gradient) Linearize(ReadOnlySpan<double> parameters) {
        ...
        for (int index = 0; index < residual.Rows; index++) {
            ...
            for (int a = 0; a < n; a++) {
                ...
                for (int b = a; b < n; b++) normal[SymmetricMatrix.FlatIndex(n, a, b)] += partials[a] * partials[b];
            }
        }
...
- Growth: a new geometric relation is one `Constraint` case carrying its `Residual` and `Touches` arms over the same functor; a new parametric primitive is one `SketchEntityKind` row with its arity, `Carrier`, and slice accessors; a new rank method is one `RankMethod` row; a 3D sketch tier is one slice accessor widening over the same constraint algebra.
- Boundary: the relations differ only in residual expression and analytic partials, never in the iterate, so one `Constraint` `[Union]` with a generated-`Switch` fold owns them all — compile-exhaustive, a new case breaking `Residual` and `Touches` loudly, while row cardinality and operand-kind admission both read `Residual` and need no arm; `Concentric` reuses the center-coincidence rows as sketch vocabulary over the one algebra. Every arm's Jacobian is EXACT and hand-coded closed form here, forward-mode duals being the functor's second exact source and a finite difference neither. Every `Numerics/matrix` call QuikGraph owns the partition and matching walks, and every graph verdict exits as a typed domain value. `ConstraintSystem` is immutable to its coordinates — `Arr<double>` carries the seed, so record equality reads the vector and no caller mutates one another reader folds — and the `Model` scratch is the single-writer run-local exception that never escapes the model. Every failure lifts its direct `GeometryFault` case bare onto `Fin`, and the graph-assembly and scatter loops are the named span-kernel statement exemption.
...
public abstract partial record Constraint {
    ...
    public Seq<ResidualRow> Residual(double[] p) =>
        Switch(
            ...
            angle: static (p, a) => {
                (Option<Vector3d> ua, Option<Vector3d> vb) = (a.A.Direction(p), a.B.Direction(p));
                return (ua, vb).Apply((u, v) => {
                    ...
                    double r = Math.Atan2(cross, dot) - a.Radians;
                    ...
                    return new ResidualRow(r, Seq(
                        (a.A.Offset, -dAux), (a.A.Offset + 1, -dAuy), (a.A.Offset + 2, dAux), (a.A.Offset + 3, dAuy),
                        (a.B.Offset, -dBvx), (a.B.Offset + 1, -dBvy), (a.B.Offset + 2, dBvx), (a.B.Offset + 3, dBvy)));
                }).As().ToSeq();
            },
            coincident: static (p, c) => CoincidentRows(c.A, c.B, p),
            concentric: static (p, c) => CoincidentRows(c.A, c.B, p),
            parallel: static (p, l) => {
                (Option<Vector3d> ua, Option<Vector3d> vb) = (l.A.Direction(p), l.B.Direction(p));
                return (ua, vb).Apply((u, v) => new ResidualRow(u.X * v.Y - u.Y * v.X, Seq(
                    (l.A.Offset, -v.Y), (l.A.Offset + 1, v.X), (l.A.Offset + 2, v.Y), (l.A.Offset + 3, -v.X),
                    (l.B.Offset, u.Y), (l.B.Offset + 1, -u.X), (l.B.Offset + 2, -u.Y), (l.B.Offset + 3, u.X)))).As().ToSeq();
            },
            perpendicular: static (p, l) => {
                (Option<Vector3d> ua, Option<Vector3d> vb) = (l.A.Direction(p), l.B.Direction(p));
                return (ua, vb).Apply((u, v) => new ResidualRow((u.X * v.X) + (u.Y * v.Y), Seq(
                    (l.A.Offset, -v.X), (l.A.Offset + 1, -v.Y), (l.A.Offset + 2, v.X), (l.A.Offset + 3, v.Y),
                    (l.B.Offset, -u.X), (l.B.Offset + 1, -u.Y), (l.B.Offset + 2, u.X), (l.B.Offset + 3, u.Y)))).As().ToSeq();
            },
            tangent: static (p, t) => {
                Point3d s = t.Line.Origin(p), c = t.Circle.Origin(p);
                (Option<Point3d> endOf, Option<double> radiusOf) = (t.Line.End(p), t.Circle.Radius(p));
                return (endOf, radiusOf).Apply((e, radius) => {
                    ...
                    return new ResidualRow(r, Seq(
                        (t.Line.Offset, dStartX), (t.Line.Offset + 1, dStartY), (t.Line.Offset + 2, dEndX), (t.Line.Offset + 3, dEndY),
                        (t.Circle.Offset, dCenterX), (t.Circle.Offset + 1, dCenterY), (t.Circle.Offset + 2, -2.0 * radius)));
                }).As().ToSeq();
            },
            pointOnLine: static (p, o) => {
                Point3d q = o.Point.Origin(p), s = o.Line.Origin(p);
                return o.Line.End(p).Map(e => new ResidualRow(((e.X - s.X) * (q.Y - s.Y)) - ((e.Y - s.Y) * (q.X - s.X)), Seq(
                    (o.Point.Offset, s.Y - e.Y), (o.Point.Offset + 1, e.X - s.X),
                    (o.Line.Offset, e.Y - q.Y), (o.Line.Offset + 1, q.X - e.X),
                    (o.Line.Offset + 2, q.Y - s.Y), (o.Line.Offset + 3, s.X - q.X)))).ToSeq();
            },
            midpoint: static (p, m) => {
                Point3d q = m.Point.Origin(p), s = m.Line.Origin(p);
                return m.Line.End(p).Map(e => Seq(
                    new ResidualRow(q.X - (0.5 * (s.X + e.X)), Seq((m.Point.Offset, 1.0), (m.Line.Offset, -0.5), (m.Line.Offset + 2, -0.5))),
                    new ResidualRow(q.Y - (0.5 * (s.Y + e.Y)), Seq((m.Point.Offset + 1, 1.0), (m.Line.Offset + 1, -0.5), (m.Line.Offset + 3, -0.5)))))
                    .IfNone(Seq<ResidualRow>());
            },
            axis: static (p, x) => {
                int start = x.Line.Offset + x.Lock.Component, end = x.Line.Offset + 2 + x.Lock.Component;
                double delta = p[end] - p[start];
                return x.Line.End(p).Map(_ => new ResidualRow(delta, Seq((start, -1.0), (end, 1.0)))).ToSeq();
            },
            equal: static (p, e) => {
                (Option<double> ra, Option<double> rb) = (e.A.Radius(p), e.B.Radius(p));
                Option<ResidualRow> radii = (ra, rb).Apply((left, right) =>
                    new ResidualRow(left - right, Seq((e.A.Offset + 2, 1.0), (e.B.Offset + 2, -1.0)))).As();
                (Option<Vector3d> ua, Option<Vector3d> vb) = (e.A.Direction(p), e.B.Direction(p));
                Option<ResidualRow> spans = (ua, vb).Apply((u, v) => new ResidualRow(
                    ((u.X * u.X) + (u.Y * u.Y)) - ((v.X * v.X) + (v.Y * v.Y)), Seq(
                    (e.A.Offset, -2.0 * u.X), (e.A.Offset + 1, -2.0 * u.Y), (e.A.Offset + 2, 2.0 * u.X), (e.A.Offset + 3, 2.0 * u.Y),
                    (e.B.Offset, 2.0 * v.X), (e.B.Offset + 1, 2.0 * v.Y), (e.B.Offset + 2, -2.0 * v.X), (e.B.Offset + 3, -2.0 * v.Y)))).As();
                return (radii | spans).ToSeq();
            },
            symmetric: static (p, s) => {
                Point3d pa = s.A.Origin(p), pb = s.B.Origin(p), anchor = s.Axis.Origin(p);
                return s.Axis.End(p).Map(e => {
                    double ax = e.X - anchor.X, ay = e.Y - anchor.Y;
                    double mx = 0.5 * (pa.X + pb.X) - anchor.X, my = 0.5 * (pa.Y + pb.Y) - anchor.Y;
                    ...
                    return Seq(
                        new ResidualRow(onAxis, Seq(
                            (s.A.Offset, -0.5 * ay), (s.A.Offset + 1, 0.5 * ax), (s.B.Offset, -0.5 * ay), (s.B.Offset + 1, 0.5 * ax),
                            (s.Axis.Offset, ay - my), (s.Axis.Offset + 1, mx - ax), (s.Axis.Offset + 2, my), (s.Axis.Offset + 3, -mx))),
                        new ResidualRow(perp, Seq(
                            (s.A.Offset, ax), (s.A.Offset + 1, ay), (s.B.Offset, -ax), (s.B.Offset + 1, -ay),
                            (s.Axis.Offset, -chordX), (s.Axis.Offset + 1, -chordY), (s.Axis.Offset + 2, chordX), (s.Axis.Offset + 3, chordY))));
                }).IfNone(Seq<ResidualRow>());
            },
            ground: static (p, g) => {
                Point3d q = g.Point.Origin(p);
                return Seq(
                    new ResidualRow(q.X - g.X, Seq((g.Point.Offset, 1.0))),
                    new ResidualRow(q.Y - g.Y, Seq((g.Point.Offset + 1, 1.0))));
            },
            radius: static (p, r) =>
                r.Circle.Radius(p).Map(radius => new ResidualRow(radius - r.Target, Seq((r.Circle.Offset + 2, 1.0)))).ToSeq(),
            onCircle: static (p, o) => {
                Point3d q = o.Point.Origin(p), c = o.Circle.Origin(p);
                double dx = q.X - c.X, dy = q.Y - c.Y;
                return o.Circle.Radius(p).Map(radius => new ResidualRow((dx * dx) + (dy * dy) - (radius * radius), Seq(
                    (o.Point.Offset, 2.0 * dx), (o.Point.Offset + 1, 2.0 * dy),
                    (o.Circle.Offset, -2.0 * dx), (o.Circle.Offset + 1, -2.0 * dy), (o.Circle.Offset + 2, -2.0 * radius)))).ToSeq();
            });
...
public sealed record DofReport(Determinacy Verdict, Seq<IslandVerdict> Islands) : IValidityEvidence {
    ...
    public bool IsValid => ValidityClaim.All(
        ...
        Islands.ForAll(static row =>
            row.Verdict is not null && row.Method is not null
...
public sealed record ConstraintSystem(
    ...
    public static Fin<ConstraintSystem> Build(
        ...
        return AdmissionSlots.Accumulate(probes)
            .Map(_ => new ConstraintSystem(placed, constraints, new Arr<double>(seed)))
            .ToFin();
    }
    ...
    public Fin<Solution> Solve(SolvePolicy policy) {
        ...
        return islands.TraverseM(island =>
            ...
            .Bind(solved => {
                ...
                foreach (((Seq<int> Entities, Seq<int> Constraints) island, LmResult result) in solved) {
                    int cursor = 0;
                ...
                : jacobian.ToFin(new KernelFault.InvalidResult()).Bind(j => j.DecomposeSvd()).Match(
                    Succ: svd => rows <= svd.Rank
                        ...
                        : new IslandVerdict(ordinal, ConsistentAtWitness(svd, r, rows) ? Determinacy.Redundant : Determinacy.Over,
                            Math.Max(val1: dofs - svd.Rank, val2: 0), rows - svd.Rank, svd.Rank, RankMethod.Witness),
                    Fail: _ => new IslandVerdict(ordinal,
                        ...
                        Math.Min(val1: rows, val2: dofs), RankMethod.Count));
        }).Strict();
        return islands.ForAll(static row => row.Rank == 0 && row.Deficiency == 0)
            ? new DofReport(Determinacy.Under, islands)
            : new DofReport(islands.Map(static row => row.Verdict).Fold(Determinacy.Well, Worst), islands);
    }
    ...
    private sealed class Model : ILmModel {
        ...
        Model(
            ConstraintSystem system, (Seq<int> Entities, Seq<int> Constraints) island, double[] current, int[] columns, FrozenDictionary<int, int> local) {
            this.system = system;
                ...
                foreach (ResidualRow row in system.Constraints[ordinal].Residual(scratch)) {
                    foreach ((int ci, double pi) in row.Partials) {
                        ...
                        foreach ((int cj, double pj) in row.Partials) {
                            ...
                            if (lj >= li) normal[SymmetricMatrix.FlatIndex(n, li, lj)] += pi * pj;
                        }
                    }
                }
            }
...
flowchart LR
    ...
    System -->|"dense J → DecomposeSvd + U-tail"| Witness["RankMethod.Witness"]
...
One owner per axis; capability is a case, row, column, or fold arm, never a sibling surface. Each `[RESULT]` cell names one return type: pure verdicts for total results and `Fin` for `GeometryFault` refusals.

| [INDEX] | [ROUTE]               | [AXIS_CONCERN]       | [OWNER]                   | [RESULT]                      |
| :-----: | :-------------------- | :------------------- | :------------------------ | :---------------------------- |
...
|  [05]   | `DOF_VERDICT`         | DOF verdict          | `Determinacy`/`DofReport` | `RankMethod.Analyze(system)`  |
|  [06]   | `CONSTRAINT_GRAPH`    | Constraint graph     | `ConstraintSystem`        | `Build → Fin`                 |
|  [07]   | `SKETCH_SOLVE`        | Sketch solve         | `ConstraintSystem`        | `Solve → Fin<Solution>`       |
...
- [DAMPED_GAUSS_NEWTON]: one λ-ladder functor over the residual+Jacobian floor; `Next` loop-or-done folded by `IO.FoldUntil`, packed-upper via `SymmetricMatrix.FlatIndex`.
- [LADDER_POLICY]: policy record (λ factors · `PositiveMagnitude` tolerance · step floor · `Dimension` budget) minted by `Of(Context)`.
...
- [CONSTRAINT_ALGEBRA]: `[Union]` + generated-`Switch` `Residual`/`Touches` folds, analytic partials per arm.
- [DOF_VERDICT]: `Determinacy` `[SmartEnum<int>]` whose key ordinal is the island-fold precedence, analyzed by one `RankMethod` row, each island verdict stamped with the row that decided it.
- [CONSTRAINT_GRAPH]: immutable graph + accumulating `Build` + accessor-backed `ForestDisjointSet` island decomposition.
- [SKETCH_SOLVE]: `Seq.TraverseM` over islands instantiating the private `Model : ILmModel` per component, one assembled `Solution`.
- [AUTO_JACOBIAN]: forward-mode dual scalar closing the generic-math floor it constrains on + the `IDualResidual` adapter scattering through `SymmetricMatrix.FlatIndex`.
