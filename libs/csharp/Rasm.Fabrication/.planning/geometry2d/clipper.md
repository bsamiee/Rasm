...
One cluster: `[2]-[POLYGON_ALGEBRA]` owns `PolygonAlgebra` over Clipper2 — the `ClipOp`/`OffsetEnds` axes and the `Offset`/`Clip`/`MinkowskiSum`/`ClipOpenPath` operations covering offset/inflate, Boolean clip, Minkowski sum (NFP), and the open-path screen clip; the one Clipper2 owner.
