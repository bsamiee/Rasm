...
- Entry: an overlays or navigation `RoleBehavior` composes one `useFloatingAnchor` for its placement and dismiss; the open state reads and writes through the `binding/atom.md` `AtomBinding` and drives the `onOpenChange` callback; the anchor and floating elements bind through `refs.setReference`/`refs.setFloating`, never a free React ref leaked past the hook.
