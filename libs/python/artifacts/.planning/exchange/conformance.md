...
- Boundary: no PDF authoring (that stays at `document/emit#DOCUMENT`), no font engineering (that is `typography/font#FONT`), no glyph rendering (that is `typography/shape#SHAPE`); the owner closes an already-emitted PDF, never producing one. pyhanko does NOT enforce PDF/A or PDF/UA structural conformance on its own — it treats them as ordinary PDF, so the structural-conformance verdict is authored upstream at `document/tagged#ACCESS` (the pikepdf `StructTreeRoot`/`StructElem` marked-content tree over the `document/model#NODE` `StructureNode`/`StructEltKind` family) and the `AUDIT` arm CONSUMES that structural result rather than disclaiming it, folding it into the `ConformanceVerdict`; the embed-completeness precondition the PDF/A close requires arrives from `typography/font#FONT` `EMBED_AUDIT`. The `ExternalSigner` injected-signature path covers HSM/remote signing where the signature bytes arrive pre-computed, replacing a `PdfSigner.digest_doc_for_signing` two-phase round-trip with the single `sign_pdf` call. A parallel `_PadesLevel` enum beside the policy behavior and a weak `int` certification permission are the collapsed forms — `PadesLevel` carries its LTV properties and `CertifyPerm.mdp` resolves the typed `MDPPerm`. The `AUDIT` arm reads version/XMP presence over pikepdf and DSS presence over `DocumentSecurityStore.read_dss` rather than claiming a veraPDF-grade structural verdict (no pure-Python PDF/A structural validator resolves on PyPI — the structural verdict is the `document/tagged#ACCESS` PDF/UA result, not a self-authored one); the `ValidationContext` arrives from `pyhanko_certvalidator` at the boundary, never built by this owner. This PAdES PDF-cryptographic close is orthogonal to the `exchange/credential#CREDENTIAL` C2PA cross-format content-authenticity bind — a PDF routes here, a raster/BMFF/audio asset routes to the c2pa rail.
...
class Conformance(Struct, frozen=True):
    ...
    def close(self) -> RuntimeRail[ContentKey]:
        return boundary(f"conformance.{self.step}", self._emit)

    def _emit(self) -> ContentKey:
        return ContentIdentity.of(f"conformance-{self.step}", _STEP_TABLE[self.step](self))
