...
One cluster: `[2]-[REINFORCEMENT_FAMILY]` owns the `RebarGrade` yield-strength axis, the `BarSize` nominal-bar axis, the `RebarBend` ACI 318 bend/hook scalar tuple, the `RebarSection` cross-section receipt, and the `ConnectionCatalogue.BuildRebarRows` ASTM A615/A706 row table.
