...
class Hooks:
    ...
    def subscribe[P: Struct](cls, point_id: str, tap: Tap[P] | Veto[P], *, scope: ScopeKey = DEFAULT_SCOPE) -> RuntimeRail[int]:
        def attached() -> int:
            row = cls._scoped(cls._points, scope)[point_id]  # KeyError converts on the fence: subscribing to an unregistered point refuses
            taps = cls._scoped(cls._taps, scope)
            held = taps.try_find(point_id).default_value(Block.empty()).append(Block.singleton(tap))
            ...
            if row.modality is Modality.REPLAY:  # a late subscriber drains the ring on attach, then observes forward
                cls._scoped(cls._rings, scope).try_find(point_id).default_value(Block.empty()).fold(
                    lambda _, fact: cls._observed(point_id, tap, fact), None
                )
            return len(held)
    ...
    def fire[P: Struct](cls, point_id: str, payload: P, *, scope: ScopeKey = DEFAULT_SCOPE) -> RuntimeRail[P]:
        match cls._scoped(cls._points, scope).try_find(point_id).map(lambda row: row.modality).default_value(None):
            case Modality.VETO:
            ...
            case Modality.OBSERVE:
                cls._scoped(cls._taps, scope).try_find(point_id).default_value(Block.empty()).fold(
                    lambda _, tap: cls._observed(point_id, tap, payload), None
                )
                return Ok(payload)
            case Modality.REPLAY:
                ...
                bound = cls._scoped(cls._points, scope)[point_id].buffer
    ...
    async def fire_async[P: Struct](cls, point_id: str, payload: P, *, scope: ScopeKey = DEFAULT_SCOPE) -> RuntimeRail[P]:
        match cls._scoped(cls._points, scope).try_find(point_id).map(lambda row: row.modality).default_value(None):
            case Modality.VETO:
                return cls.fire(point_id, payload, scope=scope)  # veto taps are sync transform/reject by contract; the sync arm owns them
            case Modality.OBSERVE | Modality.REPLAY as modality:
                if modality is Modality.REPLAY:
                    ...
                    bound = cls._scoped(cls._points, scope)[point_id].buffer
                    cls._rings = cls._rings.add(scope, rings.add(point_id, ring.skip(max(len(ring) - bound, 0))))
                for tap in cls._scoped(cls._taps, scope).try_find(point_id).default_value(Block.empty()):
