...
class _Contention:
    """Refused claim: the flock is held. ``phantom`` marks a stale recorded owner whose flock survives on an inherited fd."""
...
@dataclass(frozen=True, slots=True)
class _ClaimSpec:
    """One acquisition's owner-block payload and staleness tolerance, threaded whole through claim, steal, and write."""
    ...
    try:
        ...
        match held:
            ...
            case _:
                owner = decode_lease_owner(held)
                _stamp_holder(owner)
                match owner is not None and not is_lease_stale(owner, spec.tolerance):
                    case True:
                        return _Contention(owner=owner)
                    case False:
                        _LOG.warning("lease.steal", resource=resource, run_id=spec.run_id, lost=_holder(owner))
                        return _steal(fd, resource, spec, owner)
...
def _steal(fd: int, resource: str, spec: _ClaimSpec, owner: _LeaseOwner | None) -> _LeaseOwner | _Contention:
    try:
        ...
        return _Contention(owner=owner, phantom=owner is None or is_lease_stale(owner, spec.tolerance))
    won = _write_owner(fd, resource, spec)
    _LOG.info("lease.stolen", resource=resource, run_id=spec.run_id, lost=_holder(owner), won=_holder(won))
    return won
...
def exclusive_lease(resource: str, run_id: str, *, settings: AssaySettings, project: str = "", mode: str = "exclusive", scope: LeaseScope = LeaseScope.REPO) -> Generator[Result[_Held, Fault]]:
    ...
    try:
        ...
        match claim:
            ...
            case _Contention() as refusal:
                yield Error(_busy_fault(resource, refusal))
    finally:
        if isinstance(claim, _LeaseOwner):
            os.ftruncate(fd, 0)
            _FLOCK(fd, _LOCK_UN)
        os.close(fd)
