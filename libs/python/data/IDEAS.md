...
- Ripple: `python:compute` `[DATA_STUDY_INPUT]` — `compute/experiments` reads the Lance vector index over the embedding columns this `AssetStore` exposes as one of the two still-blocked study-input edges that card declares-but-holds; both endpoints stay BLOCKED until `LAKEHOUSE_RESEARCH_SETTLE` flips `[LANCE_VERSION]` settled.
...
[QUERY_PLAN_PROVENANCE]-[QUEUED]: the host-free query-plan provenance slice that survived the dropped `[LINEAGE]` — column-level `lineage_edges` derived from the optimized relational plan as a projection off the query owner, stamped on the wire for the runtime receipt to federate, never a Python lineage/version store.
- Capability: a `QueryReceipt.lineage_edges` provenance projection on the `tabular/query#QUERY` owner — column-level input->output edges resolved from the `sqlglot`/substrait optimized plan (the `SqlGate` qualify/optimize pass and the `PlanWire` Substrait blob), surfaced as a typed receipt fact, never re-minting the W3C-PROV causal DAG, tamper-evident ledger, or durable federation C# Rasm.Persistence owns.
- Shape: a `lineage_edges` field/projection on the existing `query.md#QUERY` `QueryReceipt`, derived from the already-landed `QUERY_IR_AND_SQLGATE` plan stages; zero new data owner, zero durable Python provenance store.
- Unlocks: column-level lineage over federated/portable query plans contributed to the runtime receipt sink; reproducible study-input provenance chains; the data-plane provenance leg of the tri-language lineage concert without a second durable store.
- Anchors: `tabular/query#QUERY` `QueryReceipt`/`SqlGate`/`PlanWire` (the `QUERY_IR_AND_SQLGATE` plan stages), `sqlglot` lineage over the qualified plan, the existing `tabular/query -> runtime/observability [RECEIPT]` edge.
- Tension: stays a host-free projection — never a lineage/version.py DAG owner, never a durable Python ledger (the dropped `[LINEAGE]` proved that duplicates Rasm.Persistence Version/Provenance); the durable ledger and federation remain C# Persistence consumed at the wire.
...
[LINEAGE]-[DROPPED]: a Python lineage/version.py plus provenance.py duplicates the Rasm.Persistence Version/Provenance owner (W3C-PROV causal DAG, tamper-evident ledger, op-log changefeed) that architecture law names the sole content-hash identity/federation owner per runtime consumed at the wire; the host-free slice survives as the now-OPEN `[QUERY_PLAN_PROVENANCE]` card (a `QueryReceipt.lineage_edges` projection contributed to the runtime receipt, not a durable owner) and icechunk `set_virtual_ref` relocates to gridded/virtual.md.
...
[MESH_PAYLOAD_CODEC]-[DROPPED]: a separate mesh-payload codec card is pure duplication — data already owns the mesh codec on `spatial/mesh.md#MESH` (`MeshPayload` over `rhino3dm`/`trimesh`/`meshio`, the `xdmf` FE rail and `.3dm` row landed by `[MESH_DATA_PRESERVE]`), so the geometry-side shed is pure deletion with no data-side counterpart; no card is authored and no ripple is opened, recorded only so the codec is never re-proposed as a parallel owner.
