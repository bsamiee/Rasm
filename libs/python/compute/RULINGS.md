...
- Results stamp `attributes` on the weave span at mint and the hub `Graduation` alone reaches the journal — a sink beside them is a second truth.
...
- `Provider` seats at `solvers/solve#SOLVE` — a floor `Solve` indistinguishable from a gated one on every slot is what the column forecloses.
...
- Gated import fences bracket the ENGINE MINT alone — a whole-fold `try` re-routes an interior `ImportError` onto a floor `Solve` reading converged.
