...
- N identical lookups in one flow are one `Request.TaggedClass` family plus one `RequestResolver.makeBatched` resolver settling every request; a `getMany` twin, a hand `Map` of in-flight promises as a dedup cache, and a resolver rebuilt per call site (re-minting identity, defeating the batch window) are findings.
