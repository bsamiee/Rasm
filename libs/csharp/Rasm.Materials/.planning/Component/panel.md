...
using Rasm.Element;                  // MaterialId (the appearance/substance rows), ProfileRef + SectionProperties (the seam handle + neutral receipt the deck rib section lifts onto)
...
public sealed partial class PanelKind {
    ...
    public static readonly PanelKind PlywoodSheathing = new("plywood-sheathing", ifcEntity: "IfcPlate",    ifcPredefinedType: "SHEET",      structural: false, substanceId: "wood.plywood");
    public static readonly PanelKind OsbSheathing     = new("osb-sheathing",     ifcEntity: "IfcPlate",    ifcPredefinedType: "SHEET",      structural: false, substanceId: "wood.osb");
    ...
    public static readonly PanelKind MembraneEpdm     = new("membrane-epdm",     ifcEntity: "IfcCovering", ifcPredefinedType: "ROOFING",    structural: false, substanceId: "membrane.epdm");
    public static readonly PanelKind MembranePvc      = new("membrane-pvc",      ifcEntity: "IfcCovering", ifcPredefinedType: "ROOFING",    structural: false, substanceId: "membrane.pvc");
    public static readonly PanelKind MembraneTpo      = new("membrane-tpo",      ifcEntity: "IfcCovering", ifcPredefinedType: "ROOFING",    structural: false, substanceId: "membrane.tpo");
...
public readonly record struct PanelSection(
    ...
    // The board's substance material the design seam reads by key. Deck panels use their gauge-specific cold-formed steel;
    // other panels use the product-form substance carried by PanelKind.
    public MaterialId SubstanceId => Rib.Map(static r => r.Gauge.Substance).IfNone(Kind.Substance);

    // The FINISH the appearance projection reads — the board face material (a paper-faced/glass-mat board, a galvanized deck,
    // a foil-faced foam), INDEPENDENT from SubstanceId: a coated/printed board keeps its substance SubstanceId while its
...
public static class ComponentCatalogue {
    ...
    static readonly Seq<PanelRow> PanelRows = Seq(
        ...
        // --- membrane sheet goods — roof/wall/floor remains a layout role; panel is the product form.
        new PanelRow("panel.epdm-060-roll", "membrane-epdm", 3048.0, 30480.0, 1.52, "lapped-seam", "unidirectional", "seam-adhesive", 304.8, 152.4, 0.0, "none", "none", "none", "none", "none", "", "", "", 0.0),
        new PanelRow("panel.pvc-060-roll",  "membrane-pvc",  3048.0, 30480.0, 1.52, "lapped-seam", "unidirectional", "heat-weld",     304.8, 152.4, 0.0, "none", "none", "none", "none", "none", "", "", "", 0.0),
        new PanelRow("panel.tpo-060-roll",  "membrane-tpo",  3048.0, 30480.0, 1.52, "lapped-seam", "unidirectional", "heat-weld",     304.8, 152.4, 0.0, "none", "none", "none", "none", "none", "", "", "", 0.0));
    ...
    static Seq<PanelLayer> LayupOf(PanelKind kind, Facer facer, PositiveMagnitude thickness) =>
        ...
        : kind == PanelKind.MembraneEpdm || kind == PanelKind.MembranePvc || kind == PanelKind.MembraneTpo
            ? Seq(new PanelLayer(PanelLayerRole.MembraneCore, thickness, kind.Substance))
    ...
    // MaterialId slots (SubstanceId the substance Mechanical/Thermal row, AppearanceId the face render row) resolve off the
    ...
    public static FrozenDictionary<ComponentId, Component> BuildPanelRows(Context context) =>
        Shapes
            .Choose(shape => Component.Of(ComponentFamily.Panel, shape.Id.Value, ComponentSection.Panel(shape.Section), Coring.None, shape.Standard, shape.Section.SubstanceId, shape.Section.AppearanceId, default)
