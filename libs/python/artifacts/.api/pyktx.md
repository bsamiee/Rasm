...
- capability: KTX2 and KTX1 create, read, and write; per-level, per-layer, and per-face image placement; Basis UASTC and ETC1S encode; Zstd supercompression (ZLIB reads on ingest, no deflate member writes it); ASTC encode across 24 block dimensions; Basis transcode to twenty-one target formats; key-value metadata; and the offset and size geometry of every image in the payload
...
- BLOCK ENCODE IS 8-BIT-INPUT-ONLY (measured, both legs): `compress_basis` raises `KtxError(INVALID_OPERATION)` and `compress_astc` raises `KtxError(UNSUPPORTED_FEATURE)` on a u16/f16/f32 store, and `KtxPackAstcEncoderMode.HDR` is accepted-and-inert at 8-bit input — the enum member proves a parameter, never a code path. A float/deep plane ships as uncompressed deep KTX2; ASTC composes only at LDR.
