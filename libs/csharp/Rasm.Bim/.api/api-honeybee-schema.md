...
|  [03]   | energy library — construction / material / schedule / program families | energy | the `IConstruction` set: `OpaqueConstruction`(+`Abridged`), `WindowConstruction`(+`Abridged`/`Dynamic`/`Shade` + their `Abridged`), `AirBoundaryConstruction`(+`Abridged`), `ShadeConstruction`; the `IConstructionset` set: `ConstructionSet`(+`Abridged`) with `Wall`/`Floor`/`RoofCeiling`/`Aperture`/`Door` specializations + the baked `GlobalConstructionSet` default; the `IMaterial` set: `EnergyMaterial`/`EnergyMaterialNoMass`/`EnergyMaterialVegetation` + `EnergyWindowMaterialGlazing`/`Gas`/`GasCustom`/`GasMixture`/`Blind`/`Shade`/`SimpleGlazSys`; the `ISchedule` set: `ScheduleRuleset`(+`Abridged`)/`ScheduleFixedInterval`(+`Abridged`) + the standalone `ScheduleTypeLimit`; the `IProgramtype` set: `ProgramType`(+`Abridged`) bundling the per-program load assignment. The full form inlines, the `*Abridged` form references by identifier, and both implement the same `HoneybeeSchema.Energy.I*` discriminant — this is the shared catalog Dragonfly's `ModelEnergyProperties` references by id and never re-declares |
...
|  [05]   | HVAC template families (`IHvac`) | energy  | `IdealAirSystemAbridged` (the `IIdealair` baseline system) plus the `AllAirBase`/`DOASBase`/`HeatCoolBase`/`EquipmentBase` template hierarchy and the concrete templates keyed by the `*EquipmentType` enums — `VAV`/`PVAV`/`PSZ`/`PTAC`/`FCU`/`VRF`/`WSHP`/`Radiant`/`Baseboard`/`ForcedAirFurnace`/`GasUnitHeater`/`WindowAC`/`Residential`/… each with a `withDOAS` variant — plus the freeform `DetailedHVAC`; all implement `HoneybeeSchema.Energy.IHvac` |
...
|  [05]   | `Helper.EnergyLibrary.StandardEnergyLibrary` | (static) -> `ModelEnergyProperties`                               | the bundled standards energy library (constructions/materials/schedules/program-types) keyed by `Vintages`/`ClimateZones`; `BuildingVintages` lists the available vintages, the `*Folder` statics locate the on-disk standards resources |
...
- identity seam: a `Model.ToJson()` string (UTF-8 bytes) feeds `System.IO.Hashing` `XxHash3` (`api-hashing`) for the energy-model snapshot content key, the same content-identity rail as the IFC/glTF exports.
