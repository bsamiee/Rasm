        ...
        match phase:
                ...
                rows = tuple(
                    {
                        "item": item.GlobalId,
                        "name": item.Name or "",
                        "value": str(getattr(value, "AppliedValue", "") or ""),
                    }
                    for item in model.by_type("IfcCostItem")
            ...
            case LifecyclePhase.PATCH:
                ...
                output = ifcpatch.execute(
                    {"input": "", "file": model, "recipe": recipe, "arguments": decode(arguments.encode(), type=list) if arguments else []}
                )
            ...
            case LifecyclePhase.DIFF:
                ...
                differ.diff()
                rows = tuple(
                    {"element": guid, "change": str(change)}
                    for guid, change in differ.change_register.items()
                )
