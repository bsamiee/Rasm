...
// `Realizing` row would be a parallel discriminant re-describing the field (the MODAL_ARITY rejected form) AND could
// contradict it (SubKind=Realizing with Realizing=None, or SubKind=Path with Realizing=Some). The medium (path/port)
...
public abstract partial class Relationship {
 ...
 // would hash a Generic edge's measure attributes EXACT and fork the edge key off two edges whose measures differ
 ...
 // so it cannot carry an allocating per-case reconstruction (and would build all six arms eagerly), the func-form
