...
public sealed class ExtractPoints : Component<ExtractPoints.Spec> {
    public sealed class Spec : IComponentSpec {
        ...
        public static Seq<IOutputGroup> Outputs { get; } = Seq<IOutputGroup>(
            ...
            ShapeOutput.Query(input: Geometry, port: Port.List<Point3d>(name: "Bounds Center", code: "BC", info: "Axis-aligned bounding box center for bounded geometry and primitives."), operation: static (_, _) => Query.Bounds<object, Point3d>(aspect: new Bounds.Center())),
            ShapeOutput.Query(input: Geometry, port: Port.List<Point3d>(name: "Vertices", code: "V", info: "Native vertices, point-cloud locations, polyline corners, curve endpoints, points, or bbox corners."), operation: static (_, _) => Query.Vertices<object, Point3d>()),
            ShapeOutput.Query(input: Geometry, port: Port.List<Point3d>(name: "Control Points", code: "CP", info: "NURBS control polygon points for curves and surfaces, converting through RhinoCommon when required."), operation: static (_, _) => Query.Locate<object, Point3d>(aspect: new Location.ControlPoints())),
            ShapeOutput.Query(input: Geometry, port: Port.List<Point3d>(name: "Bounding Corners", code: "B", info: "Unique axis-aligned bounding-box corners: 8 for full 3D, 4 for planar, 2 for linear, 1 for point."), operation: static (_, _) => Query.BoundingCorners<object, Point3d>()),
