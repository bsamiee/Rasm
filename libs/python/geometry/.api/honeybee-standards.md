...
- version: `2.0.7`; floor `honeybee-standards; python_version<'3.15'`
- license: GPL-3.0 (the package metadata declares the GPLv3 classifier; the Ladybug Tools house license is AGPL-3.0, and the `License` field is otherwise unset)
...
- marker: COMPANION-GATED. Manifest-pinned `python_version<'3.15'` (co-released with the Honeybee set), gated OUT of the active 3.15 interpreter; `assay api resolve honeybee-standards` returns `unsupported`. This catalog is verified against the real `2.0.7` wheel reflected on a sibling cp312 interpreter; the gate is pure manifest co-release (data-only wheel, no ABI cap) and lifts on 3.15 validation.
...
- capability: ships the baseline default energy library (`energy_default.json`: 1 construction set, 20 constructions, 16 materials, 2 program types, 8 schedules, 9 schedule type limits) and default radiance library (`radiance_default.json`: 19 modifiers, 4 modifier sets), plus empty `user_library` extension templates; provides the two filesystem paths the honeybee config layer reads to locate them
