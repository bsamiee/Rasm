...
- Growth: a new tessellation knob is one field folded into the `IdentityPolicy` and the `geom.settings()` bind; a new CAD source is one `SourceFormat` row routing the `cad#BRIDGE` hop; the OCCT iteration kernel (the 200-400 ms cold start plus the per-element iterate/serialize loop) hands across the runtime `execution/lanes#LANES` `LanePolicy` CPU-offload variant (`anyio.to_interpreter.run_sync` with `to_process.run_sync` fallback under one `CapacityLimiter`) as ONE hand-off call once that branch variant lands — the daemon stays responsive on the `anyio` event loop while the kernel runs, the lane never imports the kernel, and the offloaded `_run` callable is picklable on the `to_process` fallback; zero new surface.
