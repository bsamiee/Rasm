...
`lbt-recipes` holds the executor boundary of the recipe pipeline: queenbee owns the consumed schema and `pollination-handlers` the bound IO coercion, leaving lbt-recipes the subprocess run and its luigi/error-log evidence; the runtime composes `Recipe` + `RecipeSettings` + `Recipe.run` into the simulation-job owner.
...
- runtime results: `Recipe.run` rides the process-resource lane + `anyio` deadline scope; the luigi/error logs parse into the runtime `RecipeRun`; a transient engine-precheck retries through the `stamina` owner; the run is an OpenTelemetry span.
