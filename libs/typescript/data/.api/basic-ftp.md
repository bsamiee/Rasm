...
- `ssh2` / `webdav` (`libs/typescript/.api/ssh2.md`, `.api/webdav.md`): sibling remote-origin rows on the one origin-addressed `object/file` surface; capability flags dispatch — FTP has no server-side copy and no locks, so copy degrades to download + upload and coordination stays on the origin that owns it.
