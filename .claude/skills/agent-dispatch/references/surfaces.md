...
A workflow is a script the runtime executes outside the conversation: intermediates live in script variables, the session stays free, and only the final report lands in context. Caps: 16 concurrent agents, 1,000 per run, no mid-run user input, no direct filesystem or shell access from the script — agents do the work. Workflow subagents always run `acceptEdits` and inherit the session's tool allowlist regardless of permission mode, so long runs pre-approve the shell commands their agents need. Progress lives in `/workflows` (pause, stop, restart, per-agent tokens); the `/config` Dynamic workflow size guideline — `small` under 5 agents, `medium` under 15, `large` under 50, `unrestricted` by default — reaches the model as advice a prompt can override, beside the hard 16-concurrent and 1,000-per-run caps. Saved runs become commands under `.claude/workflows/` (nearest directory wins in a monorepo, project beats personal) and read invocation input as a structured `args` global. Runs resume within the launching session from cached agent results; cross-session recovery is the workflow-creator skill's recovery reference. The `ultracode` keyword opts one prompt in; `/effort ultracode` makes the model plan workflows for every substantive task.
