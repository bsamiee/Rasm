...
// --- [TYPES] ------------------------------------------------------------------------------ The
// definition-set SCOPE a catalogue loads: the buildingSMART standard sets alone, the COBie handover superset, or
...
// Schema pins the version a scope's dataset actually ships — LoadIFC4COBie and LoadIFC4AndCOBie carry IFC4 definitions
...
// --- [SERVICES] --------------------------------------------------------------------------- The
// offline standard-Pset/Qto template catalogue: Xbim.Properties Definitions<T> loaded once per IFC schema and
...
public static class PropertyCatalog {
    ...
    static (Definitions<PropertySetDef> Psets, Definitions<QtoSetDef> Qtos) For(ReleaseVersion schema, TemplateScope scope) =>
        Catalogues.GetOrAdd((scope.Schema.IfNone(() => Lower(schema)), scope), static key => {
    ...
    internal static Option<templatetype> TemplateTypeOf(string setName, TemplateScope scope) =>
        For(ReleaseVersion.Ifc4X3, scope).Psets[setName] is { } set && set.templatetype is var t and not templatetype.NOTDEFINED
            ? Some(t) : None;
...
// --- [OPERATIONS] ------------------------------------------------------------------------- The
// InheritanceMode classifier [H1]: the projector stamps each seam PropertySet/QuantitySet node with its precedence
...
public static class QuantityDerivation {
    ...
    // (the offline Xbim.Properties Qto catalogue under the caller's TemplateScope) supplies the class's Qto set name, its MethodOfMeasurement basis, and
