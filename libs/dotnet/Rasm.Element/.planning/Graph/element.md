...
- Entry: `Federate(sources, coordination, key)` takes `Seq<(string Source, ElementGraph Graph)>` — the source tag being the caller's own model label, never a seam-minted id — one coordination `Header`, and the kernel `Op`, returning `Fin<(ElementGraph Graph, FederationCensus Census)>`.
