...
- `VividOrange.ISerialization`: `ILoad: ITaxonomySerializable` keys the VividOrange serializer, so a load round-trips through the one taxonomy-serialization path the whole structural taxonomy shares
...
- `LanguageExt.Core`: a foreign load quantity enters through the shared `MeasureValue` gate; malformed value evidence remains `ElementFault.ValueRejected`, while provider throws stay exact exceptional errors through `Op.Catch`, with no `.ToError()` remint

[LOCAL_ADMISSION]:
- `Model/eurocode#EUROCODE_ALGEBRA` mints one carrier per `IfcStructuralLoad` subtype and lowers the neutral component rows beside it; the group topology rides the contract's own edges, so the typed load carries the action quantity alone
...
- `PointForce`/`LineForce`/… concrete classes are mutable settable-property carriers; they live inside the fold that mints them and the projection that leaves is the neutral row set, never a settable carrier crossing a contract signature
