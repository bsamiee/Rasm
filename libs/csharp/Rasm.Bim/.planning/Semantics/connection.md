...
// Connect(ConnectKind.Realizing) edge EdgeProjection authors; this page owns only the detail. The IFC Rasm_ConnectionRealization
...
public static class ConnectionProjection {
    ...
    // the Projection/semantic#IFC_EGRESS mapping, never as the in-graph set name a seam node carries.
    ...
    // Connect(ConnectKind.Realizing) edge, never re-derived here). Head of the first co-realizing mechanical fastener.
    ...
    // writes the twelve schema statics; PropertyName stays an OPEN key per Properties/property#DETAIL_SCHEMA, so an
