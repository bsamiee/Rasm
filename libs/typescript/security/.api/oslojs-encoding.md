# [@oslojs/encoding] — the total-decode hex / base32 / base64 / base64url codec matrix for token and digest material; admitted in sign only
...
- version: `1.1.0` — direct-dep hoisted binding (pnpm catalog pin); `assay api resolve` ambiguously reports a nested `0.4.1` (arctic's transitive copy, lacking the `base64url`/`IgnorePadding` rows), verified-local direct-dep wins
- license: `MIT`
...
| [INDEX] | [ALPHABET]         | [ENCODE ROWS]                                                                                          | [DECODE ROWS]                                                            | [CONSUMER / BOUNDARY]                                        |
| :-----: | :----------------- | :---------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------- | :---------------------------------------------------------- |
...
| [INDEX] | [SYMBOL]                                | [REPLACED_BY]                        | [CONSUMER / BOUNDARY]                     |
| :-----: | :-------------------------------------- | :----------------------------------- | :---------------------------------------- |
|  [01]   | `encodeBase32` / `encodeBase32NoPadding` | `encodeBase32UpperCase` / `…NoPadding` | avoid — case-ambiguous legacy alias       |
...
- Stack with `@simplewebauthn/browser` (`.api/simplewebauthn-browser.md`): the browser package ships its own `base64URLStringToBuffer`/`bufferToBase64URLString` for the `runtime:browser` half; this package is the `sign`-side base64url for the server-verified attestation/credential bytes at rest — same alphabet, opposite runtime, no cross-import.
