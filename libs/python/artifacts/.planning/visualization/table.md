...
- Packages: `great-tables` (the `GT` builder and its full `tab_*`/`cols_*`/`fmt_*`/`sub_*`/`text_*`/`opt_*`/`data_color`/`vals.fmt_*` surface plus the `loc.*` selectors, `style.*` constructors, and `nanoplot_options`/`define_units`/`from_column`/`google_font` helpers), `polars` (the `DataFrame.style -> GT` boundary, the `Reshape` frame ops, `pl.from_dataframe` interchange ingress, `polars.selectors` for the `Cols` dtype-class selector, `pl.Config` render scope, `hash_rows` and `Expr.meta.serialize` the identity seeds, `PolarsError` the `_FAULTS` base), runtime (`ContentIdentity`/`ContentKey`, `RuntimeResult`/`boundary`, `LanePolicy`/`Kernel`/`KernelTrait`).
...
class TablePlan(Struct, frozen=True):
    ...
    async def _emit(self, key: ContentKey, /) -> RuntimeResult[bytes]:
        match (await self.lane.offload(Kernel.of(partial(self._caught, key), KernelTrait.RELEASING))).bind(lambda inner: inner):
    ...
    def _caught(self, key: ContentKey, /) -> RuntimeResult[bytes]:
        return boundary(
