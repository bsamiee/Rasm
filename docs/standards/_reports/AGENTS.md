...
Scope: `docs/standards/_reports/**`. Parent standards instructions remain in force for trusted owner routes; this folder stores source material only and is excluded from the active standards corpus unless a task explicitly names it.

## [1][SCOPE]

`_reports/` stores reusable agent outputs for this folder: source scans, repo scans, code-quality findings, navigation maps, bug/error/mistake/oversight reports, standards research, critique passes, synthesis notes, correction records, promotion maps, and candidate wording.
...
When active standards work does not name this folder, treat `_reports/` as excluded source material. Do not include it in active-corpus counts, standards audits, root-file audits, routine link sweeps, generated-contract reads, or validation claims.
