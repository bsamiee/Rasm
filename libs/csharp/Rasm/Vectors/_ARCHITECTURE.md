...
- Consumer binding: `Rasm.Compute` mirrors each `OperatorRow` as one `Tensor/vocabulary#OPERATION_TABLE` `TensorOpFamily` row under `TensorOpKind.Geometry` and one `Tensor/dispatch#DDG_ADJOINT` `GeometryAdjoint`/`Backward.Operator` reverse-mode apply whose body calls this surface's `Adjoint` over the `MeshAdjointSnapshot` the `GeometryTape` recorded; the `Solver/optimizer#OPTIMIZER_LANE` `DesignProblem.OperatorRows` registry lowers `DesignVariable.Density`/`Continuous` (the shape/topology design fields) to the `cotangent-laplacian`/`gradient` rows so `DescendAdjoint` reads a real, non-degenerate gradient. A non-self-adjoint operator carries no forward-mode JVP until its forward Jacobian map grounds — `AdjointMode.Forward` on an incidence row faults rather than fabricating a gradient.
