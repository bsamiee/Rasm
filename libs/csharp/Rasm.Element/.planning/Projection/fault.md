...
`AdmissionSlots` seats the seam's ONE accumulating admission-slot algebra HERE for the same reason the band registry sits here: every slot mints `ElementFault.ValueRejected`, and `Projection` is the S0 substrate every value, vocabulary, and graph stratum already consumes, so one owner reaches every value-admission gate on the seam with no upward edge.
...
- [03]-[ADMISSION_SLOTS]: the `AdmissionSlots` K-kinded accumulating slot algebra — the `Gate` boolean slot, the `Accumulate` fold, the `Indexed` band scan, and the `OptionalPositive` scalar slot every seam value-admission composes.
...
- Owner: `AdmissionSlots` the seam-wide accumulating admission-slot algebra — four K-kinded combinators (`Gate` the boolean slot, `Accumulate` the fold, `Indexed` the numeric-vector scan, `OptionalPositive` the `Option<double>` scalar slot) every value-admission factory across the seam composes, each minting `ElementFault.ValueRejected` under the caller's `Op` and the producing page's own `<kind:value>` `Detail` literal. Consuming pages take `using static Rasm.Element.Projection.AdmissionSlots;`, so a call site reads `Gate(…)`/`Accumulate(…)` unqualified.
- Entry: `Gate(holds, key, detail)` lifts one boolean invariant into a slot; `Accumulate(slots)` folds a slot run into one slot, the `Apply` join unioning every fault through `Error.Combine`/`ManyErrors`; `Indexed(values, holds, key, name)` scans a numeric vector under one predicate and retains every offending element as `<name:index=…:value=…>`; `OptionalPositive(value, name, key)` passes an absent `Option<double>` through and rails a present non-finite or non-positive magnitude as `<name-non-positive:value>`. A closed-field product joins its slots through the tuple `.Apply` and an open run through `Accumulate`, and `.As().ToFin()` collapses ONCE at the seam return, so the public rail stays `Fin<T>` while a malformed lowering reports EVERY violated invariant in one failure.
- Auto: the slot return is the K-KINDED `K<Validation<Error>,_>`, never the concrete `Validation` — the shipped tuple `.Apply` binds only on `(K<F,A>, …)` receivers, and a concrete-carrier tuple neither infers (invariant `ValueTuple` forces exact element inference across distinct generic definitions) nor converts (a tuple conversion is not a legal extension-receiver conversion) — so the K return makes the tuple and the fold state match by identity and `.As()` re-anchors after each join; every ternary target-types the lift (the value implicit on the success arm, the bare `ElementFault` case on the fail arm, the upcast to K riding behind both), the spelled-out `Success`/`Fail` cast being the deleted ceremony; `Indexed` composes `Gate` per element so `Map`'s element type is fixed by declaration, where a lambda whose arms are the concrete carrier infers `Seq<Validation<Error,Unit>>` that `Seq`'s invariance never converts to the `Seq<K<Validation<Error>,Unit>>` the fold takes.
...
- Packages: LanguageExt.Core (`Validation<Error,_>`/`K<F,A>`/`Seq`/`Option`/`Fin` + the `Apply` tuple join, the `Fold` state thread, and the implicit `Error` lift the bare case rides), `Rasm` (the kernel `Op` op-key each slot stamps).
- Growth: a new invariant is one slot at its call site, never a per-constructor guard chain; a new slot SHAPE lands here the moment a SECOND page needs it, and stays on its own page while one page owns it — a slot whose payload is one page's domain (a `MeasureValue` column, a direction cosine, a CRS leg) composes these rather than joining them.
- Boundary: `AdmissionSlots` is the ONE accumulating-admission owner on the seam — a per-page `Gate`/`Accumulate` copy is the named defect (every copy forks the accumulation law the moment one is edited, and the twin two owners on one page mint the moment the second needs it), and so is an early-abort guard chain or a `Bind` fold that reports only the first miss where the applicative fold reports all; the slots carry NO fault vocabulary of their own — the case is always `ValueRejected` under the caller's `Op` and the producing page's `Detail` literal, so a slot minting a second arm or a bare `Error.New` bypassing the typed factory is the deleted form; a DEPENDENT check binds INSIDE its own slot (the leaf-then-composite `COMPOSITE_ADMISSION` order), never as a sibling slot reading another's result.

```csharp signature
// --- [OPERATIONS] ---------------------------------------------------------------------------
public static class AdmissionSlots {
    // One boolean slot: holds -> unit, else the NAMED ValueRejected. Both arms target-type the K-kinded lift, which is
    // the compile contract every tuple and fold join depends on.
    public static K<Validation<Error>, Unit> Gate(bool holds, Op key, string detail) =>
        holds ? unit : ElementFault.ValueRejected(key, detail);

    // Independence rides ACROSS slots: the Apply unions every fault, so one malformed lowering reports every violated
    // invariant at once rather than first-fault-wins.
    public static K<Validation<Error>, Unit> Accumulate(Seq<K<Validation<Error>, Unit>> slots) =>
        slots.Fold((K<Validation<Error>, Unit>)Success<Error, Unit>(unit),
            static (state, slot) => (state, slot).Apply(static (_, _) => unit).As());

    // One predicate over a numeric vector, every offending element retained with its index and value, so a corrupted
    // spectrum, band set, or matrix yields one complete diagnostic instead of its first bad element.
    public static K<Validation<Error>, Unit> Indexed(ReadOnlyMemory<double> values, Func<double, bool> holds, Op key, string name) =>
        Accumulate(toSeq(values.ToArray()).Map((value, index) =>
            Gate(holds(value), key, $"<{name}:index={index}:value={value:R}>")));
    ...
    public static K<Validation<Error>, Option<double>> OptionalPositive(Option<double> value, string name, Op key) =>
        value is { IsSome: true, Case: double scalar } && (!double.IsFinite(scalar) || scalar <= 0.0)
            ? ElementFault.ValueRejected(key, $"<{name}-non-positive:{scalar:R}>")
            : value;
}
```
