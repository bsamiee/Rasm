...
`Scene.geometry` is the unique decoded-geometry map and `Scene.graph.nodes_geometry` the geometry-bearing placement list. The GLB loader marks primitives from one multi-primitive glTF mesh with `geometry.metadata["from_gltf_primitive"]` and seats them beneath their source node; collapsing those children to the parent counts source mesh instances while summing `len(mesh.faces)` across the unique geometry map counts pooled triangles once. `Scene.dump()` and `to_mesh()` flatten placements and therefore cannot mint this wire census. Do not set `merge_primitives=True` on an OCCT GLB: trimesh 5's merge arm requires every primitive kwargs row to carry `visual`, while `RWGltf_CafWriter` emits valid material-less primitives, so that arm raises `KeyError` instead of loading the model.
