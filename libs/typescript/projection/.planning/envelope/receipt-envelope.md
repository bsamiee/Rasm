...
One cluster: `[2]-[RECEIPT_ENVELOPE]` owns the `ReceiptEnvelopeCarrier` payload-bound `Schema` factory.

## [2]-[RECEIPT_ENVELOPE]

- Owner: `ReceiptEnvelopeCarrier`, the generic `Schema.Struct` factory that resolves the package marker, the HLC-stamp shape, the tenant context, the correlation key, and the bound payload against `csharp:Rasm.AppHost/ports/runtime-ports#TS_PROJECTION`.
