...
[PROFILE_SIGNAL_OTLP]-[COMPLETE]: settled as law, per the `[FLEET_TELEMETRY_SCALE_ROWS]` precedent — `ARCHITECTURE.md` `[08]-[OBSERVABILITY_CONFORMANCE]` `[PROFILE_SWAP]` owns the whole migration: the row-replacement shape, the survive-the-swap invariant, the four swap-point owners, and the arming condition (the profiles signal stable across the three SDK trains). All four owners re-proven on disk with their push rows live, and the arm re-proven unfired across all three trains on the live registries — python resolves the profiles proto at `v1development` only with no SDK profiles module and no profile exporter, the js registry publishes no profiles provider or exporter package, and the C# feed carries none at any prerelease beside the Pyroscope vendor bridge. Executing the swap when the observable fires is a coordinate flip at the tabled owners, so no open card re-states the law's own trigger.
