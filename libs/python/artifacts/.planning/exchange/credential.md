...
class Provenance:
    ...
    async def close(self) -> RuntimeRail[tuple[ContentKey, CredentialEvidence]]:
        # `async_boundary` rails the c2pa raise into `RuntimeRail[tuple[bytes, CredentialEvidence]]`;
        # `ContentIdentity.of` is ITSELF a `RuntimeRail`, so the produced bytes thread through `bind`
        # to mint the key — the two rails collapse to one rather than a nested `RuntimeRail[RuntimeRail]`.
        produced = await async_boundary(f"credential.{self.tag}", self._emit)
        return produced.bind(lambda pair: ContentIdentity.of(f"credential.{self.tag}", pair[0]).map(lambda key: (key, pair[1])))
    ...
    async def _emit(self) -> tuple[bytes, CredentialEvidence]:
        return await to_thread.run_sync(self._run)
