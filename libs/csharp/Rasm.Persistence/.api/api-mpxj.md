...
- this is the schedule-FILE peer of the other Persistence interchange codecs: it sits in `[WIRE_SERIALIZATION]` beside `MessagePack`/`Sep` and complements the row-oriented (`Sep`/`Sylvan`) and columnar (Arrow/Parquet) lanes — those cannot parse a binary MPP or a P6 XER, MPXJ owns exactly that schedule-tool format space
