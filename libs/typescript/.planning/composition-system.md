...
|  [02]   | `work`           | `SqlClient`, `@effect/sql` `MessageStorage`, `@effect/cluster`     | `store` driver Layer                                      |
...
|  [02]   | new entry endpoint family                | `edge/api` group / `edge/cli` verb            | one contribution family, app assembles         |
...
|  [07]   | new data-spine capability                | the owning spine surface                      | one row per `dataflow-system.md` `[07]`        |
...
[NEW_DOMAIN]:
- Change: A seeming new domain
- Owner Surface: The standing owner
- Shape of Edit: cache/rate → `store/capability` prepared rows; search → `store/retrieve`; queue-as-data → `store`, execution → `work`; routing → `browser/route`; config/flags → `host`; agent → `ai`; notifications/email → `work/deliver`; geometry → never (C#/Python own it)
