...
- [1]-[LAKEHOUSE]: the transactional table-format lakehouse owner over one `LakeOp` operation axis crossed with one `TableFormat` provider axis.
...
class Lakehouse(Struct, frozen=True):
    ...
    def _apply_delta(self, op: LakeOp, data: pa.Table | None) -> LakeReceipt:
        match op:
            ...
            case LakeOp(tag="optimize", optimize=(target_size, zorder)):
                table = DeltaTable(self.table_uri)
                _ = table.optimize.z_order(list(zorder), target_size=target_size) if zorder else table.optimize.compact(target_size=target_size)
    ...
    def _lance_receipt(self, operation: str, *, version: int = 0, snapshot: int = 0) -> LakeReceipt:
        ...
        return LakeReceipt(
            table_uri=self.table_uri,
            table_format=TableFormat.LANCE,
            operation=operation,
            version=ver,
            files_added=snapshot,
            files_removed=0,
