...
public sealed class ExtractMesh : Component {
    ...
    private static readonly IOutputGroup Validity = Output.Query(input: Geometry, port: Port.Tree<bool>(name: "Validity", code: "V", info: "[IsValid, IsClosed, IsOriented, IsSolid, IsManifold, IsBoundaryFree] per mesh; all six derive from one Mesh.IsManifold pass plus native validity properties."), aspect: Meshes.ValidityBundle);
    ...
    private static readonly IOutputGroup PrincipalAxes = Output.Details<(double Moment, Vector3d Axis)>(
        ...
        slots: [
            Output.Plain<(double Moment, Vector3d Axis), double>(port: Port.Tree<double>(name: "Principal Moments", code: "PM", info: "Three volume-mass principal moments of inertia (X, Y, Z). Empty when the mesh is not solid."), project: static value => value.Moment),
            Output.Plain<(double Moment, Vector3d Axis), Vector3d>(port: Port.Tree<Vector3d>(name: "Principal Axes", code: "PA", info: "Three world-coordinate principal axes aligned with the moments; surfaces intrinsic geometric orientation."), project: static value => value.Axis),
        ]);
    private static readonly IOutputGroup Quality = Output.Details<MeshFaceSample>(
        ...
        slots: [
            Output.Plain<MeshFaceSample, int>(port: Port.Tree<int>(name: "Face Index", code: "FI", info: "Face index for each metric sample.", kind: PortKind.Integer, policy: PortPolicy.Index()), project: static value => value.Face),
            Output.Plain<MeshFaceSample, double>(port: Port.Tree<double>(name: "Face Quality", code: "Q", info: "Metric value for each face."), project: static value => value.Value),
        ]);
    private static readonly IOutputGroup IndexedFace = Output.Details<MeshFaceProjection>(
        ...
        aspectLabel: nameof(Meshes),
        slots: [
            Output.One<MeshFaceProjection, Mesh>(port: Port.Tree<Mesh>(name: "Indexed Face", code: "IF", info: "Mesh containing only the selected face."), project: static (value, _) => value.Isolated()),
            Output.Many<MeshFaceProjection, Point3d>(port: Port.Tree<Point3d>(name: "Face Vertices", code: "FV", info: "Vertex positions of the selected face."), project: static value => value.Vertices),
            Output.One<MeshFaceProjection, Point3d>(port: Port.Tree<Point3d>(name: "Face Center", code: "FC", info: "Centroid of the selected face."), project: static (value, _) => value.Center),
            Output.One<MeshFaceProjection, Vector3d>(port: Port.Tree<Vector3d>(name: "Face Normal", code: "FN", info: "Unit normal of the selected face."), project: static (value, _) => value.Normal),
            Output.Plain<MeshFaceProjection, int>(port: Port.Tree<int>(name: "Source Face", code: "S", info: "Source mesh face index.", kind: PortKind.Integer, policy: PortPolicy.Index()), project: static value => value.Source.Index),
        ]);
