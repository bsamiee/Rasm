...
- [02]-[POINTCLOUD]: the LAS/LAZ/COPC point-cloud interchange row over `laspy`, the uncompressed/compressed COPC octree-subset split, the columnar point-record bridge over the shared same-module `_column` builder, and the remote-COPC `guarded(RetryClass.HTTP, anyio.to_thread.run_sync, ...)` resilience envelope.
