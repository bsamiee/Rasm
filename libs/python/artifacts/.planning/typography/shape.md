...
def _gated_bidi(text: str, base_direction: str | None) -> bytes:
    ...
    # The UAX#9 logical-to-visual reorder runs BEFORE shaping so a mixed Arabic/Hebrew + Latin run
    # reaches the SHAPE arm in visual order; `base_dir='L'|'R'` overrides the auto-computed paragraph
    # direction. python-bidi is gated python_version<'3.15' (pyo3-ffi 0.27 caps the ABI at CPython 3.14),
    # so the import lives only on the gated `to_process.run_sync` worker, never on the cp315 core.
    visual = get_display(text, base_dir=base_direction) if base_direction is not None else get_display(text)
