...
[INTERCHANGE_WIRE_DECODE_PEERS]-[QUEUED]: The TypeScript interchange package and the Python companion DECODE the C#-minted MaterialWire (OpenPBR vector, now through the Thinktecture generated codec) and MtlxDocument and never re-mint the OpenPBR algebra, the ConductorMetal rows, or the MaterialX schema — a peer re-mint is the named cross-language drift defect the interchange page declares.
- Capability: The host-free peer decoders mirroring the MaterialWire/OpenPbrGroupsWire/WireProvenance/WireColor record shape (WireProvenance now carrying the six capture-evidence columns FitConditionNumber/FitRank/DominantWavelengthNm/ExcitationPurity/CctKelvin/CctDuv at positional Key(6..11)) and the MtlxDocument node-graph field-for-field as pure structural decoders reading the Thinktecture-generated-codec wire shape (the System.Text.Json camelCase format with named non-finite literals in-Rhino — FitConditionNumber's +Inf arrives as the "Infinity" token — and the MessagePack companion format where bound decoded by POSITIONAL [Key(n)] compact-array index plus Lz4BlockArray decompression past the 512-byte floor under the UntrustedData depth/size caps), the scene-linear RgbLinear triple, and the Hex preview without re-deriving the ACEScg working space or the OpenPBR lowering.
