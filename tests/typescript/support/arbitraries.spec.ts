...
describe('optional field generation', () => {
    ...
    it('the record model varies optional keys and always keeps required keys', () => {
        const samples = FastCheck.sample(Arbitraries.optionalFields({ version: FastCheck.integer(), label: FastCheck.string() }, ['version']), _SEED);
        expect(Array.every(samples, (sample) => Object.hasOwn(sample, 'version'))).toBe(true);
...
describe('distinct arrays', () => {
    it('generated arrays are pairwise distinct over a two-value domain', () => {
        const samples = FastCheck.sample(
            Arbitraries.distinctArray(FastCheck.integer({ min: 0, max: 1 }), 2, (a, b) => a === b),
            _SEED,
        );
...
describe('classification coverage', () => {
    it('full-range generators cover every parity label', () => {
        expect(Arbitraries.missingClassifications(FastCheck.integer(), (value) => (value % 2 === 0 ? 'even' : 'odd'), ['even', 'odd'])).toEqual([]);
    });

    it('over-biased generators report labels they never produce', () => {
        const missing = Arbitraries.missingClassifications(FastCheck.integer({ min: 0, max: 0 }), (value) => (value % 2 === 0 ? 'even' : 'odd'), [
            'even',
            'odd',
        ]);
    ...
    it('multi-label classification records every label produced by one sample', () => {
        const missing = Arbitraries.missingClassifications(FastCheck.integer({ min: 1, max: 1 }), () => ['positive', 'small'], [
            'positive',
            'small',
            'huge',
        ]);
