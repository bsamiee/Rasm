...
[STACKING]:
- `connectrpc`(`.api/connectrpc.md`): the generated `<Svc>ASGIApplication` serves under `hypercorn.asyncio.serve` entered via `anyio.run(partial(serve, app, config, shutdown_trigger=stop.wait), backend='asyncio')` — the one backend its server and client primitives run on — every blocking handler body bounded through `to_thread.run_sync(..., limiter=...)`; `RequestContext.timeout_ms` maps onto `fail_after`/`move_on_after`, sync entrypoints bridge through a `BlockingPortalProvider`.
