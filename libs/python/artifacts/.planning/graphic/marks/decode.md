...
async def decode(source: DecodeSource, scope: DecodeScope | None = None, /) -> RuntimeRail[RasterFact]:
    # the ONE public read entry — decoded payloads are DATA, never artifacts (no receipt, no ArtifactWork).
    # a Raster source decodes untrusted bytes crash-isolated on the process lane; a trusted Pixels frame
    # shares the address space on the thread lane with no pickle — both runtime-owned, zero folder mints.
