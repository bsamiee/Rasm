...
- `fastavro` — Schemaless codec over the exact generated CloudEvents AVSC resource; parsed once by the instance-bound Avro event-format row.
- `jsonschema` — Payload gate over separately resolved schema documents; CloudEvents `dataschema` carries only the absolute schema URI.
- `python-dateutil` — ISO-8601 superset reader every CloudEvents binding decodes its `time` attribute through, raising untyped past the binding.
