# [CSHARP_TESTING_API_POLLY_TESTING]
...
[STACKING]:
- `Polly.Core` (`libs/csharp/Rasm.AppHost/.api/api-polly-core.md`): the descriptor reads what `ResiliencePipelineBuilder` assembled, and the option types it surfaces are that catalogue's strategy-options family.
- `Polly.Extensions` (`libs/csharp/Rasm.AppHost/.api/api-polly-extensions.md`): a registry-resolved pipeline inspects identically, so a keyed pipeline pulled from `ResiliencePipelineProvider<TKey>` is the natural subject; the telemetry strategy `ConfigureTelemetry` inserts at the head appears in the roster like any other.
