...
- asset: pure-JS runtime library shipping NO `.d.ts` (no `types` field, no bundled declarations); `@types/three` (DefinitelyTyped) is NOT admitted, so `assay api query` over the TSDECL source yields EMPTY. Members below are verified against the shipped `build/three.cjs` (441 core exports), `build/three.webgpu.js` (629 exports), `build/three.tsl.js` (638 exports), and the `examples/jsm/**` addon source on disk
...
- Verify three member existence against the shipped runtime exports (three ships no `.d.ts`); never trust a `three` signature from `@types/three` recall — the DefinitelyTyped package is not admitted and can drift from the shipped 0.185.1 surface.
