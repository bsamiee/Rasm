...
def _path(graph: Handle) -> AnalyticValue:
    ...
    # than re-enumerating per endpoint. `Topology.Vertices` is the polymorphic accessor documented over
    # `Topology` OR `Graph` OR `TGraph`, so the one accessor reads BOTH the graph node list and the
    # `Wire` the geodesic returns — `Graph.ShortestPath(graph, a, b)` yields a `topologic_core.Wire`, not a
    # `Graph`, so its vertex count comes from `Topology.Vertices(wire)`, never a (non-existent for a Wire)
    # `Graph.Vertices(wire)`. A disconnected graph has no such path, so `ShortestPath` returning a
    # null/empty handle folds to `Scalar(0.0)` rather than crashing `Topology.Vertices(None)` — the row
    # stays total over the disconnected case instead of converting a legitimate no-path graph into a
    # `boundary` fault the fence would otherwise raise.
    verts = Topology.Vertices(graph)
    span = Graph.ShortestPath(graph, verts[0], verts[-1]) if len(verts) > 1 else None
    return AnalyticValue.Scalar(float(len(Topology.Vertices(span))) if span is not None else 0.0)
...
_CONSTRUCT: Final[Mapping[SourceKind, Callable[[bytes], Handle]]] = MappingProxyType({
    ...
    # `Cluster` handle: `ByIFCFile` takes the file OBJECT (not a path) and returns `list[Any]`, so the
    # `Cluster.ByTopologies` fold collapses that list to one `Handle`, shape-uniform with the rows above; no
    # path round-trip — `file.from_string` is the same in-memory SPF loader `mesh/daemon.md` opens through.
    SourceKind.IFC: lambda b: Cluster.ByTopologies(*Topology.ByIFCFile(ifcopenshell.file.from_string(b.decode("utf-8")))),
})
...
GRAPH_ANALYTIC: Final[Mapping[GraphAnalytic, Reducer]] = MappingProxyType({
    GraphAnalytic.CONNECTIVITY: lambda g, _: AnalyticValue.Scalar(float(len(Graph.ConnectedComponents(g)))),
    ...
    GraphAnalytic.SPANNING: lambda g, _: AnalyticValue.Groups(tuple(tuple(Topology.Vertices(e)) for e in Graph.Edges(Graph.MinimumSpanningTree(g)))),
...
def _graph_analytics(graph: Handle, policy: TopologyPolicy) -> Map[GraphAnalytic, AnalyticValue]:
    ...
    # `Topology.Vertices` is the polymorphic accessor over a `Graph` handle (documented over `Topology`/
    # `Graph`/`TGraph`), the one node-count source shared with the path and spanning rows.
...
- [TOPOLOGIC_HANDLES]: the `Topology.Decompose` return collection, the `Topology.Faces`/`Topology.Vertices` sub-topology accessors parallel to `Cells`, the `Topology.AdjacentTopologies(item, host)` arity, the `Topology.Analyze` handle-summary return shape, the `Topology.BoundingBox`/`Centroid`/`Contains` analysis arities, the `Topology.Union`/`Difference`/`Intersect`/`Slice(a, b)` boolean-pair arities, the `Topology.AddDictionary(topology, dict)` attach arity, the `Dictionary.ByKeysValues(keys, values)` constructor, the `Topology.ByOCCTShape(shape)` payload form (whether the byte stream is the OCCT shape the constructor reads directly or requires an `OCC.Core`-decoded shape handle), and the `Aperture.ByTopologyContext(aperture, context)` face-into-host binding confirm against the folder `.api/topologicpy.md` entrypoint tables on the companion interpreter; `Topology.ByBREPString` (entrypoints [02]), `Topology.ByOCCTShape` ([06]), `Topology.ByJSONString` ([07]), `Topology.ByOBJPath` ([03]), `Topology.Cells` (analysis [01]), `Topology.Faces` ([02]), `Topology.AdjacentTopologies` ([04]), the four boolean ops ([05]-[08]), `Topology.BoundingBox`/`Centroid`/`Contains` ([09]-[11]), `Topology.AddDictionary` ([12]), `Graph.ByTopology`/`ShortestPath`/`BetweennessCentrality` (sub-topology constructors [10]-[12]), and `Dictionary.ByKeysValues`/`ValueAtKey` ([19]-[20]) are catalogue-confirmed.
- [GRAPH_ANALYTIC_SURFACE]: the `Graph` facade carries 179 methods (`.api/topologicpy.md` PUBLIC_TYPES analysis [01]); every `GRAPH_ANALYTIC` graph member is now catalogue-confirmed against the `0.9.50` `Graph.py` source (`.api/topologicpy.md` ENTRYPOINTS [10]-[18] plus the graph-analytics axis under IMPLEMENTATION_LAW), with no phantom and no band gap. The member-by-member resolution, with the three semantic corrections the source forced into the fence: (1) the component count is `len(Graph.ConnectedComponents(graph))` — `ConnectedComponents` returns the list of island sub-graphs sorted by vertex count — NOT `Graph.Connectivity`, which the source documents as an explicit ALIAS for `Graph.DegreeCentrality` returning a per-vertex space-syntax connectivity score (a `float()` over that list is a type error), so the `CONNECTIVITY` reducer binds the components length; (2) `BetweennessCentrality`/`ClosenessCentrality`/`DegreeCentrality` each return a per-vertex score `list` ordered to the vertices (the docstring "list of centralities in the order matching the vertices"), so `_leaders` ranks the list directly; (3) the spanning structure is `Graph.MinimumSpanningTree(graph)` (the true MST `Graph`), NOT `Graph.Tree`, which the source documents as a vertex-rooted BFS/DFS traversal tree — a different structure — so the `SPANNING` reducer binds the MST; (4) the graph-side vertex accessor collapses onto `Topology.Vertices`, documented as polymorphic over `topologic_core.Topology`, `topologic_core.Graph`, OR `topologicpy.TGraph`, so it reads the graph node count AND the `ShortestPath` `Wire` vertex count from one accessor and the fence needs no gated `Graph.Vertices` (there is no `Graph.Vertices` overload over a `Wire`); `Graph.Edges(graph)` returns the constituent edge handles and is confirmed; (5) `Graph.JSONData(graph)` returns the node-link `dict` `msgspec.json.encode` consumes (the docstring return type is `dict`), while `Graph.JSONString` returns the pre-serialized string — the dict-returning `JSONData` is the codec-feeding choice. `ShortestPath(graph, vertexA, vertexB, ...)` returns a `topologic_core.Wire` with all routing parameters defaulted (`edgeKey="Length"`), so the bare three-positional call is arity-correct.
- [IFC_BYTES_CONSTRUCTOR]: CONFIRMED-AND-LOCKED. `Topology.ByIFCFile(file, includeTypes=[], excludeTypes=[], dictionaryMode="basic", clean=False, epsilon=0.01, angTolerance=0.1, tolerance=0.0001, silent=False) -> list[Any]` takes an in-memory `ifcopenshell.file` OBJECT, not a path string — the source docstring reads `file : ifcopenshell.file — The input IFC file object`. `Topology.ByIFCFile` (`Topology.py:3511`) delegates to `IFC.TopologiesByFile` (`IFC.py:2414`) at `Topology.py:3553`; the two `TopologiesByFile` defs live on DIFFERENT classes — `IFC.TopologiesByFile` (`IFC.py:2414`) does NOT shadow `IFCFastTopology.TopologiesByFile` (`IFC.py:420`); both are live and `420` is the worker. `IFC.TopologiesByFile` routes only the entity-`dict` arm (`isinstance(file, dict)`, `IFC.py:2461`) to `TopologiesByEntities` and DELEGATES the in-memory `ifcopenshell.file` object straight to `IFCFastTopology.TopologiesByFile` at `IFC.py:2465`. That worker's object arm serializes the file via `file.to_string()` through `IFCFastTopology._file_to_step_text` (`IFC.py:495`->`717`, the `to_string` probe yielding `ISO-10303-21` SPF text) then re-parses it with `IFCFastTopology.ParseText` (`IFC.py:500`) into entities for `TopologiesByEntities` (`IFC.py:501`) — an IN-PROCESS STEP/SPF round-trip in memory with NO disk I/O, not a path round-trip and not a zero-serialization handoff. The separate `Topology.ByIFCPath(path: str, ...)` owns the path-string intake, so the two spellings disambiguate the catalogue's earlier bare `(file)`: `ByIFCFile` = object handle, `ByIFCPath` = path. The `SourceKind.IFC` row is locked as `Cluster.ByTopologies(*Topology.ByIFCFile(ifcopenshell.file.from_string(b.decode("utf-8"))))`: `file.from_string` opens the SPF byte stream in memory (the `mesh/daemon.md#DAEMON` pattern, `.api/ifcopenshell.md` entrypoints [17]), `ByIFCFile` returns the per-product topology `list[Any]`, and `Cluster.ByTopologies(*topologies)` folds that list into one non-manifold `Cluster` handle uniform with the BREP/OCCT/JSON/OBJ rows so `_lift`'s `Callable[[bytes], Handle]` contract holds. Resolution path (1) is taken; paths (2) the `IFC`-facade model entry and (3) the `Topology.ByOCCTShape` OCCT bridge are NOT taken since (1) is source-confirmed. The temp-file detour (writing the byte stream to disk so the path constructor reads it) stays the FORBIDDEN form: the byte stream owns the model everywhere else in the companion, and a path round-trip re-introduces filesystem I/O the daemon's in-memory open exists to avoid. The arity gate is closed; the only remaining gate is orthogonal — `topologicpy` is manifest-absent (consumed by this page and `graph/algebra.md`) and the whole owner is dark in the default build until the AGPL-accepting `python_version<'3.15'` companion-band row lands (`nonmanifold#4-DUAL_GATE_EXCLUSION`), which is the admission gate, not an arity blocker.
