...
- Growth: the airborne spectrum-adaptation deepening (the flanking `Dn,f,w`, the ISO 717-1 `C`/`Ctr` terms) is one fold over the same `LayerSet` reading the same per-band SRI, never a parallel acoustic owner; the impact `Ln,w` (ISO 717-2 / IIC) is the DESCENDING sibling that lands once this runner carries the assembly normalized-impact spectrum — the EN 12354-2 floating-floor `ΔL_w` improvement computed from the resilient layer's seam `Acoustic.DynamicStiffnessMNPerM3` (the one material leg the airborne SRI spectrum cannot encode) and rated through the deferred descending impact `RatingContour` row via the shared sign-agnostic `RatingContour.Fit` (one contour kernel admitting the row's negative feasible shifts by data, never a second descending algorithm), the seam `Composition/acoustic#ACOUSTIC_FOLDS` note carrying the counterpart row; the acceptance verdict is now real — `AssessmentRequest.Acoustic` carries a `RequiredRw` the governing ratio reads as `RequiredRw / Rw`.
