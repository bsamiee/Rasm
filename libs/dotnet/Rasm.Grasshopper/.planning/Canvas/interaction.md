...
- Packages: Grasshopper2 (`FlexControl` register/focus surface, `IResponsive`, `Responses` and its virtual family, `ResponseRotationArgs`, `CoordinateSystem`), Eto.Forms (`KeyEventArgs`, `TextInputEventArgs`), `Rasm.Interaction` (`ResponderSpec`, `PointerPhase`, `KeyPhase`, `PointerFact`, `InputVerdict`, `UiThread`, `UiDispatch<T>`, `UiClaim`), `Rasm.Domain` (`Op`, `Lease<T>`, `FaultCell`, `HookSet`), `Shell/hooks.md`, `Shell/session.md`.
