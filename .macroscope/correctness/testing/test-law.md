...
- `tests/contracts/` custody reads live from the entry's `MANIFEST.md` `Class` field, never from a language: a DOMAIN entry admits the asset from the one producer its entry names, an INFRASTRUCTURE entry admits a mint from every branch with corpus parity as the proof, and a rule restricting an infrastructure entry to one branch is itself the drift defect. A fabricated byte set for a `DESIGN-PIN` entry and a manifest entry leading its producer are findings; graduation to `REAL` lands on the producing page and flips the manifest entry in the same change.
- Manifest entries whose producer and every decoder sit inside one branch are findings — the corpus seats a contract only where a peer runtime binds it, and the golden homes at that branch's test tree (`tests/RULINGS.md` `[04]-[STRUCTURE]`); the seat opens the day a second runtime decodes the wire.
- Infrastructure entries carrying a domain payload are findings — the entry freezes the payload-agnostic law alone and each branch-owned payload instantiating it takes its own `domain` entry at its own seam (`tests/contracts/README.md` `[01]-[AUTHORITY]`).
...
- Every tool routes reports under `.artifacts/` and work state under `.cache/<tool>/` through the tool's own documented configuration, never a wrapper script; a tool writing to the repo root is a finding against that tool's configuration.
