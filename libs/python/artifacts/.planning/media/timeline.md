...
class Timeline(Struct, frozen=True):
    ...
    async def render(self) -> RuntimeRail[Result[tuple[ContentKey, ArtifactReceipt], MediaFault]]:
        return await async_boundary(f"timeline.{self.op.tag}", self._emit)
...
# same `artifacts.media.timeline` module; the workers stay module-level so `to_process.run_sync` dispatches them by
...
- [TRIM_SEEK] [RESOLVED]: frame-accurate trim is `media/container#CONTAINER` `_decode_window` — `_seek(reader, stream, in_point)` (`InputContainer.seek(int(in_point / stream.time_base), backward=True, stream=stream)` then `CodecContext.flush_buffers()`, both verified present) positions the demuxer at the keyframe at/before the in-point and resets the decoder, then the decode keeps only frames whose `VideoFrame.time` (verified `= pts · time_base`, `0.0` for frame 0) lands in `[in_point, out_point)` — the pre-roll from the keyframe to the in-point decoded and dropped so the cut is frame-exact rather than GOP-aligned. The zero-base needs no `setpts=PTS-STARTPTS` filter: `_encode_video` re-stamps `pts` from a fresh 0-based frame index in `_drive`, which IS the zero-base. Trim composes the spine's read + encode primitives, opening no container of its own.
- [CONCAT_STRATEGY] [RESOLVED]: `Concat` is ONE arm over TWO strategies DERIVED from `_params` equality, never a knob. `_packet_concat` (lossless, when every clip shares `(codec_name, width, height, pix_fmt, time_base)`) clones the first stream through `add_stream_from_template` and re-stamps each `Packet.dts`/`pts` onto a running `offset` advanced by the clip's last `pts + duration` — verified: `Packet.dts`/`pts`/`duration`/`time_base`/`stream` are all settable (`packet.dts = value` confirmed), and the demux -> re-stamp -> `mux_one` copy performs no decode, so the join is bit-exact. `_filter_concat` (when codecs differ) decodes each clip through `_decode_video` and re-encodes the joined frame tuple through `_encode_video` (a per-clip `FilterNode.Scale` composed for a resolution mismatch, since the FFmpeg concat contract requires matching geometry). The `strategy` fact makes the lossless-vs-reencode decision observable evidence.
- [SEGMENT_SINK] [RESOLVED]: `Segment` composes the `media/container#CONTAINER` `SEGMENT`-muxer segmented sink rather than a re-implemented splitter — the decoded clip re-encodes through a profile whose `container = ContainerFormat.SEGMENT` and whose `SegmentSpec.options` carries `segment_times={cuts}`, so the container's own verified `av.open(manifest, format="segment", io_open=…)` `UPath` egress writes N segments at the cut boundaries and one manifest-keyed artifact. The N-output shape stays inside the single-entry contract because the manifest bytes are the one keyed product and the segment count rides the `facts` band — exactly the container HLS/DASH segmented-sink model, reused.
- [XFADE_DISSOLVE] [RESOLVED]: `Xfade` composes `media/filtergraph#FILTER` `cross_dissolve` over the overlapping `window = round(duration · rate)` frames — the numpy linear ramp (`under[-window:]` fades out weight `1 -> 0` as `over[:window]` fades in) rather than the native `xfade` filter, which was verified to FAIL `configure()` in-process on `av 17.1.0` under both `link_nodes` and `link_to` wiring. Both clips decode through `_decode_video` and the joined stream re-encodes through `_encode_video`. The audio cross-fade leg (`media/audio#MEDIA` `_decode_audio` + a `filtergraph` `acrossfade`, verified to `configure()` via `link_to`) is the named growth axis, the video dissolve the landed arm.
- [CLIP_DAG] [RESOLVED]: a multi-clip timeline is the media plane's strongest `core/plan#PLAN` `ArtifactPipeline` exemplar — each `Clip` carries its own parent `ContentKey` (the clip producer's minted key), `Timeline.parents` projects the clip-key tuple, and the composition root wires it as the `ArtifactWork.parents` upstream set, so a five-clip concat is a five-parent dependency front the planner's `topological_generations` drives concurrently before the join and the CPM `Schedule` orders by min-slack. A warm replay elides an already-rendered clip through the runtime lane's content-key short-circuit — the plan re-mints no key. A nested edit (a clip that is itself a `Timeline.render` product) nests the DAG without a new surface, the `Clip.key` the nested render's output key.
- [FAULT_REUSE] [RESOLVED]: `TimelineFault` is NOT a new vocabulary — every arm returns the `media/container#CONTAINER` `MediaFault` closed union, and the `av.error.FFmpegError` capture lives in the composed spine primitives (`_decode_video`/`_decode_window`/`_encode_video` each own their `try`/`except FFmpegError` -> `_media_fault`), so a timeline worker adds no second capture except at its own imperative kernels (`_packet_concat`'s demux/re-stamp loop, `_concat`'s param probe). The `_WORKER_RETRY` transient-death policy is imported from the spine, never re-minted, and the `BrokenWorkerProcess`/`BeartypeCallHintViolation` seam catches sit on `Timeline._mux` exactly as the container `_mux` — one media fault rail, one retry policy, across the whole plane.
