...
`Rasm` owns the RhinoCommon- and Eto-aware geometry, numeric, and interaction kernel below the C# app strata: the exact-arithmetic floor and the operational geometry plane above it, one body. Its robust floor is necessary, never sufficient: the operational plane composes it into the parametric forms, toolpaths, GPU residency payloads, and persistence naming every upper stratum consumes. Where no admissible package carries the robustness guarantee the kernel authors from first principles; where the ecosystem owns the concern it composes the categorical-best engine, never both.
