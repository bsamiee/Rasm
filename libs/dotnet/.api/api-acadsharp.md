...
|  [09]   | `ACadVersion`                     | enum          | drawing format version; `CadVersionPolicy` curates the write roster                 |

- [09]-[WRITE_VERSIONS]: `CadVersionPolicy` selects the write-relevant `ACadVersion` members — `AC1021` (R2007), `AC1027` (R2013), and `AC1032` (R2018).
...
- Resilient read: `CadReaderConfiguration.Failsafe` defaults `true`, so recoverable corruption routes to `NotificationEventArgs(NotificationType, Message, Exception)` and the read completes; a hard reader throw enters `Try.lift` once at the consuming boundary and retains its exception evidence.
