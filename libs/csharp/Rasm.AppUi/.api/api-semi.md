...
- `SemiPopupAnimations` exposes NO named duration resource — its durations are inline `0:0:0.1` literals (open `CubicEaseIn`, close `CubicEaseOut`, ScaleX/Y 0.98 to 1.0) and the style set mounts separately in `Application.Styles`, so a motion-token-to-Semi-duration binding is unspellable; popup and flyout timing rides the package's own motion-plan rows, and overriding a Semi popup duration means replacing the whole style.
- Semi re-emission targets the semantic `SemiColor<Role>` brush layer only — the raw `Semi<Hue><N>Color` scale binds into the brushes via `{StaticResource}` at dictionary load, so a runtime scale override re-tints nothing; value-named dimension keys (`SemiSpacing*`/`SemiThickness*`) name their own value and are never re-seeded.
