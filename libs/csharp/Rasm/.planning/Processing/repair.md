...
// the Standard order in one owner — Mint rows in declaration order DERIVE Heal.Standard; boolean
// is payload-bearing (None) and sits outside it.
[SmartEnum<string>]
...
public readonly record struct RepairPolicy(
    ...
    public bool IsValid => ValidityClaim.All(
        ...
        Intersect.IsValid, Remesh.IsValid, Arrangement.IsValid);
...
public abstract partial record HealOp {
    ...
    internal Fin<HealStep> Apply(MeshEdit edit, MeshSpace current, RepairPolicy policy, Op key) =>
        Switch(
            state: (Edit: edit, Current: current, Policy: policy, Key: key),
...
public static class Heal {
            ...
            return Status(plan.Input, context, op).Bind(first =>
                plan.Ops.Fold(
                    ...
                    (acc, heal) => acc.Bind(state =>
                        from step in heal.Apply(live, state.Space, plan.Policy, op)
    ...
    static Fin<HealStep> Retile(MeshEdit edit, CrossLattice lattice, RepairPolicy policy, Op key) {
        ...
        foreach ((int a, int b, int fa, int fb) in lattice.Segments.Concat(lattice.Coplanar)) {
    ...
    static Fin<Unit> Subdivide(MeshEdit edit, int face, List<(int A, int B, int FaceA, int FaceB)> segments, Point3d[] mark, Dictionary<Point3d, int> minted, Dictionary<(int, int, int), Point3d> triple, RepairPolicy policy, Op key) {
        ...
        return Crossed(segments, mark, triple, axis, policy, key).Bind(rows => {
            ...
            return Tessellation.Build(new TessellationOp.Points(TessellationKind.Triangulation, vertices, boundary.Concat(interior), policy.Remesh, axis), key)
                ...
                .Bind(triangles => Splice(edit, face, triangles, slot, (a, b, c), minted, mirrored, key));
    ...
    // — refuse before touching the arena, never mis-lift.
    static Fin<Unit> Splice(MeshEdit edit, int face, (Point3d A, Point3d B, Point3d C)[] triangles, Dictionary<Point3d, int> slot, (int A, int B, int C) corners, Dictionary<Point3d, int> minted, bool mirrored, Op key) {
        foreach ((Point3d ta, Point3d tb, Point3d tc) in triangles) {
            if (!slot.ContainsKey(ta) || !slot.ContainsKey(tb) || !slot.ContainsKey(tc)) return Fin.Fail<Unit>(key.InvalidResult());
        }
        edit.KillFace(face);
        foreach ((Point3d ta, Point3d tb, Point3d tc) in triangles) {
    ...
    internal static Fin<HealStep> Merge(HealOp.Boolean op, MeshSpace current, RepairPolicy policy, Op key) =>
        Arrangement.Apply(new ArrangementOp.MeshBoolean(current, op.Tool, op.Op, policy.Arrangement), key)
            .Bind(result => result is ArrangementResult.Boolean merged
                ? Fin.Succ(new HealStep(MeshEdit.Of(merged.Solid, policy.Arena), Some(merged.Receipt)))
...
|  [4b]   | Policy row       | `RepairPolicy` | `readonly record struct` + `IValidityEvidence` — 3 admitted scalars + 4 composed sibling policies, `Of` admission | `RepairPolicy.Of → Fin<RepairPolicy>`     |    —    |
