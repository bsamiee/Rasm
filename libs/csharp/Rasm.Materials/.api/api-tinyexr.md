...
- `MathNet.Numerics`(`libs/csharp/.api/api-mathnet-numerics.md`): `Fourier.Forward2D(Complex[], rows, columns, FourierOptions)` consumes the interleaved float plane `PartConversion.ToInterleavedFloat` produces, so frequency-domain height integration and periodicity scoring read an EXR level directly.
