...
| :-----: | :---------------- | :------------------------------------------ | :-------------------------------------------------------- |
|  [01]   | content-key name  | kernel `XxHash128` over the bytes           | shared with blob placement + retention; never a second id |
|  [02]   | kind taxonomy     | composed `Version/retention` `ArtifactKind` | one axis; the store catalog reads it one stratum below    |
|  [03]   | placement owner   | `Store/blobstore#OBJECT_STORE`              | the index references by content key; never the bytes      |
...
public readonly record struct ModelResultRow(UInt128 Content, ContentAddress Placement, string Fingerprint, Instant At);
...
|  [03]   | dedup key         | shared `ContentAddress.Of` (`XxHash128`) | one address for inference keys and solve sub-blocks; no second hasher |
|  [04]   | read consistency  | synchronous `Query/lane` lane            | a stale dedup is a correctness fault; never async                     |
|  [05]   | placement-only    | `Publish` records the row, not bytes     | the object store owns the payload; no 2-arg phantom                   |
...
|  [03]   | federation      | L2 prefix dispatch                  | zero cross-package dependency; composition binds the L2  |
...
public sealed partial class IndexResidency {
    ...
    public static readonly IndexResidency ScyllaWideColumn = new("scylla-widecolumn",
        "no cross-partition predicate, no ad-hoc filter, no server-side fold: a read names (tenant, kind) then narrows on the clustering prefix, and every other question re-enters through the Marten substrate the backend projects");
...
| [INDEX] | [POLICY]          | [VALUE]                                         | [BINDING]                                                    |
| :-----: | :---------------- | :---------------------------------------------- | :----------------------------------------------------------- |
|  [01]   | residency         | `IndexResidency` deployment row                 | a projection backend; not a second SoR/horizon               |
...
|  [08]   | consistency claim | the bound row's DECLARED level                  | the POCO mapper discards `ExecutionInfo`; no `Any` default   |
|  [09]   | fault fold        | `CacheFault.Lift` at ONE boundary               | inside the attempt; no driver exception crosses the boundary |
...
|  [13]   | re-drive boundary | kernel `Retriability` → `StoreVerdict.Of`       | `Store/redrive#REDRIVE_BOUNDARY`; root binds the executor    |
