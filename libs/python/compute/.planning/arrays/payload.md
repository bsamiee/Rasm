...
- Packages: `array-api-compat` (`array_namespace`, `is_*_array`, `device`, `to_device`), `array-api-extra` (`at`, `atleast_nd` namespace-portable helpers), `numpy` (`asarray`, `from_dlpack`, the Array API floor), data-branch `xarray`/`dask` labelled-array shapes, runtime (`RuntimeRail`, `BoundaryFault`, `ContentIdentity`/`ContentKey`/`IdentityPolicy`, `Receipt`/`ReceiptContributor`).
