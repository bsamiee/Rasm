...
- [24]-[CONNECTION](.planning/Semantics/connection.md): host-neutral `ConnectionDetail` realizing-element joint `[Union]` (`Bolted`/`Welded`/`Bearing`/`Cast`) over `IfcRelConnectsWithRealizingElements`, and the `csharp:Rasm.Materials/Connection` `ConnectionItem` axis IFC 4.3 wire (`ConnectionWire.Author` → `IfcReinforcingBar`/`IfcMechanicalFastener` + `IfcRelConnectsWithRealizingElements`) — the connection-item egress only; the material/composition/property/classification egress reads the projected seam `Material` subgraph via `Semantics/composition`/`classification` composed by `Projection/semantic` `Emit`, never a Materials wire.
