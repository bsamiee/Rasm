...
// capture accumulates every projector's faults, then the one Fin failure stops the bake. [SkipUnionOps] skips the implicit-conversion ops (every case carries an explicit Op) and emits NO
// per-case factory: the nested `…Case` record carries the data, the same-name-less static factory returns the base
