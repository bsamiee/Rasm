...
[PACKAGE_SURFACE]: `Magick.NET-Q16-HDRI-AnyCPU`
- package: `Magick.NET-Q16-HDRI-AnyCPU` (Apache-2.0)
- assembly: `Magick.NET-Q16-HDRI-AnyCPU` + `Magick.NET.Core`; native `Magick.Native-Q16-HDRI-arm64.dll.dylib` per RID
- namespace: `ImageMagick`
- rail: breadth-container ingest
...
- Owns: ingest decode for the breadth containers the managed estate cannot reach — AVIF, HEIF/HEIC, JPEG XL, DPX, Cineon — behind the one codec breadth row.
- Accept: span sniff through `MagickFormatInfo.Create`; decode through `new MagickImage(bytes)`; the float drain through `GetPixels().GetReadOnlyArea` at the image's own channel arity, quantum-normalized at the seam.
- Reject: any encode through this surface; a second composing site outside the codec breadth row; a container an admitted managed engine already decodes routed here; a format capability asserted from the package description rather than the measured delegate roster; a four-lane drain assumption over the image's declared arity.
