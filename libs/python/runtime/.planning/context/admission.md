...
- Cases: `RuntimeProfile` rows `TOOL` · `SIDECAR` · `PACKAGE` · `TEST`, each a row in the frozen policy table carrying eager-import, scratch-writable, OTel-emit, and lane-capacity columns.
- Entry: `RuntimeContext.admit` receives caller-owned host facts and returns the frozen record; `RuntimeContext.policy` reads the per-profile row; the owner never reads the environment and never resolves the host.
- Packages: `msgspec` (`Struct`/`field`), `expression` (`Option`/`Nothing`/`Some`).
...
PROFILE_POLICY: Final[frozendict[RuntimeProfile, ProfilePolicy]] = frozendict({
