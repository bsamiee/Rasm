...
public sealed partial class ContentAddress {
 ...
 // The H7 verification dual of the mint: re-derive a node's stored id from its current content and confirm the two
 // still agree — the tamper/corruption gate a Rasm.Persistence rehydrate and the cross-runtime parity corpus run
 // before trusting a persisted id. The dual splits by the SAME identity regime the Graph/element#NODE_MODEL mint
 // took, so each arm re-runs the EXACT mint that produced the stored id: an OCCURRENCE Object carries a random
 // Guid-v7 placement identity (NOT content-derived, H6) with no preimage to recompute, so it verifies vacuously; a
 // TYPE Object carries a DETERMINISTIC content-derived id (NodeId.RootedType over the Representations-EXCLUDED
 // Node.Object.ToTypeSeedBytes seed), so the dual re-mints NodeId.RootedType(o.ToTypeSeedBytes(tolerance).Span) and
 // compares it to the stored id — the integrity check that catches a forged or corrupted Type id a re-export or a
 // tamper introduced (the volatile Representations the seed omits keep a later geometry attach from spuriously
 // failing it); a NON-ROOTED node carries a content-self-hash id, so the dual re-projects node.ToCanonicalBytes and
 // compares. The tolerance MUST be the graph's mint-time Header.Tolerance for the content-derived arms (Type and
 // non-rooted), or the quantized re-projection drifts; an Occurrence ignores it (its id is placement, not content).
 // Each content arm routes through THIS owner's own entry — the non-rooted through Of(ReadOnlySpan<byte>) (NOT a
 // direct kernel ContentHash.Of bypass — the "ContentAddress is the ONE content-key owner" discipline holds even in
 // Verify) formatting the recomputed UInt128 X32 exactly as the NodeId.Content mint does, the Type through the seam's
 // own NodeId.RootedType so the recomputed and stored NodeId compare as the mint produced them (X32 owned once in
 // RootedType, never re-spelled here). The single producer on a mismatch is Projection/fault#FAULT_BAND
 // ElementFault.AddressUnstable.
 public static Fin<Unit> Verify(Node node, double tolerance, Op key) =>
  node switch {
   ...
   Node.Object o => NodeId.RootedType(o.ToTypeSeedBytes(tolerance).Span).Value == o.Id.Value
    ? Fin.Succ(unit)
    : ElementFault.AddressUnstable(key, $"<type-id-mismatch:{o.Id.Value}>"),
   _ => Of(node.ToCanonicalBytes(tolerance).Span).Value.ToString("X32", CultureInfo.InvariantCulture) == node.Id.Value
    ? Fin.Succ(unit)
    : ElementFault.AddressUnstable(key, $"<content-id-mismatch:{node.Id.Value}>")
  };

 // The snapshot rehydrate gate: every content-derived node's stored id re-verified through the single-node Verify
 // (each non-rooted node AND each Type Object, the Occurrence Objects passing vacuously), the Validation ACCUMULATING
 // all mismatches (a corrupt snapshot reports every drifted node at once — the Projection/projection#GRAPH_CONSTRAINT
...
- [VERIFICATION_DUAL]: `Verify` is the H7 re-derive dual the mint owes — re-running the SAME mint by the node's identity regime and comparing the recomputed id to the stored one detects a tampered or corrupted persisted node, the gate a `Rasm.Persistence` rehydrate runs before trusting an id and the assertion the cross-runtime parity corpus makes. The regime split is load-bearing: a non-rooted node's stored `NodeId` IS the hash of its canonical content, so the dual re-projects `node.ToCanonicalBytes` through this owner's `Of` entry and compares the recomputed `X32`; a TYPE `Object`'s id IS `NodeId.RootedType` over its `Representations`-excluded `ToTypeSeedBytes` seed (the deterministic-rooted mint), so the dual re-mints `NodeId.RootedType(o.ToTypeSeedBytes(tolerance).Span)` and compares — the integrity check catching a forged or corrupted Type id (an id no longer matching its own type-defining content), the `Representations` the seed omits keeping a later geometry attach from spuriously failing it; an OCCURRENCE `Object` carries a random Guid-v7 placement id (NOT content-derived, H6) with no preimage, so it ALONE verifies vacuously — a single vacuous arm over ALL `Node.Object` (treating a deterministic Type id like an uncheckable Occurrence Guid) is the deleted under-check, the Type arm the integrity dual `RootedType`'s determinism newly admits. The graph overload accumulates every drifted node applicatively (the `Validation<Error,Unit>` accumulate-all shape, independent node checks licensing accumulation while the single-node check is fail-fast `Fin`), so a corrupt snapshot reports all mismatches at once; the ONLY rail is `Projection/fault#FAULT_BAND` `ElementFault.AddressUnstable`, the verification mismatch its sole producer — the prior "a non-finite measure rails `AddressUnstable`" reading is the deleted form, because the writer NORMALIZES non-finite and the upstream `MeasureValue.Of`/`PropertyValue.Of` admissions reject a non-finite at ingress, so a drifted value surfaces as a re-derive mismatch, never an un-producible byte-projection fault.
