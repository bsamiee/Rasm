...
|  [46]   | IFC semantic-ingest model-graph residence   | cache-indexes#ARTIFACT_BLOB_INDEX + data-lanes#DOCUMENT_LANE |
...
|  [34]   | SharpFuzz                                              | snapshot-codecs     | pending                    | tests-only        |
...
Testing-infrastructure horizon: the snapshot-codec decode and sealed-artifact rejection ladder ride `SharpFuzz` `Fuzzer.OutOfProcess.Run(Action<Stream>)` over the restore lane's untrusted-data boundary — the `SnapshotCodecFuzz` harness on snapshot-codecs#FUZZ_HARNESS is the design fence and the dedicated `tests/csharp/_fuzz` project is the implementation-session transcription deliverable; the `StoreProfile.SqliteMemory` placement is the deterministic in-memory store row and the DuckDB analytical lane the live-engine round-trip; bulk-write lane throughput lands on the BenchmarkDotNet rail.
