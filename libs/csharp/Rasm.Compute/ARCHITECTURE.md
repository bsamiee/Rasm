...
Analysis          ←  csharp:Rasm.Element/Graph             # [READ]: ElementGraph concrete above the seam (no IElementProjection); SectionOf/MechanicalOf/AxisOf/CompositionOf/Material/SpacesOf accessors
Analysis          →  csharp:Rasm.Element/Assessment        # [WRITE]: Node.Assessment GraphDelta content-keyed on (input subgraph, route) via AssignsToAssessment
...
Analysis/structural ←  csharp:Rasm.Materials/Profiles      # [CONTENT_KEY]: M7 ProfileRef→SectionProperties resolved once (VividOrange) and baked onto the seam graph; Compute reads, never re-resolves, admits no VividOrange
...
Analysis/energy   →  csharp:Rasm.Persistence               # [CONTENT_KEY]: EnergyPlus SqlFile results keyed on (input subgraph, route); 412-noop on the object store
Analysis          →  csharp:Rasm.Persistence/Version/retention # [CONTENT_KEY]: content-keyed Assessment.Result blob registered in the RETENTION_CLASSES blob class (content-keyed, full-history-reachable, GC-protected); 412-noop dedup
...
| [03] | Symbolic/device/learning/constitutive faults extend the `ComputeFault` 2200 band at the next-free code (the Symbolic lane owns 2213..2216 and the analysis lane 2217..2219, so a new fault enters at 2220+), never a parallel fault union or a status-code-plus-string terminal — every fault crosses the wire through the one `FaultDetail` family. | `ComputeFault` (`Runtime/admission`) | `Runtime/admission`, `Runtime/channels` |
