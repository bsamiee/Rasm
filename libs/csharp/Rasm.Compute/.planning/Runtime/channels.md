...
- [03]-[FAULT_PROJECTION]: one `FaultDetail` family plus a `StatusCode`→`WireFault` rail total over the seventeen-code taxonomy (a fifteen-row `FrozenDictionary` plus the `Unreachable` fallthrough for `OK`/`Unknown`), an in-band transaction-conflict decode arm, and a conflict receipt pack.
...
- [UPSTREAM-BLOCKED]: the companion-decoded reality-capture splat scan riding the ArtifactSync `ArtifactFrame` byte seam as a standalone artifact (NEVER a `GeometryPayload` oneof case — the oneof carries point_cloud/mesh/voxel only); the packed float buffers carry the companion-published position/scale/rotation/spherical-harmonic accessor order, reassembled via `FrameEdge.Reassemble<GaussianSplatScan>` under the whole-artifact `XxHash128` identity gate and admitted to the Compute `Rasm.Compute/Runtime/payload#RESIDENCY` `SplatScan`. The seam shape is the design record until the Python `realitycapture` companion grounds the SPZ/SOG decode wire and publishes the scan-buffer frame.
...
public sealed record ParseGuard(int SizeLimitBytes, int RecursionLimit, ExtensionRegistry Extensions) {
    ...
    public Fin<T> Read<T>(MessageParser<T> parser, ReadOnlySequence<byte> payload) where T : IBufferMessage, IMessage<T> =>
        Try.lift(() => parser.ParseFrom(CodedInputStream.CreateWithLimits(new MemoryStream(payload.ToArray(), writable: false), SizeLimitBytes, RecursionLimit))).Run()
...
- [COMPOSED_CREDENTIAL]: the live-ALC dial of the composed `CredentialPolicy` identity through a running plugin channel resolves against the running plugin process; the `CallSpine` interceptor surface, the `WireFault` `StatusCode` rail, the `ContractGuard` projection-checksum fold, the `WireDocument`/`FrameEdge.Transaction` parity choreography, and the benchmark-gated compression flip — `CompressionProviders.Winning` over the `wire-compression` `BenchmarkClaim` family fingerprint-matched to the running host, stamped by `CallSpine.Compressed` — are authored in the cluster fences.
...
- [SPLAT_SCAN_WIRE]: the `GaussianSplatScan` ArtifactSync artifact message — the companion-published gaussian-splat scan-buffer layout (packed position/scale/rotation/spherical-harmonic float buffers, the harmonic-degree band, the per-splat count) reassembled through `FrameEdge.Reassemble<GaussianSplatScan>` and admitted to the Compute `Rasm.Compute/Runtime/payload#RESIDENCY` `SplatScan` — stays [UPSTREAM-BLOCKED] until the Python `realitycapture` companion grounds the SPZ/SOG decode wire and publishes the scan-buffer frame; the message rides the generic `ArtifactFrame` bytes (never a `GeometryPayload` oneof case), so the gaussian-splat residency encode at `Rasm.Compute/Runtime/payload#RESIDENCY` is fully implemented over whatever `SplatScan` it is handed and the only open leaf is the companion publishing the frame — the same [UPSTREAM-BLOCKED] leaf `csharp:Rasm.AppUi/Render/reality#SPLAT_SOURCE` carries.
