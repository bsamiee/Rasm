...
- Per-row admission attempts every row into `Validation<Error>`; live chrome unwinds in reverse and appends cleanup through `Error.Many`.
- `Notice.Fan` recursively emits one document-visible row per `Error.Many` leaf, never an opaque count.
