...
- `api-h3`(`libs/dotnet/.api/api-h3.md`): `H3Index.FromPoint(Point, int)` computes the same 64-bit cell the server computes, so `GridPathCells`/`GridDistance` in-process and `pgr_dijkstra` in-database agree on node identity.
