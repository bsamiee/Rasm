...
[STACKING]:
- View bases stack with `ReactiveUI`: a `ReactiveUserControl<TVm>` / `ReactiveWindow<TVm>` pairs with a `ReactiveObject` view-model whose validation rows ride its own `INotifyDataErrorInfo` implementation, and the code-behind body is `this.WhenActivated(d => { this.Bind(...).DisposeWith(d); this.BindCommand(...).DisposeWith(d); })` — the `WhenActivated` block is fed by `AvaloniaActivationForViewFetcher` and every disposable is tied to the view's `Loaded`/`Unloaded` lifetime.
...
- `GetSubject<T>` stacks `AvaloniaProperty` with `DynamicData` and `System.Reactive` as an `ISubject<T>` source/sink in a change-set pipeline or `ReactiveCommand` `canExecute` stream, with `BindingPriority` controlling precedence against styles and animations.
