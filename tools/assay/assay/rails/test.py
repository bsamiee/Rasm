...
if TYPE_CHECKING:
    ...
    from assay.core.model import AnyDetail
...
def _checks(routed: Routed, params: TestParams, settings: AssaySettings, mode: Mode) -> tuple[Check, ...]:
    ...
    def _cs_shape(check: Check) -> Check:
        ...
        project = next((part for part in (*(check.tail or ()), deferred) if part.endswith(".csproj")), "")
        if not project:
            return check
        ...
        mtp = ("--minimum-expected-tests", "1") if tool.mode is Mode.RUN else ("--list-tests",)
        return msgspec.structs.replace(check, tail=(*tool.input_flag, project, *coverage, "--", *mtp, *check.args.filter, *trx, *routes))

    selected = tuple(_check(t, args) for t in _rows(routed.language, params, mode) for args in (_args(t),) if args is not None)
    return tuple(_cs_shape(check) for check in expand(selected, routed, settings=settings))
