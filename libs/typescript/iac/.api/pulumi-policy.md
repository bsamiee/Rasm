...
- `PolicyPack` runs as the analyzer plugin and surfaces violations through `ReportViolation`; mandatory violations reject the lifecycle operation with no in-band policy result.

[STACKING]:
- `@pulumi/pulumi`(`.api/pulumi-pulumi.md`): the pack rides `UpOptions.policyPacks` into `LocalWorkspace` `preview` and `up`; `Drift.sweep` publishes the native `PreviewResult` returned by `Stack.previewRefresh()`.
