export const meta = {
  ...
  phases: [
    ...
    { title: 'Rebuild', detail: 'per NON-FROZEN page (1 agent/file): rebuild(max) -> critique(xhigh, 6-checklist + capability-completeness) -> redteam(max, counterfactual + cold re-review), every stage ADVERSARIAL and every stage first LISTS then FULLY reads the entire docs/stacks/python/ doctrine in README routing order, pooled at CAP=8 (the frozen folders are NOT per-file rebuilt)' },
    { title: 'Final-Collapse', detail: 'FOLDER-COHERENT BATCHES (BATCH pages/agent) over the WHOLE scope INCLUDING the frozen folders, each batch a collapse(max) -> critique(xhigh) -> redteam(max) sequence pooled at CAP=8: collapse + unify across files into one rail/polymorphism/shape system, break illusory differentiation, reduce chaff without removing capability; cross-batch concerns surface as residuals' },
  ],
}
