...
[STRUCT_SEED]:
- Law: the seed is a `msgspec.Struct(frozen=True, gc=False)` when three or more named running quantities travel together; `gc=False` drops the per-step allocation from the tracked set, which is load-bearing because the fold re-allocates the immutable seed once per element. A two-value seed stays a bare `tuple`; the named struct earns its keyword fields only when the running values are evidence a reader names.
