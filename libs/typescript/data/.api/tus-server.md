...
|  [06]   | `.onUploadCreate` / `.onUploadFinish`                        | hook           | admission point; the finalize fold and the landed reply |
...
|  [15]   | `EVENTS`                                                     | events         | EventEmitter lifecycle taps beside the hook points      |
...
`handle(IncomingMessage, ServerResponse) -> Promise<void>` serves node, `handleWeb(Request) -> Promise<Response>` serves any fetch-shaped runtime, and `cleanUpExpiredUploads() -> Promise<number>` sweeps expired staging; the hooks anchor the ingest folds, so tus internals stay invisible past this boundary.

| [INDEX] | [SURFACE]                                                  | [SHAPE]  | [CAPABILITY]                                               |
| :-----: | :--------------------------------------------------------- | :------- | :--------------------------------------------------------- |
|  [01]   | `new Server({ datastore, ...options })`                    | ctor     | one staging-band server; hooks close over the ingest folds |
...
|  [06]   | `server.on(EVENTS.POST_FINISH, (req, res, upload) => ...)` | instance | observability beside the finish hook, not the boundary     |
...
- `DataStore` is the abstract port and `@tus/s3-store` the composed implementation: the server owns protocol conformance, the store owns bytes, and the ingest pipeline subclasses neither.
...
- `onResponseError` closes every fault path last, taking the thrown value and returning a replacement `{ status_code, body }` or `undefined` to keep the derived pair: this is the error-MAPPING point, and observation alone is a return of `undefined`.
...
- `effect`: each dispatch lifts through `Effect.tryPromise`, the server constructs inside `Effect.acquireRelease` at the owning service, and hook bodies run the ingest folds through the owning `ManagedRuntime` — a hook is a boundary adapter whose thrown refusal is the tus-conformant abort, so the pipeline's tagged faults project onto `ERRORS` rows inside `onResponseError` and never leak a stack into the reply body.
...
- throw a `{ status_code, body }` carrier out of a hook and map every remaining pipeline fault onto an `ERRORS` row inside `onResponseError`, so no reply falls through to the 500 `UNKNOWN_ERROR` default.
