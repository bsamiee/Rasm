# [@deck.gl/mesh-layers] — instanced 3D-mesh + glTF-scenegraph rows; scope:viewer project-local
...
- experimental knobs are underscored: `_instanced`, `_animations`, `_lighting`, `_imageBasedLightingEnvironment` are advanced — instance the plain transform accessors first.
...
- `_instanced`/`_animations`/`_lighting`/`_imageBasedLightingEnvironment` are experimental (underscore); reach for them only for delta-coordinate meshes, animated glTF, or PBR IBL.
