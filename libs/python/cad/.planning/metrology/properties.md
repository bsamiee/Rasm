...
`measured` is the provider's one measurement of an exact B-rep shape: validity admission, the dimensional census, mass and centroid at the elected dimension, surface area, and the reconciliation delta against emitted mesh evidence, folded into one `BrepMeasure`. Measurement reads a `TopoDS_Shape` and nothing else — no reader, no document, no emitted file — so every leg holding a shape reaches one owner instead of importing a sibling's modeling page for it.

Seating measurement here dissolves the strata edge the flat corpus carried, where the tessellation leg imported measurement off the B-rep page and paid a `watertight=`/`mesh_volume_m3=` argument pair the B-rep leg can never supply. `Closure` replaces that pair arm for arm with the `BrepMeasure` presence rule, so a leg states what it measured instead of threading two nullable knobs. This owner imports `faults` alone among siblings, and `brep/operation#OPERATION` and `tessellation/mesh#MESH` both compose it without either reaching the other.

## [01]-[INDEX]

- [02]-[LADDER]: Validity admission, the indexed topology census, and the dimensional precedence table electing mass and centroid.
- [03]-[MEASURE]: `Closure` over the wire's own presence rule, the reconciliation delta, and the one `BrepMeasure` mint.

## [02]-[LADDER]

- Owner: `_elected` — one election over `_LADDER`, whose row order IS the dimensional precedence and whose refusal names the rung.
...
- Owner: `measured` — the one `BrepMeasure` mint; every leg holding a shape reaches it and no leg rebuilds a field of it.
...
- Law: correspondence is no measurement — it rides `ExecuteResponse.correspondence` from `brep/provenance#CORRESPONDENCE`, so this mint takes a shape and a closure and nothing an arm's history answered.
...
- Growth: a new closure disposition is one `Closure` case beside one `_PRESENCE` row; a new measured field derives inside this mint, so no caller grows an argument and no leg learns a second field.
- Boundary: this owner builds the value; publishing it as `ExecuteResponse.measure` or `TessellateResponse.measure` belongs to `service/provider#PROVIDER`.
...
from expression import case, effect, tag, tagged_union
...
@effect.result[BrepMeasure, CadFault]()
def measured(shape: TopoDS_Shape, closure: Closure = UNMEASURED, /):
    held = yield from _admitted(shape)
    census = _topology(held)
    volume_m3, center = yield from _elected(held, census)
    area = GProp_GProps()
