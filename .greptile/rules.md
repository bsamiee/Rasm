...
- Attack fold accumulators: a seed starting completeness flags true fabricates a computed receipt over an empty input set — demand an explicit empty-set rail or false-seeded flags, with one sibling fold railing empty while its neighbours do not as the tell; a fact stream folded into a name-keyed bag silently drops all but one entry under a repeated key — set-valued findings emit as one typed list value under one name.
