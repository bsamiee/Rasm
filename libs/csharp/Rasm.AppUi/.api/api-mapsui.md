...
- reprojection at the boundary: domain coordinates arrive in their source CRS; `SphericalMercator.FromLonLat` (or the `ProjectionDefaults.Projection` seam for non-mercator) reprojects to the EPSG:3857 `Map.CRS` at the layer-build edge — the internal model is one CRS, the boundary owns the transform, mirroring the api-wide boundary-mapping law.
- snapshot into the capture rail: `MapControl.GetSnapshot(layers, RenderFormat.Png, quality)` is the Render/capture leg for the map surface — a `byte[]` image the export pipeline (PDFsharp page, OOXML embed) consumes, the geo analogue of the 3D viewport capture.
