...
- Law: resilience splits by plane — the worker's processing body composes its own `Step.run` budget, so a saturated provider re-drives inside the step while a malformed payload fails terminal and dead-letters through the divert; the lane's `retry` schedule prices only the queue's storage replay, and no per-family predicate exists on either plane.
...
[JOB_FAMILY]:
- Owner: `Job.family` — the family mint. `family({ name, payload, key, lane, success, error })` declares the `DurableQueue` (the payload schema, the `idempotencyKey` from the `key` projection, the typed settlement schemas) and assembles the caller surface: `enqueue(payload)` runs `DurableQueue.process` under the lane's storage-replay schedule and suspends the enqueuing workflow until a worker settles the item — success flows the item's typed value as `Either.right`, a terminal settlement failure (the worker's own step budget spent) diverts to the `Job.Dead` left carrying the payload, the family, the dedupe key, and its fault classification; `replay(dead)` re-enqueues straight from dead evidence, so quarantine review is a fold over values, never a table surgery.
...
- Packages: `@effect/experimental` (`PersistedQueue`), `@effect/workflow` (`DurableQueue`, `WorkflowEngine`), `effect` (`Effect`, `Either`, `Schema`, `Types`), `@rasm/ts/kernel` (`Budget`, `FaultClass`).

```typescript
import type { PersistedQueue } from "@effect/experimental"
...
declare namespace Job {
  ...
  type Family<P extends Schema.Schema.Any, A, AI, E, EI> = {
    readonly queue: DurableQueue.DurableQueue<P, Schema.Schema<A, AI>, Schema.Schema<E, EI>>
    readonly enqueue: (
      payload: Schema.Schema.Type<P>,
    ) => Effect.Effect<Either.Either<A, Dead<Schema.Schema.Type<P>>>, never, Runtime | Schema.Schema.Context<P>>
    readonly replay: (
  ...
  type Shape = Types.Simplify<
    typeof _lanes & {
      ...
      readonly family: <P extends Schema.Schema.Any, A, AI, E, EI>(spec: Spec<P, A, AI, E, EI>) => Family<P, A, AI, E, EI>
    }
  >
  type _Rows<T extends Contract = typeof _lanes> = T
  type _Keys<K extends keyof Contract = Lane> = K
}

const _family = <P extends Schema.Schema.Any, A, AI, E, EI>(
  spec: Job.Spec<P, A, AI, E, EI>,
): Job.Family<P, A, AI, E, EI> => {
