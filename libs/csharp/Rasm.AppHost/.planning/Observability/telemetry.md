...
public sealed partial class ForeignSource {
    ...
    // Polly owns its meter as a library-static singleton keyed on a wire name rather than a per-registration
    // instance, so ONE process-wide scope carries every pipeline's streams and `AddMeter` subscribes it by name in
    // any order — which is the discriminant seating this scope here rather than at [05], where a registration verb
    // constructs every instrument it mounts. Metric-only: no resilience assembly opens an ActivitySource, so this
    // projection never asks the row for a trace scope. Its key spells that meter's own name verbatim.
...
// SpineSampler owns the per-entry volume verdict a rule row cannot express: rules select on category, level, and event
// identity alone, so neither the EVENT stride's must-never-thin law nor the degradation-driven floor is
// reachable from them. Audit-grade strides return true unconditionally — an exclusion by CONSTRUCTION rather
// than by a probability that happens to round up — and the chatty floor thins against the live rank, so a
// degraded host sheds its own noise without a configuration edit.
public sealed class SpineSampler(Func<DegradationLevel> level, Func<double> draw) : LoggingSampler {
    // Audit exclusion DERIVES from the band registry rather than a literal stride: an EVENT-kind band is
    // one whose records are the evidence an incident is read from, so every one of them keeps unconditionally
    // and a new event band inherits the exclusion with no edit here.
    ...
    static bool Audited(int eventId) =>
        FaultBand.OwnerOf(eventId).Match(Some: static band => band.Kind == BandKind.Event, None: static () => false);
...
public sealed class SpineLossFold(Action<SpineLossFact> emit, Redactor redactor) : ILoggingFailureListener {
    ...
    // failures differing only in their message join would hash alike. AppendRedacted rents per part, and the
    // cap applies to the composed redacted text, so no full unredacted string is ever materialized.
...
// phases and Materials its construction phases on contexts this root mints, so their names must reach the SAME
// registration this root's own roster does — an unregistered name resolves to a POSITIONLESS token whose writes
// drop with nothing raised, so a contributor whose roster never joins reads as instrumented and reports nothing.
...
public static class LatencySpine {
    // One frozen registration per composition, folded from this root's own roster beside every contributor's —
    // the same downward-contribution shape the hook census takes, so a composition registers exactly one
    // vocabulary and no contributor reaches its own `RegisterCheckpointNames` call to split the table. All THREE
    // axes fold: a tag or measure name is as droppable as a checkpoint, and registering checkpoints alone left
    // every pivot dimension a contributor keys on unregistered. `ThrowOnUnregisteredNames` turns the drop into a
    // boot failure, so a contributor added without its roster fails loudly instead of exporting silent holes.
...
public sealed class TelemetryComposition : IDisposable {
    ...
    // Each emitting package's latency vocabulary, carried as three name sequences rather than through the port
    // column: a roster is plain text this assembly resolves without referencing the package that minted it, where
    // the port's `Planes` roster carries types it cannot. One frozen registration folds them all, so a contributor
...
public static class SignalGovernance {
    ...
    // EVENT names gate on the SAME roster, because Tier-0 binds every signal name a branch mints — instrument,
    // dimension, resource, span, log, AND event — to one capability vocabulary, and an event type whose subject
    // no board ever plots is a subscription no query can join to the metrics it is about. The grammar already
    // isolated the segment, so this leg reads `EventType.Domain` rather than re-splitting a dotted name: one
    // entry, two request shapes, and a new signal plane is an overload rather than a second gate.
...
|  [71]   | event-type naming gate         | `EventType.Domain` against the roster      | `Rostered` on the `Wire/topics#TOPIC_FABRIC` mint          |
|  [72]   | log verdict-plane binding      | audit stride + degradation floor           | `AddSampler(LoggingSampler)` over `SpineSampler`           |
|  [73]   | redaction install seam         | the derived closure, never a bare seal     | `RedactionRegistration.Bind` inside `GovernLogs`           |
|  [74]   | serilog sink set               | wire, shared file, hot, audit              | `SerilogSinks.For` behind the pipeline arbitration         |
|  [75]   | durable log file residence     | host-keyed path, shared append, daily roll | `WriteTo.File(shared: true)` on the `SerilogSinks` leg     |
|  [76]   | latency phase pivots           | one dimension per registered checkpoint    | `LatencyCheckpoint.Pivot` folded into `LatencySpine.Own`   |
|  [77]   | incident flush accounting      | one point per drained buffer scope         | `IncidentBuffers.Flush` over `BufferScope.Items`           |
|  [78]   | federated classification proof | branch values prove against the roster     | `RedactionRegistration.Federated` at composition           |
...
// strongest of its rows, so the closure needs no hand-authored composite table and no ordering convention
// beyond this column.
[SmartEnum]
...
public sealed partial class DataClassification {
    ...
    // The producer-only host-path bit (branch RULINGS: only the producer knows a payload embeds a host path);
    // Rasm.Rhino Objects/authoring.md emits it — an unrostered value reaches the fail-closed fallback and ERASES.
    public static readonly DataClassification HostPath = new("host-path", redactor: RedactorKind.Hmac);
    ...
    // Unknown declares the never-reviewed arm rather than inferring it from absence: [UnknownDataClassification] seals a
    // member onto it and the erase treatment makes that seal fail closed by law, where relying on the fallback
    // makes "nobody looked" and "no row matched" indistinguishable at review time.
    public static readonly DataClassification Unknown = new("unknown", redactor: RedactorKind.Erase);
...
// than throwing, so a bounded detail never materializes a full string only to be cut; AppendRedacted rents
...
public static class RedactedText {
    ...
    public static Fin<int> Into(Redactor redactor, Span<char> sink, string value) =>
        redactor.TryRedact(value, sink, out var written, format: default)
            ? Fin.Succ(written)
            : Fin.Fail<int>(new Fault.Capacity(Detail: $"<redaction-sink:{sink.Length}>"));
