...
- Boundary: the half order is FIXED and load-bearing — physical half first as the NodaTime `Instant` Unix-tick `long`, logical half second as the monotone `ulong`, both little-endian on the wire, byte-identical to `Rasm.Compute/Runtime/codecs#CONTENT_ADDRESSING` `InterchangeIdentity.Compose` so a content key and a causal stamp seal one frame every peer re-derives from the same two-half order; an off-by-one-half pack corrupts the whole causal order, so the python `hlc-two-half` reproduction fixture and the kernel reconciliation corpus pin the pack/unpack parity against this owner; the packed projection is `physical_ticks<<64 | logical` as one `UInt128`, the layout the python `Hlc.packed` peer holds bit-identical; the fence lives at `[PORT_RECORDS]` — this owner states the law, the port states the mechanics.
