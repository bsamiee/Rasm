...
SimpleIDML is pure-Python over `zipfile`+`lxml`, and `lxml` is manifest-gated `python_version<'3.15'`, so the WHOLE mutation fold crosses the runtime `anyio.to_process` worker seam onto the sub-3.15 interpreter — the same gated band `document/emit#EMIT` dispatches its lxml arms over — and only the serialized package bytes plus the picklable `IdmlFact` evidence cross back, never a live `IDMLPackage`/`_Element`. Each top-level `@use_working_copy` mutation EXTRACTS the package to a temp tree, mutates, REPACKAGES, overwrites the input file, closes the input, and returns a FRESH path-backed instance — so `_mutate` MUST thread the returned instance forward (a `BytesIO`-backed package crashes the decorator's `os.unlink(filename)`), bracket every spill and instance in one `contextlib.ExitStack`, and drain `Path(final.filename).read_bytes()` off the last instance. Each production is one `@receipted(Redaction.STRUCTURAL)` `_emit` returning the stepped owner the harvest weave drains, contributing the existing `core/receipt#RECEIPT` `ArtifactReceipt.Office` case (the IDML package IS an Office-class structured-document Zip archive) over the mutated-package byte count, never a new receipt case.

## [01]-[INDEX]

- [01]-[INDESIGN]: IDML template-mutation owner over a `base: IdmlSource` admitted once through the `IndesignPayload` `TypedDict`+`TypeAdapter` and a closed fourteen-case `IdmlStep` `tagged_union` fold (`Insert`/`AddPages`/`ImportXml`/`PlacePdf`/`SetAttributes`/`AddNote`/`MergeLayers`/`RemoveContent`/`SuffixLayers`/`RemoveLayer`/`RemoveOrphanLayers`/`RemoveGuides`/`AddStory`/`LeafToNode`) threaded over one running `IDMLPackage` through the verified SimpleIDML `insert_idml`/`add_pages_from_idml`/`import_xml`/`import_pdf`/`set_attributes`/`add_note`/`merge_layers`/`remove_content`/`suffix_layers`/`remove_layer`/`remove_orphan_layers`/`remove_guides_on_layer`/`add_story_with_content`/`xml_element_leaf_to_node` `@use_working_copy` algebra (the base `prefix` applied once) and drained once, crossing the runtime `anyio.to_process` worker seam because `lxml` is gated off cp315, validating each destination anchor AND each source `only` selector against the live `xml_structure` lxml tree through one `_resolved` guard while non-empty-checking each layer/story/content `identifier` apart, returning the `spreads`/`stories`/`pages`/`fonts`/`styles`/`layers`/`tags`/`nodes` structural `IdmlFact` inventory, admitted through `Idml.of` over the closed `IdmlFault` precondition vocabulary (`payload`/`empty_data`/`bad_prefix`/`empty_anchor`/`empty_ref`, the `bad_prefix` arm enforcing SimpleIDML's `\A\w+\Z` prefix constraint), produced through the one modal-arity `produced(Idml | Iterable[Idml])` entrypoint, and folding into the shared content key plus the `@receipted`-harvested `ArtifactReceipt.Office` case.
...
- Packages: `SimpleIDML` ([RESEARCH] not-yet-admitted, member chain VERIFIED against `Starou/SimpleIDML` `src/simple_idml/idml.py` — `from simple_idml import idml`/`idml.IDMLPackage(path)` the `zipfile.ZipFile` subclass forcing `ZIP_STORED`; `prefix(prefix) -> IDMLPackage` requiring `\A\w+\Z` (raising `BaseException` otherwise); `insert_idml(other, at, only) -> IDMLPackage` (`only` a required source xpath); `add_pages_from_idml([(pkg, page_number, at, only), …]) -> IDMLPackage`; `import_xml(xml, at) -> IDMLPackage` (bytes or str; content-control via the `simpleidml-setcontent`/`-ignorecontent`/`-forcecontent` source-XML attributes); `import_pdf(pdf_path, at, crop="CropContentVisibleLayers", page_number=1) -> IDMLPackage`; `set_attributes(xpath, items, element_id=None) -> IDMLPackage` (an `href` item rebinds the image resource path); `add_note(note, author, at, when=None) -> IDMLPackage`; `merge_layers(with_name=None) -> IDMLPackage`; `remove_content(under) -> IDMLPackage`; `suffix_layers(suffix) -> IDMLPackage`; `remove_layer(layer_id) -> IDMLPackage` (transitively calls `remove_guides_on_layer`); `remove_orphan_layers() -> IDMLPackage` (folds over `designmap.layer_nodes` against `referenced_layers`); `remove_guides_on_layer(layer_id) -> IDMLPackage`; `add_story_with_content(story_id, xml_element_id, xml_element_tag) -> IDMLPackage`; `xml_element_leaf_to_node(xpath, xml_content_ref) -> IDMLPackage` (a `Rectangle` is rewritten to a `TextFrame`); the sixteen-method `@use_working_copy` surface in full, the two non-step members `prefix` (base-applied) and `add_page_from_idml` (singular, batched by `add_pages_from_idml`); `spreads`/`stories`/`pages`/`font_families`/`style_groups`/`tags`/`referenced_layers`/`xml_structure` (lxml `_Element`)/`export_xml()` the introspection surface; the `@use_working_copy` decorator's extract-mutate-repackage-overwrite-return-fresh contract) on the runtime sub-3.15 worker band pending admission; `lxml` (the `xml_structure` `_Element.xpath` anchor validation, manifest-gated `python_version<'3.15'` — the dependency constraint that puts the whole fold on the worker seam, shared with `document/emit#EMIT`); `anyio` (`to_process.run_sync(limiter=)` the worker-seam crossing, `CapacityLimiter` the module-level `_GATE` gated-band bound shared with `export/layered#LAYERED`); `msgspec` (`Struct` frozen `IdmlSource`/`IdmlFact`/`Idml`); `pydantic` (`TypeAdapter(IndesignPayload)`/`ValidationError` the untrusted-`template` admission); `expression` (`tagged_union`/`tag`/`case` the `IdmlStep`/`IdmlFault` families, `Result`/`Ok`/`Error` the admission rail, `Block` the modal carrier); `frozendict` (the `SetAttributes` attribute map); stdlib `re` (the module-level `_PREFIX` `\A\w+\Z` pattern), `contextlib.ExitStack` (the worker resource bracket), `tempfile.NamedTemporaryFile(delete_on_close=False)` (the path-backed spill), `pathlib.Path` (the `as_uri` PDF `file:` URL and the backing-file drain); runtime (`content_identity.ContentIdentity`/`ContentKey`, `faults.RuntimeRail`/`async_boundary`, `receipts.Receipt`/`Redaction`/`receipted`/`Signals`).
...
- Boundary: a from-scratch InDesign-document synthesizer, a per-op `_compose`/`_import`/`_place` writer family beside the one `_mutate` dispatch, a per-op base re-open beside the one threaded fold, a `datas`/`prefixes`/`ats` triple-list zipped at the call site beside the one `IdmlSource` row, a `dual` `_anchored`/`_sourced` validator pair beside the one `_resolved` guard, three parallel `sources`/`anchors`/`identifiers` `match` properties re-enumerating the same case family beside the one `facts` projection, a hand-typed `PDFCrop_EnumValue` literal or a `_CROP` table restating `PdfCrop`'s own values beside `crop.value`, the `CropPDF` spelling where the canonical `CROP_PDF` token is `PDFCrop`, an `href` resolution claimed in prose but never spelled (now the `SetAttributes` case), a hand-edited IDML Zip-member XML string beside the `IDMLPackage` methods, a `BytesIO`-backed `IDMLPackage` that crashes the working-copy `os.unlink`, an in-capsule cp315 mutation beside the `anyio.to_process` worker seam the lxml gate forces, a bare unbounded `to_process` offload trusting the per-loop default process limiter beside the `_GATE`-bounded crossing, a live `IDMLPackage`/`_Element` returned across the worker boundary, an untrusted `.idml` reaching the interior without the `IndesignPayload` `TypeAdapter` gate, a prefix admitted on emptiness alone where `IDMLPackage.prefix` demands `\A\w+\Z`, a prose-only receipt beside the wired `@receipted`/`contribute`, and a `StrEnum`-plus-`dict[str, object]` erased-bag dispatch are the deleted forms; no UI, no live InDesign editor, no page-geometry synthesis. `SimpleIDML` owns the IDML package mutation and introspection — the Zip-archive structure, the named XML tag tree, the verified side-effect algebra, and the `xml_structure`/`export_xml` introspection read — and this owner composes that algebra plus the `xml_structure` read (anchor validation and the structural inventory) through the `@use_working_copy` new-instance boundary, re-implementing no IDML serialization. The InDesign-Server SOAP `save_as` conversion to `.indd`/`.pdf`/`.jpeg` stays outside this package (it needs a live network InDesign Server seat and a co-mounted working directory the host-free engine does not own) — this owner produces the editable `.idml` deliverable, never an `.indd`/`.pdf` server render. The placed flat layout arrives from `composition/compose#COMPOSE` keyed by the same `ContentKey` and binds as a placed PDF through `PlacePdf` or as XML-tagged content through `ImportXml`; the editable-named-layer SVG/PDF/TIFF hand-off stays `export/layered#LAYERED`'s — IDML is the InDesign template-mutation complement, layered the Illustrator/Acrobat/Photoshop named-layer complement, each meeting its native editor family.
...
if TYPE_CHECKING:
    from simple_idml.idml import IDMLPackage  # [RESEARCH] not-yet-admitted; the worker-side package type `_resolved` reads `xml_structure` off
...
# subprocess pool instead of fanning out at the per-loop process-limiter default — the gated-band bound
...
- [IDML_WORKER_SEAM] [RESOLVED]: the IDML mutation runs on the runtime sub-3.15 worker band, NOT in-capsule on cp315 — SimpleIDML's `idml.py` imports `from lxml import etree` at module scope, and `lxml` is manifest-gated `python_version<'3.15'` (no CPython 3.15 wheel for the libxml2 C-extension, confirmed against `pyproject.toml:154` and the folder `.api/lxml.md` header), so `import simple_idml` cannot resolve on the cp315 core. The whole `_mutate` fold crosses `anyio.to_process.run_sync(_mutate, plan)` onto the sub-3.15 interpreter — the same gated band `document/emit#EMIT` dispatches its `Band.WORKER` lxml/pikepdf arms over. Only the serialized package bytes plus the picklable `IdmlFact` (bytes + ints) cross back; a live `IDMLPackage` (a `zipfile.ZipFile` subclass holding an open file) and the `xml_structure` `_Element` tree are unpicklable across the boundary. A worker death surfaces as `anyio.BrokenWorkerProcess` the runtime `reliability/faults#FAULT` `CLASSIFY` `resource` row converts; an in-worker provider raise re-raises in the host where `async_boundary` lifts it to a `BoundaryFault`. The `produced`/`_produced` sequential-await fold matches `document/emit#EMIT`'s `_emitted` exactly — the established artifacts-producer convention threading independent productions through one `async_boundary`. The whole `Idml` (the `base` bytes plus the `steps: tuple[IdmlStep, ...]` `tagged_union` fold) pickles ONTO the worker as readily as the `IdmlFact` pickles back — `expression.tagged_union` instances serialize by tag-and-payload, so the outbound `IdmlStep`/`IdmlSource`/`PdfCrop`/`frozendict` payload crosses with no bespoke encode hop — and every offload threads the module-level `_GATE` `anyio.CapacityLimiter` so N concurrent `produced` calls share a fixed subprocess pool instead of fanning out at the per-loop process-limiter default, the gated-band bound `export/layered#LAYERED`'s `ORA` worker carries.
...
- [SIMPLEIDML_SIGNATURE_LOCK] [RESEARCH]: the SimpleIDML member chain is VERIFIED against `Starou/SimpleIDML` `src/simple_idml/idml.py` (the `IDMLPackage(zipfile.ZipFile)` class), `src/simple_idml/decorators.py` (the `@use_working_copy` extract-mutate-repackage contract), and `src/simple_idml/__init__.py` (`SETCONTENT_TAG`/`IGNORECONTENT_TAG`/`FORCECONTENT_TAG` the content-control attribute names, `VERSION = "1.3.1"`) — every mutation and introspection member this owner names exists with the spelled signature, and the full sixteen-method `@use_working_copy` surface was enumerated so the fourteen step-eligible verbs are modeled rather than a slice (`prefix` and the singular `add_page_from_idml` the two non-step members). SimpleIDML is NOT-yet-admitted in `pyproject.toml` and carries no folder `.api` catalogue: admit `simpleidml` on the sub-3.15 worker band (alongside the already-gated `lxml`), author its `.api` catalogue with reflection-verified members, then resolve the exact `_spill`/backing-file-drain temp-file plumbing against the installed surface — the `from simple_idml import idml` boundary import marks the not-yet-admitted dependency until then.
