...
- Receipt: `RefineReceipt` on `Refit`/`Offsets` — target versus achieved deviation, rounds spent, terminal sample count — plus the offset trim census (`TrimmedCrossings` · `KeptSegments`) on the `Offsets` case itself; `StationField.FrameDefect` carries the max `|X̂·Ŷ|` orthonormality residual as the wire's own validity witness — the SoA reduction is the `TensorPrimitives.Max` lane whose speed claim is the registered `Parametric.FrameDefectClaim` ledger row (`Domain/telemetry.md` `BenchClaim`), proven under the corpus gate, never assumed. Result carriers ARE `NurbsForm` values, so identity is their `ToEncodeForm()` projection into `ReconcileOp.Encode` — no receipt duplicates it.
...
public static class Parametric {
    // Registered speed claim (Domain/telemetry.md BenchClaim): the StationField orthogonality
    // reduction proves itself under the corpus gate; correctness never rides the claim.
