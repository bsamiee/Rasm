...
- Packages: `Annotation/typeface.md` (`FaceDecoration`); `Document/tables.md` (`ResourceName`); `Document/session.md` (`DraftFault`); `Domain/results` (`ValidityClaim`); `Domain/validation` (`CapabilitySet<T>`, `FactoryBridge.Accept`); RhinoCommon `TextEntity`/`Leader`/`AnnotationBase` per `.api/api-rhinocommon-annotation.md`; Thinktecture.Runtime.Extensions; LanguageExt.Core.
...
- Packages: `Domain/validation` (`Acceptance.Rows`, `Admit.Need`); `Document/tables.md` (`ResourceId`); RhinoCommon `TextFields` per `.api/api-rhinocommon-annotation.md`.
...
- Law: `OutlineProduct` carries its owned natives as a BASE POSITIONAL column and exposes typed `Release()`; cleanup never parks inside a value the caller may not inspect.
...
- Law: native-bearing answers expose one total `Release()` fold; cleanup faults stay typed on that result and no disposer parks them inside the answer.
