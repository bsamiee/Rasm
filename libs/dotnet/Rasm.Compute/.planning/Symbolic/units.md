...
- [04]-[PARSE_FORMAT]: culture-scoped parse and format edges; the seam `MeasureEvidence` result and its family-indexed render.
...
- Owner: the projection half of `QuantityFamily` — the `ByType` seam-identity index, its `Of(QuantityType)` inverse, `Catalogue()`, the `Targets` picker, and the result-shaped `Render`.
...
- Law: the seam result's own fields are a `QuantityType` token, plain strings and doubles, and two seam value objects, so it serializes through the package wire context while UnitsNet types never cross a JSON or proto wire — conversion-at-admission is what enforces the recorded UnitsNet-serialization SKIP.
