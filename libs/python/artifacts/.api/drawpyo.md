...
- `ContentIdentity` rail: the owning page encodes `File.xml` to bytes and derives the content key through `rasm.runtime.identity#ContentIdentity.of("diagram-drawio", xml_bytes)` over the rendered bytes (never a second serialization), so the `.drawio` artifact is content-addressed in the same artifact index the cache-check law uses — drawpyo never mints identity.
