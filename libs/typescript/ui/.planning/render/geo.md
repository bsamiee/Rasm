...
- [1]-[GEO_SERIES_LAYER]: the `GeoSeriesLayer` `Data.TaggedEnum` over the maplibre base substrate and the deck.gl overlay layer set, dispatching `geojson`/`tile`/`geoarrow`/`cell-index` source arms total under `$match`.
