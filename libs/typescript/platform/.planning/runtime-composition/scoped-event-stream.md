...
[SCOPED_EVENT_STREAM]: the generic scoped `addEventListener`->`removeEventListener` Stream bridge over a non-`WindowEventMap` target.
...
- Cases: the bridge is generic over the emitted event type `E` and the listener shape, so `window.navigation`'s `navigate` ingress (`NavigateEvent`, absent from `WindowEventMap`), the `workbox-window` `Workbox` lifecycle target (`WorkboxLifecycleEvent`), and a `PerformanceObserver` registration each compose this one owner instead of re-deriving the `Stream.asyncScoped`+`Effect.acquireRelease` skeleton; the `add`/`remove` pair arrives as a closure over the precise target so the element type stays exact — `routing/app-router.md`'s `navigate` stream, `offline-cache/service-worker-host.md`'s `SwLifecycle` stream, and `web-vitals/performance-budget.md`'s vital stream each lift their target's native event shape through this bridge with zero `as any` and zero `EventListenerOrEventListenerObject` widening.
