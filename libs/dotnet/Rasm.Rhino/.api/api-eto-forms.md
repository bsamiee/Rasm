...
- `libs/dotnet/Rasm.Rhino/.api/api-eto-platform.md`: native hosting, the native-parent attach and detach pair, and the style re-application seam cross into the platform-handler boundary, and the `Themed*Handler` backend classes register there.
...
- `Eto.Threading.Thread` stays subordinate to the Rhino host marshal owner (`libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md`); an Eto-level main-thread test never replaces the host marshal seam.
...
- Reject: a re-tabling of the branch construction spine, immediate 2D painting (`libs/dotnet/Rasm.Rhino/.api/api-eto-drawing.md`), platform-handler and native-hosting selection plus the `Themed*Handler` backends (`libs/dotnet/Rasm.Rhino/.api/api-eto-platform.md`), document-owned Rhino windows and panels (`libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md`), and leaking `Eto.Forms.*` types past the UI owner
