...
- **Link validation offload**: `FileOp.Do(new FileExchange.ArchiveValidate(...))` invokes `FileResourceGraph.Validate(parallel: true)`, which performs `File.Exists` checks across all linked block archives, render texture files, and file references via PLINQ. The operation parallelizes across worker threads but executes synchronously from the calling thread. UI-thread callers MUST wrap the dispatch in `Task.Run` to avoid stutter on archives with hundreds of linked resources. Headless and batch processes can invoke directly; the `parallel: false` overload yields a sequential fallback for diagnostics.
- **PDF publish supports mixed sheet + blank pages**: `FilePublishTarget.Pdf(Target, Prefix, Suffix, Annotate)` sequences `Prefix` blank pages first, then captured sheet pages, then `Suffix` blank pages. Each `FilePdfPage(WidthDots, HeightDots, Dpi, Annotate)` in `Prefix`/`Suffix` invokes `FilePdf.AddPage(int, int, int)` and runs its own annotation callback. The top-level `Annotate` callback applies only to sheet pages. All annotation callbacks run INSIDE `op.Catch`; throws convert to `Fin.Fail` per page. `FilePdf.SetCustomPages` is intentionally unused — it REPLACES previously-added pages and is incompatible with viewport capture.
...
- **Linked block refresh**: `FileSheetEdit.RefreshLinks(Option<Seq<string>> Archives, bool SkipUpToDate)` filters live `InstanceDefinition` instances by `UpdateType is Linked or LinkedAndEmbedded` (the canonical linked discriminant), then by `Archives` path subset if supplied, then optionally by `ArchiveFileStatus != LinkedFileIsUpToDate` when `SkipUpToDate: true`. Each surviving definition is refreshed via `InstanceDefinitionTable.RefreshLinkedBlock`; the first failed refresh short-circuits the rail with `InvalidResult`. Synchronous; wrap in `Task.Run` for UI-thread callers with large block counts.
- **Header/footer interpolation**: `CaptureDecor.HeaderText` / `FooterText` accept `{key}` tokens resolved against `RhinoDoc.Strings` when used by `Exchange.FileSheet`. Built-in tokens: `{page}` (page name), `{index}` (1-based page index), `{total}` (total page count). Unknown keys pass through verbatim.
