# [@use-gesture/react] — the continuous-pointer gesture recognizer in the `act/gesture` plane; the analog drag/pinch/wheel layer react-aria's discrete model leaves out
...
- package: `@use-gesture/react` (10.3.1, MIT, © Poimandres)
...
- peer: `react@>= 16.8.0` (hooks); dep `@use-gesture/core@10.3.1` — the framework-agnostic engine this package wraps in hooks and re-exports (`@use-gesture/core/utils`, `/actions`, `/types`)
...
- Reject: a discrete press/focus routed through a pointer gesture (react-aria owns it), a JSX-spread bind where `preventDefault` is needed (use `target`), an external mutable ref where `memo` fits, a second gesture hook on one element, a gesture on a vaul sheet (vaul owns its drag), and the full `useGesture` where `createUseGesture` would tree-shake
