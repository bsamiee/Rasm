...
[STACKING]:
- `Npgsql`(`libs/dotnet/.api/api-npgsql.md`), `Npgsql.EntityFrameworkCore.PostgreSQL`(`.api/api-npgsql-ef.md`): url, `params`, `headers`, `body`, and `timeout_milliseconds` bind as `NpgsqlParameter` values through `FromSql`/`RelationalDatabaseFacadeExtensions.SqlQuery<T>`; no EF translator covers the `net` schema.
