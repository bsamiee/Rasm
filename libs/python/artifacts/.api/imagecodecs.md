...
- `psdtags`(`.api/psdtags.md`) / `tifffile`(`.api/tifffile.md`): the container owner builds the layer/channel structure, then the layered-TIFF `_tiff` arm ALONE runs each `PsdChannel.data` 2-D array through `packbits_encode(channel, axis=0)` (method 1) or `delta_encode(channel, axis=-1)` + `deflate_encode(raw=True)` (method 3) before serialization, inverting on read; `tifffile` already calls `imagecodecs` internally for its strip and tile codecs. The native `_psd` arm crosses no member here — `psd-tools`(`.api/psd-tools.md`) carries its own stdlib-zlib and bundled-RLE channel cores.
