...
lazy from tests.python.testkit.strategies import resolve
...
class LawRecord(msgspec.Struct, frozen=True):
    """Registered law keyed by subject, law, owner module, and optional subject module."""
...
def _resolvable(subject: object) -> TypeIs[TypeForm[object]]:
    """True when ``resolve`` can own the subject: classes, PEP 695 aliases, and parameterized forms."""
    return isinstance(subject, type | TypeAliasType) or bool(get_args(subject))


def auto_exempt(subject: object) -> bool:
    """True when the census must not demand a law: StrEnum, method-free frozen struct, or value-only object."""
    match subject:
...
def _public_surface(package_name: str) -> tuple[dict[str, object], tuple[tuple[str, str], ...]]:
    """Collect a package's public simple-name → object surface and its ``(module, error)`` import failures."""
...
def spec[**P](subject: object, *, given: bool = True, profile: str | None = None, markers: tuple[str, ...] = (), timeout: float | None = None, law: str | None = None, events: tuple[Callable[[object], str], ...] = ()) -> Callable[[Callable[P, None]], Callable[P, None]]:
    ...
    def _decorator(fn: Callable[P, None]) -> Callable[P, None]:
        ...
        MANIFEST.append(LawRecord(subject=_qualname(subject), law=law or fn_name, module=getattr(fn, "__module__", "<unknown>"), subject_module=getattr(subject, "__module__", "") or ""))
...
def consume_covers(module: object) -> None:
    ...
    Raises:
        TypeError: When a ``COVERS`` entry is neither a type nor a callable — value-only symbols are auto-exempt.
    """
...
def register_sut(package: str, *, exempt: frozenset[str] = frozenset(), suite: Path | None = None) -> None:
    """Record a SUT package for law-coverage gating; a repeat merges exemptions and keeps the first suite.

    Args:
        package: Fully-qualified package name.
        exempt: Symbol simple-names explicitly exempt from law coverage, each justified at the call site.
        suite: Directory whose law modules carry this package's census; ``None`` derives the calling conftest's directory.
    """
    ...
    SUT_PACKAGES[package] = Sut(exempt=(prior.exempt if prior is not None else frozenset()) | exempt, suite=prior.suite if prior is not None and prior.suite is not None else derived)
...
def _importable(folder: Path, /) -> str:
    ...
    itself under the flat no-``src`` law, or ``src`` where a foreign checkout carries one — so the dotted name is that
    package's path under the root, the one name its modules carry once installed. Registering the folder path instead
    imports the tree a second time under a shadow name, minting duplicate classes beside the live ones. A manifest-less
...
def register_tree(source_root: Path, suite_root: Path) -> tuple[str, ...]:
    """Register every child folder under ``source_root`` that carries Python source, from disk shape.

    A folder registers under the name its modules import by, with the same-named folder under ``suite_root`` as its
    suite; a sourceless folder never registers, so a planning-only tree adds zero census obligations until code lands.
    """
...
def uncollected_laws() -> dict[str, tuple[str, ...]]:
    """Per-package law modules on disk but never imported by this session's collection.

    Collection imports every selected law module, so a dotted name absent from ``sys.modules`` means that module's
    laws never reached the manifest and the package census would report false gaps.
    """
    gaps = {package: tuple(sorted(name for name in _law_modules(sut.suite) if name not in sys.modules)) for package, sut in SUT_PACKAGES.items() if sut.suite is not None}
...
def assert_law_coverage(*, only: frozenset[str] | None = None) -> None:
    ...
    for package, sut in SUT_PACKAGES.items():
        ...
        covered = global_covered | frozenset(rec.subject.rsplit(".", 1)[-1] for rec in MANIFEST if rec.subject_module == package or rec.subject_module.startswith(f"{package}."))
        ...
        assert not gaps, f"Law coverage gap in '{package}': {len(uncovered)} symbol(s) have no law, {len(failures)} module import failure(s):\n" + "\n".join(gaps)


# --- [EXPORTS] --------------------------------------------------------------------------

__all__ = ["spec", "consume_covers", "register_sut", "register_tree", "assert_law_coverage", "auto_exempt", "uncollected_laws", "MANIFEST", "LawRecord", "Sut"]
