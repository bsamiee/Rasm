...
- Packages: SkiaSharp, Silk.NET.WebGPU, CommunityToolkit.HighPerformance, System.Numerics.Tensors, Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, Rasm (project — `Custody`, `FaultBand`), Rasm.Materials (project)
...
- Law: the roughness the ladder seats on is ADMITTED, never defaulted. `UnitInterval` admission rides the kernel `FactoryBridge.Accept` bridge, so an out-of-domain specular roughness refuses on the result rather than collapsing to the struct default and silently seating level zero — the brightest mirror level of every dome.
...
- Packages: SkiaSharp, Silk.NET.WebGPU, Thinktecture.Runtime.Extensions, LanguageExt.Core, Rasm (project — `Custody`), Rasm.Materials (project)
