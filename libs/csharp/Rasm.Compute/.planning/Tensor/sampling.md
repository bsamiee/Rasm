...
- Boundary — Halton reads its base per dimension from `JoeKuo.Primes[d]` (dimension 0 → 2) because a single shared base collapses every coordinate onto one radical-inverse sequence; `Equidistributed.Base` is the family discriminant (`2` Sobol, prime `> 2` Halton), never the per-coordinate radix.
...
- Boundary — net-quality figures are the genuine Warnock L2 star-discrepancy and the worst-2D-projection discrepancy, not a decorative `blockExponent / dimensions` ratio; the worst-projection figure is load-bearing because a net can be uniform in full dimension yet degenerate on a 2-D projection.
...
public sealed partial class Scramble {
    ...
    // The binary leg scrambles the whole 32-bit fixed-point Sobol coordinate; the base-b leg scrambles each radix-b Halton digit in place.
