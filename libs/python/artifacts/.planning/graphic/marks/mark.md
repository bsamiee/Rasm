...
- Law: `MarkFault` is the plane-wide rail vocabulary. `unscannable` identifies a carrier-less generated symbol, while `uncreatable` identifies a scan-only symbology presented to encode; structured cases retain offending symbology, option locations, and source-shape evidence.
