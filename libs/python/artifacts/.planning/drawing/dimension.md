...
- Auto: `_facets(op)` is one total or-pattern projecting each case's `(family, tol)` tail once, never a per-tag `getattr`; `_lower(msp, op, standard, offset)` reads the facets, derives `over = dict(standard.dimstyle(family)) | _tol_over(tol)` and `text = _dim_text(tol)`, then matches the op to its verified `ezdxf` builder — the `override=` dict is the `drawing/standard#DIMSTYLE` DIM-variable derivation scaled by the ISO 5455 factor merged with the per-op tolerance variables, so a `1:50` architectural dimension draws its 2.5 mm text and oblique tick at paper scale with zero per-arm literal. The ISO 129-1 tolerance lowers by mode: `Symmetric(pm)` onto `{"dimtol": 1, "dimtp": pm, "dimtm": pm}`, `Deviation(u, l)` onto a `MTextEditor().stack(f"+{u}", f"{l}", "^")` stacked-fraction MText appended to the `<>` measurement, `Limits(u, l)` onto `{"dimlim": 1, "dimtp": u, "dimtm": l}` (ezdxf computes the limit values), `Basic(v)` onto the `{"dimgap": <negative>}` boxed-text convention — the three distinct native mechanisms, never a hand-formatted `± "` string. `_stack(dim)` threads one `kiwisolver.Solver`: each stackable dimension's offset is a `Variable` keyed on the op index, a `required` constraint pins the first at its base offset, `required` min-separation constraints keep consecutive dimension lines ≥ DIMDLI apart, a `weak` equal-gap constraint distributes a chain evenly, `updateVariables()` writes each solved `value()` back, and `_shifted` applies each offset perpendicular to the measured direction through one `numpy` normal so a baseline set and a bank of parallel linear dimensions stack without overlap; a `strength` band separates the hard no-overlap snap from the aesthetic even distribution. The `_layered` arm composes the ISO 129-1 components under one `_pen`-resolved discipline sRGB (the DIMS-layer `Standard.rgb`): `_construction` lifts each extension line and dimension line/arc from `ezdxf.math.ConstructionLine`/`ConstructionArc`/`ConstructionCircle` over the op's anchor points and the DIMEXE/DIMEXO offsets into the one penned `_svg` group so the strokeless lines actually render, `_terminator_layer` draws each terminator as a self-contained filled/stroked ISO 129-1 mark per the resolved `dimblk`/`dimtsz` kind (`_terminator_kind` → a filled arrow triangle, a stroked oblique tick, a filled dot, or an open chevron — the correct ISO 129-1 default, the tapered variable-width stroke-to-outline composing the LANDED `graphic/vector#VECTOR` `outline` public function for a non-default terminator, never a phantom foreign call), `_annotation_layer` outlines the ISO 3098 measurement text through `ziafont.Font(...).text(..., color=pen).drawon(...)` to font-independent `<path>` geometry, and `_tolerance_layer` typesets a `Limits`/`Basic` expression through `ziamath.Latex(expr, color=pen).drawon(svg, x, y, valign="axis")` seated at the math axis — each a named `Layer` row `export/layered#LAYERED` binds, so the placed dimension survives on a sheet without the CAD font, editable and layer-separated.
...
def _stack(dim: Dimension, /) -> Map[int, float]:
    ...
    # anchor at the first base offset, `required` DIMDLI min-separation between consecutive lines, and a `weak`
    # equal-gap distribution — the offset stack a fixed per-dimension literal gets wrong on a dense chain.
    ...
    for lower, upper in pairwise(lanes):  # Exemption: kiwisolver.Solver is the stateful native sink; constraints add in place
        solver.addConstraint(offsets[upper] - offsets[lower] >= step)
        solver.addConstraint((offsets[upper] - offsets[lower] == step) | kiwisolver.strength.weak)
...
def _construction(op: DimOp, over: Override, /) -> tuple[Block[bytes], Box]:
    # the LAYERED geometry: extension/dimension lines lifted from ezdxf.math.ConstructionLine over the op
    # anchors and the DIMEXE offset — the anchor math, never hand-rolled trig; a best-effort editable projection.
    match op:
        case DimOp(tag="radius", radius=(center, radius, angle, *_)) | DimOp(tag="diameter", diameter=(center, radius, angle, *_)):
            edge = _polar(center, radius, angle)
            leader = ezmath.ConstructionLine(center, edge)
            return Block.singleton(_polyline((leader.start, leader.end))), _bounds((center, edge))
        case DimOp(tag="angular3p", angular3p=(_base, center, p1, p2, *_)) | DimOp(tag="arc3p", arc3p=(_base, center, p1, p2, *_)):
            legs = (ezmath.ConstructionLine(center, p1), ezmath.ConstructionLine(center, p2))
            return Block.of_seq(_polyline((leg.start, leg.end)) for leg in legs), _bounds((center, p1, p2))
        case _:
...
def _annotation_layer(dim: Dimension, pen: str, /) -> Layer:
    ...
    # dimension survives on a sheet without the CAD font — typography/shape#SHAPE owns the full shaped run.
    ...
    for op in dim.ops:
        ...
        if label := _annotation_text(op, tol):
            font.text(label, size=float(dim.standard.dimstyle(family).get("dimtxt", 2.5)), valign="base", color=pen).drawon(canvas, anchor[0], anchor[1])
...
def _tolerance_layer(dim: Dimension, pen: str, /) -> Layer:
    ...
    for op in dim.ops:
        ...
        if (expr := _tol_latex(tol)) is not None:
            ziamath.Latex(expr, size=float(dim.standard.dimstyle(family).get("dimtxt", 2.5)), color=pen).drawon(canvas, anchor[0], anchor[1], valign="axis")
...
- [OFFSET_STACK] [RESOLVED]: the dimension-line offset stack is one `kiwisolver.Solver` + `strength` (verified `Solver.addConstraint`/`updateVariables`, `Variable.value`, `strength.weak`/`required`, `UnsatisfiableConstraint`) — a `required` anchor at the first base offset, `required` DIMDLI min-separation between consecutive lines, and a `weak` equal-gap distribution over the stackable linear/aligned/baseline ops keyed on the stable op index, `updateVariables()` writing each solved `value()` back and `_shifted` applying it perpendicular through one `numpy` normal (verified `np.asarray`/`np.hypot`). This is the reading-map `apiUnderutilized` target closed: a naive page places dimension lines at fixed offsets and a dense chain overlaps; the Cassowary solve distributes them. The `kiwisolver` catalog names dimension/sheet alignment as an explicit constraint-layout target. Justified on PACKAGE (the verified `kiwisolver` surface) and DOMAIN (DIMDLI baseline spacing + no-overlap chain distribution).
- [LAYERED_DECOMPOSITION] [RESOLVED]: the `LAYERED` target composes the reading-map `apiUnderutilized` catalogs (`ziamath`, `ziafont`) into named editable `export/layered#LAYERED` `Layer` rows WITHOUT double-rendering what `ezdxf` already draws — the `DXF`/`SVG`/`PDF` targets LEAD with `add_*_dim().render()` (ezdxf owns all terminators/text natively), while the `LAYERED` target authors the ISO 129-1 COMPONENTS under one `_pen`-resolved discipline sRGB (`Standard.rgb` over the DIMS layer): the extension/dimension-line geometry from `ezdxf.math.ConstructionLine`/`ConstructionArc`/`ConstructionCircle` anchor math (verified present) inside one penned `_svg` group so the strokeless `fill="none"` lines actually render, the terminator family as SELF-CONTAINED filled/stroked SVG marks (a filled arrow triangle, a stroked oblique tick, a filled dot, an open chevron — directly renderable, no foreign outline pass), the ISO 3098 measurement text outlined through `ziafont.Font(...).text(..., color=pen).drawon(...)` (verified `Font.text`/`Text.drawon` + the `color` param), and a `Limits`/`Basic` expression typeset through `ziamath.Latex(expr, color=pen).drawon(svg, x, y, valign="axis")` (verified `Latex.svg`/`drawon`/`getsize` + the `color` param, `valign='axis'` seating at the math axis). This is a genuinely distinct deliverable (font-independent, layer-separated, editable) from the flat `SVGBackend` output, so no ISO component is re-drawn twice. Justified on PACKAGE (every member reflection-verified) and DOMAIN (an editable layer-separated dimension is the drawing-editor deliverable the flat render is not).
