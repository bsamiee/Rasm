...
Rasm.AppHost owns the post-fetch update concern: a `UpdateManager`-borne state machine that downloads a found release, admits it through the ONE `Sandbox/admission#SUPPLY_CHAIN_GATE` `SupplyChainGate.Admit` before a byte is staged, stages it, drains the node, rolls it over through `ApplyUpdatesAndRestart`, and mints a typed `UpdateReceipt` on every phase, plus a three-row `UpdateChannel` vocabulary carrying feed routing and downgrade policy, plus a fleet-wide rolling-update conductor that walks the cluster serving set in a `RollStrategy`-shaped (canary, blue-green, linear-wave) health-gated wave. The page owns the update rail, the channel axis, the progressive-rollout strategy axis, and the rollover-drain handshake that runs `DrainConductor` before the restart hands the process to Velopack and the `FleetRoll` fleet conductor that paces the strategy-shaped wave over `Wire/coordination#MEMBERSHIP_VIEW` `MembershipView.Serving`. The `UpdateCheck(ReleaseIdentity)` outbound hop stays the detect leg at outbound-resilience; everything after a release is found composes here over Velopack, the `Sandbox/admission` gate, the `DrainConductor` fold, `ReceiptSinkPort`, and the generated metric attributes.
    ...
    public IO<UpdateReceipt> Stage(UpdateInfo found, IProgress<int> progress, CancellationToken token) =>
        Downgrade(found.TargetFullRelease) && !channel.AllowVersionDowngrade
            ...
            : from start in IO.lift(() => host.Clock.GetCurrentInstant())
              ...
              // The ONE Sandbox/admission gate verifies the download BEFORE it stages, so a forged or
...
public sealed partial class RollStrategy {
    ...
    public partial Seq<Seq<MemberRecord>> Cohorts(Seq<MemberRecord> nodes);

    public RollPlan Plan(Seq<MemberRecord> nodes) => new(Cohorts(nodes), Bake);
...
public static class FleetRoll {
    ...
    public static IO<Seq<FleetRollReceipt>> Roll(
        ...
        strategy.Plan(membership.Serving) is var plan
            ? Wave(plan.Cohorts, 0, strategy, plan, rollNode, probe, bake, sink, clock, tenant)
            : IO.pure(Seq<FleetRollReceipt>());

    static IO<Seq<FleetRollReceipt>> Wave(
        Seq<Seq<MemberRecord>> cohorts, int index, RollStrategy strategy, RollPlan plan,
            ...
            : cohorts.Head.Value.AsEnumerable().Select(static (node, slot) => (node, slot)).ToSeq()
                ...
                .Bind(here => strategy.Advances(here) && !cohorts.Tail.IsEmpty
                    ...
                    ? (plan.BakeWindow > Duration.Zero ? bake(plan.BakeWindow) : IO.pure(unit))
                        .Bind(_ => Wave(cohorts.Tail, index + 1, strategy, plan, rollNode, probe, bake, sink, clock, tenant))
                        .Map(rest => here + rest)
                    : IO.pure(here));
