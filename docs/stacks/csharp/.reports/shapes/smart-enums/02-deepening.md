...
[SERIALIZER_SEAM]:
- Converter participation is decided at definition time: the generator stamps the converter-factory attribute directly onto the vocabulary type, so serializers bind without options registration, converter lists, or reflection walks. Generation activates per framework only when the matching serializer-integration assembly is referenced by the compilation; the per-enum frameworks flags then narrow within what is present, so a flags value naming an absent framework is inert, never an error.
...
- Conversion resolution is factory-first and selects the last declared factory matching the integration's filter — object-factory attribute declaration order is a precedence list where later wins; key conversion is only the fallback when no factory matches. Consequence: a serialization-claiming factory rewires every metadata-driven integration that resolves conversion at runtime, not merely the converters stamped on the type — wire-shape redirection propagates to integrations the declaration never names.
...
- A static-abstract invoker utility forwards to `Validate`/`Parse`/`TryParse` from contexts that hold the constraint generically; it is the sanctioned bridge where expression trees, older reflection layers, or boxed dispatch cannot name static abstracts directly — reach for it before hand-rolling delegate caches over generated statics.
- A culture-policy equality-comparer family operates over anything key-convertible to string: vocabularies and string-keyed value objects equate and hash by key under ordinal or culture policy in one comparer, replacing per-type comparer declarations for boundary set and dictionary membership. The trap: it compares key values under its own policy, which can be stricter or looser than the vocabulary's case-insensitive identity — pick the ordinal-ignore-case member to stay aligned with default key policy, or document the deliberate divergence.
[DIAGNOSTIC_TOPOLOGY]:
...
- Every item field must be public — a non-public static field of the enum type is an error, not a silently skipped member. A private scratch field of the enum type is only legal under the ignore attribute, which removes it from items, lookups, and dispatch entirely: there is no hidden-row tier, and row visibility is decided at the type level, never per row.
- Exactly one identifier is reserved for the key member name — the generated item list's own name, rejected ordinal-case-insensitively — so key-member renaming is otherwise free.
...
- The span admission plane — span parsing, span lookup, span factory, span identifier projection, and the ref-struct-permitting dispatch constraints — is compiled conditionally inside the generated source, so the public surface of one declaration is a function of the compiled target. Multi-targeted libraries get per-target surfaces from a single vocabulary declaration, and shared source written against the span plane must be confined to targets that generate it.
[GENERATOR_OBSERVABILITY]:
- The generator reads compiler-visible build properties for a file logger (path, level, unique-path-per-process toggle, initial buffer size) and an opt-in counter that watermarks every generated file with a monotonically increasing `// COUNTER:` header. The counter is the direct instrument for incremental-generation churn — a counter that climbs on edits to unrelated files marks a broken incremental pipeline — and the log answers "why did this type not generate" with evidence instead of attribute-staring.
- The empty-output case is observable, not silent: when a code generator produces no code for a matched declaration, the generator logs a warning through this channel. Without a configured log path it degrades to self-logging errors only, so the file logger must be switched on before the evidence exists.
