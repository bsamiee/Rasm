...
_QUANTILE_FRACTIONS = np.array([0.0, 0.25, 0.5, 0.75, 1.0])  # five-number summary positions, no uncatalogued np.percentile
_TAUBIN_K = 0.05  # the Taubin pass-band target `1/lamb - 1/nu`; nu = lamb/(1 - K*lamb) holds it constant in (0, 0.1) across every lamb, unlike a fixed additive nu offset
...
class MeshQuality(ReceiptContributor):
    ...
    @overload
    def apply(self, op: MeshQualityOp) -> "RuntimeRail[MeshQualityResult]": ...
    @overload
    def apply(self, op: Sequence[MeshQualityOp]) -> "RuntimeRail[Block[MeshQualityResult]]": ...
    def apply(self, op: MeshQualityOp | Sequence[MeshQualityOp]) -> "RuntimeRail[MeshQualityResult] | RuntimeRail[Block[MeshQualityResult]]":
        # the @overload arms carry the precise per-input-shape output so a caller narrows on what it passes, never re-matching the widened union
        match op:  # arity is a property of the argument: a lone op lifts through one boundary, a sequence folds under one Disposition
            case MeshQualityOp() as one:
                return boundary(f"mesh.quality.{one.tag}", lambda: self._arm(one))
            case batch:
                rails = Block(boundary(f"mesh.quality.{one.tag}", lambda one=one: self._arm(one)) for one in batch)
    ...
    def contribute(self) -> Iterable[Receipt]:
        ...
        facts: dict[str, object] = {  # native scalars; the receipts owner's enc_hook=repr renderer serializes without a str() coerce
            "op": r.op, "backend": r.backend.value, "faces_before": r.faces_before,
    ...
    def _arm(self, op: MeshQualityOp) -> Outcome:
        ...
        match op:
            case MeshQualityOp(tag="decimate", decimate=target_faces):
                out = self._mesh.simplify_quadric_decimation(face_count=target_faces)  # keyword: positional arg 0 is `percent`, not the face budget
    ...
    def _topology(self) -> ExactTopology:  # one fold over the single cached Manifold: genus + exact counts + kernel mass, never a lone genus()
        m = self._manifold()
        genus = sum(int(c.genus()) for c in m.decompose())  # genus() is defined per connected component (.api topology row [04] "call decompose first"); sum over the disconnected bodies
        return ExactTopology(genus, m.num_vert(), m.num_edge(), m.num_tri(), float(m.volume()), float(m.surface_area()))
    ...
    def _to_manifold(mesh: trimesh.Trimesh) -> "manifold3d.Manifold":
        ...
        faces = np.asarray(mesh.faces)
        if len(mesh.vertices) > np.iinfo(np.uint32).max:  # past ~4.29B verts the 32-bit Mesh truncates indices; Mesh64 is the f64 carrier (.api type row [04]), so its positions are f64 too
            return manifold3d.Manifold(manifold3d.Mesh64(vert_properties=np.asarray(mesh.vertices, dtype=np.float64), tri_verts=faces.astype(np.uint64)))
        return manifold3d.Manifold(manifold3d.Mesh(vert_properties=np.asarray(mesh.vertices, dtype=np.float32), tri_verts=faces.astype(np.uint32)))
```
