...
| :-----: | :----------------- | :-------------- | :--------------------------------------------- |
|  [01]   | `TileLayer`        | composite class | viewport-driven 2D tile streaming              |
|  [02]   | `MVTLayer`         | composite class | Mapbox Vector Tiles (extends `TileLayer`)      |
|  [03]   | `Tile3DLayer`      | composite class | OGC 3D Tiles / `i3s` scene streaming           |
|  [04]   | `TerrainLayer`     | composite class | terrain mesh from height tiles                 |
|  [05]   | `TripsLayer`       | layer class     | animated time-windowed trip paths              |
|  [06]   | `GreatCircleLayer` | layer class     | great-circle arcs between source/target        |
|  [07]   | `H3HexagonLayer`   | composite class | H3 cell hexagons                               |
|  [08]   | `H3ClusterLayer`   | composite class | merged H3 cell-set polygons                    |
|  [09]   | `S2Layer`          | composite class | S2 cell polygons                               |
...
|  [03]   | `Tile3DLayerProps`                   | type alias    | extends `CompositeLayerProps`                      |
|  [04]   | `TerrainLayerProps`                  | type alias    | extends `CompositeLayerProps`                      |
|  [05]   | `TripsLayerProps`                    | type alias    | extends `PathLayerProps`                           |
|  [06]   | `GreatCircleLayerProps`              | type alias    | extends `ArcLayerProps`                            |
|  [07]   | `H3HexagonLayerProps`                | type alias    | extends `CompositeLayerProps`                      |
|  [08]   | `H3ClusterLayerProps`                | type alias    | extends `CompositeLayerProps`                      |
|  [09]   | `S2LayerProps`                       | type alias    | extends `CompositeLayerProps`                      |
...
| [INDEX] | [SYMBOL]                              | [TYPE_FAMILY] | [RAIL]                                                                      |
| :-----: | :------------------------------------ | :------------ | :-------------------------------------------------------------------------- |
|  [01]   | `_Tileset2D`                          | class         | viewport-to-tile-index strategy (override for custom indexing)              |
|  [02]   | `_Tile2DHeader<DataT>`                | class         | per-tile load state, bounds, and content                                    |
...
|  [06]   | `RefinementStrategy`                  | type alias    | `'never' \| 'no-overlap' \| 'best-available' \| RefinementStrategyFunction` |
|  [07]   | `ZRange`                              | type alias    | `[minZ, maxZ]` numeric tuple                                                |
|  [08]   | `URLTemplate`                         | type alias    | `string \| string[] \| null`                                                |
...
| :-----: | :------------------------------------------------------------ | :------------- | :-------------------------------------------------------------- |
|  [01]   | `data` prop                                                   | data prop      | `URLTemplate` — tile URL template `{x}/{y}/{z}` or array        |
...
|  [06]   | `extent` prop                                                 | uniform prop   | `number[] \| null` — data bounding box                          |
|  [07]   | `TilesetClass` prop                                           | dispatch prop  | `typeof Tileset2D` — custom indexing scheme                     |
|  [08]   | `refinementStrategy` prop                                     | dispatch prop  | `RefinementStrategy` (default `'best-available'`)               |
|  [09]   | `zRange` prop                                                 | uniform prop   | `ZRange \| null` — min/max height for 3D culling                |
...
|  [02]   | `binary` prop               | toggle prop    | parse tiles as binary `BinaryFeatureCollection` (default `true`)            |
...
| [INDEX] | [SURFACE]                             | [ENTRY_FAMILY] | [RAIL]                                                    |
| :-----: | :------------------------------------ | :------------- | :-------------------------------------------------------- |
|  [01]   | `TileLayer.getTileData(tile)`         | method         | resolves one tile's data (override or `getTileData` prop) |
|  [02]   | `TileLayer.renderSubLayers(props)`    | method         | maps a loaded tile to renderable `Layer`/`LayersList`     |
|  [03]   | `TileLayer.isLoaded` getter           | accessor       | `boolean` — all viewport tiles resolved                   |
|  [04]   | `MVTLayer.getRenderedFeatures(max?)`  | method         | `Feature[]` — features rendered in current viewport       |
|  [05]   | `_getURLFromTemplate(template, tile)` | utility        | substitutes `{x}/{y}/{z}` into a `URLTemplate`            |
