...
- Owner: the command, availability, and receipt wire shapes the gateway binds — sourced from `csharp:Rasm.AppUi/commands/commands-availability#TS_PROJECTION` and `csharp:Rasm.AppHost/ports/runtime-ports#TS_PROJECTION`; the diagnostics-evidence and support-bundle shapes ride their owning consumers.
