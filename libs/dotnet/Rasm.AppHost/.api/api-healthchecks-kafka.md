...
- builder hook: `Configure` runs against `ProducerBuilder<string, string>` before `Build()`, the hook for `SetKeySerializer`/`SetValueSerializer`, statistics/log/error handlers, and OAuth bearer-token refresh; serializers default to UTF-8 string serializers because the probe message is `<string, string>`.
