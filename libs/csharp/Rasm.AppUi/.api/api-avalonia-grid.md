...
- Thinktecture rows (`api-thinktecture-json`): table rows are `[ValueObject]`/record DTOs with Thinktecture `SmartEnum`/`ValueObject` cells; `DataGridColumn.Binding` targets the strongly-typed members and `CustomSortComparer` consumes the value-object ordering, so the grid never sees stringly-typed cells.
