...
|  [07]   | `socket.ts`                        | Run loop over the `SocketServer` tag, link `Ref`, one handler per connection |
|  [08]   | `jobs.ts`                          | In-flight `Ref`, deadline, `run` and `probe` claims, result spill            |
...
|  [26]   | `Stream.fromSchedule(Schedule.spaced(tickMs))` with `(count + 1) % (beatMs / tickMs)` selecting the beat | `Effect.repeat(beat, { schedule: Schedule.spaced(HEARTBEAT.beatMs) })` and `Effect.repeat(expiry, { schedule: Schedule.spaced(HEARTBEAT.tickMs) })`, each forked in the connection scope | `Effect.d.ts:13921`, `Schedule.d.ts:1008`            |
|  [27]   | `Effect.forEach(tail)` then `Effect.forever(Effect.flatMap(pull, Effect.forEach(...)))` | `Stream.runForEach(Stream.concat(Stream.fromArray(tail), Stream.fromPull(Effect.succeed(pull))), receive)`              | `Stream.d.ts:463`, `:892`, `:3009`, `:14689`, `Socket.d.ts:57` |
...
|  [38]   | Link `busy{identity, job, startedAt, lastBeat}` beside the in-flight `Ref`, `_claimed` and `_settled` transitions, `probe` dispatching past the `Ref` | Link `listening \| attached`, `dispatch` reads the attached connection and takes replies `until` the `jobId` matches, every probe runs through `run` with `record: false` | `Effect.d.ts:9059`, `:13814`, `Queue.d.ts:573`, `:1223` |
...
|  [49]   | `(frame): frame is Extract<Frame, {_tag: 'hello'}> => frame._tag === 'hello'` and the `attached` twin under `Effect.filterOrFail` | `Match.type<Frame>().pipe(Match.tags({hello: Effect.succeed}), Match.orElse(...))` and `Match.type<Link>().pipe(Match.tagsExhaustive({attached: Effect.succeed, listening: ...}))`; `Predicate.isTagged` answers `self is {_tag: K}`, which `filterOrFail`'s `B extends A` refuses, and `Match.tag('hello', Effect.succeed)` reads `ReturnType` of a generic function as `Effect<unknown>` | `Match.d.ts:279`, `:1037`, `:1076`, `Predicate.d.ts:1352` |
|  [50]   | `_expiry`, a forked `Effect.repeat` every `HEARTBEAT.tickMs` comparing `now - lastBeat` and writing the close | `Stream.timeout(HEARTBEAT.expiryMs)` over the frame stream, the normal close written once the stream ends | `Stream.d.ts:2711` |
...
|  [11]   | `ResultNotDecodable` | Decode failure over stdout, a response file, or a frame           | Sidekick truncates a stack at 500 |
|  [12]   | `HostThrew`          | Host ran the job and reported a failure in band                   | Every in-band shape               |

No variant exists that no consumer tells apart. A Photoshop modal refusal arrives as `HostThrew`; InDesign's `modalState` is a field of the `state` frame and of `health`, a read rather than a refusal; a cancellation interrupts the handler fiber and answers nothing.

`HostRejection` is data a host produces, so it is one `Schema.Union` (`Schema.d.ts:3921`) of `Schema.TaggedStruct` members (`Schema.d.ts:4869`) with `type HostRejection = typeof HostRejection.Type`. The same schema decodes a plugin's `failed` frame and an Illustrator response file. An empty field record is `Record<never, never>`, because Biome refuses `{}`.
...
| :-----: | :------------------ | :---------------------------------------------------------------------------------- |
|  [01]   | `ScriptThrew`       | Response file or frame carries `{name, message, stack}`, Acrobat adds `extMessage`  |
...
|  [10]   | `MalformedParams`   | Plugin's frame decode failed                                                        |
...
`name` and `version` come from the server's `package.json`, the one place that states them, through a typed JSON import in `main.ts`. Each tool file builds its own `Toolkit.make` and exports one `Layer` from `McpServer.toolkit` over `toolkit.toLayer`, so the tool, its schemas, and its handler stay local and inferred; `main.ts` merges those layers under `McpServer.layerStdio` through `Layer.provideMerge` (`Layer.d.ts:2116`). The layer derives each tool's published JSON Schema from its Effect `Schema` and decodes every argument before the handler runs, so a handler receives constructed branded values and validates nothing again. Strict tools reject excess properties, and the layer supplies `McpSchema.McpRequestContext` to each handler.
...
Output is one closed `Schema.Union` per tool discriminated by `kind`. Every union carries a `{kind: 'error', error: BridgeError}` member, so the published output schema stays closed; the layer renders a success as structured content beside its JSON text, and a declared failure as `isError: true` with the error's `message` as plain text and no fields (`McpServer.js:1146`–`1160` at rc.115), which is why a `BridgeError` travels in the success union and never in the failure channel. `health` carries no error member: each of its failures is a row value.

`jobs.ts` measures a success value's JSON text before the toolkit hands it over. Above 200 000 characters it writes `.artifacts/creative-cloud/<host>/results/<jobId>.json` through `effect/FileSystem` (`FileSystem.d.ts:363`) and answers `{kind: 'file', path, bytes, sha256}`. The threshold is one constant of that file.
...
|  [06]   | Done per host    | Illustrator and Acrobat: exit `0` with the reply decoded; Photoshop and InDesign: a `done` frame |
...
|  [02]   | InDesign    | Plugin exports to `.artifacts/creative-cloud/indesign/<jobId>/<name>.<ext>` and the frame carries the path |
...
Photoshop and InDesign attach over a WebSocket the server listens on.

| [INDEX] | [STEP]                                                                        | [DECLARATION]                                |
| :-----: | :---------------------------------------------------------------------------- | :------------------------------------------- |
|  [01]   | `NodeSocketServer.layerWebSocket({host, port})` provides `SocketServer`       | `NodeSocketServer.d.ts:78`                   |
|  [02]   | `SocketServer` holds `address` and `run(handler)`, one handler per connection | `unstable/socket/SocketServer.d.ts:17`–`:18` |
|  [03]   | Address is a `NetAddress` value with `address` and `port`                     | `unstable/net/NetAddress.d.ts:55`–`:58`      |
|  [04]   | Socket is pull-based, `socket.reader` yields a `Reader` with `pull`           | `unstable/socket/Socket.d.ts:92`, `:110`     |
|  [05]   | `Socket.readerString` reads text batches, `socket.writer` writes a frame      | `unstable/socket/Socket.d.ts:192`, `:127`    |
|  [06]   | Frame decodes once through `Schema.fromJsonString` and `decodeEffect`         | `Schema.d.ts:6729`, `:1192`                  |
|  [07]   | First batch is the handshake, the rest is `Stream.fromPull` over the same pull, run by `Stream.runForEach` | `Stream.d.ts:463`, `:3009`, `:14689`, `Socket.d.ts:57` |
|  [08]   | A clean close fails the pull with a `SocketError`, which ends the stream and the connection scope | `unstable/socket/Socket.d.ts:7`–`:9`  |

Bind host is the IPv4 literal, because `localhost` binds `::1` alone and refuses IPv4 loopback, while the plugin dials `ws://localhost:<port>`, the domain its manifest declares. Frames are plain JSON text with no compression and no payload-size option, as Sidekick sends them; the only size limit is the MCP result spill.

| [INDEX] | [SIDE] | [STATE]                         | [TRANSITION]                                                                   |
| :-----: | :----- | :------------------------------ | :----------------------------------------------------------------------------- |
|  [01]   | Server | `listening`                     | First frame decoding as `hello` gives `attached{identity, lastBeat}`           |
|  [02]   | Server | `listening`                     | Any other first frame closes that socket                                       |
|  [03]   | Server | `attached{identity, lastBeat}`  | `beat`, `state`, `done`, and `failed` refresh `lastBeat`, a `state` frame updates `identity.state` |
|  [04]   | Server | `attached{identity, lastBeat}`  | A close or a 10 s silence gives `listening`, and a job waiting on the reply queue answers `transportClosed` |
|  [05]   | Server | Any                             | Connection arriving outside `listening` is closed by its own handler           |
|  [08]   | Plugin | `idle`                          | Tick dials the URL and gives `dialing{since}`                                  |
|  [09]   | Plugin | `dialing{since}`                | `onopen` sends `hello` and gives `attached`, `onclose` gives `idle` after 2 s  |
|  [10]   | Plugin | `attached`                      | `beat` every 5 s, a `job` frame gives `busy{job}`                              |
|  [11]   | Plugin | `attached`                      | 10 s without a server `beat` closes the socket inside a `try` and gives `idle` |
|  [12]   | Plugin | `busy{job}`                     | Executor resolves to `done` or `failed` and gives `attached`                   |

The server's link value carries its connection, the socket `Writer` and the reply `Queue`, in the `attached` member, so one `Ref` per endpoint holds state and connection together and `linkState` strips the connection for `health`. The link holds no `busy` member: the in-flight `Ref` of [06] is the one owner of the running job and of `hostBusy{startedAt}`, and `dispatch` writes the `job` frame to the attached connection and takes from the reply queue `until` a reply's `jobId` is the job's (`Effect.repeat`, `Effect.d.ts:13814`), so a reply for another id is dropped as Sidekick drops `No pending request for id`. A close fails the reply queue with `transportClosed{code: 1000}`, the normal-closure code the server itself sends on silence; the peer's own close code is not read. Heartbeat figures come from Sidekick: a beat every 5 000 ms after the handshake, a 10 000 ms expiry the plugin resets on the handshake and on every beat, a 5 000 ms handshake timeout, and a 2 000 ms reconnect delay. The server forks one loop in the connection scope, `Effect.repeat` (`Effect.d.ts:13921`) of the beat frame under `Schedule.spaced(HEARTBEAT.beatMs)` (`Schedule.d.ts:1008`), and reads the frame stream under `Stream.timeout(HEARTBEAT.expiryMs)` (`Stream.d.ts:2711`), which ends the stream after 10 000 ms without a frame; the handler then writes the normal close and its scope release returns the link to `listening`, so no tick counter and no clock comparison exist on the server; the plugin ticks once a second, and a transition happens in that one place. Frame handling is one `Effect.fnUntraced` over the decoded frame whose `Match` calls the pure link transitions `_beat` and `_stated`, each a function from the link value to the next, and offers a `done` or `failed` frame to the connection's reply queue. `frames.ts` exports `Link` as both the schema and its type, the Effect convention. A plugin nulls all four socket handlers before abandoning a socket, because UXP delivers queued events late, and it wraps `close()`, which can throw. UXP reports a refused connection through `onclose` rather than a throw, so the dial keeps one flag for whether the socket opened.

| [INDEX] | [FRAME]  | [DIRECTION]     | [FIELDS]                                                            |
| :-----: | :------- | :-------------- | :------------------------------------------------------------------ |
|  [01]   | `hello`  | Plugin → server | `plugin`, `version`, `host: {name, version}`, `uxp`, `app?`, `dom?` |
|  [02]   | `beat`   | Both            | None                                                                |
|  [03]   | `state`  | Plugin → server | `modalState`, `activeDocumentId`, `modified`                        |
|  [04]   | `done`   | Plugin → server | `jobId`, `value`, `autocorrections?`                                |
|  [05]   | `failed` | Plugin → server | `jobId`, `rejection: HostRejection`                                 |
|  [06]   | `job`    | Server → plugin | `jobId`, `kind`, `body`, `suspendHistory: {documentId, name}?`, `commandName?` |

`host.version` comes from `require('uxp').host` and `uxp` from `require('uxp').versions.uxp`, because the Photoshop UXP DOM exposes no `app.version`; InDesign adds `app.version` and `app.scriptPreferences.version`. `value` holds the body's JSON-serialisable return or an export path under `.artifacts/`, and a DOM object is never returned. Sidekick's reply codes map onto the rejection members: a missing parameter is `MalformedParams`, an unknown method is `UnknownMethod`, and a script error is `ScriptThrew`.
...
|  [03]   | `link`          | State machine value of the link, `identity` included when attached, `null` on an osascript host |
...
|  [07]   | `lastError`     | `{reason, count, secondsAgo}` of the last recorded failure, held in the same `Ref`, `count` climbing while the `reason` tag repeats and never cleared by a success |
...
|  [04]   | Add a host             | `main.ts`: one more tool layer in the merge and, for a socket host, one more endpoint and socket layer; `health.ts` changes only for a host on a third channel, which adds one arm to its `Match.discriminatorsExhaustive('channel')` table |
