...
| :-----: | :---------------------------- | :------------- | :----------------------------------------- |
|  [01]   | `CompressedColumnStorage<T>`  | abstract class | CSC matrix base; owns arrays and operators |
|  [02]   | `CSparse.Double.SparseMatrix` | concrete class | CSC double matrix; inherits CCS base       |
|  [03]   | `CoordinateStorage<T>`        | concrete class | COO triplet accumulator for CSC conversion |
|  [04]   | `DenseColumnMajorStorage<T>`  | concrete class | dense column-major matrix backing          |
|  [05]   | `SymbolicColumnStorage`       | concrete class | symbolic CSC used by factorization phase   |
...
|  [02]   | `ILinearOperator<T>`      | interface     | matrix-vector multiply seam         |
|  [03]   | `ISparseFactorization<T>` | interface     | factorization solve seam            |
|  [04]   | `ISolver<T>`              | interface     | in-place solve seam                 |
...
| [INDEX] | [SYMBOL]                                      | [TYPE_FAMILY]  | [CAPABILITY]                            |
| :-----: | :-------------------------------------------- | :------------- | :-------------------------------------- |
|  [01]   | `CSparse.Double.Factorization.SparseCholesky` | concrete class | direct sparse Cholesky (SPD)            |
|  [02]   | `CSparse.Double.Factorization.SparseLDL`      | concrete class | direct sparse LDL' (symmetric)          |
|  [03]   | `CSparse.Double.Factorization.SparseLU`       | concrete class | direct sparse LU with partial pivoting  |
|  [04]   | `CSparse.Double.Factorization.SparseQR`       | concrete class | direct sparse QR (overdetermined/under) |
...
|  [02]   | `ColumnOrdering.MinimumDegreeAtPlusA` |       1 | AMD on `A'+A`; valid for Cholesky and LDL'    |
...
| :-----: | :------------------------------------------------------ | :------------- | :-------------------------- |
|  [01]   | `SparseMatrix(rows, cols)`                              | constructor    | empty CSC double matrix     |
|  [02]   | `SparseMatrix(rows, cols, valueCount)`                  | constructor    | pre-sized CSC double matrix |
|  [03]   | `SparseMatrix(rows, cols, values, rowIndices, colPtrs)` | constructor    | CSC double from raw arrays  |
|  [04]   | `CoordinateStorage<T>(rows, cols, nz)`                  | constructor    | COO triplet accumulator     |
|  [05]   | `Converter.ToCompressedColumnStorage(storage)`          | static factory | COO to CSC conversion       |
...
| [INDEX] | [SURFACE]                                                   | [CALL_SHAPE] | [CAPABILITY]                 |
| :-----: | :---------------------------------------------------------- | :----------- | :--------------------------- |
|  [01]   | `SparseMatrix.Multiply(ReadOnlySpan<double>, Span<double>)` | instance     | `y = Ax` sparse GEMV         |
|  [02]   | `SparseMatrix.Multiply(alpha, x, beta, y)`                  | instance     | `y = αAx + βy` scaled GEMV   |
|  [03]   | `SparseMatrix.TransposeMultiply(x, y)`                      | instance     | `y = A'x` transposed GEMV    |
|  [04]   | `SparseMatrix.TransposeMultiply(alpha, x, beta, y)`         | instance     | `y = αA'x + βy` scaled       |
|  [05]   | `SparseMatrix.Add(alpha, beta, other, result)`              | instance     | `result = αA + βB`           |
|  [06]   | `SparseMatrix.DropZeros(tolerance)`                         | instance     | structural zero removal      |
|  [07]   | `SparseMatrix.Keep(Func<i,j,v,bool>)`                       | instance     | predicate-based entry filter |
|  [08]   | `SparseMatrix.L1Norm` / `InfinityNorm` / `FrobeniusNorm`    | instance     | matrix norm family           |
...
| [INDEX] | [SURFACE]                                             | [CALL_SHAPE]   | [CAPABILITY]                            |
| :-----: | :---------------------------------------------------- | :------------- | :-------------------------------------- |
|  [01]   | `SparseCholesky.Create(A, ColumnOrdering)`            | static factory | factors SPD CSC matrix                  |
|  [02]   | `SparseCholesky.Create(A, ColumnOrdering, IProgress)` | static factory | factors with progress reporting         |
|  [03]   | `SparseCholesky.Create(A, int[])`                     | static factory | factors with explicit permutation       |
|  [04]   | `SparseLDL.Create(A, ColumnOrdering)`                 | static factory | factors symmetric CSC matrix            |
|  [05]   | `SparseLDL.Create(A, ColumnOrdering, IProgress)`      | static factory | factors with progress reporting         |
|  [06]   | `SparseLU.Create(A, ColumnOrdering, tol)`             | static factory | factors square CSC matrix with pivot    |
|  [07]   | `SparseLU.Create(A, ColumnOrdering, tol, IProgress)`  | static factory | factors with progress and pivot tol     |
|  [08]   | `SparseLU.Create(A, int[], tol)`                      | static factory | factors with explicit column ordering   |
|  [09]   | `SparseQR.Create(A, ColumnOrdering)`                  | static factory | factors rectangular CSC matrix          |
