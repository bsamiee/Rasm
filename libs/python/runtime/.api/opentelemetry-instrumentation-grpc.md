...
[INTEGRATION_STACK]:
- grpcio leg: `aio_server_interceptor(tracer_provider, filter_)` threads into the `grpc.aio.server(interceptors=[...])` built from `libs/python/.api/grpcio.md`; `aio_client_interceptors(...)` is the four-element list passed to `grpc.aio.insecure_channel(target, interceptors=[...])`. The sync legs use `grpc.server(interceptors=[server_interceptor(...)])` and `grpcext.intercept_channel(grpc.insecure_channel(target), client_interceptor(...))`. One interceptor object per channel/server, never per-call.
