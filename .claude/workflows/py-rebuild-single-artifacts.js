export const meta = {
  name: 'py-rebuild-single-artifacts',
  description: 'Artifacts-scoped variant of py-rebuild-single: hostile ground-up rebuild of libs/python design pages to world-class py3.15 (ADT collapse, AOP, unified rails, full both-tier .api-stacking) AND justified IN-PLACE capability extension, but with most artifacts folders (composition, core, document, exchange, export, graphic) FROZEN OUT of the per-file rebuild so the rate-limited budget concentrates on the under-refined pages — EXCEPT graphic/vector.md, which is re-admitted into the per-file rebuild even though graphic is otherwise frozen. The frozen folders are STILL folded into the cross-file reconcile (repaired whenever another page change spans them) AND into the final folder-wide pass (collapse/critique/redteam over EVERY page, including the three). Per non-frozen design page, 1 agent per file in a 3-step ADVERSARIAL pipeline — rebuild(max) -> critique(xhigh) -> redteam(max), every stage hostile: assume the fence is naive/junior/illusory until it survives attack, never accept mature, hunt the fake/decorative code that reads advanced but is hollow, collapse + stack both .api tiers, AND close the concept capability gaps by growing the existing owner in place. Then a cross-file reconcile, then a whole-scope collapse/critique/redteam BATCHED into folder-coherent groups (BATCH pages/agent, each group a collapse -> critique -> redteam sequence pooled at CAP=6 so the whole corpus is never held by one agent; cross-batch concerns surface as residuals). CAP=6, BATCH=8. args = optional folder scope; empty/no-args defaults to artifacts, ALL = all of libs/python.',
  phases: [
    { title: 'Discover', detail: 'list every design page under the target, then split off the frozen artifacts folders (composition/core/document/exchange/export/graphic, minus the re-admitted graphic/vector.md) from the per-file set' },
    { title: 'Rebuild', detail: 'per NON-FROZEN page (1 agent/file): rebuild(max) -> critique(xhigh, 6-checklist + capability-completeness) -> redteam(max, counterfactual + cold re-review), every stage ADVERSARIAL and every stage first LISTS then FULLY reads the entire docs/stacks/python/ doctrine in README routing order, pooled at CAP=6 (the frozen folders are NOT per-file rebuilt)' },
    { title: 'Final-Collapse', detail: 'FOLDER-COHERENT BATCHES (BATCH pages/agent) over the WHOLE scope INCLUDING the frozen folders, each batch a collapse(max) -> critique(xhigh) -> redteam(max) sequence pooled at CAP=6: collapse + unify across files into one rail/polymorphism/shape system, break illusory differentiation, reduce chaff without removing capability; cross-batch concerns surface as residuals' },
  ],
}

// --- [CONSTANTS] -------------------------------------------------------------------------
const CAP = 6
const BATCH = 8 // pages per final-collapse batch agent; folder-coherent groups packed under this budget, each batch a 3-stage chain pooled at CAP
...
const DEFAULT_SCOPE = 'artifacts' // empty/no-args scopes here (this is the -artifacts workflow); pass ALL for the whole libs/python sweep
// frozen from the per-file rebuild; still reconciled + final-pass folder-wide. REBUILD_INCLUDE
// re-admits specific pages into the per-file rebuild even though their parent folder is frozen.
const REBUILD_EXCLUDE = [
  'libs/python/artifacts/.planning/composition',
  'libs/python/artifacts/.planning/core',
  'libs/python/artifacts/.planning/document',
  'libs/python/artifacts/.planning/exchange',
  'libs/python/artifacts/.planning/export',
  'libs/python/artifacts/.planning/graphic',
]
const REBUILD_INCLUDE = [
  'libs/python/artifacts/.planning/graphic/vector.md',
]
const FROZEN_LABEL = REBUILD_EXCLUDE.map((p) => p.split('/').pop()).join('/')
...
const SCOPE = rawScope === 'ALL' ? '' : (rawScope || DEFAULT_SCOPE)
...
const COMMENTS = 'COMMENT HYGIENE: code fences are agent-facing — comment for the next agent, never as a tutorial. KEEP the canonical ' +
  ...
  'low-value comment.'
const WIDE = [
  'WIDER-SCOPE REBUILD (BATCHED) — this is the FINAL pass, after every non-frozen design page ran its own per-file rebuild -> critique -> redteam ' +
    'and the cross-file reconcile closed the residuals it could see. This is NOT alignment: it is a SECOND, WIDER REBUILD over the SAME scope (' + SWEEP + '), ' +
    'spanning EVERY page including the frozen folders (' + FROZEN_LABEL + ', minus the re-admitted graphic/vector.md) held out of the per-file ' +
    'rebuild — they are IN SCOPE here. The corpus is split into FOLDER-COHERENT BATCHES so no single agent must hold all of it at once: you OWN one ' +
    'batch of pages (the exact list is named in the TASK) and you EDIT only those pages. READ your whole batch IN FULL, PLUS any sibling page ' +
    'elsewhere under ' + SWEEP + ' that your batch pages directly compose, seam to, or duplicate (read-only, for cross-file context) — never read the ' +
    'entire corpus blindly. Then drive YOUR BATCH, with that wider context in view, into ONE coherent body. Same hostile stance as the per-file ' +
    'passes: assume the batch is naive, fragmented, and illusory until it survives an aggressive cross-file attack; the burden of proof is on the ' +
    'code, never on you.',
  'WIDE OBJECTIVES (fix every one in place across YOUR BATCH; a concern whose rightful owner lives in ANOTHER batch you cannot edit is surfaced as a ' +
    '{files, claim} residual naming both files, never silently dropped): (1) RIGHT-PLACE OWNERSHIP — all logic lives in its ONE rightful owner page; ' +
    'a concern needlessly SPLIT across your batch files collapses to the single owner (the sibling page composes that owner per ' +
    'COMPOSED_IMPLEMENTATION, never a second mint), and any differentiation that should be ONE owner is unified; a split into another batch is a ' +
    'residual. (2) UNIFY THE STACK — ONE rail family across the batch and the siblings you read ' +
    ...
    'within the batch collapse into ONE unified owner / pipeline / flow per `SHAPE_BUDGET` + `DEEP_SURFACES`, a cross-batch duplicate becomes a ' +
    'residual. (3) BREAK ILLUSORY ' +
    ...
    'in your batch that PRETEND to model two concepts but model one collapse to one; ceremony carrying no real capability is deleted; a name / ' +
    'signature / prose promising capability the body does not implement is a defect; treat dense, confident-looking fences with MORE suspicion, not ' +
    'less. (4) REDUCE ' +
    'FOOTPRINT WITHOUT LOSING CAPABILITY — strip chaff and illusory differentiation to shrink the batch, NEVER by deleting functionality; every ' +
...
const isExcluded = (p) => !REBUILD_INCLUDE.includes(p) && REBUILD_EXCLUDE.some((pre) => p === pre || p.indexOf(pre + '/') === 0)
const groupKey = (p) => folderOf(p) + '/' + (subOf(p).split('/')[0] || subOf(p))
const batchLabel = (pages) => { const a = groupKey(pages[0]).split('/').pop(); const b = groupKey(pages[pages.length - 1]).split('/').pop(); return a === b ? a : a + '..' + b }
const packBatches = (pages, budget) => { const groups = new Map(); for (const p of pages.slice().sort()) { const k = groupKey(p); (groups.get(k) || groups.set(k, []).get(k)).push(p) } const out = []; let cur = []; for (const g of groups.values()) { if (cur.length && cur.length + g.length > budget) { out.push(cur); cur = [] } cur.push(...g) } if (cur.length) out.push(cur); return out }
...
const batchList = (pages) => pages.map((p) => '  - ' + p).join('\n')
const finalCollapsePrompt = (pages) => [LAW, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', PATLAW, '', BOUNDARIES, '', PROSE, '', COMMENTS, '', WIDE, '', 'TASK: ' +
  'WIDE-SCOPE COLLAPSE of YOUR BATCH (' + pages.length + ' design pages, edit ONLY these):\n' + batchList(pages) + '\nRead every page in this batch IN ' +
  'FULL, plus any sibling page elsewhere under ' + SWEEP + ' that these pages directly compose, seam to, or duplicate (read-only context). Walk the ' +
  'batch for concerns split across its files, parallel / near-duplicate shapes / pipelines / logic-flows, divergent rail families, divergent ' +
  'polymorphism approaches, and divergent owner forms; COLLAPSE each into its ONE rightful owner / pipeline / flow within the batch (the sibling ' +
  'composes the owner, never re-mints it), UNIFY the rail / shape / AOP system, and BREAK every illusory differentiation, decorative-complexity, ' +
  'and fake-advanced fence. Reduce footprint by stripping chaff and illusory differentiation — NEVER by deleting capability; preserve and densify ' +
  'every capability into its owner. Read the operative docs/stacks/python/ pages and BOTH .api tiers (shared `libs/python/.api/*.md` + folder ' +
  '`libs/python/<folder>/.api/*.md`); verify every cited member (a member you cannot verify is a phantom to delete). EDIT only the batch pages via ' +
  'Edit/Write, regressing none and raising quality on every touched page; a collapse whose rightful owner is a page OUTSIDE this batch is a {files, ' +
  'claim} residual naming both. Return verdict + collapsed (each cross-file collapse / unification made, naming the files + the owner / pipeline / ' +
  'flow) + residual (each {files, claim} for a cross-batch concern or anything you could not fully resolve) + summary.'].join('\n')
const finalCritiquePrompt = (pages) => [LAW, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', PATLAW, '', BOUNDARIES, '', PROSE, '', COMMENTS, '', WIDE, '', 'TASK: ' +
  'HARSHER CROSS-FILE AUDIT of the wide-scope collapse of YOUR BATCH (' + pages.length + ' pages, edit ONLY these) + FIX IN PLACE:\n' + batchList(pages) + '\n' +
  'You are an ULTRA-HARSH, UNAGREEABLE auditor: assume a cross-file defect remains until you prove otherwise; trust NOTHING the collapse pass or ' +
  'the prose claimed. Re-walk every page in this batch (plus the siblings under ' + SWEEP + ' it seams to, read-only): a concern still split across ' +
  'the batch files, a parallel / near-duplicate shape or pipeline or logic-flow not yet unified, a second rail family / second polymorphism ' +
  'approach / divergent owner form still standing, an illusory differentiation or decorative-complexity fence the collapse missed, chaff still ' +
  'inflating the footprint, a capability dropped instead of densified. Run the 12-signal COLLAPSE_SCAN across the batch and apply every triggered ' +
  'move. Repair every hit in place across the batch pages, preserving all capability and regressing no page; a defect whose rightful owner lies in ' +
  'another batch is a {files, claim} residual naming both. Return verdict + collapsed + residual + summary.'].join('\n')
const finalRedteamPrompt = (pages) => [LAW, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', PATLAW, '', BOUNDARIES, '', PROSE, '', COMMENTS, '', WIDE, '', 'TASK: ' +
  'HARSHEST CROSS-FILE RED-TEAM of YOUR BATCH (' + pages.length + ' pages, edit ONLY these) — the LAST and most aggressive pass over it:\n' + batchList(pages) + '\n' +
  'Trust nothing the collapse / critique claimed; the burden of proof is on the batch. COUNTERFACTUALLY attack the cross-file ownership: is each ' +
  'shared concept on its ONE rightful owner; is each rail family / polymorphism approach / owner form genuinely unified to ONE across the batch; ' +
  'does any pair of pages still model ' +
  ...
  'cross-file consumer untouched or broken loudly at type-check (`ANTICIPATORY_COLLAPSE`)? Fix every defect in place across the batch pages; a ' +
  'concern whose owner spans into another batch is a {files, claim} residual naming both. If the batch is genuinely one coherent, fully-unified ' +
  'body, prove it by finding nothing — never invent churn. Return verdict + collapsed + ' +
...
const processBatch = async (b) => {
  ...
  for (const st of COLLAPSE_STAGES) {
    const r = await agent(st.build(b.pages), { label: 'collapse-' + st.key + ':' + b.label, phase: 'Final-Collapse', schema: COLLAPSE_SCHEMA, effort: st.effort, stallMs: 900000 })
    if (r === null) break
    logs[st.key] = r
  }
  return { batch: b.label, logs, ok: Object.keys(logs).length === COLLAPSE_STAGES.length }
}
...
const allPages = ((inv && inv.pages) || []).filter(Boolean)
const excluded = allPages.filter(isExcluded)
const pending = allPages.filter((p) => !isExcluded(p)).map((p) => ({ page: p }))
const total = pending.length
log('Discover under ' + SWEEP + ': ' + allPages.length + ' design pages; ' + total + ' per-file rebuild targets, ' + excluded.length + ' frozen ' +
  'from per-file rebuild (still reconciled + final folder-wide pass); bounded pool CAP=' + CAP + ', ' + STAGGER_MS + 'ms staggered roll-out')
...
if (clusters.length) {
  ...
  reconciled = (await pipeline(
    ...
    (cl) => agent([LAW, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', PATLAW, '', BOUNDARIES, '', 'TASK: RECONCILE these cross-FILE residuals the ' +
      'critique AND red-team passes deferred. There is NO severity — treat EVERY residual as must-address. Read EVERY listed file IN FULL (frozen ' +
      'folder files included — fix them in place when a residual spans them), plus any sibling page in the same folder needed for context, so the ' +
      'reconcile considers the FULL folder, not only the in-scope targets. For each: if it ' +
...
const finalBatches = packBatches(allPages, BATCH).map((pages) => ({ pages, label: batchLabel(pages) }))
log('Final-Collapse: ' + allPages.length + ' pages (incl. frozen) packed into ' + finalBatches.length + ' folder-coherent batches of <=' + BATCH + '; ' +
  'each a collapse(max) -> critique(xhigh) -> redteam(max) sequence, pooled at CAP=' + CAP)
const collapseDone = (await pool(finalBatches, CAP, processBatch)).filter(Boolean)
const collapsedAll = collapseDone.flatMap((d) => Object.values(d.logs).flatMap((l) => (l && l.collapsed) || []))
const finalResidualRaw = collapseDone.flatMap((d) => Object.values(d.logs).flatMap((l) => (l && l.residual) || []))
const finalResidual = [...new Map(finalResidualRaw.map((r) => [(r.files || []).slice().sort().join(',') + '|' + r.claim, r])).values()]
const finalComplete = collapseDone.filter((d) => d.ok).length
log('Final-Collapse: ' + finalComplete + '/' + finalBatches.length + ' batches complete; ' + collapsedAll.length + ' collapses, ' + finalResidual.length + ' ' +
  'residual (deduped, cross-batch -> resolve-residuals)')

return { root: ROOT, scope: SCOPE || 'ALL', discovered: allPages.length, frozenFromRebuild: excluded, complete: done.filter((r) => r.ok).length, incomplete: done.filter((r) => !r.ok).length, total: total, clusters: clusters.length, hard_residual: hard_residual, dropped: dropped, finalBatches: finalBatches.length, finalComplete: finalComplete, finalCollapsed: collapsedAll, finalResidual: finalResidual }
