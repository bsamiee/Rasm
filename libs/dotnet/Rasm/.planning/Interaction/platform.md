...
- Output: `HandlerSeat` publishes `Seated` and `Refused`; `Handlers.Census` publishes `MintFact` through the observer and parks observer faults on the injected `FaultCell` through `FaultRail.Isolate` rather than failing the raise — a `void` reporter licenses a discard and bounds nothing, while the cell's parks, sheds, and declined parks all read as numbers.
