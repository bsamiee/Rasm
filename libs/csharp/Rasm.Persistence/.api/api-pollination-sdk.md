...
`PollinationSDK` supplies the Pollination cloud compute transport — the generated OpenAPI `*Api` REST clients (`JobsApi`/`RunsApi`/`ProjectsApi`/`ArtifactsApi`/`RecipesApi` + the account/org/team/registry/subscription leg), the `Client.Configuration` token-auth surface, and the high-level `Wrapper` orchestration (`JobInfo`/`JobRunner`/`ScheduledJobInfo`/`RunInfo` + the run-asset family). It is the Sync sub-domain's cloud-job leg, a sibling to the Speckle object-graph sync (`api-speckle.md`) in the `[VERSIONING_SYNC]` roster: submit a Rasm model/recipe inputs to a Pollination project as a `Job`, watch the `Run` to completion, and pull the result assets back. The artifact transport is presigned S3 — `ArtifactsApi.CreateArtifactAsync` returns an `S3UploadRequest`, so the bytes ride the SAME S3/S3-compatible object plane the folder's `Store/remote` owner already drives (`AWSSDK.S3`, `Minio`, `api-objectstore.md`), and a downloaded `RunAsset` lands as a content-keyed (`XxHash128`) blob in the Persistence store. Like Speckle, the SDK runs OUTSIDE-RHINO on the sync/egress sidecar: it carries its OWN vendored `LBT.RestSharp` (RestSharp 106) + `LBT.Newtonsoft.Json` fork closure plus a local `Microsoft.Data.Sqlite` job cache, and the in-Rhino plugin assembly never references it.
...
- rail: sync
- composition law: `JobInfo` wraps a `Job` + project slug + local folder; `RunJobAsync` uploads assets then submits, returning a `ScheduledJobInfo`; the run handle pulls assets back, which the Sync owner lands content-keyed.
...
[STACK]:
- sync-sibling seam: `PollinationSDK` is the cloud COMPUTE-job leg of the Sync sub-domain, parallel to Speckle's object-GRAPH sync (`api-speckle.md`) — both in `[VERSIONING_SYNC]`, both OUTSIDE-RHINO with isolated dependency closures, both projecting a Rasm domain owner to a cloud transport at the Sync boundary. Speckle owns graph diff/merge; Pollination owns recipe-run submission and result retrieval. They never share a transport.
- object-store seam: the `S3UploadRequest`/artifact download composes `api-objectstore.md` (`AWSSDK.S3`/`Minio`) for the byte transfer; the content-key (`XxHash128`) and `AsStream` body bridge are the folder's, not Pollination's.
- result-landing seam: a `Run` result + its `RunOutputAsset`s map onto the Persistence artifact/provenance owner — a completed cloud run becomes a content-addressed, lineage-tracked artifact set, not a loose file download.
...
[PACKAGE_SCOPE]:
- This page carries `PollinationSDK` API facts only; the canonical Sync case, the `ContentKey` projection of downloaded assets, and the provenance landing are owned by the `Sync` design pages.
...
- Object-store lane: the artifact byte transfer is owned by `api-objectstore.md` (`AWSSDK.S3`/`Minio`); this page records only that `ArtifactsApi.CreateArtifactAsync` returns a presigned `S3UploadRequest`.
- Sync-sibling boundary: Speckle (`api-speckle.md`) owns object-graph diff sync; Pollination owns cloud compute-job transport. Distinct rails in the same `[VERSIONING_SYNC]` group.
