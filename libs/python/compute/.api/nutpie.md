...
| :-----: | :------------------------------- | :------------------ | :------------------------------------------------------------------------------- |
|  [01]   | `compile_pymc.CompiledPyMCModel` | compiled PyMC model | compiled PyMC model extending `CompiledModel`                                    |
|  [02]   | `compile_stan.CompiledStanModel` | compiled Stan model | compiled Stan model extending `CompiledModel`                                    |
|  [03]   | `compiled_pyfunc.CompiledModel`  | base compiled type  | base with `n_dim`, `coords`, `shapes`, `reparameterized_names`, `benchmark_logp` |
|  [04]   | `compiled_pyfunc.PyFuncModel`    | pyfunc model type   | model compiled from a Python log-density callable                                |
...
| :-----: | :----------------------------- | :--------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  [01]   | `compile_pymc_model`           | PyMC compiler    | `(model, *, backend='numba', gradient_backend='pytensor', initial_points, jitter_rvs, default_initialization_strategy='support_point', var_names, freeze_model, **kwargs) -> CompiledModel`                                                                                             |
|  [02]   | `compile_stan_model`           | Stan compiler    | `(*, code, filename, extra_compile_args, extra_stanc_args, dims, coords, model_name, cleanup=True, cache=False, prune_cache=True) -> CompiledStanModel`                                                                                                                                 |
|  [03]   | `sample`                       | NUTS runner      | `(compiled_model, *, draws, tune, chains, cores, seed, save_warmup=True, progress_bar=True, sampler='nuts', adaptation='diag', init_mean, return_raw_trace=False, blocking=True, progress_rate=100, zarr_store, store_unconstrained=False, **kwargs) -> DataTree \| _BackgroundSampler` |
|  [04]   | `zarr_store.from_url`          | store factory    | constructs the matching Zarr store from a URL scheme                                                                                                                                                                                                                                    |
|  [05]   | `prune_stan_cache`             | cache management | `(max_entries=16, min_age=timedelta(days=14)) -> None`                                                                                                                                                                                                                                  |
|  [06]   | `CompiledModel.benchmark_logp` | logp bench       | times log-density and gradient evaluations on the compiled model                                                                                                                                                                                                                        |
