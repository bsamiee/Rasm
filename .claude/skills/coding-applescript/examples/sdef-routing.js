...
const fourCharCode = (text) =>
    Array.from(text).reduce((acc, ch) => (acc << 8) | ch.charCodeAt(0), 0) >>>
    0;
...
function copyDictionary(appPath) {
    ...
    const status = $.OSACopyScriptingDefinitionFromURL(
    ...
    const error = $();
    const document = $.NSXMLDocument.alloc.initWithDataOptionsError(
        ObjC.castRefToObject(out[0]),
        0,
        error,
    );
    if (document.isNil())
        throw new Error(`dictionary parse failed with ${error.code}`);
    return document;
}
...
function nodes(context, expression) {
    ...
    if (found.isNil())
        throw new Error(`XPath failed with ${error.code}: ${expression}`);
    return Array.from({ length: found.count }, (_, index) =>
        found.objectAtIndex(index),
    );
}
...
function catalogOf(document) {
    ...
    nodes(document, '//command').forEach((node) => {
        ...
        commands.set(name, {
            code,
            parameters: nodes(node, 'parameter/@name').map(
                (parameter) => parameter.stringValue.js,
            ),
            groups: nodes(node, 'access-group/@identifier').map(
                (group) => group.stringValue.js,
            ),
        });
...
function routeOf(row, catalog) {
    ...
    if (declared !== undefined) {
        const missing = row.parameters.filter(
            (label) => !declared.parameters.includes(label),
        );
        const scriptingTargets = declared.groups.filter(
...
function dispatch(appPath, verdict, operand) {
    if (verdict.route === 'terminology') {
        return {
            sent: 'terminology',
            value: Application(appPath)[camelVerb(verdict.verb)](operand),
        };
    }
    const target = $.NSAppleEventDescriptor.descriptorWithApplicationURL(
        $.NSURL.fileURLWithPath(appPath),
    );
    const event =
    ...
    if (operand !== undefined) {
        event.setParamDescriptorForKeyword(
            $.NSAppleEventDescriptor.descriptorWithString(operand),
            fourCharCode('----'),
        );
    }
    const error = $();
...
function run(argv) {
    ...
    if (!appPath)
        throw new Error('usage: <app-bundle-path> [verb[=operand] ...]');
    ...
    const sends = verdicts
        .filter(
            (verdict) =>
                operands.has(verdict.verb) && verdict.route !== 'refused',
        )
        .map((verdict) => ({
            verb: verdict.verb,
            ...dispatch(appPath, verdict, operands.get(verdict.verb)),
        }));
