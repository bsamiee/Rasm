...
- `stream-unzip` is the exact streamed inverse of `stream-zip`: a download or response body yields `Iterable[bytes]` straight into `stream_unzip`, which `package/archive#Archive`'s `_zip_drain` re-emits as `(name, size, chunks)` so the `CompressionAlgo.ZIP_STREAM` unpack arm closes the round-trip the `_zip_members`/`stream_zip` pack arm opens — without a `zipfile` whole-archive seek-and-buffer. The decode-not-remint law holds: the unpack arm recovers the member roster, it never re-authors the container.
- the per-member triple is the `package/bundle#BUNDLE` `BundleManifest.of` row material: `_zip_drain` drains each member's `chunks` accumulating the byte total and a rolling `xxhash.xxh3_128` digest into a `(name, size, digest)` `MemberTriple` under the `_ARCHIVE_CEILING` aggregate output budget (the SAME 16-byte content digest the single-blob `_walked` and the `py7zr` `_Xxh3Factory` arms yield, so the `ContentIdentity.key(f"member-{algo}", digest)` member key is uniform across every codec path) — never the weak stored 32-bit CRC; the recovered triples feed `BundleManifest.of`'s content-key fold and the bundle owner's `core/receipt#RECEIPT` `ArtifactReceipt.Bundle` `entries` slot, on the shared `expression` `Result` rail (`async_boundary` lifts a corrupt or hostile container to `Result.Error`, never an empty manifest).
...
- `allowed_encryption_mechanisms` DERIVES from the one `ZipStreamKnobs.password` encryption discriminant through `_zip_trust` — `None` admits `{NO_ENCRYPTION}` alone, a set password admits `{AE_2, AES_256}` alone (exactly what `stream-zip` writes) — so legacy ZipCrypto and the weak AE-1/short-key variants refuse BEFORE decryption and the allow-list can never reject the archive its own pack leg produced; a violation surfaces as the typed `EncryptionMechanismNotAllowed` leaf the `Result` rail carries, observable on the shared `structlog`/`opentelemetry` span.
