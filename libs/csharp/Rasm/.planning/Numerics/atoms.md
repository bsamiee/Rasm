...
public sealed partial class PerceptualColor {
    ...
    public (byte Red, byte Green, byte Blue, double Alpha) ToRgb() =>
        AsUnicolour().MapToRgbGamut(GamutMap.OklchChromaReduction).Rgb.Byte255.Clipped switch {
            { } clipped => ((byte)clipped.R, (byte)clipped.G, (byte)clipped.B, Alpha),
        };
    public (double Red, double Green, double Blue, double Alpha) ToRgb(RgbProfile profile) =>
        AsUnicolour().ConvertToConfiguration(profile.Configuration).MapToRgbGamut(GamutMap.OklchChromaReduction).Rgb switch {
