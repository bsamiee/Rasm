...
- [04]-[DERIVATIVE_ROWS]: plane contract, raster spec roster, per-row product.
...
- Output: `Disk.Intake` — `ObjectStore.Landed` beside the file coordinate, `{ key, bytes, written, path }` — tying filesystem path to durable key.
...
- Law: intake never buffers the file — identity is content-addressed, so the key cannot exist before the last byte is hashed, and intake is therefore TWO bounded streaming passes over the seekable file: `fs.stream(path)` feeds the `Ingest` identity fold, then a fresh `fs.stream(path)` feeds the streaming conditional put — constant memory at any size; a `readFile` single-pass intake is the memory defect the pipeline already bans.
...
- Law: the watch stream is admission, not truth — a watched drop directory emits candidate paths, each admitted through the same gated intake, and every candidate settles as an `Either` element on the success channel: `Either.right` the `Disk.Intake` (412 dedup included), `Either.left` the candidate's own refusal, `ObjectFault` from the filesystem and store legs or `HookVeto` from an app policy — so one malformed file never ends the long-lived source, and only watcher transport failure fails the stream itself.
...
- Law: direct `node:fs` imports are banned on this plane — capability rides the Tag, the tracing and error channel come with it; chokidar and the platform binding are the only places a filesystem module name exists.
- Law: the gated intake carries the `rasm.data.object.admit` hook point — the veto runs on the path candidate with its `stat` size before a byte is hashed, so app policy refuses at the admission boundary, and the observe fan runs on the landed value after the reference row commits; both compose the optional registry, so a composition without hooks pays nothing.
...
- Law: `HookVeto` rides the intake channel WHOLE and never folds into `ObjectFault` — the veto's `denied` class is caller-blamed and non-retryable, the only `ObjectFault` reason a policy refusal fits is `io`, and `io` classes `unavailable`, the arm the recovery policy re-drives; re-spelling hands an armed app policy to that retry loop, so intake states `ObjectFault | HookVeto` and each fault keeps its own family's class.
...
const _sealed = (path: string) =>
  Effect.gen(function* () {
    ...
    const identity = yield* Ingest.identity(Ingest.chunked(flow, Ingest.cut))
...
const _intake = (path: string, retention: Retain.Class, owner?: string) =>
  Effect.gen(function* () {
    ...
    const landed = yield* _sealed(path)
    yield* store.refer(landed.key, custodian, retention)
    yield* Hook.tapped("objectAdmit", { key: landed.key, owner: custodian, bytes: Option.some(landed.bytes) })
    return landed
...
const _egress = (key: Digest.Key<"content">, path: string) =>
  Effect.flatMap(FileSystem.FileSystem, (fs) => Stream.run(Ingest.range(key), fs.sink(path))).pipe(
...
- Output: `Derive.Product` — `{ name, key, grant, info, dominant, measure }` — the row name, the derivative's own content key, the presigned `ObjectStore.Grant` its row's policy asked for, the codec provenance, the optional placeholder color seeded from `stats().dominant`, and the source measures the encode consumed.
...
- Law: the analysis record drives three decisions and rides the product — `dominant` seeds the placeholder color, `isOpaque` retires a redundant alpha channel under `alpha: "opaque"`, and `entropy` selects the row's quality off a declared descending ladder so a flat plane and a photographic plane stop sharing one hardcoded number; `sharpness` is the third measure and rides the product beside them, so a consumer reads WHY a row took its grade without re-running the analysis. Ladder read folds through `Option` — no rung met leaves the row's declared options untouched, so a partial ladder forges no quality number.
...
- Law: the grant is minted only where the row's policy asks — `Derive.Product.grant` is `Option<ObjectStore.Grant>` folded on the row's own `grant` column, because a presign every derivative pays for is a signature nothing reads until a serving edge consumes one; the row that declares a policy gets its URL, every other row carries `Option.none()` and its key.
...
- Entry: `Asset.pipe(sourceKey, rows)` after an image lands (a disk intake, an upload finalize) — the raster rows travel the same array as the container and `ktx` rows, so a caller mixing a thumbnail rendition with a KTX2 encode states one row array and reads one product array; re-running is a proven noop end to end because every re-put lands 412 and every grant re-mints against the same keys.
- Output: one `Derive.Product` per row; the batch's span carries source key, row count, and total encode span.
...
- Law: the row correlates its codec at the DECLARATION and the encode boundary takes sharp's own union — one annotated projection folds the grade substitution into `Parameters<Sharp["toFormat"]>[1]`, so a row's stated options stay type-correlated where a caller writes them and the boundary needs no cast where it consumes them.
...
- Law: legs partition the census by the surface that DECIDES — the codec gate, the native engine, and the engine-blind tail — so a refusal names its surface without re-deriving it from the stage.
...
const _fanout = <R extends Derive.Row, F, H, I, E, Env>(
  ...
  sourceKey: Digest.Key<"content">,
  rows: ReadonlyArray<R>,
): Effect.Effect<ReadonlyArray<Derive.Product<I>>, DeriveFault | E, Env | ObjectStore> =>
  Effect.gen(function* () {
    const store = yield* ObjectStore
