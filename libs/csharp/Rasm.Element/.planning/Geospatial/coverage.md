...
// The geospatial readings of an admitted kernel lattice, attached to the receiver this package does not own. The
...
public static class LatticeGeodesy {
 extension(CellLattice lattice) {
  ...
  // The fractional cell coordinate of a planar world sample, through the lattice's own stored inverse — invertibility
  ...
  // The placement's content projection: the twelve coefficients then the three-axis census.
...
// A RECORD-root union (the PropertyValue/TableRow/GraphMutation form), not the class-root [Equatable] [GRAPH_FAMILY]
// form CoverageGrid itself takes: this family is a transient READ RESULT with no stored member, no content-key
// contribution, and no diff to drill, so Thinktecture's record-generated structural equality is the whole equality
// requirement and stacking Generator.Equals here would buy a member drill nothing descends into. The private root
...
public sealed partial record CoverageGrid {
 ...
 // the fractional integer part is a cell's top-left corner, so cell (c,r) spans [c,c+1)×[r,r+1) and the containing
 ...
 // The CONTAINING in-bounds discrete cell for a planar world coordinate (the explicit Raster read): FLOOR(Fractional)
