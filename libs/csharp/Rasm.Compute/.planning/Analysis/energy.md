...
- [03]-[MODEL_BUILD]: `EnergySimulation.BuildModel` folds the graph into an in-process `OpenStudio.Model` and translates it to IDF across the SWIG boundary rails.
- [04]-[SIMULATION_RUN]: `EnergySimulation.Run` drives the EnergyPlus subprocess and folds `eplusout.sql` into one fact stream with its annual-completeness validity.
...
- Owner: `EnergySimulation.BuildModel` the in-process OpenStudio model builder; `OsmBuild` the build receipt (IDF path with translator-and-skip log facts); `ConfigureRun`/`SetpointSchedules`/`InternalLoads`/`Condition`/`BuildSurface`/`BuildOpenings`/`BuildConstruction`/`Layer`/`Vertices` the model-object folds; the SWIG `Optional<T>`→`Fin<T>`, `IDisposable`, and `Path` boundary discipline.
