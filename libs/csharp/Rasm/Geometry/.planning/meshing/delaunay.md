...
- [1]-[TESSELLATION]: `Tessellation` `[Union]` (`Triangulation`/`Tetrahedralization`) over one `SimplexStore`; the Bowyer-Watson `Insert` cavity fold driven by exact `InCircle`/`InSphere` predicates; constraint segment/facet `Recover` by predicate-guarded flips; `ToMesh` projection.
