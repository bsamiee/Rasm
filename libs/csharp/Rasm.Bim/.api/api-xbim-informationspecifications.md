...
- schema/classification/property: the `IfcTypeFacet`/`IfcClassificationFacet`/`IfcPropertyFacet` match fields resolve against the `Semantics/classification#CLASSIFICATION_AXIS` axis and the `Semantics/properties#PROPERTY_SETS` keyed store the `validation#IDS_FACETS` fold reads as settled vocabulary — the `ValueConstraint` `DataType`/`NetTypeName` coercion uses the IFC measure metadata (`Helpers.Measures`), reconciled with the `UnitsNet` SI coercion the property store owns, never a re-derived measure table
