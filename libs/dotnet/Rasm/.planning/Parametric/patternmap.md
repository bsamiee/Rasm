...
- [02]-[PATTERNING]: `PatternLattice` the five-row basis-law vocabulary; `PatternSeat` + `WallpaperGroup` the 17 theorem-closed Seitz rows over ONE seat generator; `SymmetryFold`/`MirrorRight`/`MirrorGrant`/`MaterialSymmetry` the derived material-legality algebra; `PatternPlan`/`PatternPolicy` the orbit and mapping rows; `PatternOp` the two-case request `[Union]` folded by ONE `Apply`; `InstanceStream` the planar/mapped result `[Union]`; `PatternReceipt` the honesty evidence; the orbit, log-field, locate, and transport kernels.
...
public abstract partial record InstanceStream {
    ...
    public sealed record Mapped(
        ...
        Arr<double> Radius, Arr<double> FrameDefect, PatternReceipt Receipt) : InstanceStream;
}
