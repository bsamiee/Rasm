...
[NOTIFICATION_POLICY_ROUTE]: `NotificationPolicyPolicy.{activeTimings?,contactPoint?,continue?,groupBies?,groupInterval?,groupWait?,matchers?,muteTimings?,policies?,repeatInterval?}` — `groupBies` optional per route and inherited from the parent when absent, `matchers` an array of `{label,match,value}` all required, and the nesting bottoms out at five route levels because each depth carries its own generated interface named by one more `Policy` suffix
[NOTIFICATION_POLICY_LEAF]: the fifth level drops `policies?` entirely and re-REQUIRES `groupBies`, so the deepest route is a terminal leaf that cannot nest and must name its grouping — a tree built by one recursive projection over a uniform route type fails at depth five on both counts, and the fold either bounds itself at four or special-cases the leaf
