...
- `HatchPattern` re-declares the user-string roster `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns.
...
- `Linetype` re-declares the user-string roster `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Point3d`/`Plane`/`Line`/`Transform` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): table `bool`/`int` outcomes project to `Fin<Unit>`/`Fin<int>`; nullable `Find*`/`GetBoundaryLinetype` reads lift to `Option<A>`; `Create -> Hatch[]` and roster reads (`InstalledFonts`, `GetDefaultHatchPatterns`, `ReadFromFile`) land as `Seq<A>`; `GetSegment`/`InUse`/`CreateDisplayGeometry` `out` results fold into `Fin<A>` carrying the payload only on success.
...
- `Rasm` kernel: hatch planes, base points, pattern transforms, dash lengths, taper points, and font point sizes compose the kernel numeric and unit owners; every `Color`-valued fill, boundary, and hatch field composes the kernel color owner, never a host channel average.
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-annotation.md`: a `DimensionStyle.Font` and an embedded linetype shape resolve here; `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-display.md` draws hatches (`DrawHatch`) and builds a `DisplayPen` from a `Linetype` (`FromLinetype`).
