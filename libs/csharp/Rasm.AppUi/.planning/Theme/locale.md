...
- Owner: `LocalePolicy` user-settings options record; `ResolvedLocale` resolve product; `LocaleRuntime` atomic locale cell carrying the composition-bound `Propagate` column.
- Entry: `public Fin<Unit> Apply(LocalePolicy policy)` — `Fin` aborts on an unresolved tag or zone; one swap publishes the full resolved set and runs `Propagate` so every culture-holding surface outside the cell follows the same publication.
...
public sealed record LocaleRuntime(Atom<ResolvedLocale> Cell, IDateTimeZoneProvider Zones, Func<ResolvedLocale, Unit> Propagate) {
    public static Fin<LocaleRuntime> Boot(LocalePolicy policy, IDateTimeZoneProvider zones, Func<ResolvedLocale, Unit> propagate) =>
        Compose(policy, zones).Map(resolved => (propagate(resolved), new LocaleRuntime(Atom(resolved), zones, propagate)).Item2);

    public ResolvedLocale Current => Cell.Value;

    public Fin<Unit> Apply(LocalePolicy policy) =>
        Compose(policy, Zones).Map(resolved => ignore((Cell.Swap(_ => resolved), Propagate(resolved))));
