...
- Packages: `ifcopenshell` (`open`/`file.from_string`/`guess_format`/`geom.settings`/`geom.iterator`/`geom.serializers.gltf`/`geom.serializers.buffer`), runtime (`ServerHost`/`ContentIdentity`/`LanePolicy`/`ReceiptContributor`/`RuntimeRail`).
...
class TessellationDaemon:
    ...
    def semantic(self, model: "ifcopenshell.file") -> dict[str, str]:
        return {"schema": model.schema_identifier, "project": model.by_type("IfcProject")[0].Name or ""}
```
