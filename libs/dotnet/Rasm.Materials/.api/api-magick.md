...
- One composing site: `Raster/codec#RASTER_CODEC` `RasterEngine.Breadth` is this package's sole boundary — the row is reached only where the sniffed container matches no managed row, and no other page spells an `ImageMagick` member. A second composing site re-opens the single-boundary pin before it lands.
