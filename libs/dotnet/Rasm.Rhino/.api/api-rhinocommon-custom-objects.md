...
This catalog is the sole member-depth home for the whole `Rhino.UI` in-viewport widget family; `libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md` registers it and tables none of it.
...
- `Rhino.DocObjects.RhinoObject.OnSelectionChanged() -> void` / `GetBoundingBox(RhinoViewport viewport) -> BoundingBox` / `GetTightBoundingBox(ref BoundingBox tightBox, bool growBox, Transform xform) -> bool` — selection and view-dependent extent hooks.
- protected-virtual closure: `OnDraw`, `GetBoundingBox`, `GetTightBoundingBox`, `OnDuplicate`, `OnDeleteFromDocument`, `OnAddToDocument`, `OnPick`, `OnPicked`, `OnSelectionChanged`, `OnTransform`, `OnSpaceMorph` — no `OnGeometryChanged`, `OnAttributesChanged`, or `OnFlash` exists.
- PUBLIC-virtual closure, equally overridable and equally part of the authoring surface: `IsActiveInViewport(RhinoViewport viewport) -> bool`; the render-mesh cache family `IsMeshable(MeshType meshType) -> bool` / `MeshCount(MeshType meshType, MeshingParameters parameters) -> int` / `CreateMeshes(MeshType meshType, MeshingParameters parameters, bool ignoreCustomParameters) -> int` / `DestroyMeshes(MeshType meshType) -> void` / `GetMeshes(MeshType meshType) -> Mesh[]`; the provider-parameter pair `GetCustomRenderMeshParameter(Guid providerId, string parameterName) -> IConvertible` / `SetCustomRenderMeshParameter(Guid providerId, string parameterName, object value) -> void`; and the carrier accessors `Geometry -> GeometryBase` / `Attributes -> ObjectAttributes` / `ShortDescription(bool plural) -> string`.
- `BrepObject`, `CurveObject`, `MeshObject`, and `PointObject` override only the internal commit hook, and the four `Custom*Object` bases override only `Dispose(bool)`, so every virtual above reaches a custom derivation UNSEALED and a `sealed override` on any of them is admissible.
- `HasCustomRenderMeshes` / `CustomRenderMeshesBoundingBox` / `RenderMeshes` are NON-virtual RDK delegators taking a viewport, plug-in, and display-pipeline context; no `OnGetRenderMeshes` override point exists, so custom render-mesh supply is a `Rhino.Render.CustomRenderMeshes.RenderMeshProvider` registration, never an object hook.
...
- `Rhino.UI.UserInterfaceSlider.ValueChanged -> EventHandler` / `OnValueChanged() -> void` (protected virtual) / `ValueAsFormattedString() -> string` — the change event, its override hook, and the precision-formatted render.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Line`/`Transform`/`BoundingBox`/`Interval` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): each override hook returns `Fin<Unit>` or `Fin<A>` so a custom-object program is a total record of result-returning delegates; grip topology reads fold to `Option<GripObject>` and `Seq<A>`, and the disposable `CustomObjectGrips` rides a leased using-scope.
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-runtime-extensions.md`): the grip-widget shape vocabulary wraps as a keyed `SmartEnum`; draw marks, transform policies, and the four custom-object geometry kinds collapse into generated unions owning total dispatch inside the override program.
