...
`psd-tools` owns in-process Adobe PSD/PSB parse, inspect, blend-faithful composite, structural authoring, and round-trip for the artifacts layered-raster domain. `PSDImage` is the document root and top-of-tree `GroupMixin`; every node discriminates over the closed `Layer` family, and `composite`/`topil`/`numpy` egress its pixels, masks, effects, and text across the Pillow/NumPy boundary. `export/layered` composes it as the SOLE native `.psd`/`.psb` owner — the `_psd` arm authors the channel-stack document and that same owner reopens the finished bytes as structural evidence — never re-implementing the record parser, compositor, RLE/ZIP codec, or PIL/NumPy bridge it owns, nor re-authoring the IFC/semantic models the C# boundary holds.
...
|  [04]   | `PSDImage.save(fp, mode='wb', **kwargs) -> None`                               | instance | encode to a path or binary sink         |
|  [05]   | `PSDImage._record.write(fp)`; `image_data.set_data(channels, header)`          | record   | write the record to a path or sink      |
...
|  [16]   | `pil_io.convert_layer_to_pil(...) -> Image.Image \| None`          | static   | lower PIL boundary (ICC/mode)        |
...
|  [19]   | `numpy_io.get_array(layer_or_psd, channel, ...)`                   | static   | lower NumPy band boundary (batch)    |
...
- `create_pixel_layer`/`create_group` mint-and-attach, `PixelLayer.frompil`/`Group.new`/`group_layers` mint detached onto a named `parent`, and `move_to_group`/`move_up`/`move_down`/`delete_layer`/`lock`/`unlock` own relocation and lifecycle. `frompil` carries pixels, `top`/`left`, and `compression` only — `blend_mode`, `opacity` (0–255), `visible`, and `lock(ProtectedFlags...)` land as post-construction state on the minted node, and each survives the save/reopen round trip.
- DOCUMENT MODE selects where layer alpha lands, and the choice is not recoverable afterward: under an `RGB` document a `frompil` of an RGBA image diverts the alpha into the layer MASK slot — a later `create_mask` then refuses, the mask already taken, and the transparency channel reads back wrong — while an `RGBA` document rides that alpha on the transparency channel with no mask and round-trips byte-exact. An alpha-bearing layer stack authors into `PSDImage.new(mode='RGBA', ...)`.
- DIMENSION law splits by TIER, and the split is what makes PSB reachable. The api tier — `save`, `composite`, `topil`, `numpy` — routes through `check_pixel_size`, which applies the PSD 30000 px per-axis cap to PSB documents ALIKE (the upstream comment at `api/utils.py` says so outright), so `PSDImage.save` refuses a lawful PSB canvas inside its merged-preview refresh even though `new` already derived version 2 for it. The encode tier enforces the TRUE Adobe bound, `_MAX_DIMENSION` at 300000 per axis. A document past 30000 therefore authors normally and writes through the record tier, never through `save`.
- `save` takes a binary file object, but it refreshes the merged preview through `composite` whenever the tree `is_updated()` — every authored document — so it both re-does work a compositing caller already owns and imports the api-tier cap. The repo save tail is instead `image_data.set_data(channels, header)` with a caller-supplied preview followed by `_record.write(fp)` on the documented `psd_tools.psd` low-level tier: ONE tail for PSD and PSB, no version branch, the bytes never touching the filesystem.
- DEPTH reach is bounded by the PIL ingress, not the record writer: `frompil` admits pixels only as a `PIL.Image.Image`, and PIL carries no multichannel plane past 8 bits, so an alpha-bearing layer stack authors as the display-referred 8-bit family. A `depth=16` document does not round-trip an 8-bit source exactly and a `depth=32` document returns NaN planes; deep planes belong to the TIFF/ORA/EXR owners.
...
- `save` re-emits the whole tree with a per-channel `Compression`; `ZIP`/`ZIP_WITH_PREDICTION` deflate through stdlib `zlib` and RLE rides the bundled `_rle.abi3.so`, so every method is reachable on any supported interpreter. `ColorMode` is the document color model, so a CMYK or LAB PSD round-trips and composites in its native space through the `pil_io.get_pil_mode(color_mode, alpha)` bridge.
- Parse/IO failure raises ordinary exceptions; the two `UserWarning` subclasses are non-fatal size signals lifted to the layered-export fault channel once at the boundary.
...
- compression (self-contained): `Compression.ZIP`/`ZIP_WITH_PREDICTION` deflate through stdlib `zlib` and `RLE` rides the bundled `_rle.abi3.so` — psd-tools carries its channel codecs end to end and routes none through `imagecodecs`/`numcodecs`, so all four methods are reachable with no capability probe, unlike the layered-TIFF leg that must detect its backend.
- `expression`(`.api/expression.md`): `LayeredExport.of` returns `Result[LayeredExport, ExportFault]`; a readback failure converts to the closed boundary fault and the verified bytes and layer count return as `LayerFact`.
- `msgspec`(`.api/msgspec.md`): `Layer` is the frozen typed authoring row the `_psd` arm folds into `PixelLayer.frompil` plus the post-construction attribute set, and `PSDImage.open` verifies the finished tree by layer name — no parallel DTO or runtime type decorator.
...
- within-lib: `export/layered` is the primary consumer — its `_psd` arm authors the document through `PSDImage.new`/`Group.new`/`PixelLayer.frompil`, seeds the merged preview from its own `pyvips` composite, writes the record to a `BytesIO` sink, then reopens ITS OWN bytes with `PSDImage.open(max_alloc_bytes=...)` and proves every authored layer addressable through `find`; author and readback share one `LanePolicy.offload(Kernel.of(..., KernelTrait.HOSTILE), ...)` crossing, only the finished bytes-and-evidence `LayerFact` returning to the async caller.
