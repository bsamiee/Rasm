...
- Skill bundles are judged under the skill-writer craft law: third-person trigger descriptions without keyword stuffing, `SKILL.md` a lean router with reference banks one hop down, and no body rule whose deletion changes no agent output; a bundle whose every invocation consumes its whole reference set stays flat, and its mandatory whole-set load is the ruled design, never a partial-loading finding.
- `allowed-tools` is a designed grant judged by the documented substitution contract and the bundle's own consumption: `${CLAUDE_PROJECT_DIR}` is the documented allowed-tools substitution while `${CLAUDE_SKILL_DIR}` binds skill content and bash injection only, and a tool the bundle's mandatory reference reads or ruled rails consume is load-bearing — a portability swap or minimization demand against either voids the grant it edits.
- A `SKILL.md` root sentence carrying an exact member spelling, flag roster, signature, or field tuple re-teaches mechanism one hop up — the root states rulings and routes; a frontmatter `description` enumerating the bundle's internal files or section labels is stale on first rename and carries zero trigger power.
- A bundled `templates/` file carries slot tokens at every fill region, zero finished domain logic, and project-agnostic placeholders; a slotless worked instance under `templates/` is misfiled, its finished body owed to `examples/` or the owning reference. A template's header states its fill-and-replace contract in the bundle's sibling-consistent schema — never narration to delete.
- A declared parameter, slot, or branch in a bundled template or example that no dispatch route makes live is dead capability — the finding demands the wiring or the deletion.
