...
class TemplatePipeline(Struct, frozen=True):
    ...
    def of(cls, node: DocumentNode, /, **raw: Unpack[TemplatePayload]) -> Result["TemplatePipeline", TemplateFault]:
        try:
            # STRICT admission (shapes.md `[SHARED_REFINEMENT]` ingress law): the payload is typed keyword
            # material, so a coercible-but-wrong binding (a raw `"pdf/a-3b"` where `PdfVariant` is declared, a
            # `1` where `trusted: bool` is) is REFUSED at the seam rather than silently coerced into a
            # surprising interior value; the `extra_items=object` render band is unconstrained (`object`
            # admits every value), so strictness bites only the declared, precisely-typed keys.
