...
- [CREDENTIAL_SETTINGS] [RESOLVED]: `Settings.from_dict(config: dict) -> Settings` / `Settings.from_json(json_str: str) -> Settings` / `Settings.set(path, value)` / `Settings.update(data)`, `Context.builder() -> ContextBuilder`, and `ContextBuilder.with_settings(settings: Settings).with_signer(signer: Signer).build() -> Context` verify against the installed `c2pa-python 0.36.0` (the `c2pa.__all__` `Settings`/`ContextBuilder` public set, `Context` confirmed a `ContextProvider` subclass so it feeds `Builder.from_json(context=)`/`Reader.try_create(context=)`). The page formerly threaded a per-case `context: Context | None` treated as an OPAQUE campaign value — but without a configured trust-anchor set the `validation_state`/`validation_results` the `CredentialEvidence` carries are UNANCHORED, and credential was the lone `exchange/` producer with no `pydantic-settings` deployment owner where `exchange/detect#DETECT` `DetectSettings` and `exchange/metadata#METADATA` `MetaSettings` both admit one — an env-admission asymmetry. `CredentialSettings(BaseSettings)` (`RASM_CREDENTIAL_` prefix, verified against the shared `pydantic-settings` `.api` — `BaseSettings`/`SettingsConfigDict(env_prefix=, frozen=, extra=)`) admits the trust-list paths (`trust_anchors`/`trust_config`/`allowed_list`, read to PEM content), the default RFC-3161 `ta_url`, and the verify options (`verify_trust`/`verify_timestamp_trust`/`remote_manifest_fetch`/`ocsp_fetch` — all confirmed ACCEPTED keys on the installed `Settings.from_dict` c2pa-rs `verify` section) ONCE at the composition root, its `context()` projecting one configured verify `Context` via `Settings.from_dict({"verify": {...}, "trust": {...}})` + `Context.builder().with_settings(settings).build()` that each Sign/Read/Embed case threads. The `trust` section folds in only when a trust file is configured (an empty-string anchor faults the COSE certificate parse on the installed core, verified), so a no-trust deployment carries the verify policy alone. `Reader.with_fragment` takes no `context`, so the `ReadFragment` asymmetry is structural. The reasoned rejections (`detailed_json`/`crjson`/`reserve_size`/whole-builder-archive) stay rejected.
