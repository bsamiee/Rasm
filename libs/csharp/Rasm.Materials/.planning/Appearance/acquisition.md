...
public sealed record Provenance(string Device, int WavelengthCount, double FitResidual) {
    ...
    // The model attribution a neural capture carries and every other capture genuinely lacks. TYPED ABSENCE, never
    // an empty-string sentinel: a goniophotometer row has no model card, and a reader distinguishing "no model" from
    // "a model whose id happened to be blank" is exactly what an Option makes unnecessary. The pair travels together
    // because a licence is meaningless without the artefact it governs, and the wire flattens both to their keys.
...
public abstract partial record CaptureSource {
    ...
    // The photo-to-PBR PLANE product: an admitted Raster/set#TEXTURE_SET assembled from the neural#MODEL_REGISTRY
    // stage plan's outputs, beside the Seq<StageResult> evidence that produced it. Distinct from SvbrdfMap by what
    // SURVIVES: SvbrdfMap is a per-texel parameter field that collapses to one row and is then discarded, while this
    // arm keeps the planes — the acquired material stays spatially varying and the averaged row is the SUMMARY the
    // seam AppearanceSummary and the LOD fallback read, not the whole result. Fallback is the caller's own base row
    // for every column the set does not carry, so the arm names no library key and hardcodes no neutral.
...
public static class Acquisition {
    ...
    public static Fin<AcquiredMaterial> Import(CaptureSource source, Provenance provenance, Op key) =>
        source.Switch(
    ...
    // The PLANE arm closes the round trip photo → stages → planes → SHADEABLE MATERIAL: the admitted set binds
    // through the ONE Raster/set#SET_BIND Average target, whose measured per-channel fold over each plane's base
    // level is the summary the seam key carries — never a pyramid tail, never a re-derivation here — and the set
    // rides back beside it so the caller keeps the spatially-varying material rather than only its mean.
    ...
    static Provenance Attributed(Provenance provenance, TextureSet planes, Seq<StageResult> stages) =>
        stages.Choose(static result => ModelRegistry.Rows.TryGetValue(result.ModelCard, out ModelCard? card) ? Some(card!) : Option<ModelCard>.None)
        ...
        switch {
            var governing => Provenance.Of(CaptureMethod.NeuralPlanes, planes.Digest.ToValue(), provenance.WavelengthCount,
        ...
        guard(samples.Count > (conductor.IsSome ? 2 : 3), MaterialFault.Parameter(key, $"<measured-brdf-underdetermined:{samples.Count}>")).ToFin()
            ...
            .Bind(fit => SpectralUpsample.ToSpd(
                ...
                .Map(row => AcquiredMaterial.Summary(
                             row with { Ior = fit.Eta, Anisotropy = fit.Anisotropy },
