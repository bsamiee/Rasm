...
- Entry: `public static SpatialDelta Diff(OpLogEntry old, OpLogEntry next)` reads the op-log OLD/NEW geometry pair into a typed `SpatialDelta` discriminating enter (null OLD), exit (null NEW), move (centroid shift over threshold), and reshape (boundary change at equal centroid); the `SpatialOp` rows project their translated predicate through the catalogued `DbFunctions` per row.
