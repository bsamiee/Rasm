...
- `anyio` rail: drawpyo is pure-Python and fully synchronous; the owning page offloads the `File.write`/`File.xml` serialization through the instance `lane.offload(..., modality=Modality.THREAD)` seam off the event loop (the same shared-address-space thread arm the `drawsvg` `_render` and `numpy`-touching siblings take), so a large multi-page `.drawio` serialization never blocks the loop while the `msgspec` receipt owner stays event-loop-side.
