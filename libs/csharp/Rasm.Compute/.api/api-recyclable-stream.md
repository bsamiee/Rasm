...
- `Runtime/codecs#TILE_PARTITION`: tileset-manifest emit writes a `Utf8JsonWriter` straight into a rented stream, commits each node past the writer's own `BytesPending` watermark, and keys off `GetReadOnlySequence()` through the incremental identity above — so a policy-depth octree rides chained blocks rather than a growable writer's doubling ladder through the large-object heap.
