...
from tests.python.testkit.runtime import PROFILE_STATEFUL
...
class ValidityCase[T](msgspec.Struct, frozen=True, gc=False):
    """Case carrier for ``validity_matrix``: the label surfaces on failure."""
...
class ProjectionCase[I](msgspec.Struct, frozen=True, gc=False):
    """Case carrier for ``projection_matrix``: ``oracle`` computes the reference when present, else ``supported_out``."""
...
class MetamorphicRelation[T, R](msgspec.Struct, frozen=True, gc=False):
    """One ``metamorphic_sweep`` relation: ``transform`` derives the follow-up input, ``relate`` asserts the output relation."""
...
def refutes[T](witness: T, law: Callable[..., None], *args: object, **kwargs: object) -> None:
    """Assert ``law(witness, *args, **kwargs)`` raises ``AssertionError``; a surviving witness names the law a tautology.
...
def validity_matrix[T](cases: Iterable[ValidityCase[T]] | Iterable[tuple[str, T, bool]], valid: Callable[[T], bool], *, subtests: RowCarrier | None = None) -> None:
    """Assert each case's validity verdict; ``subtests`` reports each breach independently instead of stopping the fold."""
    folded = 0
    for raw in cases:
...
def support_matrix(*rows: tuple[str, Callable[[], bool], bool], subtests: RowCarrier | None = None) -> None:
    """Assert each ``(label, probe, expected)`` row; ``subtests`` reports each breach independently instead of stopping the fold."""
...
def projection_matrix[I](cases: Iterable[ProjectionCase[I]], project: Callable[[I], object], *, subtests: RowCarrier | None = None) -> None:
    """Assert each case's projection outcome; ``subtests`` reports each breach independently instead of stopping the fold."""
    folded = 0
    for case_ in cases:
        folded += 1
        with _row_scope(subtests, case_.label):
...
def assert_ok[T, E](result: Result[T, E], *, then: Callable[[T], None] | None = None) -> T:
    """Assert ``Ok`` and return the inner value, running ``then`` over it; an ``Error`` surfaces its payload.

    Raises:
        AssertionError: When the result is ``Error`` or an unexpected shape.
    """
    match result:
...
def assert_error[T, E](result: Result[T, E], *, then: Callable[[E], None] | None = None) -> E:
    """Assert ``Error`` and return the error, running ``then`` over it; an ``Ok`` surfaces its value.

    Raises:
        AssertionError: When the result is ``Ok`` or an unexpected shape.
    """
    match result:
        case Result(tag="error", error=e):
            if then is not None:
...
def assert_some[T](opt: Option[T], *, then: Callable[[T], None] | None = None) -> T:
    ...
    Raises:
        AssertionError: When the option is ``Nothing`` or an unexpected shape.
    """
    match opt:
...
def assert_none(opt: Option[object]) -> None:
    ...
    Raises:
        AssertionError: When the option is ``Some`` or an unexpected shape.
    """
    match opt:
        case Option(tag="none"):
            return
...
def assert_roundtrip[T](value: T, typ: type[T], *, codec: msgspec.json.Encoder | msgspec.msgpack.Encoder | None = None) -> T:
    ...
    The re-encode step catches non-deterministic codecs that structural equality misses; the decode lane derives from
    the encoder family, so one oracle proves both wire legs — JSON by default, MessagePack under a msgpack ``codec``.
    """
...
def model_based[M: RuleBasedStateMachine](machine_cls: type[M], *, profile: str | None = None, settings: hyp_settings | None = None) -> None:
    """Drive a stateful Hypothesis machine under explicit ``settings``, a named ``profile``, or the stateful lane."""
