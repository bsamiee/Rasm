...
- [MESHIO_INTERCHANGE]: the `meshio.read(path, file_format)`, `meshio.write(path, mesh, file_format)`, `meshio.Mesh(points, cells, point_data, cell_data, field_data, point_sets, cell_sets)`, `meshio.CellBlock(cell_type, data)`, `Mesh.cells_dict`/`cell_data_dict`/`cell_sets_dict` the per-type-merged read views, `Mesh.point_data`/`cell_data`/`point_sets`/`cell_sets`/`field_data` the write-side block-parallel surface, the `Mesh.cell_sets_to_data()`/`point_sets_to_data()` promoters and the `cell_data_to_sets(key)`/`point_data_to_sets(key)` inverters, and the `ReadError`/`WriteError` failure types verify against `compute/.api/meshio.md` ([ENTRYPOINTS] `[01]`/`[02]`, `Mesh` members `[10]`/`[11]`/`[12]`/`[03]`/`[04]`/`[05]`/`[06]`/`[07]`/`[15]`/`[16]`/`[17]`/`[18]`, PUBLIC_TYPES `[03]`/`[04]`). The read inverts only the integer-kind columns (`np.issubdtype(..., np.integer)`) since `cell_data_to_sets`/`point_data_to_sets` invert an integer label field, so a float solution field round-trips through `cell_fields` rather than being force-inverted into sets. `cell_data` and `cell_sets` values parallel the `cells` block list as per-block array lists, so the read projects the per-type-merged `cell_data_dict[k][cell_type]`/`cell_sets_dict[k][cell_type]` views — the same `cell_type` key the merged `cells_dict[cell_type]` connectivity uses, guarded by `if cell_type in by_type` for a name that does not touch the element's block — rather than the block-0 `cell_data[k][0]` array (correct only on a single-block mesh) or a `next(...)`-over-blocks scan; the write wraps each array in a single-element list; the `[MESHIO_TOPOLOGY]` law mandates preserving block order and reading per-type connectivity through `cells_dict`/`cell_data_dict`/`cell_sets_dict`. `file_format=None` triggers extension detection while an explicit string disambiguates the ambiguous `.msh`/`.dat` extensions the `extension_to_filetypes` mapping flags. The four `*_to_data`/`*_to_sets` promoters are the round-trip the `[MESHIO_TOPOLOGY]` law names for carrying Gmsh/physical-group tags through integer-label formats: the write promotes named sets into integer fields outbound and the read inverts integer fields back into named sets inbound, so a region tag survives both directions rather than the write-only promotion that left the inbound recovery unimplemented. `meshio` is pure-Python and cp315-clean (the catalogue's `floor: unconditional manifest dependency`), so the interchange is a top-level import running unconditionally, never deferred behind `TYPE_CHECKING`; the `ReadError`/`WriteError` and skfem assembly raises convert once at the `boundary` fence through the `runtime/reliability/faults.md#FAULT` `CLASSIFY` fold.
