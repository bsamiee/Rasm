...
class Termination(StrEnum):
    # result-shape adjudicator carried as a route policy value, one table-driven `adjudicate` over
    # three host-result shapes: `CODED` reads the integer `.status`, `FLAGGED` the boolean `.success`,
    ...
    def adjudicate(self, result: "opt.OptimizeResult | None") -> SolveStatus:
        # `FEASIBLE` arm folds `SUCCESS` over the `None` assignment carrier; `CODED`/`FLAGGED` ride a
...
class ProgramSolve(Struct, frozen=True):  # GC-tracked: carries the host `OptimizeResult` and assignment tuple
    # raw host carrier and nothing derived: the iterate and objective are read LAZILY off the
...
class ProgramRoute(Struct, frozen=True):  # GC-tracked: carries the entry/iterate/carriers closures
    # buffer-and-key projection is NOT a route column: `_project` owns every per-tag `Carried`
...
def _program_receipt(intent: ProgramIntent, seed: int) -> "RuntimeRail[OutcomeReceipt]":
    ...
    # iterate and objective are read only on SUCCESS, so a non-success carrier's `None` fields are
