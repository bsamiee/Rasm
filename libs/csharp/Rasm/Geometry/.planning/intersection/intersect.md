...
- Packages: `Rasm`/Vectors (`Point3d`/`Vector3d`/`Line`/`Plane`/`Polyline`/`MeshSpace` — composed for primitive geometry and the result projection), `Rasm.Geometry.Numerics` (`Predicate` `Orient2D`/`Orient3D`, `Sign`, `Lpi` homogeneous line-line construction, `Expansion` — the exact-sign straddle floor and the exact `OrderKey`, composed never re-minted), `Rasm.Geometry.Spatial` (`SpatialIndex`/`SpatialQuery` `Overlap`/`Range`/`Ray` broad-phase — composed, never re-minted), `Rasm.Geometry.Healing` (`MeshEdit.OfMesh` the triangulated soup carrier — the SAME healing working set, composed never re-minted), `Rasm.Geometry` (`GeometryKeyPolicy` string-key comparer, `GeometryFault` band-2400 union — composed, never re-minted), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox (`List<T>`, `Dictionary<TKey,TValue>`, `Stack<T>`).
...
// --- [MODELS] -----------------------------------------------------------------------------
public sealed record Crossing(Point3d Site, Expansion OrderKey, int FaceA, int FaceB) {
...
public sealed record IntersectStore(
    ...
    public static IntersectStore Empty(int capacity) =>
        new(new[] { 0 }, new Point3d[capacity], new Expansion[capacity], new int[capacity], new int[capacity], new int[capacity], new bool[capacity], new Stack<int>());

    internal int Add(Crossing crossing) {
        int slot = FreeList.Count > 0 ? FreeList.Pop() : Live[0]++;
        (Site[slot], OrderKey[slot], FaceA[slot], FaceB[slot]) = (crossing.Site, crossing.OrderKey, crossing.FaceA, crossing.FaceB);
    ...
    internal Crossing At(int slot) => new(Site[slot], OrderKey[slot], FaceA[slot], FaceB[slot]);
}
...
public static class Intersection {
    ...
    static (Crossing A, Crossing B) Order(Crossing first, Crossing second) =>
        Expansion.SignOf(Expansion.Difference(first.OrderKey, second.OrderKey)) == Sign.Positive ? (second, first) : (first, second);
    ...
    static Crossing LineLine(Line a, Line b, int faceA, int faceB) {
        ...
        return new Crossing(site, point.Lambda, faceA, faceB);
    }
    ...
    static Crossing PlanePoint(Line edge, Point3d a, Point3d b, Point3d c, int faceA, int faceB) {
        ...
        return new Crossing(site, order, faceA, faceB);
    }
...
| :-----: | :---------------- | :---------------- | :------------------------------------------------------------------------------------------------------------------- | :---------------------------------------- | :-----: |
|   [1]   | Intersection      | `IntersectOp`     | `[Union]` (`SegmentSegment`/`SegmentTriangle`/`TriangleTriangle`/`RayMesh`/`MeshMesh`/`PlaneMesh`) over one `IntersectStore` + `Apply` | `Intersection.Apply → Fin<IntersectResult>` |    6    |
|  [1a]   | Operation kind    | `IntersectKind`   | `[SmartEnum<string>]` six rows + emits-chain/is-mesh-level columns                                                    | discriminant (pure)                       |    6    |
|  [1b]   | Crossing carrier  | `Crossing`        | materialized `Point3d` `Site` (decided by exact `Orient3D`/`Orient2D` signs) + exact `Expansion` `OrderKey` + face provenance | carrier (`Round → Site`)         |    —    |
