...
- dependency: `NetTopologySuite` `[2.0.0, 3.0.0)` only (the `Geometry`/`Ordinates`/`CoordinateSequenceFactory`/`PrecisionModel` algebra — the 2.6.0 core is pinned); NO `NetTopologySuite.Features` dependency (this codec is geometry-only, attributes live in the SQLite row columns)
