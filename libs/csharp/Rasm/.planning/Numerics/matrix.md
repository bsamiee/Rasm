...
- Cases: `SparseMatrix` `FromTriplets` (duplicate-summing assembly) · `Multiply` · `ToDense` · `SolveDetailed`/`Solve` (BiCgStab + fallback) · `SingularSolveDetailed`/`SingularSolve` (gauge) · `SolveIndefiniteDetailed`/`SolveIndefinite` (SparseLU, pivot tol ∈ [0,1]) · `SmallestEigenpairsDetailed` (LOBPCG) · `NonZeros`; `SparseHermitian` `FromTriplets` · `Multiply` · `SmallestEigenpairsDetailed` (Hermitian LOBPCG); `CholeskySparse` `Of` · `SolveDetailed`/`Solve` · `FactorNonZeros`.
...
internal static class MatrixKernel {
    ...
    internal static Fin<SparseHermitian> AssembleHermitian(Dimension order, IEnumerable<(int Row, int Col, Complex Value)> triplets, Op op) {
        ...
        if (raw.Exists(static t => !double.IsFinite(t.Value.Real) || !double.IsFinite(t.Value.Imaginary)) || raw.Exists(t => t.Row < 0 || t.Col < 0 || t.Row >= order.Value || t.Col >= order.Value || t.Row > t.Col) || raw.Exists(static t => t.Row == t.Col && Math.Abs(value: t.Value.Imaginary) > EpsilonPolicy.SqrtEpsilon)) return Fin.Fail<SparseHermitian>(op.InvalidInput());
        ...
        if (upper.Exists(static t => !double.IsFinite(t.Value.Real) || !double.IsFinite(t.Value.Imaginary) || (t.Row == t.Col && Math.Abs(value: t.Value.Imaginary) > EpsilonPolicy.SqrtEpsilon))) return Fin.Fail<SparseHermitian>(op.InvalidResult());
...
#pragma warning restore CA1031
    ...
    private static Fin<GaugeStage> SolveKkt(Matrix<double> aSym, Matrix<double> massNullspace, LinearVector b, double operatorScale, Op key) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                if (Math.Abs(value: aSym[i, j]) > EpsilonPolicy.SqrtEpsilon * operatorScale) entries.Add(item: (i, j, aSym[i, j]));
        double constraintScale = Math.Max(val1: 1.0, val2: massNullspace.InfinityNorm());
        for (int i = 0; i < n; i++)
            for (int c = 0; c < m; c++)
                if (Math.Abs(value: massNullspace[i, c]) > EpsilonPolicy.SqrtEpsilon * constraintScale) { entries.Add(item: (i, n + c, massNullspace[i, c])); entries.Add(item: (n + c, i, massNullspace[i, c])); }
