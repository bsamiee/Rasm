...
- Anchors: `graph/nonmanifold.md#3-IFC_BYTES_CONSTRUCTOR`, `.api/topologicpy.md`, `Topology.ByIFCFile`, `Topology.ByIFCPath`, `Cluster.ByTopologies`, `ifcopenshell.file.from_string`, and the `<'3.15'` worker admission.
...
[KISS_MATCHER_FALLBACK_FGR]-[BLOCKED]: select the `open3d` Fast Global Registration bootstrap on the `>='3.13'` interpreter without `kiss-matcher` resolves.
- Capability: `ScanRegistration._engine` resolves the `BootstrapEngine` per engine selection — `KISS_MATCHER` below the `_KISS_FLOOR` `(3, 13)` anchor, `OPEN3D_FGR` at or above it — so the `GLOBAL` arm mints the initialization-free coarse pose on every interpreter the registration owner runs on, the FGR fallback seeding `small-gicp` fine refinement exactly as the `kiss-matcher` primary does.
...
- Anchors: `scan/registration.md#REGISTRATION` `BootstrapEngine.OPEN3D_FGR` arm / `_engine` floor read / `_KISS_FLOOR` `(3, 13)` anchor, `.api/kiss-matcher.md` (the cp38–cp312-only package floor, the marker-over-classifier rule), `.api/open3d.md` (`registration.registration_fgr_based_on_feature_matching`/`compute_fpfh_feature`/`estimate_normals`/`KDTreeSearchParamHybrid`), the `small-gicp` fine-refinement seed.
