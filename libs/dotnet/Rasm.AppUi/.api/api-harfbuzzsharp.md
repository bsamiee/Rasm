...
- `Tag.Parse` coerces silently, so tag admission validates the four-character shape at one owner and yields a typed result value; a tag string reaching `Feature` unvalidated is the escape this admission exists to close.
