...
- `Grpc.Net.Client`(`.api/api-grpc-client.md`): `IMessage<T>` payloads serialize on the call path under the channel's message-size bounds, and the `fault.FaultDetail{domain, case, correlation, stamp, tenant, recovery, violations}` detail crosses inside `google.rpc.Status.details` as one `Any` that `Any.Is(FaultDetail.Descriptor)` selects and `Unpack<FaultDetail>()` opens; it never serializes or rehydrates a source fault union.
