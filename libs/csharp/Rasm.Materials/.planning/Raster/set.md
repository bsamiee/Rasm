...
- Cases: ingest-source {`Stems` (a scanned directory's probes — the INFERENCE arm), `Declared` (a manifest of already-resolved rows — the VALIDATION arm: a C#-side re-admission or the `SetIngest.Roster` product), `Peer` (the decoded python-minted `Appearance.Set.pbr` arm — the `tests/contracts/manifest.json` classification-input consumer arm)} · refusal {`conventionDivergent`, `peerRoleDivergent`, `peerRowVocabulary`, `peerPackVocabulary`, `packPlaneNarrow`, `componentsRefuted`, `unclaimed`}.
...
public abstract partial record IngestSource {
    ...
    // tests/contracts/proto/rasm/contracts/appearance/set.proto (managed derivation from
