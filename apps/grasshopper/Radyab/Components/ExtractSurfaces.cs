...
public sealed class ExtractSurfaces : Component {
    public ExtractSurfaces() : base(self: typeof(ExtractSurfaces)) { }
    ...
    private IOutputGroup IndexedFace => Output.Details<FaceProjection>(
        input: Geometry,
        aspect: runtime => shape => Rasm.Analysis.Query.FaceProjections(geometry: shape.Inner, selector: Faces.At(index: runtime.Read(port: Index).ToNullable())),
