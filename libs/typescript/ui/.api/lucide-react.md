...
| :-----: | :-------------------- | :------------ | :-------------------------------------------------------------------------------------- |
|  [01]   | `LucideProps`         | interface     | `ElementAttributes & { size?, absoluteStrokeWidth? }`                                   |
|  [02]   | `LucideIcon`          | type alias    | `ForwardRefExoticComponent<Omit<LucideProps,'ref'> & RefAttributes<SVGSVGElement>>`     |
|  [03]   | `LucideConfig`        | type alias    | global default prop config shape for `LucideProvider`                                   |
|  [04]   | `LucideProviderProps` | type alias    | `LucideConfig & { children: ReactNode }`                                                |
|  [05]   | `IconNode`            | type alias    | `[elementName: SVGElementType, attrs: Record<string, string>][]`                        |
|  [06]   | `SVGAttributes`       | type alias    | `Partial<SVGProps<SVGSVGElement>>`                                                      |
|  [07]   | `ElementAttributes`   | type alias    | `RefAttributes<SVGSVGElement> & SVGAttributes`                                          |
|  [08]   | `SVGElementType`      | type alias    | `'circle' \| 'ellipse' \| 'g' \| 'line' \| 'path' \| 'polygon' \| 'polyline' \| 'rect'` |
...
| :-----: | :------------------------------------- | :------------- | :----------------------------------------------------------------- |
|  [01]   | `<IconName />`                         | icon component | any of the ~1 500 named `LucideIcon` exports                       |
|  [02]   | `Icon`                                 | dynamic icon   | `ForwardRefExoticComponent<IconComponentProps>` — takes `iconNode` |
|  [03]   | `createLucideIcon(iconName, iconNode)` | factory        | produces a `LucideIcon` from raw `IconNode` data                   |
...
| [INDEX] | [SURFACE]                                                                                | [ENTRY_FAMILY] | [RAIL]                                    |
| :-----: | :--------------------------------------------------------------------------------------- | :------------- | :---------------------------------------- |
|  [01]   | `LucideProvider({ children, size, color, strokeWidth, absoluteStrokeWidth, className })` | provider       | injects default `LucideProps` via context |
|  [02]   | `useLucideContext()`                                                                     | hook           | returns the active `LucideProps` defaults |
