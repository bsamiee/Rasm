...
`pyvips` owns the fused streaming-raster engine the `graphic/raster/io#IO` `RasterEngine.LIBVIPS` arm and the `graphic/color/managed#MANAGED` `_icc_apply` arm compose: one lazily-evaluated `Image` whose factory family loads a source, whose generated libvips-operation methods transform it demand-driven, and whose `write_to_*`/`numpy` egress computes the whole pass exactly once. Every arm crosses the `runtime/execution/lanes#LANE` process seam through its `lane.offload(Kernel.of(_worker, KernelTrait.HOSTILE), ...)` worker body — pyvips binds the `pyvips[binary]` extra's own bundled `libvips` dylibs (`pyvips-binary`, the kleisauke lineage sharp pins), so it rides `WORKER_BAND` like the other host-native packages — captures `pyvips.Error` into `RasterFault.engine` and the dlopen `OSError` of an unprovisioned `libvips` into `RasterFault.provision`, folds one `msgspec` `RasterFact`/`ManagedFact`, and projects `core/receipt#RECEIPT` `ArtifactReceipt.Preview`. It is the fused-pipeline / large-image / managed-ICC-egress owner where `pillow` (`.api/pillow.md`) is the in-process pure-Python working surface.
...
- egress: `write_to_buffer(format_string, **kwargs)` keys the encoder by `format_string` (`".png"`/`".jpg"`/`".webp"`/`".tif"`), and that ONE suffix literal is simultaneously the call spelling and its own `get_suffixes()` key — the `graphic/raster/io#IO` `_CODEC` row's libvips writer column carries it, the `graphic/color/managed#MANAGED` `_CODEC_OPTS` row its managed peer. Encoder options (`Q`/`compression`/`lossless`/`effort`/`subsample_mode`/`bitdepth`, `keep=ForeignKeep` for EXIF/ICC/XMP) ride `**kwargs` per the row's own `(quality, effort)` option builder. Containers this build registers no saver for — `.bmp`, `.jxl`, and `.qoi` on the current libvips — carry no suffix at all, so the row routes that engine to the `imagecodecs` array writer rather than handing `write_to_buffer` a spelling `get_suffixes()` never offered.
