...
- [1]-[TRANSACTIONAL_OUTBOX]: owns the same-txn `Outbox` `Model.Class`, the `FOR UPDATE SKIP LOCKED` relay loop, the `LISTEN`/`NOTIFY` wake, and the `DeliverySink` tagged axis.
