...
declare namespace Law {
    ...
    type ArgsOf<A extends Arbs> = {
        readonly [K in keyof A]: A[K] extends FastCheck.Arbitrary<infer T>
            ? T
            : Schema.Schema.Type<A[K]>;
    };
    ...
    interface Shape<S, A extends Arbs, E, R> {
        ...
        readonly predicate: (
            subject: S,
            args: ArgsOf<A>,
        ) => Effect.Effect<boolean, E, R | TestServices.TestServices>;
        readonly witness: {
            readonly label: string;
            readonly foil: S;
            readonly args: ArgsOf<A>;
        };
    }
...
class LawRefuted extends Data.TaggedError('LawRefuted')<{
    readonly law: string;
    readonly args: unknown;
}> {}

class LawTautology extends Data.TaggedError('LawTautology')<{
    readonly law: string;
...
): Effect.Effect<void, E | LawRefuted, R> =>
    Effect.flatMap(verdict, (held) =>
        held ? Effect.void : Effect.fail(new LawRefuted(evidence)),
    );

const _survives = <A, E, R>(
    work: Effect.Effect<A, E, R>,
): Effect.Effect<boolean, never, R> =>
    Effect.match(work, { onFailure: () => false, onSuccess: () => true });

const Law = {
    audit: <S, A extends Law.Arbs, E, R>(
        shape: Law.Shape<S, A, E, R>,
    ): Effect.Effect<void, LawTautology, R | TestServices.TestServices> =>
        Effect.flatMap(
            Effect.exit(
                shape.predicate(shape.witness.foil, shape.witness.args),
            ),
            (exit) =>
                Exit.match(exit, {
                    ...
                    onSuccess: (held) =>
                        held
                            ? Effect.fail(
                                  new LawTautology({
                                      law: shape.name,
                                      witness: shape.witness.label,
                                  }),
                              )
                            : Effect.void,
                }),
        ),
    make: <S, const A extends Law.Arbs, E = never, R = never>(
        shape: Law.Shape<S, A, E, R>,
    ): Law.Law<S, R> => ({
        name: shape.name,
        register: (api, subject) => {
            api.effect.prop(shape.name, shape.arbitraries, (args) =>
                _held(shape.predicate(subject, args), {
                    law: shape.name,
                    args,
                }),
            );
            api.effect(`${shape.name} refutes ${shape.witness.label}`, () =>
                Law.audit(shape),
            );
        },
    }),
    register: <S, R>(
        api: Law.Api<R>,
        subject: S,
        laws: ReadonlyArray<Law.Law<S, R>>,
    ): void => {
    ...
    associative: <A>(
        options: Law.Stock<
            Law.Binary<A>,
            { readonly a: A; readonly b: A; readonly c: A }
        > & {
    ...
    idempotent: <A>(
        ...
        Law.make({
            ...
            predicate: (combine, { a }) =>
                Effect.sync(() =>
                    (options.equals ?? Equal.equals)(combine(a, a), a),
                ),
            witness: options.witness,
        }),
    identity: <A>(
        ...
        Law.make({
            ...
            predicate: (combine, { a }) =>
                Effect.sync(() => {
                    ...
                    return (
                        alike(combine(options.empty, a), a) &&
                        alike(combine(a, options.empty), a)
                    );
                }),
            witness: options.witness,
        }),
    ...
    inverse: <A, B>(
        ...
        Law.make({
            ...
            predicate: (dual, { a }) =>
                Effect.sync(() =>
                    (options.equals ?? Equal.equals)(dual.from(dual.to(a)), a),
                ),
            witness: options.witness,
        }),
        ...
        Law.make({
            ...
            predicate: (subject, { input }) =>
                Effect.zipWith(
                    ...
                    subject(input),
                    (first, second) =>
                        (options.equals ?? Equal.equals)(first, second),
                ),
            witness: options.witness,
        }),
    homomorphic: <A, B>(
        ...
        Law.make({
            ...
            predicate: (to, { a, b }) =>
                Effect.sync(() =>
                    (options.equals ?? Equal.equals)(
                        to(options.combine(a, b)),
                        options.combineImage(to(a), to(b)),
                    ),
                ),
            witness: options.witness,
        }),
    monotone: <A>(
        ...
        Law.make({
            ...
            arbitraries: { a: options.arb },
            predicate: (step, { a }) =>
                Effect.sync(() => options.order(a, step(a)) <= 0),
            witness: options.witness,
        }),
    total: <I, E, R>(
        options: Law.Stock<
            (input: I) => Effect.Effect<unknown, E, R>,
            { readonly input: I }
        > & { readonly arb: FastCheck.Arbitrary<I> },
    ...
    roundtrip: <A, I>(
        ...
        return Law.make({
            ...
            predicate: (subject, { value }) =>
                Effect.match(
                    Effect.flatMap(
                        Schema.encode(subject)(value),
                        Schema.decode(subject),
                    ),
                    {
                        onFailure: () => false,
    ...
    machine: <Model extends object, Real>(
        ...
        Law.make({
            ...
            predicate: (setup, { run }) =>
                _survives(Effect.try(() => FastCheck.modelRun(setup, run))),
            witness: options.witness,
        }),
    machineAsync: <Model extends object, Real>(
        ...
        Law.make({
            ...
            predicate: (setup, { run }) =>
                _survives(
                    Effect.tryPromise(() =>
                        FastCheck.asyncModelRun(setup, run),
                    ),
                ),
            witness: options.witness,
        }),
    interleave: (
        ...
        Law.make({
            ...
            predicate: (subject, { schedule }) =>
                Effect.promise(() => subject(schedule)),
            witness: options.witness,
        }),
} as const;
