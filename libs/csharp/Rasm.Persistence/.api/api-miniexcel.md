...
- `Apache.Arrow`(`libs/csharp/.api/api-arrow.md`)/`DuckDB.NET`(`.api/api-duckdb.md`): `GetReader`'s `MiniExcelDataReader : IDataReader` is the row source the columnar materializer pulls, and the binary columnar path stays theirs — a workbook is never read as a columnar file.
