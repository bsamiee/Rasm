...
- Each dialog takes a caller-supplied parent, so the parent is the Rhino-owned window the host bridge resolves (`api-rhino-ui.md`), never a boundary-minted top-level window.
...
- The OS projection and the Rhino in-app meter (`api-rhino-ui.md` `StatusBar`) are one pair over one progress fact, not two owners: a long render publishes its fraction once and both sinks read it.

[STACKING]:
- `api-eto-drawing`(`../../.api/api-eto-drawing.md`): the page `Graphics` primitive set every `OnPrintPage` body composes, and the `RectangleF`/`SizeF` carriers the printable area and page size arrive as.
- `api-eto-forms`(`../../.api/api-eto-forms.md`): `Control`, `Window`, and `DialogResult` are the carriers the source control and both dialogs receive; the control tree itself is authored there.
- `api-eto-platform`(`api-eto-platform.md`): the `AttachNative`/`DetachNative` pair a control-sourced `Print` brackets internally; a boundary never nests its own attach inside a print job.
- `api-rhino-ui`(`api-rhino-ui.md`): the Rhino host marshal owner and the document-owned parent window — a print job and both dialogs cross the host thread there first, and the parent a dialog receives is resolved through the bridge.
- `api-eto-runtime`(`api-eto-runtime.md`): a background producer requesting a print marshals through the registered application dispatch before touching the job.
- `LanguageExt.Core`(`../../.api/api-languageext.md`): a print job composes as an effect — `PrintDocument.Print` and the `On*` callbacks fold into an `Eff<A>`/`IO<A>` pipeline whose per-page render is a step, and each `ShowDialog` result folds to `Fin<A>` so a cancelled `DialogResult` rides a typed rail. `PrintDocument` is resource-scoped through the `use` rail, its construction and disposal bracketing one scope; `Option<A>` lifts a null `PrintSettings` so an unconfigured job is `None`; the progress-fraction bound is a `Fin` guard, never a caught `ArgumentOutOfRangeException`.
...
- Reject: interactive windows and file dialogs (`../../.api/api-eto-forms.md`), the page `Graphics` primitive set (`../../.api/api-eto-drawing.md`), ambient application dispatch and clock (`api-eto-runtime.md`), a boundary-minted parent window or a print job off the host thread (`api-rhino-ui.md`), a boundary-side margin or copy default restating a host `[DefaultValue]`, and leaking a raw `PrintDocument` or `DialogResult` branch past the owning rail
