...
|   [5]   | Native-BLAS asset presence per RID (`Control.TryUseNativeMKL`/`TryUseNativeOpenBLAS` boolean probe; native MKL x64-only, OpenBLAS osx-arm64 asset checked empirically) | `LinearProvider.Select` probe over the restored native asset graph; managed fallback proven on osx-arm64 | numeric-lane#DENSE_ALGEBRA, numeric-lane#SPARSE_SOLVE |
|   [6]   | ORT-GenAI live multi-token generation against a genai-format model asset (streaming contract runtime-proved; the model artifact is the residual gate) | `GenerativeRun.Stream` over a genai-format model directory; the token-streaming member shape is FINALIZED | model-lane#GENERATIVE_RUN |
...
|  [14]   | Numeric lane                     | numeric-lane#DENSE_ALGEBRA, numeric-lane#SPARSE_SOLVE, numeric-lane#KERNEL_LOWERING, numeric-lane#PROVIDER_CLAIMS                                                           | static + spec (factorization solve, sparse ingestion, row-block shard exactness) + start gate 5 |
|  [15]   | Generative run                   | model-lane#GENERATIVE_RUN                                                                                                                                                  | static + spec (search-option fold, guidance constraint, chat-template assembly) + start gate 6 |
|  [16]   | TS projection handoff            | remote-lane#TS_PROJECTION, progress-and-observation#TS_PROJECTION, receipts-and-benchmarks#TS_PROJECTION                                                                   | descriptor-set emission at app-root creation; contracts transcribe at the TS workspace |
...
Residual host-bridge and native-asset work is the close-out surface. The SPIKE owners on the charter DENSITY_BAR carry the live probes named in their page RESEARCH clusters: `ModelSessions`, `RunOps` against model-lane#RESEARCH (`[EP_EXECUTION]`, `[CANCELLATION]` — in-host ONNX dylib ALC load and the Terminate-latch cadence); `WireChannels`, `CallSpine` against remote-lane#RESEARCH (`[TRANSPORTS]`, `[PIPE_SECURITY]`, `[REQUEST_COMPRESSION]`, `[COMPOSED_CREDENTIAL]`). The numeric-lane owners `LinearProvider`, `DenseOps`, and `SparseOps` are FINALIZED — the managed terminal is proved end-to-end on osx-arm64 against the real MathNet + CSparse stack, and native MKL/OpenBLAS EXECUTION is the documented per-RID build/deploy gate the `LinearProvider` RID axis already models (TASKLOG `[1]-[RESIDUAL_TIER3_PROBES]`: native MKL x64-only, native-openblas with no osx-arm64 asset, `Control.TryUse*` returning false on this RID), never an open owner spike — a deploy-asset gate, never a page RESEARCH probe. The GPU-EP execution residual (`AppendExecutionProvider_CUDA(0)`/`_DML(0)` member shape FINALIZED, execution gated on NVIDIA-Linux/Windows RID + driver) is the genuine GPU-hardware residual; the GENERATIVE_RUN owner is FINALIZED with live multi-token streaming runtime-proved. Each remaining SPIKE owner is fully shaped at transcription; its exit is the residual native, bridge, or live-server probe passing on the gate named beside the START_GATES row that unblocks it.
