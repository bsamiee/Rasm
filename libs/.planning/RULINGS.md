...
`libs/` rulings settle repo-spanning decisions.
...
- `CloudNative.CloudEvents.Mqtt` rejected — it pins MQTTnet 4.x against the repo's 5.x, reads a set-only `PayloadSegment`, and is structured-only.
...
- Branch prose states its own domain and never a peer's ceiling — a graduation result names what crosses and leaves policy to whoever reads it.
...
- OTLP is the ONE metrics-store ingest door and remote-write is declined repo-wide — a second leg forks the ingest contract per store.
- Analytics-store families add no descriptor column and foreclose `cap` — unbounded dimensionality is exactly what a cardinality budget destroys.
...
- Transports state a missing coordinate on `degrade` — replay, backpressure, and settlement each go absent on engines the repo already ships.
...
- Unbounded append retry admits only behind content-keyed idempotence AND a result type whose faults are transient by TYPE — both are preconditions.
...
- Columnar DDL plants at the tier that installs the store and whose writer fills the relation — the planter leads its sort key with tenant.
- Reliability rendering rides the deploy tier's realized backend projection — an objective naming a store is a contradicting second selection.
...
- CESQL evaluation is TOTAL — every operator returns a value beside an accumulated error list, so the result accumulates rather than short-circuits.
...
- Repo surfaces spell no version segment — packages, event types, and storage keys reshape in place with every consumer updated same-change.
...
- Producer-only facts are result columns — spans, meters, logs, and CloudEvents project from the column at the boundary; a fact minted beside it forks.
...
- Standalone boundary ledgers do NOT re-enter — the folder `ARCHITECTURE.md` `[03]-[CONTRACTS]` is the one boundary record, mirrored at both endpoints.
...
- Broker deployments do NOT enter `iac` — addresses are operator-supplied, and a decode against an absent repo fails rather than provisioning one.
