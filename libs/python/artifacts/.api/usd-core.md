...
|  [12]   | `UsdShade.Material`                  | material node   | `Define`; `CreateSurfaceOutput`/`CreateDisplacementOutput`/`CreateInput`      |
...
- value: a geometry attribute carries one `Vt` typed array (`Vt.Vec3fArray` points/normals, `Vt.IntArray` counts/indices, `Vt.FloatArray` widths) and a transform-op one `Gf` value, so a point buffer is one `CreatePointsAttr(Vt.Vec3fArray.FromNumpy(points))` set, never a per-vertex append. `Vt.Vec3fArray(ndarray)` raises `TypeError` on a bare ctor, so `FromNumpy` owns the buffer path and widens dtype (`float64 (N,3)` -> `Vec3fArray`).
...
- material: a PBR material is `UsdShade.Material.Define` + `UsdShade.Shader.Define` (the `UsdPreviewSurface` node) wired through `ConnectableAPI`, then attached through `UsdShade.MaterialBindingAPI.Apply(prim).Bind(material)` — the bind is distinct from the define; a per-face region is a `UsdGeom.Subset` the binding targets, so one mesh carries N material regions without N meshes. `material.CreateDisplacementOutput().ConnectToSource(surface.CreateOutput("displacement", Sdf.ValueTypeNames.Token))` wires the displacement terminal — the shader's `token outputs:displacement` (renderType `terminal displacement`) is DISTINCT from its `float inputs:displacement` scalar, and `Material.CreateDisplacementAttr`/`GetDisplacementAttr` is the legacy schema-declared attribute no `Compute*Source` resolves — a decoy reaching nothing.
- preview-graph: four `CreateIdAttr` node ids compose the whole preview-surface graph — `UsdPreviewSurface` (connectable inputs `diffuseColor`/`emissiveColor`/`specularColor` `Color3f`, `metallic`/`roughness`/`clearcoat`/`clearcoatRoughness`/`opacity`/`opacityThreshold`/`ior`/`displacement`/`occlusion` `Float`, `normal` `Normal3f`, and the interfaceOnly `useSpecularWorkflow` `Int` admitting no connection; outputs `surface`/`displacement`), `UsdUVTexture` (inputs `file` `Asset`, `st` `Float2`, `wrapS`/`wrapT`/`sourceColorSpace` `Token`, `scale`/`bias`/`fallback` `Float4`; output ports `r`/`g`/`b`/`a` `Float` and `rgb` `Float3`), `UsdTransform2d` (inputs `in` `Float2`, `rotation` `Float`, `scale`/`translation` `Float2`; output `result`), and the `UsdPrimvarReader_float`/`_float2`/`_float3` arity family (input `varname` `String`, output `result`). `wrapS`/`wrapT` admit `black`/`clamp`/`repeat`/`mirror`/`useMetadata` and `sourceColorSpace` admits `raw`/`sRGB`/`auto`; the sampler decode is `value * scale + bias` after an 8-bit file normalizes to `[0, 1]`. `Shader.Define` is get-or-create over one `Sdf.Path`, so samplers sharing one st primvar name share one reader prim by construction.
