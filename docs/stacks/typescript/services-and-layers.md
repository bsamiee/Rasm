...
- Law: the graph under test is the production composition with edges re-provided — substitute at the port seam, keep every interior edge real — because a proof against a hand-assembled parallel graph proves the parallel graph; a service whose edges were baked through `dependencies` re-opens them as `DefaultWithoutDependencies` with the substitute Layer, never a re-declared service.
