...
- Law: extension-name ceilings are BRANCH-owned. `core.v1.event.CloudEvent._validate_extension_attributes` matches `^[a-z0-9]+$` over a one-character floor and rejects the reserved `data` name — a charset rule carrying no length bound — so `EXTENSION_FMT` states the specification's twenty-character ceiling and the mint proves it BEFORE construction. Package surfaces narrower than the specification state a fact about the package, never a ceiling on what the repo carries.
...
from rasm.runtime.faults import EVENT_DOMAIN, EVENT_LAG, EVENT_NAIVE, EVENT_SOURCE, EVENT_TYPE, RuntimeResult
...
class EventType(Struct, frozen=True, order=True, gc=False):
    ...
    @classmethod
    def of(cls, domain: str, subject: str, fact: str, /) -> RuntimeResult[Self]:
        return cls.parse(_spelled(domain, subject, fact))

    @classmethod
    def parse(cls, spelling: str, /) -> RuntimeResult[Self]:
        return (
...
class Source(Struct, frozen=True, order=True, gc=False):
    ...
    @classmethod
    def of(cls, domain: str, capability: str, /) -> RuntimeResult[Self]:
        return cls.parse(f"rasm:{domain}/{capability}")

    @classmethod
    def parse(cls, reference: str, /) -> RuntimeResult[Self]:
        return (
...
def _profiled(event_type: EventType, source: Source, /) -> RuntimeResult[None]:
    return Ok(None) if event_type.domain == source.domain else Error(EVENT_SOURCE.raised(source.reference))


def stamped(produced: datetime, arrived: datetime, /) -> RuntimeResult[float]:
    return (
...
from rasm.runtime.faults import EVENT_EXTENSION, EVENT_MINT, BoundaryFault, Disposition, RuntimeResult, boundary, traversed
...
class Codec[V](Struct, frozen=True, gc=False):
    ...
    admit: Callable[[object], RuntimeResult[V]]
...
def _ordinal(raw: object, /) -> RuntimeResult[int]:
    parsed = (
...
def _instant(raw: object, /) -> RuntimeResult[Timestamp]:
...
def _timestamp(raw: datetime, /) -> RuntimeResult[Timestamp]:
...
class MessageEnvelope(Struct, frozen=True, gc=False):
    ...
    def event(self) -> RuntimeResult[CloudEvent]:
    ...
    def decoded(cls, event: BaseCloudEvent, /) -> RuntimeResult[tuple[Self, Block[str]]]:
...
def admitted(attributes: Mapping[str, object]) -> tuple[RuntimeResult[Extensions], Block[str]]:
...
def _renderable(extensions: Extensions, /) -> RuntimeResult[Extensions]:
    cells = Block.of_seq(
        boundary(
            EVENT_EXTENSION,
...
def _admitted(row: Codec[Any], raw: object, /) -> RuntimeResult[Any]:
    return row.admit(raw)


def _body(event: BaseCloudEvent, /) -> RuntimeResult[tuple[datetime, EventData]]:
...
def _subject(raw: str | None, /) -> RuntimeResult[Option[WireKey]]:
    if raw is None:
        return Ok(Nothing)
    try:
...
def _schema(value: Option[str], /) -> RuntimeResult[Option[str]]:
    return value.map(lambda raw: _absolute(raw).map(Some)).default_value(Ok(Nothing))


def _uri_reference(raw: object, name: str, /, *, nonempty: bool = True) -> RuntimeResult[str]:
    if (
...
def _absolute(raw: str, name: str = "dataschema", /) -> RuntimeResult[str]:
...
def _minted(attributes: Attributes, payload: object, /) -> RuntimeResult[CloudEvent]:
    try:
...
def _strict(event: BaseCloudEvent, /) -> RuntimeResult[CloudEvent]:
...
- Law: `write_data`/`read_data` complete the upstream `Format` protocol but do not mint a second payload result. The sealed payload union lowers directly; a binary read remains opaque because no `Any.type_url` rides that content mode. Compression belongs after the complete transport body at the binding/store owner; the deleted `Frame` identity pair had no binding site, failure declaration, or consumer.
...
- Auto: version-factory fallthrough is refused. `core.bindings.common.get_event_factory_for_version` answers `SPECVERSION_V1_0` for EVERY unknown version string, so an unrecognized `specversion` decodes as the current generation rather than refusing; every row here binds `CloudEvent` explicitly and refuses a foreign `specversion` at the boundary. `amqp` and `rabbitmq` hard-bind that same class, whose own required-attribute gate REFUSES every `specversion` but the current one — so those two raise where the auto-detecting `http`/`kafka` pair silently decodes a peer's unknown generation as this one.
...
from rasm.runtime.faults import EVENT_DECODE, EVENT_ENCODE, EVENT_FORMAT, Catch, RuntimeResult, boundary
...
class EventFormat(Struct, frozen=True, gc=False):
    ...
    @classmethod
    def bound(cls) -> RuntimeResult["EventFormat"]:
    ...
    def decode(self, body: bytes, /, *, media: str) -> RuntimeResult[Block[BaseCloudEvent]]:
    ...
    def admit(self, events: Block[BaseCloudEvent], /) -> RuntimeResult[Decoded]:
        return _admitted_events(events)

    def admitted(self, event: BaseCloudEvent, /) -> RuntimeResult[Decoded]:
        return self.admit(Block.singleton(event))

    def codec(self, suffix: Suffix, envelope: MessageEnvelope, /) -> RuntimeResult[Format]:
        return self._row(suffix, "structured").bind(
    ...
    def _decoded(self, row: FormatRow, batch: bool, body: bytes, /) -> RuntimeResult[Block[BaseCloudEvent]]:
        if batch:
    ...
    def _row(self, suffix: Suffix, mode: str, /) -> RuntimeResult[FormatRow]:
    ...
    def _framed(self, media: str, /) -> RuntimeResult[tuple[FormatRow, bool]]:
        return boundary(
            EVENT_FORMAT,
    ...
    def _media_row(self, media: MediaType, raw: str, /) -> RuntimeResult[tuple[FormatRow, bool]]:
...
def _admitted_events(events: Block[BaseCloudEvent], /) -> RuntimeResult[Decoded]:
...
) -> RuntimeResult[BaseCloudEvent | Block[BaseCloudEvent]]:
    if isinstance(value, Block):
