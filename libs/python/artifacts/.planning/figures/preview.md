...
- Receipt: each operation contributes `receipt/receipt#RECEIPT` `ArtifactReceipt.Preview` carrying the content key and the pixel dimensions.
...
GATED: frozenset[PreviewOp] = frozenset({PreviewOp.THUMBNAIL, PreviewOp.CONVERT, PreviewOp.MONTAGE, PreviewOp.TRANSFORM})
