export const meta = {
  name: 'survey-gaps-appui',
  description: 'Single-folder capability-gap survey-AND-APPLY for ONE C# AEC folder, hardcoded in the F const at the top of the body. Runs that folder pipeline once: one opus xhigh survey+synthesize agent (reads the folder + central manifest, emits the gaps AND six research facets), six opus xhigh research agents (one facet each, find best-in-class modern packages and self-validate the six-condition gate so no nonsense is admitted), one opus xhigh execute agent (consolidates the six findings, admits the central pins, updates the csproj + README, restores and verifies the install on osx-arm64 at newest), and one sonnet stub agent (writes a one-line placeholder .api catalog per admitted package). A disposable per-folder twin of survey-gaps, run one folder at a time. Persistence is already done. The full .api authoring and the design-page integration are deferred — a focused rebuild-api run fills the stubs and plan-cs threads the capability into the design pages.',
  whenToUse: 'Run the per-folder capability-gap survey-and-apply for the single folder hardcoded in the F const.',
  phases: [
    { title: 'Survey', detail: 'one opus xhigh agent: domain, admitted packages, hand-rolls, gaps, AND the six research facets', model: 'opus' },
    { title: 'Research', detail: 'six opus xhigh agents, one facet each: best-in-class modern candidate discovery with a self-validated six-condition gate', model: 'opus' },
    { title: 'Execute', detail: 'one opus xhigh agent: consolidate the six findings, admit the central pin + csproj reference + README, restore and verify the install at newest on osx-arm64, self-heal or revert', model: 'opus' },
    { title: 'Stub', detail: 'one sonnet agent: write a one-line placeholder .api catalog file per admitted package, nothing else', model: 'sonnet' },
  ],
}
...
const F = { name: 'AppUi', primary: false, doc: 'libs/csharp/Rasm.AppUi', api: 'libs/csharp/Rasm.AppUi/.api', planning: 'libs/csharp/Rasm.AppUi/.planning', csproj: 'libs/csharp/Rasm.AppUi/Rasm.AppUi.csproj', note: '' }
...
const LAW = [
  'Rasm monorepo, PER-FOLDER capability-gap survey-AND-APPLY of the C# AEC stack (KERNEL -> AEC-DOMAIN -> APP-PLATFORM -> HOST-BOUNDARY -> APP ' +
    'strata, depend strictly upward). This run works ONE folder end-to-end so the apply only ever consolidates a single folder findings and ' +
    'nothing is dropped. The mission is NOT a per-package keep/replace audit — it is to find AND admit the world-class MODERN packages this ' +
    ...
    'PRIMARY weight of the stack (already done); the rest carry Compute/Bim/Materials/Fabrication/Geometry/AppHost concerns. Central pins live in ' + MANIFEST + '.',
  'BAR: the folder must be able to build world-class, bleeding-edge, production-ready, professional-standard apps RIGHT NOW — ' +
    'ultra-dense/complex/rich, minimal-to-no hardcoding, maximal parameterization, easily extended, NO fake or illusory capability. Treat the ' +
    'folder as likely SHORT-SIGHTED in scope: assume valuable modern packages are missing and hunt them. Where a folder hand-rolls a concern a ' +
    'real package owns, that is a gap. Do not hold back: many additions are fine IF each is best-in-class and non-overlapping.',
  ...
  'MANDATED DEEP AREAS (when this folder owns the concern, the survey MUST raise them as facets and the research MUST sweep them exhaustively, not ' +
    'sample): for Persistence — PostgreSQL 18.4+ SQL/extension surface (EVERY first-party/contrib extension AND the top 25 community/third-party ' +
    'extensions: PostGIS, pgvector/pgvectorscale, TimescaleDB, pg_search/ParadeDB, pg_duckdb, Citus, pg_partman, pg_cron, pgRouting, h3-pg, Apache ' +
    'AGE, and beyond) and DuckDB (EVERY first-party extension AND the top 25 community extensions: spatial, httpfs, iceberg, delta, ' +
    'postgres/mysql/sqlite scanners, vss, fts, h3, ducklake, and beyond), plus store/backend types (columnar, time-series, graph, vector, search, ' +
    'object, embedded, lakehouse) and ingress/egress codecs; for Bim — IFC (model graph, geometry/tessellation, validation, BCF, IDS), scheduling ' +
    '(CPM/resource-leveling/4D), cost (estimating/QTO/cost models), GIS/geospatial; for Materials — appearance/spectral/measured-BSDF/datasets; ' +
    'for Compute — solvers/optimization/ML/numerics; for Geometry — host-neutral geometry/mesh/spatial capability; for AppHost — ' +
    'runtime/identity/observability/agent/delivery host capability. Rhino/GH2/AEC-adjacent capability is in scope where it is host-neutral.',
  ...
  'WRITE DISCIPLINE — strict phase ownership: the SURVEY and RESEARCH stages are READ-ONLY analysis and write NOTHING. ONLY the EXECUTE stage ' +
    ...
    'csproj reference + README mention.',
].join('\n')
...
const executePrompt = (f, survey, research) => [
  ...
  'TASK (EXECUTE for folder ' + f.name + ' — WRITE the four owned artifacts only): the six research+self-validation outputs for THIS folder are ' +
    ...
    'anything out of the folder scope — this is the per-folder plan, done here so nothing is dropped. THEN apply each surviving package NOW. The ' +
    'owning folder is ' + f.name + ' (csproj ' + f.csproj + ', .api root ' + f.api + ', central manifest ' + MANIFEST + ').' + (f.note ? ' ' + f.note : ''),
...
log('=== survey-gaps single-folder: ' + F.name + ' ===')
...
log('survey-gaps ' + F.name + ' complete: ' + applied.length + ' packages admitted, ' + apiStubs.length + ' .api stubs for the focused rebuild-api ' +
  'run')
