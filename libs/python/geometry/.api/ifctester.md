...
- `ifc/analysis#ANALYSIS` IDS arm runs three NAMED steps: `ids.open(spec, validate=True)` mints the document beside the typed roster of facets whose `uri` nothing dereferenced, one async resolve expands those URIs over the runtime transport and writes each answer onto the facet's own `Restriction(options=..., base=...)`, and only the resolved type reaches `Ids.validate(model)` -> fold each `Specification.status` into a `compliance` `AnalysisRow`; `ifctester` carries no HTTP client, so an unexpanded `uri` would otherwise grade a narrower applicable set than the spec declared; `status is None` (skipped) collapses to skip, distinct from `False`, and `passed_entities`/`failed_entities` set sizes with `Json(ids).report()` `Results.percent_checks_pass` and `ResultsSpecification.total_applicable_fail` sharpen the per-check failing fraction the boolean verdict drops, keying `AnalysisResult.evidence`.
