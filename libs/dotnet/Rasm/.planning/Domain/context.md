...
`Context` is host-neutral by construction: every member is unit- and scalar-driven except the `Of(RhinoDoc?)` factory, so a federation runtime that never sees a `RhinoDoc` constructs the identical bundle through the scalar and unit factories. Kernel operations pass measures as bare `double` — `Tolerance.Value` is a public scalar and `ToleranceLane.Dimension` a signature token, never a UnitsNet quantity; `Rasm.Element` `MeasureValue` remains the branch's dimensioned carrier.
...
public sealed partial class ToleranceLane {
    ...
    public static readonly ToleranceLane Join = new(key: "join", band: Band.Length, dimension: UnitsNet.Length.BaseDimensions, derive: static context => context.Absolute.Value * context.Relative.Value);
...
public readonly record struct Tolerance(ToleranceLane Lane, double Value) : IValidityEvidence {
    ...
    public static Fin<Tolerance> Of(ToleranceLane lane, double value) =>
        lane.Band.Admits(value)
            ? new Tolerance(lane, value)
            : new KernelFault.OutOfRange(value, lane.Band.Refuse(lane.Key, value).Message);
}
```
...
public sealed record ModelUnit {
    ...
    public static Fin<ModelUnit> Of(UnitSystem value) => value switch {
        ...
        _ => Try.lift(() => Of(LengthUnit.FromKnownUnitSystem(value))).Run().Bind(static inner => inner),
    };

    public static Fin<ModelUnit> Of(LengthUnit value) => Try.lift(() => {
...
public sealed record Context {
    ...
    public Fin<Context> Override(ToleranceLane lane, double value, Enum unit) {
