...
- Resample: the resample case folds through the `FieldEngine.gated()` carrier — the frozen value object that imports `interpax`/`jax` ONCE behind the band and runs `jax.config.update("jax_enable_x64", True)` BEFORE the interpolant is constructed, the `solvers/quadrature.md#QUADRATURE` `QuadEngine.gated()`/`solvers/differential.md#DIFFERENTIAL` `SolveEngine.gated()` x64 contract every sibling JAX route carries. The x64 promotion is load-bearing: an `Interpolator{1,2,3}D` is a JAX pytree, so on the float32 default its `grad`/`vjp` through the sampled values degrades below double precision — distinct from the pure-`scipy` `skfem` eval/project path, which needs no x64 because it never touches JAX, so only the interpax resample folds the gated engine. `engine.resample` dispatches on the coordinate-tuple arity through the module-level `_INTERPOLATOR` table, whose `int`-keyed `"Interpolator{1,2,3}D"` constructor names resolve through `getattr(self.interpax, _INTERPOLATOR[dim])` — the cp315-clean module-level row catalog keying the gated symbol behind a name-deferred resolver, the same discipline `solvers/quadrature.md#QUADRATURE` `_QUAD` rows run through `getattr(self.quadax, row.adaptive)`, never a `lambda ix: ix.*` closure-per-row or a body-local re-bind. The selected `Interpolator{1,2,3}D` is constructed ONCE (the catalog's preferred reusable form over the one-shot `interp{1,2,3}d` recompiled per call), the `(N, dim)` query split into the `(dim, N)` per-axis columns through the cataloged `reshape(-1, dim).T` for `dim>1` / the raw query for 1-D — never the phantom `np.atleast_2d` the branch `numpy.md` does not catalog — and evaluated with the bounded `ResampleMethod` kernel, never a free `str`. Because every `Interpolator{1,2,3}D` is a JAX pytree module under x64, the constructed interpolant is `vmap`/`grad`/`jit`-compatible with respect to BOTH the query points and the sampled values — the same `equinox`/`optimistix`/`diffrax` differentiable leaf the `compute/.api/interpax.md` `[SIBLING_INTEGRATION]` records, without a boundary conversion. The numpy floor serves the 1-D arity through `np.interp`; the 2-D/3-D arities are gated-only because no in-numpy regular-grid interpolant matches the differentiable `interpax` kernel.
- Entry: `FieldQuery.evaluate` enters one `boundary(f"field.{self.tag}", lambda: _dispatch(self)).bind(lambda rail: rail)` returning `RuntimeRail[FieldReceipt]`, joining the inner `@railed` `_dispatch` rail without double-wrapping — the rail-join shape `solvers/mesh.md#EXCHANGE` `MeshExchange.run` and `numerics/array.md#PAYLOAD` `ArrayPayload.admit` hold. `_dispatch` is the `@railed` `effect.result` chain whose `match` dispatches the three cases through total `assert_never` exhaustion: the interpolate arm folds the readout array and split components into the `interpolate` case, the project arm the target DOF vector and round-trip transfer residual into the `project` case, and the resample arm `yield from`-binds the `_key` `RuntimeRail[ContentKey]` (the only case without a `MeshField` to inherit `content_key` from) before folding the `FieldEngine.gated()`-floored query-point evaluation into the `resample` case, so a canonical-encode fault rides the one rail rather than a `match`/`raise` re-raise. The interpolate and project arms hold no fallible derive and reuse the `field.content_key` the mesh owner already minted, so only the resample arm threads the `_field`-style identity rail. Each arm folds into the `@receipted(_REDACTION)` `_emit` builder that returns the `FieldReceipt` contributor and emits its harvested stream on exit, so receipt egress is a decorator rail on the pure builder while the fallible resample derive stays on the rail. The interpolate numpy floor reads the DOF vector norm and peak at the mesh nodes directly when `skfem` is unavailable, passing `result=None` to the shared status floor so the interpolate extent stays reachable on cp315 without the gated basis, the verdict adjudicated by the residual-against-tolerance floor identical to the quadrature numpy floors; the project case has no numpy floor because the L2 basis-to-basis transfer is owned by gated `skfem` with no in-numpy equivalent, so projection is reachable only on the gated band, and the resample 2-D/3-D arities ride the `FieldEngine` JAX band likewise, the 1-D arity flooring to `np.interp` on the bare cp315 core.
...
# cp315-clean `int` arity so the table is a module-level constant of plain strings; `FieldEngine.resample`
# resolves the gated symbol through `getattr(self.interpax, _INTERPOLATOR[dim])` off the engine's folded
...
# The gated `interpax`/`jax` modules folded into ONE frozen value object with behavior — the
# `solvers/quadrature.md#QUADRATURE` `QuadEngine.gated()`/`solvers/differential.md#DIFFERENTIAL`
# `SolveEngine.gated()` discipline every sibling JAX route runs. `gated()` imports once behind the band
...
# `skfem` handle is the gated module with no cp315 stub, so it is typed `Any`, not `object` that would
...
# Regular-grid resample over the JAX-differentiable interpax interpolant. The gated `interpax`/`jax`
# modules fold through `FieldEngine.gated()` so the x64 promotion fires ONCE behind the band before the
...
# `QuadEngine.gated()`/`solvers/differential.md#DIFFERENTIAL` `SolveEngine.gated()` x64 contract every sibling
...
# phantom `np.atleast_2d`). The numpy floor serves 1-D through `np.interp` on the bare cp315 core when jaxlib
...
def _resample_receipt(key: ContentKey, axes: GridAxes, values: np.ndarray, query: np.ndarray, method: ResampleMethod) -> FieldReceipt:
    dim = len(axes)
    try:
        sampled = FieldEngine.gated().resample(axes, values, query, method)
    except ImportError:
...
- [INTERPAX_RESAMPLE]: the resample case realizes the `[INTERPAX_QUADAX_USAGE]` deferred consumer the `compute/.api/interpax.md` catalog records for `solvers/field` and the `solvers/quadrature.md#QUADRATURE` route defers ("the `interpax` `interp2d`/`interp3d` family serves on `solvers/field`"). The `interpax.Interpolator1D(x, f, method)`/`Interpolator2D(x, y, f, method)`/`Interpolator3D(x, y, z, f, method)` reusable interpolant objects verify against `compute/.api/interpax.md` — JAX pytree modules constructed ONCE and then called with query points (the catalog's preferred reusable form over the one-shot `interp{1,2,3}d` recompiled per call), `vmap`/`grad`/`jit`-compatible with respect to both the query points and the sampled values, so the resampled field is an autodifferentiable JAX-pytree leaf a downstream `equinox`/`optimistix`/`diffrax` objective consumes per the `compute/.api/interpax.md` `[SIBLING_INTEGRATION]` row, without a boundary conversion. The `FieldEngine.gated()` carrier imports `interpax`/`jax` once behind the band and runs `jax.config.update("jax_enable_x64", True)` BEFORE the interpolant is constructed: a JAX pytree defaults to float32, so without the x64 promotion the resampled field's `grad`/`vjp` degrades below double precision — the same x64 contract the `solvers/quadrature.md#QUADRATURE` `QuadEngine.gated()` and `solvers/differential.md#DIFFERENTIAL` `SolveEngine.gated()` routes carry on the gated band, the deleted form being a gated solve left on the JAX default float32. The pure-`scipy` `skfem` eval/project path needs no x64 because it never touches JAX, so only the interpax resample folds the engine. The module-level `_INTERPOLATOR` `FrozenDict[int, str]` keys the coordinate-tuple arity onto the `"Interpolator{1,2,3}D"` constructor name that resolves through `getattr(self.interpax, _INTERPOLATOR[dim])` inside `FieldEngine.resample` — the cp315-clean module-level name-deferred-resolver discipline the quadrature `_QUAD` `FrozenDict` row catalog runs through `getattr(self.quadax, row.adaptive)`, never a `lambda ix: ix.*` closure-per-row or a body-local table re-bound per call — the `(N, dim)` query splitting into the `(dim, N)` per-axis columns through the cataloged `reshape(-1, dim).T` (the branch `numpy.md` catalogs `reshape`/`transpose` but NOT `atleast_2d`, so the phantom `np.atleast_2d` is the deleted form), and the `ResampleMethod` `Literal` (`"nearest"`/`"linear"`/`"cubic"`/`"cubic2"`/`"catmull-rom"`/`"monotonic"`/`"monotonic-0"`) is the bounded interpax kernel vocabulary the catalog records, never a free `str`. `interpax` (jaxlib) carries no cp315 wheel, so the 2-D/3-D resample rides the gated band; the 1-D arity floors to `np.interp` on the bare cp315 core, the multidimensional arities being gated-only because no in-numpy regular-grid interpolant matches the differentiable kernel.
