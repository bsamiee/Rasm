...
No state means no resource graph and no `${...}` reference syntax. Values flow between commands by reading a field from one command's JSON output and passing it as a literal flag to the next, across providers as readily as within one.
