...
- [02]-[POLICY_FAMILY]: the `Tenancy` tagged family, the locus derivation, the isolation dispatch.
- [03]-[SCOPED_WRITE]: the coordinate pin fold, the `within` transformer, the RLS policy ensure.
- [04]-[SCOPE_FAMILY]: `ScopeKey`, the `Tenant` service, the `Stores` LayerMap, verify-at-construction.
- [05]-[PORT_SATISFACTION]: the security state ports satisfied by Layers built from these scopes.
