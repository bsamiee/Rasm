...
`Rasm.Materials` is the host-neutral AEC-domain projector onto the `Rasm.Element` boundary. `Component`, `Appearance`, `Properties`, and `Projection` collapse to one owner per axis; the one `ComponentProjector : IElementProjection` lowers every owner into the shared `ElementGraph`. Its `Project` fold splits the `Substance` and Type-minting `Type` arms, mints the deterministic-rooted Type `Object` from canonical content, and authors the content-keyed `Material`/`Appearance` subgraph the contract `Assemble` fold merges. AEC peers depend up on `{Rasm, Rasm.Element}` and align by contract.
...
│   ├── Steel.cs           # SteelSeed.Roster spans the registered domains beside the generated cold-formed table; SteelSeed.Law binds it
...
├── Raster/                # Texture-map generation — the plane substrate, the bake engine, and its container codecs
...
├── Properties/            # Typed engineering-property source lowered onto the contract property sets
...
│   ├── Sustainability.cs  # SustainabilityCatalogue in exact roster parity with its engineering sibling; Lower mints the contract cases
...
└── Projection/            # One IElementProjection onto the Rasm.Element boundary + the observability, benchmark, and analytics projections
    ├── Component.cs       # Project folds payload-complete Substance and Type cases onto Fin<GraphDelta> behind the ProjectionGate veto
    ├── Observability.cs   # MaterialsPoint roster over the kernel IHookRoster floor; MaterialsHooks.Live mints the one folder hook set
    ...
    └── Analytics.cs       # Dataset rows and projection folds; column types, storage tiers, dialects, and DDL home at Rasm.Persistence
```

VividOrange grounds the structural section, capacity, and rebar data in-folder, never a hand-keyed literal; the per-page consumption law lives on the owning pages. Return type names the result: a `SurfaceShade`/`Unicolour` carrier where the result is total, `Fin<T>` where a banded fault routes, the contract `Fin<GraphDelta>` from the projector.
...
- S1 seat — `Properties` lowers engineering dimensional mints through the S0 `QuantityRow` and basis-relative scalars to the contract factories.
...
- S2 seat — the `Appearance` egress composes `Raster` planes and sets; `Raster` names no egress type, so the plane module stands alone.
...
---
flowchart LR
    accTitle: Materials AEC-domain projection interfaces
...
---
flowchart LR
    accTitle: Materials platform, compute, and cross-runtime interfaces
...
flowchart LR
    ...
    Mint f1@-.->|"seed-law refusal"| Fault[/Banded fault channel/]
...
flowchart LR
    ...
    accDescr: How a compiled material program becomes a content-keyed baked set the wire and the contract re-bind.
...
|  [09]   | new contract payload                | `Rasm.Element` composition  | contract growth the projector composes, never a local remint      |
...
- Materials owns substance, appearance, and buildable type: one `Component`, one capacity pipeline, one `MaterialGraph`, one `TextureSet`.
...
- `SEED_ROW_LAW` seats standards data as in-fence C# under per-column provenance, and every seed row flows the one catalogue-to-solver pipeline.
...
- Model authors mint Occurrence `Object`s and `Rasm.Bim` ingests `IfcElementType` into the same Type; the `Bake` inheritance is the contract's.
...
- Every out-of-gamut, non-finite, or degenerate result fails to its banded fault, never a propagated NaN or sentinel.
