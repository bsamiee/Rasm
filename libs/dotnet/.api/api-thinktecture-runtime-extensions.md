...
`Thinktecture.Runtime.Extensions` mints every generated domain owner in the branch: one declaration attribute over a `partial` type emits that owner's key member, factory pair, validation hook, conversion operators, comparison policy, and exhaustive dispatch, and each policy property narrows the emission rather than replacing it. Its runtime half carries the static-abstract contracts and the cached metadata a companion adapter binds, so an adapter projects a generated owner onto its own wire while identity, admission, and key projection stay here.
...
- `IValidationError<out T> where T : class`: one static abstract member — `static abstract T Create(string message)` — so a custom validation-evidence type carries a message-only mint and every generated admission path reaches it through the constraint, never through an instance or a container.
...
- `Metadata.Keyed`: exposes `KeyType`, `ValidationErrorType`, `ConvertToKey`/`ConvertToKeyExpression`, and `GetKey`, so a store or codec takes the OUTBOUND conversion as a delegate or an expression tree. The INBOUND half is not on `Keyed` — `ConvertFromKey`, `ConvertFromKeyExpression`, and `ConvertFromKeyExpressionViaConstructor` sit on the `Keyed.SmartEnum` and `Keyed.ValueObject` cases, non-null on the value-object case and nullable on the smart-enum case where an unknown key has no owner to mint; the `*ViaConstructor` form bypasses the validation gate for a store that already holds admitted values, which is why a read path and a write path pick different members off the same metadata.
- `Metadata.Keyed.SmartEnum` adds `Items` (a `Lazy<IReadOnlyList<SmartEnumItemMetadata>>`, so the roster materializes on first read), `TryGetFromKey`, and `DisableSpanBasedJsonConversion`; `Metadata.ComplexValueObject` carries `AssignableMembers` alone. The two union cases DIVERGE and never share a member spelling: `Metadata.AdHocUnion` carries `MemberTypes` beside `ConvertToValue`/`ConvertToValueExpression`/`GetValue`, because an ad-hoc union HAS a current value to project, while `Metadata.RegularUnion` carries `TypeMembers` and nothing else — a nested-case union holds no single value to convert. An adapter therefore reads one shape per owner kind after the `Switch`, never a reflection probe.
- `Metadata.RegularUnion.TypeMembers`: a `Type` list and the whole of what regular-union generation publishes about its cases — the generator emits `Switch`/`Map` dispatch and NO per-case name, ordinal, or discriminator member. A stable case token therefore cannot be composed off this package: reading `.Name` off a member of the list is a runtime type probe wearing metadata, answering whatever the trimmer or AOT compiler left of the nested record's name and turning a rename into a silent token change. A consumer needing one mints it in its own generator as the leaf's `nameof` (`Rasm` `Domain/results#FAULT_BAND` `FaultId.Case`), and reads this list only for a build-time or startup PROOF, where reflection is legitimate.
...
|  [14]   | `ValidateFactoryArguments(ref TValidationError?, ref TValue)` | static   | domain validation hook           |
...
[LOCAL_ADMISSION]:
- `Directory.Build.props` injects `Thinktecture.Runtime.Extensions` under one central identity for every workspace library, so generated owners and every adapter agree on one generator contract set.
- Generator support annotations remain internal implementation details of each compilation; workspace manifests carry no annotation package or generator override.
...
- Owning vocabularies lift `TryGet` onto `Option<T>` where a domain carrier requires optional lookup, so the throwing `Get` stays behind that lift.
