...
`@types/geojson` is the RFC 7946 object model as TypeScript: a declaration-only (`.d.ts`), zero-runtime package that declares one closed discriminated type algebra keyed by the string-literal `type` tag and exposes it as the UMD global `export as namespace GeoJSON`. It owns the geometry union (`Point`/`MultiPoint`/`LineString`/`MultiLineString`/`Polygon`/`MultiPolygon`/`GeometryCollection`), the `Feature<G, P>`/`FeatureCollection<G, P>` containers generic over geometry and properties, the coordinate primitives (`Position`, `BBox`, `GeoJsonProperties`), and the two narrowing discriminant unions (`GeoJsonTypes`, `GeoJsonGeometryTypes`) derived from the `type` field. There is nothing to call and nothing to construct — it is the value vocabulary the render side speaks, admitted `import type`. In `ui/viewer` it is the interaction-scale DOM geometry value that deck's `GeoJsonLayer`, `maplibre-gl`'s `GeoJSONSource`, and `@turf/turf`'s pure ops all consume, distinct from the bulk-columnar `apache-arrow` route `@geoarrow/deck.gl-geoarrow` owns and from the core `WkbParser` port's Schema-typed `GeoFeature.Geometry`; `scope:viewer` project-local.
...
- rail: viewer/geo
- `Geometry` is the closed seven-member union (`GeometryObject` is its alias); each member is a `GeoJsonObject` refined by a literal `type` and a `coordinates` nesting depth (or `geometries` for the collection). The union is the discriminant surface — a `switch` on `.type` narrows to exactly one member.
...
- rail: viewer/geo
- `GeoJsonObject` is the base (`type` + optional `bbox`); `Feature<G, P>` wraps one geometry plus `properties` and an optional `id`; `FeatureCollection<G, P>` is the `Feature` array; `GeoJSON<G, P>` is the top union of all three. `G extends Geometry | null` admits the null-geometry feature; `P` threads the property shape. The two `["type"]` lookup unions are the discriminant vocabularies a boundary decoder switches on.
...
[GEOJSON_TOPOLOGY]:
- One closed type algebra, discriminated by the literal `type` tag: every shape refines `GeoJsonObject`, and narrowing is a `switch (obj.type)` over `GeoJsonTypes`/`GeoJsonGeometryTypes` — never an ad-hoc field probe. A new object kind is impossible by construction (RFC 7946 fixes the union); the vocabulary is fixed data, not an open interface to extend.
- The two type parameters thread the whole graph: `G extends Geometry | null` flows from `Feature`/`FeatureCollection`/`GeoJSON` into their `geometry`/`features`, and `P` (default `GeoJsonProperties`) types `Feature.properties`. A layer typed `FeatureCollection<Polygon, MyProps>` carries both facts to every accessor — one generic instantiation, never a per-property re-declaration.
- `Position` stays the wide `number[]` on purpose: the package refuses the `[number, number] | [number, number, number]` narrowing (marginal benefit, large breaking impact), so a caller needing 2D/3D discrimination writes a type guard returning a predicate rather than expecting the package to narrow.
