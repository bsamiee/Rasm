...
- [04]-[WINDOW]: `ShellCapability`, `WindowRole`, `MountPhase`, `ChromeStyler`, `WindowChrome`, `WindowSpec`, `WindowVerb`, `IMount`, `MountCustody`, `WindowMount` — the window posture, its skin seam, its live verbs, the custody machine every mount on this page composes, and the mount that owns every host object the realize minted.
...
- [06]-[PRINT]: `PrintRoute`, `PageFrame`, `PageSpan`, `PrintScope`, `CollatePosture`, `PageOrder`, `PrintSpec`, `PrintPage`, `PrintPageFact`, `PrintReceipt`, `PrintPlan` — the deferred document run, its sheet-composed page geometry, and its per-page evidence.
...
- Owner: `IntentKey` the verb identity, `PlacementKey` the surface identity, `GroupKey` the radio-group identity, `SubMenuKey` the submenu identity, `JobName` the print job's; `CommandKind` the closed behaviour family; `CommandRole` the mint column; `PlacementSlot` one surface occurrence; `IntentRow` one verb; `IntentReceipt` one invocation fact; `IntentTable` the materialized deck every projection borrows from.
...
- Law: the receipt carries `Fin<Unit>` and DERIVES its fault, so the settled bool and the parallel fault column both boundaries carried collapse into the outcome that already holds both. NAMED LOSS: none — `Fault` is a projection, not a stored fact.
...
- Receipt: none — the lease IS the evidence; a release fault is retained on the popup rather than raised into the host event pump.
...
- Law: the skin is ONE column whose PRESENCE is the declaration. A declaration row beside a styler slot admits two decorative corners — a declared-bare window holding a styler nothing runs and a declared-skinned window dressing nothing — which is the legal-corner defect this page's own capability law forecloses everywhere else. A receipt renders and a diff compares `Styler.IsSome`, which is the same fact the row carried, and the host function itself stays uncompared because no value equality could compare it anyway.
...
- Law: teardown is the exact inverse of construction — content detaches, the element receipt releases, the window disposes, then the chrome the window carried releases in mint-reverse order — and every step runs even when an earlier one refuses, each refusal accumulating on the mount's own ledger. A teardown fault never rides an unwinding stack, because a raise from a `finally` REPLACES the primary error.
...
- Receipt: `WindowMount` exposes the surface, the realized plant, and the accumulated release faults; the leased mount IS the transfer of ownership.
...
public sealed class WindowMount : IMount, IDisposable {
    ...
    private readonly ElementReceipt plant;
    ...
    public ElementReceipt Plant => plant;
    public Seq<Error> ReleaseFaults => teardown.Value;
...
- Receipt: `Fin<TResult>` for the modal over its `PromptSettle` crossing, `PickerResult` for the picker and `Fin<Option<TResult>>` for its typed demand; all three are settled facts and none exposes a live host dialog.
...
- Entry: `Run` answers `IO<PrintReceipt>`; printer interaction and document lifetime begin only when the caller executes the effect, and that execution crosses the marshal ONCE so job construction, every page callback, both dialog routes, and disposal share one UI-affine scope. ONE carrier: `IO` already carries failure, and a `Fin` inside it makes a caller run the effect and then match a second rail for one outcome.
...
- Law: nothing inside the run's scope re-marshals. A page or completion callback fires from the run the driver is already blocking on, so presence lands through the affinity assertion and a refusal reaches the receipt's fault roster; a blocking crossing from inside that callback would wait on the very thread that raised it.
...
- Receipt: `PrintReceipt` is this driver's RAW run outcome alone — printer-evidence vocabulary belongs to the publishing surface that folds these facts into its own.
