...
// sourced OVERLAY-FIRST from the committed schema-span table (derived offline from the published
// buildingSMART per-version change indices), else the per-member GG [VersionAdded] lowered through
// ReleaseMap, else the class floor. GG attributes are a CROSS-CHECK, never the truth source: on
// IfcWallTypeEnum GG annotates the 2x3-era STANDARD/POLYGONAL/ELEMENTEDWALL as IFC4 and
// leaves the IFC4-added MOVABLE/PARAPET/PARTITIONING unannotated — schema-divergent both ways.
...
public sealed partial class IfcClass {
    ...
    public static readonly IfcClass Wall = new("IfcWall", IfcDomain.Architecture, IfcSchema.Ifc2X3, instantiable: true, Seq(
        // Token spans are SCHEMA truth off the committed overlay, not GG attributes: the IFC4 change log ADDS
        // MOVABLE/PARAPET/PARTITIONING/SOLIDWALL and DEPRECATES (never removes) STANDARD/POLYGONAL/ELEMENTEDWALL,
        // where GG annotates the retired trio IFC4 and the added trio not at all.
        new PredefinedRow("ELEMENTEDWALL", ReleaseVersion.Ifc2X3), new PredefinedRow("MOVABLE", ReleaseVersion.Ifc4),
        new PredefinedRow("PARAPET", ReleaseVersion.Ifc4),
        new PredefinedRow("PARTITIONING", ReleaseVersion.Ifc4), new PredefinedRow("PLUMBINGWALL", ReleaseVersion.Ifc2X3),
        new PredefinedRow("POLYGONAL", ReleaseVersion.Ifc2X3), new PredefinedRow("RETAININGWALL", ReleaseVersion.Ifc4X1),
        new PredefinedRow("SHEAR", ReleaseVersion.Ifc2X3), new PredefinedRow("SOLIDWALL", ReleaseVersion.Ifc4),
        new PredefinedRow("STANDARD", ReleaseVersion.Ifc2X3), new PredefinedRow("WAVEWALL", ReleaseVersion.Ifc4X3)));
    public static readonly IfcClass Bridge = new("IfcBridge", IfcDomain.Infrastructure, IfcSchema.Ifc4X1, instantiable: true, Seq(   // schema-derived introduction IFC4X2 → seam Ifc4X1 (ReleaseMap fold); class-birth tokens inherit it
    ...
    public static readonly IfcClass WallStandardCase = new("IfcWallStandardCase", IfcDomain.Architecture, new SchemaSpan(ReleaseVersion.Ifc2X3, Some(ReleaseVersion.Ifc4X3)), instantiable: false, Seq<PredefinedRow>());   // Retirements overlay: the permissive ingress stamps this CLOSED window as the node's own span, every Resolve consumer folds onto Wall
    ...
    // Classification("ifc", Key) the seam Object node carries, Canonical folding the retired *StandardCase/
    ...
    // entity. The retired subtypes still hold committed rows, and the fold decides WHO reads them: the permissive
    // TryGet ingress resolves the retired row itself and stamps its CLOSED SchemaSpan as the node's own span (the
    // source entity's true window [H8]), while every Resolve consumer — the legality vocabulary arm, AuditTarget,
    // the egress author — reads the folded BASE row, so a 2x3 StandardCase re-emits as the surviving IFC4 class
    // instead of faulting on a window the schema itself retired. A window-rejects-at-egress reading of these rows
    // is unreachable by construction and describes a gate no consumer holds.
...
- Owner: `IfcVocabularyEmitter` the OFFLINE BCL-reflection emitter — a repo tool run on GeometryGym pin bumps whose output is the committed `IfcClass` row region; the committed table is the system of record, so there is no runtime reflection, no Roslyn generator, and no sidecar file. `ReleaseMap` the ONE GG-to-seam release table (the `Lower` fold and its exact-name-preimage `Raise` inverse) the emitter AND the runtime `Projection/semantic#SEMANTIC_PROJECTOR` `ReleaseLower` / `Projection/egress#IFC_EGRESS` `ReleaseRaise` lowerings share. `VocabularyRow` the emitter row currency one render line commits. The overlays are committed FROZEN data on two tiers: `ClassIntroductions` is the SCHEMA-DERIVED span table — class rows AND dotted `"IfcWall.PARAPET"`-form token rows, one dictionary whose key shape discriminates the tier, generated offline from the published buildingSMART per-version change indices (GG carries NO class-level `[VersionAdded]` at all — decompile-proven on `IfcBridge`/`IfcAlignment`/`IfcRoad` — and its token attributes diverge from schema truth both ways, so the table is the truth source and the GG attribute is the emit-time cross-check census); `AbstractSupertypes` (the schema-abstract set the CLR flag misreports), `Retirements` (the `*StandardCase`/`*ElementedCase` closed windows — no `VersionRemoved` attribute exists), and `DomainRoots`+`DomainOverrides` (the hierarchy-root discipline claims spanning the WHOLE `IfcObjectDefinition` closure — the element disciplines plus the `General`/`Controls`/`Construction` backbone roots — plus the per-entity re-pins) stay the hand-judgment tiers.
...
public static class IfcVocabularyEmitter {
    public static Fin<string> Emit(
        ...
        Try.lift(() => toSeq(gg.GetExportedTypes()
                .Where(static t => typeof(IfcObjectDefinition).IsAssignableFrom(t) && !t.IsGenericType))).Run()
    ...
    // Class IntroducedIn: the REQUIRED schema-derived ClassIntroductions row — GG declares NO
    // class-level [VersionAdded] anywhere (decompile-proven: IfcBridge/IfcAlignment/IfcRoad all
    // bare), so the attribute slot passes null by construction and a class the committed table
    // does not cover FAILS the emit introduction-miss. Instantiable: !IsAbstract AND
    // !AbstractSupertypes — the CLR flag's honest half plus the overlay for the schema-abstract
    // concretes. RemovedIn: the Retirements overlay alone (no VersionRemoved attribute exists;
    // the *StandardCase/*ElementedCase windows close here).
    private static Fin<VocabularyRow> RowOf(
        ...
        Introduced(entity.Name, attribute: null, introductions, inherit: None, key)
    ...
    // OVERLAY-FIRST from the schema-derived dotted rows, else the per-member [VersionAdded] lowered through
    // ReleaseMap, else the class floor — exact for a class-birth enum (IfcBridgeTypeEnum is wholly unattributed
    // and wholly inherited); the GG attribute is a cross-check, schema-divergent both ways on grown enums
    // (IfcWallTypeEnum's IFC4 additions unannotated, its retired 2x3 trio mis-annotated IFC4).
