...
- concurrency axis: `ChartExport.lane.offload` moves `vlc.vegalite_to_*` and `svg_to_*` onto `Modality.THREAD`, moves the `visualization/chart/export#PREPASS` vegafusion work onto `Modality.PROCESS`, and applies `RetryClass.OCCT` only to the subprocess lane; `LanePolicy` owns limiter, deadline, and retry composition, so no V8 render blocks the event loop and no folder-local limiter exists.
