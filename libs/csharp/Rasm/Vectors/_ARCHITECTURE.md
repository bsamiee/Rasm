...
4. `[implemented]` Mesh and spectral receipts exist for topology, features, flatten, remesh, descriptors, scalar isolines, SDF, and segmentation. `MeshSegmentation` now executes scalar threshold components, scalar band components, seeded scalar region growing, and descriptor-scalar clusters with face/vertex assignments and typed receipts. `LaplacianCache` exposes cache-hit facts for spectral bases, cotangent/IDT assemblies report skipped degenerate faces, SDF signed-heat receipts carry final Poisson solve facts, and robust tufted Laplacian, signpost transport, closed SignedHeat, true watershed/normalized-cut segmentation, and true tangent log-map remain unsupported until they land truthfully.
5. `[partial]` Sampling, domains, and modes use the right owners: `SampleKind`, `ExtractionDomain`, `CurveProjection`, `SurfaceProjection`, and `SupportSpace`. Public callers route through factory admission, cloud mass reaches deterministic candidate weighting through `CloudKernel.MassOf`, curve/surface modes use admitted factories, and density/adaptive sampling is still deterministic priority selection. Finish only by adding a proven density/blue-noise rail with receipts; do not claim Bridson Poisson disk, Yuksel weighted sample elimination, OT/CCVT, or mesh-spectrum validated blue noise from the current candidate-thinning code.
...
Matrix, extraction admission, mesh segmentation, mesh/spectral receipt facts, and cloud/alignment/transport mass diagnostics are close enough to mark implemented. Remaining partial areas stay partial because they still contain missing receipt propagation, unsupported algorithms, or native-runtime behavior that has not been proven in RhinoWIP.
...
- `[implemented]` `Extraction.cs`: domain-backed glyph/grid/bundle sampling, mesh scalar isolines, surface `IsoStatus`, point-cloud section curves, and raw native contour counts are implemented. `ExtractionReceipt` owns receipt construction, PL mesh-isoline plateau/branch facts are preserved, and null/raw domain plus policy payloads fail through factory admission; glyph/bundle runtime geometry proof remains bridge-owned.
...
- `[partial]` `Sample.cs`: explicit, mesh, support, cloud, weighted, scalar-density, and adaptive sampling all execute through `SampleKind` and return `SampleReceipt`. Weighted sample mass now routes through `CloudKernel.MassOf`; mesh surface candidates come from the mesh owner; current `PoissonDisk` is deterministic candidate radius filtering, current `Weighted` is normalized candidate mass propagation, current `Lloyd`/`Capacity` are nearest-candidate relaxation modes, and scalar/adaptive density remains deterministic priority selection. Future blue-noise work must extend `SampleKind` with algorithm-specific receipts before claiming Bridson active-list Poisson disk, Yuksel weighted sample elimination, OT/CCVT capacity satisfaction, or mesh-spectrum validation.
- `[partial]` `Space.cs`: `SupportSpace` owns support admission, closest hits, signed distance, containment distance, tangent/frame projection, and `SurfaceSpace` sampling. Cluster support validity now reuses `CloudKernel.MassOf`; repeated point/vector/frame acceptance can still shrink inside existing projection owners.
...
- `[implemented]` Mesh diagnostics: `LaplacianCache` exposes spectral cache-hit facts, cotangent/IDT Laplacians count skipped degenerate faces, descriptor receipts carry cache/skip facts, sparse fallback solve status is distinct from residual convergence, and boundary-source signed heat surfaces the final Poisson solve receipt. Robust tufted Laplacian, flipped signpost vector heat, closed SignedHeat, runtime scalar-isoline geometry proof, and true tangent log-map remain unsupported.
...
- `[partial]` `Sample.cs`: weighted and scalar-field-driven sampling route through `SampleKind`; density maps and programmatic priorities control deterministic selection and output mass through `CloudKernel.MassOf`. Non-mesh density behavior and blue-noise claims remain unsupported.
...
- `[partial]` Future blue-noise sampling: `Sample.cs` owns admission/execution, `Mesh.cs` owns mesh candidate/domain geometry when real cells or mesh sampling are needed, and `Spectral.cs` owns spectral quality metrics consumed by sampling receipts. Bridson requires active-list annulus sampling with grid neighbor proof and min-distance receipts; Yuksel requires oversample/elimination-order/weight receipts and must not be conflated with normalized candidate mass; OT/CCVT requires capacity residual, cell mass/area, iteration, and convergence receipts beyond current nearest-candidate relaxation; mesh-spectrum validated sampling requires Laplacian/spectral-basis reuse plus radial mean, anisotropy, or graph-spectrum evidence against random/farthest/current modes.
