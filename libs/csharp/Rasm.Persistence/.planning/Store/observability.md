...
public sealed record SlotRegistry(FrozenSet<string> Slots) {
    ...
    public static SlotRegistry Mount(params ReadOnlySpan<StoreSlot> slots) {
        ...
        var forked = keys.GroupBy(static key => key, StringComparer.Ordinal)
    ...
    public static readonly FrozenDictionary<string, InstrumentArm> Arms =
        new Dictionary<string, InstrumentArm> {
            ...
            [SqliteStatHarvest.ConnectionSlot.ToString()] = static (set, payload) =>
                ...
                from done in MemoryRegions.TraverseM(row => set.Level(SqliteMemory, row.Value, payload.GetProperty(row.Field).GetInt64())).As()
                select unit,
            ...
            // Keyed level entries mount for a PARTITIONING tenant only — the kernel root row's `Tags` is empty,
            // so a single-tenant census contributes no `rasm.tenant` entry rather than a zero-valued sentinel;
            // class and tier stay receipt and census facts so the tenant cap never multiplies by their product.
            [StoreUsage.Slot.ToString()] = static (set, payload) =>
                StoreUsage.Decode(payload).Bind(census =>
                    toSeq(census
                        .Filter(static row => row.Tenant.Partitions)
                        .GroupBy(static row => row.Tenant.Entry))
                        .TraverseM(group => Seq(
                            ...
                            .TraverseM(measure => set.Level(measure.Name, group.Key, measure.Value)).As()).As()
                        .Map(static _ => unit)),
