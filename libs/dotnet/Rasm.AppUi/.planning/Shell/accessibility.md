...
flowchart LR
    ...
    accDescr: The screen catalog and the resolved theme grid crossing into the access proof sweep, the theme gate candidate rosters folding through the contrast gate into perceptual assertions, and both meeting in one access audit whose capability set is admitted against the whole check roster.
    ScreenCatalog --> AccessProof
    ResolvedTheme --> AccessProof
    ThemeGate --> ContrastGate
