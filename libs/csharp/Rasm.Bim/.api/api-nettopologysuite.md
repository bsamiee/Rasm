...
| :-----: | :------------------- | :------- | :------------------------------------------------------------------------------------------------------- |
|  [01]   | `Geometry`           | geometry | abstract root; `OgcGeometryType`, `Dimension`, `SRID`, `EnvelopeInternal`, `Coordinates`, `UserData`, `IsValid`/`IsSimple`/`IsEmpty`/`IsRectangle`, `Area`/`Length`, `Centroid`/`InteriorPoint`, `Apply(ICoordinateSequenceFilter)` (the in-place ordinate-edit visitor — `Coordinates` returns a detached `Coordinate[]`, so a reprojection writes back through this filter, not the array). Owns the predicate, boolean, and serialization methods enumerated in [03]/[04] |
