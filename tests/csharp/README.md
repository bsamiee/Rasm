...
- `tests/csharp/_architecture` carries the assembly boundary laws beside the infra-primitive suites proving the kits, the gate, and snapshot hygiene.
...
- Manual MTP runs route TRX with `--report-trx --results-directory .artifacts/csharp/trx/<project>`; assay-run suites route into its artifact scope.
...
- Clash node-link: `csharp:Rasm/Spatial/index#SPATIAL_INDEX` emits `NodeLinkProjection`, its `[WIRE]` region freezing the golden a scenario asserts.
