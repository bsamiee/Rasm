...
- Owner: `ScanRegistration` — the frozen owner discriminating by `RegistrationMode` over a `RegistrationSession`; `RegistrationPolicy` is the one tuning carrier for every voxel/correspondence/Tukey/solver-gain/multiway bar including the graduation ceilings, with a derived `voxel_schedule`; `RegistrationResult` the `gc=False` result whose `of` factory ravels the transform once and defaults the single-pose tuple, sharing one `_from_tensor` projector across the tensor arms and carrying the `span_facts` the weave stamps on the live span; `BootstrapEngine` (`KISS_MATCHER` | `OPEN3D_FGR`) the global-coarse-pose vocabulary and `NonRigidEngine` (`CPD` | `FILTERREG`) the probabilistic-warp one, `FilterRegObjective` the geometry term the permutohedral row discriminates on. `DeformationField` is the sealed non-rigid product — warped positions, per-point displacement magnitudes, and the mixture's own `sigma2`/`q` convergence evidence — riding the ONE result carrier's optional slot, never a parallel non-rigid result: every arm answers `RegistrationResult`, and the arms that recover a warp fill one more slot on it.
...
- Law: `_distributed` records the `REGISTRATION_TRANSFORM` charter row through the graduation `charter_record` derivation on the parent side of the offload — the HOSTILE kernel's meter is the worker's no-op, so a worker-side record meters nothing — reading the one `span_facts` fold the result already stamps on the span, so the measure and the span can never disagree and no spelling is hand-picked here.
- Law: `bench` rides the graduation `bench_boundary` fold over the whole `register` crossing — arity re-proof, offload, solver, weave — cloud-size-parameterized: the subject keys the exact `RegistrationMode` row and the source point count as `rasm.geometry.scan.registration.<mode>.p<points>`, so a latency row compares like-for-like across scan densities; latency and throughput rows per arm, zero instrument rows, graduation's `bench_terminal` wrapping the fold in the runtime `JobRun.bounded` envelope for a process-terminal run.
- Entry: `register` admits a session and a mode and returns `RuntimeResult[RegistrationResult]`. Its weave opens the seeded span, `async_boundary` fences the offload, `_flat` absorbs the lane's already-fenced result un-nested, and the cleared `Ok` stamps the result's `span_facts` on the live span while an `open3d`/`kiss-matcher` raise stays an `Error(BoundaryFault)` on it. Kernels take the lane conduit's pickled tap as a trailing offload arg and beat the graduation `GeometryPulse.REGISTRATION` point through `pulsed` — one solve-start beat per mode, one per multiway edge — so a `Hooks` tap streams convergence progress under the lane's lossy drop law.
...
- Output: `RegistrationResult` retains the actual transform, fitness, optional RMSE, optional deformation field, and provider convergence diagnostics. The non-rigid arm measures fitness and RMSE on the warped cloud through the same provider evaluation fold used by the rigid arms; deformation magnitudes remain a full quantile ladder beside the per-point field.
- Packages: `kiss_matcher`, `open3d`, `small_gicp`, `probreg` (the compiled registration backends, each a module-scope `lazy import`/`lazy from` so the marked distributions stay cold until their own arm runs — never an eager module-top import and never a function-local one the module-top roster hides), `numpy` (transform assembly via `np.eye`/`np.ravel`/`np.reshape`, never the uncatalogued `np.identity`/`ndarray.flatten`), `expression` (`Block.mapi` the per-edge multiway fold, `Block.collect` the session-preimage flat-map), `msgspec`, and the geometry graduation spine (`evidence_run`/`GeometrySubject`, `charter_record` the charter measure authority, `bench_boundary`/`bench_terminal`, `GeometryPulse` the pulse id roster) and runtime results per the fence imports, the mid-operation payload being the runtime `StageMark` this page marks with its own closed `RegistrationStage` roster.
...
- Boundary: the cleaned input `Cloud` is `scan/ingestion#INGESTION`'s product and carrier mint; deviation against a reference is `scan/deviation#DEVIATION`; surface reconstruction is `scan/reconstruction#RECONSTRUCTION`. The deformation field mints HERE and crosses to the deviation owner, which partitions it against its own signed band and never re-solves a warp; a live `probreg` `Transformation` never leaves this kernel, because it is an `open3d`-coupled native handle the pickle boundary cannot carry. No mesh repair, tessellation, or durable store here.
...
def _distributed(result: RegistrationResult, composition: ScopeKey) -> RegistrationResult:
    charter_record(GeometrySubject.REGISTRATION_TRANSFORM, result.span_facts, composition=composition)
    return result
...
class ScanRegistration(Struct, frozen=True):
    ...
    async def register(
        ...
        async def dispatch() -> "RuntimeResult[RegistrationResult]":
            if len(session) < 2:
            ...
            async def granted(grant: LaneGrant) -> "RuntimeResult[RegistrationResult]":
    ...
    async def bootstrapped(self, session: RegistrationSession, mode: RegistrationMode) -> "RuntimeResult[RegistrationResult]":
        if mode not in _SEEDABLE:
    ...
    def bench(self, session: RegistrationSession, mode: RegistrationMode, *, rounds: int = 32, warmup: int = 4) -> "RuntimeResult[Benchmark]":
        return bench_boundary(
            bench_subject(EvidenceScope.SCAN_REGISTRATION, mode, f"p{len(session[0])}"), partial(self.register, session, mode), rounds=rounds, warmup=warmup
        )
