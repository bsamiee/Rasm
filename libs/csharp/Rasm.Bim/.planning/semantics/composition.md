...
public static class MaterialProjection {
    ...
    // the seam Op-keyed smart-constructor (which owns the empty-set / non-positive-thickness / unnormalized-fraction
    // admission -> ElementFault), and Mint the content-keyed Node.Material ONCE — the four per-modality factories
    ...
    public static Fin<Node.Material> Project(BaseClassIfc relatingMaterial, double tolerance, Op key) =>
        relatingMaterial switch {
            ...
            IfcMaterialConstituentSet set => Mint(set.Name ?? "", tolerance, MaterialComposition.ConstituentSet(ConstituentsOf(set), key)),
            IfcMaterial material          => Mint(material.Name ?? "", tolerance, MaterialComposition.Single(MaterialId.Of(material.Name ?? ""), key)),
    ...
    static Fin<Node.Material> LayerSetOf(IfcMaterialLayerSet set, double tolerance, Op key) =>
        Mint(set.Name ?? "", tolerance, MaterialComposition.LayerSet(LayersOf(set), key));
    ...
    static Fin<Node.Material> ProfileSetOf(IfcMaterialProfileSet set, double tolerance, Op key) =>
        set.MaterialProfiles.AsIterable().HeadOrNone()
            ...
            .Bind(profile => Mint(set.Name ?? "", tolerance,
                MaterialComposition.ProfileSet(MaterialId.Of(profile.Material?.Name ?? set.Name ?? ""), ProfileRefOf(profile), key)));
    ...
    // SI-native seam entry stamping the Dimension.LengthDim canonical symbol), never the raw record ctor that would
    // strand a mm value as a "metre" measure (the mm-vs-metre import trap).
    ...
    // at IFC ingest). The draft id is a discarded placeholder, and a failed composition admission threads through.
    ...
    static Fin<IfcMaterialDefinition> Definition(DatabaseIfc db, MaterialComposition composition, MaterialId key, Func<ProfileRef, Option<IfcProfileDef>> profiles) {
        ...
        return composition.Switch(
            ...
            layerSet:      s => Fin.Succ<IfcMaterialDefinition>(new IfcMaterialLayerSet(
                                    s.Layers.Map(l => new IfcMaterialLayer(new IfcMaterial(db, l.Material.Value), l.ThicknessMm.Si / lengthScale, l.LayerName)), key.Value)),
    ...
    public static IfcMaterialSelect AuthorUsage(IfcMaterialDefinition definition, MaterialUsage usage) => usage.Switch(
        ...
        profileSet: u => definition is IfcMaterialProfileSet ps
                           ? new IfcMaterialProfileSetUsage(ps, (IfcCardinalPointReference)u.CardinalPoint) { ReferenceExtent = u.ReferenceExtent }
                           : (IfcMaterialSelect)definition);
