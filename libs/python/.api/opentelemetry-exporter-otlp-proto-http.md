...
| :-----: | :---------------------------------------------------------- | :------------- | :-------------------------------------------- |
|  [01]   | `OTLPSpanExporter(...)`                                     | construction   | span exporter with full TLS/header/env config |
|  [02]   | `export(spans: Sequence[ReadableSpan]) -> SpanExportResult` | export         | encode + POST batch with retry loop           |
|  [03]   | `force_flush(timeout_millis=30000) -> bool`                 | flush          | no-op true (HTTP path holds no queue)         |
|  [04]   | `shutdown()`                                                | lifecycle      | set shutdown flag, abort in-flight backoff    |
...
| [INDEX] | [SURFACE]                                                               | [ENTRY_FAMILY] | [RAIL]                                 |
| :-----: | :---------------------------------------------------------------------- | :------------- | :------------------------------------- |
|  [01]   | `OTLPMetricExporter(..., preferred_temporality, preferred_aggregation)` | construction   | metric exporter, temporality prefs     |
|  [02]   | `export(metrics_data, timeout_millis=10_000) -> MetricExportResult`     | export         | encode + POST `MetricsData` with retry |
|  [03]   | `force_flush(timeout_millis=10_000) -> bool`                            | flush          | no-op true                             |
|  [04]   | `shutdown(timeout_millis=30_000) -> None`                               | lifecycle      | release session, abort backoff         |
...
| [INDEX] | [SURFACE]                                             | [ENTRY_FAMILY] | [RAIL]                                   |
| :-----: | :---------------------------------------------------- | :------------- | :--------------------------------------- |
|  [01]   | `OTLPLogExporter(...)`                                | construction   | log-record exporter with full config     |
|  [02]   | `export(batch: Sequence[LogData]) -> LogExportResult` | export         | encode + POST `LogData` batch with retry |
|  [03]   | `force_flush(timeout_millis=10_000) -> bool`          | flush          | no-op true                               |
|  [04]   | `shutdown()`                                          | lifecycle      | set shutdown flag, abort backoff         |
