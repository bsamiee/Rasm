...
The journal consumes the `Shell/events.md` bounded drain off the UI thread: one single-reader loop drains `EvidenceDrain.Reader`, appends each fact under its owning `DocumentToken` identity, and accounts every shed element, so publication on the paint path stays a non-blocking `TryWrite` while the fold pays its cost on the consumer's own pace. `Shell/hooks.md` `Replay` re-enters captured rows for deterministic replay.
...
- Law: document attribution derives from the fact — a `DocumentCase` fact keys its own partition, a `GhEvidence` receipt keys the document its projector named, and an unattributable fact lands in the session partition rather than being dropped.
...
- Law: `Mount` owns the single-reader contract — one consumer loop drains `ReadAllAsync` under the journal's cancellation source, appends each event through the same `Append` gate, and disposal cancels the loop, completes consumption, and releases the journal exactly once; the drain's `SingleReader` promise is structural because the journal is the only reader the lease admits.
...
public sealed class SessionJournal : IDisposable {
    ...
    public static Fin<Lease<SessionJournal>> Mount(
        ...
        return from source in op.Need(drain)
               ...
               from consuming in op.Catch(body: () => Fin.Succ(Op.Side(action: () => _ = Task.Run(async () => {
                   await foreach (UiEvent fact in source.Reader.ReadAllAsync(cancellationToken: journal.drain.Token))
                       ignore(journal.Append(fact: new JournalFact.EventCase(Fact: fact), document: DocumentOf(fact: fact), key: op));
               }, cancellationToken: journal.drain.Token))))
    ...
    public void Dispose() => Op.SideWhen(
        ...
        action: () => {
            ignore(Op.Of(name: nameof(SessionJournal)).Catch(body: () => Fin.Succ(Op.Side(action: drain.Cancel))));
            drain.Dispose();
        });

    private static Option<Guid> DocumentOf(UiEvent fact) => fact.Fact switch {
        UiFact.DocumentCase document => document.DocumentId,
        UiFact.GraphCase graph => graph.SubjectId,
        _ => Option<Guid>.None,
    };
}
```
