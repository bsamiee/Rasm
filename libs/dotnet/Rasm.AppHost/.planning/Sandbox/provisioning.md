...
public sealed class UpdateRail {
    ...
    public UpdateRail(FeedBinding feed, Lifecycle host, SupplyChainGate.Runtime gate, Meter meter) {
...
- Output: the channel key tags every generated update instrument; a refused binding rides the boot `Fin` rail naming the channel and its key.
...
sequenceDiagram
    ...
    Drain-->>Rail: PhaseCommit
    Rail->>Rail: UpdateOutcome.Restarted
