...
- `Quantize`/`FixedScaleOffset`/`BitRound` discard precision by `digits`/`scale`/`keepbits` irreversibly; the array result retains the selected lossy parameter where a caller grades fidelity.
