...
- Entry: `Captures.Run(DocumentSession, MonotonicTimeline, CaptureRequest) : Fin<CaptureArtifact>` is the sole public execution entry; `CaptureRequest.Transparent`/`Depth`/`Sequence` admit each modality; internal `Captures.Stage(session, plans, consume)` shares settings acquisition with the Exchange landing without exposing `ViewCaptureSettings`.
