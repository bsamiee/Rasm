...
public sealed class ExtractSurfaces : Component<ExtractSurfaces.Spec> {
    public sealed class Spec : IComponentSpec {
        ...
        public static Seq<IOutputGroup> Outputs { get; } = Seq<IOutputGroup>(
            ...
            ShapeOutput.Query(input: Geometry, port: Port.List<Brep>(name: "Top Surface", code: "TS", info: "Trimmed face(s) with maximum world-Z centroid; ties within tolerance."), operation: static (_, _) => Rasm.Analysis.Query.Faces<object, Brep>(aspect: Rasm.Analysis.Faces.Top)),
            ShapeOutput.Query(input: Geometry, port: Port.List<Brep>(name: "Bottom Surface", code: "BS", info: "Trimmed face(s) with minimum world-Z centroid; ties within tolerance."), operation: static (_, _) => Rasm.Analysis.Query.Faces<object, Brep>(aspect: Rasm.Analysis.Faces.Bottom)),
            ShapeOutput.IndexedFaceDetails(
                input: Geometry,
                index: Index,
                ...
                component: Port.List<ComponentIndex>(kind: PortKind.Generic, name: "Face Component", code: "CI", info: "Source Brep face component index selected by the clamped Index input."),
