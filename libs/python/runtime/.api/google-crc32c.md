...
| :-----: | :------------------------------------- | :------------- | :-------------------------------------------------------------------- |
|  [01]   | `value(data: bytes) -> int`            | digest         | CRC32C of a buffer; the fence compares this against `data_crc32c`     |
|  [02]   | `extend(crc: int, data: bytes) -> int` | digest         | extend over an appended buffer; `extend(value(a), b) == value(a + b)` |
|  [03]   | `Checksum(data=b"")`                   | streaming      | incremental digest (`update`/`digest`/`hexdigest`/`copy`/`consume`)   |
