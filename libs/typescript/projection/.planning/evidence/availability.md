...
- [1]-[AVAILABILITY_GATE]: Owns `AvailabilityStore`, the `availabilityStore` two-arm constructor, and the `isEnabled` conjunction read.
- [2]-[GATE_POLICY]: Owns `DegradationLevel` (the ordered health frame), its `Order`, and `gatePolicy` (the one frozen level-to-admitted-tier table the conjunction reads).
