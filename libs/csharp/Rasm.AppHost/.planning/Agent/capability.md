...
public static class DescriptorSurface {
    public static IServiceCollection Describe(IServiceCollection services, params ReadOnlySpan<CapabilityDescriptor> rows) {
        ...
        // alike fold here, so the rasm.apphost.capability.roster gauge reads a live level. Span
        // length hoists to a local because a lambda cannot capture the ref-struct parameter.
        var admitted = rows.Length;
        ignore(LevelCells.Live.Roster.Swap(count => count + admitted));
