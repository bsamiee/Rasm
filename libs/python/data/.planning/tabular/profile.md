...
class Interrogation(Struct, frozen=True):
    ...
    @classmethod
    def of(cls, label: str, plan: "pb.Validate", data: Any, steps: int, key: ContentKey) -> "Interrogation":
        return cls(
            plan=plan,
            label=label,
...
class QualityProfile(Struct, frozen=True):
    ...
    ) -> "RuntimeResult[Interrogation]":
        sampling = (sample_n, sample_frac, get_first_n, extract_limit)

        def completed(plan: "pb.Validate", key: ContentKey) -> Interrogation:
            run = Interrogation.of(self.label, plan, data, len(self.steps), key)
            match Block.of_seq(run.failed_fraction.values()).sort(reverse=True).try_head():
                case Option(tag="some", some=worst):
                    Metrics.record({"rasm.quality.breach_fraction": worst}, domain="quality", kind=run.grade.name)
        ...
        with _TRACER.start_as_current_span(
            ...
            return interrogated.bind(
                lambda plan: self.fingerprint(sampling).map(
                    lambda key: completed(plan, key)
                )
            )

    @beartype(conf=FAULT_CONF)
    async def report(self, data: Any, report: ProfileReport, interrogated: "Option[Interrogation]" = Nothing) -> "RuntimeResult[ProfileFrame]":
        return await async_boundary(PROFILE_REPORT, lambda: on_thread(self._report, data, report, interrogated), catch=_probe_raises())

    def fingerprint(self, sampling: tuple[int | None, float | None, int | None, int]) -> "RuntimeResult[ContentKey]":
    ...
    def _report(self, data: Any, report: ProfileReport, interrogated: "Option[Interrogation]") -> ProfileFrame:
        ...
        carried = interrogated.map(lambda run: run.grade).default_value(None)
        match report:
...
---
flowchart TD
    accTitle: Profile probe plan and result flow
    accDescr: ProbeStep rows, bound tables, thresholds, and actions fold into one pb.Validate; a single interrogate yields the Interrogation carrying the graded plan and the Interrogation with its ContentKey, and report threads that same run into the ProfileFrame handed to the great-tables tier.
    ...
    interr -->|"n_passed·n_failed·f_passed·f_failed·above_threshold"| result["Interrogation"]
    data -->|"get_row_count·get_column_count"| result
