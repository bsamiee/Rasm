...
The agent-session-as-actor binding — `AgentSessionActor`, the addressable `@effect/cluster` `Entity` that pins one long-lived agent conversation to one shard single-writer-per-key, so a multi-turn agent session is a sharded actor whose mailbox serializes concurrent turns rather than racing two writers on one conversation. It is the third member of the one `./agent` subpath closure: where `folder:agent/runtime#AGENT_RUNTIME` owns the per-step durable agent composition and `folder:agent/mcp#MCP_TRANSPORT` owns the host tool transport, this page owns the SESSION lifecycle as a placed actor. The actor reuses the `folder:messaging/entity#ENTITY` addressable-actor placement mechanics (`Entity.make`/`toLayer`/`getShardId`, `ClusterSchema.Persisted`) and the `folder:agent/runtime#AGENT_RUNTIME` `DurableAgent` step; the session STATE — the `AgentJournal` cursor — persists through the one `folder:persistence/store#STORE_BOUNDARY` `PgClient`, never a parallel store, while the live per-session RESOURCE — the persisted `Chat` handle and the host tool-transport session — is held through the `folder:messaging/entity#ENTITY` `EntityResource` ROW bound to the session key (acquired once on `Start`, shared across every `Turn`, released on `End` or idle-TTL eviction), never re-acquired per `Turn`. This page is node-only and crosses no .NET wire beyond the agent's own tool transport.

## [01]-[INDEX]

- [01]-[SESSION_ACTORS]: owns the agent-session `Entity`, the single-writer-per-session placement, the durable session lifecycle, and the journal-cursor resume.

## [02]-[SESSION_ACTORS]

- Owner: `AgentSessionActor`, the `@effect/cluster` `Entity` that places one agent conversation per key single-writer — a `Start`/`Turn`/`Interrupt`/`End` `RpcGroup` behavior, a `toLayer` handler pinned to one shard per session id, and the `getShardId`/`getShardGroup` placement; the session lifecycle composes the `runtime#AGENT_RUNTIME` `DurableAgent` step over the placed per-key state.
- Cases: the actor protocol is one `RpcGroup` of `Rpc.make` procedures — `Start` (open a session, acquire the persisted `Chat` resource through the `messaging/entity#ENTITY` `EntityResource` ROW and hydrate the `AgentJournal` cursor), `Turn` (run one `runtime#AGENT_RUNTIME` `DurableAgent` step against the session's shared `Chat` resource, the single-writer mutation), `Interrupt` (suspend the in-flight agent workflow by execution id), `End` (flush, fire the `EntityResource` `CloseScope`, and release) — every procedure `annotateRpcs(ClusterSchema.Persisted, true)` so the session messages route through `MessageStorage` for exactly-once replay; `Entity.make("AgentSession", [...rpcs])` builds the entity and `entity.toLayer(handlers)` installs the behavior, the handler reaching the `EntityResource`-held `Chat`, the `AgentJournal`, and the one `SqlClient`. The persisted `Chat` handle is the `EntityResource.make({ acquire, idleTimeToLive })` resource bound to the session key — `Start` acquires it, every `Turn` reads it through `EntityResource.get` (re-acquiring only after an idle eviction), and `End`/`Interrupt` fire `CloseScope` to release it deterministically — so the conversation handle survives a runner restart and is shared across turns rather than re-hydrated per message. The single-writer-per-session contract is the shard manager's placement: `getShardId(sessionId)` pins one shard per session so two concurrent `Turn` messages on one session serialize through one mailbox and never race the conversation, the property a request-driven proxy cannot model; `getShardGroup` routes a per-tenant agent pool to its tenant shard group. Interrupt/resume is the agent's own resume composed with the actor's durability — `Interrupt` suspends the `ClusterEngine` agent workflow, and a later `Turn` or an external resume re-hydrates the persisted `Chat` and replays from the `AgentJournal` cursor, so a restarted runner reattaches the exact session conversation.
- Entry: the actor rides the SAME `execution/backplane#RUNNER_AND_SCHEDULING` `RunnerBackplane` shard manager the cluster engine and the `messaging/entity#ENTITY` actors layer onto — the agent session is the agent-tier actor modality, distinct from the `messaging/entity#ENTITY` collaborative-document actor; the `runtime#AGENT_RUNTIME` `DurableAgent` runs inside the `Turn` handler as a durable unit on `execution/engine#ENGINE`; the per-session state (the persisted `Chat` handle, the `AgentJournal` cursor) persists through the one `persistence/store#STORE_BOUNDARY` `PgClient`, never a second store; the `EntityProxy.toRpcGroup(actor)` projection derives the request-tier RPC the panel-to-host session dispatch dials, so a session is started and advanced over the one RPC surface rather than a hand-built client.
- Wire: the actor carries no .NET wire type of its own — its only wire contact is transitively through the `mcp#MCP_TRANSPORT` host tool transport a `Turn` may drive, which consumes the decoded host MCP projection at the `interchange` boundary; the session state and the placement are node-internal.
- Packages: `@effect/cluster` for the `Entity`/`Sharding` placement (`Entity.make`/`toLayer`/`client`/`getShardId`/`getShardGroup`, `ClusterSchema.Persisted`, `EntityProxy.toRpcGroup`, `EntityProxyServer.layerRpcHandlers`) and the `EntityResource.make`/`CloseScope` per-session resource lifetime the `messaging/entity#ENTITY` ROW owns, `@effect/rpc` for the session behavior `RpcGroup`, `@effect/ai` for the persisted `Chat` session the `DurableAgent` step drives, `@effect/sql`/`@effect/sql-pg` for the per-session state through `persistence/store#STORE_BOUNDARY`, `@effect/platform-node` for the driver host.
