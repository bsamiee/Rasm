...
|  [04]   | `Processor.getOptimizedCPUProcessor(inBD, outBD, flags)`         | CPU executor with explicit bit depths and optimization flags   |
...
|  [12]   | `Baker.setInputSpace/setTargetSpace/setFormat` then `bake()`     | LUT egress; `Baker.getFormats()` lists every writable format   |
