...
`ElementFault` is the seam's one closed `[Union]` fault family over structural-graph, value-admission, projection, and content-verification failures. Its seven direct `Fault` cases derive numeric identity from `FaultBand.Element` and `[FaultCase]`; no category registry or conversion layer exists.
