...
- Cases: `DerivationStage` rows 7; `DeriveDepth` rows 3; the stage rail is ONE `Fin` LINQ expression — DfM gate → routing rows → fleet match → depth-gated assembly → compose; setup stays the single-setup dormant fold until `Fixturing/setups` lands (QUEUED row 15 — `Setup.Schedule(Seq<Operation>, SchedulePolicy)` is the TYPE contract the setup stage composes; `PlannedStep.Setup` reads its partition then, `0` now); program and documentation are RECORDING stages by law (keys `None`, the `Document` toggle marks traveler intent).
...
public static class Derivation {
    ...
    // single-setup dormant fold (QUEUED Fixturing/setups); program/documentation RECORD — Run(Post)/Run(Document) emit.
...
flowchart LR
    ...
    Plan -.->|4 setup QUEUED Fixturing/setups| Setup["single-setup dormant"]
    ...
    Capability["Spec/capability Gate (QUEUED)"] -.->|CapabilityVerdict input-carried run-N→N+1| Plan
```
