...
- Span pairing (D50): `fan_out` wraps the bounded task group in one `assay.fan_out` parent span (`assay.checks_total=N`), and each `_guarded` gets one child span keyed `check.tool.name` with `run_id`/`tool` attributes that nests under it through anyio's context copy at `start_soon` (runtime-verified: children carry `parent==fan_out.span_id`). An explicit OTel `Link` is therefore redundant — the parent context already propagates, so `traced` gains no `links` param. Retry correlation binds in `traced` at the engine seam, not in `logged` (which the engine never weaves), so a retried spawn's attempts share the tool span and the structlog `claim` keys correlate via the shared `run_id`. **Phase-2 caveat:** the engine-seam `traced` is a sync `Hom` decorator woven over the async `Spawn`, so the child span currently opens/closes around coroutine *creation*, not the await — it is degenerate (zero-duration, no result status); the async-aware fix is tracked for runtime verification, while the parent `assay.fan_out` span (which wraps the real task-group await) is correct.
