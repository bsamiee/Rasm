...
- arrow-result seam: every statement returns Arrow `RecordBatch`, so a Spark/Hive/Impala result and an in-process batch are the SAME `Apache.Arrow` type (`libs/dotnet/.api/api-arrow.md`), folding directly to Parquet (`api-parquetsharp.md`) or a Delta table (`api-deltalake.md`) with zero re-materialization
