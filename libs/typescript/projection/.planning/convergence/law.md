...
- [CRDT_LAW_ADMISSION]: [RESOLVED] — the `crdt` column-family op carries the `CrdtOpWire` join-semilattice algebra on the `OpLogEntryWire.payload` slot, decoded by `causality/vector#CRDT_SEMILATTICE` off the `interchange/Codec/codec#CRDT_OP_DECODE` ten-arm union; the convergence law that gates a `CrdtOpWire` arm asserts the same delivery-order independence over the version-vector `vectorJoin` least-upper-bound rather than the scalar LWW state.
