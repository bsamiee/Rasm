...
- `openid-client` (`.api/openid-client.md`): `discovery` resolves `jwks_uri` into `createRemoteJWKSet` for `fetchUserInfo` and for the `id_token` `authorizationCodeGrant` returns — that package checks the token's claims but reaches no key set, so `jwtVerify(idToken, createRemoteJWKSet(jwksUri), { issuer, audience })` here holds the signature authority for both the browser ceremony and the machine plane; `randomDPoPKeyPair`/`getDPoPHandle` take their `cnf.jkt` from `calculateJwkThumbprintUri`, and both packages share the one WebCrypto `CryptoKey` path.
