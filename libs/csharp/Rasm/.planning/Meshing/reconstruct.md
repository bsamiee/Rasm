...
`Reconstruction` owns ONE `Reconstruct` entry over `ReconstructionPolicy`, and each policy case returns a `Spatial/fields` value with typed evidence. `SignedHeatSpine`, `MeshSdf`, and `IsoSurface` own the delegated signed-distance and extraction rails. Numeric kernels compose `Numerics/calculus` and `Numerics/matrix`; failures stay on `Fin`.
...
- Boundary: `SignedHeatSpine` owns one heat→divergence→Poisson→calibrate law. Dropped octree knobs and unread `Degree` stay excluded until a real discretization consumes them. Boundary-source rows reject flipped intrinsic snapshots; closed-grid rows admit only watertight, solid, closed, oriented topology. `PoissonGrid.SampleTrilinear` returns a positive outside value instead of clamping to an edge. Native evaluator callbacks count failures with `Interlocked`; every linear solve routes through `Numerics/matrix`, and `Op.Catch` converts the native callback boundary.
...
// desynchronizable duplication violates this page's receipt law.
[SmartEnum<int>]
...
// SdfMeshMethod IS the classification: mature Status/Domain enums were 1:1:1 renames carried
...
public static class MeshSdf {
    ...
    // SpatialIndex owns cached-moment descent and exact leaf solid angles; a local walker is deleted.
...
One owner per axis; capability is a case, row, or fold arm, never a sibling surface. Each `[RAIL]` names one return rail, and indexed notes name each kind.
...
- [RECONSTRUCTION_KERNELS] — RBF composes kernel-matrix interpolation or smoothed least squares; MLS gates rank and normal agreement; Levin alternates a covariance-plane seed with Brent refinement before a ridge height fit; APSS uses Pratt normalization and Taubin projection. Receipt laws cover interpolation residuals, oriented-sign recovery, sample counts, and Poisson interior sign.
- [SCREENED_POISSON] — Dense-lattice Poisson composes trilinear normal splats, one-sided seven-point stencils, screening outer products, and explicit boundary rows. Definite systems use `CholeskySparse`; singular systems use `SingularSolveDetailed` under a pinned gauge. `UnscreenedEquivalence` witnesses zero screening structure and data residual.
- [SIGNED_HEAT_SPINE] — ONE four-stage law spans P1 tet FEM, boundary-source CR, and closed-surface volume-grid rows. `SignedHeatTime.Resolve` derives row-specific time from characteristic cell size. Receipts witness source proximity, sign consistency, residual gates, and every stage solve.
- [MESH_SDF_AND_ISO] — Cached-moment winding queries classify sign and native closest points measure distance; both signed-heat methods project the shared spine. `Mesh.CreateFromIsosurface` counts parallel evaluator failures through `Interlocked`, and any failure invalidates the receipt. Approximate fields never feed predicate decisions.
