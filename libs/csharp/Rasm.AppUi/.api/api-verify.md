...
| :-----: | :-------------------------------------------------------------------- | :--------------- | :------------------------- |
|  [01]   | `UseStreamComparer(StreamCompare, extensions?)`                       | `SettingsTask`   | custom byte comparison     |
|  [02]   | `UseStringComparer(StringCompare, extensions?)`                       | `SettingsTask`   | custom text comparison     |
|  [03]   | `UniqueForRuntime` / `UniqueForTargetFramework[AndVersion]`           | `SettingsTask`   | host-specific key          |
|  [04]   | `UniqueForAssemblyConfiguration`                                      | `SettingsTask`   | configuration-specific key |
|  [05]   | `AddNamedGuid` / `AddNamedDateTime` / `AddNamedTime` / `AddNamedDate` | `SettingsTask`   | named-value stabilization  |
|  [06]   | `OnVerifyMismatch` / `OnFirstVerify` / `OnVerify`                     | `SettingsTask`   | verification hooks         |
|  [07]   | `AddExtraSettings(Action<JsonSerializerSettings>)`                    | `SettingsTask`   | serializer configuration   |
|  [08]   | `AppendValue` / `AppendValues`                                        | `SettingsTask`   | named snapshot context     |
|  [09]   | `DisableDiff`                                                         | `SettingsTask`   | suppress diff launch       |
|  [10]   | `UseDirectory` / `UseFileName`                                        | `VerifySettings` | snapshot placement         |
|  [11]   | `UseParameters` / `UseTextForParameters`                              | `VerifySettings` | parameter key              |
|  [12]   | `AddScrubber` / `ScrubLinesContaining`                                | `VerifySettings` | content scrubbing          |
|  [13]   | `IgnoreMember`                                                        | `VerifySettings` | member omission            |
|  [14]   | `AutoVerify`                                                          | `VerifySettings` | automatic acceptance       |
