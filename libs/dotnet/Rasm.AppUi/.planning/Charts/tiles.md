...
- Owner: `WatchBound` and `WatchSample` — the bound pair and the value-with-age carrier every comparator reads; `WatchComparator` — the breach vocabulary; `WatchRule` — one alert row; `WatchCrossing` — the raised fact; `WatchFold` — the probed sample stream and the armed subscription; `FeedHealth` and `FeedFreshness` — the consumer end of the feed-freshness seam.
