...
[STACKING]:
- `UnitsNet`(`libs/dotnet/.api/api-unitsnet.md`): SAF and the Bim `QuantitySet` share the quantity path — `UnitsNetExtensions.CreateUnit<TUnit>` lifts a SAF cell scalar to a typed quantity, every exchanged magnitude mints FROM an already-SI shared value through the quantity's own `From*` factory and reads back through its typed SI accessor (`Meters`/`Newtons`/`Pascals`/`NewtonsPerMeter`/`NewtonMetersPerRadian`/`Kelvins`), so neither `ToUnit(UnitSystem.SI)` nor `QuantityTypeConverter` is reached on the SAF lane, and the bound `Length`/`Pressure`/`Force` typed-struct surface is the SAF field-type set
...
- `GeometryGymIFC_Core`(`.api/api-geometrygym-ifc`): SAF and the IFC structural-analysis entity graph are the two wires that MEET at the shared structural payloads — IFC the in-memory semantic authority, SAF the spreadsheet exchange; both lower onto the `Exchange/saf#SAF_EXCHANGE` neutral `Generic` edge/bag attrs over the shared `ElementGraph`, never two parallel structural models
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-json.md`): the SAF `Excel*` enums are boundary vocabulary the projector lowers onto neutral `PropertyValue.Enumerated` rows over the schema's own allowed-set, never a type re-exported across the boundary
- `LanguageExt.Core`: `IExcelExportService.Export` returns an `ExcelExportResult` with `IsSuccess` + `ValidationResults`, not an exception; the Bim codec lifts a failed result BARE onto `Fin<T>` through the `Model/faults#DETAIL_ROSTER` mint `Detail.SafInvalid.At(stage, ExcelValidationResult.Format(result.ValidationResults))` (the row's own arm supplies the case and the band IS the `Expected` `Code`), threading the `ExcelValidationResult` rows as the detail

[LOCAL_ADMISSION]:
- SAF is the XLSX EXCHANGE wire over the `Exchange/saf#SAF_EXCHANGE` shared payloads and never the canonical model — the shared `ElementGraph` is the in-memory authority and `ExcelModel` round-trips those payloads to and from `.xlsx` through the `SafCodec.Run` entry, whose `SafOp` value carries the direction
...
- SAF read/write throws remain exact exceptional errors through `Try.lift`; unmapped object types and error-severity validation verdicts mint `BimFault.Refused` under the model codec/rejected axes
- `ExcelModel.SystemOfUnits` (`Metric`/`Imperial`) and `OriginalVersion` are decode context — every SAF quantity crosses as its own typed SI accessor read, never a `ToUnit` coercion, so the element graph is unit-normalized regardless of the workbook regime
