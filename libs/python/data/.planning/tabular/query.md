...
class QueryEngine(Struct, frozen=True):
    ...
    @beartype(conf=FAULT_CONF)
    async def run(self, spec: QuerySpec) -> "RuntimeRail[tuple[pa.Table, QueryCensus]]":
        def completed(admitted: QuerySpec, table: pa.Table, fold: "Provenance") -> "RuntimeRail[tuple[pa.Table, QueryCensus]]":
