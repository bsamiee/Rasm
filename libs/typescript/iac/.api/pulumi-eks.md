...
- Realized-evidence outputs read straight off the component: `cluster.oidcProviderArn`/`oidcProviderUrl`/`oidcIssuer` are the IRSA anchors minted under `createOidcProvider`, `cluster.clusterSecurityGroupId`/`nodeSecurityGroupId` the security-group ids, `cluster.instanceRoles`/`autoModeNodeRoleName` the node IAM identities, and `cluster.defaultNodeGroup` the default capacity handle. `AccessEntryArgs.accessPolicies` is a `{policyArn, accessScope}` map keyed by entry name on `accessEntries`.
...
|  [03]   | `cluster.provider`        | ready-bound provider handle; explicit `k8s.Provider({ kubeconfig })` remains the arm-seam spelling         |
...
- rail: prepared-aws
- Each node group and addon is `new X(name, args, opts?)`. Every node group binds `cluster` (required) plus the scaling axis `instanceTypes`/`scalingConfig`/`capacityType`/`amiType`/`operatingSystem`/`diskSize`/`labels`/`taints`/`launchTemplate`/`enableIMDSv2`/`kubeletExtraArgs`/`nodeadmExtraOptions`/`releaseVersion`/`forceUpdateVersion`/`ignoreScalingChanges`; every addon binds `cluster` (required) plus `resolveConflictsOnCreate`/`resolveConflictsOnUpdate`. `VpcCniAddon` adds the CNI knobs `enableNetworkPolicy`/`enablePrefixDelegation`/`enablePodEni`/`warmEniTarget`/`warmIpTarget`/`warmPrefixTarget`/`customNetworkConfig`/`externalSnat`. `NodeGroupV2` adds `minRefreshPercentage`/`launchTemplateTagSpecifications`; `Addon` carries `addonVersion`/`configurationValues`/`serviceAccountRoleArn`/`preserve`; `getRoleProvider` is the function twin of `ClusterCreationRoleProvider`. The `createManagedNodeGroup`/`createNodeGroupSecurityGroup`/`createStorageClass` function twins compose the component rows inside `.apply` folds.
...
|  [04]   | `Addon`                       | generic EKS-addon lifecycle (`addonName` required)                                                  |
|  [05]   | `VpcCniAddon`                 | CNI specialization (`clusterName` required); post-plane twin of `ClusterArgs.vpcCniOptions`         |
|  [06]   | `ClusterCreationRoleProvider` | `{ profile?, region? }` → `role: Output<aws.iam.Role>`; creator identity for `creationRoleProvider` |
