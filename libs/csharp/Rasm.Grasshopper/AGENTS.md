...
- Keep snapping on one rail. `SnappingConstraints.SnapObject` already includes native wire snapping; call `SnapWires` only for custom rectangle/bounds probes. Grid snapping belongs inside `SnappingPolicy` and `LayoutOp`, not a side service or dormant setting.
...
- For dirty-tree target work, prove UI changes with the Grasshopper project build, UI-only formatting where needed, and `git diff --check` on the target. Run `uv run python -m tools.quality static check` when requested, but classify unrelated changed-file failures outside `libs/csharp/Rasm.Grasshopper/UI` instead of chasing them.
