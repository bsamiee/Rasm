...
- import-surface reality: top-level `pdf_oxide` re-exports the roots, factories, value objects, and signing/crypto/barcode/split/logging functions; return-type classes and the stream-safety + OCR-model functions live on the inner `pdf_oxide.pdf_oxide` module — reach a return type through its owning method's return value, never `from pdf_oxide import Page`.
- reflection outranks the shipped `.pyi`: `page(index)` yields the edit-surface `PdfPage` (`add_text`/`set_text`, no `.words`), while `pages` iteration yields the extract-surface `Page` (`.words`/`.markdown`/`.render`).
- license axis: every concern `pymupdf` owns and `pdf_oxide` covers — render, layout-aware extract, search, redaction, sanitize, outline/widget/annotation recovery — routes through `pdf_oxide` on the closed/distributed/SaaS path, reserving the AGPL `pymupdf` arms for permissive or internal pipelines. Supersession is per-concern, never a parallel second engine on equal license footing.
- extract axis (`document/lens#LENS`): `extract_words(profile=ExtractionProfile.academic())`/`extract_text_lines`/`extract_chars` carry geometry → `RunNode` leaves; `extract_spans` → styled spans; `extract_tables(table_settings=)` → `TableNode`; `within(page, bbox)`/`PdfPageRegion` is the region arm; the `to_*_all` family converts whole documents. An `ExtractionProfile` is the layout-tuning knob, never a hand-rolled gap-threshold table.
- render axis (`document/emit#DOCUMENT` raster + `document/egress#FINISH`): `render_page(dpi=, format=, jpeg_quality=, excluded_layers=)` emits encoded bytes; `render_pixmap` returns the premultiplied-RGBA `RenderedPixmap` the `graphic/raster/io#IO` owner wraps (unpremultiply before a straight-alpha consumer); `flatten_to_images(dpi=)` rasterizes the document.
