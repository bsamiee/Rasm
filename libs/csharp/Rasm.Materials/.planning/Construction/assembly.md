...
- [PANEL_BUILDUP_REUSE]: a `Component/panel#PANEL_FAMILY` board run is the SAME buildup geometry with NO new owner — the board face/core/face `PanelLayer` stack coerces through the EXISTING `[03]` `CompositionAuthor.LayerSet`, whose `MeasureValue.Of(_, UnitsNet.Units.LengthUnit.Millimeter, key)` admission (the `[H2]` seam coercion) already lifts each board-layer thickness into the seam `IfcMaterialLayerSet` without a second coercion, and the multi-ply gypsum/sheathing/rigid-board buildup stacks each board onto a `NormalOffsetMm`-offset `Placement` exactly as a wall layer set does. The panel run adds NO `Placement` column and NO `ConstructionFault` case: the strength-axis run direction rides the `Component/panel#PANEL_FAMILY` `PanelOrientation` the `Construction/layout#ASSEMBLY_FOLD` `SheetCoursing` reads, the closing rip-cut board rides `Placement.Cut`, the field/edge fastener stations are `Placement` rows the `SheetCoursing` emits, and an over-long sheet course rails the EXISTING `ConstructionFault.Course` — the `Placement`/`CompositionAuthor`/`ConstructionFault` surface is total over the panel family as authored.
