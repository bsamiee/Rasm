...
flowchart LR
    ...
    accDescr: One compute entry discriminating input shape into three policy-selected arms behind an animated content-key cache short-circuit, arm rejects converging on one red fault path that rejoins the fold, and a dotted evidence trace feeding the single solution.
    ...
    ArmB --> Fold
    ArmC --> Fold
    ArmA e3@-.->|"reject"| Fault[/Fault path/]
