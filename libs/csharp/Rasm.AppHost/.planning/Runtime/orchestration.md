...
- Auto: each `WorkflowStep` carries its `StepKind`, an attempt count, and the resume cursor (the wire-stable keys plus the step index) so a step is replayable from durable state, never a live closure; the instance's `Chain` is the `EventLog.Chain` head so a committed step's `CommandReceipt` chains into the same hash-chained log a live command chains into, and the step's own `Hash` IS that chain link read off the dispatch head rather than a second digest minted here, so the instance's integrity is the chain's tamper-evidence and the durable step row and the durable log entry name one content identity; a `StepKind.Timer` resolves through `SchedulePort.Next` so a durable wait is one `ScheduleEntry` row, a `StepKind.Signal` suspends the instance to `waiting` until the matching channel signal arrives or the timeout fires, and a `StepKind.PersistentJob` registers its `ScheduleEntry` so a recurring job survives restart; the saga compensation is a `StepKind.Compensation` whose `CommandIntent` rolls forward the prior step's undo through `CommandAlgebra.Batch`'s reverse-fold, never a phantom undo.
...
- Packages: Rasm (kernel `CorrelationId`/`TenantContext` vocabulary), Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, BCL inbox
...
- Packages: Rasm (kernel `CorrelationId`/`TenantContext` vocabulary), LanguageExt.Core, NodaTime, Thinktecture.Runtime.Extensions, BCL inbox
...
public static class Orchestrator {
    ...
    static IO<WorkflowInstance> Dispatch(OrchestrationRuntime runtime, WorkflowInstance instance, WorkflowStep step, CommandIntent intent) =>
        ...
        from settled in receipt.Txn is CommandTxn.Committed or CommandTxn.Compensated
            // `Commit` seats the chain link `CommandDispatch.Run` just minted and published as the step's hash —
            // read off the dispatch chain head, never re-derived here. Minting a second descriptor-only digest
            // beside the chain was the deleted form: it gave one command two content identities, so a step and its
            // own log entry disagreed on what the step was and the instance's chain-integrity claim was empty.
    ...
    // Each unwound step also RETURNS its own charged vector through the one broker the forward leg debited,
    // so a compensated saga restores the balance work the tenant no longer holds had drawn — under a bound
    // seam that return is the coordination `BudgetCredit` increment, under none the per-process cell. Refund
    // rides the STEP's receipt because the charge was per command, and an absent receipt yields no unwind row
    // at all rather than a fabricated zero. Refusal never blocks the unwind: the instance is already faulting.
    static IO<WorkflowInstance> Compensate(OrchestrationRuntime runtime, WorkflowInstance instance, WorkflowStep failed) =>
        instance.Steps.Filter(static s => s.Status == StepStatus.Committed && s.Kind is StepKind.Activity).Rev()
            .Choose(committed => committed.Kind is StepKind.Activity a
                ...
                : Option<(CommandIntent Intent, CostVector Charged)>.None)
