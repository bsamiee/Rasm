...
- An external member appears as settled fence code only when the folder's `.api/` catalogue verifies its spelling and its deprecation status read from the published declarations — existence alone never admits, and a member-level `@deprecated` bans the member even where its family survives; otherwise it is a marked RESEARCH item, never prose and never fence code; a fence contradicting a sibling RESEARCH item fails.
