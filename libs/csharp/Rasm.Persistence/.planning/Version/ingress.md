...
// envelope-source set the gate reads, the bounded batch width one `Consume` drains, the `ResumeOrigin` a
// rebalance resolves against, the `Fallback` reset governing a group with NO committed offset at all, and the
// `PollInterval` pinned onto the client. `Fallback` is a row value because it decides whether a fresh
// subscription replays the topic's whole retained history or joins live, and leaving it to the client default
// makes that a broker-configuration accident on a rail whose dedup budget is finite; `Error` is the arm a source
// takes when a missing committed offset is itself the fault to surface rather than a position to guess.
// `PollInterval` is pinned for the same reason AND because the rebalance resolve budget DERIVES from it — an
// unpinned interval leaves that budget reading a client default no row on this page states.
...
// conservation sum — no envelope was offered, so folding it in would inflate a count of records by a count of
...
public static class CdcIngress {
        ...
        from folded in Range(0, source.Batch).FoldM(
            ...
            (state, _) => IO.liftAsync(async () => {
                ...
                await consumer.ConsumeAndProcessMessageAsync(async (result, _, processingToken) => {
                    // End-of-partition results carry a position the offset law still stores and no envelope the
                    // conservation fold ever counted, so `Consumed` reads what the broker actually handed over.
    ...
    // letter, because no envelope was offered and the conservation fold counts what the broker handed over —
