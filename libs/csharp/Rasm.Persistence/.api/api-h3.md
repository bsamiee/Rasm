...
- license: BSD-3-Clause (`<license type="file">LICENSE</license>`; Uber H3 reference port)
...
- dependency: `NetTopologySuite` 2.5.0 (the geometry types `Point`/`Polygon`/`MultiPolygon`/`Geometry`/`LineString`/`Coordinate`/`GeometryFactory` flow through; co-pinned via the admitted NTS stack)
...
|  [03]   | `H3.Model.Mode`           | enum               | `Cell`/`UniEdge`/`Vertex` index-mode discriminant on the high bits          |
...
|  [02]   | `DirectedEdgeToCells(this H3Index edge)` (= `GetIndexesFromUnidirectionalEdge`) | edge | `(origin, destination)` tuple from an edge index |
|  [03]   | `EdgeLengthMeters/Kilometers/Radians(this H3Index edge)` | metric         | great-circle edge length in the chosen unit               |
|  [04]   | `CellAreaInMSquared/KmSquared/RadiansSquared(this H3Index)` | metric       | exact spherical cell area                                 |
|  [05]   | `GetRadiusInKm(this H3Index)` / `GetFaces(this H3Index)`  | metric         | cell circumradius / icosahedron faces a cell touches      |

## [04]-[IMPLEMENTATION_LAW]

[V4_NAME_ALIASING]:
- pocketken.H3 4.0.0 ships BOTH the v4-canonical names (`GridRing`, `GridDiskDistances`, `GridDistance`, `GridPathCells`, `CompactCells`, `UncompactCells`, `CellToLocalIj`, `LocalIjToCell`, `ToDirectedEdge`, `DirectedEdgeToCells`, `IsValidDirectedEdge`) AND the pre-v4 legacy names (`GetHexRing`, `GetKRing`, `DistanceTo`, `LineTo`, `Compact`, `UncompactToResolution`, `ToLocalIJ`, `FromLocalIJ`, `GetUnidirectionalEdge`, `GetIndexesFromUnidirectionalEdge`) as distinct method pairs over the same body. Pin the v4-canonical spelling so the managed call site matches the `h3-pg` SQL function name (`h3_grid_ring_unsafe`, `h3_grid_disk_distances`, `h3_grid_distance`, `h3_compact_cells`) one-to-one — the parity is the whole point of the dual admission. There is no managed method literally named `GridDisk`: the hollow ring is `GridRing`, the filled disk-with-distances is `GridDiskDistances`.
...
[STACKING]:
- ingest parity with `h3-pg`: `H3Index.FromPoint(point, res)` at ingest computes the identical cell id `h3-pg`'s `h3_lat_lng_to_cell((lng, lat), res)` computes server-side, so a row's `h3_cell` generated column (DDL on the `Schema/ddl` extension/generated-column rows) and the managed cell agree bit-for-bit — the dual admission exists to make in-process and in-database indexing one vocabulary.
- spatial lane prefilter: `Polyfill.Fill(region, res)` / `GridDiskDistances(center, k)` produce the candidate cell set a `Query/lanes` PostGIS lane intersects, turning a radius/region query into a hierarchical cell-set membership test the H3 GiST/BRIN index on the `h3_cell` column answers, with `CompactCells` collapsing the set to the densest mixed-resolution key before the predicate.
...
- routing seam: `GridPathCells`/`GridDistance` feed the `pgrouting` GEO_LANES routing capability (`Store/provisioning` server-extension row) with H3-cell graph nodes, so the in-process path and the in-database `pgr_dijkstra` share the cell-id node space.
...
- Owns: managed Uber-H3 v4 cell indexing, hierarchy/disk/path/region algebra, and the NTS-geometry ↔ cell bridge
