...
const DESIGN_BAR = [
  ...
  'STACK .api CAPABILITY by the file language: C# uses the per-package `<pkg>/.api/*.md` catalogs + the universal Thinktecture/LanguageExt rails ' +
    '(no central tier; Geometry catalogs live at `libs/csharp/Rasm/.api/`); Python mines BOTH tiers — the shared `libs/python/.api/*.md` AND the ' +
    'folder `libs/python/<folder>/.api/*.md`; TypeScript mines the shared `libs/typescript/.api/*.md` Effect/Schema/React tier AND the area ' +
    '`libs/typescript/<area>/.api/*.md`. Cite ONLY members confirmed in those catalogs; maximize the shared/universal rails, never only the folder ' +
    'set.',
...
const ADVERSARIAL = 'HOSTILE STANCE: assume the residual is REAL and the spanned code is naive, junior, or ILLUSORY until proven otherwise — the ' +
  ...
  'catalogued package surface. Resolve the residual PROPERLY and FULLY, never punt it.'
const RESIDUAL_LAW = [
  ...
  'RESEARCH MANDATE — no guessing: where a claim hinges on a real package/host member, VERIFY it via `uv run python -m tools.assay api` over the ' +
    ...
    'domain/math contract, research the real convention before deciding. Cite only verified members; a member you cannot verify is a phantom — do ' +
    'not assert it.',
...
const investigatePrompt = (r) => [DESIGN_BAR, '', ADVERSARIAL, '', RESIDUAL_LAW, '', 'TASK (INVESTIGATE — read-only, produce the resolution PLAN, ' +
  'edit NOTHING): residual `' + r.id + '`.\nCLAIM: ' + r.claim + filesLine(r) + (r.hint ? '\nWHERE TO LOOK: ' + r.hint : '') + '\nRead EVERY ' +
  'design page the claim spans + its siblings + the relevant `.api` catalogs (the right tier for the file language). Where the claim hinges on a ' +
  'real package/host member, VERIFY via `uv run python -m tools.assay api` (read tools/assay/README.md first). Where it hinges on a domain/math ' +
  'contract, research the real convention. Then DECIDE the canonical resolution (the strongest principled design the owning language bar admits; ' +
  'resolve deferred decisions, do not punt). Return the PLAN: `id`, the `decision` (what the canonical resolution IS), the exact repo-relative ' +
  '`files` to edit, the precise `edits` per file, the verified `.api` `api_members` to cite, and an `open_question` ONLY if it needs a true ' +
  'operator product decision.'].join('\n')
const implementPrompt = (cluster) => {
  ...
  return [DESIGN_BAR, '', ADVERSARIAL, '', RESIDUAL_LAW, '', 'TASK (IMPLEMENT — fix-in-place across these design pages at the owning language ' +
    'bar): apply EVERY resolution plan below to the spanned pages, preserving all capability and regressing no page. Unify each cross-file ' +
    'contract/seam CONSISTENTLY across all pages it touches; cite only verified `.api` members; signature-lock (never create source). Files in ' +
    'this cluster: ' + JSON.stringify(files) + '\nPLANS:\n' + JSON.stringify(cluster, null, 1) + '\nReturn the fix-log (files edited + verdict + ' +
    'applied), with `residual` only for a genuine still-cross-file item you could not close from these files.'].join('\n')
}
const verifyPrompt = (p) => [DESIGN_BAR, '', ADVERSARIAL, '', 'TASK (ADVERSARIAL VERIFY): residual `' + p.id + '`. Read the spanned design pages ' +
  'from disk and prove whether the resolution is ACTUALLY done: implemented (not hedged or merely described), correct (math/domain/`.api`-grounded ' +
  '— re-check any cited member against the catalog/`assay api`), and CONSISTENT across every page it spans. Status `resolved` ONLY if genuinely ' +
  'complete and correct; else `open` with exactly what remains. Default to `open` on doubt.\nCLAIM: ' + (p.claim || '') + '\nINTENDED RESOLUTION: ' + (p.decision || '') + '\nFILES: ' +
  '' + JSON.stringify(p.files || [])].join('\n')
