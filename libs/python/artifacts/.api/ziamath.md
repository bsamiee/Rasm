...
- `declareoperator` REBINDS `latex2mathml.commands.FUNCTIONS` — measured, `FUNCTIONS = FUNCTIONS + (name,)` over a process-global TUPLE, never an in-place append. A per-render operator vocabulary is therefore a snapshot of that binding and a `finally` rebind; the same discipline over a list would alias the mutated object and restore nothing, leaking one formula's vocabulary into every sibling parse and poisoning the content-addressed cache, so the tuple identity is the load-bearing fact. The baseline a consumer captures is whatever ziamath's own import-time `declareoperator` calls left, not the bare latex2mathml roster, so it reads per render rather than freezing once.
