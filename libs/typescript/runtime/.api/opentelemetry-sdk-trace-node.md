...
- Reject: calling `.register()` under the effect facade (effect owns global context wiring — a double registration); using this leg in the browser (`sdk-trace-web` owns that); re-documenting or re-implementing the base roster (it is re-exported, catalogued in `opentelemetry-sdk-trace-base.md`); importing outside `scope:runtime`; treating the node lane as permanent — it collapses at `[OTEL_PIN_BLOCK]`.
