...
Stryker.NET (root `stryker-config.json`, the one config for solution mode; root residency is deliberate self-defense — a bare `dotnet stryker` at repo root auto-detects `Workspace.slnx`, enumerates every project, launches a whole-solution build, and writes root `StrykerOutput/` before any guard (observed live in the [10].4 probe), so auto-discovery must find the capped config; assay still owns every gated mutation invocation and passes `--config-file` as an absolute anchor):
- `"test-runner": "mtp"`, `"solution": "Workspace.slnx"`, `"concurrency": 4` (default is half the logical processors — uncapped relative to the safe posture), `"output": ".artifacts/csharp/stryker"`, `"with-baseline"` + Disk provider, thresholds high 90 / low 85 / break 80, reporters json + html (schema JSON feeds the unified gate). Assay's mutation route points at this config; bare runs land in `.artifacts/` through the config's own output routing. Watch items: Stryker.NET #3516 (per-test coverage under MTP) and #3655 (bail).
...
- Stryker litter proof: after wiring the mutation rows' `--config-file stryker-config.json` / `--configFile stryker.config.json` (both root-resident; + Stryker.NET `--output .artifacts/csharp/stryker`, TRX `--report-trx --results-directory .artifacts/csharp/trx/<project>` per the C# leg residuals), compose the mutation argv for both languages and prove zero root litter; delete the stray root `.stryker-tmp/`. Also emit `mutation-testing-report-schema` JSON from mutmut via the small shim ([07]).
...
- StrykerJS: one config at root `stryker.config.json` — root residency is deliberate self-defense: StrykerJS auto-discovers only in cwd, and a bare invocation without the config falls back to a root `.stryker-tmp/` sandbox (observed live); auto-discovery makes every invocation litter-proof (vitest runner, temp under `.cache/stryker/`, reporters json+html into `.artifacts/typescript/stryker`, incremental ON in CI with the incremental file uncommitted per StrykerJS #6004). Assay's mutation row may still pass `--configFile` explicitly. The C# `stryker-config.json` moved to root under the same law — the [10].4 probe disproved the fails-before-writing assumption: a bare `dotnet stryker` auto-detects `Workspace.slnx`, starts a whole-solution build, and writes root `StrykerOutput/` first. Per-package configs arrive with TS buildout. Browser-mode suites never mutated (unsupported).
...
4. Safe-by-default tool posture (defense-in-depth; never assume assay is the invoker): every heavy tool is bounded by its own AUTO-DISCOVERED configuration so a bare invocation is small and self-limiting — concurrency capped well below core count, per-run and per-test timeouts set, mutate/target scopes explicit (never a default that sweeps the world), outputs and temp routed to `.artifacts`/`.cache`. Per tool, verified against current docs, never memory: StrykerJS root config gains its concurrency cap + dry-run and test timeouts; Stryker.NET is re-audited for bare-run reachability (if `dotnet stryker` at repo root can auto-detect `Workspace.slnx` and start a whole-solution mutation with default parallelism, the config moves to root auto-discovery with the same caps — the "fails before writing" assumption must be PROVEN, not presumed); `[tool.mutmut]` gains whatever concurrency/timeout bounds mutmut 3.x supports; pytest/xdist, hypothesis profiles, vitest, and BenchmarkDotNet configs re-checked for unbounded defaults; testcontainers cleanup (reaper) semantics confirmed so abandoned runs cannot strand containers. The Python litter-policing gate generalizes from a known-name roster to unexpected-root-entry detection over an explicit allowlist of legitimate root files. The `tests/` law docs gain the one-paragraph heavy-lane invocation law (what a human or agent may launch directly and what must ride assay).

Live acceptance audit (after the terminal sweep — the campaign's closing gate): a truthful full-surface exercise of the assay operator under real usage, no mocks, no shortcuts. (a) Every verb of every rail runs live: `static` (per-language scopes AND `--all` with the full C# solution build), `test run|list|coverage` per language, `code` search/query, `docs`, `package`, `provision` (read-only probes), `self-test`, `delta` over the history store. (b) `api` proven against ALL FOUR artifact sources — host DLL, NuGet cache, Python dist, node_modules `.d.ts` — resolve + surface + member queries, no errors, cache entries typed and replayed. (c) Adversarial truth checks: seed one deliberate defect per language (a scratch file with a real violation), prove `static` FAILS with correctly folded diagnostics and honest exit codes, then remove the seeds — a gate that cannot be seen to fail is not accepted. (d) Bridge live: `bridge build` emits real closures; empty-corpus short-circuit proven; the Gate harness runs its stand-in rows and, when the host is free, its live lane against a real Rhino launch — the supervisor lifecycle (launch, handshake, quit ladder) exercised for real. (e) Hygiene invariants: `.cache/`/`.artifacts/` size and entry deltas recorded before/after (bounded growth, no unbounded cache litter), zero new root entries, retention pruning observed working. (f) Bare-invocation bounds: each heavy tool's discovered config is asserted to carry its concurrency cap, timeouts, explicit scope, and routed outputs (config-level checks plus a bounded smoke where cheap — never a full mutation run). Every failure found is fixed before the campaign closes; the audit report is the campaign's final artifact.

## [11]-[INFRA_ASCENSION]

Runs after the e2e platform lands; supersedes [10].3 for the test estate. Order: residual clearance, then the ascension workflow, then LT/LA close the campaign.

Residual clearance (direct agents, pre-workflow): `libs/typescript/kernel/.api/hash-wasm.md` [04] false LE-to-BE normalize claim corrected against the live probe; the `@effect/vitest` catalog tier resolved to one tier (dev tooling belongs to `tests/typescript/.api/` per the tests README — move or amend the law, never duplicate); a shared PG/S3 container image pin decided so the harness container lanes activate; `tools/assay/oracle.py` and `rails/provision.py` densify headroom; `SnapshotHygieneLaws` lane placement; the automation Executor port engine-signature change; `bridge.py` `_selection` docstring truth; the pnpm node_modules-sync warning; the xunit1045 info notes stay deliberate.

The ascension workflow: an ephemeral campaign workflow — authored through `.claude/skills/workflow-creator` after a FULL read of `libs/.planning/campaign-method.md` and `.claude/workflows/rebuild.js`, deleted after landing — running three sequential passes per scope (initial rebuild-grade, critique, redteam; the campaign-method role law and both naivety axes) over the entire test infrastructure. TypeScript is the primary pressure; the Python and C# estates are the FLOOR, never the ceiling — all three rise equally. Scope: root test tooling (`vitest.config.ts`, `vite.config.ts` plus factory, both stryker configs, biome test-plane rules, nx test targets, the playwright config), `tests/typescript/**` including the full e2e platform (config, fixtures, drivers, self-proof suites — refined and pushed with the same aggression as the kit), `tests/python/**` shared infra, `tests/csharp/**` shared infra (`_testkit`, `_scenariokit`, `_architecture`, `_benchmarks`), `tests/contracts` law surfaces, and the per-language README law docs. Every TS-writing pass grounds in the full `docs/stacks/typescript` doctrine and reads `libs/typescript/.planning/` in full — every durable page there is demand signal for kit and e2e capability; task-log and ideas pages are skipped, and pages absent at run time (the blueprint retires with its own workflow) are simply absent. Mandates: nothing naive, table-stakes, surface-level, or under-scoped survives; deep foresight without spam; external APIs ultra-stacked from official sources, never memory; new packages and tooling admitted where capability demands (central manifests, `.api` stubs, README manifests per the admission law); new files and folders where ownership demands, particularly TypeScript; ground-up rebuild over patching; zero test coupling and zero exotic patterns that buy runtime limitations. Language passes may upgrade the tests themselves where new infra absorbs them — a minor concern, Python first. Tail: one full-estate smoothing pass over `tests/` shared infra (consistency, density, docs trues), then every language gate green with zero errors.
