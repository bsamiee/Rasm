...
def vjp(fn: Callable[[np.ndarray], np.ndarray], x: np.ndarray, cotangent: np.ndarray, mode: DiffMode) -> "RuntimeRail[VjpReceipt]":
    return boundary(f"vjp.{mode.tag}", lambda: _vjp(fn, x, cotangent, mode))


def _vjp(fn: Callable[[np.ndarray], np.ndarray], x: np.ndarray, cotangent: np.ndarray, mode: DiffMode) -> VjpReceipt:
    match mode:
