...
- Packages: `pint` (`UnitRegistry`, `Quantity`, `Measurement`, `to`, `to_base_units`, `dimensionality`, `DimensionalityError`), `uncertainties` (`ufloat`, `correlated_values`, `covariance_matrix`, `correlation_matrix`, `wrap`, `umath`, `UFloat.nominal_value`, `UFloat.std_dev`, `UFloat.error_components`), `numpy` (`asarray`), runtime (`RuntimeRail`, `boundary`, `BoundaryFault`, `Receipt`/`ReceiptContributor`).
...
class UncertainQuantity(Struct, frozen=True):
    ...
    @classmethod
    def correlated(
        cls, nominals: Sequence[float], covariance: Sequence[Sequence[float]], unit: str, tags: tuple[str, ...], /
...
def covariance_provenance(quantities: Sequence[UncertainQuantity]) -> RuntimeRail[np.ndarray]:
    return boundary("quantity.covariance", lambda: np.asarray(covariance_matrix([q.magnitude.cell for q in quantities]), dtype=np.float64))
