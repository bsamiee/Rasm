...
- `Metadata.RegularUnion.TypeMembers`: a `Type` list and the whole of what regular-union generation publishes about its cases — the generator emits `Switch`/`Map` dispatch and NO per-case name, ordinal, or discriminator member. A stable case token therefore cannot be composed off this package: reading `.Name` off a member of the list is a runtime type probe wearing metadata, answering whatever the trimmer or AOT compiler left of the nested record's name and turning a rename into a silent token change. A consumer needing one mints it in its own generator as the leaf's `nameof` (`Rasm` `Domain/rails#FAULT_BAND` `FaultId.Case`), and reads this list only for a build-time or startup PROOF, where reflection is legitimate.
