...
_NAN_BASE: "dict[str, str]" = {name: name.removeprefix("nan") for name in Reduction.__value__.__args__}
...
_SKIPNA_BASE: "frozenset[str]" = frozenset({"sum", "prod", "mean", "std", "var", "max", "min", "median", "quantile"})
_FALLBACK_CALL: "dict[GrouperKind, Callable[[xr.Dataset, tuple[object, ...]], object]]" = {
...
class ReductionPolicy(Struct, frozen=True):
    ...
    @property
    def base(self) -> str:
        return _NAN_BASE[self.func] if isinstance(self.func, str) else "reduce"
...
class FieldSelection:
    ...
    @staticmethod
    def Scan(*by: str, func: "ScanFunc | Scan" = "cumsum", policy: ReductionPolicy = ReductionPolicy()) -> "FieldSelection":
...
def _scan(
    ...
    grouped = _FALLBACK_CALL[fallback[0]](dataset, fallback[1])
    return getattr(grouped, policy.scan if isinstance(policy.scan, str) else "cumsum")()
```
...
- Packages: `xarray` (`Dataset.to_netcdf(encoding=)`/`to_zarr`/`to_dataframe`/`sizes`/`data_vars`/`nbytes`, banned-module-level, function-local), `pyarrow` (`Table.from_pandas`/`Table.to_batches`/`RecordBatch.serialize` the Arrow lowering target, the byte span summed over every serialized `Buffer.size` and the content key folded over the whole batch stream as an `Iterable[bytes]`, not an unverified `Table.nbytes` and not a first-batch-only digest), runtime (`ContentIdentity`/`ContentKey`/`RuntimeRail`/`boundary`/`Receipt`/`ReceiptContributor`).
...
class FieldReceipt(Struct, frozen=True):
    ...
    def contribute(self) -> Iterable[Receipt]:
        yield Receipt.of(
            "field", ("emitted", self.engine, {"dims": ",".join(self.dims), "variables": self.variables, "stored": self.bytes_stored})
        )
...
def _write(field: "FieldDataset", dataset: "xr.Dataset", target: ResourceRef, encoding: "FieldEncoding") -> "RuntimeRail[FieldReceipt]":
    ...
    return ContentIdentity.of("field", source).map(
        lambda key: FieldReceipt(
            engine=field.engine.value, dims=tuple(dataset.sizes), variables=len(dataset.data_vars), bytes_stored=int(dataset.nbytes), content_key=key
        )
    )


def _to_arrow(dataset: "xr.Dataset") -> "tuple[pa.Table, tuple[str, ...], int, list[pa.Buffer]]":
    import pyarrow as pa  # noqa: PLC0415

    table = pa.Table.from_pandas(dataset.to_dataframe().reset_index())
    batches = [batch.serialize() for batch in table.to_batches()]
    return table, tuple(dataset.sizes), len(dataset.data_vars), batches


def _arrow_receipt(
    table: "pa.Table", dims: tuple[str, ...], variables: int, batches: "list[pa.Buffer]"
) -> "RuntimeRail[tuple[pa.Table, FieldReceipt]]":
    return ContentIdentity.of("field.arrow", [bytes(buffer) for buffer in batches]).map(
        lambda key: (
            table,
            FieldReceipt(
                engine="arrow", dims=dims, variables=variables, bytes_stored=sum(buffer.size for buffer in batches), content_key=key
            ),
        )
    )
```
...
- [ARROW_LOWERING]: the `Dataset.to_dataframe().reset_index()` flatten and the `pyarrow.Table.from_pandas` lowering the `_to_arrow` egress uses are catalogue members (`to_dataframe`/`to_pandas` `xarray` `.api` L41, `pyarrow.Table.from_pandas` L85, `to_batches` L110); the Arrow byte span sums every `RecordBatch.serialize().size` over the whole `Table.to_batches()` stream and the content key folds the whole batch stream through `ContentIdentity.of` as an `Iterable[bytes]` (the `Source` union admits it) rather than the unverified `pyarrow.Table.nbytes` property absent from the folder `pyarrow` `.api` and rather than a first-batch-only digest that would collide two cubes differing past the first batch, the `Dataset.nbytes` byte-count accessor the write-path `FieldReceipt` reads confirms against the live `xarray` distribution, and the directory content-key hashes the Zarr v3 `zarr.json` root metadata (`zarr_format=3` default per the folder `zarr` `.api` `[04]-[ARRAY_STORE_TOPOLOGY]`), not the v2 `.zmetadata`.
