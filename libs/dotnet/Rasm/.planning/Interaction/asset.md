...
- Entry: `AssetOrigin.Resolve(extent, stack)` answers the raster the origin produced at the asked extent in the asked product shape — a decode, a set selection, a host vector or source-text rasterization, or a replay of the draw program. The `Pixels` ask reads its rows through `Interaction/paint`'s `PixelLease.Bytes`, so one lock-and-copy owner serves this resolve and every other pixel egress.
