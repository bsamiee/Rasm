...
|  [02]   | `SplitObjectsByGrid(...)` / `SplitObjectsAtDistance(...)` / `SplitObjectsByStep(...)` / `SplitObjectsByPartsNumber(...)` | `Splitter` | split objects |
|  [03]   | `MergeObjects(...)` / `MergeSequentially(...)` / `MergeSimultaneously(...)` | `Merger` | merge objects/files |
|  [04]   | `Resize(...)` / `ResizeObjectsGroup(...)`          | `Resizer`             | scale durations    |
|  [05]   | `Repeat(...)`                                      | `RepeaterUtilities`   | repeat range       |
...
- Boundary intake reads `MidiEventReceivedEventArgs.Event` and maps to the canonical InputFabric event shapes at the edge — the `Midi` case is one arm on the single `InputFabric` edge that also lifts the `Gamepad`/`Haptic` SDL2 (`api-silk-input`, `api-silk-sdl`) and `Hid` HidSharp (`api-hidsharp`) SpaceMouse streams, folding every device onto the one `CommandIntent` table; a MIDI control surface raises existing parameter intents through that one fold, never a parallel MIDI device→intent edge. DryWetMidi carries its own native multimedia clock and no SDL2 dependency, so the `Midi` capsule shares no native bundle with the SDL2 peers; raw `MidiEvent` types do not propagate into domain code.
