...
from rasm.runtime.faults import FAULT_CONF, BoundaryFault, Disposition, RuntimeResult, traversed
...
_MEDIA_CLASS: Final[Map[str, MediaClass]] = Map.of_seq([
    ...
    ("image/vnd.radiance", MediaClass.TEXTURE),
...
class Detect:
    ...
    @overload
    async def of(self, source: Source, /) -> RuntimeResult[DetectIdentity]: ...
    @overload
    async def of(
        self, source: Iterable[Source], /
    ) -> RuntimeResult[Block[DetectIdentity]] | RuntimeResult[tuple[Block[DetectIdentity], Block[BoundaryFault]]]: ...
    async def of(
        self, source: Source | Iterable[Source], /
    ) -> RuntimeResult[DetectIdentity] | RuntimeResult[Block[DetectIdentity]] | RuntimeResult[tuple[Block[DetectIdentity], Block[BoundaryFault]]]:
        match source:
            case Source() as one:
                return await self._detected(one)
            case sources:
                return await self._many(Block.of_seq(sources))

    async def _detected(self, source: Source, /) -> RuntimeResult[DetectIdentity]:
        match self.engine:
    ...
    async def _many(
        ...
        async with anyio.create_task_group() as group:
            handles: Block[TaskHandle[RuntimeResult[DetectIdentity]]] = sources.map(lambda one: group.start_soon(self._detected, one))
