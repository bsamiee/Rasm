...
- `[2]-[EGRESS]`: the native object-store egress owner over one `StoreOp` axis composing the runtime transport.
...
class EgressReceipt(Struct, frozen=True):
    ...
    def contribute(self) -> Receipt:
        return Receipt.of(
            "emitted", "object-egress", self.path,
            {"op": self.operation, "bytes": str(self.byte_length), "etag": self.e_tag},
        )
...
class ObjectEgress(Struct, frozen=True):
    ...
    def _receipt(self, operation: str, path: str, byte_length: int, e_tag: str) -> EgressReceipt:
        return EgressReceipt(
            operation=operation, path=path, byte_length=byte_length, e_tag=e_tag,
