...
[PROVISIONING]:
- Enables: tier-2 server and native runtime closure proofs through sanitized Assay evidence projected from the Forge-owned `rasm-provision` and `forge-scientific-env` executables.
...
- Surface: `provision up|down|status|doctor|ports|inventory|extensions|plan|env|verify` is the Rasm campaign command surface. Forge-only debugging commands such as `paths`, `self-test`, `prune`, and `psql <service>` are intentionally absent from Assay.
