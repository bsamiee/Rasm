...
Zarr v3 shapes a codec chain as `[array->array transforms..., one array->bytes serializer, bytes->bytes compressors...]`; `BytesCodec` serializes canonically and `ShardingCodec` nests an inner chain per sub-chunk. `zarr.codecs` re-exports the numcodecs-backed roster (`Delta`, `Blosc`, `FixedScaleOffset`, `Quantize`, `BitRound`, …) from `zarr.codecs.numcodecs`, each subclassing the same role base under a `**codec_config` constructor, so the native `zarr.codecs` rows below and the re-exported ones share one import path and differ by construction shape alone.
- Every constructor below is KEYWORD-ONLY: `BytesCodec(*, endian='little')`; `ShardingCodec(*, chunk_shape, codecs=(BytesCodec(),), index_codecs=(BytesCodec(), Crc32cCodec()), index_location='end')` with `index_location` a `ShardingCodecIndexLocation`; `TransposeCodec(*, order)`; `ScaleOffset(*, offset=0, scale=1)`; `CastValue(*, data_type, rounding='nearest-even', out_of_range=None, scalar_map=None)`; `BloscCodec(*, typesize=None, cname='zstd', clevel=5, shuffle=None, blocksize=0)`; `ZstdCodec(*, level=0, checksum=False)`; `GzipCodec(*, level=5)`; `Crc32cCodec()`/`VLenUTF8Codec()`/`VLenBytesCodec()` take nothing
