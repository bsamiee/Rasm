...
public static partial class LifecycleAssessment {
    ...
    // fallback ladder, decodes the per-module ScopeSet onto the seam StageGwp vector, and accumulates the enriching delta
    // the composition root applies before the sync RunCarbon, so a fully-declared model needs no network call.
    ...
    static Seq<AssessmentFact> StageFacts(NodeId id, ReadOnlyMemory<double> stageGwp) =>
        LifecycleStage.Items.ToSeq().Map(stage => DomainMeasure($"{id.Value}/gwp-{stage.Module}", stageGwp.Span[stage.Index], "kgCO2e"));
...
- [EN_15978_TAKEOFF]: the whole-building embodied-carbon takeoff folds `AssemblyAggregator.AggregateEnvironmental` over each element's seam `MaterialComposition` (the relocated engine owns the per-module `Σ(StageGwp_i · quantity_i)` fold), so a baked declaration and an EC3-resolved declaration fold identically onto the `LifecycleStage` band; the per-ply geometric quantity comes from the Compute-owned `TakeoffOf` reading the element's baked `Qto_*BaseQuantities` (net volume + net side area, dimension-true), the cradle-to-gate `A1-A3` and the whole-life total projections over the per-module vector, the `EmbodiedCarbonIntensity` the GWP per element area. The seam `StageGwp` carries its `MeasurementBasis` — the EC3 ingress `Normalize`s every resolved declaration to per-ONE-unit of its native `declared_unit` basis and TAGS it (PerM3/PerM2/PerKg) before it lands on the `Environmental` case, so the aggregator scales each ply by the basis-matching element quantity through the SAME `DeclaredQuantity` owner the cost fold uses, and a baked `PerM3` declaration and an EC3-resolved native-basis declaration both fold correctly (the `Rasm.Materials` projector authors its baked `Environmental` `PerM3`). Ripple counterpart: `Rasm.Materials` `Properties/sustainability` (RETIRE the aggregator half; KEEP single-material `Environmental` authoring lowered onto the seam per-m³), `Rasm.Element/Composition/material` (the seam `Environmental` case + `LifecycleStage` band + `OfEnvironmental` admission), and `Rasm.Compute` `Analysis/aggregator` (`AggregateEnvironmental` + `ElementQuantity`/`PlyQuantity`).
