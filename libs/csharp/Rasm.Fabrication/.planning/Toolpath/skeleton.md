...
- Owner: `SkeletonEvent` `[Union]` the wavefront event (`Edge` an edge collapse keyed by its leading vertex index, `Split` a reflex-vertex split carrying the opposing edge and the exact hit point) the propagation resolves in time order; `Wavefront` the shrinking offset polygon state the propagation steps, each vertex moving along its angular bisector scaled to the inward edge-offset speed, with `Reflex` the exact `Orient2D` convexity verdict per vertex; `ClearanceArc` the medial-axis arc tagged with its inscribed-circle clearance radius (the wavefront `Time` at the traced arc endpoint, the local channel half-width); `StraightSkeleton` the static surface owning `MedialAxis` (the skeleton arc set), `OffsetAt` (the exact constant-offset polygon at a given inset, the offset that subsumes `OffsetRing`), `Clearance` (the per-arc clearance field over the same propagation), and `ClearanceAt` (the per-point clearance-radius lookup the engagement controller reads).
...
- Entry: `public static Fin<Seq<Edge3>> MedialAxis(Loop polygon)`, `public static Fin<Seq<Loop>> OffsetAt(Loop polygon, double inset)`, `public static Fin<Seq<ClearanceArc>> Clearance(Loop polygon)`, and `public static Fin<double> ClearanceAt(Loop polygon, Point3d point)` — `Fin<T>` routes `FabricationFault.OpenLoop` on a non-closed polygon and the kernel `GeometryFault.DegenerateInput` on a zero-area or self-intersecting input, each lowered with `.ToError()`; `MedialAxis` returns the skeleton arc set, `OffsetAt` the exact offset polygon at the inset, `Clearance` the per-arc inscribed-radius field, `ClearanceAt` the clearance radius at the nearest arc to a query point.
- Auto: `MedialAxis` runs the wavefront propagation — `Wavefront.Of` emits each vertex's angular bisector scaled by `1/sin(θ/2)` so a unit-time advance offsets every supporting edge inward by exactly one unit, and `Propagate` processes the earliest `SkeletonEvent` under a `Count²+8` step budget: `NextEvent` scans every adjacent-bisector meeting time (`RayMeet`, the `Edge` collapse) and every reflex-vertex-versus-non-incident-edge split time (`SplitMeet`, the `Split`), takes the earliest strictly-future event, `Advance` moves all vertices to that event time tracing each non-degenerate segment as a skeleton arc, and `Apply` drops the collapsed vertex (`Edge`) or inserts the split hit point (`Split`) and `Rebuild` recomputes the bisectors of the changed wavefront. `Reflex` classifies each vertex by `Predicate.Orient2D` exact sign and `SplitMeet` gates the opposing-edge side and the along-edge containment exactly, so a near-collinear bisector or a grazing split classifies deterministically and never seeds a phantom event. `OffsetAt` reads the wavefront state at the inset time through the `Geometry2D/algebra#POLYGON_ALGEBRA` `Offset` — for a convex inset this matches the Clipper2 offset, and the skeleton resolves the reflex-vertex split a per-vertex-normal offset cannot. `Clearance` runs the same propagation through `PropagateClearance`, tagging every traced arc with the event `Time` at which its endpoint was reached — and because the unit-speed bisector march offsets every supporting edge inward by exactly one unit per unit time, that time IS the inscribed-circle radius at the arc point, the local channel half-width; `ClearanceAt` projects a query point onto every arc through the segment-distance `Project`, takes the nearest arc, and returns its clearance radius, so the trochoidal engagement controller and the `GUARD` reach check read the exact half-width per point off the propagation the kernel already runs, never a second distance transform.
- Receipt: `MedialAxis` returns the `Edge3` arc set directly, `OffsetAt` the `Loop` set, and `Clearance` the `ClearanceArc` field (arc plus inscribed radius) — the geometry IS the evidence the trochoidal generator reads, the clearance scalar the wavefront time itself; no generic skeleton ledger and no re-derived distance field.
- Packages: `Rhino.Geometry` (`Point3d`/`Vector3d` — composed), `Rasm.Numerics` (`Predicate.Orient2D` — settled, the bisector/side verdict), Clipper2 (via `Geometry2D/algebra#POLYGON_ALGEBRA` — the convex-inset offset cross-check), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox.
- Growth: a weighted straight skeleton (per-edge speed for variable-kerf offset) is one speed column on `Wavefront`; a polygon-with-holes medial axis is one hole-edge arm on the propagation; the clearance field is the `Time`-tagged arc set the same propagation already carries, one `ClearanceArc` projection, never a second pass; zero new surface.
- Boundary: `StraightSkeleton` is the ONE medial-axis owner and the trochoidal generator reads it, never a second skeleton routine; the clearance radius is the wavefront `Time` at the traced arc point and a re-derived distance transform or a second inscribed-circle pass is the deleted form — `Clearance` reads the propagation the kernel already runs and `ClearanceAt` the nearest-arc lookup, never a parallel signed-distance field; the `SkeletonEvent` time read dispatches through the generated total `Switch(edge:, split:)` and a `ev switch` pattern cascade with a `_` catch-all is the deleted form — a new event case fails the build until its arm lands; the convex-inset offset cross-checks the `Geometry2D/algebra#POLYGON_ALGEBRA` `Offset` and never re-mints the Clipper2 polygon offset for the convex case — the skeleton owns only the reflex-split and medial-axis construction Clipper2 does not expose; every bisector turn and opposing-edge side reads `Predicate.Orient2D` exact sign and a `double` cross at the call site is the named robustness defect; a hand-rolled per-vertex-normal `OffsetRing` is the deleted form this kernel subsumes.
...
// --- [MODELS] -----------------------------------------------------------------------------
public readonly record struct ClearanceArc(Edge3 Arc, double Clearance);
...
public static class StraightSkeleton {
    ...
    public static Fin<Seq<ClearanceArc>> Clearance(Loop polygon) =>
        !polygon.Closed
            ? Fin.Fail<Seq<ClearanceArc>>(FabricationFault.OpenLoop("skeleton:clearance-open").ToError())
            : polygon.Count < 3
                ? Fin.Fail<Seq<ClearanceArc>>(GeometryFault.DegenerateInput("skeleton:clearance-degenerate").ToError())
                : Fin.Succ(PropagateClearance(Wavefront.Of(polygon), Seq<ClearanceArc>(), polygon.Count * polygon.Count + 8));

    public static Fin<double> ClearanceAt(Loop polygon, Point3d point) =>
        Clearance(polygon).Map(field => field
            .OrderBy(c => Project(point, c.Arc))
            .HeadOrNone()
            .Match(Some: c => c.Clearance, None: () => 0.0));
    ...
    static Seq<ClearanceArc> PropagateClearance(Wavefront front, Seq<ClearanceArc> field, int budget) =>
        budget <= 0 || front.Count < 3
            ? field
            : NextEvent(front).Match(
                None: () => field,
                Some: ev => {
                    double time = EventTime(ev);
                    var (advanced, traced) = Advance(front, time);
                    Wavefront resolved = Apply(advanced, ev);
                    return PropagateClearance(resolved, field.Concat(traced.Map(a => new ClearanceArc(a, time))), budget - 1);
                });

    static double Project(Point3d p, Edge3 arc) {
        Vector3d ab = arc.B - arc.A; double len2 = ab * ab;
        double t = len2 < 1e-18 ? 0.0 : Math.Clamp((p - arc.A) * ab / len2, 0.0, 1.0);
        return p.DistanceTo(arc.A + t * ab);
    }
...
- [SKELETON_EVENTS] The wavefront-event resolution is realized as the `NextEvent`/`Advance`/`Apply` author-kernel grounded on the Aichholzer-Aurenhammer construction over the exact `Predicate.Orient2D` floor: `NextEvent` is the earliest-event scheduler over the `RayMeet` edge-collapse and `SplitMeet` reflex-split candidates, `Advance` is the unit-speed bisector march tracing the arcs, and `Apply` is the vertex-drop / hit-insert wavefront edit with `Rebuild` bisector recomputation. The two settled numeric assumptions are the bisector speed scale `1/sin(θ/2)` (so unit time equals unit inset) and the `Count²+8` propagation budget bounding the event count on a simple polygon; the convex case cross-checks the `Geometry2D/algebra#POLYGON_ALGEBRA` `Offset` as the correctness witness, and the reflex-split case is the genuinely-new author-kernel content no managed library carries.
- [CLEARANCE_FIELD] The medial-axis clearance field is realized as the `Clearance`/`PropagateClearance` projection over the existing wavefront: the `1/sin(θ/2)` bisector speed scale makes unit propagation time equal unit inward inset, so the wavefront `Time` at any traced arc point is exactly the inscribed-circle radius — the distance to the nearest boundary edge, the local channel half-width. `PropagateClearance` is the same `NextEvent`/`Advance`/`Apply` schedule as `Propagate` carrying the event `Time` onto each traced `ClearanceArc`, and `ClearanceAt` is the nearest-arc segment-distance lookup. The settled assumption is that the wavefront time read at the arc endpoint is the inscribed radius at that point (exact under the unit-speed scaling); the `TROCHOIDAL_ENGAGEMENT_LIMIT` constant-engagement step and the `GUARD` tool-radius reach check read this field rather than re-deriving a distance transform the propagation already encodes.
