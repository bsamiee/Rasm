...
- Boundary: the self-telemetry spans and metrics cross no wire contract — they ship from the host root to the collector, which is the only telemetry path; the observability stack that backs the collector is provisioned by `@rasm/services` `provisioning.md`; `MetricRegistry` is the single instrument owner and an inline metric construction outside it is the named defect; the browser worker pool is a main-thread-offload concern and never a durable tier — the durable cluster is the `@rasm/services` concern; `LocalPersistence` is the single browser-local store and a direct `localStorage`/`IndexedDB`/`idb-keyval` access outside it is the named defect; the offline-queue drain reaches the `@rasm/interchange` `CommandGateway` across the package boundary and never re-dials a transport here; the node OTel SDK never enters this browser edge.

```ts contract
interface MetricRegistry {
  readonly counters: Record<string, Metric.Metric.Counter<number>>;
  readonly histograms: Record<string, Metric.Metric.Histogram<number>>;
  readonly gauges: Record<string, Metric.Metric.Gauge<number>>;
