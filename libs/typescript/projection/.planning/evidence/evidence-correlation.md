...
- Growth: a new receipt payload lands as one payload row bound through `envelope/receipt-envelope#RECEIPT_ENVELOPE`, zero new carrier; a new evidence kind lands as one fold arm; the `byContentKey` lookup and the `RedBlackTree` index never grow.
...
The `foldStream` accumulator is the persistent `RedBlackTree` itself, so the make-fork-update scaffold from `keyed-fold#KEYED_FOLD` carries unchanged; `RedBlackTree.insert` returns a structurally-shared tree so the held `SubscriptionRef` swaps to the new root in O(log n) and a reconnect-replay re-inserts to the identical node, never re-sorting the prefix. The ordered read is `RedBlackTree.values` for the whole timeline and `greaterThanEqual` for the bounded since-T tail.
