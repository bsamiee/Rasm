...
- `LanguageExt.Core`(`.api/api-languageext.md`): every throwing `Create` and `Solve` enters `Try.lift`, preserving the foreign exceptional `Error` until a documented provider refusal maps to an owning fault.
...
- `Rasm.Compute`: `Tensor/factor` `SparseOps.ToCsc`/`SparseOps.Diagonal` are the canonical `CoordinateStorage<double>` -> `CompressedColumnStorage<double>` finalization owners every assembled operator reaches a factorization through, never page-local `Converter` calls; the owned frame `ShapeFamily.Frame`/`Beam2Euler`/`Beam2Timoshenko` kernels assemble reduced global stiffness through `CoordinateStorage<double>.At` at `Solver/assembly#OPERATOR_ASSEMBLY` and factor behind the `Solver/contract` `SolveLane`, continuum and frame problems sharing one `ISparseFactorization<double>` interface; `.mtx` exchange rides `MatrixMarketReader`/`MatrixMarketWriter` with large reads routed through `Microsoft.IO.RecyclableMemoryStream`.
