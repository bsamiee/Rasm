...
public static class ConnectionProjection {
    ...
    static Map<PropertyName, PropertyValue> AccessoryDetail(IfcDiscreteAccessory accessory) =>
        FastenerOf(accessory).Match(
            Some: fastenerType => Rows(
                Joint("Bolted"),
                Token("AccessoryType", accessory.PredefinedType.ToString()),
                Token("FastenerType", fastenerType)),
            None: () => Rows(
                Joint("Bolted"),
                Token("AccessoryType", accessory.PredefinedType.ToString())));
