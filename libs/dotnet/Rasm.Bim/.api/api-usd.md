...
[STACKING]:
- `SharpGLTF`(`libs/dotnet/.api/api-sharpgltf.md`): USD and glTF are sibling scene codecs on `format#FORMAT_AXIS` sharing one kernel mesh vocabulary; a glTF export of a USD scene crosses that shared vocabulary, never a USD↔glTF direct converter minted in Bim.
...
- `format#FORMAT_AXIS`: `usd`/`usdz` `InterchangeFormat` rows and the `InterchangeCodec.UsdStage` slot drive `import#IMPORT_PIPELINE` `BimIo` and `export#EXPORT_PIPELINE` `BimExport` by row data, never an `if (usd)` call-site branch; the `FrameNormalization` row coerces the imported Y-up op stack (`UsdGeomXformable` / `UsdGeomXformCache.GetLocalToWorldTransform` `GfMatrix4d`) onto the canonical Z-up kernel frame.
- mesh bridge: a kernel mesh crosses `UsdGeomMesh` through the typed-array boundary — `GetPointsAttr().Get(VtValue)` yields a `VtVec3fArray` of `GfVec3f` and `GetFaceVertexIndicesAttr()`/`GetFaceVertexCountsAttr()` the topology; export builds `VtVec3fArray`/`VtIntArray` and writes them through the attribute `Set` (never a `Set*Attr`), with `UsdGeomPrimvarsAPI.CreatePrimvar` carrying uv/color.
...
- shading boundary: `UsdShadeMaterial`/`UsdShadeShader` (UsdPreviewSurface), read and authored through `UsdShadeMaterialBindingAPI`, map to the `Semantics/appearance#APPEARANCE_PROJECTION` `AppearanceSummary` host-neutral PBR record, reconciled with the `dotnet:Rasm.Materials` OpenPBR owner at the content-key boundary.
