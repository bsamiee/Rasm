...
[LOCAL_ADMISSION]:
- `CompressionPolicy.Zstd*` (`Version/snapshots#COMPRESSION_HASHING`) configures a `Compressor` through `SetParameter` (level + `contentSizeFlag` so the frame self-describes its decoded size + `checksumFlag` for a frame-embedded integrity tag), so the snapshot header stores only the `CompressionId`, never a sidecar length.
