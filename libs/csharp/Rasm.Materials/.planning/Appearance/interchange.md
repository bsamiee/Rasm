...
- [02]-[MATERIAL_WIRE]: `MaterialWire.Summary` lowers a row to the seam `AppearanceSummary` (the CONTRACTED `Projection/component#COMPONENT_SUBGRAPH` entry, content-keyed by the seam `ContentAddress`), `MaterialWire` carries the canonical OpenPBR-vector payload, `OpenPbrGroupsWire` the parameter-group projection GENERATED by the `WireMap` `[Mapper]` under the RMG completeness gate, `WireProvenance` the receipt, `WireCodec` the Thinktecture + source-generated System.Text.Json / MessagePack serialization (the binary leg LZ4-compressed and untrusted-hardened), and the C#-mints / peers-decode single-mint law with the TS and Python decode contracts.
...
public static partial class WireMap {
    ...
    // SpecularColor/CoatColor stay White (the OpenPBR neutral-tint baseline — OpenPbrSurface carries no specular/coat colour column, so a wire
    // tint stays speculative); FuzzColor auto-maps by NAME from OpenPbrSurface.FuzzColor, the sheen-tint white→base lerp surface.md mints —
    // re-aiming it from BaseColor corrupts the fuzz tint on the wire.
...
```csharp signature
// --- [TYPES] -------------------------------------------------------------------------------
// MtlxPort closes the MaterialX 1.39 typed-port axis — float/integer/color3/vector3/filename/surfaceshader; every MtlxInput carries the REAL
...
public static class Mtlx {
    ...
    // Disney tint lowers into specular_color (white specular at tint 0, the base hue at tint 1), and the mm-band radius splits below — a
    // zero-radius row scales (1,1,1) so the float radius alone zeroes the effect.
