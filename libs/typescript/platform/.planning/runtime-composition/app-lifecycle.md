...
[APP_LIFECYCLE]: the closed Phase enum and the merged page-lifecycle fold.
...
- Boundary: `AppLifecycle` is the single page-lifecycle axis — `web-vitals/performance-budget.md`'s `visibilityState`-hidden terminal CLS/INP flush, `feature-flags/remote-config.md`'s foreground-refresh trigger, and `fault-capture/crash-telemetry.md`'s drain each subscribe to the `transitions` stream or read the `phase` cell, so a private `visibilitychange`/`pagehide` listener reopened at any of those owners is the named three-ingress defect; the cell is advanced only by the fold and the boot/stop edges and never mutated by a consumer; `AppLifecycle` emits no command, dials no transport, and `ui` reads a lifecycle-derived affordance through the `AtomBinding`, never importing `platform`.
