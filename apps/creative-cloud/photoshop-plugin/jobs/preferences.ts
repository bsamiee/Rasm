...
import { Applied, GetPreferences, Preferences, SetPreferences } from '@rasm/creative-cloud-server/photoshop/jobs';
import { Array, Effect, Equal, identity, Option, Predicate, Record, Result, Schema } from 'effect';
...
const _write = ({ section, key, value }: Write): Effect.Effect<Result.Result<Row, Refusal>, HostRejection> => {
    ...
    if (!Reflect.has(target, key)) {
        return Effect.succeed(Result.fail({ section, key, reason: { _tag: 'unknownKey' as const } }));
    }
    const from = _value(target, key);
    const locked = section === 'notifications' && app.preferences.notifications.quietMode;
    return Effect.try({ try: () => Reflect.set(target, key, value), catch: identity }).pipe(
        Effect.matchEffect({
            onFailure: (cause) =>
                locked ? Effect.fail(HostRejection.cases.preferenceLocked.make({ section, key })) : Effect.succeed(Result.fail({ section, key, reason: { _tag: 'threw' as const, cause } })),
            onSuccess: () => {
                const to = _value(target, key);
                return Effect.succeed(
                    Equal.equals(to, value) || !Equal.equals(to, from) ? Result.succeed({ section, key, from, to }) : Result.fail({ section, key, reason: { _tag: 'unchanged' as const } }),
                );
            },
        }),
    );
...
const getPreferences: Handler = handler(GetPreferences, Preferences, ({ sections }) =>
    Effect.sync(() => {
        ...
        return { kind: 'preferences' as const, values: Record.fromIterableWith(reads, ({ section, values }) => [section, values]), unreadable: Array.flatMap(reads, ({ unreadable }) => unreadable) };
    }),
);
