...
const HYGIENE_LOG_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['folder', 'verdict', 'summary', 'pages', 'comments_cut', 'reduction_pct'],
    properties: {
        folder: { type: 'string' },
        verdict: { type: 'string', enum: ['trimmed', 'refined', 'clean'] },
        pages: { type: 'integer' },
        comments_cut: { type: 'integer' },
...
const STYLE =
    'PROSE QUALITY — apply docs/standards/style-guide.md: lead each section with the controlling rule; one idea per paragraph; close on ' +
...
const sliceCtx = (u) =>
    !u.slice
        ...
        : '\nPAGE SLICE ' +
          u.slice +
          ' of 2 (this folder holds ' +
          u.pages +
          ' design pages; the corpus is split so ' +
          ...
          'planning tree sorted lexicographically by path; your pages are ' +
          (u.slice === 1 ? 'the FIRST half of that ordering (1..ceil(N/2))' : 'the SECOND half of that ordering (after ceil(N/2))') +
          '. PROCESS and EDIT only your slice pages — the sibling slice agent owns the rest ' +
          'concurrently; a FAMILY sweep runs across your slice; the RIPPLE + CURRENT-STATE law still governs an out-of-tree far end.';

const ctx = (u) =>
    '\nFOLDER: ' +
    u.folder +
    '\nPLANNING TREE (your primary surface): ' +
    u.planning +
...
const tidyPrompt = (u) =>
    [
        ...
        ADVERSARIAL,
        '',
        'TASK: SURGICAL HYGIENE PASS over EVERY design page under this folder`s .planning tree. ' +
            ...
            'was already maximally tight (then `refined`/`clean`).' +
            ctx(u),
    ].join('\n');

const critiquePrompt = (u) =>
    [
        DOCTRINE,
        '',
        ADVERSARIAL,
        '',
        'TASK: HOSTILE HYGIENE AUDIT + FIX IN PLACE over EVERY page under this folder`s ' +
            ...
            'quality-only edits, `clean` ONLY when a full attack found nothing.' +
            ctx(u),
    ].join('\n');

const redteamPrompt = (u) =>
    [
        DOCTRINE,
        '',
        ADVERSARIAL,
        '',
        'TASK: ADVERSARIAL RED-TEAM + FIX IN PLACE over this folder`s .planning tree — the ' +
            ...
            'is the fix-log of edits made: folder, pages, reduction_pct, summary; verdict `clean` ONLY when the full cold attack found nothing.' +
            ctx(u),
    ].join('\n');
...
const processUnit = async (u) => {
    // true per-unit pipeline: each stage consumes the prior stage's on-disk product; a dead stage ends its unit's chain
    const logs = {};
    for (const st of STAGES) {
        const r = await slot(() =>
            agent(st.build(u), {
                label: st.key + ':' + u.folder + (u.slice ? '#' + u.slice : ''),
                phase: 'Tidy',
                schema: HYGIENE_LOG_SCHEMA,
                effort: st.effort,
                stallMs: STALL,
            }),
        );
...
const inv = await agent(
    'List every PACKAGE FOLDER under ' +
        SCOPE +
        ' that owns design pages in a `.planning/` tree. For each, return {folder ' +
        ...
        'planning tree themselves, so the list never caps what they process. Do not cd; do not edit anything.',
    { label: 'discover', phase: 'Discover', schema: DISCOVER_SCHEMA, model: 'sonnet', effort: 'low', stallMs: STALL },
);
...
const pcts = done
    .map(lastLog)
    .filter((l) => l && typeof l.reduction_pct === 'number')
...
log(
    'Tidy: ' +
        ...
        '/' +
        workUnits.length +
        ' units fully passed (tidy -> critique -> redteam); comments cut ' +
        commentsCut +
        '; mean prose reduction ' +
        reduction +
        '%',
);

return {
    workflow: 'tidy-planning-docs',
    scope: SCOPE,
    folders: units.length,
    units: workUnits.length,
    complete: complete.length,
    incomplete: done.filter((r) => !r.ok).length,
    comments_cut: commentsCut,
    reduction_pct_mean: reduction,
    results: done.map((r) => ({ folder: r.folder, slice: r.slice, log: lastLog(r) })),
};
