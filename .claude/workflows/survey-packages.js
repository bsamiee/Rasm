...
const LAW = [
  ...
  'PYTHON RUNTIME GATE (binding when language=python): the workspace floor is CPython 3.15 (pyproject.toml requires-python >=3.15) on osx-arm64, and admissibility is decided by an ACTUAL `uv lock` + `uv sync` + import on cp315, NEVER by wheel-presence alone. A package is admitted UN-GATED when it is (a) pure-Python / a cp315-clean wheel (abi3 cp3x<=15 or py3-none-any), OR (b) a native sdist that SOURCE-BUILDS on cp315 via the Forge scientific toolchain (the shapely/pyarrow/pyogrio/geopandas pattern) — PREFER the source-build over a marker, and verify it by running `uv lock`+`uv sync` and importing the module. A `python_version<3.15` companion marker is honest ONLY for a package with a real, reported cp315 build failure — wheel-absence ALONE is NOT a reason to gate (that is the reflexive hedge to eliminate). Heavy native substrates (GDAL/GEOS/PROJ/Arrow/HDF5/tesseract/libvips) are Forge-provisioned. After any Python add you MUST `uv lock`+`uv sync` and import-verify it on cp315 (the ruff/ty gate does NOT exercise the install); revert only a package that genuinely cannot resolve/build. Reject legacy/abandoned distributions.',
  'AGGRESSIVE ADDITION, ZERO HEDGING (future-looking justification): admit a package whenever it provides unique, modern capability appropriate to the folder domain/scope. DECLINE ONLY for a real reason — (a) it is old / unmaintained / low-quality, (b) a strictly stronger alternative is already admitted, or (c) it is out of the folder scope/domain. NEVER decline because there is no CURRENT consumer: planned consumers are real design pressure and zero current consumers never lowers the capability bar. Once justified, implement NOW and FULLY with NO hedging — never pin or document a capability as future / planned / deferred / a separate-future-owner. A justified package is admitted, INSTALLED/RESOLVABLE on the floor, .api-documented, README-listed, AND integrated into the relevant EXISTING design pages by growing the existing owner IN PLACE (a case / row / field / operation, the rebuild-csharp expand-in-place mentality), not merely mentioned beside it. An ALREADY-ADMITTED package missing its README row, its .api, its actual install, or its real integration is an INCOMPLETE admission to COMPLETE now (manifest + install + .api + README + real design integration), never a thing to re-shelve as future.',
