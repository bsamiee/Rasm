...
- `protected virtual CreateCommands() -> void` / `RegisterCommand(Command command) -> bool`; `GetCommands() -> Command[]`; `HostUtils.RegisterDynamicCommand` composes the runtime command family of `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-commands.md`
...
- `PlugIn.GetPluginSettings(Guid plugInId, bool load) -> PersistentSettings` / `SavePluginSettings(Guid plugInId) -> void` / `SaveSettings() -> void` / `CommandSettings(string name) -> PersistentSettings` — the settings tree, whose typed writer surface `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns
- `PlugIn.SettingsSaved -> EventHandler<PersistentSettingsSavedEventArgs>` — fires the change notification `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns; `RaiseOnPlugInSettingsSavedEvent()` / `FlushSettingsSavedQueue()` raise and drain it
...
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-fileio.md`: `FileWriteOptions`/`WriteFileResult` write-policy detail and the direct format engines compose there; this catalog owns only their dispatch-contract role
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md`: `PersistentSettings` node and `SettingsSaved` event compose there; `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-rendercontent.md` and the render catalogs own the `RenderPlugIn` content-registration and change-queue surfaces
