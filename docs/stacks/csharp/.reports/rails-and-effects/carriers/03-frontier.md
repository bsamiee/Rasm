...
[FIN_UNSAFE_VALUE_ACCESSOR_LAW]:
- `Fin<A>.SuccValue` and `Fin<A>.FailValue` are public abstract properties on the base class, with `Succ` and `Fail` nested types providing overrides. They are labeled "Unsafe access" in the library's documentation — they do not guard the case and throw when accessed on the wrong discriminant. The only safe read paths are pattern matching via `{ IsSucc: true }` flat form, `case Fin<A>.Succ s` (giving `s.Value` as the public positional field), and `case Fin<A>.Fail f` (giving `f.Error`). The `SuccValue`/`FailValue` names are accessible from consuming assemblies and appear in decompiled bodies, but their use is restricted to host-boundary adapters performing explicit unwrap-or-throw semantics.

[VALIDATION_BIBIND_CONSTRAINT_ASYMMETRY]:
- `Validation<F,A>.BiBind<F1,A1>(Func<F, Validation<F1,A1>>, Func<A, Validation<F1,A1>>)` carries no compile-time constraint on `F1`. The target failure type may be any type, including non-semigroup types. This is the structural escape hatch: `BiBind` bypasses the monoid requirement entirely because it replaces both rails with a new `Validation<F1,A1>` supplied by the caller — the library does not need to combine `F1` values internally. Contrast `BindFail<F1>` which delegates to `BiBind` with `SuccessI<F1,A>` as the success arm; because subsequent `Apply` accumulation on the result may need to combine `F1` values, `BindFail` is the conservative form. The pattern: `BiBind` for structural type-change where accumulation semantics are not preserved; `BindFail` when the result must remain a law-abiding `Validation`.

[CATCHOF_SEMANTICS_FOR_MANYERRORS]:
- `catchOf<E,M,A>` and `catchOfFold<E,M,A>` handle `ManyErrors` differently. `catchOf` calls `es.Filter<E>().ForAllM(Fail)`: `ForAllM` on `ManyErrors` is sequential-short-circuit — it folds left-to-right, binding each atomic error into the next via `Bind`, stopping on first failure. `catchOfFold` calls `es.Filter<E>().FoldM(Fail)`: `FoldM` on `ManyErrors` starts with `M.Empty<A>()` and combines each result via `M.Combine` — when `M = Fin`, this accumulates errors; when `M = Eff`, this accumulates the last error. The `ForAllM`/`FoldM` distinction encodes the fail-fast vs. accumulate policy for multi-error `ManyErrors` payloads inside a single catch handler.
...
- `Option<A>.BiMap<B>(Func<A,B> Some, Func<Unit,B> None)` and `Option<A>.BiMap<B>(Func<A,B> Some, Func<B> None)` are two overloads distinguished by the `None` branch signature. The `Func<Unit,B>` overload passes the result through `Check.NullReturn`, which throws `ResultIsNullException` when the mapping function returns `null`. The `Func<B>` overload does not call `Check.NullReturn` — it returns the result of `None()` directly without a null guard. When the `None` producer may return a null reference type value, the `Func<Unit,B>` overload is the defensive form; the `Func<B>` overload silently admits `null` from the `None` branch and can produce a `Some` wrapping `null`.
...
- `Natural<Fin, Option>` discards the error: `Succ → Some(value)`, `Fail → None`. The reverse direction uses `ToFin()` (the zero-arg overload), which maps `None` to `Fail(Errors.None)`. Two consequences: (1) `Fin` to `Option` and back is not round-trip-safe — a `Fail` case survives both directions but with the original error replaced by `Errors.None`; (2) any carrier-polymorphic pipeline using `Natural.transform<Option, M, A>` will generate `Errors.None`-tagged failures from `None`, making subsequent `HasCode` or `Is` predicates behave incorrectly unless the caller tests `error.IsEmpty`. The correct ingress pattern when `None` must carry diagnostic identity is always `option.ToFin(Error.New(...))` or `option.ToFin(Errors.SomeMeaningfulCode)` rather than relying on the natural transformation. `Natural<Fin, IO>` preserves the error: `Succ → IO.Pure`, `Fail → IO.fail<A>(error)`.

[VALIDATION_PARTITION_SURFACE]:
- For `Validation`, the batch partition surfaces are `ValidationExtensions.Fails<F,S>(IEnumerable<Validation<F,S>>)` → `IEnumerable<F>` and `Fails<F,S>(Seq<Validation<F,S>>)` → `Seq<F>`. These extract the failure values from a sequence of validations as a flat projection. There is no symmetric `Succs` on `Validation` through this surface — the success side is not partitioned in parallel. Contrast `Fin`: `K<F, Fin<A>>.Partition()` returns `(Seq<Error> Fails, Seq<A> Succs)` as a single tuple, `Fails()` returns just the error sequence, and `Succs()` returns just the success sequence — all three are defined on any `Foldable<F>` container of `Fin<A>`, not on `Fin<A>` itself. The `Fin` partition surface requires a wrapping foldable (e.g., `Seq<Fin<A>>` as `K<Seq, Fin<A>>`); there is no `Fin<A>.Partition()` instance method.
...
- The `Validation` static class implements `Bifunctor<Validation>`. `BiMap<L,A,M,B>` (the first-argument map) is implemented by delegating to `BiBind`: the `Fail` arm calls `f(x).As2()` and the `Succ` arm calls `SuccessI<Y,A>` (the identity success cast). The instance-level `MapFail<F1>` produces a new `Validation<F1,A>` by mapping the failure type. `MapFail` and the `Bifunctor.BiMap` first-map are not the same operation: `MapFail` only touches the failure side; `Bifunctor.BiMap` uses `BiBind` internally and operates through the higher-kinded `K<Validation, L, A>` surface rather than directly on `Validation<F,A>`, carrying no constraint through that path.
