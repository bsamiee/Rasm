...
The two-coordinate snapshot diff the conflict inspector reads — `asOfDiff` materializes one keyed map at two `AsOf` coordinates through `asof#AS_OF_QUERY` and folds the union of keys present in either snapshot into one closed `DiffEntry` family, so the inspector reads "what changed between version V and version W", "between the stable frontier and now", or "across the event-time mark" as one structural delta rather than a hand-diffed pair of `HashMap`s. `DiffEntry` is the closed four-case `Data.TaggedEnum` (`Added`/`Removed`/`Changed`/`Unchanged`) a total `diffEntry` dispatch resolves per key by the presence pair and one `Equivalence` over the materialized value, `DiffSummary` is the one fact-stream count carrier slotting each verdict by its tag rather than four parallel counters, and `asOfDiff` is the one diff entrypoint over any two coordinates the same `asOfQuery` projection answers — never a second store holding a parallel history, never a per-coordinate-pair diff function. The diff is pure over the two materialized snapshots: it re-reads no changefeed, re-mints no clock, and re-folds no convergence state — both snapshots arrive settled from `asof#AS_OF_QUERY`, which sources the Version coordinate off the retained `window#DATAFLOW` `Index#reconstructAt` trace, the EventTime coordinate off the `watermark#WATERMARK` mark, and the Stable coordinate off the `causality/frontier#STABILITY_FRONTIER` horizon, so the diff inherits the same coordinate algebra and adds only the set-difference over the two resulting key spaces.

## [01]-[INDEX]

- [01]-[AS_OF_DIFF]: Owns `DiffEntry`, `DiffSummary`, `valueEquivalence`, `diffEntry`, `summarize`, and `asOfDiff` — the closed verdict family, the one fact-stream count carrier, and the two-coordinate diff fold over the materialized snapshots.
- [02]-[DIFF_PROJECTION]: Owns `AsOfDiffView`, `diffSince`, and `asOfDiffStore` — the live `Projection`-faced diff the conflict inspector binds through the `fold/projection#PROJECTION` `Atom.subscribable` seam, re-materializing the diff whenever either coordinate's store advances.
...
- Packages: `effect` for `Effect`, `Stream`, `SubscriptionRef`, `Scope`, and `HashMap`; the `Projection`/`projectStore`/`derive` face arrives owned from `fold/projection#PROJECTION` and the `asOfDiff` fold from `[2]-[AS_OF_DIFF]`; `@effect-atom/atom` is named in the binding seam only, never imported into the fold interior (the `fold/projection#BINDING_SEAM` consumer law).
