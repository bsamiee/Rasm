...
- [02]-[TOPIC_FABRIC]: `Topic` rows over a `DrainSurface.Broadcast` fan, the `DomainEvent` carrier, and native drop receipts.
...
- [05]-[LOSS_RECEIPT]: One native loss account consumed by the local instrument fan.

## [02]-[TOPIC_FABRIC]

- Owner: `DomainEvent` the topic-carrying event carrier stamped with its topic's delivery ordinal, carrying the `EventType` fact identity and `EventSource` producer identity the durable projection publishes; `Topic` `[SmartEnum<string>]` the rich native topic axis carrying its `TopicDurability` and shed threshold; `TopicDurability` `[SmartEnum<string>]` the branch-local two-row axis declaring whether a row's producer commits the fact to the op-log in its own transaction; native `DropClass` and `DropReceipt` the loss vocabulary and evidence; `TopicHead` the fan capsule carrying the topic's `Atom<ulong>` offer cursor; `BusFault` `[Union]` fault family in the 4730 band.
...
- Receipt: a published event is one `DomainEvent` on the fan; a declined offer accounts as one native `DropReceipt` with `DropClass.Missed`, minted by the subscription's gap fold in flight and by the conductor's residual at drain through the topic's one `FanMiss` projection; no parallel topic or delivery mirror exists.
...
| :-----: | :---------- | :------------------------------------- | :----------------------------------------------------- |
|  [01]   | `Command`   | in-process best-effort; fact committed | a declined offer receipts `Missed`, final in-process   |
|  [02]   | `Lifecycle` | in-process best-effort; fact committed | a declined offer receipts `Missed`, final in-process   |
|  [03]   | `Health`    | in-process best-effort alone           | a declined offer receipts `Missed`, final for the fact |
|  [04]   | `Delivery`  | in-process best-effort; fact committed | a declined offer receipts `Missed`, final in-process   |
|  [05]   | `Collab`    | in-process best-effort; fact committed | opaque payload; a miss receipts `Missed`, final here   |
...
- Receipt: each forward jump in the arriving `Offset` mints one `FanMiss` the topic's projection turns into a native `DropReceipt` with `DropClass.Missed`; a backward arrival counts on the seat's own re-offer tally and mints nothing, because the dedupe window is what absorbs it and a second account of one re-offer is the merged tally this page refuses everywhere else. A successful in-process callback mints no delivery receipt; explicit outbound and outbox hops own delivery evidence.
...
- Receipt: the bus drain folds into the `DrainReceipt` the conductor mints; a shed dispatch is a receipted no-op minting one native drop receipt rather than a fault; the drain residual mints the closing missed receipts onto the same sink seat the in-flight gaps feed, and every `DropReceipt` fans under `ReceiptKind.Drop` so the bus's whole loss account reaches a board rather than an in-process delegate alone.
- Packages: Rasm (kernel `ReceiptSinkPort`), Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, BCL inbox
...
flowchart LR
    ...
    accDescr: A DomainEvent enters EventBus.Dispatch from an interior producer or the companion event-ingress door; the degradation shed gate either mints a Shed drop receipt or hands the event to the topic fan, which stamps the topic's next dense offset through the drain-surface broadcast builder; each subscription is one bounded consumer sink seated directly as a fan sink, folding the arriving offset into its seat and minting a Missed drop receipt for any gap before the composed dedupe window admits the event; the bus drains under the runtime conductor, which compares the head cursor against each seat and mints the residual Missed receipt for tail loss; the durable leg leaves this bus entirely and never returns an event to it.
    ...
    Dispatch -- shed --> Drops["native DropReceipt stream"]
...
- Owner: native `DropClass` and `DropReceipt`; the two named mints project topic rows and loss facts directly without an intermediate wire mapper.
- Packages: Rasm (kernel `FanMiss`), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox
- Growth: a new loss cause is one `DropClass` row only when existing rows cannot state it; a new mint site composes the same receipt rather than a parallel carrier.
...
public sealed record DropReceipt(
    ...
    ulong First,
    ulong Last) {
    public ulong Count => checked(Last - First + 1);

    public static DropReceipt Missed(Topic topic, FanMiss miss) => new(
        topic, Some(miss.Sink), DropClass.Missed, miss.First, miss.Last);

    public static DropReceipt Shed(Topic topic, ulong cursor) => new(
        topic, None, DropClass.Shed, cursor, cursor);
}
```
