...
[STACKING]:
- `optimistix`(`.api/optimistix.md`): `OptaxMinimiser(optim, rtol, atol, norm=max_norm)` lifts any optimizer or `chain` into an `AbstractMinimiser`, so it solves inside `optimistix.minimise` with the `Solution`/`RESULTS` verdict and `ImplicitAdjoint`/`RecursiveCheckpointAdjoint` differentiation rather than a hand-written loop.
