...
- `VividOrange.Loads`/`VividOrange.Cases`(`.api/api-vividorange-cases`): a design-layer seam sharing no compiled member — a construction `IStage` selects the `ILoadCase` set live during that phase, joining the `Planning/schedule#SCHEDULE` `ConstructionTask` to the `Model/structural#STRUCTURAL_PROJECTION` load rows active at that task, the sole compiled meeting point the shared `VividOrange.Countries` context
