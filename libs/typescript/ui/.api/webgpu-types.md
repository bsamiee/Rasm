# [@webgpu/types] — the ambient WebGPU IDL surface under `three`'s WebGPU path; a tsconfig `types` entry, not an import; `scope:viewer` project-local
...
- package: `@webgpu/types` (0.1.71, BSD-3-Clause, © W3C / GPU for the Web WG) — the `0.x` tracks the living WebGPU IDL, moving as the standard evolves
...
- rail: type substrate
- `navigator.gpu` (`GPU`) is the root; `requestAdapter` → `GPUAdapter` → `requestDevice` is the async negotiation the viewer runs once at boot. Features and limits are the capability gate — the WebGL2-fallback decision reads them.

| [INDEX] | [SYMBOL]                                                                                          | [TYPE_FAMILY]  | [CONSUMER / BOUNDARY]                                              |
| :-----: | :----------------------------------------------------------------------------------------------- | :------------- | :---------------------------------------------------------------- |
|  [01]   | `NavigatorGPU` (`readonly gpu: GPU`) / `GPU` (`requestAdapter(options?)`, `getPreferredCanvasFormat(): GPUTextureFormat`, `wgslLanguageFeatures`) | entry singleton | `viewer/scene/glb` — `navigator.gpu.requestAdapter()`; `getPreferredCanvasFormat()` picks the swap-chain format |
|  [02]   | `GPUAdapter` (`features: GPUSupportedFeatures`, `limits: GPUSupportedLimits`, `info: GPUAdapterInfo`, `requestDevice(descriptor?)`) / `GPURequestAdapterOptions` (`powerPreference`, `forceFallbackAdapter`, `featureLevel`) | adapter | the capability probe; `powerPreference: 'high-performance'` selects the discrete GPU on a multi-GPU laptop |
|  [03]   | `GPUDeviceDescriptor` (`requiredFeatures`, `requiredLimits`, `defaultQueue`) / `GPUSupportedFeatures` (`ReadonlySet<string>`) / `GPUSupportedLimits` / `GPUAdapterInfo` | device request | `requiredFeatures: GPUFeatureName[]` negotiates capabilities up front — an unsupported feature rejects the request, the viewer falls back |
|  [04]   | `GPUFeatureName` (`'depth-clip-control'`/`'timestamp-query'`/`'float32-filterable'`/`'texture-compression-bc'`/`'texture-compression-etc2'`/`'texture-compression-astc'`/…) / `GPUPowerPreference` (`'low-power'`/`'high-performance'`) | capability vocab | the parameterized feature/power-preference unions the negotiation discriminates on — one union owns the whole capability axis; the `texture-compression-*` members are what `KTX2Loader.detectSupport` probes through `GPUDevice.features` to pick the GLB texture transcode target |
...
| [INDEX] | [SYMBOL]                                                                                          | [TYPE_FAMILY]  | [CONSUMER / BOUNDARY]                                              |
| :-----: | :----------------------------------------------------------------------------------------------- | :------------- | :---------------------------------------------------------------- |
|  [01]   | `GPUDevice` (`createBuffer`/`createTexture`/`createSampler`/`createBindGroup`/`createShaderModule`/`createRenderPipeline`/`createComputePipeline`/`createCommandEncoder`, `createRenderPipelineAsync`/`createComputePipelineAsync`, `queue`, `features`, `limits`, `lost`, `pushErrorScope`/`popErrorScope`, `destroy`) | device | `viewer/scene/glb` — the resource + pipeline factory; `features: GPUSupportedFeatures` is the capability set `three`'s `renderer.hasFeature(...)` and `KTX2Loader.detectSupport` read for `texture-compression-*`; `lost: Promise<GPUDeviceLostInfo>` is the device-loss re-init seam |
|  [02]   | `GPUQueue` (`submit`, `writeBuffer`, `writeTexture`, `copyExternalImageToTexture`, `onSubmittedWorkDone`) / `GPUCommandBuffer` | submission     | `queue.submit([encoder.finish()])` per frame; `writeBuffer` uploads geometry/uniforms |
|  [03]   | `GPUBuffer` (`mapAsync`, `getMappedRange`, `unmap`, `size`, `usage`, `mapState`) / `GPUTexture` (`createView`, `destroy`) / `GPUTextureView` / `GPUSampler` / `GPUExternalTexture` / `GPUQuerySet` | resources | GPU-resident mesh/texture data; `mapAsync`+`getMappedRange` is the CPU↔GPU readback (GPU picking, compute results) |
|  [04]   | `GPURenderPipeline` / `GPUComputePipeline` (`getBindGroupLayout`) / `GPUBindGroup` / `GPUBindGroupLayout` / `GPUPipelineLayout` / `GPUShaderModule` (`getCompilationInfo`) | pipelines | the compiled render/compute programs and their binding layouts; `getCompilationInfo` surfaces WGSL diagnostics |
|  [05]   | `GPUCommandEncoder` (`beginRenderPass`/`beginComputePass`/`copyBufferToBuffer`/`copyTextureToBuffer`/`finish`) / `GPURenderPassEncoder` (`setPipeline`/`setBindGroup`/`setVertexBuffer`/`draw`/`drawIndexed`/`drawIndirect`/`end`) / `GPUComputePassEncoder` (`dispatchWorkgroups`/`dispatchWorkgroupsIndirect`/`end`) / `GPURenderBundleEncoder` / `GPURenderBundle` | encoders | the per-frame command recording; `dispatchWorkgroups` runs a compute shader (meshopt decode, GPU picking, particle sim) |
...
| [INDEX] | [SYMBOL]                                                                                          | [TYPE_FAMILY]  | [CONSUMER / BOUNDARY]                                              |
| :-----: | :----------------------------------------------------------------------------------------------- | :------------- | :---------------------------------------------------------------- |
|  [01]   | `GPUBufferDescriptor` (`size`, `usage`, `mappedAtCreation`) / `GPUTextureDescriptor` (`size`, `format`, `usage`, `dimension`, `mipLevelCount`, `sampleCount`) / `GPUSamplerDescriptor` / `GPUTextureViewDescriptor` | resource config | `usage` is a `GPUBufferUsage`/`GPUTextureUsage` flag OR; `format` a `GPUTextureFormat` |
|  [02]   | `GPURenderPipelineDescriptor` (`vertex`, `fragment`, `primitive`, `depthStencil`, `multisample`, `layout`) / `GPUComputePipelineDescriptor` (`compute`, `layout`) / `GPUVertexState` / `GPUFragmentState` / `GPUProgrammableStage` | pipeline config | the full pipeline spec; `GPUVertexBufferLayout`/`GPUVertexAttribute` describe the vertex stream |
|  [03]   | `GPUBindGroupDescriptor` / `GPUBindGroupLayoutDescriptor` / `GPUBindGroupLayoutEntry` (`buffer`/`sampler`/`texture`/`storageTexture`/`externalTexture` binding layouts) / `GPUPipelineLayoutDescriptor` | binding config | the shader-resource binding declaration; the layout entry is a discriminated binding-kind union |
|  [04]   | `GPURenderPassDescriptor` / `GPURenderPassColorAttachment` (`loadOp`, `storeOp`, `clearValue`, `view`, `resolveTarget`) / `GPURenderPassDepthStencilAttachment` / `GPUColorTargetState` / `GPUBlendState` / `GPUDepthStencilState` / `GPUPrimitiveState` / `GPUMultisampleState` | pass config | the render-pass attachment + blend/depth spec; `loadOp`/`storeOp` are `GPULoadOp`/`GPUStoreOp` |
...
| [INDEX] | [SYMBOL]                                                                                          | [TYPE_FAMILY]  | [CONSUMER / BOUNDARY]                                              |
| :-----: | :----------------------------------------------------------------------------------------------- | :------------- | :---------------------------------------------------------------- |
|  [01]   | `GPUTextureFormat` / `GPUVertexFormat` / `GPUIndexFormat` / `GPUTextureDimension` / `GPUTextureViewDimension` / `GPUTextureAspect` | format vocab | the pixel/vertex/index/texture-shape unions — the format axis of every resource |
|  [02]   | `GPUPrimitiveTopology` / `GPUCullMode` / `GPUFrontFace` / `GPUBlendFactor` / `GPUBlendOperation` / `GPUCompareFunction` / `GPUStencilOperation` / `GPULoadOp` / `GPUStoreOp` | pipeline-state vocab | the rasterizer/blend/depth-op unions the pipeline descriptor keys on |
|  [03]   | `GPUAddressMode` / `GPUFilterMode` / `GPUMipmapFilterMode` / `GPUBufferBindingType` / `GPUSamplerBindingType` / `GPUTextureSampleType` / `GPUStorageTextureAccess` / `GPUQueryType` | sampler/binding vocab | the sampler + binding-layout unions |
|  [04]   | `GPUBufferUsage` / `GPUTextureUsage` / `GPUShaderStage` / `GPUColorWrite` / `GPUMapMode` (`declare var` flag namespaces) | flag namespaces | the bitwise usage/stage/write/map flags OR-combined into a descriptor `usage`/`visibility` — `GPUBufferUsage.VERTEX \| GPUBufferUsage.COPY_DST` |
...
| [INDEX] | [SYMBOL]                                                                                          | [TYPE_FAMILY]  | [CONSUMER / BOUNDARY]                                              |
| :-----: | :----------------------------------------------------------------------------------------------- | :------------- | :---------------------------------------------------------------- |
|  [01]   | `GPUCanvasContext` (`configure(config)`, `unconfigure()`, `getConfiguration()`, `getCurrentTexture()`, `canvas`) / `GPUCanvasConfiguration` (`device`, `format`, `usage`, `viewFormats`, `colorSpace`, `toneMapping`, `alphaMode`) / `GPUCanvasAlphaMode` / `GPUCanvasToneMappingMode` | presentation | `canvas.getContext("webgpu")` → configure with the device + format; `getCurrentTexture()` is the per-frame render target |
|  [02]   | `GPUError` / `GPUValidationError` / `GPUOutOfMemoryError` / `GPUInternalError` / `GPUPipelineError` / `GPUErrorFilter` | error surface  | `pushErrorScope('validation')`/`popErrorScope()` captures pipeline/resource errors; the WGSL/binding validation the viewer logs |
|  [03]   | `GPUDeviceLostInfo` (`reason`, `message`) / `GPUDeviceLostReason` / `GPUUncapturedErrorEvent` / `GPUCompilationInfo` / `GPUCompilationMessage` / `GPUCompilationMessageType` | loss + diagnostics | `device.lost` resolves on loss (GPU reset, tab background) → the viewer re-initializes; `getCompilationInfo` carries WGSL line diagnostics |
