...
- Packages: Grasshopper2 (the eight paint events, `CanvasPaintEventArgs`, `CanvasBackgroundPaintEventArgs.OverrideDefaultPainting`, `ControlGraphics`, `Skin`), LanguageExt.Core, `Rasm.Domain` (`Lease<T>`, `FaultCell`, `Cell`, `HookSet`), `Rasm.Interaction` (`UiFault`, `UiThread`, `UiDispatch<T>`), `Shell/session.md` (`GhSession`, `ScopeTarget`), `Shell/hooks.md` (`GrasshopperPoint`, `HookSignal`).
...
- Law: the path hand-off is a two-lease bracket — the kernel `PathSpec.Build` mints an owned Eto path lease, the composer reads `IGraphicsPath.Bounds` from that admitted figure, `LayerPaint.Stroked` converts it into an owned `Lease<CGPath>`, and the composer releases the Eto lease the moment `Stroked` returns; the `FillMode` argument is inert on a stroke projection and passes `Winding` by declaration.
...
flowchart LR
    ...
    Anchor -->|"background veto fires"| Hooks["HookSet"]
...
| :-----: | :--------------- | :------------------------------ | :-------------------------------------------------- | :-----: |
|  [01]   | event window     | `PaintPhase` + `PaintScene`     | eight contained host rows, result-typed scene reads |    8    |
...
Mark vocabulary, specs, stock, lifetime brackets, tally, probe, colour crossing, and OS palette roster are the kernel paint module's (`Mark`, `PathSpec`, `FillSource`, `TransformSpec`, `StrokeSpec`, `Dash`, `TypeFace`, `GlyphBlock`, `PaintProgram`, `PaintStock`, `PaintTally`, `PaintColor`, `ChromeRole`); the ten hand rollback ladders, three CAS triples, `PaintLifetime`, `PaintHook`, and the six `Finite`/`Admitted` duplicates deleted onto it.
