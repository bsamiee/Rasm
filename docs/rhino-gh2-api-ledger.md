...
| **[15]** | `InputAdder.AddCurveLocus`, `AddSurfaceLocus`, output equivalents | `PortKind.CurveLocus` and `PortKind.SurfaceLocus` bind native GH2 locus parameter types. |
...
| **[6]** | `Tree<T>.WithTopology`, `Flatten`, `Graft`, `WithSimplifiedPaths` | `PortData<T>` preserves trees, but component-level topology transforms are not yet first-class. |
...
- `libs/csharp/Rasm.Grasshopper` models ports as item, twig, or tree data with metadata, null state, and native locus parameter kinds.
