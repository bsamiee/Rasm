...
- `wire#vocab` appearance (`csharp:Rasm.Materials/Appearance/interchange` seam): `viewer/scene.md` [08] consumes generated `appearance.Material` and `Set` beside element `AppearanceWire`, resolves every plane through the complete `PlaneRef.artifact`, and binds the decoded values onto `MeshPhysicalMaterial`; a peer TypeScript OpenPBR algebra is the `CROSS_LANGUAGE_WIRE` drift defect.
