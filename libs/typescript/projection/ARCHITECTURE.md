...
│   ├── causal-buffer.md        #   CausalBuffer dependency-gated release + DeliveryVerdict Held/Released + causalDelivery
│   └── stability-frontier.md   #   stabilityFrontier SortedSet-of-cursors greatest-lower-bound meet
├── temporal-query/             # the point-in-time as-of read axis over the version and event-time coordinates
│   ├── as-of-query.md          #   AsOf coordinate (Version/EventTime/Stable) + asOfQuery reconstructAt projection
│   └── as-of-diff.md           #   asOfDiff — the two-coordinate snapshot diff the conflict inspector reads
...
- `causal-delivery` is the discipline the verdict alone cannot enforce: `causality-graph` tags each op's order but applies it immediately, so an out-of-causal-order arrival paints a state no peer ever held; `causal-delivery` buffers an op until its `context` version vector is dominated by the held cursor and releases the dependency-satisfied suffix in causal order — `causal-buffer` owns the dependency-gated release discipline (`CausalBuffer`, the `DeliveryVerdict` Held/Released dispatch, `causalDelivery`), and `stability-frontier` owns the `SortedSet`-of-cursors greatest-lower-bound meet that names the causally-stable horizon `frontier-gc` finalizes against and the Merkle reconciliation treats as the stable prefix unit — one discipline over the `version-vector` `dominates` algebra, never a second ordering surface.
- `temporal-query` is the as-of read axis the always-latest fold lacks: a read-model tier answers "the state as of version V / event time T / the stable frontier", so the `AsOf` coordinate discriminates one query surface that materializes the keyed map through the `d2ts` `Index.reconstructAt` version-trace for the windowed path and the watermark replay-to-mark fold for the versionless path — `as-of-query` owns the `AsOf` coordinate and the `asOfQuery` projection (the Version coordinate reading the `d2ts` retained trace the `standing-query/window-fold` dataflow re-founding produces), and `as-of-diff` owns the two-coordinate diff the conflict inspector reads — never a second store holding a parallel history.
