...
`libs/typescript` is the platform's host-free web/edge half — the third first-class peer runtime beside the C# producer and the Python evaluation companion: a lib-grade substrate of capability domains that compose into whole web and edge products, shipped as one npm package `@rasm/ts` with per-domain subpath exports and `server`/`browser`/`wasm` conditions. Its relation to the siblings is decode-only and contract-coupled: C# owns every wire vocabulary and the tessellation rail, the branch decodes each family exactly once at one keyed registry, mints no wire, owns no geometry and no IFC semantics, and re-derives content identity bit-identically from the one C#-owned seed — complete in isolation, adoptable in any monorepo, aligned by wire bytes, frozen contract corpora, and the descriptor drift gate, never by import.
...
- `@effect/opentelemetry` — bridges OTLP; the runtime otel sub-domain owns the wire, every folder emits through the core `Convention` vocabulary.
- `@effect/vitest` — binds `@rasm/ts-testkit` combinators to colocated specs as the spec runner; catalogued at `tests/typescript/.api/`.
