...
- [01]-[QUANTITY_TABLE]: twenty-one frozen quantity rows; conversion exactly once at admission; tolerance-keyed equivalence.
...
- Owner: `QuantityFamily` `[SmartEnum<string>]` twenty-one rows, keyed ordinal-ignore-case through the shipped `ComparerAccessors.StringOrdinalIgnoreCase` accessor; `UnitMetadata` the metadata-sourcing owner over `QuantityInfo.BaseUnitInfo`/`UnitInfos`/`UnitType`.
