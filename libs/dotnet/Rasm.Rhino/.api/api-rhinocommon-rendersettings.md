...
`RenderSettings.PostEffects` is the settings-side effect roster; `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-render.md` owns the `PostEffect` authoring, execution, and gating family this container configures.
...
- `ICurrentEnvironment.ForBackground` / `ForReflectionAndRefraction` / `ForLighting -> RenderEnvironment` (get/set) — the resolved current-environment triple behind `RhinoDoc.CurrentEnvironment`; the accessor is `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-document.md`'s.
...
- `Sun.Enabled -> bool` / `ManualControlOn -> bool` / `Intensity -> double` / `Accuracy -> Sun.Accuracies` / `Light -> Light` / `Hash -> uint` — sun enablement, manual override, derived scene contribution, and data CRC.
- `Sun.Vector -> Vector3d` (get/set) — unit sun-TOWARD-scene ray in document world coordinates: azimuth `0`/altitude `0` under the default north reads `(0, -1, 0)` and the zenith reads `(0, 0, -1)`, so a survey frame's scene-toward-sun ray is its negation. Reads unitize whatever the setter took. Writes are INERT unless `ManualControlOn` — they neither throw nor report, so a write under a sited sun silently keeps the solved position; under manual control a zero or non-finite ray sets azimuth `180`/altitude `0`, a due-south horizon sun indistinguishable from a measured one, so admission refuses the degenerate ray upstream.
- `Sun.North -> double` (get/set) — compass north as a COUNTER-CLOCKWISE angle off `+X`, `90` (the default) seating north on `+Y` and making the world frame the `+X`-east/`+Y`-north survey frame outright. It rotates the WORLD frame, never the reading: `Azimuth` stays east-of-North against it, so holding the angles and moving `North` swings `Vector` while both readings hold. Writes wrap into `[0, 360)`.
- `Sun.Azimuth -> double` / `Altitude -> double` / `Latitude -> double` / `Longitude -> double` / `TimeZone -> double` / `DaylightSavingOn -> bool` / `DaylightSavingMinutes -> int` — sun position and time context; the georeference `Latitude`/`Longitude`/`North` slice is `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-document.md`'s.
- `Sun.SetPosition(double azimuthDegrees, double altitudeDegrees) -> void` — manual angle pair; it writes `Azimuth`/`Altitude` and the derived `Vector` together and, like the `Vector` setter, wants `ManualControlOn` first.
- `Sun.GetDateTime(DateTimeKind) -> DateTime` / `SetDateTime(DateTime, DateTimeKind)` — sun date/time. `SetDateTime` DOES NOT honour `DateTimeKind.Utc`: it re-bases the instant by the MACHINE's local offset rather than by `Sun.TimeZone`, so a UTC instant lands shifted and the solved position moves with it. `SunDirection` or the kernel almanac serves any derivation that must reproduce off-machine.
- OBSOLETE: `Sun.ManualControl -> bool` (use `ManualControlOn`) / `Sun.SetPosition(DateTime, double, double)` (use `SetDateTime` with the `Latitude`/`Longitude` properties) — both still compile-warn on the installed distribution and neither belongs in a fence.
- `Sun.SunDirection(double, double, DateTime) -> Vector3d` / `AltitudeFromValues(double, double, double, int, DateTime, double, bool) -> double` / `ColorFromAltitude(double) -> Color` / `JulianDay(double, int, DateTime, double) -> double` / `TwilightZone() -> double` / `Here(out double, out double) -> bool` — static sun-position and astronomical solvers. `SunDirection` points sun-toward-scene like `Vector` but answers in the FIXED `+X`-east/`+Y`-north survey frame, ignoring `Sun.North`: one document rotating its compass moves the instance `Vector` and leaves this return untouched, so reading the two through one frame assumption skews every angle by the bearing.
...
- Settings configure, rendering consumes: `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-render.md` reads a `RenderSettings` at render time, and this catalog never runs a render.
- The post-effect container is the settings side of that split: `PostEffectCollection` orders, selects, and parameterizes the roster while `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-render.md`'s `PostEffect`/`PostEffectPipeline` executes it, so a configuration write never reaches an execute body and an execute body never edits the roster.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Vector3d` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): the `RenderSettings` aggregate rides a `using` bounded by `Eff` while every sub-owner rides the aggregate's window UNBRACKETED — a lease over one releases nothing document-side and leaks the free-floating native; a settings read projects a detached value record, and a commit-through-accessor outcome crosses as `Fin<Unit>`.
...
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-rendercontent.md`: owns the content the per-usage `RenderEnvironment` binding points at.
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-document.md`(`RhinoDoc.RenderSettings`, `MaterialTable`/`LightTable`) and `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-fileio.md`(`File3dmSettings.RenderSettings`): the document and archive accessors mint and commit a `RenderSettings`, and the georeference `Sun` slice stays with the document catalog.
