...
```ts contract
export type Model = typeof BedrockFoundationModelId.Encoded   // 91 ids, section [05]
...
`BedrockFoundationModelId` is a 91-id `Schema.Literal` spanning every Bedrock provider family — Amazon Titan/Nova, Stability, AI21 Jamba, Anthropic Claude, Cohere Command/Embed, DeepSeek, Meta Llama, Mistral. Regenerable via `aws bedrock list-foundation-models --output json | jq '[.modelSummaries[].modelId]'`.

```ts contract
declare const BedrockFoundationModelId: Schema.Literal<[/* 91 ids: amazon.titan-*, amazon.nova-*, stability.*, ai21.jamba-*, anthropic.claude-*, cohere.*, deepseek.r1-*, meta.llama*, mistral.* */]>
...
The guardrail-trace corpus is ONE assessment tree parameterized by policy kind, feeding `ConverseTrace`/`ConverseStreamTrace` → `FinishPartMetadata.bedrock.trace`. `GuardrailTraceAssessment` holds `inputAssessment`/`outputAssessments` maps of `GuardrailAssessment`, each a record over six policy owners:
...
- Sibling providers: this row peer-depends on `@effect/ai-anthropic` (see `effect-ai-anthropic.md`) — it consumes that package's `prepareTools` and `Generated.BetaCacheControlEphemeral` to run Claude-on-Bedrock tools, and forwards Anthropic beta opt-ins via `params["anthropic-beta"]`. The `AmazonBedrockConfig` tag id copy-paste bug (`@effect/ai-google/…`) is a spelling hazard `provider.ts` must respect.
- Native guardrails: Bedrock is the provider whose asymmetry populates the `provider.ts` guardrail-gate cell natively — set `Config.Service.guardrailConfig` per call, and the `GuardrailTraceAssessment` tree surfaces on `FinishPartMetadata.bedrock.trace`, an alternative to the design's app-level moderation folds.
- Design consumers: `model/provider.ts` (row + tier-routing via `Config` + the guardrail gate, read from `FinishPartMetadata.bedrock.trace`), `tool/toolkit.ts`+`tool/mcp.ts` (the eight `requiresHandler:true` tools bound via `Toolkit.toLayer`). No `model/token.ts` or `embed/embedder.ts` binding — both asymmetry cells are empty.
