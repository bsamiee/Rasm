...
- Fault causes split with the construction path: `makeMemory` wraps `@effect/wa-sqlite`'s `SQLiteError`, whose `code` is the C API's NUMERIC primary result code, while `OpfsWorker.run` posts `e.message` across the port, so the worker-backed client's `SqlError.cause` is a bare STRING and no field read reaches it. One lane answers two cause shapes, and a classifier keyed on `code` reads nothing at all on the durable path.
...
- DDL rides `withTransaction` atomically on the wa-sqlite engine exactly as on the native profiles — a minted shadow and its `sqlite_sequence` row roll back together, `ALTER TABLE … RENAME` carries the counter row, and `CREATE TABLE … AS SELECT` drops every key, constraint, default, and `NOT NULL` — so the cutover's own-DDL mint and its `INSERT INTO sqlite_sequence … SELECT` seed run unchanged here; the memory lane's `SqlError.cause` carries only the numeric primary `code`.
