...
|  [17]   | `tx.TryGet(db, out byte[])` / `(…, Span<byte>, out int)`            | static   | try-read into caller storage                |
|  [18]   | `tx.TryGet(db, IBufferWriter<byte>)` / `(…, byte[] buffer)`         | static   | try-read into a pooled sink                 |
|  [19]   | `tx.ContainsKey(db)`                                                | static   | existence probe with no value copy          |
...
|  [10]   | `LightningCursor.Put(value, CursorPutOptions)`                | instance | positioned write, `AppendData` bulk-loads   |
...
- `MDBValue` addresses mmap memory only its own transaction guarantees, so a decode, `CopyTo`, or `CopyToNewArray` lifts the bytes out before `Commit`, `Abort`, or `Dispose` releases the page; `Read<T>` and `Cast<T>` decode unmanaged shapes in place.
...
- `MaxDatabases` binds BEFORE `Open` or not at all: its setter raises `InvalidOperationException("Can't change MaxDatabases of opened environment")` on an opened environment, so the named-keyspace roster is a provisioning fact the `EnvironmentConfiguration` carries at construction, never a cap a later keyspace widens. `Open(EnvironmentOpenFlags = None, UnixAccessMode = Default)` then fixes the sync posture for the WHOLE environment, so a per-database durability posture has no expression and a mixed roster opens under its strictest member.
- `Delete(db)` and `Delete(db, value)` are two different retirements on a `DuplicatesSort` keyspace and the compiler separates them by arity alone: the whole-key form drops the key and EVERY dup value under it, so reaching for it to retire ONE member erases every other member's entry silently. Dup-scoped removal names the value, and a caller holding only the key cannot express one.
- `LightningException : Exception` publishes `public int StatusCode { get; }` — the `MDBResultCode` integer LMDB returned — and renders as `LightningDB {StatusCode}: {Message}`, so a RAISED code re-enters the same verdict fold a RETURNED code takes and stamping the CLR type name into a status slot forks one engine's taxonomy in two.
- `GetMultiple()` and `NextMultiple()` both answer `(MDBResultCode resultCode, MDBValue key, MDBValue value)` where the value is a PACKED page of fixed-width dup values, so a `DuplicatesFixed` dup set drains one native crossing per page and the caller slices by its own member width; without that flag the page has no fixed stride and the walk falls back to per-value `NextDuplicate` or `AllValuesFor`.
