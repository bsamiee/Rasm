...
- Coordinates are `int`: the `Nesting/stock` owner ceils each part's footprint and the sheet extent to `int` on the way in, and offsets the returned `Rect.X`/`Rect.Y` back into the layout coordinate frame on the way out. There is no `uint` domain (contrast `RectpackSharp`, which is `uint`).
