...
const allRes = []
for (const r of done) for (const st of ['crit', 'redteam']) { const l = r.logs && r.logs[st]; if (l && l.residual) for (const x of l.residual) allRes.push(norm(x, r.folder)) }
