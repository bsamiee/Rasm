...
- version: `2.3.0`; floor `honeybee-energy-standards; python_version<'3.15'`
...
- marker: COMPANION-GATED. Manifest-pinned `python_version<'3.15'` (co-released with the Honeybee/Ladybug set), gated OUT of the active 3.15 interpreter; `assay api resolve honeybee-energy-standards` returns `unsupported`. This catalog is verified against the real `2.3.0` wheel installed in the `~/ladybug_tools/python` cp310 companion environment; the gate is pure manifest co-release (data-only wheel, no ABI cap) and lifts on 3.15 validation.
...
- layering axis: the library is ADDITIVE on top of the `honeybee-standards` defaults floor. At `honeybee_energy.lib` import, `_load*` reads `folders.defaults_file` FIRST (the small always-resolvable `honeybee-standards` baseline: 20 constructions, 2 program types, 8 schedules, 1 construction set), THEN scans `folders.standards_extension_folders` and merges this extension. The registries (`OPAQUE_CONSTRUCTIONS`/`PROGRAM_TYPES`/`SCHEDULES`/`CONSTRUCTION_SETS`) enumerate the union, so a count of ~1845 program types / ~3347 schedules means this extension is installed — those are its contribution, not `honeybee-standards`'.
