...
// land here: list ORDER is what Graph/wire#WIRE_CODEC emits — the canonical fingerprint sorts its own sections and
// never reads it — and a hash-ordered set forfeits that deterministic emission, since Generator.Equals hashes
// through per-process-randomized string hashing.
