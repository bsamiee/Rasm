...
| :-----: | :------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------- | :--------------- |
|   [1]   | `AutomationProperties` attached-setter and `AutomationLiveSetting` member spellings behind the identity and announcement columns | `uv run python -m tools.assay api query --key avalonia --symbol Avalonia.Automation.AutomationProperties` | AUTOMATION_PEERS |
|   [2]   | Automation-peer synthesis over Skia-drawn chart, tile, and preview visuals                                 | `uv run python -m tools.assay api query --key livecharts --symbol LiveChartsCore.SkiaSharpView.Avalonia.CartesianChart` | AUTOMATION_PEERS |
|   [3]   | VoiceOver reach into embedded-root content across the NSView boundary on Rhino panel rows                  | `tests/csharp/libs/Rasm.Rhino/UI/scenarios/avalonia-embed-a11y.verify.csx`                              | AUTOMATION_PEERS |
|   [4]   | Live-region projection of `AutomationProperties` SetLiveSetting and SetName transitions through the macOS automation backend | `uv run python -m tools.assay api query --key avalonia-desktop --symbol Avalonia.Native --grep Automation` | AUTOMATION_PEERS |
|   [5]   | `KeyboardNavigation` attached-setter spellings and `KeyboardNavigationMode.Cycle` trap behavior on the dialog session overlay root | `uv run python -m tools.assay api query --key avalonia --symbol Avalonia.Input.KeyboardNavigation`     | KEYBOARD_NAV     |
|   [6]   | Peer-walk route for headless peer-presence audits over ControlAutomationPeer                               | `uv run python -m tools.assay api query --key avalonia --symbol Avalonia.Automation.Peers.ControlAutomationPeer` | COMPLIANCE_PROOF |
