...
public static class ScheduleCpm {
    public static Fin<Map<string, CriticalPath>> Schedule(this ScheduleNetwork network, Op key, Option<BimHooks> hooks = default) =>
        network.Tasks.IsEmpty
            ? Fin.Succ(Map<string, CriticalPath>())
            : Opened(CpmStage.Ordered, hooks, key).Bind(_ => Graph(network, key)).Bind(graph => {
                ...
                return Opened(CpmStage.Forward, hooks, key)
                    ...
                    .Bind(forward => {
                        ...
                        return Opened(CpmStage.Backward, hooks, key)
                            .Bind(_ => Backward(graph, calendars, duration, projectFinish, key))
                            .Bind(backward => Opened(CpmStage.Floated, hooks, key)
                                .Bind(_ => Paths(graph, network, calendars, forward, backward, duration, key)))
                            .Bind(paths => Opened(CpmStage.Settled, hooks, key).Map(_ => paths));
                    });
            });

    static Fin<Unit> Opened(CpmStage stage, Option<BimHooks> hooks, Op key) => Fin.Succ(stage.Beat(hooks, key));
