...
- [03]-[CENSUS] — the `Probe` port, the sampled gauge sweep, and the hook-bus delta feeder; `Pulse`.
...
- Law: three tables stand because three consumers do — `_WORK` answers the `mark` fold at the emitting call site, `_TAP` answers the bus delta feeder, and `_GAUGES` answers the level sweep, so each table's row type fixes the mounted instrument exactly and no consumer casts; the board fold reads all three through one concatenation, so a new instrument is one row in its owning table and appears on every downstream projection by construction.
...
- Law: the port keeps the strata clean — the data journal's `Journal.census` statement satisfies `Probe` at the app root, so the outbox truth crosses the boundary as a value and this module imports no SQL surface; the queue depth arrives from the durable-queue owner's own read through the same binding, and that binding composes `Tenancy.sweep` around the census statement because the census answers per app across every tenant on a FORCE-RLS relation — an unpinned sample reads an empty outbox as healthy and a tenant-pinned one reports one slice as the plane's whole depth.
- Law: the probe is total by contract — the satisfying binding internalizes its store faults (the prior sample or a zero census stands in), because a broken gauge sweep must degrade a dashboard, never fail a process. Contracts type against a FAILURE and a defect escapes them, so the sweep folds defects itself: an unchecked store fault costs one interval and reads on the logger, where an unfolded one kills the repeat fiber and freezes every gauge at its last value for the process lifetime — a dashboard reading stale levels as live ones.
...
- Growth: a new census dimension is one `Census` field and one `_GAUGES` row reading it; a new bus tally is one `_FEED` entry over its `_TAP` row.
...
- Law: the floor is one root decision — the Layer merges once at the composition root beneath the export lane, so the OTLP log leg and any file logger both inherit it; a per-module level override is a `Logger.withMinimumLogLevel` region on the owning logger, never a second root install.
- Law: this floor governs the Effect logger alone — the SDK's own diagnostic stream rides `Export.Policy.diagnostic` into `diag.setLogger` at `emit#LANES`, and the `LoggerProvider` scope configurator stays unmounted, so exactly one owner governs each stream and a record dropped at one floor is never re-admitted at another.
...
- Law: each row's execution boundary is the metric plane it selects — `rasm.*` rows govern Effect-minted series through the producer projection at `emit#GOVERNANCE`, `v8js.*` rows govern the raw provider through the SDK's own view engine, and the aggregation re-arm only reaches instruments the raw provider owns because a collected point cannot be re-bucketed; the row shape is one vocabulary and the boundary follows the selection.
