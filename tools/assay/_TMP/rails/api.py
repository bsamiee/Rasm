...
from tools.assay._TMP.composition.settings import ArtifactScope, AssaySettings  # noqa: TC001  # registry passes both at runtime
...
def _spawn(settings: AssaySettings, scope: ArtifactScope, tool: Tool, *args: str) -> Completed:
...
def _surface(settings: AssaySettings, scope: ArtifactScope, source: _Source) -> Result[_Surface, Fault]:
    ...
    non-zero exit on a *present* assembly is ``FAILED`` evidence, surfaced as the fault message so
    ``doctor``/``query`` report it without crossing the ``Error`` seam as a process fault.
    """
...
def _run_surface(
    ...
    attempts = tuple((asm, _spawn(settings, scope, tool, str(asm))) for asm in assemblies)
    match any(done.returncode == 0 for _, done in attempts):
        case False:
            ...
            return Error(Fault(("api", "surface", source.key), status=RailStatus.FAILED, message=detail[:1024]))
        case True:
...
def _run_decompile(settings: AssaySettings, scope: ArtifactScope, tool: Tool, fqn: str, assemblies: tuple[Path, ...]) -> Completed:
    ...
    attempts = tuple(_spawn(settings, scope, decompile_tool, str(asm)) for asm in ordered)
...
def doctor(settings: AssaySettings, scope: ArtifactScope, p: ApiParams) -> Result[Report, Fault]:
    ...
    app = _rhino_app(settings)
    version = _spawn(settings, scope, surface_tool, "--version") if surface_tool is not None else Completed(("ilspycmd",), 1)
