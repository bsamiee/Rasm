...
public sealed class DegradationCell(
    ...
    Fin<DegradationReading> Fanned(DegradationReading reading, AppHostFact fact) =>
        rail.Fire(at: fact.At, fact: fact, key: key).Map(_ => reading);
}
```
...
- Entry: `Sweep(AlertEngine.Runtime runtime, AlertCell cell, DegradationReading reading, Instant at)` returns `IO<Seq<AlertReceipt>>` — the composition's one stream entry, folding the whole roster against one reading and committing each rule's advanced state; `Observe(Runtime, AlertRule, AlertState, DegradationReading, Instant)` is the per-rule leg it folds — it resolves `rule.Signal` through `Project`, delegates to the pure value-fold `Evaluate`, and delivers each transition receipt on `IO`; `Evaluate(AlertRule, AlertState, double, Instant, CorrelationId, Op)` returns `(AlertState State, Option<AlertReceipt> Fired)`; `Backtest(AlertRule, Seq<(Instant, double)>, CorrelationId, Op)` returns `Seq<AlertReceipt>` by replaying that pure fold without a delivery runtime. One fold, two front doors: the live stream resolves and delivers, while the back-test feeds historical values and only returns evidence.
...
- Receipt: `AlertReceipt` — rule id, rule version, severity, condition key, the firing value, the transition row, `Instant`, and the correlation the producing reading carried, so an alert joins the snapshot that fired it rather than rooting a fresh causal frame; the delivery arm writes it through `ReceiptSinkPort.Send` under `ReceiptKind.Alert`, so the alert crosses on the one stamped `ReceiptEnvelope` every sibling receipt rides and the outbound fan reaches it without a second transport; the receipt kind is receipt-only by declaration — an alert names a `HealthSignal` where every `InstrumentFan` arm names a mounted instrument, so a fan arm here mints the second grader the Boundary below and the folder ruling both forbid.
...
public static class AlertEngine {
    ...
    public static IO<(AlertState State, Option<AlertReceipt> Fired)> Observe(
        Runtime runtime, AlertRule rule, AlertState state, DegradationReading reading, Instant at) =>
        Project(rule, reading).Match(
            Some: value => Deliver(runtime, Evaluate(rule, state, value, at, reading.Snapshot.Correlation, runtime.Key)),
    ...
    public static Seq<AlertReceipt> Backtest(AlertRule rule, Seq<(Instant At, double Value)> history, CorrelationId correlation, Op key) =>
        history.Fold((State: AlertState.Clear, Fired: Seq<AlertReceipt>()), (acc, sample) =>
            Threaded(acc, Evaluate(rule, acc.State, sample.Value, sample.At, correlation, key))).Fired;
    ...
    static IO<(AlertState State, Option<AlertReceipt> Fired)> Deliver(
        ...
        step.Fired.Match(
            Some: receipt => runtime.Sink
                .Send(receipt.Correlation, TenantContext.Current, TelemetrySource.AppHost,
                    ReceiptKind.Alert.Key, JsonSerializer.SerializeToElement(receipt, SuiteContracts.Host))
                .Map(_ => step),
            None: () => IO.pure(step));
