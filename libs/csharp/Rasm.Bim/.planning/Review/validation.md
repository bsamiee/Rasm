...
- Receipt: `ModelHealth` stores the tier receipts TYPED — the `TemplateFinding` rows keep the failing-axis evidence a fix pass acts on, the `IdsAudit` rows keep the spec-level cardinality rule and feed `Reconcile` unchanged — and `Findings` is the derived projection, never a third stored copy; the two cases keep their native element identities (the seam `NodeId` for the baseline, whose audit covers authored elements not yet exported; the IFC `GlobalId` for the authored tier, whose exchange is defined over the IFC-visible universe): the identity is the tier's own audit universe, never coerced onto one axis.
- Packages: Rasm.Element, Thinktecture.Runtime.Extensions, LanguageExt.Core
- Growth: a new baseline verdict axis is the properties owner's `TemplateVerdict` row and rides the `Baseline` case untouched; a new facet, cardinality, or relation rides the authored tier's own growth rows; a third verdict source is one `ModelFinding` case plus one `Findings` arm with every consumer `Switch` broken loudly at compile time; never a second model-health entry, and never a per-tier report type.
...
// --- [MODELS] -----------------------------------------------------------------------------
// The two-tier model-QA receipt — the ONE entry Rasm.AppUi and the review pipeline read. The tier receipts stay
...
public sealed record ModelHealth(Seq<TemplateFinding> Baseline, Seq<IdsAudit> Authored) {
    ...
    // the baseline's seam Bake rail (the authored fold is total over the frozen graph).
    public static Fin<ModelHealth> Audit(ElementGraph graph, Seq<IdsSpecification> specifications, Func<IfcClass, Option<BsddClass>> dictionary, Op key) =>
        TemplateAudit.Run(graph, dictionary, key)
            .Map(baseline => new ModelHealth(baseline, specifications.Map(spec => spec.Audit(graph))));
