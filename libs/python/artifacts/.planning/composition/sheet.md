...
- [ENGINE_CLOSE_DEEPENED]: each frame author threads its OWN engine's full archival close rather than deferring the whole close to a later `Stamp`/`exchange/conformance` pass — the reading-map underutilization the three arms carried. The `weasyprint` arm threads the FULL `write_pdf` close beside `pdf_variant`/`pdf_tags`/`output_intent`: `attachments=[Attachment(file_obj=, name=, description=, relationship="Data")]` re-embeds the `OutputIntent` ICC as a PDF associated file, a `<head>` `<meta author/description/keywords>` seed folded into weasyprint's OWN variant XMP through `custom_metadata=True` (the ENGINE owns the XMP — its `pdf_variant` machinery writes the conformant `pdfaid` packet — so the hand-built `_xmp` ElementTree packet is NOT threaded into `xmp_metadata=` where it could drop archival fields the variant requires; `_xmp` stays only for the pymupdf-based reportlab/typst `Stamp` arms that have no engine XMP generator), and `pdf_identifier=_pdf_id(title, standard)` a stable 16-byte `/ID`. The `reportlab` arm gains the `lib.colors.CMYKColor` process-black discipline ink (`setStrokeColor`/`setFillColor`, press-faithful separations vs the sRGB default), the ISO 7200 info-dict (`setTitle`/`setAuthor`/`setSubject`/`setKeywords`/`setCreator` from `TitleBlock.metadata()`) so the frame self-describes rather than depending on a later `Stamp`, and a navigable `bookmarkPage`+`addOutlineEntry` outline mirroring `document/emit#DOCUMENT`'s `PDF_AUTHOR` `afterFlowable`. The `Stamp` arm gains a non-`DRAFT` archival-hygiene preamble — `subset_fonts(fallback=True)` + a metadata-preserving `scrub` (hidden-text/javascript/clean-pages/thumbnails on; metadata/xml/embedded/attached/redactions/reset/remove-links OFF so the pass never nukes the info-dict/XMP/associated-files it then writes) — mirroring the sibling `composition/imposition#IMPOSE` `Marks.finished`. Every member verifies against the folder catalogs: `weasyprint` `Attachment`/`attachments=`/`custom_metadata=`/`pdf_identifier=`, `reportlab` `lib.colors.CMYKColor`/`setStrokeColor`/`setFillColor`/`setTitle`/`setAuthor`/`setSubject`/`setKeywords`/`setCreator`/`bookmarkPage`/`addOutlineEntry`, `pymupdf` `subset_fonts(fallback=)`/`scrub`. Justified on PACKAGE (the verified deeper reportlab/weasyprint/pymupdf close surface the reading map flagged) and DOMAIN (a press-faithful, self-describing, archival-hygiene drawing sheet).
- [CONTENT_KEY_DETERMINISM_SETTLED]: every engine now pins a stable document identifier so the framed/placed/filled/stamped bytes are BYTE-DETERMINISTIC across runs — the content-addressed pipeline's cache elides a re-render only when the same sheet keys to the same `ContentKey`, and a random per-run `/ID` (the PDF writer default) makes two renders of one sheet byte-distinct, so the cache NEVER hits. The `weasyprint` arm pins `pdf_identifier=_pdf_id(title, standard)` (a `hashlib.sha256(...).digest()[:16]` over the sheet identity), the `typst` arm already pins `timestamp=0`, and the three pymupdf-serializing arms (`Place`/`Fill`/`Stamp`) pin `tobytes(..., no_new_id=True)` — the direct sheet analogue of the sibling `composition/imposition#IMPOSE` `no_new_id`. `tobytes(no_new_id=)` and weasyprint `pdf_identifier=` are verified members. Justified on PACKAGE (verified determinism knobs) and CONSUMER (the `core/plan#PLAN` content-keyed elision + the runtime session-cache hit/miss both require byte-stable producer output).
- [CROPMARKS_ACROFORM_DEFERRED]: two reading-map reportlab surfaces are DELIBERATELY not adopted, because each regresses a correctness invariant the page holds. `Canvas(cropMarks=True)` ENLARGES the reportlab media box to fit the trim marks, so the emitted page rect no longer equals `_SIZES[size]`; the `Fill` arm reads `page.rect` and re-derives every title-block cell from it, so a cropped frame would offset every filled value from its drawn label — the exact illusory-alignment defect `[MODAL_ARITY_SETTLED]` deleted, re-introduced through the media box. `canvas.acroForm.textfield` interactive title-block fields would DUPLICATE the drawn title-block labels (the drawn grid IS the title block) with a second, editable field layer no reader expects on a construction drawing. Both are real reportlab surfaces but out of this arm's shed boundary until a verified crop-box/`Fill`-rect reconciliation (read `page.trimbox` where it differs) and a fields-XOR-drawn-labels decision land; neither is a capability gap the sheet concept demands.
