...
- [RESEARCH_REGISTER]: R1 carried — no admitted producer for the ASTM C90/TMS 602/ACI 216.1 rosters exists; every column stays AUTHORED/PUBLISHED at today's density. The `CmuPhysics` axes landing surface is RESOLVED as `CmuSeed.Table`: the brief's `[COMPONENT_DETAIL]` widens only masonry + glazing, so a cmu lane flip is brief-illegal and the designation-keyed seed join is the ONE legal landing — no `component.md` lane-law edit, the Compute reader next-campaign forward-only.
