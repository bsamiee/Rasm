...
- Growth: one armor label is one `PemLabel` row; one bundle-element kind is one `PemBlock` carried in the existing ordered bundle, never a parallel message envelope; one fault is one `PemFault` case; a new credential material kind rides the label axis already; zero new surface.
