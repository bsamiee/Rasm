...
public static class CompositionRoot {
        ...
        new RootBinding.Seated("control-inbound", static (services, inputs) =>
            Fin.Succ(services.Add(ServiceDescriptor.Describe(
                ...
                provider => new ControlRuntime(
                    ...
                    // Cooperative budget is spent AS GIVEN: `ControlInbound.DrainRuntime` already folded it to
                    // `min(inherited, DrainCooperative)` under the hop-topology law, so the composed fold
                    // reads the `DrainThread` FORCED row alone. Applying that minimum a second time narrows a
                    // budget the parent already narrowed, and neither seat can see the other doing it.
                    Drain: cooperative => Conducted(provider, inputs.DrainThread(), cooperative),
    ...
    // field-identical off the `DrainParticipantPort` contributor fan, and one `DrainThread` mint per drain
    // carries the conductor's whole telemetry tail. `Seal` belongs to the act, not to a caller after it —
    // that context is minted per drain, so draining without exporting its frozen ledger discards exactly the
    // checkpoints the phase recorded.
    static IO<DrainReceipt> Conducted(IServiceProvider provider, DrainThread thread, Duration cooperative) =>
        from receipt in provider.GetRequiredService<Lifecycle>().Drain(
            toSeq(provider.GetServices<DrainParticipantPort>()).Map(static row => (row.Name, row.Band, row.Rank, row.Drain)),
            thread.Latency, cooperative, thread.Forced.Allotted, thread.Checkpoint, thread.Instruments)
