...
using Rasm.Domain;                                   // Op (the boundary-admission key SectionCapacityResolver.Resolve rails the ComponentFault on)
...
using Rasm.Materials.Component;                      // ComputedSection, ComponentFault (the parent COMPONENT_OWNER section receipt + band-2300 rail)
using Rasm.Materials.Component.Steel;                // DesignCapacity, CompactnessClass (the steel#STEEL_FAMILY LRFD receipt lifted WHOLE as SteelLrfd)
using Rasm.Materials.Component.Timber;               // TimberCapacity (the timber#TIMBER_FAMILY EC5 receipt lifted WHOLE as TimberEc5)
using Rasm.Materials.Component.Cmu;                  // CmuStrength (the cmu#CMU_FAMILY f'm the MasonryCompression case feeds)
using Rasm.Materials.Component.Reinforcement;        // RcSection (the reinforcement#RC_SECTION IConcreteSection input the RC cases solve over)
using Thinktecture;
...
// The capacity owner is its OWN Rasm.Materials.Component.Capacity sub-namespace (the SAME per-page sub-namespace law the
// family pages follow); it composes the parent COMPONENT_OWNER (ComputedSection/ComponentFault), the three structural
// family sub-namespaces it lifts receipts from (Steel/Timber/Cmu), and the Component.Reinforcement RC section — every
// cross-page type imported by its sub-namespace-qualified using above.
namespace Rasm.Materials.Component.Capacity;

// --- [TYPES] -------------------------------------------------------------------------------
// The RC-build dispatch for SectionCapacityResolver.Resolve — the TWO capacity surfaces built FROM an RcSection input (the
...
// by design — it does NOT mirror the full SectionCapacity case set (a redundant parallel discriminant).
[SmartEnum<string>]
...
// "outside the capacity surface" rather than the silent +∞ of a divide-by-epsilon.
...
public abstract partial record SectionCapacity {
    ...
    // and InertiaZzMm4 (the minor-axis fibre stress), the full biaxial combined action never a major-axis-only slice.
    ...
    public static Fin<SectionCapacity> Resolve(RcSection rc, CapacityKind kind, DiagramResolution resolution, Op key) =>
        kind.Switch(
            rcInteraction: _ => Try(() => new ForceMomentEngine(rc.Section, resolution.ToSettings()).Mesh).ToFin()
                .MapFail(e => ComponentFault.Capacity(key, $"<rc-interaction-solve:{e.Message}>"))
            ...
            // trap leaks a throw past the Fin; here every VividOrange throw rails ComponentFault.Capacity at the edge.
            rcElastic: _ => Try(() => {
            ...
            }).ToFin().MapFail(e => ComponentFault.Capacity(key, $"<rc-elastic-solve:{e.Message}>")));
