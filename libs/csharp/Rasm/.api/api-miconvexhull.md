...
- vs Triangle (`api-triangle`): `Triangle.NET` owns the CONSTRAINED-DELAUNAY / quality mesh of a planar straight-line graph (boundary + holes + quality/angle refinement, used by Materials' `InteractionDiagram` to discretize a section into `AnalyticalFaces`); MIConvexHull owns the UNCONSTRAINED point-cloud Delaunay (no boundary constraint, no quality refinement, but N-dimensional). Constrained-2D + quality → Triangle; unconstrained point-cloud / ND → MIConvexHull. They are complementary computational-geometry owners, not duplicates.
...
- vs LibTessDotNet (`api-libtess`): LibTess fills closed CONTOURS by winding rule (boundary input); MIConvexHull triangulates a POINT CLOUD (scattered-point input). Different inputs — a Voronoi/Delaunay over points is not a contour fill.
- vs Clipper2's ear-clip (`api-clipper2`): `Clipper.Triangulate(Paths64, out Paths64, useDelaunay=true)` is a polygon-INTERIOR ear-clip with an optional Delaunay edge-flip — it triangulates the AREA bounded by a simple `Paths64` (a fill of a closed region); MIConvexHull's `CreateDelaunay` is the point-cloud Delaunay (the connectivity of scattered points, no boundary). "Triangulate the area inside this polygon" → `Clipper.Triangulate` (or `LibTessDotNet` for messy winding, `Triangle` for a quality constrained mesh); "connect these scattered points into a Delaunay complex" → MIConvexHull. Both say "Delaunay" but over different inputs (bounded polygon area vs unbounded point set) — never interchangeable.
- vs SharpVoronoiLib (`api-sharpvoronoilib`): SharpVoronoiLib owns the 2D Fortune's-sweep Voronoi with explicit edge/site/clipping geometry (a 2D-specialized planar Voronoi); MIConvexHull's `VoronoiMesh` is the N-dimensional Delaunay-dual Voronoi as an abstract cell/edge graph. 2D planar Voronoi with edge clipping → SharpVoronoiLib; ND Voronoi as Delaunay dual → MIConvexHull.
...
- Reject: calling the ND `Create` on 2D data (it errors `DimensionTwoWrongMethod` — use `Create2D`); ignoring `Result.Outcome` and reading a possibly-null `Result`, or taking the throwing `ConvexHullGenerationException` path instead of the outcome fold; consuming GShark's internal `ConvexHull.GenerateHull` as the kernel hull (use this typed-result `Create`); using MIConvexHull for a CONSTRAINED/quality 2D mesh (that is `api-triangle`) or a 2D edge-clipped Voronoi (that is `api-sharpvoronoilib`); touching the package's public-but-internal `FaceConnector`/`ObjectManager`/`IndexBuffer`/`ConvexFaceInternal` plumbing; expecting robust/exact arithmetic (it is `double`-precision Quickhull — near-degenerate input is `DegenerateData`, escalate to the kernel's exact-predicate ladder where robustness is required).
