...
|  [08]   | `Image` / `ImageView`     | raster carriers  | `to_image` output (`Image`) and the custom-stride decode input view (`ImageView` over `ImageFormat` `RGB`/`BGR`/`RGBA`/`BGRA`/`ABGR`/`ARGB`/`Lum`/`LumA`) |
