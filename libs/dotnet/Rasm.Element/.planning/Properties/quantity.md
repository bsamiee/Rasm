...
- Output: a `MeasureValue` is the unit-checked magnitude a takeoff, a property facet, and a cost join read; `measure.Uncertainty` is absent exactly where the value is exact, and the uncertainty producer stays above the seam.
