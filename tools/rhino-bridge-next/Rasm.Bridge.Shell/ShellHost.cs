...
public sealed class ShellHost : IDisposable {
    ...
    private async Task PrepareQuitAsync(Connection connection, CancellationToken ct) {
        Admit(connection: connection);
        bool hadModified = await pump.OnUiThreadAsync(job: static () => {
        ...
        Publish(evt: Fact(key: "quit.prepared", value: hadModified ? "docs-marked-clean" : "docs-already-clean"));
    }
    ...
    private static void WritePoisoned(int rhinoPid, string fault) {
        ...
        try {
            ...
            RhinoApp.WriteLine(message: $"[rasm-bridge-next] poisoned endpoint write failed: {error.Message}; fault was: {fault}");
        }
    }
