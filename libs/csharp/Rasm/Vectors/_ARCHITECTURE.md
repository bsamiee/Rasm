...
6. `[partial]` Field, kernels, and SDFs include derivative-aware kernels, anisotropic tensor metrics, RBF receipts, oriented MLS receipts, half-space/slab/capped-cone primitives, closed planar single-region profile-extrusion SDFs, Lipschitz bounds, mesh-backed SDF receipts, and native iso-surface receipts. Validated factory admission is now the external construction path for field/falloff/integrator union cases, `FieldNabla` owns shared kernel/periodic/reconstruction/profile/iso-surface/plane validation, extraction preserves successful `IsoSurfaceReceipt` facts, native iso-surface receipts carry static-proven parallel callback and fixed `0.001` tolerance labels, and mesh-backed SDF receipts reuse topology facts. Keep runtime bridge proof for mesh/support-backed evaluator safety and closed SignedHeat as the remaining partial work.
...
- `[partial]` `Field.cs`: iso-surface routes through intent/extraction, and successful extraction receipts preserve the native `IsoSurfaceReceipt`. `ScalarField.IsoSurfaceDetailed` still owns the Rhino callback; failed native evaluator attempts still return `Fin` failure rather than a partial failure receipt. Static RhinoWIP decompile proof labels `Mesh.CreateFromIsosurface` as parallel-callback and fixed `0.001` root tolerance; runtime bridge proof still owns evaluator-safety labels for mesh/support-backed scalar fields.
...
- `[partial]` SDF outputs: `ScalarField.LipschitzBound()` exists, mesh-backed signing routes through generalized winding / boundary-source signed heat, and SDF/iso-surface receipts expose approximate/native facts. Mesh SDF receipts reuse topology facts and no longer advertise an unused unsupported status; watertight preflight, closed SignedHeat, runtime callback-safety proof, and product-strength native extraction scenarios remain incomplete.
