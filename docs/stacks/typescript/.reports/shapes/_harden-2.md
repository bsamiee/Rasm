...
const route = (demanded: HashSet.HashSet<Facet>, pool: ReadonlyArray<Signature>): Option.Option<{ readonly elected: Family; readonly handSpell: HashSet.HashSet<Facet> }> =>
    pipe(pool, Array.filter((c) => HashSet.isSubset(demanded, c.facets)),
        Array.sort(Order.combineAll([
            Order.mapInput(Order.number, (s: Signature) => HashSet.size(s.facets)),
            Order.mapInput(Order.number, Signature.$match({ Nominal: () => 1, Reference: () => 1, Structural: (s) => s.cardinality })),
            Order.mapInput(Order.number, Signature.$match({ Nominal: () => 0, Reference: () => 0, Structural: (s) => s.channels })),
            Order.reverse(Order.mapInput(Order.number, (s: Signature) => s.declarations)),
        ])), Array.last,
        Option.map((s) => ({
            elected: Signature.$match(s, { Nominal: () => 'Literal', Reference: () => 'taggedEnum', Structural: () => 'TaggedClass.Union' } satisfies Record<Signature['_tag'], Family>),
            handSpell: HashSet.difference(demanded, s.facets),
        })));
```
...
const distinctSorted = (rows: ReadonlyArray<Reading>): ReadonlyArray<{ readonly row: Reading; readonly digest: number }> =>
    pipe(Array.dedupeWith(rows, sameReading), Array.sort(byPrecedence), Array.map((row) => ({ row, digest: Hash.structureKeys(row, keys) })));
```
...
const weigh = (code: typeof Code.Type): number =>
    pipe(Schema.decodeSync(Decomposed)(code), ([scope, , subject]) =>
