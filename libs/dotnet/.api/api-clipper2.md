...
- `Rasm`(`Meshing/delaunay#TESSELLATION`): each `Tessellation.BoundedCell.Ring` off the kernel `Tessellation.VoronoiDual` bounded-cell overload scales to a `Path64` at the fabrication precision (the Fabrication `Geometry2D/algebra` `CellDiagram` carries the ring downstream), so a relaxed partition machines through `ClipperOffset` pocketing — the kernel bounded-cell overload owns the clipped point-site diagram.
