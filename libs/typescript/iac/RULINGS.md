...
- `_assetsUnder` proves published addresses at graph construction — an address never joined against the artifact holding the bytes is a declaration.
- Every Helm chart row pins `fullnameOverride` AND its rendered Service — chart-name arithmetic re-opens the dead addresses the pin closes.
...
- Residence rows carry `signals` and `exporter.routes` as separate columns — one column serving both leaves a two-writer gateway plane unspellable.
- `otelarrow` arms on the collector-to-collector hop under its own bounds — the gateway-to-backend hop is one in-namespace call to non-Arrow backends.
...
- Producers arm with their ingest rows on ONE coordinate — `observe.costs` arms the opencost chart and its `_SCRAPED` job; a split pair reads healthy.
...
- Provider-vocabulary coordinates type off the generated const — arm-invariant rosters close at `StackSpec`, arm-dependent ones at the arm's `_vocab`.
- Provider vocabularies admit whole at the estate — the spec takes the entire exported roster and preference between members rides a policy row.
...
- Rollout stages traffic behind Gateway weights and the pointer write is the one atomic cutover — abort tears those weights down, never a second verb.
...
- CRD estates no chart here installs earn a folder-tier `.api` catalogue — the cluster's controller owns the definitions member fences verify against.
