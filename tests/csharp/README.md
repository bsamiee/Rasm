...
- [04]-`Numeric.cs`: carries the `Norm` smart-enum vocabulary (`MaxAbs`, `L1`, `LInf`, `Frobenius`) and independent numeric/geometry oracles (`OrientSign`, `ShoelaceArea`, `SignedVolume`, residuals, spectral references) that return values, never assert mid-flight.
...
- [06]-`Seams.cs`: carries the `Shape<TValue>` substitution union (`Canned`/`FanOut`/`Factory`), `SeamProbe`, `VariantWriter`, `TmpRoot`, `NdjsonOracle`, and the `Timeline` deterministic clock over `FakeTimeProvider`.
