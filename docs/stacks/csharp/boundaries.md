...
- Law: project at the exact read site into `Option<T>`, `Fin<T>`, or a closed `[Union]` state family — the projector owns the absence vocabulary, and no sentinel survives into an interior or persisted shape.
- Law: the present branch never manufactures `Some(null)`; a value that can become null after transformation returns absence explicitly instead of relying on the carrier to erase it.
- Reject: sentinel checks in interior flow; nullable payloads riding past the seam.
...
- Law: one capsule acquires, projects, and releases — borrowed and owned lifetimes are cases of one `[Union]` surface, where the owned case constructs its `SafeHandle` with `ownsHandle: true` and releases, and the borrowed case constructs with `ownsHandle: false` and projects without disposal.
...
- Law: callers receive values or rails, never live handles; the capsule converts its own exceptions.
- Exemption: the add-ref/release window inside the capsule kernel is the named platform-forced statement seam.
- Reject: scattered `using`, public handle fields, parallel borrowed/owner wrapper types, or a second disposer registry.

```csharp conceptual
public sealed class ResourceHandle(bool ownsHandle) : SafeHandleZeroOrMinusOneIsInvalid(ownsHandle) {
...
public abstract partial record Lease {
    ...
    static Fin<TResult> Borrow<TResult>(ResourceHandle handle, Func<nint, TResult> copy) {
        var added = false;
        try { handle.DangerousAddRef(ref added); return Fin.Succ(copy(handle.DangerousGetHandle())); }
...
[REF_SAFE_PROJECTION]:
- Law: live memory crosses as a scoped view whose result is a value copy; any value returned from the projector is detached from the foreign lifetime.
- Accept: `readonly ref struct` views and `scoped` delegates for synchronous projection.
- Reject: storing spans, boxing live views, returning views, or crossing `await` with foreign memory.

```csharp conceptual
public readonly ref struct BoundaryView {
    private readonly ReadOnlySpan<byte> bytes;
    public BoundaryView(ReadOnlySpan<byte> bytes) => this.bytes = bytes;
    public int Length => bytes.Length;
    public Option<byte> First => bytes is [var head, ..] ? Optional(head) : None;
}
...
public static class ViewBoundary {
    ...
    public static Digest Capture(ReadOnlyMemory<byte> memory) =>
        Project(memory, static view => new Digest(view.Length, view.First));
}
```
...
- Exemption: attach/detach `+=`/`-=` wiring, the window-borrow open, and the posted-callback body are the named platform-forced statement seam.
...
- Law: cancellation normalizes once into the seam's failure vocabulary — caller shutdown, local deadline, and external timeout carry distinct evidence when behavior differs — and cancellation is never transient: it is the first arm every retry predicate refuses.
- Law: `ExecutionContext` flow and `SynchronizationContext` affinity are separate decisions; a context-free callback takes the unsafe registration form only when it reads no ambient state.
- Reject: `Thread.CurrentThread` tests, ambient context reads inside reusable transforms, fire-and-forget posts.

[HANDOFF_DRAIN]:
- Law: a high-frequency callback submits intent and returns — a committed cell is the latest-value register for a per-tick consumer, a `Channel<T>` is the log for a consumer that must see every intermediate; the consumer's right selects the carrier, and producer back-pressure is independent of consumer pacing.
...
[MEMO_KEY]:
- Law: memo keys encode every dimension that changes output.
...
- Reject: last-write-wins or best-effort parse for owned protocol shapes.

[CONTRACT_SURFACE]:
- Law: generation mode is contract coverage, not tuning — `JsonTypeInfo<T>` coordinates carry name, requiredness, converter, order, and discriminant — and resolver, converter, and options instances are stable contract identities; policy variance travels through values the converter reads, never fresh option graphs.
- Law: modifier fusion is ordered policy — rename before redaction is a different contract than redaction before rename.
- Law: a captured resolution fault replays on every request — sticky, non-transient, classified expected-non-retriable at the seam.
- Reject: reflection fallback where the contract claims generated coverage; per-call resolver construction, reordered converter lists, or post-seal mutation.
...
- Reject: converter-per-case sprawl, case converters bypassing the family owner, sentinels returned from converters, or unanchored interior exceptions.
...
public sealed class FrameConverter : JsonConverter<Frame> {
    public override Frame Read(ref Utf8JsonReader reader, Type type, JsonSerializerOptions options) {
        ...
        return buffered.RootElement.GetProperty(nameof(Frame.Single.Kind)).GetString() switch {
            ...
            Frame.KindB => buffered.RootElement.Deserialize(WireContext.Default.FrameBlock) ?? throw new JsonException("<null-frame>"),
    ...
    public override void Write(Utf8JsonWriter writer, Frame value, JsonSerializerOptions options) =>
        value.Switch(
            ...
            block:  static (w, b) => JsonSerializer.Serialize(w, b, WireContext.Default.FrameBlock),
            state: writer);
}
```
