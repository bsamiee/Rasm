...
- traversal: every `Relation`/`Expression` is double-dispatch-folded via `Accept<TReturn, TState>` against a `RelationVisitor`/`ExpressionVisitor` — a transform or extraction subclasses the visitor and overrides the relevant `Visit*` methods. The two bases DIVERGE (decompile-verified 0.15.0): `RelationVisitor` throws `NotImplementedException` for unhandled relation kinds (partial relation visitors fail loudly), while `ExpressionVisitor` returns `default(TOutput)` for every unhandled expression kind (literal/field/IfThen arms fold children and return default) — so an expression pushdown reads null as the graceful "inexpressible" signal (`Query/federation#PredicateLowering`), never a throw.
