...
- [AGING_CALIBRATION]-[BLOCKED]: which per-row terminal curve and AO-to-age gamma each `WeatheringEffect` binds so its `Weathering.Drift` CIEDE2000 trace matches a measured aging series (Cu→CuCO₃ chromaticity, Fe₂O₃ deepening); a published measured series admits the calibration, and no admitted dataset carries one.
