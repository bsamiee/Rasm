...
[INTEGRATION_STACK]:
- embedding-rank rail: `SemanticDiscovery.Index` batches descriptor self-descriptions through `IEmbeddingGenerator<string,Embedding<float>>` and freezes each `Embedding<float>.Vector` into a `FrozenDictionary<string,ReadOnlyMemory<float>>`.
- rank execution: `Rank` embeds the intent and scores each row through `TensorPrimitives.CosineSimilarity(query.Span, candidate.Span)`, providing a zero-copy BCL primitive over frozen memory. Tokenization bounds text before embedding, and the cosine score is metered as `ModelTokens` through the grant broker and traced on the GenAI span.
