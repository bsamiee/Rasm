...
- `expression`(`.api/expression.md`): `BackendUnavailableError` (the `getSurfaceClass`-`None` arm) and the both-form `TypeError` map at the boundary to `Result[RasterFact, RasterFault]`; the `try/except` lives only in the boundary adapter, and a glyph batch folds through `Block`-collected `Result`s.
- `structlog`/`opentelemetry`(`.api/structlog.md`): each render span binds glyph count, COLR version, backend, format, bounds, and byte length; startup observation binds the resolved backend and `fileExtension`.
- `beartype`(`.api/beartype.md`): the boundary signatures — `glyphName: str`, `palette` RGBA-tuple list, `location` dict, the `(xMin, yMin, xMax, yMax)` bounds tuple — check at the result edge so a malformed palette or non-normalized location fails before the native draw.
