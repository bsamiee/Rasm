...
- Entry: `Frames` derives the artifact identity and partitions a staged stream into 64 KiB payloads, returning the partition copy's evidence with the frames; `Fetch` validates one generated `FetchRequest`, unwraps and validates each generated `FetchResponse`, writes its frame incrementally, proves terminal extent and hash, then returns the one copy-granted `FrameCopy`; `Put` validates the shared frames, wraps each in `PutRequest`, and admits the nested `PutResponse.artifact`; `Reassemble` admits and parses an already-held protobuf frame run; `Bytes` admits and hash-verifies an already-held raw frame run, then returns its exact-size `ReadOnlyMemory<byte>` with the copy grant its caller emits through `ReceiptSurface`; `Patch` unions, normalizes, validates, and merges field masks; `WireFrames.Frame` transcribes one `FrameSeed`.
- Receipt: inbound flattening returns its allocation evidence to the caller that owns receipt emission; outbound framing wraps the caller-owned memory and allocates no second artifact body; `StreamPool` alone owns recyclable-manager events, typed `AllocationEvidence`, and subscription detachers.
- Packages: Google.Protobuf (`FieldMask.Union`/`Normalize`/`IsValid`/`Merge`, `UnsafeByteOperations.UnsafeWrap`), Grpc.Core.Api (`AsyncServerStreamingCall<T>`, `AsyncClientStreamingCall<TRequest,TResponse>`), Grpc.Net.Common (`AsyncStreamReaderExtensions.ReadAllAsync`), Microsoft.IO.RecyclableMemoryStream, CommunityToolkit.HighPerformance, Mapperly, Thinktecture.Runtime.Extensions, LanguageExt.Core, Rasm.Contracts (project — generated `ArtifactRef`, `ArtifactFrame`, `FetchRequest`, `FetchResponse`, `PutRequest`, `PutResponse`, and `ArtifactService`), Rasm (project — `Op` and allocation/call rails), BCL inbox (`IncrementalHash`, `SHA256.HashData`)
    ...
    public static IO<Fin<ArtifactRef>> Put(
        ...
        Admit(partition.Frames).Match(
            Succ: admitted => CallSpine.Awaited(async () => {
                    ...
                    PutResponse receipt = await call.ResponseAsync.ConfigureAwait(false);
                    ArtifactRef expected = admitted.Head.Artifact;
                    return ParseGuard.Validated(receipt).Bind(valid =>
                        valid.Artifact.Sha256 == expected.Sha256
                        && valid.Artifact.ArtifactBytes == expected.ArtifactBytes
                            ? Fin.Succ(valid.Artifact)
                            : Fin.Fail<ArtifactRef>(new ComputeFault.WireDecodeRejected("<artifact-put-receipt>")));
                }, token).Map(result => result.Bind(static admitted => admitted)),
