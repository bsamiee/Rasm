...
- Owner: `WorkerKind` closes the worker family; a kind's standing obligations are its `KIND_POLICY` row — fidelity latch and restart class — so a new worker kind is one member plus one row, never a sibling vocabulary. `Kernel` unifies the four crossing spellings the branch grew — a name-string gate, a by-reference module kernel, a page-local `_run_kernel`, a per-family `_dispatch` — into one frozen value: `Kernel.of` classifies the target once, and every downstream hop reads isolation, retry, deadline, enforcement, and wire off the value.
...
- Auto: `fidelity()` latches `tblib.pickling_support.install()` once per interpreter — every exception crossing a pickle seam thereafter carries its worker-side frames, so the faults lift classifies the true cause instead of a flattened marker; the latch runs in every pool initializer whose `KIND_POLICY` row obliges it and inside `shipped`, so a spawned interpreter re-latches cold. The latch runs default-off locals capture, and the explicit `Traceback` carrier stays out — the pickle rail is the fabric's whole tblib surface, receipts carrying facts, never frames.
...
- Growth: a new worker kind is one `WorkerKind` member plus one `KIND_POLICY` row; a new isolation answer is one `KernelTrait` member plus one `TRAIT_ROW` row with every call site untouched; a new shipping form is one `Shipping` member plus one `shipped` arm; a new enforcement arm is one `Enforcement` member plus one offload projection row; a new payload crossing is one `Wire` member plus one `exported` arm.
    ...
    def of[U](
        ...
        *,
        ...
        if trait is KernelTrait.SANDBOXED and not isinstance(target, bytes | bytearray):
            # the shape owns the answer both ways: a SANDBOXED declaration over a callable or name pair is a mis-declared
...
def shipped[T](kernel: Kernel[T], *args: object) -> T:
    ...
    try:
        ...
        # block detaches into owned material before the handles below close its backing buffer; nested containment stays
        # the kernel's idempotent-material contract, mirroring _spanned's bare-ndarray wire law.
...
class Device(Struct, frozen=True, gc=False):
    ...
    # the binding crosses at spawn, so a worker's device is fixed before any native runtime reads the variable.
    index: int
...
class WorkerPool:
    ...
    def __init__(self, kind: PoolKind, enforcement: Enforcement, workers: int, placement: Placement = None) -> None:
        ...
        match kind, enforcement, placement:
            ...
            case (WorkerKind.REMOTE, _, placed) if not isinstance(placed, RemoteEndpoint):
                # the symmetric admission for the fleet arm: an endpoint-less REMOTE capsule could memoize, dial
    ...
    async def _remote[T](self, carrier: dict[str, str], kernel: Kernel[T], args: tuple[object, ...]) -> RuntimeRail[T]:
        ...
        with move_on_after(kernel.deadline.default_value(float("inf"))):
            # the seal crosses the worker band INSIDE the deadline scope and its own boundary fence: an unpicklable
    ...
    async def submit[T](self, kernel: Kernel[T], *args: object) -> RuntimeRail[T]:
        ...
        async def crossing() -> T:
            ...
            if self._enforcement is Enforcement.TERMINAL:
                # the TERMINAL deadline rides pebble's schedule(timeout=) kill, its payload cloudpickle-sealed so a
...
- Growth: a new probe dimension is one `SupervisionPolicy` ceiling field plus one `_weighed` term; a new actuation is one `_actuate` arm; a new supervised subject is one `Supervisor.watch` registration.
    ...
    async def _respawn(self, charge: Charge) -> bool:
        ...
        match charge.kind:
            case WorkerKind.DAEMON:
                ...
                return True
            case kind:
                # the roll receipt is the respawn verdict: a fresh arm that never flips WARM is a failed restart, held down
