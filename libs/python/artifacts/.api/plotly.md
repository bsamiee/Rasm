...
| :-----: | :----------------------- | :------------------- | :------------------------------ |
|  [01]   | `graph_objects.Figure`   | figure model         | traces, layout, and frames      |
|  [02]   | `graph_objects.*` traces | trace types          | 2D/3D/statistical/geo traces    |
|  [03]   | `graph_objects.Layout`   | layout               | axes, legend, annotation, scene |
|  [04]   | `io`                     | serialization module | JSON/HTML/image emit            |
|  [05]   | `io.templates`           | template registry    | named figure-theme axis         |
|  [06]   | `io.renderers`           | renderer registry    | output renderer axis            |
...
| :-----: | :--------------------- | :--------------------------------------------------------------------------------- | :--------------- |
|  [01]   | `graph_objects.Figure` | `Figure(data=None, layout=None, frames=None, skip_invalid=False, **kwargs)`        | build a figure   |
|  [02]   | `Figure.add_trace`     | `add_trace(trace, row=None, col=None, secondary_y=None) -> Figure`                 | append a trace   |
|  [03]   | `Figure.update_layout` | `update_layout(dict1=None, overwrite=False, **kwargs) -> Figure`                   | configure layout |
|  [04]   | `Figure.update_traces` | `update_traces(patch=None, selector=None, row=None, col=None, **kwargs) -> Figure` | patch traces     |
...
| [INDEX] | [SURFACE]        | [CALL_SHAPE]                     | [CAPABILITY]                     |
| :-----: | :--------------- | :------------------------------- | :------------------------------- |
|  [01]   | `io.to_image`    | figure plus static export policy | static image bytes (PNG/SVG/PDF) |
|  [02]   | `io.write_image` | figure, file, and export policy  | write static image to file       |
|  [03]   | `io.to_json`     | figure plus JSON policy          | figure JSON                      |
|  [04]   | `io.from_json`   | JSON value plus output policy    | parse figure JSON                |
|  [05]   | `io.write_html`  | figure, file, and HTML policy    | interactive HTML export          |
|  [06]   | `io.to_html`     | figure plus HTML policy -> `str` | interactive HTML string          |
