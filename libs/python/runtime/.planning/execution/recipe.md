...
from rasm.runtime.faults import SCOPES, BoundaryFault, RuntimeRail, Scope, async_boundary, boundary
...
class RecipeReceipt(Struct, frozen=True):
    ...
    def contribute(self) -> Iterable[Receipt]:
        yield Receipt.of("rasm.runtime.recipe", self)
...
class RecipeExecution(Struct, frozen=True):
    ...
    async def interface(self, spec: RecipeSpec) -> "RuntimeRail[RecipeInterface]":
        ...
        # queenbee Job/JobArgument against this; execution always returns through `execute`).
        return await async_boundary("recipe.interface", lambda: _interface(spec))
    ...
    async def _prepared(self, spec: RecipeSpec) -> "RuntimeRail[_Staged]":
        ...
        return acquired.bind(lambda _n: boundary("recipe.prepare", lambda: _staged(spec)).bind(lambda staged: staged))
    ...
    async def _fetched(self, live: ResourceRoot, assets: Block[AssetFetch]) -> "RuntimeRail[int]":
        ...
        for asset in assets:  # Exemption: async sequential acquisition — each read awaits the roots rail; the carrier rebinds per step.
            ...
            acc = acc.bind(lambda n: fetched.map(lambda _a: n + 1))
        return acc
...
def _staged(spec: RecipeSpec) -> "RuntimeRail[_Staged]":
    ...
    # A handler ValueError raised by input coercion crosses the enclosing boundary fence.
    ...
    def staged() -> _Staged:
        ...
        key = ContentIdentity.of("recipe", f"{recipe.name}:{recipe.tag}:".encode() + handled.read_bytes())
...
RECIPES: Final[Map[RecipeName, RecipeRow]] = Map.of_seq([
    (RecipeName.ANNUAL_DAYLIGHT, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.ANNUAL_DAYLIGHT_ENHANCED, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.ANNUAL_IRRADIANCE, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.CUMULATIVE_RADIATION, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.DIRECT_SUN_HOURS, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.DAYLIGHT_FACTOR, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    ...
    (RecipeName.IMAGELESS_ANNUAL_GLARE, RecipeRow(outputs=("results",), engines=frozenset({Engine.RADIANCE}), workers=2)),
    (RecipeName.ANNUAL_ENERGY_USE, RecipeRow(outputs=("eui",), engines=frozenset({Engine.OPENSTUDIO, Engine.ENERGYPLUS}), workers=2)),
    (RecipeName.ADAPTIVE_COMFORT_MAP, RecipeRow(outputs=("results",), engines=_COMFORT_ENGINES, workers=2)),
    (RecipeName.PMV_COMFORT_MAP, RecipeRow(outputs=("results",), engines=_COMFORT_ENGINES, workers=2)),
    (RecipeName.UTCI_COMFORT_MAP, RecipeRow(outputs=("results",), engines=_COMFORT_ENGINES, workers=2)),
])
```
