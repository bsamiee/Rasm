...
[ENTRYPOINT_SCOPE]: object enumerator settings — the complete twenty-one-axis filter product

Every member below is public and settable; the constructor takes no argument and seats the defaults noted.

| [INDEX] | [SURFACE]               | [SHAPE]  | [CAPABILITY]                                                        |
| :-----: | :---------------------- | :------- | :------------------------------------------------------------------ |
|  [01]   | `NormalObjects`         | property | unlocked, unhidden rows; default true                               |
|  [02]   | `LockedObjects`         | property | locked rows or rows on locked layers; default true                  |
|  [03]   | `HiddenObjects`         | property | hidden rows or rows on hidden layers; default false                 |
|  [04]   | `IdefObjects`           | property | instance-definition members, not the references; default false      |
|  [05]   | `DeletedObjects`        | property | deleted rows; default false                                         |
|  [06]   | `SubObjectSelected`     | property | rows with only a component selected; default false                  |
|  [07]   | `ActiveObjects`         | property | rows saved in the file; default true                                |
|  [08]   | `ReferenceObjects`      | property | reference rows not saved in the file; default false                 |
|  [09]   | `IncludeLights`         | property | render-light rows; default false                                    |
|  [10]   | `IncludeGrips`          | property | grip rows; default false                                            |
|  [11]   | `IncludePhantoms`       | property | phantom rows; default false                                         |
|  [12]   | `SelectedObjectsFilter` | property | restrict to the selection; default false                            |
|  [13]   | `UseFastSelection`      | property | walk only the selection set; ignored unless `SelectedObjectsFilter` |
|  [14]   | `VisibleFilter`         | property | restrict to visible rows; default false                             |
|  [15]   | `ObjectTypeFilter`      | property | `ObjectType` flag mask; default every type                          |
|  [16]   | `ClassTypeFilter`       | property | managed `Type` filter; default none                                 |
|  [17]   | `LayerIndexFilter`      | property | one layer index; `-1` means every layer                             |
|  [18]   | `MaterialIndexFilter`   | property | one material index; `int.MinValue + 1` means every material         |
|  [19]   | `NameFilter`            | property | one object name; reads back `"*"` when unset, meaning every name    |
|  [20]   | `ViewportFilter`        | property | one live `RhinoViewport`; default none                              |
|  [21]   | `SpaceFilter`           | property | one `ActiveSpace`; default `None`                                   |

- Three axes carry SENTINELS rather than absence: `LayerIndexFilter` at `-1`, `MaterialIndexFilter` at `int.MinValue + 1`, and `NameFilter` normalizing `null`/empty/`"*"` to a stored `null` that reads back as `"*"`. A composer stating "every row" writes the sentinel explicitly instead of relying on a default it never named.
- `UseFastSelection` is `since 9.0` and the host's own remark bars it while the selection is actively changing; it is silently ignored unless `SelectedObjectsFilter` is also set, so the dependency is an admission fact, not a runtime hope.
- The type is a MUTABLE class with no copy member, so an instance handed across a boundary stays reachable by its author; a value crossing a signature builds a fresh instance at the call rather than carrying one.
...
- `Layer.HasSelectedObjects(bool checkSubObjects) -> bool` / `CommitChanges() -> bool` / `Default() -> void`; `Layer` re-declares the user-string roster `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Transform` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt`(`libs/dotnet/.api/api-languageext.md`): the out-`bool` and out-parameter table lookups fold to `Fin<A>`/`Option<A>`, and a multi-object mutation batch accumulates failures through `Validation<Error, A>` where a single edit short-circuits on `Fin`
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-runtime-extensions.md`): `UnitSystem`, `ObjectType`, and `ModelComponentType` wrap as keyed `SmartEnum` policy owners, and `RuntimeSerialNumber` wraps as a `ValueObject` document key so the host `uint` never leaks
- `Hashing`(`libs/dotnet/.api/api-hashing.md`): document and table content keys derive from `XxHash128` for the persistence artifact index
