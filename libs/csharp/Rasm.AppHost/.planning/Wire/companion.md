...
- [GENERATED_MEMBER_RAIL]-[BLOCKED]: do the generated `ControlService.ControlServiceBase`/`ControlServiceClient` members and their request and reply message spellings resolve on the member rail rather than only through the spec-compile gate; route: `uv run python -m tools.assay api query --key Grpc.Core.Api`, blocked until the assay source map registers that transitive package.
- [LAUNCHD_MARSHALLING]-[BLOCKED]: does the `launch_activate_socket` `int**`/`nuint*` marshalling fill a descriptor the `SafeSocketHandle(int, bool)` adoption serves, and does `free(fds)` release the whole malloc'd array; route: register a launchd service socket, run `LaunchdActivation.Inherit`, and read the accepted connection beside `lsof` on the inherited fd — blocked in a headless session, which has no registered launchd socket.
