...
|   [6]   | TransactionRequest  | idempotency_key=1 bytes; ops=2 repeated Any; expected_epoch=3 uint64; hlc_physical=4 google.protobuf.Timestamp; hlc_logical=5 uint64; correlation=6 string | flagship: the in-process `DocumentTransaction` verb set field-for-field, ops as polymorphic Any envelopes |
|   [7]   | TransactionReceipt  | idempotency_key=1 bytes; committed=2 bool; new_epoch=3 uint64; applied=4 repeated string; conflict=5 FaultDetail; hlc_physical=6 google.protobuf.Timestamp; hlc_logical=7 uint64 | flagship: mirrors the `DocumentTransaction` typed receipt field-for-field; the `conflict` slot carries the FaultDetail the retry owner decodes through `WireDocument.Conflict` in band |
|   [8]   | QueryRequest        | scope=1 string; predicate=2 Struct; cursor=3 string; mask=4 google.protobuf.FieldMask                                        | read verb carrying the field-mask projection               |
|   [9]   | QueryResponse       | rows=1 repeated Struct; cursor=2 string; total=3 int64                                                                       | masked read result; the mask names the projected columns   |
...
- Owner: `RemoteTransport` `[SmartEnum<string>]` four rows with streaming, credential, affinity, and dial columns; `WireKeyPolicy` ordinal comparer accessor; `StreamShape` and `NodeSelection` row vocabularies; `WireTransition` `[Union]` the typed prior→next connectivity-transition family the receipt carries; `ComputeEndpoint` endpoint identity record; `WireChannels` — attach, open, warm-via-`ConnectAsync`, observe-via-connectivity-fold, redial; the in-process row consumes the `TestServer.CreateHandler` handler seam.
...
- [TRANSPORTS]: FINALIZED — the in-process row's `TestServer.CreateHandler` handler seam is decompile-verified (`Microsoft.AspNetCore.TestHost` `TestServer.CreateHandler()` returns the `ClientHandler` `HttpMessageHandler` dispatched into `GrpcChannel.ForAddress(endpoint.Address, new GrpcChannelOptions { HttpHandler = handler() })`), and `WireChannels.InMemory` carries the test-only handler factory from the spec composition root; `GrpcChannel.ConnectAsync(CancellationToken = default)`, `State`, and `WaitForStateChangedAsync(ConnectivityState, CancellationToken = default)` are decompile-verified on the net10.0 asset and warm/observe the channel with no live host; the only residual is the live-host UDS attach scenario, named on [TRANSPORTS_LIVE].
- [TRANSPORTS_LIVE]: the Kestrel `ListenUnixSocket` server leg inside the live Rhino-plugin app root and the `Grpc.Core.Api` transitive route proof. [SPIKE] genuine tier-3 LIVE-HOST: the UDS server bind and the in-host channel dial cannot be settled from decompile — only the in-process `TestServer` handler is reachable on this host, and the live UDS attach choreography closes against the running integrated host (charter `bridge` gate, ROADMAP START_GATES [3]-[4]).
...
- [COMPOSED_CREDENTIAL]: the `CredentialPolicy.Composed`/`Bearer`/`Mint` arms, the `CompressionProviders` axis, the full five-arity `CallSpine` interceptor surface plus the `Awaited` fault-capture edge, the per-call `WithIdentity`/`Compressed` edges, the `DeadlineClass.HopTotal` budget threading through `WithDeadline`/`WithCancellationToken`/`WithCredentials`, the `WireFault` total seventeen-code `StatusCode` `FrozenDictionary` rail plus the in-band `Decode(FaultDetail)` arm, the `ContractGuard` canonical projection-checksum, rpc/oneof/enum/reserved/packed/nested surface fold, the `WireTransition` typed connectivity-transition family, the `FrameEdge.Patch`/`Evidence` mask-apply and pool-event legs, and the flagship `WireDocument`/`FrameEdge.Transaction` parity choreography with the `Conflict` in-band decode are FINALIZED against the catalogued `CallCredentials.Compose(params CallCredentials[])`/`FromInterceptor`, `AsyncAuthInterceptor`/`AuthInterceptorContext`, inbox `GzipCompressionProvider`/`DeflateCompressionProvider` (Deflate wraps `ZLibStream`), the five interceptor overrides, `CallOptions.WithCredentials`, `GrpcChannelOptions.InitialReconnectBackoff`/`MaxReconnectBackoff`/`DisableResolverServiceConfig`, `GrpcChannel.ConnectAsync`/`State`/`WaitForStateChangedAsync`, `ConnectivityState`, `MethodDescriptor.InputType`/`OutputType`/`IsClientStreaming`/`IsServerStreaming`, `MessageDescriptor.NestedTypes`/`EnumTypes`/`Oneofs`/`ToProto`/`FindFieldByNumber`, `FieldDescriptor.IsPacked`/`ContainingOneof`/`JsonName`, `DescriptorProto.ReservedRange` (`Start`/`End`), `EnumDescriptor.Values`/`EnumValueDescriptor.Number`, `FieldMask.FromFieldNumbers<T>`/`FromString<T>`/`IsValid`/`Normalize`/`Union`/`Merge`/`MergeOptions`, the `RecyclableMemoryStreamManager` `StreamDisposed`/`StreamOverCapacity`/`StreamConvertedToArray`/`BufferDiscarded`/`UsageReport` events, and `Any.Pack`/`TryUnpack`/`GetTypeName`/`Is` decompile spellings; the live-ALC dial of the composed identity through a running plugin channel — and the fingerprint-matched `BenchmarkClaim` that gates the compression flip on the live host — are the only residual probes. [SPIKE] both remain genuine tier-3 LIVE-HOST probes: the composed-identity ALC dial and the benchmark-gated compression flip cannot be settled from decompile alone.
