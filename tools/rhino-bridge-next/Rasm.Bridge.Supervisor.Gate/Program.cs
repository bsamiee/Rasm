...
internal static partial class Program {
    ...
    internal static int Main(string[] args) {
        ...
        JsonArray table = new(Rows.Select(static row => (JsonNode)row).ToArray());
    ...
    private static void Signal(int pid, string signal) {
        _ = Exec.Run(file: "/bin/kill", args: [signal, pid.ToString(CultureInfo.InvariantCulture)], deadline: TimeSpan.FromSeconds(5));
    }
    ...
    private static void RowKillMidConnect(SessionPolicy policy, string scratch) {
        ...
        Report("kill9-mid-connect", pass, new JsonObject {
            ...
            ["onHostExited"] = (afterExit as SessionState.Faulted)?.Fault is BridgeFault.LaunchFailed lf ? $"launch-failed: {lf.Detail}" : afterExit.GetType().Name,
    ...
    private static void RowQuitLadderEscalation(SessionPolicy policy, string scratch) {
        ...
        string[] rungs = published.OfType<BridgeEvent.PhaseCase>().Select(static phase => $"{phase.Phase.Key}:{phase.Status.Key}").ToArray();
    ...
    private static void RowReconcileInstanceScoped(SessionPolicy policy, string scratch) {
        ...
        try {
            ...
            QuitJournal.Append(journal, new QuitJournalEntry(Pid: 1, StartedAtUnixMs: mark - 60_000, RetiredAtUnixMs: mark + 60_000, Rung: "quit.kill", PipeName: "rbx-gate"));
    ...
    private static void Proof7McpSuppression() {
        ...
        string[] installed = Directory.Exists(packages) ? Directory.GetDirectories(packages).Select(static path => Path.GetFileName(path) ?? string.Empty).ToArray() : [];
    ...
    private static void LiveCycleKillMidConnect(BundleInfo bundle) {
        ...
        QuitJournal.Append(QuitJournal.CanonicalPath, new QuitJournalEntry(
        ...
        Report("live-kill9-mid-connect", pass, new JsonObject {
            ...
            ["discriminated"] = (afterExit as SessionState.Faulted)?.Fault is BridgeFault.LaunchFailed lf ? lf.Detail : afterExit.GetType().Name,
