...
One cluster: `[2]-[NESTING]` owns the `Stock` union, the `NestPolicy`/`NoFitPolygon` records, and the `Nest` fold — no-fit-polygon feasibility (Minkowski via Geometry2D) with the bottom-left and genetic placement modes over the one NFP primitive and the injected placement-score delegate slot.
