...
- Law: `MarkFault` is the plane-wide fault vocabulary. `unscannable` identifies a carrier-less generated symbol; structured cases retain offending symbology, option locations, and source-shape evidence. No scan-only refusal exists, because every symbology this taxonomy carries has a generator — MEASURED, `Code 93`, `UPC-E`, `DataBar`, `DataBar Expanded`, `DataBar Limited`, and `DX Film Edge` all appear in `barcode_formats_list(BarcodeFormat.AllCreatable)` and write live through `write_barcode_to_svg`, so the six rows calling themselves uncreatable refused six formats the admitted writer produces.
...
class MatrixMake(TypedDict, closed=True):
    ec_level: NotRequired[ReadOnly[str]]
...
class QrSequencePayload(TypedDict, closed=True):
    make: Required[ReadOnly[QrSequenceMake]]
    render: NotRequired[ReadOnly[QrRender]]
...
TAXONOMY: Final[frozendict[Symbology, tuple[MarkClass, str | None]]] = frozendict({
    ...
    Symbology.EAN14: (MarkClass.LINEAR, None),
