...
| :-----: | :------- | :------------- | :----------- |
| [1] | `Environment` | engine root | shared compilation/render configuration, template cache, loader, autoescape, extensions |
| [2] | `Template` | compiled unit | a single compiled template with sync/async render entrypoints |
| [3] | `sandbox.SandboxedEnvironment` | restricted engine | attribute/operator-filtered environment for untrusted templates |
| [4] | `sandbox.ImmutableSandboxedEnvironment` | hardened engine | sandbox that also blocks mutation of exposed objects |
| [5] | `sandbox.SecurityError` | sandbox fault | raised when a sandboxed template touches a forbidden attribute |
...
| :-----: | :-------- | :----------- | :----------- |
| [1] | `Environment` | `Environment(block_start_string='{%', block_end_string='%}', variable_start_string='{{', variable_end_string='}}', comment_start_string='{#', comment_end_string='#}', line_statement_prefix=None, line_comment_prefix=None, trim_blocks=False, lstrip_blocks=False, newline_sequence='\n', keep_trailing_newline=False, extensions=(), optimized=True, undefined=Undefined, finalize=None, autoescape=False, loader=None, cache_size=400, auto_reload=True, bytecode_cache=None, enable_async=False)` | full engine configuration |
| [2] | `Environment.get_template` | `get_template(name, parent=None, globals=None) -> Template` | resolve and compile a named template via the loader |
| [3] | `Environment.select_template` | `select_template(names, parent=None, globals=None) -> Template` | resolve the first available of a name list |
| [4] | `Environment.get_or_select_template` | `get_or_select_template(template_name_or_list, parent=None, globals=None) -> Template` | polymorphic resolve over a name or name list |
| [5] | `Environment.from_string` | `from_string(source, globals=None, template_class=None) -> Template` | compile an in-memory source string |
| [6] | `Environment.compile_expression` | `compile_expression(source, undefined_to_none=True) -> TemplateExpression` | compile a single expression to a callable |
| [7] | `Environment.overlay` | `overlay(**config) -> te.Self` | derive a reconfigured child environment |
| [8] | `Environment.add_extension` | `add_extension(extension) -> None` | register an extension after construction |
...
| [INDEX] | [SURFACE] | [CALL_SHAPE] | [CAPABILITY] |
| :-----: | :-------- | :----------- | :----------- |
| [1] | `Template.render` | `render(*args, **kwargs) -> str` | render to a full string |
| [2] | `Template.render_async` | `render_async(*args, **kwargs) -> str` | async render; requires `enable_async=True` |
| [3] | `Template.generate` | `generate(*args, **kwargs) -> Iterator[str]` | lazy chunked render |
| [4] | `Template.stream` | `stream(*args, **kwargs) -> TemplateStream` | streamed render with `dump`/`disable_buffering` |
| [5] | `Template.make_module` | `make_module(vars=None, shared=False, locals=None) -> TemplateModule` | materialize exported names/blocks |
| [6] | `Template` | `Template(source, ...same knobs as Environment...) -> Template` | construct a standalone template with an implicit environment |
...
| [INDEX] | [SURFACE] | [CALL_SHAPE] | [CAPABILITY] |
| :-----: | :-------- | :----------- | :----------- |
| [1] | `select_autoescape` | `select_autoescape(enabled_extensions=('html','htm','xml'), disabled_extensions=(), default_for_string=True, default=False) -> Callable[[str | None], bool]` | extension-driven autoescape selector |
| [2] | `make_logging_undefined` | `make_logging_undefined(logger=None, base=Undefined) -> Type[Undefined]` | undefined subclass that logs every access |
| [3] | `is_undefined` | `is_undefined(obj) -> bool` | undefined-instance predicate |
| [4] | `clear_caches` | `clear_caches() -> None` | flush global template/lexer caches |
| [5] | `pass_context` | `pass_context(f) -> F` | mark a filter/test to receive the render `Context` |
| [6] | `pass_environment` | `pass_environment(f) -> F` | mark a callable to receive the `Environment` |
| [7] | `pass_eval_context` | `pass_eval_context(f) -> F` | mark a callable to receive the `EvalContext` |
