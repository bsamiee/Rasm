...
public readonly partial struct ProjectedCrs {
 ...
 // :6.18.3:NNNN — the code is the last-colon tail in both), and a bare authority code. The colon-tail arm gates on
 // the EPSG authority token, so an ESRI:NNNN or foreign-authority URN never mis-parses as an EPSG identity — it
 // resolves through the Wkt/projection path. A WKT-only or projection-only CRS is None; Resolution then reads Wkt.
 public Option<int> Epsg =>
  ...
  ? Some(prefixed)
  : Name.Contains("EPSG", StringComparison.OrdinalIgnoreCase) && Name.LastIndexOf(':') is var tail and >= 0 && int.TryParse(Name.AsSpan(tail + 1), out int urnCode)
  ? Some(urnCode)
...
public sealed record GeoReference(
 ...
 // the per-axis scale, so the doubles arrive in metres. FOUR independent legs ACCUMULATE (VALIDATION_MONOID): the
 ...
 (Gate(double.IsFinite(eastings) && double.IsFinite(northings) && double.IsFinite(orthogonalHeight), key,
  ...
  Gate(double.IsFinite(abscissa) && double.IsFinite(ordinate) && (abscissa != 0.0 || ordinate != 0.0), key,
   $"<map-conversion-direction-degenerate:{abscissa:R}:{ordinate:R}>"),
  ...
  AdmitCrs(projectedCrsName, wkt, mapProjection, mapZone, key).ToValidation())
 .Apply((_, _, _, _, crs) => new GeoReference(
 ...
 // implicit Validation lift.
 static Validation<Error, Unit> Gate(bool holds, Op key, string detail) =>
  holds ? unit : ElementFault.ValueRejected(key, detail);

 // The dependent CRS leg: no-identity -> None (valid), else structural-then-semantic on the Fin rail, lifted into the
 // accumulating tuple by ToValidation so a bad CRS reports BESIDE a bad conversion tuple, never instead of it.
 static Fin<Option<ProjectedCrs>> AdmitCrs(string name, string wkt, string mapProjection, string mapZone, Op key) =>
