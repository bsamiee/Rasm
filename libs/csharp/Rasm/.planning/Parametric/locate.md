...
- Growth: a new value (an `Osculating` circle at a parameter, a `Torsion` scalar) is one `LocationValue` case with its arms and columns; a new address (a `NormalizedParameter(double)` locator) is one `Locator` case plus its `ResolveParameter` arm — the curve family rides `Resolve`'s default route, so the fold stays untouched; a non-curve address adds its own `Resolve` arm; a new aggregation (an `Inflections` zero-crossing report) is one `CurvatureAggregation` case read by the sweep; a new aspect is one `Location` case plus one `Switch` arm. Zero new entrypoints, zero new runtimes.
