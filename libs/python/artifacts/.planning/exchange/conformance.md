...
class Conformance:
    ...
    def _signed(self, pdf: bytes, spec: SignSpec, /) -> bytes:
        ...
        # the `PdfSigner` engine is the deepest signing surface, driving the invisible byte-range signature
        # (`stamp_style=None`) and the visible drawing-sheet seal (`stamp_style=plan.stamp`) on ONE path,
        # replacing the bare `sign_pdf` convenience that cannot carry an appearance.
        engine = PdfSigner(meta, spec.signer.cms(), timestamper=timestamper, stamp_style=plan.stamp, new_field_spec=field)
        return self._sink(lambda sink: engine.sign_pdf(IncrementalPdfFileWriter(BytesIO(pdf)), appearance_text_params=dict(plan.text_params) or None, output=sink))
