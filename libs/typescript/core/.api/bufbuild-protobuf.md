...
- Compatibility is the corpus gate's — `buf breaking` FILE refuses a breaking source before regeneration — so no page diffs descriptors at runtime; `readUnknownFields`/`ignoreUnknownFields` preserve forward residue under the two spelled postures and a consumer refuses only a divergent `dataschema` generation.
...
- `interchange/codec`: each wire-family row selects one codec and names its framing (`binary` | `json`); every `rasm.contracts.host.v1` family rides ProtoJSON through `fromJson`/`toJson` over the generated descriptor.
...
- `@bufbuild/protoc-gen-es`(same `catalog`): emits the committed `_pb.ts` `GenMessage`/`GenService` consts of `libs/typescript/contracts` (`.api/rasm-ts-contracts.md`) — one module per SOURCE at the module-relative path, which `interchange/format` and `interchange/invoke` import by module path with no barrel between; `[02]-[BRANCH_OPTIONS]` seats the option set. `interchange/format`'s `Proto.registry` admits estate families alone; the vendored CloudEvents source generates unchanged on its own module and `Format.event`'s protobuf row takes `CloudEventSchema` through its `Core` seat.
