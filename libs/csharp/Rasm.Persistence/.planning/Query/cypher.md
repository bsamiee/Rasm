...
public static class GraphSession {
    ...
    // Edges-SQL vertex id, the GiST/BRIN H3 index, and the in-process pocketken.H3 cell agree bit-for-bit (api-pgrouting#IMPLEMENTATION_LAW).
...
// LoadVertices/LoadEdges bulk cases serve one-time backfill only, never a second steady-state producer.
public sealed class AgeGraphProjection(NpgsqlDataSource source, Identifier graph) : IProjection {
    public async Task ApplyAsync(IDocumentOperations operations, IReadOnlyList<StreamAction> streams, CancellationToken token) {
        ...
        foreach (GraphDelta delta in streams.SelectMany(static stream => stream.Events).Select(static e => e.Data).OfType<GraphDelta>()) {
            foreach (Node node in delta.Added) { await Cypher(lane, $"MERGE (n:Element {{id: $id}}) SET n.role = $role", ("id", node.Id.Value), ("role", node.GetType().Name), token).ConfigureAwait(false); }
            foreach (NodeId removed in delta.Removed) { await Cypher(lane, "MATCH (n:Element {id: $id}) DETACH DELETE n", ("id", removed.Value), ("role", ""), token).ConfigureAwait(false); }
            foreach (Relationship edge in delta.Linked) {
                await Cypher(lane, "MATCH (a:Element {id: $from}), (b:Element {id: $to}) MERGE (a)-[:REL {kind: $kind}]->(b)", ("from", edge.Relating.Value), ("to", edge.Related.Value), token, ("kind", edge.Kind.Key)).ConfigureAwait(false);
            }
        }
    }
