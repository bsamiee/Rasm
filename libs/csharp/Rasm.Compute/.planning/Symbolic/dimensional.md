...
- [02]-[DIMENSION_MONOMIAL]: `SiAxis` rows carry the glyph and the `BaseDimensions` reader; `DimensionMonomial` carries seven SI base-dimension exponents under a generic-math rational group and projects to the seam `Dimension`.
- [03]-[DIMENSION_PROOF]: `DimensionContext` admits the intent-declared symbol bindings; `DimensionProof` folds every `Entity` node onto one accumulating `Validation` rail.
