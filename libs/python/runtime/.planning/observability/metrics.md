...
class SyncInstruments(Struct, frozen=True):
    ...
    cost_cpu: Histogram
    cost_memory: Histogram
    cost_io: Histogram
    cost_switches: Histogram
...
INSTRUMENTS: Final[Block[InstrumentSpec]] = Block.of_seq([
    ...
    InstrumentSpec("rasm.cost.cpu_time", InstrumentKind.HISTOGRAM, "ms", slot="cost_cpu", domain="cost"),
    InstrumentSpec("rasm.cost.memory_delta", InstrumentKind.HISTOGRAM, "By", slot="cost_memory", domain="cost"),
    InstrumentSpec("rasm.cost.io_bytes", InstrumentKind.HISTOGRAM, "By", slot="cost_io", domain="cost"),
    InstrumentSpec("rasm.cost.ctx_switches", InstrumentKind.HISTOGRAM, "{switch}", slot="cost_switches", domain="cost"),
...
# one attribute fold every recording path shares: the rasm.tenant baggage entry joins under its own canonical key when the
# parented context carries it — the metric attribute spells TENANT_BAGGAGE exactly, so the TS Convention tenant views and the
# tenant board queries match Python series without relabeling.
def _attributed(base: Mapping[str, object], context: "otel_context.Context") -> dict[str, object]:
    ...
    return {**base, TENANT_BAGGAGE: tenant} if isinstance(tenant, str) and tenant else dict(base)
...
class Metrics:
    ...
    def record(
        ...
        match measure:
            case Mapping() as measures:
                ...
                with cls._gate:
                    instruments = cls._sync
                Block.of_seq(measures.items()).fold(
                    lambda _, kv: getattr(instruments, _DOMAIN_SLOT[kv[0]]).record(kv[1], attributes, context=context), None
                )
            case amount:
                context = otel_context.get_current()
                with cls._gate:
                    duration = cls._sync.duration
                duration.record(amount, _attributed({"rpc.method": method, "outcome": outcome}, context), context=context)
    ...
    def retry_hook(cls) -> RetryHook:
        def hook(details: RetryDetails) -> None:
            ...
            with cls._gate:
                retries = cls._sync.retries
            retries.add(1, attributes, context=context)
...
- Entry: `install(scope=)` latches per composition over the one `latched`-guarded train activation and takes no profile argument — the gate is the installed provider, the same law the instrument fold holds — so a PACKAGE/TEST process patches against the no-op providers at zero export cost, a later composition's receipt truthfully records zero newly activated rows, and activation happens once at the composition root, never at library altitude. DBAPI wrapping likewise activates at the composition root alone: the data-side consumer hands its own admitted driver module in (duckdb, ADBC DBAPI), so this folder imports and patches nothing it does not admit.
