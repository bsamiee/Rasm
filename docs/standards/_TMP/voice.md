...
*Early (0-4):* Period segments input—necessary for structure, insufficient for retrieval.
...
**Use When:** Comparing > 2 entities on > 2 dimensions.
**Format:** Align columns per formatting.md §4.3. Keep cell content concise.

---
### [3.2][DIAGRAMS]

**Use When:** Describing flows > 3 steps or hierarchies > 2 levels.
**Optimization:** Use `graph TD` for sequential flow. Use component, container, sequence, state, or ER diagrams for architecture.
...
**Accuracy Tradeoff:**
*Incorrect comments:* 78% accuracy loss—worse than missing comments.
...
Doc comment structure applies across languages. Language-specific format (XML, Google, TSDoc) defined in code-documentation.md §3.
...
[REFERENCE] Language-specific formats: [→code-documentation.md§3](code-documentation.md#3structure)
...
[IMPORTANT] Max 2-3 keywords per section. Diminishing returns after 5.
