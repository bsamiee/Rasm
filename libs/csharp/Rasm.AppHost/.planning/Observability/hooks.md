...
Settled composition: `HookId`, `HookModality`, `HookPoint<TFact>`, `HookRegistry`, `IsolatedFault`, and the synchronous-fire law arrive from the kernel signal capsule — one capsule type for the branch, this page an instance, never a re-declaration; `EventBus.Dispatch`, `Topic`, and `DomainEvent` from Wire/topics#BUS_CONDUCTOR; `PhaseReceipt` and `FaultBand` from Runtime/lifecycle; `ReceiptSinkPort` and `ReceiptEnvelope` from Runtime/ports#PORT_RECORDS. Hook ids share the instrumentation-scope discipline — package-qualified, registry-enforced unique — so collision and shadowing die structurally at composition.
