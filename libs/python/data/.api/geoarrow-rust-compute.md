...
- installed: `0.6.3` reflected on Python 3.13 from the bundled stubs (`_compute.pyi`, `enums.py`, `types.py`) and `importlib.metadata` (companion gated `<3.15` in the manifest)
- license/floor: dual MIT / Apache-2.0 (geoarrow-rs project); `requires-python >=3.9`; native PyO3 wheel (per-CPython ABI, e.g. `_compute.cpython-313-darwin.so`, NOT abi3 — the wheel is interpreter-version-specific); runtime deps `arro3-core>=0.4` (the lightweight Arrow runtime carrying `Array`/`ChunkedArray`/`Table`), `geoarrow-rust-core` (the `GeoArray`/`GeoChunkedArray`/`Geometry` carriers), and `pyproj` (CRS handling)
...
- algorithm axis: the GeoRust kernels (QuickHull hull, RDP/VW/VW-preserve simplification, Chaikin smoothing, polylabel pole-of-inaccessibility, discrete Fréchet distance) are owned by the wheel; compose them — never re-derive a Python geometry algorithm the native surface already exposes.
...
- boundary: `geoarrow-rust-compute` owns native geometry compute over Arrow capsules and emits `arro3.core` `Array`/`ChunkedArray`/`Table` for scalar/table results; geometry-array construction and type metadata route to `geoarrow.rust.core`; file IO (GeoParquet/FlatGeobuf/Arrow IPC) routes to `geoarrow.rust.io`; predicate/overlay operations the wheel does not expose route to the GEOS-backed owner, not a local reimplementation.
- stacking: the compute output is an Arrow-capsule carrier the rest of the data rail reads zero-copy — an `arro3` `Table` from `explode` or a `GeoArray` from a transform flows through the PyCapsule interface into a `duckdb`/`ducklake` ingestion, a polars frame, or a `geoarrow.rust.io` GeoParquet write; the per-CPython-ABI native wheel binds the same interpreter the rest of the companion band runs, so it is loaded at the boundary, never re-exported as a thin wrapper.
