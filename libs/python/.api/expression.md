...
[PUBLIC_TYPE_SCOPE]: typeclass / capability protocols — structural bounds satisfied to compose with `pipe`, `sort`, `sum`.
...
[PUBLIC_TYPE_SCOPE]: persistent collections (`expression.collections`) — immutable and structurally shared; every update returns a new structure.
...
- `extra.result.traverse` / `sequence` short-circuit on the first `Error`; `sequence` is `traverse` with the identity function, and `traverse` carries `(T -> Result[R, E]) × Block[T] -> Result[Block[R], E]`.

[ENTRYPOINT_SCOPE]: persistent collection operations — module-level callables (`block.map`, `seq.filter`, `map.add`) compose under `pipe`; methods mirror them.
...
- `msgspec`/`pydantic`(`.api/msgspec.md`, `.api/pydantic.md`): a decode is the `Result`-minting boundary — wrap `msgspec.json.decode(buf, type=T)` or a pydantic `model_validate` in `effect.try_` so the `ValidationError` becomes `Result.Error`, and a `@tagged_union` discriminant maps onto a `msgspec.Struct`-tagged union or a pydantic discriminated union at the wire edge.
- `anyio`(`.api/anyio.md`): an `anyio.to_thread.run_sync`/`run_process` offload return lifts into `Result`; a `fail_after` `TimeoutError` or a `BrokenWorkerProcess` maps to a typed `Error` at the boundary, never escaping raw.
- `structlog`(`.api/structlog.md`): fold a `Result.Error` payload into one structured event at egress (`log.error(event, error=err)`); the `Ok` path stays silent.
- owning folders: compose `pipe`/`compose` into point-free pipelines over the curried collection/monad callables, `effect.*` builders for deep bind chains, and `extra.result.traverse`/`pipeline` for short-circuiting ROP sequencing over a `Block`.
