...
Smell test: `await parallel(...)`, then a plain transform (`flat`/`map`/`filter`) with no cross-item dependency, then another `parallel(...)` — that middle transform does not need the barrier; make it a pipeline stage. Second smell, the fan-after-fan: two independent fans sequenced by an accidental `await` — both derive from the SAME upstream product and neither reads the other's results — serialize for nothing, holding the second fan's every lane hostage to the first fan's slowest; spread both into ONE pool (`Promise.all([...fanA, ...fanB])`). Third smell, the group-gated terminal: a per-GROUP terminal stage (a unit's seam check, a folder's align pass) gated on the WHOLE fan when its inputs are only its own group's outputs — chain per group (`Promise.all` over groups, each awaiting its own members then its own terminal, agents slotted per [02]) so every group's terminal fires the moment ITS inputs land. A barrier between fans is earned only by a real data edge from one fan's results into the other's inputs; a workflow never holds a lane whose inputs are ready. When in doubt, `pipeline`.
...
- A dead CRITICAL lane (`agent()` returned null) earns bounded re-dispatch on a PROGRESSIVE backoff ladder — the null is causeless, so the ladder covers both death classes: a short first backoff (~a minute) catches a transient transport death, a window-length second (~tens of minutes) waits out a usage-limit reset; a single uniform backoff class spends half an hour recovering a death the first minute recovers. The final death isolates the lane, NEVER the chain — every downstream stage still runs against current disk, consuming what exists and recording what does not. An unbounded retry loop and a chain that dies with its weakest lane are the two failure shapes this replaces.
...
- `stallMs` observes BETWEEN-call progress only — time inside a live call is invisible to it, so raising it for a long in-call build is a no-op; it is a stall override for slow between-call work, never a wait license and never hang protection for a blocking external call (that is the wall-clock watchdog, external-lanes reference).
