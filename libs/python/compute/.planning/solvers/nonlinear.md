...
# R->R scalar root — so a cp315 floor run for a 1-D bracketing solve returns a meaningful residual at the
...
- [JAX_PYTREE]: `x0` is an arbitrary JAX pytree the Optimistix solver tracks through iteration, so the body floats the rail to float64 with `jax.config.update("jax_enable_x64", True)`, lifts and reads back per-leaf with `jax.tree_util.tree_map(jnp.asarray, ...)`, and contracts residuals with the pytree-total `NormKind`-selected `optimistix` norm (`max_norm`/`rms_norm`/`two_norm` are `(PyTree[Array]) → Shaped[Array, '']`) rather than `numpy.asarray`, which flattens a pytree and breaks a non-array leaf; the fixed-point residual subtracts `fn(v) - v` leaf-wise through `jax.tree_util.tree_map` first because `-` on two pytrees is undefined. The least-squares `rank` slot sums the leaf element counts over `jax.tree_util.tree_leaves` (`_tree_rank`) so a structured converged value reports its true free-parameter count where a flat `value.size` assumes one array leaf. `Solution.result` is a `RESULTS` `EnumerationItem` carrying ONLY a `_value` int code (no `.name`, and `RESULTS[item]` yields the human message), so `NonlinearEngine.verdict` recovers the member-name string by inverting `RESULTS._name_to_item` off `int(result._value)` and the `solvers/receipt.md#RECEIPT` `_status` fold maps that name into `SolveStatus`; the gated route passes the string, never the imported `RESULTS` type, keeping the receipt owner cp315-clean. Under a `filter_vmap` sweep `solutions.result` is a batched `RESULTS` the `verdict` reduces to the single worst-case termination member by `jnp.max` over the per-row `_value` codes (the zero `successful` making `max == 0` iff every start converged) plus the name inversion — NEVER `RESULTS.promote`, which is inheritance-widening that raises on a same-class member rather than a vmap combine, exactly as `optimization/design.md#DESIGN` resolves its multi-start ensemble — so the batched receipt records its true aggregate verdict rather than a `result=None` residual-floor fiction, the batched `_tree_rank` dividing the stacked leaf count by the leading sweep width to recover the per-row rank; `has_aux` rides as a pytree leaf through both paths.
