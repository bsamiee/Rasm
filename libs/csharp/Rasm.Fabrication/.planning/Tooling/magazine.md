...
public sealed record SlotMap(Seq<(int Slot, ToolAssembly Assembly)> Slots) {
    // The tool-less floor: Schedule over Empty yields zero changes and the prologue degrades to absence.
    public static readonly SlotMap Empty = new(Seq<(int, ToolAssembly)>());
