...
The module `__all__` exports exactly `runtime` (singleton `VegaFusionRuntime`), `set_local_tz`, `get_local_tz`, and `get_column_usage`. `ChartState`, `VariableUpdate`, `Variable`, `CommPlan`, and `PreTransformWarning` are REAL `class`/`TypedDict` definitions in the `vegafusion.runtime` module (the native core exports `PyChartState`/`PyChartStateGrpc`/`PyVegaFusionRuntime` and the Python layer wraps them) — they are not string-only annotations. The reachability is the trap: `vegafusion/__init__.py` does `from .runtime import runtime`, binding the name `runtime` on the package to the singleton *instance*, which shadows the submodule attribute — so the dotted form `vegafusion.runtime.ChartState` FAILS (the parent attribute is the `VegaFusionRuntime` instance, not the module). The classes are reachable only through `importlib.import_module("vegafusion.runtime")` / `sys.modules["vegafusion.runtime"]` (which is still the real module), or simply by typing against the structural `TypedDict` shapes the owner already holds. `ChartState` itself is normally obtained as the `runtime.new_chart_state(...)` return value, never imported. The `transformer` submodule converts dataframes to Arrow IPC `bytes`/`pa.Table` for the inline-dataset path.

| [INDEX] | [SYMBOL]      | [TYPE_FAMILY]     | [RAIL]                                                                 |
| :-----: | :------------ | :---------------- | :-------------------------------------------------------------------- |
|  [01]   | `runtime`     | runtime singleton | `VegaFusionRuntime` transform-execution owner (worker pool)           |
|  [02]   | `ChartState`  | interactive state | real class in `vegafusion.runtime`; the `new_chart_state` return; reach via `sys.modules`, never `vegafusion.runtime.ChartState` (shadowed by the singleton) |
|  [03]   | `CommPlan` (`TypedDict`) | watch plan | `get_comm_plan`/`get_watch_plan` return; `{client_to_server, server_to_client: list[Variable]}` |
|  [04]   | `VariableUpdate` / `Variable` (`TypedDict`) | update record | `ChartState.update` element; `{name, namespace: Literal['data','signal'], scope: list[int]}` (+ `value: Any` on `VariableUpdate`) |
|  [05]   | `PreTransformWarning` (`TypedDict`) | warning | element of every `pre_transform_*` warnings list; `{type: Literal['RowLimitExceeded','BrokenInteractivity','Unsupported'], message: str}` |
