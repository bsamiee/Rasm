...
[ENTRYPOINT_SCOPE]: the measurement entries — hook, render-prop, viewport source, HOCs, `viewBox` recovery, and the standalone debounce.

| [INDEX] | [FAMILY]    | [SURFACE]                                                    | [CAPABILITY]                                            |
| :-----: | :---------- | :----------------------------------------------------------- | :------------------------------------------------------ |
|  [01]   | hook        | `useParentSize(UseParentSizeConfig?)`                        | primary entry — `ResizeObserver` on the ref'd container |
...
- `@observablehq/plot`(`.api/observablehq-plot.md`) + `uplot`(`.api/uplot.md`): Plot takes `width`/`height` in its options and uplot via `setSize({width,height})`; the same `useParentSize` numbers feed both, so one measurement hook serves every engine and only the application differs.
