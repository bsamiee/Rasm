...
- [VECTOR](.planning/graphic/vector.md): `Vector` host-free SVG-geometry primitive every visual + document plane composes — ONE modal owner over the closed `VectorOp` family into `RuntimeRail[Block[VectorResult]]`, exposing the `bounds`/`transform`/`fit`/`path`/`svg`/`measure`/`sample`/`flatten`/`boolean`/`outline`/`text_path`/`rasterize` geometry surface the `composition/compose#COMPOSE` placement and `typography/shape#SHAPE` text-on-path seams import one hop in-process; totals over `Result[VectorResult, VectorFault]`.
...
- [RENDER](.planning/scene/render.md): `Scene3d` 3D scientific-visualization render owner over the closed-payload `SceneOp` (`Image`/`Export`/`Frames`/`Ingest`/`Compose`) — `pyvista`/VTK datasets, the closed `FieldFilter` clip/slice/threshold/contour/warp/glyph/streamlines/decimate + mesh-repair family, PBR + classic-lighting materials, offscreen software-GL render on the native-VTK worker lane, emitting the orbit rgb24 frame sequence `media/container#CONTAINER` ingests; every arm mints `(ContentKey, ArtifactReceipt)`.
