...
Rasm.AppUi resolves every visual constant through one generated token catalogue: a handful of appearance SEED rows expanded by one pure fold into the whole role ladder under typed `TokenKey`s, generated metric scales beside the paint ladder, layered elevation and material rows, orthogonal `ThemeVariantRow` and `DensityRow` families that project the same generation rather than carrying parallel columns, and one typed host-preference probe family. Sibling pages carry the rest of the theme module: `Theme/semi.md` the shipped-key correspondence and its conformance check, `Theme/tokens.md` the data-colormap catalog, `Theme/emission.md` the dictionary emission, the swap capsule, the Styles chain, and the control-skin table.
...
public sealed class PreferenceCell(
    ...
    public static PreferenceCell OfPlatform(
        ...
        Func<PreferenceRow, Option<PreferenceValue>> hostPort,
        Func<Action<PreferenceRow>, IDisposable> hostFlips) =>
        new(host: row => row.Switch(
                state: (Settings: settings, Port: hostPort),
...
| :-----: | :------------------------ | :-------------------------------------------------- | :----------------------------- | :------------ |
|  [01]   | rhino-panel, rhino-modal  | `RunningInDarkMode` read, `ThemeChanged` flips      | host-attach port columns       | settled       |
|  [02]   | gh2-companion             | same host appearance row as rhino                   | host-attach port columns       | settled       |
|  [03]   | avalonia-desktop, sidecar | `GetColorValues()` read, `ColorValuesChanged` flips | port absent, withheld defaults | settled       |
...
- Auto: every metric family re-derives from the elected policy inside `Resolve`, so a density change is one row value; the virtualization extent ledger re-realizes on the resolved `extent-*` change reported by the swap fact.
