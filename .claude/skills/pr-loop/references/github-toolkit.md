...
## S0 — Identify the PR and pin the head SHA (first thing, every iteration)
...
## S1 — Snapshot all feedback in one pass

Four orthogonal sources; none subsumes another. Threads are the only source carrying resolution state.
...
## S2 — Per-reviewer check-run / status state (who finished vs still running)

Two disjoint systems coexist on a commit; read both. Both are keyed on the SHA, so they are fresh when you pass `$HEAD`.
...
"Reviewer X still running" iff any check-run with `app.slug==X` (or `app.id`, or name match) has `status != completed`. "All finished" iff no such row remains — this is the wait predicate Step 2 blocks on.

## S3 — Re-trigger a specific bot, idempotently

Detect "already running" before posting; guard against duplicate triggers on the same head.
...
## S4 — Batch-resolve review threads (one GraphQL round-trip)

`resolveReviewThread` takes only `threadId: ID!`; resolving an already-resolved thread is a no-op (safe to over-include).
...
## S5 — Push, then detect reviews targeting the LATEST commit

After push the head advances; re-pin `$HEAD` and treat every prior review/comment as stale until proven by SHA equality.
...
## S6 — Convergence read + merge surface (read only; the loop never merges)
...
## S7 — Polling discipline (wait without busy-spinning)
