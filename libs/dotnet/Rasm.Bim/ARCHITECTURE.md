...
`Rasm.Bim` owns the host-neutral BIM/IFC domain and the IFC arm of the `Rasm.Element` contract, lowering GeometryGym models into the canonical `ElementGraph` under its own IFC-semantic legality. Terminal BIM refusals land on the compact `BimFault` band and captured foreign errors retain their cause. Consumers reach the domain through the shared `Bake(objectNode)` fold; Bim references no AEC peer, aligns through the shared element graph and content-keyed wire, and cedes simulation to Compute.
...
│   ├── Query.cs           # BimLeaf wraps the shared leaf in one arm beside IFC-vocabulary leaves; ElementQuery binds one Selection<NodeId>
...
│   ├── Systems.cs         # MEP connectivity VIEW over the element graph; the projector already lowered every distribution entity
...
│   └── Observability.cs   # BimPoint roster onto the kernel IHookRoster floor; BimTelemetry projects facts as a hook subscriber
...
│   ├── Appearance.cs      # AppearanceProjection.Project extracts front-face surface styles onto the neutral summary the graph node holds
│   ├── Connection.cs      # Whole realizing-element surface, fasteners to accessories, lowered onto shared detail bags
│   ├── GeoReference.cs    # GeoReferenceProjector.Project switches the one HasCoordinateOperation onto Header and Coverage
│   ├── Feature.cs         # GeoFeature row: NTS Geometry, attributes, shared CRS, typed IsValidOp verdict, on-shape Anchor, H3 Cell
...
│   ├── Validation.cs      # Shared ModelAudit composes WHOLE by value; stored tier verdicts compare structurally, so a re-load reads as no change
...
└── Projection/            # IFC arm of the Rasm.Element contract
    ├── Semantic.cs        # INGRESS half: a live DatabaseIfc lowers to a graph delta; IfcLegality decides relationship legality
    ...
    ├── Value.cs           # No IfcValue or dataType string crosses the contract signature; both narrowing halves live here
...
- S0 `Model` — settled vocabulary consuming no sibling: the `BimFault` union, the `BimLeaf`/`ElementQuery` query algebra over the contract closure.
- S0 hooks — the observability hooks seat at the vocabulary floor, so every stratum fires typed facts downward and none returns.
- S0 offline — `IfcVocabularyEmitter` PRODUCES that roster at design time and no runtime fence reaches it; the region markers are the boundary.
...
- S1 law — enrichment owners read the element graph; the geospatial ingest fronts admit site context alone, and no S1 owner touches IFC bytes.
- S2 law — projector ingress and legality stand as the only writers onto the contract; every other stratum reads.
...
---
flowchart LR
    accTitle: Bim same-branch domain and storage boundaries
...
---
flowchart LR
    accTitle: Bim cross-runtime, presentation, and host boundaries
...
`Rasm.AppHost` composes the `BimHooks` dispatcher per instance, admits the `Rasm.Bim` meter and the `BimPoint.Scopes` trace planes at its telemetry root, and binds the announced message envelope its `Exchange/events#EVENT_PROJECTION` subscription mints onto its broker transports. Span custody stays the kernel `SpanBand` that root owns and the meter scope stays `TelemetrySource.Bim`; neither grammar derives from the other.
...
`typescript:core` decodes that projection behind its own `interchange/codec` `WkbParser` port over raw bytes and mints a `Wire.GeoFeature` landing its family roster excludes; `typescript:ui` reaches the landing through `@rasm/core` alone, so no geo edge runs from here to it. Persistence's geo-store takes the GeoPackage blob leg without a runtime crossing, and `GeoWkb` stays the interior OGR-to-NTS bridge, never a shared wire.

Every `[CONTENT_KEY]` edge derives its typed `UInt128` through `ContentHash.Of` over the shared `CanonicalWriter` fold, joining the Compute content-addressing space; per-page key tuples live on the owning implementation pages.

## [04]-[INTERNAL]

One ingress law rules the interior: every foreign source lowers once through a projector onto the shared `GraphDelta`, every egress re-authors from the frozen element graph, and round-trip drop facts accumulate on the `FidelityLog` `Writer` carrier beside their values, never as side effects. Per-arm wiring lives on the owning implementation pages.
...
flowchart LR
    ...
    accDescr: How foreign bytes become element graph content and re-author to legal IFC with drops accumulated as facts.
    ...
    Projector e4@--> Delta[[Element GraphDelta]]
    Delta e5@--> GraphN[[Frozen element graph]]
