...
public static partial class Analyze {
    ...
    private static global::Rasm.Analysis.Operation<TGeometry, TOut> ScalarCurvature<TGeometry, TOut, TNative>(Op key, Requirement requirement, int count, StatKind kind, Func<Op, TNative, int, Context, Fin<Seq<double>>> project) where TGeometry : notnull where TNative : notnull => typeof(TOut) switch {
        ...
        Type output when output == typeof(StatProfile) => CurvatureOperation<TGeometry, TOut, TNative, StatProfile>(key: key, requirement: requirement, count: count, project: (op, native, sampleCount, context) => project(arg1: op, arg2: native, arg3: sampleCount, arg4: context).Bind(values => StatProfile.Curvature(values: values, kind: kind, key: op).Map(static profile => Seq(profile)))),
        _ => key.Unsupported<TGeometry, TOut>(),
    };
    ...
    private static Fin<Seq<StatProfile>> SurfaceProfiles(Op key, Seq<SurfaceCurvature> curvatures) =>
        BorrowCurvatures(curvatures: curvatures, project: owned =>
            (SurfaceScalars(key: key, curvatures: owned, kind: StatKind.Gaussian).Bind(values => StatProfile.Curvature(values: values, kind: StatKind.Gaussian, key: key)),
             SurfaceScalars(key: key, curvatures: owned, kind: StatKind.Mean).Bind(values => StatProfile.Curvature(values: values, kind: StatKind.Mean, key: key)))
