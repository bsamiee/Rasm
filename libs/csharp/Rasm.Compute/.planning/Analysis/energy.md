...
public static partial class EnergySimulation {
    ...
    // artifact, read-only + immutable so the connection never mutates or locks the solver's file: the
    // SWIG SqlFile exposes NO accessor and NO generic SQL exec for the TabularDataWithStrings report table, so
    // the setpoint-not-met validity hours (the SystemSummary "Time Setpoint Not Met" rows ASHRAE 90.1 caps at
    // ~300 h/yr) read here — the ONE tabular query family, parameterized on (report, table, row, column),
    // never a per-metric method ladder; a missing/malformed table stays (Extraction, Foreign)-railed as an
    // absent fact, never a fabricated zero.
    static Seq<AssessmentFact> TabularFacts(string sqlPath) {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={sqlPath};Mode=ReadOnly;");
        connection.Open();
        return Seq(
