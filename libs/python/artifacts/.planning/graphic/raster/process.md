...
- Output: each acceptor folds into the canonical `RasterFact` declared here — scikit arms through `_save_array` and pillow arms through `_save_pillow`. `FieldLaw.QUANTITATIVE` preserves pre-rescale octets in `RasterFact.field`; numeric stamps remain native `float` values in `RasterFact.score`.
