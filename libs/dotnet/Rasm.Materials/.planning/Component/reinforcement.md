...
public static class RebarSchedule {
    ...
    public static Fin<RebarBend> StandardHook(BarRow bar, RebarUsage usage, HookKind kind, RebarHook hook) =>
        guard(usage.Stirrup == (kind != HookKind.Development),
                new KernelFault.InvalidValue(nameof(kind), "a hook kind matching bar usage"))
            .ToFin()
            .Map(_ => new RebarBend(
                hook.BendDegrees,
...
public sealed partial class ForceBasis {
    ...
    public Fin<double> ForceKn(BarRow bar, MaterialGrade grade) =>
        from arm in grade.RebarArm.ToFin(new ComponentFault.GradeBodyMissing(grade, ComponentFamily.Reinforcement))
        from admitted in guard(arm.Rolls(bar), new KernelFault.InvalidValue(nameof(bar), "a bar rolled by its grade system"))
        from force in ProjectKn(bar, arm).ToFin(new ComponentFault.GradeBandMissing(ComponentFamily.Reinforcement, typeof(ForceBasis)))
        select force;
}
...
public sealed partial class TendonBasis {
    ...
    public Fin<double> ForceKn(MaterialGrade strand) =>
        from arm in strand.StrandArm.ToFin(new ComponentFault.GradeBodyMissing(strand, ComponentFamily.Reinforcement))
        from force in ProjectKn(arm, strand.Authority).ToFin(new ComponentFault.GradeBandMissing(ComponentFamily.Reinforcement, typeof(TendonBasis)))
        select force;
}
...
public abstract partial record ReinforcementRow {
    ...
    public Validation<Error, Unit> Coherence() => Switch(
        bar: row => AdmissionSlots.Accumulate(Seq(
            AdmissionSlots.Gate(row.Grade.Family == ComponentFamily.Reinforcement, new ComponentFault.GradeFamilyMismatch(row.Grade, ComponentFamily.Reinforcement)),
            AdmissionSlots.Gate(row.Grade.RebarArm.IsSome, new ComponentFault.GradeBodyMissing(row.Grade, ComponentFamily.Reinforcement)),
            AdmissionSlots.Gate(row.Grade.Admits(row.Size), new KernelFault.InvalidValue(nameof(row.Size), "a size admitted by its grade system")))),
        tendon: row => AdmissionSlots.Accumulate(Seq(
            AdmissionSlots.Gate(row.Strand.Family == ComponentFamily.Reinforcement, new ComponentFault.GradeFamilyMismatch(row.Strand, ComponentFamily.Reinforcement)),
            AdmissionSlots.Gate(row.Strand.StrandArm.IsSome, new ComponentFault.GradeBodyMissing(row.Strand, ComponentFamily.Reinforcement)))));
}

public static class ReinforcementDetail {
    public static Fin<PropertyBag> Of(ReinforcementRow row, EvidenceGrade source) =>
        from section in row.Section.ToFin(new KernelFault.InvalidValue(nameof(row.Section), "a reinforcement cross-section"))
...
public static class ReinforcementSeed {
    ...
    public static readonly SeedLaw<ReinforcementRow> Law = SeedLaw<ReinforcementRow>.Of(
        ...
        standard: static row => row.Grade.Standard,
        detail: Some<Func<ReinforcementRow, SectionProfile, Fin<PropertyBag>>>(
            static (row, _, key) => ReinforcementDetail.Of(row, Tabulated)),
...
- Owner: `RcSection` the reinforced-concrete section over the `VividOrange.Sections` `IConcreteSection` AND the held `ConcreteSectionProperties` transformed-section carrier; `RebarLayout` `[Union]` the closed rebar-arrangement axis (`FaceCount`/`FaceSpacing`/`PerimeterCount`/`PerimeterSpacing`/`Placed`) collapsing the four `VividOrange.Sections` layout-engine constructors plus the loose-bar ingress; `EnGrade` the EN-grade admission boundary lowering the `VividOrange.Materials` derivation throws onto the typed `ComponentFault` channel; `RcSectionBuilder` the one assembler minting the `IConcreteSection` the `capacity#SECTION_CAPACITY` solvers consume.
...
- Entry: `RcSectionBuilder.Of(concrete, concreteGrade, barGrade, link, layout, coverMm, annex)` lowers grades through `EnGrade`, proves the link and every layout bar through `MaterialGrade.Admits`, builds the profile-faithful `ConcreteSection`, and captures its `ConcreteSectionProperties`; absent grade bodies fail `GradeBodyMissing`.
- Packages: VividOrange.Sections (`ConcreteSection`, `Rebar`/`Link`/`LongitudinalReinforcement`, `FaceReinforcementLayer`/`PerimeterReinforcementLayer`, `MinimumReinforcementSpacing` with the settable `MaximumAggregateSize`, `SectionFace`, `BarDiameter`; the `InvalidMaterialTypeException`/`InvalidProfileTypeException` throws trapped here), VividOrange.Sections.SectionProperties (`ConcreteSectionProperties` — the transformed-section columns `RcSection` reads), VividOrange.Materials (`EnConcreteMaterial`/`EnRebarMaterial` + their factories; the `ArgumentException`/`MissingNationalAnnexException` throws trapped here), VividOrange.Standards (`En1992`/`NationalAnnex`), VividOrange.Profiles (`IProfile` via `component#SECTION_SOLVER` `ProfileOf`), UnitsNet (`Length` at the edge), Rasm.Domain (`Try.lift` preserving exact exceptional errors), Thinktecture.Runtime.Extensions (`[Union]`), LanguageExt.Core (`Fin`/`Seq`).
- Growth: a new rebar arrangement is one `RebarLayout` case binding its engine ingress through `RebarPlacement` (the generated `Switch` breaks every dispatch site at compile time); a new tendon force basis is one `TendonBasis` row and a new prestressing authority one `ComponentAuthority.Jacking` pair, never a branch; a new transformed-section read is one projection on `RcSection` over the held carrier; a new constitutive concrete law is a `capacity#SECTION_CAPACITY` concern over the same `IConcreteSection` — never a per-arrangement builder, never a hand-keyed `f_yk`/`f_ck` where the EN grade carries it, never a re-summed bar area where `ConcreteSectionProperties` carries it.
...
public static class EnGrade {
    ...
    internal static Option<ComponentFault.GradeDerivation> GradeRefusal(Error cause) =>
        cause.Exception.Case is ArgumentException or MissingNationalAnnexException or InvalidSteelSpecificationException
            ? Some(new ComponentFault.GradeDerivation(cause))
            : None;
}
...
public static class RcSectionBuilder {
    ...
    static Fin<RebarLayout> ValidateLayout(RebarLayout layout, MaterialGrade grade) =>
    ...
    static Validation<Error, Unit> Shape(RebarLayout layout) => layout.Switch(
        faceCount: item => AdmissionSlots.Gate(item.Count > 0, new KernelFault.OutOfRange(nameof(item.Count), item.Count, "positive")),
        faceSpacing: item => AdmissionSlots.Gate(double.IsFinite(item.MaxSpacingMm) && item.MaxSpacingMm > 0.0,
            new KernelFault.OutOfRange(nameof(item.MaxSpacingMm), item.MaxSpacingMm, "finite and positive")),
        perimeterCount: item => AdmissionSlots.Gate(item.Count > 0, new KernelFault.OutOfRange(nameof(item.Count), item.Count, "positive")),
        ...
        placed: item => AdmissionSlots.Gate(double.IsFinite(item.YMm) && double.IsFinite(item.ZMm),
            new KernelFault.InvalidValue(nameof(RebarLayout.Placed), "finite section-plane coordinates")));
