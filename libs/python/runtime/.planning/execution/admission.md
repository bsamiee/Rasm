...
- Growth: a new context field is one `RuntimeContext` column; a new profile one `RuntimeProfile` member with one `PROFILE_POLICY` row; a new feature one `Feature` case with its membership in the affected rows' `admitted` sets; a new killswitch one `Killswitch` case, one `KILLSWITCH_FEATURE` disabling edge, and its `tripped` memberships — never a parallel boolean knob; a new attribute dimension one entry in the `attribute` projection.
