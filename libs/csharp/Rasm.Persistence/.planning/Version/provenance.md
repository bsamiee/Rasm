...
public static class CausalDag {
    public static Seq<ProvEdge> Derive(Seq<OpLogEntry> changefeed, Func<UInt128, Option<CommitNode>> resolve) =>
        changefeed.Bind(entry => {
            ...
            Seq<ProvEdge> informed = node.ToSeq().Filter(static c => c.IsMerge).Bind(c => commit.ToSeq().Bind(activityKey => c.Parents.Map(parent => ProvEdge.Of(ProvRelation.WasInformedBy, activityKey, parent, entry.Stamp))));
    ...
    public static ProvBundle Bundle(Seq<ProvEdge> lineage, StoreActor authority, Option<SignedAuthorship> attestation, Instant at) {
        ...
        return new ProvBundle(rolling.GetCurrentHashAsUInt128(), lineage, new ProvNode.Agent(authority.Subject, AgentClass.Of(authority), attestation.IsSome), at);
    }
...
- Packages: System.IO.Hashing, NodaTime, LanguageExt.Core, Thinktecture.Runtime.Extensions, BCL inbox (the KMS verify rides `Element/identity#KMS_CUSTODY` `Custody.Verify` over the resolved `SigningKeyring`, never a direct provider call here).
