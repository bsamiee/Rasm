...
internal static class Verbs {
    ...
    internal static Eff<SupervisorRuntime, SessionEnvelope> Session(SupervisorVerb verb) {
        ...
        return Eff<SupervisorRuntime, SessionEnvelope>.Lift(f: runtime => Fin.Succ(value: Pending(verb: verb, runtime: runtime)));
    }
    ...
    // Capability gate: this build carries no host-session kernels, so every verb folds the same
    // typed `supervisor.session` capability gap through the real terminal fold — envelope shape,
    // exit codes, and the one-document stdout contract hold from the first build.
    private static SessionEnvelope Pending(SupervisorVerb verb, SupervisorRuntime runtime) {
        string runId = Guid.NewGuid().ToString(format: "n");
        return SessionFold.Run(
            runId: runId, verb: verb,
            final: new SessionState.Faulted(
                Fault: new BridgeFault.CapabilityAbsent(Capability: "supervisor.session", ProbeReceipt: "session kernels absent in this build"),
                At: verb.EntryPhase, Done: Seq<ScenarioReceipt>()),
            stream: Seq<BridgeEvent>(), spoolTail: (0L, 0L),
            reportDir: Path.Combine(runtime.ArtifactRoot, verb.Key, runId));
    }
...
internal static class Program {
    ...
    private static SupervisorRuntime Compose(CancellationToken root) {
        ...
        return new SupervisorRuntime(
            ...
            ArtifactRoot: Path.Combine(Environment.CurrentDirectory, ".artifacts", "rhino"),
