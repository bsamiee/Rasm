...
- `tools/cs-analyzer`'s rule register IS the code — `Kernel/Catalog.cs` with both `AnalyzerReleases.{Shipped,Unshipped}.md` ledgers, whose divergence is a build failure; a `RuleRow` without its ledger entry or witness spans, and a prose catalog of the rules anywhere, are findings. Suppression rides `[CspExempt(justification)]` from its owning assembly, never `#pragma` or `.editorconfig`.
...
- Analyzer changes land with their suite change under `tests/dotnet/tools/cs-analyzer`; the TS rule roster proves live through the `tests/typescript/_architecture` lint gauge, never a text diff.
