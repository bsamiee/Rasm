...
[TEXTURE_COORDINATE_BINDING]-[QUEUED]: an ingested texture resolves the UV coordinates it samples through, not the style alone.
- Capability: a textured surface carries the per-vertex UV correspondence its maps sample, so an imported style renders at the coordinates its author intended rather than at whatever the tessellation happened to produce.
- Shape: `libs/csharp/Rasm.Bim/.planning/Semantics/appearance.md` gains the coordinate leg on the ingest fold and the egress author; the representation-item side lands at `libs/csharp/Rasm.Bim/.planning/Exchange/import.md`, where the tessellated face set already decodes.
- Unlocks: `Exchange/export#EXPORT_RAIL` binds a real `CoordinateSet` per `ChannelImage` instead of the default set, and `[SURFACE_TEXTURE_INGEST]` stops carrying a coordinate assumption it cannot state.
- Anchors: the `IfcSurfaceTexture.Parameter` UV-name list this pass records but does not read; `IfcTextureCoordinate` and its `Maps` binding on the representation item; the `ChannelImage.CoordinateSet` column already present on the glTF binder.
- Tension: IFC binds coordinates on the REPRESENTATION ITEM while the style binds on the surface, so the two halves meet only where a projector holds both — the seam node holds neither, which is why the coordinate leg is a separate landing rather than a column on `SurfaceTexture`.
- Ripple: follows `[SURFACE_TEXTURE_INGEST]`.
...
[GLTF_TEXTURE_CHANNEL]-[COMPLETE]: `Exchange/export#EXPORT_RAIL` binds texture maps onto the pooled glTF material through the `GltfChannel` roster — `ChannelImage.Of` admits a CANONICAL channel name and refuses one glTF has no slot for, its row names the `KnownChannel` targets one `ImageBuilder` threads (the `orm` pack reaching both the occlusion and metallic-roughness references) and the `KhrExtension` the channel obliges, and `MaterialFinish` keys the material pool on the seam `AppearanceKey` plus every bound map so a textured element never inherits an untextured neighbour's material; registration unions the payload's own obliged rows with the policy roster.
