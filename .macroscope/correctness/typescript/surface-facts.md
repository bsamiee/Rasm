...
- Emit-shaping options (`declaration`, `isolatedDeclarations`) bind only declaration-emitting packages in their own configs — a shared `tsconfig.base.json` carrying them taxes every non-emitting project, and a per-project waiver re-spelling the base's absent emit gate is redundant residue.
