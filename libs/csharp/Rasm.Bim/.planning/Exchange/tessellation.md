...
- [02]-[TESSELLATION_BRIDGE]: imported IFC geometry crossing to the companion rail and re-importing as GLB, the `GeomSetting` capability vocabulary, the published re-drive cadence, and the dual-key `TessellationOutcome` receipt.
    ...
    public static Fin<TessellationCross> Admit(
        ...
        ContentHash.Admit(receipt.ContentKey.Span, key).Bind(reported =>
            reported == request.ContentKey
                ? Peer(receipt, key).Map(peer => new TessellationCross(glb, peer))
