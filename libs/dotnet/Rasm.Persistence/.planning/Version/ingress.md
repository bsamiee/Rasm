...
- Boundary: Kafka instrumentation owns causal links; Persistence owns consumer construction and contributes no extension roster. A reference-only event resolves through the injected storage port before apply, and both payload arms re-hash against `subject` before the store sees bytes. `TryApply` remains atomic, duplicate JSON keys refuse, processing cancellation reaches every effect, and the rebalance handler contains its own resolution faults.
...
| :-----: | :--------------- | :------------------------------------------------- | :---------------------------------------------------------- |
|  [01]   | consume boundary | instrumented builder twin, flags explicit          | `AddKafkaConsumerInstrumentation` rides the AppHost root    |
...
|  [05]   | source gate      | admitted message envelope source set               | a mixed topic cannot leak an unvetted op onto the op-log    |
...
|  [14]   | refusal shape    | `ConsumeException` throws, `Error` callbacks       | the settle fold reads both; neither path sees the other     |
...
|  [17]   | throw crossing   | one `Try.lift` into `IngressFault.Lift`            | only documented exception families mint stage cases         |
