...
|  [02]   | `Language`          | grammar       | compiled grammar object built from a grammar capsule; carries `abi_version`/`semantic_version`/`name` plus the symbol/field/state introspection surface and `copy`/`__eq__`/`__hash__` |
...
|  [09]   | `Language.copy()` / `__eq__` / `__hash__`                | identity       | copy or key a grammar; `Language` is hashable |
