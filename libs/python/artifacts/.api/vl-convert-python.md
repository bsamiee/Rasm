...
Every Vega-Lite row shares `vl_version` (a `get_vegalite_versions` entry, e.g. `'v6.4'`/`'6.4'`; default latest), `config` (chart-config dict), `theme` (a `VegaThemes` name from `get_themes`), `format_locale`, and `time_format_locale` (a `*LocaleName` or an inline dict). Raster/SVG/scenegraph rows add `show_warnings` and `allowed_base_urls` (the external-data SSRF fence — a `list[str]` of permitted base URLs; deny-by-default — the caller must supply an explicit permitted-base-URL list, no external base URL is reachable otherwise); raster rows add `scale` (factor) and `ppi`; JPEG adds `quality`; HTML adds `bundle` (inline all deps vs CDN) and `renderer` (a `Renderer`). `vegalite_to_vega`/`url` carry no locale/base-url args. The owner places `vlc.vegalite_to_svg`/`png`/`pdf`/`html`/`jpeg` as typed `VL_RENDER` cells whose lambdas name the exact provider keywords and read the admitted `ChartRenderPolicy` fields directly, never a positional wall, a string-keyed `getattr` spread, or an erased kwargs projection. Each call shape below abbreviates that shared tail as `...`.
...
[VISUALS_RENDER]:
- import: `import vl_convert as vlc` in the fence prelude; native converter calls execute only through the render lane, never on the event loop.
- render axis: the `vegalite_to_*`/`vega_to_*` family is one converter surface keyed by output format × spec dialect; the `visualization/chart/export#EXPORT` owner carries the per-format member pair as a `VlRow(vl, vega, text)` cell in the module-level `VL_RENDER` table totalled over `ExportFormat`, and each typed cell calls the provider with explicit format-correct keywords while the spec's own `$schema` selects the dialect converter. Vega-Lite is the authoring dialect; `vegalite_to_vega` materializes the compiled Vega when the owner caches or inspects compilation before raster. `vegalite_to_scenegraph`/`vega_to_scenegraph` expose the resolved scenegraph dict for measurement without committing to a raster.
...
- concurrency axis: `ChartExport.lane.offload` moves `vlc.vegalite_to_*` and `svg_to_*` across as `KernelTrait.RELEASING` kernels and the `visualization/chart/export#PREPASS` vegafusion work as `KernelTrait.HOSTILE`, worker-death retry riding the HOSTILE trait row alone; `LanePolicy` owns limiter and deadline composition, so no V8 render blocks the event loop and no folder-local limiter exists.
...
- evidence: `ContentIdentity.key` folds the length-framed canonical input preimage into the node key and the bytes contribute exactly the `core/receipt#RECEIPT` `ArtifactReceipt.Chart(key, engine, format, scale, theme, byte_len)` fact (`engine` = matched `ChartSpec.tag`, `format` = `ExportFormat.value`, `scale`/`theme` = `ChartRenderPolicy` values, `byte_len` = rendered length) under one `structlog` (`../../.api/structlog.md`) event inside one `opentelemetry` (`../../.api/opentelemetry-api.md`) span; this engine contributes no parallel visuals-receipt type. Universal `libs/python/.api` rails stack on this folder catalog.
...
- universal-rail weave: runtime `LanePolicy` composes the shared `anyio` (`../../.api/anyio.md`) offload and `stamina` (`../../.api/stamina.md`) retry rails; `msgspec` (`../../.api/msgspec.md`) carries the `ArtifactReceipt.Chart` fact, `structlog` (`../../.api/structlog.md`) plus `opentelemetry` (`../../.api/opentelemetry-api.md`) carry event/span evidence, and `expression.Result` (`../../.api/expression.md`) carries typed failure — no render arm re-implements those rails.
