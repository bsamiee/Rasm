...
- version: `1.0.1` — single hoisted copy; `assay api resolve` resolves it cleanly via TSDECL (no transitive-version ambiguity, unlike the `@oslojs/encoding` sibling)
- license: `MIT`
...
- runtime: `runtime:neutral` — pure JS, no `node:*`; the `sign/` "node-only subpath" caveat is `@node-rs/argon2`'s native constraint, not this package
...
|  [03]   | `RandomReader { read(bytes: Uint8Array): void }`                    | entropy port   | satisfied by WebCrypto `crypto.getRandomValues`; never owns entropy |
...
| [INDEX] | [SUBPATH]              | [VALUE_SET]                                                              | [CONSUMER / BOUNDARY]                                  |
| :-----: | :--------------------- | :---------------------------------------------------------------------- | :---------------------------------------------------- |
|  [01]   | `/sha1`                | `sha1` / `SHA1` (legacy interop only — HOTP OID, never new digests)     | RFC-4226 interop OID; not a new-material digest        |
