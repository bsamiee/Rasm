...
- `[2]-[SEAMS]` is the curated, file-level record of every cross-folder and cross-language alignment, in one ` ```text seams ` fence with aligned columns: `<SourceFile> <glyph> <lang:pkg/subdomain> # [<KIND>]: <shared shape>`. Glyphs: `→` produces/projects, `←` consumes/reads, `⇄` shared shape. `<KIND>` ∈ {`[WIRE]`, `[PROJECTION]`, `[PORT]`, `[CONTENT_KEY]`, `[TESSELLATION]`, `[GRADUATION]`, `[RECEIPT]`, `[TRANSPORT]`, `[FAULT]`, `[SHAPE]`, `[BOUNDARY]`}. Every edge appears on both endpoint folders with mirrored glyph; an in-package relation stays in the codemap and is never a seam.
