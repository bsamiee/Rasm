...
- Growth: one part-window policy value or one managed-path threshold per provider row; the content-defined dedup window is the `snapshots#CONTENT_CHUNKING` chunk fold the part window spans whole chunks of, never a second chunker here; zero new surface.
