...
- Nothing crashes and nothing drops under it — the grant releases only as the previous delivery dequeues, so the producer silently runs ONE unacknowledged delivery deep, five hundred times narrower than the builder path.
...
- send verbs split on bound: `ISend.Send` awaits its `MessageId` through a `TaskCompletionSource`, while `SendChannel.Send` enqueues onto the same bounded queue and returns; both traverse `InternalSend`, so the pending cap governs each.
