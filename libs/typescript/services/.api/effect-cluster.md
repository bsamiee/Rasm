...
Dependency catalogue for `@effect/cluster` (v0.59.0), the node-tier distributed sharding, entity, durable-workflow, and runner-backplane surface. Grounded from the installed `node_modules/@effect/cluster/dist/dts/*.d.ts` declarations. The package is a `node`-tagged satellite that never enters the browser bundle. Owner-symbol consumers: `ClusterEngine` (`execution/engine#ENGINE` — layers `ClusterWorkflowEngine` over `Sharding`); `RunnerBackplane` + `ScheduledWork` (`execution/backplane#RUNNER_AND_SCHEDULING` — protocol/message-storage/runner-storage/runner-health backplane, snowflake id source, cluster singletons, durable cron); `SqlBoundary` (`persistence/store#STORE_BOUNDARY` — the one `SqlClient` surface `SqlMessageStorage` + `SqlRunnerStorage` ride).
...
The cluster-backed `WorkflowEngine` from `@effect/workflow`: register/execute/poll/interrupt/resume workflows, activity execution, durable-deferred + durable-clock plumbing. This is the surface `ClusterEngine` (`execution/engine#ENGINE`) layers.
