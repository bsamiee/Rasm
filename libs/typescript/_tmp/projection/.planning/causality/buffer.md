...
- [01]-[CAUSAL_BUFFER]: Owns `DeliveryVerdict`, `BufferCell`, `contextOf`, `deliverable`, `releaseSuffix`, `bufferMerge`, and the `causalBuffer` per-entity pending-queue fold.
- [02]-[CAUSAL_DELIVERY]: Owns `causalDelivery`, the composition root forking the `causalBuffer` store and the `frontier#STABILITY_FRONTIER` cursor meet into one `Scope` and emitting the released causal-ordered changefeed `Stream`.
