...
- Entry: `Operation<TGeometry, TOut>(Op key)` is the generated `Switch` fold from case to operation threading the CALLER-OWNED `Op` — `AnalysisQuery.LocationCase.Build` receives `AnalysisVerb.Location.Op` and hands it through, so this page mints no key of its own — and `AnalysisQuery.Location` is the ONLY public route in; no case exposes a second executable surface.
...
flowchart LR
    ...
    accDescr: AnalysisQuery.Location threads the verb key into the Location case fold; each case lowers through one Locate builder that leases the native, reads its selector or division row, and returns to the Analysis runtime.
    Query["Analysis/query AnalysisQuery.Location · Op key"] -->|Location.Operation with key| Location["Location case Switch"]
...
| :-----: | :----------------- | :---------------- | :------------------------------------------ | :-----: |
|  [01]   | location operation | `Location`        | `Operation<TGeometry,TOut>(Op) → Operation` |   11    |
