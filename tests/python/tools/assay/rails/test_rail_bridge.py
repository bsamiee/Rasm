"""Laws for tools.assay.rails.bridge — BridgeParams, bridge_lease, build, check, clean, client_run, doctor, first_fault, launch, quit, verify."""
...
def test_bridge_module_public_surface() -> None:
    """Bridge module __all__ names exactly the 11 expected public symbols — no silent omission or addition."""
    expected = frozenset((
        "BridgeParams",
        "bridge_lease",
        "build",
...
register_law(_markers, "no_markers_yields_empty_tuples")
register_law(_markers, "evidence_marker_decoded_into_facts")
register_law(_markers, "capture_marker_decoded_into_captures")
register_law(_markers, "mixed_markers_split_by_kind")


@pytest.mark.parametrize(
    "marker_lines, expected",
    [
        ("plain text", ((), ())),
        ('rasm.rhino-bridge.evidence=facts={"key":"value"}', (('{"key":"value"}',), ())),
        ('rasm.rhino-bridge.capture={"png":"base64data"}', ((), ('{"png":"base64data"}',))),
        ('rasm.rhino-bridge.evidence=facts={"a":1}\nnoise\nrasm.rhino-bridge.capture={"b":2}', (('{"a":1}',), ('{"b":2}',))),
    ],
    ids=["no-marker-empty", "evidence-marker", "capture-marker", "mixed-markers-split"],
)
def test_markers(marker_lines: str, expected: tuple[tuple[str, ...], tuple[str, ...]]) -> None:
    """_markers decodes evidence/capture JSON payloads from execute stdout into typed (facts, captures) tuples."""
    phase = _BridgePhase(phase="execute", status=RailStatus.OK, outputs=(_BridgeOutput(source="stdout", text=marker_lines),))
    assert _markers(_BridgeResult(status=RailStatus.OK, phases=(phase,))) == expected
...
register_law(_fold_scenarios, "marker_stdout_surfaces_typed_facts_and_captures")


def test_fold_scenarios_marker_stdout_surfaces_typed_fields(assay_root: AssayHarness, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A summary built from marker-bearing execute stdout exposes (stem, JSON) tuples; no note carries a facts:/capture: prefix."""
    _install_dll(assay_root)
    report_dir = tmp_path / "verify"
    report_dir.mkdir()
    scenario = _make_scenario(assay_root, "tests/bridge/Marker.verify.csx")
    text = 'rasm.rhino-bridge.evidence=facts={"key":"value"}\nrasm.rhino-bridge.capture={"png":"base64data"}'
    phase = _BridgePhase(phase="execute", status=RailStatus.OK, outputs=(_out("stdout", text),))
    payload_json = msgspec.json.encode(_BridgeResult(command="check", status=RailStatus.OK, phases=(phase,)))
    monkeypatch.setattr(_bridge_mod, "run_check", lambda *a, **kw: Ok(receipt(("rasm-bridge",), 0, stdout=payload_json)))
    (report_dir / "Marker.json").write_bytes(payload_json)
    report = assert_ok(_fold_scenarios(assay_root.settings, report_dir, (scenario,), ("bridge", "verify", "")))
    assert isinstance(report.detail, VerifySummary)
    assert report.detail.facts == (("Marker", '{"key":"value"}'),)
    assert report.detail.captures == (("Marker", '{"png":"base64data"}'),)
    assert not any(note.startswith(("facts:", "capture:")) for note in report.notes)


# --- [BRIDGE_LEASE]

register_law(bridge_lease, "serializes_contenders_to_busy")


def test_bridge_lease_second_contender_sees_busy(assay_root: AssayHarness) -> None:
    """Two bridge_lease contenders serialize on the one "bridge" resource: the holder's action runs, a nested second acquisition is BUSY."""
    ...
    assert_error_status(nested[0], RailStatus.BUSY)
    # The lease releases with the action: a follow-up acquisition succeeds.
...
register_law(client_run, "args_and_timeout_route_to_run_check")


def test_client_args_and_timeout_route_to_run_check(assay_root: AssayHarness, monkeypatch: pytest.MonkeyPatch) -> None:
    """client_run appends CLI args to the client tool command and maps timeout seconds onto the run_check deadline."""
    captured: list[tuple[tuple[str, ...], object]] = []

    def seam(check: Check, **kwargs: object) -> Result[Completed, Fault]:
        captured.append((check.tool.command, kwargs["deadline"]))
        return Ok(receipt(("rasm-bridge",), 0))

    monkeypatch.setattr(_bridge_mod, "run_check", seam)
    before = time.monotonic()
    assert_ok(client_run(assay_root.settings, "doctor"))
    assert_ok(client_run(assay_root.settings, "check", "--flag", timeout=30.0))
    (_, bare_deadline), (command, timed_deadline) = captured
    assert bare_deadline is None
    assert command[-2:] == ("check", "--flag")
    assert isinstance(timed_deadline, float)
    assert timed_deadline >= before + 30.0
