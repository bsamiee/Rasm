...
- Law: cone math is kernel-owned — the spot cone crosses as `VectorCone` (apex, unit axis, admitted half-angle), half-angle and solid-angle questions read `VectorCone.HalfAngle`/`SolidAngle` directly off the stamped cone, beam radius per unit distance reads `VectorCone.Spread`, and inline spot trigonometry beside the owner is the deleted form.
