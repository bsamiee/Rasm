...
The `crdt` column-family `OpLogEntryWire.payload` decodes through the `version-control#CRDT_WIRE` `CrdtOpWire` union — the single-mint `op`-discriminated owner carrying the `set | write | add | remove | increment | insertAfter | delete | maintain | beat | leave` arms over `HlcWire` cells. This page consumes that shape and declares no parallel `CrdtOpWire`.
