...
| :-----: | ----------------- | ------------- | ------------------------------------------------------------------- |
|   [1]   | `SVG`             | document root | parse/build an SVG tree; iterate elements, resolve viewport/viewBox |
|   [2]   | `Path`            | segment seq   | mutable ordered path segments; `d()` serialize, `*` transform       |
|   [3]   | `Matrix`          | affine        | spec-faithful 2D affine transform; compose, invert, apply           |
|   [4]   | `Point`           | coordinate    | 2D point with complex-number compatibility                          |
|   [5]   | `Length`          | unit value    | CSS/SVG unit (`mm`/`cm`/`in`/`px`/`pt`/`pc`/`%`) with `value(...)`  |
|   [6]   | `Color`           | color value   | SVG/CSS color parse and channel access                              |
|   [7]   | `Angle`           | angle value   | CSS angle (`deg`/`rad`/`grad`/`turn`)                               |

- rail: figure

| [INDEX] | TYPE          | KIND  | ROLE                                          |
| :-----: | ------------- | ----- | --------------------------------------------- |
|   [1]   | `Rect`        | shape | axis-aligned rectangle                        |
|   [2]   | `Circle`      | shape | circle by center and radius                   |
|   [3]   | `Ellipse`     | shape | ellipse by center and two radii               |
|   [4]   | `Polygon`     | shape | closed point sequence                         |
|   [5]   | `Polyline`    | shape | open point sequence                           |
|   [6]   | `SimpleLine`  | shape | single line segment                           |
...
| :-----: | -------------------------------------------- | ----------- | ---------------------------------------------------------- |
|   [1]   | `SVG.parse(source, reify=True, ...)`         | parse       | parse SVG from filename/file/`str` into a document tree    |
|   [2]   | `SVG.elements()`                             | iterate     | iterate resolved shape/path elements of the document       |
|   [3]   | `Path(d)`                                    | construct   | build a path from SVG `d` path-data string                 |
|   [4]   | `Path.d()`                                   | serialize   | emit the SVG `d` path-data string                          |
|   [5]   | `Path.bbox()`                                | query       | bounding box `(xmin, ymin, xmax, ymax)` of the path        |
|   [6]   | `Matrix(*components)`                        | construct   | build affine from `(a, b, c, d, e, f)` or transform string |
|   [7]   | `Matrix.scale(sx, sy)`                       | factory     | uniform/non-uniform scale matrix                           |
|   [8]   | `Matrix.translate(tx, ty)`                   | factory     | translation matrix                                         |
|   [9]   | `Matrix.rotate(angle)`                       | factory     | rotation matrix                                            |
