...
- Entry: `FilterDialect.Evaluate(envelope)` answers a `MatchVerdict`; `Cesql.Compile(text, key)` parses ONCE at subscription admission and lands an unparseable expression as `EgressFault.SettingsRejected` — never a delivery; `CesqlExpression.Evaluate(envelope)` runs the compiled tree per event.
