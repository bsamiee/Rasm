...
// One host CPU stack: the frame names top-down (outermost first) plus the self time attributed at the
// leaf — the shape EventPipe SampleProfiler stacks and Pyroscope span-profile stacks both fold to.
public sealed record ProfileStack(Seq<string> Frames, Duration Self);

// A correlation-keyed profile batch: AppHost delivers the stacks captured for one evidence correlation,
// so the fold joins them onto the frame carrying that same identity — CPU stacks and receipt durations
// read as one flame surface without a profiler reference entering this folder.
public sealed record ProfileSample(CorrelationId Correlation, Seq<ProfileStack> Stacks);

// Composition-bound sample source: AppHost owns capture and returns the correlation's stacks (None when
// no capture is live), so DevLoop binds it with the frame's correlation and folds delivered samples only.
public delegate IO<Option<ProfileSample>> ProfileSampleSource(CorrelationId correlation);
...
public sealed record FlameNode(string Frame, Duration Self, Seq<FlameNode> Children) {
    ...
    // and the OPTIONAL host profile subtree — a matched ProfileSample folds its stacks into a cpu-profile
    ...
    public static FlameNode Of(HudSample hud, Seq<(string Node, Duration Elapsed)> solves, Seq<PassTiming> passes, Option<ProfileSample> profile = default) =>
        new("frame", Duration.Zero,
            ...
            + profile.Map(static sample => FromStacks("cpu-profile", sample.Stacks)).ToSeq());

    // Prefix-merge the stacks into one profile subtree: shared frame prefixes collapse to shared nodes
    // and each stack's self time accrues at its own leaf, so a hot path renders as a wide bar. A stack is
    ...
    static FlameNode FromStacks(string root, Seq<ProfileStack> stacks) =>
        stacks.Fold(new FlameNode(root, Duration.Zero, Seq<FlameNode>()), static (tree, stack) => Graft(tree, stack.Frames, stack.Self));
