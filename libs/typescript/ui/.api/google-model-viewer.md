...
- `viewer/scene.md` [08]: `environmentImage`/`exposure`/`shadowIntensity` bind the decoded OpenPBR IBL, while `materialFromPoint`/`.model` read the scene-graph materials the appearance projection mirrors; generated `appearance.v1.Material` is decode-only, re-authored nowhere here.
