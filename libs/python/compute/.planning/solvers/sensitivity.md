...
- Engine carrier: `DiffEngine` is the ONE frozen value object folding the gated `jax`/`equinox` modules so the JAX flow runs as two spec-driven rails — the transform rail `_transformed` and the directional rail `_directional` — rather than a `_autodiff`/`_jvp`/`_vjp`/`_hvp` helper family. The gated `import jax`/`import equinox` pair, the array/pytree product-read fork, and the `DiffStatus.EXACT, accuracy=0, implicit=True` summary tail each live once on the carrier's `gated`/`flatten` and the two rail bodies rather than once per mode helper. `DiffEngine.gated()` imports `jax`/`equinox` once behind the gated band, runs the rail-wide `jax.config.update("jax_enable_x64", True)` float64 promotion (a reverse-mode adjoint pulled back through a `lineax`/`optimistix`/`diffrax` solve assumes float64, and the x32 default silently degrades the implicit-function-theorem gradient), and returns the populated carrier. `_dispatch` is the single owner of `gated()`: it resolves the one `_SPEC[(target, mode.tag)]` row, constructs the carrier in the JAX-requiring arm, and threads the carrier plus that `DiffSpec` into `_transformed` (which reads the spec's `apply` transform selector and `keywords` projection) or `_directional` (which calls the spec's `apply` applicator), so neither rail helper re-imports `jax`, re-reads the table, or runs a second `gated()` beside the dispatch. The JAX flow (mode → spec → transform/applicator → `flatten` → `_summary`) is one integrated rail rather than a flat per-library call. The finite-difference mode and the UNSUPPORTED pairing bypass the carrier entirely, the modes reachable on cp315 where `jax` is absent.
...
# `contribute` — matching the runtime owner's "wraps a ReceiptContributor-returning op". `_dispatch` resolves
# the one `_SPEC` row first: an absent `(target, mode.tag)` cell is the single UNSUPPORTED verdict (a `PYTREE`
# HVP or `PYTREE` finite-difference). One total `match` over the mode tag then selects the rail — the
# finite-difference floor short-circuits to `_finite_difference` BEFORE any carrier build (the one
# cp315-reachable arm where jax is absent), while the transform and directional arms build the single
# `DiffEngine.gated()` carrier here (the gated import + float64 promotion fire once) and thread it plus the
# resolved `spec` into the matching rail so no rail helper re-imports jax or re-reads the table. The
# directional tangent rides through as a pytree (the carried `Pytree` payload), never `np.asarray`-d at the seam.
...
def _dispatch(fn: Callable, x: Pytree, mode: Differentiation, target: DiffTarget, policy: DiffPolicy) -> DiffReceipt:
    if (spec := _SPEC.get((target, mode.tag))) is None:
...
# this body runs. It is the one mode reachable on cp315 where jax is absent: the findiff coefficient surface
...
- [DGSM_SEAM]: the two sensitivity owners stay on disjoint rails. `experiments/study.md#STUDY` owns SAMPLED global sensitivity over SALib — Sobol, Morris, FAST, RBD-FAST, delta, PAWN, and the derivative-based DGSM screen — and its DGSM route is self-contained: `SALIB_ROUTES["dgsm"]` draws the `SALib.sample.finite_diff` perturbation design and reads the `SALib.analyze.dgsm` `dgsm` derivative-based index over it, never a consumer-supplied Jacobian. This owner is the LOCAL autodiff sensitivity floor: a `ForwardJacobian`/`ReverseJacobian` emits the exact per-axis derivative field of one objective at one point through the implicit-adjoint chain, which a derivative-based screen wanting a machine-exact gradient field reads in place of a finite-difference probe — the autodiff derivative is the higher-fidelity reading of the same `∂y/∂x` field SALib's DGSM estimates by sampling. The two never share a wire: this owner exposes a Jacobian a consumer differentiates against, `Study`'s sampled DGSM owns its own `finite_diff` design end-to-end, and the only coupling is the shared `∂y/∂x` concept, not a method call. The `argnums` receipt witness is the differentiated-argument provenance intrinsic to the emitted product — the `int | tuple[int, ...]` index the JAX transform differentiated by — so a `ReverseJacobian` over a multi-argument objective records WHICH inputs the `∂y/∂xᵢ` field is against on the same receipt that carries it, the way the `accuracy` order and the `implicit_adjoint` flag are self-describing rather than reconstructed from the call site. No consumer reads it back to demultiplex a stream; it is the product's own identity, and `Study`'s sampled DGSM neither emits nor reads it.
