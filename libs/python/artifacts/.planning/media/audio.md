...
def _decode_audio(blob: bytes) -> tuple[tuple[Pcm, ...], int]:
    ...
    # sample rate — the standalone primitive the media plane's `timeline`/`analysis` consumers compose (it produces
