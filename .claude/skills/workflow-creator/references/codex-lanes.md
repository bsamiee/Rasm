...
Orchestrator code builds that pair codex-shaped — the leak lives in a shared prompt-builder: one builder feeding both native `agent()` lanes and codex lanes hands codex the Claude register — hostile-stance paragraphs, intensifier stacks, multi-law prose blocks the codex skill's prompt contract bans. Codex obeys that register literally: it over-uses tools, probes out of territory into files the lane never named, and doubles tokens and latency for zero depth gain.
