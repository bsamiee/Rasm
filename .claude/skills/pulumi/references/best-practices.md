...
Program-level law for writing, reviewing, and refactoring Pulumi code: each section carries its detection signals and a rejected/accepted pair. The quick-reference table closes the file.
...
Component law in full — type URNs, `registerOutputs()`, args design, multi-language packaging — is `references/components.md`, routed from the root.
