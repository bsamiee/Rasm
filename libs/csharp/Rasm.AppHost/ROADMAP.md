...
|   [8]   | HybridCache L1 TTL recheck against the injected clock | `curl -sL raw.githubusercontent.com/dotnet/extensions/main/src/Libraries/Microsoft.Extensions.Caching.Hybrid/Internal/DefaultHybridCache.cs` | resource-lanes#CACHE_PORT |
|   [9]   | UDS peer-credential raw-option read on macOS and Linux | `dotnet run` UDS accept spike asserting LOCAL_PEERCRED          | outbound-resilience#DISCOVERY_ATTACH        |
|  [10]   | dump and gcdump tool admission for the process-dump row | `dotnet package search dotnet-gcdump --exact-match`             | support-bundles#CAPTURE_PIPELINE            |
|  [11]   | web app-root static-asset compile under the shared framework | `dotnet build` scratch web root compiling `UseStaticFiles` + `MapFallbackToFile` | host-profiles#PROFILE_AXIS    |
|  [12]   | assay test rail row for the package test project | `uv run python -m tools.assay test run --target Rasm.AppHost.Tests` | every G4 proof below                    |
...
|  [10]   | `Outbound.cs`      | outbound-resilience#HOP_AXIS, #HTTP_PIPELINES, #KEYED_PIPELINES, #DISCOVERY_ATTACH, #OWNERSHIP_LAW      | G3 + G4: admission fold, owner-conflict evidence, breaker enforcement; G5: gates [2] and [9] |
