...
- owner: `artifacts`
- rail: visuals
- version: `4.10.1`; license `MIT` (JetBrains); pure-Python wheel over an embedded Kotlin/JS plot-rendering core (`lets-plot-kotlin` JARs bundled in the wheel — the grammar executes in-process, not in a browser)
- marker: gated `python_version<'3.15'` in the manifest (`artifacts: host-free grammar-of-graphics self-render to svg/png/pdf bytes`); the gated band imports `lets_plot` at module scope on the sub-3.15 worker and renders IN-PROCESS — `PlotSpec.to_*` is pure-Python over the bundled core, so the lets-plot arm does NOT cross the runtime `anyio.to_process` (`.api/anyio.md`) subprocess seam the matplotlib/vegafusion arms take
...
[VISUALS_GGGRAMMAR]:
- import: `from lets_plot import *` (or targeted `from lets_plot import ggplot, aes, ...`) at boundary scope only on the gated `python_version<'3.15'` worker; module-level import on the cp315 core is banned by the manifest import policy and would fail the gate (lets-plot is gated, never imported on the core).
...
- palette axis: `scale_color_manual(values=...)`/`scale_fill_manual(values=...)` are the canonical discrete palette injection — the `figures/color#COLOR` `ColorReceipt.coords` hex array is passed as `values`, threaded onto the chart in the gated band beside its render, never an ad-hoc per-engine color pick.
...
- runtime seam: lets-plot is gated `python_version<'3.15'` and never resolves in the cp315-core process, but unlike the matplotlib/vegafusion arms it renders IN-PROCESS on the gated band — `PlotSpec.to_*` is pure-Python over the bundled Kotlin/JS core, so the lets-plot arm does NOT cross the runtime `anyio.to_process` (`.api/anyio.md`) subprocess seam; the gated-band worker imports `lets_plot` at module scope and self-renders to bytes directly. `geom_livemap` map plots serialize to HTML only (the base-map tiles are HTML-bound).
...
- Accept: grammar-of-graphics charts built and rendered IN-PROCESS on the gated `python_version<'3.15'` worker, self-rendering to SVG/PNG/PDF/HTML bytes through a file-like `path` sink (no subprocess seam, no browser), feeding the `figures/chart` `ChartSpec.LetsPlot` case with the `figures/color#COLOR` palette threaded through `scale_color_manual`/`scale_fill_manual`
