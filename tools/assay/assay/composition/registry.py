...
from assay.composition.store import ArtifactScope, prune_python_artifacts
from assay.core.aspect import checked_call, compose, Layer, logged, RING, Slot, traced
from assay.core.exec import EngineExecutor, Executor
from assay.core.govern import measure, RESOURCE
...
_PYPROJECT: Final[Path] = Path(__file__).resolve().parents[2] / "pyproject.toml"  # the assay member manifest owns the version
...
REGISTRY: Final[tuple[Bind, ...]] = (
    ...
    Bind(Claim.INIT, "python-lib", init_rail.python_lib, InitParams, "Mint a libs/python member: manifest + rasm namespace seat + suite conftest."),
    Bind(Claim.INIT, "python-app", init_rail.python_app, InitParams, "Mint an apps python project and append its workspace member row."),
    Bind(Claim.INIT, "check", init_rail.check, InitParams, "Census gate: every governed-tree manifest is a declared workspace member."),
)
