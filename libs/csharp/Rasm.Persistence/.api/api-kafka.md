...
- native: transitive `librdkafka.redist` (librdkafka LICENSES.txt) ships `runtimes/osx-arm64/native/librdkafka.dylib` plus `osx-x64`/`linux-*`/`win-*`; pulled by `Confluent.Kafka` `include="All"`, P/Invoke-loaded by the client handle
- rail: cdc-egress
...
|  [03]   | `IAsyncSerializer<T>`   | async codec        | `Task<byte[]>` async serialize   |
...
- rail: cdc-egress
- note: rows [01]-[10] configure the builder; [11]-[19] act on the built `IProducer<TKey, TValue>`
...
|  [18]   | `OAuthBearerSetToken(token, lifetimeMs, principal)` | runtime        | feeds minted OIDC token to refresh handler  |
|  [19]   | `OAuthBearerSetTokenFailure(error)`                 | runtime        | signals OAUTHBEARER refresh failure         |
...
|  [06]   | `Committed(partitions, t)` / `Committed(t)` | query          | committed offsets, explicit or all-assigned |
...
- native runtime: the transitive `librdkafka.redist` dependency ships `runtimes/osx-arm64/native/librdkafka.dylib`, P/Invoke-loaded by the client handle; `Library.Version`/`Library.VersionString` report the linked native version and `Library.Load(path)` forces an explicit native load
...
- `IDeserializer<T>.Deserialize` consumes `ReadOnlySpan<byte>` with an `isNull` flag while `ISerializer<T>.Serialize` returns `byte[]`; the async codecs return `Task<byte[]>`/`Task<T>` and the async deserialize side consumes `ReadOnlyMemory<byte>`
...
- Kafka consumers disable `EnableAutoCommit` and commit through `StoreOffset` plus explicit `Commit`, so the committed offset never outruns durable downstream apply; `ConsumeResult.IsPartitionEOF` marks the live edge, `GetWatermarkOffsets`/`QueryWatermarkOffsets` derive consumer lag, and `OffsetsForTimes` resolves a replay cursor from a wall-clock `TopicPartitionTimestamp`.
...
- OIDC credential stacking (control-plane): a SASL/OAUTHBEARER cluster wires `SetOAuthBearerTokenRefreshHandler` so each librdkafka refresh callback mints a fresh token from the admitted `OpenIddict.Client` and feeds it back through `IClient.OAuthBearerSetToken(token, lifetimeMs, principal, extensions?)`, or signals `IClient.OAuthBearerSetTokenFailure(error)` on a mint failure; `IClient.SetSaslCredentials` rotates the credential in place without rebuilding the producer. Refresh handler plus token-set seam is the load-bearing binding between broker security and the runtime token authority.
