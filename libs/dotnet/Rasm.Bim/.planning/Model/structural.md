...
`StructuralProjection` lowers the GeometryGym structural-analysis entity surface onto NEUTRAL shared payloads — the `Map<PropertyName, PropertyValue>` attribute bag a `Relations/relation#EDGE_ALGEBRA` `Relationship.Generic` edge or a structural `Properties/property#PROPERTY_BAG` `PropertySet` node carries — so a `Rasm.Compute` frame solve reads the idealization off the ONE `Graph/element#ELEMENT_GRAPH` `ElementGraph` it already holds, never a second store. `Projection/semantic#SEMANTIC_PROJECTOR` composes it as the deep structural half it keeps out of the general fold.

An idealized analytical line is no payload this reader produces: it content-keys into the member `Object` node's `RepresentationContentHash` under the `Axis` slot, resolved one-hop through the shared `GeometrySource` port. `Model/eurocode#EUROCODE_ALGEBRA` owns the EN 1990 combination algebra and `Exchange/saf#SAF_EXCHANGE` the SAF XLSX codec; both compose this reader's row vocabulary and its ONE magnitude admission rather than minting a second. `SemanticProjector` supplies both regimes — the per-projection `Projection/value#UNIT_INGRESS` `UnitScheme` and the elected `Option<EurocodePolicy>` — so no member re-derives one off its own entity's database.

## [01]-[INDEX]

- [02]-[STRUCTURAL_PROJECTION]: `StructuralRow` the shared row vocabulary with its measure column, `LoadFamily` the closed `IfcStructuralLoad` row set, `RestraintFamily` the release/support policy value, and `StructuralProjection` — polymorphic `Attrs` entity-to-bag lowering, the three-way DOF `Verdict` restraint fold, the stated rigid-end `OffsetOf` vector read, the ONE `Admit` magnitude entry, the load reading, the `AtStart`/`Station` transient discriminants, and the entity-keyed `Author` re-stamp.

## [02]-[STRUCTURAL_PROJECTION]

- Owner: `StructuralProjection` the static structural-analysis reader, producing neutral shared payloads and never a stored record; `StructuralRow` the `[SmartEnum<string>]` page row vocabulary whose key IS the row name and whose `Measure` column fixes the IFC measure identity that row admits under; `LoadFamily` the closed `[SmartEnum<string>]` row set over the `IfcStructuralLoad` subtypes carrying the FE idealization token and the component projection, which `Model/eurocode#EUROCODE_ALGEBRA` and `Exchange/saf#SAF_EXCHANGE` dispatch on through the same generated `Switch`; `RestraintFamily` the policy value carrying one restraint family's row maps beside the `Dofs` and `Consumed` sets derived off them.
- Entry: `Attrs(BaseClassIfc? entity, UnitScheme scale, Option<EurocodePolicy> eurocode, IIfcProfileStore profiles)` lowers every supported structural entity through one `Fin<Map<PropertyName, PropertyValue>>` dispatch; `profiles` is the ONE content-addressed fragment lane the mandatory `IfcRelConnectsWithEccentricity.ConnectionConstraint` geometry preserves through. `AtStart`/`Station` return `Option` discriminants, so unresolved topology emits no assertion. `Author(DatabaseIfc, IfcObjectDefinition, Map<PropertyName, PropertyValue>, Op)` re-stamps the verified restraint and single-force constructors and RETURNS the unconsumed row names as typed residue the egress folds.
- Law: every magnitude leaves its reader model-NATIVE beside the IFC measure type its source attribute declares and crosses ONE `Admit` entry that resolves the frozen `Projection/value#PROPERTY_LOWERING` `MeasureDimensions` row, coerces on it, and traverses `MeasureValue.OfSi` without re-casing its kernel refusal. Independent columns ACCUMULATE — a six-DOF restraint with two malformed springs names both rather than reporting the first.
- Auto: `RestraintOf` takes a `RestraintFamily` POLICY VALUE, so the rel-level member END RELEASE and the connection-level joint SUPPORT land as two families on one bag and neither reads as the other; the `IfcBoundaryNodeConditionWarping` arm precedes its own base and reads the sealed seventh DOF through the ONE `IfcInternals` `[UnsafeAccessor]` capsule [SEALED_PAYLOAD_RULING]. Each DOF row is a THREE-WAY verdict — zero the fixity `Boolean`, positive the SI spring `Measure`, negative-finite or non-finite the typed `KernelFault.OutOfRange` — because a row asserting a support cannot lower from a magnitude the reading never established. The 1D load families SHARE the consumer-neutral `Force`/`Moment` axis rows the `Rasm.Compute` probes read for point AND uniform actions, the family discriminated by the `LoadFamily` row and each component's own IFC measure type, never by the row name.
- Output: payloads land on the ONE shared `ElementGraph` — the six-DOF restraint, frame, supported length, stated rigid-end offset, and `AtStart` on the `IfcRelConnectsStructuralMember` `Generic` edge, the applied load and `Station` on the `IfcRelConnectsStructuralActivity` `Generic` edge, and the group / case / result-group / analysis-model / member definitions on structural `PropertySet` nodes, so the `Rasm.Compute` runner reads support fixity-or-stiffness, end release, rigid-end offset, and load components off the member's incident edges through the SAME `Rasm.Element` `StructuralRows` statics this reader stamps.
- Packages: GeometryGymIFC_Core, Rasm.Element, Rasm, Thinktecture.Runtime.Extensions, LanguageExt.Core
- Growth: a new boundary-condition kind is one arm on `RestraintOf` reading the next subtype's stiffness selects through the SAME `object`-pattern reducer; a new restraint SEMANTICS is one `RestraintFamily` value with no reducer edit; a new applied-load family is ONE `LoadFamily` row, whose addition breaks the eurocode carrier and SAF action switches loudly rather than falling into a catch-all; a new structural entity with a definition bag is one arm on `Attrs`; a new page row is one `StructuralRow` carrying its own measure identity; a new degree of freedom or load axis is one `Properties/property#PROPERTY_BAG` `StructuralRows.Axes` entry at the contract owner every family absorbs; a measure type this reader stamps whose `MeasureDimensions` row is absent lands as one row THERE and none here.
- Boundary: this reader produces ONLY neutral shared payloads — an inline `AxisCurve`/`Vector3` analytical-coordinate field on the graph node is the named contract violation, the `AtStart`/`Station`/`Frame` topology reads being TRANSIENT and emitting Boolean/scalar attributes alone; the entity-to-bag reading is ONE polymorphic `Attrs` and a `RestraintAttrs`/`LoadAttrs`/`GroupAttrs` sibling family the deleted form; the rel-level RELEASE and the connection-level SUPPORT are two row families reduced through one policy value, the `rel.AppliedCondition ?? connection.AppliedCondition` fusion that let a released beam end read as a support being the deleted form; a negative-finite or non-finite stiffness FAULTS on the DOF's own verdict, because a fabricated `Boolean(false)` asserts a free DOF no reading established and no egress filter retracts; every row name resolves to an OWNER-declared static — the cross-package vocabulary through `Rasm.Element` `StructuralRows` and every page row through `StructuralRow`, so a call-site `PropertyName.Create` forks the key space between this writer and its non-referencing `Rasm.Compute` reader, and a call-site re-mint of a family the contract owner already names (`Family("ReleaseTranslation")` beside the owner's own `ReleaseTranslation`) forks CUSTODY even while the bytes still match — the `Family(stem, keys)` mint reaches the page-local families alone; the rigid-end offset is a STATED column read off the eccentricity constraint's own three length measures in the relating member's local axes, never a scalar this reader derives off `SupportedLength` and never a fabricated zero, so a connection declaring no eccentricity carries no offset row and the reader's option refuses on the absence by name; the magnitude roster is MIXED and the split is a ROW property — a row READ off a GeometryGym attribute admits under the measure type that attribute declares, while a row this reader DERIVES or reads off a bare `IfcReal` takes the DIMENSION-ONLY mint and stays dimension-anonymous, because a stamped name is fabricated identity where nothing was ingested; the sealed warping stiffness reads through the ONE capsule and is UNAUTHORABLE, so its support row rides OUTSIDE `RestraintFamily.Consumed` and reaches the egress `FidelityLog` as `Projection/fidelity#FIDELITY_LEDGER` `FidelityDrop.StructuralResidue` rather than re-authoring a stiffness the file never declared.
...
public sealed partial class StructuralRow {
    public static readonly StructuralRow LoadType = new("LoadType", None);
    public static readonly StructuralRow GlobalOrLocal = new("GlobalOrLocal", None);
    public static readonly StructuralRow Source = new("Source", None);
    public static readonly StructuralRow ActionClassRow = new("ActionClass", None);
    public static readonly StructuralRow ImposedCategory = new("ImposedCategory", None);
    public static readonly StructuralRow LoadGroupType = new("LoadGroupType", None);
    public static readonly StructuralRow ActionType = new("ActionType", None);
    public static readonly StructuralRow ActionSource = new("ActionSource", None);
    public static readonly StructuralRow Purpose = new("Purpose", None);
    public static readonly StructuralRow AnalysisTheory = new("AnalysisTheory", None);
    public static readonly StructuralRow IsLinear = new("IsLinear", None);
    public static readonly StructuralRow ResultFor = new("ResultFor", None);
    public static readonly StructuralRow ModelType = new("ModelType", None);
    public static readonly StructuralRow LoadedBy = new("LoadedBy", None);
    public static readonly StructuralRow HasResults = new("HasResults", None);
    ...
    public PropertyName Name => PropertyCategory.Neutral.Row(Key);
...
internal sealed partial class LoadFamily {
    internal static readonly LoadFamily SingleForce = new(nameof(IfcStructuralLoadSingleForce), "point",
        static load => load is IfcStructuralLoadSingleForce f
            ? Seq((StructuralRows.Force["X"], Named<IfcForceMeasure>(), f.ForceX),
                  (StructuralRows.Force["Y"], Named<IfcForceMeasure>(), f.ForceY),
                  (StructuralRows.Force["Z"], Named<IfcForceMeasure>(), f.ForceZ),
                  (StructuralRows.Moment["X"], Named<IfcTorqueMeasure>(), f.MomentX),
                  (StructuralRows.Moment["Y"], Named<IfcTorqueMeasure>(), f.MomentY),
                  (StructuralRows.Moment["Z"], Named<IfcTorqueMeasure>(), f.MomentZ))
            : Empty);
    internal static readonly LoadFamily LinearForce = new(nameof(IfcStructuralLoadLinearForce), "uniform",
        static load => load is IfcStructuralLoadLinearForce l
            ? Seq((StructuralRows.Force["X"], Named<IfcLinearForceMeasure>(), l.LinearForceX),
                  (StructuralRows.Force["Y"], Named<IfcLinearForceMeasure>(), l.LinearForceY),
                  (StructuralRows.Force["Z"], Named<IfcLinearForceMeasure>(), l.LinearForceZ),
                  (StructuralRows.Moment["X"], Named<IfcLinearMomentMeasure>(), l.LinearMomentX),
                  (StructuralRows.Moment["Y"], Named<IfcLinearMomentMeasure>(), l.LinearMomentY),
                  (StructuralRows.Moment["Z"], Named<IfcLinearMomentMeasure>(), l.LinearMomentZ))
            : Empty);
    internal static readonly LoadFamily PlanarForce = new(nameof(IfcStructuralLoadPlanarForce), "point",
        static load => load is IfcStructuralLoadPlanarForce p
            ? Seq((StructuralRows.PlanarForce["X"], Named<IfcPlanarForceMeasure>(), p.PlanarForceX),
                  (StructuralRows.PlanarForce["Y"], Named<IfcPlanarForceMeasure>(), p.PlanarForceY),
                  (StructuralRows.PlanarForce["Z"], Named<IfcPlanarForceMeasure>(), p.PlanarForceZ))
            : Empty);
    internal static readonly LoadFamily Temperature = new(nameof(IfcStructuralLoadTemperature), "point",
        static load => load is IfcStructuralLoadTemperature t
            ? Seq((StructuralRows.DeltaT["Constant"], Named<IfcThermodynamicTemperatureMeasure>(), t.DeltaT_Constant),
    ...
    internal static readonly LoadFamily Configuration = new(nameof(IfcStructuralLoadConfiguration), "trapezoid",
        static load => load is IfcStructuralLoadConfiguration cfg && Ramp(cfg) is { Count: >= 2 } ramp
            ? Seq((StructuralRows.Start["X"], Named<IfcLinearForceMeasure>(), ramp[0].Force.LinearForceX),
                  (StructuralRows.Start["Y"], Named<IfcLinearForceMeasure>(), ramp[0].Force.LinearForceY),
                  (StructuralRows.Start["Z"], Named<IfcLinearForceMeasure>(), ramp[0].Force.LinearForceZ),
                  (StructuralRows.End["X"], Named<IfcLinearForceMeasure>(), ramp[^1].Force.LinearForceX),
                  (StructuralRows.End["Y"], Named<IfcLinearForceMeasure>(), ramp[^1].Force.LinearForceY),
                  (StructuralRows.End["Z"], Named<IfcLinearForceMeasure>(), ramp[^1].Force.LinearForceZ),
                  StructuralRow.SpanStart.Cell(ramp[0].At), StructuralRow.SpanEnd.Cell(ramp[^1].At))
            : Empty);
    ...
    internal partial Seq<(PropertyName Name, Option<string> Measure, double Native)> Vectors(IfcStructuralLoad load);
...
public static class StructuralProjection {
        ...
        entity switch {
            ...
            IfcStructuralAnalysisModel model =>
                ...
                select Seq(
                    ...
                    .Fold(Map((StructuralRow.ModelType.Name, Enumerated(model.PredefinedType.ToString(), ModelKinds))),
    ...
    private static Seq<(PropertyName Name, Option<string> Measure, double Native)> OffsetOf(
        ...
        (relation as IfcRelConnectsWithEccentricity)?.ConnectionConstraint is IfcConnectionPointEccentricity e
            ? Seq((StructuralRows.Offset["X"], Named<IfcLengthMeasure>(), e.EccentricityInX),
                  (StructuralRows.Offset["Y"], Named<IfcLengthMeasure>(), e.EccentricityInY),
                  (StructuralRows.Offset["Z"], Named<IfcLengthMeasure>(), e.EccentricityInZ))
            : Seq<(PropertyName, Option<string>, double)>();

    // --- [RESTRAINT]

    private static Fin<Map<PropertyName, PropertyValue>> RestraintOf(
        IfcBoundaryCondition? condition, RestraintFamily family, UnitScheme scale) => condition switch {
    ...
    private static Fin<Map<PropertyName, PropertyValue>> Frame(IfcAxis2Placement3D? system) =>
        ...
        && ratios.ForAll(double.IsFinite)
            ? ratios.Traverse(ratio => (PropertyValue.Of(new PropertyValue.Number(ratio))).ToValidation()).As().ToFin()
    ...
    private static Fin<Map<PropertyName, PropertyValue>> SixDof(
        ...
        Option<string> translationMeasure, Option<string> rotationMeasure, RestraintFamily family, UnitScheme scale) =>
        Seq(
    ...
    private static Fin<(PropertyName Name, PropertyValue Value)> Verdict(
        ...
        reading.Native switch {
            ...
            _ => new KernelFault.OutOfRange(
                        Label: name.ToString(),
                        Scalar: reading.Native,
                        Requirement: "be non-negative and finite"),
        };
    ...
    private static Fin<(Dimension Signature, Option<QuantityType> Type, double Si)> Resolve(
        Option<string> measure, double native, UnitScheme scale) =>
        measure.Match(
            Some: type => PropertyLowering.MeasureDimensions.TryGetValue(type, out Dimension row)
                ? Fin.Succ((Signature: row, Type: Some(QuantityType.Create(type)), Si: PropertyLowering.Coerce(scale, native, type, row, null)))
                : Fin.Fail<(Dimension Signature, Option<QuantityType> Type, double Si)>(
                    new BimFault.Refused(BimScope.Model, BimReason.Unmapped,
    ...
    internal static PropertyValue Enumerated(string selected, Seq<string> allowed) =>
    ...
    private static Fin<Map<PropertyName, PropertyValue>> LoadOf(
        IfcStructuralActivity? activity, UnitScheme scale, Option<EurocodePolicy> eurocode) =>
        Optional(activity).Bind(static candidate => Optional(candidate.AppliedLoad).Map(load => (Activity: candidate, Load: load)))
            .Bind(static pair => LoadFamily.Of(pair.Load).Map(family => (pair.Activity, pair.Load, Family: family)))
            .Match(
                Some: pair => {
                    ...
                    return from measures in Measures(pair.Family.Vectors(pair.Load), scale)
                           ...
                           select Map(
                               ...
                               .AddRange(row.Imposed.Map(static category =>
                                   (StructuralRow.ImposedCategory.Name, (PropertyValue)new PropertyValue.Text(category.ToString()))).ToSeq())
                               .AddRange(measures)
                               .AddRange(factors);
                },
                None: static () => Fin.Succ(Map<PropertyName, PropertyValue>()));
    ...
    private static Fin<Map<PropertyName, PropertyValue>> GroupOf(
        ...
        from combination in group.PredefinedType == IfcLoadGroupTypeEnum.LOAD_COMBINATION_GROUP
            ...
            : Fin.Succ(Map<PropertyName, PropertyValue>())
        select Map(
            (StructuralRow.LoadGroupType.Name, Enumerated(group.PredefinedType.ToString(), LoadGroupKinds)),
            (StructuralRow.ActionType.Name, (PropertyValue)new PropertyValue.Text(group.ActionType.ToString())),
            (StructuralRow.ActionSource.Name, new PropertyValue.Text(group.ActionSource.ToString())))
            .AddRange(Optional(group.Purpose).Map(static purpose =>
                (StructuralRow.Purpose.Name, (PropertyValue)new PropertyValue.Text(purpose))).ToSeq())
            .AddRange(measures)
    ...
    private static Fin<Seq<PropertyName>> Consume(
        Map<PropertyName, PropertyValue> attrs, Seq<PropertyName> names, Action stamp) =>
        Try.lift(() => {
                stamp();
    ...
    internal static double Si(Map<PropertyName, PropertyValue> attrs, PropertyName name) =>
