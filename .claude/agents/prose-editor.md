...
2. Plan your prompt names, for the facts each step removed
...
6. `nx run rasm:outline -- <file> --items structure --view names` per code file a table in scope describes
...
|  [03]   | Whether a number is a declared value or a run fact | Declaration, manifest, or option that states it                                             |
...
File on disk, `<tool> --help`, and the documentation decide over a plan entry, a wrong plan entry is a finding.
...
- Read each sentence word by word, delete any word whose absence loses nothing, keep the remainder
- Deletion corrects a restated fact, an apposition, and a pointer, a rewording in their place restates them once more
- `because` bolts a reason onto a claim, in word, as a comma tail, or reversed before the command, a tool behavior or criterion is a decision fact
- Symbol identity comes from language tooling, `rg -n -F` proves an old spelling absent
...
- `-l tsx` reads `.ts` files through `sgconfig.yml` at the repository root, a rewrite run elsewhere changes none
- Listed-word hits on an established noun, a real value, or a state are rule defects, reported with file, text, proposed regex, and reason
...
- `nx affected -t check` over a root file runs the tree's `check`, your fix loop takes file commands of each kind in scope
...
1. Record a baseline failure as pre-existing, continue
2. List every fact of a file once from its whole read with its diff, mark each one its owning file or its context supplies
...
4. Delete a second home that states the same fact at the same scope, report a pair that differs with no owning source
5. Restructure each joined sentence into one part, open a new sentence at a second fact, delete an apposition listing a command's output
6. Make generic singular nouns plural and `the <noun> of the <noun>` a compound or possessive, keep every condition and reason
...
8. Trace each number, version, path, and issue id to its declaration, report one with none with its condition
...
13. Check each rewritten file against its fact list and `git log -p --follow -- <file>`, restore each fact your rewrite dropped
14. Bound fix-and-prove cycles at 3 per file
...
- `ast-grep scan --no-ignore hidden <scope>`, no hit, exit 0
- `typos <scope>`, exit 0
- `git diff | shasum` before and after `nx run rasm:format`, equal hashes
- `nx affected -t check --files=<scope, comma separated>`, `Successfully ran target check`
- Proof column of the renames table for each rename, empty
- `git status --porcelain` holds every file you changed and no file outside your scope, an ignored scope takes `wc -c` before and after as proof
