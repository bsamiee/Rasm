...
lazy from artifacts.graphic.marks.decode import DecodeSource, _zxing_decode
            ...
            case MarkOp(tag="decode", decode=(source, scope)):
                match source:
                    case DecodeSource(tag="raster"):
                        try:
                            fact = Ok(await to_process.run_sync(_zxing_decode, source, scope, limiter=_MARK_LANE))  # untrusted bytes crash-isolated
                        except BrokenWorkerProcess as broken:
                            fact = Error(MarkFault(worker=type(broken).__name__))
                    case DecodeSource(tag="pixels"):
                        fact = Ok(await to_thread.run_sync(_zxing_decode, source, scope, limiter=_MARK_LANE))  # trusted numpy frame shares the address space, no pickle
                    case _ as unreachable:
                        assert_never(unreachable)
            case _ as unreachable:
                assert_never(unreachable)
...
flowchart LR
    ...
    One -->|"decode Raster source (to_process)"| Worker["to_process.run_sync(_zxing_decode, source, scope)"]
    One -->|"decode Pixels source (to_thread)"| Direct["to_thread.run_sync(_zxing_decode, source, scope)"]
