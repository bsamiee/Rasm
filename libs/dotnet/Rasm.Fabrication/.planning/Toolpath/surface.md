    ...
    private static Fin<Option<SurfaceDriveSet>> Laid(
        ...
        kind.Switch(
            ...
            kernel: (state, row) => policy.Layout
                ...
                .Bind(layout => Try.lift(() => layout(state.Mesh, row, state.StepOver)).Run().Bind(static inner => inner)))
...
internal static class OpenCamLib {
    ...
    internal static Fin<SurfaceSample> Position(SurfaceRun run) =>
        Try.lift(() => PositionNative(run)).Run().Bind(static inner => inner);
    ...
    private static Fin<SurfaceSample> Region(SurfaceRun run, OpenCamBinding binding, SurfaceStrategy.Raster raster) =>
        Unit(
            ...
            operation => raster.Region.Fold(
                ...
                (result, corner) => result.Bind(_ => Gate(run, () => OpenCamNative.OperationAppendPoint(operation, corner.X, corner.Y, corner.Z)))),
            ...
            .Bind(set => Seq(
                ...
                .Traverse(batch => Unit(
                    ...
                    operation => batch.Drives.Fold(
                        ...
                        (result, drive) => result.Bind(_ => Gate(run, () => OpenCamNative.OperationAppendFiber(
                            operation,
                            drive.Points[0].X,
                            drive.Points[0].Y,
                            drive.Points[0].Z,
    ...
    private static Fin<int> Gate(SurfaceRun run, params ReadOnlySpan<Func<int>> steps) =>
        toSeq(steps.ToArray()).Fold(
            Fin.Succ(0),
            (result, step) => result.Bind(_ => step() is var status && status == 0
                ? Fin.Succ(status)
...
OCL_SHIM_EXPORT int ocl_op_get_bucket_size(void* op) {
    return op == nullptr ? kBadHandle : Op()->bucketSize;
}
...
OCL_SHIM_EXPORT int ocl_op_loop_count(void* op) {
    return op == nullptr ? 0 : static_cast<int>(Op()->groups.size());
}
...
OCL_SHIM_EXPORT int ocl_op_fiber_count(void* op) {
    return op == nullptr ? 0 : static_cast<int>(Op()->fiberRows.size());
}
...
OCL_SHIM_EXPORT int ocl_op_witness_count(void* op) {
    return op == nullptr ? 0 : static_cast<int>(Op()->witnesses.size());
}
...
OCL_SHIM_EXPORT void ocl_op_destroy(void* op) { delete Op(); }
