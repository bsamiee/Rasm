...
- [1]-[MATERIAL_COMPOSITION]: `BimMaterial` record with its `Of(MaterialRow)` flat-name seam, the `BimMaterialComposition` `[Union]` (`Single`/`LayerSet`/`ProfileSet`/`ConstituentSet`), the `MaterialLayer`/`MaterialProfile`/`MaterialConstituent` rows, the `MaterialProjection.Project`/`ProjectAll` fold from the live `IfcMaterialSelect` surface, and the `BimElement.Materials` typed binding.
