...
public static partial class EnergySimulation {
    ...
    // the opening edges). IfcWindow -> FixedWindow, else Door; the glazing/opaque construction builds through the SAME
        ...
        graph.OpeningsOf(spaceId, hostIdentifier).Bind(opening =>
            geometry.Footprint(opening.Representations).Match(
                Some: ring => {
                    ...
                    sub.setSurface(host);
                    sub.setSubSurfaceType(opening.Classification.Code == EnergyGraphReads.WindowClass ? "FixedWindow" : "Door");
