...
|  [03]   | `LoadType(string) -> Type?`                                     | instance | typeless type-header resolution hook           |
|  [04]   | `ThrowIfDeserializingTypeIsDisallowed(Type)`                    | instance | typeless deserialization allowlist hook        |
...
- `ObjectStore`(`Rasm.Persistence/.api/api-objectstore.md`): `MessagePackStreamReader.ReadArrayAsync` is the framed-ingest reader over a length-delimited multi-snapshot blob body, yielding one sequence per element under the codec's own `SequencePool`.
...
- Only a type carrying `[MessagePackObject]` or `[MessagePack.Union]` enters a row — negotiation tests the attribute before admission, so an element graph without it routes to its own codec; stored binary payloads carry codec, schema, compression, and redaction class in their own metadata.
- JSON projection is diagnostic output and never owns a payload — the human-readable JSON peers are the folder JSON codecs, and `ConvertToJson`/`SerializeToJson` stay diagnostic bridges.
