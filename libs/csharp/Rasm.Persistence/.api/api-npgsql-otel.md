...
- Query-observability depth rides the `pg_stat_statements`/`pg_stat_io` harvest receipts under the `store.stat.statements`/`store.stat.io` slots, so this package owns only the driver meter/span subscription, never the query-stats surface; `auto_explain` is server-log posture the provisioning roster owns, outside the managed rail.
- Because `NpgsqlMetricsOptions` is knob-free, all bucketing/cardinality control is expressed as OpenTelemetry meter-view rows on the `MeterProviderBuilder`; the estate composition subscribes the `Npgsql` meter by name, `AddNpgsqlInstrumentation` the equivalent package spelling whose knob-free delegate adds nothing over it.
