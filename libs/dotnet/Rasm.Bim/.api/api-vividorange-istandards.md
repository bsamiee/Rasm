...
- with `LanguageExt.Core`: `MissingNationalAnnexException` and the `Cases` `NotImplementedException` are captured by `Op.Catch` as exact exceptional errors, so the Eurocode dispatch never throws across the Bim fold or remints a message
