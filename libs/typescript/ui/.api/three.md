...
|  [07]   | `Plane` / `Line3` / `Triangle`                                       | math primitive  | `viewer/scene/glb` — section cut, measure span |

- [05]-[DRAW_ORDER]: `BufferAttribute`/`InterleavedBufferAttribute` share the `getX`/`getY`/`getZ` component readers, so a fold over positions never re-derives a stride the parse owns, and `attribute.setUsage(DynamicDrawUsage)` marks a buffer the CPU rewrites per frame. `geometry.setIndex(attribute)` selects the indexed buffer renderer BEFORE the primitive-mode branch, so the index governs draw ORDER for `Points` exactly as it does for `Mesh` — which is what makes a per-view back-to-front reorder an index rewrite rather than a position shuffle.
- [07]-[SECTION_PRIMITIVES]: `Plane` carries `normal` + `constant` and is built from `setFromNormalAndCoplanarPoint(normal, point)` or `setFromCoplanarPoints(a, b, c)`; `intersectsBox(Box3)` is the boolean a BVH bounds callback prunes on, and `intersectLine(Line3, target, clampToLine = true)` writes the crossing into `target` and returns `null` where the segment misses — the pair a triangle-to-plane cut is assembled from, since three ships no triangle-plane segment member.
...
- [01]-[WIDENING_COPY]: promoting a parsed `MeshStandardMaterial` to `MeshPhysicalMaterial` has exactly ONE working idiom, and the two the API appears to offer both destroy the target. `physicalTarget.copy(standardSource)` THROWS — the physical override writes `undefined` across the scalar lobes from the source's absent fields and then dereferences its absent `clearcoatNormalScale`, leaving the target half-corrupted. `physicalTarget.setValues(standardSource)` completes but blind-assigns `type` and `uuid` off the source and ALIASES its `defines` object, so the target reports `MeshStandardMaterial` (which is what program routing reads) and loses `PHYSICAL`. Only `MeshStandardMaterial.prototype.copy.call(physicalTarget, standardSource)` works: it carries every shared property, leaves the physical lobes at constructor defaults, and preserves `uuid`, `type`, and `isMeshPhysicalMaterial` — followed by a MANDATORY `target.defines = { STANDARD: '', PHYSICAL: '' }`, because that copy assigns a fresh `{ STANDARD: '' }` and `PHYSICAL` is what gates `#define IOR` and `USE_SPECULAR` in the shipped program. `copy.call` also transfers the source's texture handles, so the displaced source retires through `dispose()` alone and never a slot sweep.
- [03]-[PREMULTIPLY_SCOPE]: `Texture.premultiplyAlpha` is a DOM-SOURCE knob. WebGL sets `UNPACK_PREMULTIPLY_ALPHA_WEBGL` from it, and that pixel-store has no effect on `ArrayBufferView` uploads; WebGPU honours it only in `_copyImageToTexture`, the `copyExternalImageToTexture` path, while the `DataTexture` branch rides `queue.writeTexture`, which carries no premultiply parameter. `DataTexture`, transcoded `CompressedTexture`, and `KTX2Loader` payloads therefore ignore the flag SILENTLY on both backends, and three warns nothing — including the loader's own stamp off the KTX2 DFD alpha-premultiplied flag. Premultiplication for data-texture payloads belongs to the producer; the consumer's blend-side fact is `Material.premultipliedAlpha`, which selects the blend factors and is also what `SubtractiveBlending`/`MultiplyBlending` require.
- [04]-[WORKING_SPACE]: `ColorManagement.spaces` registers exactly TWO — `srgb-linear` (the default `workingColorSpace`) and `srgb`. `define(spaces)` is a bare `Object.assign` onto that roster with NO validation, so a custom space installs silently, but a TEXTURE tagged with one does not survive upload: WebGL's `verifyColorSpace` hard-`error()`s on any colour space that is neither `LinearSRGBColorSpace` nor `NoColorSpace` and whose transfer is not `SRGBTransfer`, and the WebGPU path shader-converts none of them; float and half-float uploads receive no conversion at all. Producers therefore convert a non-Rec.709 primaried plane before upload and tag it `LinearSRGBColorSpace` here. Custom spaces reach only `Color`-object math through `convert`/`colorSpaceToWorking` and an output transform via `outputColorSpaceConfig`, never a texture tag.
...
- [04]-[NO_REBIND]: `detectSupport` only writes the instance's `workerConfig` field. That config is read at WORKER CREATION and posted into each worker's `init` message, and the `WorkerPool` creates workers lazily then caches them, so a re-call after the first parse leaves already-spawned workers on the old target and new ones on the new — a split-brain, never a rebind. `dispose()` compounds it: it disposes the pool and revokes the worker blob URL while leaving `transcoderPending` resolved, so a disposed instance handed another buffer spawns workers against a revoked URL. Consumers therefore mint a FRESH loader on backend swap or context loss and call `detectSupport` on the new renderer before its first load; the loader also warns when a second instance loads the transcoder, which is why the fresh mint pairs with disposing the displaced one.
...
|  [10]   | `pass(scene, cam)` / `.setMRT(mrt(...))` / `.getTextureNode(n)` / `output` | post / MRT       | `viewer/probe` — capture pass |
...
- [12]-[COMPUTE_READBACK]: `StorageBufferNode` carries the `StorageBufferAttribute`/`StorageInstancedBufferAttribute` its constructor wrapped AS `.value`, so the readback handle is the node itself and no parallel attribute roster exists. `getArrayBufferAsync(attribute, target = null, offset = 0, count = -1)` accepts ANY buffer attribute — every attribute path grants `COPY_SRC`, so a storage-instanced or plain instanced attribute needs no second call shape — and its one gate throws when `offset` or `count` is not a multiple of four. `target` has three arms: `null` allocates a fresh `ArrayBuffer` per call, a `ReadbackBuffer(maxByteLength)` binds a persistent mapped GPU buffer whose `.buffer` holds the mapped data and which THROWS `ReadbackBuffer must be released before being used again` unless `.release()` ran since the last read (`.dispose()` destroys it; `info.memory.readbackBuffers`/`readbackBuffersSize` count the live set), and a plain `ArrayBuffer` is written into in place and returned — the arm that keeps a per-frame pass allocation-free with no cross-frame release obligation. `instancedArray(count | TypedArray, type)` handed a typed array wraps THAT array, so a CPU-side write into it with `attribute.needsUpdate = true` IS the upload — no second staging copy.
...
|  [03]   | `renderer.getPixelRatio()` / `.readRenderTargetPixelsAsync(rt, ...)` | frame readback    | `viewer/probe` — pixel readback         |
...
- `wire#vocab` appearance (`dotnet:Rasm.Materials/Appearance/interchange` contract): `viewer/scene.md` [08] consumes generated `appearance.Material` and `Set` beside element `AppearanceWire`, resolves every plane through the complete `PlaneRef.artifact`, and binds the decoded values onto `MeshPhysicalMaterial`; a peer TypeScript OpenPBR algebra is the `CROSS_LANGUAGE_WIRE` drift defect.
...
- Widen a standard material to physical through `MeshStandardMaterial.prototype.copy.call(target, source)` with the `{ STANDARD, PHYSICAL }` re-stamp — the widening `copy` and `setValues` are both foreclosed by [01]-[WIDENING_COPY].
