...
- Auto: the fold is total and effect-free — closed-form astronomy over finite admitted input carries no `Fin` result; `SolarSite` ACCUMULATES its latitude and elevation clauses so a caller learns both, types its timezone as a NodaTime `Offset` whose own range forecloses the hand guard, and CANONICALIZES longitude into `[-180, 180)` by `ref`, so one meridian has one spelling and `179.9` and `-180.1` are not two values one wrap apart.
