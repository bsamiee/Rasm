...
class Analysis(Struct, frozen=True):
    ...
    async def produce(self, /) -> RuntimeRail[Result[tuple[ContentKey, ArtifactReceipt], MediaFault]]:
        return await async_boundary(f"media.analysis.{self.op.tag}", self._emit)
...
- [FRAME_METADATA_UNREACHABLE] [RESOLVED]: PyAV 17.1.0 exposes NO `.metadata` attribute on `VideoFrame`/`AudioFrame` (the `av/video/frame.pyi` and `av/audio/frame.pyi` carry only `to_ndarray`, and a constructed frame's `hasattr(frame, "metadata")` is `False`), so the metadata-injecting filters — `silencedetect` (`lavfi.silence_start`/`silence_end`), `scdet` (`lavfi.scd.score`), `astats` (`metadata=1`) — cannot surface their results through the frame API. The design routes each measurement only through a value-surfacing native path: `showwavespic`/`showspectrumpic` produce an image `VideoFrame` (pulled at the sink), `av.filter.loudnorm.stats` returns a JSON `bytes`, `select='gt(scene,t)'` EMITS only the cut frames (verified: one hard cut yielded exactly one emitted frame at `gt(scene,0.3)`, counted at the sink with its `pts`), and `thumbnail` picks representative frames — no metadata read anywhere. `Silence` therefore has an EMPTY `_NATIVE` set and is always the numpy RMS-threshold substitute; this is the honest capability contract, not the illusory "route to silencedetect when present" the naive map assumed.
- [LOUDNESS_NATIVE] [RESOLVED]: the EBU R128 integrated-LUFS/true-peak-dBTP/loudness-range band is `av.filter.loudnorm.stats(loudnorm_args: str, stream: AudioStream) -> bytes` — verified exported at `av.filter.loudnorm.stats`, a two-pass measurement that appends `:print_format=json` and returns the `input_i`/`input_tp`/`input_lra`/`output_*` JSON, exactly the gated loudness the single-pass `media/audio#MEDIA` encode cannot expose (the deferral `audio.md` recorded). The op hands `reader.streams.audio[0]` straight to `stats` and `msgspec.json.decode`s the three scalars onto the `analysis` facts band. The substitute is a numpy RMS-dBFS mean marked `gated: false` — an approximate level, never claimed as true LUFS, since K-weighting-plus-gating is exactly what the filter owns.
- [AUDIO_VISUAL_NATIVE] [RESOLVED]: `showwavespic`/`showspectrumpic` are audio-input/video-output filters — verified that `add_abuffer(sample_rate=, format="fltp", layout="stereo") -> showwavespic(s=WxH) -> buffersink` configures and that a decoded `AudioFrame` pushed through yields one image `VideoFrame` pulled at the sink, `to_ndarray(format="rgba")` the render. The substitute is the numpy envelope (`np.abs` peak, `np.maximum.reduceat` windowed RMS) and the framed-`rfft` STFT magnitude grid, colormapped through `_colormap` and encoded by `graphic/raster/io#IO` `render_png`. CROSS-BRANCH (RESIDUAL): `compute/analysis/signal#DSP` `Signal.apply(SignalOp.Spectral(time_frequency=True))` is the scipy-backed spectrogram the substitute stands in for, and `compute/analysis/transform#TRANSFORM` `SpectralReadout.CENTROID` the pitch/centroid seam — both verified present at `compute/.planning/analysis/{signal,transform}.md`, composed as the richer cross-branch depth behind the numpy floor, not re-imported into this worker.
- [SCENE_AND_THUMB] [RESOLVED]: scene detection routes native to `select='gt(scene,{threshold})'` (metadata-free — each emitted frame IS a cut, counted with `float(frame.pts * frame.time_base)`) and substitute to `graphic/raster/measure#MEASURE` `structural_similarity` on adjacent decoded pairs (a cut where SSIM `< 1 - threshold`), the SSIM owner composed at the wire, never a re-imported `skimage.metrics`; thumbnail routes native to the `thumbnail` filter's representative-frame pick and substitute to a numpy per-stratum `np.var` pick, both assembled into one contact-sheet through `graphic/raster/io#IO` `montage` + `render_png`. All native filters (`showwavespic`/`showspectrumpic`/`loudnorm`/`select`/`thumbnail`) verified present in the admitted build's 448-filter `av.filter.filters_available`; the `_NATIVE <= media_filters` subset test is the capability route, so a limited wheel degrades to the numpy/measure floor with the same evidence shape. CROSS-FILE (RESIDUAL): `graphic/raster/io#IO` must expose `render_png`/`montage`, `graphic/raster/measure#MEASURE` a `frame_similarity` seam, `media/filtergraph#FILTER` the `media_filters` probe.
- [RECEIPT_MEDIA_CASE] [RESOLVED]: every op contributes the EXISTING `ArtifactReceipt.Media(key, container, codec, duration, bytes, frames, bit_rate=0, facts)` — `receipt.md` line 14 already names the `analysis` scene-cut/silence-span facts band, so the loudness LUFS/true-peak/LRA, the waveform peak/RMS, and the spectral centroid ride the SAME band with ZERO receipt edit (the anticipatory collapse the shared `Media` case was shaped for). The `AnalysisEvidence` carrier is analysis-owned and never crosses into the receipt owner, which imports no `av` handle. The image arms key a PNG blob, the measurement arms a compact `msgspec.json` facts blob, both through `ContentIdentity.of(...)`, entering `core/plan#PLAN` `ArtifactPipeline` as one `ArtifactWork` node whose `parents` are the source container's content key. No parallel analysis-receipt rail, exactly as `container`/`audio`/`subtitle` contribute the one `Media` case.
