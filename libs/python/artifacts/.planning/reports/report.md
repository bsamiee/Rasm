...
- Receipt: each render contributes `receipt/receipt#RECEIPT` `ArtifactReceipt.Report` keyed by the content key over the rendered tree; the notebook kind additionally keys the executed-notebook archive.
...
- Boundary: a hand-rolled section-composition loop is the deleted form; the report binds figure content keys produced by `figures/chart#CHART`, `figures/table#TABLE`, and `figures/scene#SCENE`, never re-rendering them. Notebook conversion to PDF/HTML hands the rendered tree to the document axis; no live kernel server, no UI.
...
def _execute_notebook(source: str, parameters: dict[str, object]) -> bytes:
    ...
    executed = papermill.execute_notebook(source, None, parameters=parameters, engine_name=None, progress_bar=False, log_output=False)
    sink = BytesIO()
