...
- Exemption: the registrar and observer callbacks are the platform-forced statement boundary — emissions are `void`-discarded inside them; the sample projections beside them are the marked admission kernel for the untyped `PerformanceEntry` records and the library's own `Metric` union.
