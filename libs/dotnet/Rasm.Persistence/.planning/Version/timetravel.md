...
public readonly record struct Reconstruction(TimeCut Cut, long Version, long EventsToCut, Option<long> SnapshotFloor, UInt128 Address);
...
public sealed record TimeLog(
    ...
    ModelId Model, ProjectionContext Frame);

// --- [OPERATIONS] ----------------------------------------------------------------------
public static class TimeTravel {
    public static IO<Reconstruction> Reconstruct(AsOfQuery query, TimeLog log) => Folded(query, log).Map(static fold => fold.Settled);
...
|  [08]   | edge key         | upstream `ContentAddress.Of(edge, tolerance)` streaming     | the retired `ToCanonicalBytes` twin materialized bytes  |
|  [09]   | bisect descent   | `O(log n)` probes of a MONOTONE predicate                   | three-case verdict; the bound is never folded           |
|  [10]   | frame address    | upstream `GraphMembers` step + `OfGraph(members)`           | byte-identical to the recompute; reheader re-seeds      |
