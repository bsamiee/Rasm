...
[STACKING]:
- `Wacton.Unicolour`(`libs/dotnet/.api/api-unicolour.md`): `ArtistPaint.All` pigment reflectance feeds the main owner's `Unicolour(Configuration, Pigment[], double[])` Kubelka-Munk mix under `ArtistPaint.Configuration`, and `Macbeth.All` patches feed `Difference(Unicolour, DeltaE.Ciede2000)` — this package supplies the measured tables, the main owner runs every mix, difference, and conversion.
