...
- [04]-[CNPG_CLUSTER]: the admission boundary, operator charts, per-scope custody, pooler, backups; `Postgres`.
- [05]-[APP_FINALIZE]: data-tier escalation, empty Database targets, and the replication interface; `Postgres`.
...
- Growth: a leaf-node or gateway topology axis is one values group when a multi-site deployment earns it; a TLS listener row composes the `Certs` chain when the origin leaves the cluster.
...
- Law: `admit` is the one entry — the extension matrix, the pooling axis, and every realized scope's recovery source prove on the typed `DataRefused` channel before a chart is declared, each fault naming its axis; a refused spec never half-constructs a tier and construction-time refusal has no spelling left on this page.
...
- Law: `_granted` refuses unknown extensions and ungranted dependency capabilities on the fault channel before any target reaches the provider.
...
- Law: the replication interface is the typed static pair — `Postgres.publication(name, { cluster, database, target }, child)` and `Postgres.subscription(name, { cluster, database, publication, external }, child)` construct the CNPG `Publication`/`Subscription` CRs for the multi-region or tenant-migration deployment; the pair is dormant capability with a typed spelling, so a replication topology is rows at the composition site, never a tier rewrite.
