...
[NOTICE_PROJECTION]-[COMPLETE]: landed as `delivery/notice.md` — `TransmittalNotice.projections`/`announce` over the runtime message envelope algebra, with `subject` minted over the encoded payload and `dataschema`/`partitionkey`/`sequence`/`authcontext`/`correlation`/`dataclassification`/`severity` derived off the fired identities; `NoticeBinding` deletes whole (its `lower` member shadowed `str.lower`, so `NoticeBinding.BINARY.lower()` raised), as do `NoticeWire` and the frozen struct that held a mutable unhashable event.
