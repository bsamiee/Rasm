...
- Growth: a new authoring capability is one `AuthorVerb` row and one `IfcApiVerb` table row and reuse of the matching `AuthorPayload` case; a new capability dimension is one `Capability` member the `in row.cap` tests pick up without a new column; a new argument shape is one `AuthorPayload` case with its `to_kwargs` projection. Zero new dispatcher, no per-usecase method.
