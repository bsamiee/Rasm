...
- metadata axis: `Pdf.open_metadata` yields a `PdfMetadata` mapping with `load_from_docinfo` (legacy `/Info` -> XMP migration), `pdfa_status`/`pdfx_status` conformance probes, and namespace registration; `Pdf.docinfo` is the raw `/Info` dictionary, never a parallel metadata model. `open_metadata(set_pikepdf_as_editor=False, update_docinfo=False)` is the read-only audit form that never mutates the bytes it inspects. Decode trap: an XMP struct bag under `parseType="Resource"` — the PDF Declarations `pdfd:declarations` bag — defeats the `PdfMetadata` mapping view (the dict read yields whitespace), so its member URIs (`pdfd:conformsTo`) read off the raw `/Metadata` stream bytes via an element-tree parse, never the mapping view.
