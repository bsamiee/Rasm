...
- Cases: ingest-source {`Stems` (a scanned directory's probes), `Declared` (a C#-side manifest re-admitted through the same alias law), `Peer` (the decoded python-minted `AssetSetManifest` — the `tests/contracts/MANIFEST.md` `[02.18]` classification-input consumer arm)}.
