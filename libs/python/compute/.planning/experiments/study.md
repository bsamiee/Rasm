...
- Packages: `SALib` (`ProblemSpec` fluent `sample`/`set_samples`/`set_results`/`analyze` with the `samples`/`analysis` accessors, the `SALib.sample.<morris|sobol|fast_sampler|finite_diff|latin>` and `SALib.analyze.<morris|sobol|fast|rbd_fast|delta|pawn|dgsm|hdmr>` submodules resolved through `importlib.import_module`), `scipy` (`stats.qmc.LatinHypercube`/`Sobol`/`Halton` with the SPEC-007 `rng` constructor keyword and `qmc.discrepancy`, the `stats.norm`/`triang`/`truncnorm` `.ppf` percent-point functions inverse-transforming the unit draw to a declared `AxisDist` marginal over the exact `SALib.util._nonuniform_scale_samples` parameterization, with `lognorm` as `exp(norm.ppf(x, ln_mean, ln_std))`), `scikit-learn` (the `pipeline.Pipeline` of `preprocessing.StandardScaler` and a `SurrogateKind`-resolved `base.BaseEstimator` regressor `ensemble.GradientBoostingRegressor`/`ensemble.RandomForestRegressor`/`svm.SVR`/`linear_model.Ridge`, scored through `model_selection.cross_val_score` reading `metrics.r2_score`), `numpy` (`linspace`, `meshgrid`, `stack`, `asarray`, `where`, `exp`, `polynomial.Polynomial.fit`), `expression` (`tagged_union`/`case`/`tag`, `Ok`/`Error`/`Some`/`Nothing`/`Option` with `Option.map`/`default_value`/`is_some` the `Objective.batch` fast-lane fold, the `Map` route/class tables), `msgspec` (`Struct`, `gc=False` on the container-free `SalibRoute`/`Measured` leaves, the `params`-`tuple`-carrying `ParamAxis` and the `RowScorer`+`Option[BatchScorer]`-carrying `Objective` GC-tracked exactly as `experiments/inference.md#INFERENCE` keeps its `hdi`-tuple `PosteriorSummary`), `beartype` (`@beartype(conf=FAULT_CONF)` fencing the `_execute` body so a contract violation folds onto the rail through the `CLASSIFY` `api` row), `opentelemetry-api` (`trace.get_tracer`/`start_as_current_span`/`Span.set_attributes`/`is_recording`/`set_status`/`Status`/`StatusCode` the one `study.run` span), `time.perf_counter` folded once through the `_timed` measurement primitive the `MeasurementMode.evaluate` arms share, runtime (`FAULT_CONF`/`RuntimeRail`/`boundary`, the railed `ContentIdentity.of`/`ContentKey`/`IdentityPolicy`, `Receipt`/`Redaction`/`receipted`/`ReceiptContributor` the `@receipted(_REDACTION)` egress aspect).
- Growth: a new param axis is one `ParamAxis` coordinate; a new input marginal is one `AxisDist` member plus one `rescale` `.ppf` arm and one `bounds` arm matching its `SALib.util._nonuniform_scale_samples` parameterization, reaching the SALib `dists` channel and the qmc inverse-transform by that single add over the dist's `params` vector; a new SALib analyzer is one `StudyMethod` case plus one `SALIB_ROUTES` `SalibRoute` row, no new body; a new `qmc` engine or numpy floor is one arm on `StudyMethod._qmc`/`design`; a new surrogate estimator is one `SurrogateKind` member plus one `SURROGATE_CLASS` row resolving its scikit-learn class; a new measurement is one `MeasurementMode` member reading the shared `Measured` fold; zero new surface.
- Boundary: no job framework, farm scheduler, substrate selection, or C# receipt minting; classical DOE, sensitivity, and surrogate only — a neural surrogate or an acquisition-driven active-learning loop is out of charter, and study evidence does not cross the graduation rail (`graduation/handoff.md#GRADUATION` lists no `study` axis; the `StudyReceipt` streams onto the receipt rail and stops). A standalone benchmark owner, a parallel experiment tracker, a detached `_design`/`_indices` free function the union should own, a parallel sibling `StudyMethod.Lhs`/`Sobol`/`MorrisScreen` factory family re-wrapping each case where the union's keyword constructor (`StudyMethod(lhs=n)`) is the one construction surface the `experiments/inference.md#INFERENCE` `Distribution` discipline holds, a 2-field `(low, high)` `ParamAxis` overload that silently truncates the 3-param `triang` or 4-param `truncnorm` SALib vector into a divergent standard marginal where `params: tuple[float, ...]` carries the full `_nonuniform_scale_samples` vector, a `lognorm` arm calling scipy's `lognorm.ppf` shape form where SALib computes `exp(norm.ppf(x, ln_mean, ln_std))`, a `triang` arm reading `(low, high)` as `loc=0`/`scale=width` where the SALib convention is `c=peak`/`loc=start`/`scale=end-start`, a `truncnorm` arm passing `(low, high)` as the standardized `(a, b)` with implicit `loc=0`/`scale=1` where SALib standardizes `(lower-mean)/std`/`(upper-mean)/std` over `loc=mean`/`scale=std`, a `problem['bounds']` row built from `(low, high)` where SALib reads the row as the dist's full parameter vector when `dists[i]` is set, a four-positional `Receipt.of` against the two-argument contract, a `contribute` returning a single `Receipt` against the `Iterable[Receipt]` port, a positional `SALIB_ROUTES` tuple indexed by position where the `SalibRoute` value object names its fields, a bare `boundary` lambda left untraced where the `study.run` span weaves it, a `study.run` span left `UNSET` on a fault arm, an inline `Signals.emit` on the `Ok`/`Error` arms where the `@receipted(_REDACTION)` aspect owns egress, an unfenced `_execute` body where `@beartype(conf=FAULT_CONF)` raises the canonical violation the `api` row catches, an `Error` arm re-minting and re-emitting a `Receipt.of(owner, fault)` and re-annotating an ERROR status the fence's `_convert` already owns, a bare-`ContentKey` assignment off the railed `ContentIdentity.of` where the in-fence `match` re-raises a hash `Error` onto the `boundary`, a `batch = lambda:` statement binding where a nested `def` carries the closure, an `estimator() -> object` untyped carrier where `base.BaseEstimator` names the protocol root, a hand-rolled Morris or Saltelli implementation, a hand-rolled stratified-LHS over a numpy floor where the `qmc` engine owns it, a caller pre-transforming a non-uniform input where `problem['dists']` and the `AxisDist`-keyed `ParamAxis.rescale` `.ppf` fold declare it, a parallel non-uniform axis type beside the one `ParamAxis`+`AxisDist`+`params` value object, a `gc=False` `ParamAxis` opting the `params`-`tuple`-carrying struct out of GC tracking against the container-free-leaf-only opt-out the `SalibRoute`/`Measured` leaves take, a `qmc.scale` uniform-only scaling surface beside the distribution-aware `rescale`, eight near-identical SALib match arms, an in-sample score on the scikit-learn `Surrogate` row where cross-validation is the honest generalization estimate (the per-axis polynomial floor's in-sample `R^2` is a cheap univariate screening diagnostic, not a generalization claim), and a `NotImplementedError` sensitivity stub are the deleted forms. SALib is pure-Python over numpy/scipy, so its core runs where numpy resolves; the scipy `qmc` engine family and scikit-learn surrogate rows carry the `python_version<'3.15'` marker, and the numpy factorial and `numpy.polynomial` surrogate floors run on cp315.
...
# `SALib.util._nonuniform_scale_samples` exactly, so a single axis declares one marginal both the qmc-path
# `rescale` ppf and the SALib `dists` channel read identically — never a 2-tuple overload that silently
# truncates the 3-param `triang` or 4-param `truncnorm` vector into a divergent standard marginal.
...
class ParamAxis(Struct, frozen=True):
    ...
    # `Measured` rows take does not apply, exactly as `experiments/inference.md#INFERENCE` keeps its
    ...
    # `scipy.stats.<dist>.ppf` form `SALib.util._nonuniform_scale_samples` uses, so the qmc draw and the
...
class StudyMethod:
    ...
    # `experiments/inference.md#INFERENCE` `Distribution(normal=(0.0, 1.0))` discipline, no parallel
...
_TRACER: Final = trace.get_tracer("compute.study")
...
SALIB_ROUTES: Final[Map[SalibTag, SalibRoute]] = Map.of_seq([
    ...
    ("hdmr", SalibRoute("latin", "hdmr", "ST", True)),  # per-input total-order; `Sa`/`Sb` are per-component-term, not per-axis
])
...
class Study(Struct, frozen=True):
    ...
    def run(self, objective: Objective, /, *, seed: int = 0) -> RuntimeRail[StudyReceipt]:
        ...
        # `experiments/inference.md#INFERENCE` and `experiments/model.md#ASSET` hold.
    ...
    def _execute(self, objective: Objective, seed: int) -> StudyReceipt:
        ...
        # than a bare-`ContentKey` use of the railed owner.
        match ContentIdentity.of("study", design.tobytes(), IdentityPolicy()):
            case Ok(key):
...
- [SALIB_PROBLEMSPEC]: `SALib` resolves on the cp315 core (pure-Python over numpy/scipy). The owner drives the SALib family through one `ProblemSpec` fluent pipeline — `ProblemSpec(problem)`, `spec.sample(func, *args, **kwargs).samples`, `spec.set_samples(X)`, `spec.set_results(Y)`, `spec.analyze(func).analysis` — per the `.api` `[LOCAL_ADMISSION]` "use `ProblemSpec` for new analysis pipelines". The `ProblemSpec` constructor, the `sample`/`set_samples`/`set_results`/`analyze` pipeline methods, and the `samples`/`analysis` accessors verify against `compute/.api/salib.md` `[03]-[ENTRYPOINTS]`. The eight analyzers collapse to the `SALIB_ROUTES` `Map[SalibTag, SalibRoute]` table — the named `SalibRoute(sample_module, analyze_module, result_key, needs_design)` value object whose `sampler()`/`analyzer()` resolve `import_module(f"SALib.sample.{sample_module}")`/`import_module(f"SALib.analyze.{analyze_module}")`, so the sample and analyze submodules verified in `salib.md`'s sample/analyze submodule tables drive the one `StudyMethod.design` sampler body and the one `StudyMethod.indices` analyzer body rather than eight near-identical match arms or a four-positional tuple indexed by position. A non-uniform input rides the `problem['dists']` channel (`unif`/`norm`/`lognorm`/`triang`/`truncnorm`) the `salib.md` `[04]-[IMPLEMENTATION_LAW]` problem-dict and `[RAIL_LAW]` "Accept ... `problem['dists']` for non-uniform inputs" rows declare, so the SALib samplers shape their own marginals and the caller never pre-transforms an input the package owner rejects.
- [SALIB_RESULT_KEYS]: the eight analyze submodules (`morris`, `sobol`, `fast`, `rbd_fast`, `delta`, `pawn`, `dgsm`, `hdmr`) are each catalogued in `compute/.api/salib.md` `[02]` analyze-submodule table, and the two `ResultDict` key families the `.api` `[04]-[IMPLEMENTATION_LAW]` documents anchor the table — Sobol `S1`/`S2`/`ST` and Morris `mu`/`mu_star`/`sigma`. The `SalibRoute.result_key` field selects the per-method scalar each analyzer returns over its `ResultDict`: `morris_screen` reads `mu_star` and the Sobol/FAST variance routes read `ST` (both confirmed key families); `rbd_fast` reads the first-order `S1`, `delta` the Borgonovo moment-independent `delta`, `pawn` the per-input `median` of the KS statistic, `dgsm` the derivative-based `dgsm` index, and `hdmr` the per-input total-order `ST` (the `salib.md` `[03]` HDMR row catalogues an `Sa`/`Sb`/`ST` expansion where only `ST` is per-input and length `num_vars`; the `Sa`/`Sb` arrays index the HDMR component terms, so they overflow the per-axis `strict=True` zip) — the standard per-input `ResultDict` key each analyzer emits over an array of length `num_vars`, read by `route.result_key` off the `analysis` accessor rather than re-declared. `ProblemSpec.analyze` threads the stored sample matrix and result array to each analyzer over its signature — the `needs_design` rows (`rbd_fast.analyze(problem, X, Y, M=10, ...)`, `delta.analyze(problem, X, Y, ..., method='all')`, `pawn.analyze(problem, X, Y, S=10)` are catalogued in `salib.md` `[03]` analyze table; the `dgsm`/`hdmr` analyzers take `(problem, X, Y, ...)` over their imported signature; `morris.analyze(problem, X, Y)`) call `set_samples(design)` before `analyze`, while the `Y`-only rows (`sobol.analyze(problem, Y)`/`fast.analyze(problem, Y)`) read the responses against their structured design — each keyed by axis name so a `RunHistory` compare join lands on matching rows.
- [SCIPY_QMC]: the `scipy.stats.qmc.LatinHypercube`/`Sobol`/`Halton`/`discrepancy` spellings and the SPEC-007 `rng` constructor keyword (`seed` is the deprecated interim alias) verify against `compute/.api/scipy.md` `[03]-[ENTRYPOINTS]` `scipy.stats.qmc`; the `.random(n)` draw is the catalogued engine surface, and `qmc.discrepancy(unit)` folds the unit-cube uniformity score onto the receipt over an all-`unif` design. The unit draw is shaped to a declared `AxisDist` marginal through the `scipy.stats.norm`/`triang`/`truncnorm` frozen-distribution `.ppf` percent-point function catalogued in `scipy.md` `[03]` `scipy.stats` continuous-distribution table (`pdf`/`cdf`/`ppf`/`rvs` surface), so `ParamAxis.rescale` reproduces the exact `SALib.util._nonuniform_scale_samples` inverse transform — `triang.ppf(x, c=peak, loc=start, scale=end-start)`, `exp(norm.ppf(x, ln_mean, ln_std))` for `lognorm`, and `truncnorm.ppf(x, (lower-mean)/std, (upper-mean)/std, loc=mean, scale=std)` over the dist's full `params` vector — rather than a hand-rolled or 2-tuple-truncated marginal that diverges from the SALib `dists`-shaped draw. The per-dist `params` arity is enforced by the tuple unpack in `rescale`/`bounds` (`lower, upper, mean, std = self.params`), which raises a `ValueError` inside the `@beartype(conf=FAULT_CONF)`-fenced `_execute` body and folds onto the `boundary` rail rather than escaping. The engine family and the `scipy.stats` distributions carry the `python_version<'3.15'` marker and resolve once scipy ships cp315 wheels.
