...
- dependency: `NetTopologySuite` 2.6.0 (the central pin, exact match) + `ParquetSharp` 23.0.0.2 (the native columnar engine, exact match) — the package itself is managed but its `ParquetSharp` transitive ships the `runtimes/osx-arm64/native/libparquet` native dylib, so the codec inherits ParquetSharp's RID-keyed native runtime
...
[RAIL_LAW]:
- Package: `GISBlox.IO.GeoParquet` (1.1.1, MIT, pure-managed `lib/net10.0` AnyCPU IL; deps `NetTopologySuite` 2.6.0 + `ParquetSharp` 23.0.0.2, the ParquetSharp transitive carrying the osx-arm64 native `libparquet`)
