...
PagerDuty product semantics
    Source of truth: [PagerDuty incident priority guidance](https://support.pagerduty.com/main/docs/incident-priority).
    Claim: PagerDuty product surfaces distinguish customizable incident priority/classification from other product concepts such as urgency and alert severity; a local incident process must map priority, urgency, and alert-severity behavior into any response profile it uses.
    Last verified: 2026-06-04
...
Local incident process
    ...
    Last verified: per published runbook.
...
The base response path is `# [RECOVER_OBSERVABLE_SYMPTOM]`, the metadata block, `## [1][TRIGGER]`, `## [2][IMPACT]`, `## [3][SAFETY_PREREQUISITES]`, `## [4][TRIAGE]`, `## [5][MITIGATION]`, `## [6][ESCALATION]`, `## [7][VERIFICATION]`, and `## [8][EVIDENCE_CAPTURE]`. Every runbook also carries `## [9][BOUNDARIES]` and `## [10][REVIEW_CHECKLIST]`. `Escalation` is always present; only its triggering criteria vary by local profile. A runbook whose response has no reversible mitigation must say so in `Rollback or abort` and require escalation before any irreversible action.
...
```markdown template
<!-- source-only: runbook base template; add conditional sections only when their triggers apply -->
...
- `Mitigation` (required, repeatable actions): bounded containment or state-changing response actions, each paired with expected result and verification.
...
- `Mitigation`: per action, the mitigation class it belongs to, the exact command or path, the expected result, and the verification check that confirms it.
...
The mitigation table is an example lookup, not an external taxonomy. Keep only actions the local operational corpus supports, and attach local evidence in published runbooks:
...
- [ ] Each mitigation step names its locally supported mitigation class and pairs an expected result with a recovery check.
