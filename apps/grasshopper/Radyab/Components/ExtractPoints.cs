...
public sealed class ExtractPoints : ShapeComponent {
    private static readonly Seq<IOutput<Shape>> OutputSlots = Seq<IOutput<Shape>>(
        ...
        ShapeOutput(port: Port.List<Point3d>(name: "Control Points", code: "CP", info: "NURBS control polygon points for curves and surfaces, converting through RhinoCommon when required."), query: Query.Locate<object, Point3d>(aspect: new Location.ControlPoints())),
        ShapeOutput(port: Port.List<Point3d>(name: "Bounding Corners", code: "B", info: "Unique axis-aligned bounding-box corners: 8 for full 3D, 4 for planar, 2 for linear, 1 for point."), query: Query.BoundingCorners<object, Point3d>()),
