...
// --- [MODELS] --------------------------------------------------------------------------
public sealed record PolicyConstraint<T>(Seq<string> Sets, Func<T, Validation<Error, Unit>> Holds);
...
public static class OptionsAdmission {
    ...
    public static Validation<Error, T> Refine<T>(T policy, Seq<PolicyConstraint<T>> constraints, Option<Seq<string>> ruleSets = default) where T : notnull =>
        constraints
            .Filter(constraint => constraint.Sets.IsEmpty
                || (ruleSets is { IsSome: true, Case: Seq<string> sets } && constraint.Sets.Exists(sets.Contains)))
            .Traverse(constraint => constraint.Holds(policy))
            .As()
            .Map(_ => policy);
