...
`codex exec` runs gpt-5.5 as a non-interactive agent in its own context window and returns one final message. Defaults come from `~/.codex/config.toml` — gpt-5.5 at medium reasoning — so a bare invocation is already correctly configured; pass flags only to deviate. Choosing WHERE work runs across Claude's own surfaces (subagent, fork, team, workflow) is the agent-dispatch skill; this skill owns the codex leg once dispatch picks it.

## [01]-[ROUTING]

- [01]-[AUTHORING_SKILLS](references/authoring-skills.md): the on-disk format, discovery roots, trigger mechanics, `agents/openai.yaml`, and the deltas from Claude-side skills — for creating or porting a skill into `~/.codex/skills`, `~/.agents/skills`, or a repo `.agents/skills`.
