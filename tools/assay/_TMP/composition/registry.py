...
from tools.assay._TMP.composition.catalog import select, TOOLS  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.composition.settings import ArtifactScope, AssaySettings  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.core.aspect import _RING, checked, compose, logged, traced  # noqa: PLC2701  # intra-staging import; _TMP is the package root
...
from tools.assay._TMP.rails import (  # noqa: PLC2701  # intra-staging import; _TMP is the package root
    api as api_rail,
...
from tools.assay._TMP.rails.api import ApiParams  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.rails.bridge import BridgeParams  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.rails.docs import DocsParams, FaultedPromotion  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.rails.package import PackageParams  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.rails.static import StaticParams  # noqa: PLC2701  # intra-staging import; _TMP is the package root
from tools.assay._TMP.rails.test import TestParams  # noqa: PLC2701  # intra-staging import; _TMP is the package root
...
def _distill(fault: Fault, duration_ms: float) -> Diagnostic:
    ...
    The auto-observability payoff: the bounded recent-events ring ``rail`` seated into ``_RING`` and
    ``ring_processor`` appended in place — surviving the engine's ``anyio.run`` because the copied
    context-var reference points at the SAME ``deque`` — is read back here at envelope time. A statement
    ``match`` on ``fault.status`` names the ``failing_step``: ``TIMEOUT``/``BUSY`` are unambiguous, and a
    ``FAULTED`` splits ``validation`` vs ``spawn`` off the ring's last event (a ``validate_detail`` codec
    raise logs no rail event, so an empty ring or a non-``validation`` tail reads ``spawn``). The ``hint``
    synthesizes the last ring event with the elapsed wall time so an agent retriages off the wire without
    re-running. Rides ``Envelope.error_context`` only — the success branch leaves it ``None`` so
    ``omit_defaults`` keeps the success wire terse.
    """
    ...
    match fault.status:
        case RailStatus.TIMEOUT:
            step = "timeout"
        case RailStatus.BUSY:
            step = "lease_busy"
        case _:
            step = "validation" if "validation" in last else "spawn"
...
def _guard(thunk: Callable[[], Result[Report, Fault]]) -> Result[Report, Fault]:
    ...
    The docs ``--strict`` promotion raises ``FaultedPromotion`` carrying the unpromoted ``Report``
    through the ``checked ▷ logged ▷ traced`` seam, and ``_validated`` raises ``msgspec.MsgspecError``
    on a malformed ``Report.detail``; this converts either to ``Fault{argv=(), status=FAULTED}``. The
    marked boundary keeps the sentinels riding exactly the one ``raise`` in docs / the codec and the
    one ``except`` here — neither crosses a seam as a raised exception into domain logic.
    """
    try:
        return thunk()
    except (FaultedPromotion, msgspec.MsgspecError) as promoted:
...
def _leaf(bind: Bind) -> Callable[..., Envelope]:
    ...
    ``Annotated[bind.params, Parameter(name="*")]`` flattens the frozen ``@dataclass`` fields onto the
    CLI as flat flags; the command defaults to a zero-arg ``bind.params()`` so a no-token invocation
    injects the param defaults. Identity is set directly rather than via ``functools.wraps`` — ``wraps``
    would stamp a ``__wrapped__`` pointing at the defaultless ``run`` closure, which cyclopts follows
    during signature inspection and rejects (a ``name="*"`` flatten param needs a default on the
    inspected signature). The explicit ``__annotations__`` override pins the flattened param type and the
    ``Envelope`` return so ``__cyclopts_returncode__`` drives the exit code.
    """
    runner = rail(bind)
    ann = Annotated[bind.params, Parameter(name="*")]  # type: ignore[name-defined]  # ty: ignore[invalid-type-form]  # bind.params is the runtime frozen-dataclass Params type; the cyclopts flatten reads it as a value, which both checkers reject in Annotated but is the canonical binding

    def command(params: ann = bind.params()) -> Envelope:  # noqa: B008  # cyclopts needs a concrete default on the flatten param; the frozen Params instance is immutable so the shared default is safe
