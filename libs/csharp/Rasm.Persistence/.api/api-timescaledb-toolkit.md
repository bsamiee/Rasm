...
- `npgsql`(`libs/csharp/.api/api-npgsql.md`): `NpgsqlDataReader.GetDouble` and `GetFieldValue<Instant>` read every accessor projection; a summary type carries no managed mapping, so the accessor closes server-side before the reader sees a column.
