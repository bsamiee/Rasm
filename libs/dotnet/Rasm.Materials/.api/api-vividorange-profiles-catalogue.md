...
- `UnitsNet`(`libs/dotnet/.api/api-unitsnet.md`): a catalogued AISC (`Inch`), catalogued EN (`Millimeter`), and measured/parametric section are the SAME `UnitsNet.Length` type, so the Materials family axis folds them through one typed surface and the family gate `q.Dimensions.Equals(Length.Info.BaseDimensions)` applies uniformly.
