...
[STACKING]:
- `api-messagepack`(`libs/dotnet/.api/api-messagepack.md`): `MessagePackCompression.Lz4BlockArray` frames LZ4 inside the serializer at ext type 98, so `SnapshotFormat.Admits` binds that codec to `CompressionPolicy.None` and this package frames only what MessagePack leaves raw.
