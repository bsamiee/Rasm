...
- `SafeHandle` (`System.Runtime.InteropServices.SafeHandle`) is the BCL owner for unmanaged handles: it reference-counts through `DangerousAddRef`/`DangerousRelease`, runs `ReleaseHandle` in a constrained execution region even on thread abort, and survives the GC finalizer. For any raw `IntPtr` native handle, the capsule's inner representation must derive from `SafeHandle` — not hold a bare `IntPtr` — so P/Invoke marshaling never races disposal. `IsClosed` and `IsInvalid` are the canonical validity gates; `DangerousGetHandle` is a last resort only inside a `DangerousAddRef`/`DangerousRelease` guard block.
- `CriticalHandle` (non-reference-counting sibling) is the correct base when the native API has no reference-count semantics. `SafeHandle` is not a drop-in because its ref-count can hold a handle open past the capsule's logical disposal when any P/Invoke marshaling layer performs `DangerousAddRef`.
...
```csharp
// Capsule: Borrowed/Owned cases, projection via bounded callback
...
- The correct shape: `Atom<Option<TSession>>` where the `SwapMaybe` call is the atomic gate. `SwapMaybe(current => current.IsSome ? current : Some(construct()))` is one CAS operation — probe (is `None`) and construction (produce `Some`) are the same lambda, executed under spin-lock. If the atom already has a value, `SwapMaybe` returns the unchanged current value without calling `construct()`.
...
- `Atom<A>.SwapMaybe(Func<A, Option<A>>)` may call its function multiple times during CAS retry. Construction inside `SwapMaybe` must therefore be idempotent: if called multiple times, all but one result must be safe to discard — or the construction is deferred until after the CAS, which then reintroduces the gap.
- When construction is not idempotent, CAS a sentinel (`Atom<Option<Guid>>`) first, then construct and update to the final value, then release the sentinel on failure. `SwapMaybe(None → Some(newGuid))` is the exclusive entry; `Swap(Some(_) → None)` is the failure-path release.
...
- Connections opened for maintenance operations (backup, integrity check, compaction, schema migration) acquire exclusively within the lifecycle state that permits them. No maintenance connection persists across a lifecycle transition; the transition itself is the signal to abort and quarantine.
...
- Each step's deadline is a cancellation window derived from `TimeProvider`, not `Thread.Sleep` or `Task.Delay(hardcoded)`. `CancellationTokenSource(TimeSpan delay, TimeProvider provider)` creates a time-limited cancellation scope testable via `FakeTimeProvider` without wall-clock coupling.
...
static async ValueTask DrainAsync(TimeProvider clock, CancellationToken host)
{
    ...
    _fence.Set();                                        // step 1: fence new work
    await _drainCts.CancelAsync();                       // step 2: signal
    await _computeWorker.DrainAsync(drainCts.Token);     // step 3: compute drain
    await _persistLayer.FlushAsync(drainCts.Token);      // step 4: persistence flush
    // steps 5 + 6: UI and handles — synchronous from here, host controls order
}
```
...
- A disposal-ordering guard that must hold a lock across synchronous disposal of children uses `Lock.EnterScope()` with a `using` declaration. The compiler prevents any usage pattern that would escape the scope across an `await`.
...
- When a type implements both, `await using` calls `DisposeAsync`; `using` calls `Dispose`. If `DisposeAsync` is the canonical disposal, `Dispose` may call `DisposeAsync().AsTask().GetAwaiter().GetResult()` only as a last resort for sync-only callers — and the type's documentation must state this. The preferred design: implement only the interface that matches the actual disposal path.
