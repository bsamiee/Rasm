...
| [INDEX] | [SYMBOL] | [RAIL] | [CAPABILITY] |
|:-----: |:--------------------- |:------ |:------------------------------------------------------------------------------------------------------- |
| [01] | `Model` | energy | the HBJSON root (`: IDdBaseModel, IModel`); `Properties` (`ModelProperties`), `Version` (the baked schema version), `Rooms` (`List<Room>`), `OrphanedFaces`/`OrphanedShades`/`OrphanedApertures`/`OrphanedDoors`, `ShadeMeshes`, `Units` (`Units` enum, default `Meters`), `Tolerance`, `AngleTolerance`. `new static FromJson(string)` -> `Model`, `DuplicateModel` |
...
| [INDEX] | [SURFACE] | [CALL_SHAPE] | [CAPABILITY] |
|:-----: |:--------------------------------- |:-------------------------------------------------------------------------- |:----------------------------------------------------------- |
| [01] | `new Model` | `(string identifier, ModelProperties properties, …, Units units = Meters, double tolerance =, double angleTolerance =)` | build a model from rooms/orphaned surfaces + the property store; `properties` is required (non-null) |
