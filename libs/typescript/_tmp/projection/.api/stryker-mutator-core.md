...
| :-----: | :----------- | :------------ | :------------------------------- |
|  [01]   | `Stryker`    | class         | programmatic mutation runner     |
|  [02]   | `StrykerCli` | class         | CLI entry point over `commander` |
...
| [INDEX] | [SYMBOL]                  | [TYPE_FAMILY] | [CAPABILITY]                                                            |
| :-----: | :------------------------ | :------------ | :---------------------------------------------------------------------- |
|  [01]   | `StrykerOptions`          | interface     | full configuration schema (all fields required)                         |
|  [02]   | `PartialStrykerOptions`   | type alias    | `DeepPartial<StrykerOptions>` for `Stryker` constructor                 |
|  [03]   | `CoverageAnalysis`        | string union  | `"off" \| "all" \| "perTest"`                                           |
|  [04]   | `LogLevel`                | string union  | `"off" \| "fatal" \| "error" \| "warn" \| "info" \| "debug" \| "trace"` |
|  [05]   | `MutationScoreThresholds` | interface     | `high`, `low`, `break` thresholds                                       |
|  [06]   | `CommandRunnerOptions`    | interface     | `{ command: string }`                                                   |
|  [07]   | `MutatorDescriptor`       | interface     | excludedMutations and plugin fields                                     |
...
| [INDEX] | [SYMBOL]             | [TYPE_FAMILY] | [CAPABILITY]                                                                     |
| :-----: | :------------------- | :------------ | :------------------------------------------------------------------------------- |
|  [01]   | `Mutant`             | interface     | initial mutant shape: `id`, `fileName`, `location`, `mutatorName`, `replacement` |
|  [02]   | `MutantTestCoverage` | type alias    | `Mutant` plus `coveredBy` and `static` fields                                    |
|  [03]   | `MutantResult`       | type alias    | `Mutant & schema.MutantResult` — final reportable result                         |
|  [04]   | `MutantStatus`       | string union  | re-export from `mutation-testing-report-schema/api`                              |
...
| :-----: | :----------------------------------------------- | :-------------- | :--------------------------------------------------------- |
|  [01]   | `new Stryker(cliOptions: PartialStrykerOptions)` | constructor     | creates runner with merged CLI + file config               |
|  [02]   | `Stryker#runMutationTest()`                      | instance method | runs full mutation test, returns `Promise<MutantResult[]>` |
|  [03]   | `Stryker.run(injector, args)`                    | static method   | internal; runs inside a prepared injector scope            |
...
| [INDEX] | [SURFACE]                                                                     | [ENTRY_FAMILY]  | [CAPABILITY]                               |
| :-----: | :---------------------------------------------------------------------------- | :-------------- | :----------------------------------------- |
|  [01]   | `new StrykerCli(argv, program?, runMutationTest?, runMutationTestingServer?)` | constructor     | wires `commander` CLI to the runner        |
|  [02]   | `StrykerCli#run(createInjectorImpl?)`                                         | instance method | parses `argv` and dispatches to the runner |
|  [03]   | `guardMinimalNodeVersion(processVersion?)`                                    | guard function  | throws if Node version is below minimum    |
...
| :-----: | :----------------- | :------------------------------ | :-------------------------------- |
|  [01]   | `testRunner`       | `string`                        | `"command"`                       |
|  [02]   | `coverageAnalysis` | `CoverageAnalysis`              | `"perTest"`                       |
|  [03]   | `mutate`           | `string[]`                      | `["src/**/*.ts"]` pattern         |
|  [04]   | `checkers`         | `string[]`                      | `[]`                              |
|  [05]   | `reporters`        | `string[]`                      | `["clear-text","progress"]`       |
|  [06]   | `concurrency`      | `number \| string \| undefined` | `n-1` logical CPUs                |
|  [07]   | `thresholds`       | `MutationScoreThresholds`       | `{ high: 80, low: 60, break: 0 }` |
|  [08]   | `plugins`          | `string[]`                      | all `@stryker-mutator/*`          |
|  [09]   | `timeoutFactor`    | `number`                        | `1.5`                             |
