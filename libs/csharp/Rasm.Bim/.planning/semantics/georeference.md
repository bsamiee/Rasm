...
- [01]-[GEO_PROJECTION]: `GeoReferenceProjector.Project` folding the model georeferencing level onto the seam `GeoReference` twelve-tuple — `IfcMapConversion`/`IfcMapConversionScaled` over `IfcProjectedCRS` (LoGeoRef 50, per-axis scale composed with the `IfcProjectedCRS.MapUnit` `SIFactor()` model-unit↔CRS-unit factor + the seam `ProjectedCrs` EPSG parse + fault-on-unresolvable), the `IfcSite` `RefLatitude`/`RefLongitude`/`RefElevation` WGS84 fallback (LoGeoRef 30), else `Identity` [M1].
