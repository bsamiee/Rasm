...
- dependency: `ladybug-core==0.44.52` (consumes its `DataCollection`/`EPW`/`datatype`)
...
[ENERGY_COMFORT]:
- import: `import ladybug_comfort` and the submodules at boundary scope only; module-level import is banned by the manifest import policy, and the companion-env marker means the import resolves only under cp<3.15.
