...
- Packages: `open3d` (`geometry.PointCloud.estimate_normals`/`orient_normals_consistent_tangent_plane`/`cluster_dbscan`/`select_by_index`/`KDTreeSearchParamHybrid`, `geometry.TriangleMesh.create_from_point_cloud_poisson`/`create_from_point_cloud_ball_pivoting`/`create_from_point_cloud_alpha_shape`/`remove_vertices_by_mask`/`.vertices`/`.triangles`, `utility.DoubleVector`), `trimesh` (`Trimesh`/`Trimesh.is_watertight`/`is_winding_consistent`/`euler_number`/`is_volume`/`body_count`/`volume`/`area`/`Trimesh.export` for the GLB egress open3d does not write), `numpy` (`asarray`/`sort`/`where` over the open3d vertex/triangle/density/label arrays — `numpy.quantile` is uncatalogued, the order statistic reads off `sort`), `expression` (`Ok`/`Error` the span `match`, `Block.of_seq`/`Block.fold` the cluster-join carrier, `Map.of_seq`/`Map.empty` the `_CONSTRUCT` table and the keep-all `Redaction`), `msgspec` (`Struct`/`gc=False` on the leaf `MeshQuality`/`ConstructSpec`/`ReconReceipt`), `opentelemetry-api` (`trace.get_tracer`/`start_as_current_span`/`Span.is_recording`/`Span.set_attributes`/`Span.set_status`/`Status`/`StatusCode` the one `geometry.reconstruct` span), `beartype` (`@beartype(conf=FAULT_CONF)` the shared domain conf fencing the in-page `_trim_poisson` density reduction, `beartype.vale.Is` the `DensityField` finiteness refinement — the sibling `scan/deviation.md#DEVIATION` `SignedField`/`scan/ingestion.md#INGESTION` `_structured` parity), runtime (`RuntimeRail`/`boundary`/`FAULT_CONF`/`Receipt`/`Redaction`/`receipted`/`ContentKey`/`LanePolicy`, the `ReceiptContributor` port `ReconReceipt.contribute` satisfies structurally). The compute `GraduationReceipt.graduates` `_admit` fence owns the graduation-LEDGER finiteness refinement, distinct from the in-page density-array finiteness this page fences locally, so the page never re-fences the ledger the compute owner already gates; compute (`GeometrySubject`/`GraduationReceipt`/`HandoffAxis`). The `open3d` and `trimesh` compiled imports are function-local at boundary scope under `# noqa: PLC0415` per the manifest import policy; this page shares the `open3d` admission row under `python_version<'3.15'` with `ingestion.md`/`registration.md`/`deviation.md`.
- Growth: a new reconstruction algorithm is one `ReconstructionMethod` row plus one `_CONSTRUCT` `ConstructSpec` row binding its static constructor — never a new dispatch arm; a new normal-estimation search param or a Poisson density-trim threshold is one `ReconPolicy` field threaded through `normal_search`/`radii`; a new manifold-quality statistic or residual key is one `MeshQuality` field folded inside the one `MeshQuality.fold` and one `residuals`/`ceiling` row; the multi-object split is the one optional `cluster_dbscan` pre-pass already on the owner, never a parallel per-object class; a stricter graduation bar is one tighter `MeshQuality.ceiling` row, never a second admission direction; the open3d reconstruction kernel (the CPU-bound Poisson/ball-pivoting solve over the `<'3.15'` companion band) hands across the optional `lane: LanePolicy | None` field through the runtime `execution/lanes#LANES` `LanePolicy.offload` per-subinterpreter variant (`anyio.to_interpreter.run_sync` under one `CapacityLimiter`, the no-pickle PEP-734 hop, degrading to `anyio.to_thread.run_sync` only where a cp315 build ships no runnable `concurrent.interpreters`, NEVER a `to_process` pickle round-trip the lanes owner rejects as the process-pool serialization tax) as ONE `offload(kernel, *args)` hand-off call over the already-landed lane — the lane never imports the kernel; zero new surface, no parallel per-method class family.
...
if TYPE_CHECKING:  # type-only: every runtime open3d call does its own boundary-scope `import open3d as o3d  # noqa: PLC0415`, so the module loads clean on cp315 and the import-policy ban on module-level open3d holds (the ingestion/registration sibling shape)
...
# functions, every open3d call deferred to the offloaded kernel that imports the companion-band package.
