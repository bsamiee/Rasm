...
- Entry: `Access.author` dispatches the op over the input PDF through the one `_ARM[op]` lookup inside the `async_boundary` capsule and returns a `RuntimeRail[ContentKey]`; `TAG` adds the marked-content structure layer plus the catalog/XMP requirements and re-emits, `AUDIT` proves the authored structure. `Access.of` is the admission gate returning `Result[Access, AccessFault]` — `TAG` without an `AccessParams.source` tree is `AccessFault.incomplete` before any arm runs, so the interior is total over admitted owners. `TAG`/`AUDIT` resolve in-process over the cp315-core `pikepdf` wheel (`cp314-abi3`, the qpdf C++ bound through nanobind, ungated in the manifest), so the owner runs one synchronous object-model fold inside the async `_emit` exactly as `document/egress#FINISH` finishes a `pikepdf` tree — never a subprocess seam nor a `python_version<'3.15'` gated band (the `graphic/color/managed#MANAGED` ICC apply and the `export/layered#LAYERED` OCG band are the gated legs; tagged-structure authoring is not).
...
class Access(Struct, frozen=True):
    ...
    @receipted(Redaction.STRUCTURAL)  # drains `contribute` off the stepped owner, emits via Signals.emit_async
    async def _emit(self) -> Self:
        return self._stepped()  # the sync object-model fold runs in-process inside the async weave
...
def _tag(access: "Access") -> AccessFact:
    ...
    pdf = pikepdf.open(BytesIO(access.pdf))
...
- [RECEIPT_AND_ASPECT] [RESOLVED]: the receipt is WIRED, not described — the `@receipted(Redaction.STRUCTURAL)` weave over the thin pure `_emit` drains `Access.contribute` off the stepped owner and emits through the runtime `Signals.emit_async`, the artifacts convention `document/egress#FINISH`/`document/emit#DOCUMENT` establish and the `core/receipt#RECEIPT` `ReceiptContributor` law mandates (the prior page only narrated the receipt in prose with no `contribute`/`@receipted` surface, the unwired-receipt-law defect). The `TAG` arm contributes `ArtifactReceipt.Egress(key, bytes, pages, 0, elements, figures)` mapping the structure-element count onto `outline_depth` and the figure count onto `overlays`, and the `AUDIT` arm `ArtifactReceipt.Pdf(key, bytes, pages)` — the settled `core/receipt#RECEIPT` `Egress`/`Pdf` reuse target (README `[20]-[TAGGED]`, IDEAS `[ACCESSIBILITY]`), never a fifteenth case. The owner runs the synchronous object-model fold inside the async `_emit` exactly as `document/egress#FINISH` finishes a `pikepdf` tree in-process; `async_boundary` converts a `pikepdf.PdfError` into the runtime `BoundaryFault`, and `Access.of` rejects a `TAG` without a `source` tree into `AccessFault.incomplete` before the fold.
