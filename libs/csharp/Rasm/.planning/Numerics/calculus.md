...
|  [02]   | Kernel profiles        | `KernelKind` + `KernelProfile`/`KernelProfileStatus` | `[SmartEnum<int>]` rows with value + two derivative columns   |    6    |
|  [03]   | Reconstruction weights | `WeightKernelFamily`                             | `[SmartEnum<int>]` with `Interpolating` capability column     |    5    |
|  [04]   | Radial decay           | `Falloff`                                        | `[Union]` incl. the metric-sampler anisotropic case           |    6    |
