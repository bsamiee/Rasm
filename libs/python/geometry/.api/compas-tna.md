...
- rpc-bridge integration: `graph/algebra.md#bridged` resolves `compas.rpc.Proxy.function("compas_tna.equilibrium.horizontal_numpy")` to run the scipy-backed solvers out of process; the `_numpy` solvers return `(form, force)` precisely for this RPC round-trip, and the blocking call crosses to the event loop through `anyio.to_thread.run_sync`, sharing the one form-finding fault rail with `compas_dr`.
