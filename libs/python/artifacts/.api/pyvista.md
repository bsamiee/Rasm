...
- abi: pure-Python over the `vtk` C++ native runtime (the ABI/native surface); runtime dependencies `numpy`, `matplotlib` (colormaps/scalar bars), `scooby`, `pooch`, `Pillow` (screenshot encode), `typing_extensions`. NO CPython 3.15 wheel for `vtk` — both `pyvista>=0.48.4` and `vtk>=9.6.2` are manifest-gated `python_version<'3.13'`; the cp315-core owner imports neither and crosses the runtime subprocess seam
...
|  [02]   | `pyvista.wrap`        | `wrap(dataset: vtkDataObject | trimesh.Trimesh | meshio.Mesh | numpy.ndarray | None, *, validate=None) -> DataObject` | zero-copy wrap a VTK/array/trimesh/meshio source |
...
- mesh axis: the `DataSet` family (`PolyData`/`UnstructuredGrid`/`StructuredGrid`/`RectilinearGrid`/`ImageData`) is one mesh hierarchy; mesh kind is a subclass row, never a parallel per-source wrapper; `MultiBlock` collects named datasets and fans filters over its blocks.
...
- export axis: `export_gltf(inline_data=, save_normals=)`/`export_vrml`/`export_obj`/`export_html` emit scene files; the VTK glTF exporter handles surface `PolyData`, so `extract_surface` precedes glTF export. `PolyData.save` writes a single mesh file (binary by default) where a full scene is not needed.
