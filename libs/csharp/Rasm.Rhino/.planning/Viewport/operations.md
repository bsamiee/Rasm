...
One camera operation rail (`Rasm.Rhino.Viewport`). Every way a camera changes — keyboard and mouse gestures, projection changes including the `IsometricCamera` axonometric family and the `CameraAngle` lens, view and construction-plane stacks, subject framing through `ZoomBoundingBox`, the full named-view table family with paced restores, clipping-plane participation, defined-view settings scopes, kernel view-convention lowering, and motion-compiled pose sequences — is one case of one `CameraOp` `[Union]` executed by one `Cameras.Apply` entry. Execution absorbs what the census-era service scattered: scope resolution rides the `ViewportLease` (single and broadcast are the target's arity, never sibling surfaces), the session demand lands every host mutation on the Rhino command thread, detail edits commit through `CommitViewportChanges`, redraw collapses to the policy row's columns after the batch, and every application returns a `CameraReceipt` naming what moved. Killed forms are the per-modality union family (`CameraGesture`/`CameraProjection`/`CameraPath`/`CameraEdit` as parallel dispatch roots), the separate single/broadcast/rig executors, and the `Architecture.cs` recipe catalog — architectural conventions live as kernel `ViewConvention` rows lowered through `VectorIntent.View`, and this rail only seats the projected pose.
...
- Boundary: `RestoreScope` touches process-global host settings; it composes only inside `Cameras.Apply`, whose session demand lands settings-scoped operations on the command thread so two concurrent scopes cannot interleave their capture/restore pairs.
...
- Boundary: every host mutation rides the session demand's command-thread landing; a background caller pays one marshal per `Apply`, never per row — and the receipt is the only output, so no live handle leaves the rail.

```csharp
// --- [TYPES] --------------------------------------------------------------------------------
[SmartEnum<int>]
public sealed partial class RedrawWhat {
    public static readonly RedrawWhat None = new(key: 0, perRow: static _ => unit, terminal: static _ => unit);
    public static readonly RedrawWhat Views = new(key: 1, perRow: static view => Op.Side(view.Redraw), terminal: static _ => unit);
    public static readonly RedrawWhat Document = new(key: 2, perRow: static _ => unit, terminal: static document => Op.Side(document.Views.Redraw));

    [UseDelegateFromConstructor]
    internal partial Unit PerRow(RhinoView view);

    [UseDelegateFromConstructor]
    internal partial Unit Terminal(RhinoDoc document);
}
...
// --- [MODELS] -------------------------------------------------------------------------------
public sealed record ApplyPolicy(RedrawWhat Redraw, bool CommitDetails) {
    public static ApplyPolicy Default { get; } = new(Redraw: RedrawWhat.Views, CommitDetails: true);
    public static ApplyPolicy Silent { get; } = new(Redraw: RedrawWhat.None, CommitDetails: true);
}
...
public static class Cameras {
    public static Fin<CameraReceipt> Apply(DocumentSession session, ViewportTarget target, CameraOp operation, Option<ApplyPolicy> policy = default, Op? key = null) {
        ...
        return from lease in ViewportLease.Of(session: session, target: target, key: op)
               from receipt in session.Demand(
                   use: document => Execute(document: document, lease: lease, operation: operation, plan: plan, key: op),
                   key: op,
        ...
        from outcomes in lease.UseAll(borrow: row =>
            ...
            from clips in operation.Switch(
                ...
                frameCase: static (ctx, op) => {
                    ...
                    padded.Inflate(amount: op.Subject.Diagonal.Length * op.Padding.Value);
                ...
                motionCase: static (ctx, op) => op.Frames.Last.ToFin(Fail: ctx.Op.InvalidInput())
                    .Bind(pose => Seated(pose: pose, viewport: ctx.Row.Viewport, key: ctx.Op)))
            from committed in row.Detail.Match(
                Some: detail => plan.CommitDetails
            ...
            key: key)
        from _ in Fin.Succ(value: plan.Redraw.Terminal(document: document))
    ...
    // One full-pose seat spelling the pose-carrying arms share; the seat core itself is CameraPose.Seat.
    ...
    // One kernel-to-host projection lowering: ViewProjectionIntent rows map onto ProjectionChange cases.
...
flowchart LR
    ...
    Rail -->|one command-thread demand| Host["DocumentSession.Demand"]
