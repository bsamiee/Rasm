...
lazy from tests.python.testkit.laws import consume_covers
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    (config.pluginmanager.register(importlib.import_module("tests.python.testkit.bench"), "testkit-bench") if hasattr(config.pluginmanager.hook, "pytest_benchmark_update_json") and not config.pluginmanager.hasplugin("testkit-bench") else None)
...
def log_processors() -> tuple[Processor, ...]:
    """Processor chain spliced before the ``log_events`` capture sink; empty unless a package conftest overrides it."""
    return ()
