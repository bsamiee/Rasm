...
The package re-exports each module as a namespace from the root (`export * as HttpClient from "./HttpClient.js"`, etc.); the namespace name equals the module name. Owner-symbol consumers across packages: `FetchHttpClient.layer` backs the browser `HttpClient` in `@rasm/host` `BrowserPlatform`; `HttpClient`/`HttpClientRequest`/`HttpClientResponse` back `@effect/rpc` `RpcClient.makeHttp` for `InternalRpc` (`@rasm/node-durable`); `KeyValueStore` backs `LocalPersistence` (`@rasm/host`); `Worker`/`WorkerPool`/`Transferable`/`WorkerError`/`WorkerRunner` back `DecodeWorkerPool` (`@rasm/host`); `MsgPack`/`Ndjson`/`ChannelSchema` back the schema-channel encoders consumed by `SnapshotFeed` (`@rasm/state`) and the worker rails; `Socket` backs any WebSocket leg; `Error.PlatformError` is the platform fault family `FaultDetailRail` reconstructs; `Runtime.runMain` is the `@rasm/node-durable` node-entry boundary.

---

## [1] — FetchHttpClient

The browser `HttpClient` binding. `BrowserPlatform` provides `layer` so every `HttpClient` consumer (the RPC-over-HTTP client) dials through the global `fetch`.
...
`layer` is the sole binding the browser composition installs — it requires nothing and produces `HttpClient`. `Fetch` and `RequestInit` are optional `Context` overrides (a custom `fetch` impl, or default `globalThis.RequestInit` fields like `credentials`/`mode`); neither is supplied day-one.

## [2] — HttpClient

The polymorphic HTTP client surface. `HttpClient` is `HttpClient.With<HttpClientError>` — the error-typed face; the request-issuing methods discriminate on body shape via the `Options.NoBody`/`Options.NoUrl` projections.
...
// Context.Tag — the dependency RpcClient.makeHttp and any HttpClient consumer requires
export declare const HttpClient: Context.Tag<HttpClient, HttpClient>

// module-level method forms (resolve the Tag, then dispatch) — each yields Effect<…, HttpClientError, HttpClient>
...
Transform combinators (dual `(self, …)` / `(…) => self` forms) the planning posture composes onto the client — selected:
...
// retry: typed-policy or Effect.Retry.Options form — Retry.Return widens E/R from the schedule
...
export declare const tapRequest: { /* same shape over the request */ }
...
// tracer-propagation knobs — relevant to the correlation/traceparent posture
...
export declare const makeWith: <E2, R2, E, R>(postprocess: HttpClient.Postprocess<E2, R2> /* widened */, preprocess: HttpClient.Preprocess<E2, R2>) => HttpClient.With<E, R>
export declare const layerMergedContext: <E, R>(effect: Effect.Effect<HttpClient, E, R>) => Layer<HttpClient, E, R>
```

The `HttpClient` is one polymorphic dispatch surface; `RpcClient.makeHttp` consumes the `HttpClient` `Context.Tag` and `BrowserPlatform`'s `FetchHttpClient.layer` provides it. `signal: AbortSignal` threading lives inside `make`, not on the public method face — Effect interruption maps to fetch cancellation through the runtime, not a per-call flag.

## [3] — HttpClientRequest

Immutable request value. The `Options.NoBody`/`Options.NoUrl` projections derive off the one `Options` interface — never parallel option types.
...
// method-discriminated constructor — GET/HEAD take NoBody, the rest take NoUrl
...
// modify { (self, options) | (options) } rebuilds a request with overrides (dual form)
```
...
// schema-typed body decode at the response edge
...
// byte stream of the body — used for streamed payloads
...
// status-class fold (2xx/3xx/4xx/5xx + numeric + orElse)
...
// re-exported decoders from HttpIncomingMessage
export { schemaBodyJson, schemaBodyUrlParams, schemaHeaders } from "./HttpIncomingMessage.js"
```

`schemaJson`/`schemaNoBody` are the response-edge decode entry — `Schema.decodeUnknown` at the wire boundary, never an interior re-validate.

## [5] — HttpClientError

The HTTP client fault rail — two `_tag`-discriminated `YieldableError` classes carrying a typed `reason`. This is the platform-side family `FaultDetailRail` reconciles alongside the gRPC fault detail.
...
## [6] — request support: HttpMethod / Headers / HttpBody / UrlParams / Cookies
...
```ts
// @effect/platform/Headers — a branded readonly string-record; one Schema, dual-form mutators
...
```ts
// @effect/platform/HttpBody — a tagged union of body shapes
export type HttpBody = Empty | Raw | Uint8Array | FormData | Stream
export declare namespace HttpBody {
  interface Proto extends Inspectable { readonly [TypeId]: TypeId; readonly _tag: string; readonly contentType?: string; readonly contentLength?: number }
}
...
```ts
// @effect/platform/UrlParams — a readonly array of [key, value] pairs
...
```ts
// @effect/platform/Cookies — branded record of Cookie; CookiesError is a typed-reason YieldableError
export declare const TypeId: unique symbol
...
export declare class CookiesError extends CookiesError_base<{ readonly reason: "InvalidName" | "InvalidValue" | "InvalidDomain" | "InvalidPath" | "InfinityMaxAge" }> { /* _tag: "CookieError" */ }
...
The platform-wide fault rail — two `Schema.TaggedError` classes with a shared `module` literal axis and a `reason` literal axis on `SystemError`. This is the one platform fault family; `KeyValueStore`/`PlatformConfigProvider`/`HttpBody.file` all fail into it.
...
// the module discriminant shared by both errors
...
export declare class BadArgument extends BadArgument_base {
  // _tag: "BadArgument"; fields: module, method: string, description?: string, cause?: Defect
...
export declare class SystemError extends SystemError_base {
  // _tag: "SystemError"; fields: reason: SystemErrorReason, module, method: string,
  //   description?: string, syscall?: string, pathOrDescriptor?: string | number, cause?: Defect
  readonly [TypeId]: typeof TypeId
  get message(): string
}
...
// helper to mint custom typeId-branded YieldableError classes
...
The platform key-value face `LocalPersistence` binds. `forSchema` derives a typed `SchemaStore` projection off the one store — never a parallel typed store. `layerStorage` over `globalThis.localStorage` is the browser binding; `layerMemory` is the test binding.
...
export declare const layerStorage: (evaluate: LazyArg<Storage>) => Layer.Layer<KeyValueStore>            // browser localStorage/sessionStorage binding
...
The `DecodeWorkerPool` backing surface: a `PlatformWorker` spawns a `BackingWorker`, a `WorkerManager` lifts it into typed `Worker`s, and `makePool`/`makePoolLayer` build the pool. The `SerializedWorker*` variants thread `Schema.TaggedRequest` types end-to-end — the form `DecodeWorkerPool` uses to ship schema-typed decode requests to the worker.
...
export declare namespace Worker {
  interface Options<I> { readonly encode?: (message: I) => Effect.Effect<unknown, WorkerError>; readonly initialMessage?: LazyArg<I> }
}
...
export declare namespace WorkerPool {
  // fixed size, or min/max with timeToLive
  type Options<I> = Worker.Options<I> & ({ readonly size: number; readonly concurrency?: number; readonly targetUtilization?: number; readonly onCreate?: … }
    | { readonly minSize: number; readonly maxSize: number; readonly timeToLive: Duration.DurationInput; readonly concurrency?: number; readonly targetUtilization?: number; readonly onCreate?: … })
}
...
// schema-typed pool — threads Schema.TaggedRequest result/error/context per request
...
`SerializedRunner.Handlers<A>` is a `_tag`-keyed record of process functions; each returns a `Stream`/`Effect`/`Layer` for that request — the polymorphic handler dispatch the worker module implements.
...
export declare class WorkerError extends WorkerError_base {
  // Schema.TaggedError, _tag: "WorkerError"; fields: reason: "spawn"|"decode"|"send"|"unknown"|"encode", cause: Defect
...
`WorkerError.Cause` is the serializable cause schema that crosses the worker boundary — it carries the full `Cause` of a worker-side defect back to the pool.

## [12] — Transferable

Zero-copy transfer collection for the worker boundary — the `Collector` accumulates `globalThis.Transferable` handles a message carries, and `schema` annotates a `Schema` so its encoded form contributes transferables.
...
export declare class Collector extends Collector_base {}   // Context.TagClass "@effect/platform/Transferable/Collector"
...
`Transferable.Uint8Array` is the schema that marks a `Uint8Array` payload for zero-copy transfer — the decode worker ships the `ArrayBuffer` without a structured-clone copy.

## [13] — schema-channel encoders: MsgPack / Ndjson / ChannelSchema

The framing-codec layer. Each is an `effect/Channel` transform between `Chunk<Uint8Array>` (or `Chunk<string>`) and `Chunk<A>`; the `*Schema` variants thread a `Schema.Schema<A, I, R>` so the channel decodes/encodes in one pass with a `ParseError` rail. `SnapshotFeed` (msgpack persistence) and the worker rails compose these.

```ts
// @effect/platform/MsgPack
export declare class MsgPackError extends MsgPackError_base<{ readonly reason: "Pack" | "Unpack"; readonly cause: unknown }> {}   // _tag: "MsgPackError"
...
// transformOrFail-shaped Schema codec form
...
```ts
// @effect/platform/Ndjson — newline-delimited JSON framing; byte and string variants
export declare class NdjsonError extends NdjsonError_base<{ readonly reason: "Pack" | "Unpack"; readonly cause: unknown }> {}   // _tag: "NdjsonError"
...
```ts
// @effect/platform/ChannelSchema — pure Schema encode/decode as a Channel (no framing)
...
WebSocket / transform-stream transport as an `effect/Channel`. `layerWebSocket(url)` + `layerWebSocketConstructorGlobal` is the browser binding; `makeWebSocketChannel` yields a duplex channel a `Stream` consumes.
...
export declare class SocketGenericError extends SocketGenericError_base<{ readonly reason: "Write" | "Read" | "Open" | "OpenTimeout"; readonly cause: unknown }> {}   // _tag: "SocketError"
...
The node-entry boundary. `runMain` is the single-shot entry that runs the application `Effect`, installs a pretty logger and error reporter, wires SIGINT teardown, and exits the process — the `@rasm/node-durable` node entry calls `runMain(program)`.
...
`runMain` itself is not exported from this abstract module — `makeRunMain` is the factory and the platform driver (`@effect/platform-node` `NodeRuntime.runMain`) supplies the bound `RunMain`. The abstract `Teardown`/`RunMain` shapes are what the node entry types against.

## [16] — PlatformConfigProvider

Config-provider layers sourced from the filesystem — a dotenv file or a directory tree. The node tier composes `layerDotEnv` to source `RuntimeConfig`/`SecretResolver` values from a `.env` at boot.
...
`layerDotEnvAdd`/`layerFileTreeAdd` merge their provider onto the ambient `ConfigProvider` (additive); the non-`Add` forms replace it. Both require a `FileSystem` (and `Path` for the tree form), so they are node-tier-only — the browser sources config through `RuntimeConfig` literals, not the filesystem.
