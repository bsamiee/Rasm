...
- Law: `Contribution.protocols` and `Contribution.codecs` are the RPC serve rosters — protocol rows (`http` and `websocket` as path-parameterized factories, `worker` as the runner Layer) crossed with serialization rows (`json`, `ndjson`, `msgpack`) — two orthogonal axes selected once at the app root; a transport or codec choice inside a handler, or a procedure re-declared per transport, is the named defect.
