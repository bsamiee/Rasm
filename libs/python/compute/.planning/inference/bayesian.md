...
[BAYESIAN]: the prior/likelihood/posterior graph, the sampler-backend axis, and arviz diagnostics on one `Inference` owner.
    ...
    match plan.backend:
        case "pymc_native":
            ...
            return pymc.sample(
                ...
                random_seed=plan.seed, step=step, progressbar=False, return_inferencedata=True, model=model,
            )
        case "numpyro_jax":
