...
[INTEGRATION_STACK]:
- `exchange/metadata#METADATA` ownership: `iptcinfo3` is SUPERSEDED on the RASTER carrier and carries no manifest row — the rebuilt `Metadata` owner folds EXIF/IPTC/XMP/ICC in ONE cross-format pass over `pyexiftool`, so the former four-provider split (`iptcinfo3` IIM + `libxmp` XMP + `pyvips` ICC + `exif`) is no longer composed and `iptcinfo3` has no live `metadata.py` consumer. The read/write folds below document the SUPERSEDED IIM integration as API reference, not a live path.
