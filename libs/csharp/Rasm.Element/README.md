...
`Rasm.Element` owns the canonical property-graph element model at the lowest AEC-DOMAIN seam between the `Rasm` kernel and the AEC peers: one authoritative `ElementGraph` folds header, neutral node-and-relationship vocabulary, and a built-once incidence index, and the consumer element is a memoized `Bake` over the reachable subgraph rather than a second stored record. Peers project onto this graph and never reference each other — alignment travels through the cross-stratum contracts and the content-keyed wire, and the seam references the kernel alone, carries no host geometry, and re-mints no identity the kernel owns.
