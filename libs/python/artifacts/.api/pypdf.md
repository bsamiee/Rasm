...
[STACKING]:
- `expression`(`libs/python/.api/expression.md`): `DependencyError`, `PdfReadError`, and `PdfStreamError` convert to `BoundaryFault` at `async_boundary`; `decrypt` returns `PasswordType`, and `extract_text` returns `""` for undecodable spans.
