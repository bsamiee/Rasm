...
- `api-arrow`(`libs/csharp/Rasm.Persistence/.api/api-arrow.md`): the Persistence partition owns the IPC, ADBC, Flight, Flight-SQL, and compression halves of the same distribution and carries the estate's earned behavioural law for them — read its `[SUBSTRAIT_COMMAND_UNROUTED]` topology entry before writing any plan-carrying or Flight-facing member, since the `FlightSqlServer` dispatch matches neither generated Substrait command shape. A sealed `RecordBatch` crosses to the one lake custodian at `Rasm.Persistence/Query/columnar#FLAT_TABLE_EGRESS` `Land`, which owns writers, residence, hive generation, and index custody; the `#FLIGHT_RESULT_PLANE` Flight server is the READ end serving plans back, never a landing door, so a producer dialing Flight to write forks lake custody.
