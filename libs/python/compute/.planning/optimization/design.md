...
- [1]-[DESIGN]: field/mesh/density inverse-design over an Equinox-parameterized objective through Optimistix `minimise`/`least_squares` with the implicit-adjoint gradient, folding one `OptimizationReceipt` on one `DesignProblem` owner.
