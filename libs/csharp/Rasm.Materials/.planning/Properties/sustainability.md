...
- Owner: `SustainabilityRow` the published-data ingress record; `SustainabilityUncertainty` the named relative-uncertainty profile scalar row overloads lift through; `SustainabilityCatalogue` the registered-row database; `Lower` the row→seam-case lowering; the EN 15978 / EN 15804+A2 / ISO 14040 lifecycle-and-unit-cost source.
...
// physics). A flat DATA record (the raw EPD/unit-cost values), NOT a parallel union.
...
public sealed record CostDatum(
    ...
    ScalarDatum Supply,
    ScalarDatum Install,
    ScalarDatum Lifecycle);

public readonly record struct SustainabilityUncertainty(
    double StageGwp,
    double Fraction,
    double Supply,
    double Install,
    double Lifecycle) {
    public static readonly SustainabilityUncertainty Catalogue = new(0.15, 0.10, 0.20, 0.25, 0.25);
}
...
public sealed record SustainabilityRow(
    ...
    ImmutableArray<ScalarDatum> StageGwp,
    string Epd,
    int ValidUntilYear,
    ScalarDatum Recycled,
    ScalarDatum Recovery,
    ...
    public SustainabilityRow(
        ...
        SustainabilityUncertainty uncertainty,
        double[] stageGwp,
        string epd,
        int validUntilYear,
        ...
        : this(
            environmentalBasis,
            Scalars(stageGwp, PropertyEvidenceDefaults.Declaration("epd", epd, validUntilYear), uncertainty.StageGwp),
            epd,
            validUntilYear,
            ScalarOf(recycledContent, PropertyEvidenceDefaults.Declaration("epd", epd, validUntilYear), uncertainty.Fraction),
            ScalarOf(endOfLifeRecovery, PropertyEvidenceDefaults.Declaration("epd", epd, validUntilYear), uncertainty.Fraction),
            cost.Map(c => new CostDatum(
                c.Currency,
                c.Basis,
                ScalarOf(c.Supply, PropertyEvidenceDefaults.Catalogue, uncertainty.Supply),
                ScalarOf(c.Install, PropertyEvidenceDefaults.Catalogue, uncertainty.Install),
                ScalarOf(c.Lifecycle, PropertyEvidenceDefaults.Catalogue, uncertainty.Lifecycle))),
            classification,
            PropertyEvidenceDefaults.Declaration("epd", epd, validUntilYear)) { }
    ...
    public SustainabilityRow(
        ...
        Option<(string Currency, string Basis, double Supply, double Install, double Lifecycle)> cost,
        Option<(string System, string Code)> classification)
        : this(environmentalBasis, SustainabilityUncertainty.Catalogue, stageGwp, epd, validUntilYear, recycledContent, endOfLifeRecovery, cost, classification) { }
    ...
    static ImmutableArray<ScalarDatum> Scalars(ReadOnlySpan<double> values, PropertyEvidence evidence, double relativeUncertainty) {
        ImmutableArray<ScalarDatum>.Builder builder = ImmutableArray.CreateBuilder<ScalarDatum>(values.Length);
        foreach (double value in values) { builder.Add(ScalarOf(value, evidence, relativeUncertainty)); }
...
public static class SustainabilityCatalogue {
    ...
    public static Fin<Seq<MaterialPropertySet>> Lower(SustainabilityRow row, Op key) =>
        row.StageGwp.Length != LifecycleStage.Count
            ...
            : from basis in MeasurementBasis.Parse(row.EnvironmentalBasis, key)
              from environmental in MaterialPropertySet.OfEnvironmental(basis, MaterialPropertySet.Environmental.CarbonMatrix(StageValues(row.StageGwp)), Central(row.Recycled), Central(row.Recovery), row.Epd, row.ValidUntilYear, key, row.Evidence)
              from cost in row.Cost.Match(
                  ...
                  Some: c => from currency in Currency.Parse(c.Currency, key)
                             ...
                             from priced in MaterialPropertySet.OfCost(currency, costBasis, Central(c.Supply), Central(c.Install), Central(c.Lifecycle), key, EvidenceOf(c))
                             select Seq(priced))
              select Seq(environmental) + cost;

    static double Central(ScalarDatum datum) => datum.Value.CentralValue;

    static PropertyEvidence EvidenceOf(CostDatum datum) =>
        PropertyEvidenceDefaults.Normalize(datum.Supply.Evidence);

    static ReadOnlyMemory<double> StageValues(ImmutableArray<ScalarDatum> values) {
    ...
    // empty lifecycle set, never a fault. Cost columns are USD unit-cost estimates at the matching basis (supplementary
    ...
    public static readonly FrozenDictionary<MaterialId, SustainabilityRow> Rows = new (MaterialId Id, SustainabilityRow Row)[] {
        ...
        // generic structural-steel alias (properties.md metal.steel — the unspecified-grade S235 baseline the Component.SubstanceId resolves): shares the WorldSteel section eco-profile (GWP tracks mass not grade)
        ...
        //     parity with properties.md so a projected board's substance resolves a mechanical/thermal row THERE and this
        //     lifecycle row HERE). fibre-cement (ASTM C1325; per-m3; Etex/James-Hardie EN 15804+A2 fibre-cement EPD ~350
        //     kgCO2e/m3, portland-cement A1-A3, mineral so NO biogenic — the sign profile matches masonry not timber)
        ...
        // negative A1-A3 sequestration, positive C combustion release, exactly the timber-row sign convention; OSB -982
        // kgCO2e/m3 the CORRIM cradle-to-gate carbon balance, plywood ~-800 the denser-panel biogenic-inclusive datum)
