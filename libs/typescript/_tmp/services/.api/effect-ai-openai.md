...
Grounded from installed `node_modules` type declarations (`@effect/ai-openai` `0.40.1`, MIT, `dist/dts/*.d.ts`; peers `@effect/ai` `0.36.0`, `@effect/platform`, `effect` `3.21.4`; ESM, node `>=24`). Covers every load-bearing public surface the TS planning pages consume — the eight owner modules re-exported from the package root. The `Generated` module (1236 OpenAPI-derived `Schema.Class` owners + the `make` Client factory, a 219-method REST `Client` interface, and a `ClientError` rail) is summarized as a category, not enumerated; only the named anchors the owner modules reference are spelled exactly. Sibling provider packages (`@effect/ai`, `@effect/ai-anthropic`) share the same `LanguageModel`/`EmbeddingModel`/`Tool`/`Tokenizer`/`Telemetry` contracts — this package is the OpenAI binding. Tier: `node` (routed under the node-durable driver host alongside the other `@effect/ai*` provider layers); requires `HttpClient.HttpClient` from `@effect/platform` (node binding `NodeHttpClient.layer`, browser binding `FetchHttpClient.layer`). It resolves through the shared adapter seam — `effect-ai.md` `PROVIDER_RESOLUTION`/`STACKING_LAW` own the composition-root layer stack (`HttpClient` → `OpenAiClient.layer` → `OpenAiLanguageModel.model` → core `LanguageModel.LanguageModel`); this page documents only OpenAI's DELTA surface (Responses-API streaming events, dual embedding batching, provider-defined tools, telemetry namespacing).
...
## [STACKING] — how the OpenAI delta composes onto the shared rails

- Composition root (one arm of the `ai.md` `AiProvider` literal, `effect-ai.md` `STACKING_LAW`): `HttpClient.HttpClient` (`NodeHttpClient.layer`, `effect-platform-node.md`, node tier) → `OpenAiClient.layer({ apiKey, organizationId?, projectId?: Redacted })` / `layerConfig(... Config.redacted)` resolving through the `security/secret#SECRET_STORE` `SecretStore` → `OpenAiLanguageModel.layer({ model })`, or `layerWithTokenizer` when `Tokenizer.truncate`/token-budgeting is needed (folds `OpenAiTokenizer.layer`) → core `LanguageModel.LanguageModel`.
- Streaming: `createResponseStream` folds the 49-member `ResponseStreamEvent` union under `Stream.runForEach` (`effect.md`), discriminating on `event.type` — one fold surface, never parallel per-event handlers; the core `streamText` collapses the same events into `Response.StreamPart<Tools>`.
- Dual embedding batching: `OpenAiEmbeddingModel.model(id, { mode: "batched" | "data-loader", ... })` is one polymorphic entry resolving `EmbeddingModel.EmbeddingModel`; the scoped `layerDataLoader` is the windowed batch-embed rail `search/embedding#EMBEDDING` feeds the fused-rank vector arm, `layerBatched` the per-call-coalesced form.
- Provider-defined tools (`WebSearch`/`FileSearch`/`CodeInterpreter`/`WebSearchPreview`) stack as `Toolkit.make(...)` rows alongside user `Tool.make` tools (`agent/runtime#AGENT_RUNTIME`); the `FileSearch.filters` comparison/compound node is the one canonical filter algebra, parameterized — never a per-operator constructor.
- Telemetry: `OpenAiTelemetry.addGenAIAnnotations` extends the core `Telemetry.GenAITelemetryAttributes` with `gen_ai.openai.request.*` rows (`serviceTier`, `systemFingerprint`), mutating the ambient `Effect.withSpan` span in place with `system: "openai"` (`effect-ai.md` `TELEMETRY_LAW`) for the `execution/slo.md` reader. All failures collapse to `AiError.AiError`.
