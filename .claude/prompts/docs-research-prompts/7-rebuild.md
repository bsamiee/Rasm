...
- Read the finalized sibling pages in `<TARGET>`'s folder — the doctrine's atlas states which — as binding law and corpus voice: their territory is never restated, referenced, or contradicted, and their settled vocabulary, owners, and rails arrive as given. `<TARGET>` itself stays unread until reconciliation. The replacement takes whatever form serves it best — siblings are reference, not a mold, and the incumbent is neither.
...
- The region ledger's page regions at the reports root mark the territory finalized pages own: a target card that lands on one is re-teach, cut at the plan.
...
- Findings-grade prose only: state as fact and law, with no meta-commentary, no process residue, no route pointers, and no narration of what the page is or does. The page ends at its last card section — no confirmation, checklist, collapse-test, or gate tail.
...
- Card lines are dense, never heavy: one controlling clause per line in declarative register — the line instructs or rejects, never describes; clauses after the first qualify the line's one decision, a second decision splits the line, and a line names the surfaces it legislates as code spans, never paraphrasing a nameable attribute, knob, operator, or generated member.
