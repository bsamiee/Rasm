...
- within-lib concurrency: the `export/layered` `TIFF` arm crosses ONE `HOSTILE` process worker boundary (`self.lane.offload(...)`, reusing the ORA `pyvips` composite path), so the `pyvips` composite, the `psdtags` graph author, and the `imagecodecs` compression share that single crossing with `tifftag`/strip `maxworkers` fans bounded inside it, never a second `anyio` worker nor the `RELEASING` thread arm the `SVG`/`PDF` arms use.
- within-lib domains: the layered-export policy is a typed `msgspec.Struct` whose validated `BlendMode`/`PsdCompressionType` values feed the `PsdBlendMode[blend.name]` index, the `beartype`-annotated boundary surfaces an out-of-bounds bbox as the `export/layered` `ExportFault`, and the author/parse/codec are pure and deterministic, never `@retry`-wrapped.
