...
- `api-npgsql`(`libs/csharp/.api/api-npgsql.md`)/`api-npgsql-ef`(`.api/api-npgsql-ef.md`): every surface lands as raw SQL over the Npgsql connection — the `SET`/`CALL` bootstrap and `duckdb.*` admin calls through `NpgsqlBatch`, the storage DDL via `MigrationBuilder.Sql`, and the `read_*` projections via `FromSql`/`SqlQuery` on the boundary the native analytical baseline rides.
