...
- Packages: `fsspec`, `s3fs`, `gcsfs`, `obstore` (`store.from_url(retry_config=...)` ENTRYPOINTS [01] / `store.RetryConfig` PUBLIC_TYPES config [03], the `TYPE_CHECKING`-only `TypedDict` the `STORE_RETRY` dict literal conforms to / `get_async` ENTRYPOINTS [01] / `GetResult.bytes()` PUBLIC_TYPES [02] / `GetResult.stream(min_chunk_size)` method returning `BytesStream` PUBLIC_TYPES [02]/[11] / `Bytes` zero-copy buffer PUBLIC_TYPES [01] / `list` ENTRYPOINTS [01] `chunk_size`/`return_arrow=True` / `sign_async` ENTRYPOINTS [09] `expires_in: timedelta` over a `SignCapableStore` / `exceptions.BaseError` PUBLIC_TYPES exceptions [01]), `universal-pathlib` (`UPath`/`resolve`/`is_relative_to`/`read_bytes`/`open`), `httpx` (`AsyncClient.get` ENTRYPOINTS [03]/`AsyncClient.stream` ENTRYPOINTS [06]/`Response.aiter_bytes` ENTRYPOINTS [04]/`Response.content` the buffered-body accessor/`Response.raise_for_status` ENTRYPOINTS [01] bound through the `AsyncClient(event_hooks={"response": [...]})` construction seam ENTRYPOINTS [01]/`Auth` PUBLIC_TYPES auth [01] with the `auth_flow` generator the `_BearerAuth` overrides — `httpx` ships no bearer-`Auth` class, so the custom flow is the catalog seam — and the per-call `auth=` argument/`Timeout` PUBLIC_TYPES [09]/`Limits` PUBLIC_TYPES [10]/`HTTPError` PUBLIC_TYPES [01]), `asyncssh` (`connect` ENTRYPOINTS [01] `options=` form / `SSHClientConnectionOptions` PUBLIC_TYPES connection [03] the one connection-config value object the options-law mandates / `start_sftp_client` ENTRYPOINTS [05] / `read_known_hosts` ENTRYPOINTS [02] / `SFTPClient.open` ENTRYPOINTS [06] / `SFTPClientFile.read` ENTRYPOINTS [07] / `SSHKnownHosts` PUBLIC_TYPES [04] / `Error` PUBLIC_TYPES [01]), `anyio` (`to_thread.run_sync` thread dispatch ENTRYPOINTS [01], the bounded worker-thread offload for the blocking filesystem `read_bytes`/`open`/`read` so the cp315 event loop never blocks; `create_task_group` ENTRYPOINTS [02] the structured scope the `drain` teardown `aclose`s every pooled client under — branch catalog `libs/python/.api/anyio.md`), `expression` (`Error`/`Ok`/`Option`/`Option.map`/`Option.to_optional`/`Option.default_value`/`tagged_union`/`case`/`tag`), `pydantic` (`SecretStr` PUBLIC_TYPES [09] the repr/serialization-masking carrier the `http` bearer and `ssh` password ride, un-masked through `.get_secret_value()` only at the `_BearerAuth`/`_ssh_options` seam, the bare-`str` secret the `execution/admission#SETTINGS` boundary law deletes), `arro3.core` (`RecordBatch` the obstore Arrow listing element-type carried by `survey(StoreView.LIST)`, type-checking annotation only), `reliability/resilience#RESILIENCE` (`guarded(cls, thunk, subject=...)` — the retried-traced-railed acquisition every WHOLE arm delegates to; the per-class retry, the derivation span, and the terminal-fault lift are all the resilience owner's, never re-spelled here), `reliability/faults#FAULT` (`RuntimeRail`/`BoundaryFault`).
...
- Boundary: this page owns acquisition only. It mints no default root creation, root litter, product store-root derivation, bridge staging-root ownership, service API layer, companion-control transport, or AEC-collaboration client. Speckle terminates C#-side (`cs:Rasm.Persistence/Sync/collaboration`) per the cross-`libs/` `EXTERNAL_SYSTEMS_TERMINATE_CSHARP` boundary, and the companion reads any Speckle-sourced artifact through the canonical wire, never a `specklepy` client. The deleted forms:
...
- [STREAMING_ACQUISITION]: `GetResult.stream(min_chunk_size)` returns a `BytesStream` async iterator of `Bytes` chunks; the `STREAM` arm calls `result.stream(STREAM_CHUNK)` so the same `STREAM_CHUNK` floor governs the obstore stream and the filesystem `_file_chunks` block read (the obstore Rust core coalesces above the floor), never a no-argument property read of the bound method object. `httpx.AsyncClient.stream` + `Response.aiter_bytes` carry the HTTP body under `httpx.Timeout`/`httpx.Limits`; full-body reads are reserved for small payloads. The `STREAM` arm returns the lazy `AsyncIterator[Bytes]` un-fenced because the `get_async` materialize, the `_sftp_session` connect, and the `httpx.stream` open all run inside the generator body, so a provider raise surfaces during iteration at the `evidence/identity#IDENTITY` `ContentIdentity.of` consumer's boundary. The chunk `AsyncIterator[Bytes]` folds into the `evidence/identity#IDENTITY` `Source` `stream` arm where the buffer-protocol `Bytes` feeds `xxh3_128.update` with no `bytes(...)` re-copy, no second hashing pass, no whole-artifact buffer. The streaming `xxh3_128` updater rides the `python_version<'3.15'` companion band the `evidence/identity#IDENTITY` `[XXHASH_PARITY]` seed inherits; the cp315 `xxhash` wheel gates the end-to-end streaming hash.
