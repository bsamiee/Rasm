...
- `NetTopologySuite.IO.GeoJSON4STJ` / `NetTopologySuite.IO.GeoPackage` (`api-nts-geojson4stj.md`, `api-nts-geopackage.md`): `GetCellBoundaries` returns one `MultiPolygon` per cover, the shape the GeoJSON4STJ writer serializes as a single GeoJSON document and the GeoPackage writer stores as one blob — never a hand-spelled GeoJSON document at either consumer.
