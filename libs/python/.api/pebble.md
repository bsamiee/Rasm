...
|  [07]   | `ProcessPool._pool_manager.worker_manager.workers -> dict[int, Process]`               | private  | live `{pid: worker}` map     |
...
[STACKING]:
- `execution/workers`(`runtime/.planning/execution/workers.md`): the PRIMARY consumer. `WorkerPool` binds pebble to the `(WorkerKind.PROCESS | WorkerKind.GPU, Enforcement.TERMINAL)` arm as `pebble.ProcessPool(max_workers=loky.cpu_count(only_physical_cores=True), max_tasks=64, initializer=partial(_worker_boot, self._boot), context=get_context("spawn"))` — the boot re-arms fidelity, the device pin, and the telemetry install per the kind's `KIND_POLICY` row — and `submit` mints the future through `self._pebble.schedule(sealed_kernel, args=(cloudpickle.dumps((None, carrier, kernel, args)),), timeout=kernel.deadline.to_optional())` — the 4-tuple's `boot` slot non-`None` only on the remote floor — one sealed bytes payload so a closure-bearing argument survives the stdlib pickler; a cooperative cancel escalates through `ProcessFuture.cancel()` to reclaim the killable slot, `drain()` calls `close()`+`join()`, `retire()` calls `stop()`+`join()`. `ProcessExpired` (ctor `(msg, code=0, pid=None)`; the death evidence rides its `.exitcode`/`.pid` attributes) and `TimeoutError` convert onto the fault channel here — pebble owns the terminal-kill arm, the pool owns the executor lifecycle; `worker_manager.workers` is the supervisor's arm-scoping pid read.
...
- `psutil`(`.api/psutil.md`): `Supervisor` weighs each arm's OWN named pids (`WorkerPool.pids()`, total across every arm — pebble's map read included) under one `oneshot()` per worker with `memory_info().rss` against `SupervisionPolicy.rss_ceiling`, off the loop under the supervisor's own band — no process-tree walk and no complement — rolling the pebble arm on a `DEGRADED` breach and retiring-then-respawning on `DEAD`; `max_tasks=64` recycling and the psutil ceiling probe are the two RSS-bounding arms — pebble reclaims on a fixed task cadence, psutil on a live ceiling breach.
...
- `reliability/resilience`(`runtime/.planning/reliability/resilience.md`): `ProcessExpired`/`BrokenProcessPool`/`TerminatedWorkerError` match by module-qualified spelling (`_backoff("pebble.common.types.ProcessExpired", ...)`) in the `RetryClass.WORKER` `Policy` row (`attempts=3`, `timeout=120.0`, `wait_initial=0.5`), import-free at the BASE tier, so a died-worker leg respawns and retries inside one span; distinct from `RetryClass.OCCT`, which targets the `anyio.BrokenWorkerProcess`/`BrokenWorkerInterpreter` pair.
