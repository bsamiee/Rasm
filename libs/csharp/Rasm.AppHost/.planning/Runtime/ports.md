...
Rasm.AppHost exposes exactly seven typed port records as its only INWARD cross-package seam; siblings adapt to them and no sibling assembly enters the AppHost graph. The OUTWARD tier is named, never denied: the settled sibling-owned vocabularies AppHost composes by reference — the Compute intent rail and channel policy, the Persistence coordination op-union/token/changefeed shapes, and AppHost's own outward exports — are `[WIRE_VOCABULARY]` ledger rows composed at the boundary, never re-declared, never a reversed interior dependency, and never smuggled under a seven-ports-only claim. The owned axes are the port-record family with the HLC receipt envelope, the HLC two-half fan-in law, the boot-minted `TenantContext` tenancy primitive stamped on the envelope, the suite JSON wire law with its contract-merge rule, and the TS tooling map. Drain bands, deadline rows, phase vocabulary, the classification taxonomy, and the degradation vocabulary arrive settled from the package spine and compose here as port payloads.

## [01]-[INDEX]

- [01]-[PORT_RECORDS]: Seven inward port records; `HLC` envelope and `TenantContext` cross-process primitives.
- [02]-[HLC_FANIN]: The two-half stamp law — one mint, fixed half order, cross-runtime parity.
