...
public partial record Boundaries : IAspect {
    ...
    public global::Rasm.Analysis.Operation<TGeometry, TOut> Operation<TGeometry, TOut>() where TGeometry : notnull => Switch<global::Rasm.Analysis.Operation<TGeometry, TOut>>(
        ...
        outlineCase: static o => (typeof(TGeometry) == typeof(Mesh) && typeof(TOut) == typeof(Polyline) && o.Plane.IsValid)
            ? Analyze.Cast<TGeometry, TOut>(key: OutlineKey, operation: global::Rasm.Analysis.Operation<Mesh, Polyline>.Build(key: OutlineKey, state: (Op: OutlineKey, o.Plane), evaluator: static (state, geometry) => state.Op.Accept(values: geometry.GetOutlines(plane: state.Plane)).ToEff()))
            : (typeof(TGeometry) == typeof(Mesh) && typeof(TOut) == typeof(Polyline))
                ? Analyze.Cast<TGeometry, TOut>(key: OutlineKey, operation: global::Rasm.Analysis.Operation<Mesh, Polyline>.Reject(key: OutlineKey, fault: OutlineKey.InvalidInput()))
...
public static partial class Analyze {
    public static global::Rasm.Analysis.Operation<TGeometry, TOut> Boundaries<TGeometry, TOut>(Boundaries aspect) where TGeometry : notnull => Aspect<Boundaries, TGeometry, TOut>(aspect: aspect);
}
