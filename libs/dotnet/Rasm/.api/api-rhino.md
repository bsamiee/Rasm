...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the point, vector, frame, segment, extent, affine, and rotor algebra resolves there whole; this partition composes it — `Point3f`/`Vector3f` widen into it, every primitive solid is a named-part composition over its carriers, and every reference type inherits its `GeometryBase` bounds and identity.
...
- `Numerics/calculus#SOLAR_EPHEMERIS`: `SunPosition.Direction` mints the survey ray as a `Vector3d` and `OfDirection` inverts it through `IsValid`, `Length`, and `operator /(Vector3d, double)`, so the host sentinel screens where a finiteness probe passes it and the bijection keeps double throughout.
