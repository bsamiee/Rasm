...
[STACKING]:
- `System.Xml`(`libs/dotnet/.api/api-bcl-xml.md`): `ProtectedConfigurationProvider.Encrypt(XmlNode)` and `Decrypt(XmlNode)` exchange DOM nodes, so section protection composes the owned XML surface instead of a private parser.
