...
- [HATCH_RENDER_MULTILEADER_TRANSFORM] [RESOLVED]: four reading-map underutilization gaps closed by growing the existing owners in place (verified against ezdxf 1.4.x), never a new file or parallel surface. (1) HATCH FILL — the `hatch` case's `int` fill color widened to the `HatchFill` `tagged_union` (`Solid`/`Pattern`/`Gradient`) dispatched in the `_build_entity` hatch arm over `set_solid_fill(color=)`/`set_pattern_fill(name, color=, scale=, angle=, definition=)`/`set_gradient(color1=colors.RGB, color2=colors.RGB, rotation=)` (all three verified on the `Hatch` entity); solid-only hatch was the biggest gap against the AEC material-indication telos, and the `Pattern` case carries an optional `PatternLine` definition the `drawing/standard` owner resolves from the verified 172-entry `tools.pattern.ISO_PATTERN` table and lowers on (dxf owning the fill call, the ISO 128-50 semantics staying `drawing/standard`). (2) RENDER POLICY — `PageSpec.dark` (a `BackgroundPolicy` bool knob) collapsed into the `RenderPolicy` bundle over seven owned StrEnums mirroring the ezdxf `config` policy enums by verified member name (`BackgroundPolicy`/`LineweightPolicy`/`HatchPolicy`/`TextPolicy`/`ColorPolicy`/`ProxyGraphicPolicy`/`LinePolicy`), `_rendered` deriving each ezdxf enum via `getattr(dwgconfig.<Policy>, member.name)` so the payload stays ezdxf-free (laziness + consumer ergonomics preserved), plus the `RenderContext(doc, ctb=, export_mode=)` CTB plot-style + paperspace-export seam (verified ctor); `LineweightPolicy.ABSOLUTE` is the ISO 128 mm-lineweight fidelity the prior hardcoded single-policy `Configuration` dropped. (3) MULTILEADER — the modern leader-with-content entity added as the `multileader` `DxfEntity` case beside the legacy `leader`, `MLeaderKind` selecting `add_multileader_mtext`/`add_multileader_block` (both verified, both exposing `set_content`/`add_leader_line(side, vertices)`/`build(insert)`), each `LeaderLine` folding through `add_leader_line(getattr(mleader.ConnectionSide, side.value), …)` (the `ConnectionSide` members verified lowercase `left`/`right`/`top`/`bottom`, hence by-value derivation) — the ISO 128-2 leader the `drawing/annotate` keynote owner lowers onto. (4) TRANSFORM — the seventh `DxfOp` case `Transform(source, spec)` over `transform.inplace(entities, m)`/`transform.copies(entities, m) -> (logger, list)` (both verified), `TransformSpec` composing `Matrix44.chain(scale, z_rotate, translate)` (verified) and scoping the affine to a `doc.query(eql)` selection under a `TransformMode`, `doc.audit()` re-validating before `_serialize` — a first-class CAD op the composed `Matrix44` bridge already used but never exposed. Each is one case/field/member + one arm; no `ArtifactReceipt.Cad` shape change (a transformed/authored doc contributes the same case). The acknowledged deferred axes stay deferred: `r12writer` streaming `DxfFormat`, `EntityQuery.union`/`difference`/`intersection` set-algebra on `Selection`, `select.PlanarSearchIndex` reuse.
- [CROSS_FILE]: the new `ArtifactReceipt.Cad(key, dxfversion, units, artifact, bytes, layers, blocks, errors, fixes, counts)` case lands on `core/receipt.md` (the `cad` `ArtifactKind` token + case tuple + `Cad` mint + the `cad` or-pattern arm in `slot` + the special `_facts` arm flattening `counts` like `preview`/`verdict`) so `Dxf.contribute` type-checks against the real receipt owner — the receipt union had NO `cad` case, so the prior fence's `ArtifactReceipt.Cad(...)` was a phantom against the sibling owner (MODEL-COHERENCE); the real case is now present. The `export/dxf` codemap node and the `export/dxf → composition/sheet`, `export/dxf → graphic/vector`, `graphic/vector ↔ export/dxf`, and `export/dxf → geospatial` (GeoJSON) seams land on `ARCHITECTURE.md` `[01]`/`[02]`; the `[DXF]` router row lands on `README.md` `[01]` and the `ezdxf` roster row on `README.md` `[02]`. The `drawing/standard` ISO-vocabulary lowering onto the `TableEntry` rows is a future-page consumer contract, not authored here.
