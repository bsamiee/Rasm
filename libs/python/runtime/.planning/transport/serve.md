...
- Boundary: the serve runtime rides the cp315 core floor through `grpcio` (`1.81.1`, `[CORE]` ungated, resolving transitively via `google-cloud-storage` and declared explicit-core) and `protobuf`; only `grpcio-tools` stub regeneration rides the companion lane (`python_version<'3.15'`), so server boot never depends on the companion environment and the cp315 core never imports `grpc_tools`. The UDS serve leg reads no OS keyring (peer identity is the kernel accept-time credential); `keyring` is the `execution/admission#SETTINGS` `SecretBoundary` owner for the outbound transport legs, not a serve-side concern.
