...
[CLASSIFICATION_TAXONOMY_OWNER]-[COMPLETE]: the boot federation proof landed — `RedactionRegistration.Federated` resolves every branch-declared `(taxonomy, value)` pair against the owner roster and refuses composition naming the offending package and spelling; carriage is the kernel `TelemetryContributorPort.Classifications` column (`ClassifiedValue` pairs, no type reference at any branch), Fabrication and Rhino publish their rosters on it, the composition derives its `ClassificationRoster` set from the contributed ports, `RedactionRegistration.Bind` is now genuinely called from the composition root's logging chain, and the closure generalized past pairs so a multi-annotation member resolves its own row instead of the erasing fallback.
[LANE_ROW_COMPLETENESS]-[COMPLETE]: the two-altitude closure proof landed at `Runtime/laneguard.md` — `Register` returns `Fin<IServiceCollection>` proving the roster-to-row bijection at composition under the boot-strict latency precedent, `Proven` probes every roster row through `TryGetPipeline` on the built provider, and the lane key collapsed to the one `WorkLane.PipelineKey` extension all four reading sites share; the `WorkLane` roster itself now declares at the spine per the branch dispatch-vocabulary ruling, with Compute's `LaneProfiles.Closed()` keyed fold refusing an unprofiled row by name.
[ONE_GOVERNED_MODEL_PLANE]-[COMPLETE]: every model draw the suite makes — chat, embedding, and image — now resolves through the one `ModelGovernance` fold, with decorator order carrying the cache-identity and redaction-idempotence laws, a token-measured history bound against the resolved route window, and the `FunctionInvoker` hook enforcing the brokered receipt contract; the raw `IEmbeddingGenerator` and the direct `IImageGenerator` draw are both deleted forms.
[SANDBOX_WASM_VEHICLE]-[COMPLETE]: the wasm isolation row holds a real owned vehicle rather than prose — module, store, linker, and instance on `PluginInstance` symmetric with the process row's child — and the wall guarantee rests on epoch preemption, the only mechanism the embedding exposes that reaches a guest inside a host-free loop.
...
[FLEET_DEPLOY_ANNOTATIONS]-[COMPLETE]: `RollAnnotationWire` record and per-wave projection fold landed on `Sandbox/provisioning#ROLLOVER_DRAIN`, fanned under `ReceiptKind.Roll` and joined to the `AppHostWireContext` roster.
...
[MACHINE_OBSERVATION_LANE]-[COMPLETE]: `MachineLane`/`MachineObservationWire` decode lane landed on `Wire/livewire.md` fanned under `ReceiptKind.Observation`; the `MachineObservation` domain vocabulary is Fabrication-owned per its observation card.
...
[PARTITIONED_OBSERVABLE_LEVELS]-[COMPLETE]: realized kernel-side per the substrate-homing collapse — keyed `LevelCells` families with `Reader` project per-key levels as tagged `Measurement<long>` rows through the multi-measurement observe overload at `libs/dotnet/Rasm/.planning/Domain/instrument.md` `[03]-[MOUNT]`, so a bespoke per-partition gauge is the deleted form.
[CONTRIBUTED_ARM_ROSTER]-[COMPLETE]: contributor projections arrive as composition-root ARGUMENTS to `InstrumentFan.Mount(..., params ReadOnlySpan<TelemetryContributorPort>)` at `Observability/instruments.md` `[03]-[RECEIPT_PROJECTION]` — the static `[CONTRIBUTED_ARMS]` table is the deleted form a composition argument supersedes; each domain package's projection, port mint, and custody rides its own port.
