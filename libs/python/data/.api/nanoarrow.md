...
| [INDEX] | [SYMBOL]       | [TYPE_FAMILY]   | [ROLE]                                   |
| :-----: | :------------- | :-------------- | :--------------------------------------- |
|  [01]   | `CArray`       | C array holder  | low-level `ArrowArray` C struct wrapper  |
|  [02]   | `CArrayStream` | C stream holder | low-level `ArrowArrayStream` C struct    |
|  [03]   | `CSchema`      | C schema holder | low-level `ArrowSchema` C struct wrapper |
|  [04]   | `CBuffer`      | C buffer holder | typed buffer from Arrow buffer accessor  |
...
|  [03]   | `extension_type(storage_schema, name, meta, ...)` | extension type | custom extension schema        |
...
| [INDEX] | [SURFACE]                                            | [ENTRY_FAMILY]    | [CAPABILITY]                      |
| :-----: | :--------------------------------------------------- | :---------------- | :-------------------------------- |
|  [01]   | `array(obj, schema)`                                 | array factory     | wrap any Arrow-compatible object  |
|  [02]   | `Array(obj, schema, device)`                         | array constructor | multi-chunk wrapper construction  |
|  [03]   | `Array.from_chunks(obj, schema, validate)`           | chunked intake    | build from iterable of chunks     |
|  [04]   | `ArrayStream(obj, schema)`                           | stream factory    | wrap any stream-compatible object |
|  [05]   | `ArrayStream.from_path(obj, *args, **kwargs)`        | IO intake         | open stream from file path        |
|  [06]   | `ArrayStream.from_readable(obj)`                     | IO intake         | open stream from file-like object |
|  [07]   | `ArrayStream.from_url(obj, *args, **kwargs)`         | IO intake         | open stream from URL              |
|  [08]   | `ArrayStream.read_all()`                             | consume           | collect all chunks into `Array`   |
|  [09]   | `ArrayStream.read_next()`                            | consume           | pull next `Array` chunk           |
