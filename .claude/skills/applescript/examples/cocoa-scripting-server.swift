...
final class ShapeIndex {
    ...
    func replaceAll(with incoming: [ShapeRecord]) {
        ...
        positionsByValue = Dictionary(uniqueKeysWithValues: INDEXED_KEYS.map { indexed in
            ...
            for (position, record) in incoming.enumerated() { buckets[indexed.project(record), default: []].append(position) }
            return (indexed.cocoaKey, buckets)
        })
    }

    func position(ofID uniqueID: String) -> Int? { positionByID[uniqueID] }
...
@objcMembers final class Shape: NSObject {
    ...
    dynamic var name: String { record.name }
    dynamic var layer: String { record.layer }

    override func scriptingIsEqual(to object: Any) -> Bool { compare(object) { $0 == $1 } }
    override func scriptingBegins(with object: Any) -> Bool { compare(object) { $0.hasPrefix($1) } }
    override func scriptingContains(_ object: Any) -> Bool { compare(object) { $0.contains($1) } }
...
@objcMembers final class ShapeContainer: NSObject {
    ...
    dynamic var scriptableShapes: [Shape] { facades }
...
final class ShapeExportCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let destination = evaluatedArguments?["DestinationURL"] as? URL,
              let shapes = evaluatedReceivers as? [Shape] else {
