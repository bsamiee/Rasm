...
# reportlab/weasyprint/typst author raise is `RuntimeError`/`ValueError`, and the stream/`.svgz`
# file fault is `OSError`. The boundary narrows on this real raise tuple so `async_boundary`
...
_SIZES: frozendict[SheetSize, Dimensions] = frozendict({
    ...
    SheetSize.JIS_B1: (2031.81, 2920.06),
})
...
_STANDARD: frozendict[Standard, StandardRow] = frozendict({
    ...
    Standard.ARCHIVAL: StandardRow(version=(2, 0), standards=("a-3b",), variant="pdf/a-3b", tags=False),
    Standard.ACCESSIBLE: StandardRow(version=(1, 7), standards=("ua-1",), variant="pdf/ua-1", tags=True),
    Standard.PRESERVED: StandardRow(version=(2, 0), standards=("a-3b", "ua-1"), variant="pdf/a-3b", tags=True),
})
...
class Scale(Struct, frozen=True):  # the drawing scale, drawn as a ratio AND a graphic divided bar
    ...
    def bar(self, x: float, y: float, /) -> tuple[tuple[Box, str], ...]:
        # the graphic scale bar as alternating filled/clear division rects plus the unit caption, drawn at
        # (x, y) bottom-left; an empty tuple when `bar_length` is zero so the author skips it cleanly.
        if self.bar_length <= 0.0 or self.segments <= 0:
            return ()
        step, height = self.bar_length / self.segments, _ROW_H * 0.4
        divisions = tuple(((x + i * step, y, x + (i + 1) * step, y + height), "fill" if i % 2 else "clear") for i in range(self.segments))
...
class TitleBlock(Struct, frozen=True):
    ...
    def grid(self) -> Block[FieldRow]:
        ...
        head = (FieldRow("Project", self.project, group="identity"), FieldRow("Sheet", self.sheet_number, group="identity"),
                FieldRow("Title", self.sheet_title, span=2, group="identity"), FieldRow("Discipline", self.discipline, group="identity"),
                FieldRow("Scale", self.scale.ratio, group="identity"), FieldRow("Date", self.date, group="identity"),
                FieldRow("Drawn", self.drawn_by, group="approval"), FieldRow("Checked", self.checked_by, group="approval"),
...
class SheetOp:  # the closed request vocabulary lowered once into Composed
    ...
    frame: tuple[SheetSize, Engine, TitleBlock, Standard, OutputIntent] = case()
    place: tuple[bytes, tuple[FigurePlacement, ...]] = case()
    fill: tuple[bytes, SheetSize, tuple[FieldRow, ...]] = case()
    stamp: tuple[bytes, TitleBlock, Standard, OutputIntent, tuple[tuple[str, bytes], ...]] = case()
    preview: tuple[bytes, float] = case()

    @staticmethod
    def Frame(size: SheetSize, engine: Engine = Engine.REPORTLAB, title: TitleBlock = TitleBlock(), *, standard: Standard = Standard.DRAFT, output_intent: OutputIntent = None) -> "SheetOp":
        return SheetOp(frame=(size, engine, title, standard, output_intent))
    ...
    @staticmethod
    def Fill(sheet: bytes, size: SheetSize, rows: FieldRow | Iterable[FieldRow]) -> "SheetOp":
        return SheetOp(fill=(sheet, size, _tupled(rows)))
...
def _composed(op: SheetOp) -> Composed:  # the one pure render fold both `of` and `contribute` read
    ...
    match op:
        case SheetOp(tag="frame", frame=(size, engine, title, standard, intent)):
            data = _FRAME[engine](_SIZES[size], title, standard, intent)
        ...
        case SheetOp(tag="fill", fill=(sheet, size, rows)):
            document = pymupdf.open(stream=sheet, filetype="pdf")
            page, bound = document[0], dict(TitleBlock(fields=tuple(rows)).cells(_SIZES[size]))  # value rects from the ONE grid geometry
            for row, rect in bound.items():  # the page is the live native handle the engine mutates in place
...
def _draw_one(document: "Document", placement: FigurePlacement) -> tuple[int, bool, bool]:  # (xref, visible, locked); xref 0 if unlayered
    ...
    page.show_pdf_page(pymupdf.Rect(*placement.cell), docsrc, pno=placement.page, keep_proportion=placement.keep_proportion, rotate=placement.rotate, clip=clip, oc=xref)
...
def _frame_reportlab(dims: Dimensions, title: TitleBlock, standard: Standard, intent: OutputIntent) -> bytes:
    ...
    if title.revisions:
        ...
        revs.textLines("\n".join(f"{r.mark} {r.date} {r.description}" for r in title.history()))
        canvas.drawText(revs)
    for label, glyph in (("north", title.north.glyph), ("key", title.key_plan.figure)):
        if glyph:
            offset = 0.0 if label == "north" else _BLOCK_H + _MARGIN
            canvas.drawImage(ImageReader(BytesIO(glyph)), width - _MARGIN - _BLOCK_W, _MARGIN + offset, width=_MARGIN * 2, height=_MARGIN * 2, preserveAspectRatio=True, mask="auto")
    for shape, mark in title.scale.bar(width - _MARGIN - _BLOCK_W, _MARGIN + _BLOCK_H + _ROW_H):  # the divided graphic scale bar above the block
        if mark in ("fill", "clear"):
            canvas.rect(shape[0], shape[1], shape[2] - shape[0], shape[3] - shape[1], stroke=1, fill=int(mark == "fill"))
        else:
...
def _frame_typst(dims: Dimensions, title: TitleBlock, standard: Standard, _intent: OutputIntent) -> bytes:
    ...
    revs = ", ".join(f"[{_escape(r.mark)}], [{_escape(r.date)}], [{_escape(r.description)}]" for r in title.history())
    source = (
    ...
    return typst.compile(source.encode(), output=None, format="pdf", pdf_standards=_STANDARD[standard].standards, ignore_system_fonts=True)
...
def _frame_weasyprint(dims: Dimensions, title: TitleBlock, standard: Standard, intent: OutputIntent) -> bytes:
    ...
    cells = "".join(f"<tr><th>{_html(r.label)}</th><td>{_html(r.value)}</td></tr>" for r in title.grid())
    title_tag = f"<title>{_html(title.sheet_title or title.sheet_number)}</title>" if tags else ""
...
def _placed_layers(op: SheetOp, names: tuple[str, ...]) -> tuple[Layer, ...]:
    match op:
        case SheetOp(tag="frame", frame=(size, _engine, _title, _standard)):
            w, h = _SIZES[size]
...
def _xmp(title: TitleBlock, standard: Standard) -> str:
    # the archival XMP packet built through the `lxml` element tree (never a hand-concatenated string the
    # `language.md` STRING_RECOVERY law rejects): the Dublin-Core descriptive fields plus the `pdfaid` part/
    # conformance the `Standard` declares, so the metadata states the PDF/A claim rather than only the engine token.
    from lxml import etree

    rdf = etree.Element(etree.QName(_NS["rdf"], "RDF"), nsmap=dict(_NS))
    description = etree.SubElement(rdf, etree.QName(_NS["rdf"], "Description"))
