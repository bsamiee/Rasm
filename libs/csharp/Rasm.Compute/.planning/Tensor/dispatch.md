...
// at the sweep's own cadence edge, the `Runtime/codecs#HDF_ARCHIVE` sanctioned accumulation form, each segment
// declaring its own step count at `Begin`.
public sealed class TapeSpill : IDisposable {
    readonly H5Dataset<float[]> slot;
    readonly HdfWriter writer;
    readonly int steps;
    readonly int width;

    TapeSpill(H5Dataset<float[]> slot, HdfWriter writer, int steps, int width) =>
        (this.slot, this.writer, this.steps, this.width) = (slot, writer, steps, width);

    // One step-chunk per accepted step: chunks are [1, width], so the resident primal is ONE chunk — the spill's
    // whole memory bound — and the forward sweep pushes ordinals in index order through the archive cursor,
    // which refuses out-of-order at admission ahead of the library's mid-encode chunk-once fault.
    public static TapeSpill Begin(int steps, int width, Stream sink, HdfArchivePolicy policy) {
        H5Dataset<float[]> slot = new(fileDims: [(ulong)steps, (ulong)width], chunks: [1u, (uint)width], datasetCreation: policy.Creation());
        H5File graph = new() { ["tape"] = slot };
        graph.Attributes["steps"] = steps;
        graph.Attributes["width"] = width;
        return new(slot, HdfArchive.Begin(graph, sink, policy), steps, width);
    }

    public Fin<Unit> Push(int step, ReadOnlyMemory<float> primal) =>
        Try.lift(() => {
            if (primal.Length != width) { throw new InvalidDataException($"<hdf5-tape-width:{primal.Length}:{width}>"); }
            writer.WriteChunk(slot, primal.ToArray(), step, [steps, 1], [1u, (uint)width]);
            return Unit.Default;
    ...
    // span, so the backward pass holds one step-chunk resident whatever the tape length; the try bracket is the
    // span boundary, every fault leaving on the `<hdf5-…>` slug rail.
    public static Fin<Unit> Replay(HdfHandle handle, string dataset, int step, Span<float> primal) {
        ...
        catch (Exception ex) { return Fin.Fail<Unit>(new ComputeFault.ModelRejected($"<hdf5-tape-replay:{step}:{ex.Message}>")); }
    }
...
// `[Equatable]`: a coloring is deterministic from its sparsity pattern, so caching it across Newton iterations
// keys on value identity — `Colors` otherwise reference-compares (`Pattern` is a Seq, already structural).
[Equatable]
