...
One cluster: `[2]-[STEEL_FAMILY]` owns the `SteelClass` subtype axis, the `SteelSection` section-property record, the `ProfileUnit` projection that flows a steel section through the masonry-shaped `Resolve` fold, and the `ProfileCatalogue.BuildSteelRows` AISC row table.
