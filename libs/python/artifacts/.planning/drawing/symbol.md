...
- [VECTOR_BOOLEAN_OFFSET] [RESOLVED]: `graphic/vector/region#REGION`'s `RegionOp` carries `boolean` and `outline` cases (its family is `boolean`/`outline`/`warp`/`wind`/`contains`/`facts`/`clip`/`text_path`/`transform`/`fit`/`serialize`/`rasterize`, with public `boolean(sources, op, fill)`/`outline(source, width, cap, join, miter, dash)`/`facts(source)` functions), so a `RegionOp.boolean`/`RegionOp.outline` composition resolves today; the one still-rejected phantom is a private-method reach into that owner's module fold. This owner builds the north arrow, the revision triangle, and the match line from `schemdraw` primitives (a filled north half PLUS a hollow south half, a closed `SegmentPoly`, a heavy dashed `Segment`) as the correct self-contained default. The reading-map "single-silhouette north union" is REJECTED, not deferred: the ISO north arrow is TWO-TONE (a filled north half and a hollow south half), so a `RegionOp.boolean(UNION)` merging them into one silhouette would REGRESS the two-tone reading — the schemdraw two-poly is the correct form, not a naive approximation. The genuinely-available `skia-pathops` REFINEMENTS that do NOT regress — a filled-band / double-line match line and a revision-cloud offset — compose the LANDED `graphic/vector/region#REGION` `outline`/`boolean` (verified `boolean(sources, op, fill)`/`outline(source, width, cap, join, miter, dash)` on that owner's public surface) directly at the SVG-bytes seam, shared with `drawing/annotate#ANNOTATE`/`drawing/dimension#DIMENSION`, never authored twice.
