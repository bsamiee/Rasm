# [GEOMETRYCORE_GEOMETRY_KERNEL]

Rasm.GeometryCore geometry kernel: the adaptive-precision exact-predicate floor every higher geometry owner in the suite rides, authored from first principles because no admitted external geometry library carries a robustness guarantee ([ADMISSIONS_RECORD]). The page owns the closed `Predicate` family — `Orient2D`/`Orient3D`/`InCircle`/`InSphere` — each evaluated through one two-stage adaptive arithmetic: a fast IEEE-754 `double` filter gated by a forward error bound, falling back to exact expansion-arithmetic only where the filter's interval straddles zero, returning an EXACT `Sign` verdict in every case. Under it sits `Expansion`, the sign-exact nonoverlapping floating-point expansion kernel (Shewchuk-style `TwoSum`/`TwoProduct`/`Grow`/`ScaleExpansion`/`ExpansionSum` folds with `Estimate`/`Sign` projections), and `ErrorBound`, the static-permanence error-bound row table that owns the per-predicate filter thresholds derived once from machine epsilon. This is the [R1] interior-double exception made concrete: `Expansion` and `ErrorBound` operate on RAW `double` because robust arithmetic is mathematically defined over IEEE-754 doubles, never unit-bearing quantities; `NumericsPolicy` is the named owner documenting the filter-then-exact contract and the only sanctioned interior-double scope. The kernel composes `Rasm`/Vectors `Point3d` coordinates (the domain's native scalar) at the seam, emits only `Sign` verdicts across public signatures, and computes no hash and mints no identity.

Wire posture: this page is INTERIOR-SUBSTANCE and carries no TS_PROJECTION cluster. No predicate crosses a transport; the only public output is a `Sign` verdict consumed at the in-process seam by the spatial index, healing, topology, and constraint owners. `Expansion`, `ErrorBound`, and the interior `double` arrays never sit between wire and rail — they are the sanctioned interior-double owners of [R1].
...
|   [2]   | INTERIOR_NUMERICS  | `Expansion` sign-exact expansion arithmetic; `ErrorBound` static-permanence filter rows; `NumericsPolicy` the named [R1] interior-double exception owner |

## [2]-[ROBUST_PREDICATES]

- Owner: `Sign` `[SmartEnum<int>]` the closed ternary verdict (`Negative`/`Zero`/`Positive`, key `-1`/`0`/`+1`) every predicate returns; `Predicate` the static surface whose four members `Orient2D`/`Orient3D`/`InCircle`/`InSphere` each run the two-stage `ErrorBound`-gated fold — a fast `double` determinant filter, then an `Expansion` exact fallback only where the filtered interval straddles zero; coordinates arrive as `Rasm`/Vectors `Point3d` and the predicate reads `.X`/`.Y`/`.Z` once at the seam.
...
- Growth: a new predicate (`ParallelOrder`, `CompareDistance`, segment-segment intersection sign, Delaunay-flip sign — the [REFINEMENT_HORIZON] widening) is one `Predicate` member riding the same `ErrorBound` stage table and `Expansion` fold; a new error band is one `ErrorBound` row; zero new surface, no sibling fast/exact predicate types.
- Boundary: the four predicates are members on ONE `Predicate` static owner and a per-predicate class or a separate `FastOrient2D`/`ExactOrient2D` pair is the deleted form — the two stages are branches inside one member, gated by `ErrorBound.Stage`, never two surfaces; the verdict union is the closed `Sign` SmartEnum and a raw `int`/`double` sign leaking across a public signature is the named defect, callers match on `Sign.Negative`/`Sign.Zero`/`Sign.Positive`; coordinates are `Rasm`/Vectors `Point3d` read at the seam and a GeometryCore-local point struct is the deleted form ([PROHIBITIONS]); the fast filter is the IEEE-754 `double` determinant gated by `ErrorBound`, the exact fallback is the `Expansion` fold, and loosening a predicate to pass a near-degenerate case by widening the filter band instead of taking the exact branch is the named correctness defect ([PROHIBITIONS]) — a sign verdict is exact or it is a defect; the interior `double` arithmetic inside the filter and the `Expansion` fold is the [R1] sanctioned exception scoped by `NumericsPolicy`, and an interior `double` escaping a public predicate signature is the named seam violation.
...
One owner per axis; capability is a member, case, or row, never a sibling surface. `[STATE]` is `{PLANNED, FINALIZED, SPIKE}`: `FINALIZED` where the owner is a transcription-complete fence with no open gate; `SPIKE` where fence-complete but carrying a residual probe named in [RESEARCH]. Both predicate and numeric owners are `FINALIZED` — pure-managed, no host probe, the predicate algorithm and the expansion arithmetic are fully transcribed and depend on no live-host member spelling (FMA, `double.Epsilon`, and `Point3d.X/Y/Z` are stable BCL/Vectors vocabulary). The tier-2 numeric-validation harness that proves sign-exactness is NAMED in [RESEARCH] but does not hold an owner at SPIKE: the kernel is correct by construction (filter-then-exact is total), the harness is the law-matrix that asserts it.

The `[RAIL]` cell names the one return rail each owner exposes — pure verdicts because every predicate result is total and exact; no `GeometryFault` routes here (a sign is always defined, even for coincident input where the verdict is `Zero`).

| [INDEX] | [AXIS/CONCERN]            | [OWNER]          | [KIND]                                                                        | [RAIL]                                          | [CASES] |  [STATE]   |
| :-----: | :------------------------ | :--------------- | :--------------------------------------------------------------------------- | :--------------------------------------------- | :-----: | :--------: |
|   [1]   | Sign verdict              | `Sign`           | `[SmartEnum<int>]` ternary (`Negative`/`Zero`/`Positive`) + `Of`/`Flip`/`Times` | `Sign.Of → Sign` (pure, total)                  |    3    | FINALIZED  |
|   [2]   | Exact predicates          | `Predicate`      | static surface + `Orient2D`/`Orient3D`/`InCircle`/`InSphere` filter-then-exact members | `Predicate.Orient2D → Sign` (pure, total, exact) |    4    | FINALIZED  |
|  [2a]   | Expansion arithmetic      | `Expansion`      | `readonly struct` + `TwoSum`/`TwoProduct`/`Grow`/`Sum`/`Scale`/`Multiply`/`SignOf` fold | `Expansion.SignOf → Sign` (pure, exact)          |    7    | FINALIZED  |
|  [2b]   | Filter stage              | `ErrorBound`     | static-permanence `record` rows + `Of → Stage` projection                     | `ErrorBound.Of → Stage` (pure)                  |    4    | FINALIZED  |
|  [2c]   | Interior-double policy     | `NumericsPolicy` | static const owner — the [R1] exception + error-bound coefficients            | constants (read by `Predicate`/`Expansion`)     |    —    | FINALIZED  |

## [5]-[RESEARCH]

- [NUMERIC_DETERMINISM] — tier-2 numeric-validation HARNESS (NAMED, not a SPIKE-held owner): the sign-exactness law-matrix the `test run` G3 gate ([PROOF_GATES]) asserts. The harness is `PredicateLaws` (a CsCheck property suite under `testing-cs`): it generates near-degenerate input by perturbing collinear/cocircular/cospherical configurations at the `Epsilon` scale and asserts (1) every `Predicate` member returns the EXACT sign of the rational-arithmetic determinant computed independently in `System.Numerics.BigInteger` over scaled-integer coordinates — the exact oracle the kernel is proved against; (2) the antisymmetry law `Orient2D(a,b,c) == Orient2D(b,a,c).Flip` and its 3D/incircle/insphere analogues hold under coordinate permutation parity; (3) the filter path and the exact path agree on every input where the filter passes (the filter is conservative, never wrong); (4) the predicates are translation-invariant (a uniform `Point3d` offset never flips a verdict). The harness needs NO live-host probe — `BigInteger`, `Math.FusedMultiplyAdd`, and `Point3d` are stable; it is the law authority that flips no owner to SPIKE because the kernel is total by construction. The one residual the harness watches is the FMA-availability assumption: `Math.FusedMultiplyAdd` is a BCL contract that lowers to a hardware FMA on osx-arm64, but a future FMA-free RID would route `TwoProduct` through the named Dekker-split fallback (`NumericsPolicy.Splitter`, the `2^27+1` constant already seated) — confirming that fallback's bit-exactness against the FMA path on a non-FMA target is the only deferred numeric probe, and it gates a fallback row, never the FMA-path predicates which are FINALIZED.
