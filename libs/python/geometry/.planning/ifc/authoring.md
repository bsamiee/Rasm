...
class Authored(Struct, frozen=True):
    schema: str
    model: str
    ...
    depth: int = 0
    stamped: bool = False
    unrecorded: Option[BoundaryFault] = Nothing

    @property
    def shells(self) -> tuple[int, ...]:
        return _shells(self.reached)
    ...
    def span_facts(self) -> Mapping[str, object]:
        return {
            "schema": self.schema,
            "model": self.model,
            "verbs": len(self.facts),
            "guids": len(self.guids),
            "reach": sum(self.shells),
            "edited": self.edited,
            "depth": self.depth,
            "stamped": self.stamped,
        } | self.unrecorded.map(lambda fault: {"unrecorded": fault.tag}).default_value({})
...
def _evidence(authored: Authored) -> "Block[Fact]":
    target = Party(kind="model", key=authored.model)
    actor = Party(kind=Actor.SERVICE, key=OWNER)
    return authored.facts.map(
    ...
    async def apply_async(self, model: "ifcopenshell.file", script: tuple[AuthorOp, ...]) -> "RuntimeRail[Authored]":
        match self.apply(model, script):
            case Result(tag="ok", ok=authored):
                ...
                return Ok(landed.map(lambda _landed: authored).default_with(lambda fault: replace(authored, unrecorded=Some(fault))))
            case refused:
