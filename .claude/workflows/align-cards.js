...
// --- [DOCTRINE] --------------------------------------------------------------------------
const LAW = [
  'Rasm monorepo. CLAUDE.md card law governs. READ libs/.planning/campaign-method.md for the exact card schema and voice. This is an ' +
    'ALIGNMENT/REFINEMENT pass: re-align each card to the CURRENT state of the folder it governs (status, anchors, what is now realized in-corpus ' +
    'vs still-deferred, a denser/better approach WITHIN the ideas/tasks lane, correct/relocate slugs and anchors). Make cards logical and aligned, ' +
    'NOT coupled. Apply the 7-point density rubric. NO NEW cards (ripple counterparts already exist; do not add more) — the single exception is ' +
    'the completeness stage, which MAY add exactly one missing genuinely-deferred card per real gap. Genuinely DEFERRED only; no ' +
    'test/meta/decision/create-file cards. Fix-in-place; edit ONLY your assigned file. C# PLANNING-HOMING: cards at ' +
...
const inv = await agent('List every card file under ' + SWEEP + ' — files named exactly IDEAS.md or TASKLOG.md (at package roots and at .planning ' +
  'tiers). Return each as a repo-relative path. Use find; do not cd.', { label: 'discover', phase: 'Cards-Discover', schema: DISCOVERY_SCHEMA, model: 'sonnet', effort: 'low' })
...
const aligned = (await pool(CARD_FILES, CAP, (f) => agent([
  ...
  'TASK: ALIGN/REFINE the cards in ' + f + ' to the current state. Read the file, then read the folder it governs (its .planning pages + ' +
    'ARCHITECTURE + README) to check each card against reality: is an IDEA/TASK now realized in-corpus (mark COMPLETE/DROPPED with a one-line ' +
    'disposition)? is a status stale? is an anchor wrong or now better-placed? does a thesis need refining for a denser/better approach the corpus ' +
    'now enables? Correct/refine/disposition in place. Do NOT add new cards here. Read the wider corpus as needed for cross-folder logic ' +
    '(aligned-not-coupled). Return the fix-log.',
...
const verify = (await parallel([
  () => agent([LAW, '', 'TASK: BIDIRECTIONALITY verify + FIX IN PLACE across ALL discovered card files (' + filesList + '). For every Ripple, ' +
    'confirm the named counterpart card EXISTS on the other side and references back; repair any dangling/asymmetric/slug-collision Ripple (edit ' +
    'whichever side is wrong). Confirm no in-corpus-done work is still carded and no card duplicates a realized page. Return fix-log; set file to ' +
    'the first card file you edited (or the bidirectionality root).'].join('\n'), { label: 'verify:bidir', phase: 'Cards-Verify', schema: FIXLOG_SCHEMA, effort: 'xhigh', stallMs: 300000 }),
  () => agent([LAW, '', 'TASK: COMPLETENESS verify + FIX IN PLACE. For each target folder, read its plan/charter/README to learn its intended card ' +
    'band and scope, then confirm its IDEAS/TASKLOG pool matches that band; flag any folder short of band or any genuinely-deferred unit missing a ' +
    'card. Confirm pre-existing cards were correctly absorbed/dispositioned. Where a genuinely-deferred unit is missing a card, you MAY add ' +
    'exactly that card here (the one place a missing deferred card is filled) — never duplicate a realized page, never add test/meta/decision ' +
    'cards. Return fix-log; set file to a card file you edited.'].join('\n'), { label: 'verify:complete', phase: 'Cards-Verify', schema: FIXLOG_SCHEMA, effort: 'xhigh', stallMs: 300000 }),
])).filter(Boolean)
