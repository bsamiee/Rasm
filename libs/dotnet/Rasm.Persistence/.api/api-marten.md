...
|  [09]   | `IEvent` / `IEvent<T>`                | interface     | the stored-event message envelope                                |
...
- Both carry a Guid-id and a string-key ctor; `FetchStreamPlan` extends that pair with `version`, `timestamp`, and `fromVersion`, so the AS-OF bound the direct fold accepts rides the plan unchanged. Missing streams yield `null` from the state plan and an empty list from the event plan.
...
- Allocation fencing sharpens that evidence: the detector records the mark above which the sequence allocated nothing, so a session idle-in-transaction since before the gap's numbers were drawn reserved none and stops counting as a live owner — unsticking a mark parked behind the daemon's leadership advisory lock or a leaked ORM session. Marten discounts its own lock connections, and an unfenced probe stays byte-identical to plain transaction evidence.
- `SkipStaleGapsDespiteLiveTransactionsAfter` measures from a server-stamped first-observation moment persisted beside the mark, so the cap clock survives a daemon restart or a distribution handoff, while the settle threshold stays on the in-memory clock and gives every fresh gap its full window. Proven-dead spans clear WHOLE up to the ceiling recorded at first observation, so a field of dead gaps costs one cycle rather than one cycle each.
- Rolling range partitions split a document table by a duplicated date column across a moving window, and provisioning the leading edge rides ordinary schema migration with a purely additive diff.
- Retiring the trailing edge escapes migration, which never removes data, so the startup pass drops aged partitions under `ApplyAllDatabaseChangesOnStartup()` and runs BEFORE the configuration assertion — once the clock crosses a period boundary the database legitimately lacks the new partition the pass is about to create.
...
[STACKING]:
- `api-npgsql`(`libs/dotnet/.api/api-npgsql.md`) / `api-npgsql-ef`(`.api/api-npgsql-ef.md`): Marten rides the one shared `NpgsqlDataSource` bound through `StoreOptions.Connection(dataSource)`, so the EF-mapped identity row and the events share a connection pool and enlist one transaction.
- `api-thinktecture-json`(`libs/dotnet/.api/api-thinktecture-json.md`): event and document serialization is STJ through the registered `ThinktectureJsonConverterFactory`, projecting each `[ValueObject]`/`[SmartEnum]` owner to its key; `RegisterValueType<T>` then keys streams and document ids by that owner, and `NodaTime` instants ride the event `Timestamp`/`Created` columns.
- `api-ara3d-bimopenschema`(`.api/api-ara3d-bimopenschema.md`): the columnar egress is a co-transactional `FlatTableProjection` whose `Project<T>(StatementMap)` rows land the BIM analytics frame; structural EAV-generic maps are Persistence-owned, BIM-typed maps a Bim-implemented boundary projection.
...
- within-lib: the `Version/` engine composes this surface as its append path — `GraphDelta` bodies on per-model streams, an inline `SingleStreamProjection` holding authoritative containment read-your-writes consistent, and `UseSourceGeneratedDiscovery` compiling the projection/aggregation fold at build time rather than resolving it by runtime reflection; the `AdmitMarten` leg applies schema through `IMartenStorage` as the single-writer migration step.
