...
- `Rasm.Compute` (`Runtime/wire`, `Runtime/receipts`, `Runtime/progress`, `Analysis/lifecycle`): `SplatMapper`, `FaultDetailMapper`, `AllocationMapper`, `ProgressUpdateMapper`, and `Ec3Wire`+`EpdCodec` are the boundary-mapper estate; no transaction mirror remains.
- `Rasm.AppUi` (`Shell/controls#TS_PROJECTION`, `Diagnostics/evidence`, `Render/viewpoint`): `ControlMap` projects the closed `ControlIntent` union through its generated total `Switch`; `EvidenceMap` projects receipts; `ViewpointMap` transcribes the BCF camera and markup correspondences. `PlanMap`, `OpLogMap`, `ExportCardMap`, `GalleryIntake`/`DiffFolds`, `ProfileMap`/`EvidencePlan`, and `TableColumnMap` complete the branch mapper estate.
