...
while [ "$#" -gt 0 ]; do case "$1" in
    ...
    --head) HEAD=$2; shift 2 ;;
    *) echo "resolve-threads: unknown arg: $1" >&2; exit 2 ;;
esac; done
[ -n "$DISP" ] && [ -f "$DISP" ] && [ -n "$HEAD" ] || { echo "resolve-threads: --disposition <file> and --head required" >&2; exit 2; }
...
while IFS= read -r row; do
    ...
    [ -n "$tid" ] || continue
    if [ "$(jq -r '.viewer_can_resolve' <<<"$row")" = "false" ]; then NO_WRITE+=("$key"); continue; fi    # jq // would coerce false to the default

    resolve=0; reply=1
    case "$verdict" in
        fixed|upgraded)
            sha=$(jq -r '.commit_sha // ""' <<<"$row")
            if [ -n "$sha" ] && git merge-base --is-ancestor "$sha" "$HEAD" 2>/dev/null; then resolve=1
            else GATE_FAILS+=("$key"); reply=0; fi ;;
        pushed-back)
            [ "$(jq -r '.author_is_bot // false' <<<"$row")" = "true" ] && resolve=1 ;;
        deferred) ;;
        stale) resolve=1; reply=0 ;;
        *) reply=0 ;;
    esac
