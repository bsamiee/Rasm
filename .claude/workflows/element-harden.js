export const meta = {
  name: 'element-harden',
  whenToUse: 'After element-architect-build; granular adversarial hardening of the newly-built Rasm.Element rebuild content.',
  description: 'Per-page hostile rebuild -> critique -> redteam over the newly-built Rasm.Element / Bim / Materials / Persistence / Compute design pages, then folder-wide reconcile, then a whole-stack final-align. Stacks BOTH the shared libs/csharp/.api tier AND each folder .api, and preserves the section-4-RT architecture invariants. Per ELEMENT-REBUILD-PLAN.md.',
  phases: [
    { title: 'Discover' },
    { title: 'Harden' },
    { title: 'Reconcile' },
...
const rawArgs = Array.isArray(args) ? args : (typeof args === 'string' ? [args] : [])
const wanted = rawArgs.map((x) => String(x).trim()).filter((x) => x && x.toUpperCase() !== 'ALL')
const FOLDERS = wanted.length ? ELEMENT_FOLDERS.filter((r) => wanted.some((w) => r.endsWith(w) || r.endsWith('Rasm.' + w) || r === w)) : ELEMENT_FOLDERS
...
const FIXLOG_SCHEMA = { type: 'object', additionalProperties: false, required: ['file', 'verdict', 'summary'],
  properties: { file: { type: 'string' }, verdict: { type: 'string', enum: ['rebuilt', 'refined', 'clean'] }, collapsed: { type: 'string' }, extended: { type: 'string' },
    residual_high: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['files', 'claim'], properties: { files: { type: 'array', items: { type: 'string' } }, claim: { type: 'string' } } } }, summary: { type: 'string' } } }
...
const ADVERSARIAL = [
  'ADVERSARIAL STANCE — EVERY stage (author, critique, redteam) is HOSTILE: assume the fence is NAIVE/SHALLOW/ILLUSORY until it survives an aggressive attack; ' +
...
const authorPrompt = (page) => [DOCTRINE, '', 'TASK: HOSTILE GROUND-UP HARDEN of ' + page + ' to the ULTRA-ADVANCED bar AND domain-complete capability. DISBELIEVE ' +
  ...
  'package/domain/consumer source); modern C#14/net10, all-backticked high-signal prose, real full code fences. Fix-in-place. Report collapsed (before->after) + ' +
  'extended (additions + cited sources); residual_high = each {files,claim} for a CROSS-FILE item only.'].join('\n')
const critiquePrompt = (page) => [DOCTRINE, '', 'TASK: ULTRA-HARSH DOCTRINAL + CAPABILITY-COMPLETENESS AUDIT + FIX IN PLACE of ' + page + '. Assume a violation exists ' +
  'in every fence until proven otherwise; trust nothing the prose claims. Run the mechanical checklists and REPAIR every hit: COLLAPSE_SCAN (3+ parallel shapes/sibling ' +
  'names/repeated arms -> one owner); OWNER_CHOOSER (re-derive each shape`s owner from the 5 discriminants); KNOB_TEST (delete each param; collapse flags to policy ' +
  'values/input-shape); ASPECTS (definition-time via source-gen, composition-time via effect transformers); RAILS (narrowest carrier; closed Expected fault; total ' +
  'Switch; no exception control flow); STRATA/MEMBERS/MODERN (depend upward; no phantom member; C#14/net10; full docs/stacks/csharp + domain shard; BOTH .api tiers ' +
  'maximized); CAPABILITY-COMPLETENESS + ILLUSION (a collapsed owner can still be a naive slice — grow it; delete decorative/speculative padding); and the SECTION-4-RT ' +
  'INVARIANTS (a page contradicting one is a defect). Read ' + PLAN + ', docs/stacks/csharp, both .api tiers. EDIT to fix every hit. residual_high = {files,claim} cross-file only.'].join('\n')
const redteamPrompt = (page) => [DOCTRINE, '', 'TASK: ADVERSARIAL ARCHITECT RED-TEAM + FIX IN PLACE of ' + page + ' — the LAST + most aggressive pass; trust nothing the ' +
  'author/critique claimed. (A) COUNTERFACTUAL on the core choice — is the owner/algebra/dispatch categorically the strongest the doctrine admits, or does a denser ' +
  'owner / a deeper admitted-package primitive collapse the whole fence? Rebuild to it. (B) ANTICIPATORY_COLLAPSE — does the next case/dimension land as ONE case/row/' +
  'policy value with every consumer broken LOUDLY at compile time (total Switch, no silent _)? (C) LONG-TAIL — attack every input/output/edge/failure mode; both ingress ' +
  'AND egress parameterized. (D) STRATA/BOUNDARY — no downward dep, no host-type leak, no concern owned twice, geometry/mesh/IFC meet at one wire owner. (E) PHANTOMS/' +
  'SURFACE-SPRAWL — collapse hand-rolled code the .api tiers already own; delete unverifiable members. (F) CAPABILITY-COMPLETENESS + ILLUSION — attack the owner for ' +
  'domain-completeness independent of how collapsed it looks; grow it in place. (G) SECTION-4-RT FIDELITY — verify the page upholds every 4-RT invariant relevant to it. ' +
  'Repair every defect in place; if the strongest form is already present, prove it by finding nothing. residual_high = {files,claim} cross-file only.'].join('\n')
const STAGES = [
...
const processPage = async (page) => {
  ...
  for (const st of STAGES) {
    const r = await agent(st.build(page), { label: st.key + ':' + folderOf(page) + ':' + page.split('/').pop(), phase: 'Harden', schema: FIXLOG_SCHEMA, effort: st.effort, stallMs: 300000 })
    if (r === null) break
    logs[st.key] = r
  }
  return { page, logs, ok: Object.keys(logs).length === STAGES.length }
}
const finalPrompt = (task) => [DOCTRINE, '', 'TASK: WHOLE-STACK ' + task + ' across the rebuilt folders (' + FOLDERS.join(', ') + '). Hold ALL in view at once and align ' +
  'them as ONE coherent stack. Read each folder ARCHITECTURE.md ([01]-[DOMAIN_MAP] + [02]-[SEAMS]) + README + ' + PLAN + ' + every page a cross-folder seam spans. ' +
  'OBJECTIVES, fix each in place across the spanned folders: (1) SEAMS/WIRES — every cross-folder producer/consumer is recorded in BOTH endpoint ARCHITECTURE [02]-[SEAMS] ' +
  'with ONE shared shape (IElementProjection/IGraphConstraint <-> projectors; the GeometryRef/ContentAddress content-key <-> Persistence <-> kernel); (2) NO DUPLICATION — ' +
  'a concept owned twice collapses to its rightful stratum owner; (3) NO STRATA VIOLATION — depend strictly upward; no host-type leak; no GeometryGym below the seam; ' +
  '(4) SECTION-4-RT FIDELITY across the stack; (5) ISOLATION-not-coupling — each package usable standalone, peers never referencing each other. Fix in place; align at the ' +
  'seam, never trespass an owner interior. Return aligned + residual {files,claim}.'].join('\n')

// --- [COMPOSITION] -----------------------------------------------------------------------

phase('Discover')
const inv = await agent('Resolve the rebuilt Rasm.Element-campaign design pages to harden. For EACH folder root below, find every design page (repo-relative *.md) ' +
...
const isDesignPage = (p) => isAllowedFolderFile(p) && p.includes('/.planning/') && p.endsWith('.md') && !EXCLUDED_PAGE_NAMES.has(p.split('/').pop())
const pages = [...new Set((inv && inv.pages || []).filter(isDesignPage))]
log('Discover: ' + pages.length + ' pages across ' + FOLDERS.length + ' folders; pooling at CAP=' + CAP)
if (!pages.length) { log('No pages — run element-architect-build first, or pass a folder subset'); return { folders: FOLDERS, total: 0 } }

// --- [HARDEN]

phase('Harden')
const done = (await pool(pages, CAP, (p) => processPage(p))).filter(Boolean)

// --- [RECONCILE]

const norm = (x, page) => { const files = Array.isArray(x.files) ? x.files.filter(isDesignPage) : []; return { files: files.length ? files : [page], claim: x.claim } }
const allRes = []
...
log('Harden: ' + done.length + '/' + pages.length + ' pages; reconcile ' + uniq.length + ' residuals -> ' + clusters.length + ' clusters')
phase('Reconcile')
const reconciled = clusters.length ? (await pool(clusters, 1, async (cl) => {
  const fix = await agent([LAW, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', SEAM, '', PATLAW, '', 'TASK: RECONCILE these cross-FILE residuals (no severity; every one is ' +
    'must-address). Blast radius = the owning folder(s); read EVERY listed file. Fix real cross-file defects in place (unify the shared type/seam/contract, repair ' +
    'strata/boundary, extend a shared owner spanning files), preserving all capability; if a residual is factually wrong, leave it and say why. Residuals:\n' + JSON.stringify(cl, null, 1)].join('\n'),
    { label: 'reconcile-fix', phase: 'Reconcile', schema: FIX_SCHEMA, effort: 'max', stallMs: 300000 })
  if (!fix) return null
  const verify = await agent([LAW, '', SEAM, '', 'TASK: ADVERSARIAL VERIFY, one verdict per claim — read the named files from disk, classify fixed/invalid/open (default ' +
...
if (FOLDERS.length > 1) {
  phase('Final-Align')
  for (const st of [{ k: 'align', e: 'max' }, { k: 'critique', e: 'xhigh' }, { k: 'redteam', e: 'max' }]) {
    const r = await agent(finalPrompt(st.k), { label: 'final-' + st.k, phase: 'Final-Align', schema: ALIGN_SCHEMA, effort: st.e, stallMs: 600000 })
    if (r === null) break
    alignLogs[st.k] = r
  }
} else { log('Final-Align skipped: single-folder run') }

return {
  folders: FOLDERS, complete: done.filter((r) => r.ok).length, incomplete: done.filter((r) => !r.ok).length, total: pages.length,
  clusters: clusters.length, openClaims, finalAligned: Object.keys(alignLogs).length,
}
