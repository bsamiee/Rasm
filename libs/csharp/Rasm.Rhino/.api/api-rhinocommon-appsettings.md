...
Analysis owners share the round-trip shape and feed the `VisualAnalysisMode` overlays of `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-display.md`; each pairs a `{Owner}State` snapshot.
...
- A plugin reads its own tree through `Rhino.PlugIns.PlugIn.GetPluginSettings(Guid plugInId, bool load) -> PersistentSettings` (`libs/csharp/Rasm.Rhino/.api/api-rhinocommon-plugins.md`), sharing the `PersistentSettings` node type this family stores under.
...
- analysis owners are one family: each preset repeats the round-trip and differs only in its `*State` payload, so an analysis mode is a `VisualAnalysisMode` row of `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-display.md` reading its settings owner.
...
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md`: `PersistentSettings` under every owner is the custody spine, and its typed writer/reader surface composes there rather than being re-minted per settings family.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-display.md`: `*AnalysisSettings` owners feed the `VisualAnalysisMode` false-color overlays, and `ViewSettings` restore-scope flags compose the defined-view restore there.
...
- alias, shortcut, and never-repeat mutation enters through the registry's `Update(..., replaceAll)`; `PersistentSettings` typed I/O routes to `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md`.
