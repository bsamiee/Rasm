...
- [1]-[PROFILE_OWNER]: the `Profile` canonical unit shape, the `ProfileFamily` family-kind axis, the `ProfileId` key, the `ProfileFault` band, the dimensional value-objects, and the `ProfileCatalogue` registered-row table.
