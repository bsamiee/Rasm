...
- `Unofficial.laszip.netstandard`(`libs/csharp/.api/api-laszip.md`): the reality-capture pair's other leg — `ScanFormat` dispatches E57 here and `.las`/`.laz` there, both folding into ONE `ScanBatch` currency, so no format sees a per-codec point model
