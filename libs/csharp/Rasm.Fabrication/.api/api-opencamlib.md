...
- The shim mirrors the `ocl::Operation` lifecycle: `ocl_op_create(kind)` → `ocl_op_set_surface(op, stlHandle)` → `ocl_op_set_cutter(op, cutterHandle)` → `ocl_op_set_sampling`/`ocl_op_set_threads` → `ocl_op_run(op)` → `ocl_op_get_clpoints(op, double* out, int* n)` → `ocl_op_destroy(op)`; the `MillingCutter` ctor map is `ocl_cutter_cyl(d, l)`/`ocl_cutter_ball(d, l)`/`ocl_cutter_bull(d, r, l)`/`ocl_cutter_cone(d, angle, l)`/composite variants — one shim ctor per `CutterForm` row
- `STLSurf` is a handle family of its own (`ocl_stl_create(tris, n)`/`ocl_stl_destroy`) rather than operation-owned state, because `setSTL` binds by reference: one triangle upload serves every capsule a run creates, and a per-capsule re-marshal of the same mesh is the deleted form
- `Waterline::reset()` clears fibers and loops in place, so one waterline capsule sweeps an entire Z-level set through `setZ`/`run`/`getLoops`/`reset`; `LineCLFilter` (`setTolerance`/`addCLPoint`/`run`/`getCLPoints`) is the upstream CL-point simplifier the drop-cutter lanes pipe through before the managed boundary decodes them
