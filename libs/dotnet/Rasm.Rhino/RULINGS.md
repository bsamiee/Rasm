...
- `PluginFault.Unreachable` alone carries `Retriability.Transient` — the Zoo hops are the only network-backed calls.
- `IParticipant` and `ILicenseProgram` floor foreign participation and licensing as INSTANCE INTERFACES, the type itself forbidding the null.
- `MountRegistry` keeps its name over kernel `HookMounts`: name-addressed discovery and cross-plugin arbitration outrank composition-frozen mounting.
...
- Kernel `Rasm/Interaction/` owns the retired Eto band whole; host residue stays at `Display/draw` world marks and the `HostUi/shell` presenter axis.
- Host values both planes admit seat at the LOWEST importing sub-domain, holding the declared import edge — `PartIndex` at `Commands/selection`.
...
- PUBLIC entries and foreign-derived templates prove altitude on a zero-caller census, the template carrying its exemption; INTERNAL members owe one.
