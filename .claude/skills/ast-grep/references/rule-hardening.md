...
|  [16]   | Composite `not` or `inside` ahead of a cheap `has` in a rule object | Explicit `all:` list, the cheap clause first, 80 ms to 8 ms |
...
5. Run `ast-grep test -U --filter '^<survivor>$'` once, then without `-U`, and read the survivor's snapshot for every original invalid source
...
Devices prove themselves through `test_match_code_rule` on the sibling that needed them and on the near miss they must keep out, and a device that changes neither result leaves. Use `rule-building` for the checker probe that leaves an arm with the linter.
...
- Relational keys of a util body are unordered, and a capture a later key reads binds under `all:` in binding order
...
3. Split the residual into an `<id>-by-hand` sibling rule with `severity: error`, no `fix`, and a `note` stating the manual form
...
Fixture apps with zero findings gate the strictest tier. Rust's `_` wildcard matches literally, the one documented shape a util cannot fold, and ten documents differ by which parameter is `_`.
