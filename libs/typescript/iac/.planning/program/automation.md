...
const _ops = ["up", "preview", "refresh", "destroy"] as const
const _opTypes = [
...
- Law: the family is in-process by construction — a deploy fault never crosses a wire (the receipt does), so the zero-codec `Data.TaggedError` form is correct and promotion to `Schema.TaggedError` would rewrite only the declaration.
...
- Boundary: `ParseError` from receipt and output decodes re-spells into this family at the decode seam (`alien` with the parse message as detail); the rank fold over accumulated faults follows the rails law via `DeployFault.byRank`.
...
[AUTOMATION_RUN]:
- Owner: `Automation` — `stack` acquires the idempotent workspace, `run` drives one ledger op to a receipt, `receipt` folds an event batch (the member `policy/drift` shares), and `ephemeral` brackets a stack whose release destroys it. The `_host` Config surface is the one deploy-host read: `PULUMI_BACKEND_URL` selects the self-managed state store, `PULUMI_CONFIG_PASSPHRASE` rides `Config.redacted` and unwraps exactly once into `envVars`, `PULUMI_PROJECT` defaults, `PULUMI_HOME` is optional — no `Pulumi.yaml` exists because `projectSettings` carries the same facts programmatically.
- Law: the ledger is one exhaustive record — `_LEDGER` maps each op to its `Stack` method under a mapped contract, so a fifth op is a compile error at the record; `up`/`preview` receive the policy-pack and gate options, `refresh`/`destroy` the minimal projection, and every arm receives `signal` and `onEvent`.
...
- Law: the receipt folds from the event stream only — `summary` from the last `SummaryEvent`, steps from every `resourcePreEvent`, diagnostics from every `DiagnosticEvent` — never from per-op result shapes, so all four ledger arms share one fold; the assembled record decodes through `RunReceipt` and a decode failure re-spells as an `alien` fault.
- Law: `Automation.Options` carries call-local intent only — `parallel`, `expectNoChanges`, `policyPacks`/`policyPackConfigs` (the `policy/guard` attachment), `refresh` — and the option arrays keep the engine's mutable-array spelling because the record is a boundary mirror consumed once.
- Entry: `Automation.stack(spec, program)` then `Automation.run(stack, spec.name, "up", { policyPacks })`; `Automation.ephemeral(spec, program)` under a `Scope` for review stacks.
- Growth: a new host fact is one `_host` row; a new call-local option is one `Options` field threaded to the arms that accept it.
- Boundary: the `PulumiFn` the stack runs is `provider/dispatch.md`'s product; `Stack.addEnvironments` (the demoted ESC row) is `secret/inject.md`'s prepared member; `previewRefresh` composes `receipt` from `policy/drift.md`.
- Packages: `effect` (`Config`, `Effect`, `Option`, `Redacted`, `Schema`, `Array`, `pipe`); `@pulumi/pulumi/automation` (`LocalWorkspace`, `Stack`, `EngineEvent`, `PulumiFn`).

```typescript
import { LocalWorkspace, type EngineEvent, type PulumiFn, type Stack } from "@pulumi/pulumi/automation"
...
const Automation = {
  ...
  run: (stack: Stack, name: string, op: RunReceipt.Op, options?: Automation.Options): Effect.Effect<RunReceipt, DeployFault> =>
    Effect.flatMap(
  ...
  ephemeral: (spec: StackSpec, program: PulumiFn): Effect.Effect<Stack, DeployFault, Scope.Scope> =>
    Effect.acquireRelease(Automation.stack(spec, program), (stack) =>
      Effect.orDie(Automation.run(stack, spec.name, "destroy"))),
} as const
