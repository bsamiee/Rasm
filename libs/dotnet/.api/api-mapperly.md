...
- `LanguageExt`(`.api/api-languageext.md`): a generated mapping method returns the bare target and throws or defaults per `ThrowOnMappingNullMismatch`, so `Op.Catch` admits the call at the boundary and the carrier stays outside the generated body with the original exception preserved.
