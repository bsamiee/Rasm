...
- [02]-[VECTOR_INGEST]: `GeoVector` fold over the `GeoVectorSource` `[SmartEnum]` — the shapefile/CityJSON/GeoPackage/GeoJSON/OGR-universal decode arms producing `GeoFeature` rows, the OGR↔NTS WKB bridge, the MBR/attribute server-side push-down, and the symmetric write side.
...
public sealed partial class GeoVectorSource {
    ...
    public static readonly GeoVectorSource FlatGeobuf = new("flatgeobuf", managed: false, ogrDriver: "FlatGeobuf");
...
public static class GeoVector {
    ...
    // long-tail container formats (GeoPackage/FlatGeobuf/KML/GML/FileGDB). CityJSON carries NO write arm — it is
    ...
    // API (the inverse of Gdal.FileFromMemBuffer). The one driver path covers GeoPackage/FlatGeobuf/KML/GML/
