...
- [1]-[OBJECT_STORE]: owns the `ObjectStore` `Effect.Service` over `S3Service`, the `ObjectKey` content-addressed brand, the `PresignGrant` verb axis, the `AssetCodec` streaming fan-out, and the `LifecyclePolicy` rows over `listObjectsV2Stream`.
