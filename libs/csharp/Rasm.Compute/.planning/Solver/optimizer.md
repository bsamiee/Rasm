...
public sealed partial class OptimizerKind {
    ...
    // The smooth constrained-NLP row: the vendored cslsqp SlsqpSolver (span-based sequential quadratic
    // programming, the matched Oberbichler pair with HyperJet) for sizing/calibration problems that are
    // neither topology-adjoint nor MILP — bound + inequality constraints, exact HyperJet gradients.
...
public sealed record DesignProblem(
    ...
    // `Smooth` is the hyperdual-authored scalar objective the slsqp row differentiates EXACTLY through the
    // SensitivityLaw hyper-dual leg (a sizing/calibration objective authored once over DDScalar); the
    // black-box `evaluate` oracle stays the ONLY solve coupling — Smooth is a gradient SOURCE, never a
    // second evaluation path, and its primal must agree with the oracle's scalarized objective.
...
public static class Optimizer {
    ...
    // The vendored cslsqp SQP row (ISC, span-based, the matched Oberbichler pair with HyperJet;
    // source-vendored until its NuGet release lands — the recorded admission form, members verifying
    // against the vendored tree at first compose): a smooth constrained NLP over continuous design
    // variables. The gradient source is the ONE Sensitivity family — the problem's hyperdual `Smooth`
    // objective yields the EXACT gradient through SensitivityLaw.Gradient; absent OR on a gradient fault, a
    // central finite-difference over the black-box oracle keeps the row honest (never a silent zero gradient
    // the SQP would read as stationarity).
    ...
    // The scalar objective the vendored SQP minimizes: the oracle-evaluated objective sum. An oracle FAULT is
    // captured into the shared sink (first fault wins) and surfaced as the typed SolveSlsqp failure after the
    // solve — NaN poisons the step rather than synthesizing a double.MaxValue the SQP would read as a real objective.
