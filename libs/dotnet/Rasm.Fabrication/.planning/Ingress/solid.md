...
- Owner: `SolidMesh` is the sole detached millimeter carrier; `SolidTopology` owns incidence, shell, orientation, genus, and bounds measurement; `SolidWeldEvidence` retains the conditioning fold; `SolidRepairEvidence` retains the heal session; `Units` resolves the source unit and mints the contract `MeasureEvidence` for it, never a lane-local twin of that result's columns; `ImportedSolid` is the settled evidence carrier every projection reads.
