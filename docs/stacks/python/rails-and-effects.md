...
- Law: a worker kernel returning `Result[T, E]` crossed through a runtime offload comes back as one rail nested over the carrier, and the owner flattens it exactly once at the offload call — the outer boundary fault migrates onto one domain case, then `bind` threads the inner carrier — so the composed signature reads `Result[T, E | BoundaryFault]`; annotating the outer rail as if the inner carrier did not exist mis-types every downstream `map`, and transporting the inner fault by a custom exception across the worker seam is the rejected inversion.
