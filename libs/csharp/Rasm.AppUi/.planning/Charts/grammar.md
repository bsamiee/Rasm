...
- Receipt: each kind row is its own headless render-hash twin — `Baseline` re-keys the lane to the row's variant-density cell through the kernel content address, so the proof lane captures the same materialized chart through `CaptureRenderedFrame` and the `FrameHash` baseline derives from one row with no parallel fixture.
