...
- [02]-[RECEIPT]: `ArtifactReceipt` discriminates artifact evidence by kind over native-scalar `case()` payloads, owns its roster-derived `_CASES`/`_KEYS`/`_BAND` tables, projects through `contribute` onto the runtime spine, and hands off outward through `graduates`.
