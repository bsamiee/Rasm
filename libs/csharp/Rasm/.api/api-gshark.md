...
- vs LibTessDotNet (`api-libtess`): a GShark `PolyLine`/`Polygon` boundary that needs FILL triangulation (messy/holey/self-intersecting winding) feeds `LibTessDotNet` `ContourVertex[]` at the boundary — GShark evaluates the boundary curve, LibTess fills it; the two stack as evaluate→sample→tessellate, GShark never tessellating a filled region itself.
