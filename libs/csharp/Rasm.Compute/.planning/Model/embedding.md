...
public static class Embedding {
    ...
    public static Fin<EmbeddingVector> Encode(ReadOnlyMemory<float> unit, VectorEncoding encoding, string modelKey, VectorPolicy policy, Option<ProductCodebook> codebook = default) {
        ...
        // Codebook PRESENCE is the mint arm's own gate and is not restated here: two spellings of one law drift, and
        // the arm is the site that cannot proceed without it. Codebook LAYOUT proves here because the byte-length
        // gate below reads `Subspaces` and a wrong layout would size the buffer before the arm ever ran.
