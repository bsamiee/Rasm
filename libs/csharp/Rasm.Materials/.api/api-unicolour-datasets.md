...
|  [17]   | `Nord.PolarNight` / `SnowStorm` / `Frost` / `Aurora`                      | static field    | named Nord group `IEnumerable<Unicolour>`            |
|  [18]   | `Nord.All`                                                                | static property | all 16 Nord swatches as `IEnumerable<Unicolour>`     |
|  [19]   | `ArtistPaint.BoneBlack` .. (19 pigments)                                  | static field    | Kubelka-Munk artist `Pigment` reflectance values     |
...
- colourmap: `Colourmap` is abstract; each concrete map carries a static `Lookup` `IEnumerable<Unicolour>` and overrides `Map(double x)` by interpolating the lookup table in `Rgb` with `HueSpan.Shorter`.
