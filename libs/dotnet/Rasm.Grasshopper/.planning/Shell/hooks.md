...
- Entry: the dispatch is `HookSet<GrasshopperPoint, HookSignal, HookScope>.Of(gates, taps, span, cell)` — minted ONCE by `Platform/composition.md`'s mount roster with the composition's own `FaultCell`, never a page static. Raise site takes the dispatch as a REQUIRED parameter and calls `hooks.Fire(at, fact)` — the kernel's guarded transforming arity has no lawful site on this roster (the refusal-only law below); replay is `hooks.Replay(at, captured)` over a retaining point; scoped teardown is `hooks.Release(scope)`.
