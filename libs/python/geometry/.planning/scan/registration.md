...
class RegistrationResult(Struct, frozen=True):
    ...
    def _from_tensor(
        ...
        # one fold for every tensor-backend arm (MULTISCALE/COLORED_ICP): the open3d
        # `.fitness` is the matched-source fraction, so `fitness * |source|` is the inlier
        # estimate the receipt carries rather than zeroing it.
...
class ScanRegistration(Struct, frozen=True):
    ...
    def _multiway(self, session: tuple["o3d.t.geometry.PointCloud", ...]) -> RegistrationResult:
        ...
        for i, edge in enumerate(self._bootstrap(cloud, session[0], engine) for cloud in session[1:]):
        ...
        return RegistrationResult.of(
            ...
            engine=engine, poses=tuple(tuple(pose.flatten()) for pose in poses),
        )
