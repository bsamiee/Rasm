...
`Rasm.Domain` statistics is the kernel's evidence layer over scalar samples: every summary, extremum, quantile, and moment a host evaluation produces becomes one typed, admitted receipt minted here. One four-moment Welford recurrence, one order-statistic reader, and one bounded-selection cell serve every carrier the branch measures in, so a folder holding durations, distances, or dimensioned quantities reads the same statistic — and keeps its best k of a stream — rather than re-spelling it.
    ...
    private static Fin<Stat<TCarrier>> Admit(Moments held, StatContext context, Op key) =>
        held.Count > 0
            ? from minimum in TCarrier.From(held.Minimum)
              ...
              from receipt in key.AcceptValue(value: new Stat<TCarrier>(
...
- Receipt: `Distribution` and `QuantileSketch` conform to `IValidityEvidence` (`Distribution`: nested summary evidence, finite median, non-negative IQR and MAD, every percentile row in `[0,100]` with a finite value; `QuantileSketch`: unit-open fraction, non-negative count, finite heights — marker width is STRUCTURAL on `MarkerRow`, so no conjunct re-checks it).
- Packages: LanguageExt.Core (`Seq`/`Arr`/`Fin`/`Option`/`Validation`/`Traverse`/`ToFin`/`Order`), Foundation `[BoundaryAdapter]`, BCL inbox (`Math`, `PriorityQueue<TElement,TPriority>` + `Comparer<T>.Create` — the one heap the selection cell wraps).
...
flowchart LR
    ...
    accDescr: Carrier samples fold through one Welford recurrence into Stat, Distribution, SampleMoment, and extrema receipts, each re-entering the validation oracle before downstream consumers read them.
