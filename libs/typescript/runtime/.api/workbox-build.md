...
[LOCAL_ADMISSION]:
- VALUE-import `workbox-build` only in the Node build script; `browser/shell.ts` imports `RuntimeCaching`/`StrategyName` type-only. `vite-plugin-pwa` (tooling catalog) internalizes `injectManifest`/`generateSW` into the Vite pipeline — the browser catalog owns the type boundary, the tooling catalog owns the plugin wiring — under the folder-local `# browser` catalog group.
