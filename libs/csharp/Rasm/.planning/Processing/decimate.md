...
|  [1a]   | Decimation kind     | `SimplifyKind`   | `[SmartEnum<string>]` + `Reversible`/`PreservesTopology`/`Resamples` columns + `Weigh` `[UseDelegateFromConstructor]` row | `Weigh → Fin<Unit>` (rail-bound row)                |    4    |
