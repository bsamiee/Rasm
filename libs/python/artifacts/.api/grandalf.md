...
- Every vertex carries `view = VertexViewer(w, h)` and every edge `view = EdgeViewer()` before layout; `draw()` reads `view.w`/`view.h` for spacing and writes `view.xy` and `view.setpath(pts)`, and the owner projects those values into `LayoutMap` and `RouteMap` through `float(...)`.
