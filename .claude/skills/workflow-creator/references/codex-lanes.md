...
A review stage mid-chain (a critique between an implement and a red-team) can itself be a codex lane that WRITES — sol at the operator-default effort in a `workspace-write` sandbox, editing the unit's pages in place under the same review prompt a native agent takes. One composition contract keeps the chain coherent:
