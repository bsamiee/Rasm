...
class ContentEdit(Struct, frozen=True):
    ...
    def fold(self, page: "pikepdf.Page") -> bytes:
        ...
        stack: list[LayerMode] = []
        kept: list[tuple[list[object], object]] = []
        for operands, op in pikepdf.parse_content_stream(page):
            token = str(op)
            if token in ("BDC", "BMC"):
                mode = self.layer_mode(_oc_name(page, operands)) if operands and str(operands[0]) == "/OC" else LayerMode.KEEP
                stack.append(mode)
                if mode is not LayerMode.KEEP:
                    continue
            elif token == "EMC":
                mode = stack.pop() if stack else LayerMode.KEEP
                if mode is not LayerMode.KEEP:
                    continue
            if any(m is LayerMode.STRIP for m in stack) or token in self.drop_operators:
                continue
            kept.append(([rename.get(tok, tok) if isinstance(tok, pikepdf.Name) else tok for tok in operands], op))
...
def _oc_name(page: "pikepdf.Page", operands: "list[object]") -> str:
    ...
    tag = operands[1] if len(operands) > 1 else None
    if isinstance(tag, pikepdf.Dictionary):
        return str(tag.get(pikepdf.Name.Name, ""))
    properties = page.obj.get(pikepdf.Name.Resources, pikepdf.Dictionary()).get(pikepdf.Name.Properties, pikepdf.Dictionary())
    ocg = properties.get(pikepdf.Name(str(tag)), pikepdf.Dictionary()) if tag is not None else pikepdf.Dictionary()
...
def _rewrite(egress: DocumentEgress) -> FinishFact:
    ...
    removed = egress.edit.strip_layers | egress.edit.flatten_layers
    layers = _strip_ocg_catalog(pikepdf, pdf, egress.edit.strip_layers) if removed else 0
    sink = BytesIO()
    pdf.save(sink, linearize=True)
...
- [OCG_LAYER_EDIT]: the REWRITE OCG named-layer strip/flatten is composed from the verified pikepdf object model rather than a catalogued OCG-named surface: `parse_content_stream`/`unparse_content_stream` (catalogue page-assembly rows `[07]`/`[08]`), `Pdf.make_stream`/`page.obj[Name.Contents]` write-back (rows `[09]`/`[06]`), `Object`/`Dictionary`/`Array`/`Name` (object-model rows `[01]`/`[02]`/`[03]`/`[04]`), `Object.get` (the `Object`-row mapping accessor), and `Pdf.Root` (the `Pdf` document-root catalogue access) are all settled catalogue rows, so the `LayerMode` marked-content walk (the `/OC` `BDC`/`BMC`…`EMC` span whose first operand is `/OC` and whose second operand resolves through `_oc_name` to the OCG `/Name`) and the `_strip_ocg_catalog` `/Root/OCProperties` prune (the `/OCGs` membership array and the `/D` default-config `/ON`/`/OFF`/`/Order` reference arrays) are fence code over the catalogued primitives. The PDF OCG vocabulary itself — the `/OCProperties`/`/OCGs`/`/OCMD`/`/D`/`/ON`/`/OFF`/`/Order`/`/Properties` `/Name` tokens — is ISO-32000 PDF structure addressed through the generic `pikepdf.Name("/…")` constructor (the same leading-slash spelling the catalogued `Name` row requires for `rename_resources`), not a pikepdf-named helper, so no phantom OCG method is asserted. The `_oc_name` resolver reads the `/OC` tag two ways the spec admits — an inline property `Dictionary` carrying `/Name` directly, or a `/Name` operand indexing `page.obj/Resources/Properties` to an OCG dict (dereferencing one `/OCMD` `/OCGs` indirection to the member OCG) — both over the catalogued `Dictionary.get` accessor, never a hand-walked xref. A high-level OCG construction/authoring surface (minting `/OCProperties` and binding a named layer) is NOT an egress concern and stays out of this fence ([RESEARCH:PIKEPDF_OCG_AUTHOR]) — it is the `folder:../export/layered#LAYERED` owner's drawsvg/pikepdf concern, and `assay api` reflection that lands an explicit pikepdf optional-content surface (should one exist beyond the generic object model) deepens the catalogue before any authoring spelling promotes; the settled egress path is strip/flatten only, the inverse subtraction over an already-layered artifact. The `FinishFact.layers_removed` count is the `_strip_ocg_catalog` catalog-prune cardinality, riding the `ArtifactReceipt.Egress` `overlays` layer-composition slot rather than a seventh receipt field, since a layer strip and a watermark overlay are the two directions of the one layer-membership axis the case already measures.
