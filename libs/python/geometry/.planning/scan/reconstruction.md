...
- [1]-[RECONSTRUCTION]: method-discriminated surface reconstruction, the shared normal-estimation pre-step, the optional multi-object cluster split, and the GLB egress routing `mesh-utility` repair.
...
- Growth: a new reconstruction algorithm is one `ReconstructionMethod` row plus one `_construct` arm binding its static constructor; a new normal-estimation search param or a Poisson density trim threshold is one `ReconPolicy` field; the multi-object split is the one optional `cluster_dbscan` pre-pass already on the owner, never a parallel per-object class; the open3d reconstruction kernel (the CPU-bound Poisson/ball-pivoting solve over the `<'3.15'` companion band) hands across the runtime `execution/lanes#LANES` `LanePolicy` CPU-offload variant (`anyio.to_interpreter.run_sync` with `to_process.run_sync` fallback under one `CapacityLimiter`) as ONE hand-off call once that branch variant lands — `LanePolicy` is imported from runtime so the seam exists, the lane never imports the kernel, and the offloaded `_construct` callable is picklable on the `to_process` fallback; zero new surface, no parallel per-method class family.
...
- [OFFLOAD_LANE]: the open3d reconstruction kernel is one of the five heavy geometry CPU kernels the `GEOMETRY_KERNEL_OFFLOAD_LANE` seam hands to the runtime `execution/lanes#LANES` `LanePolicy` CPU-offload variant. SEQUENCE-AFTER the runtime branch owner lands the variant; the geometry-side realization is then a uniform one-call growth on the `_construct` kernel (the multi-second Poisson/ball-pivoting solve passed to the lane, picklable on the `to_process` fallback), never a new owner and never a second concurrency surface in geometry.
