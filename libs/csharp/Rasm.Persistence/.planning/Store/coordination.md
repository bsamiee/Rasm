...
// ONE truth projection, one cursor advance (#OUTBOX_CURSOR). A new concern is a case here, never a
...
public abstract partial record CoordinationOp {
    ...
    // fence WATERMARK — the highest generation its ledger has observed — which is the one read both the budget port and
    // the outbox relay need, so ONE case serves both and a per-consumer twin is the deleted form.
...
// deterministic refusal it must not. No instant — the receipt envelope stamps the HLC.
...
// (`Rasm.AppHost/RULINGS.md` `[02]-[SHAPE]`), so per-event delivery state has no envelope row to occupy, and the
...
public readonly record struct CaseSql(Seq<LockScope> Locks, bool RequiresToken, Option<string> Guarded, string Truth, Option<string> Wake, Seq<(string Name, object Value)> Binds) {
    ...
    public static CaseSql For(CoordinationOp op, Instant now) => op.Switch(
        ...
        // Pending rows come off the COMMITTED op-log the outbox spine already owns — no envelope table, so the read is
    ...
    // one order and a repeated unit — raising `21000` on the credit's `ON CONFLICT`, silently dropping an amount on
    // the debit's `UPDATE … FROM` — cannot be spelled. Debit guards ride the statement's own `WHERE`, which
...
|  [17]   | outbox park     | attempt and status on the per-sink cursor row | committed op-log stays one owner; no envelope table                  |
...
- Growth: a new egress sink is ONE `outbox_cursor` row minted on first drain (advance and park both upsert), zero coordination edits; zero new surface — an envelope-table outbox, a per-event delivery-state table, a per-sink advance verb, a trigger-based cursor writer, or a coordination-side read of the pump is the deleted form.
