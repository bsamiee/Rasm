...
- `libs/csharp/.api/api-languageext.md`: panel registration, page activation, dialog results, and resource loads cross through `Op.Catch`; nullable results bind `Optional(...).ToFin(Fail: op.InvalidResult())`, so a dialog result or loaded preview image crosses as `Fin<A>`, never a nullable host handle.
