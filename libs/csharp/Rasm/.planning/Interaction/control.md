...
- Auto: the two budgets on the tree case are one value type read on two axes — a node budget caps what the whole expansion admits and a depth budget caps how far one path descends, and a linear chain inside the node budget still overflows the runtime stack, which is the one failure no rail catches.
...
- Packages: Eto.Forms for the widget, cell, layout, and placement rosters (registered at `libs/csharp/.api/api-eto-forms.md` and its two boundary partitions); Thinktecture for the row vocabularies and their delegate columns; `Domain/validation` for `ICapability` and `CapabilitySet`.
...
- Packages: Eto.Forms for the grid and tree verb surface and for the themed control family (registered at `libs/csharp/.api/api-eto-forms.md`); `Interaction/dispatch` for the crossing.
