...
- Each constraint-layout span binds `__kiwi_version__`, distinct from the Python wrapper `__version__`.
...
- `rasm.runtime.faults` (shared result) — `async_boundary` (wrapping the layout `_compute`) catches the typed solver-fault family and maps each fault with its carried `constraint`/`edit_variable` onto a `RuntimeResult` case, so an over-constrained diagram returns a typed `Result` fault rather than crashing the render.
...
- `libs/python/.api/numpy.md` (shared result) — solved `var.value()` coordinates collect into a `numpy` `float64` `(N, 2)` point matrix consumed by `visualization/diagram/glyphset#GLYPHSET` marks and `data/graph/graph#GRAPH` route geometry, the same numeric substrate the force/radial/layered policies produce, never a parallel coordinate representation.
