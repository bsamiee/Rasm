...
[CLUSTERS]: one cluster — `[2]-[TABLE]`, the great-tables styled-table export owner over HTML/LaTeX/PDF, carrying the full publication surface (header/footnote/source-note structure, summary-row algebra, `loc`-targeted cell-style injection, column-control family, `fmt_image`/`fmt_nanoplot` cells) and the `tab_options` global-theme + `cols_merge_range` + `data_color(autocolor_text)` merge folds — every feature a data-driven spec row in the one `reduce`-over-spec pipeline, never a per-feature method chain.
...
class TablePlan(Struct, frozen=True):
    ...
    def _build(self) -> bytes:
        ...
        structured = reduce(
            lambda gt, item: STRUCTURE[item[0]](gt, item[1]),
            self.structure.items(),
            themed,
        )
        columned = reduce(
            lambda gt, item: COLUMNS[item[0]](gt, item[1]),
            self.columns.items(),
            structured,
        )
        spanned = reduce(
        ...
        summarized = reduce(
            lambda gt, item: gt.summary_rows(fns=item[1], columns=item[0]),
            self.summaries.items(),
            formatted,
        )
        styled = reduce(
            lambda gt, spec: STYLERS[spec[0]](gt, LOCATIONS[spec[1]](), spec[2]),
            self.stylers,
            summarized,
        )
        colored = reduce(
            lambda gt, item: gt.data_color(columns=item[0], palette=item[1], autocolor_text=True),
            self.fills.items(),
            styled,
        )
        match self.fmt:
