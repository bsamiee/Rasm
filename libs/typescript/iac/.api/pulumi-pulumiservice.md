...
- Reject: hand-rolled `api.pulumi.com` REST, the deprecated `github` block, automatic remediation where explicit `refresh` is required, hosted schedules doubled over local sweeps, dual writers per environment, org-wide tokens for single-stack consumers
