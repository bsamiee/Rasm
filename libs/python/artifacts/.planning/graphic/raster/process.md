...
The produced-raster behavior engine AND the raster plane's vocabulary owner. This page declares the shared vocabulary every raster sibling composes downward — the `Transform` StrEnum sub-axis, the `TransformInput`/`TransformArm` carriers, the canonical `RasterFact`, `ConvertFormat`, and the `Frame` pixel alias — and imports NOTHING raster-internal, so the plane's import direction is `io → measure → process` and policy binds above the vocabulary: the codec-POLICY rows binding `ConvertFormat` → engine/save-args and the `_REFERENCE_REQUIRED` admission rows are `graphic/raster/io#IO`'s; the measurement rows are `graphic/raster/measure#MEASURE`'s; both import this page. TWO produced-raster engines fold through one dispatch shape: the pillow working families — ImageChops two-image channel algebra, ImageMath band expressions, ImageEnhance perceptual factors, Color3DLUT grade application, the procedural gradient/noise/mandelbrot/spread sources — and the ten scikit-image transform families (`denoising`/`restoration`/`color`/`exposure`/`segmentation`/`graph`/`morphology`/`thresholding`/`geometric-transform`/`filters`); the scikit-image rows are census-gated — the marker drops when the cp315 wheel lands — while the pillow families carry the plane ungated.

Every acceptor is a pure NumPy/PIL transform: it owns no rail, raises into the `graphic/raster/io#IO` boundary that catches `(ValueError, OSError, KeyError)`, and never re-validates an already-admitted `TransformInput`. pillow and scikit-image are host-native worker packages, so acceptors execute only inside io's worker crossing riding the runtime `lanes.offload(kernel, retry=RetryClass.OCCT)` bound — this page mints no limiter, no retry policy, and no worker handle; the dtype-scale gates the providers refuse (`estimate_sigma` reads noise in the operand's own scale, `img_as_ubyte` rejects a float outside `[-1, 1]`) are re-imposed at admission so each member runs on the dtype its algorithm assumes.

## [01]-[INDEX]

- [01]-[PROCESS]: the raster plane's vocabulary owner plus the produced-raster engine over fifteen acceptors — the shared `Transform`/`ConvertFormat`/`Frame`/`RasterFact`/`TransformInput`/`TransformArm` vocabulary declared HERE with zero raster-internal imports (direction `io → measure → process`), the `TRANSFORMS` `Final[Map[Transform, TransformArm]]` folding the ninety-seven census-gated scikit-image rows into ten acceptors (`_denoise`/`_restore`/`_color`/`_expose`/`_segment`/`_graph`/`_morphology`/`_threshold`/`_geometric`/`_filter`) and the twenty-two ungated pillow rows into five (`_chops`/`_math`/`_enhance`/`_grade`/`_source`), each resolving its `TransformArm.member` through one `getattr(<provider>, member)` and merging the row's `kwargs` policy column under the caller `opts` (`row.kwargs | opts`) so every per-member default rides its row and no magic literal scatters into a body, the operand/axis variance pushed onto the `FilterChannel` and `MorphKind` row dispositions, every diagnostic stamped NATIVE onto the `frozendict[str, float | str]` `RasterFact.score` band (`_save_pillow` folding `ImageStat.Stat` mean/stddev facts onto every pillow fold), all dispatch-table-folded with zero parallel inline dispatch dict and zero mutable module table.

## [02]-[PROCESS]

- Owner: the raster plane's shared vocabulary and the produced-raster half of the `Transform` sub-axis the `graphic/raster/io#IO` `Raster` owner dispatches. The vocabulary is declared here as one owner block — `Transform` the 161-member StrEnum sub-axis (97 scikit-image produced + 22 pillow produced + 42 measured, the measured rows' acceptor bodies on `graphic/raster/measure#MEASURE`), `ConvertFormat` the encode-format vocabulary, `Frame` the `NDArray[np.uint8]` pixel alias, `RasterFact` the plane's ONE fact shape (`msgspec.Struct`: bytes + dimensions + the `frozendict[str, float | str]` score band — the band stays `frozendict`, msgspec-native and hashable; `Map` is the table form, never a payload slot), `TransformInput` the frozen-dataclass `(image, kind, reference, mask, opts)` carrier every acceptor reads with `opts` an immutable `frozendict[str, float]` matching the io `RasterOp.transform` payload, and `TransformArm` the frozen-dataclass row carrying the provider `member` an acceptor resolves through one `getattr`, the acceptor `arm`, the `kwargs` `frozendict` policy column merged under caller `opts`, and the `FilterChannel`/`MorphKind` dispositions. `TransformArm`/`TransformInput` are frozen dataclasses, not `msgspec.Struct`, because the row carries a behaviour callable and the carrier threads an in-process `numpy` `Frame` — neither is a wire shape. `TRANSFORMS` is the one `Final[Map[Transform, TransformArm]]` built `Map.of_seq`, keyed by the `Transform` value; io resolves a kind against this table falling to `graphic/raster/measure#MEASURE`'s `MEASURE_TRANSFORMS` at its composed lookup, so the full 161-member dispatch resolves with each member landing in exactly one page's rows. The table is the egress-grade collapse: a row binds the callable arm, its settled provider member, and its default kwargs; the op routes by one table lookup, never a per-operation sibling function and never a re-discriminating `match` inside an arm beyond the per-kind signature variance the provider forces.
- Cases: the five pillow acceptors fold the twenty-two ungated rows — `_chops` (CHOPS_MULTIPLY/SCREEN/OVERLAY/SOFT_LIGHT/HARD_LIGHT/DIFFERENCE/ADD/SUBTRACT/ADD_MODULO/DARKER/LIGHTER — member-derived `getattr(ImageChops, member)(base, overlay)` two-image channel algebra over the decoded `reference`, the `add`/`subtract` `scale`/`offset` riding their rows) · `_math` (MATH_LINEAR — `ImageMath.lambda_eval` gain/bias affine on the `"F"`-converted operand, re-encoded through `_save_array`'s robust normalizer) · `_enhance` (ENHANCE_COLOR/CONTRAST/BRIGHTNESS/SHARPNESS — member-derived `getattr(ImageEnhance, member)(rgb).enhance(factor)`, the factor a row default the caller overrides) · `_grade` (LUT_3D — `ImageFilter.Color3DLUT(size, table)` trilinear grade application, the flattened float32 table arriving on the `reference` channel so LUT AUTHORING stays `graphic/color/managed`'s and this arm only APPLIES) · `_source` (SOURCE_LINEAR_GRADIENT/RADIAL_GRADIENT 256-level L ramps resized to the requested frame, SOURCE_NOISE `effect_noise`, SOURCE_MANDELBROT `effect_mandelbrot` over the row extent, EFFECT_SPREAD random per-pixel displacement of the operand). The ten scikit-image acceptors fold the ninety-seven census-gated rows — `_denoise` (DENOISE_BILATERAL/NL_MEANS/TV/WAVELET, each routing the `estimate_sigma` noise level to its own member kwarg through `_DENOISE_NOISE` over `img_as_float`) · `_restore` (INPAINT biharmonic over a thresholded `as_gray` mask, ROLLING_BALL background subtraction, DECONVOLVE Richardson-Lucy/WIENER/UNSUPERVISED_WIENER over a uniform `_psf`, UNWRAP_PHASE) · `_color` (SEPARATE_STAINS/COMBINE_STAINS over the `hed_from_rgb`/`rgb_from_hed` matrices, the member-derived YCBCR/RGB2HSV/RGB2LAB/LAB2RGB conversions) · `_expose` (CLAHE, EQUALIZE, RESCALE_INTENSITY, MATCH_HISTOGRAMS, GAMMA, LOG, SIGMOID over the `is_low_contrast` gate) · `_segment` (SLIC/FELZENSZWALB/QUICKSHIFT superpixels, marker WATERSHED, CHAN_VESE/MORPHOLOGICAL_CHAN_VESE/MORPHOLOGICAL_GEODESIC level-sets, FIND_BOUNDARIES, CLEAR_BORDER — `regionprops_table` counts + `mark_boundaries` overlay) · `_graph` (RAG_CUT_THRESHOLD/RAG_CUT_NORMALIZED/RAG_MERGE over the `slic`+`rag_mean_color` graph, MIN_COST_PATH `route_through_array`) · `_morphology` (the eighteen members under the `MorphKind` disposition) · `_threshold` (the twelve global/local/hysteresis members through one binary-cut arm beside the multi-Otsu `np.digitize`) · `_geometric` (RESIZE/RESCALE/ROTATE/SWIRL/WARP_POLAR/WARP projective dewarp/RADON/IRADON) · `_filter` (the twenty-three gradient/ridge/band-pass/rank members under the `FilterChannel` disposition) — each one `TRANSFORMS` row, matched by io's composed-table lookup, never a sibling op per provider call.
- Auto: each acceptor re-dispatches only on the per-kind signature variance its provider forces while pushing every other split into a row policy — `_chops` splits only the `scale`/`offset`-bearing ADD/SUBTRACT pair from the uniform two-image fold; `_source` splits the operand-consuming EFFECT_SPREAD and the sized noise/mandelbrot sources from the member-derived gradient ramps; `_denoise` admits to float, estimates one scalar `sigma` via `estimate_sigma(average_sigmas=True)`, and routes it to the member's own noise kwarg through `_DENOISE_NOISE` (`sigma_color` bilateral, `h`+`sigma` nl-means, `sigma` wavelet, the TV row carrying `weight`); `_restore` branches INPAINT/DECONVOLVE/WIENER/UNSUPERVISED_WIENER (the `(deconvolved, posterior)` tuple destructured)/UNWRAP_PHASE/rolling-ball over one `_psf` builder; `_color` branches the stain matrices from the member-derived conversions; `_expose` branches MATCH_HISTOGRAMS (reference image) from the rest; `_segment` branches the five level-set/boundary/border forms from the channel-axis superpixel default, then overlays `mark_boundaries` and counts `regionprops_table`; `_graph` branches MIN_COST_PATH from the RAG trio (`merge_hierarchical` over the module-level `_weight_mean_color`/`_merge_mean_color` skimage-idiom callbacks vs the member-derived `(labels, rag, thresh)` cuts); `_morphology` Otsu-binarizes then reads the row's `MorphKind` under one total `match` — BINARY_FOOTPRINT/BINARY_PLAIN/GRAY_FOOTPRINT/RECONSTRUCT/PRUNE/ATTRIBUTE/ISOTROPIC/FLOOD — closed by `assert_never`; `_threshold` branches HYSTERESIS (the mask directly) then THRESHOLD_MULTIOTSU (`np.digitize`) vs the binary cut serving every scalar and local-array threshold uniformly; `_geometric` branches the sizing/warp/sinogram forms including the WARP homography from the 4 corners to the opts-displaced corners; `_filter` reads the row's `FilterChannel` under one total `match` — GRAY (luminance, no channel axis: gradients/ridges/canny/gabor), CHANNELED (`channel_axis` injected: unsharp/gaussian/butterworth/difference-of-gaussians), PLAIN (median), RANK (uint8 luminance + `disk(radius)` footprint) — plus the `feature.canny` module split and the `gabor` real/imag tuple. `channel_axis` is `_channels(image)` per member (grayscale-safe `-1`/`None`), the disposition rides the row, and no member can land in two channel or morphology buckets at once.
- Receipt: each acceptor folds into the canonical `RasterFact` declared HERE — the scikit arms through `_save_array`, the robust display-normalizer that passes a uint8/bool/`[0, 1]`-float array straight to `util.img_as_ubyte` and `exposure.rescale_intensity`s every out-of-range float or integer-label array to `[0, 1]` first, so an edge magnitude exceeding `1.0`, a negative Laplacian, or a multi-Otsu label field re-encodes to a viewable PNG without a per-acceptor min-max; the pillow arms through `_save_pillow`, which folds `ImageStat.Stat` luminance `mean`/`stddev` facts under the acceptor's own stamp so every pillow fold carries a native statistical band. io projects the fact to `core/receipt#RECEIPT` `ArtifactReceipt.Preview(key, width, height, scores)` at its rail boundary, threading `fact.score` straight onto the `Preview.scores` band with no coerce — `_denoise` the `sigma` float, `_restore` the `masked`/`background` fraction or `iterations` int, `_expose` the `contrast` gate str, `_segment`/`_graph` the `regions` count, `_morphology`/`_threshold` the `foreground` fraction or class count, `_geometric` the output `shape` str, `_filter` the mean `response` float, `_chops` the `blend` member str, `_math` the `gain`/`bias`, `_enhance` the `factor`, `_grade` the `lut_size`, `_source` the `source` member str — distinct from the measurement scores `graphic/raster/measure#MEASURE` stamps.
- Growth: a new produced-raster member is one `Transform` member HERE plus one `TRANSFORMS` row carrying its provider `member`, acceptor, and default `kwargs` — landing on the matching acceptor with zero new acceptor when the provider surface is already mined (a new global threshold is one `_threshold` row, a new ridge filter one `_filter` row defaulting `FilterChannel.GRAY`, a new blend one `_chops` row, a new perceptual factor one `_enhance` row, a new procedural source one `_source` row, a new band expression one `_math` row carrying its float policy); a new family is one acceptor plus its rows; the `MorphKind`/`FilterChannel` dispositions absorb a new operand/footprint sibling as one row plus its disposition, never a body edit; a new measured-score member is one `Transform` member HERE plus one row on `graphic/raster/measure#MEASURE`. The genuine deferrals stay: `random_walker`/`expand_labels`/`join_segmentations` (a seed-label or second-segmentation input the acceptor does not synthesize), `active_contour` (an explicit init snake), `calibrate_denoiser` (returns a denoiser, not a raster), `convert_colorspace` (a string space-pair the float-only `opts` cannot express; its concrete conversions landed member-derived), and the `Color3DLUT.generate(size, callback)` callable-row form (lands when a theme-owned transfer callable has a row spelling) — each one row once its input arrives.
- Packages: `pillow` (`ImageChops.multiply`/`screen`/`overlay`/`soft_light`/`hard_light`/`difference`/`add`/`subtract`/`add_modulo`/`darker`/`lighter`, `ImageMath.lambda_eval`, `ImageStat.Stat`, `ImageEnhance.Color`/`Contrast`/`Brightness`/`Sharpness`, `ImageFilter.Color3DLUT`, `Image.linear_gradient`/`radial_gradient`/`effect_noise`/`effect_mandelbrot`/`Image.Image.effect_spread` — the ungated engine); `scikit-image` (the ten families at the members the rows name — census-gated on the cp315 wheel, the pillow families carrying the plane meanwhile); `numpy` (operand algebra, `frombuffer`/`digitize`/`ptp`/`nan_to_num`); `msgspec` (`Struct` — the `RasterFact` wire shape); `expression` (`Map.of_seq` the dispatch tables); stdlib `dataclasses`/`enum`/`io`.
- Boundary: a per-provider-call sibling function, a parallel acceptor per `Transform` member, a mutable module dispatch dict, a `.get(key, magic)` default in a body, an erased `dict` opts bag, and a sibling re-declaration of any vocabulary owner declared here are the deleted forms — `RasterFact` in particular is the plane's ONE fact shape (`marks/encode`'s minimal twin resolves to this canonical). No IO/convert/thumbnail/montage working surface and no policy above the vocabulary: the codec-POLICY rows binding `ConvertFormat` → engine/save-args and the `_REFERENCE_REQUIRED` admission rows stay `graphic/raster/io#IO`'s — the reference-consuming rows here (the two-image `CHOPS_*`, `LUT_3D`, MATCH_HISTOGRAMS) are named on that gate. No measurement half: the `_measure`/`_register`/`_metrics` acceptors that PRODUCE scores rather than a transformed raster are `graphic/raster/measure#MEASURE`'s, which imports this page's substrate and contributes its `MEASURE_TRANSFORMS` rows to io's composed lookup. The families here all PRODUCE a new raster array; the measurement families stamp a scalar without a new pixel raster — the clean produced-raster-vs-measured-score axis the split cuts.
...
class Transform(StrEnum):  # the 161-member raster sub-axis vocabulary, declared HERE; measured-row acceptor bodies live on graphic/raster/measure
...
class RasterFact(Struct, frozen=True):
    # the plane's ONE fact shape; the score band stays frozendict — msgspec-native and hashable, the table form is Map, never a payload slot
    data: bytes
    width: int = 0
    height: int = 0
...
The engine is the egress-grade collapse over fifteen acceptor families and one vocabulary owner block: a `TransformArm` row names the provider `member` the acceptor resolves through one `getattr`, carries the acceptor `arm`, and threads the `kwargs` `frozendict` policy column plus the `FilterChannel`/`MorphKind` operand disposition — every per-member default (the deconvolution `num_iter`/`psf`/`balance`, the rolling-ball/footprint/isotropic `radius`, the watershed `markers`, the level-set `num_iter`, the prune `min_size`/`area_threshold`, the resize `rows`/`cols`, the `threshold_local` `block_size`, the Niblack/Sauvola `window_size`, the chops `scale`/`offset`, the enhance `factor`, the LUT `size`, the source `sigma`/extent/`quality`/`distance`) riding its row and merged under the caller `opts` (`row.kwargs | opts`) so no magic literal scatters into a body. `TRANSFORMS` is one `Final[Map[Transform, TransformArm]]` built `Map.of_seq` — the V16 table form — while every payload band (`RasterFact.score`, `TransformArm.kwargs`, `TransformInput.opts`) stays bare builtin `frozendict`, msgspec-native and hashable. The pillow acceptors make the plane's working engine ungated: `_chops` folds the eleven two-image channel-algebra members over the decoded reference, `_math` evaluates the gain/bias band affine through `lambda_eval` and re-encodes through `_save_array`'s robust normalizer, `_enhance` drives the four perceptual factors, `_grade` applies an upstream-authored trilinear table (authoring stays `graphic/color/managed`'s), `_source` generates the procedural gradient/noise/mandelbrot fields and the spread displacement, and `_save_pillow` stamps `ImageStat.Stat` luminance `mean`/`stddev` beside each acceptor's own fact. The scikit-image acceptors keep the census-gated ten-family engine: `_denoise` routes the scalar `estimate_sigma` to each member's own kwarg through `_DENOISE_NOISE`; `_restore` shares one `_psf` builder; `_filter` reads each row's `FilterChannel` so a member cannot land in two channel buckets; `_morphology` reads `MorphKind` so binary/grayscale/reconstruct/prune/attribute/isotropic/flood operands never cross; `_threshold` serves the full global family and the local binarizers through one binary-cut arm beside the multi-Otsu `np.digitize`; `_save_array` admits the native producer dtype and owns the viewable projection; `_channels` keeps every channel axis grayscale-safe. Each acceptor stamps a NATIVE `float`/`int` diagnostic (or a genuinely-categorical `str`) onto the `frozendict[str, float | str]` band the canonical `RasterFact` declares HERE, so the `core/receipt#RECEIPT` `ArtifactReceipt.Preview.scores` projection reads metric signals as numbers with no coerce. `graphic/raster/measure#MEASURE` imports the vocabulary and the `TransformInput`/`TransformArm`/`_save_array`/`_luminance`/`_channels` substrate from this page and contributes `MEASURE_TRANSFORMS`; `graphic/raster/io#IO` imports both pages downward and composes the 161-member lookup — the plane's import direction is `io → measure → process` with zero raster-internal imports here.
