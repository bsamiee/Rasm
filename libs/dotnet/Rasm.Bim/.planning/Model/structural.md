...
- Entry: `Attrs(BaseClassIfc? entity, UnitScheme scale, Option<EurocodePolicy> eurocode, IIfcProfileStore profiles)` lowers every supported structural entity through one `Fin<Map<PropertyName, PropertyValue>>` dispatch; `profiles` is the ONE content-addressed fragment lane the mandatory `IfcRelConnectsWithEccentricity.ConnectionConstraint` geometry preserves through. `AtStart`/`Station` return `Option` discriminants, so unresolved topology emits no assertion. `Author(DatabaseIfc, IfcObjectDefinition, Map<PropertyName, PropertyValue>)` re-stamps the verified restraint and single-force constructors and RETURNS the unconsumed row names as typed residue the egress folds.
