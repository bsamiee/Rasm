...
public sealed partial class RatingContour {
 ...
 // The PUBLIC railed admission (BOUNDARY_ADMISSION): a cross-assembly span is RAW material — a Compute mass-law
 ...
 // The Acoustic-owned StcWeighted/Rw reads enter FitAdmitted directly: their stored spectrum was admitted at
 // Acoustic.Of, and re-gating admitted evidence is the re-validation the doctrine deletes.
 public Fin<int> Fit(ReadOnlySpan<double> s, Op key) {
  ...
  return FinitenessGate(values.AsMemory(FirstIndex, Contour.Length), key, "contour-band-non-finite")
   .As().Map(_ => FitAdmitted(values)).ToFin();
 }

 // The one indexed window-finiteness admission BOTH gates share — the railed Fit window above and the Acoustic.Of
 // sound-reduction vector — accumulating every bad band so a corrupted spectrum yields one complete diagnostic.
 internal static K<Validation<Error>, Unit> FinitenessGate(ReadOnlyMemory<double> bands, Op key, string name) =>
  Accumulate(toSeq(bands.ToArray()).Map((band, index) =>
   double.IsFinite(band)
    ? Success<Error, Unit>(unit)
    : Fail<Error, Unit>(ElementFault.ValueRejected(key, $"<{name}:index={index}:value={band:R}>"))));
 ...
 // The shared contour slide over an ADMITTED window: the unfavourable-deviation sum (and the row-capped worst band)
 // is monotone in the shift, so the downward scan returns the FIRST (highest) feasible shift — RatingOffset + shift
 // IS the rating (a larger feasible shift is a better STC AND a better IIC). Sense flips only the deviation
 // orientation, so STC/Rw (ascending, offset 0 → rating = shift) and the impact row (descending, offset 110 →
 // IIC = 110 + shift) are ONE kernel; a span loop is the named EXPRESSION_SPINE kernel exemption. ReadOnlySpan
 // input: every caller (a stored ImmutableArray spectrum, Compute's freshly-built mass-law double[]) yields one
 // with zero copy; the kernel reads and never stores, so it earns no content-equality ImmutableArray.
...
public sealed partial record Acoustic {
 ...
 // constant finite and strictly positive — accumulating every indexed band fault plus the independent scalar faults.
 // The interior (the Fit windows, the ΔL_w consumer, the Double canon) never re-guards. The Option parameters default None so a
 ...
 public static Fin<Acoustic> Of(ReadOnlyMemory<double> absorption, ReadOnlyMemory<double> sri, Op key,
  ...
  (Gate(absorption.Length == AcousticBand.Count && sri.Length == AcousticBand.Count, key, $"<acoustic-band-arity:absorption={absorption.Length}:sri={sri.Length}:expected={AcousticBand.Count}>"),
   AbsorptionGate(absorption, key),
   SriGate(sri, key),
   PositiveGate(dynamicStiffness, "acoustic-dynamic-stiffness-non-positive", key),
   PositiveGate(flowResistivity, "acoustic-flow-resistivity-non-positive", key),
   PositiveGate(lossFactor, "acoustic-loss-factor-non-positive", key))
 ...
 private static double? NonPositive(Option<double> scalar) =>
 ...
 private static K<Validation<Error>, Unit> AbsorptionGate(ReadOnlyMemory<double> bands, Op key) =>
  Accumulate(toSeq(bands.ToArray()).Map((band, index) =>
   band is >= 0.0 and <= 1.0
    ? Success<Error, Unit>(unit)
    : Fail<Error, Unit>(ElementFault.ValueRejected(key, $"<acoustic-absorption-out-of-unit:index={index}:value={band:R}>"))));

 private static K<Validation<Error>, Unit> SriGate(ReadOnlyMemory<double> bands, Op key) =>
  RatingContour.FinitenessGate(bands, key, "acoustic-sri-non-finite");

 private static K<Validation<Error>, Unit> PositiveGate(Option<double> scalar, string name, Op key) =>
  NonPositive(scalar) is { } bad
   ? Fail<Error, Unit>(ElementFault.ValueRejected(key, $"<{name}:{bad:R}>"))
   : Success<Error, Unit>(unit);
