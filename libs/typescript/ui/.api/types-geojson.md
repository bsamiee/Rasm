...
- `maplibre-gl` (`.api/maplibre-gl.md`): a `FeatureCollection` is a `GeoJSONSource` payload — `GeoJSONSource.setData(collection)` pushes a derived overlay into the basemap's own vector pipeline, `bbox`/`center` driving the camera through the `viewer/geo/project` sync contract shared with the deck overlay `viewState`.
