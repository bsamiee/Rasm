...
- [BULK_EMISSION]: the live RETURNING old/new SQL image `MergeWithOutputAsync` emits on a live pg server behind the ReturningOldNew capability column — the binary-COPY route spelling is settled (`BulkCopyType.ProviderSpecific` for pg, `MultipleRows` for the sqlite downgrade, with the raw `NpgsqlConnection.BeginBinaryImport`/`NpgsqlBinaryImporter` path available).
