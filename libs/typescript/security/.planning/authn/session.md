...
import { AccessClaims, Alphabet, Crypto, Jwt, Probe } from "../crypt/sign.ts"
...
const _family = Fault.Class.family(["expired", "notFound", "reuse", "mismatch", "throttled", "store"] as const, {
...
- Law: the bearer seam and the rotation seam both carry their denominator — `BearerGuard` taps `Reject.mark("bearer")` before re-spelling the verify fault to `mismatch` and composes `Reject.measured("bearer")` so every admitted presentation lands its `bearer`-kinded admission and wall span, and `refresh` composes `Reject.measured("reuse")` under the kind its own breach arm marks; the folder's two highest-traffic ceremonies therefore publish a ratio rather than a bare refusal rate, and a credential-stuffing spike separates from a traffic spike on the same key set.
...
- Boundary: `authn/*` resolves a `CredentialRef` and calls `establish`; `Cookie` frames the pair; the ports carry state; `crypt/verify`'s `Curb` owns the refresh budget row; the runtime serve wave mounts `BearerGuard`.
- Packages: `crypt/sign` (`Jwt.mint`/`verify`, `Crypto.token`/`.uuid`/`.fingerprint`/`.matches`); `crypt/verify` (`Reject`, `Curb`); `access/audit` (`Witness`, `SecurityFact`); `@effect/platform` (`HttpApiMiddleware`, `HttpApiSecurity`); `effect` (`Config`, `DateTime`, `Duration`, `Effect`, `Option`, `Redacted`, `Schema`).
    ...
    Effect.map(Jwt, (jwt) => ({
      bearer: (token: Redacted.Redacted<string>) =>
        jwt.verify(token).pipe(
          ...
          Effect.mapError((fault) => new SessionFault({ reason: "mismatch", detail: fault.detail })),
        ),
    })),
  )
}
...
class Token extends Effect.Service<Token>()("security/authn/Token", {
  effect: Effect.gen(function* () {
    ...
    const refresh = (presented: Redacted.Redacted<string>): Effect.Effect<TokenPair, SessionFault> =>
      ...
      }).pipe(Reject.measured("reuse"), Effect.withSpan("security.token.refresh"))
...
- Boundary: `Token` owns the token values; `crypt/sign` mints and compares the CSRF token and owns the `Alphabet` row the CSRF mint composes; the edge applies `Cookies.toSetCookieHeaders` and lifts the request cookie/header; the runtime serve wave mounts `SessionGuard` as one more guard row.
- Packages: `@effect/platform` (`Cookies.makeCookie`, `Headers.redact`, `HttpServerRequest`); `crypt/sign` (`Crypto.token`/`.matches`, `Alphabet`); `crypt/verify` (`Reject`); `effect` (`DateTime`, `Duration`, `Effect`, `Option`, `Redacted`, `Schema`).
    ...
    Effect.map(Effect.all({ cookie: Cookie, jwt: Jwt }), ({ cookie, jwt }) => ({
      cookie: (token: Redacted.Redacted<string>) =>
        Effect.gen(function* () {
          ...
          // the cookie by name, the echoed header by field — and Cookie.verify already lands the csrf ledger row,
          // so this seam only re-spells the refusal onto the session family.
          ...
          yield* Array.contains(_SAFE, request.method)
            ...
            : cookie.verify(
              ...
              ).pipe(Effect.mapError((fault) => new SessionFault({ reason: "mismatch", detail: fault.reason })))
          return claims
