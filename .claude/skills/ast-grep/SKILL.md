...
Every rule proves through `test_match_code_rule` at omitted or `warning` severity: an `error` diagnostic exits nonzero and the tool discards the JSON, the final severity returns when the rule lands. Every non-match debugs through `dump_syntax_tree` against the target snippet. Search rules stay inline. Durable rules are project rule files discovered through `sgconfig.yml`.

[TEMPLATES]:
- [01]-[SGCONFIG](assets/templates/sgconfig.template.yml): Project config: config keys, snapshot directory, parser overrides, injection entries
...
- [03]-[RULE_REWRITE](assets/templates/rule-rewrite.yml): Rewrite fields: rewriters, `rewrite()` transform, `joinBy`, the `FixConfig` deletion doc
- [04]-[UTIL](assets/templates/util.yml): Global utility rule: parameterized `arguments:`, kind guard, call form, local shadowing rule
- [05]-[RULE_TEST](assets/templates/rule-test.yml): Test file: id-bound `valid:`/`invalid:` fixtures, snapshots written by `test -U`

[EXAMPLES]: proven rules with test files under `rule-tests/`. Pick by mechanism and adapt, never copy whole:
...
`ast-grep outline` maps source structure before any full read or edit: line-numbered top-level items (imports, functions, classes, structs, interfaces, modules, enums, flagged imported/exported) with their direct members (fields, methods, constructors, variants, flagged public). Output is syntax-local, with no reference resolution, type inference, re-export chains, or call graphs. Those questions route through [02]-[SEARCH] after outline names the files.

Every outline task: resolve target paths from the task, search hits, or `git diff --name-only`, run the owning row, escalate the located symbol with `--match <symbol> --view expanded`, then Read only the printed line range.
...
Structural search runs on the MCP tools with inline YAML rules. Patterns are valid code under the language's tree-sitter grammar, carrying whole-node metavariables: `$VAR` one named node, `$$VAR` one unnamed node, `$$$MULTI` lazy zero-or-more, `$_` non-capturing. Smart matching skips unnamed target nodes: the less a pattern specifies, the more it matches. Specify only what the query fixes.

Each search runs this sequence:
1. Query fits one AST node: `find_code` with the pattern, done
2. Anything structural: start from the most specific positive rule, refine relationally, filter captures
3. Unknown node kind: `dump_syntax_tree` on representative target code with `format=cst`, or `format=pattern` for a mis-parsing pattern
4. Prove with `test_match_code_rule` against a matching and a non-matching snippet, returning to step 3 on every non-match
...
- Prefix name searches capture whole nodes, `$NAME($$$)` with `constraints: NAME: { regex: '^<prefix>' }`. `use$HOOK` is no metavariable.
- `field: <role>` on `has`/`inside` pins same-kind children by parent relation, `field: key` splits an object key from its value
- Rules need a positive field, `pattern` or `kind`: `regex`, `field`, or a negation alone never matches
- Capture threading: the first rule naming `$VAR` defines its content and later rules only re-match it, order `all` accordingly
...
Rewrite extends a proven [02]-[SEARCH] rule with patching fields, each match replacing exactly one target node's text with the instantiated template. Templates are unparsed text: metavariables land anywhere, an undefined metavariable fails the parse under `scan` and lands empty under `run -r`, a declared-but-unmatched one lands empty in both, and `$VARName` lexes as `$VARN` + `ame` (appended text takes a `replace` transform). Multiline templates re-indent relative to the match's column. `run` covers the pattern-only path. The full field set (`fix`, `FixConfig`, `transform`, `rewriters`) runs under `scan --inline-rules`.

Each rewrite runs this sequence:
...
2. Attach the owning rewrite row: derived text lands in `transform`, multi-node edits in `rewriters`
...
5. Apply with `-U` (`-U` overrides `-i`). Nested matches rewrite outer-first, re-run until zero changes to prove depth and idempotence together.
...
|  [02]   | Mass removal   | `fix: ''`, a dangling separator is removed via `fix: { template: '', expandEnd: { regex: ',' } }`                   |
...
- Rewriter scope is sealed: metavariables, `transform`, `utils` stay inside their rewriter, cross-reference only by id inside `rewrite()`
- Optional trailing commas take another pattern variant under `any:`. `joinBy` absent splices in place, present discards unmatched text.
- Risk splits into sibling docs: the guarded rule carries `fix`, the residual carries `severity: error` and a `note` for manual action
- Captures landing in an operator, member-access, or return position take template parentheses: substitution ignores precedence
- `run` exits 1 on zero matches and 0 on a hit, the inverse of `scan`: shell chains over the subcommands read opposite ways

## [04]-[RULE_CRAFT]

Each device below is the standard form for its problem, developed per [02]-[SEARCH] and landing unchanged in durable files.

- Anchor every `regex` at both ends the rule fixes: matching is unanchored substring search, `Exemption` also matches `NoExemptionHere`
...
- `stopBy` forms: default neighbor for direct relations, `end` for the whole axis, a rule for a bounded walk (same-kind stopper pins the nearest owner)
...
- `rule:` demands a positive field, `constraints:` does not, negative and relational capture guards live there
...
- Suppression binds to the match's first or last line, never a middle one, report the tight offender node to put the waiver beside the defect
- `strictness: signature` matches shape while keeping capture identity, a duplicate-shape search, never a name ban
- Precision proves empirically before landing: scan a real codebase and count, a rule firing wider than meant is a semantic invariant in disguise
- `dump_syntax_tree` decides node wrapping and field names, a construct parsing as `ERROR` is unenforceable

## [05]-[DURABLE_RULES]

Durable rules are project structural rules enforced by a scanned gate: `sgconfig.yml` at the root, every YAML under `ruleDirs` a rule. Rules qualify on proofs: the violation is provable by node shape alone, no existing gate owns it (linter, analyzer, type checker, compiler, generator diagnostic), and it encodes a project rule rather than generic hygiene. Scope-, type-, or cross-file-dependent invariants never become rules. Each rule owns a whole violation family through `any:` and `utils:`, splitting only when message or fix diverges, with shared deep structure across split rules landing in a parameterized global util carrying an explicit `kind` guard. Every rule is `severity: error`, the scan exits nonzero and blocks.

```text
sgconfig.yml                     # Keys: ruleDirs, utilDirs, testConfigs, languageGlobs, customLanguages, languageInjections
rules/<language>/<area>.yml      # One topic file per rule family, split by ---, id equals filename, unique project-wide
utils/<util-id>.yml              # Global utils: explicit id + language, own constraints, a local utils: block shadows the same id
...
language: <language>
severity: error
files: ['<scope-glob>']          # ignores: carves out exempt boundaries, both relative to sgconfig.yml, never ./-prefixed
utils:
  <util-id>: { <family-shape> }
rule: { <[02] rule> }
constraints: { <VAR>: { regex: '<grammar>' } }
fix: <template>                  # Only when the repair is mechanical, [03] applies
...
Each rule lands through this sequence:
...
3. Author from the owning `assets/templates/` file, develop the rule per [02]-[SEARCH] and the fix per [03]-[REWRITE], proved both ways
...
| :-----: | :--------------------- | :----------------------------------------------------------------------------------------------------- |
|  [01]   | Banned construct       | construct kinds + `ignores:` on exempt boundary globs, or `not: { inside: <marker-comment rule> }`     |
...
- Unparseable rules or duplicate ids abort the whole scan, inline `---` bundles tolerate both, drafts never sit under `ruleDirs`
...
- Suppression is rule-scoped, `ast-grep-ignore: <rule-id>` on the next or same line, the `--error` built-ins keep bare and stale waivers fatal
...
- Green can prove nothing: omitted `severity` lands `hint`, `test` passes zero cases, a `files:` rule under `scan -r` scans nothing
