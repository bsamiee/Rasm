...
THE STEEL SEED FAMILY GROUNDED IN THE PUBLISHED SECTION DATABASE. `SteelSeed.Roster` is the full registered AISC American and EN 10365 European domain beside the generated SSMA cold-formed table and the fabricated rows, and `SteelSeed.Law` is the `SeedLaw<SteelRowSeed>` value `ComponentFamily.Steel` binds — the traverse, the coherence census, the profile route, the detail fold and the result-typed `Component.Of` lift all belong to `component#COMPONENT_SEED`. Each row carries one published `ICatalogue` identity, one policy-selected `MaterialGrade` steel row, one `SectionProfile.Catalogued` or admitted parametric profile, its section-map membership deriving from the profile's own topology. `SectionSolver.Solve` owns the twenty-column integral and the open-section supplement; `SteelDesign` owns the result-typed AISC/AISI/EN capacity projection, the composite augmentation, and the EN 1993-1-2 fire facts over that capacity. `SteelClass` carries the profile taxonomy, the Table B4.1 slenderness row, the buckling-curve imperfection factors, the grade-band thickness selector, and the `IfcProfileDef` subtype the seeded realization bag publishes; `SteelJurisdiction` owns the classification ladder each `DesignBasis` runs. Growth stays a registered catalogue member, a policy row, or an authored fabricated row rather than a per-shape type.

## [01]-[INDEX]

- [02]-[STEEL_FAMILY]: the `SteelTopology` open/closed/solid axis, the `SteelClass` eleven-row subtype axis with the TOTAL `OfShape` folds, the `StainlessForm`/`StainlessRow`/`StainlessBands` EN 10088 proof-cell registry and the product-form recovery, the `GradeProperties.Steel` physics members (the thickness-banded EN yield, the stainless routing, the one `DesignYieldMpa` entry), the `SteelJurisdiction` ladder table with the EN 1993-1-4 reduced-ε row, the `SectionDims` published-dims currency, the `SteelShape.Of` catalogue admission boundary, the `CompositeDetail` augmentation, the generated `ColdFormedRow`/`ColdFormedSections` SSMA table, `CompactnessClass` + `SteelDesign`'s one polymorphic `Capacity` entry over the profile arm, the grade row and the `capacity#SECTION_CAPACITY` `DesignBasis`, the `SteelFireFacts` EN 1993-1-2 surface, and the `SteelSeed.Roster`/`Law`/`Capacity` triple the policy row binds.
...
- Boundary: `SteelDesign.Capacity` binds yield from the admitted grade and product-form thickness band. Missing or unparsed published cells fail `GradeBandMissing`; documented provider refusals preserve their exact cause through `GradeDerivation`.
- Boundary: `SteelDesign` reads ONLY canonical `ComputedSection` columns (`Iw`, `GoverningRadiusMm`, `Avy`, `J/c`) — a re-minted dimension or a parallel `SteelBeamCheck` surface has no place here, and `DesignCapacity.TorsionalNmm`/`FlexuralMinorNmm` are the one source `CapacityLift.Steel` reads onto `SectionCapacity.SteelMember`. The DESIGN CODE is `DesignCapacity.Basis` DATA rather than a per-code capacity type, and the resistance BODY is the basis's own `ComponentAuthority`, so the retired `SteelBody` enum was one fact spelled twice. Steel carries `DetailLane.Realization` because `SteelClass.IfcSubtype` reaches the Bim profile lane only as a seeded `DetailSchema.ProfileSubtype` row. The AISI data path is CLOSED in-page — `FormOf` lowers the `ColdFormedC` and `Corrugated` arms straight onto `SectionDims`, so no reverse row lookup and no designation parse exists.
- Boundary: `SteelFireFacts` is the WHOLE EN 1993-1-2 surface this page publishes — the section factor, the Table 3.1 retention pair, and the §4.2.4 critical temperature in ONE result-typed record rather than three loose statics. Its consumer is LANDED: `capacity#SECTION_CAPACITY` `CapacityLift.Fire` mints over `FireState.Steel(DesignCapacity, SteelFireFacts)`; the family-side half is LANDED beside the seed: `SteelSeed.Capacity` dispatches on `CapacityPlacement.FireExposure` through the `SteelFire` §4.2.5.1 unprotected-member temperature step onto `CapacityLift.Fire`.
...
public sealed partial class SteelClass {
    ...
    public static Fin<SteelClass> OfShape(AmericanShape shape) => shape switch {
        ...
        _ => Fin.Fail<SteelClass>(new KernelFault.InvalidValue(nameof(shape), "a declared American steel shape")),
    };

    public static Fin<SteelClass> OfShape(EuropeanShape shape) => shape switch {
        ...
        _ => Fin.Fail<SteelClass>(new KernelFault.InvalidValue(nameof(shape), "a declared European steel shape")),
    };
}
...
public readonly record struct StainlessRow(
    ...
    public Fin<double> ProofMpa(StainlessForm form) =>
        Cell(form).ToFin(new ComponentFault.GradeBandMissing(ComponentFamily.Steel, typeof(StainlessForm)));
...
public partial record GradeProperties {
    public sealed partial record Steel {
        public Fin<double> DesignYieldMpa(SectionProfile origin, double bandThicknessMm, NationalAnnex annex) =>
            Stainless.Match(
...
public sealed record SteelShape(
    ...
    static Fin<IProfile> Outline(ICatalogue catalogue) =>
        catalogue is IProfile profile
            ? Fin.Succ(profile)
            : new ComponentFault.ProfileMismatch(ComponentFamily.Steel, catalogue.GetType());

    static Fin<SteelClass> ClassOf(ICatalogue catalogue) => catalogue switch {
    ...
    static Fin<SectionDims> DimsOf(ICatalogue catalogue) =>
        (catalogue switch {
            ...
            _ => Fin.Fail<(double, double, double, double, double, double)>(new ComponentFault.ProfileMismatch(ComponentFamily.Steel, catalogue.GetType())),
        })
        .Bind(raw =>
            from depth in FactoryBridge.Accept<PositiveMagnitude>(candidate: raw.Item1)
            from width in FactoryBridge.Accept<PositiveMagnitude>(candidate: raw.Item2)
            from web in FactoryBridge.Accept<PositiveMagnitude>(candidate: raw.Item3)
            from flange in FactoryBridge.Accept<PositiveMagnitude>(candidate: raw.Item4)
...
public sealed partial class SteelJurisdiction {
    ...
    public static Fin<SteelJurisdiction> Of(DesignBasis basis) =>
        TryGet(basis.Key, out SteelJurisdiction? row) && row is { } served
            ? Fin.Succ(served)
            : new ComponentFault.BasisUnsupported(basis, ComponentFamily.Steel);
...
public static class SteelDesign {
    ...
    public static Fin<DesignCapacity> Capacity(SectionProfile profile, MaterialGrade grade, ComputedSection s, CapacityPlacement placement) =>
    ...
    static Fin<(SteelClass Class, SectionDims Dims, Option<CompositeDetail> Composite)> FormOf(SectionProfile profile) => profile switch {
        ...
        _ => Fin.Fail<(SteelClass, SectionDims, Option<CompositeDetail>)>(new ComponentFault.ProfileMismatch(ComponentFamily.Steel, profile.GetType())),
    };
...
public static class FireRetention {
    ...
    public static Fin<(double Ky, double KE)> At(double temperatureC) =>
        double.IsFinite(temperatureC)
            ? Fin.Succ(Sample(Math.Clamp(temperatureC, Rows[0].TemperatureC, Rows[^1].TemperatureC)))
            : new KernelFault.OutOfRange(nameof(temperatureC), temperatureC, "finite");
...
public abstract partial record SteelRowSource {
    ...
    public sealed record Plated(Func< Fin<SectionProfile>> Build) : SteelRowSource;
}
...
public static class SteelSeed {
    ...
    static readonly Seq<SteelRowSeed> Augmented = Seq(
        ...
        new SteelRowSeed("steel.pg-1200x400-500", new SteelRowSource.Plated(static key => SectionProfile.AsymmetricIShape.Of(
            ...
            topFlangeThicknessMm: 25.0, bottomFlangeThicknessMm: 32.0, webThicknessMm: 12.0, filletMm: 0.0)), MaterialGrade.S355),
        new SteelRowSeed("steel.zed-200x75x25", new SteelRowSource.Plated(static key => SectionProfile.Zed.Of(
            ...
            thicknessMm: 2.5, topLipMm: 20.0, bottomLipMm: 18.0, innerFilletMm: 3.75)), MaterialGrade.A653Gr50),
        new SteelRowSeed("steel.pg-ss-800x300", new SteelRowSource.Plated(static key => SectionProfile.IShape.Of(
            depthMm: 800.0, widthMm: 300.0, webMm: 8.0, flangeMm: 20.0, filletMm: 0.0, flangeToeMm: 20.0)), MaterialGrade.Ss14301));
    ...
    public static readonly SeedLaw<SteelRowSeed> Law = SeedLaw<SteelRowSeed>.Of(
        ...
        source: Source,
        standard: Standard,
        detail: Some<Func<SteelRowSeed, SectionProfile, Fin<PropertyBag>>>(Detail),
        appearance: static _ => MaterialId.Create("metal.iron"));

    static Validation<Error, Unit> Coherence(SteelRowSeed row) =>
        AdmissionSlots.Accumulate(Seq(
            AdmissionSlots.Gate(
                row.Grade.Family == ComponentFamily.Steel,
                new ComponentFault.GradeFamilyMismatch(row.Grade, ComponentFamily.Steel)),
            AdmissionSlots.Gate(
                row.Grade.SteelArm.IsSome,
                new ComponentFault.GradeBodyMissing(row.Grade, ComponentFamily.Steel))));

    static Fin<SectionProfile> ProfileOf(SteelRowSeed seed) =>
        seed.Source.Switch(
            state: seed,
            rolled: static (x, r) => SteelShape.Of(r.Catalogue, x.Grade, Standard(x), x.Key)
            ...
            formed: static (x, f) => x.Grade.SteelArm
                .ToFin(new ComponentFault.GradeBodyMissing(x.Key, x.Grade, ComponentFamily.Steel))
    ...
    static Fin<PropertyBag> Detail(SteelRowSeed seed, SectionProfile profile) =>
    ...
    static Fin<MaterialGrade> GradeOfComponent(Component component) =>
        component.Profile is SectionProfile.Catalogued c
            ...
            : toSeq(MaterialGrade.Items)
                ...
                .ToFin(new ComponentFault.GradeUnavailable(ComponentFamily.Steel, component.SubstanceId));
}
...
public static class SteelFire {
    ...
    public static Fin<double> TemperatureC(ComputedSection s, PositiveMagnitude exposureMinutes) {
        ...
        return double.IsFinite(theta) && theta >= AmbientC
            ? Fin.Succ(theta)
            : Fin.Fail<double>(new KernelFault.OutOfRange(nameof(theta), theta, "finite and at least ambient temperature"));
    }
}
```
