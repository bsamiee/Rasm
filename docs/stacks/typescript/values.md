...
Everything around the value plane is shed by kind: vocabulary tables and the type-derivation algebra are `derivation.md`'s, Schema owners and decoded collection admission are `shapes.md`'s, the carrier with its in-flow `Option`/`Either` folds and the `Clock` behind `DateTime.now` are `rails-and-effects.md`'s, dispatch over values is `surfaces-and-dispatch.md`'s, keyed accumulators threaded through incremental dataflow are `streams.md`'s, the decode seam that admits these values is `boundaries.md`'s, and the kernel mark with the statement seam is `language.md`'s.
