...
│   ├── Retrieval.cs         # Retrieval.Run entry over StoreProfile and RetrievalOp; VectorRoute coupling, RetrievalFault family
│   ├── Topology.cs          # Kind-filtered QuikGraph view memoized per read snapshot; the boundary keeps the view vocabulary
...
│   ├── Backend.cs           # Parameterized backend row set over ColumnType columns; one branch-owned provisioning emitter
│   ├── Serving.cs           # BackendScope bounds, BackendPlan dialect lowering, BackendReach dispatch, BackendLanding
│   ├── Datasets.cs          # Custodian-declared dataset rows, distinct from every wire-handed dataset a producer owns
...
│   └── Federation.cs        # FederationPlan admission over three ingress forms; the FederationLowering preserved-semantics path
...
└── Store/                   # Durable-home and coordination substrate
    ...
    ├── Placement.cs         # ObjectChecksum transport-versus-identity split and the sealed write-stance columns
...
`Rasm.Compute`'s analysis path hands two typed row streams over one shape boundary. `SeriesLane.Ingest` lands the temporal leg — this package's own `Query/datasets#SERIES_ROSTER` `SeriesPoint` — under the `SeriesKind.Assessment` row, while the neutral `AssessmentRow` result catalog stays Compute-owned vocabulary crossing as producer-handed row data onto the `Query/datasets#ASSESSMENT_ROWS` dataset this custodian declares — a landing, never a twin.
...
---
flowchart LR
    accTitle: Persistence domain and kernel content boundaries
...
---
flowchart LR
    accTitle: Persistence platform and cross-runtime boundaries
...
- Boundary and content-keyed wire carry every sibling-domain and host alignment; no AEC peer or host-SDK type is referenced.
...
- Typed projection records and the boundary `ElementGraph` are the only egress.
- Provider failure converts once per carrier; each sub-domain returns its canonical result.
- Generated code owns converters, formatters, and generation artifacts.
...
- `ProjectionContext` is the one time and causal boundary, seating the kernel `CorrelationId`/`TenantContext` pair; the HLC is the one causal clock.
