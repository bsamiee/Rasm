...
- Boundary: the current family is app material arriving as a `Schema.Union` value; the relations carrying this journal envelope are `journal/append.md`'s journal rows and `journal/retain.md`'s export rows. The native op-log has a distinct thirteen-position envelope and does not alias this upcast coordinate.
