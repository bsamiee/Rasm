...
- quantity seam: the `Measurements.*` values are bare `double` + a `Units`/`NativeUnits` string — at the `ToolAssembly` boundary they coerce to `UnitsNet` typed `Length`/`Angle` (`libs/csharp/.api/api-unitsnet.md`) so the holder geometry and the swept-volume clearance are dimensioned, never a raw `double` mm/inch ambiguity; the `NativeUnits` string drives the `UnitParser` coercion
