...
- `msgspec`/`pydantic`(`.api/msgspec.md`, `.api/pydantic.md`): the symmetric `from_dict`/`to_dict` on every type is the codec boundary, and `EPW.to_file_string`/`from_file_string` serializes the weather file for the artifact domain.
