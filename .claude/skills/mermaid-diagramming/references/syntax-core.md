...
sequenceDiagram
    ...
    accDescr: An API querying a user database and warming a created cache participant that is later destroyed.
    participant API as Public API
    actor DB as User Database
    API->>+DB: query rows
    DB-->>-API: row batch
...
classDiagram
    ...
    note for Developer "namespace member"
```
...
erDiagram
    ...
    accDescr: A person holding customer accounts with keyed and commented attributes.
    direction LR
    p["Person"] {
