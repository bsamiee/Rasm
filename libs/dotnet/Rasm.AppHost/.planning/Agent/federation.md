...
- Owner: the `FederationProjection` fold EXTENSION — `Resources`/`Prompts`/`Templates` projection arms added to the tool fold; `FederationSubscription` the `McpClient.SubscribeToResourceAsync` per-uri handler adapter draining a peer resource-update into the one bounded `Wire/livewire#LANE_SUBSTRATE` `SubscriptionLane` as one `ExternalValue` (the reused at-edge carrier, never a federation-local value type).
