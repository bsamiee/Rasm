...
// Returns the current OTel span; works under both Tracer.layer/NodeSdk.layer and OtlpTracer.layer
// (where the span is an OTel-Span-conformant wrapper).
...
// Set the effect's parent span from an OTel SpanContext (data-last + data-first overloads).
...
export const layerEmpty: Layer.Layer<Resource.Resource>  // since 2.0.0
```
...
// module-local: interface Scope { readonly name: string } (not exported)
```
...
export declare class OtlpSerialization extends Context.TagClass<
  OtlpSerialization, "@effect/opentelemetry/OtlpSerialization", {
    readonly traces: (data: unknown) => HttpBody.HttpBody    // encodes trace data for transmission
    readonly metrics: (data: unknown) => HttpBody.HttpBody   // encodes metrics data for transmission
    readonly logs: (data: unknown) => HttpBody.HttpBody      // encodes logs data for transmission
  }
>() {}

export const layerJson: Layer.Layer<OtlpSerialization>      // application/json
export const layerProtobuf: Layer.Layer<OtlpSerialization>  // application/x-protobuf
```
