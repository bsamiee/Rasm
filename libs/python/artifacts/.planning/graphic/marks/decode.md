...
- [02]-[DECODE]: zxing-cpp `read_barcodes` substrate — `DecodeScope` owns detector policy, format scope, capability query, and scan; `DecodedSymbol` owns per-symbol admission and recovery; `TAXONOMY` derives explicit format scopes with typed `unscannable` refusal; `graphic/marks/encode#MARK` composes the shared `RasterFact`/`MarkFault` layer.
...
- Output: `DecodeScope.scan` returns shared `RasterFact` data; `DecodeFact` keys numeric `COUNT`/`VALID`/`BUILD` and the `SYMBOLS` blob `DecodedSymbol.recovered` reconstructs. An open source with no symbol is absence (`count=0`), never a fault.
...
flowchart LR
    ...
    Outcome["encode Mark.of: MarkOp.Decode / MarkOp.Verify (lane dispatch by source case)"] --> Scan["DecodeScope.scan(source) -> Result[RasterFact, MarkFault]"]
