...
| :---------------- | :------------------------- | :------------------------------------------------ |
| catalog resolve   | `pnpm install`             | catalogMode strict resolves @rasm/services        |
| typecheck         | tsgo `--noEmit`            | zero diagnostics; isolatedDeclarations emits .d.ts |
| subpath split     | `node -e import('@rasm/services')` | @pulumi/* not loaded on the runtime path  |
