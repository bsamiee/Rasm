...
|  [02]   | `RESULTS`  | result enum    | `equinox.Enumeration`; items `successful`, `max_steps_reached`, `singular`, `breakdown`, `stagnation`, `conlim`, `nonfinite_input` |
...
- `Solution.result` is an `equinox.Enumeration` item: branch with `solution.result == lx.RESULTS.successful`, and read the human message via `lx.RESULTS[solution.result]`. Under `throw=True` (default) a non-successful result raises at the JAX boundary; pass `throw=False` to inspect `result`/`stats` inside a traced study rail.
