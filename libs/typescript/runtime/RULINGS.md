...
- Private universal HTTP interfaces and internal protocol subpaths stay outside branch architecture.
...
- Ambient OTel globals serve foreign libraries and `Carrier` spells every branch boundary — `CompositePropagator` continues a trace across a foreign hop.
...
- Cumulative bus tallies reach counters as DELTAS against one held sample — a counter set to a running total re-counts every prior interval.
...
- Boundaries closing verdicts over domain classes declare the store channel — a decoding discharge fails `SqlError` beside `ParseError`, never the cause.
...
- Inbound W3C hops continue exactly once, at `Edge.guard` — a mounted protocol re-extracting the same headers mints a second trace of one hop.
...
- `present` spells credential placement on every egress-lane and engine row — where the credential lives, what a rotation costs; no second column.
- Rotating credentials ride the transport's own refresh hook — dial-time authenticator, provider callback, per-call stamp — never a timer beside it.
...
- `Hooks.Dispatch` IS the app's `Tap.Bus` seat — a runtime bus table, publish permit, or replay journal forks the core veto order and breach count.
- Transports hand `Carrier.extract`'s extraction WHOLE to `Propagation.ingress` — destructuring at the boundary drops a census no second reader keeps.
