...
- `api-macos-native.md`: in-process `RhinoCore.InvokeInHostContext` marshal composes the Rasm host main-thread rail rather than a bespoke dispatcher; a `ViewCaptureWriter` subclass is NOT a composition route — vector egress composes `ViewCapture.CaptureToSvg` in `api-rhinocommon-display.md`, and a page seating the writer as a delivery row publishes capability nothing can drive
