...
How the six modules compose into the one design rail (`execution/ai#AI_ACTIVITY`, `agent/runtime#AGENT_RUNTIME`), stacking onto the universal tier (`effect.md`, `effect-platform-node.md`, `effect-opentelemetry.md`) and the sibling `@effect/ai` core (`effect-ai.md`):

- Provider resolution: `AnthropicLanguageModel.model`/`.modelWithTokenizer`/`.layer`/`.layerWithTokenizer` produce `AiModel.Model<"anthropic", LanguageModel.LanguageModel (| Tokenizer.Tokenizer), AnthropicClient>`, satisfying the core `LanguageModel.LanguageModel` tag (`effect-ai.md` `[MODEL_RESOLUTION_LAW]`). The design's `providerModel("anthropic", model)` row selects it; provider swap against `effect-ai-amazon-bedrock.md`/`-openai`/`-google`/`-openrouter` is one composition-root layer change.
- Layer stack: `AnthropicLanguageModel.layer({ model })` over `AnthropicClient.layer`/`layerConfig` over `NodeHttpClient.layer` (`effect-platform-node.md`). `layerConfig` reads `apiKey`/`apiUrl`/`anthropicVersion` from `Config` cells (adds `ConfigError`); the `apiKey` (sent `x-api-key`) resolves through the `security/secret#SECRET_STORE` `SecretStore` and stays `Redacted`.
- Generation + rails: `LanguageModel.generateText`/`generateObject`/`streamText` (`effect-ai.md` free functions, `AiError.AiError` channel) or the persisted `Chat.generateText`/`Chat.layerPersisted` (`agent/runtime`); wrap each in `execution/ai#Resilience.breaker` + retry `Schedule`. `createMessageStream` yields `Stream<MessageStreamEvent>` folded per `type` over the eight SSE event members.
- Tokenizer: `modelWithTokenizer`/`layerWithTokenizer` fold `AnthropicTokenizer.layer` into the provides set for token budgeting and `Tokenizer.truncate`; plain `model`/`layer` omit it.
- Tools: the eleven `AnthropicTool.*` constructors (Bash ×2, CodeExecution ×2, ComputerUse ×2, TextEditor ×4, WebSearch) plus the closed `ProviderDefinedTools` union compose into an `@effect/ai` `Toolkit.make(...)` alongside user `Tool.make` tools, passed via `GenerateTextOptions.toolkit`; `prepareTools(options, config)` is the request-builder helper (reused by the Bedrock provider) and `getProviderDefinedToolName` maps a wire tool name to its toolkit id.
- Provider metadata + telemetry: the `@effect/ai/Prompt`/`Response` augmentations attach the `anthropic` key (`cacheControl`, `AnthropicReasoningInfo` thinking/redacted-thinking, citations, `BetaUsage`/`stopSequence` on `FinishPartMetadata`); `@effect/ai` `Telemetry.addGenAIAnnotations` with `WellKnownSystem "anthropic"` writes GenAI spans exported through `@effect/opentelemetry` (`effect-opentelemetry.md`).
- Per-effect transport: `AnthropicConfig.withClientTransform` installs a request-scoped `HttpClient` transform (retry/metrics/logging span), distinct from the layer-time `transformClient`.
- Cross-provider: `@effect/ai-amazon-bedrock` reuses this package's `Generated.BetaCacheControlEphemeral` for its Anthropic-on-Bedrock provider tools and mirrors these tool families (`effect-ai-amazon-bedrock.md`).
