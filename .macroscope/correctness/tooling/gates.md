...
`uv run assay static` is the one three-language static entrypoint; each language resolves to one canonical checker invocation, and the central manifest is the truth source for which toolchain exists.
