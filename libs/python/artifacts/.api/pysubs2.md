...
| [INDEX] | [SURFACE]                       | [CALL_SHAPE]                                  | [CAPABILITY]                                      |
| :-----: | :------------------------------ | :-------------------------------------------- | :------------------------------------------------ |
|  [01]   | `pysubs2.make_time`             | `make_time(…) -> int`                         | author ms from unit mixes or frame counts         |
|  [02]   | `time.ms_to_str`                | `ms_to_str(…) -> str`                         | render ms as `[-]H:MM:SS[.mmm]` for display       |
...
|  [06]   | `ms_to_frames`                  | `ms_to_frames(…) -> int`                      | ms-to-frame; non-positive fps raises `ValueError` |
|  [07]   | `formats.substation.parse_tags` | `parse_tags(…) -> list[tuple[str, SSAStyle]]` | tagged text to override-aware styled fragments    |
