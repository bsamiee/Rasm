...
Residual host-bridge and native-asset work is the close-out surface. The SPIKE owners on the charter DENSITY_BAR carry the live probes named in their page RESEARCH clusters: `ModelSessions`, `RunOps` against model-lane#RESEARCH (`[EP_OPTIONS]`, `[CANCELLATION]`); `WireChannels`, `CallSpine` against remote-lane#RESEARCH (`[TRANSPORTS]`, `[PIPE_SECURITY]`, `[REQUEST_COMPRESSION]`, `[COMPOSED_CREDENTIAL]`); `LinearProvider`, `DenseOps`, `SparseOps` against numeric-lane#RESEARCH (`[NATIVE_BLAS_EXECUTION]` — native MKL x64-only behind the documented RID gate, native-openblas osx-arm64 asset the live probe, managed lane proven). The GPU-EP execution residual (`AppendExecutionProvider_CUDA(0)`/`_DML(0)` member shape FINALIZED, execution gated on NVIDIA-Linux/Windows RID + driver) and the GENERATIVE_RUN live multi-token model-asset gate (streaming contract runtime-proved) are the genuine hardware/asset residuals. Each SPIKE owner is fully shaped at transcription; its exit is the residual native, bridge, live-server, or model-asset probe passing on the gate named beside the START_GATES row that unblocks it.
