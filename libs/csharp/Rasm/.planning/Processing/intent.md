...
public abstract partial record VectorIntent {
    ...
    private Fin<TOut> Dispatch<TOut>(Context context, Op op) => Switch(
        ...
        // cloud.md's CloudKernel.Winding owns the ring-normal derivation + angle-sum fold; this arm only routes.
        windingCase: static (state, intent) =>
            CloudKernel.Winding<TOut>(cloud: intent.Value, query: intent.Query, key: state.Key),
