...
| [INDEX] | [SYMBOL]                                                                            | [ROLE]                              |
| :-----: | :---------------------------------------------------------------------------------- | :---------------------------------- |
|  [01]   | `Room2DEnergyProperties` (`room2d.properties.energy`)                               | per-room energy assignment          |
|  [02]   | `ModelEnergyProperties` (`model.properties.energy`)                                 | model-level resource registry       |
|  [03]   | `BuildingEnergyProperties`, `StoryEnergyProperties`, `ContextShadeEnergyProperties` | building/story/context energy hosts |

[ENERGY_MEMBERS]: assignment attributes then operations.
- [01]-[ROOM2DENERGYPROPERTIES]: `program_type`, `construction_set`, `hvac`, `shw`; `is_conditioned`/`has_window_opening` flags, `process_loads`/`total_process_load`, `window_vent_control`/`window_vent_opening`; `add_default_ideal_air()`, `add_process_load(...)`, `remove_process_loads()`.
- [02]-[MODELENERGYPROPERTIES]: aggregates `materials`, `constructions`, `face_constructions`, `shade_constructions`, `construction_sets`, `global_construction_set`, `schedule_type_limits`, `schedules`, `program_types`, `hvacs`, `shws`; `to_honeybee(new_host)` carries energy props, `apply_properties_from_dict(data)` loads them.
- [03]-[BuildingEnergyProperties / StoryEnergyProperties / ContextShadeEnergyProperties]: construction-set inheritance and context-shade constructions.

[VALIDATION_SCOPE]: `ModelEnergyProperties.check_all(...)` plus the duplicate-identifier checks it folds (`check_duplicate_construction_set_identifiers`, `check_duplicate_program_type_identifiers`, `check_duplicate_hvac_identifiers`, `check_duplicate_shw_identifiers`) — energy-resource integrity before translation. Model-level resource collections (`materials`, `constructions`, `face_constructions`, `shade_constructions`, `construction_sets`, `global_construction_set`, `schedule_type_limits`, `schedules`, `program_types`, `hvacs`, `shws`) are the aggregated registries those checks scan.
...
| [INDEX] | [SYMBOL]                                                                 | [ROLE]                                 |
| :-----: | :----------------------------------------------------------------------- | :------------------------------------- |
|  [01]   | `FourthGenThermalLoop` (`des.loop`)                                      | conventional hot/chilled district loop |
|  [02]   | `GHEThermalLoop` (`des.loop`)                                            | GHE-based ambient (5th-gen) loop       |
|  [03]   | `GroundHeatExchanger` (`des.ghe`)                                        | borefield footprint geometry           |
|  [04]   | borefield parameters (`des.loop`)                                        | hydraulic + design value-objects       |
|  [05]   | `ThermalConnector` (`des.connector`), `ThermalJunction` (`des.junction`) | piping segments + junctions            |

[DES_MEMBERS]: constructor then operations per system object.
- [01]-[FOURTHGENTHERMALLOOP]: `FourthGenThermalLoop(identifier, chilled_water_setpoint=6, hot_water_setpoint=54)` carries its two water setpoints; `to_des_param_dict(buildings, tolerance=0.01)` emits the system-parameter inputs.
- [02]-[GHETHERMALLOOP]: `GHEThermalLoop(identifier, ground_heat_exchangers, connectors, clockwise_flow=False, soil_parameters=None, fluid_parameters=None, pipe_parameters=None, borehole_parameters=None, design_parameters=None)` — `ground_heat_exchangers: list[GroundHeatExchanger]`, `connectors: list[ThermalConnector]`, borefield-sizing parameters; same `to_des_param_dict(buildings, tolerance=0.01)` surface.
- [03]-[GROUNDHEATEXCHANGER]: `GroundHeatExchanger(identifier, geometry)` — `geometry`, `boundary_2d`, `hole_polygon2d`; `from_geojson_dict(...)`/`to_geojson_dict(...)`.
- [04]-[BOREFIELD_PARAMETERS]: `SoilParameter`/`FluidParameter`/`PipeParameter`/`BoreholeParameter`/`GHEDesignParameter` carry ground conductivity, fluid, pipe geometry, borehole layout, and sizing targets a `GHEThermalLoop` composes.
...
- `RoadNetwork` — models the road graph (`from_dict`, `to_geojson_dict`) paired with the electrical layout.

[REOPT_SCOPE]: techno-economic analysis (`dragonfly_energy.reopt`)
- `REoptParameter` — carries the REopt run config; `to_assumptions_dict(base_file, urdb_label)` builds the NREL REopt POST body. Composed of `FinancialParameter`, `WindParameter`, `PVParameter`, `StorageParameter`, `GeneratorParameter`, `GroundMountPV` — the per-technology sizing/cost inputs.

## [04]-[TRANSLATION_AND_RUN]

[TRANSLATE]: the headline writer (`dragonfly_energy.writer`)
- `model_to_urbanopt(model, location, point=Point2D(0,0), shade_distance=None, use_multiplier=True, add_plenum=False, solve_ceiling_adjacencies=False, des_loop=None, electrical_network=None, road_network=None, ground_pv=None, folder=None, tolerance=0.01)` — emit an URBANopt feature GeoJSON + per-building OSW/HBJSON in one pass, optionally layering the DES loop / electrical / road / ground-PV networks. DES system-parameter JSON itself is produced by the `run.run_des_sys_param` stage below, not a separate writer.
...
- `anyio` — drives the long, blocking, subprocess/Docker-bound `run_*` pipeline: each stage runs through `anyio.run_process` / `anyio.to_thread.run_sync` inside a task group with a cancel scope + deadline so a wedged URBANopt/Modelica run reclaims cleanly.
...
- `dragonfly-core` — hosts the model this extends; build/zone the massing there, attach `.properties.energy` here, then `model_to_urbanopt` (DES via the `run_des_*` pipeline).
- `honeybee-energy` — supplies the object library `.properties.energy` wraps: `room2d.properties.energy.hvac`/`.construction_set`/`.program_type`/`.shw` are honeybee-energy objects; the room-level energy model and EnergyPlus IDF generation live there.
- `honeybee-openstudio` — bridges OSW->OpenStudio/EnergyPlus, driven by `run_urbanopt`; a separately admitted runtime companion (dragonfly-energy ships no `openstudio` extra), not a pip extra of this package.
