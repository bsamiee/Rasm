...
One page owns the browser platform bindings layer — `BrowserPlatform`, the one `Layer.mergeAll` of the `@effect/platform` fetch HTTP client, the `@effect/platform-browser` local-storage key-value store, and the worker-manager binding that every host owner composes for its IO primitives. The layer is config-free stock bindings sitting directly above `RuntimeConfig` in the graph; it owns no policy and authors no decode.
...
- Packages: `@effect/platform` for the `HttpClient`/`KeyValueStore`/`Worker.WorkerManager` service tags, the `FetchHttpClient.layer` fetch client, and the `Worker.layerManager` over-`PlatformWorker` lift; `@effect/platform-browser` for the `BrowserKeyValueStore` and `BrowserWorker.layerPlatform` browser bindings.
