...
| :-----: | :------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------- |
|  [01]   | `CloudEvent`              | sealed envelope — `Id`, `Source` (`Uri?`), `Type`, `Subject`, `Time` (`DateTimeOffset?`), `DataContentType`, `DataSchema`, `Data` (`object?`) |
|  [02]   | `CloudEventAttribute`     | one typed attribute row — `Name`, `Type`, `IsRequired`, `IsExtension`, `Parse(string)`, `Format(object)`, `Validate(object)`                  |
|  [03]   | `CloudEventAttributeType` | closed value-space vocabulary — statics `String`, `Boolean`, `Integer`, `Binary`, `Uri`, `UriReference`, `Timestamp`                          |
|  [04]   | `CloudEventsSpecVersion`  | spec vocabulary — `V1_0`, `Default`, per-attribute rows (`TypeAttribute`, `SourceAttribute`, …), `FromVersionId(string?)`                     |
...
| [INDEX] | [SYMBOL]                | [CAPABILITY]                                                                                                        |
| :-----: | :---------------------- | :------------------------------------------------------------------------------------------------------------------ |
|  [01]   | `JsonEventFormatter`    | `: CloudEventFormatter` — structured-mode JSON codec; optional `(JsonSerializerOptions?, JsonDocumentOptions)` ctor |
...
| :-----: | :---------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------- |
|  [01]   | `new CloudEvent(CloudEventsSpecVersion specVersion, IEnumerable<CloudEventAttribute>? extensionAttributes)` | envelope minted with its declared extension-attribute roster                           |
|  [02]   | `this[CloudEventAttribute attribute]` / `this[string attributeName]`                                        | typed attribute read/write — the extension stamp path (`object?` get/set)              |
|  [03]   | `CloudEvent.SetAttributeFromString(string name, string value)`                                              | wire-string admission through the attribute's own `Parse`                              |
...
|  [07]   | `CloudEventAttribute.CreateExtension(string name, CloudEventAttributeType type)`                            | extension-attribute mint — the distributed-tracing `traceparent`/`tracestate` row path |
...
| [INDEX] | [MEMBER]                                                                                                                                                           | [CAPABILITY]                                                 |
| :-----: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------- |
|  [01]   | `EncodeStructuredModeMessage(CloudEvent cloudEvent, out ContentType contentType)` → `ReadOnlyMemory<byte>`                                                         | one-body JSON envelope emit (`application/cloudevents+json`) |
|  [02]   | `DecodeStructuredModeMessage(ReadOnlyMemory<byte> body, ContentType? contentType, IEnumerable<CloudEventAttribute>? extensionAttributes)` → `CloudEvent`           | one-body decode admitting declared extensions                |
|  [03]   | `EncodeBatchModeMessage(IEnumerable<CloudEvent> cloudEvents, out ContentType contentType)` → `ReadOnlyMemory<byte>`                                                | batch emit                                                   |
|  [04]   | `DecodeBatchModeMessage(ReadOnlyMemory<byte> body, ContentType? contentType, IEnumerable<CloudEventAttribute>? extensionAttributes)` → `IReadOnlyList<CloudEvent>` | batch decode                                                 |
|  [05]   | `ConvertToJsonElement(CloudEvent cloudEvent)` / `ConvertFromJsonElement(JsonElement, IEnumerable<CloudEventAttribute>?)`                                           | in-memory `JsonElement` bridge                               |
|  [06]   | `EncodeBinaryModeEventData(CloudEvent cloudEvent)` / `DecodeBinaryModeEventData(ReadOnlyMemory<byte>, CloudEvent)`                                                 | binary-mode data-only codec for transport bindings           |
