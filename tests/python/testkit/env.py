...
def _provision_store(spec: ObjectStore) -> Provisioned[s3fs.S3FileSystem]:
    """Serve one threaded moto endpoint as an ``s3fs`` view rooted at ``spec.bucket``, carrying presigned ``url`` and e-tags beyond the fsspec algebra."""
...
def provision(spec: EnvSpec) -> Provisioned[Awaitable[asyncssh.SSHClientConnection]] | Provisioned[AbstractFileSystem] | Provisioned[s3fs.S3FileSystem]:
