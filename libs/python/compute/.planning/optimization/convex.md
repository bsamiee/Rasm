...
- [1]-[CONVEX]: linear / quadratic / second-order-cone / semidefinite disciplined-convex programs over `cvxpy` DCP atoms and the Clarabel conic backend, folding one content-keyed `ConvexReceipt` duality-gap optimality certificate on the one `ConvexProgram` owner.
