...
- [02]-[SPARSE_SOLVE]: CSR ingestion axis; `FactoredOp` capability owner; criterion-stack iterative with the solver ladder and the budget criterion; overdetermined sparse-QR least-squares route; two-container exchange (`.mtx` · HDF5 archive).
...
- Owner: `SparseFormat` `[SmartEnum<string>]` ingestion-axis rows carrying the CSR-conversion `ingest` delegate as row data; `FactorKind` `[SmartEnum<string>]` direct-factor rows carrying the capability columns (rank-1 edit, transpose-solve, inertia, reentrancy, symmetry), the fill-formula and transpose-solve-recovery delegate, AND the permutation-keyed `create` factory as row data; `IterativeMethod` `[SmartEnum<string>]` closed solver-factory axis whose `Ladder` column also composes the rungs into the `CompositeSolver` fall-through row, with `MethodSetup` the `IIterativeSolverSetup<double>` projection of one row and the `IterationPolicy` record (tolerance · max-iter · criterion stack · preconditioner · clock · deadline · cancellation); `FactoredOp` the typed sparse-operator value owning the factorization instance, the cached AMD permutation, the `ColumnOrdering` it was factored with, symbolic fill, and kind discriminant; `Edit` `[Union]` the structural-edit dialect; `SparseOps` the direct-and-iterative sparse-solve fold over CSR-backed MathNet storage and CSparse CSC direct factorizations, and the two-container exchange surface — the `.mtx` pair and the HDF5 archive pair over `Runtime/codecs#HDF_ARCHIVE` — each carrying both directions of the one portable operator correspondence — ingestion routes through the `SparseFormat` row's `ingest` delegate, direct factorization through the `FactorKind` row's `create` delegate, neither through a parallel `FrozenDictionary` keyed by the same enum.
...
- Packages: MathNet.Numerics, CSparse, System.Numerics.Tensors, Microsoft.IO.RecyclableMemoryStream, PureHDF (`H5File`, `H5Group`, `H5Dataset<T>`, `H5DatasetCreation`, `NativeDataset.Read<T>(H5DatasetAccess, Span<T>, …)`, `HyperslabSelection`, `IH5Object.Attribute`), NodaTime, Thinktecture.Runtime.Extensions, LanguageExt.Core, Rasm (project, kernel signal capsule), Rasm.Persistence (project), BCL inbox
...
public static class SparseOps {
    ...
    // HDF5 is the SECOND exchange container beside `.mtx`, composing `Runtime/codecs#HDF_ARCHIVE`. Layout is the
    // scipy sparse group convention — `indptr`/`indices` int32, `values` float64, extents in the `shape`
    // attribute, `format` naming the major axis — so an external corpus crosses without translation.
    const int ExchangeChunk = 1 << 16;

    static uint[] Chunk(int length) => [(uint)Math.Min(length, ExchangeChunk)];
    ...
    public static Fin<SparseCompressedRowMatrixStorage<double>> ReadArchive(HdfHandle archive, string group, SparseFormat format) =>
        Try.lift(() => {
            long[] shape = archive.Group(group).Attribute("shape").Read<long[]>();
            int major = checked((int)(format.MajorIsRow ? shape[0] : shape[1]));
            NativeDataset values = archive.Dataset($"{group}/values");
            ...
            archive.Dataset($"{group}/indptr").Read<int>(archive.Access, indptr.AsSpan(), new HyperslabSelection(0, (ulong)indptr.Length));
            archive.Dataset($"{group}/indices").Read<int>(archive.Access, indices.AsSpan(), new HyperslabSelection(0, (ulong)nonZeros));
            values.Read<double>(archive.Access, payload.AsSpan(), new HyperslabSelection(0, (ulong)nonZeros));
            return (Rows: checked((int)shape[0]), Columns: checked((int)shape[1]), Major: indptr, Minor: indices, Payload: payload);
        }).Run()
            .MapFail(static error => (Error)new ComputeFault.PayloadOverBounds($"<hdf5-sparse-read:{error.Message}>"))
            .Bind(read => Ingest(format, read.Rows, read.Columns, read.Major, read.Minor, read.Payload));

    // `WriteArchive` repairs the `.mtx` metadata drop: MatrixMarket carries pattern symmetry alone, so kind,
    // ordering, symbolic fill, and ‖A‖_F re-derive at every ingest — here they ride group attributes and the
    // applied AMD permutation rides its own dataset, so a reproduction artifact re-factors under the policy it
    // was built with. Data-carrying chunked datasets encode in index order inside the one create-only `Begin`
    // session; `.mtx` stays the SuiteSparse interop surface, never retired.
    public static Fin<RecyclableMemoryStream> WriteArchive(StreamPool pool, CorrelationId correlation, HdfArchivePolicy policy, FactoredOp op) =>
        pool.Get(correlation, new StreamGrant.Open())
            .Bind(staged => Try.lift(() => {
                    try {
                        H5DatasetCreation creation = policy.Creation();
                        H5Group group = new() {
                            ["indptr"] = new H5Dataset<int[]>(op.A.ColumnPointers, chunks: Chunk(op.A.ColumnPointers.Length), datasetCreation: creation),
                            ["indices"] = new H5Dataset<int[]>(op.A.RowIndices, chunks: Chunk(op.A.RowIndices.Length), datasetCreation: creation),
                            ["values"] = new H5Dataset<double[]>(op.A.Values, chunks: Chunk(op.A.Values.Length), datasetCreation: creation),
                            ["permutation"] = new H5Dataset<int[]>(op.Permutation, chunks: Chunk(op.Permutation.Length), datasetCreation: creation),
                        };
                        group.Attributes["shape"] = new long[] { op.A.RowCount, op.A.ColumnCount };
                        group.Attributes["format"] = "csc";
                        group.Attributes["kind"] = op.Kind.Key;
                        group.Attributes["ordering"] = (int)op.Ordering;
                        group.Attributes["fill"] = op.Fill;
                        group.Attributes["frobenius"] = op.FrobeniusNorm;
                        group.Attributes["symmetric"] = op.Kind.Symmetric;
                        HdfArchive.Begin(new H5File { ["A"] = group }, staged, policy).Dispose();
                        staged.Position = 0;
                        return staged;
                    }
                    catch { staged.Dispose(); throw; }
                }).Run()
                .MapFail(static error => (Error)new ComputeFault.PayloadOverBounds($"<hdf5-sparse-write:{error.Message}>")));
...
// value a caller writes down and a plan cache keys on. It rides inside `ShardDispatch.Farm` and reaches no local lowering.
