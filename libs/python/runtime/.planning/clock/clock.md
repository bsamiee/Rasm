...
| [INDEX] | [SHAPE]       | [ROLE]                                                                                                            |
| :-----: | :------------ | :---------------------------------------------------------------------------------------------------------------- |
|  [01]   | `Hlc`         | the two-64-bit-half cell: NodaTime physical instant high, per-node logical counter low, bit-identical to C# `Hlc` |
|  [02]   | `Ordering`    | the three-case causal verdict carrying the C# `Hlc.CompareTo` sign as case payload, with `fold[T]` owner dispatch |
|  [03]   | `ElementId`   | the causal/content-stable peer-local identity the CRDT RGA and OR-set address by                                  |
|  [04]   | `CausalFrame` | the `(Hlc, Tenant)` inbound frame owning the railed `decode` and dual-shape `attributes`, both keyed off `SLOTS`  |
