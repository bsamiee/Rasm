...
- Functions abstract computations so they work for any suitable argument rather than one particular value. A square function, for example, expresses how to square any number instead of only one specific number.
...
- Everything is an expression in this paradigm. Conditionals and functions can therefore be combined with smaller expressions to form more complex ones.
