...
- [1]-[EMBEDDING_LIFECYCLE]: owns the `EmbeddingProfile` migration-row table, the `BatchEmbed` data-loader-plus-cache entrypoint, the durable re-embed schedule, and the boundary against `fused-rank`'s staleness column and the gated BM25 arm.
