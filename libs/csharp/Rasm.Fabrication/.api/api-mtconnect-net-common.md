...
[INTEGRATION_STACK]:
- content identity seam: `CuttingToolAsset.GenerateHash(includeTimestamp: false)` yields the stable structural digest of a tool definition, but it is NEVER a second mint — the durable content key reconciles onto the kernel `ContentHash.Of` seed-zero federation entry (the ONE mint, K9), which reaches `XxHash128.HashToUInt128` through itself. The cutting-tool catalogue and the `Remnant`/`Stock` lineage share that single federation entry, each over its own canonical digest; a raw `System.IO.Hashing`/`GenerateHash` second hasher is the forbidden second-hasher defect
...
- persistence: a `CuttingToolAsset` and its `ContentHash.Of` content key flow to the Persistence artifact index as a content-addressed durable tool-catalogue row alongside the `CutProgram` AST — the durable-row fold is the Fabrication-authored demand on the held-open `[ARTIFACT_CONTENT_KEY_FEDERATION]` blocker, never a composed Persistence contract; the tool library is a content-keyed table, the asset key its row key
