...
class Register(Struct, frozen=True):
    ...
    async def _emit(self) -> RuntimeResult[Composed]:
        crossed = await self.lane.offload(Kernel.of(_composed, KernelTrait.RELEASING), self)
        match crossed:
            case Result(tag="ok", ok=composed):
                Metrics.record({BYTE_VOLUME: float(len(composed.data))}, domain=DOMAIN, kind="register", scope=self.lane.scope)
                return Ok(composed)
            case refused:
