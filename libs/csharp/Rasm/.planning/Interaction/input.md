...
- Packages: Eto.Forms for `Screen`, `Mouse`, `Keyboard`, `Cursor`, and `Cursors` (rosters verified in `libs/csharp/.api/api-eto-runtime.md`); Eto.Drawing for the geometry carriers (prelude-aliased); `Numerics/atoms` for the admitted scalars.
...
- Packages: Eto.Forms for the event surfaces (rosters verified in `libs/csharp/.api/api-eto-forms.md` and `api-eto-runtime.md`); `System.Threading.Channels` for the bounded drain; `Parametric/projections` for the timeline that stamps; LanguageExt.Core for the leases and rails.
