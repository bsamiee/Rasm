...
public abstract partial record MaterialPropertySet {
 ...
 public void CanonicalBytes(CanonicalWriter w) => Switch(
  ...
  environmental: e => { w.Ordinal(4).String(e.Basis.Key).Measure(e.GlobalWarmingPotential); foreach (double s in e.StageGwp.Span) { w.Double(s); } return w.Double(e.RecycledContent).Double(e.EndOfLifeRecovery).String(e.Epd).Ordinal(e.ValidUntilYear); },
...
public static class MaterialPropertyAccess {
 extension(Seq<MaterialPropertySet> properties) {
  ...
  public Option<T> Property<T>() where T : MaterialPropertySet =>
   properties.Choose(static p => p is T t ? Some(t) : None).HeadOrNone();
...
- [TYPED_LOOKUP_COLLAPSE]: the per-case property reads collapse to ONE generic `Property<T>()` over the case type — the six `Choose(p => p is T t ? Some(t) : None).HeadOrNone()` bodies a per-discipline reader would enumerate share one generative structure, so the body lives ONCE on the generic owner and the named per-discipline accessors (`props.Mechanical`/`props.Thermal`/…, the consumer-contract surface the `Rasm.Compute` aggregator reads) are one-line projections of it, never six re-implemented `Choose` bodies; `ForDiscipline(discipline)` is the orthogonal dual (a runtime `Discipline` value rather than a static case type), the `Rasm.Compute` analysis route reading a discipline's property set through whichever entry it holds.
