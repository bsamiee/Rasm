...
[STACKING]:
- `api-languageext`(`libs/dotnet/.api/api-languageext.md`): nullable state reads (`Shout.StreamPath`, `Listen.CurrentDependency`, `Relay.FrozenCachedData`, `TreeViewerObject.DisplayGradient`) lower to `Option<T>`; a construction that rejects null (`ColourSwatchObject` colour) folds to `Fin<T>` where the reject maps to `Error`; the empty-array normalizers (`TimerObject.TargetIds`, `PresetPickerObject.SelectedNames`, `TextInputObject.Values`) carry as `Seq<T>`; `Relay.SafeDisconnect() -> ActionList` sequences through `Eff`; and slider/toggle/button interactive state rides an `Atom` cell.
