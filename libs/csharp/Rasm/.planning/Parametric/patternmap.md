...
public sealed record PatternPlan(
    ...
    TangentLogMapAlgorithm Algorithm, Option<ScalarField> Density = default) : IValidityEvidence {
...
public abstract partial record InstanceStream {
    ...
    public sealed record Planar(
        Arr<(double U, double V)> Site, Arr<double> Spin, Arr<bool> Mirrored, Arr<int> Anchor, Arr<int> Seat) : InstanceStream;
...
public static class Patterning {
    ...
    static Fin<InstanceStream.Planar> OrbitOf(PatternPlan plan, Op? key) {
        ...
        (List<(double U, double V)> site, List<double> spin, List<bool> mirrored, List<int> anchor, List<int> seat) =
            (new List<(double U, double V)>(), new List<double>(), new List<bool>(), new List<int>(), new List<int>());
