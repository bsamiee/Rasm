...
- `ContentIdentity` rail: the owning page mints the node key pre-run through `ContentIdentity.key("schematic", ...)` over the length-framed canonical spec⊕theme chunks, so keyed elision probes the warm seed before rendering; schemdraw mints no identity.
