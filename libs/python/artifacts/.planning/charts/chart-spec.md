...
[CLUSTERS]: two clusters — `[2]-[CHART]`, the 2D chart-spec union over the three engines; `[3]-[EXPORT]`, the host-free static-export backend rows with the vegafusion transform pre-pass.
...
async def _vega_to_bytes(spec: dict[str, object], fmt: ExportFormat) -> bytes:
    settled = (
        json.loads(await to_process.run_sync(_vegafusion_pre_transform, json.dumps(spec)))
        if _has_transforms(spec)
        else spec
    )
...
def _has_transforms(spec: dict[str, object]) -> bool:
    return "transform" in spec or any(
        "transform" in layer for layer in spec.get("layer", ()) if isinstance(layer, dict)
    )
```
