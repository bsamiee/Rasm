...
- [TERMINATOR_SELF_CONTAINED] [RESOLVED]: the prior `_terminator_layer` called `VectorOp.Outline(...)` and `Vector.over(ops)._worked(ops)` — BOTH phantoms. the pre-split `graphic/vector.md` exposed NO `VectorOp.Outline` case (its then-verified `VectorOp` family was `transform`/`bounds`/`serialize`/`rasterize`/`measure`/`sample`/`flatten`/`subpaths`/`project`), and `_worked` is a private MODULE-LEVEL function on that page, not a `Vector` method (the `Vector` rail is the async `over`/`of` returning `RuntimeRail[Block[VectorResult]]`), so the call was a hard `AttributeError` dressed as dense composition, AND its `_polyline` centerlines were strokeless (invisible) awaiting a never-landed outline pass — a self-admitted illusory rail ("the anticipated arm"). The rebuild draws each ISO 129-1 terminator as a self-contained filled/stroked SVG mark (a filled arrow TRIANGLE is the correct closed filled arrowhead; the oblique tick and open chevron are stroked; the dot is filled) penned by the discipline sRGB, and threads the same pen through the construction/text/tolerance layers so the whole editable projection carries the CAD pen — exactly the self-contained stance the finalized `drawing/symbol#SYMBOL`/`drawing/annotate#ANNOTATE` siblings take. The tapered variable-width terminator stroke-to-outline composes the LANDED `graphic/vector/region#REGION` `outline` — its public `outline(source, width, cap=CapStyle.BUTT_CAP, join=JoinStyle.MITER_JOIN, miter=4.0, dash=None)` and `RegionOp.outline` case are present on that owner (the current `graphic/vector` `VectorOp` family is `transform`/`bounds`/`fit`/`serialize`/`rasterize`/`measure`/`sample`/`flatten`/`subpaths`/`project`/`boolean`/`outline`/`region`), shared with `symbol`/`annotate`, never authored twice. The self-contained filled arrow triangle is the CORRECT ISO 129-1 filled terminator and stays the default (a fill, not a stroked centerline `outline` would convert), so it is not force-routed through `outline`; the still-rejected phantom is the OLD `Vector.over(ops)._worked(ops)` private-method reach (`_worked` remains a private module fold on that owner, never a `Vector` method). Justified on PACKAGE (the landed `graphic/vector/region` `outline`/`RegionOp.outline`) and DOMAIN (a filled arrow triangle IS the ISO 129-1 filled terminator, self-contained by default) and CONSUMER (the tapered outline the composable premium over the correct base).
