...
- Law: `StackOutputs.custody` is the second half of that one catalog — the variables a namespace custody cell may carry (`DOPPLER_TOKEN`, the encoded `SECURITY_LEASE_SPEC` the security custodian decodes its whole scope out of) are deploy-to-process spellings exactly as the channel rows are, and both the minting tier (`operate/secret.md`'s leased cell) and the stamping tier (`kube/workload.md`'s env fold) read this record rather than either spelling a literal the other must match by hand; `Cell` is the shape both sides pass, so the mint and the stamp are one type and a second custody variable is one row here reaching both ends at compile time. Two independent literals for one variable name is the silent fork this seat deletes, and it seats here for the same reason the channels do — a process reads it, and neither writing tier sits below the other.
