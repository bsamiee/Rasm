...
Region-adjacency-graph (RAG) merging over a label image plus minimum-cost-path routing (the latter is the `skimage.future.graph` family promoted to `skimage.graph` in 0.26.0; `skimage.future.graph` no longer exists).
