...
The hop rides the `cadquery-ocp` `OCP` binding, the sole PyPI OCCT path. The daemon routes its `cad(source_bytes, fmt)` case here; the bridge mints no `ContentKey`, since the daemon keys the SOURCE bytes through the runtime `rasm.runtime.identity` `ContentIdentity.of` with `TessellationPolicy.spec` folded into the canonical seed bytes before the offload hop, so a cache hit skips the OCCT pass entirely. `TessellationPolicy` is MINTED HERE beside `BridgeFormat` — the mesher knobs (`deflection`/`angle_tolerance` and their canonical `spec` seed bytes) are geometry-owned, never runtime `IdentityPolicy` fields; `StepBridge.tessellate` consumes it in-signature and the `mesh/daemon`/`mesh/brep` consumers import it downward. One daemon content-addresses AEC (IFC) and mechanical (CAD-STEP) geometry through one keyed admission. The wire aligns to the C# `StepIso10303` codec, which requests CAD tessellation from this companion rather than re-implementing a managed reader.
...
# the geometry-owned mesher policy — MINTED HERE at tier 1 so StepBridge consumes it in-signature and
# the daemon/brep consumers import it downward (a daemon-sited mint would force a cad -> daemon back-edge).
# `spec` is the canonical seed-byte projection the daemon folds into its policy-keyed cache seed; the
# seed-zero GLB wire key stays distinct (the two-key discipline). No runtime IdentityPolicy field carries these knobs.
