...
const _assertProperty = <E, R>(
    ...
    Effect.flatMap(verdict, (holds) =>
        holds ? Effect.void : Effect.fail(new PropertyViolationError(evidence)),
    );

const _succeeds = <A, E, R>(
...
const Property = {
        ...
        Effect.flatMap(
            Effect.exit(
                definition.predicate(
                    definition.counterexample.implementation,
                    definition.counterexample.args,
                ),
            ),
            (exit) =>
                Exit.match(exit, {
                        ...
                        holds
                            ? Effect.fail(
                                  new InvalidPropertyCounterexampleError({
                                      ...
                                      counterexample:
                                          definition.counterexample.label,
                                  }),
                              )
                            : Effect.void,
                }),
    ...
    define: <S, const A extends Property.Arbitraries, E = never, R = never>(
        ...
        register: (api, subject) => {
            ...
            api.effect(
                `${definition.name} rejects ${definition.counterexample.label}`,
                () => Property.verifyCounterexample(definition),
            );
        },
    }),
    register: <S, R>(
        api: Property.Api<R>,
        subject: S,
        properties: ReadonlyArray<Property.Registered<S, R>>,
    ): void => {
    ...
    > =>
        Property.define({
            ...
            predicate: (setup, { run }) =>
                _succeeds(Effect.try(() => FastCheck.modelRun(setup, run))),
            counterexample: options.counterexample,
        }),
