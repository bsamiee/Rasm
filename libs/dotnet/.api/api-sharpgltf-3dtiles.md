...
- Storage authors through three parallel folds: `AddPropertyTable(class, featureCount, name)` → `UseProperty().SetValues<T>`/`SetArrayValues<T>`; `AddPropertyTexture(class).CreateProperty(texture, channels)`; `AddPropertyAttribute(class).CreateProperty()`; `BinaryTable` and `ComponentCount` encode byte width, string and array offsets, and element counts.
