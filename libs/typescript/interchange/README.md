...
- [01]-[TRANSPORT](.planning/Transport/transport.md): Protocol-selection transport, the four clients, the capability tuple, the framing fold, and the `buf` + capability-SDK codegen input.
- [02]-[GATEWAY](.planning/Transport/gateway.md): Outbound verb gateway, the dial-time availability gate, and the intent registry.
- [03]-[CODEC](.planning/Codec/codec.md): Codec-keyed dispatch table for both directions, the GeoJSON geometry vocabulary row, the backpressured segment-stream decode, and the compression-codec admission.
- [04]-[PATCH](.planning/Codec/patch.md): Recorded-intent six-verb `PatchOpWire` union, the error-accumulating `applyPatch` fold, the `createTests` concurrency guard, and the `FieldMask` lower.
- [05]-[FRAME](.planning/Codec/frame.md): Content-addressed reassembly, the owned `Crc32`, the `XxHash128` content key, and the transferable worker boundary.
- [06]-[PARITY](.planning/Codec/parity.md): Frozen-corpus content-key reproduction binding, the LE↔BE normalize, and the HLC two-half round-trip.
- [07]-[INVENTORY](.planning/Contract/inventory.md): Thirteen-cluster cluster-to-rail map, the three suite anchors, and the versioning law.
- [08]-[DESCRIPTOR](.planning/Contract/descriptor.md): Runtime descriptor-diff verdict (`Identical`/`Additive`/`Breaking`) over `createFileRegistry` reflection, the declaration-order checksum, and the dial-time gate.
- [09]-[REFINEMENT](.planning/Ingress/refinement.md): Brand identity slots (`Guid`/`ContentKey`/`OrdinalKey`/`JsonPointer`), the filter bounds, and the decode-budget ingress ceilings.
