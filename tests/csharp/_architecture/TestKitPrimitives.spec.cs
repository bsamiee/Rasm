...
public sealed class TestKitPrimitiveLaws {
    ...
    public void ScanAssemblyFoldsLawMarkers() {
        Seq<LawRecord> manifest = Laws.ScanAssembly(specAssembly: typeof(TestKitPrimitiveLaws).Assembly);
    ...
    public void CoverageCreditsManifestAndExemptions() {
        ...
        Seq<LawRecord> covering = toSeq(surface).Map(static name => new LawRecord(Subject: typeof(Op), Name: name, Member: Optional(name), DeclaringType: typeof(TestKitPrimitiveLaws)));
