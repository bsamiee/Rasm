...
| [INDEX] | [FAMILY]            | [DRIVES]                                             |
| :-----: | :------------------ | :--------------------------------------------------- |
|  [01]   | `TEXTFLAGS_*`       | `get_text`/`get_textpage` flag composition           |
|  [02]   | `TEXT_OUTPUT_*`     | `get_text` output kind                               |
|  [03]   | `PDF_REDACT_*`      | `apply_redactions(images=, graphics=, text=)` policy |
|  [04]   | `PDF_ENCRYPT_*`     | `save(encryption=)` strength                         |
|  [05]   | `PDF_PERM_*`        | `save(permissions=)` bitmask                         |
|  [06]   | `PDF_WIDGET_TYPE_*` | `Widget.field_type` discriminant                     |
|  [07]   | `PDF_ANNOT_*`       | `Annot.type` discriminant / `set_flags`              |
|  [08]   | `STAMP_*`           | `add_stamp_annot(stamp=)`                            |

- [01]-[TEXTFLAGS]: `TEXT`/`WORDS`/`BLOCKS`/`DICT`/`RAWDICT`/`HTML`/`XHTML`/`XML`/`SEARCH` presets + `TEXT_PRESERVE_*`/`TEXT_DEHYPHENATE` bits.
- [02]-[TEXT_OUTPUT]: `TEXT`/`HTML`/`JSON`/`XHTML`/`XML`.
- [03]-[PDF_REDACT]: image `NONE`(0)/`REMOVE`(1)/`PIXELS`(2)/`REMOVE_UNLESS_INVISIBLE`; line-art `NONE`(0)/`REMOVE_IF_TOUCHED`(1)/`REMOVE_IF_COVERED`; text `REMOVE`(0)/`NONE`(1)/`REMOVE_INVISIBLE` (text family value order is INVERTED vs image — `0` is remove).
- [04]-[PDF_ENCRYPT]: `KEEP`/`NONE`/`RC4_40`/`RC4_128`/`AES_128`/`AES_256`.
- [05]-[PDF_PERM]: `PRINT`/`MODIFY`/`COPY`/`ANNOTATE`/`FORM`/`ACCESSIBILITY`/`ASSEMBLE`/`PRINT_HQ`.
- [06]-[PDF_WIDGET_TYPE]: `UNKNOWN`/`BUTTON`/`CHECKBOX`/`RADIOBUTTON`/`TEXT`/`LISTBOX`/`COMBOBOX`/`SIGNATURE`.
- [07]-[PDF_ANNOT]: `TEXT`/`FREE_TEXT`/`LINE`/`SQUARE`/`HIGHLIGHT`/`INK`/`STAMP`/`REDACT`/`WIDGET`/… kind codes + `PDF_ANNOT_IS_*` flag bits.
- [08]-[STAMP]: `Approved`/`Confidential`/`Draft`/`Final`/`TopSecret`/… 14 preset stamps.
...
- rail: pdf
- call: `open(filename=None, stream=None, filetype=None, ...)` (module fn, alias of `Document`); `Document(filename=None, stream=None, filetype=None, rect=None, width=0, height=0, fontsize=11)`; `save(filename, garbage=0, clean=0, deflate=0, deflate_images=0, deflate_fonts=0, incremental=0, ascii=0, expand=0, linear=0, no_new_id=0, appearance=0, pretty=0, encryption=1, permissions=4095, owner_pw=None, user_pw=None, preserve_metadata=1, use_objstms=0, compression_effort=0, raise_on_repair=False, ...)`; `Document.ez_save`: target plus defaulted kwargs; `Document.convert_to_pdf`: page range plus rotation; `Document.authenticate`: password; `insert_pdf(docsrc, *, from_page=-1, to_page=-1, start_at=-1, rotate=-1, links=1, annots=1, widgets=1, join_duplicates=0, show_progress=0)`; `Document.insert_file`: source path/stream plus splice policy; `select(list[int])`; `Document.delete_pages`: range or list of page indices; `Document.copy_page` / `move_page` / `fullcopy_page`: source/target index; `close() -> None`; `with pymupdf.open(...) as doc:` (`__enter__` returns the `Document`, `__exit__` calls `close`); `set_metadata(m: dict) -> None`; `set_xml_metadata(metadata: str) -> None`.

Document rows carry path/stream/filetype input, save compaction, encryption, conversion, password, and splice policy.

| [INDEX] | [SURFACE]                                            | [CAPABILITY]                                          |
| :-----: | :--------------------------------------------------- | :---------------------------------------------------- |
|  [01]   | `open`                                               | open from path or in-memory stream                    |
|  [02]   | `Document`                                           | constructor; `pymupdf.open` is the module-level alias |
|  [03]   | `Document.save`                                      | compaction, linearization, and encryption knobs       |
|  [04]   | `Document.ez_save`                                   | deflate/garbage/use-objstms defaults on               |
|  [05]   | `Document.convert_to_pdf`                            | convert a non-PDF document to PDF bytes               |
|  [06]   | `Document.authenticate`                              | unlock an encrypted document                          |
|  [07]   | `Document.insert_pdf`                                | splice pages from another `Document`                  |
|  [08]   | `Document.insert_file`                               | splice pages from any supported file/stream           |
|  [09]   | `Document.select`                                    | reorder/subset to a page-index list in place          |
|  [10]   | `Document.delete_pages`                              | drop pages (`delete_page` for one)                    |
|  [11]   | `Document.copy_page` / `move_page` / `fullcopy_page` | duplicate or relocate a page internally               |
|  [12]   | `Document.close` / context manager                   | deterministic native-handle release                   |
|  [13]   | `Document.set_metadata`                              | write info dict; `{}` clears it                       |
|  [14]   | `Document.set_xml_metadata`                          | write archival PDF/A XMP metadata                     |

- [03]-[SAVE_POLICY]: `compression_effort` tunes brotli object-stream effort; `deflate_fonts` shrinks embedded fonts; `preserve_metadata` keeps the info dict.
- [12]-[CLOSE_POLICY]: The `with` bracket closes the live handle on scope exit rather than GC-reaping it.
- [13]-[INFO_DICT]: The document info dict carries title/author/subject/keywords/creator/producer.

[ENTRYPOINT_SCOPE]: outline and embedded files
- rail: pdf
- call: `get_toc(simple=True) -> list[[level, title, page, ...]]`; `set_toc(toc, collapse=1) -> int`; `embfile_names() -> list[str]`; `embfile_get(item: int \| str) -> bytes`; `embfile_info(item: int \| str) -> dict`; `embfile_count() -> int`; `embfile_add(name, buffer_, filename=None, ufilename=None, desc=None) -> None`; `embfile_upd(item, buffer_=None, filename=None, ufilename=None, desc=None)`; `embfile_del(item: int \| str)`; `outline -> Outline` (then `.title`/`.dest`/`.next`/`.down`); `set_toc_item(idx, dest_dict=, ...)` / `del_toc_item(idx)`.
...
| [INDEX] | [SURFACE]                                | [CAPABILITY]                                        |
| :-----: | :--------------------------------------- | :-------------------------------------------------- |
|  [01]   | `Document.get_toc`                       | recover the nested outline as level/title/page rows |
|  [02]   | `Document.set_toc`                       | author/replace the outline from level rows          |
|  [03]   | `Document.embfile_names`                 | list embedded-file entry names                      |
|  [04]   | `Document.embfile_get`                   | read one embedded file's bytes by name/index        |
|  [05]   | `Document.embfile_info`                  | embedded-file metadata (filename, desc, size)       |
|  [06]   | `Document.embfile_count`                 | embedded-file entry count                           |
|  [07]   | `Document.embfile_add`                   | attach a new embedded file                          |
|  [08]   | `Document.embfile_upd`                   | replace an embedded file's bytes/metadata           |
|  [09]   | `Document.embfile_del`                   | drop an embedded file                               |
|  [10]   | `Document.outline`                       | linked-node outline tree for recursive walks        |
|  [11]   | `Document.set_toc_item` / `del_toc_item` | edit/drop a single outline entry in place           |

[ENTRYPOINT_SCOPE]: redaction-grade scrub, bake, and font subset
- rail: pdf
- call: `scrub(attached_files=True, clean_pages=True, embedded_files=True, hidden_text=True, javascript=True, metadata=True, redactions=True, redact_images=0, remove_links=True, reset_fields=True, reset_responses=True, thumbnails=True, xml_metadata=True) -> None`; `bake(*, annots=True, widgets=True) -> None`; `subset_fonts(verbose=False, fallback=False) -> int \| None`.
...
| [INDEX] | [SURFACE]               | [CAPABILITY]                                                |
| :-----: | :---------------------- | :---------------------------------------------------------- |
|  [01]   | `Document.scrub`        | redaction-grade removal of hidden/attached/metadata content |
|  [02]   | `Document.bake`         | flatten annotations and form widgets into content           |
|  [03]   | `Document.subset_fonts` | subset embedded fonts in place, returning byte delta        |

[ENTRYPOINT_SCOPE]: annotation recovery and image-document assembly
- rail: pdf
- call: `annots(types=None) -> Iterator[Annot]`; `info -> dict` (`content`/`title`/`subject`/`name`/`creationDate`/`modDate`); `rect -> Rect`; `type -> (int, str)`; `get_file() -> bytes` / `update_file(buffer=, filename=, ufilename=, desc=)`; `field_name -> str` (instance attr); `field_value -> str \| bool \| None` (instance attr); `field_type -> int` (instance attr); `is_pdf -> bool` (+ `is_reflowable`/`is_stream`/`is_form_pdf` siblings); `new_page(pno=-1, width=595, height=842) -> Page`; `tobytes(garbage=0, deflate=False, ...) -> bytes`.
...
| [INDEX] | [SURFACE]                        | [CAPABILITY]                                                       |
| :-----: | :------------------------------- | :----------------------------------------------------------------- |
|  [01]   | `Page.annots`                    | iterate existing annotations on the page                           |
|  [02]   | `Annot.info`                     | annotation metadata dictionary                                     |
|  [03]   | `Annot.rect`                     | annotation bounding rectangle                                      |
|  [04]   | `Annot.type`                     | annotation type number and name (`PDF_ANNOT_*`)                    |
|  [05]   | `Annot.get_file` / `update_file` | read/replace a `FileAttachment` annotation's payload               |
|  [06]   | `Page.rect`                      | the page rectangle (media/crop box)                                |
|  [07]   | `Widget.field_name`              | the form field's name                                              |
|  [08]   | `Widget.field_value`             | the form field's current value                                     |
|  [09]   | `Widget.field_type`              | the form field's `PDF_WIDGET_TYPE_*` int discriminant              |
|  [10]   | `Document.is_pdf`                | opened-format discriminant; `not is_pdf` gates single-image intake |
|  [11]   | `Document.new_page`              | append a blank page and return it                                  |
|  [12]   | `Document.tobytes`               | serialize the in-memory document to PDF bytes                      |
...
| [INDEX] | [SURFACE]                  | [CAPABILITY]                                                |
| :-----: | :------------------------- | :---------------------------------------------------------- |
|  [01]   | `Page.get_pixmap`          | rasterize a page                                            |
|  [02]   | `Page.get_text`            | extract text in plain/dict/json/words/html modes            |
|  [03]   | `Page.get_textpage`        | build a reusable text model                                 |
|  [04]   | `Page.get_textpage_ocr`    | Tesseract-OCR a page into a searchable `TextPage`           |
|  [05]   | `Page.search_for`          | locate text regions                                         |
|  [06]   | `Page.get_images`          | enumerate embedded images                                   |
|  [07]   | `Page.apply_redactions`    | burn in redactions with module `PDF_REDACT_*` int flags     |
...
|  [15]   | `table.TableFinder.tables` | resolved tables from `find_tables`                          |
|  [16]   | `table.Table.extract`      | row-major cell text of one resolved table                   |
|  [17]   | `table.Table.to_pandas`    | resolved `DataFrame` feeding the table/dataframe owner      |
|  [18]   | `table.Table.to_markdown`  | resolved table as a GitHub-flavoured Markdown grid          |
|  [19]   | `table.Table` shape        | resolved-table geometry and row/header objects              |

[ENTRYPOINT_SCOPE]: page-box geometry
- rail: pdf — the five PDF page boxes: MediaBox (physical sheet), CropBox (visible region), TrimBox (finished trim), BleedBox (bleed extent), ArtBox (meaningful content). `composition/sheet#SHEET`'s V7 page-box law authors TrimBox/BleedBox for the bleed/creep geometry a press fold needs; the read attrs and the `set_*` setters are the write surface (each box a `Rect`; boxes are inherited/clamped — CropBox clamps to MediaBox).
- call: `Page.mediabox` / `cropbox` / `trimbox` / `bleedbox` / `artbox -> Rect`; `set_mediabox(rect: Rect)`; `set_cropbox(rect: Rect)`; `set_trimbox(rect: Rect)`; `set_bleedbox(rect: Rect)`; `set_artbox(rect: Rect)`.

| [INDEX] | [SURFACE]             | [CAPABILITY]                                        |
| :-----: | :-------------------- | :-------------------------------------------------- |
|  [01]   | `Page` box read attrs | read five unrotated boxes in PDF coordinates        |
|  [02]   | `Page.set_mediabox`   | set physical sheet; reset other boxes — call FIRST  |
|  [03]   | `Page.set_cropbox`    | set visible region clamped to MediaBox              |
|  [04]   | `Page.set_trimbox`    | set finished-trim cut line inside bleed             |
|  [05]   | `Page.set_bleedbox`   | set printed-past-trim extent read by the press fold |
|  [06]   | `Page.set_artbox`     | set the meaningful-content box                      |

- [01]-[BOX_ORIGIN]: `mediabox_size -> Point` and `cropbox_position -> Point` give sheet size and crop origin.

[ENTRYPOINT_SCOPE]: vector drawing, glyph authoring, and annotation/redaction authoring
- rail: pdf
- call: `new_shape() -> Shape`; then `draw_line`/`draw_rect`/`draw_circle`/`draw_bezier`/`draw_polyline`/`draw_oval`/`draw_sector`/`draw_curve` + `finish(...)` + `commit(overlay=)`; `Page.draw_*`: `draw_line` / `draw_rect` / `draw_circle` / `draw_bezier` / `draw_polyline` (+ all `Shape` verbs); `get_drawings(extended=False) -> list[dict]`; `insert_htmlbox(rect, text, *, css=None, scale_low=0, archive=None, rotate=0, oc=0, opacity=1, overlay=True) -> tuple`; `write_text(rect=None, writers=, ...)` over a `TextWriter` (`append(pos, text, font=, fontsize=)`); `Page.insert_text` / `insert_textbox`: point/rect + `fontname`/`fontsize`/`color`/`rotate`/`align`; `add_redact_annot(quad, text=None, fontname=None, fontsize=11, align=0, fill=None, text_color=None, cross_out=True) -> Annot`; `add_highlight_annot` / `add_text_annot` / `add_freetext_annot` / `add_line_annot` / `add_rect_annot` / `add_circle_annot` / `add_ink_annot` / `add_stamp_annot` / `add_file_annot` / `add_squiggly_annot` / `add_strikeout_annot` / `add_underline_annot` / `add_polyline_annot` / `add_polygon_annot`; `find_tables(clip=None, vertical_strategy="lines", horizontal_strategy="lines", snap_tolerance=3, join_tolerance=3, edge_min_length=3, intersection_tolerance=3, text_tolerance=3, strategy=None, add_lines=None) -> table.TableFinder`; `widgets(types=None) -> Iterator[Annot]` / `add_widget(widget: Widget) -> Annot`; `get_links() -> list[dict]` / `insert_link(dict)` / `delete_link(dict)`; `show_pdf_page(rect, docsrc, pno=0, keep_proportion=True, overlay=True, oc=0, rotate=0, clip=None) -> int`.
...
| [INDEX] | [SURFACE]                                        | [CAPABILITY]                                            |
| :-----: | :----------------------------------------------- | :------------------------------------------------------ |
|  [01]   | `Page.new_shape`                                 | accumulate vector geometry then burn it onto the page   |
|  [02]   | `Page.draw_*`                                    | one-shot vector primitive (auto-`Shape` + `commit`)     |
|  [03]   | `Page.get_drawings`                              | recover page vector paths (fills/strokes/clips)         |
|  [04]   | `Page.insert_htmlbox`                            | flow styled HTML/CSS into a rect; return fill spillover |
|  [05]   | `Page.write_text`                                | commit positioned-glyph text built by a `TextWriter`    |
|  [06]   | `Page.insert_text` / `insert_textbox`            | simple single-line or wrapped-box text insert           |
|  [07]   | `Page.add_redact_annot`                          | mark a region before `apply_redactions`                 |
|  [08]   | `Page.add_*_annot`                               | author every kind; `Annot.update()` bakes appearance    |
|  [09]   | `Page.find_tables`                               | resolve ruled/text tables natively into a `TableFinder` |
|  [10]   | `Page.widgets` / `add_widget`                    | iterate/author interactive form-field widgets           |
|  [11]   | `Page.get_links` / `insert_link` / `delete_link` | recover/author page `LINK_*` objects                    |
|  [12]   | `Page.show_pdf_page`                             | vector-copy a clipped source PDF region into the page   |

- [12]-[PDF_PLACEMENT]: `clip` selects the source region; `rect` receives it; `oc=` optionally binds an OCG/OCMD xref. The native figure-placement/imposition primitive never rasterizes before embedding.
...
| [INDEX] | [SURFACE]                                 | [CAPABILITY]                                      |
| :-----: | :---------------------------------------- | :------------------------------------------------ |
|  [01]   | `Document.add_ocg`                        | create an optional-content group; return its xref |
|  [02]   | `Document.set_ocmd`                       | mint an OCMD over member OCG xrefs                |
|  [03]   | `Document.set_oc` / `get_oc` / `get_ocmd` | bind/read an object's optional-content membership |
|  [04]   | `Document.layer_ui_configs` / `set_layer` | enumerate/toggle layer UI configurations          |
|  [05]   | `Document.journal_enable`                 | open journal and bracket an operation             |
|  [06]   | `Document.journal_undo` / `journal_redo`  | step the journal backward/forward                 |
|  [07]   | `Document.save_snapshot`                  | write an incremental snapshot tied to the journal |

- [02]-[OCMD_POLICY]: `policy` accepts `AnyOn`/`AllOn`/`AnyOff`/`AllOff`; `ve` carries a visibility expression. OCMD expresses nested-layer/radio-group hierarchy that `add_ocg` alone cannot express.
- [03]-[OCMD_READ]: `get_ocmd(xref)` recovers a minted OCMD.

[ENTRYPOINT_SCOPE]: reflowable story layout
- rail: pdf
- call: `Story(html="", user_css=None, em=12, archive=None)`; `write(writer, rectfn, positionfn=None, pagefn=None)`; `write_stabilized(writer, contentfn, rectfn, ...)`; `place(where, flags=0) -> (more, filled)` / `draw(device, matrix=None)`; `add_pdf_links(document_or_stream, positions)`; `DocumentWriter(path, options="")`; `begin_page(mediabox) -> Device`; `end_page()`; `close()`; `paper_rect(s: str) -> Rect`.
...
| [INDEX] | [SURFACE]                   | [CAPABILITY]                                    |
| :-----: | :-------------------------- | :---------------------------------------------- |
|  [01]   | `Story`                     | build a reflowable HTML story                   |
|  [02]   | `Story.write`               | one-shot layout across writer pages             |
|  [03]   | `Story.write_stabilized`    | regenerate HTML and iterate until stable        |
|  [04]   | `Story.place` / `draw`      | manual slice-by-slice layout + render           |
|  [05]   | `Story.add_pdf_links`       | inject live links from placed element positions |
|  [06]   | `DocumentWriter`            | drawn-page sink for path or `BytesIO`           |
|  [07]   | `DocumentWriter.begin_page` | open a page and return its draw device          |
|  [08]   | `DocumentWriter.end_page`   | close the current page                          |
|  [09]   | `DocumentWriter.close`      | finalize the written document                   |
|  [10]   | `paper_rect`                | named paper-size rect (e.g. `"a4"`)             |
...
- page-box axis: the five boxes read as `Page.mediabox`/`cropbox`/`trimbox`/`bleedbox`/`artbox` `Rect`s and write as `set_mediabox`/`set_cropbox`/`set_trimbox`/`set_bleedbox`/`set_artbox` — `set_mediabox` FIRST (it resets the others to the sheet), then TrimBox inside the sheet and BleedBox around TrimBox for the `composition/sheet#SHEET` V7 bleed/creep geometry; CropBox clamps to MediaBox. `paper_rect("a4")` seeds the ISO sheet; never hand-compute box coordinates a setter owns.
