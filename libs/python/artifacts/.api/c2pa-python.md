...
- `anyio` (`libs/python/.api/anyio.md`): the native `libc2pa_c` core is GIL-releasing and the `ta_url`/remote transport is blocking I/O, so `Builder.sign`/`Reader.json`/`with_fragment` cross the runtime `LanePolicy.offload(Kernel.of(..., KernelTrait.RELEASING))` seam — the lane owning the band, boundary, and rail — keeping the work off the event loop. The in-process ctypes binding needs NO `HOSTILE` process crossing (distinct from the process-crossing `exchange/detect`/`metadata` siblings).
