...
- Boundary:
  ...
  - Propagation MECHANICS belong to AppHost `TraceContext` and the frame's CARRIER BODIES belong here: the spine's `Inject<TCarrier>`/`Extract<TCarrier>`/`Continue<TCarrier>` take any getter/setter delegate pair, and a domain carrier's concrete pair seats beside its consuming egress leg — the seating the NATS carrier takes at its egress owner, while the CloudEvents pair seats at `Rasm/Domain/event#ENVELOPE_MINT` as `EventCarrier.Read`/`.Write` because one kernel owner holds that whole attribute space and an unrostered field must DROP on write rather than mint an attribute every decode reads untyped — so `CollabCarrier` binds the pair over `CollabWireContext` here, following the NATS seating a carrier with no kernel owner takes, and a collab adapter row inside `telemetry.md` is the rejected form.
...
// take the other seating for the opposite reason: their attribute space has ONE kernel owner, so that pair
// seats there rather than beside any consumer, and this frame has no such owner to seat at. Injection
