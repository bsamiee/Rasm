...
The scikit-image produced-raster transform engine. ONE engine over the nine transform-engine families that PRODUCE a new raster — `restoration` (the four denoisers plus inpaint/rolling-ball and the deconvolution family Richardson-Lucy/Wiener/unsupervised-Wiener/phase-unwrap), `color` (H&E/HDX stain unmixing plus the YCbCr broadcast colorspace), `exposure` (CLAHE/equalize/rescale/histogram-match/gamma/log/sigmoid), `segmentation` (SLIC/Felzenszwalb/quickshift/watershed/Chan-Vese/morphological-Chan-Vese), `morphology` (skeletonize/medial-axis/thin/opening/closing/erosion/dilation plus white-/black-tophat/reconstruction/remove-small-objects over the `MorphKind` disposition), `thresholding` (Otsu/local/multi-Otsu plus Li/Yen/Isodata/triangle/mean/minimum and the Niblack/Sauvola local document binarizers), `geometric-transform` (resize/rescale/rotate/swirl/warp-polar/Radon/inverse-Radon), and `filters` (unsharp/gaussian/difference-of-gaussians/median/sobel/laplace/frangi/butterworth/gabor/canny plus the scharr/prewitt/roberts/farid gradient and sato/hessian/meijering ridge operators) — folded by the `TRANSFORMS` `frozendict` member-acceptor-kwargs table over the worker band. This page owns the shared transform substrate the measure half composes: the frozen-dataclass `TransformInput`/`TransformArm` carriers and the `_save_array`/`_luminance`/`_channels` helpers, plus the base `TRANSFORMS` table carrying these nine families' seventy-one rows. The `Raster`/`RasterOp` owner, the `Transform` StrEnum vocabulary, and the `_gated_raster` dispatcher live on `graphic/raster/io#IO`; the measurement half (`_measure`/`_register`/`_metrics` plus `MEASURE_TRANSFORMS`) lives on `graphic/raster/measure#MEASURE` and imports this page's substrate. Every acceptor yields one typed `RasterFact` (declared on `graphic/raster/io#IO`, imported here) so the `_gated_raster` `transform` arm folds one shape into `core/receipt#RECEIPT` `ArtifactReceipt.Preview`.
...
class MorphKind(StrEnum):
    ...
    BINARY_PLAIN = "binary-plain"          # binary op, no footprint: skeletonize / medial_axis / thin
    ...
    PRUNE = "prune"                        # component removal below a size floor: remove_small_objects
...
def _color(tx: TransformInput) -> RasterFact:
    ...
    match tx.kind:
        ...
        case _:  # YCBCR broadcast colorspace
...
def _morphology(tx: TransformInput) -> RasterFact:
    ...
    match row.morph:
        ...
        case MorphKind.PRUNE:
            result = member(binary, min_size=int(opts["min_size"]))
