        ...
        research: [
            {
                key: 'rhinocommon',
                catalogs:
                    'api-rhinocommon-document.md, api-rhinocommon-commands.md, api-rhinocommon-display.md, ' +
                    'api-rhinocommon-blocks.md, api-rhinocommon-fileio.md, api-rhinocommon-geometry.md, api-rhino-ui.md',
                charge:
                    'RhinoCommon on Rhino 9 WIP: RhinoDoc lifecycle and events, object tables (objects/layers/materials/groups/' +
    ...
    },
    {
        ...
        research: [
            ...
            },
            {
                key: 'gh2ui',
                catalogs:
                    'api-gh2-canvas.md, api-gh2-editor.md, api-gh2-flex.md, api-gh2-interaction.md, ' +
                    'api-eto-forms.md, api-eto-runtime.md, api-eto-drawing.md, api-macos-native.md',
                charge:
                    'GH2 UI + Eto inside Grasshopper: canvas paint phases and picking, skinning, wire shapes, snapping/' +
...
const codexPrompt = (label, task, schema, writes) => {
    ...
    return [
        ...
        '(4) READY: do NOT relay the report body through your output — build the MECHANICAL headline with jq (never your own ' +
            ...
            base +
            '-report.json); top=$(jq -r \'[.entries[].files[0]] | group_by(.) | max_by(length) | .[0] // "none"\' ' +
            base +
            '-report.json). ' +
            'Return the RECEIPT: ok=true, report=' +
            base +
            '-report.json, entries=that count, headline="<entries> entries | <kinds> | top: <top>", failure empty. ' +
...
const researchPrompt = (folder, lane) =>
    [
        CONTEXT,
        MANDATE,
        INFO_LAW,
        'TASK: CURRENT-ONLY HOST-API RESEARCH for ' +
            folder.name +
            ' - lane ' +
            lane.key +
            '. ' +
            ...
            'existing folder .api stub catalogs IN PLACE at ' +
            folder.root +
            '/.api/ - each stub declares its owned namespace ' +
            'scope; you grow it to a verified-member catalog (exact signatures, integration shape per capability cluster, what ' +
            'the census-era source missed), preserving each stub file scope and name: ' +
            lane.catalogs +
            '. These files are ' +
            'YOURS alone this run - no other lane writes them' +
            (folder.key === 'gh' ? ' (the Rhino folder owns its own copies ' + 'of the eto catalogs; never edit outside ' + folder.root + ')' : '') +
            '. REPORT PRODUCT: one `catalog` entry per ' +
...
const architectPrompt = (folder, survey, kernelReport, unmapped) =>
    [
        CONTEXT,
        MANDATE,
        READ_FIRST,
...
const implementPrompt = (folder, unit, preSwap, reports, kernelReport, scopes) =>
    [
        CONTEXT,
        MANDATE,
        READ_FIRST,
...
const redteamPrompt = (folder, unit, scopes, nav, crit) =>
    [
        ...
        'PRIOR CLAIMS (UNVERIFIED - a previous pass asserts these edits and verdicts; refutation targets you judge against ' +
            'CURRENT disk, never a settled record): ' +
            JSON.stringify(crit ? { files: crit.files, verdict: crit.verdict, summary: crit.summary } : {}),
        GIT_GROUND,
...
const docsPrompt = (folder, unitReports) =>
    [
        ...
        'TASK: the index docs for ' +
            folder.root +
            ' per libs/.planning/README.md [02] - you are their ONE writer. Read ' +
...
const lawPrompt = (results, backlog) =>
    [
        CONTEXT,
        MANDATE,
        WRITE_FULLY,
        'TASK: SERIALIZED LAW TAIL - you are the ONE writer for central manifests and cross-repo law docs, and the drain for ' +
            ...
            'apply now that no concurrent writer runs): ' +
...
phase('Survey');
const kernelRecon = slot(() => recon(kernelPrompt(), { label: 'recon:kernel', phase: 'Survey', schema: MAP, scope: ['libs/csharp/Rasm'] })); // shared across folders; awaited before each architect

const results = (
    await Promise.all(
        ACTIVE.map(async (folder) => {
            const lanes = folder.census
                .map((lane) => ({ kind: 'census', lane, schema: REGISTER, writes: false, scope: [folder.root + ': ' + lane.paths] }))
                .concat(
                    folder.research.map((lane) => ({
                        kind: 'research',
                        lane,
                        schema: MAP,
                        writes: true,
            ...
            const arch = await slot(() =>
                agent(architectPrompt(folder, survey, kernelReport, unmapped), {
            ...
            const unitReports = await Promise.all(
                units.map(async (unit, i) => {
                    ...
                    const fix = await slot(() =>
                        agent(implementPrompt(folder, unit, arch.preSwap, reports, kernelReport, scopes), {
                    ...
                    const crit = await slot(() =>
                        agent(critiquePrompt(folder, unit, scopes, navOf([fix])), {
                    ...
                    const rt = await slot(() =>
                        agent(redteamPrompt(folder, unit, scopes, navOf([fix, crit]), crit), {
                    ...
                    return {
                        ...
                        verdict: (rt && rt.verdict) || (crit && crit.verdict) || fix.verdict,
...
log('Close: ' + BACKLOG.length + ' deferred backlog row(s), ' + results.flatMap((r) => r.packageDeltas).length + ' package delta(s)');
const law = await slot(() =>
    agent(lawPrompt(results, BACKLOG), {
        label: 'law',
        phase: 'Close',
        model: 'fable',
        effort: 'high',
...
const accept = await slot(() =>
    recon(acceptPrompt(results), { label: 'acceptance', phase: 'Close', schema: REGISTER, scope: results.map((r) => r.root) }),
);
...
return {
    ...
    acceptance: accept.ok ? { report: accept.report, entries: accept.entries, headline: accept.headline } : 'dropped: ' + accept.failure,
};
