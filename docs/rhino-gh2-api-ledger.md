...
| :-----: | ----- | ------------- |
| **[1]** | `IDataAccess.GetPear<T>`, `GetTwig<T>`, `GetTree<T>` | Universal `Bridge.Read<T>` reads item, twig, and tree ports with metadata and null state. |
...
| **[3]** | `Garden.TreeFromList<T>` | Tree outputs preserve metadata and null arrays under the active GH2 data access path prefix. |
...
| [INDEX] | [API] | [GAP] |
| :-----: | ----- | ----- |
| **[1]** | `AreaMassProperties.Compute(IEnumerable<GeometryBase>)` | Current mass queries still dispatch per geometry; future aggregate queries should use native collection overloads. |
| **[2]** | `VolumeMassProperties.Compute(IEnumerable<GeometryBase>)` | Same aggregate gap for volume moments and centroid calculations. |
| **[3]** | `LengthMassProperties.Compute(IEnumerable<Curve>)` | Curve collections should avoid manual fold/sum paths when a native aggregate is available. |
| **[4]** | `AreaMassProperties.Sum`, `VolumeMassProperties.Sum`, weighted variants | Current result combination does not yet expose native mass-property composition. |
| **[5]** | `Box(Plane, GeometryBase)` | Oriented bounds currently use `GeometryBase.GetBoundingBox(Plane, out Box)`; constructor can simplify some direct box creation flows. |
| **[6]** | `SubD.ToBrep(SubDToBrepOptions)` | SubD conversion is present indirectly; explicit options should be promoted when tolerances or extraordinary vertices matter. |
| **[7]** | `SubD.UpdateSurfaceMeshCache` | Mesh-cache refresh is not yet exposed before SubD-heavy topology extraction. |
| **[8]** | `Tree<T>.WithTopology`, `Flatten`, `Graft`, `WithSimplifiedPaths` | `PortData<T>` preserves trees, but component-level topology transforms are not yet first-class. |
| **[9]** | `IDataAccess.GetPears<T>` | Item/twig compatible reads are still separated by access; cross-access control reads can use this API later. |
| **[10]** | `IDataAccess.Verify*` and `Rectify*` methods | Parameter verification remains implicit in GH2; explicit policy-driven rectification is not yet wrapped. |
...
| [INDEX] | [API] | [REASON] |
| :-----: | ----- | -------- |
| **[1]** | Scrutor composition | No non-GH DI boundary exists yet; static component specs remain smaller. |
...
- `libs/csharp/Rasm.Grasshopper` now models ports as item, twig, or tree data with metadata, null state, and optional tree topology.
- GH2 imperative calls remain quarantined in boundary adapters such as `Bridge`, `Param`, and component overrides.
- `Query.Many` now uses `Traverse` while preserving aggregated error semantics from the previous applicative fold.
- Future query slices should prioritize native aggregate mass-property overloads and GH2 tree topology transforms before adding local routing code.
