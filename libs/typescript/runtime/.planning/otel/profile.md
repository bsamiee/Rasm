...
const _PROFILERS = {
  // one row per native sampler: the package's aggregate start covers wall and heap and never reaches cpu, so the roster is the wider surface
  cpu: { start: Pyroscope.startCpuProfiling, stop: Pyroscope.stopCpuProfiling },
        ...
        Effect.zipRight(
          ...
          Effect.isEffect(work)
            ...
            : Effect.sync(() => {
              ...
              // BOUNDARY ADAPTER: the engine callback returns void, so the thunk's value crosses through the closed cell.
