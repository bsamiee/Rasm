...
[INTEGRATION_LAW]:
- The `DataClassification` axis is the SAME taxonomy the `Element/identity#KEY_ENVELOPE` DEK-envelope keys on: a `Personal`/`Credential`/`Secret` `DataClassification` ceiling on the `ElementIdentity` row gates the at-rest envelope demand (the `EnvelopeKeyring` wrap), and the redaction column gates the exporter masking — one classification value drives both the KMS-wrap demand and the redactor selection, so a `Secret`-classified field is both at-rest encrypted and signal-redacted from one annotation.
