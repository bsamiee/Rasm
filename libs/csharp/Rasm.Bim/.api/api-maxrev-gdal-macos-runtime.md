...
dylib closure: `libgdal` 3.13.1, the SWIG `*_wrap` P/Invoke targets the `OSGeo.GDAL`/`OGR`/
`OSR` bindings call, OGR's `libgeos`, OSR's `libproj`, and the full raster/vector format and
...
- version: `3.13.1.534` (GDAL 3.13.1; pinned in lockstep with `MaxRev.Gdal.Core` — the bindings and runtime versions MUST match)
...
- asset: `build/net461/MaxRev.Gdal.MacosRuntime.Minimal.arm64.targets` — a legacy-MSBuild-only copy step (see [04])
...
| :-----: | :-------------------------------- | :------- | :------------------------------------------------------------------------------------------------------- |
|  [01]   | `libgdal.39.3.13.1.dylib` (+ `libgdal.39.dylib` symlink) | geometry | the GDAL 3.13.1 core: every raster driver, the OGR vector driver registry, the GDAL utility algorithms — the library `OSGeo.GDAL.Gdal`/`OSGeo.OGR.Ogr` ultimately drive |
...
|  [04]   | `libkmlbase`/`libkmldom`/`libkmlengine` (1.3.0) + `libxerces-c-3.3` + `libexpat.1` + `libxml2.16` + `libtinyxml2.11` + `liburiparser.1` | geometry | the KML/GML/CityGML/XML-family vector parsers (LibKML + Xerces + expat + libxml2) |
...
- the bundled `build/net461/*.targets` is a LEGACY-only fallback: its `<None Include=".../runtimes/osx-arm64/native/**">` copy step is gated on `MSBuildRuntimeType != 'Core'` (full-framework MSBuild), so it is inert under the SDK. A design that assumes the `.targets` does the copy on a modern build is wrong — the runtime asset graph does.
...
- the rejected forms: a RID-agnostic publish (assets do not stage), a version skew with `MaxRev.Gdal.Core` (ABI fault), and treating the bundled `.targets` as the asset-copy mechanism under the SDK (it is legacy-only).

[RAIL_LAW]:
- Package: `MaxRev.Gdal.MacosRuntime.Minimal.arm64`
- Owns: the osx-arm64 native GDAL 3.13.1 dylib closure backing `MaxRev.Gdal.Core` — `libgdal` + the SWIG `*_wrap` P/Invoke targets + `libgeos`/`libproj` + the raster/vector format and compression dependency tree
