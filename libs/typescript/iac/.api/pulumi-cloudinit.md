...
|  [03]   | `GetConfigResult.rendered`                                   | `string` — assembled user-data body; `.id` is its checksum             |
|  [04]   | `gzip` / `base64Encode`                                      | `boolean` toggles; set explicitly per boundary, never ambient          |
...
|  [02]   | `contentType` | `Input<string>`            | handler: `text/cloud-config` (declarative YAML), `text/x-shellscript` (imperative) |
