...
- ABI floor: a PRE-1.0 contract — the section + reinforcement member set may break across a minor bump. The
 ...
 AND the `SectionFace` + `BarDiameter` enums live in the transitive `VividOrange.ISections` floor, centrally
 pinned. The `IProfile`/`IPerimeter` inputs and the `ILocalPoint2d`/`ILocalPolyline2d` geometry returns come from the
 `VividOrange.IProfiles` / `VividOrange.Profiles.Perimeter` / `VividOrange.Geometry` floor; the `IMaterial`
 grades from `VividOrange.IMaterials` (`api-vividorange-materials.md`). `UnitsNet` is the shared quantity
 floor; `ITaxonomySerializable` is the `VividOrange.ISerialization` floor contract. That floor marker's FQN is the
...
(`api-vividorange-uncertainties.md`); the `TaxonomyJsonSerializer` does NOT serialize the types, so a
...
- rail: profiles / connection
- note: the concrete `VividOrange.Geometry` carriers BEHIND the `ILocalPoint2d` bar `Position` and the `ILocalPolyline2d` layer `GetPath`/`GetRebars` returns — and the section-plane geometry a parametric `IProfile` perimeter (outer + void loops) is assembled from before `new Perimeter(outer, voids)`. The section plane is `VividOrange.Geometry`'s Y-Z: a `LocalPoint2d` is `{ Length Y; Length Z; }`, NOT an X-Y pair. Interior code threads the `ILocalPoint2d`/`ILocalPolyline2d` interface; a Materials parametric-section builder constructs these concrete carriers at the edge, never a `Rhino.Geometry` type.
