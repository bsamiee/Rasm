...
- call: `parse(sql, read=None, dialect=None, **opts) -> list[Expr | None]`; `parse_one(sql, read=None, dialect=None, into=None, **opts) -> Expr`
- call: `maybe_parse(sql_or_expression, *, into=None, dialect=None, prefix=None, copy=False, **opts) -> Expr`
- call: `transpile(sql, read=None, write=None, identity=True, error_level=None, **opts) -> list[str]`; `tokenize(sql, read=None, dialect=None) -> list[Token]`
- call: `find_tables(expression) -> set[Table]`; `Dialect.get_or_raise(dialect) -> Dialect` (classmethod)
- call: builder DSL — `select(*expr, dialect=None, **opts) -> Select`, `from_`, `condition`, `and_`/`or_`/`not_`, `column(col, table=None, db=None, catalog=None, *, quoted=None)`, `func(name, *args, dialect=None)`, `cast(expression, to, dialect=None)`, `case(...)`, `alias`, `table`, `subquery`, `insert`, `delete`, `merge`, `union`/`intersect`/`except_`, `to_column`/`to_table`/`to_identifier`

| [INDEX] | [SURFACE]              | [CAPABILITY]                                                              |
| :-----: | :--------------------- | :------------------------------------------------------------------------ |
|  [01]   | `parse`                | parse multi-statement SQL into a list of trees                            |
|  [02]   | `parse_one`            | parse one statement (optionally `into` a target subtype)                  |
|  [03]   | `maybe_parse`          | accept a string or existing `Expr`, parsing only the string               |
|  [04]   | `transpile`            | rewrite SQL from `read` dialect to `write` dialect                        |
|  [05]   | `tokenize`             | lex without parsing (lexeme-level inspection)                             |
|  [06]   | `find_tables`          | collect non-CTE tables referenced in a query                              |
|  [07]   | `Dialect.get_or_raise` | dialect gate: resolve name/settings/instance or raise                     |
|  [08]   | builder DSL            | construct an `Expr` tree programmatically instead of string concatenation |
...
- call: `Expr.sql(dialect=None, copy=True, **opts) -> str`
- call: `Expr.find(*expression_types, bfs=True) -> E | None`; `Expr.find_all(*expression_types, bfs=True) -> Iterator[E]`
- call: `Expr.walk(bfs=True, prune=None) -> Iterator[Expr]`; `Expr.find_ancestor(*expression_types) -> Expr | None`
- call: `Expr.transform(fn, *args, copy=True) -> Expr`; `node.replace(other)`
- call: `optimizer.optimize(expression, schema=None, db=None, catalog=None, dialect=None, rules=RULES, **kwargs) -> Expr`
- call: `optimizer.qualify(expression, dialect=None, db=None, catalog=None, schema=None, expand_alias_refs=True, expand_stars=True, infer_schema=None, isolate_tables=False, qualify_columns=True, allow_partial_qualification=False, validate_qualify_columns=True, quote_identifiers=True, identify=True, ...) -> Expr`
- call: `optimizer.scope.build_scope(expression) -> Scope | None`; `traverse_scope(expression) -> list[Scope]`; `find_all_in_scope(scope, expression_types)`
- call: `lineage.lineage(column, sql, schema=None, sources=None, dialect=None, trim_selects=True, on_node=None, **kwargs) -> Node`
- call: `diff(source, target, matchings=None, delta_only=False, **kwargs) -> list[Edit]`
- call: `Select.selects -> list[Expr]` (property)

| [INDEX] | [SURFACE]                     | [CAPABILITY]                                                                                     |
| :-----: | :---------------------------- | :----------------------------------------------------------------------------------------------- |
|  [01]   | `Expr.sql`                    | generate the SQL string for this tree under `dialect`                                            |
|  [02]   | `Expr.find` / `find_all`      | first / all nodes matching any given `Expr` subtype                                              |
|  [03]   | `Expr.walk` / `find_ancestor` | visit every node (prunable) / climb to a matching ancestor                                       |
|  [04]   | `Expr.transform` / `replace`  | recursive bottom-up rewrite / swap a node in its parent                                          |
|  [05]   | `optimizer.optimize`          | run the 14-rule pipeline into optimized, schema-qualified form                                   |
|  [06]   | `optimizer.qualify`           | normalize and qualify tables/columns; mandatory pre-step                                         |
|  [07]   | `optimizer.scope.build_scope` | resolve and walk name scopes (source resolution, alias maps)                                     |
|  [08]   | `lineage.lineage`             | column-level lineage tree from a target column to its sources                                    |
|  [09]   | `diff`                        | AST edit set (`Insert`/`Remove`/`Move`/`Update`/`Keep`) between two trees                        |
|  [10]   | `Select.selects`              | the projection list; each entry's `alias_or_name` is the output column the lineage fold iterates |
