...
public static class TensorKernels<T> where T : IFloatingPointIeee754<T> {
    ...
    public static readonly FrozenDictionary<TensorOpFamily, FoldKernel<T>> Fold = new Dictionary<TensorOpFamily, FoldKernel<T>> {
        ...
        [TensorOpFamily.Average] = static x => TensorPrimitives.Sum(x) / T.CreateChecked(x.Length),
        [TensorOpFamily.StdDev] = static x => T.Sqrt(TensorPrimitives.SumOfSquares(x) / T.CreateChecked(x.Length) - (TensorPrimitives.Sum(x) / T.CreateChecked(x.Length)) * (TensorPrimitives.Sum(x) / T.CreateChecked(x.Length))),
    }.ToFrozenDictionary();
...
- [KERNEL_LOWERING]: the matrix and structural rows resolve through `numeric-lane#KERNEL_LOWERING`; the live GEMM and im2col patch-projection and strided-window fold land as numeric-lane rows against the MathNet provider, leaving each matrix and pooling row `Map`-resolving the moment its lowering row binds.
