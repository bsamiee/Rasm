...
- `DrawingContext.Custom()` opens the custom-draw path, lands in `op.Render(ImmediateDrawingContext)`, and reaches Skia by probing `TryGetFeature<ISkiaSharpApiLeaseFeature>()` on that context; `op.Bounds` in global coordinates drives the dirty rect and `op.HitTest` answers picking without recursing to children.
