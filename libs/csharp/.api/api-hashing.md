...
- `Rasm.Bim`: folds vertex buffer, property-set bytes, georeference frame, and clock fact into one snapshot fingerprint through a single `XxHash3` accumulation finalized by `GetCurrentHashAsUInt64`; a SharpGLTF `MemoryAccessor` region or glTF buffer feeds `Append` zero-copy, and an STJ receipt writer feeds `AsStream()` so wire bytes hash as produced.
...
- Every digest crossing a receipt boundary carries algorithm, width, seed, and input domain as fields.
