...
- assembly: `Triangle`
- alias: `Aliases="TriangleNet"` in `Rasm.csproj` — `extern alias TriangleNet;` is REQUIRED at every kernel use site to disambiguate the package-id `Triangle` from the kernel `Triangle` type
...
- frameworks: multi-target `net35` + `net45` + `netstandard1.5` (three `lib/` assets); the `net10.0` consumer binds `lib/netstandard1.5/Triangle.dll` by TFM precedence (the highest netstandard available). The `netstandard1.5` asset pulls the `NETStandard.Library` reference-package meta-dependency (`[1.5.0-rc2-24027, )`) — a build-graph floor only, no runtime closure on net10
...
[MESH_TOPOLOGY]:
- Coordinates are `double` (`Point.X`/`Y`); `Triangle` is a FLOATING-POINT mesher with `RobustPredicates` adaptive-precision orient/incircle — robust to ROUNDING but NOT an exact-rational kernel. For exact-arithmetic robustness (degenerate cocircular input, exact constrained-edge recovery) the consumer routes through the `Rasm` kernel's `Geometry/.planning/Meshing/delaunay.md` `Adaptive.Resolve` four-tier Bowyer-Watson; `Triangle` is the production 2D quality path where the input is well-conditioned.
- `QualityOptions.MinimumAngle` is the Ruppert termination bound: angles above ~20.7 deg are guaranteed to terminate; pushing it higher risks non-termination on tight input angles — bound `SteinerPoints` to cap the insertion budget. `MaximumArea` is the global element-size cap; `VariableArea` + `RegionPointer` give per-region sizing; `UserTest` (`Func<ITriangle,double,bool>`) is the custom refine predicate (return true to split — the FEA gradient/error-driven sizing hook).
...
- `Rasm` kernel (the direct owner): `Triangle` is the 2D quality-refinement path layered beside the authored exact `Tessellation`. Every kernel use writes `extern alias TriangleNet;` — the alias is mandatory (the kernel owns a `Triangle` type). The kernel's well-conditioned 2D constrained-mesh + FEA-grid concerns compose `Triangle`; the exact-robustness and 3D concerns stay on the authored Bowyer-Watson + `Arrangement`.
...
- Accept: the kernel's well-conditioned 2D constrained/quality FEA-section meshing with `extern alias TriangleNet;`; the transitive `VividOrange.InteractionDiagram` section fibre-grid mesh
- Reject: any direct `Triangle` call minted in the `Rasm.Materials` AEC-DOMAIN folder (it consumes the welded `IForceMomentMesh`); a use site that omits `extern alias TriangleNet;` (the package-id `Triangle` collides with the kernel `Triangle` type); using `Triangle` where exact-rational robustness on degenerate input is required (route to the kernel `Adaptive.Resolve` Bowyer-Watson) or where 3D tetrahedralization is needed (`Triangle` is 2D-only — the kernel owns 3D); duplicating the unconstrained point-set Delaunay/ND-Voronoi (`MIConvexHull` owns it) or a standalone clipped point-site Voronoi (`SharpVoronoiLib` owns it)
