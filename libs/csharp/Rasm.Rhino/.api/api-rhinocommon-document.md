...
- `Layer.HasSelectedObjects(bool checkSubObjects) -> bool` / `CommitChanges() -> bool` / `Default() -> void`; `Layer` re-declares the user-string roster `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Transform` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
