...
public static class Coordination {
    ...
    public static ImpactReport Between(ElementGraph graph, ModelDiff before, ModelDiff after, ScheduleNetwork schedule, CostSchedule cost) {
        ...
        var systems = DistributionNetwork.View(graph).Map(s => (View: s, Members: toHashSet(s.Members)));
    ...
    static RuleVerdict Traced(CoordinationRule.Reachable rule, ElementSet applicable) {
        var targets = ElementSet.Query(applicable.Graph, ElementQuery.Of(rule.Target)).Ids;
        var systems = DistributionNetwork.View(applicable.Graph).Map(s => (View: s, Members: toHashSet(s.Members)));
        var (pass, violated) = Ids(applicable).Partition(id => systems
