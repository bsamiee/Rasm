...
- Entry: `PluginTelemetryHost.Open(AssemblyLoadContext alc, ResolvedProfile resolved, Seq<TelemetryContributorPort> contributors, params ReadOnlySpan<KeyValuePair<string, object>> extra)` — builds the minimal per-ALC service provider, the explicit tracer and meter providers, and the unload hook; the resolved row, the contributed roster, and the root's own discriminator rows are the whole input, so the capsule derives its identity delegate and its view projection from their one owner and no caller hand-threads a resource lambda or a view row.
...
- Growth: a new plugin-visible meter or source is one `AddMeter`/`AddSource` row in `Open`; a new resource dimension is one detector row inside `ResourceIdentity.Compose` or one `extra` row a plugin root supplies; a new stream shape is one `Dimensions` edit at the declaring roster both compositions inherit through the same view predicate; `FlushBound` is the one unload-flush policy value.
...
public sealed class PluginTelemetryHost : IDisposable {
    ...
    public static PluginTelemetryHost Open(AssemblyLoadContext alc, ResolvedProfile resolved,
        Seq<TelemetryContributorPort> contributors, params ReadOnlySpan<KeyValuePair<string, object>> extra) {
