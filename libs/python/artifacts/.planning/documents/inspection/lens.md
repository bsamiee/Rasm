...
- Boundary: `DocumentLens` recovers nodes and never authors a PDF — authoring stays at the `documents/document-plan#DOCUMENT` lowering arm, and the `REDACT`/`ANNOTATE` arms edit-in-place an already-emitted PDF, never compose a new document. A `get_text`/`get_words`/`extract_images`/`search`/`get_outline` reader-verb family scattered beside the emit axis is the deleted form; the recovered tree is `documents/model#NODE` `DocumentNode`, never a re-minted text model. OCR over a scanned page with no embedded text is the one `[3]-[RESEARCH]` frontier and never a phantom `LensOp` row. The content key is consumed from runtime, never re-minted; the extracted-tree corpus stays at the runtime columnar lane concept level, never a second store. The async dispatch shape aligns on the verified `documents/document-plan#DOCUMENT` `async_boundary` so emission and recovery share one fault capsule shape.
...
from rasm.artifacts.receipt.artifact_receipt import ArtifactReceipt
...
GATED: frozenset[LensOp] = frozenset(
    {
        LensOp.WORDS,
        LensOp.SEARCH,
        LensOp.EMBEDDED,
...
def _pypdf_recover(op: str, payload: bytes, params: dict[str, object]) -> tuple[DocumentNode, ...]:
    ...
    match LensOp(op):
        case LensOp.EXTRACT_TEXT:
            return tuple(
                _run_node(page.extract_text(extraction_mode=params.get("mode", "plain")), index)
                for index, page in enumerate(reader.pages)
            )
        case LensOp.EXTRACT_IMAGES:
            return tuple(
                _figure_node(image.name, image.data, index)
                for index, page in enumerate(reader.pages)
...
def _gated_recover(op: str, payload: bytes, params: dict[str, object]) -> tuple[DocumentNode, ...]:
    ...
    match LensOp(op):
        case LensOp.WORDS:
            return tuple(
                _run_node(word, index, (x0, y0, x1, y1))
...
def _annotation_node(
    annot: AnnotKind, contents: str, target: tuple[float, float, float, float], page: int
) -> AnnotationNode:
    return AnnotationNode(
        meta=_meta("annotation", page, contents.encode(), bounds=target),
        annot=annot,
        target=target,
        contents=contents,
    )
...
def _outline_nodes(outline: object, /) -> tuple[DocumentNode, ...]:
    return tuple(
        _structure_node(item.title, item.title.encode(), item.page)
        for item in outline
...
def _redact_nodes(document: object, params: dict[str, object]) -> tuple[DocumentNode, ...]:
    ...
    return tuple(
        BlockNode(meta=_meta("block", index, page.get_text().encode()), block=BlockKind.PARAGRAPH)
        for index, page in enumerate(document)
    )
...
def _annotate_nodes(document: object, params: dict[str, object]) -> tuple[DocumentNode, ...]:
    ...
    return tuple(
        _annotation_node(AnnotKind.NOTE, annot.info["content"], tuple(annot.rect), page_index)
...
def _story_nodes(document: object, params: dict[str, object]) -> tuple[DocumentNode, ...]:
    ...
    writer.close()
    return tuple(
        PageNode(meta=_meta("page", index, str(index).encode()), media_box=tuple(rect))
