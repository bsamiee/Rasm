# [webdav] — the DAV remote-origin row: Nextcloud/SharePoint/Apache lanes, ranged streams, RFC 4918 locks
...
- `ssh2` / `basic-ftp` (`libs/typescript/.api/ssh2.md`, `.api/basic-ftp.md`): sibling remote-origin rows on the one origin-addressed `object/file` surface — capability flags dispatch: DAV carries server-side copy/move and locks but no exec; a flag the origin lacks degrades by row (no server-side move ⇒ copy + delete), never by fork.
