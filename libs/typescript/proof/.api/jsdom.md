...
[STACK: `jsdom.serialize()` + `corpus/golden`] — `serialize()` returns the exact WHATWG HTML fragment serialization, and `nodeLocation()` (under `includeNodeLocations`) returns `parse5` source offsets. A spec asserting a rendered/serialized document against a frozen `corpus/golden` byte fixture uses these two as the reproducer — jsdom's parser is the standards oracle, the frozen bytes are the expectation.

[STACK: `jsdom` + `fast-check`] — a property generating HTML fragments runs each case through `JSDOM.fragment()` (the cheapest parse — no `Window`) and asserts a structural invariant; `Schema`-derived arbitraries from `law/arbitrary.ts` feed the predicate. Full-`Window` specs use `new JSDOM()` per case only when script execution is under test.
