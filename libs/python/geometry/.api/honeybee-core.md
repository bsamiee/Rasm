...
- version: `1.64.55`; floor `honeybee-core; python_version<'3.15'`
- license: AGPL-3.0 (the Ladybug Tools copyleft; AGPL obligations attach to any networked redistribution)
- abi: pure-Python `py3-none-any` wheel, no compiled payload and no metadata `Requires-Python` cap
- marker: COMPANION-GATED. The manifest pins `python_version<'3.15'` because the Honeybee/Ladybug set is co-released and floor-pinned together (the `honeybee-openstudio` binary `openstudio` SDK caps the set), so honeybee-core is gated OUT of the active 3.15 interpreter and `assay api resolve honeybee-core` returns `unsupported` (nearest candidate is the unrelated C# `HoneybeeSchema` NuGet). This catalog is verified against the real `1.64.55` wheel reflected on a sibling cp312 interpreter; the gate is manifest convenience (no ABI cap), so it lifts to `python_version` floor removal once the set is validated on 3.15.
- depends-on: `ladybug-core` (units/location/EPW), `ladybug-geometry` (the geometry primitive tier honeybee objects wrap), `pydantic-openapi-helper`; the HBJSON validation tier `honeybee-schema` (pydantic v2) is the wire-contract sibling
...
|  [06]   | `Autocalculate` / `NoLimit` / `Unassigned`               | alt-number sentinel | sentinel numerics replacing magic `None`/autosize fields |
...
|  [02]   | `Model.from_dict(data, cleanup_irrational=False)`                     | HBJSON dict              | reconstruct a live `Model` (geometry + re-attached extension props) |
...
|  [04]   | `Model.from_hbjson(hbjson_file, cleanup_irrational=False)`            | file path                | read an `.hbjson` file                                   |
...
|  [06]   | `Room.intersect_adjacency` / `Room.group_by_adjacency` / `group_by_story` / `group_by_orientation` | room list                | split coincident faces; group rooms by topology/attribute |
|  [07]   | `Room.automatically_zone` / `Room.stories_by_floor_height`     | rooms                    | derive thermal zones / story assignment            |
...
| [INDEX] | [SURFACE]                                                       | [CALL_SHAPE]             | [CAPABILITY]                                       |
| :-----: | :------------------------------------------------------------- | :----------------------- | :------------------------------------------------- |
|  [01]   | `Model.check_all(raise_exception=True, detailed=False, all_ext_checks=False)` | flags                    | run every geometric-validity check (solid, planar, self-intersecting, adjacency, sub-face validity, duplicate ids) |
...
[CAPTURE_GAP]:
- extension serialization is selective, not all-or-nothing (verified against `honeybee-core 1.64.55`): `Model.to_dict(included_prop=None, ...)` and `ModelProperties.to_dict(include=None)` take an extension-KEY list, not a boolean — `included_prop=['energy']` serializes geometry plus the energy extension and drops radiance, while `included_prop=None` serializes EVERY registered extension. The owner picks the extension key set per export rather than post-filtering a full dict. `Model.from_dict(data)` reconstructs geometry and then runs `properties.apply_properties_from_dict(data)` internally, so a round-trip is symmetric ONLY when the inbound dict carries the extension props the outbound `included_prop` selected; a geometry-only export re-imports with default (empty) extension props.
- `check_all` has two distinct return contracts: with `detailed=False` (default) it returns a single concatenated error string and `raise_exception=True` raises `ValueError` on the first defect; with `detailed=True` it returns a `list[dict]` of error objects (each with `type`, `code`, `error_type`, `element_id`, `message`) and does NOT raise — this is the only shape that folds into a `Result`/validation receipt without an exception trampoline. `Model.validate(json_output=True)` wraps the same machinery into a JSON `ValidationReport` string for CLI/handoff. `all_ext_checks=True` additionally runs every registered extension's `check_all` (energy/radiance), so a full-stack validation is one call, not one per extension.
- `dict_to_object(honeybee_dict, raise_exception=True)` dispatches on the dict `type` value (`'Model'`/`'Room'`/`'Face'`/`'Aperture'`/`'Door'`/`'Shade'`/`'ShadeMesh'`); an unrecognized `type` returns `None` when `raise_exception=False` rather than raising, so the boundary decoder checks for `None` before promoting. This is the geometry owner's single honeybee-object decode entry — there is no per-type `from_dict` dispatch to write.
