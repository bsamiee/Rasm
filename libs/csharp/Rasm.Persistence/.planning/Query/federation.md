...
- [04]-[FLIGHT_RESULT_PLANE]: the Arrow Flight return wire — the `ReplayKey`-ticketed `FederationFlight` producer whose `GetFlightInfo` admits-and-executes a command-descriptor plan and whose `DoGet` streams the held result's record batches zero-copy to a cross-runtime consumer, and the host binding contract that subclass satisfies.
