...
Teams are gated behind `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`: one lead session plus full peer sessions with independent context windows, a shared task list, and direct peer messaging — mesh where subagents are hub-and-spoke. Spawning the first teammate forms the team; cleanup rides session exit.
