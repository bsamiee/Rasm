...
[TOPOLOGY]:
- native-first: `Otlp.layer` is the default export path — Effect's `Tracer`/`Metric`/`Logger` serialize straight to the OTLP endpoint over `HttpClient`; `NodeSdk`/`WebSdk` recover only SDK-only exporters (OTLP-gRPC, vendor exporters, batch processors).
...
[PIN_BLOCK_PARITY]: criteria the block carries — each names the seat the pin withholds and what the repo loses until it opens; every criterion clears before the block retires
...
- Law: criteria state capability facts re-proved on the installed lane, never preferences — the browser exporter build declares neither `compression` nor `keepAlive` while the node build declares both, so criterion [02] is a SENDER column rather than a lane-wide gap.
