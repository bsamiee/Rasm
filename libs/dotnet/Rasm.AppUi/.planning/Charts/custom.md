...
- Boundary:
  ...
  - A GROUPING, ORDERING, or RANGE leaves the result carrier for a bare enumerable, so every one of them re-enters through `toSeq` before a carrier read consumes it: the band walk, the declutter fold, the squarify order, the trip prefix, the timeline merge, and the stroke-step index each carry that re-entry. The same boundary decides every reduction: an unseeded extreme or total over the carrier is ambiguous between the two surfaces and reaches neither, so each folds from its own identity — zero for a share, the infinities for a column bound, the clock bounds for a window — which also makes the fold total over the empty run the emptiness gate beside it was standing in for.
  ...
  - The squarify packing is a BOUNDED FIXPOINT over its own node count: the walk settles inside two steps per node plus the closing lay, so the ceiling carries it, and past settlement the step is the identity. The recursive spelling was tail-shaped against a runtime that guarantees no tail call and a large treemap exhausted the stack outside every guard this page declares. A ceiling reached with cells still unplaced REFUSES as `ChartFault.VisualDegenerate` — a success-shaped fall-through certifies a truncated packing as a complete one.
...
public sealed record CustomVisualStyle(
         ...
         Slot(labels.Ceiling > 0, $"{kind.Key}: label ceiling must be positive"))
            ...
            .Bind(_ =>
                ...
                select new CustomVisualStyle(
                    ...
                    Bound(TextStyleRow.Resolve(labelRole, chain), chain, label, ink)));
    ...
    static LabelChannel Bound(TextStyleRow style, FontChain chain, LabelPlan label, SKColorF ink) {
        Fin<ShapedText> Shape(string text) =>
            ShapingSurface.Shape(text, style, label.Spec,
                FaceRequest.Of(style, chain, label.Palette, Seq(label.Spec.Language.Name)),
                label.Cabinet, label.Posture, label.Cache);
        return new LabelChannel(
            ...
            Draw: (canvas, text, at) => Shape(text).Bind(shaped => {
                label.Ink.SetColor(ink, SKColorSpace.CreateSrgb());
                return ShapingSurface.DrawLabel(canvas, shaped, label.Ink, at.X, at.Y);
            }));
    }
}
...
public sealed partial class CustomVisual {
    ...
    public IO<Fin<(VisualArtifact Artifact, VisualRecord Record)>> Materialize(
        ...
        from artifact in image.Match(
            Succ: shot => IO.pure(shot.Owned).Bracket(
                ...
                .IfFail(error => (fun(shot.Record.Dispose)(), Fin.Fail<(VisualArtifact, VisualRecord)>(error)).Item2),
            Fail: error => IO.pure(Fin.Fail<(VisualArtifact, VisualRecord)>(error)))
        select artifact;
...
flowchart LR
    ...
    accDescr: A closed visual payload packs once into per-element strokes and one sealed record that the live surface and the render twin both replay into one artifact.
...
- Boundary:
  ...
  - `PlanMap` is the ONE mapper and it is READER-FREE: every `PlanTask` column comes from exactly one `ConstructionTask` member or one `PlanCell` member, so `RMG020` keeps source-side force and the ignore roster is the planner's own solve inputs a render never reads. `EnabledConversions` excludes `ExplicitCast` because LanguageExt carriers cross this mapper and the default binds `Option<T>`'s throwing cast in preference to a registered converter. A hand positional projection beside it is the deleted form.
