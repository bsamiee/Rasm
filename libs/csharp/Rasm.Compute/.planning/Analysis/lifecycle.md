...
- Entry: `public Task<Fin<StatisticsDto>> Statistics(string omf, LciaMethod method)` · `public Task<Fin<Seq<Epd>>> Search(string omf, int page)` · `public Task<Fin<Epd>> ByUuid(string uuid)` — every read GET-only (the POST/PUT/PATCH publisher verbs are absent from the typed surface, so a write cannot be emitted with a READ-scope token), each returning `Fin<T>` where a non-2xx mints `ComputeFault.AnalysisFailed(Admission, Timeout)` with the HTTP status riding the typed `Code` slot (the seam `Diagnostic`'s own `Option<int>`, never smuggled through the message — a transport miss is transient by the seam's own `Timeout` column, so the bounded `Transient` retry gate can clear it) and a decode failure the deterministic `AnalysisFailed(Extraction, Foreign)` (2219) on the band rather than throwing into domain flow.
...
public sealed class Ec3Service(HttpClient http, HybridCache cache, JsonSerializerContext json) {
    ...
    // transient 429/5xx never poisons a content-key; the cached value is the source-gen DTO. Exemption: the HybridCache +
    // HTTP boundary — the throw converts back onto the Fin rail exactly once at this seam.
            ...
            return Fin.Succ(await cache.GetOrCreateAsync(slot, (http, json, path, decode),
                static async (state, ct) => {
                    ...
                    return !response.IsSuccessStatusCode
                        ? throw new Ec3Boundary(new ComputeFault.AnalysisFailed(SolvePhase.Admission, FailureKind.Timeout, $"<ec3:{state.path}>", Some((int)response.StatusCode)))
...
- Entry: `public static Fin<AssessmentResult> RunCarbon(ElementGraph graph, AssessmentRequest.Carbon request, AssessmentSink sink, ClockPolicy clocks)` folds `AssemblyAggregator.AggregateEnvironmental` over each target's seam `MaterialComposition` and baked `TakeoffOf` geometry, emitting the `whole-life-gwp`/`embodied-carbon-intensity`/per-stage-`A1-A3`…`D`/`recycled-content` facts, the governing ratio the whole-life carbon against the policy target; `public static Task<Fin<GraphDelta>> EnrichCarbon(ElementGraph graph, Ec3Service ec3, AssessmentRequest.Carbon request, ClockPolicy clocks, Op key)` resolves each undeclared ply through the EC3 fallback ladder and returns the enriching delta the composition root applies before `RunCarbon`.
...
public static partial class LifecycleAssessment {
    ...
    public static Fin<AssessmentResult> RunCarbon(ElementGraph graph, AssessmentRequest.Carbon request, AssessmentSink sink, ClockPolicy clocks) =>
    ...
    // The async EC3 ingress — resolves each undeclared ply through the product-EPD -> category-statistic fallback ladder,
    ...
    // the same DeclaredQuantity owner the cost fold uses), then embed the carbon GwpTotal row into the full seam
    ...
    // read here — a per-kg basis resolves to mass at aggregation (volume × the ply Mechanical.Density). None only when no
    // dimension resolves (a bare-count declaration with no kg_per_declared_unit); no unit keeps the PerM3 default.
    ...
    // The carbon-only per-stage GwpTotal row embeds into the full seam matrix through CarbonMatrix (the write dual of
...
// the baked Qto_*BaseQuantities, net preferred over gross, scanned across the Qto set so a wall/slab/beam reads without a
...
public static class LifecycleGraphReads {
    ...
    extension(ElementGraph graph) {
        public Fin<ElementQuantity> TakeoffOf(NodeId element) {
            ...
            return volume.IsNone && area.IsNone
                ...
                // The Fabrication NestYield.WasteAreaMm2 seam quantity — when the graph carries a nest-yield
    ...
    // The Fabrication nest-yield decode: the seam-baked waste-area quantity (NestYield.WasteAreaMm2 lowered
...
// --- [OPERATIONS] --------------------------------------------------------------------------
public static partial class LifecycleAssessment {
    public static Fin<AssessmentResult> RunCost(ElementGraph graph, AssessmentRequest.Cost request, AssessmentSink sink, ClockPolicy clocks) =>
        request.Targets.Fold(
