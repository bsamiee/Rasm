...
[TABLE_MUTATION]: `f.Objects.Add(geometry, attributes=None)` `AddObject(obj)` `AddPoint(...)` `AddLine(...)` `AddPolyline(...)` `AddArc(arc, attrs=None)` `AddCircle(...)` `AddSphere(...)` `AddCurve(...)` `AddSurface(...)` `AddBrep(...)` `AddExtrusion(...)` `AddMesh(...)` `AddPointCloud(...)` `AddHatch(...)` `AddTextDot(text, location, attrs=None)` `AddInstanceObject(...)` `Delete(...)`; sibling tables carry `Add(component)` and iteration
[TABLE_LOOKUP]: `f.Objects.FindId(guid) -> File3dmObject | None` `f.Layers.FindId(guid)` `f.Layers.FindName(name, parentId) -> Layer | None` `f.Layers.FindIndex(index) -> Layer` `f.InstanceDefinitions.FindId(guid)` `f.InstanceDefinitions.FindIndex(index) -> InstanceDefinition`
[ATTRIBUTES]: `attrs.Id` `attrs.Name` `attrs.LayerIndex` `attrs.IsInstanceDefinitionObject` `attrs.Visible` `attrs.Mode` `attrs.ObjectColor`/`ColorSource` `attrs.MaterialIndex`/`MaterialSource` `attrs.LinetypeIndex`/`LinetypeSource` `attrs.PlotColor`/`PlotWeight` `attrs.GroupCount`/`AddToGroup(index)`/`GetGroupList()` `attrs.GetUserString(key)`/`SetUserString(key, value)`/`GetUserStrings()` `attrs.Transform(t) -> bool`
[LAYERS]: `Layer.Index` `Layer.FullPath` `Layer.Name` `Layer.Id` `Layer.ParentLayerId` `Layer.Color` `Layer.Visible`/`Locked` `Layer.LinetypeIndex`/`RenderMaterialIndex` `f.Layers.Add(layer) -> int` `f.Layers.AddLayer(name, colorTuple) -> int`
[INSTANCES]: `InstanceReference(idefId, xform)` `ir.Xform -> Transform` `ir.ParentIdefId -> UUID` `f.Objects.AddInstanceObject(instanceReference, attributes=None)` `f.InstanceDefinitions.Add(name, description, url, urlTag, basePoint, geometryTuple, attributesTuple) -> int` `idef.Id`/`Name`/`GetObjectIds()`/`IsInstanceGeometryId(guid)`
...
- One table carries TWO absence regimes: `FindId(guid)` and `FindName(name, parentId)` answer `None` on a miss while `FindIndex(index)` raises `IndexError` for every out-of-range ordinal, negative included. A `None` check over a `FindIndex` result is unreachable and the escaping `IndexError` is the real absence signal, so an ordinal lookup admits at the boundary through a caught `IndexError` and a GUID or name lookup through an option-shaped `None` test.
- `GeometryBase.Transform(xform)`, `Translate`, `Rotate`, and `Scale` MUTATE the receiver and answer `bool`, never a new geometry, so a placement that must retain its preimage calls `geom.Duplicate()` first and moves the copy; the `bool` reports whether the kernel applied the matrix, not whether the result is valid.
- `Transform.Multiply(a, b)` composes as `a` AFTER `b` — the right operand hits the point first — so a place-then-orient chain reads right to left and reversing the pair yields a different placement without failing.
- `ObjectAttributes.Id` is the all-zero GUID until the object enters `f.Objects`, after which it equals the GUID the `Add*` call returned; `LayerIndex` is the `Layer.Index` ordinal, and `Layer.FullPath` is the `::`-delimited ancestry a display name never carries. `Layer.PathSeparator` is exposed as a static the binding cannot call from either the class or an instance, so the delimiter reads off `FullPath` and never off that member.
- Instance-definition member geometry LIVES in `f.Objects` beside model objects, flagged `attrs.IsInstanceDefinitionObject == True`, so any sweep of the object table filters on that flag or counts block content as placed model geometry. A placement is an `InstanceReference` whose `ParentIdefId` resolves through `f.InstanceDefinitions` and whose `Xform` is the placement matrix — the members themselves stay at definition coordinates.
