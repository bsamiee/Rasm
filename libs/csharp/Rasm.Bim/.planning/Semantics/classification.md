...
// dictionary-certified authoring lowering, never a second admission.
        ...
        string.IsNullOrWhiteSpace(response.Code) || string.IsNullOrWhiteSpace(response.Uri)
            ...
            : from status in StatusOf(response.Status, key)
              ...
              select new BsddClass(
                  ...
                  toSeq((response.Hierarchy ?? []).OrderBy(static item => item.Level)).Map(RefOf),
...
public sealed record BsddFederation(Map<string, Seq<string>> Equivalence, Map<string, string> Names) {
    ...
    public Seq<string> EquivalentSet(string classUri) =>
        toSeq(Equivalence.Find(classUri).IfNone(Seq<string>()).Add(classUri).Distinct().OrderBy(identity));
...
public static class BsddResolution {
    ...
    // The graph-scale ENRICHMENT fold: every "ifc"-classified occurrence carrying NO classification in a target
    // system resolves ranked candidate codes ONCE per distinct (entity class, predefined token, name) key — the
    // bSDD IFC-entity alignment widened from the single authoring lookup into the bulk suggestion pass an
    // authoring surface confirms row by row through Certify, so the package produces classifications instead of
    // only validating them. Server relevance IS the rank (the search wire's own order), the roster the admission
    // (HitOf drops an unrostered dictionary), and the transport failure faults by the Search law — enrichment is
    // authoring-time, never the ingest path.
    public static Fin<Seq<ClassificationSuggestion>> Suggest(ElementGraph graph, Seq<ClassificationSystem> targets, BsddPort port, Op key) =>
        toSeq(graph.ObjectNodes
            .Filter(o => o.Classification.System == "ifc"
                && o.Kind == ObjectKind.Occurrence
                && targets.ForAll(target => !o.Classifications.Exists(c => string.Equals(c.System, target.Key, StringComparison.OrdinalIgnoreCase))))
            .GroupBy(static o => (Code: o.Classification.Code, Token: o.PredefinedType.Token,
                                  Text: o.Name is { Length: > 0 } name ? name : o.Classification.Code)))
            .TraverseM(group => Search(group.Key.Text, targets, Some(group.Key.Code), port, key)
                .Map(hits => toSeq(group).Bind(o => targets.Map(target =>
                    new ClassificationSuggestion(o.Id, target, hits.Filter(hit => hit.System == target))))))
            .As()
