...
public sealed record BrickAssembly(
    ...
    private static Fin<Unit> Validate(BrickRun run, double lengthMm) =>
        Positive(value: run.HeightMm, name: nameof(run.HeightMm))
            ...
            .Bind(_ => run.Bond.Kind == BondKind.Template
                ? Fin.Succ(value: unit)
                : Fin.Fail<Unit>(error: new BrickFault.UnsupportedLayout(Detail: $"Bond '{run.Bond.Key}' is generated; layout requires a template bond.")))
            .Bind(_ => run.Bond.Fits(brick: run.Unit.Unit)
                ? Fin.Succ(value: unit)
                : Fin.Fail<Unit>(error: new BrickFault.IncompatibleBond(BondKey: run.Bond.Key, UnitKey: run.Unit.Key)));
    ...
    private static Fin<double> Positive(double value, string name) =>
        double.IsFinite(value) && value > 0.0
            ...
            : Fin.Fail<double>(error: new BrickFault.InvalidLayout(Detail: $"{name} must be positive and finite (got {value:R})."));
    ...
    private static Fin<Seq<BrickPlacement>> Course(BrickRun run, CourseTemplate template, int course, double lengthMm, double headJointMm) =>
        template.Sequence.Count > 0
            ? Fin.Succ(value: Repeat(run: run, template: template, course: course, lengthMm: lengthMm, headJointMm: headJointMm))
            : Fin.Fail<Seq<BrickPlacement>>(error: new BrickFault.InvalidLayout(Detail: $"Bond '{run.Bond.Key}' course {course} has no orientation sequence."));
