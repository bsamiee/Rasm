...
class Access(Struct, frozen=True):
    ...
    def _key(self) -> ContentKey:
        # key-over-INPUT: canonical (op ⊕ source pdf ⊕ params) minted PRE-RUN so keyed admission probes
        # the warm seed BEFORE the object-model fold runs — never a key over the authored bytes.
    ...
    async def _authored(self) -> Self:
        # the sync pikepdf + pdf_oxide (Rust) object-model folds are GIL-releasing native — they cross the
        # runtime thread lane, never a folder-minted limiter; the stepped owner is the `ReceiptContributor`.
