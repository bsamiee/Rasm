...
[STACKING]:
- `npgsql`(`libs/dotnet/.api/api-npgsql.md`): the `jsonb` resolver result rides raw `Npgsql` through `FromSql`/`SqlQuery`, the query document and `variables` bound as parameters — never an EF-translated member, never a runtime-concatenated GraphQL string.
