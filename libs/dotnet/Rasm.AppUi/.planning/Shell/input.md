...
- [07]-[DEVICE_DRIVERS]: The four admitted SDK boundary capsules, their open and arm legs, and the device identities.

## [02]-[HOTKEY_DERIVATION]

- Owner: `GesturePolicy` — the per-surface chord, scope, and return-key policy record carrying the binding fold; `ReturnPosture` — the two-row policy value naming where the return key lands.
...
- Owner: `BehaviorRow` — the admitted trigger and action roster carrying each row's catalogued type, its bound knob, and the motion token its interval resolves from; `BehaviorBridge` — the static intent-binding surface over those rows.
...
- Boundary: the roster IS the admission table — a markdown mirror of it beside the rows was a second authority for the knob and timing columns nothing could read, and `Interval` is the arm that makes the timing column load-bearing rather than declared. The roster excludes `FileSystemWatcherTrigger`, `NetworkInformationTrigger`, `HttpRequestAction`, and `WriteTextToFileAction`; asset reload, connectivity, outbound requests, and export enter through their owning planes. `TimerTrigger` carries surface-local micro-cadence only, while throttle and debounce intervals resolve from `Theme/motion#MOTION_AXIS` through the row's own token at composition. `EventTriggerBehavior` and the catalogued routed-event trigger family own event admission, and `RoutedEventTriggerBehavior` (`Avalonia.Xaml.Interactions.Custom`) carries the `RoutedEvent`, `RoutingStrategies`, and `SourceInteractive` overrides a named-event trigger cannot express. The bridge binds the COMMAND alone — `Intent` seats `Command` and pins `PassEventArgsToCommand` false, and the package's own `CommandParameter` and `InputConverter` columns stay unbound, so a per-row or per-field verb resolves its OWN materialized `ReactiveCommand` rather than sharing one command through a parameter an untyped `object` would carry; a bridge-supplied parameter column is therefore unspellable by construction and every consumer that needed one — the form chrome's per-field operations first among them — resolves a command per subject instead. Compiled XAML binding and `BehaviorBridge.Intent` are the complete view-binding surface; no ceremonial method names rejected ReactiveUI property binders.
...
- Outcome: admitted payloads raise their command intents and return the command deck's `DeckOutcome`.
...
- Entry: `public static Fin<DeviceSession> Open(DeviceDriver driver, DriverRuntime runtime)` — the source leg: one arm per SDK folding enumerate → open → decode → teardown, returning the resolved identity, the fabric projection the `InputDevice` arm reads, and the normalized sample stream; `public static Fin<DeviceSink> Arm(DeviceDriver driver, DriverRuntime runtime)` — the actuator leg over the same four cases, returning its resolved identity, the `DeviceOutput` sink the drive fold consumes, and the scoped teardown, and sealing `BindingRejected` for a backend that carries no actuator; `public static IO<DeviceSession> Reopen(DeviceDriver driver, DriverRuntime runtime)` and `public static IO<DeviceSink> Rearm(DeviceDriver driver, DriverRuntime runtime)` — the same two legs on the re-drive path, admitting the family's transient cases alone.
...
- Identity: `DeviceIdentity` carries the driver class, device id, and channel count once per resolving leg through `DriverRuntime.Evidence`; `DeviceSession` and `DeviceSink` retain that same value. The source leg's `Admit` and actuator leg's `Bound` evidence folds own distinct resolved and absent instrument pairs.
...
public sealed record DeviceIdentity(DeviceClass Driver, string Id, int Axes);
...
public sealed class DeviceSink(DeviceIdentity identity, DeviceOutput output, IDisposable teardown) : IDisposable {
    private readonly CompositeDisposable held = new(teardown);
...
public sealed record DriverRuntime(
    ...
    Func<DeviceIdentity, Unit> Evidence,
    Action<Error> Fault,
    IScheduler Sampler,
...
public static class InputDrivers {
    ...
    private static Fin<DeviceSession> Admit<TSource>(
        ...
        Func<TSource, DeviceIdentity> coordinates,
        Func<TSource, DeviceIdentity, DeviceSession> session) =>
        found.Match(
            Some: source => coordinates(source) switch {
                var identity => Fin.Succ(Minted(runtime, Resolved, kind, identity, session(source, identity))),
            },
    ...
    private static Fin<DeviceSink> Bound<TSource>(
        ...
        found.Match(
            Some: source => bind(source).Map(armed => Minted(runtime, Armed, kind, armed.Identity, armed)),
    ...
    private static T Minted<T>(DriverRuntime runtime, InstrumentSpec row, DeviceClass kind, DeviceIdentity identity, T answer) {
        ignore(runtime.Evidence(identity));
    ...
    private static Fin<DeviceSession> HidOpen(DeviceDriver.Hid source, DriverRuntime runtime, DeviceClass kind) =>
        Admit(runtime, kind, $"{kind.Key}:{source.VendorId:x4}:{source.ProductId:x4}",
            ...
            _ => new DeviceIdentity(kind, $"{source.VendorId:x4}:{source.ProductId:x4}", source.Channels.Count),
            (_, identity) => new DeviceSession(
                identity,
                new InputDevice.Verbed(identity.Id, kind, source.ToIntents),
                Generations(
    ...
    private static Fin<DeviceSession> GamepadOpen(DeviceDriver.Gamepad source, DriverRuntime runtime, DeviceClass kind) =>
        Trapped(source.Context, kind)
            .Bind(context => Admit(runtime, kind, $"{kind.Key}:{source.Index}",
                    ...
                    pad => new DeviceIdentity(kind, $"{kind.Key}:{pad.Index}", (pad.Thumbsticks.Count * 2) + pad.Triggers.Count + pad.Buttons.Count),
                    (pad, identity) => new DeviceSession(
                        identity,
                        new InputDevice.Verbed(identity.Id, kind, source.ToIntents),
    ...
    private static DeviceSink Motorised(
        ...
        new(new DeviceIdentity(kind, $"{kind.Key}:{pad.Index}", pad.VibrationMotors.Count),
    ...
    private static unsafe Fin<DeviceSession> HapticOpen(DeviceDriver.Haptic source, DriverRuntime runtime, DeviceClass kind) =>
        Stick(source).Bind(found => Admit(runtime, kind, $"{kind.Key}:{source.Index}",
            found,
            held => new DeviceIdentity(kind, $"{kind.Key}:{source.Index}", source.Api.JoystickNumAxes((Joystick*)held)),
            (held, identity) => new DeviceSession(
                identity,
                new InputDevice.Verbed(identity.Id, kind, source.ToIntents),
                Polled(source.Cadence, runtime.Sampler, () => Deflections(source, held, identity.Axes, runtime, kind)),
    ...
    private static unsafe DeviceSink Rumbling(
        ...
        new(new DeviceIdentity(kind, $"{kind.Key}:{source.Index}", 1),
    ...
    private static Fin<DeviceSession> MidiOpen(DeviceDriver.Midi source, DriverRuntime runtime, DeviceClass kind) =>
        Named(() => Melanchall.DryWetMidi.Multimedia.InputDevice.GetByName(source.DeviceName)).Bind(found =>
            Admit(runtime, kind, $"{kind.Key}:{source.DeviceName}",
                found,
                _ => new DeviceIdentity(kind, source.DeviceName, source.Controls.Count + source.Notes.Count),
                (_, identity) => new DeviceSession(
                    identity,
                    new InputDevice.MidiSurface(identity.Id, source.ToParameters),
                    Generations(
                    Observable.Merge(
    ...
    private static DeviceSink Echoing(
        ...
        source.Feedback.ToFrozenDictionary(
            ...
            var echo => new DeviceSink(
                new DeviceIdentity(kind, source.FeedbackName, echo.Count),
