...
- rail: object/store
- `Upload extends EventEmitter`, `constructor(options: Options)`; `params` spreads into every leg — `Bucket`/`Key`/`Body`/`IfNoneMatch`/`ChecksumAlgorithm`/`ContentType`/`Metadata` ride it.
...
| :-----: | :------------------------------------------------------------- | :------------- | :--------------------------------------------------- |
|  [01]   | `new Upload({ client, params, partSize, queueSize })`          | construct      | the streaming conditional put; 412 either leg noops  |
|  [02]   | `upload.done(): Promise<CompleteMultipartUploadCommandOutput>` | run            | one `Effect.tryPromise`; 412 → `written: false`      |
|  [03]   | `upload.abort(): Promise<void>` / `Options.abortController`    | teardown       | bridges fiber interruption to `AbortMultipartUpload` |
|  [04]   | `upload.on("httpUploadProgress", (progress) => ...)`           | progress       | transfer evidence onto telemetry, never domain state |
