...
[STACK]:
- The direct sparse solve stacks under a LanguageExt `Fin` rail exactly as the dense lane does (`Geometry/Numerics/matrix` `MatrixKernel` — the one MathNet/CSparse access path): `Try.lift(() => SparseCholesky.Create(A, MinimumDegreeAtPlusA).Solve(b, x)).Run()` maps a singular/indefinite factorization to a typed `GeometryFault.SingularSystem` row over `Fin<T>`, never a thrown exception in the receipt path; the in-place `Solve` writes the caller's `x` buffer so no result store is allocated per iterate
