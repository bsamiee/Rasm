...
The professional domain map of `geometry` — the host-free geometry and IFC/BIM companion and load-bearing cross-boundary owner. The `mesh` IfcOpenShell GLB tessellation daemon plus the `scan`, `ifc`, and `graph` domains.
...
├── mesh/                     # IfcOpenShell GLB tessellation daemon (the load-bearing cross-boundary owner), CAD-STEP hop, and robust mesh algebra
...
```text seams
*              →  csharp:Rasm.Compute          # HandoffAxis geometry case IDS/clash/BCF (graduation)
mesh           ⇄  csharp:Rasm.Bim/Exchange     # GLB tessellation rail / TessellationRequest (tessellation)
mesh/daemon    ⇄  csharp:Rasm.Compute/Runtime  # ComputeService/ArtifactSync gRPC GLB tessellation (wire)
mesh/daemon    →  csharp:Rasm.Compute/Runtime  # ServerHost ComputeService/ArtifactSync GLB + semantic header (transport)
ifc            ←  csharp:Rasm.Bim/Exchange     # BimWire model vocabulary IFC ingest (projection)
mesh/daemon    →  csharp:Rasm.AppUi/Render     # SharpGLTF GLB import per-element tessellation (shape)
mesh/daemon    ←  csharp:Rasm.Bim/Model        # IFC GLB tessellation reference for deviation (shape)
ifc            →  csharp:Rasm.Bim/Review       # IDS validation evidence via ifctester (boundary)
mesh/cad       →  python:runtime/evidence      # ContentIdentity.of keyed GLB bytes (content-key)
mesh/daemon    →  python:runtime/evidence      # ContentIdentity.of keyed GLB bytes with policy seed (content-key)
graph/algebra  ⇄  python:compute/graduation    # HandoffAxis geometry case (graduation)
ifc/analysis   →  python:compute/graduation    # geometry HandoffAxis case IDS/clash/BCF (graduation)
mesh/daemon    ⇄  python:runtime/transport     # ComputeService/ArtifactSync gRPC GLB tessellation (wire)
```
