...
from rasm.data.tabular.columnar import _PREDICATE_NODES, QueryReceipt
...
class SqlGate(Struct, frozen=True):
    ...
    def edges(self, text: str) -> tuple[LineageEdge, ...]:
        outputs = tuple(sel.alias_or_name for sel in self._qualified(text, self.write).selects)
        roots = (sqlglot_lineage(name, text, schema=self.schema, dialect=self.write) for name in outputs)
...
class QueryEngine(Struct, frozen=True):
    ...
    @beartype(conf=FAULT_CONF)
    async def run(self, spec: QuerySpec, cluster: str | None = None) -> "RuntimeRail[pa.Table]":  # noqa: PLR0911
        match spec:
            case QuerySpec(tag="sql", sql=(text, gate)):
                return await self._local("sql", lambda: self._bound().sql(gate.transpile(text) if gate else text).to_arrow_table())
            case QuerySpec(tag="rel", rel=(flt, project, group_by)):
                return await self._local("rel", lambda: self._relational(flt, project, group_by))
            ...
            case QuerySpec(tag="remote", remote=(sql, dsn, driver, op, transport)):
                ...
                return await guarded(RetryClass.RPC, anyio.to_thread.run_sync, _REMOTE[selected], sql, dsn, op, transport, self.inputs, subject="query.remote")
            case QuerySpec(tag="streaming", streaming=(plan, runner)):
                return await guarded(RetryClass.WIRE, anyio.to_thread.run_sync, _stream, plan, runner, cluster, self.inputs, subject="query.streaming")
            case unreachable:
    ...
    def _relational(self, flt: str | None, project: tuple[str, ...], group_by: tuple[str, ...]) -> pa.Table:
...
_COMPARE: Final[dict[Comparator, Callable[[NwExpr, Any], NwExpr]]] = {
...
# the predicate-bearing node widening is owned by the lower `columnar#SCAN` owner and imported
# here so the scan-receipt and query-receipt predicate counts read identical node classes —
# `query` depends on `columnar`, so the constant homes in the lower owner, never redefined.
