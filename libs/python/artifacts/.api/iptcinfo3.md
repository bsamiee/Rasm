...
[INTEGRATION_STACK]:
- `exchange/metadata#METADATA` ownership: `iptcinfo3` is RETAINED per the roster lock but SUPERSEDED on the RASTER carrier — the rebuilt `Metadata` owner folds EXIF/IPTC/XMP/ICC in ONE cross-format pass over `pyexiftool` (standing) / `pyexiv2` (optional in-process), so the former four-provider split (`iptcinfo3` IIM + `libxmp` XMP + `pyvips` ICC + `exif`) is no longer composed and `iptcinfo3` has no live `metadata.py` consumer; flagged for the final `pyproject` reconciliation (keep-vs-remove). The read/write folds below document the SUPERSEDED IIM integration as API reference, not a live path.
