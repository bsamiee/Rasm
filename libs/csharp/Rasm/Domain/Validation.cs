...
internal static class OpAcceptance {
    ...
    internal static Option<bool> ValidityOf(object? source) =>
        source switch {
            ...
            Stat s => Some(s is { Count: > 0, Minimum: double minimum, Maximum: double maximum, Mean: double mean, Variance: double variance, Rms: double rms } && RhinoMath.IsValidDouble(minimum) && RhinoMath.IsValidDouble(maximum) && RhinoMath.IsValidDouble(mean) && RhinoMath.IsValidDouble(variance) && RhinoMath.IsValidDouble(rms) && minimum <= maximum && variance >= 0.0 && rms >= 0.0 && (s.Context switch { StatContext.NoneCase => true, StatContext.MetricCase { Value: not null } => true, StatContext.ToleranceCase t => RhinoMath.IsValidDouble(t.Value) && t.Value >= 0.0 && t.WithinTolerance == (Math.Max(val1: Math.Abs(value: minimum), val2: Math.Abs(value: maximum)) <= t.Value), _ => false })),
            Distribution d => Some(value: ValidityOf(source: d.Summary).IfNone(noneValue: false) && RhinoMath.IsValidDouble(d.Median) && RhinoMath.IsValidDouble(d.Iqr) && d.Iqr >= 0.0 && d.Percentiles.ForAll(static p => RhinoMath.IsValidDouble(p.Percentile) && p.Percentile is >= 0.0 and <= 100.0 && RhinoMath.IsValidDouble(p.Value))),
