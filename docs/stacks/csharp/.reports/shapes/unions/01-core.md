...
- Shared option surface (both families, via `UnionAttributeBase`): `SwitchMethods`, `MapMethods`, `ConversionFromValue` (implicit by default), `ConversionToValue` (explicit by default), `SwitchMapStateParameterName` (`"state"` by default), `ConstructorAccessModifier` (public by default), `DefaultStringComparison` (ordinal-ignore-case by default), `SkipToString`, `SkipEqualityComparison`, `UseSingleBackingField`, `FactoryMethodGeneration`. Regular-union adds `NestedUnionParameterNames`. Ad-hoc adds per-member `TxName` / `TxIsNullableReferenceType` / `TxIsStateless`.
