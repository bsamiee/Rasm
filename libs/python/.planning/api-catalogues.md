...
The project venv runs `requires-python='>=3.15'` on the normal GIL CPython build (`cpython-315`, not `cpython-315t`). Distributions with a cp315 wheel reflect in place via `assay api query`. Source-buildable scientific distributions reflect in the default floor through `forge-scientific-env` so GDAL, GEOS, PROJ, HDF5, netCDF, Arrow, Fortran, and BLAS resolve from the machine toolchain. Marker-gated distributions reflect out-of-band on their interpreter floor (`python_version<'3.15'` for hard accelerator/runtime ceilings, `python_version<'3.13'` for the geometry-companion + VTK native set); their `.api` pages carry verified members plus the capture-gap note recording that the default `>=3.15` `assay api query` resolves no source against that interpreter until a fork/companion lock scope admits the lower environment.
