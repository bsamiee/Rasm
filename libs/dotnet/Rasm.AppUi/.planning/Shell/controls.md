...
- [03]-[MATERIALIZE_FOLD]: The `ControlFactory` intent-to-control fold; the context-column and package admission law; one `BehaviorBridge.Intent` bridge; total automation derivation.
...
|  [04]   | `Icon`     | sibling owner    | `Theme/assets` `IconSurface.Resolve` — the one ranked asset lookup           |
...
|  [10]   | `Activate` | third-party bind | `BehaviorBridge.Intent` over `Xaml.Behaviors.Avalonia` — the one command hop |
...
|  [05]   | selection list              | admitted  | `Segmented` select posture — coerced single selection, sliding thumb   |
