...
def _result_names() -> FrozenDict[int, str]:
    ...
    # built once behind the deferred import so the module loads on the cp315 core. An `EnumerationItem`
...
def _floor(tag: str, objective: "Objective") -> "Callable[[ContentKey], OutcomeReceipt]":
    ...
    # where the cp315 floor must stay reachable) and reconstructs the structured PyTree per probe through
...
def _ravel(params: "PyTree") -> "tuple[np.ndarray, Callable[[np.ndarray], PyTree]]":
    # the pure-numpy flatten/restore over the design leaves the cp315 floor probes, the host mirror of
    # `jax.flatten_util.ravel_pytree` (which would pull the gated jaxlib wheel): a single-array `params`
...
def _design_key(tag: str, params: "PyTree", descent: "Descent", restarts: int, seed: int) -> "RuntimeRail[ContentKey]":
    ...
    # cp315 floor probes, so a re-run from the same start keys identically on the gated Optimistix and
...
- [JAX_ADJOINT]: the `equinox.filter_value_and_grad(Objective.cost, has_aux=True)` pass folds the converged objective and the first-order/KKT residual in one trace, carries the `python_version<'3.15'` marker, and verifies against `compute/.api/equinox.md`/`compute/.api/jax.md` once the jaxlib wheel resolves; it differentiates the scalar objective for the `field`/`density` `minimise` routes and the least-squares cost `½‖r‖²` for the `mesh` `least_squares` route (whose gradient is `Jᵀr`) w.r.t. the inexact-array design leaves only, returns the reported objective scalar (`‖r‖` on the `RESIDUAL` route, the raw scalar otherwise) as the differentiation aux so the objective is never re-evaluated, and the gradient PyTree's L∞ stationarity norm reads `optimistix.max_norm` directly over the PyTree (confirmed `(x: PyTree[Array]) → Shaped[Array,'']`), so the converged objective and residual are computed on-device through `jax.numpy`/filter transforms and `numpy.asarray` never touches a JAX PyTree. The `Objective.cost` projection (the RECEIPT objective) and the distinct `Objective.target` projection (the SOLVE input the `least_squares` route feeds the raw residual VECTOR and the `minimise` route the scalar) are the two shape-keyed reductions of the one `fn`, retiring the prior twin `_converged_outcome`/`_objective_scalar` helpers that each rebuilt the scalar reduction; the `filter_value_and_grad` differentiates `cost` (its `½‖r‖²` yielding the `Jᵀr` stationarity gradient) while optimistix differentiates `target` (the residual itself, owning the `½‖r‖²`/`Jᵀr` reduction internally), so the converged-design residual the receipt folds and the LM Jacobian the solver forms are each derived from the correct reduction rather than one conflated scalar. The residual is the stationarity gradient `∇f` (respectively `Jᵀr`) that vanishes at the converged design the receipt folds and the `solver` `HandoffAxis` case at `graduation/handoff.md#GRADUATION` reads. The central-difference residual floor runs on the FLAT raveled numpy design buffer only — `_ravel` flattening the same design leaves `_design_key` keys over (the host mirror of `jax.flatten_util.ravel_pytree`, no jaxlib import) and `_floor_cost` restoring the structured PyTree through the captured `unravel` before each `objective.fn` call so a multi-leaf design (a `(N, 3)` node-coordinate field, an `(H, W)` material grid, or a tuple of design blocks) feeds `objective.fn` its real shape rather than a stacked-array mis-shape from a bare `np.asarray(params)` — folding the same `Shape`-keyed `(½‖r‖², ‖r‖)`-vs-`(scalar, scalar)` reduction `Objective.cost` owns; each coordinate is probed by a `_FD` `np.zeros` one-hot rather than a materialized dense `np.eye(x0.size)` basis whose O(n²) allocation a realistic SIMP field cannot afford, so the floor reproduces the same first-order quantity through `np.fromiter`/`‖·‖∞` unconditionally on cp315.
