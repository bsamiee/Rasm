...
The climate and thermal-comfort owner — the weather substrate of the energy plane and its point-comfort tier. `Climate.of` admits EPW weather by payload shape, and the admitted document answers four families on one `ladybug` substrate: the `series` collection algebra, the `derive` projection family, the `solar` sun-geometry family, and the `comfort` point-comfort tier with SolarCal MRT. This page is the energy plane's ONLY solar owner. The owner re-implements no weather, psychrometric, or comfort kernel; `ladybug-core` and `ladybug-comfort` own those, and this page composes them into typed, railed, receipted evidence.

The Ladybug band is AGPL-3.0 network copyleft on the standing companion-lane charter: `ladybug`/`ladybug_comfort` import function-local at the boundary kernels — a module-top import loads the AGPL band into every companion start — and evidence crosses the wire, never a link into a distributed host binary. Every operation threads the `rasm.geometry.graduation` `evidence_run` weave, the admitted document keys once over its canonical file-string bytes so re-ingestion dedupes in the persistence ledger, and comfort evidence graduates under `GeometrySubject.THERMAL_COMFORT` against caller ceilings.
...
- Owner: `Climate` holds the decoded `EPW`, its `Location`, and the weather `ContentKey`. `ClimateField` members ARE the `EPW` property names, so field access is one `getattr` derivation; `Reduce.method` derives its ladybug spelling from its own fields, so the reduction family is one derived name; `ComfortModel` keys the `COMFORT` row (module, class, parameter keyword, result properties), so a comfort run is one row-resolved `from_epw` call with the serializable `*Parameter.from_dict` config carried as data.
- Entry: `Climate.of` admits `bytes | str | Path | Mapping` by payload shape — the modality is the shape, never a `source_kind=` knob. The `Wea`/`DDY` projections serialize through their own `to_file_string` and reach the recipe boundary as handler inputs, never a second reader. `comfort` routes the EPW solar fields through SolarCal to the `rad_temperature` MRT input internally; `mrt()` is the explicit arm only for a caller feeding a non-EPW thermal model.
- Auto: collection alignment is a precondition the fold lifts — `are_collections_aligned` before any cross-series op, never a silent ladybug raise; the psychrometric/sky scalar kernels reach consumers as named composed functions, never re-derived; batch vectorization over sensor grids stays `numpy` inside the pointwise `ladybug_comfort` kernels; the `comfort_map` rows consume the simulation matrices the `energy/simulate` recipe products feed back.
- Receipt: the COMFORT tier passes `Some(percent_comfortable)` so `graduates` stays reachable; the graduation residual is the discomfort fraction `1 - percent_comfortable/100` against the caller ceiling, so a comfort run crossing to compute carries measured evidence, never a count that clears any ceiling.
- Packages: `ladybug-core` and `ladybug-comfort` per the fence imports, every binding function-local at boundary scope on the AGPL companion posture; the spatial `map.*` kernels are the `energy/simulate` readback surface.
- Growth: a new field is one `ClimateField` member; a new grain one `Grain` member — the method name derives; a new projection one `Derived` case, IP-units over `EPW.convert_to_ip` the named next; a new sun answer one `SolarQuery` case; a new comfort model one `COMFORT` row. `Adaptive` is not weather-drivable — it builds `from_air_and_rad_temp` over model results, so its home is the `energy/simulate` readback, its `AdaptiveParameter` already serializable through this page's parameter discipline; urban-microclimate EPW morphing enters as one more `Derived` case with its own package admission.
