...
- `documents/document-plan.md` — the document-mode emission axis: PDF authoring/render/raster/assemble/repair (reportlab/weasyprint/typst), Office, and structured-text on one backend-per-mode table, each backend a lowering arm folding from `DocumentNode`, with the Typst introspection (`TYPST_QUERY`/`TYPST_DATA`) rows on the held `Compiler` font-cached world for batched data-bound renders.
...
- `tables/table-plan.md` — the great-tables publication-table owner exporting HTML/LaTeX/PDF over one `reduce`-over-spec build: the `tab_options` theme, header/footnote/source-note structure, summary-row algebra, `loc`-targeted cell styling, column-control family, and `data_color(autocolor_text)` data-driven fill.
- `imaging/preview.md` — the raster/preview owner over pillow, scikit-image, segno, and python-magic, with the `MARK` encoded-mark arm carrying a `Symbology` sub-axis (segno QR/Micro-QR settled, python-barcode linear (1D) symbologies RESEARCH-pending the catalogue sync).
...
- `color-management/colorimetry.md` — the colour-science colorimetry, spectral, CAM16, and palette owner feeding the visual sub-domains, plus the `MANAGED` ICC/LUT/CCTF color-managed raster egress (colour-science tone-curve chain settled on the core, pillow `ImageCms` apply RESEARCH-pending on the gated band, forcing the async owner).
- `typography/conformance.md` — the fonttools subset/instance, uharfbuzz OpenType text shaping (`SHAPE` -> `PositionedGlyphRun`), pyhanko PAdES-signing, and pyhanko/pikepdf conformance-audit close (`AUDIT` -> `ConformanceVerdict`: signature validity, coverage, DSS/LTV material, archival metadata).
