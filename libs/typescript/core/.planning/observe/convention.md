...
- Law: `ring` and `tenant` stay unspellable as a row's fan — the identity projection stamps `ring` on every series and `tenant` on every series a tenant-pinned identity mints, so a row restating one forks the correspondence and re-declares a dimension it never owns.
...
// Identity dimensions no instrument row declares: the projection stamps `ring` on every series this branch mints and
// `tenant` on every series a tenant-pinned identity mints, so they carve out of the domain grammar and enter the
// metric-plane roster once rather than on fifty-odd rows — unconditionally, because a governor filters per ENTRY.
...
- Law: a key only SOME entries carry stays rostered unconditionally, because the governor filters PER ENTRY — `rasm.tenant` rides the estate fold whether or not a process pins one, so an entry carrying it survives while an untagged entry reads the same either way, and narrowing the roster to what one deployment stamps strips the key from every deployment that does stamp it. The untagged entries are the untagged WHOLE, never a `key=""` cell standing beside the tagged ones, so a read slicing that axis sums them rather than matching an empty value. A family whose key IS its own cell's identity — `cacheName`, `laneName`, `poolScheme` — never reaches that state at all: its unmeasured cell publishes no point, and the silence is the absence.
