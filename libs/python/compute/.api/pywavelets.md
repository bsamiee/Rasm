...
| :-----: | :------------------ | :----------------- | :----------------------------------------------------------------------------- |
|  [01]   | `Wavelet`           | discrete wavelet   | `Wavelet(name)` — filter bank (`dec_lo`/`dec_hi`/`rec_lo`/`rec_hi`), `wavefun` |
|  [02]   | `ContinuousWavelet` | continuous wavelet | `ContinuousWavelet(name)` — CWT basis with `wavefun`                           |
|  [03]   | `WaveletPacket`     | 1D packet tree     | `WaveletPacket(data, wavelet, mode, maxlevel)` — full/best-basis 1D packets    |
|  [04]   | `WaveletPacket2D`   | 2D packet tree     | 2D wavelet-packet decomposition tree                                           |
|  [05]   | `Modes`             | extension modes    | `symmetric`/`reflect`/`periodic`/`periodization`/`zero`/`constant`/`smooth`    |
...
| [INDEX] | [SURFACE]                           | [CALL_SHAPE]                                                                                  | [CAPABILITY]                                |
| :-----: | :---------------------------------- | :-------------------------------------------------------------------------------------------- | :------------------------------------------ |
|  [01]   | `swt`/`iswt`                        | `swt(data, wavelet, level=None, start_level=0, axis=-1, trim_approx=False, norm=False)`       | stationary (undecimated) DWT and inverse    |
|  [02]   | `swt2`/`iswt2`                      | `swt2(data, wavelet, level, ...)`                                                             | stationary 2D DWT and inverse               |
|  [03]   | `cwt`                               | `cwt(data, scales, wavelet, sampling_period=1.0, method='conv', axis=-1)` -> `(coefs, freqs)` | continuous wavelet transform                |
|  [04]   | `threshold`                         | `threshold(data, value, mode='soft', substitute=0)`                                           | soft/hard/garrote/greater/less thresholding |
|  [05]   | `wavelist`                          | `wavelist(family=None, kind='all')` -> `list[str]`                                            | enumerate the wavelet catalogue             |
|  [06]   | `families`                          | `families(short=True)` -> `list[str]`                                                         | wavelet family names                        |
|  [07]   | `wavefun`                           | `Wavelet.wavefun(level=8)`                                                                    | scaling/wavelet functions on a grid         |
|  [08]   | `dwt_max_level`                     | `dwt_max_level(data_len, filter_len)` -> `int`                                                | maximum useful decomposition level          |
|  [09]   | `frequency2scale`/`scale2frequency` | `scale2frequency(wavelet, scale, precision=8)`                                                | CWT scale↔frequency conversion              |
