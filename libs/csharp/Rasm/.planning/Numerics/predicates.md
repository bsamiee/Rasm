...
namespace Rasm.Geometry.Numerics;
...
|  [04]   | Exact predicates            | `Predicate`       | ONE static family — 4 direct four-tier ladders + axis-projected implicit `Orient2D` + `Compare` order key + implicit in-circum pair (spanning the whole explicit/implicit × axis lattice) | `Predicate.<member> → Sign` (pure, total, exact)   |    8    |
