...
def _translated(building: BuildingModel, target: TranslateTarget, folder: Path) -> "RuntimeRail[Path]":
    ...
    def in_process() -> Path:
        ...
        return Path(writer(building.model))
    ...
    def cli() -> Path:
        ...
        return Path(run_osw(osw, measures_only=(target is not TranslateTarget.IDF)))
