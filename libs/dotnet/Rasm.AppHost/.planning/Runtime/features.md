...
One feature-flag, progressive-rollout, and experimentation owner for the runtime spine: a frozen `FlagDefinition` row family compiles into one config-backed `OpenFeature` `InMemoryProvider`, a deterministic sticky-bucketing evaluator seats each subject in a stable rollout segment off the kernel `ContentHash` of subject-plus-flag, every evaluation projects to one canonical `FlagVerdict` the model-routing and fleet-roll consumers read, and the operator kill-switch collapses onto the flag row's provider-native disabled gate rather than a parallel switch. The page produces the `FlagVerdict` contract `Agent/reasoning#MODEL_GOVERNANCE` resolves a `ModelRoute` from and `Sandbox/provisioning#ROLLOVER_DRAIN` resolves a `RollStrategy` from — this page owns *which variant, for whom, at what exposure*, the consumers own *what the variant does* — and it owns the flag-definition axis, the targeting-rule and segment vocabulary, the config-backed provider registration, the sticky-bucketing evaluator, and the verdict projection. It consumes the eight-row `ConfigSource` chain on the one `ConfigurationManager` and the `Overlay`/`OperatorOverride`/`ReloadOutcome` transition from `Runtime/config#POLICY_VALUES`, the `ContentHash.Of`/`ContentHash.Halves` content-address pair and its `CanonicalWriter` from `Rasm/Domain/identity`, `TenantContext.Slug` and `CorrelationId` from `Runtime/ports`, `ClockPolicy`, and `DataClassification` for the targeting-attribute redaction boundary, minting no eighth port. `OpenFeature` owns evaluation and variant behavior, Thinktecture owns the domain vocabularies, protobuf owns the peer contract, and LanguageExt owns the result carriers.
...
- Boundary: `Register` is the features owner's boot gate and seats FOUR concerns at once, so `Runtime/modules#MODULE_LEDGER` composes it in the runtime module fold and holds the returned provider handle for `Reload` — a compile that no module reaches leaves the whole subsystem unregistered while every consumer resolves `FlagVerdict.Inert` and reads it as policy.
- Boundary: `Variant` is a name this branch carries twice — the flag arm here and the `Fabrication` process variant — and they stay two owners under one spelling because neither ever crosses the other's boundary: this one is a `[ValueObject<string>]` whose whole domain is the OpenFeature variant map, and a cross-folder rename would move a wire key three surfaces read (the provider map, the verdict, the decode boundary) to settle a collision no consumer can experience.
...
- Entry: `Assign(FlagDefinition flag, EvaluationContext context)` returns `Variant` — folds the flag's ordered rules and returns the first matching rule's seated variant, falling to the flag default when no rule matches; `BucketOf(FlagKey key, string subject)` returns `int` in `[0,100)` — the stable rollout bucket from the kernel `ContentHash.Of` framed writer over the subject and the flag key, projected through `ContentHash.Halves(...).Low` and folded modulo 100.
...
- Auto: bucketing is a pure deterministic fold inside the flag evaluator; the assigned `Variant` and the `Reason` ride the `VERDICT_PROJECTION` `FlagVerdict`.
...
- Boundary: the verdict is the only cross-page features contract — a consumer reaching the `IFeatureClient` directly, a second verdict shape, and a re-derived bucket at a consumer are the deleted forms; the fault lift and exposure emit ride the registered `Hook`; the projection reads `FlagEvaluationDetails<Value>` and never the raw `FeatureProviderException`; consumers map `Variant` to their own row families and the features owner never owns consumer behavior; the generated protobuf message is built directly, while descriptor validation remains centralized at shared decode and gRPC boundaries rather than repeated in every projection.
...
- Auto: the kill-switch flip rides the config page's `ReloadOutcome`; the forced-off evaluation rides the normal `FlagVerdict` carrying `Reason.Disabled`.
...
---
flowchart LR
    accTitle: One features owner projecting a verdict to the routing consumers
