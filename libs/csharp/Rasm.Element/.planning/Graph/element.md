...
public abstract partial class Node {
    ...
    Option<OwnerHistory> History, SchemaSpan Span, Seq<Classification> Classifications = default) : Node {
        ...
        // Absent for every enumerated PredefinedType, which is why it is `Option` and not an empty-string sentinel; the
        // egress re-deriving the label from `Name` was the bounded drop this column closes. ONE column serves BOTH
        // kinds — an occurrence's `IfcObject.ObjectType` and a type's `IfcElementType.ElementType` are the same concept
        // on two entity slots, which the Bim ingress reads through one two-arm `UserLabel` and the egress re-stamps
        // through the matching pair; the type signature bag carries no label row, so routing the type side there was
        // the same `Name` substitution under another name. Absence-carrying like `ExternalId`, so no string-equality
        // attribute applies.
    ...
    quantitySet: q => { w.Ordinal(3); w.String(q.Bag.SetName); w.String(q.Bag.Inheritance.Key); w.Ordinal(q.Bag.Source.Key); w.Ordinal(q.Bag.Values.Count); foreach (var (n, m) in q.Bag.Values.OrderBy(static e => e.Key.Value, StringComparer.Ordinal)) { w.String(n.Value); w.Measure(m); } },
    ...
    public Node Relabel(NodeId id) => Switch<Node>(
        @object: o => new Object(id, o.Kind, o.ExternalId, o.Classification, o.PredefinedType, o.ObjectType, o.Name, o.Tag, o.Representations, o.History, o.Span, o.Classifications),
...
- Packages: `Generator.Equals` (`[Equatable]` snapshot equality, `[StringEquality]`/`[UnorderedEquality]`/`[OrderedEquality]`/`[IgnoreEquality]` member policies, `Inequalities` diff, and the generated `EqualityComparer.Default` reused as the LINQ/`HashSet` key comparer outside generated code), QuikGraph (`BidirectionalGraph`/`SEdge` topology view + `AlgorithmExtensions`), LanguageExt.Core (`Seq`/`Map`/`Option`/`Fin`), System.Collections.Frozen/Immutable, NodaTime (`Instant`), `Rasm` (the kernel `Op` op-key).
...
public sealed partial class ElementGraph {
    ...
    // source->participant leg per roster member — so reachability/LCA traverse THROUGH the realizing node and reach every
    // n-ary participant, never an endpoints-only From->To shortcut that hides them.
    topology = new(() => {
    QuikGraph.BidirectionalGraph<NodeId, QuikGraph.SEdge<NodeId>> graph = new(allowParallelEdges: true);
    graph.AddVertexRange(nodes.Keys);
    foreach (Relationship edge in edges) { foreach ((NodeId from, NodeId to) in edge.DirectedPairs) { graph.AddEdge(new QuikGraph.SEdge<NodeId>(from, to)); } }
    return graph;
    });
    }
    ...
    var (occProps, occQty, occMaterials, occAssessments, appearance, coverages, observations) =
    toSeq(EdgesAt(root.Id)).Fold(
    (Props: Seq<PropertyBag>(), Qty: Seq<QuantityBag>(), Mat: Seq<BakedMaterial>(), Asm: Seq<AssessmentPayload>(), App: Option<AppearanceSummary>.None, Cov: Seq<CoverageGrid>(), Obs: Seq<ObservationSeries>()),
    ...
    // Measured evidence rides the OCCURRENCE alone — TypeResolutionOf gathers no observation, because a Component is a
    // catalogue entry no sensor is mounted on and a type-inherited series would claim every realization reports one
    // instrument's data. The one Seq the named inheritance deliberately does not union.
    Relationship.Assign a when a.Subject == root.Id && a.SubKind == AssignKind.Observation && Nodes.TryGetValue(a.Definition, out var d) && d is Node.Observation obs => acc with { Obs = acc.Obs.Add(obs.Series) },
    ...
    Seq<Classification> classifications = UnionBy(
        root.Classifications.Filter(classification => classification != root.Classification),
        typeFold.Map(static t => t.Type.Classifications).IfNone(Seq<Classification>()).Filter(classification => classification != root.Classification),
    ...
    Option<(Node.Object Type, Seq<PropertyBag> Props, Seq<QuantityBag> Qty, Seq<BakedMaterial> Materials, Seq<AssessmentPayload> Assessments, Option<AppearanceSummary> App)> TypeResolutionOf(NodeId occurrence) =>
    TypeObjectOf(occurrence).Bind(typeId => Find<Node.Object>(typeId).Map(typeObj => {
    var (props, qty, mats, asms, app) = toSeq(EdgesAt(typeId)).Fold(
    (Props: Seq<PropertyBag>(), Qty: Seq<QuantityBag>(), Mat: Seq<BakedMaterial>(), Asm: Seq<AssessmentPayload>(), App: Option<AppearanceSummary>.None),
    (acc, edge) => edge switch {
    ...
    Relationship.Associate r when r.Subject == typeId && Nodes.TryGetValue(r.Resource, out var res) && res is Node.Appearance ap => acc with { App = ap.Summary },
    _ => acc,
    });
    return (typeObj, props, qty, mats, asms, app);
    }));
    ...
    // string key passes none and takes the default. The hand-rolled Exists-fold this replaces re-derived
    // first-wins as a quadratic membership probe and could not admit a comparer at all.
