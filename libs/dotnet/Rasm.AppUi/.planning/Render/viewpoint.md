...
`Viewpoint` is the portable view state the whole package addresses a saved view by: one `ViewCamera` lens, an optional `SectionBox` clip volume, the `VisibilityOverride` rows every posture and every interaction fold answers, the selection, and the source-addressed measurements — captured once, serialized through the package wire context, and projected onto the `cs:Rasm.Bim/Review/issues#BCF_ARCHIVE` `BcfViewpoint` exchange contract through one generated mapper. `OverrideState` is the closed vocabulary of what a renderer can do to an element, so a hidden element carrying a transparency has no spelling; `OverrideRole` is the one row family the version-diff and analysis-participation postures both project through. `ViewRegistry` is the one named-view owner — standard views derived from the view cube's own signed axis triples, user bookmarks, captured cameras, and traversal history as one row family under one `ViewKey` address space, recalled through the `Render/animation` camera track. This page owns that view state, the override vocabulary and its folds, the BCF projection mapper, the named-view registry with its cube and projection toggle, and the viewport chrome key roster; the render graph that consumes a `FrameView` lives in `Render/pipeline`, the overlay-plane manipulators in `Render/measure`.

## [01]-[INDEX]

- [02]-[VIEWPOINT_CODEC]: Camera, section box, override state and role, the interaction fold, the display postures, and the generated `BcfViewpoint` mapper.
...
- Output: `Viewpoint` projects through `Render/pipeline#TS_PROJECTION` `ResidencyMap` onto generated `Render.ViewpointWire` and serializes through AppHost `WireJson`, so the browser, markup, and cross-process coordination read protobuf's one JSON mapping and this page mints no DTO, serializer context, or codec twin.
...
public static class ViewpointCodec {
    ...
    public static Fin<Viewpoint> FromBcf(string key, int revision, BcfViewpoint bcf, Instant at) =>
        bcf.Camera.ToFin(new ViewportFault.ContextUnavailable($"viewpoint/bcf-camera:{bcf.Guid}"))
            ...
            .Bind(camera => Viewpoint.Capture(revision, camera,
...
- Output: a recall folds through `Viewpoint.Capture` when the caller pins it, so a named view and a shared BCF viewpoint share one `Viewpoint` shape keyed by the registry; the pin draws its revision from `ViewRevisions.Next`, so re-pinning a key a user already saved supersedes the prior reading at every depot rather than tying with it.
            ...
            var key => Ring.Visit(key) switch {
                var ring => this with {
                    ...
                    Rows = Rows
                        ...
                        .AddOrUpdate(new NamedView("view.history.entry", ViewOrigin.Visited, camera, section, overrides, MotionToken.Fast, at)),
                },
            },
        };
            ...
            var direction => toSeq(Enumerable.Range(0, Math.Abs(delta)))
                .Fold(
                    ...
                    (held, _) => direction.Next(held.Registry.Ring).Match(
                        Some: key => (held.Registry with { Ring = held.Registry.Ring.Stepped(direction.Step) }, held.Taken + 1),
                        None: () => held)) switch {
                var walked => walked.Landed
                    .ToFin(new ViewportFault.ContextUnavailable($"view/history-end:{direction.Key}"))
                    .Bind(key => walked.Registry.Recall(from)
...
flowchart LR
    ...
    accDescr: Interaction folds, version ghosts, and display postures all project onto one override state, which the viewpoint captures and the BCF mapper exchanges.
