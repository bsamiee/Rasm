...
public static class Panelization {
    ...
    // Each seed snaps to its nearest tessellation vertex; per-seed EnsureGeodesicDistances(source.Mesh,
    // Seq(vertex), key) → per-vertex argmin labels → wall crossings at t = δ(u)/(δ(u) − δ(v)) chained
    // into closed cell polygons (n-gon offset rows); an unclosable wall chain routes 2449 Panel naming the cell.
