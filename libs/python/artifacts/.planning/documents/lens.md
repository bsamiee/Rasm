...
- [1]-[LENS]: `DocumentLens` (Struct, frozen) + `LensOp` nine-op closed `StrEnum`; text/image/word/region/search/embedded/outline/redaction/annotation/`Story` recovery into `DocumentNode`, one total `match` over one `async_boundary`, gated MuPDF/XML arms on the subprocess seam and pure-Python pypdf arms on the core.
