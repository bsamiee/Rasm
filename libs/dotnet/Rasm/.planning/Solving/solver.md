...
- Cases: `SolveStatus` closes over the three ways an iterate ends — `Converged` on the residual tolerance, `Stationary` on a step floor the residual never cleared, `Exhausted` on a spent budget with a live descent direction — and its KEY ordinal IS the severity the island fold maxes over, so no bool accumulator re-states that ordering. `LmPass` is the continue-or-done carrier the driver folds and never interprets. The singular outcome rides the `Fin` failure channel as `GeometryFault.SingularSystem`, kept off the status vocabulary. `ObjectiveSense` `Minimize`/`Maximize` is the branch objective-direction vocabulary seated beside the status roster: the kernel iterate itself always minimizes, and a consumer whose objective points the other way folds `Sign * objective` through the row rather than a call-site sign literal — Compute's optimizer descent rows and Fabrication's orientation scoring compose these two cases, so direction spells once for the branch.
