...
- [01]-[ARRANGEMENT]: `ArrangementKind` discriminant; `Arrangement` `[Union]` (`MeshBoolean`/`PlanarOverlay`/`CellComplex`) over one `ArrangementStore` SoA; the `Apply` cell-arrangement fold composing exact-sign crossing subdivision, constrained re-tetrahedralization substrate, GWN cell classification, `BooleanOp` cell-keep predicate, and `DuplicateWeld` boundary weld; the typed `BooleanReceipt` evidence.
