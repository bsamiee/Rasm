...
- `Microsoft.Extensions.Telemetry.Abstractions`(`libs/dotnet/.api/api-telemetry-abstractions.md`): owns the `IEnrichmentTagCollector` the enricher fills and the `HttpRouteParameterRedactionMode` and classified-tag redaction the `DataClassification` maps resolve against before any sink observes them.
