...
- Packages: SharpGLTF.Core, SharpGLTF.Toolkit, GeometryGymIFC_Core, Openize.Drako, Alimer.Bindings.MeshOptimizer, AssimpNetter, UniversalSceneDescription, NodaTime, LanguageExt.Core
...
public static partial class BimExport {
    public static Fin<ExportArtifact> Export(InterchangeFormat format, ImportedGeometry geometry, InterchangePolicy policy, ClockPolicy clocks) =>
        ...
        : format.Codec == InterchangeCodec.SceneExchange
            ? Try.lift(() => Sealed(format, SceneBytes(format, geometry), policy, clocks.Now)).Run().MapFail(static error => new BimFault.CodecReject($"scene-export:{error.Message}").ToError())
        : format.Codec == InterchangeCodec.UsdStage
            ? Try.lift(() => Sealed(format, UsdBytes(format, geometry), policy, clocks.Now)).Run().MapFail(static error => new BimFault.CodecReject($"usd-export:{error.Message}").ToError())
...
public static class TileAvailability {
    public static Fin<byte[]> Author(Seq<TileNode> tiles, subtree.ImplicitSubdivisionScheme scheme) =>
        Try.lift(() => scheme == subtree.ImplicitSubdivisionScheme.Octree
            ...
            .MapFail(static error => new BimFault.ModelRejected($"subtree-author:{error.Message}").ToError());
    ...
    public static Fin<Map<(int Level, int X, int Y, int Z), byte[]>> AuthorMany(Seq<TileNode> tiles, subtree.ImplicitSubdivisionScheme scheme) =>
        Try.lift<Map<(int Level, int X, int Y, int Z), byte[]>>(() => scheme == subtree.ImplicitSubdivisionScheme.Octree
            ...
            .MapFail(static error => new BimFault.ModelRejected($"subtree-author-many:{error.Message}").ToError());
