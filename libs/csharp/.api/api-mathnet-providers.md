...
- convenience entry: `Matrix<T>.SolveIterative(input, solver, iterator = null, preconditioner = null)` allocates and returns the solution (`Vector<T>` or `Matrix<T>` by right-hand side); `TrySolveIterative` writes a caller-owned `result` and returns `IterationStatus` — convergence reads `Iterator<T>.Status`, never the returned vector alone

[STACK]:
- The dense solve stacks under a LanguageExt `Fin` rail emitting a typed receipt (`Geometry/Numerics/matrix` `MatrixKernel` — the one MathNet access path): `Try.lift(() => spd.Cholesky().Solve(rhs)).Run()` maps a singular/indefinite factorization to a typed `GeometryFault.SingularSystem` over `Fin<Vector<double>>`, never a thrown exception — the factorization is the BLAS-class numeric, the `Try.lift`/`Fin` is the rail, the `SolveReceipt` (solution, `SolvePath`/`SolveStop`, the recomputed true relative residual) is the typed evidence
