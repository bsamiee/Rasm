...
- Entry: `Livewire.status`/`Livewire.coerced`/`Livewire.receipt` per-family schemas; `Livewire.decode(family, octets)` the keyed one-shot whose return follows the family through the `_Landing`-mapped row contract; `Livewire.feed` the union schema for the mixed binding channel — one decode, the `_tag` dispatches, and `$`-style panel dispatch stays a `Match.tagsExhaustive` record at the consumer.
...
const _rows: { readonly [K in keyof _Landing]: Schema.Schema<_Landing[K], Uint8Array> } = {
  BindingStatusWire: ProtoCodec.family(ProtoCodec.suite.BindingStatusWire, BindingStatus),
  CoercedValueWire: ProtoCodec.family(ProtoCodec.suite.CoercedValueWire, CoercedValue),
  WriteReceiptWire: ProtoCodec.family(ProtoCodec.suite.WriteReceiptWire, WriteReceipt),
}
...
const Livewire: {
  ...
  readonly status: (typeof _rows)["BindingStatusWire"]
  readonly coerced: (typeof _rows)["CoercedValueWire"]
  readonly receipt: (typeof _rows)["WriteReceiptWire"]
  readonly feed: typeof _feed
  readonly decode: <K extends Livewire.Family>(family: K, octets: Uint8Array) => Effect.Effect<Livewire.Decoded<K>, ParseResult.ParseError>
} = {
  Status: BindingStatus,
  Coerced: CoercedValue,
  Receipt: WriteReceipt,
  status: _rows.BindingStatusWire,
  coerced: _rows.CoercedValueWire,
  receipt: _rows.WriteReceiptWire,
  feed: _feed,
