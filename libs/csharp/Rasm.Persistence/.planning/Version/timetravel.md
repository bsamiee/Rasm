...
public static class TimeTravel {
    ...
    public static IO<ScrubReel> Scrub(AsOfQuery query, ScrubWindow window, TimeLog log) =>
        ...
        // form). Each resulting graph is addressed once; no digest-only `(prior, delta)` shortcut can reconstruct the
        // order-independent full-graph address without the graph's canonical member set.
        let reel = windowed.Fold((Frames: Seq<ScrubFrame>(), Graph: seeded, Address: ContentAddress.OfGraph(seeded)), static (acc, e) => {
            ...
            ContentAddress nextAddress = ContentAddress.OfGraph(next);
            return (acc.Frames.Add(new ScrubFrame(acc.Frames.Count, e.Version, e.Data.Lifecycle, new Hlc(Instant.FromDateTimeOffset(e.Timestamp), (ulong)e.Version), ActorOf(e), acc.Address.Value, nextAddress.Value)), next, nextAddress);
        })
        ...
        return holds(floor)
            ...
            : Search(0, versions.Count, 0L).Bind(found => found.Flip < versions.Count && byVersion.Find(versions[found.Flip]) is { IsSome: true, Case: IEvent<GraphEvent> flipEvent }
                ? from before in log.ReconstructAt(versions[found.Flip] - 1L)
                  ...
                  select new BisectOutcome(Some(FlipFrame(found.Flip, flipEvent, before, after)), found.Probes, HeldAtFloor: false)
                : IO.pure(new BisectOutcome(None, found.Probes, HeldAtFloor: false)));
    }

    // Each side of the flip receives the canonical full-graph address used by every other AS-OF consumer.
    static ScrubFrame FlipFrame(int index, IEvent<GraphEvent> e, ElementGraph before, ElementGraph after) {
        ContentAddress priorAddress = ContentAddress.OfGraph(before);
        ContentAddress afterAddress = ContentAddress.OfGraph(after);
    ...
    // Sink emission demands the AppHost `Runtime/ports` primitives (`CorrelationId`, `TenantContext`), so the frame's
    // raw `Guid`/`UInt128` values map AT THIS EDGE — `CorrelationId.Create` lifts the frame correlation and
    // `TenantContext.Current` reads the boot-stamped ambient (a bare `frame.Tenant` `UInt128` has no conversion
    // to `TenantContext`; passing it is the non-compiling seam drift, and re-minting a `TenantContext` from the
    // raw id would fabricate a slug the boot mint owns).
    static IO<(ElementGraph Graph, TimeTravelReceipt Receipt)> Folded(AsOfQuery query, TimeLog log) =>
        ...
        from _ in log.Sink.Send(CorrelationId.Create(log.Frame.Correlation), TenantContext.Current, "Rasm.Persistence", receipt.Slot, JsonSerializer.SerializeToElement(receipt, ElementJson.Options))
