...
- Reject: storing a live mutable `H3Index` instance, the `H3Index.Invalid` sentinel crossing the boundary unwrapped, a second coordinate model beside the Bim NTS stack, the legacy method names where the call must match `h3-pg`, hand-rolled great-circle/area math the `H3GeometryExtensions` metrics own

[CONSUMER]:
- No `Rasm.Bim/.planning` design fence composes `pocketken.H3` yet: the `Rasm.Bim.csproj` references the central `pocketken.H3` pin and the README rosters it as the "H3 hexagonal DGGS site-context keyer," but the `Semantics/geospatial#GEOSPATIAL_SEAM` site-context owner today indexes through the `GeoModel` `STRtree` envelope broad-phase and projects via `GeoFeature.ToElement` without a DGGS keying arm. The natural future consumer is that seam — an H3 cell column beside the `GeoFeature` row and a `GridDiskDistances`/`Fill` coarse-bucket prefilter beside the `STRtree` narrow-phase — keying in lockstep with the `Rasm.Persistence` `h3-pg` server index. The pin is wired and catalogued ahead of that promotion.
