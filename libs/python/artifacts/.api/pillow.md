...
- worker-seam axis: every Pillow op is synchronous native CPU work; it crosses ONE caller-threaded `lane.offload(Kernel.of(_worker, KernelTrait.HOSTILE), op)` seam whose HOSTILE trait routes onto the warm loky pool binding the shared `WORKER_BAND` the `exchange/detect`/`exchange/metadata`/`graphic/raster/*`/`graphic/color/managed` worker crossings share — never a per-owner `CapacityLimiter(slots)` knob that oversubscribes the host, never the unbounded per-loop default, never the inline event loop, and never a `to_interpreter` subinterpreter (which shares the host interpreter version and cannot host the native stack). The trait row's `WORKER` retry recovers a transient OOM/signal worker death before the slot faults, and the terminal raise crosses the runtime `async_boundary` conversion, never a local catch.
