...
- Four orthogonal absence axes — parameter sentinel, wire tri-state (`msgspec.UNSET` / Pydantic `MISSING`), computed `Option` rail, vocabulary-encoded inherit/null policy — each own distinct invariants; only the axis that materialized absence may interpret it; interior folds receive collapsed canonical values, never raw axis carriers past the admitting decorator.
- Six binary absence descriptors compose through import-time implication closure and dominance partial order into one `EncodingDomain` and `ForkOutcome` per concept — `evaluate(ConceptId)` caches an immutable `DecisionSnapshot` at composition-root import; witness kinds mint at stage `n` and consume only at legal stage `n+1`, never surviving past `CANONICAL_MATERIALIZE`.
- Galois connection between descriptor bases and distinguishable absence postures (`α`, `γ`) plus adjoint collapse/encode functors (`L ⊣ R`) certifies encode policy as total `frozendict[PostureClass, EncodeFn]` on the decision snapshot with unit/counit adequacy rows—not parallel projection-local `omit_defaults` / `exclude_unset` flags; ingress `WitnessTag` and egress `CounitTag` occupy separate namespaces with erasure at canonical materialize and mint at egress encode.
- Isolated per-layer absence proofs (`evaluate`, semigroup legality, Galois adequacy chains) can all pass while federation coupling locks fail — `assemble(ConceptId)` materializes one frozen `FederationRow` whose `lock_certificate` and `content_hash` are mandatory merge and worker-boot gates; enum closure or snapshot hash parity alone is false-green.
- Wire `tag_field` string differs from pydantic discriminator literal for the same variant without seam remap row — attribution targets projection lattice owner, not domain `match`.
- Identical token sets maintained separately on ingress model, msgspec struct, domain enum, OpenAPI enum array, and test fixture dict — collapse to one vocabulary owner with generative schema export.
- Parallel unconstrained `Literal` band and `StrEnum` for the same concept without boundary adapter justification — collapse to one primary surface with documented remap at ingress or proof layer.
- Handler map keyed by one enum invoked with members from another enum class because `.value` strings collide — collapse ingress admission to owning enum type.
- `exclude_unset=True` conflated with PEP 661 parameter omission or pydantic experimental `MISSING` tri-state — collapse to axis-specific contract row per field.
- Post-import enum member injection or runtime `Enum` mutation — collapse to import-time owner registration before root `frozendict` publish.
...
- `from builtins import sentinel` on Python `>=3.15` with one module-global binding `NAME = sentinel("NAME")` where variable name matches sentinel name — static checkers emit error on name mismatch per PEP 661; carry in union type (`T | MISSING` or `T | type(MISSING)`), compare with `is`, never `==`; each semantic absence value binds exactly once per module — repeated `sentinel("NAME")` calls produce distinct objects and break identity law.
- PEP 661 sentinels are truthy, support `__or__`/`__ror__` for runtime `typing.Union` construction, and `__copy__` returns self — boolean guards (`if value:`) are defects; narrowing uses `is` / `is not` exclusively; until `>=3.15`, `typing_extensions.sentinel` backport must re-export `builtins.sentinel` when available.
- Pickle and copy preserve sentinel identity only when the sentinel is importable by module and attribute name; module-level binding is mandatory for persistence and multiprocessing; wire replay uses token admission, not sentinel reconstruction from bytes or stored `repr` strings.
