...
[INTEGRATION_STACK]:
- mesh seam: the `OcctMesh` (`Vertices`/`TriangleIndices`) maps at the boundary to the `geometry3Sharp` `DMesh3` (`api-geometry3sharp`) and the kernel `Rasm` mesh — plan-cs boundary-maps the triangle soup, so a STEP-imported solid flows `OcctShape.ImportStep` → `Triangulate` → `OcctMesh` → `DMesh3`/kernel mesh for the planar-section (`Toolpath/slicing` `MeshPlaneCut`) and HLR projection (`Posting/projection`) rails; the OCCT handle never crosses, only the mesh
...
- arc-offset seam: a planar section of the tessellated `OcctMesh` produces line-only contours (the mesh has no curvature); when the section needs arc-comp toolpaths it is re-fit through `g3.BiArcFit2` (`api-geometry3sharp`) or carried as bulge into `CavalierContours` (`api-cavaliercontours`) — the OCCT path is line-sourced, so the biarc refit applies here (unlike a natively-bulge `ACadSharp` arc)
...
- persistence: a STEP/IGES export (`ExportStep`/`ExportStl`) and the imported solid's content hash flow to `Rasm.Persistence/Schema` as a content-addressed durable artifact alongside the `CutProgram` AST, the export file the durable manufacturing-geometry record
...
- Reject: a leaked (undisposed) `OcctShape`/`OcctPoint3d`; passing a kernel `Rasm` geometry type into the OCCT ABI; seeking an assembly/XCAF/color/PMI reader from this package (managed-unbound — `Posting`/`Bim` own that elsewhere or it is out of scope); seeking HLR from OCCT (`libTKHLR` ships but is unbound — `Posting/projection` owns HLR); reading the raw `.Native.OcctStatus` enum instead of catching `OcctException`; loading this wrapper into an in-Rhino net48 plugin ALC (the native toolkits are process-global)
