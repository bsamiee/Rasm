...
- Offline custody therefore reaches the transport, never the exporter's persistence: `OtlpExporterOptions.HttpClientFactory` installs a `DelegatingHandler` over a directly-constructed `FileBlobProvider`, keeping queue policy, replay cadence, and typed refusals under exactly one persistence owner per exporter — the exporter env pair stays unset wherever this leg is selected.
