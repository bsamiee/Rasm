...
const byFacets = Order.mapInput(Order.number, (s: Signature) => HashSet.size(s.facets));
const byCardinality = Order.mapInput(Order.number, Signature.$match({ Nominal: () => 1, Reference: () => 1, Structural: (s) => s.cardinality }));
const byChannels = Order.mapInput(Order.number, Signature.$match({ Nominal: () => 0, Reference: () => 0, Structural: (s) => s.channels }));
const parsimony = Order.reverse(Order.mapInput(Order.number, (s: Signature) => s.declarations));
const family = Match.type<Signature>().pipe(Match.withReturnType<Family>(),
    Match.discriminatorsExhaustive('_tag')({ Nominal: () => 'Literal', Reference: () => 'taggedEnum', Structural: () => 'TaggedClass.Union' }));
const route = (demanded: HashSet.HashSet<Facet>, pool: ReadonlyArray<Signature>): Option.Option<{ readonly elected: Family; readonly handSpell: HashSet.HashSet<Facet> }> =>
    pipe(pool, Array.filter((c) => HashSet.isSubset(demanded, c.facets)),
        Array.sort(Order.combineAll([byFacets, byCardinality, byChannels, parsimony])), Array.last,
        Option.map((s) => ({ elected: family(s), handSpell: HashSet.difference(demanded, s.facets) })));
```
...
- Boundary: end the chooser `*Exhaustive` with `Match.withReturnType` first pinning the codomain; `orElse` carries no `extends never` constraint and silently absorbs a widening.
...
- Use: `Self.extend<Sub>("SubId")({...newFields})` for structural widening — `Proto` survives, `instanceof Parent` holds, and a parent-versus-new key collision throws inside `extendFields` at module-load.
...
class Interval extends Schema.Class<Interval>('Interval')(
    Schema.Struct({ lower: Schema.Number, upper: Schema.Number, sample: Schema.Number }).pipe(
        Schema.filter((s): Schema.FilterIssue | undefined =>
            s.lower > s.upper ? { path: ['upper'], message: '<upper-below-lower>' }
                : s.sample < s.lower || s.sample > s.upper ? { path: ['sample'], message: '<sample-out-of-bounds>' }
                : undefined),
    ),
) {
    get span(): number { return this.upper - this.lower; }
}
class WeightedInterval extends Interval.extend<WeightedInterval>('WeightedInterval')({
    weight: Schema.optionalWith(Schema.Positive, { default: () => 1 }),
}) {}
const admit = Schema.decodeUnknown(WeightedInterval);
```
...
[DECLARATION_MERGING]:
- Use: a `namespace X` grows the value side and an `interface X` widens the type side through the single imported name, deleting a companion `const XStatics`, a mirror `interface XShape`, and a `type X = typeof X` re-export.
- Law: order is class-then-namespace because `const X = Schema.Struct(...)` binds only the value namespace, so a following `namespace X` is a redeclaration error and a namespace above its class reads a TDZ binding.
- Boundary: free-static availability is per-family — `Data.Class`/`Data.TaggedClass` reserve nothing, `Schema.Class` walls `make`/`fields`/`extend`/`annotations`/`ast`, `Effect.Service` walls `Default`/`make`/`use`/`key` plus every `accessors: true` method name, and `Data.taggedEnum` has no merge target (later author-attached seeds forcing a `Schema.Union` of `TaggedClass`).
...
```typescript conceptual
class ResolveFault extends Schema.TaggedError<ResolveFault>()('ResolveFault', { key: Schema.String }) {}
class Resolve extends Schema.TaggedRequest<Resolve>()('Resolve', {
    payload: { key: Schema.NonEmptyString }, success: Schema.Number, failure: ResolveFault,
}) {}
class Count extends Schema.TaggedRequest<Count>()('Count', {
    payload: { prefix: Schema.String }, success: Schema.Int, failure: Schema.Never,
}) {}
const Query = Schema.Union(Resolve, Count);
const dispatch = (request: typeof Query.Type): Effect.Effect<void> =>
    Match.value(request).pipe(Match.tagsExhaustive({
        Resolve: (arm) => Request.completeEffect(arm, arm.key.length > 0 ? Effect.succeed(arm.key.length) : Effect.fail(new ResolveFault({ key: arm.key }))),
        Count: (arm) => Request.completeEffect(arm, Effect.succeed(arm.prefix.length)),
    }));
```
...
class Reading extends Schema.Class<Reading>('Reading')({
    station: Schema.String, captured: Schema.Number, magnitude: Schema.Number,
}) {}
const keys = ['station', 'captured', 'magnitude'] as const satisfies ReadonlyArray<keyof Reading>;
const sameReading = Schema.equivalence(Reading);
const byPrecedence = Struct.getOrder({
    station: Order.string, captured: Order.reverse(Order.number), magnitude: Order.number,
} satisfies { readonly [K in keyof Reading]: Order.Order<Reading[K]> });
const canonicalize = (rows: ReadonlyArray<Reading>): HashMap.HashMap<number, Reading> =>
    pipe(rows, Array.sort(byPrecedence), Array.dedupeWith(sameReading),
        Array.map((r) => [Hash.structureKeys(r, keys), r] as const), HashMap.fromIterable);
```
...
```typescript conceptual
const Scope = Schema.Literal('read', 'write', 'admin');
const Subject = Schema.Literal('user', 'doc', 'org');
const Code = Schema.TemplateLiteral(Scope, '.', Subject);
type Row = { readonly audited: boolean; readonly quorum: number };
const Policy = {
    'read.user': { audited: false, quorum: 0 }, 'read.doc': { audited: false, quorum: 0 }, 'read.org': { audited: false, quorum: 1 },
    'write.user': { audited: true, quorum: 1 }, 'write.doc': { audited: true, quorum: 1 }, 'write.org': { audited: true, quorum: 2 },
    'admin.user': { audited: true, quorum: 2 }, 'admin.doc': { audited: true, quorum: 2 }, 'admin.org': { audited: true, quorum: 3 },
} as const satisfies Record<typeof Code.Type, Row>;
const Decomposed = Schema.TemplateLiteralParser(Scope, '.', Subject);
const weigh = ([scope, , subject]: typeof Decomposed.Type): number =>
    Match.value(scope).pipe(Match.when('admin', () => 4), Match.when('write', () => 2), Match.when('read', () => 1), Match.exhaustive) *
    Match.value(subject).pipe(Match.when('org', () => 3), Match.when('doc', () => 2), Match.when('user', () => 1), Match.exhaustive);
```
...
class Account extends Model.Class<Account>('Account')({
    ...
    secret: Model.Sensitive(Schema.Redacted(Schema.NonEmptyString)),
    profile: Model.JsonFromString(Schema.Struct({ displayName: Schema.String })).pipe(Model.fieldFromKey({ select: 'profile_json', update: 'profile_json' })),
    role: Model.Field({ select: Schema.String, insert: Schema.String, update: Schema.String }),
    createdAt: Model.DateTimeInsert,
    updatedAt: Model.DateTimeUpdate,
}) {}
const { select, insert, update, json, jsonCreate, jsonUpdate } = Account;
```
