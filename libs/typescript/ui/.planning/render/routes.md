...
- [1]-[OBSERVATION_ROUTES]: the `EvidenceTimelineRoute` HLC-ordered receipt timeline with `CausalVerdict` `SkewBand` confidence rendering, the `BenchmarkRoute` fingerprint-gated dashboard, and the `CollectorPanel` telemetry read surface.
