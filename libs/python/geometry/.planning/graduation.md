...
- Contract: the compute campaign inherits this fence's pin as frozen wire data with geometry the named demanding consumer — the `SUBJECTS` tuple (derived from the union, thirteen literals), the admission shape `graduates(owner, HandoffAxis(geometry=subject), key, measured, ceiling)` compute's fold decodes the carrier into, and the residual-over-ceiling `_admit` direction (`measured[name] <= ceiling[name]` for every ceiling row). The pin is contract DATA stated once in the fence's contract block; `HandoffAxis` and `GraduationReceipt` appear in no geometry prelude, an out-of-union subject rails typed on the compute side, and a geometry union change ships a compute ripple, never a silent admit.
