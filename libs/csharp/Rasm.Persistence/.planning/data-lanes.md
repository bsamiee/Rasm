...
- [DUCKDB_FAULT_KEY]: the `DuckDBAppender.Clear` abort-state contract on a partially-appended batch under a live engine — whether `Clear` resets the appender to a re-appendable state or seals it, exercised against a mid-batch native fault.
