...
- `RocksDbException(nint errptr) : RocksDbSharpException` marshals the errptr string and exposes no stable typed status beyond `Exception`; consumers preserve the exact exception and never infer recovery from `Message`.
