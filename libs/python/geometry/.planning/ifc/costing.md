...
if TYPE_CHECKING:  # companion-band: the model annotations resolve here; every runtime ifcopenshell use is a function-local `import ifcopenshell  # noqa: PLC0415` at boundary scope so the cp315 module loads clean (the selector sibling shape)
    import ifcopenshell
