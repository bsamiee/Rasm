...
| :-----: | :------------------------------------ | :----------- |
|  [01]   | `Optional<T>` family                  | `[01]`       |
|  [02]   | `OptionalDouble`                      | `[02]`       |
|  [03]   | `*Vector` family                      | `[03]`       |
|  [04]   | `Path` / `OptionalPath`               | `[04]`       |
|  [05]   | `Point3d` / `OpenStudioUtilitiesCore` | `[05]`       |

- [01]: the `boost::optional` marshaling — `OptionalDouble`/`OptionalModel`/`OptionalWorkspace`/`OptionalConstruction`/…; `is_initialized()`/`isNull()`, `get()`, `value_or(T)`. Every "load"/"get"/result accessor that can miss returns one — check `is_initialized()` before `get()` (a bare `get()` on an empty optional faults in native code)
- [02]: the `boost::optional<double>` the `SqlFile` annual accessors return (`: IDisposable`) — the missing-output carrier lowered to `Option<double>` at the result edge
- [03]: the `std::vector` marshaling — `Point3dVector` (`: IEnumerable<Point3d>`, the surface-geometry vertex carrier), `MaterialVector` (`: IEnumerable<Material>`, the layered-construction layer list), `LogMessageVector` (the translator warning/error sink), `SurfaceVector`/`SpaceVector`/…; `IList`-shaped (indexer, `Count`, `Add`)
- [04]: the `boost::filesystem::path` wrapper; there is NO `Path(string)` ctor — build from a string with the static `OpenStudioUtilitiesCore.toPath(string)`, read back with `OpenStudioUtilitiesCore.toString(path)`. The REQUIRED carrier at every file API (`IdfFile`/`SqlFile`/`EpwFile`/`Workspace.save`) — a raw `string` is not accepted
- [05]: the surface-vertex value (`: IDisposable`) the `Point3dVector` carries; `OpenStudioUtilitiesCore` the SWIG global-function host (`toPath`/`toString`/`createUUID`) the file boundaries route through, distinct from the `*PINVOKE` shims
...
| :-----: | :------------------------------------------------ | :----------- |
|  [01]   | `Model`                                           | `[01]`       |
|  [02]   | `Space` / `Building`                              | `[02]`       |
|  [03]   | `Surface`                                         | `[03]`       |
|  [04]   | `ThermalZone`                                     | `[04]`       |
|  [05]   | `Construction` / `LayeredConstruction`            | `[05]`       |
|  [06]   | `StandardOpaqueMaterial`                          | `[06]`       |
|  [07]   | `StandardGlazing`                                 | `[07]`       |
|  [08]   | `WeatherFile` / `SimulationControl` / `RunPeriod` | `[08]`       |

- [01]: the OSM model (`: Workspace`); ctor `Model()`; the root every model-object hangs off and the `EnergyPlusForwardTranslator` input — built here from the graph, never loaded from an `.osm` (that is Bim's exchange concern)
- [02]: `Space : PlanarSurfaceGroup` (`new Space(model)`) the conditioned-volume each spatial Object node folds into; `Building : ParentObject` the model root container
- [03]: `new Surface(Point3dVector vertices, Model model)` — the bounding surface each Object-node face becomes; carries its `Construction` and is owned by the model
- [04]: `ThermalZone : HVACComponent` (`new ThermalZone(model)`) the zone each `Space` is assigned to — the EnergyPlus thermal-balance unit
- [05]: `Construction : LayeredConstruction` (`new Construction(model)`); `setLayers(MaterialVector)` orders the layered-material stack the seam `MaterialComposition.LayerSet` lowers to
- [06]: `: OpaqueMaterial : Material`; ctor `StandardOpaqueMaterial(Model model, string roughness, double thickness, double conductivity, double density, double specificHeat)` — the opaque layer built from the seam thermal properties so the OSM U-value matches the `Analysis/aggregator` ISO 6946 fold
- [07]: `: Glazing : ...`; the glazing layer for a transparent construction (the seam glazing thermal/optical properties)
- [08]: `WeatherFile : ModelObject` (the site/design-day context the `EpwFile` attaches to), `SimulationControl`/`RunPeriod : ParentObject` (the annual-run period and control flags)
...
| [INDEX] | [SYMBOL]                                   | [CAPABILITY] |
| :-----: | :----------------------------------------- | :----------- |
|  [01]   | `EnergyPlusForwardTranslator`              | `[01]`       |
|  [02]   | `Workspace`                                | `[02]`       |
|  [03]   | `IdfFile` / `IddFileType`                  | `[03]`       |
|  [04]   | `EpwFile`                                  | `[04]`       |
|  [05]   | `SqlFile`                                  | `[05]`       |
|  [06]   | `LogMessageVector` / `Logger` / `LogLevel` | `[06]`       |

- [01]: `: IDisposable`; `translateModel(Model)` → `Workspace` (the IDF for EnergyPlus); `errors()`/`warnings()` → `LogMessageVector` for the translator receipt — the ONE translator this runner uses (the reverse/gbXML/SDD/three.js translators are the Bim exchange concern)
- [02]: `: IDisposable`; the generic IDD-object store the IDF is; `save(Path, bool overwrite)` writes the IDF to the scratch run directory
- [03]: the EnergyPlus IDF text model and its IDD schema key (read back when post-processing a foreign IDF)
- [04]: `: IDisposable`; `EpwFile(Path)` the EnergyPlus weather file attached to the `WeatherFile` and passed to the subprocess (`-w`); decompile-verified site accessors `latitude()`/`longitude()`/`timeZone()`/`elevation()` → `double` and `data()` → the `EpwDataPoint` vector whose rows carry `directNormalRadiation()`/`diffuseHorizontalRadiation()`/`globalHorizontalRadiation()` → `OptionalDouble` — the `Analysis/daylight` `WeatherIngress` reader
- [05]: `: IDisposable`; `SqlFile(Path)` the EnergyPlus results SQLite; the annual accessors `totalSiteEnergy()`/`netSiteEnergy()`/`totalSourceEnergy()`/`netSourceEnergy()` → `OptionalDouble`, the end-use `electricityHeating()`/`electricityCooling()`/`electricityInteriorLighting()`/`electricityTotalEndUses()`/`naturalGasHeating()`/… → `OptionalDouble`, the per-month `energyConsumptionByMonth(EndUseFuelType, EndUseCategoryType, MonthOfYear)`/`peakEnergyDemandByMonth(…)` → `OptionalDouble` — the post-run result reader
- [06]: the native log sink; the translator `warnings()`/`errors()` returns `LogMessageVector` — fold into the Compute receipt fact stream, never the console
...
| :-----: | :--------------------------------------------------------------------------------------------- | :----------- |
|  [01]   | `new Model()`                                                                                  | `[01]`       |
|  [02]   | `new Space(model)` / `new ThermalZone(model)`                                                  | `[02]`       |
|  [03]   | `new Surface(vertices, model)`                                                                 | `[03]`       |
|  [04]   | `new Construction(model)` + `setLayers(MaterialVector)`                                        | `[04]`       |
|  [05]   | `new StandardOpaqueMaterial(model, roughness, thickness, conductivity, density, specificHeat)` | `[05]`       |

- [01]: `()` → `Model` — the empty OSM model the graph is folded into (one `using` owner)
- [02]: `(Model)` → owned model object — a conditioned volume / a thermal zone per spatial Object node
- [03]: `(Point3dVector, Model)` → `Surface` — a bounding surface from the Object-node face vertices
- [04]: `(Model)` then `(MaterialVector)` — a layered construction from the seam `MaterialComposition.LayerSet`
- [05]: `(Model, string, double×4)` → layer — an opaque layer from the seam thermal properties (the ISO 6946 U-value match)
...
| [INDEX] | [SURFACE]                                          | [CAPABILITY] |
| :-----: | :------------------------------------------------- | :----------- |
|  [01]   | `new EnergyPlusForwardTranslator().translateModel` | `[01]`       |
|  [02]   | `idf.save`                                         | `[02]`       |
|  [03]   | `new EpwFile`                                      | `[03]`       |

- [01]: `(Model)` → `Workspace` — OSM → EnergyPlus IDF; then `errors()`/`warnings()` for the receipt
- [02]: `(OpenStudioUtilitiesCore.toPath(idfPath), overwrite: true)` → `bool` — persist the IDF to the scratch run directory
- [03]: `(OpenStudioUtilitiesCore.toPath(epwPath))` → `EpwFile` — the weather file passed to the subprocess
...
| [INDEX] | [SURFACE]                                                                     | [CAPABILITY] |
| :-----: | :---------------------------------------------------------------------------- | :----------- |
|  [01]   | `EnergyToolchain.Resolve` → `Process` `energyplus -w <epw> -d <out> -r <idf>` | `[01]`       |
|  [02]   | `new SqlFile`                                                                 | `[02]`       |
|  [03]   | `sql.totalSiteEnergy()` / `electricityHeating()` / `electricityCooling()`     | `[03]`       |

- [01]: `(EnergyToolchainPolicy)` → `Fin<string>` then the subprocess — OpenStudio does NOT run EnergyPlus — the resolved binary subprocess does (the parameterized discovery boundary)
- [02]: `(OpenStudioUtilitiesCore.toPath(sqlPath))` → `SqlFile` — open `eplusout.sql` after the run
- [03]: `()` → `OptionalDouble` — the annual site energy / end-use demands — lower `OptionalDouble` to `Option<double>`, derive EUI = total ÷ conditioned floor area
...
- with `Rasm.Persistence`: the saved IDF/SQL artifacts (UTF-8/SQLite bytes) feed the content-keyed `XxHash128` artifact index (`libs/csharp/.api/api-hashing.md`) on the same content-identity rail as every other Compute artifact.
