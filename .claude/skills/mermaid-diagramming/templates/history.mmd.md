...
Refill by renaming branches and commit ids to repository truth — ids stay unique, a branch predates its checkout, and a cherry-picked merge names its `parent:`. 2px rails, scaled dots, hollow merge rings, recessed commit chips, and gold tag chips are fixed law — a refill renames history, never strips the fidelity surface.
