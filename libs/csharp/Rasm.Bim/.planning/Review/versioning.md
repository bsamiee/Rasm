...
public sealed record BimRepository(Map<ContentAddress, BimCommit> Commits, Map<string, BimBranch> Branches) {
    ...
    // The ONE seal both Commit and CommitMerge funnel through, so the composing rail has exactly one edge to mint
    // Exchange/events#EVENTS BimEvent.CommitLanded from; the unchanged-model no-op returns the head without reaching
    // here, which is what makes the event stream free of no-change rows.
