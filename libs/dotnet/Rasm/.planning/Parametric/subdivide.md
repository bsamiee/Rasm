...
- Packages: `Rasm.Numerics` for the sparse operators, the `Dimension` level budget, and the Stam EVD (`Matrix.DecomposeEigenDetailed`) with its retained eigenvector factor (`Matrix.DecomposeLu`, `LuResult.SolveDetailed`, `LinearSolution`), `Rasm.Meshing` for the `MeshSpace` quad publish and `MeshEdit` tri arena, `Rasm.Domain` for `Op`/`Cell`/`Transition`/`ValidityClaim` (tolerance is `MeshSpace.Tolerance`, never a second `Context` column), Rhino.Geometry for the native quad types, Thinktecture.Runtime.Extensions for `[SmartEnum]` delegate columns, LanguageExt.Core for the `Fin` result, the `Validation` admission fan-in, and the `Atom` cache cell, and BCL `ArrayPool<double>` for level staging.
