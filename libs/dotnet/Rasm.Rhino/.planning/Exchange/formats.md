...
- Packages: `Exchange/operations` (`ExchangeFault`), `Domain/results` (`Op`), LanguageExt.Core (`Atom`, `HashMap`), RhinoCommon (`FileImportPlugIn`, `FileExportPlugIn`, `FileTypeList.AddFileType`, `WriteFileResult`) per `.api/api-rhinocommon-plugins.md`.
