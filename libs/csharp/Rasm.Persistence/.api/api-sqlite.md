...
[INTEGRATION_STACK]:
- `Store/engine#MAINTAIN`/`#REGISTER`/`#BACKUP`/`#WITH_SNAPSHOT` are the boundary capsules over this ADO + raw-interop ceremony: the `Maintain` `$destination` bind pins `SqliteType.Text` on a typed `SqliteParameter` (no `AddWithValue` inference); blob payloads stream through the constructed `SqliteBlob` write stream and the `GetStream` read path (whole-payload `byte[]` materialization is the deleted pattern); paged backup steps `sqlite3_backup_*` over `Handle` rather than the whole-file `BackupDatabase`
- `Store/engine#REGISTER` registers `uuid7` and `xxh128` scalar UDFs and an `instant_iso` collation via `CreateFunction`/`CreateCollation`, so the SQLite leg of the identity policy and chronological `ExtendedIso` ordering live as connection registrations
- `Store/engine#EXTENSION_LOAD` arms `EnableExtensions` for the db_config loader path only and resolves artifacts through the OS loader (`LoadExtension`), with the single-RID `e_sqlite3` dylib set from `api-sqlitepcl`
