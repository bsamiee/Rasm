...
- [01]-[EMBEDDING]: the `VectorEncoding` encoding axis, the `VectorScore` metric axis, the `EmbeddingVector` content-keyed carrier, the `QuantizationPolicy` budget, and the `Score`/`Encode`/`Rank` fold over the `TensorPrimitives` SIMD surface crossing to the Persistence vector lane by reference.
