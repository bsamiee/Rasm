namespace Radyab.Components;

[IoId("F51A09A8-A5A5-467A-ADBA-C950511A0020")]
[Nomen(
    name: "Extract Surfaces",
    info: "Trimmed faces, direction-ranked top/bottom faces, indexed face details, and centroid UV frame for Brep, BrepFace, Surface, and SubD values.",
    category: "Radyab",
    section: "Extraction")]
public sealed class ExtractSurfaces : Component<ExtractSurfaces> {
    [Input] private readonly Port<Shape> Geometry = Port.Shape();
    [Input] private readonly Port<int> Index = Port.Index(info: "Zero-based face selector; missing Index defaults to face 0 and supplied values clamp to [0, count-1].");
    [Input] private readonly Port<Vector3d> Direction = Port.Optional<Vector3d>(name: "Direction", code: "D", info: "Ranking direction for Top/Bottom surfaces; missing Direction uses world Z.");
    [Output] private IOutputGroup AllSurfaces => Output.FaceDetails(input: Geometry, faces: Port.List<Brep>(name: "All Surfaces", code: "AS", info: "Every face as a trimmed single-face Brep. Mesh input is intentionally rejected."), indices: Port.List<int>(name: "Surface Indices", code: "SI", info: "Source Brep face index aligned with every extracted surface."), selector: static (_, _) => Faces.All);
    [Output] private IOutputGroup TopSurface => Output.Query(input: Geometry, port: Port.List<Brep>(name: "Top Surface", code: "TS", info: "Trimmed face(s) with maximum centroid projection along Direction; ties within tolerance."), operation: (access, runtime) => Rasm.Analysis.Query.Faces<object, Brep>(aspect: Faces.Top(axis: runtime.Value(access: access, port: Direction).IfNone(Vector3d.ZAxis))));
    [Output] private IOutputGroup BottomSurface => Output.Query(input: Geometry, port: Port.List<Brep>(name: "Bottom Surface", code: "BS", info: "Trimmed face(s) with minimum centroid projection along Direction; ties within tolerance."), operation: (access, runtime) => Rasm.Analysis.Query.Faces<object, Brep>(aspect: Faces.Bottom(axis: runtime.Value(access: access, port: Direction).IfNone(Vector3d.ZAxis))));
    [Output] private IOutputGroup IndexedFace => Output.IndexedFaceDetails(input: Geometry, index: Index, faces: Port.List<Brep>(name: "Indexed Surface", code: "IS", info: "Trimmed single-face Brep at Index input; missing Index defaults to 0, supplied values clamp to [0, count-1]. Empty when zero faces."), frames: Port.List<Plane>(name: "UV Frame", code: "UV", info: "Native U/V frame at the indexed face centroid. X=surface U direction, Z=orientation-corrected normal, Y completes the basis."), centers: Port.List<Point3d>(name: "Face Center", code: "FC", info: "Area centroid of the indexed trimmed face."), normals: Port.List<Vector3d>(name: "Face Normal", code: "FN", info: "Orientation-corrected indexed face normal at the face centroid."), indices: Port.List<int>(name: "Face Index", code: "FI", info: "Source Brep face index selected by the clamped Index input."), components: Port.List<ComponentIndex>(name: "Face Component", code: "CI", info: "Source Brep face component index selected by the clamped Index input."), domains: Port.List<Interval>(name: "UV Domains", code: "UD", info: "Indexed face domains as two intervals: U first, then V."));
}
