...
`Transform`, `BoundingBox`, `Point3d`/`Vector3d`/`Plane`/`Line`/`Box`, and `TransformSimilarityType` register from the substrate `libs/dotnet/.api/api-rhinocommon.md`.
...
`ArchivableDictionary` registers from the persistence catalog `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md`.
...
`GeometryBase.Transform(Transform)` and the `GetBoundingBox(bool)`/`GetBoundingBox(Transform)`/`GetBoundingBox(Plane, out Box)` bounds triple register from the substrate `libs/dotnet/.api/api-rhinocommon.md`; `IsDocumentControlled` registers from `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-runtime.md` on the `CommonObject` base, and `GeometryBase` re-declares the user-string roster `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md` owns.
