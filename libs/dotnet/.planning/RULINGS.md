...
- Contract property-bag row names are single-owned by their `Rasm.Element` declarer — a call-site `PropertyName.Create` forks the bag's key space.
...
- Material occurrence-usage rides the shared `Associate` edge's typed `MaterialUsage` payload — a parallel usage node double-owns the fact.
...
- Typed results a consumer boundary cannot carry outward park on the composing app's evidence cell — a `void` delegate licenses no discard.
...
- Hook censuses freeze at the composition's ONE `HookRegistry.Mount` — a contributing folder hands its `Points` in and calls nothing itself.
...
- Library tiers classify retriability and execute none — the root-bound executor drives, so a tier-local policy forks the solution's one schedule.
- Store bands seat executors by BOUNDARY — `RemoteStoreFault` and `CacheFault` at `OutboundHop`, `CoordinationFault` at the store strategy.
- `HopOutcome` is transport-neutral, so its exception arms alone are HTTP-shaped — a store driver joins the keyed lane as a row, never a second family.
...
- `SolutionAudit` is `Rasm.Grasshopper`'s completion-audit noun and `RunEvidence` is `Rasm.Fabrication`'s — one name means one thing solution-wide.
...
- `KernelInstrument` owns every `rasm.fault.*` key — an emitter prefixing its own project segment forks one axis into a per-package pair.
...
- `CanonicalWriter` at `Rasm/Domain/identity` is the ONE preimage writer — peers compose it and re-export nothing, so one framing keys the solution.
- `HookSet<TPoint,TFact,TOwner>` is the ONE hook mechanism — a folder declares its `<Package>Point` roster and closed fact union alone.
...
- MessagePack frames the op-log envelope; its `crdt` slot carries generated `CrdtOpWire` across the registered process boundary.
...
- Projection boundary rows enter an `ARCHITECTURE.md` boundaries map only on a live consumer fence — the row re-enters the moment a consuming page names it.
- `CapabilitySet<TCapability>.Wire` orders by ordinal key — semantic precedence is an explicit column on the domain owner that consumes it, never declaration position.
- Boundary edges collapse to ONE per home owner, counterpart, kind, and direction — members join ` + `; a both-ends mint keeps its own arrow.
- Boundary labels name shapes DECLARED at the producing package's own fences — a substrate or BCL type sharing the spelling rides no boundary edge.
- Boundary labels name a member the CONSUMING package spells at its own fence too — a producer declaration alone registers reach, never a crossing.
- Host-boundary ends DERIVE — the boundary end keys (kernel owner, sub-domain, kind) and the kernel end joins those members alphabetically.
