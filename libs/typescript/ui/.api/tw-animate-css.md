...
- `act/transition` native View Transitions (`.api/types-react.md` `ViewTransition`): the alternative at the same boundary, chosen XOR tw-animate — native cross-document or shared-element transition versus keyframe enter/exit, never layered on one element.
