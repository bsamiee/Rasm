...
- Boundary: `band.Settle` is one lane's cooperative-then-forced drain owned whole by the concurrency surface's participation law — the band walk composes it as an opaque callback and owns only the phase-cell advance, the `Share` budget split, and the last-band receipt pin; the per-lane token mechanics and the serialisable drain permit are settled law, never re-derived here.
