...
- `System.IO.Hashing`(`libs/csharp/.api/api-hashing.md`): hash the artifact once with `XxHash3` for the content-key, then pass the SHA-256/384 digest straight to `VerifyDigestAsync(digest, HashAlgorithmType.Sha256, …)` so verify reuses the digest instead of rereading the stream.
