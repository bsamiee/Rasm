...
- Packages: `Process/owner` (`FabricationPolicy.Additive`, `FabricationInput`, `FabricationResult.AdditiveResult`); `Process/faults` (`FabricationFault`, `Admission`); `Additive/support` (`SupportPlan`, `SupportPolicy`); `Additive/scanpath` (`ScanPolicy`); `Additive/production` (`AdditiveBuild`, `BuildJob`, `BuildOutcome`); `Additive/implicit` (`ImplicitOp`); kernel `Rasm.Domain` (`AdmissionSlots`); `Rasm.Meshing` (`LayerPlan`, `SlicePolicy`, `OffsetPolicy.Of`); `Rasm.Domain` (`Context.Canonical`/`Override`, `ToleranceLane.Arc`); `Rasm.Numerics` (`PositiveMagnitude`).
