...
class KeyedThread(Struct, frozen=True):
    # the Gate-#3 host-wire thread a keyed node states so the AppHost boundary can mint the Persistence
    # `Admit(kind, key, bytes, classification, at, sourceKey)` row post-drain: `classification` the
    # `DataClassification` category-plus-name wire tag ("" = the host `NoDataClassification` default),
    # `source` the source-artifact key the index's `sourceKey` family projection groups by.
    classification: str = ""
...
# `keyed` optionally carries the `KeyedThread` host-wire thread; `Admission(keyed=None)` stays the default.
...
class PipelinePlan(Struct, frozen=True):
    ...
    # the host-wire projection: each keyed node's stated `KeyedThread`, read beside the drain receipts
    # so the AppHost boundary mints the Persistence blob-index row without re-probing the node set.
...
class ArtifactPipeline(Struct, frozen=True):
    ...
    lane: LanePolicy = _DEFAULT_LANE
    # the elision seed `of(warm=)` threads — in-session receipts now; the durable cross-run fill is
    # HOST-WIRE GATED on the AppHost py↔Persistence boundary filling `warm` from the Persistence
    # artifact index (the wire does not yet exist — no silent close).
    cache_seed: Map[ContentKey, ArtifactReceipt] = Map.empty()
    ...
    async def _planned(self, /) -> PipelinePlan:
        ...
        # runtime thread lane — never the event loop, where a large producer-set
...
## [03]-[ADMISSION_LOWERING]

- Each node lowers through its OWN `lowered` admission `match` onto the lane `Admit` arm, covering all three cases the `runtime/execution#LANE` union declares — `keyed` → `Admit(keyed=(key, work))` the `(ContentKey, Work)` cache unit the lane's `keyed` row probes the threaded session cache with, `bare` → `Admit(bare=work)` the always-run unit, `retried` → `Admit(retried=(cls, work))` the transient-guarded unit re-spawning a worker cold-start under the `RetryClass` the case mandates — never a hardcoded single lowering, never a `_LOWER` table beside the case's own `match`.
- Selection law: `keyed` is the default (cache-eligible, the elision spine); `bare` is the forced-live non-addressable one-shot; `retried(RetryClass.OCCT)` is reserved for a WHOLE-producer transient — a native render/press/raster crossing a worker that can die. A `keyed` node with an internal offload-retry KEEPS `keyed` and rides `LanePolicy.offload(kernel, retry=RetryClass.OCCT)` inside its own `_emit`; retry binds only through the runtime `guarded` `POLICY` table — zero artifacts-minted limiters or the runtime retry class callers.
- Gate #3 thread: `Admission(keyed=KeyedThread(classification, source))` states the host-wire persistence thread; `Admission(keyed=None)` admits under the host default. The runtime `Admit` shape is UNCHANGED — the thread rides `PipelinePlan.threads`, the projection the AppHost boundary reads beside the drain receipts to mint the Persistence `Admit(kind, key, bytes, classification, at, sourceKey)` blob-index row (kind off the receipt tag, bytes/at off production, classification/sourceKey off the thread).

## [04]-[CONTENT_KEY_NODES]

- `core/issue` is THE constructing owner — the one composition root calling the terminal producers' `emit()`, folding the nodes into `ArtifactPipeline.of(nodes, lane=..., warm=..., targets=...)`, planning, and draining the CPM fronts through the runtime `LanePolicy.drain`. The plan imports no producer (the `Work` thunk carries the call), so no producer→plan→producer cycle forms; nothing in artifacts imports `issue`.
- Key-over-input: `ArtifactWork.key` mints over the producer's frozen request/spec owner's canonical bytes ⊕ its parent keys (`ContentIdentity.of` under `CANONICAL_POLICY`) — deterministic and computable BEFORE `work` runs, so `keyed` admission probes the `of(warm=)`-seeded `cache_seed` and the lane short-circuits before production; `_emit` threads the same key into its terminal receipt (`receipt.slot == node.key`, the hit/miss match). A separate output content-address rides a receipt evidence fact only where the store needs it (Gate #3); it is never the elision key.
- The 42-producer roster lands exactly this contract — `emit() -> ArtifactWork | Iterable[ArtifactWork]` per producer, a batch producer one node per member (per-member elision), a composite ONE aggregate node whose `parents` are its member keys: document `emit`/`report`/`egress`/`lens`/`tagged`; typography `font`/`shape`/`layout`/`math`; visualization `table`, `chart/export`, `diagram/draw`+`diagram/schematic`; scene `render`; graphic `marks/encode`, `raster/io`, `color/managed`; exchange `conformance`/`credential`/`metadata`; drawing `dimension`/`annotate`/`symbol`/`detail`/`schedule`; composition `compose`/`imposition`/`sheet`; specification `section`; delivery `register`/`transmittal`; export `dxf`/`layered`/`indesign`; media `container`/`timeline`/`subtitle`/`analysis`/`synthesis`; package `codec`/`archive`/`delta`. Substrate and vocabulary pages mint no node; `core/issue` mints no receipt case.
