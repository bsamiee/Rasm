...
// The branch ACL is the `Element/identity#AUTHORITY` `GrantSet` narrowed to the branch lane (`AclScope.Branch`) —
// the ONE object-authorization vocabulary, NOT the disjoint AppHost effect-gating `Capability` (a cross-stratum
// name the identity owner forbids sharing). `Movable` gates on a `GrantSet` demand the caller selects per operation
...
|  [04]   | branch grant           | `Element/identity` `GrantSet` lane (never AppHost `Capability`) | `Movable(actor, demand)` gates `Grant.Write`/`Merge`/`Rebase`/`ForcePush`; no parallel enum |
...
- Owner: `Hlc` the hybrid-logical-clock stamp the Marten event `Timestamp`, the changefeed projection, the CRDT merge, the commit cell, and the wire all read; `CrdtOpWire` the `[MessagePack.Union]` op encoding the `OpLogEntry.Payload` carries for `column-family=crdt` rows; `CrdtWire` the static codec owning the byte-canonical content key, the `Encode`/`Decode` pair through the package `PersistenceResolver`, and the `UntrustedData` restore-lane decode; `ParitySlot` the `[SmartEnum<string>]` corpus-leg axis carrying its producer-owner label; `ParityVector` the one frozen-fixture carrier (canonical byte-deriving input plus an `Option<UInt128>` digest that is the design-pin gap until host validation stamps it); `ContentParityCorpus` the static surface minting the Persistence leg of the `ONE_WIRE_FIXTURE_CORPUS` and reconciling a local corpus against the golden one.
...
public readonly record struct ParityVector(ParitySlot Slot, ReadOnlyMemory<byte> Canonical, Option<UInt128> Digest) {
    public UInt128 Pin() => Digest is { IsSome: true, Case: UInt128 digest } ? digest : XxHash128.HashToUInt128(Canonical.Span);
    public ParityVector Stamped() => this with { Digest = Some(XxHash128.HashToUInt128(Canonical.Span)) };
    public bool Holds(ParityVector pinned) => Slot == pinned.Slot && Pin() == pinned.Pin();
}

public static class ContentParityCorpus {
    public const long Seed = 0L;

    public static ParityVector Cell(Hlc cell) {
        var buffer = new ArrayBufferWriter<byte>(16);
        cell.WriteTo(buffer);
        return new ParityVector(ParitySlot.HlcCell, buffer.WrittenMemory.ToArray(), None);
    }
    ...
    public static ParityVector CommitPreimage(Seq<UInt128> parents, Seq<UInt128> opKeys, Hlc cell) {
        ...
        return new ParityVector(ParitySlot.CommitKey, buffer.WrittenMemory.ToArray(), None);
    }

    public static ParityVector Op(CrdtOp op) =>
        new(ParitySlot.CrdtOp, CrdtWire.EncodeCompanion(op).ToArray(), Some(CrdtWire.ContentKey(op)));

    public static Fin<ParityVector> Contribute(ParitySlot slot, ReadOnlyMemory<byte> canonical) =>
        slot.MintedHere
            ? Fin.Fail<ParityVector>(Error.New(8264, $"<parity-owner-mints:{slot.Key}>"))
            : Fin.Succ(new ParityVector(slot, canonical, Some(XxHash128.HashToUInt128(canonical.Span))));
    ...
    public static Validation<Error, Unit> Reconcile(HashMap<ParitySlot, ParityVector> local, HashMap<ParitySlot, ParityVector> golden) =>
        toSeq(golden).Traverse(slot => local.Find(slot.Key) is { IsSome: true, Case: ParityVector held } && held.Holds(slot.Value)
            ...
            : Validation<Error, Unit>.Fail(Error.New(8263, $"<parity-drift:{slot.Key.Key}@{slot.Value.Slot.Producer}>"))).As().Map(static _ => unit);
}
```
...
|  [05]   | parity corpus         | one `XxHash128` seed-zero convention  | `ParityVector` shape authorable now; `Digest` stamps on host validation |
