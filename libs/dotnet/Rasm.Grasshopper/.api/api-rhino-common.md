...
- `api-rhino-ui`(`libs/dotnet/.api/api-rhino-ui.md`): the registered value prompts settle the `PickerSpec` fast lane, their `bool` return with its `out`/`ref` channel lifting to `Fin<string>`/`Fin<double>` where `false` maps to `Fault.InvalidResult`
