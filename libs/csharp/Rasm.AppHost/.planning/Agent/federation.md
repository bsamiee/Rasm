...
- Owner: `FederationProjection` the static peer-to-descriptor inversion fold; the `Sessions` cell the held `McpClient` seats in per admitted server; `FederationRuntime` the held composition state the fold reads — the `FederationCatalog`, the session accessor, the live `PeerSchemas` id→schema map, and the `McpRuntime` the reused `CommandAIFunction` closes over.
...
public static partial class FederationProjection {
    public static IO<Seq<CapabilityDescriptor>> Project(FederationRuntime runtime, FederatedServer server) =>
        ...
        let rows = toSeq(tools)
        from _published in Publish(runtime, server, rows)
        select rows.Map(tool => Descriptor(server, tool));

    // Peer schemas publish ONCE per projection rather than once per tool, on the fold that already
    // carries the effect, so `Descriptor` is pure and reads pure. Re-projection OVERWRITES by design — a peer
    // may change its schema — so this commits rather than claims, and the verdict is READ: a bounded CAS that
    // spends its attempts leaves the id-to-schema map without this peer's contracts, and `SDK_CODEGEN` would
    // then pin the suite's own shape against peer rows it never saw.
    static IO<Unit> Publish(FederationRuntime runtime, FederatedServer server, Seq<McpClientTool> tools) =>
        tools.Map(tool => ($"federated.{server.Value}.{tool.Name}", JsonNode.Parse(tool.JsonSchema.GetRawText())!)) is var parsed
        && Cell.Commit(runtime.PeerSchemas,
               held => parsed.Fold(held, static (map, row) => map.AddOrUpdate(row.Item1, row.Item2)),
               Cell.SwapBudget) is Transition<HashMap<string, JsonNode>>.Contended spent
            ? IO.fail<Unit>(new FederationFault.TransportRejected($"peer-schema-contended:{spent.Attempts.Value}"))
            : IO.pure(unit);
...
public static class FederatedDispatch {
    // Compile answers the SAME `CommandBody` a native op answers, because the surface key is what the composed
    // dispatch seam routes on — a sentinel that could only ever refuse costs the federated rows every
    // guarantee the algebra carries.
    ...
    static IO<Fin<DispatchReceipt>> Hopped(FederationRuntime runtime, FederatedServer server, FederatedCall call) =>
        ...
        select Fin.Succ(new DispatchReceipt($"federated.{call.Server}", call.Tool, Duration.FromTimeSpan(span)));
...
- Entry: `ProjectResources(FederationRuntime runtime, FederatedServer server)` returns `IO<Seq<CapabilityDescriptor>>` — lists the peer's resources, prompts, and templates and folds each into a brokered descriptor, the same fold the tool projection runs extended with three more list-and-wrap arms; `Subscribe(FederationRuntime runtime, FederatedServer server, string uri, BindingSpec spec, ChannelWriter<ExternalValue> sink)` returns `IO<IAsyncDisposable>` — binds `McpClient.SubscribeToResourceAsync(uri, handler, options, ct)` registering the per-uri update handler at subscribe and returning the SDK unsubscribe handle, so a peer resource-change drains into the bounded lane as one `ExternalValue`.
...
- Receipt: each projected resource/prompt/template descriptor is one `CapabilityDescriptor` the `#FEDERATION_PROJECTION` `Federate` fold lands in the same per-peer snapshot the tool rows ride; each drained resource-update is one `ExternalValue` carrying its federated resource-descriptor key as the unit, which the `Wire/livewire#LANE_SUBSTRATE` lane consumer resolves, brokers, and mints one `CommandReceipt` from — this page writes the keyed value and mints no receipt of its own; no parallel federation-resource receipt.
...
- Boundary: WITHDRAWN this pass — `FederatedServerWire` and `FederatedDescriptorWire`, two declared faces with no producer, no manifest row, and no decoder in any branch (`rg` over `libs/typescript`, `libs/python`, `tests/contracts` returns zero for both). Federated descriptors already cross as `DiscoveryResultWire` rows under `federated.{server}.*` surface keys in the ONE capability catalog and a federated tool result already rides `ReceiptEnvelopeWire<ToolResultWire>`, so both withdrawn faces described a second catalog the page's own boundary named the deleted form.
...
public static partial class FederationProjection {
    ...
    static CapabilityDescriptor ResourceDescriptor(FederatedServer server, string uri) =>
        CapabilityDescriptor.Of(
            ...
            cost: CostModel.Constant(new MeterVector(HashMap((CostUnit.Calls, 1L)))),
