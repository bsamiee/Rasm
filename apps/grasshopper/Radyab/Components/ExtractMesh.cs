...
public sealed class ExtractMesh : Component {
    ...
    private static readonly IOutputGroup Validity = Output.Query(input: Geometry, port: Port.List<bool>(name: "Validity", code: "V", info: "[IsValid, IsClosed, IsOriented, IsSolid, IsManifold, IsBoundaryFree] per mesh; all six derive from one Mesh.IsManifold pass plus native validity properties."), aspect: Meshes.ValidityBundle);
