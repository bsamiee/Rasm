...
- `ShutLiningCurveInfo(Curve curve, double radius, int profile, bool pull, bool isBump, IEnumerable<Interval> curveIntervals = null, bool enabled = true)` — read-only `Curve : Curve`, `Enabled : bool`, `Radius : double`, `Profile : int`, `Pull : bool`, `IsBump : bool`, `CurveIntervals : ReadOnlyCollection<Interval>`; the per-profile carrier `Mesh.WithShutLining` consumes.
- `MeshDisplacementInfo(RenderTexture texture, TextureMapping mapping)` — read-only `Texture : RenderTexture`, `Mapping : TextureMapping`; `get/set` with constructor defaults: `Black : double` (0.0), `White : double` (1.0), `BlackMove : double` (0.0), `WhiteMove : double` (1.0), `MappingTransform`/`InstanceTransform : Transform` (Identity), `PostWeldAngle : double` (40.0), `RefineSensitivity : double` (0.5), `SweepPitch : double` (1000.0), `ChannelNumber : int` (1), `FaceLimit : int` (10000), `FairingAmount : int` (4), `RefineStepCount : int` (1), `MemoryLimit : int` (64, megabytes).
