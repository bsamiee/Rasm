...
Rasm agents use `tools/assay provision up|down|status|doctor|ports|inventory|extensions|plan|env|verify`. Assay accepts Forge schema-v2 JSON only and projects sanitized `ProvisionRun` evidence. Direct `rasm-provision`, `psql`, `paths`, `prune`, `self-test`, Docker/Compose, cleanup, and diagnostic JSON remain Forge-level debugging surfaces.
