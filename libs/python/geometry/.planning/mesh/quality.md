...
class QualityBackend(StrEnum):  # only the two tiers this owner dispatches for exact-versus-Euler genus
    SPINE = "spine"            # numpy Euler characteristic V−E+F=2−2g over the half-edge fold's unique-edge count, always-available cp315 core
...
# plain (mesh,) args (the lane never imports it). It builds the Manifold per hop, gates status() (the .api law:
# `Manifold(mesh)` sets a non-NoError status rather than raising on a non-2-manifold soup), and returns the picklable
# ExactTopology VALUE — a live Manifold cannot cross the boundary, so the capsule caches the computed value across
# metric passes, never a live _solid, the cross-pass reuse the mesh/spatial sibling names as this owner's.
...
- [TRIMESH_CONDITIONING]: the `Decimate`/`Subdivide`/`Smooth` arms bind the `.api`-confirmed entrypoints exactly — `Trimesh.simplify_quadric_decimation(face_count)` (`trimesh.md` smoothing/remesh row [14]) returns a new quadric-collapsed `Trimesh`, `remesh.subdivide_to_size(vertices, faces, max_edge)` (row [15]) returns refined vertices/faces, and `smoothing.filter_taubin`/`filter_laplacian`/`filter_humphrey` (rows [10]/[11]/[12]) mutate in place. The smoothing keyword pairs are the catalogued call-shapes: row [10] `filter_taubin(mesh, lamb=0.5, nu=0.5, iterations=10)` is `mesh plus lamb/nu`, where `lamb` is the positive shrink step on even passes and `nu` is the positive dilation magnitude `filter_taubin` SUBTRACTS on odd passes (the `vertices -= nu * dot` un-shrink) — both in `[0,1]` with the stable pass-band `0 < 1/lamb − 1/nu < 0.1`, so the `Smooth` arm folds `lamb=factor`, `nu=factor/(1 − _TAUBIN_K·factor)` from the one `factor` row — solved from the reciprocal band so `1/lamb − 1/nu == _TAUBIN_K` for every `lamb` — never a negative `nu` (which double-negates the odd pass into a second shrink, defeating the band-pass) nor a bare iteration count that drops the dilation; row [11] `filter_laplacian(mesh, lamb=0.5, iterations=10)` is `mesh plus lamb/iterations` with `lamb` the step, and row [12] `filter_humphrey(mesh, alpha=0.1, beta=0.5, iterations=10)` is `mesh plus alpha/beta`. `Trimesh.simplify_quadric_decimation(percent=None, face_count=None, aggression=None)` (`trimesh.md` row [14]) binds only the catalogued `face_count` argument — the `Decimate` case carries `target_faces` alone and the arm reads the realized face count off the returned `Trimesh`; the optional `aggression` quality knob stays unbound, so the budget is the one parameter and the realized coarsening is observed, not asserted.
- [CELL_SHAPE_STATISTICS]: the per-triangle aspect ratio and equiangular skewness are computed entirely from catalogued `numpy` reductions over the triangle edge-vector array — `np.asarray` (creation row [10]) / `np.stack` (manipulation join row [09]) / `linalg.norm` (linalg row [08]) for the three edge lengths, `min`/`max` (math row [03]) for the longest/shortest ratio, `sum` (math row [01]) for the edge-dot cosine, `clip` (math row [11]) to bound the cosine domain, the skew range, AND the quantile gather indices, and `sort` (math row [15]) plus `round` (ufunc row [19]) for the five-number summary in `_quantiles` rather than `np.percentile` (uncatalogued, so the five fractions in `_QUANTILE_FRACTIONS` scale to rounded index positions gathered in one vectorized pass off the sorted view, never a named lambda per fraction). The equiangular skewness uses the cosine-domain formulation — the minimum interior-angle cosine over the three corners, mapped so the equiangular cell (`cos 60° = 0.5`) lands at 0 skew and the degenerate cell at 1 — avoiding `arccos`, which the `numpy.md` ufunc rows do not enumerate (rows [18]/[19] carry `sin`/`cos`/`tan`/`arctan2` and `abs`/`sign`/`round` but no bare `arccos`). The angle-defect statistics fold `Trimesh.vertex_defects` (`trimesh.md` property row [12], the angle defect per vertex — a curvature signal, named `angle_defect_mean`/`angle_defect_std` for what it carries, never mislabelled vertex valence) through `mean`/`std` (`numpy.md` math row [02]). The boundary/interior/non-manifold edge classification derives the exact per-edge incidence from the sorted directed-half-edge array built off `mesh.faces` (always-available, no cached topology axis) — `np.concatenate` (join row [08]) stacks the three directed half-edges per face, `np.sort(..., axis=1)` endpoint-sorts each so an edge and its reverse collide, and `numpy.unique(half_edges, axis=0, return_counts=True)` (set-ops row [16], counts requested) groups them: incidence 1 is a boundary edge, 2 is an interior-manifold edge, ≥3 is a non-manifold edge, and the returned `unique_edges` row count IS the `E` axis for the Euler characteristic — one fold serves both the incidence classification and the edge count, never a second `Trimesh.edges_unique` cached-property read. This is exact by construction — every directed half-edge is counted once and grouped by its sorted endpoint pair — and never depends on a positional alignment of `edges_unique` against `face_adjacency`, which carries no such ordering guarantee.
- [EXACT_TOPOLOGY_TIER]: the `MANIFOLD3D` tier folds one `ExactTopology` inside the offloaded module-level `_topology_kernel`, which builds a `Manifold` from the surface's `Mesh`/`Mesh64` (`manifold3d.md` type rows [03]/[04], `Mesh64` selected past the `uint32` vertex ceiling) and gates `status() == Error.NoError` (`manifold3d.md` lines 45/156 — ingest sets a non-`NoError` `status()` rather than raising on a non-2-manifold soup, so the kernel raises `QualityFault(rejected=status.name)` the lane's `async_boundary` converts rather than reading a phantom topology) — then the exact genus sums `genus()` (`manifold3d.md` topology row [04], whose contract is "call `decompose` first" since genus is defined per connected component) over `decompose()` (operators/transforms row [07], the topologically-disconnected component split), while `num_vert()`/`num_edge()`/`num_tri()` (query row [07]) and `volume()`/`surface_area()` (measurement row [05]) read off the whole solid in the same fold, so the exact integer genus, the exact edge/face/vertex counts, and the kernel mass supersede the spine's Euler genus, `trimesh`-derived counts, and `trimesh` `area`/`volume` together rather than reading a lone `genus()` and re-deriving the rest from the looser spine axes. The decompose-then-sum is load-bearing: a bare `genus()` on a multi-body manifold is undefined per the catalogue contract, so the fold splits first and sums the per-body genera. The `SPINE` tier derives genus from the Euler characteristic `V − E + F = 2 − 2g` over `len(vertices)`/the half-edge fold's `len(unique_edges)`/`len(faces)` — exact for a closed orientable surface and an admitted caveat for a surface with boundary. `QualityBackend.resolve` probes `importlib.util.find_spec("manifold3d")` and returns `MANIFOLD3D` when the wheel loads, falling to `SPINE` otherwise. The kernel returns the picklable `ExactTopology` VALUE — a live `Manifold` cannot cross the no-pickle PEP 734 boundary — so the capsule caches that value on `_exact` across metric passes; this is the in-process `ExactTopology` result reuse the `mesh/spatial.md#SPATIAL` sibling names as this owner's, distinct from spatial's per-hop `Manifold` rebuild that holds no cache because its exact-clearance arm rebuilds the `Manifold` per hop (spatial casts rays on the trimesh Embree intersector spine, not `manifold3d`; only its `MANIFOLD3D` `Clearance` arm rebuilds the `Manifold` per `min_gap` fold, the per-hop build the no-pickle boundary forces). The gated tier resolves only on the companion lane and the spine genus is the always-available default.
...
- [TOPOLOGY_BAND]: because the exact integer topology routes through `manifold3d` (the one `.api`-confirmed exact-topology surface), the `MANIFOLD3D` `ExactTopology` fold is a companion-band enrichment over the always-available Euler-characteristic spine genus and `trimesh`-derived counts/mass — there is no `Ray`/`Clearance`-style hard gate, since the metric fold never faults on the bare cp315 venv: the spine derives genus from the catalogued `trimesh` topology axes and mass from the cached `area`/`volume` properties, and the gated tier only sharpens an already-valid grade with exact genus, exact counts, and the kernel mass superseding the trimesh measure off the offloaded `_topology_kernel`'s `Manifold`, the returned `ExactTopology` value cached on `_exact`. The conditioning arms (`Decimate`/`Subdivide`/`Smooth`) are wheel-free on the project venv; the exact-topology tier is the `QualityBackend` row, never a parallel topology owner.
```
