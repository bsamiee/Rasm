...
describe('metric changes', () => {
    it.effect('changed counters record their previous and current values', () =>
        Effect.gen(function* () {
            ...
            const row = yield* _metricChange(capture.metricChanges, 'test_support_token_reuse');
    ...
    it.effect('failing effects still record metric changes from their failure path', () =>
        Effect.gen(function* () {
            ...
            expect(Option.isSome(_metricChange(capture.metricChanges, 'test_support_token_reuse'))).toBe(true);
        }),
    );
});
