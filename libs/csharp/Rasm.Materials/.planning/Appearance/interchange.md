...
public static class WireCodec {
    ...
    public static Fin<MaterialWire> FromJson(string payload, Op key) =>
        Captured(() => JsonSerializer.Deserialize<MaterialWire>(payload, Json), "json", key);

    public static ReadOnlyMemory<byte> ToMessagePack(MaterialWire wire) => MessagePackSerializer.Serialize(wire, MessagePack);

    // Pooled-buffer egress: the heavy LZ4 OpenPBR payload writes straight into the transport's rented IBufferWriter<byte>,
    ...
    public static void ToMessagePack(IBufferWriter<byte> sink, MaterialWire wire) => MessagePackSerializer.Serialize(sink, wire, MessagePack);

    public static Fin<MaterialWire> FromMessagePack(ReadOnlyMemory<byte> payload, Op key) =>
        Captured(() => MessagePackSerializer.Deserialize<MaterialWire>(payload, MessagePack), "messagepack", key);
    ...
    public static Fin<MaterialWire> FromMessagePack(in ReadOnlySequence<byte> payload, Op key) {
        ReadOnlySequence<byte> sequence = payload;
        return Captured(() => MessagePackSerializer.Deserialize<MaterialWire>(sequence, MessagePack), "messagepack", key);
    }

    // The ONE decode capture terminal every decode leg rides: the codec throw converts into MaterialFault.Graph WITH
    // the provider message (recovery preserves diagnostic context), and a nil payload deserializing the reference-type
    // record to null binds onto the typed rail — never an escaping exception, a discarded cause, or a crossing null.
    static Fin<MaterialWire> Captured(Func<MaterialWire?> decode, string leg, Op key) =>
        Try.lift(decode).Run()
            .MapFail(error => MaterialFault.Graph(key, $"<material-wire-{leg}:{error.Message}>"))
            .Bind(wire => wire is { } decoded ? Fin.Succ(decoded) : MaterialFault.Graph(key, $"<material-wire-{leg}-null>"));
}
```
