...
const LAW = [
  ...
  'MANDATE — INTEGRATION-SHAPED, NOT SURFACE-LEVEL: a rebuilt .api documents (a) the package full ADVANCED surface (combinators, hooks, native pipelines, discriminators, async mirrors — not just the basic members), AND (b) the INTEGRATION patterns the dense design should compose — how this library STACKS with the other admitted libs into single rails (e.g. a decode hook feeding a discriminated model under a retry context with a telemetry span) — INCLUDING the SHARED/UNIVERSAL catalog tier where the language has one (`libs/python/.api/` for Python: expression/msgspec/pydantic/pydantic-settings/stamina/beartype/structlog/opentelemetry/anyio; `libs/typescript/.api/` for TypeScript: effect/effect-platform/Schema/react), so a folder/area catalog documents stacking ONTO those universal rails, not only its sibling-folder libs. C# has NO central tier — its universals are Thinktecture/LanguageExt, and the `libs/csharp/Rasm/.api/` catalogs serve the `Rasm/Geometry` planning effort. The catalog GUIDES the rebuild toward first-class, stacked usage. Reject surface-level member lists.',
...
// --- [COMPOSITION] -----------------------------------------------------------------------
phase('API-Discover')
const inv = await agent('List every .api catalog file under ' + SWEEP + ' — markdown files at paths matching */.api/*.md (and any nested .api subdirs). Return each as a repo-relative path. If none exist, return an empty list. Use find; do not cd.', { label: 'discover', phase: 'API-Discover', schema: DISCOVERY_SCHEMA, model: 'sonnet', effort: 'low' })
...
log('API rebuild: ' + done.length + '/' + totalBatches + ' batches (' + totalFiles + ' catalogs); reconcile ' + uniq.length + ' residuals -> ' + clusters.length + ' clusters')
let reconciled = []
if (clusters.length) {
  phase('Reconcile')
