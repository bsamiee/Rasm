...
class Simulation(Struct, frozen=True):
    ...
    async def job(self, run: RunSpec, model: Path, source: str) -> "RuntimeRail[Job]":
        # the ruled cloud-submission construction: the runtime interface projection validates the
        # recipe contract, then the Job folds the SAME inputs `simulate` stages — path inputs as
        # JobPathArgument over ProjectFolder sources, scalar extras as JobArgument value rows;
    ...
    def receipt(
        ...
        # the one SimulationReceipt construction site (the sibling model/district `receipt()` shape):
        # the model key folds from the held building, so the EUI-measured BUILDING_ENERGY graduation
        # path is reachable from every results/crossing product.
...
def _job(interface: "RecipeInterface", run: RunSpec, model: Path, source: str) -> "Job":
    # one submission document over the projected contract: the v1beta1 Job carries ONE inner
    # argument list (one parametric run), artifact paths as ProjectFolder sources — never a
    # hand-stitched dict beside the queenbee schema owners.
...
def _translated(building: BuildingModel, target: TranslateTarget, folder: Path) -> "RuntimeRail[Path]":
    ...
    def in_process() -> Path:
        # the in-process writers return the serialized document string and take no folder
        # parameter; this kernel owns the durable artifact write.
