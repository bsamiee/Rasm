...
Rasm.Compute model extension-ops: one `CustomOps` registration fold over the extensions bundle and the custom-op library rows plus the string-tensor output boundary (empty-slot allocator AND egress reader). The page owns the `CustomOps` registration-and-string-boundary fold; registration extends the `sessions#SESSION_CAPSULE` `ModelSessions` boundary capsule and rides `Microsoft.ML.OnnxRuntime.Extensions`/`Microsoft.ML.OnnxRuntime`, the `SessionPolicy` lifecycle record arrives settled from `sessions#SESSION_CAPSULE`, the native-asset evidence rides the `identity#MODEL_IDENTITY` `ModelLoad` receipt, and the string INGRESS rides `inference#INFERENCE_MODES` `RunInput.Strings`. The `CustomOps.Register`/`StringSlots`/`StringEgress` fold crosses to `sessions#SESSION_CAPSULE` as the one registration step the open fold composes.

## [1]-[INDEX]

| [INDEX] | [CLUSTER]     | [OWNS]                                                                                                       |
| :-----: | :------------ | :----------------------------------------------------------------------------------------------------------- |
|   [1]   | EXTENSION_OPS | Extension and custom-op registration with asset evidence; bidirectional string-tensor boundary (ingress + egress) |
