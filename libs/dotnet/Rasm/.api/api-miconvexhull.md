...
- kernel fold: kernel code implements `IVertex`/`IVertex2D` on its point type or adapts `Rasm.Spatial`, carries index and payload on the vertex or a custom `ConvexFace`, and reads connectivity from `Faces`, `Cells`, or `Edges`. `Spatial/cloud` seats one `CloudVertex` implementing both `IVertex` and `IVertex2D` with the cluster index as its payload, reads facet adjacency, outward normal, simplex vertices, and dual endpoints straight off the package default `DefaultConvexFace`, `DefaultTriangulationCell`, and `VoronoiEdge` carriers under file aliases, and `CloudHullRejection` keys the outcome vocabulary off the package ordinals so the returning and throwing families report one cause.
