...
def _meta(role: str, label: str, page: int) -> NodeMeta:
    return NodeMeta(key=ContentIdentity.of(f"report-{role}-{page}", label.encode()), role=role, page=page)


def _runs(role: str, page: int, *lines: str) -> tuple[RunNode, ...]:
    return tuple(RunNode(meta=_meta(f"{role}-run", line, page), text=line, font_key="body", size=11.0) for line in lines if line)
...
def _section_node(index: int, section: Section) -> SectionNode:
    return SectionNode(
        ...
        children=(
            ...
            *(
                FigureNode(
                    meta=_meta("figure", ref.alt, index), asset_key=ref.asset_key, alt=ref.alt,
                    media_type=ref.media_type, intrinsic=ref.intrinsic, caption=_runs("caption", index, ref.caption),
                )
...
def _output_page(spec: ReportSpec, role: str, output: str | bytes) -> PageNode:
    # the rendered output's own type is the discriminant: a text exporter's `str` wraps as a `CODE`
    # HTML/markup leaf, a binary `PDF`/`WEBPDF` exporter's `bytes` as a hex `QUOTE` leaf, so an
    # assembled-PDF byte payload never mis-renders as an HTML code block.
...
async def _compose_arm(plan: "ReportPlan") -> "ReportFact":
    ...
    page = PageNode(
        ...
        media_box=_box(spec.paper),
        children=(*outline, *(_section_node(index, section) for index, section in enumerate(spec.sections, start=len(outline)))),
    )
...
async def _template_arm(plan: "ReportPlan") -> "ReportFact":
    from jinja2 import (
        ChoiceLoader, DictLoader, Environment, FileSystemLoader, ModuleLoader,
        PackageLoader, PrefixLoader, StrictUndefined, select_autoescape,
    )
    ...
    match spec.loader:
        case ReportLoader.DICT:
            loader = DictLoader({"<root>": spec.source, **spec.section_templates})
        case ReportLoader.FILESYSTEM:
            loader = FileSystemLoader(list(spec.template_roots))
        case ReportLoader.PACKAGE:
            loader = PackageLoader(spec.package, spec.package_path)
        case ReportLoader.PREFIX:
            loader = PrefixLoader({prefix: DictLoader({"<root>": source}) for prefix, source in spec.section_templates.items()})
        case ReportLoader.MODULE:
            loader = ModuleLoader(spec.module_path)
        case _ as unreachable:
            assert_never(unreachable)
    factory = Environment if spec.trusted else ImmutableSandboxedEnvironment
    env = factory(
        loader=ChoiceLoader([loader, DictLoader({})]),
        extensions=list(spec.extensions),
        autoescape=select_autoescape(enabled_extensions=("html", "xml")),
        undefined=StrictUndefined, enable_async=True, trim_blocks=True, lstrip_blocks=True,
    )
    html = await env.from_string(spec.source).render_async(sections=spec.sections, figures=spec.figures)
    page = _output_page(spec, "template", html)
...
async def _notebook_arm(plan: "ReportPlan") -> "ReportFact":
    ...
    executed = await NotebookClient(parameterized, **spec.engine.client_kwargs()).async_execute()
    archive = jupytext.writes(executed, fmt="ipynb").encode()
    output, _resources = nbconvert.get_exporter(spec.export.value)(**spec.export_policy.exporter_kwargs()).from_notebook_node(executed)
...
async def _reflow_arm(plan: "ReportPlan") -> "ReportFact":
    spec = plan.spec
    pdf, count, box = await to_process.run_sync(_reflow, spec.source, spec.user_css, spec.em, spec.paper.value)
...
def _reflow(html: str, user_css: str, em: float, paper: str) -> tuple[bytes, int, Rect]:
    # the gated subprocess lane (`anyio.to_process.run_sync`): the native MuPDF Story layout runs
    # off the loop, the buffer's reflowed PDF bytes are the artifact, never a discarded sink.
    import pymupdf

    rect = pymupdf.paper_rect(paper)
    buffer = io.BytesIO()
    writer = pymupdf.DocumentWriter(buffer)
    story = pymupdf.Story(html=html, user_css=user_css or None, em=em)
    more, count = 1, 0
    while more:
...
- [JUPYTEXT_SOURCE]: the `ReportSource` rows route through `jupytext.reads(text, fmt)`/`jupytext.writes(notebook, fmt)` — `ipynb`/`md:myst`/`py:percent`/`py:light`/`md`/`Rmd`/`qmd` are `fmt` short forms on the one conversion pair, the source-vs-string distinction the `text` argument, never a per-format reader — and `AUTO` passes `fmt=None` so `reads` content-detects through its own `guess_format`/`divine_format` path, the correct detection idiom that replaces the prior fence's `guess_format(source, ".ipynb")[0]` whose hardcoded `.ipynb` extension hint mis-detected a percent/myst source. The executed node serializes to the audit-archive bytes through `jupytext.writes(executed, "ipynb")` on the cp315 core. The `reads(text, fmt=None, ...)`/`writes(notebook, fmt, ...)` spellings verify against the folder `.api` catalogue for `jupytext` read/write rows `[02]`/`[04]`; the `py:light`/`Rmd`/`qmd` `fmt` rows are catalogued in the package-surface capability line.
