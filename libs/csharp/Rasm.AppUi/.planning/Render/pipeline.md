...
- Growth: a new frame stage is one `RenderPass` case breaking the topological dispatch at compile time; a new backend is one `GpuBackend` row constructed with its target delegate and its `GpuBinding` case — Skia Graphite re-admits as one `SkiaGraphite` row the moment SkiaSharp ships its Recorder/Context surface; zero new surface.
...
public sealed record RenderGraph(
    ...
    public static Fin<Unit> Observe(InstrumentSet set, FrameReceipt receipt) =>
