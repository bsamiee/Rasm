...
| :-----: | :--------------------- | :---------------------------- | :------------------------------------------------------ |
|  [01]   | `D2`                   | class                         | version-free dataflow graph host                        |
|  [02]   | `StreamBuilder<T>`     | class                         | `IStreamBuilder<T>` base implementation                 |
|  [03]   | `RootStreamBuilder<T>` | class (extends StreamBuilder) | input source with `sendData(collection)` only           |
|  [04]   | `ID2`                  | interface                     | `newInput`, `step`, `finalize` contract (no `frontier`) |
|  [05]   | `IStreamBuilder<T>`    | interface                     | `pipe(…operators)`, `writer`, `connectReader`, `graph`  |
|  [06]   | `PipedOperator<I, O>`  | type alias                    | `(stream: IStreamBuilder<I>) => IStreamBuilder<O>`      |
...
| [INDEX] | [SYMBOL]                    | [TYPE_FAMILY]  | [CAPABILITY]                                                                                                                              |
| :-----: | :-------------------------- | :------------- | :---------------------------------------------------------------------------------------------------------------------------------------- |
|  [01]   | `MultiSet<T>`               | class          | signed-multiplicity collection; subset of d2ts `MultiSet` — no `join`/`reduce`/`iterate`/`count`/`min`/`max`/`distinct`/`sum` on instance |
|  [02]   | `MultiSetArray<T>`          | type alias     | `[T, number][]`                                                                                                                           |
|  [03]   | `KeyedData<T>`              | type alias     | `[key: string, value: T]`                                                                                                                 |
|  [04]   | `KeyValue<K, V>`            | type alias     | `[K, V]`                                                                                                                                  |
|  [05]   | `Index<K, V>`               | class          | flat key-value index; `get`, `join`, `addValue`, `append`, `entries`, `keys`, `has`, `size` — no version axis                             |
|  [06]   | `DifferenceStreamReader<T>` | class          | read handle; `drain()` → `MultiSet<T>[]`, `isEmpty()`                                                                                     |
|  [07]   | `DifferenceStreamWriter<T>` | class          | write handle; `sendData(collection)`, `newReader()`                                                                                       |
|  [08]   | `Operator<T>`               | abstract class | base operator (no frontier fields)                                                                                                        |
|  [09]   | `UnaryOperator<Tin, Tout>`  | abstract class | single-input operator; `inputMessages()` → `MultiSet<Tin>[]`                                                                              |
...
| :-----: | :--------------------------------------- | :------------- | :----------------------------------- |
|  [01]   | `new D2()`                               | constructor    | creates version-free dataflow graph  |
|  [02]   | `D2#newInput<T>()`                       | factory        | returns `RootStreamBuilder<T>`       |
|  [03]   | `D2#step()`                              | execution      | runs one incremental step            |
|  [04]   | `D2#run()`                               | execution      | runs to quiescence                   |
|  [05]   | `D2#finalize()`                          | lifecycle      | signals graph completion             |
|  [06]   | `RootStreamBuilder#sendData(collection)` | data ingestion | sends `MultiSet<T>` (no version arg) |
...
| :-----: | :--------------------------------------------------------------- | :------------------------------------------------------------ | :------------------------------- |
|  [01]   | `map<T, O>(f)`                                                   | `PipedOperator<T, O>`                                         | element-wise transform           |
|  [02]   | `filter<T>(f)`                                                   | `PipedOperator<T, T>`                                         | predicate filter                 |
|  [03]   | `negate<T>()`                                                    | `PipedOperator<T, T>`                                         | negate multiplicities            |
|  [04]   | `concat<T, T2>(other)`                                           | `PipedOperator<T, T\|T2>`                                     | stream union                     |
|  [05]   | `consolidate<T>()`                                               | `PipedOperator<T, T>`                                         | merge multiplicity cancellations |
|  [06]   | `output<T>(fn)`                                                  | `PipedOperator<T, T>`                                         | side-effecting observer          |
|  [07]   | `keyBy<T, K>(keyFn)`                                             | `PipedOperator<T, Keyed<K, T>>`                               | add key to stream elements       |
|  [08]   | `unkey<K, V>()`                                                  | `PipedOperator<Keyed<K,V>, V>`                                | strip key from keyed stream      |
|  [09]   | `rekey<K1, K2, V>(keyFn)`                                        | `PipedOperator<Keyed<K1,V>, Keyed<K2,V>>`                     | rekey by new function            |
...
| :-----: | :------------------------------------------------------------- | :----------------------------------------------------- | :--------------------------------------- |
|  [01]   | `topKWithFractionalIndexBTree<K, V1>(comparator, options?)`    | `PipedOperator<T, KeyValue<K, [V1, string]>>`          | B-tree-backed fractional-indexed topK    |
|  [02]   | `orderByWithFractionalIndexBTree<T>(valueExtractor, options?)` | `(stream) => IStreamBuilder<KeyValue<K, [V, string]>>` | B-tree-backed fractional-indexed orderBy |
|  [03]   | `loadBTree()`                                                  | `Promise<void>`                                        | loads B-tree module before first use     |
