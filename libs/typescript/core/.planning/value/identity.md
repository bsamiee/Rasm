...
- Law: consumers derive, the floor never encodes their vocabulary — `convention#IDENTITY_PROJECTION` maps this value into attribute space, the runtime boot stamps it, data store scopes partition on the tenancy projection — so a convention rename lands in the consumer's vocabulary, never here.
