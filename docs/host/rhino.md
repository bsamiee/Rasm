...
When documenting Rhino 9/WIP members such as transform scale classification, vector decomposition, pseudo-inverse behavior, or new mesh operations, cite the local `uv run python -m tools.quality api query <key> <symbol>` command used to prove them (member queries use a dotted `Type.Member` symbol). Do not infer availability from older public pages.
