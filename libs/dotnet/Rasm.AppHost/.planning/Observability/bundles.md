...
- [03]-[ARTIFACT_ROSTER]: Contributor factory rows, the contributed-port boundary, the one masking ledger, and the event-trace pump.
...
- Packages: Rasm (kernel `ContentHash`, `Cell`, `Transition`, `MonotonicTimeline`), Microsoft.Extensions.Telemetry.Abstractions, Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, BCL inbox
...
Policy rows bind through configuration: rows one through six freeze into the `SupportPolicy` record and the retention-sweep-cadence row binds the `Sweep` `ScheduleEntry` registration `Runtime/modules#MODULE_LEDGER` seats. Every capture and retention literal traces to this table:
...
|  [05]   | hook-faults       | `HookFaults`      | the bus's `FaultCell` parked rows beside its shed and lost counters |
