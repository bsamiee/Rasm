...
type Comparand = float | int | str | "Column"
...
class StepKind:
    ...
    aggregate: tuple[Columns, Stat, Operator, float, float] = case()
...
class ProbeStep(Struct, frozen=True):
    ...
    def append(self, plan: "pb.Validate", tables: "ProbeTables") -> "pb.Validate":
        ...
        match self.kind:
            ...
            case StepKind(tag="ordered", ordered=(columns, True, allow_stationary, _, na_pass)):
                return plan.col_vals_increasing(tables.cols(columns), allow_stationary=allow_stationary, na_pass=na_pass, thresholds=t, actions=a)
            case StepKind(tag="ordered", ordered=(columns, False, allow_stationary, tol, na_pass)):
                return plan.col_vals_decreasing(tables.cols(columns), allow_stationary=allow_stationary, decreasing_tol=tol, na_pass=na_pass, thresholds=t, actions=a)
...
class QualityProfile(Struct, frozen=True):
    ...
    def interrogate(self, data: Any, *, sample_n: int | None = None, sample_frac: float | None = None, get_first_n: int | None = None, extract_limit: int = 500) -> "RuntimeRail[ProfileReceipt]":
        ...
        ).bind(lambda plan: self.fingerprint().map(lambda key: ProfileReceipt.of(self.label, plan, data, len(self.steps), key)))
    ...
    def fingerprint(self) -> "RuntimeRail[ContentKey]":
...
class ProbeTables(Struct, frozen=True):
    ...
    aggregate: Map[tuple[Stat, Operator], Callable[..., "pb.Validate"]]
    ...
    def bind(cls, pb: Any) -> "ProbeTables":
        ...
        return cls(
            ...
            aggregate=Map.of_seq(((stat, op), getattr(v, f"col_{stat}_{op}")) for stat in _STATS for op in _OPS),
...
- [POINTBLANK_PROVISION]: `pointblank` is absent from the data manifest at authoring; execution provisions it into the cp315 environment (`installed 0.24.0` reflected on cp315 per the `.api` package surface). The whole-package boundary-scoped import (`import pointblank as pb` under `# noqa: PLC0415`) holds because pointblank transitively loads the manifest-banned module-level `polars`/`great_tables`, exactly as the sibling `tabular/contract#COLLECTION` `dataframely` and `tabular/contract#QUALITY` `pandera` arms import at boundary scope. The `pb.Thresholds`/`pb.Actions`/`pb.FinalActions`/`pb.Schema` annotations resolve under `TYPE_CHECKING`, and the runtime `pb` enters through exactly three boundary-scoped `import pointblank as pb` sites — `_plan`, `_report`, and `ProfileReceipt.of` — plus the `ProbeTables.bind` classmethod that captures `pb` on the `ProbeTables.ns` field once per plan; the `Validate`/`DataScan`/`col_summary_tbl`/`missing_vals_tbl`/`preview`/`get_row_count`/`get_column_count` uses and the `Schema`/`starts_with`/`ends_with`/`contains`/`matches`/`everything`/`first_n`/`last_n` resolution the `ProbeTables.cols`/`ProbeTables.schema` methods mint read that one captured `ns` rather than re-importing per step, so the per-step fold carries zero import overhead. No fence member contradicts a sibling RESEARCH item: the `pointblank` step, grade, action, and report surfaces this page settles are disjoint from the `pandera`/`dataframely` surfaces the `tabular/contract` page settles.
