...
|  [03]   | `ParseOptions` (ts)         | `{ syntax: 'typescript', tsx?, decorators? }` | `tsx` per-extension; `decorators: true` strictly tolerant |
|  [04]   | `Module`                    | `{ type: 'Module', body: ModuleItem[] }`      | the walk root                                             |
|  [05]   | `ImportDeclaration`         | `{ source: StringLiteral, typeOnly }`         | static import; `typeOnly` is declaration-level plane fact |
...
- Boundary: parse failure is a loud throw, never an empty harvest — silence turns an unparsable file into a vacuous green.
