...
public abstract partial record PageEvent {
    ...
    public Fin<Seq<TObject>> Objects<TObject>() where TObject : RhinoObject => Op.Of().Need(Args).Map(static args => toSeq(args.GetObjects<TObject>()));
    public Fin<Seq<RhinoObject>> Objects(ObjectType filter) => Op.Of().Need(Args).Map(args => toSeq(args.GetObjects(filter: filter)));
    public Fin<Seq<RhinoObject>> SupportedObjects() => Op.Of().Need(Args).Map(static args => toSeq(args.Objects));   // args.Objects auto-filters by the page's declared SupportedTypes — no explicit filter needed
...
public abstract class RasmOptionsPage : global::Rhino.UI.OptionsDialogPage {
    ...
    public Fin<Unit> Navigate(PageNav nav) => Op.Of().Need(nav).Bind(valid => valid.Apply(page: this));
...
public abstract class RasmPropertiesPage : global::Rhino.UI.ObjectPropertiesPage {
    ...
    protected Fin<Unit> Modify(Func<PageEvent, Fin<Unit>> change) =>
        Op.Of().Need(change).Bind(valid => RhinoUi.Protect(valid: () => {
            Fin<Unit> result = Fin.Fail<Unit>(error: Op.Of().InvalidResult());
...
public sealed record PageRegistration<TPage>(TPage Page, PageHost Host) where TPage : class {
    public Fin<Unit> Add(ICollection<TPage> pages) =>
        from page in Op.Of().Need(Page)
        from validPages in Op.Of().Need(pages)
        from added in Host switch {
            ...
            _ => Fin.Fail<Unit>(error: Op.Of().InvalidInput()),
        }
        select added;
