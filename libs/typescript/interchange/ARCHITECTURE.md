...
- `codecs`: the byte-to-typed decode and encode rail family read by codec key (proto, messagepack `useBigInt64`, json-stj `Schema.Class`), with `EncodeRail` the direction-keyed write mirror and `GeometryRail` the embedded-GeoJSON row; one `DecodeRail` owner, every codec a row.
