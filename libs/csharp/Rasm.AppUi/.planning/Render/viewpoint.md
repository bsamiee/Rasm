...
- Entry: `Viewpoint.Capture(key, camera, section, overrides, selection, measurements, at)` — the six-column admission, every defect named on one `Validation` fold; `Viewpoint.Encode()`/`Viewpoint.Decode(blob)` — generated `Render.V1.ViewpointWire` ProtoJSON through the one AppHost `WireJson` posture; `ViewpointCodec.ToBcf(guid, view, source)`/`FromBcf(key, bcf, at)` — the exchange projection.
...
- Receipt: `Viewpoint` projects through `Render/pipeline#TS_PROJECTION` `ResidencyMap` onto generated `Render.V1.ViewpointWire` and serializes through AppHost `WireJson`, so the browser, markup, and cross-process coordination read protobuf's one JSON mapping and this page mints no DTO, serializer context, or codec twin.
