...
- Packages: `Rasm`/Vectors (`TopologyReceipt` via the Genus-tolerant `ProjectionRow` — composed), `Rasm.Geometry.Meshing` (`MeshEdit` dirty-bitset seed), `Rasm.Geometry.Arrangement` (`BooleanOp`/`BooleanReceipt` + its canonical `Empty` — the composed payload; `BooleanRoute` rides inside the receipt), Rasm.Domain (`IValidityEvidence`/`ValidityClaim` — the registered validity fold), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox.
