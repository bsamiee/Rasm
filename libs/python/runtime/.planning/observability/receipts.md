...
- Packages: `expression` (`tagged_union`/`case`/`tag` tagged-union [01]-[03], `Map` over the redaction table, the `PHASE_LEVEL` level table, and the `LEVEL_METHOD` table of `(sync, async)` bound-method selector pairs, `Map.empty` constructor [06] minting the keep-all `_OPEN` policy; `Option.of_optional`/`Option.default_with` option-ops ENTRYPOINTS [04]/[07] the `_sink` fold resolving the optional `sink` override to the per-emit `get_logger` default, `Option.of_optional`/`Option.default_value` [04]/[07] the `redact` processor's bound-`Redaction`-or-`_OPEN` fold, and `Map.try_find`/`Option.map`/`Option.default_value` lookup [08]/option-ops [01]/[07] the `Redaction._classify` un-classified-field fold), `msgspec` (`Struct`, `json.Encoder(*, enc_hook, order, ...)` stateful-codec ENTRYPOINTS [01] with `enc_hook=repr` so an unencodable bound value renders through its repr rather than raising and `order="deterministic"` so the one encoder serves both the renderer and the canonical `hash`-class input), `structlog` (`get_logger`/`wrap_logger`/`configure`/`contextvars.merge_contextvars`/`processors.add_log_level`/`processors.CallsiteParameterAdder`/`processors.dict_tracebacks`/`processors.TimeStamper`/`processors.EventRenamer`/`processors.JSONRenderer`/`BytesLoggerFactory`/`make_filtering_bound_logger`/`testing.capture_logs` ENTRYPOINTS bound-logger [04] the parameterized `sink` test seam binds, the `FilteringBoundLogger` `debug`/`info`/`warning`/`error` sync level methods and the `adebug`/`ainfo`/`awarning`/`aerror` async mirrors ENTRYPOINTS bound-method [02]/[03] the `(sync, async)` `LEVEL_METHOD` selector pairs bind, the async mirror offloading the chain to a worker thread per the structlog `STACKS_WITH` "anyio structured concurrency" row), `opentelemetry-api` (`trace.get_current_span`, `trace.format_trace_id` ENTRYPOINTS [07], `trace.format_span_id` ENTRYPOINTS [08], `propagate.extract` ENTRYPOINTS [11], `propagate.get_global_textmap` ENTRYPOINTS [13], `Context` PUBLIC_TYPES [05], `context.attach`/`context.detach` ENTRYPOINTS [02]/[03], `TextMapPropagator` PUBLIC_TYPES [07], `DefaultGetter` PUBLIC_TYPES [10]), `psutil` (`Process.memory_info` process-metrics ENTRYPOINTS [01] off the one own-process handle, guarding `NoSuchProcess`/`ZombieProcess`/`AccessDenied` PUBLIC_TYPES [02]/[03]/[04] under `contextlib.suppress` so the drained emit never raises on a dead-process race), `hashlib` (stdlib `blake2b` keyed digest for the `hash` classification, cp315-native — never the companion-gated `evidence/identity#IDENTITY` `xxhash` content owner, a distinct concern in a distinct lane). The SDK `LoggerProvider`/`OTLPLogExporter` are consumed by `observability/telemetry#TELEMETRY`, never imported here.
...
- Boundary: this owner mints no AppHost envelope, health status, support-bundle capture, exporter ownership, provider install, or C# receipt, and the suite classification taxonomy stays AppHost-owned. The redaction `hash` class uses stdlib `hashlib.blake2b` for cp315-native correlation tokens and never imports the companion-gated `xxhash` the `evidence/identity#IDENTITY` content-addressing owner holds; log-field redaction and content identity are distinct concerns in distinct lanes, and sharing the hasher would trample that owner and break the cp315 venv resolution. The `continue_inbound` `propagate.extract` and the `attach` `context.attach`/`context.detach` token-pair are the same aligned `Context` stitch the sibling `execution/lanes#LANE` `traced_kernel` shim runs on the subinterpreter-offload hop, but the two stay distinct seams over distinct hops — receipts owns the inbound gRPC W3C extract per the ARCHITECTURE `[WIRE]` seam, lanes owns the intra-process PEP 734 offload stitch — an aligned pattern, not a merged owner.
...
type Contributing[**P] = Callable[P, ReceiptContributor] | Callable[P, Awaitable[ReceiptContributor]]
...
# the keep-all policy the chain-resident `redact` folds a line carrying no bound `Redaction`
# through; depends on the `Redaction` model so it anchors here, never top-level [CONSTANTS].
...
# stdlib-bridge record, a direct log call) folds through the keep-all `_OPEN` policy untouched,
...
def receipted[**P](redaction: Redaction) -> Callable[[Contributing[P]], Contributing[P]]:
    def wrap(operation: Contributing[P]) -> Contributing[P]:
        if iscoroutinefunction(operation):
            @wraps(operation)
            async def harvested_async(*args: P.args, **kwargs: P.kwargs) -> ReceiptContributor:
        ...
        @wraps(operation)
        def harvested(*args: P.args, **kwargs: P.kwargs) -> ReceiptContributor:
...
- [REDACTION_CLASSIFICATION]: the field-redaction policy is a `Classification`-keyed `expression` `Map[str, Classification]` table discriminating `drop` (field removed before the line), `mask` (fixed `***` sentinel), and `hash` (a truncated keyed `hashlib.blake2b(value, key=salt, digest_size=8).hexdigest()` correlation token) — a `drop`/`mask`/`hash` member added on the `Classification` literal plus one `_reduce` arm, never a parallel allow/deny set. The policy is applied by the chain-resident `redact` processor positioned after `merge_contextvars`/`trace_context`, never a pre-chain `_render`-side fold: `_render` binds the per-emit `Redaction` onto the event under the reserved `_REDACTION` key, `redact` reads it back (folding a line with no bound policy through the keep-all `_OPEN` via `Option.of_optional(event.get(_REDACTION)).default_value(_OPEN)`) and `apply` scrubs the fully-assembled line — receipt facts, the `merge_contextvars` `Correlation`/`RuntimeContext` ambient fields, and the injected `trace_id`/`span_id` — while skipping the `_REDACTION` carrier, so a classified secret arriving through `bind_contextvars` cannot reach the line the way a pre-chain `apply` over only the projected facts would leak it; the absent-policy case is the structural `Nothing` of `classified.try_find(key).map(self._reduce).default_value((value,))` folding the un-classified field to the keep-tuple, so `_reduce` matches only the real closed vocabulary under `assert_never` rather than a stringly `"keep"` sentinel outside the `Classification` set, and the variable-arity `tuple[object, ...]` return (`()` for `drop`, one element otherwise) lets the `apply` dict-comprehension drop a field by yielding zero pairs without a second filtering pass. The `hash` class hashes the `order="deterministic"` `_ENCODE` of the value, the canonical key order making a structured secret yield the same token across log lines regardless of mapping insertion order, so the one encoder serves both the renderer and the redaction hasher. `blake2b` is the stdlib keyed hasher (cp315-native, no wheel gate), chosen deliberately over the `evidence/identity#IDENTITY` `xxhash` content owner: log-field redaction and content-addressing are distinct concerns in distinct lanes, and `xxhash` is companion-gated `<3.15` (absent from the default cp315 venv per `.api/xxhash.md`), so importing it here would both trample that owner and break cp315 resolution. The correlation-preserving `hash` class lets two redacted log lines carrying the same secret correlate by token without leaking the value, which a flat `***` mask cannot. The classification taxonomy is settled.
