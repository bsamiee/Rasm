...
`Variable("x")` is the free symbol; the linear form is built by operator overloading, never by a constructor call — `2 * x` is a `Term`, `2 * x + y - 30` is an `Expression`, and a relational operator turns an expression into a `Constraint` whose rhs is a `float`/`Variable`/`Term`/`Expression`. `__eq__`/`__ge__`/`__le__` produce a `Constraint`; `__ne__`/`__gt__`/`__lt__` are `NoReturn` (strict orderings and inequality are intentionally unsupported — Cassowary models `==`/`<=`/`>=` only). Build alignment/separation expressions directly from the operator overloads; never hand-construct a `Term`/`Expression` tuple.

| [INDEX] | [MEMBER]                                                      | [KIND]    | [ROLE]                                                   |
| :-----: | :------------------------------------------------------------ | :-------- | :------------------------------------------------------- |
|  [01]   | `Variable(name="", context=None)`                             | construct | named free unknown; `context` holds an opaque payload    |
|  [02]   | `Variable.value()`                                            | read      | solved value after `updateVariables()` (`0.0` pre-solve) |
|  [03]   | `Variable.name()` / `setName(name)`                           | identity  | get/set the name (in `Solver.dumps()` provenance)        |
|  [04]   | `Variable.context()` / `setContext(obj)`                      | payload   | get/set the opaque context object threaded through       |
...
|  [07]   | `Term(variable, coefficient=1.0)`                             | construct | explicit `Term`; `coefficient()`/`variable()`/`value()`  |
|  [08]   | `Expression(terms, constant=0.0)`                             | construct | explicit `Expression`; `terms()`/`constant()`/`value()`  |
|  [09]   | `expr == rhs` / `expr <= rhs` / `expr >= rhs` -> `Constraint` | relation  | build a `Constraint` from an expression + relational op  |
...
| [INDEX] | [MEMBER]                                           | [KIND]     | [ROLE]                                                                |
| :-----: | :------------------------------------------------- | :--------- | :-------------------------------------------------------------------- |
|  [01]   | `Constraint(expression, op, strength="required")`  | construct  | explicit build; `op` `"=="`/`"<="`/`">="`, `strength` float/band      |
|  [02]   | `constraint \| strength`                           | priority   | lower a constraint's strength (`(x==y) \| "weak"`); band or float     |
|  [03]   | `Constraint.expression()` / `op()` / `strength()`  | read       | the constraint's expression, relational op, and numeric strength      |
|  [04]   | `Constraint.violated()`                            | introspect | unsatisfied in the current solver state (post-solve QA)               |
|  [05]   | `strength.weak` / `medium` / `strong` / `required` | band       | the four priority floats (`1.0`/`1e3`/`1e6`/`1001001000.0`)           |
|  [06]   | `strength.create(a, b, c, weight=1.0)`             | blend      | symbolic priority between bands (`strength.create(0,1,0,2)`=2×medium) |
...
|  [05]   | `Solver.addEditVariable(variable, strength)` | mutate     | register an edit variable (non-`required`; raises `BadRequiredStrength`)    |
