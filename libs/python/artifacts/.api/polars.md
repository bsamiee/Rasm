...
- [06]-[FRAME_SHAPING]: `DataFrame.select(*exprs)` / `.with_columns(*exprs)` / `.filter(*pred)` / `.sort(by)` / `.rename(map)` / `.cast(dtypes)` / `.head(n)` / `.slice(off, len)` / `.pivot(...)` / `.unpivot(...)` / `.group_by(keys).agg(*exprs)` / `.top_k(k, by=)` — last-mile shaping of the settled frame at the boundary, re-projecting/re-ordering/casting/rolling-up the QTO/schedule frame to the exact display shape before `.style`. The transforms are the `data`-owned API; artifacts uses them only to land the display frame, never to author the source.
