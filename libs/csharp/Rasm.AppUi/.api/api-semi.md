...
[STACKING]:
- `libs/csharp/.api/api-unicolour.md`: `Unicolour.Palette(Unicolour, ColourSpace, int, HueSpan, bool)` ramps the `ControlIntent` vocabulary `Theme/tokens` owns, its `.Rgb.Hex` steps landing as the `Semi<Hue><N>Color` twins and the `SemiColor<Role><State>` brushes reading them; `Contrast(Unicolour)` gates the pairs a high-contrast variant admits.
