...
|  [08]   | serving                 | `<Svc>ASGIApplication(service, *, interceptors, read_max_bytes, compressions)` under hypercorn | uvicorn, a hand route table |
|  [09]   | dialing                 | `<Svc>Client` / `ConnectClient.execute_*` over a `pyqwest` client | a second RPC transport; raw HTTP framing    |
|  [10]   | cross-cutting per call  | one `MetadataInterceptor` `on_start`/`on_end` pair                | per-arity wrappers; handler prologues       |
|  [11]   | deadline                | `ctx.timeout_ms` (REMAINING, recomputed per read)                 | an unbounded handler; a hand deadline clock |
|  [12]   | refusal                 | `ConnectError(code, message, details=[<detail>])`                 | a stringified fault; a hand trailer         |
...
- Law: corpus `protovalidate` rules are the constraint authority and this runtime does not evaluate them, so a branch producer proves the same law on its own rail before construction and a consumer treats a decoded value as admitted by its peer — never re-validating field by field, never trusting a rule the peer's runtime skipped.
...
- Law: cross-cutting work rides the `MetadataInterceptor` pair — `on_start(ctx)` returns the token `on_end(token, ctx, error)` receives with the terminal error — spanning every rpc shape once, so admission, the deadline lift off `ctx.timeout_ms`, contextvars, timing, and span enrichment seat on one object and no handler carries a prologue; server spans are `OpenTelemetryMiddleware`'s off the ASGI scope with `exclude_spans=["receive", "send"]`, client spans open in the client interceptor's `on_start` and close in `on_end` off `ConnectError.code`.
...
- Accept: one `ConnectError(code, message, details=[detail])` raised at the serve edge, its `details` carrying the corpus fault-detail message as ONE packed `Any`; ingress reads `ErrorDetail.value(DetailClass)` over `ConnectError.details` — protocol-agnostic on Connect, gRPC, and gRPC-Web — and a raise carrying no decodable detail answers absence, never a forged verdict.
