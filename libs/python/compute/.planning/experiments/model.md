...
- Receipt: `ModelAssetManifest.contribute` emits one `Receipt.of("emitted", ...)` row carrying the io-names, opset, preprocessing, model-card, and validation verdict; the manifest graduates through `graduation/handoff.md#GRADUATION` on the model-asset axis only after validation passes.
