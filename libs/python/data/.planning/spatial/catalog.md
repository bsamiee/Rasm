...
class StacReceipt(Struct, frozen=True):
    ...
    def contribute(self) -> Receipt:
        return Receipt.of("emitted", "catalog", self.endpoint, {"items": str(self.item_count), "matched": str(self.matched)})
...
def stac_geoparquet_egress(
    table: "pa.RecordBatchReader | pa.Table", output_path: str, *, schema_version: str | None = None
...
- Packages: `pystac` (`Item.assets`/`Asset.href`/`Asset.media_type`/`MediaType`), `tabular/egress` (`ObjectEgress`/`StoreOp.GetRange`, `tabular/egress.md#EGRESS` L12), `gridded/tensor` (`TensorStore.virtual`, `gridded/tensor.md#TENSOR` L132), runtime (`ContentIdentity`/`RuntimeRail`/`boundary`/`ReceiptContributor`).
- Growth: a new archival format is one more `MediaType` row routed to `GetRange`; a new cube backend is the `gridded/tensor` `TensorBackend` axis with zero change here; a per-collection cube is one `concat_dim` on the existing `TensorStore.virtual`; zero new surface.
- Boundary: reads the settled `tabular/egress` `GetRange` and `gridded/tensor` `virtual` fences and never re-mints either — no second object-store transport, no second virtual-cube builder, no full-object read where a byte window applies, no durable asset store; a per-asset egress class family and a re-derived virtual cube are the deleted forms.
...
from rasm.data.gridded.tensor import TensorStore
...
def fold_virtual_cube(items: "Iterable[object]", ref: ResourceRef, *, media_type: str, concat_dim: str = "time") -> "RuntimeRail[object]":
    def _fold() -> object:
        sources = tuple(asset.href for item in items for asset in item.assets.values() if asset.media_type == media_type)
