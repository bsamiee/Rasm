...
layer(TestDatabases.pglite(_DDL))('PGlite queries', (it) => {
    it.effect('seeded DDL round-trips through schema-decoded queries', () =>
        Effect.gen(function* () {
            ...
            const rows = yield* database.decoded(_Record)('SELECT key, priority FROM records ORDER BY priority DESC');
            expect(rows).toEqual([
    ...
    it.effect('NOTIFY delivers one payload after the subscription scope opens', () =>
        Effect.scoped(
            Effect.gen(function* () {
                ...
                yield* database.rows('SELECT pg_notify($1, $2)', ['events', '<event-a>']);
    ...
    it.effect('the subscription excludes messages from other channels', () =>
        Effect.scoped(
            Effect.gen(function* () {
                const database = yield* TestDatabase;
                const mailbox = yield* database.listen('events');
                yield* database.rows('SELECT pg_notify($1, $2)', ['other', '<foreign-event>']);
                yield* database.rows('SELECT pg_notify($1, $2)', ['events', '<event-b>']);
        ...
        Effect.gen(function* () {
            ...
            const error = yield* Effect.flip(
                database.rollbackTransaction(
                    Effect.zipRight(database.exec("INSERT INTO records VALUES ('<key-rollback>', 1);"), Effect.fail('rejected' as const)),
                ),
            );
