...
<completion_bar>
Done is: every owning catalog current against the INSTALLED version — the span's additions, removals, and signature moves landed; every consumer fence integrating the delta — changed members corrected, new capability composed where the charter admits, dead workarounds deleted; zero version anchors, tombstones, or resolved research rows in touched files; the docs gate clean over everything touched. A no-change verdict is earned by the full defect sweep, never a skim; a blocked item returns as a RIPPLE row with evidence, never a partial edit.
</completion_bar>

<context_gathering>
Read fully, in order, before the first edit:
1. `.claude/skills/docgen/templates/api-catalog.template.md` — owns the catalog schema.
2. Each owning catalog whole, then each sibling catalog its `[STACKING]` rows name.
3. Complete changelog span — every release between old and new, from the named repo (releases, CHANGELOG, whatsnew); a span skimmed at its newest release is an incomplete read.
