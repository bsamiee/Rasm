...
class PulseBeat(Struct, frozen=True, gc=False):
    # one polymorphic payload every pulse point speaks: stage names the kernel milestone, done/total carry monotonic
    # progress (total=0 where the kernel cannot know its extent); the struct pickles whole across the conduit proxy.
    stage: str
    done: int
    total: int = 0
...
class GeometryHandoff(Struct, frozen=True):
    ...
    def contribute(self) -> Iterable[Receipt]:
        ...
        if not (breaches := self.breaches).is_empty():
            yield Receipt.of("rasm.geometry.graduation", BoundaryFault(boundary=(f"graduation.{self.subject.value}", ";".join(breaches))))
    ...
    def of(cls, subject: GeometrySubject, key: ContentKey, table: "Mapping[str, Sequence[object] | np.ndarray]") -> "RuntimeRail[EvidenceFrame]":
        ...
        return (
            ...
            if len(widths) <= 1 and -1 not in widths
                ...
                BoundaryFault(
                    boundary=("geometry.frame", f"ragged-frame:{subject.value}:" + ",".join(f"{name}={width}" for name, width in extents.sort()))
                )
            )
        )
...
def _priced(
    ...
    priced = boundary(f"{scope.value}.priced", close)
    priced.swap().map(lambda fault: trace.get_current_span().add_event("rasm.geometry.evidence.refused", {"rasm.fault.tag": fault.tag}))
