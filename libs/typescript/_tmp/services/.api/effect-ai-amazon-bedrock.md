...
How the six modules compose into the one design rail (`execution/ai#AI_ACTIVITY`, `agent/runtime#AGENT_RUNTIME`), stacking onto the universal tier (`effect.md`, `effect-platform-node.md`, `effect-opentelemetry.md`) and the sibling `@effect/ai` core (`effect-ai.md`) + Anthropic provider (`effect-ai-anthropic.md`):

- Provider resolution: `AmazonBedrockLanguageModel.model(model, config?)` / `.layer({ model })` produces `AiModel.Model<"amazon-bedrock", LanguageModel.LanguageModel, AmazonBedrockClient>`, which satisfies the core `LanguageModel.LanguageModel` tag (`effect-ai.md` `[MODEL_RESOLUTION_LAW]`). The design's `providerModel("amazon-bedrock", model)` row selects it; provider swap against `effect-ai-anthropic.md`/`-openai`/`-google`/`-openrouter` is one composition-root layer change.
- Layer stack: `AmazonBedrockLanguageModel.layer({ model })` over `AmazonBedrockClient.layer`/`layerConfig` over `NodeHttpClient.layer` (`effect-platform-node.md`). `layerConfig` reads `accessKeyId`/`secretAccessKey`/`sessionToken`/`region` from `Config` cells (adds `ConfigError`), the credential values resolved through the `security/secret#SECRET_STORE` `SecretStore` and held `Redacted`.
- Generation + rails: `LanguageModel.generateText`/`generateObject`/`streamText` (`effect-ai.md` free functions, `AiError.AiError` channel) or the persisted `Chat.generateText`/`Chat.layerPersisted` (`agent/runtime`); wrap each in `execution/ai#Resilience.breaker` + retry `Schedule`. `converseStream` yields `Stream<ConverseResponseStreamEvent>` folded per `type`, backed by `EventStreamEncoding.makeChannel` over `Channel` (`effect.md`) decoding the AWS event-stream frames.
- Tools: the eight `AmazonBedrockTool.*` provider-defined constructors compose into an `@effect/ai` `Toolkit.make(...)` alongside user `Tool.make` tools, passed via `GenerateTextOptions.toolkit`; the cache-control schema reuses `@effect/ai-anthropic/Generated.BetaCacheControlEphemeral` (`effect-ai-anthropic.md` `[6]`).
- Provider metadata + telemetry: the `@effect/ai/Prompt`/`Response` augmentations attach the `bedrock` key (`cachePoint`, `AmazonBedrockReasoningInfo`, `ConverseTrace` guardrail trace on `FinishPartMetadata`); `@effect/ai` `Telemetry.addGenAIAnnotations` with `WellKnownSystem "aws.bedrock"` writes GenAI spans exported through `@effect/opentelemetry` (`effect-opentelemetry.md`).
- Per-effect transport: `AmazonBedrockConfig.withClientTransform` (tag id `@effect/ai-google/AmazonBedrockConfig`, the upstream copy-paste) installs a request-scoped `HttpClient` transform (retry/metrics/logging span), distinct from the layer-time `transformClient`.

## [8] — Gaps

None at the structural level: all six modules of `@effect/ai-amazon-bedrock` `0.16.1` were reflected from the installed `node_modules/@effect/ai-amazon-bedrock/dist/dts` declarations and every exported owner is captured above with exact spelling. The deep nested inline object literals in the `ConverseResponseStreamEvent.Type` `metadata` branch (the fully-expanded guardrail-trace tree) are summarized to their owning `Schema.Class` names rather than re-transcribed inline, since each inline shape is the structural mirror of a named guardrail class in section [4]; planning owners reference the named classes, not the inline expansion.
