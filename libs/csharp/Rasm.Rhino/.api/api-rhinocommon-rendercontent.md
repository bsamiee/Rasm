...
- `RenderContent.FindChild(string) -> RenderContent` / `SetChild(RenderContent, string) -> bool` / `DeleteChild(string, ChangeContexts) -> bool` / `DeleteAllChildren(ChangeContexts) -> void` — child-slot graph mutation. `SetChild(RenderContent renderContent, string childSlotName) -> bool` is the live spelling and its first parameter is named `renderContent`, never `child`; the `(RenderContent, string, ChangeContexts)` overload EXISTS but is `[Obsolete]` — the change reason rides the enclosing `BeginChange`/`EndChange` scope — so a named-argument call binding `cc:` reaches the retired member.
...
- `PreviewJobSignature()` / `PreviewJobSignature(nint)` / `PreviewJobSignature(int width, int height, uint sig, Guid id)` (`[CLSCompliant(false)]`) / `ImageWidth() -> int` / `ImageHeight() -> int` / `Compare(PreviewJobSignature) -> bool` / `CppPointer -> nint` / `Dispose() -> void` — the preview-job identity `GenerateRenderContentPreview` consumes and `PreviewRenderedEventArgs` carries; `PreviewJobSignature : IDisposable`, the `nint` constructor is the non-owning host-vended form, and the three-argument `(int, int, uint)` constructor is `[Obsolete]`.
...
- `Rhino.Render.PhysicallyBasedMaterial` and its nested `ParametersNames` are `[Obsolete]` WHOLE-CLASS, superseded by `Rhino.Render.ParameterNames.PhysicallyBased` and `Rhino.Render.ChildSlotNames.PhysicallyBased`; the type still ships, so under a `using Rhino.Render; using Rhino.DocObjects;` prelude the simple name `PhysicallyBasedMaterial` is ambiguous and the live document type spells `Rhino.DocObjects.PhysicallyBasedMaterial`.
...
- `SimulatedEnvironment.BackgroundColor -> Color` / `BackgroundImage -> SimulatedTexture` / `BackgroundProjection -> SimulatedEnvironment.BackgroundProjections` — bake payload; `SimulatedEnvironment : IDisposable`, so every bake rides a lease; `ProjectionFromString(string)` / `StringFromProjection(BackgroundProjections)` round-trip the projection name. `new SimulatedEnvironment()` is the public parameterless mint and `ConstPointer() -> nint` is public; the `BackgroundImage` GETTER mints a fresh parent-backed `SimulatedTexture` on EVERY read and NEVER returns null, so image absence reads as `ConstPointer() == IntPtr.Zero` and never as a null projection — an `Optional(image)` lift admits an empty environment as an imaged one.
- `SimulatedTexture(RhinoDoc)` / `SimulatedTexture(RhinoDoc, Texture)` — doc-aware constructors; `ConstPointer() -> nint` is public and resolves through the owning environment when the instance is parent-backed.
...
- `ICurrentEnvironment` — `ForBackground` / `ForReflectionAndRefraction` / `ForLighting -> RenderEnvironment` (get/set); the current-environment usage triple `RhinoDoc.CurrentEnvironment` returns, and that accessor is `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-document.md`'s.

[COLLECTION_KINDLIST]:
- `new RenderContentCollection()` / `new RenderContentKindList()` / `new RenderContentKindList(RenderContentKindList)` — both types carry a PUBLIC parameterless constructor and the kind list adds a copy constructor, so the corpus mints either directly instead of waiting for a host-vended instance.
...
- `RenderContentCollection.GetForcedVaries() -> bool` / `SetForcedVaries(bool)` / `GetSearchPattern() -> string` / `SetSearchPattern(string)` / `ContentNeedsPreviewThumbnail(RenderContent c, bool includeChildren) -> bool` — editor selection-set state the content-editor seam reads; the thumbnail probe takes TWO parameters, so a single-argument call does not compile.
...
- Content authoring and configuration are this catalog; live texture evaluation (`CreateEvaluator`, `SimulateTexture`) and post-effect execution are `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-render.md` — one graph configures, the other evaluates, and the two never merge.

[STACKING]:
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Transform`/`Interval` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/csharp/.api/api-languageext.md`): content `bool` and index outcomes project to `Fin<Unit>`/`Fin<int>`; nullable content reads (`FromId`, `Find`, `FindChild`) lift to `Option<RenderContent>`; table and collection rosters land as `Seq<A>`; a `RenderContent`/`SimulatedEnvironment`/`TextureMapping` `IDisposable` rides a `using` bounded by `Eff`, and a failed create or refused assign crosses as `Fin<A>`.
...
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendersettings.md`: `RenderSettings.RenderEnvironment(EnvironmentUsage, EnvironmentPurpose)` binds a `RenderEnvironment` into the document render environment, and the content event carrier reports `RenderSettings.EnvironmentUsage` — settings own the binding, this catalog owns the content.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-document.md`: `RhinoDoc.RenderMaterials`/`RenderEnvironments`/`RenderTextures` are the table accessors, and `RenderMaterial.ToMaterial`/`ConvertToPhysicallyBased` bridge to the `Rhino.DocObjects.Material`/`PhysicallyBasedMaterial` document surface the document catalog owns.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-geometry.md`: `TextureMapping` factories consume `Plane`/`Interval`/`Sphere`/`Cylinder`/`Mesh` and recovery returns them; the geometry catalog owns those carriers, this catalog owns the mapping.
