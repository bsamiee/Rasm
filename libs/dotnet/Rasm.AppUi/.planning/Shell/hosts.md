...
- [05]-[NATIVE_ASSETS]: Per-RID Skia and HarfBuzz rows with load-identity facts.
...
// --- [MODELS] --------------------------------------------------------------------------
public sealed record SurfacePort(
...
public sealed class SurfaceRuntime : IDisposable {
    private readonly Ring<RasmEvent<Rasm.Contracts.Event.Extensions>> events;
    ...
    private SurfaceRuntime(
        ...
        Ring<RasmEvent<Rasm.Contracts.Event.Extensions>> events,
        Func<InstrumentSpec, Option<(string Slot, string Value)>, Unit> count) =>
        (Line, Hooks, this.events, Count) = (line, hooks, events, count);

    public MonotonicTimeline Line { get; }
    public HookSet<AppUiPoint, AppUiFact, TelemetrySource> Hooks { get; }
    public Func<InstrumentSpec, Option<(string Slot, string Value)>, Unit> Count { get; }

    public static Fin<SurfaceRuntime> Open(
        MonotonicTimeline line,
        Rasm.Numerics.Dimension eventCapacity,
        EventExtensionContract<Rasm.Contracts.Event.Extensions> contract,
        Hlc clock,
        ...
        var tap = new HookTap<AppUiPoint, AppUiFact, TelemetrySource>(
            ...
            Observe: fact => fact.Event(AppUiTelemetry.Capability, publish)
                .Bind(request => RasmEventEnvelope.Publish(request, contract, clock, publish))
                .Bind(envelope => RasmEventEnvelope.Admit(envelope, contract, publish))
                .Bind(row => {
    ...
    public EvidenceSource Evidence() => new EvidenceSource.Live(events.Parked);
    ...
    public void Dispose() =>
        Release().IfFail(error => ignore(Hooks.Faults.Park(AppUiPoint.Surface.Id, error)));
}
...
public sealed record SurfaceRow(
    ...
    SurfaceMode Mode) {
    public Func<double> Scale => Port.Scale;
    public Func<Seq<PixelRect>> Displays => Port.Displays;
    public Func<bool> OnUiThread => Port.OnUiThread;
    public Func<Action<SurfaceFact>, IDisposable> Facts => Port.HostFacts;
}
...
public static class Surfaces {
    ...
    public static Fin<SurfaceRow> Row(ConsumptionProfile profile, SurfaceMount mount, SurfacePort port) =>
        from admitted in Admits(profile, mount)
        select mount.Switch(
            state: (Port: port, Held: profile.Held),
            panel: static (s, own) => Embedded(s.Port, s.Port.PanelMount(own.PanelId), s.Held),
            modal: static (s, own) => Embedded(s.Port, s.Port.ModalMount, s.Held),
            companion: static (s, own) => Embedded(s.Port, s.Port.CompanionMount, s.Held),
            standalone: static (s, own) => Rooted(s.Port, s.Held,
                static b => b.UsePlatformDetect().With(SkiaBudget).UseReactiveUI(), s.Port.RunLoop, SurfaceMode.Interactive),
            offscreen: static (s, own) => Rooted(s.Port, s.Held,
    ...
    public static Fin<Unit> Boot(
        ConsumptionProfile profile, SurfaceMount mount, SurfacePort port, SurfaceRuntime runtime, Func<AppBuilder> entry) =>
        from row in Row(profile, mount, port)
    ...
    public static Fin<SurfaceSession> Mount(
        ConsumptionProfile profile, SurfaceMount mount, SurfacePort port, Control content, SurfaceRuntime runtime) =>
        from row in Row(profile, mount, port)
        from gate in SurfaceScheduler.For(row, runtime).Affinity(nameof(Mount))
        from session in Gauged(runtime, MountLane.Mount, MountOp, () => Seated(row, profile, content, runtime))
        select session;

    private static Fin<SurfaceSession> Seated(
        SurfaceRow row, ConsumptionProfile profile, Control content, SurfaceRuntime runtime) =>
        from assets in Identified(runtime)
        from attached in row.Attach(content)
        let session = new SurfaceSession(assets, row.Reach, row.Displays, Counted(row, runtime), attached.Teardown)
        from fired in runtime.Hooks.Fire(
            AppUiPoint.Surface,
            new AppUiFact.Surface(profile.HostKey, attached.Descriptor, row.Scale(), attached.Handle.Map(static value => value.ToString())),
            MountOp,
            body: _ => Fin.Succ((runtime.Count(Mounted, Some((AppUiTelemetry.HostSlot, profile.HostKey))), session).Item2))
        select fired;
    ...
    private static Fin<AssetCensus> Identified(SurfaceRuntime runtime) =>
        NativeAssets.Current
            ...
            .Bind(census => census.Present
                .Traverse(fact => runtime.Hooks.Fire(
                    AppUiPoint.NativeAsset,
                    new AppUiFact.NativeAssetIdentity(fact),
                    MountOp,
                    body: _ => Fin.Succ(runtime.Count(NativeAssets.Resolved, Some((AppUiTelemetry.LibrarySlot, fact.Library))))))
                .As()
                .Map(_ => census));
    ...
    private static Fin<T> Measured<T>(SurfaceRuntime runtime, (Fin<T> Value, GaugedSpan<MountLane> Span) measured) =>
        runtime.Hooks.Fire(
            AppUiPoint.DispatcherLag,
            new AppUiFact.DispatcherLag(measured.Span.Lane.Key, Duration.FromTimeSpan(measured.Span.Elapsed)),
            measured.Span.Work,
            body: _ => measured.Value);
    ...
    private static SurfaceRow Embedded(SurfacePort port, Func<EmbedCapsule, Fin<IDisposable>> mount, CapabilitySet<HostCapability> held) => new(
        Port: port,
        Build: static builder => EmbedOptions.Embedded.Admit(builder),
        Start: Setup,
        Marshal: port.HostMarshal,
        Attach: content => new EmbedCapsule(content).Mounted(mount, port.ReleaseRetainedView),
        Reach: Reaches(held),
        Mode: SurfaceMode.Interactive);

    private static SurfaceRow Rooted(
        SurfacePort port, CapabilitySet<HostCapability> held,
...
- Entry: `SurfaceScheduler For(SurfaceRow row, SurfaceRuntime runtime, Option<TimeProvider> virtualTime = default)` — pure projection over the resolved row and the composition runtime; the UI-thread predicate reaches the row through its port, the deterministic-time capability is the mode row's own admission, and the count column is sourced once from the runtime, so no parallel host discriminator, `onUiThread` parameter, or scheduler-local meter threads beside them.
...
- Boundary: `Affinity` is the single thread-affinity assertion and a per-call-site access check is the rejected form; the UI-thread predicate originates once at the port's `OnUiThread` column and reaches the scheduler through `row.OnUiThread`, which is the port's own column read through the row — one source, no parallel parameter — and that column answers true on exactly ONE thread per process, the host's own main thread, where `Dispatcher.UIThread.CheckAccess()` also answers true, while every other thread reads false and its `Post` lands back on that same thread, so the marshal column, the dispatcher, and the affinity assertion all read one boundary and a per-host thread regime is unrepresentable; so the access-assertion spelling stays a port delegate and never a hardcoded dispatcher call inside a dispatch arm; a failed `Affinity` assertion folds its `SurfaceFault.ThreadAffinity` into the `Surfaces.Affinity` count through the one `AppUiTelemetry.Contribute` spine, so off-thread access is counted evidence on the timeline and a scheduler-local meter is the deleted form; embedded mounts marshal through the port's host column, standalone and offscreen mounts post through the `AvaloniaScheduler` UI scheduler; the virtual `TimeProvider` a test composition supplies is admitted by the MODE row rather than by this projection, so an interactive mount handed one drops it and only the command-journal replay lane runs under deterministic time; `ObserveOn` rides `Ui` exactly once inside binding capsules, never at call sites.
...
- Owner: `RuntimeId` — the RID value object whose admission is what keeps a blank row from claiming every host; `NativeAssetRow` — per-RID asset rows; `NativeAssets` — the frozen row table, the live-RID resolution, and the identity census. Load identity is the `Render/capture` `NativeAssetFact` the evidence union already carries.
...
- Evidence: `NativeAssetFact` carries the library, optional file version, path, and RID; the census rides `SurfaceSession.Assets`, every `Present` row fires `AppUiFact.NativeAssetIdentity`, and the mount transaction counts every `Absent` row on the asset-absent instrument, so a missing-architecture load is a counted absence on the spine.
...
public static class NativeAssets {
    ...
    public static readonly InstrumentSpec Resolved = InstrumentSpec.Create(
        ...
        "native assets resolved by library", Seq(AppUiTelemetry.LibrarySlot), None, None, None);
