...
|  [03]   | `ProcessPool.map(function, *iterables, timeout=None, chunksize=1)` | submit map     | chunked map; `timeout` applies per chunk         |
...
|  [06]   | `ThreadPool.map(function, *iterables, timeout=None, chunksize=1)`  | submit map     | chunked map; `timeout` bounds the wait only      |
...
[STACKING]:
- `execution/workers`(`runtime/.planning/execution/workers.md`): `WorkerPool` binds pebble to the one `(WorkerKind.PROCESS, Enforcement.TERMINAL)` arm key — `pebble.ProcessPool(max_workers=loky.cpu_count(only_physical_cores=True), max_tasks=64, initializer=fidelity)` — and `submit` drives `self._pebble.schedule(shipped, args=(kernel, *args), timeout=kernel.deadline.to_optional())`; `drain()` calls `close()`+`join()`, `retire()` calls `stop()`+`join()` and drops the arm memo. Worker death is typed at this boundary: `ProcessExpired(pid, exitcode)` or `TimeoutError` converts onto the fault rail, so pebble owns the terminal-kill arm and the pool owns the executor lifecycle.
...
- `cloudpickle`(`.api/cloudpickle.md`): the stdlib-pickle seam's escape hatch — `Kernel.of` calls `cloudpickle.dumps(fn)` into the Struct `payload` the instant a qualname carries `<lambda>` or `<locals>`, and the worker-floor `shipped` gate calls `cloudpickle.loads(kernel.payload)` far-side; pebble transports only the reference-picklable `shipped` and the stdlib-picklable `Kernel` carrying those bytes, so a closure crosses pebble's ForkingPickler as data rather than degrading to a thread.
...
- `anyio`(`.api/anyio.md`): pebble's `schedule`/`submit` return a `concurrent.futures`-shaped `ProcessFuture` whose blocking `.result()` bridges off the loop through `anyio.to_thread.run_sync(pending().result)` — pebble owns the killable worker, anyio owns the non-blocking await. Distinct from `anyio.to_process.run_sync`, the COOPERATIVE pickle-hop subprocess anyio owns for the non-terminal `PROCESS` arm, which cannot interrupt a native loop — the exact gap pebble fills.
