...
[DISTILL] - one pass after every fix wave, run by the `reviewer-harvest` agent (`.claude/agents/reviewer-harvest.md`) — its definition owns the executor law (classify-first, dual framing, integration-over-accretion, per-surface grain, trim law, gates, typed return) and is the single versioned tuning surface:
- Distill the LESSON, never the instance: improvements land as end-state guidance naming what reviewers push toward; refuted classes harden guards — do-not-flag rows citing the refuting ruling; a recurring refuted class hardens the existing owner's wording in place, never a duplicate owner.
- Split by altitude: universal lessons land at global scope; language-bound gains land only in language-scoped rows, blocks, or files.
- Each reviewer surface receives the fact in its OWN idiom at its own grain, one owner per fact per surface; `docs/` receives a rule only when refute-first proves no reviewer surface owns it — most lessons are already legislated, and the gap is reviewer-config knowledge; an impressive universal pattern earns an additional snippet-first strengthening of the owning `docs/stacks/<language>/` fence, never forced.
- Sibling rulings sharing one root consolidate into one richer rule that REPLACES them — sharpening over adding — and touched reviewer prose obeys the trim law: identical guidance in fewer words, payload never dropped.
- Distillation commits and pushes to origin immediately, on its own lane: reviewer-config and docs hardening never waits on, mixes with, or rides the branch carrying the reviewed work — hosted reviewers read the indexed default branch, so an unpushed guard protects nothing. Commit scope on this lane is the reviewer configs, `docs/`, and `tools/`; every other surface stays uncommitted so iterative rounds keep reviewing it.
