...
class MediaFault:
    # the closed av-boundary cause vocabulary the whole media rail threads, the sibling of graphic/vector/region#REGION
    # `RegionFault` and graphic/raster/io#IO `RasterFault`: each case is structurally addressable so a downstream
...
# same `artifacts.media.container` module as the owner fence; the workers stay module-level so the runtime process lane
