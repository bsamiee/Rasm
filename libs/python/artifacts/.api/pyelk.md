...
Every exception descends from `ElkError`, the one diagram-layout provider failure `async_boundary` traps; its leaf subclasses preserve the provider cause for `BoundaryFault`. `validate_graph` checks id presence and shape, not duplicate ids, so the caller deduplicates upstream by `str(node_index)`.
...
`pyelk.options` resolves layout options: `ALIASES` maps short keys onto canonical `elk.*` keys, `DEFAULTS` seeds the global option floor, and the resolver helpers fold global → parent → element overrides into the effective option set. `LayeredPolicy` lowers its fields onto the `elk.*` keys — the option document is the knob surface, never a positional argument family — and `get_padding`/`get_spacing`/`get_effective_options` re-read a resolved value so a design page sizes a swimlane to the resolved container padding rather than re-deriving it.
...
`pyelk.graph` helpers (re-exported through `pyelk.elk`) operate on the ELK-JSON `dict` — index the node set, normalize edges, validate the document, size a compound graph, deep-copy before mutation. `validate_graph` fails a malformed adjacency frame at the boundary as a typed `InvalidGraphException` before the layout pass, `normalize_edges` canonicalizes the source/target shape, and `deep_copy_graph` keeps the input immutable while `layout` writes positions in place.
