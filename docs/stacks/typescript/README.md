...
| :-----: | :--------------------- | :---------------------- | :------ |
|   [1]   | owner and domain shape | shapes.md               | planned |
...
[SHAPE]:
- `SINGLE_SOURCE` — a concept has one source: a single rich owner fusing its runtime value with its static type. Its variant union, codec, equality, keys, and every projection derive from that owner, never written beside it; the owner is shaped to absorb the whole family it anchors, so new cases, fields, and modalities land inside it rather than as new artifacts. A type, schema, brand, or constant that restates what the owner already carries — or a nested owner standing in for one of its parts — is the defect the derivation deletes, and changing the owner reshapes every dependent site at compile time.
...
- `EVIDENCE_FUSION` — the static type is extracted from the one owner, never restated: `typeof`, `keyof`, indexed access, and `Schema.Schema.Type`/`Schema.Schema.Encoded` lift the type from the runtime declaration, and nominal identity, optionality, and defaults are inline field modifiers on the owner — never standalone interfaces, mirror types, or parallel brands.
