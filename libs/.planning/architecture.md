...
- TypeScript is the first-class host-free web/edge platform: with capability domains composing whole products; wire decode is a boundary concern inside `core` interchange, not the branch's identity. It consumes the C# wire and the GLB rail only and owns no geometry; its test infrastructure lives under `tests/`, never in the branch.
