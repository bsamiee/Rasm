...
- Owner: `ArtifactIndexRow` — content-addressed catalog row for execution-provider warm-start contexts, ONNX profiling traces, IFC semantic-ingest model graphs, content-defined chunks, and Compute interchange artifacts (tessellated GLB, chunked field, tile content, re-exported glTF) on the blob lane.
- Cases: `EpContext`, `OnnxProfile`, `IfcSemantic`, `ChunkContent`, `Interchange` kind rows.
