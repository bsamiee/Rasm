...
[STACKING]:
- `api-eto-platform`(`libs/csharp/.api/api-eto-platform.md`): the registered handler root this backend seats into — the kernel `Rasm/Interaction` platform estate owns the registered identity, registration, mint-event, and shared-instance reads, and `Platform/native.md` composes this partition's macOS answers.
