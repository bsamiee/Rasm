...
# the native-offload bounded slot the `of` path threads: the `_composed` fold offloads off the event
# loop through one `CapacityLimiter`-bounded `to_thread` band (no process seam) so the GIL-releasing
# `pymupdf`/`typst`/`weasyprint` render never blocks the loop; `to_thread` is forced for the whole fold —
# the pure-Python `reportlab` arm included — because the fold returns the `msgspec`-backed `Composed`
# owner the subinterpreter `to_interpreter` arm cannot load the C-extension for, so the shared-address
# thread is the only lane, distinct from the per-loop `current_default_thread_limiter()` default this slot replaces.
...
class Sheet(Struct, frozen=True):
    ...
    async def _keyed(self) -> ContentKey:
        # the `_composed` frame/place fold offloads off the loop through the bounded `to_thread` band so
        # the GIL-releasing `pymupdf`/`typst`/`weasyprint` render never blocks it; `contribute` re-enters
        # the same deterministic fold synchronously off the loop, the engine raise riding into `_FAULTS`.
...
def _composed(op: SheetOp) -> Composed:  # the one pure render fold both `of` and `contribute` read
    match op:
        ...
        case SheetOp(tag="place", place=(sheet, placements)):
            with pymupdf.open(stream=sheet, filetype="pdf") as document:  # the live native handle closes deterministically once the placed bytes are serialized
        ...
        case SheetOp(tag="stamp", stamp=(sheet, title, standard, intent, attachments)):
            with pymupdf.open(stream=sheet, filetype="pdf") as document:  # the live native handle closes once the stamped bytes are serialized
                ...
                for name, payload in (*attachments, *(((f"{intent[0]}.icc", intent[1]),) if intent is not None else ())):  # source files + the PDF/A ICC output-intent ride as associated files; the ternary is parenthesized so `*` unpacks its result (a bare `*x if c else y` is a SyntaxError)
...
def _frame_typst(dims: Dimensions, title: TitleBlock, standard: Standard, _intent: OutputIntent) -> bytes:
    # typst's PDF/A output-intent is the compiler's own; the ICC is not a frame-author input
    width, height = dims
