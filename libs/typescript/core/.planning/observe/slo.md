...
- Law: the windowed distribution backs NO indicator — every bound-bearing case either divides a bucket share (`Latency` through the `le` fan) or compares a level (`Saturation`, `Freshness`), and a summary answers neither: its reservoir reports the ranks its mint named and carries no bucket the share could divide, so a latency objective over one would compile a breach expression against a fan that does not exist. `_LatencyMetric` already refuses it by intersecting the histogram roster, so the carve is structural rather than a guard; a summary is the operator-facing read for a range no ladder pre-names, and a measure that must carry an objective takes a bucketed row instead.
