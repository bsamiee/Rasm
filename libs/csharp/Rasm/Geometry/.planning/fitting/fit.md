...
public sealed partial class FitKind {
    ...
    public static readonly FitKind Cylinder = new("cylinder", minimalSamples: 6, freeParameters: 5);
...
public static class Fitting {
    ...
    static (Matrix<double> J, Vector<double> R) Linearize(FitPrimitive shape, Point3d[] cloud, int[] inliers, FitKind kind, double[] parameters) {
        ...
        for (int row = 0; row < n; row++) r[row] = at.Distance(cloud[inliers[row]]);
        for (int col = 0; col < m; col++) {
            double[] forward = (double[])parameters.Clone();
            forward[col] += h;
            FitPrimitive bumped = Unpack(shape, forward);
            for (int row = 0; row < n; row++)
                j[row, col] = (bumped.Distance(cloud[inliers[row]]) - r[row]) / h;
        }
        return (j, r);
    }
    ...
    static double Norm(FitPrimitive shape, Point3d[] cloud, int[] inliers) {
        double sum = 0.0;
        foreach (int i in inliers) { double d = shape.Distance(cloud[i]); sum += d * d; }
    ...
    static double[] Pack(FitPrimitive shape) =>
        shape switch {
            ...
            FitPrimitive.Cylinder c => new[] { c.Surface.Center.Origin.X, c.Surface.Center.Origin.Y, c.Surface.Center.Origin.Z, Math.Atan2(c.Surface.Axis.Y, c.Surface.Axis.X), c.Surface.Radius },
    ...
    static FitPrimitive Unpack(FitPrimitive shape, double[] p) =>
        shape switch {
            ...
            FitPrimitive.Cylinder _ => new FitPrimitive.Cylinder(new Rhino.Geometry.Cylinder(new Circle(new Rhino.Geometry.Plane(new Point3d(p[0], p[1], p[2]), AxisFrom(p[3], Math.PI / 2.0)), Math.Max(p[4], 0.0)))),
...
flowchart LR
    ...
    Refine -->|JᵀJ + λ·diag → Cholesky| ConstraintSolver["constraints/solver iterate"]
...
| :-----: | :-------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------- | :------------------------------------ | :-----: |
|   [1]   | Primitive fit         | `FitOp`          | `[Union]` (`Plane`/`Sphere`/`Cylinder`/`Cone`/`Torus`/`Line`) over one `FitStore` + `Apply`                     | `Fitting.Apply → Fin<FitReceipt>`     |    6    |
...
|  [1b]   | Fitted geometry       | `FitPrimitive`   | `[Union]` (6 cases) over `Vectors` values + one `Distance` orthogonal-distance arm                              | `FitPrimitive.Distance → double` (pure) |    6    |
