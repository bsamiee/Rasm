...
- `Npgsql`/`EF`(`libs/dotnet/.api/api-npgsql.md`, `Rasm.Persistence/.api/api-nts-ef.md`): a PostGIS geometry column persists as binary and the SAME `Geometry` serializes to GeoJSON at the web egress boundary through these converters on the shared `JsonSerializerOptions` — the path stays provider-agnostic at the `Geometry` boundary and codec-specific only at the wire; one converter serves the Persistence API-response projection and the Bim site-context ingest.
