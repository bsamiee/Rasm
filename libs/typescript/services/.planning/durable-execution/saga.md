...
- Entry: the saga's success/error/payload schemas are the workflow's OWN schemas (success encodes `SagaTerminal`), so the saga workflow becomes callable only by feeding it into `InternalRpc.fromWorkflows` (messaging/internal-rpc#INTERNAL_RPC), which derives the start/`Discard`/`Resume` procedures over the SAME wire `Schema` the saga workflow defines — a saga-local proxy member, a second `RpcGroup`, or a hand-built saga DTO is the named defect. Every step boundary writes one `AgentJournal` row through a `Model.makeRepository` `insertVoid` over the one `SqlClient` `persistence/store-boundary#STORE_BOUNDARY` owns (`checkpoint` on forward commit, `failed` on compensation) so the saga's progress is durably replayable, never a parallel ledger.
