...
- version: `1.121.15`; floor `honeybee-energy; python_version<'3.15'`
...
- marker: COMPANION-GATED. Manifest-pinned `python_version<'3.15'` (co-released with the Honeybee/OpenStudio set), gated OUT of the active 3.15 interpreter; `assay api resolve honeybee-energy` returns `unsupported`. This catalog is verified against the real `1.121.15` wheel reflected on a sibling cp312 interpreter; the gate is manifest convenience (no ABI cap on this pure-Python wheel) and lifts on 3.15 validation.
- depends-on: `honeybee-core` (the object-model spine), `honeybee-standards` (the constructions/schedules/programs data backend), `ladybug-comfort`/`ladybug-core`; an external EnergyPlus install and the OpenStudio CLI are runtime requirements for the `run` rail (the simulation engine, not a Python dependency)
...
|  [04]   | `HVAC_TYPES_DICT` (20 templates)                        | HVAC registry    | `VAV`/`PVAV`/`PSZ`/`PTAC`/`FCU`/`VRF`/`WSHP`/`Baseboard`/`Radiant`/`ForcedAirFurnace`/`GasUnitHeater`/`EvaporativeCooler`/`WindowAC`/`Residential` (+ `*withDOAS` variants), keyed by name with per-template `equipment_type` vintage/efficiency vocabularies |
...
- rail: energy-modeling
- The blocking EnergyPlus/OpenStudio CLI boundary. `run_idf`/`run_osw` shell out to the external engine; the owner offloads them through `anyio.to_thread.run_sync` under a `stamina` retry and an `opentelemetry` span, never blocking the event loop.
...
|  [04]   | `to_openstudio_sim_folder(model, directory, epw_file=None, sim_par=None, ...)` | model + dir + epw        | one-call: write the full OpenStudio simulation folder from a `Model` |
...
- Accept: energy-property assignment feeding the energy-modeling owner; standards resolution through `lib.*_by_identifier`; abridged HBJSON validated through `honeybee-schema.energy` (the `pydantic` rail); the `run_idf`/`run_osw` subprocess offloaded through `anyio.to_thread.run_sync` under `stamina` + `opentelemetry`; result rows promoted to the `numpy`/`xarray` data tier; `check_all(detailed=True)` folded into the `expression` `Result` rail
