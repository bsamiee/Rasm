...
- Packages: `three` (`MeshPhysicalMaterial` lobes, `Color`, `LinearSRGBColorSpace`, `FrontSide`/`DoubleSide` — members verified against the shipped runtime; `@types/three` is not admitted); `@rasm/ts/core` (`Material`, `PbrGroups`, `AppearanceSummary`, `ContentKey`); `effect` (`HashMap`, `Option`).
