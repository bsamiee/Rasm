...
- Law: a write-sandbox gate reads each leaf for its write surface — a redirection target (`>`, `>>`, `&>`, `>|`, `n>`) or a write-verb destination, verb-specific (`tee` every operand, `dd` its `of=`, `cp`/`mv`/`ln`/`install` the final operand) — and canonicalizes and sandbox-checks it exactly as an `Edit`/`Write` `file_path`; `apply_patch` names its targets through `^\*\*\* (?:(?:Add|Update|Delete) File|Move to): (.+)$` inside `tool_input.command`, never a `file_path`, so a gate parses the envelope and checks each. A classifier reading only argv[0] or an absent `file_path` misses the write.
