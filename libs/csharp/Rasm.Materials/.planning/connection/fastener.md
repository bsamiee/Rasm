...
- [1]-[FASTENER_FAMILY]: the `FastenerKind` member-type discriminant, the `FastenerStandard` spec discriminant, the `FastenerGrade` proof/tensile axis, the `ThreadSize` nominal-thread axis, the `FastenerSection` thread receipt, and the `ConnectionCatalogue.BuildFastenerRows` ISO 898-1 / SAE J429 row table.
