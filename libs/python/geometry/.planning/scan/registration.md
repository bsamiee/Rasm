...
- [FALLBACK_CARD]: the `_engine` engine-selection read and the `_bootstrap_fgr` arm are the structure the usage card `[KISS_MATCHER_FALLBACK_FGR]` finalizes — when `kiss-matcher` is unavailable, the `GLOBAL` mode degrades to the `open3d` Fast Global Registration path that mints the coarse pose seeding the fine modes and multiway edges. The arm body is authored here as the dispatch row; the card owns the active worker-lane package-resolution selection and verification.
