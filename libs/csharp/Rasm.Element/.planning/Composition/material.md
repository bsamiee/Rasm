...
public readonly partial record struct SectionProperties(
 ...
 // The receipt's OWN canonical contribution (the MaterialComposition.CanonicalBytes co-location discipline): every
...
// The generic sampled-function carrier for STATE-DEPENDENT material properties — the measured-curve evidence the
...
// The Composition/acoustic banded-carrier pattern generalized to a DECLARED axis: Axis strictly increasing and
...
public sealed partial record SampledCurve {
 ...
 // The ONE finiteness span scan this owner and the MaterialPropertySet Matrix slot share.
...
public abstract partial class MaterialComposition {
 ...
 // admission through the Of factory (the relation#EDGE_ALGEBRA / acoustic#ACOUSTIC_FOLDS shape), so an empty
 ...
 // the typed set members in declared order, layer thicknesses quantized through MeasureValue. The ProfileSet
 ...
 public void CanonicalBytes(CanonicalWriter w) => Switch(
  ...
  layerSet: s => { w.Ordinal(1).Ordinal(s.Layers.Count); foreach (MaterialLayer l in s.Layers) { w.String(l.Material.Value).Measure(l.Thickness).String(l.LayerName); } return w; },
  profileSet: s => { w.Ordinal(2).String(s.Material.Value).String(s.Profile.Standard).String(s.Profile.Designation).U128(s.Profile.ContentKey).Bool(s.Section.IsSome); s.Section.IfSome(x => x.CanonicalBytes(w)); return w; },
  constituentSet: s => { w.Ordinal(3).Ordinal(s.Constituents.Count); foreach (MaterialConstituent c in s.Constituents) { w.String(c.Material.Value).String(c.Category).Double(c.Fraction); } return w; });
 ...
 // Each layer is an independent admission slot, so every bad row is retained with its index in one ManyErrors.
 public static Fin<MaterialComposition> OfLayerSet(Seq<MaterialLayer> layers, Op key) =>
  ...
  .Apply((_, _) => (MaterialComposition)LayerSet.Seed(layers))
  .As().ToFin();
 ...
 private static K<Validation<Error>, Unit> Gate(bool admits, Op key, string detail) =>
  admits ? Success<Error, Unit>(unit) : Fail<Error, Unit>(ElementFault.ValueRejected(key, detail));

 private static K<Validation<Error>, Unit> Accumulate(Seq<K<Validation<Error>, Unit>> slots) =>
  slots.Fold((K<Validation<Error>, Unit>)Success<Error, Unit>(unit), static (state, slot) => (state, slot).Apply(static (_, _) => unit).As());
}
```
...
public abstract partial class MaterialPropertySet {
 ...
 public sealed partial class Hygrothermal(double porosity, MeasureValue waterContent80Rh, MeasureValue freeWaterSaturation, Option<double> waterAbsorptionKgPerM2SqrtS, Option<SampledCurve> sorptionIsotherm, Option<SampledCurve> liquidTransport, Option<SampledCurve> moistureConductivity, PropertyEvidence evidence) : MaterialPropertySet(evidence) {
  ...
  // The MEASURED-FUNCTION evidence a WUFI-class transient solver runs on where the two-point anchors under-model —
  ...
  // w80/wf anchors, one isotherm truth never two), the liquid-transport table Dw(w) (axis kg/m³, values m²/s), and
  // the moisture-dependent conductivity λ(w) (axis kg/m³, values W/(m·K)).
 ...
 // one K-kinded Validation slot ([ADMISSION_SLOTS] owns the K<Validation<Error>,_> compile contract), the tuple
 ...
 // The relative isotherm-anchor agreement band the curve↔anchor refinement reads.
 ...
 // whose fault NAMES the offending column, so the tuple .Apply reports the union. The slot return type is the
 // K-KINDED K<Validation<Error>,_>, not the concrete Validation: the shipped tuple Apply binds ONLY on (K<F,A>, …)
 // receivers (classic extensions, 2..10-arity — decompile-verified 5.0.0-beta-77, no concrete-carrier tuple overload
 // exists), and a concrete-Validation tuple neither infers (invariant ValueTuple forces exact element inference
 // across distinct generic definitions) nor converts (a tuple conversion is not a legal extension-receiver
 // conversion) — the K return makes the tuple match by identity, and .As() re-anchors after the join. Guarded owns
 // only the lift + fault mint (the call site keeps its NaN-rejecting relational pattern); Positive gates a measured
 // column finite-and-strictly-positive (the raw-double form after the MeasureValue.Of SI coercion, the MeasureValue
 // form direct); PositiveSi guards-then-mints a consumer-domain quantity through the TYPED OfSi (the registry-less
 // signatures); OptionalPositive/Rayleigh gate Option-carried columns when Some and pass None (absence is legal,
 // never a sentinel). Every ternary target-types the K-kinded slot through the Validation lift (value implicit on
 // the success arm, the bare fault on the fail arm, the reference upcast to K riding behind both) — the spelled-out
 // cast is the deleted ceremony.
 ...
 private static K<Validation<Error>, Option<double>> OptionalPositive(Option<double> value, string name, Op key) =>
...
- [SAMPLED_CURVE]: the transient simulations the seam targets consume material FUNCTIONS, not scalars — EN 15026/WUFI runs on the full sorption isotherm `w(φ)`, the liquid-transport coefficient `Dw(w)`, and the moisture-dependent conductivity `λ(w)`, per-material measured curves the WUFI database ships — so `SampledCurve` generalizes the `Composition/acoustic#ACOUSTIC_FOLDS` banded-carrier pattern to a DECLARED axis: two `[OrderedEquality]` `ImmutableArray<double>` stores (strictly-increasing finite `Axis`, matching-arity finite `Values`) admitted once at `SampledCurve.Of`, the railed piecewise-linear clamped `At(x, key)` query and owner-internal `AtAdmitted(x)` derived read (the WUFI table-interpolation convention — a fitted parametric form beside the samples is the drift defect), and a `CanonicalBytes` projection that count-prefixes each adjacent `Axis`/`Values` run independently through the `Double` canon so framing stays injective when the schema evolves and a re-sampled curve forks the property identity. The `Hygrothermal` case carries the three curves `Option`-beside its two-point anchors — the anchors stay the required minimum every datasheet carries, and the `OfHygrothermal` composite refinement enforces curve↔anchor agreement (`AtAdmitted(0.8)`↔`w80`, `AtAdmitted(1.0)`↔`wf`, the relative `IsothermAnchorTolerance` band) so the two stores of one isotherm can never disagree (DERIVED_LOGIC's one-truth discipline over redundant evidence); a state-dependent column (a fire route's temperature-dependent stiffness, an advanced energy route's `λ(T)`) is one `Option<SampledCurve>` column on its owning case, the carrier itself unit-neutral with units riding the owning column's declaration.
- [STRUCTURAL_EQUALITY_DRILL]: `MaterialComposition` and `MaterialPropertySet` are CLASS-root `[Union]` + `[Equatable]` (the `[GRAPH_FAMILY]` form), and `MaterialLayer`/`MaterialConstituent`/`SectionProperties` are `[Equatable]` record structs — the `Graph/element#NODE_MODEL` `Node.Material` `[Equatable]` drill descends ONE `[Equatable]` link per hop, so a record-root `MaterialComposition`/`MaterialPropertySet` is an opaque equality leaf and the `Rasm.Persistence` 3-way `StructuralMerge` keys `Nodes[id].Composition` / `Nodes[id].Properties[i]` WHOLE rather than localizing a changed layer thickness / constituent fraction / section column / property column to `Nodes[id].Composition.Layers[2].Thickness` etc. (the RFC 6902 patch member granularity the merge keys its egress on). A class-root `[Union]` surrenders Thinktecture's record-generated equality (so `[Equatable]` is the SOLE equality owner, never stacked on a record-root union) but keeps the generated `Switch`/`Map`; a class-root case has no compiler `with`, so `ProfileSet.With`/`WithSection` reconstruct through the public positional ctor. The drill bottoms at the `Properties/quantity#MEASURE_VALUE` `MeasureValue` leaf (a native-equality record struct the `Generator.Equals` `DefaultEqualityComparer` compares atomically — the measure IS the leaf, no `[Equatable]` needed), the `Composition/acoustic#ACOUSTIC_FOLDS` `Acoustic` carrier the `MaterialPropertySet.Acoustic` case wraps (itself `[Equatable]` there), and the flat `ImmutableArray<double>` impact matrix (`[OrderedEquality]` content equality).
