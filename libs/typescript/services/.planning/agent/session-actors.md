...
The agent-session-as-actor binding — `AgentSessionActor`, the addressable `@effect/cluster` `Entity` that pins one long-lived agent conversation to one shard single-writer-per-key, so a multi-turn agent session is a sharded actor whose mailbox serializes concurrent turns rather than racing two writers on one conversation. It is the third member of the one `./agent` subpath closure: where `folder:agent/agent-runtime#AGENT_RUNTIME` owns the per-step durable agent composition and `folder:agent/mcp-transport#MCP_TRANSPORT` owns the host tool transport, this page owns the SESSION lifecycle as a placed actor. The actor reuses the `folder:messaging/entity#ENTITY` addressable-actor placement mechanics (`Entity.make`/`toLayer`/`getShardId`, `ClusterSchema.Persisted`) and the `folder:agent/agent-runtime#AGENT_RUNTIME` `DurableAgent` step; the session STATE — the `AgentJournal` cursor — persists through the one `folder:persistence/store-boundary#STORE_BOUNDARY` `PgClient`, never a parallel store, while the live per-session RESOURCE — the persisted `Chat` handle and the host tool-transport session — is held through the `folder:messaging/entity#ENTITY` `EntityResource` ROW bound to the session key (acquired once on `Start`, shared across every `Turn`, released on `End` or idle-TTL eviction), never re-acquired per `Turn`. This page is node-only and crosses no .NET wire beyond the agent's own tool transport.
