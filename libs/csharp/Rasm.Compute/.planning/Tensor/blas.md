...
- Owner: `ComparerAccessors.StringOrdinal` accessor; `LinearProvider` `[SmartEnum<string>]` RID-keyed MathNet-provider rows carrying the `Control.TryUse*` probe and `Control.Use*` activate delegates as inline row columns; `DenseSubstrate` `[SmartEnum<string>]` the execution-substrate axis choosing the managed `Matrix<double>` route or the native `torch.linalg` ATen leg, each carrying its `Available` probe and a substrate-determinism tag; `FactorRoute` `[Union]` shape-spine carrying mode/symmetricity/vector-demand/rank-tolerance as case data so the operand structure selects the factorization; `Admission` the one-pass finite/symmetry/singular gate; `TolerancePolicy` the scale-derived threshold record; `Factorization` `[Union]` one-case-per-decomposition collapsing to one held solve admission; `AtenFloor`/`AtenDense` the native-substrate runtime probe and route-discriminated `torch.linalg` solve leg; `DenseRoute`/`DenseOps` the shape-routed solve, held-handle refinement, and spectral folds over MathNet `Matrix<double>`; `SolveTerminal` `[Union]` partitioning the verdict so budget-exhaustion survives as a retryable case.
...
public static class Admission {
    ...
    // MathNet `Matrix<double>.Cholesky()` THROWS ArgumentException on a non-square or non-PD operand, so the
    // SPD gate wraps it in `Try.lift` (a bare `spd.Cholesky()` in a `Fin`-returning expression is the illusory
    // no-throw form); DeterminantLn finiteness is the second gate, catching a degenerate factor that did not throw.
...
// ATen dense leg `DenseRoute.Solve` dispatches to when `DenseSubstrate.Active` is `NativeAten`. It
// route-discriminates the native factorization by the SAME `FactorRoute` case the managed leg switches on,
// never collapsing every square operand onto one general `solve_ex` that discards the SPD/symmetric structure.
// `torch.inference_mode(true)` is the PUBLIC no-grad scope — `TorchSharp.InferenceMode` is internal, unconstructable
// from a consumer — and only the egressed `double[]` crosses the boundary, so no `Tensor` escapes onto the wire.
// The un-witnessed solution returns to `DenseRoute.Solve`, where the ONE shared `Witness` gate against the
// ORIGINAL MathNet operator admits both legs identically.
...
public static class LevenbergMarquardt {
    // The canonical exact-Jacobian arm: the residual is authored ONCE over the HyperJet hyper-dual scalar,
    // DDScalar.Variables seeds the n parameters, each residual component's GetGradient() IS the exact
    // Jacobian row, and the primal Value rides free — no second residual authoring, no finite differences.
    public static Fin<LmResult> Minimize(Func<DDScalar[], DDScalar[]> residual, Vector<double> initial, LmPolicy policy) {
        // Memoize the hyper-dual pass by theta reference: the inner fold evaluates residual then jacobian on the
        // SAME theta per step, so one DDScalar.Variables seeding feeds both the Value vector and the GetGradient()
        // rows — never a second AD pass that doubles work and desyncs a stateful residual.
        ...
        return Minimize(
            ...
            theta => {
                DDScalar[] r = At(theta);
                var j = Matrix<double>.Build.Dense(r.Length, theta.Count);
