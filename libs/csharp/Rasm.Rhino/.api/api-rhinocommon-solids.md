...
|  [11]   | `SurfaceFilletBase`             | class engine   | nine static profile factories filling caller-owned fillet lists  |
...
- `Rhino.Geometry.SurfaceFilletBase` — nine static profile factories (`CreateRationalArcsFilletSrf`, `CreateNonRationalQuinticArcsFilletSrf`, `CreateNonRationalQuinticFilletSrf`, `CreateNonRationalQuarticArcsFilletSrf`, `CreateNonRationalQuarticFilletSrf`, `CreateNonRationalCubicArcsFilletSrf`, `CreateNonRationalCubicFilletSrf`, `CreateG2ChordalQuinticFilletSrf` and kin), each `(BrepFace faceA, Point2d uvA, BrepFace faceB, Point2d uvB, double radius, double tolerance, List<Brep> trimmedBrepsA, List<Brep> trimmedBrepsB, int rail_degree, [double TanSlider, double InnerSlider,] bool bTrim, bool bExtend, List<Brep> Fillets) : bool` filling caller-owned lists; the concrete `SurfaceFillet`/`SurfaceToRailFillet` engines are internal.
