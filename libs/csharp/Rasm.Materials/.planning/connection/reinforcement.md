...
- [IFC_REINFORCING_WIRE]: a reinforcing bar round-trips to IFC 4.3 as an `IfcReinforcingBar` (`PredefinedType` ∈ {MAIN, LIGATURE, STUD, PUNCHING, RING, SHEAR, EDGE, ANCHORING, SPACEBAR}) carrying `NominalDiameter` (the `RebarSection.DiameterMm`), `CrossSectionArea` (`NominalAreaMm2`), `BarLength`, `BarSurface` (PLAIN | TEXTURED), and `SteelGrade` (the `RebarGrade.Key`); a reinforcement mesh round-trips as an `IfcReinforcingMesh` (`MeshLength`/`MeshWidth`/`LongitudinalBarNominalDiameter`/`TransverseBarNominalDiameter`/`LongitudinalBarSpacing`/`TransverseBarSpacing`) over the same `RebarSection` columns. The wire mapping is the `Rasm.Bim` boundary projection (`MaterialId`-keyed to the `properties#MATERIAL_PROPERTY` `Mechanical` `YieldStrengthMpa` the `SteelGrade` asserts), host-neutral here — this page emits the scalar columns the wire reads, never an IFC entity. The `RebarBend` ACI 318 hook geometry maps to the `IfcReinforcingBar` `BendingShapeCode`/`BendingParameters` (the ISO 3766 / BS 8666 schedule shape) the federation reads, the angle and bend-diameter the scalar tuple already carries.
