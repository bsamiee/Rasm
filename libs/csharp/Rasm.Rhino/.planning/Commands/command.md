...
- Owner: `RasmCommand<TSelf,TState>` — the abstract `Rhino.Commands.Command` derivation: identity, the flow declaration, the seed, and the policy row are the whole subclass surface. `CommandRegistry` — the typed projection over the static host registry. `CommandPulse` `[SmartEnum<int>]` — the begin/end/undo/prompt event rows binding the static host events through the document `Subscription` algebra.
...
- RESEARCH: the `CommandEventArgs`, `UndoRedoEventArgs`, and `CommandPromptChangedEventArgs` member surfaces and the `MostRecentCommandDescription` member spellings are unverified — the pulse rows deliver signal-only payloads and `Recent` answers a tally until the members verify, at which point the payloads become typed projections with zero new surface.
...
public sealed partial class CommandPulse {
    ...
    public static readonly CommandPulse UndoRedo = new(key: 2, observe: static deliver => Subscription.Attach(
        ...
        handler: (_, _) => ignore(deliver(arg: unit))));
    public static readonly CommandPulse PromptChanged = new(key: 3, observe: static deliver => Subscription.Attach(
