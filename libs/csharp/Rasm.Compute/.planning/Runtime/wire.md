    ...
    private static Fin<Option<FieldMask>> Numbered(Seq<int> numbers) =>
        numbers.IsEmpty
            ...
            : Try.lift(() => Some(FieldMask.FromFieldNumbers<QueryResponse>(numbers))).Run()
                .MapFail(static error => (Error)new ComputeFault.PayloadOverBounds($"<query-field-unknown:{error.Message}>"));
...
```proto signature
// Header of the corpus-homed suite contract file `rasm/compute/v1/compute.proto`. Package and namespace are declared HERE and
// nowhere else: the package qualifies every `FullName` the drift surface keys on and every service spelling a peer
// dials, and the namespace states the same family the package does, so one coordinate moves when a family moves.
syntax = "proto3";
...
- [03]-[GENERATE]: remote token streaming rides a correlation-keyed server stream; faults ride FaultDetail
...
- [06]-[TESSELLATE]: bridges tessellation to the companion over two hops — `Runtime/codecs#TWO_HOP_TESSELLATION` owns the `TessellationJob` build and the receipt admission, the python geometry serve end implements the rpc against the same two messages and projects its fully-qualified name from this mint, and faults ride FaultDetail
...
- [15]-[SYNC]: frame law below; FieldMask partials; Any artifact envelopes
...
- [21]-[FAULTDETAIL]: `package=1 string; code=2 int32; case=3 string; message=4 string; evidence=5 map<string,string>; correlation=6 string; hlc_physical=7 google.protobuf.Timestamp; hlc_logical=8 uint64; tenant=9 string` — the one typed fault payload every trailer and in-band conflict slot carries; `tenant` renders the `rasm.tenant` dimension through `TenantId.Wire` so a peer attributes without a second join, the untagged whole crossing empty
...
- [24]-[DISPATCHTOOLREQUEST]: `tool=1 string; arguments=2 google.protobuf.Struct` — the requested tool key beside its open argument envelope, admitted within the additive-only contract
...
- [30]-[DISPATCHRECEIPT]: `executor=1 string; selection=2 string; elapsed=3 google.protobuf.Duration` — projects `DispatchReceipt`, the dispatch evidence the command spine decodes at its seam, so no executing stratum's own receipt type reaches the envelope
- [31]-[COMMANDREPLY]: `descriptor=1 string; txn=2 string; reason=3 string; refusal=4 FaultDetail; compensation=5 DispatchReceipt; charged=6 map<string,int64>; elapsed=7 google.protobuf.Duration; correlation=8 string; tenant=9 string; tenant_slug=10 string; at=11 google.protobuf.Timestamp; dispatch=12 DispatchReceipt` — projects `CommandReceipt` field-for-field, `charged` rendering the `CostVector` map under its `CostUnit` keys and tenancy crossing as the `TenantId.Wire` text beside its slug
    ...
    public Fin<T> Read<T>(MessageParser<T> parser, ReadOnlySequence<byte> payload) where T : IBufferMessage, IMessage<T> =>
        payload.Length > SizeLimitBytes
            ...
            : Try.lift(() => parser.WithExtensionRegistry(Extensions).ParseFrom(payload)).Run()
                .MapFail(static error => new ComputeFault.PayloadOverBounds($"<inbound-parse-rejected:{error.Message}>"));
...
public static class ContractGuard {
    ...
    public static Fin<Seq<FileDescriptor>> Build(Seq<ByteString> serialized) =>
        serialized.IsEmpty
            ? Fin.Fail<Seq<FileDescriptor>>(new ComputeFault.EquivalenceMiss("<descriptor-set-empty>"))
            : Try.lift(() => toSeq(FileDescriptor.BuildFromByteStrings(serialized)))
                .Run()
    ...
    private static Seq<string> MessageSurface(MessageDescriptor message) =>
        Seq($"{message.FullName}:message")
            ...
            .Concat(toSeq(message.ToProto().ReservedRange).Map(range => $"{message.FullName}.reserved:{range.Start}-{range.End}"))
    ...
    public static Fin<Option<FaultDetail>> Decode(RpcException error) =>
        Optional(error.Trailers.GetValueBytes(DetailsTrailer)).Match(
            ...
            Some: static bytes => Try.lift(() => Google.Rpc.Status.Parser.ParseFrom(bytes))
                .Run()
                .Map(static rich => toSeq(rich.Details)
...
type FaultDetailWire<TCase extends string = string> = { package: RasmPackage; code: number; case: TCase; message: string; evidence: Record<string, string>; correlation: string; hlcPhysical: string; hlcLogical: bigint; tenant: string; };
