...
- Land `xxhash` in the synced `.venv` and prove the runtime `identity/content-identity` `XxHash128`/`XxHash3` digest-endianness reproduces the C# `System.IO.Hashing` seed bit-identically, so the one content key the whole branch trusts is verified before any consumer keys an output by it.
...
- Draws on the gap that `xxhash` is the one manifest-declared package (`xxhash>=3.7.0`) absent from the provisioning host, so its digest endianness against the C# seam is the single unverified link in the content-identity chain every package and the wire depend on; the parity is a reflection-and-fixture gate, not a design open question.

[ASYNC_BOUNDARY_ONE_RAIL]:
- Hold the runtime `reliability/faults` fault lift as one rail family with a synchronous `boundary` and an awaitable `async_boundary` sibling, so every consumer crossing the `anyio.to_process.run_sync` subprocess seam — the five gated `artifacts` producers (charts, scene3d, documents, compression, imaging) — converts the gated-band fault exactly once over an awaited thunk rather than passing a coroutine to the sync lift.
- Unlocks a truthful async boundary across the gated-render floor: a gated arm awaited across the subprocess seam returns `RuntimeRail[ContentKey]`, not `RuntimeRail[Coroutine]`, so the host-free render rail carries the same one-fault-conversion law the synchronous owners hold.
- Draws on the gap that the runtime `boundary` was synchronous-only while the gated `artifacts` pages bet on awaiting `to_process.run_sync`, so each gated producer silently lifted an un-awaited coroutine; the async sibling is one entry on the existing rail, never a parallel async fault owner.

[API_CATALOGUE_COVERAGE]:
- Author the missing folder `.api/<pkg>.md` catalogue every finalized fence names as a load-bearing code span yet no catalogue covers, drawing each real surface from a uv-sync reflection pass on the resolving interpreter rather than guessing: the compute JAX-scientific and inference families (`optimistix`, `lineax`, `diffrax`, `equinox`, `numpyro`, `nutpie`, `pymc`, `arviz`, `SALib`, `python-flint`, `mpmath`, `array-api-compat`, `array-api-extra`, `scikit-fem`, `skl2onnx`), the data graph/columnar/interop families (`rustworkx`, `zarr`, `cubed`, `awkward`, `narwhals`, `arro3-core`, `nanoarrow`), and the runtime capture quartet reclassified by resolvability.
- Unlocks the one-catalogue-per-library `[1]-[DOC_SET]` invariant holding across the branch: a finalized fence's external members are settled only against a present catalogue, so the FEM page's `skfem` spellings, the JAX solver `Solution.stats` spellings, the inference `SamplerBackend` spellings, and the data `narwhals`/`arro3-core`/`nanoarrow` interop and `rustworkx` graph spellings each graduate from `RESEARCH` to fence code only once the owning catalogue lands.
- Draws on the gap that the folder-tier finalized fences name these external members as settled while the owning `compute/.api/` and `data/.api/` catalogues are absent — the `[6]-[REVIEW]` fence-truthfulness rule fails any fence whose external member has no present catalogue — and that the runtime capture task conflated three core-resolvable spellings (`obstore`/`apscheduler`/`keyring`) with the install-gated `xxhash`. Each catalogue is captured on the interpreter the package resolves on: the cp315 core for the pure-Python rows (`mpmath`/`array-api-compat`/`array-api-extra`/`narwhals`/`SALib`), the gated `<'3.15'` jaxlib/FEM/Arb band for the native rows.
