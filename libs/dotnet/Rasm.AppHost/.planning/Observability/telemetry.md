...
- Stamps ride the envelope per this section's Law, so no slot occupies a trace or span id and no slot widens the promoted-baggage allowlist — the allowlist stays the tenancy and correlation pair, and a stamp promoted onto spans puts a per-event value on a dimension every series groups by.
...
- Owner: `LogPipeline` `[SmartEnum<string>]` arbitration column; `SpineLog` generated delegates; `SpineSampler` the branch's per-entry volume verdict; `BufferScope` the hold vocabulary and `IncidentBuffers` the two-scope hold-and-replay seat; `SpineLossFact`/`SpineLossFold` the sink-failure listener and its receipt-borne fact; `SerilogProjectionPolicy` the shaping surface; `SerilogSinks` the concrete app-root sink set behind the pipeline arbitration; `SerilogHost` the service-aware bridge.
...
public sealed class SpineLossFold(Action<SpineLossFact> emit, Redactor redactor) : ILoggingFailureListener {
    ...
    public void OnLoggingFailed(object sender, LoggingFailureKind kind, string message, IReadOnlyCollection<LogEvent>? events, Exception? exception) =>
        emit(new SpineLossFact(
            sender.GetType().Name,
            kind,
            Bounded(redactor, message, exception),
            events?.Count ?? 0,
    ...
    static string Bounded(Redactor redactor, string message, Exception? exception) {
        ...
        if (exception is not null) {
            RedactedText.Appended(composed.Append(": "), redactor, exception.Message);
        }
        return composed.Length <= DetailCap ? composed.ToString() : composed.ToString(0, DetailCap);
    }
}
...
public sealed record SerilogSinks(
    ...
    static Fin<SerilogSinks> Rooted(
        (IBatchedLogEventSink Wire, string Root, string Host, long Cap, IDestructuringPolicy Shaper, ILoggingFailureListener Loss) row) =>
        row.Root.Length > 0 && row.Host.Length > 0
            ? Fin.Succ(new SerilogSinks(
                ...
                Loss: row.Loss))
            : Fin.Fail<SerilogSinks>(new TelemetryFault.Composition($"serilog-durable-residence:{row.Host}"));
}
...
- Owner: `TelemetryDomain` `[SmartEnum<string>]` the branch domain roster under Tier-0 `[08]-[OBSERVABILITY_CONFORMANCE]` — each row a capability subject carrying its `Head`/`Measure` name projections, so every `rasm.*` instrument name and rasm-owned dimension key in this branch resolves a row or refuses at admission; `SignalCapability` `[SmartEnum<string>] : ICapability<SignalCapability>` the per-signal posture vocabulary and `TelemetrySignal` `[SmartEnum<string>]` the four governance rows carrying that set beside their ratio column; `TelemetryFault` `[Union]` the page's banded refusal family; `ResourceIdentity` the one detector-composed `Action<ResourceBuilder>` every provider owner consumes; `TelemetryComposition` the disposable composition capsule every governance entry folds — resolved row, boot correlation, determinism context, contributed roster, offline policy, receipt delegate, clock, timeline, the process `SpanBand` folded from the contributed trace planes, and the opened per-signal queue set; `SignalGovernance` the registration fold binding the `Observability/instruments#PROVIDER_LIFETIME` `ProviderProgram` onto the hosted root.
...
- Receipt: `OtlpOfflineFact` — the durable-egress evidence every queue outcome projects through the composition-injected receipt delegate onto the `Observability/instruments#RECEIPT_PROJECTION` `ReceiptKind.Offline` arm; `SpineLossFact` reaches `ReceiptKind.Loss` from `[04]`.
...
public sealed class TelemetryComposition : IDisposable {
    ...
    TelemetryComposition(ResolvedProfile resolved, CorrelationId root, DeterminismContext determinism,
        ...
        OtlpOfflinePolicy offline, Action<OtlpOfflineFact> emit, ClockPolicy clocks, IConfigurationSection hmacKeys,
        Func<DegradationLevel> level, FrozenDictionary<string, OtlpOfflineQueue> queues, SpanBand band) =>
        (Resolved, Root, Determinism, Contributors, Latency, Classifications, Offline, Emit, Clocks, HmacKeys, Level, Queues, Band) =
            (resolved, root, determinism, contributors, latency, classifications, offline, emit, clocks, hmacKeys, level, queues, band);
    ...
    public static TelemetryComposition Of(ResolvedProfile resolved, CorrelationId root, DeterminismContext determinism,
        Action<OtlpOfflineFact> emit, ClockPolicy clocks, IConfigurationSection hmacKeys, Func<DegradationLevel> level,
        ...
        Opened(resolved, root, determinism, [.. contributors], latency,
            OtlpOfflinePolicy.For(resolved), emit, clocks, hmacKeys, level);

    static TelemetryComposition Opened(ResolvedProfile resolved, CorrelationId root, DeterminismContext determinism,
        Seq<TelemetryContributorPort> contributors, Seq<LatencyRoster> latency, OtlpOfflinePolicy offline,
        Action<OtlpOfflineFact> emit, ClockPolicy clocks, IConfigurationSection hmacKeys, Func<DegradationLevel> level) =>
        new(resolved, root, determinism, contributors, latency,
            ...
            offline, emit, clocks, hmacKeys, level,
            offline.Open(emit, clocks.Line),
    ...
    Unit Released() {
        ...
        Band.Dispose();
        return ignore(toSeq(Queues.Values).Iter(static queue => queue.Dispose()));
    }
}
...
public static class ConformanceMint {
    ...
    static ConformanceReceipt Built(
        ...
        Seq<ConformanceRow> rows = toSeq(Seq(
            ...
            Row("metric.dimension.absence", "declared keys allowlist, absent entry shapes on the same row", $"{nameof(SignalGovernance)}.{nameof(SignalGovernance.Views)}"),
            Row("metric.process.series", "memory, virtual, cpu, threads, uptime", "AddProcessInstrumentation"),
            Row("propagation.dialect", "w3c-tracecontext + baggage, one global composite", $"{nameof(Correlation)}.{nameof(Correlation.Spine)}"),
            Row("propagation.registration", "one composite seated as the SDK default", "Sdk.SetDefaultTextMapPropagator"),
            Row("propagation.adoption", Joined(toSeq(TenantAdoption.Items).Map(static row => row.Key)), $"{nameof(TenantAdoption)}.Items"),
            Row("tenant.key", TenantContext.TenantSlot, $"{nameof(TenantContext)}.{nameof(TenantContext.TenantSlot)}"),
            Row("tenant.allowlist", Joined(Seq(TenantContext.TenantSlot, CorrelationId.Slot)), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.Baggage)}"),
            Row("sample.trace.ratio", Scalar(TelemetrySignal.Trace.Ratio(composition.Resolved.Profile)), $"{nameof(TelemetrySignal)}.{nameof(TelemetrySignal.Ratio)}"),
            Row("sample.log.floor", Scalar(TelemetrySignal.Log.Ratio(composition.Resolved.Profile)) + " at warning and below", "AddRandomProbabilisticSampler"),
            ...
            Row("egress.protocol", program.Protocol.ToString(), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.Protocol)}"),
            Row("egress.compression", program.Compression.ToString(), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.Compression)}"),
            Row("egress.translation", "prometheus utf8-preserving, dotted names, suffixes on", nameof(TelemetryDomain)),
            Row("egress.signals", Joined(toSeq(TelemetrySignal.Items).Filter(static row => row.Capabilities.Admits(SignalCapability.Exported)).Map(static row => row.Key)), $"{nameof(SignalCapability)}.{nameof(SignalCapability.Exported)}"),
            Row("egress.batch.span", Square(program.SpanBatch.MaxQueueSize, program.SpanBatch.MaxExportBatchSize, program.SpanBatch.ScheduledDelayMilliseconds, program.SpanBatch.ExporterTimeoutMilliseconds), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.SpanBatch)}"),
            Row("egress.batch.log", Square(program.LogBatch.MaxQueueSize, program.LogBatch.MaxExportBatchSize, program.LogBatch.ScheduledDelayMilliseconds, program.LogBatch.ExporterTimeoutMilliseconds), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.LogBatch)}"),
            Row("egress.reader.cadence", string.Create(CultureInfo.InvariantCulture,
                $"{program.ReaderCadence.ExportIntervalMilliseconds}/{program.ReaderCadence.ExportTimeoutMilliseconds}"), $"{nameof(ProviderProgram)}.{nameof(ProviderProgram.ReaderCadence)}"),
            Row("egress.mtls", Joined(Seq(OtlpTrust.AuthorityVariable, OtlpTrust.CertificateVariable, OtlpTrust.KeyVariable)), $"{nameof(OtlpTrust)}.{nameof(OtlpTrust.Mount)}"),
            Row("queue.arming", "durable root beside a bound otlp provider", $"{nameof(OtlpOfflinePolicy)}.{nameof(OtlpOfflinePolicy.For)}"),
            Row("queue.bounds", composition.Offline.Bounds, $"{nameof(OtlpOfflinePolicy)}.{nameof(OtlpOfflinePolicy.Bounds)}"),
            Row("queue.drain.bound", Scalar(DeadlineClass.OtlpDrain.Allotted.TotalSeconds) + "s gauged", $"{nameof(DeadlineClass)}.{nameof(DeadlineClass.OtlpDrain)}"),
            Row("queue.residence", "deploy volume over local disk, host-keyed", $"{nameof(BootVariable)}.{nameof(BootVariable.QueueRoot)}"),
            Row("queue.dispositions", Joined(toSeq(OfflineDisposition.Items).Map(static row => row.Key)), $"{nameof(OfflineDisposition)}.Items"),
            Row("queue.framing", "length and transfer-encoding re-derived on replay", $"{nameof(OtlpOfflineQueue)}.FramingHeaders"),
            Row("queue.transport", "synchronous and async handler legs", nameof(PersistentOtlpHandler)),
            Row("log.identity", "the resource projection, verbatim", nameof(RootEnricher)),
            Row("log.process.tags", "thread id; process id off", "AddProcessLogEnricher"),
            Row("log.pipeline", Joined(toSeq(LogPipeline.Items).Map(static row => row.Key)), $"{nameof(LogPipeline)}.{nameof(LogPipeline.Owner)}"),
            Row("log.sinks", "wire, shared host-keyed file, hot console, audit console", $"{nameof(SerilogSinks)}.{nameof(SerilogSinks.For)}"),
            Row("log.destructuring", "4 deep / 1024 chars / 64 items", $"{nameof(SerilogProjectionPolicy)}.{nameof(SerilogProjectionPolicy.Shape)}"),
            Row("log.event.stride", Scalar(FaultBand.SpineEvents.Key) + "-" + Scalar(FaultBand.SpineEvents.Key + FaultBand.SpineEvents.Span - 1), $"{nameof(FaultBand)}.{nameof(FaultBand.SpineEvents)}"),
            Row("latency.vocabulary", "own rows + every contributed roster, boot-strict", $"{nameof(LatencySpine)}.{nameof(LatencySpine.Register)}"),
            Row("latency.phases", Joined(toSeq(LatencyCheckpoint.Items).Map(static row => $"{row.Key}:{row.Pivot.Key}")), $"{nameof(LatencyCheckpoint)}.Items"),
            Row("latency.export", "drain-band flush", $"{nameof(LatencySpine)}.{nameof(LatencySpine.Seal)}"),
            Row("redaction.install", "the derived closure, never a bare seal", $"{nameof(RedactionRegistration)}.{nameof(RedactionRegistration.Bind)}"),
            Row("redaction.classes", Joined(toSeq(DataClassification.Items).Map(static row => $"{row.Key}:{row.Redactor.Key}:{row.Grade.Key}")), $"{nameof(DataClassification)}.Items"),
            Row("redaction.closure", "every non-empty subset, strongest member treatment", $"{nameof(RedactionRegistration)}.Closure"),
            Row("redaction.federation", "branch values prove against the roster at boot", $"{nameof(RedactionRegistration)}.{nameof(RedactionRegistration.Federated)}"),
            Row("redaction.tally", "one point per masked manifest entry, by classification", $"{nameof(AppHostMeasure)}.{nameof(AppHostMeasure.RedactionTags)}"),
            Row("profile.rail", "service-root profiler endpoint, span-linked", nameof(PyroscopeSpanProcessor)),
    ...
    static string Scalar<T>(T value) where T : IFormattable =>
        value.ToString(format: null, formatProvider: CultureInfo.InvariantCulture);

    static string Square(int queue, int batch, int delay, int timeout) =>
        string.Create(CultureInfo.InvariantCulture, $"{queue}/{batch}/{delay}/{timeout}");
}
```
