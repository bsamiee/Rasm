...
public sealed record KvEntry(
    ...
    public static Fin<KvEntry> Admit(
        ...
        key.Length == 0
            ? Fin.Fail<KvEntry>(Error.New("<kv-empty-key>"))
...
public static class KvLane {
    ...
    extension(StoreProfile profile) {
        public Fin<string> UpsertSql() =>
            profile.Switch(
                state: unit,
                postgresServer:   static (_, _) => Fin.Succ(UpsertGrammar),
                sqliteEmbedded:   static (_, _) => Fin.Succ(UpsertGrammar),
                sqliteMemory:     static (_, _) => Fin.Succ(UpsertGrammar),
                duckDbAnalytical: static (_, _) => Fin.Fail<string>(Error.New("<kv-no-row-store:duckdb-analytical>")),
                fileSnapshot:     static (_, _) => Fin.Fail<string>(Error.New("<kv-no-row-store:file-snapshot>")),
                blobRemote:       static (_, _) => Fin.Fail<string>(Error.New("<kv-no-row-store:blob-remote>")));
    }
}
```
...
- Entry: `public static EmbeddingIdentity Of(ReadOnlySpan<byte> content, string modelId, EmbeddingArity arity)` — pure value; embedding identity is content hash times model id times arity, so re-embedding the same content under a different precision dedupes per arity; `public static string Fuse<TRow>(VectorQuery<TRow> vector, FullTextQuery text, int k, bool pgSearch)` builds the `VectorBranch`/`LexicalBranch` `FusionBranch` pair and hands them to `FusionRank.FuseSql(Seq(vector, lexical), k, FusionRank.DefaultRrfConstant)`, so the two-branch hybrid and the three-branch `#FUSION_RANK` federated fusion are literally one fold at arity 2 and 3 and the `rrfConstant`/`Σ weight/(rrfConstant+rank)` algebra has exactly one owner.
...
public sealed partial class FullTextMode {
    ...
    // `pdb.*` builder (`.api/api-pg-search.md` `[04]`) on the BM25 route and the native `*_tsquery`
    // function on the tsvector baseline, so a mode reparameterizes the SQL rather than a switch arm.
    public static readonly FullTextMode Match = new("match", bm25: "pdb.parse($terms)", native: "websearch_to_tsquery($terms)");
    public static readonly FullTextMode Prefix = new("prefix", bm25: "pdb.phrase_prefix(ARRAY[$terms])", native: "to_tsquery($terms || ':*')");
    public static readonly FullTextMode Phrase = new("phrase", bm25: "($terms)::pdb.slop(0)", native: "phraseto_tsquery($terms)");
...
public static class HybridRetrieve {
    public static FusionBranch VectorBranch<TRow>(VectorQuery<TRow> vector) =>
        new("vector", 1.0,
            $"SELECT key, ROW_NUMBER() OVER (ORDER BY embedding {vector.Metric.Op} $probe) AS rank FROM corpus ORDER BY embedding {vector.Metric.Op} $probe");

    public static FusionBranch LexicalBranch(FullTextQuery text, bool pgSearch) =>
        pgSearch
            ? new("lexical", 1.0,
                $"SELECT key, ROW_NUMBER() OVER (ORDER BY pdb.score(key) DESC) AS rank FROM corpus WHERE corpus @@@ {text.Mode.Bm25} ORDER BY pdb.score(key) DESC")
            : new("lexical", 1.0,
                $"SELECT key, ROW_NUMBER() OVER (ORDER BY ts_rank(tsv, {text.Mode.Native}) DESC) AS rank FROM corpus WHERE tsv @@ {text.Mode.Native} ORDER BY ts_rank(tsv, {text.Mode.Native}) DESC");

    public static string Fuse<TRow>(VectorQuery<TRow> vector, FullTextQuery text, int k, bool pgSearch) =>
...
|  [10]   | hybrid fusion                | postgres-server | `HybridRetrieve.Fuse` binds the `VectorBranch`/`LexicalBranch` pair to `Query/federation#FUSION_RANK` `FusionRank.FuseSql` via `FromSql`                                    | the one n-ary RRF fold owns `Σ weight/(rrfConstant+rank)` and `DefaultRrfConstant=60`; dense embedding from `Compute/models#INFERENCE_MODES` `Embed`; `LexicalBranch(pgSearch:false)` selects the native FTS `ts_rank` projection when pg_search absent; projects identities then re-queries the store; a second RRF surface here is the deleted form |
...
- Entry: `public static SpatialDelta Diff(string entityKey, Option<Geometry> old, Option<Geometry> next)` discriminates enter (None old), exit (None next), move (centroid shift over `MoveThreshold`), and reshape (boundary change at equal centroid); `public static SpatialDelta Of(OpLogEntry entry)` decodes the replica-identity before-image WKB from `entry.Image` as the OLD operand and the WKB body from `entry.Payload` as the NEW (Delete → NEW `None`, Insert → empty `Image` → OLD `None`) so a single changefeed row diffs precisely without the prior entry, and `public static Seq<SpatialDelta> Fold(Seq<OpLogEntry> changefeed)` groups the `geometry` column family by `EntityKey`, HLC-orders each group, and derives the OLD operand from the head row's `Image` (falling back to its `Payload`) against the last row's `Payload` so the geo-diff is one pure fold over the changefeed; `public Expression<Func<TRow, bool>> Predicate<TRow>(Expression<Func<TRow, Geometry>> column, Geometry probe)` projects the row's translated DE-9IM predicate (or the `ST_Distance <= 0` form for the `Distance` row) and the `Within`/`Knn` projections cover the proximity `DbFunctions`.
...
public static class SpatialDiff {
    ...
    // and an Insert's Image is empty, the kind discriminating both ends from the one record.
