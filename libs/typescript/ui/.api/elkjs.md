...
[STACKING]:
- `@xyflow/react` (`.api/xyflow-react.md`): the canvas boundary — each node's measured `width`/`height` and the id graph build the `ElkNode` request, `elk.layout` answers absolute `x`/`y`, and the result lands as one `NodeReplaceChange[]` batch through `applyNodeChanges`, followed by `fitView` from `useReactFlow`; `ElkEdgeSection.bendPoints` rides an edge's `data` for a custom edge component to path through `BaseEdge`, so the canvas never learns the solver and the solver never learns React.
- `effect` (`libs/typescript/.api/effect.md`): `Effect.tryPromise` lifts `elk.layout` onto a typed failure, `Layer.scoped` over `Effect.acquireRelease` holds the client with `terminateWorker()` as its release, and `Effect.timeout` bounds a runaway solve — so a leaked worker and an unbounded wait both become typed faults.
- `@effect/platform-browser` (`libs/typescript/.api/effect-platform-browser.md`): the worker spawn is the integration point — `BrowserWorker.layer(spawn)`'s spawn factory shape is exactly the `workerFactory` this constructor takes, so the package mints its solver worker with the same `new Worker(new URL('elkjs/lib/elk-worker.min.js', import.meta.url), { type: 'module' })` bundler-resolved URL its other workers use. This worker speaks its own `{ cmd, id }` protocol, so it holds its own handle and never joins a serialized `Worker.makePool`.
