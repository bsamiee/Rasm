...
public static class EdgeProjection {
    ...
    public static Fin<Seq<Relationship>> All(IfcProject project, Map<string, NodeId> rooted, double tolerance, UnitScale scale, TemplateScope templates, IIfcProfileStore profiles, FidelityLog log, Op key) {
        var edges = Decomposition(project, rooted, key)
            ...
            .Concat(Seq(MaterialEdges(project, rooted, tolerance, scale, templates, profiles, log, key)));
    ...
    static Fin<Seq<Relationship>> MaterialEdges(
        ...
        project.Extract<IfcRelAssociatesMaterial>().AsIterable().Select(rel =>
            ...
            from bags in SemanticProjector.DefinitionOf(relating)
                .Map(definition => MaterialProjection.ImportedPsets(definition, rooted, scale, templates, log, key))
                .IfNone(FinSucc(Seq<PropertyBag>()))
            ...
            select elements.Map(element => (Relationship)new Relationship.Associate(element, material, usage))
                .Concat(bags.Map(bag => (Relationship)new Relationship.Assign(
                    material, SemanticProjector.PropertySetNode(bag, tolerance).Id, AssignKind.PropertyDefinition))))
