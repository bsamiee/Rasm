...
[TWO_D_MATRIX_ZXING]-[COMPLETE]: zxing-cpp landed as the 2D-matrix encoded-mark arm in the `figures/preview#PREVIEW` owner — `Symbology` gained `DATA_MATRIX`/`PDF417`/`AZTEC`/`MAXICODE` rows on the `_zxing` arm (zxing `BarcodeFormat` member resolved via `barcode_format_from_str`, `create_barcode(**ZXING_CREATE_KEYS).to_svg(add_quiet_zones=)`), `PreviewOp.Decode`/`_zxing_decode` over `read_barcodes` lands the round-trip op, and `.api/zxing-cpp.md` is authored against the cp314-bound 3.0.0 distribution; the central pin pre-existed (`zxing-cpp; python_version<'3.15'`). Three 3.0 facts recorded: MaxiCode is creatable (NOT decode-only), `str(BarcodeFormat)` is the separated display name (resolve via `barcode_format_from_str`, never re-parse `.name`), and the `|` union operator is deprecated (`BarcodeFormat.All` is the all-formats default, no `BarcodeFormat.Any`). Rides the gated companion subprocess seam; the separate-2D-matrix-owner routing note is resolved in this one owner. The `SYMBOLOGY`/`ENCODED_MARK_OWNER` ideas' realization for the 2D-matrix half.
