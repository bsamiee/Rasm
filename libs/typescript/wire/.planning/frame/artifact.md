...
- Entry: `ArtifactFrame.reassembled(frames)` — `Stream<Frame> -> Stream<Either<Artifact, WireFault>>`: the fold gathers, completed chunk sets join once, the joined octets mint through `ContentKey.mint`, and the declared key either matches (a right `Artifact`) or refuses (`parity` left with both keys as evidence); `ArtifactFrame.frame` is the single-frame byte schema for point decode.
...
- Law: the mint is delegated, never local — `ContentKey.mint` is the kernel's one `XxHash128` seed-zero fold with the `:x32` spelling and LE→BE normalization inside; exactly three delegating sites exist branch-wide (here, the `browser/transport` worker, `store/object`), and this module is the wire's one.
...
const _verified = (key: ContentKey, bands: Chunk.Chunk<Uint8Array>): Effect.Effect<readonly [Artifact, Uint8Array], WireFault> =>
  Effect.gen(function* () {
    ...
    return ContentKey.same(minted, key)
