...
public sealed record CostSchedule(
    ...
    // Identity mints through the ONE kernel seed-zero `ContentHash` over the seam `CanonicalWriter` fold — the
    // hasher every federation, solver, cache, and diff edge in this package shares by its own ruling, which the
    // Review/diff owner already rules a hand-rolled `XxHash128`/`Encoding.UTF8` string-join preimage the deleted
    // form. The writer is not a formatting convenience: it LENGTH-PREFIXES every string, so no delimiter choice
    ...
    public (UInt128 QuantityKey, UInt128 ResourceKey) Identity => (
        ContentHash.Of(Items
            .OrderBy(static i => i.GlobalId, StringComparer.Ordinal)
            .Fold(new CanonicalWriter(0.0).Ordinal(Items.Count), static (writer, item) => item.Values
                .Fold(item.PricedGlobalIds
                    .OrderBy(static g => g, StringComparer.Ordinal)
            ...
            .ToBytes().Span),
        ContentHash.Of(Resources
            .OrderBy(static r => r.GlobalId, StringComparer.Ordinal)
...
public sealed record CarbonLine(
    ...
    public double WholeLife => KgCo2eByStage.Values.Sum();
}
...
public sealed record CarbonRollup(Seq<CarbonLine> Lines, Map<LifecycleStage, double> ByStage, double TotalKgCo2e, Seq<CarbonGap> Gaps) {
    public static CarbonRollup Of(Seq<CarbonLine> lines, Seq<CarbonGap> gaps) {
        Map<LifecycleStage, double> byStage = lines.Fold(Map<LifecycleStage, double>(), static (acc, line) =>
            line.KgCo2eByStage.Fold(acc, static (m, stage, kg) => m.AddOrUpdate(stage, held => held + kg, () => kg)));
        return new CarbonRollup(lines, byStage, byStage.Values.Sum(), gaps);
    }
}
...
public static class CarbonEstimate {
    ...
    public static Fin<CarbonRollup> Rollup(ElementGraph graph, ElementSet scope, Func<Node.Object, Option<GeometryMeasures>> measures, Op key) =>
        scope.Objects
            .TraverseM(obj => measures(obj).Match(
                None: () => FinSucc((Lines: Seq<CarbonLine>(), Gaps: Seq(new CarbonGap(obj.Id, None, "geometry-unresolved")))),
                Some: geometry => graph.Bake(obj.Id, key).Bind(element =>
                    QuantityDerivation.Decompose(geometry, element.Materials, element.Section, key)
                        .Map(shares => toSeq(shares).Fold(
                            (Lines: Seq<CarbonLine>(), Gaps: Seq<CarbonGap>()),
                            (acc, share) => Priced(obj.Id, element.Materials, share.Key, share.Value).Match(
    ...
    // multi-ply element weight aggregate stays the Rasm.Compute frozen boundary), PerM2/PerItem and a missing
    // environmental set land typed gaps. The six-stage band prices in one pass off the seam StageAt row.
    static Either<CarbonGap, CarbonLine> Priced(NodeId element, Seq<BakedMaterial> materials, MaterialId material, MeasureValue share) =>
        materials.Find(baked => baked.Material.MaterialKey == material).Match(
            ...
            Some: baked => baked.Material.Properties
                ...
                .Match(
                    ...
                    Some: environmental => Quantity(environmental.Basis, share, baked).Match(
                        ...
                        Some: quantity => Right<CarbonGap, CarbonLine>(new CarbonLine(
                            ...
                            toSeq(LifecycleStage.Items).Fold(Map<LifecycleStage, double>(), (band, stage) =>
                                band.Add(stage, environmental.StageAt(stage) * quantity)),
                            environmental.Evidence)))));

    // The basis-resolved quantity the per-unit factor multiplies: the volumetric share for PerM3, the one-material
    // mass (share x own declared density) for PerKg, None for the area/item bases a volumetric takeoff cannot honor.
    static Option<double> Quantity(MeasurementBasis basis, MeasureValue share, BakedMaterial baked) =>
        basis.Switch(
            state: (Share: share, Baked: baked),
            perM3:   static (s, _) => Some(s.Share.Si),
            perKg:   static (s, _) => s.Baked.Material.Properties
                .Choose(static p => p is MaterialPropertySet.Mechanical m ? Some(m.Density.Si) : Option<double>.None)
                .Head
                .Map(density => s.Share.Si * density),
            perM2:   static (_, _) => Option<double>.None,
            perItem: static (_, _) => Option<double>.None);
}
```
