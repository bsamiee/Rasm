...
- Domain packages do not land here — provider codecs live in the AEC peers that project onto this graph, so the seam stays provider-free.
...
- Seam `Vector3` owns domain math and BCL `Matrix4x4`/`Vector3` stay LAYOUT carriers — a BCL carrier in domain position escapes the tolerance reads.
...
- `Predicate<TLeaf>` is the ONE boolean selection closure — a stored `Func<…, bool>` query filter is the deleted form wherever this seam reaches.
