...
- `OcctException` carries `StatusCode` on every native failure; consumers catch it and lower to the fabrication fault channel, never reading the internal `OcctStatus` enum.
...
[STACKING]:
- `ACadSharp`(`libs/dotnet/.api/api-acadsharp.md`): the 2D complement — `ACadSharp` admits 2D profiles (polylines, arcs into `Loop`), this admits 3D solids (STEP/IGES into B-rep into mesh); a 3D STEP solid tessellates then planar-sections to 2D loops, a 2D DXF profile goes straight to `Loop`, neither duplicating the other.
...
- `geometry3Sharp`(`libs/dotnet/.api/api-geometry3sharp.md`): the `OcctMesh` triangle soup crosses through `Vertices` and `TriangleIndices` to the geometry3Sharp mesh owner — only the admitted mesh crosses, never the live `OcctShape` handle.
- kernel mesh boundary: an imported solid flows `ImportStep` into `Triangulate` into `OcctMesh` into the kernel `MeshSpace` vocabulary for the planar-section and `DrawingProjection` pipelines, while dirty STL routes through the predicate-gated `HealOp`; `OcctVector3d`/`OcctPointCoordinates`/`OcctBoundingBox` map one-directionally at the boundary to the kernel `Vector3d`/`Point3d`/box.
- kernel persistence boundary: a `ExportStep`/`ExportIges`/`ExportStl` mints through `ContentKey.Of` into `ContentHash.Of` and enrolls in the Persistence artifact index, never a local hasher or schema-local content-key fork.

[LOCAL_ADMISSION]:
- solid ingress enters at `ImportStep`/`ImportIges` under a `using`, tessellated through `Triangulate` at a tolerance-driven deflection (finer linear deflection for a small precision part); the `OcctMesh` triangle soup is the cross-boundary payload, never the live `OcctShape` handle.
