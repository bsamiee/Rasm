...
- Anchors: `figures/preview#PREVIEW`, `PreviewOp.MARK`, `PreviewOp.DECODE`, `Symbology`, `segno`, `python-barcode` `get_barcode_class`, `zxingcpp.create_barcode`, `zxingcpp.read_barcodes`, `SVGWriter`, `PROVIDED_BARCODES`, `.api/python-barcode.md`, and `.api/zxing-cpp.md`.
- Tension: the QR/Micro-QR (segno), linear 1D (python-barcode), and 2D-matrix DataMatrix/PDF417/Aztec/MaxiCode (zxing-cpp) rows plus the `PreviewOp.Decode` round-trip op have all landed (`TWO_D_MATRIX_ZXING` complete); `zxing-cpp` source-builds from sdist and imports on the cp315 core beside `segno`/`python-barcode`, so all three `Mark` arms resolve in-process and only the Pillow-opening `Decode` op crosses the subprocess seam. The residual is the `CONTENT_KEYED_REUSE_AND_SIGNAL` keying spine, not the symbology surface.
...
[SYMBOLOGY]-[QUEUED]: elevate the MARK preview arm into a full encode-decode encoded-mark owner spanning `segno` QR/Micro-QR, `python-barcode` linear 1D, and `zxing-cpp` 2D-matrix DataMatrix/PDF417/Aztec/MaxiCode, all on the cp315 core, round-trip verifiable.
...
- Unlocks: 2D-matrix codes — DataMatrix/PDF417/Aztec/MaxiCode, once routed to a separate owner — plus round-trip verification, all in one owner (landed via `TWO_D_MATRIX_ZXING`).
- Anchors: `figures/preview#PREVIEW`, `PreviewOp.MARK`, `PreviewOp.DECODE`, `Symbology`, `segno.make`/`make_sequence`, `python-barcode` `get_barcode_class`, `zxingcpp.create_barcode`/`BarcodeFormat`/`Barcode.to_svg`/`read_barcodes`, `.api/zxing-cpp.md`.
- Tension: `zxing-cpp` source-builds from sdist and imports on the cp315 core (un-gated) beside `segno` and `python-barcode`, so its dependency-free 2D-matrix `Mark` arm resolves in-process; only the `Decode` op crosses the subprocess seam (it opens a raster through the gated-band Pillow worker). This is the higher-order parent of the `ENCODED_MARK_OWNER` idea and refines it rather than duplicating it. The one `Symbology` axis spanning QR/Micro-QR, linear 1D, and 2D-matrix plus the `PreviewOp.Decode` round-trip op landed (`TWO_D_MATRIX_ZXING` complete), so the encode-decode owner thesis is realized; what remains is folding the decode-confidence fact into the `CONTENT_KEYED_REUSE_AND_SIGNAL` measurement edge, not new symbology coverage.
- Ripple: `data` `[CORPUS_INGEST]`
