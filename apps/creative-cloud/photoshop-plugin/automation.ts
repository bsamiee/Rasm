...
const _LOADED = `${manifest.name} (Loaded)`;
...
const _SHOW = ChildProcess.make('osascript', [
    '-e',
    `tell application "System Events" to tell process "${_HOST.processName}" to click menu item "${panel.label.default}" of menu 1 of menu item "${manifest.name}" of menu "Plugins" of menu bar 1`,
]);
...
const AutomationError = Schema.TaggedUnion({ notLoaded: { report: Schema.String } });
...
const _deploy = Effect.gen(function* () {
    ...
    const report = yield* read(_HOST.id, _HOST.bundleId, _REPORT_MS, doJavascript('app.systemInformation'), Option.none()).pipe(
        Effect.filterOrFail(String.includes(_LOADED), (text) => AutomationError.cases.notLoaded.make({ report: text })),
    ...
    yield* Effect.when(Effect.orDie(reply(_SHOW)), Effect.succeed(views === 0));
