...
from rasm.runtime.faults import BoundaryFault, FaultTag, RuntimeRail, latched
...
# new synchronous histogram is one `INSTRUMENTS` row + one slot, never a hardcoded `duration=` re-mint.
_HIST_SLOT: Final[dict[str, str]] = {
    "companion.request.duration": "duration",
    "artifact.byte_volume": "artifact_bytes",
    "artifact.compression_ratio": "artifact_ratio",
}
...
class Metrics:
    ...
    def record_artifact(cls, kind: str, byte_volume: int, ratio: float) -> None:
        # the per-artifact byte-volume + compression-ratio histograms the runtime receipt-harvest path records
        # off each emitted `ArtifactReceipt._facts` (the `artifact` kind discriminant plus the byte/ratio scalars),
        # keyed by the carried kind so a per-kind size/ratio distribution routes off one stream — the metrics-owned
        # instrument the artifacts emit seam composes, exactly as `retry_hook` is `reliability/resilience#RESILIENCE`-composed.
        attributes, context = {"artifact": kind}, otel_context.get_current()
        cls._sync.artifact_bytes.record(byte_volume, attributes, context=context)
        cls._sync.artifact_ratio.record(ratio, attributes, context=context)
...
- [INSTALL_LATCH]: the observable `create_*` calls register process-global callbacks the SDK `PeriodicExportingMetricReader` pulls every cycle, so a second `install` (a re-admission, a re-entrant composition root, a test re-bootstrap) without an idempotency guard would register the callback set twice and the reader would fire every `Observation` twice per export — the exact hazard the `reliability/faults#FAULT` `latched` aspect defends for the telemetry provider trio. `install` is therefore wrapped by that owner's receipt-agnostic `latched[R, **P](read, write, reentrant)` decorator imported directly (`from rasm.runtime.faults import latched`, no local redefinition), closed over the `(read, write)` accessor pair onto the `_receipt` latch (not a hardcoded `Metrics._receipt` reference) and the `reentrant = lambda prior: replace(prior, outcome=MeterOutcome.REENTRANT)` restamp closure, so the `[R, **P]` parametricity is real and the aspect carries no `MeterReceipt`-pinned `case MeterReceipt()` class pattern — the exact form telemetry names a deleted re-pinning. It matches `read()` `None` versus a `prior` receipt and on re-entry returns `reentrant(prior)` before the fold body runs, writing the latch as the single terminal store on first install — one cross-cutting decorator genuinely shared with the telemetry owner (one definition, two latches), not a forked guard, and the at-most-once mint CPython's atomic class-attribute store already guarantees on the read side. The held synchronous recorders collapse into one `SyncInstruments` frozen carrier (the `duration`/`artifact_bytes`/`artifact_ratio` `Histogram`s and the `retries` `Counter`) seeded by `_seed_sync(meter)` — one `get_meter` resolution minting the four recorders off the no-op API meter at class body, the installed meter in the fold routing each histogram by its `_HIST_SLOT` field — and swapped as one reference in the install fold — the sibling `MetricState`/`InstalledProviders` atomic-swap idiom — rather than two standalone class attributes each resolving its own meter and constructed twice, so the synchronous and observable instruments share one create fold and the imperative `record`/`retry_hook` path reads one carrier. The latch and carrier spellings are settled.
