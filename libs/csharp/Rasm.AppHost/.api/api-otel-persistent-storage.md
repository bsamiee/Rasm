...
- Offline custody therefore reaches the transport, never the exporter's persistence: `OtlpExporterOptions.HttpClientFactory` installs a `DelegatingHandler` that owns a directly-constructed `FileBlobProvider`, so queue policy, replay cadence, and refusal receipts stay branch-typed. Exactly one persistence owner arms per exporter — the exporter env pair stays unset wherever the handler leg is selected.
- `SignalGovernance.Govern` (hosted roots) constructs one provider per transmission owner and drives its lease-gated drain; `PluginTelemetryHost.Open` opens no disk queue, so an ALC capsule's failed batches die with the capsule.
