...
- [SPAN_ERROR]: every offload rail folds INSIDE the span scope — its Error arm sets `Status(StatusCode.ERROR)` carrying the fault tag and emits the correlated structured error log from `fault.facts()` before the span closes, so a failed render never exits an `UNSET` span or an uncorrelated log.
