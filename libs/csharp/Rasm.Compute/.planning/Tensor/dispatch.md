...
// The six-row DDG operator table — Compute-owned; the kernel declares only DiscreteCalculus (Numerics/Spectral.cs)
...
public static class SensitivityLaw {
    ...
    // The hyper-dual scalar leg — the THIRD leg of the ONE Sensitivity family (geometry tape, symbolic tape,
    // hyperdual): a smooth scalar objective authored once over the HyperJet hyper-dual scalar yields the
