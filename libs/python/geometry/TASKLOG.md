...
[STRUCTURAL_SECTION_PROPS_GATED]-[BLOCKED]: land the `sectionproperties` warping/plastic/shear enrichment tier in the `ifc/structural` owner, gated `<'3.15'`.
- Capability: `IfcStructural` resolves the `WARPING` `EnrichmentTier` arm — the `sectionproperties` triangular-FE warping/plastic/shear layer over the same profile rings the numpy spine integrates — so the warping torsion, shear-center, shear-area, and plastic-moduli receipts no closed-form integral derives land on the `WARPING`-tier `SectionReceipt` slots without touching the cp315-clean spine.
...
- Anchors: `ifc/structural.md#STRUCTURAL` `EnrichmentTier.WARPING` arm / `SectionReceipt` FE slots / `_interior_point` void marker / the `fe-area` residual ledger, `.api/sectionproperties.md` (`pre.Geometry.from_points`/`create_mesh`/`analysis.Section`/`calculate_*`/`get_*` accessors, the `[CAPTURE_GAP]` note), the `numpy` ring spine, the `python_version<'3.15'` gated band.
- Tension: `sectionproperties` ships a pure-Python `py3-none-any` wheel but its native mesh backend `cytriangle` (cp311–cp314 wheels, LGPLv3) makes the tier a rail-policy gated enrichment row with no cp315 wheel, so the arm is [BLOCKED] until the `<'3.15'` companion band runs; the catalogue's `[CAPTURE_GAP]` note leaves the exact `from_points` positional contract and the `get_*` accessor spellings a live-run confirmation before the fence accessor calls bind, and the spine never re-routes a closed-form integral through the FE solver.

[KISS_MATCHER_FALLBACK_FGR]-[BLOCKED]: select the `open3d` Fast Global Registration bootstrap on the `>='3.13'` interpreter where no `kiss-matcher` wheel resolves.
- Capability: `ScanRegistration._engine` resolves the `BootstrapEngine` per interpreter floor — `KISS_MATCHER` below the `_KISS_FLOOR` `(3, 13)` anchor, `OPEN3D_FGR` at or above it — so the `GLOBAL` arm mints the initialization-free coarse pose on every interpreter the registration owner runs on, the FGR fallback seeding `small-gicp` fine refinement exactly as the `kiss-matcher` primary does.
...
- Anchors: `scan/registration.md#REGISTRATION` `BootstrapEngine.OPEN3D_FGR` arm / `_engine` floor read / `_KISS_FLOOR` `(3, 13)` anchor, `.api/kiss-matcher.md` (the cp38–cp312-only wheel floor, the marker-over-classifier rule), `.api/open3d.md` (`registration.registration_fgr_based_on_feature_matching`/`compute_fpfh_feature`/`estimate_normals`/`KDTreeSearchParamHybrid`), the `small-gicp` fine-refinement seed.
- Tension: `kiss-matcher 1.0.2` ships cp38–cp312 wheels only (its advertised `3.13` classifier ships no wheel, so the marker not the classifier is authoritative) and `open3d` is the cp312-max Forge companion lane, so the FGR fallback covers the `>='3.13'` band where neither primary wheel resolves; [BLOCKED] until the interpreter-floor selection lands, the FGR arm body staying authored as the dispatch row in the meantime.
