...
|  [07]   | `store.ObjectStore`       | union alias   | `S3Store \| GCSStore \| AzureStore \| HTTPStore \| LocalStore \| MemoryStore` |
|  [08]   | `store.ObjectStoreMethods` | protocol     | shared store-surface `Protocol` (`copy`/`get`/`list`/`put`/`prefix`, pickle `__getstate__`/`__setstate__`); the type-check contract the six stores satisfy |
...
[STORAGE_TOPOLOGY]:
- `ObjectStore` is the union alias `S3Store | GCSStore | AzureStore | HTTPStore | LocalStore | MemoryStore`; `store.ObjectStoreMethods` is the `Protocol` capturing the shared store surface (`copy`/`get`/`list`/`put`/`prefix`/pickle `__getstate__`/`__setstate__`) the six concrete stores satisfy. Module-level functions accept any `ObjectStore` member as the first positional argument; the Rust core dispatches structurally rather than through Python method calls, so the union/protocol is for type-checking, not runtime virtual dispatch. The runtime composes the existing `store.ObjectStoreMethods`/`store.ObjectStore` surface and never re-declares a parallel store base class.
