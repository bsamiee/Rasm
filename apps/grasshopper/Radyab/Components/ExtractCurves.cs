...
public sealed class ExtractCurves : Component {
    ...
    private static readonly IOutputGroup Segments = Output.Single(input: Geometry, port: Port.Tree<Curve>(name: "Segments", code: "S", info: "Structural segments from the curve data model, Brep/SubD edges, or mesh topology edge lines."), aspect: Curves.Segments);
    private static readonly IOutputGroup IsoCurves = Output.Single<Curves, Curve>(
        input: Geometry,
        ...
        aspect: runtime => runtime.Read(port: Direction).Map(direction => Curves.Iso(
            ...
            normalized: 0.5)));
    private static readonly IOutputGroup Silhouette = Output.Single<Curves, Curve>(
        input: Geometry,
    ...
    private static readonly IOutputGroup Draft = Output.Single<Curves, Curve>(
        input: Geometry,
    ...
    private static readonly IOutputGroup Topology = Output.Details<CurveProjection>(
        ...
        slots: [
            Output.Plain<CurveProjection, Curve>(port: Port.Tree<Curve>(name: "Curves", code: "C", info: "Every curve piece: structural segments, Brep/SubD edges, surface boundary iso curves, mesh topology edge curves, and loop curves."), project: static value => value.Curve),
            Output.Plain<CurveProjection, ComponentIndex>(port: Port.Tree<ComponentIndex>(name: "Source", code: "X", info: "Source component index aligned with each curve."), project: static value => value.Source),
            Output.Plain<CurveProjection, CurveFeature>(port: Port.Tree<CurveFeature>(kind: PortKind.Enum(initial: CurveFeature.Input), name: "Feature", code: "F", info: "Feature classification aligned with each curve (boundary, naked, interior, loop, segment, iso, silhouette, draft)."), project: static value => value.Feature),
        ]);
    public ExtractCurves() : base(self: typeof(ExtractCurves), spec: ComponentSpec.Of(
        inputs: Seq<IPort>(Geometry, Direction, Angle),
        outputs: Seq<IOutputGroup>(Segments, IsoCurves, Silhouette, Draft, Topology))) { }
}
