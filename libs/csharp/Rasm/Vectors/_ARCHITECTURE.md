...
5. `[implemented]` Sampling, domains, and modes use the right owners: `SampleKind`, `ExtractionDomain`, `CurveProjection`, `SurfaceProjection`, and `SupportSpace`. Public callers route through factory admission, cloud mass reaches deterministic candidate weighting through `CloudKernel.MassOf`, curve/surface modes use admitted factories, and density/adaptive sampling remains labeled deterministic priority selection. `SampleKind.SampleElimination` implements Yuksel weighted sample elimination over admitted candidate batches with receipts for parameters, elimination counts, neighbor updates, deterministic candidate source, Euclidean metric, and explicit non-claims for maximal coverage, OT/CCVT, and mesh-spectrum validation. Current `PoissonDisk` remains deterministic candidate-radius filtering, not Bridson active-list Poisson disk; Bridson, variable-density Dwork-style sampling, OT/CCVT, and mesh-spectrum validation are separate unsupported research rails, not blockers for item 5 completion.
6. `[partial]` Field, kernels, and SDFs include derivative-aware kernels, anisotropic tensor metrics, RBF receipts, half-space/slab/capped-cone primitives, Lipschitz bounds, mesh-backed SDF receipts, and native iso-surface receipts. Validated factory admission is now the external construction path for field/falloff/integrator union cases, `FieldNabla` owns shared kernel/periodic/iso-surface/plane validation, extraction preserves successful `IsoSurfaceReceipt` facts, and mesh-backed SDF receipts reuse topology facts. Finish only the still-missing MLS/profile extrusion math and bridge-proven native callback/threading/fixed-tolerance labels.
...
- `[implemented]` `Extraction.cs`: domain-backed glyph/grid/bundle sampling, mesh scalar isolines, surface `IsoStatus`, point-cloud section curves, and raw native contour counts are implemented. `ExtractionReceipt` owns receipt construction, preserves successful iso-surface receipts, PL mesh-isoline plateau/branch facts are preserved, and null/raw domain plus policy payloads fail through factory admission; glyph/bundle runtime geometry proof remains bridge-owned.
- `[partial]` `Flow.cs`: RK tableaus, method-order metadata, adaptive stepping, endpoint/bracket event statuses, bounded bisection localization, `StreamlineTrace` receipts, and admitted fixed/adaptive integrator construction exist. Higher-order tableau moments and true RK dense output remain future work.
...
- `[partial]` `Field.cs`: vector/scalar/tensor field algebra, kernel profiles, anisotropic falloff, RBF reconstruction, SDF primitives, mesh-backed fields, and iso-surface receipts exist. Public constructor bypasses for invariant-bearing field/falloff cases are closed through internal case constructors, shared field admission routes through `FieldNabla`, and successful native iso-surface receipts carry callback/tolerance labels. MLS/profile extrusion/closed SignedHeat remain incomplete until real kernels and receipts exist.
...
- `[partial]` `Field.cs`: iso-surface routes through intent/extraction, and successful extraction receipts preserve the native `IsoSurfaceReceipt`. `ScalarField.IsoSurfaceDetailed` still owns the Rhino callback; failed native evaluator attempts still return `Fin` failure rather than a partial failure receipt. Label parallel native callback and fixed-tolerance behavior until bridge proof constrains it.
...
- `[partial]` Reconstruction: RBF interpolation/approximation uses matrix solve receipts and records mode, smoothing, sample count, residual, solve path, and factor facts. MLS remains future work until a per-query detailed reconstruction sample receipt can report neighborhood count, rank, conditioning, and nonfinite weights.
- `[partial]` SDF primitives: half-space, slab, and capped cone exist. Profile extrusion remains future work until a closed planar Rhino `Curve`, explicit plane, half-height, containment, closest-point distance, and receipt rail exist.
- `[partial]` SDF outputs: `ScalarField.LipschitzBound()` exists, mesh-backed signing routes through generalized winding / boundary-source signed heat, and SDF/iso-surface receipts expose approximate/native facts. Mesh SDF receipts reuse topology facts and no longer advertise an unused unsupported status; watertight preflight, closed SignedHeat, and fuller fixed-tolerance receipt propagation remain incomplete.
