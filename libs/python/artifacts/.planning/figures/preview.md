...
The raster image-processing and preview owner. `Preview` is ONE owner over the host-free imaging pipeline discriminating operation over the closed-payload `PreviewOp` family: pillow (I/O, resize, thumbnail, native AVIF/WebP codec conversion, montage) and scikit-image (restoration/deconvolution, exposure, segmentation, thresholding, morphology, geometric transform, filters/edges, feature and texture, region measurement, registration, and perceptual-quality metrics across the eleven-submodule catalogue) and zxing-cpp (the 2D-matrix DataMatrix/PDF417/Aztec/MaxiCode encode-decode round-trip over `create_barcode`/`Barcode.to_svg`/`read_barcodes`) on the gated `python_version<'3.15'` band, segno (QR/Micro-QR and structured-append sequence generation with the full factory-parameter axis and dependency-free SVG serialization), python-barcode (the linear 1D symbology registry over `SVGWriter`), and python-magic (libmagic MIME detection) on the cp315 core. One preview/raster surface, not a per-media-type class family, not a per-operation function family, and not an erased `params` bag. The cp315-core process imports no gated distribution, so the pillow/scikit-image raster arms and the zxing-cpp 2D-matrix arm cross the `faults`-owned `to_process.run_sync` subprocess seam. Every operation returns a `RuntimeRail[ArtifactReceipt]`.
...
- Entry: `Preview.of` is `async` over the runtime `async_boundary` and dispatches the `PreviewOp` case, returning one `RuntimeRail[ArtifactReceipt]` whose `ArtifactReceipt.Preview` carries the content key and pixel dimensions and whose `_facts` projection threads the `METRICS`/decode score map — never an erased `object` a consumer re-validates; the segno/python-barcode `Mark` arms resolve synchronously on the cp315 core inside the async capsule and render the code in-process with no Pillow dependency (segno serializers and the python-barcode `SVGWriter` need none, the symbology detects no media type), while the zxing-cpp 2D-matrix `Mark` rows, the `Decode` op, and the `Thumbnail`/`Convert`/`Montage`/`Transform` raster arms cross the runtime `reliability/faults#FAULT` `anyio.to_process.run_sync` subprocess seam onto the gated-band pillow/scikit-image/zxing-cpp worker — the genuine separate-process crossing the gated cp314 band needs because the cp315-core `execution/lanes#LANE` `to_interpreter.run_sync` subinterpreter offload shares the host interpreter version and cannot host the cp314-band `pillow`/`scikit-image`/`zxingcpp` packages.
- Auto: `_compute` folds the case through one `match` — a cp315-core `Mark` (segno or python-barcode) through `SYMBOLOGIES[symbology].arm` which carries its own segno or python-barcode member (QR/Micro-QR/structured-append each a distinct segno factory row reached by `arm` over the `SHARED_FACTORY_KEYS` plus per-row `accepts` kwarg spread, never a re-`match` inside one arm; the linear (1D) symbologies through the python-barcode `get_barcode_class(name)` registry then `SVGWriter` render, the symbology resolved by registry name not a hand-picked sub-enum) returning a zero-dimension `RasterFact` in-process; the zxing-cpp 2D-matrix `Mark` rows, the `Decode` op, and the four raster cases through `to_process.run_sync(_gated_raster, op)` where the gated-band worker re-dispatches the case by `match` at boundary scope — the 2D-matrix `Mark` reaching the `SYMBOLOGIES[symbology].member` zxing `BarcodeFormat` then `create_barcode(content, fmt, **opts).to_svg(add_quiet_zones=True)`, the `Decode` reaching `read_barcodes(image, formats=...)` and stamping each symbol's `text`/`format`/`valid`/`position` onto the score map, `Convert` reaching native AVIF through the `quality`/`effort` save kwargs, and `Transform` folding through the `TRANSFORMS[kind]` row whose `arm` selects one of ten acceptors and whose `member` names the submodule attribute the `getattr` fold resolves so the four denoise rows, the nine filter/edge rows, the five morphology rows, and the six metric rows each share one acceptor with zero parallel inline dispatch dict; the `_compute` `match` splits only on the cp315-core-vs-gated band (the segno/barcode mark resolves in-process, every gated arm crosses the one `to_process.run_sync(_gated_raster, op)` seam), never a per-op subprocess call.
...
- Packages: `segno` (`make`/`make_micro`/`make_sequence`/`QRCode.save`/`QRCodeSequence.save`/`QRCode.designator`, the `error`/`version`/`mode`/`mask`/`encoding`/`boost_error`/`eci`/`micro`/`symbol_count` factory axis and the `dark`/`light` SVG serializer kwargs, dependency-free serializers), `python-barcode` (`get_barcode_class`/`PROVIDED_BARCODES`/`SVGWriter`/`Barcode.write`/`Barcode.get_fullcode` linear (1D) symbologies, `installed: 0.16.1`), `python-magic` (`from_buffer`/`Magic`) on the cp315 core; `pillow` (`Image`/`open`/`thumbnail`/`save`/`new`/`paste`/`fromarray`, native `AvifImagePlugin` on 12.2.0) and `scikit-image` (`restoration`/`exposure`/`segmentation`/`morphology`/`transform`/`measure`/`metrics`/`feature`/`filters`/`registration`/`color`/`util`/`io`) and `zxing-cpp` (`create_barcode`/`BarcodeFormat`/`Barcode.to_svg`/`Barcode.to_image`/`read_barcodes`/`barcode_format_from_str`, the 2D-matrix DataMatrix/PDF417/Aztec/MaxiCode encode-decode round-trip, `installed: 3.0.0`) gated `python_version<'3.15'`; `numpy` (the `skimage` array backing plus the `digitize`/`linalg.norm`/`ones` PSF and flow-magnitude folds, and the zxing `Decode` BGR-array input); runtime (`content_identity.ContentIdentity`, `faults.RuntimeRail`/`async_boundary` and the `faults`-owned `anyio.to_process.run_sync` subprocess seam the gated raster and 2D-matrix arms cross — the genuine separate-process crossing distinct from the cp315-core `execution/lanes#LANE` `to_interpreter.run_sync` subinterpreter offload, both settled at their owners), `receipt/receipt#RECEIPT` (`ArtifactReceipt`).
...
- Boundary: a per-media-type preview class family, a per-symbology code class, a per-scikit-image-call sibling function, and an erased `params` bag are the deleted forms; no UI, no live viewer. segno owns QR/Micro-QR/sequence generation and serialization with no Pillow dependency, removing the former `qrcode` Pillow leak; the python-barcode linear arm uses `SVGWriter` ONLY on the in-process core path (its `ImageWriter` PNG path needs Pillow and re-introduces the leak segno removed); python-barcode is strictly linear (1D) — DataMatrix/PDF417/Aztec/MaxiCode route to the zxing-cpp 2D-matrix arm, never a phantom python-barcode member, and the former separate-future-owner routing note is RESOLVED in this owner by the zxing `SYMBOLOGIES` rows plus the `Decode` round-trip op; zxing-cpp `to_svg` is dependency-free (no Pillow on the SVG path), so a mixed QR + linear + 2D-matrix label sheet folds into one SVG-fragment receipt stream, and the zxing `Decode` is the encode/decode round-trip the segno/python-barcode arms cannot express; native AVIF rides the already-admitted pillow with no new dependency (no `pillow-avif-plugin` exists on the manifest), the AVIF-specific `quality`/`effort` encode controls riding the `Image.save` kwargs the catalogue settles; the cp315-core segno/python-barcode `Mark` arm and its media-free render run in-process; `pillow`, `scikit-image`, and `zxing-cpp` ride the gated `python_version<'3.15'` band and never resolve in the cp315-core process, so the raster and 2D-matrix arms dispatch onto the `faults`-owned `to_process.run_sync` cp314-band subprocess seam — a different interpreter version than the cp315-core `to_interpreter.run_sync` subinterpreter offload can host, where the gated-band worker imports `PIL`/`skimage`/`zxingcpp` at boundary scope so neither a module-top nor a lazy gated import lands on the core page.
...
class Preview(Struct, frozen=True):
    ...
    async def _compute(self) -> ArtifactReceipt:
        match self.op:
            case PreviewOp(tag="mark", mark=(content, symbology, opts)) if SYMBOLOGIES[symbology].arm is not _zxing:
...
def _gated_raster(op: PreviewOp) -> RasterFact:
    ...
    match op:
        ...
        case PreviewOp(tag="mark", mark=(content, symbology, opts)):
            return _zxing(content, symbology, opts)
...
flowchart LR
    ...
    Compute -->|"mark (segno/barcode)"| Sym["SYMBOLOGIES[symbology].arm"]
    Compute -->|"mark (zxing) / decode / raster"| Worker["to_process.run_sync(_gated_raster)"]
    ...
    Worker --> Zx["_zxing: barcode_format_from_str -> create_barcode -> to_svg"]
