...
| [INDEX] | [SYMBOL]                     | [TYPE_FAMILY]     | [RAIL]                                      |
| :-----: | :--------------------------- | :---------------- | :------------------------------------------ |
|  [01]   | `IOtlpExporterOptions`       | options contract  | endpoint, protocol, headers, timeout        |
|  [02]   | `OtlpExporterOptions`        | options class     | `IOtlpExporterOptions` implementation       |
|  [03]   | `OtlpExporterBuilderOptions` | multi-signal opts | default + per-signal `IOtlpExporterOptions` |
|  [04]   | `OtlpExportProtocol`         | protocol enum     | `Grpc` / `HttpProtobuf`                     |
|  [05]   | `OtlpExportCompression`      | compression enum  | `None` / `GZip`                             |
|  [06]   | `OtlpMtlsOptions`            | mTLS options      | cert paths for mutual TLS                   |
|  [07]   | `OtlpTlsOptions`             | TLS options       | server cert path                            |
...
|  [06]   | `OtlpExporterOptions.HttpClientFactory`           | factory          | custom `Func<HttpClient>` override              |
|  [07]   | `OtlpExporterOptions.ExportProcessorType`         | processor        | `Batch` default (traces only)                   |
...
|  [09]   | `OTEL_EXPORTER_OTLP_CERTIFICATE`        | mTLS         | CA certificate path                 |
