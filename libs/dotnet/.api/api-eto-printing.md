...
- Each dialog takes a caller-supplied parent, so the parent is the Rhino-owned window the host bridge resolves (`libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md`), never a boundary-minted top-level window.
...
- The OS projection and the Rhino in-app meter (`libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md` `StatusBar`) are one pair over one progress fact, not two owners: a long render publishes its fraction once and both sinks read it.

[STACKING]:
- `libs/dotnet/.api/api-eto-drawing.md`: the page `Graphics` primitive set every `OnPrintPage` body composes, and the `RectangleF`/`SizeF` carriers the printable area and page size arrive as.
- `libs/dotnet/.api/api-eto-forms.md`: `Control`, `Window`, and `DialogResult` are the carriers the source control and both dialogs receive; the control tree itself is authored there.
