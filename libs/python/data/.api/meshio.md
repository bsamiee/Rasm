...
- time-series stack: `xdmf.TimeSeriesWriter` is a context manager that writes one topology once then appends per-step `point_data`/`cell_data` against shared HDF5 heavy data — use it for transient-field export rather than re-writing the full `Mesh` per step. The paired `xdmf.TimeSeriesReader(filename)` context manager reads it back: `read_points_cells() -> (points, cells)` materializes the shared topology once (where a `ReadError` surfaces eagerly), `num_steps` is the int step count, and `read_data(k) -> (time, point_data, cell_data)` reads one timestep's named arrays — drain `for k in range(reader.num_steps): reader.read_data(k)` inside the `with` so the HDF5 handle closes on exit.
