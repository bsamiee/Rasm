...
`premise` is FLOOR-GATED — admission stands, reach does not: `build` opens on a `find_spec` row and refuses every request with `BoundaryFault(import_=)` naming the absent module while the marker holds, per the folder's floor-gate law; the gate runs at entry, ahead of the module-scope `lazy` binding's first use, so the two forms compose.
...
- Law: the floor gate leads every entry — `find_spec("premise")` refuses with `BoundaryFault(import_=("premise", ...))` before the module-scope `lazy import premise` is ever dereferenced, so the deferred binding cannot carry the failure into the offloaded band as a provider crash; every surface naming the gated coordinate derives from this one gate, the folder's floor-gate law.
