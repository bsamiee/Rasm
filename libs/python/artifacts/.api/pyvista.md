...
- target: gated sub-3.15 native-VTK worker — imported after `lane.offload(dispatched, "<kernel>", ..., modality=Modality.PROCESS)` crosses the subprocess seam; co-resolves with `vtk` and `usd-core` on that worker
...
- worker axis: `pyvista` and `vtk` import after `lane.offload(dispatched, "<kernel>", ..., modality=Modality.PROCESS)` crosses the process seam. Worker bodies stay module-level qualified-name functions, and the runtime producer imports neither native package.
...
- universal-tier `anyio` underlies `LanePolicy.offload`; every `Scene3d` modality uses its `Modality.PROCESS` rail and lane capacity, so sub-3.13 `pyvista`/`vtk`/`pxr` imports stay worker-local. `SceneOp`/`FieldFilter` and `RuntimeRail` wrap provider values and faults.
