...
public sealed class ImmersiveSession : UiLease {
    ...
    public Func<EyePass, Fin<FrameRender>> RenderEye { get; }
...
public abstract partial record XrFrameOutcome {
    ...
    public sealed record Submitted(FrameRender Frame) : XrFrameOutcome;
...
public static class XrFrame {
    ...
    private static unsafe Fin<XrFrameOutcome> Attempt(
        ...
        session.Phase.Value switch {
            ...
            { Renderable: true } renderable => Submit(session, clock, budget, quality, camera).Map(
                drawn => drawn.Match(
                    Some: XrFrameOutcome (frame) => new XrFrameOutcome.Submitted(frame),
    ...
    private static unsafe Fin<Option<FrameRender>> Drawn(
        ...
        if (step.IsFail) { return step.Map(static _ => Option<FrameRender>.None); }
        ...
        if (state.ShouldRender == 0) {
            ...
            return Guard(core.EndFrame(session.Session, &idle), nameof(XR.EndFrame))
                .Map(static _ => Option<FrameRender>.None);
        }
        ...
        if (locatedCount != EyeCount || (viewState.ViewStateFlags & ViewStateFlags.PositionValidBit) == 0) {
            return Fin.Fail<Option<FrameRender>>(new ImmersiveFault.NativeRefused(
        ...
        Seq<FrameRender> eyes = Seq<FrameRender>();
        for (int eye = 0; eye < EyeCount; eye++) {
            Swapchain swapchain = session.EyeSwapchains[eye];
            int ordinal = eye;
            Posef pose = located[eye].Pose;
            Fovf fov = located[eye].Fov;
            Fin<FrameRender> drawn = XrSwapchain.Bracket(core, XrSurface.Frame, swapchain, image => session.RenderEye(
                new EyePass(
                    new EyeView(ordinal, pose, EyeCamera(camera, pose, fov), swapchain, image),
                    clock, budget, quality, session.ComfortPolicy, motion)));
            if (drawn.Case is not FrameRender frame) {
                return Fin.Fail<Option<FrameRender>>((Error)drawn.Case);
            }
            eyes = eyes.Add(frame);
    ...
    private static Fin<FrameRender> Seal(
        ImmersiveSession session, ViewportClock clock, Seq<FrameRender> eyes,
        Seq<(string Pass, Duration Elapsed)> chrome, Seq<Error> stalled) =>
        eyes.Head.Match(
            Some: lead => Fin.Succ(new FrameRender(
                lead.Ordinal,
                lead.Backend,
            ...
            None: static () => Fin.Fail<FrameRender>(
...
public static class ImmersiveFrame {
    extension(ImmersiveMode mode) {
        public IO<(ImmersiveMode Mode, FrameRender Frame, Seq<XrEvent> Drained)> Frame(
            ...
            mode.Switch(
                immersive: s => s.Session.Frame(clock, budget, quality, camera).Bind(advanced => advanced.Outcome.Switch(
                    submitted: submit => IO.pure((mode, submit.Frame, advanced.Drained)),
                    idled: _ => graph.Frame(clock, budget, quality, camera).Map(frame => (mode, frame, advanced.Drained)),
                    demoted: fallen => graph.Frame(clock, budget, quality, camera).Map(frame =>
                        ((ImmersiveMode)new ImmersiveMode.Flat(fallen.Cause),
                         frame with { Fault = frame.Fault || fallen.Release }, advanced.Drained)))),
                flat: _ => graph.Frame(clock, budget, quality, camera).Map(frame => (mode, frame, Seq<XrEvent>())));
    }
}
```
...
- Growth: a new FB async verb is ONE `SpatialRequestKind` row (completion structure type, payload class, redrive law, payload reader) plus one `SpatialIntent` case, and the drain retires it with no new completion path; a new scene read is one `XrSpatial` member over an existing `FBScene` entrypoint; zero new surface.
...
- Output: `Drain` reads one `XrObservation` at a time off the bounded lane and writes the drained-by-kind, demoted-by-cause, overrun, and shed rows through `ImmersiveSession.Observed`, so the instruments this page declares have exactly one writer and it never runs on the compositor thread.
...
public sealed class ImmersiveDeck {
    ...
    public IO<(ImmersiveDeck Deck, FrameRender Frame)> Frame(
        RenderGraph graph, ViewportClock clock, FrameBudget budget, QualityVerdict quality, ViewCamera camera) =>
