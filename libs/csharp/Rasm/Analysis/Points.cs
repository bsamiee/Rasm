            ...
            ? Analyze.Cast<TGeometry, TOut>(key: EdgeMidpointsKey, query: Query<TGeometry, Point3d>.Build(
                ...
                evaluator: static (op, geometry) => geometry switch {
                    ...
                    _ => from runtime in Env.EnvAsks
                         ...
                         from result in Analyze.Many(key: op, values: curves.Map(static projection => { using Curve c = projection.Curve; return c.PointAtNormalizedLength(length: 0.5); })).ToEff()
                         select result,
                }))
