...
|  [08]   | `ProcessFeedRate` / `ProcessSpindleSpeed` | process envelope | `Minimum`/`Maximum`/`Nominal`/`Value` — the declared feed-rate and spindle-speed RANGE for the tool |
...
| [INDEX] | [SURFACE]                                                          | [ENTRY_FAMILY] | [CAPABILITY]                                            |
| :-----: | :----------------------------------------------------------------- | :------------- | :----------------------------------------------------- |
|  [01]   | `cuttingToolAsset.GenerateHash(bool includeTimestamp = true)` / static `CuttingToolAsset.GenerateHash(asset, includeTimestamp)` | digest | REJECTED as identity — Fabrication tool identity mints via `ContentHash.Of` over canonical asset bytes, never the provider digest (the second-hasher defect) |
|  [02]   | `CuttingToolLifeCycle.GenerateHash(lifeCycle)` / `CuttingItem.GenerateHash(item)` / `ToolLife.GenerateHash(toolLife)` | digest | REJECTED as identity — same law: canonical bytes through `ContentHash.Of`, per-component provider digests unused |
