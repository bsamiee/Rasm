...
- Accept: an IDS document loaded/authored through `Xids`, its facets read as the closed `IdsFacet` union the `validation#IDS_FACETS` page lowers to the `BimLeaf` term algebra, its `ValueConstraint`s lowered at parse onto the seam `ValueMatch` family (range literals coerced through `TryGetNetType`/`ParseValue`), the schema bridged to `ids-lib` through `IfcSchemaVersionHelper`, and the authored XML reconciled against the ifctester oracle
