...
- Planning-corpus verification is read-only: never compile, build, run analyzers, or execute test gates against `.planning/` design pages or any markdown target — nothing there compiles; fences are judged by reading against `docs/stacks/<language>/`, and member truth rides `tools.assay api` and the `.api` catalogs.
