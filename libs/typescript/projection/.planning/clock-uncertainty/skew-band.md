...
`SkewBand` is the HLC skew-band confidence projection — it folds the correlation-keyed earliest/latest pair into a `{ midpointMs, radiusMs }` render model surfaced as "within +/-N ms" product UI. This is the distributed-systems clock-uncertainty capability the C# desktop AppUi structurally cannot own: the browser dashboard renders honest interval confidence about event timing the host face has no read model to produce. The fold reads the HLC stamps the evidence rows carry; an HLC implementation that throws on excessive drift or counter overflow routes through the fault rail, never swallowed.

## [1]-[INDEX]

One cluster: `[2]-[SKEW_BAND]` owns `SkewBandWire`, `SkewInterval`, `skewInterval`, and the confidence-interval fold.
