...
- Entry: `Heal.Repair(MeshSpace, Option<Seq<HealOp>>, Option<RepairPolicy>, Op?)` is the one entrypoint over every modality — it admits the input, defaults the sequence to `Heal.Standard` and the policy to `RepairPolicy.Canonical`, refuses an empty sequence typed, and allocates the arena only past admission.
