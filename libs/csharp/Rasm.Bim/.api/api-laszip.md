...
- `csharp:Rasm.Compute/Tensor/blas`(`#DENSE_ALGEBRA`): the lifted `Vector<double>` enters the Compute dense-LA substrate — the `csharp:Rasm/Processing/register#REGISTRATION` cloud-ICP registration and covariance/PCA normal estimation — with no second point model
