...
- `NetTopologySuite`/`NetTopologySuite.Features`(`libs/dotnet/.api/api-nettopologysuite.md`): internal tilers cut against the global `NtsGeometryServices` EPSG:4326 `GeometryFactory`, so the tile clip inherits the canonical `PrecisionModel`; features arrive as `IFeature`/`FeatureCollection` rows.
