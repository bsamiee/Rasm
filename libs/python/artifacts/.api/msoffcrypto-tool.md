...
- consumers: `folder:exchange/detect#DETECT` (the `MediaClass.ENCRYPTED`/`MediaClass.OFFICE_LEGACY` routing target — never imported there, only routed to), `folder:document/egress#EGRESS` (the `PROTECT` `EgressStep` arm — `lazy import msoffcrypto` at boundary scope, folded onto the runtime `LanePolicy.offload` thread lane)
- installed: `6.0.0`
...
|  [03]   | `format.ooxml.OOXMLFile`     | format object    | ECMA-376 agile/standard/`plain`; sole subtype with `encrypt`/`keyTypes`/`type` |
...
| :-----: | :---------------------------- | :--------------------------- | :------------------------------------------------------------------------ |
|  [01]   | `OOXMLFile.format`            | `OOXMLFile` only (`"ooxml"`) | family discriminant; absent on the 97 objects (`getattr` default `""`)    |
|  [02]   | `OOXMLFile.type`              | `OOXMLFile` only             | method discriminant `agile`/`standard`/`plain`; `extensible` raises       |
|  [03]   | `OOXMLFile.keyTypes`          | `OOXMLFile` agile/std only   | `agile` all three, `standard` password+secret_key, `plain` UNSET          |
...
| [INDEX] | [SURFACE]                          | [CALL_SHAPE]              | [CAPABILITY]                                                          |
| :-----: | :--------------------------------- | :------------------------ | :-------------------------------------------------------------------- |
|  [01]   | `{Doc97,Xls97,Ppt97}File.load_key` | `load_key(password=None)` | derive + ALWAYS verify the 97 key (RC4/RC4-CryptoAPI/XOR)             |
|  [02]   | `{Doc97,Xls97,Ppt97}File.decrypt`  | `decrypt(outfile)`        | stream plaintext to `outfile` (no integrity option)                   |
|  [03]   | `BaseOfficeFile.is_encrypted`      | `is_encrypted() -> bool`  | report encryption (97 reads the on-stream `fEncrypted`/BIFF/PPT flag) |

[ENTRYPOINT_SCOPE]: in-package crypto method tier (the schedule the owner never re-rolls)
- rail: confidentiality

The `method/` tier is the proof that the owner composes the in-package key schedule rather than hand-rolling one over `cryptography`. The format objects call these static methods internally; the consumer never touches them, but a catalog that claims "the schedule is in-package" must name the surface that owns it. Naming them also defends the boundary: a request to re-derive AES/SHA against `cryptography.hazmat` directly is rejected because `ECMA376Agile`/`ECMA376Standard` already own it. Every `[SURFACE]` is under `method.*`, and `OOXMLFile.encrypt` drives the agile re-seal schedule.

| [INDEX] | [SURFACE]                            | [OWNS]                                                                                        |
| :-----: | :----------------------------------- | :-------------------------------------------------------------------------------------------- |
|  [01]   | `ecma376_agile.ECMA376Agile`         | agile `makekey_from_password`/`makekey_from_privkey`, `verify_password`, `decrypt`, `encrypt` |
|  [02]   | `ecma376_standard.ECMA376Standard`   | standard `makekey_from_password`, `verifykey`, `decrypt`                                      |
|  [03]   | `ecma376_extensible`                 | extensible placeholder (sniffed but `DecryptionError` at parse — never a usable object)       |
|  [04]   | `rc4.DocumentRC4`                    | legacy RC4 `verifypw` + stream decrypt (Office binary RC4)                                    |
|  [05]   | `rc4_cryptoapi.DocumentRC4CryptoAPI` | legacy RC4-CryptoAPI verify + decrypt                                                         |
|  [06]   | `xor_obfuscation`                    | XOR-obfuscation for `fObfuscation==1` DOC containers                                          |
