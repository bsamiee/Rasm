...
- [GENAI_CONVENTIONS]: the OTel GenAI semantic-convention attribute set (`gen_ai.system`, `gen_ai.operation.name`, `gen_ai.usage.input_tokens`/`output_tokens`) and the MCP span shape the agent#TOOL_DISPATCH dispatch stamps confirm against the live convention version at integration; the token-usage instruments mint on the `Rasm.AppHost` meter and ride the existing `AddView` cardinality caps.
