...
- Format identity singletons are verified static properties: `PngFormat.Instance`, `TiffFormat.Instance`, `WebpFormat.Instance`, `QoiFormat.Instance`, `JpegFormat.Instance` — each the reference a sniff result compares against, so format identity never rides a name string.
...
|  [21]   | `ProcessPixelRowsAsVector4(PixelRowOperation[<Point>][, PixelConversionModifiers])`   | chained   | the float row interface    |
...
- Bulk access is a five-surface ladder, each pinning the discontiguous group layout: `ProcessPixelRows` (typed spans, up to three images), `ProcessPixelRowsAsVector4` (float staging with declared conversion modifiers), `ParallelRowIterator` (struct operations with pooled `TBuffer` scratch), `PixelOperations<TPixel>.Instance` (the SIMD bulk converter — `ToVector4(Configuration, ReadOnlySpan<TPixel>, Span<Vector4>, PixelConversionModifiers)` and `FromVector4Destructive(Configuration, Span<Vector4>, Span<TPixel>, PixelConversionModifiers)`, the two-argument forms defaulting `PixelConversionModifiers.None`, byte-identical to a per-texel `ToScaledVector4` loop on `Rgba64`), and `AdvancedImageExtensions.GetPixelMemoryGroup`/`DangerousGetPixelRowMemory` (raw memory) — a whole-image span remains unsafe without `PreferContiguousImageBuffers`. Hand-rolled per-texel conversion loops beside the converter surface are the deleted form.
...
[STACKING]:
- `CommunityToolkit.HighPerformance`(`libs/dotnet/.api/api-highperformance.md`): `MemoryOwner<T>.Allocate(count)` IS an `IMemoryOwner<T>`, so `Image.WrapMemory(configuration, owner, width, height, metadata)` adopts a rental whose `T` is an ImageSharp `IPixel<T>` struct; the module's typed-texel arena structs are NOT ImageSharp pixel types, so the zero-copy bind is unreachable from `plane#TEXTURE_PLANE` and a `MemoryMarshal` reinterpretation across that boundary is the stride fork the codec bridge law forbids — the composed path stages through the converter surface instead, with `CopyPixelDataTo(Span<byte>)` as the only byte-level drain.
...
- `TextureCompressor`(`.api/api-texturecompressor.md`): a decoded container and a block-compressed payload meet at the plane arena, never each other — the landed bridge drains an `Image<TPixel>` into the arena's decoded row accessor and block encode reads the arena back out through `BitmapView<Rgba8UNorm>`/`BitmapView<Rgba32Float>`, so PNG/TIFF/WebP containers and BCn/ASTC/UASTC payloads share one plane custody with no direct codec-to-codec span hand-off.
- `System.IO.Hashing`(`libs/dotnet/.api/api-hashing.md`): the encoded byte stream feeds `XxHash128.Append` incrementally and `GetCurrentHashAsUInt128` closes the plane's content key, so identity derives from the encoded file the object store holds rather than from a re-encode.
- `Wacton.Unicolour`(`libs/dotnet/.api/api-unicolour.md`): perceptual and spectral color work stays on the `Unicolour` owner and this surface contributes container-level ICC carriage alone; `ColorSpaceConverter` runs only where an ingested asset declares a working space the appearance pipeline must reconcile, and never as a second color authority.
...
- Animated and multi-frame containers (GIF, animated WebP) are outside the texture module; frame collections serve only where a declared layer law makes a frame sequence the plane.
