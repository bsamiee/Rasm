...
- Law: `ParseError` accumulates, never throws — the code family (`Quotes`/`Delimiter`/`FieldMismatch`) lowers to the fault channel with the `ParseMeta` cursor as evidence.
...
- Law: the guard judges what the ARCHIVE asked for, and directory rows are dropped before it — `loadAsync` stamps `unsafeOriginalName` on file rows alone, so a folder row reaching `path.resolve` answers a thrown `TypeError` this channel never states and a clean archive refuses on its own first directory. Raw names therefore carry the claim wherever one survives, and the sanitized `name` answers otherwise, because jszip already collapsed the second, and materializing a folder row as a file writes an empty member no caller asked for.
