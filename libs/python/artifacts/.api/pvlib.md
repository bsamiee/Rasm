...
[STACKING]:
- `visualization/diagram/solar#SOLAR` binds one `pvlib.location.Location(latitude, longitude, altitude, tz)` per site and samples `Location.get_solarposition(pandas.date_range(...))` across the three cardinal declination days at `freq='5min'`; the returned `azimuth`/`apparent_elevation` columns feed the owned `ProjectionKind` (stereographic/orthographic/gnomonic) → `graphic/vector/path#PATH` arc generator, so a labeled sun-path chart is real ephemeris geometry, never caller-supplied azimuth/altitude rows.
