...
- Boundary: the store is a fold in the same altitude as the stream stores and stays transport-free; the degradation level arrives owned from `feed-stores/live-cells#LIVE_CELLS`, never re-derived here — this fold mints the ordered `DegradationLevel` over the decode-admitted wire `level` column and weighs it, but the level value is the C#-side health authority's; the `CommandGateway` reads the gate at dial time and writes back the command receipt, an intra-neutral read, not a tier reversal; the domain dials no transport.
