...
- Growth: a new geometry query is one `VectorOp` case plus one composable function over the existing `svgelements`/`pathops` surface — a curvature/tangent query rides `Path.point` plus a finite difference — never a re-implemented SVG geometry engine; a new boolean kind is one `BooleanOp` member (its `.name` already resolving the `pathops.PathOp` member), a new stroke cap/join one `CapStyle`/`JoinStyle` member, a winding-policy shift one `WindingRule` member, a winding-target flip one `WindingDir` member; text-on-path is the landed `text_path`/`VectorOp.TextPath` case threading a `typography/shape#SHAPE` `PositionedGlyphRun`'s per-glyph outlines along a baseline `Path` (arc-length `Path.point` + tangent-following `Matrix` + `OpBuilder` union overlap-merge), and a knockout boolean (glyph ∩ clip) composes the SAME `getPen`/`draw` pen spine directly onto the `typography/shape`+`typography/font` glyph-outline producers with no serialization hop; projective/keystone placement is the landed `warp`/`VectorOp.Warp` case over the `pathops.Path.transform` 3x3, winding normalization the landed `wind`/`VectorOp.Wind` case over the settable `Path.clockwise`, and multi-step affine composition the landed `compose` builder folding `Matrix.pre_*`/`post_*` in a declared `ComposeOrder` then handed to `transform`/`project`/`fit` — never a hand-rolled affine; a mirror/radial placement is the landed `reflect`/`polar` point helper over `Point.reflected_across`/`polar_to`; a new flatten target is one `FlattenKind` member plus one `_FLATTEN` row; a new projection mode is one `ProjectKind` member plus one `_PROJECT` row; a new resvg sizing/font/policy knob is one field on the existing `RenderPolicy` row carried into the one `svg_to_bytes` spread — never a second rasterizer; a new fault cause is one `VectorFault` case; a new outcome shape is one `VectorResult` case; zero new surface. A unit-converted measure STAYS the one deferred axis: `Length.to_mm`/`to_inch` return unit-suffixed strings (`"25.4mm"`) not scalars, so a numeric non-px measure lands as one `px` unit policy only once a `pt`/`mm` egress declares the suffix-strip contract, never the fragile string-parse the density bar rejects.
...
# the ordered-Matrix compose axis: each member NAME mirrors the svgelements `Matrix.<order>_<step>` compose method
# (`pre_scale`/`post_rotate`/…) so `getattr(matrix, f"{order}_{step}")` is ONE derivation, never a parallel map.
...
def warp(source: bytes, coeffs: tuple[float, float, float, float, float, float, float, float, float]) -> Result[bytes, VectorFault]:
    # the full 3x3 affine/PERSPECTIVE placement svgelements' 6-tuple Matrix cannot express: ingest the outline into the
    # Skia accumulator ONCE, apply pathops.Path.transform(scaleX, skewY, skewX, scaleY, translateX, translateY,
    # perspectiveX, perspectiveY, perspectiveBias) in place, draw back — the keystone/perspective dewarp a projective
    # placement needs and the affine Matrix (project/transform) has no member for.
    def _apply(shape: "pathops.Path", /) -> Result[bytes, VectorFault]:
        def _run() -> "pathops.Path":
...
def wind(source: bytes, direction: WindingDir = WindingDir.CW) -> Result[bytes, VectorFault]:
    # NORMALIZE every contour to one winding direction — region/subpaths READ winding facts but no op fixes direction,
    # which a hole-order/toolpath/nesting consumer keys per loop: simplify to canonicalize, then set the Skia
    # `clockwise` policy so a contour whose direction disagrees is reversed to match, and draw the oriented outline back.
    def _orient(shape: "pathops.Path", /) -> Result[bytes, VectorFault]:
        def _run() -> "pathops.Path":
            shape.simplify(fix_winding=True)
...
def compose(steps: tuple[tuple[ComposeStep, float, float], ...], order: ComposeOrder = ComposeOrder.POST) -> "Matrix":
    # build ONE ordered affine through the svgelements Matrix.pre_*/post_* compose family the growth prose named as an
    # axis: each step folds onto the running Matrix by `getattr(matrix, f"{order}_{step}")`, so a caller declares a
    # scale/translate/rotate/skew sequence in pre- or post-multiply order and hands the result to transform/project/fit,
    # never hand-building a 6-tuple. `rotate` takes the lone angle; scale/translate/skew take the (a, b) pair.
    matrix = Matrix()
...
def reflect(points: Iterable[Point2], across: Point2) -> tuple[Point2, ...]:
    # mirror each point across a pivot (Point.reflected_across) — the mirror/knockout placement a marks/dimension consumer keys.
    pivot = Point(*across)
...
def polar(origin: Point2, angle: float, distance: float) -> Point2:
    # place a point at (angle, distance) from an origin (Point.polar_to) — the radial witness/tick placement a dimension consumer keys.
...
class VectorOp:
    ...
    warp: tuple[bytes, tuple[float, float, float, float, float, float, float, float, float]] = case()  # 3x3 affine/perspective coeffs
    ...
    @staticmethod
    def Warp(source: bytes, coeffs: tuple[float, float, float, float, float, float, float, float, float]) -> "VectorOp":
        return VectorOp(warp=(source, coeffs))

    @staticmethod
    def Wind(source: bytes, direction: WindingDir = WindingDir.CW) -> "VectorOp":
        return VectorOp(wind=(source, direction))
...
def _dispatch(op: VectorOp, /) -> Result[VectorResult, VectorFault]:
    match op:
        ...
        case VectorOp(tag="warp", warp=(source, coeffs)):
            return warp(source, coeffs).map(lambda document: VectorResult(document=document))
        case VectorOp(tag="wind", wind=(source, direction)):
            return wind(source, direction).map(lambda document: VectorResult(document=document))
...
- [PROJECTIVE_WINDING_COMPOSE] [RESOLVED]: four edge arms consumers now demand land over already-verified members — no member is a phantom. (1) `Warp(source, coeffs)` rides `pathops.Path.transform(scaleX, skewY, skewX, scaleY, translateX, translateY, perspectiveX, perspectiveY, perspectiveBias)` — the full 3x3 affine/PERSPECTIVE the svgelements 6-tuple `Matrix` (which `project`/`transform` use) cannot express, so a keystone/perspective placement has a spelling on geometry already in the Skia accumulator; reflected present with the perspective kwargs on the installed `pathops 0.9.2` (`Path.transform` signature carries `perspectiveX`/`perspectiveY`/`perspectiveBias`, and a perspective apply round-trips). (2) `Wind(source, direction)` rides the settable `Path.clockwise` (paired with `Direction` `CW`/`CCW` and `Path.reverse()`), the winding NORMALIZATION `region`/`subpaths` READ but no op fixed — verified `clockwise` flips `False`->`True` under `reverse()` and is directly settable. (3) `compose(steps, order)` promotes the named growth axis to a real builder folding the svgelements `Matrix.pre_*`/`post_*` compose family (catalog Matrix rows [06]/[07]) by `getattr(matrix, f"{order}_{step}")`, `rotate` taking the lone angle and scale/translate/skew the pair, so a caller declares an ordered affine and hands it to `transform`/`project`/`fit` rather than a hand-built 6-tuple. (4) `reflect`/`polar` ride `Point.reflected_across`/`polar_to` (catalog value-object row [07]) for the mirror/radial placement a `marks`/`dimension` consumer keys. The `region` fact tuple gains the control-hull extent via `Path.controlPointBounds` (catalog query row [04], the control-hull bbox distinct from the tight `bounds`), appended as `Region`'s fifth element so existing positional reads of `(area, bounds, count, convex)` are untouched. Justified on PACKAGE (each was an unused verified member) and CONSUMER (keystone dewarp, toolpath/nesting winding order, ordered compose, and mirror/radial marks the placement/diagram/dimension planes key).
