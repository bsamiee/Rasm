# [@types/react] — the React 19 type vocabulary the compiler and specs typecheck against
...
|  [02]   | `Ref<T>` = `RefCallback<T> \| RefObject<T \| null> \| null` / `RefObject<T>` (`{ current: T }`) | ref algebra   | `act/gesture` + `view` — React 19 `ref` value; `RefObject.current` is mutable, `MutableRefObject` is the legacy alias |
...
|  [04]   | `ForwardedRef<T>` / `PropsWithoutRef<P>` / `PropsWithRef<P>`                          | ref plumbing       | the `forwardRef` render-fn ref param + prop ref-stripping; legacy where `ref` is now a plain prop |
...
|  [06]   | `ElementRef<T>` (deprecated → `ComponentRef<T>`) / `LegacyRef<T>` (deprecated)        | legacy ref         | never author new — `ComponentRef<T>` is the ref-target extractor; string refs are gone |
...
- rail: view/primitive
- `HTMLAttributes<T>` is ONE attribute base (over `AriaAttributes` + `DOMAttributes<T>`) extended per element into `*HTMLAttributes<T>`; `JSX.IntrinsicElements` maps each tag to `DetailedHTMLProps<*HTMLAttributes<E>, E>`. A `react-aria` hook returns a bundle that spreads onto one of these. The canary/experimental rows are the gated upgrade surface `act/transition` owns.
...
|  [06]   | `ViewTransitionProps` / `ViewTransitionClass` / `ActivityProps` / `FragmentInstance` / `SuspenseListProps` (canary/experimental) | gated upgrade | `act/transition` — the `<ViewTransition>`/`<Activity>` prop types; import from `./canary`/`./experimental` only |
...
|  [07]   | `unstable_useCacheRefresh` / `addTransitionType` / `unstable_startGestureTransition` (canary/experimental) | gated       | `act/transition` — behind a capability flag; import from `./canary`/`./experimental` |
