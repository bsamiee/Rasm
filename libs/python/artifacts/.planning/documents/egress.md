...
NATIVE: frozenset[EgressStep] = frozenset({EgressStep.ENCRYPT, EgressStep.WATERMARK, EgressStep.ATTACH})
...
class DocumentEgress(Struct, frozen=True):
    ...
    async def _emit(self) -> ContentKey:
        data = (
            await to_process.run_sync(_native_finish, self.step.value, self.pdf, self.params, self.permissions)
...
def _pure_finish(step: EgressStep, payload: bytes, params: dict[str, object], node: DocumentNode | None) -> bytes:
    match step:
...
def _outline(payload: bytes, params: dict[str, object], node: DocumentNode | None) -> bytes:
    ...
    if node is not None:
        ...
        for section in (n for n in walk(node) if isinstance(n, SectionNode)):
            ...
            parents[section.level] = writer.add_outline_item(title, section.meta.page, parent=parent)
    else:
...
def _native_finish(step: str, payload: bytes, params: dict[str, object], permissions: Permissions) -> bytes:
    from io import BytesIO
    ...
    match EgressStep(step):
        case EgressStep.ENCRYPT:
            pdf.save(
                sink,
                linearize=True,
                encryption=pikepdf.Encryption(owner=params["owner"], user=params.get("user", ""), R=6, aes=True, allow=permissions.to_pikepdf()),
            )
        ...
        case EgressStep.ATTACH:
            pdf.attachments[params["name"]] = pikepdf.AttachedFileSpec(pdf, params["data"], description=params.get("description", ""))
            pdf.save(sink, linearize=True)
        case _:
            assert_never(step)
    return sink.getvalue()
...
- [BOUNDARY_AGAINST_CONFORMANCE]: the egress close finishes structure and navigation only; PAdES cryptographic signing is owned by `folder:../typography/conformance#CONFORM` `SIGN`/`AUDIT` (pyhanko), and PDF/A archival authoring by `folder:documents/emit#DOCUMENT` `PDF_TYPST` `pdf_standards` — neither is an `EgressStep`. The `ENCRYPT` arm is qpdf AES document encryption (a confidentiality control), not a digital signature; a signed-then-encrypted archival close is the `conformance#CONFORM` `SIGN` arm composing this owner's `ENCRYPT` output, the signature applied last so encryption never invalidates the byte-range. The `ArtifactReceipt.Egress` case owned at `folder:../receipt/receipt#RECEIPT` carries the post-finish byte count, page count, encryption R level, outline depth, and overlay count, so the finish is one fact row in the shared receipt stream, never a per-step egress receipt.
