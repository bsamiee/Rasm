...
lazy from rustworkx import DAGWouldCycle, PyDAG, ancestors, descendants, digraph_find_cycle
lazy from rustworkx import is_directed_acyclic_graph, node_link_json, parse_node_link_json
lazy from rustworkx import topological_generations, topological_sort, transitive_reduction
...
class ReferenceReport(Struct, frozen=True):
    ...
    order: tuple[str, ...] = ()             # topological citation order (topological_sort -> numbering)
    generations: tuple[int, ...] = ()       # per-generation node counts (topological_generations -> depth)
...
def _resolve(detail: Detail) -> Resolved:
    ...
    report = ReferenceReport(
        order=tuple(reverse[node] for node in topological_sort(dag)),
...
def _dxf_engine(detail: Detail) -> tuple[tuple[Layer, ...], ArtifactReceipt]:
    ...
    return (Layer(name="dxf", source=data, bbox=box),), ArtifactReceipt.Drawing(key, "detail", len(detail.callouts), "ezdxf", int(box[2] - box[0]), int(box[3] - box[1]), len(data))
...
- [RECEIPT_DRAWING_CASE] [RESOLVED]: the owner contributes the shared `core/receipt#RECEIPT` `ArtifactReceipt.Drawing(key, "detail", entities, engine, width, height, bytes)` case (the callout count as `entities`, the `"drawsvg"`/`"ezdxf"` render engine in the fourth slot, the rendered extents and byte count), NOT a parallel receipt rail — the brief `[07]` legislates one shared `Drawing` case across the drawing plane, and this owner fills the case shape `core/receipt#RECEIPT` already declares (`(key, kind, entities, dimstyle, width, height, bytes)`) EXACTLY as `drawing/symbol#SYMBOL`'s own `ArtifactReceipt.Drawing(key, "symbol", …, "drawsvg", …)` mint does — the two drawing-plane producers fill the fourth slot with the render-engine descriptor uniformly, so the case is composed as-is with NO cross-file receipt edit. The receipt carries PRODUCTION evidence (kind, entity count, engine, extent, byte count); the cross-reference TOPOLOGY facts (`order`/`generations`/`edges`/`reduced`/`dangling`) `ReferenceReport` computes are documentation-quality AUDIT the `composition/sheet#SHEET` detail-index consumer reads off `ReferenceReport` before issue, not receipt-stream scalars — so they ride `ReferenceReport` and are not under-reported on the receipt (a receipt records what was produced, the reference graph records how the set cross-references). Justified on CONSUMER (the settled shared `Drawing` case + the `ReferenceReport` detail-index reader) and the brief `[07]` one-shared-case law. The single open cross-file item is `core/receipt#RECEIPT`'s own and out of this page's scope: its `Drawing` fourth slot is named `dimstyle`, yet the `symbol`/`detail` producers fill it with the render engine while `drawing/dimension#DIMENSION` fills the ISO 129-1 style name, so the slot's semantics vary by producer — whether to rename it to a neutral `style` or carry a distinct trailing `engine` scalar is the owning `core/receipt.md`'s decision, never a `detail.md` defect, and this page composes the settled case shape correctly either way.
