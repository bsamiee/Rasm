...
- Accept: a `.usd*` scene read/authored through `UsdStage` and the typed schemas, the mesh crossing the `VtVec3fArray`/`VtIntArray` seam to the kernel vocabulary, the frame normalized Y-up→Z-up through `FrameNormalization`, the shade network reconciled with `BimAppearance`/OpenPBR, and federation/options composed on USD references/variants — coexisting with the IFC semantic graph as a scene-graph peer
