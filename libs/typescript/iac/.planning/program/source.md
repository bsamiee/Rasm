...
- Law: one branch-law owner per repo — `RepositoryRuleset` for this repo; `BranchProtection` survives only when adopting a classic-protected repo, never beside the ruleset.
...
- Growth: a new gated environment is one `environments` row; a new org-level posture (`Team`, `TeamRepository`, `OrganizationRuleset`) is one row when the project grows an org.
- Boundary: the mirror mechanics are `operate/secret.md`'s; the CI workflows that run inside the shells are app repo material, never lib code; `appAuth` (GitHub-App identity) supersedes the PAT when the project earns a durable machine identity.
...
- Law: decoder rows are deployment-invariant and their digests are build facts, so `distribute` takes `decoders` as one slug-to-digest map and mints the rows itself — `_DECODERS` supplies every leaf name, a caller states one digest per slug it ships, and no roster re-spells a filename the loader resolves by name; the minted rows join the caller's own artifacts BEFORE admission, so collision and presence prove over the whole published set rather than over the app's half of it.
...
- Law: serving paths are content-addressed and immutable — `_addressed` is the ONE address projection of `_segments`, the empty leaf spelling the digest directory itself, so the file form and the directory form cannot drift; a byte change mints a new digest hence a new path and a new directory, and both boundary ends compute the derivation independently because no free-form path string ever crosses.
...
- Law: `_TYPE_REPAIR` pins the served type for every egress extension the origin cannot be trusted to resolve — the sync's content-type derivation belongs to the provider plugin, versioned outside this repo and unreachable from any argument the dialects expose, so an extension that table misses publishes a type a `nosniff` origin makes fatal for every consumer that types its read. The roster answers by stating the type where the value IS this deployment's, at the edge; a row duplicating a type the origin already resolves is inert, so the roster costs nothing when a provider table catches up and holds when it does not. `hdr` and `jxl` carry the rows because radiance and JPEG-XL are the two egress media types no provider table decides for this plane. Rows here state the SERVED type of bytes this plane already publishes and never what a codec means.
