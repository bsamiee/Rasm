...
- Packages: `premise` (`NewDatabase(scenarios, source_type='brightway', source_db=, key=, system_model=).update(sectors).write_db_to_brightway(name)`, `write_superstructure_db_to_brightway`, `write_datapackage`, `IncrementalDatabase.update`/`write_increment_db_to_brightway`, `PathwaysDataPackage(scenarios, years=, ...).create_datapackage(name)` — catalog-verified members behind the floor gate), `bw2data` (`databases.list`/`databases.version`, the registry the receipt and the carrier both read, `errors.BW2Exception` the family root the build fence names), runtime (`RuntimeRail`/`boundary`/`Catch`/`FaultRow`/`scoped`/`on_thread`).
...
class ScenarioBuild(Struct, frozen=True):
    ...
    async def build(self, kind: BuildKind, key: str) -> "RuntimeRail[ScenarioReceipt]":
        ...
        def run() -> "RuntimeRail[ScenarioReceipt]":
