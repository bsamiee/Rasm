...
- Draws on the gap that the prior page modeled only four pairwise legacy modes with no global bootstrap and no multiway optimization; technique is open3d `t.pipelines.registration.multi_scale_icp`, `registration_ransac_based_on_feature_matching` over FPFH, and the pose-graph `global_optimization` pattern.
