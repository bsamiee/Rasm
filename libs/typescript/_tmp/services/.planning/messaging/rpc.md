...
- [01]-[INTERNAL_RPC]: owns the one `RpcGroup` and the `WorkflowProxy` projection over the durable workflows.
...
- Entry: each procedure is a Schema-typed request; the node host-to-worker edge consumes the same group; the workflow proxy (`execution/saga#SAGA` derives its saga procedures here) is the sole mechanism by which a durable unit becomes a callable procedure.
