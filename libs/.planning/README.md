...
- Folder (`<pkg>/`): the four index docs at the package root — `README.md`, `ARCHITECTURE.md`, `IDEAS.md`, `TASKLOG.md` — plus design pages under one `<pkg>/.planning/<sub-domain>/<page>.md` and a `<pkg>/.api/` of generated package catalogues, one per folder-specific external library the folder uses. No `FEATURES.md`, no `.planning/README.md`. The folder `.api/` is the resource a planning pass reads to draw an external package's real API without guessing.
- Branch `.api/` (`libs/<lang>/.api/`, Python and TypeScript only): one catalogue per language-wide substrate package, authored once and never duplicated into a folder `.api/`. A folder consuming a substrate package reads the branch catalogue and lists the package in its README `## [3]-[SUBSTRATE_PACKAGES]` section. C# carries no branch `.api/`: its universal kernels (`LanguageExt.Core`, `Thinktecture.Runtime.Extensions` core) are excluded from catalogues entirely, and every other C# catalogue stays per folder.
...
- Substrate packages (all languages): a `## [3]-[SUBSTRATE_PACKAGES]` section after the domain list names the language substrate the folder consumes — the typing/rails, concurrency, observability, identity, time, numeric, wire-codegen, and test tiers — pointing to the branch registry. Python and TypeScript substrate carries a branch `libs/<lang>/.api/` catalogue; C# substrate (the globally injected `LanguageExt.Core`/`Thinktecture.Runtime.Extensions` kernels) carries none.
...
[ARCHITECTURE] — the folder's professional domain map and seam record.
...
- `[2]-[SEAMS]` is the curated, file-level record of every cross-folder and cross-language alignment, in one ` ```text seams ` fence with aligned columns: `<SourceFile> <glyph> <lang:pkg/subdomain> # <shared shape> (<kind>)`. Glyphs: `→` produces/projects, `←` consumes/reads, `⇄` shared shape. `<kind>` ∈ {`wire`, `projection`, `port`, `content-key`, `tessellation`, `graduation`, `receipt`, `transport`, `fault`, `shape`, `boundary`}. Every edge appears on both endpoint folders with mirrored glyph; an in-package relation stays in the codemap and is never a seam.
...
A package that is language-wide substrate — the typing, concurrency, observability, numeric, and wire-codegen tiers a branch's `.planning/` `[2]-[SUBSTRATE_PACKAGES]` registry names — carries one catalogue at the branch `libs/<lang>/.api/` (Python and TypeScript only; see [1]), never duplicated into a folder `.api/`, and each consuming folder lists it in its README `## [3]-[SUBSTRATE_PACKAGES]` section. A package shared across folders only as a domain seam (mesh-file interchange, object-store transport, companion native cores) keeps a per-folder catalogue in each consuming folder rather than a branch catalogue, because the seam resource is folder-local. Per-language packages that are not cross-cutting stay folder-local in the folder README and `.api/`, and are never duplicated into the cross-`libs/` core.
