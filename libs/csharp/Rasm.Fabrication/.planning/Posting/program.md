...
- Entry: `PostSource.Motion`, `PostSource.Placement`, and `PostSource.Specialized` enter one `Post.Lower` fold and diverge only inside `PostSource.Switch`; every arm opens its program on `Prologue`, which prepends the run's keyed drawing marks as one verbatim comment block ahead of the frame assignments.
