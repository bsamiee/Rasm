...
- [2]-[GEO_WIRE_DECODE]: the `GeoJsonWire` seven-type RFC 7946 union, the `Position` floor, the WGS84 bound, the absent-CRS guard.
- [3]-[SEGMENT_STREAM_DECODE]: the backpressured `decodeSegmentStream` async-array fold and the `SNAPSHOT_CODEC` decompression delegate-row.
- [4]-[CRDT_OP_DECODE]: the `CrdtOpWire` ten-arm union over the crdt column-family payload.
- [5]-[TS_PROJECTION]: the snapshot, sync, and receipt wire shapes the rails decode.
- [6]-[BCF_LIVE_WIRE_DECODE]: the Bim BCF topic/viewpoint and AppHost live-wire binding json-stj rows.
