...
|  [17]   | ComputeService  | GraphDiff          | unary         | GraphDiffRequest → GraphDiffResponse     | content-key delta over two Closure hashes; the diff algebra is `Rasm.Persistence/sync/collaboration#TRANSPORT_AXIS`, this carries the wire shape only                                           |
...
- [COMPOSED_CREDENTIAL]: the live-ALC dial of the composed `CredentialPolicy` identity through a running plugin channel, and the fingerprint-matched `BenchmarkClaim` that gates the compression flip on the live host, resolve against the running plugin process; the `CallSpine` interceptor surface, the `WireFault` `StatusCode` rail, the `ContractGuard` projection-checksum fold, and the `WireDocument`/`FrameEdge.Transaction` parity choreography are authored in the cluster fences. The open leaves are the composed-identity ALC dial and the benchmark-gated compression flip.
