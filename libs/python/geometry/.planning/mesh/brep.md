...
- Cases: `BrepOp` cases `Construct(verb, params)` (primitive box/sphere/cylinder/cone/torus over `BRepPrimAPI_*`), `Boolean(shapes, verb, fuzzy)` (n-ary fuse/cut/common/section/split over the robust `BRepAlgoAPI_*` BOPAlgo operators driven by `SetArguments`/`SetTools`, `SetFuzzyValue` tolerant intersection when the payload carries a positive fuzz, `SetRunParallel` always, the `History()` evidence read back), `Offset(profile, verb, dist, height, join)` (the `manifold3d.CrossSection` 2D-offset-simplify leg lifted through `BRepBuilderAPI_MakeFace.Add` then extruded/revolved/lofted/thickened over `BRepPrimAPI`/`BRepOffsetAPI`), `Feature(shape, verb, size)` (fillet/chamfer over `BRepFilletAPI_*`, `SEW` shell-to-solid over `BRepBuilderAPI_Sewing(tolerance)`, `NURBS` conversion over `BRepBuilderAPI_NurbsConvert`), and `Tessellate(shape, policy)` (the in-place `BRepMesh_IncrementalMesh` triangulation under the `TessellationPolicy` band, extracted to an in-memory `trimesh.Trimesh`) — matched by `match`/`assert_never`, each binding the OCCT builder family that owns the kind through the `Map` dispatch tables. Linear extrusion/revolution of a profile is the `Offset` arm's `EXTRUDE`/`REVOLVE` row, not a duplicate `Construct` primitive — `MakePrism`/`MakeRevol` are reached once, through the profile leg. The `Boolean.section` row yields a `TopoDS` wire/edge result the consumer re-feeds as a profile; the daemon and CAD hop never re-discriminate the operation past this owner.
...
class BooleanVerb(StrEnum):
    ...
    COMMON = "common"
    SECTION = "section"
    SPLIT = "split"  # BRepAlgoAPI_Splitter: arguments partitioned by tools, the non-destructive divide
...
class FeatureVerb(StrEnum):
    FILLET = "fillet"
    CHAMFER = "chamfer"
    SEW = "sew"  # BRepBuilderAPI_Sewing(size): join open shells into a solid-capable shell; size is the sewing tolerance
    NURBS = "nurbs"  # BRepBuilderAPI_NurbsConvert: convert analytic surfaces/curves to their NURBS form; size unused
...
# the edge-fold rows only: SEW/NURBS are shape-conditioning arms with their own non-maker call shapes,
# matched ahead of this table in the feature arm.
...
class BrepOp:
    ...
    def Boolean(shapes: Shapes, verb: BooleanVerb, fuzzy: float = 0.0) -> Self:
        # a positive fuzz drives BOPAlgo SetFuzzyValue tolerant intersection for near-coincident operands
    ...
    def Offset(
        ...
        # `smooth` lifts the profile through a GeomAPI_PointsToBSpline B-spline edge instead of the polyline wire
...
def _boolean(shapes: Shapes, verb: BooleanVerb, fuzzy: float) -> tuple[TopoDS_Shape, bool, bool]:
    ...
    shape = _built(op)
    history = op.History()  # the BOPAlgo provenance evidence the receipt records beside the census
...
def _wire(contour: np.ndarray, z: float = 0.0, *, smooth: bool = False) -> TopoDS_Wire:
    if smooth:
        # the Geom_* band: a periodic-fit B-spline through the contour points, lifted through
        # MakeEdge/MakeWire — the smooth profile modality, one flag, never a parallel entry.
...
def _dispatch(op: BrepOp) -> BrepResult:
    match op:
        ...
        case BrepOp(tag="offset", offset=(profile, verb, dist, height, join, smooth)):
            ...
            match verb:
                ...
                case OffsetVerb.LOFT:
                    # `ThruSections` lofts one wire skin between the base and offset OUTER rings — a holed loft is
                    # the staged `Cut`-of-two-skins arm this skin builder cannot model, so an inner-loop profile
                    # rejects here rather than silently dropping its holes.
        ...
        case BrepOp(tag="feature", feature=(target, FeatureVerb.SEW, size)):
            # the shell-conditioning row: Sewing is Perform/SewedShape, not the Build/IsDone/Shape maker family
