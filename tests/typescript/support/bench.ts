...
class BenchmarkDirectory extends Context.Reference<BenchmarkDirectory>()('rasm-test-support/BenchmarkDirectory', {
...
const _readHistory: Effect.Effect<ReadonlyArray<BenchmarkResult>, BenchmarkError, FileSystem.FileSystem | Path.Path> = Effect.gen(function* () {
    ...
    return yield* Effect.mapError(
        Effect.forEach(Array.filter(String.split(raw, '\n'), String.isNonEmpty), (line) => _decodeResult(line)),
        (error) => new BenchmarkError({ reason: 'malformed', detail: error.message }),
    );
});

const _importLatestResults: Effect.Effect<ReadonlyArray<BenchmarkResult>, BenchmarkError, FileSystem.FileSystem | Path.Path> = Effect.gen(
    function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const directory = yield* BenchmarkDirectory;
        const target = path.join(directory, _FILES.latest);
        const raw = yield* Effect.mapError(fs.readFileString(target), (error) => new BenchmarkError({ reason: 'unreadable', detail: error.message }));
        const info = yield* Effect.mapError(fs.stat(target), (error) => new BenchmarkError({ reason: 'unreadable', detail: error.message }));
        ...
        const latest = yield* Effect.mapError(_decodeLatest(raw), (error) => new BenchmarkError({ reason: 'malformed', detail: error.message }));
...
const Benchmark = {
    POLICY: _POLICY,
    summarize: (rows: ReadonlyArray<BenchmarkResult>, policy: Benchmark.Policy = _POLICY): Benchmark.Report =>
        pipe(
            Array.groupBy(rows, (row) => row.name),
            Record.collect((name, run) => {
                const history = Array.sort(
                    run,
                    Order.mapInput(Order.string, (row: BenchmarkResult) => row.timestamp),
                );
                const recent = Array.takeRight(history, policy.window);
                const baselineHz = pipe(
                    _median(Array.map(Array.dropRight(history, policy.window), (row) => row.hz)),
                    Option.getOrElse(() => 0),
                );
                const recentHz = pipe(
                ...
                return {
                    name,
                    verdict: sustained ? 'regression' : noisy ? 'noisy' : 'pass',
                    baselineHz,
                    recentHz,
            ...
            (benchmarks) => ({
                ...
                verdict: Array.isNonEmptyReadonlyArray(benchmarks)
                    ? pipe(
                          Array.max(
                              Array.map(benchmarks, (benchmark) => _SEVERITY[benchmark.verdict]),
                              Order.number,
                          ),
                          (rank): Benchmark.Verdict => (rank === _SEVERITY.regression ? 'regression' : rank === _SEVERITY.noisy ? 'noisy' : 'pass'),
                      )
    ...
    checkRegression: (policy: Benchmark.Policy = _POLICY): Effect.Effect<Benchmark.Report, BenchmarkError, FileSystem.FileSystem | Path.Path> =>
        Effect.gen(function* () {
                      ...
                      new BenchmarkError({
                          ...
                          detail: Array.join(
                              Array.filterMap(report.benchmarks, (benchmark) =>
                                  benchmark.verdict === 'regression' ? Option.some(benchmark.name) : Option.none(),
                              ),
                              ', ',
                          ),
                      }),
