...
The typed property vocabulary the `PropertySet`/`QuantitySet` bag nodes carry: one `PropertyValue` `[Union]` closing the IFC-value family (`Text`/`Measure`/`Boolean`/`Logical`/`Enumerated`/`Reference`/`Bounded`/`List`/`Table`/`Complex`) so a property carries its data type rather than a stringly-typed value, one `PropertyName` `[ValueObject<string>]` key, the ONE `ValueBag<V>` generic the `PropertyBag`/`QuantityBag` global aliases close over, and the `InheritanceMode` `[SmartEnum<string>]` owning the type→occurrence precedence fold (`Resolve<V>`) the bag `Merge` delegates to and the `Bake` (`Graph/element#ELEMENT_GRAPH`) applies. The IFC `Pset_*` roster, the bSDD template resolution, the geometry-true base-quantity derivation, and the `IfcRelDefinesByProperties` round-trip STAY in the `Rasm.Bim` `Semantics/properties` projector — the seam owns the typed value shape and the inheritance algebra only, carrying no `Pset_*` name and no geometry. The one neutral `DetailSchema` pins the detail-bag seam contract — neutral `SetName`s, stamped precedence, the closed `JointType` allowed-set, and the canonical detail `PropertyName` vocabulary — the SINGLE schema over `PropertyBag` the `Rasm.Materials` `Component` projection authors and the `Rasm.Bim` `Semantics/connection` reader round-trips. The page composes `Properties/quantity#MEASURE_VALUE` for the measured arm, the `Projection/address#CONTENT_ADDRESS` `CanonicalWriter` for the content-byte projection, and the kernel `Rasm.Domain` `Op` op-key; the fallible `PropertyValue.Of` structural admission rails `Projection/fault#FAULT_BAND` `ElementFault.ValueRejected` on a malformed composite value and recurses the `List`/`Table`/`Complex` arms, while the case constructors stay the already-validated construction.

## [01]-[INDEX]

- [02]-[PROPERTY_VALUE]: the `PropertyValue` `[Union]` typed IFC-value family, the `PropertyName` key, the `Interpolation` table-curve rule, the fallible `Of` structural admission, the recursive `Remap` node-id rewrite and its `References` reachability dual the `Relations/relation#EDGE_ALGEBRA` `Generic` edge composes (renumber and cascade in lockstep), and the canonical `Render`/`CanonicalBytes` folds.
...
- Entry: the case constructors are the typed admissions (`new PropertyValue.Measure(measure)`, `new PropertyValue.Bounded(lower, upper, setpoint)`); `PropertyValue.Of(value, key)` is the fallible structural admission a raw author crosses — railing `ElementFault.ValueRejected` on an empty/cross-type/inverted `Bounded`, a non-subset or empty-`Allowed` `Enumerated`, an empty `Table`, or an empty `Complex`, recursing the composite arms so a malformed nested value never rides a well-formed composite; `Render()` is the canonical string projection for a text-only consumer; `CanonicalBytes(CanonicalWriter)` writes the case discriminant and typed payload into the `Projection/address#CONTENT_ADDRESS` `CanonicalWriter` so a property value contributes to the node content key deterministically; `Remap(Func<NodeId, NodeId>)` is the ONE recursive node-id rewrite the `Relations/relation#EDGE_ALGEBRA` `Relationship.Remap` `Generic` arm composes so a `Rasm.Persistence` `Version/merge#STRUCTURAL_MERGE` `Reconcile` never strands a graph reference buried in a passthrough edge's payload, and `References()` is its reachability dual (`Seq<NodeId>` recursing the same `Reference`/`List`/`Table`/`Complex` arms) the `Relationship.Members` fold reads so a buried reference is indexed and cascade-swept in lockstep with the rewrite.
- Auto: `Render` dispatches the generated total `Switch` — `Text` verbatim, `Measure` the SI magnitude plus canonical unit, `Boolean`/`Logical` `TRUE`/`FALSE`(/`UNKNOWN`), `Enumerated` the selected join, `Reference` the target id, `Bounded` the `[lower, upper, setpoint]` interval, `List`/`Table` the recursive join, `Complex` the `usage{name=value;…}` named-bag join — one projection, never a per-case consumer branch; `CanonicalBytes` writes the case ordinal then the payload (a `Measure` quantized to tolerance, the `Logical` a presence bit plus the bool, every collection count-prefixed so the encoding is injective, the `Complex` sub-properties name-sorted `Ordinal`) so the content key is byte-stable across runtimes.
...
public abstract partial record PropertyValue {
 ...
 // field rides along untouched); List/Table/Complex recurse their children, every scalar arm is identity. A new
...
public sealed record DetailSchema(string SetName, InheritanceMode Inheritance, Seq<string> JointTypes) {
 ...
 // R1/R2/Rm, special-shape token) plus UnitLength the nominal unit length and CourseHeight the coursing height
 // (unit height + bed joint) the coursing tolerance and GLB tessellation read off the laid unit's bag.
 ...
 public static readonly PropertyName UnitLength = PropertyName.Create("UnitLength");
 ...
 // The JointType row VALUE over THIS schema's closed allowed-set — the schema owns Allowed, so an out-of-set token
 // rails ElementFault.ValueRejected at Of.
...
- [IFC_VALUE_FAMILY]: the `PropertyValue` ten-case union mirrors the IFC `IfcSimpleProperty` hierarchy in full plus the `IfcComplexProperty` aggregate — `IfcPropertySingleValue` (the typed scalar over the `IfcValue` select: `Text` for `IfcText`/`IfcLabel`/`IfcIdentifier`, `Measure` for `IfcMeasureValue`/`IfcDerivedMeasureValue`, `Boolean` for the two-valued `IfcBoolean`, `Logical` for the three-valued `IfcLogical`), `IfcPropertyEnumeratedValue` (`Enumerated`, its `EnumerationValues` LIST cardinality the `Seq<string> Selected` multi-select), `IfcPropertyReferenceValue` (`Reference` with `UsageName`), `IfcPropertyBoundedValue` (`Bounded`), `IfcPropertyListValue` (`List`), `IfcPropertyTableValue` (`Table` carrying the `IfcCurveInterpolationEnum` rule as the neutral `Interpolation`), and `IfcComplexProperty` riding the recursive `Complex` arm (its `HasProperties` keep their own `PropertyName`s — names the order-only `List` and the lookup `Table` cannot hold). The seam carries the full value family the `Rasm.Bim` projector narrows from at ingest without losing the three-valued logical, the multi-value enumeration, the table-curve semantics, or the named nested sub-properties, and the canonical bytes count-prefix every collection so the content key is injective across the C#/Python/TypeScript runtimes sharing the one `XxHash128` seed.
