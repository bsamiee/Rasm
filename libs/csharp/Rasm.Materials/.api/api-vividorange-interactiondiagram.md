...
- ABI floor: a `0.2.x` PRE-1.0 contract — the `InteractionDiagram`/`DiagramSettings` member set may break across a
 ...
 `VividOrange.ForceMomentInteraction` + `VividOrange.IForceMomentInteraction` (the output mesh),
 `VividOrange.Sections` + `VividOrange.ISections` (the `IConcreteSection`/reinforcement input),
 `VividOrange.Materials` + `VividOrange.IMaterials` (`AnalysisMaterialFactory`), `VividOrange.Profiles` /  `Profiles.Perimeter` / `Geometry` / `ICartesianBase`, `UnitsNet`, plus the COMPUTATIONAL-GEOMETRY
 substrate `MIConvexHull` (the N-M-M hull) and `Triangle` (the section mesher) — both
