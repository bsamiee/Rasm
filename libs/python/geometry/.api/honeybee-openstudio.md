...
- version: `0.5.5`; floor `honeybee-openstudio; python_version<'3.15'`
- license: AGPL-3.0 (Ladybug Tools copyleft); the bound `openstudio` SDK is BSD-3-Clause
- abi: pure-Python `py3-none-any` wheel ITSELF, but it imports the native `openstudio` SDK (`3.10.0`) at module load — and `openstudio` ships per-interpreter compiled binary wheels (`Requires-Python >=3.7.1`, built through the cp312 era) wrapping the C++ OpenStudio/EnergyPlus core, with NO `none-any` and NO cp315 build. `model_to_openstudio` returns a real `openstudio.openstudiomodelcore.Model`.
- marker: COMPANION-GATED (ABI-REAL). Unlike the pure-Python honeybee-core/energy/standards (manifest-convenience gating), honeybee-openstudio is gated by a HARD binary dependency: the native `openstudio` SDK has no cp315 wheel, so the floor `python_version<'3.15'` is an ABI cap, not just manifest co-release. `assay api resolve honeybee-openstudio` returns `unsupported` on the active 3.15 interpreter. This catalog is verified against the real `0.5.5` wheel + `openstudio 3.10.0` reflected on a sibling cp312 interpreter; the gate lifts only when the `openstudio` SDK ships a cp315 build.
- depends-on: `honeybee-energy` (the energy model it translates), `honeybee-standards`, and the native `openstudio` SDK (`3.10.0`, the binary OpenStudio/EnergyPlus runtime) — a HARD import-time requirement
- entry points: `honeybee-openstudio` console script (`translate` sub-commands: model-to-osm/idf/gbxml and reverse)
...
[CAPTURE_GAP]:
- the gate is ABI-real, not manifest-convenience (verified against `honeybee-openstudio 0.5.5` + its bound native `openstudio` SDK `3.10.0`): the honeybee-openstudio wheel is itself `py3-none-any`, but it hard-imports the native `openstudio` SDK at module load (through `honeybee_openstudio.openstudio`), and that SDK ships per-interpreter compiled binary wheels with no cp315 build. So this package cannot install on 3.15 even though its own code is pure Python — the floor `python_version<'3.15'` is enforced by the SDK ABI, distinguishing it from the pure-Python honeybee-core/energy/standards whose gating is manifest co-release. `model_to_openstudio` returns a real `openstudio.openstudiomodelcore.Model`, so any consumer of the return value also links the native SDK.
- in-process vs CLI is a real performance/capability fork: `honeybee_openstudio.writer.model_to_openstudio` translates in-process (no temp folder, no subprocess), whereas `honeybee_energy.run.run_osw` writes an OSW and shells out to the OpenStudio CLI. The in-process path is CPU-bound -> `anyio.to_thread.run_sync` (off-loop CPU), while the CLI path is subprocess-bound -> `anyio.to_thread.run_sync` + `stamina` retry. They produce the same OSM/IDF, but only the CLI path runs the actual EnergyPlus simulation; `model_to_openstudio` produces the model for simulation, it does not simulate. `DetailedHVAC` translation specifically requires this OpenStudio measure path.
