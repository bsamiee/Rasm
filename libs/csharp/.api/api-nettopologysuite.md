...
`NetTopologySuite` is the pure-managed JTS port supplying the OGC Simple-Features planar geometry algebra: the `Geometry` type hierarchy, the binary-predicate (DE-9IM) relate matrix, the robust overlay/boolean engine (`OverlayNG`), buffer/convex-hull/simplify/densify operations, the `STRtree` bulk-loaded R-tree spatial index with k-NN, `PreparedGeometry` for repeated-query acceleration, `GeometryFixer` validity repair, and the WKT/WKB readers/writers. It is cross-cutting float-plane geometry substrate the strata share at the discipline boundary — `Rasm.Bim` carries `Geometry`/`Feature` through the geospatial codecs and `MaxRev.Gdal.Core` OGR ingest, `Rasm.Persistence` composes the PostGIS/`Npgsql.NetTopologySuite` spatial column and the `STRtree` broad-phase, and `Rasm.Compute` runs the circulation planar side (isovist/visibility polygons, corridor medial-axis, occupant-area/offset) at the egress-discipline boundary. The kernel `Rasm` stays the EXACT-geometry owner; NTS is the float production-plane algebra, never a second exact rail.
...
- dependency: `NetTopologySuite.Features` >= (transitive; supplies `Feature`/`IFeature`/`AttributesTable`/`IAttributesTable` — the attributed-geometry shape the vector codecs read and write)
- rail: geometry
...
| :-----: | :------------------------------------------------ | :------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  [01]   | `Geometry`                                        | geometry | abstract root; `OgcGeometryType`, `Dimension`, `SRID`, `EnvelopeInternal`, `Coordinates`, `UserData`, `IsValid`/`IsSimple`/`IsEmpty`/`IsRectangle`, `Area`/`Length`, `Centroid`/`InteriorPoint`, `Apply(ICoordinateSequenceFilter)` (the in-place ordinate-edit visitor — `Coordinates` returns a detached `Coordinate[]`, so a reprojection writes back through this filter, not the array). Owns the predicate, boolean, and serialization methods enumerated in [03] |
...
|  [09]   | `CoordinateSequence`                              | geometry | packed ordinate store; `Count`, `Ordinates`/`HasZ`/`HasM`, `GetOrdinate`/`SetOrdinate`, `SetX(int, double)`/`SetY(int, double)`/`SetZ(int, double)` (the per-ordinate column writers an `ICoordinateSequenceFilter.Filter(sequence, index)` body calls) — the buffer a kernel mesh/curve maps onto without per-point `Coordinate` boxing                                                                                                                                |
|  [10]   | `ICoordinateSequenceFilter`                       | geometry | the coordinate-visitor contract `Geometry.Apply` drives; `void Filter(CoordinateSequence seq, int i)`, `bool Done` (short-circuit), `bool GeometryChanged` (fire `GeometryChanged()` after the walk) — the in-place ordinate rewrite (reprojection) seam since `Geometry.Coordinates` is a detached copy                                                                                                                                                                |
|  [11]   | `Envelope`                                        | geometry | axis-aligned 2D bbox; `MinX`/`MaxX`/`MinY`/`MaxY`, `Intersects`, `Contains`, `ExpandToInclude`, `Centre`. The `STRtree`/`Quadtree` key                                                                                                                                                                                                                                                                                                                                  |
|  [12]   | `OgcGeometryType`                                 | geometry | enum: `Point`, `LineString`, `Polygon`, `MultiPoint`, `MultiLineString`, `MultiPolygon`, `GeometryCollection`, plus the curve/surface OGC members — the discriminant a `[SmartEnum]`/match folds onto a domain footprint kind                                                                                                                                                                                                                                           |
|  [13]   | `PrecisionModel`                                  | geometry | coordinate rounding policy; `PrecisionModels.Floating`/`FloatingSingle`/`Fixed`, `Scale`, `MakePrecise` — the robustness knob `OverlayNG` and `GeometryFixer` honor                                                                                                                                                                                                                                                                                                     |
|  [14]   | `Dimension`                                       | geometry | enum: `Point`(0)/`Curve`(1)/`Surface`(2)/`False`/`True`/`DontCare` — the DE-9IM matrix cell vocabulary                                                                                                                                                                                                                                                                                                                                                                  |
...
|  [02]   | `GeometryFactory`           | geometry | builds geometries under a fixed `PrecisionModel`/`SRID`/`CoordinateSequenceFactory`; `CreatePoint`/`CreateLineString`/`CreateLinearRing`/`CreatePolygon`/`CreateMultiPoint(CoordinateSequence)`/`CreateMultiPointFromCoords(Coordinate[])`/`CreateMultiPolygon`/`CreateGeometryCollection`, `BuildGeometry(IEnumerable<Geometry>)` (dimension-folding constructor), `ToGeometry(Envelope)`, `CreateEmpty(Dimension)`. Static `Default`/`Floating`/`FloatingSingle`/`Fixed` presets |
|  [03]   | `GeometryOverlay`           | geometry | the boolean-engine strategy `NtsGeometryServices` injects; `NG` selects the robust `OverlayNG` engine, `Legacy` selects the classic overlay — `NG` is the admitted default                                                                                                                                                                                                                                                                                                         |
|  [04]   | `CoordinateSequenceFactory` | geometry | the ordinate-store strategy; `PackedCoordinateSequenceFactory` (struct-of-arrays `double[]`/`float[]`, the dense interop-friendly layout — `NetTopologySuite.Geometries.Implementation`, static singletons `PackedCoordinateSequenceFactory.DoubleFactory`/`.FloatFactory` are the canonical instances handed to `new NtsGeometryServices(...)`) vs `CoordinateArraySequenceFactory` (`Coordinate[]`)                                                                              |
...
|  [05]   | `GeometryFixer`           | geometry | makes an invalid geometry OGC-valid (self-intersection/ring-order repair); static `Fix(Geometry)` / `Fix(geom, isKeepMulti)`, instance `KeepCollapsed`/`KeepMulti` + `GetResult()` — the repair leg before an overlay or write rejects a degenerate ring                                                                                                   |
|  [06]   | `AffineTransformation`    | geometry | 2D affine matrix over geometry; `Translate`/`Scale`/`Rotate`/`Shear` composition, `Transform(Geometry)` — the planar transform distinct from the geodetic leg                                                                                                                                                                                              |
...
|  [04]   | `AttributesTable`  | geometry | dictionary-backed `IAttributesTable`; ctors from `Dictionary<string,object>`/`IEnumerable<KeyValuePair>`, `Add`/`DeleteAttribute`/`MergeWith`, `GetOptionalValue` — the carrier mapped to/from a domain `PropertySet` |
...
|  [04]   | `WKBReader.Read`                 | `(byte[])` / `(Stream)` → `Geometry`                                            | WKB parse; `HandleSRID`, `HandleOrdinates`, `RepairRings` knobs. `HexToBytes(string)` decodes a hex WKB column — the bridge from a PostGIS/OGR `ExportToWkb` buffer |
...
[PLATFORM_BOUNDARY]:
- ships a single managed `NetTopologySuite.dll` (netstandard2.1/ only); no P/Invoke, no native GEOS. The topology engine is a pure C# JTS port — this is the seam distinction from `MaxRev.Gdal.Core`, whose OGR side carries a bundled native `libgeos`. Two GEOS-equivalent engines exist in the closure; NTS owns the managed planar algebra and OGR's GEOS stays inside the GDAL native boundary.
...
- repeated predicate queries against one fixed geometry build the segment intersection tree once through `PreparedGeometryFactory.Prepare(geom)`, then call `IPreparedGeometry.Intersects`/`Contains`/... per candidate — the form for a point-in-polygon classification of many elements against one zone, or a clash narrow-phase after the broad-phase index.
- `STRtree<T>` is the broad-phase: bulk-`Insert(envelope, item)` every candidate, then `Query(searchEnvelope)` returns the overlap candidate set, or `NearestNeighbour(env, item, dist, k)` returns the k nearest. Read-only after the first query; a workflow that interleaves inserts and queries uses `Quadtree<T>`.
- the canonical spatial-join rail STACKS both: `STRtree.Query` for candidate envelopes, then `PreparedGeometry.Intersects` for the exact predicate — the 2D broad-then-narrow projection of the kernel BVH clash pattern.

[ROBUST_OVERLAY]:
- `GeometryOverlay.NG` (the `OverlayNG` engine) is the robust default: it nodes the input, snaps to the `PrecisionModel`, and avoids the topology exceptions the overlay selected by `Legacy` throws on near-degenerate input. `OverlayNGRobust.Union(IEnumerable<Geometry>)` is the parcel/footprint dissolve; `OverlayNGRobust.Overlay(a, b, op)` the direct two-geometry boolean.
- a geometry that fails `IsValid` is repaired through `GeometryFixer.Fix(geom)` BEFORE an overlay or write — the validity gate a geospatial codec runs so a malformed OGR/shapefile ring does not poison the boolean engine.
...
- `Rasm.Compute` circulation (`[V12]`): the `Analysis/circulation` egress runner resolves space boundaries as `Polygon`/`MultiPolygon` (isovist/visibility polygons via `Buffer`/predicate composition, corridor medial-axis, occupant-area computation over `Geometry.Area`), the planar boundaries arriving through the SAME `GeometrySource` content-key port every runner threads — the flow/topology algebra is `QuikGraph`/`Google.OrTools.Graph`'s, NTS the planar side. A space whose boundary cannot resolve is a typed `(Admission, Input)` failure, never a silently-skipped node.
- wire seam: `Geometry.ToBinary()`/`WKBWriter` is the compact form a domain footprint carries on the wire; the Python/TypeScript peers decode WKB through `shapely`/`turf`.
...
- Reject: geodetic datum/projection transformation (`ProjNET` owns it as default, `MaxRev.Gdal.Core` OSR as escalation), raster I/O and universal vector drivers (`MaxRev.Gdal.Core` owns them), the shapefile/GeoPackage/GeoJSON byte format (the IO codec packages own them), 3D solid geometry and meshing plus the EXACT predicate floor (the kernel `Rasm` owns them — NTS is the float production plane, never a second exact rail)
