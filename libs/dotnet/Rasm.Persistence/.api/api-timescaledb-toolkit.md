...
- `Query/backend#PROVISIONING`: its Series arm materialises `time_weight` beside `percentile_agg` in the continuous aggregate and `SeriesLane.Bucketed` projects `average`/`approx_percentile` over that state, so a cheap read and `SeriesLane.Weighted`'s raw-chunk fold answer ONE statistic; the `SeriesKind` row carries its `BackendPolicy` grain, the bucket both share, and each fold groups by series because `rollup` combines disjoint summaries alone.
