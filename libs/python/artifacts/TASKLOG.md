...
[TWO_D_MATRIX_ZXING]-[COMPLETE]: zxing-cpp landed as the 2D-matrix encoded-mark arm in the `figures/preview#PREVIEW` owner — `Symbology` gained `DATA_MATRIX`/`PDF417`/`AZTEC`/`MAXICODE` rows on the `_zxing` arm (zxing `BarcodeFormat` member resolved via `barcode_format_from_str`, `create_barcode(**ZXING_CREATE_KEYS).to_svg(add_quiet_zones=)`), `PreviewOp.Decode`/`_zxing_decode` over `read_barcodes` lands the round-trip op, and `.api/zxing-cpp.md` is authored and verified by direct import of the source-built 3.0.0 distribution on the cp315 floor; the central pin is un-gated (`zxing-cpp`, source-built from sdist on cp315 via the Parametric_Forge scientific toolchain like shapely/pyarrow). Four 3.0 facts recorded: MaxiCode is creatable (NOT decode-only), `str(BarcodeFormat)` is the separated display name (resolve via `barcode_format_from_str`, never re-parse `.name`), the `|` union operator is deprecated (`BarcodeFormat.All` is the all-formats default, no `BarcodeFormat.Any`), and the version is exposed via `importlib.metadata.version("zxing-cpp")` (the C++ extension has no `__version__`). The dependency-free `create_barcode -> to_svg` 2D-matrix `Mark` arm imports and resolves in-process on the cp315 core beside the segno/python-barcode mark arms; only the `Decode` op crosses the subprocess seam because `read_barcodes` opens its raster through the gated-band Pillow worker. The separate-2D-matrix-owner routing note is resolved in this one owner. The `SYMBOLOGY`/`ENCODED_MARK_OWNER` ideas' realization for the 2D-matrix half.
