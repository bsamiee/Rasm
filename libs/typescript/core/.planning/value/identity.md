...
- Growth: a new identity dimension (shard, cloud provider) is one field plus its interior brand or vocabulary row, with its `observe/convention#IDENTITY_PROJECTION` line landing in the same edit; a new derived surface is one consumer-side projection over the value.
...
- Law: the decode boundary owns admission — the interchange codec decodes the contract tenancy into this class at its field record; interior construction rides `AppIdentity.scoped` or `new TenantContext` over already-branded parts — one name, one owning side, both paths landing the identical value.
