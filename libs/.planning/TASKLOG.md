...
- Considerations: the evidence is keyed by the one `ContentIdentity` seed (so this seam trusts the content-identity parity precondition); the surrogate fit crosses as a content-keyed ONNX artifact the C# side runs inference over, never an in-process training loop; Python holds the `HandoffAxis` rail singular so the C# owner consumes one rail, never a handoff family. The C# determinism consumer is incomplete downstream pending the Vectors adjoint coefficients (`T-DDG-ADJOINT-ROWS`) — the `Rasm.Compute` `DesignProblem.OperatorRows` are statically empty until those land.
...
- Producers `csharp:Rasm.Compute/interchange/codecs#CONTENT_ADDRESSING` + `csharp:Rasm/Geometry/topology/reconciliation#CANONICAL_BYTE_IDENTITY` to consumers `python:runtime/identity/content-identity#IDENTITY`, `typescript:interchange/artifacts/frame-reassembly#CONTENT_HASHING`, `typescript:testing/`, and the C# shared test corpus.
- Considerations: the missing parent of `CONTENT_IDENTITY_PARITY` + `FAULT_WIRE_ROUNDTRIP` + `TRI_LANGUAGE_WIRE_PARITY`; keyed by the one `XxHash128` seed and frozen once so every harness reads it rather than re-deriving its own fixtures. Seeds the C# `[BRANCH_TEST_NODE_PROVISIONING]`, Python `[PYTHON_TEST_INFRA_FLOOR]`, and TS `testing/` infra tasks.
...
- Producer `csharp:Rasm.AppUi/viewport#TS_PROJECTION` (mints the residency manifest) to consumers `typescript:worker/` and `typescript:ui/viewport`, binding the Python splat-decode companion (SPZ/SOG).
- Considerations: the one owner is the AppUi projection (absent it the manifest is minted twice, AppUi + the TS worker interior), so the single-hash-mint invariant holds at `worker/`. GATED on `ONE_CONTENT_IDENTITY` parity, the TS `worker/` top-level, and the C# `SPLAT_REALITY_CAPTURE` frontier.
