...
- `OpenTelemetry`(`libs/dotnet/.api/api-opentelemetry.md`): one `TracerProviderBuilder.AddProcessor(BaseProcessor<Activity>)` row seats the processor ahead of every exporting processor so the tag exists at export; the `AddProcessor<T>()` overload resolves through `GetRequiredService<T>` and binds only where the type is also a service registration.
