...
| :-----: | :----------------------- | :---------------------------------- | :--------------------- |
|  [01]   | `vegalite_to_svg`        | spec plus render policy -> `str`    | render to SVG          |
|  [02]   | `vegalite_to_png`        | spec plus raster policy -> `bytes`  | render to PNG          |
|  [03]   | `vegalite_to_jpeg`       | spec plus quality policy -> `bytes` | render to JPEG         |
|  [04]   | `vegalite_to_pdf`        | spec plus render policy -> `bytes`  | render to PDF          |
|  [05]   | `vegalite_to_html`       | spec plus bundle policy -> `str`    | render to HTML         |
|  [06]   | `vegalite_to_vega`       | spec plus compile policy -> `dict`  | compile to a Vega spec |
|  [07]   | `vegalite_to_scenegraph` | spec plus render policy -> `dict`   | render to scenegraph   |
|  [08]   | `vegalite_to_url`        | spec plus fullscreen flag -> `str`  | editor URL             |
...
| [INDEX] | [SURFACE]                 | [CALL_SHAPE]                       | [CAPABILITY]                 |
| :-----: | :------------------------ | :--------------------------------- | :--------------------------- |
|  [01]   | `vega_to_svg`             | spec plus render policy -> `str`   | render a Vega spec to SVG    |
|  [02]   | `vega_to_png`             | spec plus raster policy -> `bytes` | render a Vega spec to PNG    |
|  [03]   | `vega_to_pdf`             | spec plus render policy -> `bytes` | render a Vega spec to PDF    |
|  [04]   | `register_font_directory` | font directory path                | register custom fonts        |
|  [05]   | `get_themes`              | no-arg query -> `dict`             | available named themes       |
|  [06]   | `get_vegalite_versions`   | no-arg query -> `list[str]`        | supported Vega-Lite versions |
|  [07]   | `javascript_bundle`       | snippet plus Vega-Lite version     | bundle JS for embedding      |
