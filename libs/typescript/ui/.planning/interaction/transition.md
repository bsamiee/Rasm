...
- [1]-[VIEW_TRANSITIONS]: the `SurfaceTransition` `Data.TaggedEnum` over `swap` and `preserve` lifecycle modes, and the `mountSurface` total `$match` fold mounting `<ViewTransition>` or `<Activity>`.
