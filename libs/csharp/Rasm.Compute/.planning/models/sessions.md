...
- [CANCELLATION]: the `RunOptions.Terminate` one-way latch aborts `Run`/`RunWithBinding` with the native `OnnxRuntimeException` `[ErrorCode:Fail] Exiting due to terminate flag being set to true`, which the `RunOps.Faulted` arm classifies by scope provenance into `ComputeFault.Cancelled`/`DeadlineExpired`, and the generative `Stream` polls `token.IsCancellationRequested` through the `Collect` rail; the open leaf is the latch-propagation latency and the deadline-poll cadence on the CoreML and CPU rows inside the live plugin ALC.
