...
| [03] | `Proxy.enter()` / `Proxy.exit(*args)` | none | sync context-manager protocol; `exit` runs ownership-aware teardown — `stop_server()` only when `_implicitely_started_server` (this proxy spawned it), otherwise it disconnects from a reused server |
...
| [05] | `compas.rpc.RPCServerError` / `RPCClientError` | exception | the bring-up/remote-dispatch fault surface (exported from `compas.rpc.all`); `start_server()` raises `RPCServerError("The server is not available.")` on a cold-start timeout, `_proxy` raises on a remote fault |
