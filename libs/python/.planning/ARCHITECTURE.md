...
- S1 `artifacts` — composes runtime alone; geometry scene facts cross one-way as GLB bytes through `SceneGrid.of_glb`.
- S1 artifacts data wires — `artifacts` lands `CorpusRow` and `GeoJSON` on data's planes and reads back `QualityProfile`, never an import.
...
flowchart TB
    ...
    accDescr: Package imports descend to the runtime foundation; dashed edges carry wire and counter-edge values.
    subgraph S2["S2 COMPUTE + GEOMETRY"]
        Compute[compute]
        Geometry[geometry]
    end
    subgraph S1["S1 DATA + ARTIFACTS"]
        Artifacts[artifacts]
        Data[data]
    end
    ...
    Runtime f1@-->|"forbidden: upward import"| S2
...
flowchart LR
    ...
    accDescr: Compute output crosses one handoff axis, reverse evidence returns to the same owner, and geometry remains on its canonical results.
    Geometry[geometry results] e1@-->|"runtime observation"| Runtime[runtime]
    Producers[compute producers] e2@-->|"Graduation"| Hub[compute graduation hub]
...
flowchart LR
    ...
    accDescr: Package results project into runtime observability while durable facts enter the Ledger-bound journal.
    Results[package operations · canonical results]
