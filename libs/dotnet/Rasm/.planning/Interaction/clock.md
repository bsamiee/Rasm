...
- Entry: `Of(cadence, beat, posture, faults, clock)` mints; `Start`/`Stop`/`Pause`/`Resume` are the lifecycle; `Tap` leases a beat observer; `Failures` reads the cell's parked refusals and `Shed` the parks the bound dropped.
