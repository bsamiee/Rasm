...
[STACKING]:
- `api-npgsql`(`libs/csharp/.api/api-npgsql.md`): AGE ships no managed or EF driver, so every Cypher statement rides raw `Npgsql` — `NpgsqlDataSource`/`NpgsqlCommand` execute `cypher(…)`, `agtype` columns read through `FromSql`/`SqlQuery`, and `NpgsqlDataSourceBuilder.UsePhysicalConnectionInitializer` installs the per-connection `LOAD 'age'`+`search_path` hook once per physical connection.
