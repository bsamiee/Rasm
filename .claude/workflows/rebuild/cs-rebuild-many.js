...
const ALL_FOLDERS = [
  { name: 'Persistence', planning: 'libs/csharp/Rasm.Persistence/.planning', api: 'libs/csharp/Rasm.Persistence/.api', root: 'libs/csharp/Rasm.Persistence', note: '' },
  { name: 'AppHost', planning: 'libs/csharp/Rasm.AppHost/.planning', api: 'libs/csharp/Rasm.AppHost/.api', root: 'libs/csharp/Rasm.AppHost', note: '' },
  { name: 'AppUi', planning: 'libs/csharp/Rasm.AppUi/.planning', api: 'libs/csharp/Rasm.AppUi/.api', root: 'libs/csharp/Rasm.AppUi', note: '' },
  { name: 'Bim', planning: 'libs/csharp/Rasm.Bim/.planning', api: 'libs/csharp/Rasm.Bim/.api', root: 'libs/csharp/Rasm.Bim', note: '' },
  { name: 'Fabrication', planning: 'libs/csharp/Rasm.Fabrication/.planning', api: 'libs/csharp/Rasm.Fabrication/.api', root: 'libs/csharp/Rasm.Fabrication', note: '' },
  { name: 'Compute', planning: 'libs/csharp/Rasm.Compute/.planning', api: 'libs/csharp/Rasm.Compute/.api', root: 'libs/csharp/Rasm.Compute', note: '' },
  { name: 'Materials', planning: 'libs/csharp/Rasm.Materials/.planning', api: 'libs/csharp/Rasm.Materials/.api', root: 'libs/csharp/Rasm.Materials', note: '' },
  { name: 'Geometry', planning: 'libs/csharp/Rasm/Geometry/.planning', api: 'libs/csharp/Rasm/.api', root: 'libs/csharp/Rasm', note: 'SPECIAL ' +
    'LAYOUT: the geometry design pages live under libs/csharp/Rasm/Geometry/.planning, but the governing ' +
    'ARCHITECTURE.md/README.md/IDEAS.md/TASKLOG.md and the .api catalogs live at the libs/csharp/Rasm ROOT (one level up). Read the Rasm-root docs ' +
    '+ .api as the governing context and shared capability tier; author NEW pages only under Rasm/Geometry/.planning; NEVER touch the mature ' +
    'siblings Analysis/Domain/Vectors.' },
]
...
const wanted = Array.isArray(args) ? args.filter(Boolean).map(norm)
  ...
  : null
const FOLDERS = wanted ? ALL_FOLDERS.filter((f) => wanted.some((w) => w === f.name || w === 'Rasm.' + f.name || (f.name === 'Geometry' && (w === 'Rasm' || w === 'Rasm/Geometry')))) : ALL_FOLDERS
...
const LAW = [
  'Rasm monorepo, libs/csharp planning corpus (markdown specs of intended C# package designs). CLAUDE.md manifest + WORKSPACE_LAW strata govern ' +
    ...
    'consumes the contract). This pass works ONE FOLDER end-to-end: the whole folder design corpus under its `.planning/**` plus its `.api/` ' +
    'capability tier, NOT a single page. C# PLANNING-HOMING: for the `Geometry` target the design pages live at ' +
    '`libs/csharp/Rasm/Geometry/.planning/**` while the governing `ARCHITECTURE.md`/`README.md`/`IDEAS.md`/`TASKLOG.md` and `.api/` catalogs live ' +
    'at the `libs/csharp/Rasm/` package ROOT; read those root docs + `.api/` as governing context and never trample the mature siblings ' +
  ...
  'TWO JOBS, ONE PASS: (1) INTEGRATE every newly-admitted package — each folder `.api/api-*.md` catalog that a recent survey added and a focused ' +
    ...
    'folder pages to the ULTRA bar (collapse surfaces/types, deepen bleeding-edge spellings, maximize admitted-library capability, close concept ' +
    'capability gaps). Both are untied to any card: improve the corpus objectively wherever the doctrine admits a stronger form.',
...
const INTEGRATE = [
  'NEW-CAPABILITY INTEGRATION (the distinguishing mandate of this pass): for EVERY package newly admitted to this folder (read its filled ' +
    '`.api/api-*.md` catalog to full depth + verify members via `uv run python -m tools.assay api`), the capability MUST land in the design corpus ' +
    'as COMPOSED design, never a dangling catalog. PLACEMENT, most-specific wins: (a) GROW AN EXISTING OWNER — when an existing design page ' +
    'already owns the concept the package serves, weave the package capability INTO that owner in place (a case in the closed family, a row/richer ' +
    'data on the smart-enum, a field or composed `[ValueObject]`/`[ComplexValueObject]`, an operation on the existing surface, a policy value), ' +
    'reshaped per ROOT_REBUILD + COMPOSED_IMPLEMENTATION as if it had always carried it — NEVER a parallel surface beside the owner; (b) AUTHOR A ' +
    'NEW PAGE — ONLY when the package introduces a genuinely NEW owner/concern that no existing page owns, author a new design page (and a new ' +
    ...
    'and the docs/stacks/csharp bar, then UPDATE the folder `ARCHITECTURE.md` codemap + `[02]-[SEAMS]` and the `README.md` router/package roster ' +
...
const ADVERSARIAL = [
  'ADVERSARIAL STANCE — EVERY stage (implement, critique, AND red-team) is HOSTILE: assume the existing fences are NAIVE, SHALLOW, JUNIOR, or ' +
    ...
    'enough", "done", "polished", and a prior `clean` verdict are REJECTED self-assessments — MOST of this corpus is naive, surface-level code ' +
    'dressed in the right vocabulary, and it is NOT tolerable. Default to "this fence is naive and must be rebuilt to the strongest form the ' +
    'doctrine admits" and MAKE that rebuild; a no-edit verdict is reached ONLY after a genuinely aggressive attack on the real domain + the ' +
    'verified package surface finds nothing — never a first-read concession, never to avoid work. Reject "good enough" categorically.',
  'ILLUSORY / FAKE CODE is the PRIMARY target — the MOST dangerous code PRETENDS to be advanced: it uses the doctrine vocabulary ' +
    ...
    'domain and the catalogued package surface. HUNT: a name/signature/prose PROMISING capability the body does not implement; a "rich" owner that ' +
    'is a thin slice of its concept (a 2-case union for a 20-case domain; the obvious 3 fields where the concept carries fifteen); decorative ' +
    'density carrying no real capability; a stub/sketch dressed as a finished design; a newly-admitted `.api` cited in prose but never actually ' +
    'composed into a fence; a phantom member cited but unverifiable. Every illusion is a DEFECT to rebuild.',
].join('\n')
const ULTRA = [
  ...
  'STACK CAPABILITY: FIRST mine the package `.api/*.md` catalogs (the curated, integration-shaped capability surface at ' +
    '`libs/csharp/<Package>/.api/`; for the `Geometry` effort they live at `libs/csharp/Rasm/.api/`) AND the universal Thinktecture / LanguageExt ' +
    'rails — C# has NO central `.api/` tier, so the universals are Thinktecture (generated domain shape) / LanguageExt (rails, effects, schedules, ' +
    'immutable collections) plus full docs/stacks/csharp doctrine, with MathNet / CSparse owning numeric algorithms. Compose EVERY relevant host ' +
    'API + admitted NuGet package + catalog member into single dense owners woven as ONE rail (source-generated owners, `Fold` algebra, data ' +
    'tables), ALWAYS layering the universal Thinktecture/LanguageExt rails onto the domain packages, NOT flat one-shot per-API uses. Use the ' +
    'DEEPEST operator/combinator/generated surface each package itself reaches (LIBRARY_DEPTH); reject surface-level subsets, BCL-first reflexes, ' +
...
const EXTEND = [
  'CAPABILITY EXTENSION (justified, in-place, never flat spam) — structural collapse and `.api`-stacking are NECESSARY but NOT SUFFICIENT. A page ' +
    'can be fully collapsed into one polymorphic owner and STILL be capability-thin: modeling a NAIVE slice of its domain concept. Structural ' +
    ...
    'INSIDE existing owners), every real missing concern lands as a CASE in the existing closed family, a ROW or richer data on the existing ' +
    'smart-enum, a FIELD or a composed `[ValueObject]`/`[ComplexValueObject]` on the existing record, an OPERATION on the existing surface, or a ' +
    'POLICY_VALUE — reshaping the owner per ROOT_REBUILD; NEVER a parallel type or flat appended code.',
  'GAP SOURCES (every extension MUST cite exactly one): (a) PACKAGE — a member the admitted package/host surface exposes that the concept ADMITS ' +
    'but the page IGNORES (LIBRARY_DEPTH; verify via `uv run python -m tools.assay api`) — this is the PRIMARY source for the newly-admitted .api ' +
    'packages this pass integrates; (b) DOMAIN — an attribute, metric, sub-kind, relationship, state, or operation the REAL concept demands but ' +
    'the page omits; (c) CONSUMER — a contract a sibling or downstream owner will require that has no composed spelling here yet.',
  'JUSTIFIED, NOT RANDOM: if after a real domain + package + consumer sweep the concept is genuinely complete, prove it by adding nothing — never ' +
    ...
    'attribute / consumer contract, and composes the existing rails; preserve ALL existing capability.',
].join('\n')
const PATLAW = [
  'C# PATTERN LAW: model the domain precisely — NEVER weak/unbounded/erased types where the language can express the domain; NEVER exception ' +
    'control flow in domain logic (use the LanguageExt typed rails / ROP); NEVER imperative branching where a bounded vocabulary, frozen table, ' +
    'generated `Switch`, match, or `Fold` owns the variation; NEVER mutable accumulation for domain transforms. Total generated `Switch` with ' +
    'compile-time exhaustiveness (a new case breaks every dispatch site — NEVER a runtime-silent `_` arm). Typed algorithm receipts (NEVER a ' +
    'generic `IReceipt`/ledger) when fields carry route/status/sampling/solver/spectral/mesh/extraction/benchmark/host evidence. The fault type is ' +
    'a CLOSED `[Union]` family deriving from `Expected`.',
  'Latest stable C# 14 on `net10.0` to the metal (`Nullable enable`, NRT enforced): primary constructors, collection expressions with spread, ' +
    ...
    'where they fit. Apply the docs/stacks/csharp file-organization and section-order law (`[Union]`/`[SmartEnum]`/`[ValueObject]` and generated ' +
    'case families stay inside the declaring owner block; canonical section order TYPES -> CONSTANTS -> MODELS -> ERRORS -> SERVICES -> OPERATIONS ' +
    '-> COMPOSITION -> EXPORTS).',
  'Keep conventions IDENTICAL across every package; place each package on its canonical stratum and depend strictly upward; geometry/mesh/IFC meet ' +
    ...
    '`Get`/`GetMany`/`GetBy<Key>`/`List`/`Search` names; ONE_HOP_RESOLUTION.',
].join('\n')
const BOUNDARIES = 'BOUNDARY LAW: keep every package owner strictly in its lane and on its stratum; geometry/mesh/IFC meet at the wire with one ' +
  ...
  'densifying; never introduce a downward dependency or leak a host type into a host-neutral owner. This pass realizes ONLY the current folder ' +
  'pages; a concern owned twice across a runtime, a folder mixing unrelated concerns, or coupling to a sibling owner INTERIOR (vs its seam/wire) ' +
...
const COMMENTS = 'COMMENT HYGIENE: code fences are agent-facing — comment for the next agent, never as a tutorial. KEEP the canonical ' +
  ...
  'comment genuinely earns its place; 1-2 lines only for a truly subtle invariant. NO restating the code, no narration, no ' +
  'task/process/session/history/proof/review comments, no XML-doc bloat.'
...
const FINAL = [
  'WHOLE-STACK CROSS-FOLDER ALIGNMENT — this is the FINAL pass, after every folder ran its own per-folder implement -> critique -> redteam and the ' +
    'residual reconcile. Hold ALL these C# AEC folders in view at once and align them as ONE coherent stack: ' + ROSTER + '. Read each folder ' +
    'ARCHITECTURE.md ([1]-[DOMAIN_MAP] + [2]-[SEAMS]) and README, the central manifest, and every design page a cross-folder seam spans. This is ' +
    'the ONLY pass that edits ACROSS folders — but it aligns at the seam and never trespasses a folder owner interior.',
  'ALIGN OBJECTIVES (fix every one in place across the spanned folders): (1) SEAMS/WIRES — every cross-folder producer/consumer relationship is ' +
    ...
    'where a per-folder pass created the need but could not reach the sibling. (3) NO DUPLICATION — a concept owned twice across the runtime ' +
    'collapses to the ONE rightful stratum owner; the sibling consumes the seam, never a second mint. (4) NO STRATA VIOLATION — depend strictly ' +
    'upward (KERNEL -> AEC-DOMAIN -> APP-PLATFORM -> HOST-BOUNDARY -> APP); no downward dependency, no host-type leak into a host-neutral owner, ' +
    'geometry/mesh/IFC meet at the wire with one owner per runtime. (5) GEOMETRY-KERNEL LEVERAGE — every folder that depends on the Rasm geometry ' +
    'KERNEL FULLY and PROPERLY leverages it (its geometry types, predicates, exact-precision numerics, and operations) for BOTH existing and new ' +
    'functionality, NEVER re-implementing a kernel capability; a consumer hand-rolling what the kernel owns is a defect to rewire to the kernel. ' +
    '(6) GAPS/OVERSIGHTS — hunt the mistakes no single-folder pass could see from inside one folder, and fix them.',
].join('\n')
...
const implementPrompt = (f) => [DOCTRINE, '',
  ...
  'Read EVERY design page under `' + f.planning + '/**`, the folder `.api/api-*.md` catalogs (especially the newly-admitted ones a recent survey ' +
    'added and rebuild-api filled), the governing `ARCHITECTURE.md` + `README.md`, the docs/stacks/csharp/ core + the relevant domain/ shard(s) ' +
    'for each concern, and the universal Thinktecture/LanguageExt rails; VERIFY every cited host/NuGet member via `uv run python -m tools.assay ' +
    'api` (a member you cannot verify is a phantom — delete it). Then, across the WHOLE folder corpus: (1) INTEGRATE every newly-admitted package ' +
    'capability into the design — GROW the existing owner page where one fits (a case/row/field/operation/policy-value per ROOT_REBUILD + ' +
    'COMPOSED_IMPLEMENTATION), or AUTHOR a NEW page (and sub-domain folder) ONLY where the package is a genuinely new owner with no existing home, ' +
    'built ground-up to the .planning page grammar + the docs/stacks/csharp bar, and UPDATE `ARCHITECTURE.md` codemap + `[02]-[SEAMS]` + ' +
    '`README.md`; (2) HOSTILE GROUND-UP REBUILD of the folder pages to the ULTRA bar — construct in LIFECYCLE order (admit raw ONCE -> canonical ' +
    'owner via OWNER_CHOOSER -> two-weave aspects over a thin pure core -> ONE unified `Fin`/`Validation`/`Option`/`Eff` rail with total generated ' +
    '`Switch` -> project + egress, both ingress and egress parameterized), collapse >=3 parallel shapes into one ' +
    '`[Union]`/`[SmartEnum<TKey>]`/`[ValueObject<T>]`/`[ComplexValueObject]`/source-generated case family IN THE SAME page, drive cases with a ' +
    '`Fold` algebra or frozen table, and CLOSE the concept capability gaps in place (each extension citing a package member / domain attribute / ' +
    'consumer contract). Realize ONLY `' + f.name + '` pages; never edit a sibling folder page. Modern C# 14 / net10 to the metal, high-signal ' +
    'prose all-backticked, comment hygiene, fix-in-place. Return verdict + integrated (each new .api woven in, and where) + newPages (any new page ' +
...
const critiquePrompt = (f) => [DOCTRINE, '',
  'TASK: HOSTILE DOCTRINAL-CONFORMANCE AUDIT + CAPABILITY-COMPLETENESS + INTEGRATION AUDIT + FIX IN PLACE across `' + f.name + '`. ' + scopeLine(f) + ' ' +
    ...
    'or the prose claims, and "good enough"/"mature" is rejected outright. Read the folder pages, the `.api/` catalogs, the operative ' +
    'docs/stacks/csharp/ pages AND the relevant domain/ shard(s), and the universal Thinktecture/LanguageExt rails. Run these MECHANICAL ' +
    'checklists line-by-line and REPAIR every hit in place (a fix, never a ledger note):',
  '(1) COLLAPSE_SCAN — sibling prefix/suffix names -> one modality-polymorphic entrypoint; same return rail differing only by arity -> input-shape ' +
    'discrimination; functions differing only by a literal -> a POLICY_VALUE; a `bool`/`mode`/`batch` parameter selecting two bodies -> one ' +
    'derived body or policy value; a method calling exactly one other -> delete the hop; parallel dispatch arms repeating structure -> a `Fold` ' +
    'algebra or frozen table; several types sharing fields for one concept -> one closed family; a `Get`/`GetMany`/`GetBy<Key>`/`List`/`Search` ' +
    'family -> one input-keyed polymorphic operation; a wrapper renaming a package API -> the package surface directly; 3+ parallel types / ' +
    'sibling factories / repeated switch arms / single-call helpers -> ONE `[Union]` / `[SmartEnum<TKey>]` / `[ValueObject<T>]` / ' +
    '`[ComplexValueObject]` / source-generated case family IN THE SAME page.',
  '(2) OWNER_CHOOSER — re-derive each owner from the 5 discriminants (admission, identity regime, variant arity, payload timing, openness), ' +
    'most-specific wins: invariant-bearing scalar -> `[ValueObject<TKey>]`; N-field one-concept product no discriminator -> ' +
  ...
  '(3) KNOB_TEST — delete each parameter; if the value reconstructs what it carried it was a knob -> collapse a `bool`/`mode`/`strict`/`batch` ' +
  ...
  '(4) ASPECTS — definition-time concerns (admission, identity, dispatch, serialization, grammar, logging) attach via attribute-directed SOURCE ' +
    'GENERATION; composition-time concerns attach as effect transformers — retry as `Schedule`-driven `IO<T>.Retry(Schedule)`/`Prelude.retry`, ' +
    'recovery as named catch combinators (`@catch`/`catchOf`/`CatchM` composed via `|`), resource lifetime as `Bracket`/`BracketIO`/`Finally`; the ' +
    'two weaves meet at EXACTLY ONE seam, the admission rail bridge. 2-4 co-occurring wrappers collapse into one aspect; an aspect NEVER raises ' +
    'into domain flow.',
  '(5) RAILS — RAIL_CHOOSER, the narrowest carrier chosen ONCE at admission: `Option<T>` absence, `Fin<T>` synchronous fallibility, ' +
    ...
    'policy; the fault type is a CLOSED `[Union]` family deriving from `Expected` (recovery identity via `Is`/`HasCode`/`IsType<E>`, never `==`); ' +
    'accumulate-vs-abort disposition correct (`Apply`/`&`/`.Traverse` for independents, `Bind`/`.TraverseM`/query for dependents); total generated ' +
    '`Switch` (NO `_` arm hiding a case); `.Fold`/`.Traverse`/`.Choose` with the mandatory `.As()` re-anchor; NO exception control flow in domain ' +
  ...
  '(6) STRATA/MEMBERS/MODERN — strata correctness (depend strictly upward; NO downward dependency, NO host-type leak into a host-neutral owner; ' +
    'geometry/mesh/IFC meet at the wire with one owner per runtime); cite ONLY host/NuGet members confirmed in the `.api/` catalog (verify novel ' +
    'members via `uv run python -m tools.assay api`); latest modern C# 14 on net10; FULL docs/stacks/csharp + the relevant domain/ shard ' +
    'conformance; BOTH the `.api/` catalogs AND the universal Thinktecture/LanguageExt rails maximized; the `tools/cs-analyzer` doctrine-gate clean.',
...
const redteamPrompt = (f) => [DOCTRINE, '',
  'TASK: ADVERSARIAL ARCHITECT RED-TEAM + FIX IN PLACE across `' + f.name + '`. ' + scopeLine(f) + ' You are the LAST and MOST AGGRESSIVE pass: ' +
    ...
    'proof ON THE DESIGN; trust nothing the prior passes or the prose claimed. Open the `.api/` catalogs and the universal ' +
    'Thinktecture/LanguageExt rails, the sibling pages, the operative docs/stacks/csharp/ pages, and the relevant domain/ shard. Attack from every ' +
    'direction and REPAIR every defect in place — no soft-pedalling, a fix never a ledger.',
  'PRIMARY LENS: (A) COUNTERFACTUAL on each core choice — is the owner, the algebra (`Fold`/generated `Switch`/data table), and the dispatch form ' +
    'categorically the strongest the doctrine admits, or does a denser owner or a DEEPER admitted-package primitive ' +
    '(LanguageExt/Thinktecture/MathNet/CSparse) collapse the whole fence? Rebuild to the stronger form; never defend the incumbent. (B) ' +
    'ANTICIPATORY_COLLAPSE — compute the DIFF OF THE NEXT FEATURE: the next case/dimension/knob/modality/provider lands as ONE case/row/policy ' +
    'value with every consumer untouched or broken LOUDLY at compile time (total generated `Switch`, no silent `_`)? If it would touch multiple ' +
    'sites, reshape the growth axis. (C) LONG-TAIL + MULTI-DIMENSIONAL — attack every input/output/edge/failure mode (empty, singular, plural, ' +
    'stream, malformed, concurrent, cancelled, partial-failure, version-skew); accumulate-vs-abort correct for the REAL boundary; BOTH ingress AND ' +
    'egress parameterized. (D) STRATA + BOUNDARY-INTEGRITY — a downward dependency, a host-type leak, a concern owned twice in a runtime, a folder ' +
    'mixing concerns, geometry/mesh/IFC not meeting at ONE wire owner per runtime, coupling to a sibling owner INTERIOR, OR a sibling page left ' +
    'STALE by this folder change (seam drift) is a defect: fix in-folder or log as a cross-folder residual. (E) SURFACE-SPRAWL-IN-TIME + PHANTOMS ' +
    '— an admitted package whose `.api` or the universal rails expose capability the fence re-derives by hand, flat code below the operator depth ' +
    'the packages reach, a phantom member, or a thin wrapper: collapse to package depth and verify the member (via `assay api`). (F) INTEGRATION + ' +
    'CAPABILITY-COMPLETENESS — counterfactually attack each owner for domain-completeness AND verify every newly-admitted .api capability is ' +
    'genuinely composed into a fence; an UNJUSTIFIED new page (fragmenting an existing owner) is folded back, a MISSING owner (a real new concern ' +
    'with no page) is authored.',
  'ALSO — FULL COLD ADVERSARIAL RE-REVIEW (every time): re-attack every conformance dimension with fresh hostile eyes — the COLLAPSE_SCAN signals, ' +
    'OWNER_CHOOSER per shape, the KNOB_TEST per param, the two-weave ASPECT taxonomy, rail + closed-`Expected`-fault discipline, integration + ' +
    'capability-completeness per owner, strata correctness, modern-C# 14 typing, docs/stacks/csharp + domain-shard conformance, `.api` + ' +
    'Thinktecture/LanguageExt maximization, the `tools/cs-analyzer` doctrine-gate, and prose/comment hygiene — and fix every defect. The folder ' +
    'must end objectively denser, MORE CAPABLE, more correct than the critique left it; if the strongest form is genuinely already present, prove ' +
    'it by finding nothing — never invent churn. Realize ONLY `' + f.name + '` pages; log cross-FOLDER items as residual_high. Return verdict + ' +
    'integrated + newPages + collapsed + extended + residual_high + summary.'].join('\n')
const STAGES = [
...
const finalAlignPrompt = () => [DOCTRINE, '', FINAL, '', 'TASK: WHOLE-STACK ALIGN — walk every cross-folder seam/wire/port/boundary across all ' +
  ...
  'ports/boundaries the per-folder passes surfaced, collapse any concept owned twice to its rightful stratum owner (the sibling consumes the ' +
  'seam), fix any strata violation or host-type leak, and rewire any consumer that hand-rolls a Geometry-kernel capability to leverage the kernel ' +
  'instead. Read the folders ARCHITECTURE + README + central manifest + the spanned design pages; verify any host/NuGet member via `uv run python ' +
  '-m tools.assay api`. Fix every defect in place across the spanned folders, regressing none. Return verdict + aligned (each alignment made, ' +
  'naming the folders + the seam/port) + residual (each {files, claim} for anything you could not fully resolve) + summary.'].join('\n')
const finalCritiquePrompt = () => [DOCTRINE, '', FINAL, '', 'TASK: ADVERSARIAL AUDIT of the whole-stack alignment + FIX IN PLACE. Assume a ' +
  ...
  'folders: a one-sided or stale seam, a missing mirror glyph, a concept still owned twice, a residual strata violation or downward dependency or ' +
  'host-type leak, a consumer still hand-rolling a Geometry-kernel capability, a new port the align pass missed, a gap between two folders. Repair ' +
  'every hit in place across the spanned folders. Return verdict + aligned + residual + summary.'].join('\n')
const finalRedteamPrompt = () => [DOCTRINE, '', FINAL, '', 'TASK: ADVERSARIAL RED-TEAM of the whole-stack alignment — the LAST and most aggressive ' +
  ...
  'RIGHT stratum owner; is each seam the strongest shared shape; does any consumer still under-leverage the Geometry kernel or duplicate a ' +
  'sibling; is any strata edge, boundary, or wire still wrong; will the next cross-folder growth axis (a new shared case/port/provider) land ' +
  'cleanly without touching multiple interiors? Fix every defect in place across the spanned folders; if the stack is genuinely coherent, prove it ' +
  'by finding nothing — never invent churn. Return verdict + aligned + residual + summary.'].join('\n')
const ALIGN_STAGES = [
...
if (clusters.length) {
  ...
  reconciled = (await pool(clusters, CAP, async (cl, i) => {
    const fix = await agent([LAW, '', INTEGRATE, '', ADVERSARIAL, '', ULTRA, '', EXTEND, '', PATLAW, '', BOUNDARIES, '', 'TASK: RECONCILE these ' +
      ...
      'strata/boundary issue, align a stale sibling page, or extend the shared owner in place to close a capability gap that spans folders), ' +
      'preserving all capability and regressing no file; if a residual is FACTUALLY INCORRECT, leave it and say why — never silently skip a real ' +
