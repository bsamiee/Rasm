...
- Packages: `libs/dotnet/Rasm.Rhino/.api/api-rhino-ui.md` (panel visibility semantics); LanguageExt.Core (`Fin`, `Option`, `Seq`); `Rasm/Domain` (`HookBinding`); `Rasm.Rhino/Document` (`MountRegistry`, `RhinoPoint`, `DocumentStream`, `Observation`, `EventScope`, `EventFamily`, `EventPayload`, `Delivery`, `StreamPolicy`, `Subscription`, `PluginKey`).
