...
- precision boundary: precision is fixed at two doubles, so `ddouble` deterministically refines `double` but cannot stand in for the sign-exact `Dyadic` verdict; a result needing more than ~106 bits escalates to context-free `EFloat` arithmetic, where the runtime ladder ends.
- precision refinement: the 106-bit stage recomputes the determinant with `ddouble` arithmetic and discards the value after its filter verdict; an unresolved sign advances to the independent exact `Dyadic` carrier rather than exposing `ddouble` as a result type.

[STACKING]:
- within-lib `Dyadic` (`Numerics/predicates.md`): `ddouble` sits above the `double` filter and below the context-free `EFloat` exact branch, the terminal tier of the runtime ladder; only residue unresolved at 106-bit promotes to `Dyadic.Sign`.
...
[LOCAL_ADMISSION]:
- `ddouble` is the middle predicate tier between the interior `double` filter and the sign-exact `Dyadic` terminal tier: the predicate recomputes the determinant in `ddouble` when the `double` error bound brackets zero, and only residue unresolved at 106-bit advances to exact context-free `EFloat` arithmetic.
