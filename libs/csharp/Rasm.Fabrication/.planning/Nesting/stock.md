...
- Entry: `public static Fin<NestPlan> Pack(NestRun run)` — the host-neutral cutting-stock fold: validate the cut-list and sheet, ceil every part footprint and the trim-inset sheet extent to the `int` packing domain (the `RectangleBinPack` coordinate type) adding the saw `KerfMm` and the per-part `MarginMm` (both clamped non-negative at the ceil — a negative saw geometry never shrinks a footprint), sort the parts descending by area (the placement quality is order-sensitive), then `Drive` the strategy's packer `Insert` stream across as many sheets as the parts require — a `Rect.Height == 0` return is the per-sheet infeasibility sentinel that opens a fresh sheet and re-feeds the unplaced part, and the `MassCut` arm drives the SAME multi-sheet law (one `SingleBinPack` yield query per sheet until the quantity exhausts, a sheet placing zero terminating with the remainder unplaced — the old one-sheet truncation silently under-reported every quantity past the first sheet); the returned `Rect.X`/`Y` offset back into scalar `NestPlacement` millimetres and the `NestYield` derives material-utilization from the per-sheet placed-area sums against the usable sheet area. `Fin<T>` aborts TYPED on the `NestFault` rows minted for exactly these guards — an empty cut-list (`FabricationFault.Nest(NestFault.EmptyCutList, 0)`), a part exceeding the usable sheet (`Nest(NestFault.PartExceedsStock, part)`), a heterogeneous cut-list under `MassCut` (`Nest(NestFault.HeterogeneousMassCut, part)` — a one-footprint homogeneous yield rejects a non-uniform footprint rather than silently mis-cutting copies of the head), a cut-part whose `MaterialId` differs from the sheet's (`Nest(NestFault.MaterialMismatch, part)` — both columns are carried, so an unguarded mixed-material job silently cutting steel parts from an acrylic sheet is the named defect); a degenerate or trim-exceeded sheet stays the kernel `GeometryFault.DegenerateInput` per the folder band-ownership law.
...
using Rasm.Fabrication.Process;      // FabricationFault.Nest + the NestFault guard rows
...
public static class StockNest {
    ...
    public static Fin<NestPlan> Pack(NestRun run) =>
        from nonEmpty in guard(!run.Parts.IsEmpty, FabricationFault.Nest(NestFault.EmptyCutList, 0).ToError())
        ...
        // Material consistency: both columns are carried, so a mixed-material job silently cutting parts from the
        // wrong stock is the named defect — the first offending part rails typed.
        ...
        from fits in parts.Find(p => !FitsSheet(p, run.Sheet.KerfMm, sheetW, sheetH, allowRotation)).Match(
    ...
    static int Ceil(double mm, double kerfMm, double marginMm) =>
    ...
    static Seq<(NestPlacement Placement, double Remnant)> MassCut(Seq<CutPart> parts, StockSheet sheet, int sheetW, int sheetH, bool allowRotation) =>
        parts.Head.Match(
            Some: part => {
                int w = Ceil(part.WidthMm.Value, sheet.KerfMm, part.MarginMm), h = Ceil(part.HeightMm.Value, sheet.KerfMm, part.MarginMm);
                Seq<Rect> upright = toSeq(new SingleBinPack(sheetW, sheetH).Insert(w, h, Math.Max(1, parts.Count)));
                Seq<Rect> rotated = allowRotation && w != h ? toSeq(new SingleBinPack(sheetW, sheetH).Insert(h, w, Math.Max(1, parts.Count))) : Seq<Rect>();
                bool useRotated = rotated.Count > upright.Count;
                var (dw, dh) = useRotated ? (h, w) : (w, h);
                var placements = new List<(NestPlacement, double)>();
                int remaining = Math.Max(1, parts.Count), sheetIndex = 0;
                while (remaining > 0) {
                    Seq<Rect> placed = sheetIndex == 0
                        ? (useRotated ? rotated : upright)
                        : toSeq(new SingleBinPack(sheetW, sheetH).Insert(dw, dh, remaining));
                    if (placed.IsEmpty) { break; }   // unfittable on a fresh sheet: remainder counted unplaced
                    placements.AddRange(placed.Take(remaining).Map(r => (
                        new NestPlacement(part.PartId, part.Material, sheetIndex, r.X * ResolutionMm + sheet.TrimMm, r.Y * ResolutionMm + sheet.TrimMm, r.Width * ResolutionMm, r.Height * ResolutionMm, Rotated: useRotated),
                        0.0)));
                    remaining -= Math.Min(remaining, placed.Count);
                    sheetIndex++;
                }
                return placements.ToSeq();
            },
