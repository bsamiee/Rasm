...
Mark vocabulary, specs, stock, lifetime brackets, tally, probe, colour crossing, and OS palette roster are the kernel paint module's (`Mark`, `PathSpec`, `FillSource`, `PosePlan`, `StrokeSpec`, `Dash`, `TypeFace`, `GlyphBlock`, `PaintProgram`, `PaintStock`, `PaintTally`, `Tween`, `PaintColor`, `ChromeRole`); the ten hand rollback ladders, three CAS triples, `PaintLifetime`, `PaintHook`, and the six `Finite`/`Admitted` duplicates deleted onto it.
