    ...
    def _dispatch(model: "ifcopenshell.file", kind: AnalysisKind, query: str) -> AnalysisResult:
        match kind:
            ...
            case AnalysisKind.SPACE_PROGRAM:
                ...
                rows = tuple(
                    {"space": s.GlobalId, "target": str(target), "actual": str(area), "compliant": str(area >= target)}
                    for s in spaces
            ...
            case AnalysisKind.CLASH:
                ...
                clasher.clash()
                rows = tuple({"a": c["a_global_id"], "b": c["b_global_id"]} for s in clasher.clash_sets for c in s.get("clashes", {}).values())
