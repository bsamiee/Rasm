...
- `expression`(`.api/expression.md`): each `positions` offset folds into the layout owner's closed `Item` `tagged_union` as `Item.of_penalty(hyphen_penalty, hyphen_advance, flagged=True)`, and the `KeyError`-raising `Pyphen(lang=)` construction lifts through `boundary(...)` onto a `RuntimeResult`, a missing dictionary surfacing as a typed fault.
