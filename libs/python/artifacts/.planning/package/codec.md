...
# GIL-releasing arms (zstandard/zlib-ng/py7zr/detools) ride `to_thread`, the companion-gated `lz4`/`brotli`
    ...
    async def _compress(self) -> tuple[ContentKey, BundleEvidence]:
        match self.profile:
            case CodecProfile(tag="lz4", lz4=knobs) | CodecProfile(tag="brotli", brotli=knobs):
                blob, evidence_blob = await to_process.run_sync(_gated_codec, self.algo.value, msgpack.encode(knobs), self.payloads, limiter=_CODEC_LIMITER)
    ...
    async def _recover(self, blob: bytes) -> BundleManifest:
        match self.profile:
            case CodecProfile(tag="lz4") | CodecProfile(tag="brotli"):
                recovered = await to_process.run_sync(_gated_unpack, self.algo.value, blob, limiter=_CODEC_LIMITER)
