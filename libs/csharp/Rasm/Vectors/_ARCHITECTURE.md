...
1. `[implemented]` Matrix solve ownership is in `Matrix.cs`: dense LU/QR/Cholesky, sparse BiCGStab with MathNet fallback, CSparse Cholesky, dense/sparse/generalized eigen paths, and QR least-squares receipts exist. Remaining matrix work is validation collapse and receipt hardening inside existing dense/sparse admission rails, especially eigen nonconvergence, fallback status, and sparse/Hermitian finite-result checks.
2. `[partial]` Extraction is wired through `VectorIntent.Project<TOut>` for probes, contours, iso-surfaces, glyphs, grids, stream bundles, sample receipts, and local mesh scalar isolines. Finish by tightening `VectorIntent` and policy factories so null/raw domains fail at construction, then keep runtime contour and Rhino-native tolerance proof in the bridge lane.
3. `[partial]` Cloud, alignment, and transport diagnostics are real: `CloudCorrespondenceSet` feeds ICP and Sinkhorn, `SinkhornReceipt` reports computed residual facts, weighted mass reaches metrics/alignment/transport, and footprint hull receipts exist. Finish by making `CloudKernel.MassOf` the only mass admission rail, keeping confidence empty until a kernel computes it, and leaving true GICP explicit unsupported instead of approximating it.
4. `[partial]` Mesh and spectral receipts exist for topology, features, flatten, remesh, descriptors, scalar isolines, SDF, and declared segmentation status. Finish by implementing real segmentation kernels before marking segmentation done, exposing cache/factor/skip receipts from existing `LaplacianCache` and `SpectralCore` work, and keeping robust tufted Laplacian, signpost transport, closed SignedHeat, and true tangent log-map unsupported until they land truthfully.
5. `[partial]` Sampling, domains, and modes use the right owners: `SampleKind`, `ExtractionDomain`, `CurveProjection`, `SurfaceProjection`, and `SupportSpace`. Finish by routing public callers through factory admission, using cloud mass as candidate probability where promised, and treating density/adaptive sampling as deterministic priority selection until a proven blue-noise density kernel exists.
6. `[partial]` Field, kernels, and SDFs include derivative-aware kernels, anisotropic tensor metrics, RBF receipts, half-space/slab/capped-cone primitives, Lipschitz bounds, mesh-backed SDF receipts, and native iso-surface receipts. Finish by making validated factories the only invariant-bearing construction path, implementing MLS/profile extrusion only with truthful math, and keeping native callback threading/fixed-tolerance behavior labeled until bridge-proven.

Matrix is the only broad item close enough to mark implemented. Everything else remains partial because shipped surfaces still contain bypassable constructors, raw projection paths, missing receipt propagation, unsupported algorithms, or native-runtime behavior that has not been proven in RhinoWIP.

### Backlog By Owner

- `[partial]` `Intent.cs`: `VectorIntent.Project<TOut>(Context, Op?)` remains the only consumer rail, and sample/contour/glyph/grid/stream-bundle intents route through `ExtractionDomain + SampleKind`. Move raw/null admission into existing intent factories for contour, glyph, grid, stream bundle, sample, support, mesh, and cloud cases; keep capability checks in owner modules.
- `[partial]` `Extraction.cs`: domain-backed glyph/grid/bundle sampling, mesh scalar isolines, surface `IsoStatus`, point-cloud section curves, and raw native contour counts are implemented. Move receipt construction from `ExtractionDomain` into `ExtractionReceipt`, preserve PL mesh-isoline plateau/branch facts, and add projection proof for glyph/bundle paths before treating the rail complete.
- `[partial]` `Flow.cs`: RK tableaus, method-order metadata, adaptive stepping, endpoint/bracket event statuses, bounded bisection localization, and `StreamlineTrace` receipts exist. Higher-order tableau moments, true RK dense output, and validated construction for every `FieldIntegrator` case remain future work.
- `[partial]` `Sample.cs`: explicit, mesh, support, cloud, weighted, scalar-density, and adaptive sampling all execute through `SampleKind` and return `SampleReceipt`. Complete cloud-mass probability semantics, avoid promising blue-noise behavior for deterministic priority selection, and collapse mesh triangulation/prep through the mesh owner instead of duplicating it with extraction.
- `[partial]` `Space.cs`: `SupportSpace` owns support admission, closest hits, signed distance, containment distance, tangent/frame projection, and `SurfaceSpace` sampling. Centralize cluster validity with `VectorCloud` admission and reduce repeated point/vector/frame acceptance inside existing projection owners.
- `[implemented]` `Atoms.cs`: core atoms and primitives exist: dimensions, magnitudes, intervals, axes, angles, directions, spans, frames, cones, relations, and transport. Downstream adoption remains uneven, but the atom layer itself is not a backlog blocker.
- `[partial]` `Modes.cs`: curve tangent/curvature/frame/arclength policies and surface point/frame/UV/Jacobian/metric/area-scale policies exist. Collapse repetitive output casting/acceptance in place and keep native runtime behavior bridge-owned.
- `[partial]` `Cloud.cs`: rings, polylines, clusters, weighted clusters, PCA/covariance, oriented normals, local curvature metrics, hulls, winding, Sinkhorn, and correspondence summaries are wired. Collapse duplicate finite point/mass/shape validation through `AdmitPoints`, `MassOf`, and metric projection owners; add real proof for hull and curvature metrics before strengthening status.
- `[partial]` `Align.cs`: point, plane, symmetric normal-sum, robust Welsch, and normal-weighted ICP rails return `AlignmentReceipt` and correspondences. Keep native kNN index checks local, route row weights through the cloud mass rail, and keep symmetric ICP labeled linearized instead of true GICP.
- `[partial]` `Mesh.cs`: topology, feature basics, flattening, remesh, descriptors, isolines, heat/geodesic paths, Hodge/vector-heat/cross-field constraints, generalized winding SDF, and boundary-source signed heat are wired. Implement segmentation execution, emit `NativeRejected` where receipts define it, classify unwelded/ridge/valley facts truthfully, and expose cache/factor receipts from `LaplacianCache`.
- `[partial]` `Spectral.cs`: DEC assembly, spectral basis, filters, descriptor receipts, Crouzeix-Raviart connection Laplacian, FEM heat scaffold, and CDS holonomy distribution exist and feed mesh paths. Add comparison normalization/ranking, harmonic/genus handling, public DEC wiring where needed, and receipt facts for skipped degenerate elements.
- `[partial]` `Field.cs`: vector/scalar/tensor field algebra, kernel profiles, anisotropic falloff, RBF reconstruction, SDF primitives, mesh-backed fields, and iso-surface receipts exist. Close public-constructor bypasses for invariant-bearing union cases, use existing `FieldNabla` admission patterns to reduce repeated guards, and keep MLS/profile extrusion/closed SignedHeat incomplete until real kernels exist.

### Intent And Projection Gaps

- `[partial]` `Intent.cs`: sample, contour, glyph, grid, and stream-bundle intents route through `ExtractionDomain + SampleKind` under `VectorIntent.Project<TOut>`. Remaining work is broader factory admission, not another projection rail.
...
- `[partial]` Event handling: surface/region localization uses bounded bisection, admits residuals against tolerance, sets localized termination points, and distinguishes endpoint touches from strict bracketed crossings. Cross-surface requires signed-distance-capable support and model absolute tolerance; RK dense-output coefficients remain future work.
...
- `[partial]` `Field.cs`: iso-surface routes through intent/extraction, but public `ScalarField.IsoSurface` still owns the Rhino callback. Label parallel native callback and fixed-tolerance behavior until bridge proof constrains it.
...
- `[partial]` Kernels: anisotropic falloff uses `TensorField` / `SymmetricMatrix` metrics through `sqrt(v^T M v)` and rejects invalid, nonfinite, or nonpositive metric distances. Collapse repeated finite/positive guards through existing field admission shapes.
- `[partial]` Reconstruction: RBF interpolation/approximation uses matrix solve receipts and records mode, smoothing, sample count, residual, solve path, and factor facts. MLS is declared but unimplemented.
- `[partial]` SDF primitives: half-space, slab, and capped cone exist. Profile extrusion remains missing, and oriented prism remains unnecessary until composition proves insufficient.
- `[partial]` SDF outputs: `ScalarField.LipschitzBound()` exists, mesh-backed signing routes through generalized winding / boundary-source signed heat, and SDF/iso-surface receipts expose approximate/native facts. Watertight preflight, closed SignedHeat, and fuller fixed-tolerance receipt propagation remain incomplete.
...
- `[partial]` Mesh features: classified boundary/crease/nonmanifold/ngon-skipped receipt facts exist. Ridge/valley, unwelded, and region-boundary geometry remain unsupported or undercounted until real classification logic lands.
- `[partial]` Mesh segmentation: `MeshSegmentationReceipt` is declared, but threshold regions, region growing, watershed-like bands, and descriptor clustering have no execution rail. Do not mark segmentation complete until real region assignments exist.
- `[partial]` Mesh diagnostics: `LaplacianCache` and descriptor/eigen receipts expose path facts, but cache-hit, skip-count, and factorization receipts remain incomplete. Robust tufted Laplacian, flipped signpost vector heat, closed SignedHeat, runtime scalar-isoline geometry proof, and true tangent log-map remain unsupported.
...
- `[partial]` `Cloud.cs`: `SinkhornReceipt` reports coupling summaries, source/target residual semantics, numeric status, and correspondence summaries from computed facts. Reuse normalized mass facts instead of revalidating them in transport internals.
- `[partial]` `Cloud.cs`: weighted clusters feed centroid/covariance, principal axes/frame, shape/spread, density, transport mass, and alignment row weights. kNN/radius graph outputs still need fuller mass/correspondence receipt propagation.
...
- `[partial]` `Align.cs`: receipt includes kind, approximation status, solve receipt, correspondences, residual quantiles/max, robust scale/weight range, and final step delta. Symmetric ICP remains a normal-sum linearized approximation, not true GICP.
...
- `[partial]` `Sample.cs`: weighted and scalar-field-driven sampling route through `SampleKind`; density maps and programmatic priorities control deterministic selection and output mass. Reuse `CloudKernel.MassOf` semantics where cloud mass becomes sample probability.
...
- `[partial]` `Sample.cs`: adaptive sampling uses scalar-field intensity to vary local spacing. It is deterministic priority/adaptive spacing rather than a proven density blue-noise kernel.
- `[implemented]` `Sample.cs`: `SampleReceipt` carries attempted/emitted/rejected, candidate count, spacing stats, count-density error, density admission counts, iteration count, stop kind, and domain status from the same sample execution.

### Modes, Matrices, And Product Boundary

- `[partial]` `Modes.cs`: surface point/frame/UV frame/Jacobian/metric/area-scale projections and explicit curve frame/perpendicular-frame normal/binormal policies exist. Native runtime behavior remains bridge-owned.
- `[implemented]` `Matrix.cs`: `SolveReceipt` / `EigenSolveReceipt` cover dense LU/QR/Cholesky, sparse iterative/MathNet fallback, CSparse Cholesky, dense/sparse/generalized eigen paths, and QR least-squares is owned by `Matrix.cs`. Remaining matrix work is not new surface; harden dense/sparse admission, eigen receipts, fallback/nonconvergence statuses, and downstream cache/factor receipt propagation.
