...
- `Usage` is the PACKED `(page << 16) | usage` word the descriptor carries and the value `DataValue.Usages` yields, so a usage table keys on that word rather than on a report offset: the Generic Desktop axis block runs `0x0001_0030`..`0x0001_0035` for `X`..`Rz`, and the Button page is ORDINAL — `Button1` is `0x0009_0001` and the enum runs through `Button31`, so a button block is a count rather than a roster.
