...
[SETTINGS_BRIDGE]:
- `Rhino.Render.DataSources.RhinoSettings.GetRenderSettings() -> RenderSettings` / `SetRenderSettings(RenderSettings renderSettings) -> void` — read and write the document render settings the editor binds; `RenderSettings` is `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendersettings.md`'s.
...
- `Rhino.Render.RenderTabs.RegisterTab(PlugIn plugin, Type tabType, string caption, Icon icon) -> void` / `RegisterTab(PlugIn plugin, Type tabType, Guid renderEngineId, string caption, Icon icon) -> void` — register a custom side-pane tab; both are INSTANCE members, `tabType` carries a `GuidAttribute`, and `Icon` is `System.Drawing.Icon`.
...
- `RenderPanels` registers custom render-window panels and `RenderTabs` custom side-pane tabs, both keyed by a `GuidAttribute`-decorated `Type` and resolved by render-session id. Neither is mintable: the constructors are internal and the host hands each instance to `RenderPlugIn.RegisterRenderPanels(RenderPanels)` / `RegisterRenderTabs(RenderTabs)`, both `protected virtual`, so registration is one-shot inside those overrides and every row registered after they return is ignored. `FromRenderSessionId`, `SidePaneUiIdFromTab`, and `SessionIdFromTab` are the static resolvers.
...
- Editor internals stay host-side: the `DataSources` back-ends and the `Rhino.Render.UICommands`/`Rhino.Render.Controls.Definitions` namespaces are internal and never cross the boundary, and a UI section seats through the `Rhino.UI.Controls.ICollapsibleSection` family `libs/csharp/Rasm.Rhino/.api/api-rhino-ui.md` owns.

[STACKING]:
- `LanguageExt.Core`(`libs/csharp/.api/api-languageext.md`): a `RhinoSettings`/`MetaData`/`WorldMapDayNight` `IDisposable` rides a `using` bounded by `Eff`; a settings read projects a detached value record, a `ViewSupportsShading` predicate crosses as `bool`, a panel/tab registration crosses as `Fin<Unit>`, and `FromRenderSessionId` returning `object` lifts to `Option<A>` after an edge cast.
- `Thinktecture.Runtime.Extensions`(`libs/csharp/.api/api-thinktecture-runtime-extensions.md`): `Modes`, `Shapes`, `Sizes`, `AssignBys`, `RenderPanelType`, and `ExtraSidePanePosition` map at the edge to `[SmartEnum]` owners; the PBR name space collapses to one `[SmartEnum]` keyed on `TextureType` reading `FromTextureType` per row, and the basic-material `const` names and `pbr-brdf` collapse to one `[SmartEnum<string>]` beside it — a 23-row string roster mirroring the PBR properties is the deleted duplicate of a derivation the host already owns.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendersettings.md`: `RhinoSettings.GetRenderSettings`/`SetRenderSettings` bridge the document `RenderSettings` aggregate — settings own the configuration, this catalog owns the editor binding.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendercontent.md`: PBR/basic parameter names address `RenderContent.GetParameter`/`SetParameter`, and a registered panel or tab edits `RenderContent` — the content catalog owns the content, this catalog owns the UI vocabulary and registration.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-document.md` and `libs/csharp/Rasm.Rhino/.api/api-rhino-ui.md`: `RhinoDoc`/`RhinoView`/`ViewInfo`/`TextureType` bind the settings bridge and child-slot resolver, and a registered panel or tab body composes `Rhino.UI.Controls.ICollapsibleSection`/`Rhino.UI.ObjectPropertiesPage` — the document and UI catalogs own the carriers, this catalog owns the render-editor seam.
