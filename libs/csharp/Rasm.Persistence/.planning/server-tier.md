...
public abstract partial record TimescaleStep {
    // hypertable over the OpLogEntry-rollup table; the range key is the HLC `Physical` instant.
    public sealed partial record Hypertable(string Table, string TimeColumn, string Interval) : TimescaleStep;
    // continuous-aggregate MV plus its native bgworker refresh policy, folded as one DDL pair.
...
// --- [MODELS] -------------------------------------------------------------------------
// vectorscale build-options axis (api-pgvectorscale.md §2): sentinels num_neighbors=-1
// (dimension-derived degree), num_dimensions=0 (all dimensions), num_bits_per_dimension=0
// (automatic bit selection; build-time picks 2 below 900 dims, 1 at >=900) resolve at build
// time from vector dimensionality. `Default` carries num_neighbors=50 (the literal default)
// and num_bits_per_dimension=0 (the automatic-selection sentinel), never a pinned bit width.
...
public abstract partial record IndexSpec {
    // diskann over a pgvector `vector(N)` column; `<#>` rejected on storage_layout=plain.
    ...
    // one bm25 index per table; KeyField is UNIQUE/PK and folded first (api-pg-search.md §2).
...
// pg_search 0.24.0 query projection (api-pg-search.md §3-§4); the paradedb.* namespace is
// removed. Column-operator cases execute WITHOUT @@@; builder cases take a pdb.* function on
// the right of @@@; cast cases (Fuzzy/Boost/Const/Slop) wrap any inner predicate as a
// composable ::pdb.* modifier, so a new pdb.* builder is a union case, not a sibling method.
[Union]
...
public static class SearchProvisioning {
    ...
    // pg_search scoring/highlighting/aggregation projections (api-pg-search.md §5-§6);
    // key_field-anchored, composable across joins.
...
// the three lifecycle phases of one tenant — folded by the single TenantProvision owner so
// create, teardown, and RLS-policy DDL are arms of one model.Switch, not parallel classes.
[Union]
...
- [SERVER_PROVISIONING_PROBE]: the live-PG18 round-trip against the colima timescaledb/paradedb image — the `create_hypertable`/`add_continuous_aggregate_policy`/`add_columnstore_policy` apply contract, the diskann-over-`vector` `storage_layout=memory_optimized` plus the `num_neighbors`/`search_list_size`/`num_bits_per_dimension` build-option apply, the `pg_search` 0.24.0 BM25 `@@@ pdb.parse`/`|||`/`pdb.score`/`pdb.snippet` query shape (the removed `paradedb.*` namespace asserted absent), and the RLS `current_setting('rasm.tenant')` per-tenant isolation, proven against the installed extensions before the provisioning DDL pins.
