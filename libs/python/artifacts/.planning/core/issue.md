...
- Auto: `_cleared` is the severed gate — `plan.severed` folds its `PlanFault` cause onto the `ISSUE_PLAN` row's two named coordinates, the tag and the offending key hexes, so a cyclic, untargeted, collided, or dangling graph never drains; `_driven` is COMPOSITION over `execution/lanes#LANE` `LanePolicy.driven`, not a drive: it hands the already-resolved CPM ladder through the `Fronts` `resolved` arm labelled with its own `ArtifactStage` member, seeds the warm cache, carries the drive's `Drained.cache` forward as the answer (replayed hits included), and names the gate point. The fault-block short-circuit, the cancelled-front deadline path, the threaded cache, and the sequential-front exemption are all the drive's, stated once at the owner, so results stay terminal and no second loop can drift out of any of them. Both the `planned` fact and the coverage verdicts stay the plan's; this page adds no parallel telemetry.
...
from rasm.runtime.faults import TERMINAL, BoundaryFault, FaultRow, RuntimeResult, rostered, traversed
...
class ArtifactIssue(Struct, frozen=True):
    ...
    async def issue(self, request: IssueRequest, /) -> RuntimeResult[Map[ContentKey, object]]:
    ...
    async def _issued(self, request: IssueRequest, scope: str, /) -> RuntimeResult[Map[ContentKey, object]]:
    ...
    ) -> RuntimeResult[Map[ContentKey, object]]:
    ...
    @staticmethod
    def _cleared(plan: PipelinePlan[object], /) -> RuntimeResult[PipelinePlan[object]]:
        return (
            plan.severed
    ...
    ) -> RuntimeResult[tuple[tuple[ArtifactWork[object], ...], frozenset[ContentKey]]]:
        match request:
    ...
    async def _driven(self, plan: PipelinePlan[object], /) -> RuntimeResult[Map[ContentKey, object]]:
