...
- Law: metric names are rows exactly like attribute keys — the runtime fact-stream and vital owners declare their instruments against `_metric` rows, `slo#OBJECTIVE` objectives reference the same rows, and `board#QUERY` queries build from them, so one rename propagates the whole plane at compile time.
...
- Boundary: `AppIdentity` is `value/identity`'s value — this page projects it into attribute space and owns nothing about its construction; `deployment.environment.name` is a config fact stamped by the runtime export owner's policy, deliberately absent from the identity projection.
