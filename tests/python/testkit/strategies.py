...
def _json_value(depth: int = 0) -> st.SearchStrategy[object]:
    return _JSON_SCALAR if depth >= 2 else st.one_of(_JSON_SCALAR, st.lists(_json_value(depth + 1), max_size=3), st.dictionaries(st.text(min_size=1, max_size=8), _json_value(depth + 1), max_size=3))
...
def _multiples[N](lo: object, hi: object, step: object, project: Callable[[Decimal], N], *, excl_lo: bool = False, excl_hi: bool = False) -> st.SearchStrategy[N]:
    """Draw multiplier k directly so every value is a valid in-range multiple with zero rejection.

    Fraction bounds are exact for int, float, and Decimal inputs; an exclusive bound that lands exactly on a multiple
...
def _text(mn: object, mx: object, pattern: object) -> st.SearchStrategy[str]:
    ...
    return st.nothing() if lo > hi else st.from_regex(pattern, fullmatch=True).filter(lambda s: lo <= len(s) <= hi) if isinstance(pattern, str) else st.text(min_size=lo, max_size=hi)
...
def _node(node: _mi.Type) -> st.SearchStrategy[object]:  # ruff:ignore[complex-structure]
    ...
    Raises:
        AssertionError: A node outside the closed msgspec taxonomy — unreachable while the arms stay total.
    """
    match node:
        ...
        case _mi.FloatType(ge=ge, gt=gt, le=le, lt=lt):
            ...
            return _multiples(lo_f, hi_f, step_f, float, excl_lo=open_lo, excl_hi=open_hi) if isinstance(step_f, int | float) else st.floats(min_value=lo_f, max_value=hi_f, exclude_min=open_lo, exclude_max=open_hi, allow_nan=False, allow_infinity=False)
        ...
        case _mi.BytesType() | _mi.ByteArrayType() | _mi.MemoryViewType():
            ...
            return binary.map(bytearray) if isinstance(node, _mi.ByteArrayType) else binary.map(memoryview) if isinstance(node, _mi.MemoryViewType) else binary
...
def _fbound(schema: _Schema, incl: str, excl: str) -> tuple[float | Decimal | None, bool]:
    ...
    return (a if isinstance(a, Decimal) else float(a), False) if isinstance(a, int | float | Decimal) else (b if isinstance(b, Decimal) else float(b), True) if isinstance(b, int | float | Decimal) else (None, False)
...
def _pyd_node(schema: _Schema, defs: dict[str, _Schema]) -> st.SearchStrategy[object]:  # ruff:ignore[complex-structure]
    """Map one ``pydantic-core`` schema node to a constraint-honoring strategy under the accumulated ``definition-ref`` table."""
    leaf = _unwrap(schema)
    match leaf.get("type"):
        case "int":
            ...
            return _multiples(lo_i if lo_i is not None else -_NUM_CEILING, hi_i if hi_i is not None else _NUM_CEILING, mo, int) if isinstance(mo, int) else st.integers(min_value=lo_i, max_value=hi_i)
        case "float":
            lo, excl_lo = _fbound(leaf, "ge", "gt")
            hi, excl_hi = _fbound(leaf, "le", "lt")
            mo_f = leaf.get("multiple_of")
            return (
                _multiples(lo if lo is not None else -float(_NUM_CEILING), hi if hi is not None else float(_NUM_CEILING), mo_f, float, excl_lo=excl_lo, excl_hi=excl_hi)
        ...
        case "decimal":
            ...
            if isinstance(mo_d, int | float | Decimal):
                return _multiples(lo_eff if lo_eff is not None else -_NUM_CEILING, hi_eff if hi_eff is not None else _NUM_CEILING, mo_d, lambda value: value, excl_lo=excl_lo, excl_hi=excl_hi)
            base = st.decimals(min_value=lo_eff, max_value=hi_eff, places=dp, allow_nan=False, allow_infinity=False)
            return base.filter(lambda d: (not excl_lo or lo_eff is None or d > lo_eff) and (not excl_hi or hi_eff is None or d < hi_eff)) if (excl_lo or excl_hi) else base
        case "str":
        ...
        case "list":
            ...
            return st.lists(_pyd_node(_sub(leaf, "items_schema"), defs), min_size=mn_l if isinstance(mn_l, int) else 0, max_size=mx_l if isinstance(mx_l, int) else 3)
        case "set" | "frozenset":
        ...
        case "model-fields" | "dataclass-args" | "typed-dict":
            ...
            required = {str(n): _pyd_node(_sub(f, "schema"), defs) for n, f in fields.items() if _is_schema(f) and _sub(f, "schema").get("type") != "default"}
            optional = {str(n): _pyd_node(_sub(f, "schema"), defs) for n, f in fields.items() if _is_schema(f) and _sub(f, "schema").get("type") == "default"}
...
def resolve[T](subject: TypeForm[T]) -> st.SearchStrategy[T]:
    """Register and return a constraint-bounded strategy for a type, PEP 695 alias, union, ``Literal``, ``Annotated``, or type expression."""
            ...
            match _mi.type_info(subject):
                ...
                case _mi.TypedDictType(fields=fields):
                    ...
                    def _td_build() -> st.SearchStrategy[object]:
                        return st.fixed_dictionaries({f.name: _node(f.type) for f in fields if f.required}, optional={f.name: _node(f.type) for f in fields if not f.required})
