...
public sealed partial class RetentionClass {
    ...
    public static readonly RetentionClass Blob = new("blob", StorageLane.ObjectStore, LossPolicy.NeverEvict, IdentityScheme.ContentKeyed, DataClassification.Internal, new RetentionSchedule(Duration.FromHours(12), 512L * 1024 * 1024 * 1024, int.MaxValue, Duration.FromDays(90)));
...
public static class RetentionCatalog {
    ...
    public static Fin<RetentionFact> Admit(RetentionClass cls, ContentAddress contentKey, string name, ulong epoch, DataClassification stamp, long bytes, StorageTier tier, Func<ContentAddress, bool> resident, ProjectionContext frame) =>
        !RetentionCeiling.Ranked(stamp)
            ? Fin<RetentionFact>.Fail(new RetentionFault.Unstamped(contentKey))
            : !RetentionCeiling.Admits(stamp, cls.Ceiling)
                ? Fin<RetentionFact>.Fail(new RetentionFault.CeilingBreach(stamp, cls.Ceiling))
                : cls.Scheme.Identity(contentKey, name, epoch) switch {
                    var key => Fin<RetentionFact>.Succ(new RetentionFact(cls, key, cls.Scheme.Dedups && resident(key) ? 0L : bytes, tier, frame.Now())),
                };
}
```
...
public abstract partial record SweepVerdict {
    ...
    // a race-loser row) past the injected eligibility age gate — the age-gated orphan pass closing the write-crash
    // loop while H10 keeps every referenced blob collection-free.
...
public static class RetentionSweep {
    ...
    // itself — an unreachable, fence-cleared artifact is EvictOrphan (the age-gated orphan pass; the gate rides the
    // injected predicate) and a reachable one past its age cold-tiers one rung per pass (idempotent at `Archive`)
