export const meta = {
  name: 'realize',
  whenToUse: 'Execute a root campaign DECISION/brief into its target planning folder. args = {doc, root} or an array of such pairs; campaigns run as parallel lanes. Wide read-only opus reconnaissance hands one fable executor everything on a golden platter — the doc, exact disk locations, ripple endpoints, and the full two-tier .api stacking inventory — and the executor implements the whole campaign under the language doctrine. Run cold-verify after.',
  description: 'Campaign realization in two moves. RECON: opus agents fan out read-only — page-slice mappers (root doc IN FULL + slice pages IN FULL + sibling context), one governance mapper (manifests, csproj/README registries, .api anchors, index docs), and two stacking mappers (one per .api tier: the shared libs/<lang>/.api substrate and the folder <root>/.api tier, full inventory mined to operator depth against the doc page set — doc-demanded AND beyond-doc underutilized capability, exact verified members). Mappers provide information — locations, anchors, ripples, inventory — never prescriptions. EXECUTE: one fable per campaign takes the root doc plus every map and implements everything in place at the docs/stacks/<language>/ bar: the full page set, registry and index closure, every ripple in the same pass, the two-tier stacking woven in, its own hunt past the maps. No stage after the executor; cold-verify is the separate closure gate.',
  phases: [
    { title: 'Plan', detail: 'per campaign: enumerate the page set (disk + doc-ruled new pages), partition into mapper slices', model: 'sonnet' },
    { title: 'Recon', detail: 'per campaign: page-slice + governance + two-tier stacking mappers, read-only, information not prescriptions', model: 'opus' },
    { title: 'Execute', detail: 'per campaign: one fable implements the whole campaign — doc + maps + doctrine, every ripple in-pass', model: 'fable' },
  ],
}
...
const lanes = await parallel(CAMPS.map((c) => async () => {
  ...
  const fix = await agent(CTX(c) + '\n\nTASK: EXECUTE THE CAMPAIGN (WRITER — full authority over ' + c.root + ', its ' +
    'manifest rows, its .api tier, and the index docs; you are the run\'s LAST agent). Read ' + c.doc + ' IN FULL — it is ' +
    ...
    'docs, and repair every ripple your edits expose in the same pass, both seam ends. A ruling is a FLOOR: exceed it with ' +
