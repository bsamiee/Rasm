...
- `GeometryGymIFC_Core` KEEP, sole IFC model surface — no xBIM leaf writes IFC4.3 whole across STEP/ifcXML/ifcJSON; one doing so reopens.
- `Xbim.InformationSpecifications` KEEP beside `ids-lib` — constraint engine and conformance authority; one package shipping both reopens.
- `Themis.Las` KEEP beside `Unofficial.laszip.netstandard` — LAS and LAZ are two decode legs of one ingest front; one engine spanning both reopens.
...
- Bim CLASSIFIES and CARRIES texture payloads and decodes none — an image codec or texel resample mints a second raster owner in a host-neutral leaf.
- Obliged glTF extensions register FROM the payload, unioned with the policy roster — the writer emits either way, so roster trust fails silently.
- Canonical channel names are the one vocabulary crossing Bim — `TextureMode` and `GltfChannel` own the correspondence; an unclaimed name REFUSES.
- One canonical name targets SEVERAL glTF channels — `orm` is one image read through two references, so a per-channel byte copy is the deleted form.
...
- Nation-to-annex bridging keys on the ISO alpha-2 `CountryCode` — name equality misses rows, so it seeds `AnnexRegime` and never resolves it.
...
- Interchange capability rides ONE `InterchangeCapability` vocabulary every carrier subsets, corners as `CapabilityLaw` rows — bools hide corner law.
- Export render and write-layout traits ride ONE `ExportTrait` set per carrier — a trait a material finish can represent is a corner the law bars.
- Provider scene walks are ONE fold over provider accessors — a per-provider mesh/node walk forks the placement law three ways.
...
- Serialization and container are SEPARATE wire-form axes crossed sparsely — a fused row set loses the zip law and release admissibility.
- `StyledAppearance` pairs `Node.Appearance` with its `SurfaceTexture` roster — folding a map's mean into a scalar channel is the deleted form.
...
- `Semantics/feature` owns the geospatial ROW and everything admitting into it — a second admission door forks the validity verdict reads trust.
- `Semantics/model` owns the indexed SET and every read over it — a read beside the index re-pays the broad phase the tree exists to close.
- `Semantics/vector` and `Semantics/raster` stay the two INGEST FRONTS — modality decides the front; a merged front re-keys every codec row on it.
- One GDAL acquire-use-release bracket serves every `/vsimem` leg — a per-leg `try`/`finally` re-decides one release policy at each site.
- Content keys mint through the kernel hasher over `CanonicalWriter` — an up-stratum `Rasm.Compute` `InterchangeIdentity` reach inverts the strata.
