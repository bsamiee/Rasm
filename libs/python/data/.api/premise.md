...
- rail: epd-lca
- note: `source_type` is `brightway` (reads `source_db` from the project) or `ecospold` (reads `source_file_path`); `system_model` is `cutoff` or `consequential`; `key` decrypts the IAM scenario data; `name` on the writers is one string or a per-scenario list
- call: `NewDatabase(scenarios, source_version='3.12', source_type='brightway', key=None, source_db=None, source_file_path=None, additional_inventories=None, system_model='cutoff', system_args=None, use_cached_inventories=True, use_cached_database=True, external_scenarios=None, quiet=False, keep_imports_uncertainty=True, keep_source_db_uncertainty=False, gains_scenario='CLE', use_absolute_efficiency=False, biosphere_name='biosphere3', generate_reports=True)`
- call: `PathwaysDataPackage(scenarios, years=range(2005,2105,5), source_version='3.12', source_type='brightway', key=None, ...)`; `.create_datapackage(name='pathways_<date>', contributors=None, transformations=None)`

| [INDEX] | [SURFACE]                                             | [ROLE]                                                                       |
| :-----: | :---------------------------------------------------- | :--------------------------------------------------------------------------- |
|  [01]   | `NewDatabase`                                         | the prospective-database builder (constructor in `- call:`)                  |
|  [02]   | `NewDatabase.update(sectors=None)`                    | apply prospective transforms; `None` runs all sectors, a subset scopes it    |
|  [03]   | `NewDatabase.write_db_to_brightway`                   | register the transformed database(s) into the open Brightway project         |
|  [04]   | `NewDatabase.write_superstructure_db_to_brightway`    | one superstructure DB carrying all scenarios as a scenario-difference matrix |
|  [05]   | `NewDatabase.write_datapackage`                       | emit a `bw_processing`/`datapackage` superstructure (`unfold`-shareable)     |
|  [06]   | `NewDatabase.write_db_to_simapro`                     | export to SimaPro CSV                                                        |
|  [07]   | `NewDatabase.write_db_to_olca`                        | export to openLCA (JSON-LD)                                                  |
|  [08]   | `NewDatabase.write_db_to_matrices`                    | export to sparse `numpy`/`scipy` matrices                                    |
|  [09]   | `NewDatabase.generate_scenario_report`                | a per-scenario impact report                                                 |
|  [10]   | `NewDatabase.generate_change_report`                  | a transformation change-log (`prettytable`/Excel)                            |
|  [11]   | `IncrementalDatabase`                                 | incremental builder; `.update(sectors=dict)` stacks sector transforms        |
|  [12]   | `IncrementalDatabase.write_increment_db_to_brightway` | write the per-increment databases for step sensitivity                       |
|  [13]   | `PathwaysDataPackage`                                 | the time-series builder (constructor in `- call:`)                           |
|  [14]   | `PathwaysDataPackage.create_datapackage`              | produce a `pathways`-tool datapackage spanning the year grid                 |
