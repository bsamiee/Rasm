...
- Output: `Generation.Custody` — the position it seated, the generation it left, the generation it landed, the count it re-encoded, and the digest over the bytes it wrote, in sequence order.
...
- Law: the digest absorbs each written payload in sequence order through one `Digest.Session`, so the custody row proves what the swap landed without buffering a log; a re-run recomputing a different digest names a fold that read a moving log, which the exclusive fence forecloses.
...
- Law: re-encoding RE-ADDRESSES every entry — the content key minted over superseded bytes addresses bytes the live log no longer holds, so a preservation slice landed under a superseded generation is evidence OF that generation and joins the live log through the custody row rather than through a subject; announcements are drained before the fence, so no undelivered deliverable survives carrying a stale subject.
...
- Boundary: the fold runs outside every request path and holds the app's whole log; the app supplies `recast`, and this page supplies the fence, the shadow, the counter, the identity carry, the custody row, and the swap.

```mermaid
sequenceDiagram
  accTitle: Journal generation cutover
  accDescr: The cutover fences every app, mints the shadow from the relation's own DDL, carries the counter, streams every entry through the recast into the shadow, seals the custody row, swaps the relations, and truncates the derived planes.
  ...
  O->>C: seal custody — source, target, entries, digest
...
- Output: `save` yields `Journal.Fence<number>` — `Advanced` means the store holds this fold's version, `Stale` names the version that beat it beside the one offered; the verdict is the swapped value, so a losing snapshotter never has to read success out of a statement that answered nothing.
- Output: `load` yields `Option<{ state, version }>` — present means fold-from-`version + 1`, absent means replay from origin; the option IS the protocol.
...
- Law: a load whose body fails its own state schema is `ParseError` on admission — the consuming lane discards the snapshot and replays; corruption degrades to cost, never to wrong state.
...
- Packages: `effect` (`Stream`); `journal/append.md` (`Journal.of(...).read`, `Journal.Appended`).
- Entry: lanes call `Snapshot.due(appended, cadence)` with the `Journal.Appended` the append just returned and `bound.save` when it answers true, reading the `Journal.Fence` it answers rather than discarding it; `Snapshot.hydrate(bound, journal, stream, fold)` is the one state-recovery entry every lane and rebuild composes.
...
const _due = (appended: Journal.Appended, cadence: Snapshot.Cadence): boolean =>
  Math.floor(appended.version / cadence.every) > Math.floor((appended.first - 1) / cadence.every)
