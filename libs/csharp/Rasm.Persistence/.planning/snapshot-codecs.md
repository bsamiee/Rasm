...
- [GEOMETRY_FACTORY_DEFAULT]: the `GeometryFactory.Default` SRID-4326 construction spelling the `GeoJsonProjection.Default` carries and the exact positional `new GeoJsonConverterFactory(GeometryFactory, writeGeometryBBox, idPropertyName, ringOrientation, allowModifyingAttributesTables)` ctor arity, beyond the catalogued converter-factory policy set and its SRID-4326 default.
