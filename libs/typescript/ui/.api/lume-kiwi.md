...
- `dotnet:Rasm.AppUi/Shell/solver`: the AppUi shell owns the authoritative solve as its layout capability and serializes the ordered program; TS re-solves the identical program for local latency, and a TS-side algebra rebuild that diverges from wire order is the `CROSS_LANGUAGE_WIRE` drift defect.
