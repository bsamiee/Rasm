...
import { originFold, type OriginCursor, emptyOriginCursor } from "../causality-graph/version-vector";
...
- Packages: `effect` for `Subscribable` (through the `fold-core/projection#PROJECTION` `Projection` alias) and `SubscriptionRef`; the `projectStore`/`derive` adapter arrives owned from `fold-core/projection#PROJECTION`, the frontier composing it and exposing no second read face.
...
- Boundary: the projection exposes only the meet `VersionVectorWire`, never the held-cursor antichain, so the GC and the as-of coordinate read the settled horizon and never the per-entity interior; the projection re-mints nothing — it maps the already-folded meet through the owned `fold-core/projection#PROJECTION` adapter; the atom bridge binds the `Subscribable` at the `ui` boundary, never imported into the fold interior; the domain dials no transport.
...
```mermaid
flowchart LR
  rel[causal-buffer released causal-ordered Stream]
  ...
  proj -->|dominates meet, context| gc[frontier-gc causal finalize]
  proj -->|Stable coordinate| asof[temporal-query as-of-query]
  proj -->|stable prefix unit| merkle[version-vector merkle-reconcile]
```
