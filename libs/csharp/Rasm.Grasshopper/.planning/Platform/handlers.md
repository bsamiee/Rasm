...
The Eto platform seam of the Grasshopper boundary — one capability owner (`PlatformSeam`) over the active `Platform` root: the typed platform snapshot, one polymorphic capability demand (`PlatformClaim` — feature flags, handler-type support, platform-row identity — behind one gate), and the platform-context window foreign-platform work runs inside. One handler owner (`Handlers`) covers the widget-to-handler substrate: instantiator registration through `Platform.Add<T>`, reach-row resolution over `Create`/`CreateShared`, the per-widget identity capsule (`Widget.Handler`/`NativeHandle`/`ControlObject`/`ID`/`Style`/`IsDisposed` read as one evidence record), and the leased mint census over `HandlerCreated`/`WidgetCreated`. One styling owner (`Styler`) turns `Eto.Style` registration into frozen data rows — a `StyleRow` closes its widget or handler generic at mint, a `StyleLedger` seals the registry injectively, and wearing a style is a marshalled `Widget.Style` assignment — so a canvas or panel restyles by row, never by subclass. One bridge owner (`Bridge`) holds `NativeControlHost` embedding for ready and deferred native views and registers the managed-to-AppKit contract (`IMacViewHandler`/`IMacWindow`, `MacConversions`/`CGConversions`) the macOS pages compose. No census source touches this territory — the page is pure host-capability buildout: every future GH2 panel, plugin chrome, and native-integration surface composes these rows instead of probing `Platform.Instance`, comparing stringly platform ids, subclassing controls for cosmetics, or hand-rolling `Handler` casts. Every fallible step rides an `Op`-keyed `Fin<T>` rail; every host mutation marshals through `EtoDispatch`; every observation lifetime rides the kernel `Lease<T>` rail.
...
- Growth: a new platform id is one `PlatformRow` row; a new demand modality is one `PlatformClaim` case plus one `Demand` arm; a new platform fact is one `PlatformFact` field.
...
- Growth: a new sprout origin is one `NativeSprout` case plus one `Embed` arm; a resolved conversion member is a call-site spelling on the composing page — the gate never widens.
...
`Op`, `Fault`, `Lease<T>`, `ValidityClaim`, and `EtoDispatch` are composed upstream owners; every named host member is decompile-verified against the installed `Eto.dll`, and the `Eto.Mac` bridge contract is catalog-verified. RESEARCH: the `MacConversions`/`CGConversions` member spellings, landing as call-site spellings on the composing macOS pages.
