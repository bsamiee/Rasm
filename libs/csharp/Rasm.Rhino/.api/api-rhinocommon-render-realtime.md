...
- `RealtimeDisplayModeClassInfo.RequiredDisplayTechnology -> Rhino.Display.DisplayTechnology` — `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-display.md` owns the display-technology vocabulary.
...
- `ChangeQueue.ApplyLightChanges(List<ChangeQueue.Light>)` / `ApplyDynamicLightChanges(List<Rhino.Geometry.Light>)` — the two light hooks take DIFFERENT payload types: the batch hook carries the queue's own snapshot (id, CRC, change-type, detachable `Data`), the dynamic hook carries bare geometry lights with no change discriminant, so an override spelling one signature for both compiles as a new method and never fires.
- `ChangeQueue.ApplySunChanges(Rhino.Geometry.Light)` / `ApplyMaterialChanges(List<ChangeQueue.Material>)` — sun and material hooks; the sun hook is a geometry light like the dynamic hook, the material hook a queue snapshot like the batch light hook.
...
- `RealtimeDisplayMode` HUD events, whose spelling is IRREGULAR per control family: `Hud{Play,Pause,Lock,Unlock}Button{Pressed,LeftClicked,RightClicked,DoubleClicked}` carry a `Button` infix, `Hud{ProductName,StatusText,Time}{Pressed,LeftClicked,RightClicked,DoubleClicked}` do not, and `HudPostEffects{On,Off}Button{Left,Right,Double}Clicked` publish the three click edges with NO press — thirty-four `EventHandler` events plus `MaxPassesChanged` over `HudMaxPassesChangedEventArgs.MaxPasses`. A name fold across the three families subscribes to members the assembly does not declare.
- The `Hud*` virtual predicates (`HudShow`, `HudShowControls`, `HudShowPasses`, `HudShowMaxPasses`, `HudAllowEditMaxPasses`, `HudShowCustomStatusText`, `HudRendererPaused`, `HudRendererLocked`, `HudMaximumPasses`, `HudLastRenderedPass`, `HudProductName`, `HudCustomStatusText`, `HudStartTime`) configure the HUD.
- `RealtimeDisplayMode.RegisterDisplayModes(PlugIn) -> RealtimeDisplayModeClassInfo[]` / `RegisterDisplayModes(Assembly, Guid) -> RealtimeDisplayModeClassInfo[]` / `UnregisterDisplayModes(PlugIn)` / `UnregisterDisplayModes(Assembly, Guid)` / `GetRealtimeViewport(nint[, bool]) -> RealtimeDisplayMode` / `RemoveRealtimeViewport(nint)` — static registration and viewport resolution; both register overloads return NULL rather than an empty array when the plug-in is unloaded, no descriptor matched, or the RDK plug-in is absent.
- Registration REFLECTS over the assembly's EXPORTED types under four structural gates: a `RealtimeDisplayModeClassInfo` subclass admits non-abstract with a public parameterless constructor, and its `RealtimeDisplayModeType` admits non-abstract, deriving `RealtimeDisplayMode`, with a public parameterless constructor of its own.
- The host ACTIVATES both, so an engine whose only constructor takes its program never registers; `RealtimeDisplayMode()` is `protected` parameterless and `PostConstruct()` — the virtual hook fired once the native viewport connects — is the one seam binding state the constructor cannot take.
- `RealtimeDisplayModeClassInfo`: abstract `Name` `GUID` `DrawOpenGl` `RealtimeDisplayModeType`, virtual `DontRegisterAttributesOnStart` (default `false`) `RequiredDisplayTechnology` (default `DisplayTechnology.None`) — the abstract registration descriptor; `DrawOpenGl` true skips `RenderWindow` use, and `DontRegisterAttributesOnStart` true withholds the display-attribute registration.
...
- Both registered families are HOST-CONSTRUCTED: `RealtimeDisplayMode` and `LightManagerSupport` subclasses are discovered reflectively and activated by the host, so the parameterless constructor is the contract and a program-taking constructor forecloses registration outright — the boundary shape is one abstract host adapter resolving its program from a token-keyed registry the registration entry seeds, never a constructed engine handed to the host.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/csharp/.api/api-rhinocommon.md`): the `Plane`/`Transform`/`BoundingBox` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/csharp/.api/api-languageext.md`): a `ChangeQueue`/`AsyncRenderContext`/`LightArray` `IDisposable` rides a `using` bounded by `Eff`; a nullable content resolution lifts to `Option<RenderContent>`; a `StartRenderThread` or `DeleteLight` `bool` projects to `Fin<Unit>`; an `Apply*` hook payload list crosses as `Seq<A>`.
...
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-render.md`: `RealtimeDisplayMode.StartRenderer` and `AsyncRenderContext.RenderWindow` consume the `RenderWindow` the batch catalog owns, the realtime engine writing `Color4f` pixels through its channels.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-display.md`: framebuffer and middleground events carry a `DisplayPipeline` and its `DisplayPipelineAttributes`, and `RealtimeDisplayModeClassInfo.RequiredDisplayTechnology` names a `DisplayTechnology`; the engine draws through the display pipeline's state stacks.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-rendercontent.md`: `MaterialFromId`/`EnvironmentForid`/`TextureForId` resolve `RenderMaterial`/`RenderEnvironment`/`RenderTexture`, and `MeshInstance.Decals`/`RenderMaterial` and `MappingChannel.Mapping -> TextureMapping` are content the content catalog owns.
- `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-document.md` and `libs/csharp/Rasm.Rhino/.api/api-rhinocommon-geometry.md`: `ViewInfo`/`ViewportInfo`/`RhinoDoc` bind the queue and engine, and `Rhino.Geometry.Mesh`/`Light`/`Transform`/`Plane`/`Vector2d`/`BoundingBox` are the geometry carriers a payload exposes.
