...
- Output: `Posterior.trace` retains the provider `DataTree`; `ConvergenceBar` folds against the `_RESIDUALS` table over the indispensable admission facts. A `metropolis` trace carries no `diverging` sample stat, so the measured ledger and its ceiling omit that dimension. Predictive fit is one `_score` fold behind the `loo_cells` pointwise budget, and the Pareto tail enters the convergence fold.
- Growth: a new distribution is one `Distribution` case and one `declare` arm usable in either role; a new sampler engine is one `SamplerBackend` case or one `external_nuts` name; a new convergence dimension is one `ConvergenceBar` field and one `_Residual`; a new predictive-scoring row is one `_score` arm behind its `SamplerPlan` policy field.
...
class _Residual:
    key: str
    measure: Callable[[Posterior], Option[float]]
...
class Posterior:
    ...
    @property
    def draws(self) -> int:
        return int(self.trace.posterior.sizes["chain"] * self.trace.posterior.sizes["draw"])
...
_RESIDUALS: Final[Block[_Residual]] = Block.of_seq([
    _Residual("max_rhat", lambda r: Some(r.max_rhat), lambda b: b.rhat_ceiling),
    _Residual("neg_min_ess_bulk", lambda r: Some(-r.min_ess_bulk), lambda b: -b.ess_floor),
...
class Inference:
    ...
    async def run(spec: InferenceSpec, lane: LanePolicy, *, composition: ScopeKey = DEFAULT_SCOPE) -> RuntimeRail[Posterior]:
        ...
        match settled:
            case Result(tag="ok", ok=outcome):
                return (await Journal.record(_metered(engine, spec.plan), scope=composition)).map(lambda _landed: outcome)
            case refused:
    ...
    def _fit(spec: InferenceSpec) -> "RuntimeRail[Posterior]":
        ...
        with pymc.Model() as model:
            ...
            trace = pymc.sample_posterior_predictive(
                trace,
                model=model,
                var_names=["observation"],
                random_seed=plan.seed,
                return_inferencedata=True,
                extend_inferencedata=True,
            )
        summary = arviz.summary(trace, var_names=names, kind="diagnostics")
        loo = _score(trace, plan, n_obs=int(np.ascontiguousarray(spec.observed).size))
        psense = arviz.psense_summary(trace)
        def settled(model_key: ContentKey) -> Posterior:
            return Posterior(
                trace=trace,
