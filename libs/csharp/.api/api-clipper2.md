...
- `Rasm`(`Meshing/delaunay#TESSELLATION`): each bounded dual cell's `CellReceipt` ring scales to a `Path64` at the fabrication precision, so a relaxed partition machines through `ClipperOffset` pocketing — the kernel `Tessellation.VoronoiDual` bounded-cell overload owns the clipped point-site diagram.
