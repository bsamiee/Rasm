...
- Tension: the `media/encode.md` page-authoring work-item has not landed (no `.planning` page exists yet), so the owner stays QUEUED until the `Media`/`MediaOp` `EncodeVideo`/`EncodeAudio`/`Mux` owner is authored against the present `.api/av.md`; `av`/PyAV is cp315-core with an abi3 wheel and bundled FFmpeg, so media encodes in-process synchronously inside the async boundary, never `anyio.to_process` and never a gated band, and the frame source is the `figures/scene` `SceneOp.Frames` orbit sequence so `MEDIA` is no longer a sink without a producer.
