...
- Growth: a new presence dimension lands as one `PresenceRowWire` field, never a parallel store; the TTL horizon unifies with the `standing-query/watermark#WATERMARK` frontier through the `frontier-gc#SOURCE_WIRING` `presenceReclaimable` projection — each presence row projects its `expiresAt` parsed to nanos into the one merged reclaimable feed, so an expired row is evicted by the unified frontier rule rather than a per-row `expiresAt` scan in this fold.
