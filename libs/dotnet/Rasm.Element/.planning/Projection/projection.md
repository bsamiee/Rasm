...
- Boundary: `ConstraintFinding.Key` and `ConstraintWaiver.Finding` carry `ContentAddress`; the contract preserves an unknown raised exception unchanged, and constraint identity projects numeric fault code with evidence.
...
public sealed record ProjectionContext {
    ...
    private ProjectionContext(FrozenSet<NodeId> elementIds, Header header, Instant at, CorrelationId correlation, TenantContext tenant, Seq<ConstraintWaiver> waivers) =>
        (ElementIds, Header, Key, At, Correlation, Tenant, Waivers) = (elementIds, header, at, correlation, tenant, waivers);

    public static Fin<ProjectionContext> For(Seq<NodeId> elementIds, Header header, Instant at, CorrelationId correlation, TenantContext tenant, Seq<ConstraintWaiver> waivers = default) =>
        correlation == CorrelationId.None
            ? new KernelFault.InvalidValue("projection correlation", "a minted correlation identity")
            : Fin.Succ(new ProjectionContext(elementIds.ToFrozenSet(), header, at, correlation, tenant, waivers));
...
public sealed record ConstraintFinding(ConstraintSeverity Severity, Error Violation, ContentAddress Key, Option<ConstraintWaiver> Waiver) {
    ...
    public static ConstraintFinding Of(ConstraintSeverity severity, Error violation, Seq<ConstraintWaiver> waivers) {
        ...
        return new(severity, violation, waivers.Find(waiver => waiver.Finding == key));
    }
...
public sealed record AssembledModel(ElementGraph Graph, GraphDelta Delta, Seq<ConstraintFinding> Findings);
...
public static class ProjectionAssembly {
    public static Fin<AssembledModel> Assemble(ProjectionSuite suite, ElementGraph seed, ProjectionContext ctx) =>
        suite.Projectors
            ...
            .Bind(merged => merged.AdmitOnto(seed, ctx.Key)
                .Bind(applied => Constrain(suite.Constraints, applied.Delta, applied.Graph, ctx.Waivers)
                    .Map(findings => new AssembledModel(applied.Graph, applied.Delta, findings))));
...
- Owner: `IGraphConstraint` the IFC-semantic legality floor with one `Validate`, Bim-implemented, composed in `Assemble` after the contract's structural admission.
...
- Entry: `Rasm.Bim` `Exchange/import#IMPORT_PIPELINE` decode arms construct it (one `Encode.Of` mint per decode over the lanes the source declares, one identity instance per block on a non-instanced source, one block per shading partition where the source splits a mesh by material) and `Rasm.Compute` `Runtime/tiles#TILE_PARTITION` slices baked leaves from it; `Bake()` is the ONE flatten — positions `Transform`, normals `TransformNormal`, every other lane copied rigid-invariant, each placed block inheriting its source `Material` — so a consumer needing world-space geometry calls the one owner and a consumer preserving instancing or shading partitions reads the overlay.
...
- Output: the arena's own `RoundTripWitness` — every mint measures per-lane quantization error against its dtype tolerance and carries the payload-rooted `GeometryHash` (`RoundTripWitness.Root` is `DigestRoot.Payload` on every arena this carrier holds, because a foreign decode carries no kernel `EncodeForm` source; a consumer keying dedup against source-rooted `Encode.Apply` digests must read `Root` before comparing), so a decode that silently lost precision fails `IsValid` rather than reaching a consumer; decode evidence stays with the Bim import path and partition evidence with the Compute tile lane.
...
- Growth: a new vertex attribute is one kernel `EncodingChannel` row — the arena addresses it by descriptor, `Bake` copies it by arity with no edit, and the tile partition and meshlet arm read it through `View<T>`, so the shared record gains NO column and no producing arm re-threads a buffer; a new placement semantic is one `MeshInstance` column and a new per-range fact one `MeshBlock` column; a source that begins splitting by material fills `Material` in its own decode arm with no contract edit; a per-format decode arm stays `Rasm.Bim`'s and a partition/encoding arm `Rasm.Compute`'s — the contract owns the SHAPE alone.
- Law: TWO vector types coexist by DELIBERATE CARVE, named here so neither drifts into the other's territory — the contract-owned `Graph/element#NODE_MODEL` `Vector3` is the ANALYTICAL/domain coordinate (the `AxisCurve`/`FootprintPolygon` shapes, the orientation classifier, the load-vector folds; it owns the domain algebra), while the BCL `System.Numerics` `Matrix4x4`/`Vector3` are INTERCHANGE-CARRIER LAYOUT types confined to `MeshInstance` and the `Bake` flatten, where the placement currency every decoder already speaks and the intrinsics the flatten runs on are the whole reason they are here. This carve forecloses a BCL vector crossing into analytical math and a shared `Vector3` appearing in an instance placement, standing as the declared exemption to the branch's one-coordinate law.
...
public sealed partial record ImportedGeometry(
    ...
    public static Fin<ImportedGeometry> Of(
        ...
        Seq<MeshBlock> blocks, Seq<MeshInstance> instances, Instant at) =>
        instances.Find(instance => instance.Block < 0 || instance.Block >= blocks.Count).Match(
            Some: rogue => Fin.Fail<ImportedGeometry>(new KernelFault.OutOfRange("mesh-instance-block", rogue.Block, $"fall inside [0,{blocks.Count})", Some())),
            None: () => Fin.Succ(new ImportedGeometry(formatKey, lanes, indices, vertexCount, triangleCount, blocks, instances, at)));

    public Fin<ImportedGeometry> Bake() {
        ...
        return Encode.Of(vertexTotal, toSeq(lanes).Map(static lane => (lane.Channel, lane.Placed)))
            .Map(arena => this with {
...
- [FINDING_EVIDENCE]: the `AssembledModel` carries the finding stream — an unwaived blocking finding aborts with the violations re-carried as `ManyErrors` (the `error.Filter<E>` recovery unchanged), while warnings and waived deviations ride `AssembledModel.Findings` typed, so "assemble with warnings" is a first-class outcome and a compliance dashboard folds findings off persisted `AssembledModel` values, never a log join.
