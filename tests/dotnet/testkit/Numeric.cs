...
public sealed partial class Norm {
    public static readonly Norm MaxAbs = new(static (rows, cols, at) => Numeric.Cells(rows, cols).Max(rc => Math.Abs(at(rc.Row, rc.Col))));
    public static readonly Norm L1 = new(static (rows, cols, at) => Enumerable.Range(0, cols).Max(c => Enumerable.Range(0, rows).Sum(r => Math.Abs(at(r, c)))));
    public static readonly Norm LInf = new(static (rows, cols, at) => Enumerable.Range(0, rows).Max(r => Enumerable.Range(0, cols).Sum(c => Math.Abs(at(r, c)))));
    public static readonly Norm Frobenius = new(static (rows, cols, at) => Math.Sqrt(Numeric.Cells(rows, cols).Sum(rc => at(rc.Row, rc.Col) * at(rc.Row, rc.Col))));
...
public static class Numeric {
    ...
    public static int OrientSign(double[][] simplex) {
        ...
        _ = admissible ? dim : throw new ArgumentException("OrientSign expects 3 finite 2D points or 4 finite 3D points", nameof(simplex));
    ...
    public static double ProductAt(int width, Func<int, int, double> left, Func<int, int, double> right, int row, int col) =>
        Dot(width, k => left(row, k), k => right(k, col));
    ...
    public static double EntrywiseResidual(int rows, int cols, Func<int, int, double> expected, Func<int, int, double> actual) =>
        Cells(rows, cols).Max(rc => Math.Abs(actual(rc.Row, rc.Col) - expected(rc.Row, rc.Col)));
    ...
    public static double ProductResidual(int rows, int width, int cols, Func<int, int, double> left, Func<int, int, double> right, Func<int, int, double> actual) =>
        EntrywiseResidual(rows, cols, (row, col) => ProductAt(width, left, right, row, col), actual);
    ...
    public static double FrobeniusDistance(Func<int, int, double> left, Func<int, int, double> right, int rows, int cols) =>
        Math.Sqrt(Cells(rows, cols).Sum(rc => { double d = left(rc.Row, rc.Col) - right(rc.Row, rc.Col); return d * d; }));
