...
| :-----: | :------------------------ | :------------ | :---------------------------------------------- |
|   [1]   | `ATTR_*` constants        | const string  | stable span and resource attribute name strings |
|   [2]   | `METRIC_*` constants      | const string  | stable metric instrument name strings           |
|   [3]   | `*_VALUE_*` constants     | const string  | enum value strings for bounded `ATTR_*` fields  |
|   [4]   | `SEMATTRS_*` constants    | const string  | deprecated legacy span attribute names          |
|   [5]   | `SEMRESATTRS_*` constants | const string  | deprecated legacy resource attribute names      |
...
| :-----: | :--------------------- | :----------------------------- | :-------------------------- |
|   [1]   | `ATTR_CLIENT_*`        | `ATTR_CLIENT_ADDRESS`          | client identity             |
|   [2]   | `ATTR_DB_*`            | `ATTR_DB_SYSTEM_NAME`          | database operations         |
|   [3]   | `ATTR_ERROR_*`         | `ATTR_ERROR_TYPE`              | error classification        |
|   [4]   | `ATTR_EXCEPTION_*`     | `ATTR_EXCEPTION_MESSAGE`       | exception recording         |
|   [5]   | `ATTR_HTTP_*`          | `ATTR_HTTP_REQUEST_METHOD`     | HTTP spans                  |
|   [6]   | `ATTR_NETWORK_*`       | `ATTR_NETWORK_PROTOCOL_NAME`   | network layer               |
|   [7]   | `ATTR_OTEL_*`          | `ATTR_OTEL_STATUS_DESCRIPTION` | OTel SDK metadata           |
|   [8]   | `ATTR_SERVER_*`        | `ATTR_SERVER_ADDRESS`          | server identity             |
|   [9]   | `ATTR_SERVICE_*`       | `ATTR_SERVICE_NAME`            | service identity            |
...
| :-----: | :------------------------------------ | :----------------------------- | :-------------- |
|   [1]   | `METRIC_DB_CLIENT_OPERATION_DURATION` | `db.client.operation.duration` | database client |
|   [2]   | `METRIC_HTTP_CLIENT_REQUEST_DURATION` | `http.client.request.duration` | HTTP client     |
|   [3]   | `METRIC_HTTP_SERVER_REQUEST_DURATION` | `http.server.request.duration` | HTTP server     |
...
| :-----: | :------------------------------------------------- | :-------------------------------------------------------------------- |
|   [1]   | `@opentelemetry/semantic-conventions`              | stable `ATTR_*`, `METRIC_*`, deprecated `SEMATTRS_*`, `SEMRESATTRS_*` |
|   [2]   | `@opentelemetry/semantic-conventions/experimental` | stable + experimental `ATTR_*` and `METRIC_*`                         |
...
| :-----: | :------------------------------- | :------------------------ |
|   [1]   | `ATTR_HTTP_REQUEST_HEADER(key)`  | `(key: string) => string` |
|   [2]   | `ATTR_HTTP_RESPONSE_HEADER(key)` | `(key: string) => string` |
