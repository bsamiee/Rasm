...
The decode-once law's type floor: one `Refined` vocabulary owner carries every cross-language branded primitive — Guid-v7, `OrdinalKey`, `JsonPointer`, BCP-47 `Locale` — and one `IngressBudget` owner carries the decode-budget ceilings plus the budget-gated schema combinator every untrusted seam composes. A raw `string` or `number` never occupies an identity slot past a decode seam: every wire shape, journal event, config read, and message catalog composes these schemas at its own field record, so each brand has exactly one edit site and every folder reads one nominal identity per concept. The module is `kernel/src/schema/brand.ts`; a new cross-language primitive is one interior anchor plus one owner row — never a per-folder re-declaration, never a standalone sibling export.
...
|  [02]   | `INGRESS_BUDGET` | decode-budget ceilings and the budget-gated schema seam  | `IngressBudget` |

## [2]-[REFINED_FLOOR]

[REFINED_FLOOR]:
- Owner: `Refined`, the assembled brand vocabulary — interior `_`-anchors carry the refinements, the exported owner assembles them under a `typeof`-derived stated annotation, and the merged `declare namespace Refined` hub serves every branded type off the single import, so `Refined.Guid` is the schema in value position and the branded type in type position.
...
- Law: `Refined.Locale` admits canonical BCP-47 only — the filter proves `Intl.getCanonicalLocales(tag)` returns exactly the input, so one spelling exists per locale and `ui/intl` catalogs key on it with zero normalize passes; canonicalizing a non-canonical inbound tag is a transform at the admitting seam.
- Law: derived surfaces stay truthful from the refinements alone — the pattern and range filters flow into `Arbitrary.make` and `JSONSchema.make`; the kernel-brand arbitraries live in `@rasm/ts-testkit` (`tests/typescript/_testkit`), never on a `@rasm/ts` subpath.
- Exemption: `_canonical` is a marked `// BOUNDARY ADAPTER` kernel — `Intl.getCanonicalLocales` throws `RangeError` on a malformed tag, and the `try`/`catch` folds that platform throw into the filter's `false` before anything leaves the kernel; the implementer carries the mark on its first line.
...
- Boundary: `ContentKey`, `Hlc`, `AppKey`, and `TenantKey` are richer identities owned by their own kernel pages; this floor carries only the primitives whose whole algebra is admission.
...
[INGRESS_BUDGET]:
- Owner: `IngressBudget`, the untrusted-ingress ceiling vocabulary — `floor` is the default budget row, and `bounded(schema, budget?)` composes the depth gate in front of any schema, so a JSON-bomb fails decode as a typed `ParseError` before it exhausts the runtime and the budget is recoverable from the seam declaration.
...
```typescript
declare namespace IngressBudget {
  type Shape = {
...
const _FLOOR: IngressBudget.Shape = { maxDepth: 64, maxFrames: 4096, maxAssembledBytes: 268435456 }
...
const _gate = (budget: IngressBudget.Shape): Schema.Schema<unknown, unknown> =>
...
const IngressBudget: {
  readonly floor: IngressBudget.Shape
  readonly bounded: <A, I, R>(schema: Schema.Schema<A, I, R>, budget?: IngressBudget.Shape) => Schema.Schema<A, unknown, R>
} = {
  floor: _FLOOR,
