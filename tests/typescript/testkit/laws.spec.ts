...
const _associativity = Law.make<Combine, typeof _ARGS>({
    ...
    witness: {
        label: 'subtraction foil',
        foil: (left, right) => left - right,
        args: { x: 1, y: 2, z: 3 },
    },
});
...
const _merged =
    ...
    async (schedule: FastCheck.Scheduler): Promise<boolean> => {
        ...
        const land = (value: number): Promise<void> =>
            schedule
                .schedule(Promise.resolve(value), `land ${value}`)
                .then((landed) => {
...
describe('equivalence law', () => {
    Law.register(it, (self: number, that: number) => self === that, [
        Law.equivalence({
            arb: _INTS,
            witness: {
                label: 'closeness pseudo-equivalence',
                foil: (self, that) => Math.abs(self - that) <= 1,
                args: { a: 0, b: 1, c: 2 },
            },
        }),
    ]);
...
describe('order law', () => {
    Law.register(it, Order.number, [
        Law.order({
            arb: _INTS,
            witness: {
                label: 'equality-refusing comparator',
                foil: (self, that) => (self <= that ? -1 : 1),
                args: { a: 1, b: 1, c: 1 },
            },
        }),
    ]);
