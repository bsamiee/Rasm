...
- `Rasm.Materials` `Projection/observability`: contributes the `LatencyPhase`/`LatencyMeasure` three-axis roster its bracketed constructions and accumulated quantities derive; `Rasm.Persistence` `Query/lane#READ_ROUTING` binds `ILatencyContextProvider`/`ILatencyContextTokenIssuer`/`ILatencyDataExporter` as the per-read phase ledger over `ReadPhase`; `Rasm.Rhino` and `Rasm.Grasshopper` bind the grammar attributes alone at their host capsules. Contributor names fold into the ONE `Rasm.AppHost` `LatencySpine.Register` registration (checkpoint/measure/tag axes under `ThrowOnUnregisteredNames`) — an unregistered name resolves to a positionless token whose writes drop silently, so a contributor ships its `LatencyRoster` with its binder.
