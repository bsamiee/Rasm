...
public abstract partial record ConvolveKernel {
    ...
    private static readonly double SupportPerSigma = Math.Sqrt(
        2.0 * -Math.Log(WeightKernel.Gaussian.Weight(distance: 1.0, support: PositiveMagnitude.Create(value: 2.0))) * 4.0);
...
- Law: EVERY Gaussian weight on this page is the kernel `Numerics/calculus#WEIGHT_PROFILES` `WeightKernel.Gaussian` profile, reached through `ConvolveKernel.Support`/`RangeSupport` alone — the bandwidth correspondence is READ off that row at `[02]` and no kernel body here restates one. The three-sigma halo sits strictly inside the resolved support, so no tap is zeroed by the row's own support cut and the truncation stays the page's declared one.
- Law: the SEPARABLE FOLD IS COMPOSED, never spelled. The two sample-domain axis passes call `TapSeries.Convolve`; the `[04]` height integration calls `Spectral.Transform` around its direct `TensorPrimitives.Multiply<Complex>` symbol application. Tap GENERATION stays here, because which weights fill the series is raster policy: the Gaussian table reads the kernel weight row at the support the bandwidth probe resolved, and it stages UNNORMALIZED, since the fold's resolved-weight divisor is the ONE partition-of-unity site — the renormalization that keeps a dropped border tap from darkening the rim lives at the owner, not in a page-local loop. The vertical pass hands the band's present run as a `TapWindow` under `TapBorder.Omit` — the fill already resolved the plane's edge law BY ADDRESS, so absence is the only law left, and under `Clamp` the absent slots sit at the staged run's own ends so the window simply narrows onto it; the horizontal pass folds whole rows under the page's one `EdgeMode`→`TapBorder` mapping, where `Clamp` maps to `Omit` because this page's clamp DROPS a tap from the weight sum rather than repeating the border texel — omission, never a zero-valued sample contributed under a retained coefficient. The band walk, the lane premultiply, the unsharp tail, and the stage scheduling stay this page's plane orchestration.
...
- Boundary: every loop-bearing member states its own KERNEL-EXEMPTION at the loop and each names the shape no span operator reaches — a strided gather, a clamped 2-D window, a data-dependent selection, a generator, or a side-effecting row accessor — while every whole-run elementwise crossing folds onto `TensorPrimitives` instead. Every admission, plan, schedule, and trace surface is expression-bodied; statements stop at the row kernel.
...
public abstract partial record PlaneOp {
    ...
    private static Fin<Seq<PlaneStage>> Schedule(PlaneShape input, Seq<PlaneOp> ops) =>
        ops.FoldM((Shape: input, Stages: Seq<PlaneStage>()), (carry) =>
...
internal static class PlaneKernel {
    ...
    private static Fin<double> Separable(PlaneBand window, TexturePlane source, TexturePlane destination, int layer, PlaneOp.Convolve op) {
        ...
        for (int row = 0; folded.IsSucc && row < window.Own; row++) {
            folded = TapWindow.Of(
                    extent: Dimension.Create(value: width), stride: Dimension.Create(value: lanes),
                    origin: 0, from: 0, run: Dimension.Create(value: width))
                .Bind(columns => series.Convolve(
                    vertical.Span.Slice(row * width * lanes, width * lanes),
                ...
                for (int lane = 0; lane < lanes; lane++) {
                    ...
                    for (int dy = -radius; dy <= radius; dy++) {
                        ...
                        for (int dx = -radius; dx <= radius; dx++) {
                            ...
                            double weight = WeightKernel.Gaussian.Weight(Math.Sqrt((dx * dx) + (dy * dy)), spatial)
                                          * WeightKernel.Gaussian.Weight(Distance(staging, at, centre, colour), range);
...
- Law: the divergence stages into one owned `Complex[]`, `Spectral.Transform` folds it over the plane's `CellLattice`, `TensorPrimitives.Multiply<Complex>` applies the Frankot-Chellappa symbol directly, and the inverse closes the round trip. What survives here is the Frankot-Chellappa divergence law alone: the inverse-Laplacian symbol with its zero bin held at zero, because that bin is the additive constant integration cannot recover and the evidence's mean restores it.
- Law: the bounded route assembles by TRIPLET ACCUMULATION and factors through the kernel's own SPD cache. Duplicate triplets sum and zeros drop at admission, so the Laplacian assembles by accumulation and never by hand-built compressed storage, and the factor carries the pivot-loss refusal onto the typed result rather than as a bare exception.
- Law: every INEXACT floor in this section derives from a NAMED kernel `Numerics/atoms#EPSILON_POLICY` row and states which one on site. The normal-z divisor guard takes the near-unit row, because `nz` is a component of a unit vector and that row gates every near-unit comparison in the branch; the reconstruction's spread guard takes the band-ulp row, the convergence floor no double iterate reaches below. A bare epsilon at a call site is the deleted form, and a floor whose magnitude no row explains is a tuning constant wearing a guard.
...
- Packages: `Rasm.Numerics` (composed — `Spectral.Transform`/`TransformDirection` the lattice transform, `Nabla.LatticeGradientAt`/`LatticeHessianAt` the ONE grid stencil, `CellLattice` the plane's own grid, `EpsilonPolicy.SqrtEpsilon`/`BandUlp` the two named floors this section's divisor and spread guards derive from, `SparseMatrix.FromTriplets` the accumulating assembly, `SparsePreconditioner.Milu0` and `SolveIterativeDetailed` the no-fallback Krylov solver, `CholeskySparse.Of`/`Solve` the SPD factor cache carrying the pivot-loss refusal, `Dimension`), `plane#TEXTURE_PLANE` (composed — `TexturePlane.Run`/`Grid`/`Read`/`Write`), `Rasm.Domain` (composed — `Deterministic.UnitInterval(Point3d, long, int)` the ONE replayable coordinate-keyed draw), CommunityToolkit.HighPerformance (`MemoryOwner<T>.Allocate` the solver staging, `SpanOwner<T>.Allocate` the row rental), System.Numerics.Tensors (`TensorPrimitives.Multiply<Complex>` the spectral modulation, `TensorPrimitives.Negate` the right-hand-side orientation, `TensorPrimitives.Min`/`Max`/`Sum` the reconstruction's reductions), BCL inbox (`System.Numerics.Complex`, `INumberBase<T>` the one divergence body over two stagings).
...
internal static class HeightField {
    ...
    private static Fin<HeightEvidence> Spectral(TexturePlane normal, TexturePlane height, HeightEvidence evidence) {
        ...
        return from forward in Rasm.Numerics.Spectral.Transform(
                   field, height.Grid, TransformDirection.Forward, MathNet.Numerics.IntegralTransforms.FourierOptions.Default)
               let symbol = InverseLaplacian(w, h)
               from admitted in Admit.FiniteComplexSpan(symbol)
                   ? Fin.Succ(unit)
                   : Fin.Fail<Unit>(new KernelFault.InvalidInput())
               from modulated in Try.lift(() => {
                   TensorPrimitives.Multiply<Complex>(field, symbol, field);
                   return unit;
               }).Run()
               from inverse in Rasm.Numerics.Spectral.Transform(
                   field, height.Grid, TransformDirection.Inverse, MathNet.Numerics.IntegralTransforms.FourierOptions.Default)
               select Restore(height, RealPart(field), evidence);
    }
    ...
    private static HeightEvidence Restore(TexturePlane height, ReadOnlySpan<double> solved, HeightEvidence evidence) {
        ...
        double spread = Math.Max(EpsilonPolicy.BandUlp, high - low);
