#!/usr/bin/env bash
# Attention feed for the forge-agents collector: one JSONL row per lifecycle event, carrying the emitting session's terminal identity so `forge-agents
# focus` routes a notification click back to the exact pane. Rows carry a `source` discriminator (hook here; the WezTerm bell arm appends source=bell
# rows on the same schema) so the collector folds per-source policy. Append-only; any failure exits 0 so the hook can never block the harness.
set -Eeuo pipefail
trap 'exit 0' ERR
