...
- Receipt: `VjpReceipt.contribute` emits one `Receipt.Emitted` row; the `exact` flag and the finite-difference step size are the evidence the C# graduation gate reads through the solver `HandoffAxis` case to admit or reject a sensitivity for kernel handoff.
- Packages: `jax` (`vjp`, `jacrev`, `grad`), `optimistix` (the implicit-adjoint `root_find`/`least_squares` consumed through the solver), `lineax` (the implicit-adjoint `linear_solve` consumed through the solver), `numpy` (`eye`, `column_stack`, `linalg.norm`), `solvers/receipt.md#RECEIPT` (the solver evidence the gradient annotates), runtime (`RuntimeRail`, `boundary`, `ReceiptContributor`).
...
- [JAX_VJP]: the `jax.vjp`/`jacrev`/`grad` spellings carry the `python_version<'3.15'` marker (no jaxlib cp315 wheel); the reverse-mode body verifies against the branch `.api` catalogue once the jaxlib wheel resolves. The finite-difference floor runs unconditionally on cp315.
