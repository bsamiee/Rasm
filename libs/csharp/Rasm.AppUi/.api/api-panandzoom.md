...
| :-----: | :---------------------- | :--------------------------------------------------------------------------------- |
|  [01]   | `StretchMode`           | `None`/`Fill`/`Uniform`/`UniformToFill`                                            |
|  [02]   | `ButtonName`            | `Left`/`Right`/`Middle`                                                            |
|  [03]   | `WheelBehaviorMode`     | `Zoom`/`PanVertical`/`PanHorizontal`/`None`                                        |
|  [04]   | `DoubleClickZoomMode`   | `ZoomIn`/`ZoomOut`/`ZoomInOut`/`ZoomToFit`/`None`                                  |
|  [05]   | `ContentBoundsMode`     | `Unrestricted`/`KeepContentVisible`/`FillViewport`/`KeepCentered`/`Custom`         |
|  [06]   | `ResizeBehaviorMode`    | `None`/`MaintainCenter`/`MaintainTopLeft`/`MaintainZoom`/`ReapplyStretch`/`Custom` |
|  [07]   | `ZoomIndicatorPosition` | `TopLeft`/`TopRight`/`BottomLeft`/`BottomRight`/`Custom`                           |

- `StretchMode`: Content-fit policy aligned with Avalonia `Stretch`
- `ButtonName`: Pan-drag mouse button
- `WheelBehaviorMode`: Base wheel policy with `WheelWithCtrl` and `WheelWithShift` modal overrides
- `DoubleClickZoomMode`: Double-click intent
- `ContentBoundsMode`: Policy feeding `BoundsMode` and `BoundsPadding`
- `ResizeBehaviorMode`: Policy feeding `ResizeBehavior`
...
|  [11]   | `GetNextDiscreteZoomLevel()`                        | next discrete zoom level     |
|  [12]   | `GetPreviousDiscreteZoomLevel() -> double`          | previous discrete zoom level |
|  [13]   | `SnapToGrid(double value) -> double`                | `GridSize` snapping          |
