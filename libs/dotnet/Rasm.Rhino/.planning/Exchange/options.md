...
- Packages: Generator.Equals (`[Equatable]`, `[OrderedEquality]`), LanguageExt.Core (`Fin`, `Option`, `Validation`, `Seq`), `Domain/validation` (`CapabilitySet`, `FactoryValidation`), `Domain/results` , `Domain/context` (`Tolerance`, `ToleranceLane`), `Rasm.Drawing` (`HostLayerScheme`, `LayerStandard`, `PlotStyleTable`), `Document/geometry` (`FieldOverride<T>`); RhinoCommon `Rhino.FileIO` per `.api/api-rhinocommon-fileio.md`.
