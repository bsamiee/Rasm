...
│   ├── Algebra.cs      # Clipper2 line-space owner: offset, DeltaCallback64 variable offset, Boolean/open-path clip, signed Area, Minkowski sum/diff, containment-depth NestingOrder (arc space is Arcs.cs's)
...
│   ├── Remnant.cs      # Offcut lifecycle partial — Remnant.Reconcile(Seq<Remnant>, RemnantInventory) → RemnantPlan batch seam + Claim/Release/Sweep op overload, RemnantState transitions, ReusePolicy admission, Stockable re-mint into the next Inventory
...
│   ├── Dialect.cs      # per-dialect Emit generated total Switch over the PostDialect grammar family; canned-cycle/macro/subprogram lowering; NC1 emit target
...
│   ├── Removal.cs      # PicoGK BoolSubtract voxel material-removal verify → gouge/uncut/overcut/air-cut receipts, ResidualStock + per-setup StockSnapshots
...
│   ├── Tolerance.cs    # ISO 286 generated fits + ISO 1101/ASME Y14.5 FCF/datums + ISO 1302 finish, QIF-aligned; Ra→scallop and IT→allowance derivation rows
...
Toolpath/surface         ←  csharp:Rasm/Processing            # [SHAPE]: geodesics/extract/flow/segment on-mesh path LAYOUT (K10-K13); Spatial/fields SDF the recorded OCL fallback (K8); Simplify.Apply → DecimationResult + Remeshing.Apply(RemeshOp.Isotropic) → RewriteResult LOD/conditioning pre-pass (K38 — declared, rides row 22)
...
Spec/manufacturability   ←  csharp:Rasm/Analysis+Spatial      # [SHAPE]: Faces.Top/Bottom/At + Curves.Draft (select.md:311+, K20) · Meshes census Validity/Counts/Defects/Quality/NakedEdges (inspect.md:257-266, K36) · RayQuery(Ray3d, MaxReflections) (relations.md:57) + SpatialQuery.Ray/.Winding GWN (index.md:101,:104, K29/K30) · the K1 medial clearance RADIUS min-wall probe
...
Verify/probing           ←  csharp:Rasm/{Solving/fit, Processing/sample, Spatial/neighbors} # [WIRE-DECLARED, rides row 31]: Fin<FitReceipt> Fit.Apply(FitOp, Context, Op?) — FitKind plane/sphere/cylinder/cone/torus/line, FitReceipt(Primitive, Inliers, Residual, Consensus, Trials, Iterations) (K26) · 12-case SampleKind inspection sampling (K37) · NeighborIndex.Of(NeighborSource, Op?) kNN scan front (declared beside the K-table)
Verify/probing           ←  csharp:Rasm/Analysis/measure      # [WIRE]: ConformanceMetric over ResidualSample — measured-vs-nominal verdicts (K17)
Posting/{program, optimization} ← csharp:Rasm/Domain/normalization # [SHAPE-DECLARED]: public CurveForm union (normalization.md:158) — native arc/line classification for compaction arc-fit (K34)
...
|  [10]  | `Ingress/profile ← Rasm.Bim/Exchange` | shared read pin | ACadSharp read codec; the two-format WRITE leg is the AppUi ACadSharp leg |
