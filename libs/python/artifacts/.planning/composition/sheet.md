...
- [02]-[SHEET]: `Sheet` owns the single-sheet title-block/frame/field-binding surface over the closed `SheetOp` `tagged_union` (`Frame`/`Place`/`Fill`/`Stamp`/`Preview`) folded once into a `Composed` evidence struct the `folded()` successor threads, railed `RuntimeRail[ArtifactReceipt]` over `async_boundary(catch=_FAULTS)`, dispatched to the `reportlab`/`typst`/`weasyprint` frame authors under one `PdfProfile` and to `pymupdf` for figure placement, OCG binding, title-block fill, preview, and stamp; peer owner `SheetSet` assembles `tuple[SheetIssue, ...]` over the same `TitleBlock`/`Iso7200Field` vocabulary — numbering each `drawing/regime#REGIME` `SheetId` across the set, auditing ISO 7200 conformance, and projecting the numbered set to `delivery/register#REGISTER` and `visualization/table#TABLE` as a synchronous projection minting no receipt, exactly as `drawing/regime#REGIME`.
...
class Sheet(Struct, frozen=True):
    ...
    def emit(self, /) -> ArtifactWork:
        ...
        # payload and opens a `content.derive` span per read, so the plan node and its receipt must not each pay it.
        key = self._key
    ...
    async def _folded(self, key: ContentKey, /) -> ArtifactReceipt:
        ...
        # GIL — and the receipt derives from that one fold; the terminal threads the PRE-RUN key the closure captured.
        rail = await self.lane.offload(Kernel.of(_composed, KernelTrait.HOSTILE), self.op)
        return self._receipt(key, rail.default_with(_sheet_raise))
