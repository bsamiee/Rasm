...
# ///
# PreCompact extractor: compaction discards the working set, so summarize the transcript into a durable handoff before it is lost.
...
class Line(msgspec.Struct, frozen=True):  # tolerant view over one heterogeneous JSONL record
    type: str = ""
    tool_name: str = ""
    is_error: bool = False
    file_path: str = ""
...
def main() -> int:
    try:
        payload = msgspec.json.decode(sys.stdin.buffer.read(), type=PreCompact)
    except msgspec.DecodeError:
