...
- format-delegate seam (`core/format#FORMAT`): `TemplatePipeline` binds `TemplateTarget.TOML` -> `DELEGATES[TOML] = (DocumentMode.TOML, _bare_spec)` and threads it through the emit owner's `produced` entrypoint, so the same bound `DocumentNode` tree a spec-sheet renders to DOCX/PDF/HTML also egresses as a TOML structured-data form — one owner bound many ways, the TOML producer held inside the emit owner, never re-authored in the pipeline.
...
- both-tier rails: the emit/recover compose under the shared substrate — `msgspec.to_builtins`/`from_builtins` (`libs/python/.api/msgspec.md`) is the structure<->builtins bridge feeding `dumps` and consuming `unwrap()`; `expression.Result[T, E]` (`libs/python/.api/expression.md`) lifts a `ParseError`/`ConvertError` to a typed `Error` arm; `@beartype` (`libs/python/.api/beartype.md`) guards the boundary signatures; a structured-documents receipt records the table/key/AoT tally and output byte length; `ParseError.line`/`.col` feed a positional config diagnostic into the `structlog` chain (`libs/python/.api/structlog.md`) rather than a flattened message; a network-fetched config source (never the in-memory default) wraps `stamina.retry` (`libs/python/.api/stamina.md`) at the fetch boundary, the parse itself being pure and retry-free.
