...
│   ├── repair.py             # MeshRepairOp: watertight repair, winding/normal fix, manifold3d boolean over Trimesh; mesh-file IO is the data MeshPayload seam
│   ├── brep.py               # BrepOp: cadquery-ocp B-rep evaluation, manifold3d solid algebra, mesh-brep subject
│   ├── spatial.py            # MeshSpatial: trimesh+numpy proximity/ray/contains/AABB (runtime spine); FCL/manifold3d.min_gap clearance is the offloaded worker
...
ifc             ←  csharp:Rasm.Bim/Semantics    # [WIRE]: GeoFeature WKB decode via shapely (NTS-equivalent planar peer)
...
mesh/daemon     →  python:runtime/evidence      # [CONTENT_KEY]: ContentIdentity.of source+policy-seed key; seed-zero (Some(0)) over GLB bytes = wire key
...
energy/*        →  python:compute/graduation    # [GRADUATION]: building-energy / thermal-comfort GeometryHandoff evidence
```
