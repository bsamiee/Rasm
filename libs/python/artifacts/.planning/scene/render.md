...
class Scene3d(Struct, frozen=True):
    ...
    def _key(self) -> ContentKey:
        # key-over-INPUT: canonical SceneOp payload minted PRE-RUN so keyed admission probes the warm
        # seed BEFORE any VTK render runs — the produced bytes' own content address rides the receipt FACTS.
    ...
    async def _rendered(self) -> ArtifactReceipt:
        # every arm crosses the runtime PROCESS lane (VTK is GIL-hostile native); the terminal receipt threads
        # the PRE-RUN input key (receipt.slot == node.key) and stamps the produced-output address on the facts.
        key = self._key
        match self.op:
            ...
            case SceneOp(tag="export", export=(grid, target, spec)):
                ...
                # `facts` carries the `scene/stage#STAGE` `ComputeUsdStageStats` prim/layer/up-axis/meters-per-unit/extent band on the USD arms
