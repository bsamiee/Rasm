...
[STACKING]:
- `Rasm.Contracts`(`Rasm.Contracts/.api/rasm-contracts.md`): the pinned `buf.build/grpc/csharp` remote plugin emits the `<Svc>.<Svc>Base` class `MapGrpcService<TService>` binds into that committed assembly, so an app root derives the base by project reference and no project drives a codegen pass of its own.
...
- `Microsoft.AspNetCore.TestHost`(`tests/dotnet/.api/aspnetcore-testhost.md`): `TestServer.CreateHandler()` feeds `GrpcChannelOptions.HttpHandler`, dialing a mapped endpoint with no socket.
