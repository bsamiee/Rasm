...
It carries the `Keys.Cryptography` algorithm vocabularies (`KeyWrapAlgorithm`, `EncryptionAlgorithm`, `SignatureAlgorithm`) plus the `KeyVaultKey`/`JsonWebKey` material model. It is the `KmsProvider.AzureKeyVault` arm of the `Store/encryption#KEY_ENVELOPE` `EnvelopeKeyring` — the single admitted KMS provider whose `CryptographyClient` exposes a **native** `WrapKey`/`UnwrapKey` verb pair (`RSA-OAEP-256` over the vault CMK), so the Azure arm wraps a DEK directly rather than through the `Encrypt`/`Decrypt`-as-wrap shim the AWS-KMS and GCP-KMS arms (`GenerateDataKey`/`Decrypt`/`ReEncrypt`) require. The `CryptographyClient(JsonWebKey)` local-only constructor is the path for an offline/cached wrap that performs no vault round-trip.
...
- NATIVE WRAP: unlike AWS KMS and GCP KMS (which expose no wrap verb, forcing `Encrypt`/`Decrypt`/`ReEncrypt`-as-wrap), `CryptographyClient.WrapKey(KeyWrapAlgorithm, dek)` / `UnwrapKey` is a first-class vault verb. The Azure `EnvelopeKeyring` arm therefore wraps a DEK directly with `RSA-OAEP-256`; the `Store/encryption` page's "KMS has no native wrap verb" law is the AWS/GCP arm, not this one.
...
[LOCAL_ADMISSION]:
- This package is the `azure-keyvault` arm of the `Store/encryption#KEY_ENVELOPE` `KmsProvider` `[SmartEnum<string>]`. Its `EnvelopeKeyring` delegate triple binds: `Mint` calls `CryptographyClient.WrapKey(KeyWrapAlgorithm.RsaOaep256, dek)` over a freshly generated DEK, `Unwrap` calls `UnwrapKey` and zeroes the recovered `UnwrapResult.Key` after the local bind, and `Rotate` re-wraps the persisted blob under a `destinationKeyId` — the native wrap verb means rotation is a `WrapKey` against the new version, not a decrypt-then-encrypt round trip, and the plaintext DEK never re-enters managed memory. The master key never leaves the vault.
...
[RAIL_LAW]:
- Package: `Azure.Security.KeyVault.Keys`
- Owns: master-key custody and **native** key-wrap cryptography for the `azure-keyvault` arm of the store-encryption `EnvelopeKeyring`
- Accept: `CryptographyClient.WrapKey`/`UnwrapKey` for the DEK envelope, the `CryptographyClient(JsonWebKey)` local path for an offline unwrap, `KeyClient` for master-key CRUD/rotation, `KeyWrapAlgorithm`/`EncryptionAlgorithm` well-known constants for algorithm selection, `Azure.Core` `Response<T>`/`Pageable<T>` carriers converted once at the boundary
- Reject: exporting master key material (except the explicit attested `ReleaseKey` path), `Encrypt`/`Decrypt`-as-wrap when the native `WrapKey` verb exists, local key-wrap reimplementation, per-operation client construction, raw key-URI strings inside internal code, `RequestFailedException` leaking past the keyring boundary
