...
- Entry: `BimHooks.Live(gates, taps, band, cell)` mints the dispatcher once at composition; an emitting page fires its declared row (`hooks.Fire(BimPoint.Imported, fact)`), and the guarded arity hands its body the ADMITTED fact so a veto transform governs the contract it brackets. `Points` is the census the app root hands `HookRegistry.Mount`, and `BimPoint.Scopes` enters that root's `SpanBand.Of(version, scopes)`.
...
public sealed partial class GlobalIdSet {
    ...
    public static Fin<GlobalIdSet> Admit(ImmutableArray<string> values) =>
        WireSet.Ordered(values) && TryCreate(toSeq(values), out GlobalIdSet? admitted) && admitted is { } set
            ? Fin.Succ(set)
            : Fin.Fail<GlobalIdSet>(new BimFault.Refused(BimScope.Events, BimReason.Codec, string.Join(':', new object?[] { "event-set-malformed", "globalIds" })));
...
public static class BimHooks {
    public static Fin<BimHooks> Live(Seq<BimGate> gates = default, Seq<BimObserver> taps = default,
        Option<SpanBand> band = default, Option<FaultCell> cell = default) =>
        BimHooks.Of(gates, taps, band.Map(static span => (IHookSpan)span), cell)
            ;
}
```
...
public static class BimTelemetry {
    public static BimObserver Tap(InstrumentSet set) => new(fact => Project(set, fact));

    public static Fin<Seq<IDisposable>> Depth(InstrumentSet set, BimHooks hooks) =>
    ...
    public static Fin<T> Traced<T>(
        SpanBand? band, BimPoint at, string model, Func<Fin<T>> body, params ReadOnlySpan<(string Slot, object? Value)> marks) {
        Seq<(string Slot, object? Value)> stamps = (BimInstrument.ModelSlot, (object?)model).Cons(toSeq(marks.ToArray()));
        return band is null
            ? body()
            : at.Plane.Match(
                Some: plane => band.Traced(plane, span => (Stamped(span, stamps), body()).Item2),
                None: body);
    }
...
public static class BimBenchClaims {
    ...
    internal static Fin<Unit> Proof() =>
        toSeq(InterchangeFormat.Items)
            .Filter(static format => !format.Capabilities.Admits(InterchangeCapability.Import))
            .Map(static format => (Error)new KernelFault.InvalidValue(
                Label: format.Key, Requirement: "the import capability every bench claim binds"))
