...
- [04]-[PALETTE]: Overlay owns the `Overlay.Command` vocabulary — scope, derived grant legality, keymap census, hosting shells, the copy port; `Overlay`.
...
- Law: the `size.apply` style write is floating-ui's platform-forced statement site — the middleware hands a live element and the write is the documented application; the kernel carries the exemption.
...
- Packages: `cmdk` (`Command` compound, `CommandDialog`, `Command.Loading`, `useCommandState`, `defaultFilter`); `lucide-react` (`LucideIcon` — icon-as-identity); `react-aria` (`KeyboardShortcutBindings` — the record `system/act#DISCRETE_ROWS` feeds `useKeyboard`); `react-stately` (`MenuTriggerType`'s `contextMenu` arm, `OverlayTriggerState.point`/`setPoint`); `react-aria-components` (`MenuTrigger` — the aria-menu arm); `system/intl` (labels, `useFilter` pre-normalization where locale-sensitive); `system/primitive` (`Clipboard` — the copy port); `@tanstack/react-virtual` + `@floating-ui/react` `inner`/`useInnerOffset` (the virtualized lane).
...
- Law: coordinates arrive projected, and the projection answers `Option` — `project` maps a `Presence.Point` (tagged `Sheet` or `Scene`, each carrying its own surface id) into viewport coordinates or `Option.none` for a point no registered space resolves, and the fold DROPS an unresolvable actor rather than rendering at a fabricated coordinate; `view/presence#CURSOR_PLANE`'s `Face.project` over the assembled anchor lanes is the supplying port, so a map surface's viewer projection and a plain sheet feed the same cohort and this page holds no coordinate math and no surface branch.
