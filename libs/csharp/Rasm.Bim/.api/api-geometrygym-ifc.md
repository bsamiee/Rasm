...
| [06] | `IfcZone` (no `PredefinedType`) | geometry | functional-zone grouping carries NO predefined enum in GeometryGym (`IfcZone`/`IfcSystem`/`IfcGroup` expose only `LongName`/`Name`); there is no `IfcZoneTypeEnum` — only `IfcSpatialZone`/`IfcDistributionSystem`/`IfcStructuralLoadGroup` carry a predefined kind |
...
| [12] | `IfcMaterialLayerSetUsage` | geometry | layer-set application to an element; `ForLayerSet` (`IfcMaterialLayerSet`), `LayerSetDirection` (`IfcLayerSetDirectionEnum`), `DirectionSense` (`IfcDirectionSenseEnum`), `OffsetFromReferenceLine` (`double`), `ReferenceExtent` (`double` — the layer-set size perpendicular to the layers, the 4th arg the seam `MaterialUsage.LayerSet` carries) |
...
| [19] | `IfcMaterialLayer` | geometry | one layer in `IfcMaterialLayerSet.MaterialLayers` (`: IfcMaterialDefinition`); `Material` (`IfcMaterial`), `LayerThickness` (`double`), `Priority` (`int`), `Category` (`string`), `IsVentilated` (`IfcLogicalEnum`); ctor `(IfcMaterial, double thickness, string name)` — the `Priority`/`Category`/`IsVentilated` columns are member PUBLIC and await the seam `MaterialWire` column widening (frozen this campaign) |
...
| [14] | `IfcStructuralLoadConfiguration` | geometry | the IFC varying line action (`: IfcStructuralLoad`): public `Values` (`LIST<IfcStructuralLoadOrResult>`), `Locations` (`List<List<double>>`), ctors `(val, length)` / `(vals, locations)` / `(val1, loc1, val2, loc2)` — the trapezoid lowering source `Model/structural#STRUCTURAL_PROJECTION` `Vectors` reads |
| [15] | `IfcRelConnectsStructuralMember` | geometry | connects an idealized member to a connection; `RelatingStructuralMember`, `RelatedStructuralConnection` |
| [16] | `IfcRelConnectsStructuralActivity` | geometry | binds a load/result activity to a structural item |
| [17] | `IfcRelAssignsToGroup` | geometry | assigns objects to an `IfcGroup`/`IfcSystem`/`IfcStructuralLoadGroup`; `RelatedObjects` (`SET<IfcObjectDefinition>`, the assigned members reached via `IfcGroup.IsGroupedBy`) |
...
| [03] | `IfcBuiltSystem` | geometry | built-system grouping (`: IfcSystem`); `PredefinedType` (`IfcBuiltSystemTypeEnum`) — the IFC4.3 form GG SHIPS; `IfcBuildingSystem` is ABSENT from the assembly (member), so a catalog or roster row naming it is a phantom |
| [04] | `IfcDistributionSystem` | geometry | MEP distribution system; `PredefinedType` (`IfcDistributionSystemEnum`), member element set |
| [05] | `IfcZone` | geometry | functional zone aggregating spaces across storeys; `LongName` + ctor `(IfcSpatialElement, string, List<IfcSpace>)` — carries NO `PredefinedType` in (see enum row `IfcZone`) |
...
| [08] | `IfcClassification` | geometry | the classification source dictionary `IfcClassificationReference.ReferencedSource` names (Uniclass2015/OmniClass); `Name` (`string`, the dictionary title `Ingest` matches a roster row on), `Source`/`Edition` (`string` publisher + edition the seam `Classification` lowers as the `Source` annotation + the `Edition` IDENTITY token), `EditionDate` (`DateTime`, `DateTime.MinValue` unset — the seam `Classification.EditionDate` annotation), `Specification` (`string`, the IFC4X3 rename of the `[Obsolete]` `Location` dictionary URI) |
...
| [05] | `IfcFastener` | geometry | `IfcElementComponent` (the non-mechanical sibling of `IfcMechanicalFastener`); only public scalar `PredefinedType` (`IfcFastenerTypeEnum`) — the welded/bonded bead, no public diameter |
| [06] | `IfcFastenerTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`GLUE`/`MORTAR`/`WELD` — the non-mechanical-fastener kinds; `WELD` is the weld bead (Welded joint), `GLUE`/`MORTAR` the adhesive/mortar bond (Bonded joint) |
| [07] | `IfcDiscreteAccessory` | geometry | `IfcElementComponent` (the fabricated accessory sibling of `IfcMechanicalFastener`); public `PredefinedType` (`IfcDiscreteAccessoryTypeEnum`) — the framing-connector saddle/bracket/anchorplate body, itself fastened by a separate `IfcMechanicalFastener` related via `IfcRelConnectsWithRealizingElements` |
| [08] | `IfcDiscreteAccessoryTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`ANCHORPLATE`/`BRACKET`/`SHOE`/`EXPANSION_JOINT_DEVICE`(4x2)/4x3 rail set (`BIRDPROTECTION`/`CABLEARRANGER`/`INSULATOR`/`LOCK`/`TENSIONINGEQUIPMENT`/`RAILPAD`/`SLIDINGCHAIR`/…) — `SHOE`/`BRACKET`/`ANCHORPLATE` the steel framing-connector classes the `HangerType.IfcAccessoryType` token names |
| [09] | `IfcReinforcingElement` | geometry | abstract reinforcing root under `IfcElementComponent`; `SteelGrade` carrier for the bar/mesh subtypes |
| [10] | `IfcReinforcingBar` | geometry | `IfcReinforcingElement`; public `NominalDiameter` (type-fallback get) / `CrossSectionArea` / `BarLength` (`double`), `PredefinedType` (`IfcReinforcingBarTypeEnum`), `BarSurface` (`IfcReinforcingBarSurfaceEnum`) |
| [11] | `IfcReinforcingBarTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`MAIN`/`SHEAR`/`LIGATURE`/`STUD`/`PUNCHING`/`EDGE`/`RING`/`ANCHORING`/`SPACEBAR`(4x2) — `STUD` is the cast-in bar, NOT the welded connector |
| [12] | `IfcReinforcingBarSurfaceEnum` | geometry | `NOTDEFINED`/`PLAIN`/`TEXTURED` — the bar surface finish the `IfcReinforcingBar.BarSurface` scalar carries (PLAIN = smooth, TEXTURED = deformed/ribbed) |
| [13] | `IfcReinforcingMesh` | geometry | `IfcReinforcingElement`; public `PredefinedType` (`IfcReinforcingMeshTypeEnum`) + `MeshLength`/`MeshWidth`/`LongitudinalBarNominalDiameter`/`TransverseBarNominalDiameter`/`LongitudinalBarCrossSectionArea`/`TransverseBarCrossSectionArea`/`LongitudinalBarSpacing`/`TransverseBarSpacing` (`double`, `NaN`-defaulted, dropped finite at the reader `Filter`) |
| [14] | `IfcReinforcingMeshTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED` — the welded-mesh kind on `IfcReinforcingMesh.PredefinedType` |
| [15] | `IfcTendon` | geometry | `IfcReinforcingElement`; public `PredefinedType` (`IfcTendonTypeEnum`) — the post-tensioning prestressing element peer of the bar/mesh; `mNominalDiameter`/`mCrossSectionArea`/`mTensionForce`/`mPreStress`/`mFrictionCoefficient`/`mAnchorageSlip`/`mMinCurvatureRadius` are `internal` (no public getter), diameter/area set only via the authoring ctor `(IfcObjectDefinition host, IfcObjectPlacement, IfcProductDefinitionShape, double diam, double area)`, the READ diameter recovered through the `IfcCircleProfileDef.Radius` profile channel (same as the mechanical fastener) |
| [16] | `IfcTendonTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`STRAND`/`WIRE`/`BAR`/`COATED` — the tendon kind on `IfcTendon.PredefinedType`, all `Cast` joints |
| [17] | `IfcTendonAnchor` | geometry | `IfcReinforcingElement`; public `PredefinedType` (`IfcTendonAnchorTypeEnum`), ctor `(IfcObjectDefinition host, IfcObjectPlacement, IfcProductDefinitionShape)` — the tendon anchorage device, a `Cast` joint |
| [18] | `IfcTendonAnchorTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`COUPLER`/`FIXED_END`/`TENSIONING_END` — the tendon-anchor kind on `IfcTendonAnchor.PredefinedType` |
| [19] | `IfcBearing` | geometry | `IfcBuiltElement`; public `PredefinedType` (`IfcBearingTypeEnum`), public ctors `()`/`(DatabaseIfc)`/`(IfcObjectDefinition host, IfcObjectPlacement, IfcProductDefinitionShape)` — the structural support bearing, a `Bearing` joint (a built element, NOT an `IfcElementComponent`) |
| [20] | `IfcBearingTypeEnum` | geometry | `NOTDEFINED`/`USERDEFINED`/`CYLINDRICAL`/`SPHERICAL`/`ELASTOMERIC`/`POT`/`GUIDE`/`ROCKER`/`ROLLER`/`DISK`(4x2) — the bearing kind on `IfcBearing.PredefinedType` |
| [21] | `IfcMaterialProfileSetUsage` | geometry | binds an `IfcMaterialProfileSet` to an element via `ForProfileSet`; `CardinalPoint` (`IfcCardinalPointReference`, default `MID`) + `ReferenceExtent` (`double`, default `NaN`) carry the profile placement the `Semantics/composition` `ProfileSetUsage` reciprocal reads, and the fastener/tendon nominal-diameter circle-profile channel |
| [22] | `IfcCircleProfileDef` | geometry | parametric circular profile; public `Radius` (`double`) — the fastener/tendon nominal-diameter carrier reached through `IfcRelAssociatesMaterial.RelatingMaterial` |
...
| [06] | `new IfcFastener` | `(IfcObjectDefinition host, IfcObjectPlacement placement, IfcProductDefinitionShape rep)` | the non-mechanical-fastener occurrence ctor (the weld/adhesive bead the `Emit` authors); `PredefinedType` (`IfcFastenerTypeEnum`) set after |
| [07] | `new IfcDiscreteAccessory` | `(IfcObjectDefinition host, IfcObjectPlacement placement, IfcProductDefinitionShape rep)` / `(IfcProduct host, IfcMaterialProfileSetUsage profile, IfcAxis2Placement3D placement, double length)` | the framing-connector occurrence ctor pair (same shape as `IfcMechanicalFastener`); `PredefinedType` (`IfcDiscreteAccessoryTypeEnum` `SHOE`/`BRACKET`/`ANCHORPLATE`) set after |
| [08] | `IfcElement.HasAssociations` | `SET<IfcRelAssociates>` property | the association set carrying `IfcRelAssociatesMaterial` — the read path to the fastener's profile-usage nominal diameter |
| [09] | `IfcRelAssociatesMaterial.RelatingMaterial` | `IfcMaterialSelect` property (`IfcMaterialProfileSetUsage` arm) | the material-profile chain the `DiameterOf` projection narrows to recover the fastener nominal scalars |
| [10] | `IfcMaterialProfileSetUsage.ForProfileSet` | `IfcMaterialProfileSet` → `MaterialProfiles` (`IfcMaterialProfile` list) → `Profile` | the profile chain to the `IfcCircleProfileDef.Radius` carrying the fastener nominal diameter (radius × 2) |
| [11] | `IfcReinforcingBar.NominalDiameter` | `double` property (get falls back to `IfcReinforcingBarType.NominalDiameter`) | the public bar diameter the `Semantics/connection#CONNECTION_DETAIL` `ConnectionProjection.Detail` reads onto the seam connection-detail bag |
| [12] | `IfcReinforcingBar.BarSurface` | `IfcReinforcingBarSurfaceEnum` property (get/set) | the bar surface finish (`PLAIN`/`TEXTURED`) the `ConnectionProjection.Detail` `BarDetail` arm reads onto the seam `BarSurface` token |
