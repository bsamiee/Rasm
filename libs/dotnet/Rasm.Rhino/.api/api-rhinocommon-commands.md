...
- `Rhino.Input.RhinoGet.GetFileName(GetFileNameMode mode, string defaultName, string title, object parent) -> string` — file-name dialog acquisition; cancellation returns non-admitted empty text rather than a `Result`. A five-argument overload adds `BitmapFileTypes fileTypes`, and the four-argument form forwards to it with every type admitted; a `null` parent resolves to the main Rhino window rather than a parentless dialog.
- `Rhino.Input.RhinoGet.GetFileNameScripted(GetFileNameMode mode, string defaultName) -> string` — the WINDOWLESS counterpart: no caption, no parent, no bitmap-type argument, and it answers text on the same empty-means-cancelled contract. Title presence is therefore the route discriminant between the two, and the untyped `object parent` never has to reach a caller signature.
...
[TRANSFORM_BUFFER]:
- `Rhino.Collections.TransformObjectList` — `IDisposable`; the namespace is `Rhino.Collections`, NOT `Rhino.Input.Custom`, so a using-directive over the getter namespace alone does not reach it.
- `TransformObjectList.AddObjects(GetObject go, bool allowGrips) -> int` — folds a COMPLETED object getter into the buffer and answers the added count; this is the member that populates a drag set, and there is no `AddTransformObjects` on this type.
- `TransformObjectList.Add(RhinoObject) -> void` / `Add(ObjRef) -> void` — single-row appends.
- `TransformObjectList.ObjectArray() -> RhinoObject[]` / `GripArray() -> GripObject[]` / `GripOwnerArray() -> RhinoObject[]` — the three census projections.
- `TransformObjectList.Count` / `GripCount` / `GripOwnerCount` — the three tallies, each independent of the arrays.
- `TransformObjectList.GetBoundingBox(bool regularObjects, bool grips) -> BoundingBox` — one measured extent over the chosen population.
...
[STACKING]:
- `RhinoCommon` value substrate(`libs/dotnet/.api/api-rhinocommon.md`): the `Point3d`/`Vector3d`/`Plane`/`Line`/`Transform` carriers this boundary threads cross the wire from the substrate; it composes them and re-derives none.
- `LanguageExt.Core`(`libs/dotnet/.api/api-languageext.md`): a successful `Result`/`GetResult` payload projects to `Fin<A>.Succ`, fault terminals project to typed `Error`, and `Cancel`/`Nothing` remain explicit non-fault union cases; a bounded multi-object `GetMultiple` fans per-candidate acceptance into `Validation<Error, Seq<ObjRef>>`; detached object projections and option rosters land as `Seq<A>`/`Arr<A>`, and a nullable acquired value lifts to `Option<A>`.
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-runtime-extensions.md`): the closed host discriminants — `Result`, `RunMode`, `GetResult`, `CommandLineOptionType`, and the `ObjectType` acquisition filter — wrap as `[SmartEnum<TKey>]` / `[Flags]`-backed owners; the getter terminal (point, object, option, number, string, cancel, nothing) models as a `[Union]`; a `Guid` command identity wraps as a `[ValueObject<T>]`.
