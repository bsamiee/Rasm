...
- Law: a foreign wall-clock stamp with no zone evidence — a scheduler date, a file-format local time — crosses as NodaTime `LocalDateTime`/`LocalDate`, never an `Instant` fabricated by stamping the value UTC; the fabricated instant re-bases every date by the reader's offset and survives review because it type-checks.
