...
def _projected(data: bytes, width: int, height: int, symbols: tuple[DecodedSymbol, ...], /) -> RasterFact:
    # COUNT/VALID/BUILD stamp native floats onto the widened frozendict[str, float | str] band so they reach the
    # core/receipt#RECEIPT Preview.scores metric band un-coerced beside the graphic/raster/measure perceptual floats.
...
- [SYMBOL_EVIDENCE] [RESOLVED]: `Barcode.symbology` verifies distinct from `Barcode.format` — a decoded EAN-13 reports `str(format)="EAN-13"` and `str(symbology)="EAN/UPC"`, a Micro-QR `"Micro QR Code"` and `"QR Code"` — so `DecodedSymbol` carries both, the precise format on `symbology` and the rolled-up family on `family`, never one collapsed onto the other (the zxing catalog's explicit "do not treat them as synonyms"). `zxingcpp.ImageView(frame, width, height, format, row_stride, pix_stride)` accepts the positional byte strides, so the `Pixels` arm threads `frame.strides[0]`/`frame.strides[1]` and a cropped/sliced non-contiguous frame handed by `graphic/raster` reads its true row/pixel layout rather than a packed-row guess that mis-reads a sliced view (verified: an `ImageView` over a `(10, 12, 3)` `uint8` frame with strides `(36, 3, 1)` constructs and reads). `zxingcpp.barcode_formats_list()` verifies as the build's decodable roster (48 formats on the cp315 wheel) and `barcode_formats_list(BarcodeFormat.AllMatrix)` filters it to the 2D-matrix subset, so the module-level `_ROSTER` and the `supported(family)` probe report the linked build's decodable set — the `DecodeFact.BUILD` roster size threading onto every decode receipt as capability-detection evidence, the same native-build-probe shape the media filtergraph filter-registry probe uses rather than an assumption a format decodes. Close-condition: none — the `symbology`/strides/roster surfaces are fence-local and verified against the cp315 zxing build.
...
- [SHARED_FACT] [RESOLVED]: `RasterFact` is the `graphic/raster/io#RASTER` owner's value object re-declared minimally on `graphic/marks/encode#MARK` — the `(data, width, height, score: frozendict[str, float | str])` shape, the encode re-declaration widened this pass from `frozendict[str, str]` to match the declaring owner and the `core/receipt#RECEIPT` `Preview.scores` band exactly — and the decode worker folds the same fact so `Mark._compute` projects one `ArtifactReceipt.Preview` regardless of op with no cross-owner import of the worker owner. `_projected` stamps `COUNT`/`VALID`/`BUILD` as native `float`s on that widened band (`float(len(symbols))`/`float(sum(valid))`/`float(len(_ROSTER))`), so a decode metric reaches `Preview.scores` un-coerced beside the `graphic/raster/measure#MEASURE` perceptual float band rather than a `str()`-flattened count. The typed `tuple[DecodedSymbol, ...]` projects through `RasterFact.score[DecodeFact.SYMBOLS]` as a `msgspec.json` blob (`_SYMBOLS` encodes, `_DECODER`/`recovered` decode), so the typed evidence still crosses the process seam as wire bytes on the `str` half of the union; the remaining `[SCORE_FACTS]` seam that threads those facts into the `core/receipt#RECEIPT` `_facts` projection is the sibling-tracked item on the receipt owner, never re-opened here.
