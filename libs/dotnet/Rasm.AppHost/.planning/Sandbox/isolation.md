...
- [04]-[QUOTA_CONTROL]: `CostUnit`-keyed quota table, epoch preemption, and the one seat-and-drain eviction path.
...
- Output: `Load` yields the `PluginInstance` or fails on the error channel.
- Packages: Wasmtime, Thinktecture.Runtime.Extensions, Generator.Equals, LanguageExt.Core, NodaTime, Rasm (kernel `Cell`/`Transition`/`FaultBand`), BCL inbox
