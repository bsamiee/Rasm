...
One cluster: `[2]-[TIMBER_FAMILY]` owns the `TimberForm` sawn/glulam/CLT discriminant, the `TimberGrade` strength-class axis, the `TimberSection` lamella/section record, the `TimberSection.ToUnit` projection, and the `ProfileCatalogue.BuildTimberRows` row table.
