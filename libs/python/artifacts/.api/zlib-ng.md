...
- Accept: the gzip-container/DEFLATE codec service feeding the `package/codec#CODEC` `GZIP` band and the `package/bundle#BUNDLE` `CompressionAlgo` gzip arm via a `CodecProfile.gzip` row, running on the `lane.offload(Kernel.of(..., KernelTrait.RELEASING))` thread seam, lifted onto the `expression.Result` rail
