...
- Packages: `msgspec` supplies frozen value owners and `structs.replace`; `re` supplies compiled system parsers; `frozendict` carries relation projections; `expression` supplies `Map`, `Result`, `Option`, and `Block` layers.
...
- Boundary: this owner computes the coordination, never the artifact — no rendered matrix (the `visualization/table#TABLE` renderer folds `rows`/`facts`), no second producer result.
