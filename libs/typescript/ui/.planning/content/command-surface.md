...
// (`isEnabled: (intentKey) => Effect<boolean>`) are owned by `interchange/gateway/command-gateway#COMMAND_GATEWAY`
// and `projection/availability/availability-gate#AVAILABILITY_GATE` — imported by reference, never re-minted.
