...
- [02]: a row is an `FC<P>`; `ComponentType` is the `memo`/`lazy`/HOC input union
- [03]: the `as`/polymorphic-element pattern (`@radix-ui/react-slot`) types its element against `ElementType`
...
- [02]: `act/gesture` + `view` — React 19 `ref` value; `RefObject.current` mutable, `MutableRefObject` the retired alias
- [03]: `view` — React 19 ref callbacks return a cleanup; `react-aria` `useObjectRef`/`mergeRefs` reconcile these
- [04]: the `forwardRef` render-fn ref param + prop ref-stripping; retired where `ref` is now a plain prop
...
[CONSUMER_BOUNDARY]:
- [01]: the root all event rows extend; carries `currentTarget`/`nativeEvent`/`preventDefault`
...
- [07]: the `on*` prop types; one bivariant `EventHandler<E>`, one alias per event row
...
|  [06]   | `ViewTransitionProps` / `ViewTransitionClass` / `ActivityProps` (canary/overlay)             | gated upgrade     |
...
- [05]: the `IntrinsicElements` cell shape (element attrs + `ClassAttributes<T>` ref/key)
- [06]: `act/transition` — the `<ViewTransition>`/`<Activity>` prop types; import from `./canary` only
...
|  [05]   | `ViewTransition` (`ExoticComponent<ViewTransitionProps>`) / `Activity` (`ExoticComponent<ActivityProps>`) | gated exotic      |
...
- [03]: React 19: `ref` is a prop, so author rows without `forwardRef`; `memo` redundant under react-compiler; `lazy` code-splits
- [04]: `view` — `Suspense` boundary pairs `useAtomSuspense`; `Profiler` feeds `viewer/probe` render timing
- [05]: `act/transition` — the `<ViewTransition>`/`<Activity>` upgrade rows; `./canary`, capability-gated
