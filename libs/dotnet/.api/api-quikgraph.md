...
- `LanguageExt.Core`(`.api/api-languageext.md`): `Try.lift` preserves each thrown `NonAcyclicGraphException`, `NegativeCycleGraphException`, `NoPathFoundException`, or `VertexNotFoundException` until an owning boundary classifies it; a `TryFunc` `bool`-plus-`out` refusal maps directly without exception capture.
