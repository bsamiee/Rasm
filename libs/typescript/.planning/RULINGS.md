...
- Environment spellings crossing the deploy-to-process seam derive from one typed key catalog — iac's channel-to-variable map and the runtime `Setting` group rows compose one owner, so a rename breaks both ends at compile time; a hand-comment-maintained match forks silently, and the OTLP endpoint's two live spellings are the proof.
- Each owner mints its OWN reason-discriminated `Schema.TaggedError` instantiating the shared folder-fault-shape template — reason-literal set, `_faults` class table, `get class()` with `override get message()`, the `_Rows`/`_Closed` guard pair — never an extracted base class or `makeFault` factory: the distinct `_tag` is the key consumers `catchTag` on and the serving edge projects to status, and each reason set is per-domain, so a factory erases the per-owner tag identity.
