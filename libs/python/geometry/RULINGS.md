...
- `ArtifactFrame` carries one nested `ArtifactRef` and payload — Connect orders messages, `rasm.contracts.artifact` owning extent and identity proof.
...
- Tessellation wire counts decode from emitted GLB: graph placements own elements; unique pooled faces own triangles.
