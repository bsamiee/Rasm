...
- Law: the drain's inserts cross tenants — one scope's rail lands rows for every tenant its drafts carry — so the drain fiber runs under the MAINTENANCE-PLANE session posture the tenancy owner mints: `fact_journal` registers RLS under the FORCE policy, so an unpinned INSERT refuses on the policy's check arm and parks billing truth a healthy database would accept, and a tenant pin refuses every other tenant's rows the same way.
