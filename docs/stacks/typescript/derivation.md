...
const _rows = {                                              // interior row anchor: plain as const keeps every literal; the hub guards carry its contract
...
declare namespace Tier {                                     // merged type hub: every derived type qualifies off the single import
  ...
  type Shape = Types.Simplify<typeof _rows & { readonly kinds: Kinds; readonly wire: Schema.Literal<Kinds>; readonly ceiling: <K extends Kind>(kind: K) => Ceiling<K> }> // Simplify flattens the assembled intersection at the public alias; the typeof chain stays interior
...
const Tier: Tier.Shape = {                                   // one exported owner assembles rows, companions, and operations; the annotation derives, never restates
  ..._rows,
  kinds: _kinds,
  wire: _wire,
...
interface Registry {                                         // a contributor's row: from its own module, this block is declare module "<owner-specifier>"
  readonly broad: { readonly weight: 1; readonly admit: (raw: string) => boolean }
}

declare namespace Registry {                                 // the hub merges onto the interface: one exported name carries rows, contract, and derived types
...
const toll = (grade: Signal.Row["grade"]): number =>         // consumption governance: the terminal arm carries the residue proof
...
const extend = <const Steps extends ReadonlyArray<Step>, const Next extends Step>(
  steps: Steps,
  next: Next,
): readonly [...Steps, Next] => [...steps, next]            // variadic flow: the spread proves the tuple; index arithmetic would demand a cast
...
- Law: decomposition is ASCII-bounded — a single-step `infer` split advances by UTF-16 code unit under `tsc` and by code point under `tsgo`, so a literal beyond ASCII has no parity-stable decomposition; alphabets past ASCII split at the value level, and the type derives from that computation's anchor.
...
type Feed<in Cmd, out Row> = {                               // in/out declare intent the compiler checks; the struct below witnesses it structurally
  readonly [FeedTypeId]: {
