...
public static class AppearanceProjection {
    ...
    static AppearanceSummary SummaryOf(IfcSurfaceStyle surface, double tolerance) {
        ...
        Option<T> First<T>() where T : class =>
            surface.Styles.AsIterable().Choose(static s => s is T t ? Some(t) : Option<T>.None).HeadOrNone();
    ...
    // dedup to one node — the SAME Mint the Rasm.Materials MaterialProjector uses.
