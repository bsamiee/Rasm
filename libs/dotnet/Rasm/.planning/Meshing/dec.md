...
- Cases: `AssemblyOrigin` 2 (`Assembled`/`Cached`), `FaceSkip` 2 (`Incomplete`/`Degenerate`); the `SpectralAssembly.EdgeConnection` column and the `MeshLaplacian` vocabulary arrive settled from `Numerics/spectral` and `Meshing/mesh`.
