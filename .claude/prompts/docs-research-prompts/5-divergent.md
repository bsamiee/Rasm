...
[DIRECTION]:
- Read all of `00-bedrock.md` to hold the lane's full breadth, then go past it. Take the bedrock as solved and drill one facet to the depth the lane's domain supports: the pattern that collapses several bedrock findings into one richer rule, the law that governs them, the advanced library capability they unlock when stacked, the integration or composition they enable, or the failure surface they imply. Never re-state the bedrock.
- The agent chooses its own facet — it is free to take the direction it judges strongest — but each pass and each agent takes a distinct one, so the series spreads across the lane's best advanced angles rather than refining one. The doctrine's laws are the lens for judging strength: density, polymorphic collapse, policy values, derived logic and types, library depth, and definition-time aspects. Stay inside the lane's concern and the target's domain — depth and reach, never drift.
- The first pass opens the series. Each later pass reads the prior divergent files in its lane, takes their depth as the floor, and pushes past them. Write a new `NN-<slug>.md` per pass.
