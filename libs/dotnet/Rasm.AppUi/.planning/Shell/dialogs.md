...
- Law: the crossing carries `IO<Option<TResult>>`. `Project` maps dismissal to `None`, a chosen value to `Some`, and every refusal — occupancy, unregistered host, template miss, or result-shape disagreement — to `Fin.Fail` before the effect absorbs that failure.
...
- Packages: DialogHost.Avalonia, Irihi.Ursa, ReactiveUI, Avalonia, LanguageExt.Core, Thinktecture.Runtime.Extensions, Rasm (project — `CapabilitySet`, `CapabilityLaw`, `Cell`/`Transition`, `Lease`, `FilterPlan`, `FileLocation`), Rasm.AppHost (project)
...
public static class DialogSurface {
    ...
    extension(DialogTopology root) {
        public IO<Option<TResult>> Show<TResult>(DialogAsk<TResult> ask) where TResult : notnull =>
            IO.liftAsync(async () => await Request(root, ask).ConfigureAwait(true))
                .Bind(static outcome => IO.lift(outcome));
    ...
    private static Fin<Option<TResult>> Project<TResult>(object? closing) where TResult : notnull =>
        closing switch {
            ...
            var other => Fin.Fail<Option<TResult>>(
                new DialogFault.ResultShape(typeof(TResult).Name, other.GetType().Name)),
        };
    ...
    private static async Task<Fin<Option<TResult>>> Request<TResult>(DialogTopology root, DialogAsk<TResult> ask) where TResult : notnull {
        ...
        if (reserving && root.Reserve() is { IsFail: true } refusal) {
            return refusal.Map(static _ => Option<TResult>.None);
        }
...
flowchart LR
    ...
    accDescr: A dialog intent resolving through the stack-owner projection into either the DialogHost session stack under its occupancy reservation or the Ursa overlay canvas under its layer list, both crossing a mount-bound presence fact first and both projecting their erased result onto one optional effect result.
    ...
    Overlay --> Project
    Project --> Result["IO<Option<TResult>>"]
```
