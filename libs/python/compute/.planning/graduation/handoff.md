...
`GraduationReceipt.graduates` is the one admission gate, folding the measured `Ledger` against the per-key `Ceiling` to `RuntimeRail[GraduationReceipt]`. The four sibling rejection clauses every evidence owner declares — solver residual above tolerance, convex duality gap above tolerance, posterior failing the rhat-and-ess bar, array layout failing bit-identical reproduction — collapse to the one residual-over-ceiling fold, never four inlined per-site comparisons. The fold runs inside one `content.graduate` OTel span weaving `beartype`, `msgspec`, OTel, the runtime `boundary` fault fence, and the `Signals` receipt fold into a single rail: a handoff proposing a crossing without clearing its ceiling is an `Error(BoundaryFault)` streaming the `rejected` receipt, never an emitted `planned` receipt.
...
- [01]-[GRADUATION]: the graduation receipt, the handoff-axis union, the `Ledger`/`Ceiling` `beartype.vale.Is` refinements, the `@beartype`-fenced `_admit` refinement gate `.bind`-threaded through the pure `_clear` ceiling fold woven under one `content.graduate` span with the `boundary` fault fence and the `Signals.emit` `planned`/`rejected` egress, and the `graduates` rail
...
- Refinements: `Ledger` and `Ceiling` are the `Annotated[dict[str, float], Is[...]]` refinement aliases the `@beartype(conf=FAULT_CONF)` fence on the inner `_admit` thunk checks — every residual is finite (`beartype.vale.Is[lambda m: all(isfinite(v) for v in m.values())]`), so a `NaN`/`±inf` ledger or ceiling entry raises the canonical `BeartypeCallHintViolation` INSIDE the `boundary` fence and the `CLASSIFY` `api` row folds it onto the rail rather than slipping past the `measured[k] <= cap` comparison — a `-inf` residual clears any ceiling as a silent admission and a `NaN` rejects every ceiling as a silent reject. The sign is deliberately unconstrained: a `measured` ledger carries negated-floor deficits (`experiments/inference.md#BAYESIAN` feeds `neg_min_ess_bulk = -min(ess)` so the one `measured <= ceiling` fold reads an ess floor as a max-deficit), so a `>= 0.0` clause would reject a legitimate sibling ledger at the fence. The refinement is the input contract; the ceiling clearance is the admission predicate.
- Entry: `GraduationReceipt.graduates(source_package, axis, evidence_key, measured, ceiling)` is the ONE admission rail, woven under one `content.graduate` span. It wraps the `@beartype(conf=FAULT_CONF)`-fenced `_admit` thunk in the `boundary(f"graduation.{axis.tag}", ...)` fence, then `.bind`s the railed validated ledger through the pure `_clear` ceiling fold — so the `boundary` mints exactly one `RuntimeRail[GraduationReceipt]` and `_clear` returns `Ok(GraduationReceipt(...))` when every measured residual clears its ceiling or `Error(BoundaryFault(boundary=(f"graduation.{tag}", "residual-ceiling")))` when any key exceeds it or a ceiling key is unmeasured. The two concerns stay distinct: the refinement breach is an exception the fence converts (the `@beartype` raise lands as `Error` through the `CLASSIFY` `api` row), the ceiling rejection is a pure domain predicate the `.bind` threads — never a thunk returning `RuntimeRail[GraduationReceipt]` the fence re-wraps to `RuntimeRail[RuntimeRail[GraduationReceipt]]` then unwraps through an identity `.bind`.
- Span egress: the `Ok` arm sets `Status(StatusCode.OK)`, widens the span with the `evidence_key.hex`/`residual_count` facts the rejected handoff has no receipt to carry, and streams the `planned` receipt through `Signals.emit`; the `Error` arm streams only the `rejected` `BoundaryFault` receipt because the fence's `_convert` already recorded the cause and set `Status(StatusCode.ERROR)` on the active span. The axis/subject pair both arms need is the one pre-boundary `set_attributes` write behind the `is_recording()` gate the `evidence/identity#IDENTITY` `derived` aspect holds, so a no-op span pays no attribute build and the four `span_facts` keys split across the pre-boundary and Ok-arm writes rather than a redundant full re-write on success.
- Subject carrier: the `axis` argument IS the subject carrier — a `solver`/`symbolic`/`model_asset`/`array_layout`/`unit_law`/`uncertainty_law`/`geometry`/`convex_program` case — so a caller graduating geometry evidence passes `HandoffAxis(geometry="scan-deviation")` and the rail reads the subject off the case rather than a parallel field. The admitted ledger rides into the receipt as the cleared `residuals` map, so the receipt that exists is the one whose evidence passed — there is no admitted-but-unchecked state. The sibling owners (`solvers/receipt.md#RECEIPT`, `optimization/convex.md#CONVEX`, `experiments/inference.md#BAYESIAN`, `numerics/array.md#PAYLOAD`) feed their per-axis residual ledger through this one rail rather than each inlining its own admission comparison; a consumer driving graduation under the runtime `@receipted` aspect harvests the streamed receipt without threading an `emit` call.
...
- Axis fold: `_subject` is the one total `match` over the `HandoffAxis` union resolving every case to its `str` subject through a single structural or-pattern — the `geometry` case yields its `GeometrySubject` literal, every other case its carried subject, all eight tags binding one `s` capture rather than eight identical `return s` arms — closed by `assert_never` so a new handoff kind is a compile-surfaced gap, never a silent default. The fold is the single place the union is read; `subject`, `span_facts`, and `contribute` all route through it and never probe the union by `getattr(axis, axis.tag)`.
...
- Boundary: the receipt names the wire axis it crosses on, never the managed owner that receives it — no C# `ComputeReceipt`, benchmark claim, source generation, or C# interior owner-row spelling. compute emits the `scan-deviation` `GeometrySubject` and implements no geometry: the deviation kernel, the registration transform, the reconstruction mesh, and the topology/network graphs are geometry-branch owners that consume these literals. The deleted forms:
  ...
  - an inline `structlog` line where `Signals.emit` owns receipt egress; a raw residual-ledger dump onto the span where only the bounded `span_facts` scalars are admissible.
...
_TRACER: Final = trace.get_tracer("compute.graduation")
...
class GraduationReceipt(Struct, frozen=True):
    ...
    def graduates(
        ...
        # the single `.bind` keeps one `RuntimeRail`: `_admit` returns the bare validated ledger so the
        # `boundary` does not double-wrap, and `_clear` is the ceiling fold the bind threads.
        with _TRACER.start_as_current_span("content.graduate") as span:
            ...
            rail: RuntimeRail[GraduationReceipt] = boundary(
                f"graduation.{axis.tag}", lambda: GraduationReceipt._admit(measured, ceiling)
            ).bind(lambda validated: GraduationReceipt._clear(source_package, axis, evidence_key, validated))
            match rail:
                case Ok(receipt):
                    span.set_attributes({"evidence_key": evidence_key.hex, "residual_count": len(receipt.residuals)})
                    span.set_status(Status(StatusCode.OK))
                    Signals.emit(receipt, _REDACTION)
                case Error(fault):
                    # no `set_status` here: the fence's `_convert` already set ERROR on the active span.
                    Signals.emit(Receipt.of(source_package, fault), _REDACTION)
            return rail
    ...
    def _subject(axis: HandoffAxis) -> str:
        # one or-pattern binds the carried subject off whichever case the tag selects, so the eight
...
Each axis crosses the graduation-evidence wire under one admission gate: the evidence owner supplies its measured `Ledger` and the `Ceiling` that axis must clear to `GraduationReceipt.graduates`, and a ledger key above its ceiling is an `Error(BoundaryFault)` on the rail streaming the `rejected` receipt rather than a graduated handoff. The wire axis case is the only cross-language vocabulary the receipt carries; the concrete C# owner that consumes an axis is confirmed against the C# owner planning on the graduation task, never an interior owner-row spelling baked into this owner.
