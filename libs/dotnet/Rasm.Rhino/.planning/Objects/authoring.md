...
- Law: `RasmCurveObject` alone carries `SetCurve` — the host gives only the curve kind a restage member, surfaced as a protected pass-through; the other kinds replace geometry through the table pipeline like any object.
