...
// The two geospatial readings of an admitted kernel lattice, attached to the receiver this package does not own:
// Resolution is the one comparable ground-resolution scalar LevelFor ranks a target against (the cell measure taken
// to its rank root, so a rotated planar lattice and a layered one both answer one magnitude), and CanonicalBytes is
// the placement's content projection — the TWELVE index-to-world affine coefficients (the fourth matrix row is the
// invariant [0 0 0 1], so twelve IS the whole affine) plus the three-axis census. Both are receiver-local reads over
// an already-admitted value; the lattice owns the measure and the affine, this owns the geospatial reading of them,
// and one block serves the base grid and every pyramid level so neither projection is spelled twice.
public static class LatticeGeodesy {
 extension(CellLattice lattice) {
  ...
  // The kernel is RhinoCommon-aware by branch charter and this seam is not, so the host coordinate crosses HERE and
  // nowhere else: every geospatial read a seam consumer takes lands as the seam-owned `Graph/element` `Vector3`,
  // whose whole algebra the seam already mints. `Origin` is the world corner of cell (0,0,0) — the geotransform
  // anchor a tabulation or a foreign encoder writes — and `Span` the per-axis world extent of one cell, both read
  // off the admitted affine so a rotated or anisotropic lattice reports true where an OriginX/CellSizeX quadruple
  // could only report an axis-aligned fiction. A consumer reading the kernel's own `Point3d`/`Vector3d` members
  // directly re-opens the host leak this block exists to close.
  public Vector3 Origin => Neutral(lattice.Corner(0, 0));

  public Vector3 Span => new(lattice.CellSize.X, lattice.CellSize.Y, lattice.CellSize.Z);

  public Vector3 CentreOf(int column, int row, int layer = 0) => Neutral(lattice.Center(column, row, layer));

  // The fractional cell coordinate of a planar world sample — the ONE inverse-affine crossing, so no seam member
  // outside this block constructs a host point to ask the lattice where a coordinate lands.
  public (double Column, double Row) Fractional(double x, double y) =>
   lattice.Locate(new Point3d(x, y, 0.0)) switch { var local => (local.X, local.Y) };

  public void CanonicalBytes(CanonicalWriter w) {
   Transform t = lattice.IndexToWorld;
   w.Double(t.M00).Double(t.M01).Double(t.M02).Double(t.M03)
    .Double(t.M10).Double(t.M11).Double(t.M12).Double(t.M13)
    .Double(t.M20).Double(t.M21).Double(t.M22).Double(t.M23)
    .Ordinal(lattice.Columns.Value).Ordinal(lattice.Rows.Value).Ordinal(lattice.Layers.Value);
  }
 }

 static Vector3 Neutral(Point3d host) => new(host.X, host.Y, host.Z);
}
...
public readonly partial record struct ColorBin(int Index, PerceptualColor Colour, string Category = "") {
 // The content key stays on the display-byte quadruple, projected through the kernel's ONE quantizer: identity is
...
// onto Grid, and Window reads this lattice directly instead of scaling a base-relative ratio. Of proves the run is
// the base lattice's Coarsen chain, so a level's ordinal IS its position and its affine can never drift from the base.
[Equatable]
...
// series, a diurnal noise contour, a climate-projection scenario — the NetCDF/time-enabled-COG time axis lowered onto
// the seam): every slice shares the ONE grid, pyramid, and band schema and differs only by its anchor instant and its
...
public readonly partial record struct CoverageBand(
 ...
 // The bracketing legend bins for a raw value — the greatest bin at-or-below and the least strictly above — folded
 ...
 // The PERCEPTUAL interpolation between bracketing bins a continuous Field reads — the capability the display-byte
...
public sealed partial record CoverageGrid {
 ...
 // a positional public ctor beside the gates was the bypass that let unadmitted state reach Sample/Window/LevelFor;
 ...
 // K-kinded slot, Accumulate folds the run, and .ToFin() collapses ONCE at the seam return, so a malformed
 ...
 // The pyramid IS the base lattice's Coarsen chain: one fold threads the expected lattice — the base, then each
 ...
 // The ONE policy-driven legend read, the colour dual of Sample: the SAME CoverageKind.Interpolates column that
 ...
 // The world point of a discrete cell's CENTRE — the dual of CellAt a consumer reads to place a per-cell value (a
 ...
 // The TEMPORAL dual of LevelFor: the slice governing an instant is the latest slice at-or-before it — the
 // step-function read a time-varying field takes (an hourly irradiance value holds until its successor lands).
 // The admitted stored order is strictly increasing, so the fold's last accepted slice IS the governing one;
 ...
 // The interval form: every slice anchored inside [from, to] — the range fetch a seasonal/annual analysis composes
 ...
 // four corners Locate onto the CHOSEN LEVEL'S OWN lattice (rotation-exact — an axis-aligned world rect is a
 // cell-space parallelogram), envelope, floor to containing cells, clip to that lattice's census; None when the rect
 // misses the coverage — the ONE inverse-affine+FLOOR owner serving interval reads, so a consumer never re-spells the
 // corner/floor/clamp arithmetic per fetch. Because each level carries its own affine (the Coarsen-chain law), the
 // base-to-level ratio a descriptor-less level once forced is GONE: there is no second scaling step to get wrong.
 ...
 // The total uncompressed raster byte size summed across bands at a chosen level (each level census · storage
 ...
 // The ONE level-or-base lattice resolution Window and ByteLength both read, so the "no level means the base"
 ...
 // The coverage's content projection the Graph/element#NODE_MODEL Node.Coverage arm delegates to: kind, the lattice's
