...
public abstract partial record RemoteStoreFault : Fault {
    ...
    public static Error Lift(string provider, ObjectVerb verb, ContentAddress key, Error error) => error switch {
        ...
        { Exception.Case: AmazonS3Exception { ErrorCode: "InvalidObjectState" } } => new ProviderFrozen(provider, verb, error),
        { Exception.Case: AmazonS3Exception s3 } => s3.StatusCode switch {
            HttpStatusCode.PreconditionFailed => new ProviderConflict("if-none-match", error),
            HttpStatusCode.NotFound => new ProviderNotFound(error),
            HttpStatusCode.Forbidden or HttpStatusCode.Unauthorized => new ProviderDenied(provider, s3.ErrorCode, error),
            HttpStatusCode.RequestEntityTooLarge => new ProviderOversize(provider, s3.ErrorCode, error),
            _ => new ProviderTransport(provider, verb, Some((int)s3.StatusCode), s3.ErrorCode, error),
        },
        { Exception.Case: RequestFailedException { ErrorCode: "BlobArchived" } } => new ProviderFrozen(provider, verb, error),
        { Exception.Case: RequestFailedException az } => az.Status switch {
            412 => new ProviderConflict("if-none-match", error),
            404 => new ProviderNotFound(error),
            401 or 403 => new ProviderDenied(provider, az.ErrorCode ?? "azure", error),
            413 => new ProviderOversize(provider, az.ErrorCode ?? "azure", error),
            _ => new ProviderTransport(provider, verb, Some(az.Status), az.ErrorCode ?? "azure", error),
        },
        { Exception.Case: GoogleApiException gcs } => (int)gcs.HttpStatusCode switch {
            412 => new ProviderConflict("if-generation-match", error),
            404 => new ProviderNotFound(error),
            401 or 403 => new ProviderDenied(provider, gcs.Error?.Code.ToString(CultureInfo.InvariantCulture) ?? "gcs", error),
            413 => new ProviderOversize(provider, gcs.Error?.Code.ToString(CultureInfo.InvariantCulture) ?? "gcs", error),
            int status => new ProviderTransport(provider, verb, Some(status), gcs.Error?.Code.ToString(CultureInfo.InvariantCulture) ?? "gcs", error),
        },
        { Exception.Case: PreconditionFailedException } => new ProviderConflict("if-none-match", error),
        { Exception.Case: ObjectNotFoundException or BucketNotFoundException } => new ProviderNotFound(error),
        { Exception.Case: AccessDeniedException } => new ProviderDenied(provider, nameof(AccessDeniedException), error),
        { Exception.Case: ForbiddenException } => new ProviderDenied(provider, nameof(ForbiddenException), error),
        { Exception.Case: EntityTooLargeException } => new ProviderOversize(provider, nameof(EntityTooLargeException), error),
        { Exception.Case: InvalidContentLengthException } => new ProviderOversize(provider, nameof(InvalidContentLengthException), error),
        { Exception.Case: ConnectionException } => new ProviderTransport(provider, verb, None, "connection", error),
        _ => error,
    };
    ...
    public static RemoteStoreFault Granted(ObjectVerb verb, ContentAddress key, HttpResponseMessage response) => response.StatusCode switch {
        HttpStatusCode.Forbidden => new Denied("presigned", "forbidden"),
        HttpStatusCode.NotFound => new NotFound(),
        HttpStatusCode.PreconditionFailed => new Conflict("if-none-match"),
        HttpStatusCode refused => new Transport("presigned", verb, (int)refused, refused.ToString(), Delay(response)),
    };
