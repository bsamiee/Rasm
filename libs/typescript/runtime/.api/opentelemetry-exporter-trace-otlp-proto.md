...
- catalog-verdict: KEEP as SDK-bridge protobuf peer completing the JSON/protobuf wire pair; edge-ledger fences `@opentelemetry/*` to `scope:runtime`; `[R3]`-collapse member (native `OtlpTracer` supersedes)
...
- prefer the native `Otlp`/`OtlpTracer` lane; reach for this SDK exporter only for protobuf-wire SDK-only processor/exporter capability, and record it as an `[R3]` non-collapsed dependency.
