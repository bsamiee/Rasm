...
- Owner: `ScopeTarget` carries three acquisition rows over one `Acquire()` column: `EditorHost` reads `Editor.Instance`, `CanvasHost` continues through `Editor.Canvas`, and `DocumentHost` continues through `Canvas.Document`. Every hop null-gates to `KernelFault.MissingContext`. `GhScope` closes the corresponding editor, canvas, and document cases and derives optional projections by total case dispatch. `GhCrossing.Bind` is the scope-binding wrapper (E-G53): it closes acquisition INSIDE a `Func<Fin<TOut>>` body any kernel `UiDispatch<TOut>` case wraps — `Current`, `Blocking`, or `Queued` — so the three crossing postures share one acquisition law and a pre-acquired scope smuggled across a crossing is unconstructible.
...
|  [02]   | crossing binding  | `GhCrossing.Bind`         | one wrapper, three kernel postures    |    1    |
