...
- [03]-[REDRIVE_SEAM]: `StoreHop` the band-neutral hop identity closing every process-seam plane's verb vocabulary, `StoreVerdict` the four-arm attempt verdict folding the kernel `Redrive.Settle`, `StoreRedrivePort` the composition-root-bound executor seam, and `LocalRedrive` the in-process row an unbound root degrades to.
...
- Packages: LanguageExt.Core (`IO`, `Fin`, `Error`, `Duration`), NodaTime (`Duration` the deferral window), Thinktecture.Runtime.Extensions (`[Union]`, `[SmartEnum]`), the kernel `Rasm/Domain/rails#REDRIVE` surface (`Retriability`, `RedrivePolicy`, `Verdict`, `Redrive`).
...
|  [04]   | band admission  | one `Lift` per SDK edge, one `Granted` per grant | the engine interior sees only rails; no per-leg catch              |
...
|  [07]   | hop identity    | `StoreHop` per process-seam plane                | a wide-column op never rides an object verb                        |
