...
- Build keys from typed constructors (`SymmetricSecurityKey`, `RsaSecurityKey`, `X509SecurityKey`, `MlDsaSecurityKey`) and credentials from `SigningCredentials(algorithm)`; identify keys by `ComputeJwkThumbprint`/`KeyId`, never by index
...
- Take `MlDsaSecurityKey` as the post-quantum signing path on net10, dispose it, and gate its use on `CryptoProviderFactory.IsSupportedAlgorithm(alg)`
