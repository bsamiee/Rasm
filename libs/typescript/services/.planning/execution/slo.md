...
- [1]-[SLO_BUDGET]: owns the closed `Objective` delegate-row vocabulary, the `SignalSeries` `Model.Class`, the multi-window multi-burn-rate `BurnRate` fold over the `@effect/sql-pg` signal series, the `ClusterMetricsReader` over the five `ClusterMetrics` gauges, the `AlertRoute` `Data.TaggedEnum` over the outbox `DeliverySink`, the `@opentelemetry/sdk-metrics`/`@opentelemetry/sdk-trace-node` reader, and the C#-wire health/trace decode.
