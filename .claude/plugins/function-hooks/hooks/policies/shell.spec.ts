...
const _REWRITTEN: readonly (readonly [string, string, string])[] = [
    ...
    ['mise x node@22 -- node -v', 'node -v', 'Ran node -v in place of mise x node@22 --'],
...
const _LINED: readonly (readonly [string, string])[] = [
    ['mise x -C dir node@20 -- node app.js', 'names no command or holds an option before it'],
    ['eval "$(mise env -s bash)"', 'changes nothing'],
    ['git push -n', 'A preview proves nothing, the proof is the target itself: git push'],
...
describe('command bounds', () => {
    it('moves a timeout prefix to the parameter, the smaller bound for a whole command and the larger for a partial one', () => {
        expect(commandTimeout({ command: 'timeout 5 sleep 100', timeout: 120_000 })).toStrictEqual({
            ...
            context: ['Ran sleep 100 under the Bash timeout parameter at 5000 ms'],
        });
        ...
        expect(partial.kind === 'rewrite' ? partial.context.at(-1) : '').toContain('ran capped');
