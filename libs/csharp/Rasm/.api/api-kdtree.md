...
[LOCAL_ADMISSION]:
- this kd-tree is the DISCRETE POINT-NEAREST leaf feeding `Processing/fit` (MLESAC primitive-fit, normal estimation via local k-NN PCA) and the `registration/ICP` correspondence step — the per-iteration "nearest source point to each target point" query over a static cloud.
...
- vs SharpVoronoiLib's bundled kd-tree (`api-sharpvoronoilib`): `SharpVoronoiLib 1.2.0` ships a source-included PUBLIC kd-tree of its own — `Supercluster.KDTree.KDTree<TNode>` (arity-1) + `Supercluster.KDTree.BoundedPriorityList<TElement>` under the ORIGINAL `Supercluster.KDTree` namespace — backing its `NearestSiteLookupMethod.KDTree`. This fork's type is `SuperClusterKDTree.KDTree<TDimension,TPriority,TNode>` (arity-3, namespace `SuperClusterKDTree`, assembly `KDTree.dll`), a DIFFERENT fully-qualified type (different namespace AND arity), so a design page composing BOTH this kd-tree's point k-NN and a SharpVoronoiLib Voronoi references both assemblies with NO same-FQN collision and NO `extern alias`. The namespace split is exactly why `using Supercluster.KDTree;` is doubly wrong for THIS package — beyond naming the wrong namespace for the fork (`SuperClusterKDTree`), `Supercluster.KDTree` would resolve SharpVoronoiLib's bundled arity-1 copy when that package is also referenced; reach this fork's arity-3 tree only as `using SuperClusterKDTree;`.
...
- Reject: using the package name `Supercluster.KDTree.Net` as the namespace or `using Supercluster.KDTree;` (the namespace is `SuperClusterKDTree`, the assembly `KDTree.dll`; and `Supercluster.KDTree` additionally resolves SharpVoronoiLib's bundled arity-1 `Supercluster.KDTree.KDTree<TNode>` when that package is referenced — a different FQN, no collision, but reach this arity-3 tree only as `SuperClusterKDTree`, see `api-sharpvoronoilib`); treating it as a primitive (triangle/curve/AABB) broad-phase or ray-query structure (that is the kernel BVH/octree — the kd-tree is POINT-only); expecting incremental insert/delete (the tree is build-once immutable — rebuild on change); passing an un-squared radius under `EuclideanDistance` (that metric is squared-L2); hand-writing a Euclidean `Func` the `DistanceMetrics` enum already supplies; re-implementing curve/surface continuous closest-point on the kd-tree (that is GShark's parametric `ClosestPoint`).
