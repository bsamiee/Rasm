...
A float may be written as `2.0` or `2.`:
...
This uniform ability to combine is a central advantage of an expression-only model. A statement-form `if` cannot be placed where an operator expects a value.
...
Compiler acceptance forms nested sets: all strings contain the syntactically valid expressions; those contain the expressions that type check. A syntax error never reaches the type checker, and a type error never reaches code generation or execution.
...
- A primitive number, boolean, or string evaluates to its value.
...
An imperative variable can be modeled as a labeled memory cell whose contents change over time. For example, a loop that sums squares repeatedly updates cells named `sum` and `i`. A functional `let` binding is better understood as assigning a name to a computation, not as creating a memory cell that will later be updated.
...
A top-level binding is available to following expressions. `let ... in ...` introduces a name whose scope is limited to its body and is itself an expression:
...
A local name may shadow a global name without modifying it:
...
- A Java statement-form `if` cannot serve as an operand of `>` because it does not evaluate to a value.
