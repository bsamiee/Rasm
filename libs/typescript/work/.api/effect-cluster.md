...
- rail: durable-actor
- An `Entity` binds an `@effect/rpc` `RpcGroup` (its message protocol) to a sharded, per-id-singleton actor; `Sharding` is the service that registers entities, assigns ids to shards, and routes messages. `work/engine/entity` builds one entity per durable-actor family.

| [INDEX] | [SYMBOL]                                                     | [TYPE_FAMILY]   | [CONSUMER / BOUNDARY]                                          |
| :-----: | :--------------------------------------------------------- | :-------------- | :------------------------------------------------------------- |
|  [01]   | `Entity.Entity<Type, Rpcs>`                                | actor def       | `work/engine/entity` — one entity per durable-actor family; `.toLayer(handlers)` registers it |
...
| [INDEX] | [SYMBOL]                                                     | [TYPE_FAMILY]   | [CONSUMER / BOUNDARY]                                          |
| :-----: | :--------------------------------------------------------- | :-------------- | :------------------------------------------------------------- |
|  [01]   | `MessageStorage.MessageStorage`                            | storage Tag     | `work/engine/storage` — `saveRequest`/`saveReply`/`unprocessedMessages`/`repliesFor`; satisfied by `SqlMessageStorage.layer` |
...
| :-----: | :---------------------------------------------------------------------------------------------- | :------------- | :-------------------------------------------------------- |
|  [01]   | `Entity.make(type, rpcs)` / `Entity.fromRpcGroup(type, rpcGroup)`                              | declare        | `work/engine/entity` — the actor from its `@effect/rpc` protocol |
...
| [INDEX] | [SURFACE]                                                                                       | [ENTRY_FAMILY] | [CONSUMER / BOUNDARY]                                     |
| :-----: | :---------------------------------------------------------------------------------------------- | :------------- | :-------------------------------------------------------- |
|  [01]   | `SqlMessageStorage.layer: Layer<MessageStorage, never, SqlClient \| ShardingConfig>`           | storage        | `work/engine/storage` — the durable message store; `SqlClient` satisfied by the store driver at the app root |
...
- The runner entrypoint Layers (selected via `host/exec` at the app root), K8s discovery, the durable scheduled job, the cluster singleton, and the `@effect/workflow` bridge.
...
[CLUSTER_TOPOLOGY]:
- an entity is an RPC group with durable identity: `Entity.make(type, rpcs)` binds an `@effect/rpc` `RpcGroup` to a sharded per-id singleton. The RPC protocol's payload/success/error `Schema`s ARE the message contract; `.toLayer(handlers)` registers the handler map (checked exhaustively) with `Sharding`. `work/engine/entity` codes the actor, never the mailbox or shard math.
- storage is a swappable port: `MessageStorage` is a Tag with `layerMemory`/`layerNoop` (spec) and `SqlMessageStorage.layer` (durable, over `SqlClient` + `ShardingConfig`). `work/engine/storage` composes the Tag; the `store`-owned SQL driver satisfies `SqlClient` at the app root — `work` never imports a `-pg`/`-sqlite-*` driver. Durability of a given message is the per-Rpc `ClusterSchema.Persisted` annotation, and dedupe is `SaveResult.Duplicate` keyed on `Snowflake` + primary key (at-least-once, exactly-once-effect).
- topology is config, discovery is a Layer: `ShardingConfig.layerFromEnv` reads runner address/weight, shard groups, lock intervals, and the entity mailbox/idle/timeout budgets from env/`Config` — the `iac/stack` `StackOutputs → ShardingConfig` `[SHAPE]` seam. Runner discovery is `RunnerHealth.layerK8s` over `K8sHttpClient` (`HttpClient` + `FileSystem`), discovery-only. The runner entrypoint (`HttpRunner.layerHttp`/`SocketRunner.layer`/`SingleRunner`) is selected via `host/exec` at the app root — `work` imports no `platform-node/bun` binding.
...
[STACKS_WITH]:
- `@effect/rpc` (`edge/.api/`): an `Entity` IS an `RpcGroup` — the message protocol, handler exhaustiveness, and the typed `.client` are the RPC surface. `EntityProxy.toRpcGroup`/`toHttpApiGroup` re-exposes an entity as an `edge` contribution group with the typed client for free.
- `@effect/sql` (`store/.api/effect-sql.md`): `SqlMessageStorage.layer`/`SqlRunnerStorage.layer` bind message + runner storage to the `SqlClient` Tag; the `store`-owned driver (`effect-sql-pg`/`-sqlite-*`) satisfies it at the app root. `work` composes the Tag as a `[PORT]` and never imports the driver.
...
- `@effect/workflow` (`work/.api/effect-workflow.md`): `ClusterWorkflowEngine.layer` satisfies the `WorkflowEngine` Tag over `Sharding` + `MessageStorage` — durable workflows run sharded on this runtime. `work/flow` defines the workflow; `work/engine` provides the engine.
...
- Select the runner entrypoint (`HttpRunner`/`SocketRunner`/`SingleRunner`) at the app root via `host/exec`; never import a `platform-node/bun` binding in a `work` page. Use `K8sHttpClient` for discovery only, never provisioning.
