...
- The per-backend emit path (Vello scene encode versus `SKRuntimeEffect` shader) diverges below the `RenderTarget` factory, so the factory column owns the divergence and the render-graph pass algebra stays backend-agnostic; the CPU 2D-Skia fallback stays the today-shipping floor.
...
- 3DGS rasterization is a distinct render path from triangle meshlets, demanding its own residency keying and pass case; depends on `T-BACKEND-PORT` so the splat rasterizer binds a backend target factory rather than hard-coding a second `GRContext` substrate.
...
- The manifest references the same Compute `GeometryPayload` residency the desktop reads, so the residency keying stays one owner across the wire; depends on `T-BACKEND-PORT` (WebGPU backend row) and `T-REALITY-CAPTURE` (splat residency keying) before the wire is complete.
...
- The BCF topic/component schema is openBIM exchange semantics owned by `Rasm.Bim`; AppUi composes that contract plus its own `Viewpoint` and CRDT owners into the board, so a second BCF model or a direct BCF-XML writer inside `coordination/` is the rejected form.
...
- Adaptive quality is one feedback owner over a perf budget; the governor degrades deterministically so render-hash lanes stay attributable under a budget breach.

[T-NOTEBOOK-DETERMINISM] [QUEUED] — compose notebook reproducibility onto the AppHost determinism owner
- Rework `notebook/notebook-document.md#CELL_MODEL` `CapabilityPin` to compose `Rasm.AppHost/determinism/determinism-and-replay.md` `DeterminismContext`/`EnvFingerprint` as its environment identity, and `#REPLAY_BUNDLE` `Verify` to compose `EventLog`/`ContentHash`/`ReplayVerify` content-hash identity instead of the notebook-local `hash(output)` comparison, so the notebook reproducibility-proof is one owner with the runtime determinism kernel.
- Integrate no new AppUi package — `System.IO.Hashing` (the suite `XxHash128` identity row both pages already cite) and the AppHost determinism vocabulary are admitted; the journal-replay route already binds `ProofEngine.Replay`.
- Wires align to named boundaries: `Rasm.AppHost/determinism/determinism-and-replay.md#DETERMINISM_KERNEL` (`DeterminismContext.Reproduces`), `#EVENT_LOG` (`ContentHash`/`LogEntry`), and `#REPLAY_VERIFY` (`ReplayVerify.Replay`) consumed as settled vocabulary at the package edge; the notebook reproducibility receipt rides the existing `runtime-ports#TS_PROJECTION` `ReceiptEnvelopeWire`/`LogEntryWire`, never a notebook-only wire shape.
- The notebook is a UI projection over the runtime determinism owner — a parallel reproducibility-hash fold inside `notebook/` is the rejected form; the `RecomputePlan` cell DAG aligns to the AppHost `RecomputeGraph` content-address node identity so the incremental-recompute law is one owner across the runtime and the document.
