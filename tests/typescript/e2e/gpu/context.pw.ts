...
test.describe('gpu lane', () => {
    test('webgl2 acquires under the lane flags; the adapter identity rides as evidence', async ({ page, target }) => {
    ...
    test('a bogus context kind refutes the acquisition probe', async ({ page, target }) => {
        await target.open('/panel');
