...
// The portable cutting-stock PLAN admitted from a Rasm.Materials Construction/nesting StockNest.Pack solve — the
// host-neutral scalar mirror of the Materials NestPlan/NestYield carrying NO MaterialId, NO Rhino.Geometry, and NO
// Rasm.Materials reference (the acyclic AEC strata forbids the peer edge; an app-stratum mediator above both peers
// maps a real NestPlan onto this wire). PartId indexes the FabricationInput.Profiles the mediator aligned to the
// material cut-list. Materials owns the rectangular cutting-stock yield, so when this plan is present Nest.Honor maps
// it straight to PartTransform rather than minting a second rectangular packer beside the true-shape kernel; the
// remnant/waste metrics stay on the Materials receipt, only the layout + carried utilization/sheet-count/unplaced cross.
public readonly record struct PlannedPlacement(int PartId, int SheetIndex, double XMm, double YMm, double WidthMm, double HeightMm, bool Rotated);

public sealed record CuttingPlan(Seq<PlannedPlacement> Placements, double Utilization, int SheetCount, int Unplaced);
...
public static class Nest {
    public static Fin<FabricationResult> Solve(FabricationPolicy.Nest policy, FabricationInput input) =>
        input.Profiles.IsEmpty
            ...
            : input.Plan.Match(
                // A pre-resolved Rasm.Materials cutting-stock plan is present: HONOR it (the strata peer owns the
    ...
    // Consume the portable Rasm.Materials CuttingPlan as wire data: map each PlannedPlacement straight to a
    // PartTransform — Materials owns the rectangular cutting-stock yield, Fabrication HONORS the resolved layout
    // rather than minting a second rectangular packer. The placed (XMm, YMm) is the part footprint's lower-left, so
    // each transform rotates the part about the origin by the 90° flag then offsets its rotated bbox-min to that
    // corner (the exact dual of the rect-fastpath arm's bbox-min anchoring, reusing the one Transform fold, never a
    // second rotation kernel). The rectangular remnants ride the Materials NestYield receipt, so the honored Placement
    // mints no Fabrication Remnant; an out-of-range PartId the mediator mis-aligned is dropped, an all-dropped plan
    // railing NoFit, the carried Utilization/Unplaced passed straight through.
    static Fin<FabricationResult> Honor(Arr<Loop> parts, CuttingPlan plan) {
        Seq<PartTransform> placed = plan.Placements
            .Filter(pp => pp.PartId >= 0 && pp.PartId < parts.Count)
            .Map(pp => {
                double rot = pp.Rotated ? Math.PI / 2.0 : 0.0;
                BoundingBox b = Transform(parts[pp.PartId], new PartTransform(pp.PartId, 0.0, 0.0, rot)).Bound();
                return new PartTransform(pp.PartId, pp.XMm - b.Min.X, pp.YMm - b.Min.Y, rot, pp.SheetIndex);
            });
        return placed.IsEmpty
            ? Fin.Fail<FabricationResult>(FabricationFault.NoFit("nest:empty-plan").ToError())
            : Fin.Succ((FabricationResult)new FabricationResult.Placement(placed, plan.Utilization, plan.Unplaced, Seq<Remnant>()));
    }
...
flowchart LR
    ...
    Plan["Rasm.Materials CuttingPlan (FabricationInput.Plan)"] -->|"plan present → Nest.Honor"| Honor["map PlannedPlacement → PartTransform"]
