...
| :-----: | :--------------------------------------- | :------------------ | :-------------------------------------------------------------- |
|  [01]   | `PublicKeyCredentialCreationOptionsJSON` | interface           | challenge options sent to browser for registration              |
|  [02]   | `RegistrationResponseJSON`               | interface           | browser attestation response sent to server                     |
|  [03]   | `AuthenticatorAttestationResponseJSON`   | interface           | inner attestation response fields                               |
|  [04]   | `VerifiedRegistrationResponse`           | discriminated union | `{ verified: false }` \| `{ verified: true; registrationInfo }` |
|  [05]   | `GenerateRegistrationOptionsOpts`        | type alias          | `Parameters<typeof generateRegistrationOptions>[0]`             |
|  [06]   | `VerifyRegistrationResponseOpts`         | type alias          | `Parameters<typeof verifyRegistrationResponse>[0]`              |
...
| [INDEX] | [SYMBOL]                                | [TYPE_FAMILY] | [RAIL]                                                |
| :-----: | :-------------------------------------- | :------------ | :---------------------------------------------------- |
|  [01]   | `PublicKeyCredentialRequestOptionsJSON` | interface     | challenge options sent to browser for authentication  |
|  [02]   | `AuthenticationResponseJSON`            | interface     | browser assertion response sent to server             |
|  [03]   | `AuthenticatorAssertionResponseJSON`    | interface     | inner assertion response fields                       |
|  [04]   | `VerifiedAuthenticationResponse`        | interface     | `{ verified; authenticationInfo }` result             |
|  [05]   | `GenerateAuthenticationOptionsOpts`     | type alias    | `Parameters<typeof generateAuthenticationOptions>[0]` |
|  [06]   | `VerifyAuthenticationResponseOpts`      | type alias    | `Parameters<typeof verifyAuthenticationResponse>[0]`  |
...
| [INDEX] | [SYMBOL]                       | [TYPE_FAMILY]    | [RAIL]                                                                                                  |
| :-----: | :----------------------------- | :--------------- | :------------------------------------------------------------------------------------------------------ |
|  [01]   | `WebAuthnCredential`           | type alias       | `{ id: Base64URLString; publicKey: Uint8Array_; counter: number; transports? }` — stored credential row |
|  [02]   | `Base64URLString`              | type alias       | branded `string` for base64url-encoded values                                                           |
|  [03]   | `Uint8Array_`                  | type alias       | `ReturnType<Uint8Array['slice']>`; bridges TS 5.6/5.7 generics                                          |
|  [04]   | `CredentialDeviceType`         | type alias       | `'singleDevice' \| 'multiDevice'`                                                                       |
|  [05]   | `AuthenticatorTransportFuture` | type alias       | `'ble' \| 'cable' \| 'hybrid' \| 'internal' \| 'nfc' \| 'smart-card' \| 'usb'`                          |
|  [06]   | `AttestationFormat`            | type alias       | `'fido-u2f' \| 'packed' \| 'android-safetynet' \| 'android-key' \| 'tpm' \| 'apple' \| 'none'`          |
|  [07]   | `PublicKeyCredentialHint`      | type alias       | `'hybrid' \| 'security-key' \| 'client-device'`                                                         |
|  [08]   | `COSEAlgorithmIdentifier`      | type alias (DOM) | numeric COSE algorithm ID (e.g. `-8`, `-7`, `-257`)                                                     |
|  [09]   | `VerificationMode`             | type alias       | `'permissive' \| 'strict'`; controls `MetadataService` AAGUID policy                                    |
...
| [INDEX] | [SYMBOL]              | [TYPE_FAMILY] | [RAIL]                                                               |
| :-----: | :-------------------- | :------------ | :------------------------------------------------------------------- |
|  [01]   | `MetadataService`     | singleton     | downloads FIDO MDS BLOBs; `initialize()` + `getStatement(aaguid)`    |
|  [02]   | `SettingsService`     | singleton     | `setRootCertificates()` + `getRootCertificates()` by format id       |
|  [03]   | `BaseMetadataService` | class         | concrete `MetadataService` implementation; extend for custom caching |
|  [04]   | `RootCertIdentifier`  | type alias    | `AttestationFormat \| 'mds'`                                         |
...
| :-----: | :----------------------------------------------------------------------------------- | :-------------- | :-------------------------------------------------------------------- |
|  [01]   | `generateRegistrationOptions(opts): Promise<PublicKeyCredentialCreationOptionsJSON>` | async generator | builds challenge options for browser `navigator.credentials.create()` |
|  [02]   | `verifyRegistrationResponse(opts): Promise<VerifiedRegistrationResponse>`            | async verifier  | verifies attestation; returns `verified` flag and `registrationInfo`  |
|  [03]   | `supportedCOSEAlgorithmIdentifiers: COSEAlgorithmIdentifier[]`                       | constant        | default supported algorithm IDs `[-8, -7, -257]`                      |
...
| [INDEX] | [SURFACE]                                                                             | [ENTRY_FAMILY]  | [RAIL]                                                             |
| :-----: | :------------------------------------------------------------------------------------ | :-------------- | :----------------------------------------------------------------- |
|  [01]   | `generateAuthenticationOptions(opts): Promise<PublicKeyCredentialRequestOptionsJSON>` | async generator | builds challenge options for browser `navigator.credentials.get()` |
|  [02]   | `verifyAuthenticationResponse(opts): Promise<VerifiedAuthenticationResponse>`         | async verifier  | verifies assertion; returns `verified` + `authenticationInfo`      |
...
| [INDEX] | [SURFACE]                                                                       | [ENTRY_FAMILY] | [RAIL]                                        |
| :-----: | :------------------------------------------------------------------------------ | :------------- | :-------------------------------------------- |
|  [01]   | `MetadataService.initialize(opts?): Promise<void>`                              | async init     | fetches MDS BLOBs; call once at startup       |
|  [02]   | `MetadataService.getStatement(aaguid): Promise<MetadataStatement \| undefined>` | async query    | resolves cached or freshly fetched MDS entry  |
|  [03]   | `SettingsService.setRootCertificates(opts): void`                               | mutator        | register root certs by `RootCertIdentifier`   |
|  [04]   | `SettingsService.getRootCertificates(opts): string[]`                           | accessor       | retrieve PEM-encoded root certs by identifier |
