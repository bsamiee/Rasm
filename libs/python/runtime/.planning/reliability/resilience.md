...
- Boundary: no retry around a pure transform — `stamina` rides only flaky external oracles through this table. The exported contract is branch-consumer law: the entry family, every `POLICY` row, and the `RetryClass` vocabulary; narrowing the `OCCT` target below the `BrokenWorkerInterpreter | BrokenWorkerProcess` pair is a cross-folder break.
...
def _named(*names: str) -> BackoffHook:
    # matches the raise's type or any base over the MRO by qualname (dotted names compare module-qualified) —
    # isinstance semantics over a gated provider's subclass tree with zero imports.
...
def _adbc_transient(*transient: type[Exception]) -> BackoffHook:
    ...
    # never `INVALID_ARGUMENT`/`NOT_FOUND` a re-issue cannot clear); stdlib classes retry unconditionally.
...
def _wire_transient(*transient: type[Exception]) -> BackoffHook:
    # an `AioRpcError` retries only on the transient status trio; every other status is terminal.
    def backoff(exc: Exception) -> bool:
        match exc:
...
# the sync mirror: same `Policy.schedule` source, second runtime arm, one table.
@cache
...
POLICY: Final[Map[RetryClass, Policy]] = Map.of_seq([
    ...
    # the consumer's trailer fence owns the terminal `AioRpcError` lift so the typed detail survives.
