...
[STACKING]:
- `Diagnostics/devloop.md` is the sole consumer anchor — the HUD/hot-reload/replay-verify dev loop mounts this inspector beside `HotAvalonia`'s XAML hot-reload; the two share the `Debug` `PrivateAssets` gate and never co-mount in Release; its `InspectorCapture` and `InspectorEdits` bodies are the two extension seams above, so the snapshot lands on the capture encode fold and the commit fires on the AppUi fact rail.
