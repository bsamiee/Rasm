...
- refresh manager: `ConfigurationManager<OpenIdConnectConfiguration>` (a `BaseConfigurationManager` from `Microsoft.IdentityModel.Protocols`, cataloged at `api-identitymodel-protocols.md`) wraps a metadata address, a `OpenIdConnectConfigurationRetriever`, and an `IDocumentRetriever`/`HttpDocumentRetriever`; it caches the configuration with `AutomaticRefreshInterval`/`RefreshInterval`/last-known-good and is the instance assigned to `TokenValidationParameters.ConfigurationManager` — this is the integration seam that hands the discovery JWKS to the Tokens validators and the JWT handler. The `OpenIdConnectConfigurationRetriever` here is the `IConfigurationRetriever<T>` that specializes `T = OpenIdConnectConfiguration`
