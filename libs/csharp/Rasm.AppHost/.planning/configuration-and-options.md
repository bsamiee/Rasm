...
| :-----: | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
|   [1]   | secrets-store keychain provider route (Security framework P/Invoke versus /usr/bin/security child process) behind `ConfigLayer.SecretsSource` | dotnet run on a scratch keychain spike exercising both routes for read latency, entitlement prompts, and headless behavior              | SOURCE_AXIS   |
|   [2]   | Microsoft.Extensions.Options.ConfigurationExtensions admission row owning the `BindConfiguration(section, configureBinder)` surface    | uv run python -m tools.assay api query options.configurationextensions OptionsBuilderConfigurationExtensions.BindConfiguration          | POLICY_VALUES |
|   [3]   | source-generated binder interception coverage for `Get<T>` with `BinderOptions.ErrorOnUnknownConfiguration` under EnableConfigurationBindingGenerator | dotnet build of a probe project with the generator property set, then binlog inspection of the emitted interceptors for the `Get<T>` call shape | TYPED_BINDING |
|   [4]   | host-document transition-reload route over the static in-memory mount (re-mount through the host-attach trigger versus a custom reload-token source)   | dotnet run over a scratch ConfigurationManager probe re-adding the host-document snapshot and asserting one options-monitor change invocation    | SOURCE_AXIS   |
