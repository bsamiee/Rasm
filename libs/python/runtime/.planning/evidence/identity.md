...
class ContentIdentity:
    ...
    def key(
        ...
        *,
        ...
        # the fold is total and the key returns BARE (no rail) — the fallible `canonical` `Struct` source stays on
        # the railed `of`, and `SeedReproduction` keeps `of` for its uniform `Result` fold. The `content.derive`
        # span still opens (same tracing), so `key` and `of` differ ONLY in the fallibility contract, never a `rail: bool` knob.
        lifted = IdentitySource.lift(source)
        with _TRACER.start_as_current_span("content.derive") as span:
            if span.is_recording():
                span.set_attributes({"identity.fmt": fmt, "identity.modality": lifted.tag})
            value, byte_length = lifted.fold(seed.default_with(lambda: cls.seed(fmt, policy)))
            if span.is_recording():
                span.set_attribute("identity.key", f"{value:032x}")  # `032x` render: the raw `U128` overflows the OTLP signed-int64 bound
            span.set_status(Status(StatusCode.OK))
...
# csharp:Rasm/Spatial/reconciliation#ONE_WIRE_FIXTURE_CORPUS[1]: the 52-byte int32-LE
