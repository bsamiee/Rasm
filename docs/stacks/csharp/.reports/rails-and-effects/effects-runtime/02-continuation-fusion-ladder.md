...
[DESIGN_PRESSURE]:
- The ladder is the reason a deep monadic pipeline is allocation-cheap to *build* and the reason an extension carrier must never hand-roll its own `Map`/`Bind` nodes: a step expressed outside the sanctioned continuation bases is invisible to the ladder — the chain cannot fold into it, so every operation around it allocates unfused nodes. A custom effect step is expressed as one of the legal base nodes whose `Invoke` returns a fused continuation — never as a bespoke wrapper the ladder cannot collapse into.
