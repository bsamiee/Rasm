...
public abstract partial record VectorIntent {
    ...
    // The extraction band: construction lives at extract.md's request union; this rail carries it as ONE case.
    ...
    // CloudTransportPolicy admits at CloudTransportPolicy.Of; the constructed record is the evidence —
    // only the default-struct sentinel is rejected here.
    ...
    private Fin<TOut> Dispatch<TOut>(Context context, Op op) => Switch(
        ...
        // cloud.md owns both entries — RingNormalOf (CCW-signed normal) and PlanarWindingOf (angle-sum fold);
        // this arm chains the two and projects the integer, computing nothing itself.
