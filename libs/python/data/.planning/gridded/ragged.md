...
- [AWKWARD_SURFACE]: the `awkward` ingest, export, and transform member surface the owner transcribes is catalogue-confirmed against the folder `awkward` `.api` — ingest `from_iter`/`from_numpy(regulararray=)`/`from_arrow(generate_bitmasks=)`/`from_json`/`from_parquet`/`from_buffers(form, length, container)` (ENTRYPOINTS construction L63-68); export `to_arrow_table(extensionarray=)` the native `__arrow_c_stream__`-exporting `pyarrow.Table` lowering / `to_arrow(extensionarray=)` the `__arrow_c_array__`-only `pyarrow.Array` escape / `to_parquet`/`to_json`/`to_numpy`/`to_list`/`to_backend`/`to_layout`/`to_buffers`/`to_packed` (ENTRYPOINTS export L78-85); transform `concatenate(axis=)`, `zip`/`unzip`, `flatten`/`unflatten`, `where(condition, *args)`, `sort(array, axis, *, ascending, stable, ...)`/`argsort(array, axis, ...)` (the `ascending`/`stable` order knobs the `_ordered` closure folds, order row L96), `pad_none(target, axis, *, clip)`/`fill_none`, `drop_none`/`is_none`/`mask(valid_when=)`, `with_field`/`without_field`, `with_name`, `with_parameter`, `cartesian(arrays, axis)`/`combinations(array, n, *, replacement, axis)`, the twelve reduce members, `fields`/`type`/`backend`, `firsts`/`singletons`/`local_index`/`run_lengths`, `values_astype`/`enforce_type`/`ravel`/`copy` (ENTRYPOINTS transform L92-105); the `combinations` `replacement` with-replacement knob and the `pad_none` `clip` exact-length knob ride the `combinations`/`pad` case payloads as the third tuple slot, both catalogue-confirmed on the same transform rows; the `ak.Array.{layout,fields,type,ndim,nbytes,mask,to_list,to_numpy}` properties; the `ak.Array`/`ak.Record`/`ak.ArrayBuilder` core types. The `concat` and `where` arms bind catalogued `ak.concatenate(arrays, axis=)` and `ak.where(condition, x, y)`; the `project` arm binds the catalogued `ak.Array` named-field indexing (`array[field]` returning a single `ak.Array`), the array-to-array projection the array-to-tuple `ak.unzip` cannot be — `ak.unzip` is intentionally not a transform arm because it returns a tuple of arrays, not the single `ak.Array` the axis returns, and field projection rides `project`/`fields` and named indexing, never a re-zip identity. The `_AXIS_OP` member table binds exactly the seven catalogued `member(array, axis=...)` rows — `ak.flatten`/`ak.num`/`ak.firsts`/`ak.local_index`/`ak.singletons` (transform/structure L103/L113), `ak.drop_none`/`ak.is_none` (option L107) — every member sharing the one `axis=` call-head the guarded `_apply` arm forwards, so the seven-way switch collapses onto one table read with no per-member arm; the six per-list members carry an `int` axis while `ak.drop_none` alone admits `axis=None` (its catalogued all-levels default), so the `drop` case payload is `int | None` and the `_AXIS_OP[kind](array, axis=getattr(op, kind))` forward threads the `None` straight to `drop_none` without a second arm; `ak.run_lengths`/`ak.ravel`/`ak.to_packed` stay distinct arms because they carry no `axis` knob. `awkward` is cp315-clean and imports module-top, the `awkward-cpp` native kernels driving the layout operations. Settled fence code.
