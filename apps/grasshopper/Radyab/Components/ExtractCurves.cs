...
[Nomen(
    ...
    category: "Radyab",
    section: "Extraction")]
public sealed class ExtractCurves : Component<ExtractCurves.Spec> {
    public sealed class Spec : IComponentSpec {
        ...
        public static Seq<IOutputGroup> Outputs { get; } = Seq<IOutputGroup>(
            ShapeOutput.CurveDetails(
                ...
                sources: Port.List<ComponentIndex>(kind: PortKind.Generic, name: "Curve Sources", code: "CS", info: "Component index aligned with All Curves."),
                features: Port.List<CurveFeature>(kind: PortKind.Enum(initial: CurveFeature.Input), name: "Curve Features", code: "CF", info: "Feature label aligned with All Curves."),
            ...
            ShapeOutput.Query(
                ...
                operation: static (access, runtime) => Rasm.Analysis.Query.Curves<object, Curve>(aspect: Rasm.Analysis.Curves.Silhouette(direction: runtime.Hints.Value(access: access, port: Direction).Map(static value => (Vector3d?)value).IfNone(static () => null))),
            ...
            ShapeOutput.Query(
                ...
                operation: static (access, runtime) => Rasm.Analysis.Query.Curves<object, Curve>(aspect: Rasm.Analysis.Curves.Draft(
                    direction: runtime.Hints.Value(access: access, port: Direction).Map(static value => (Vector3d?)value).IfNone(static () => null),
            ...
            ShapeOutput.Query(
                ...
                operation: static (access, runtime) => Rasm.Analysis.Query.Curves<object, Curve>(aspect: Rasm.Analysis.Curves.At(index: runtime.Hints.Index(access: access, port: Index, limit: int.MaxValue).Map(static value => (int?)value).IfNone(static () => null)))));

        private static IOutputGroup CurvesOutput(Port<Curve> port, Rasm.Analysis.Curves aspect) =>
            ShapeOutput.Query(input: Geometry, port: port, operation: (_, _) => Rasm.Analysis.Query.Curves<object, Curve>(aspect: aspect), emptyUnsupported: true);
    }
