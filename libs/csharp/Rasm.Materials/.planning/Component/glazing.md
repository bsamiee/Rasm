...
using Rasm.Materials.Component;      // Component, ComponentSection, ComponentId, ComponentStandard, ComponentAuthority, ComponentFault, ComponentCatalogue (the parent COMPONENT_OWNER)
using Thinktecture;
...
public readonly record struct GlazingSection(
    ...
    // Fire property the lowering emits is always backed), and the face dimensions positive. A malformed arity, an unmodeled
    // pane count, or a fire/EI mismatch rails the ComponentFault.Family band rather than seeding a unit whose DERIVED Build
    // silently mislabels — so a section that survives Of has a TOTAL Build read.
            ...
            : GlazingBuild.OfPaneCount(panes.Count).IsNone
                ...
                : panes.Exists(static p => p.Glass == GlassType.FireRated) != (fireEiMinutes > 0)
                    ...
                    : from fw in key.AcceptValidated<PositiveMagnitude>(candidate: faceWidthMm)
                      ...
                      select new GlazingSection(panes, cavities, spacer, edgeSeal, fw, fh, muntin, fireEiMinutes);
...
public static class ComponentCatalogue {
    ...
    // The registered ComponentId -> Component rows the parent ComponentCatalogue.Build folds. A glazing Component carries the
    // ComponentSection.Glazing arm (the cross-section FIELD) and no ComputedSection (the IGU is an IfcMaterialLayerSet), so
    // glazing contributes NO ComponentCatalogue.Sections entry and the M7 ProfileResolution dereferences a glazing
    // ComponentId to (Component, None). The appearance is the outboard pane's glass row.
    public static FrozenDictionary<ComponentId, Component> BuildGlazingRows(Context context) =>
        GlazingRows
            ...
            .ToFrozenDictionary(
                ...
                static shape => new Component(ComponentFamily.Glazing, new ComponentSection.Glazing(shape.Section), shape.Standard, shape.Section.Panes[0].Glass.Appearance),
                ComparerAccessors.StringOrdinal.EqualityComparer);
}
