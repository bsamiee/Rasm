...
Stored `Func<…, bool>` query filters are the banned form everywhere this algebra reaches: a delegate is opaque to store push-down, unserializable across a dashboard or publish wire, and unhashable for a memo key. `PredicateKey` streams any predicate value into the seam content key a replayable selection and a cache share; the JSON wire crossing rides the CONTRACTED `Rasm.Bim` `PredicateWire`/`PredicateCodec` family (`tests/contracts/MANIFEST.md` names `csharp:Rasm.Bim/Model/query#PREDICATE_WIRE` producer; the TypeScript `interchange/codec.md` `PredicateWire` duplex mirror is live). Evaluation stays with each consumer (Bim's graph-bound query, Persistence's SQL/DuckDB lowering, AppUi's live-data compile): this page owns the ALGEBRA, the verdict, and the byte projection — never an interpreter and never a second wire family beside the contract.
...
- Law: the JSON predicate crossing is the CONTRACTED `Rasm.Bim` family and this page mints no second one — `tests/contracts/MANIFEST.md` freezes `PredicateWire` under `PredicateCodec.Seal`/`Admit` (`Model/query#PREDICATE_WIRE`) with the TypeScript duplex mirror live, so the W3 Bim recompose RE-TARGETS that codec's interior onto the seam algebra (its `ByTypeWire`/`ByZoneWire` discriminators lower onto `ByAssigned`, its restriction records onto the seam `ValueMatch`) and KEEPS the frozen wire face; deleting the round-trip before the re-target lands is the barred order.
...
- Growth: a new Element-payload query dimension is one `ElementLeaf` arm and one `CanonicalBytes` case (the generated `Switch` breaks every consumer loudly), with one wire record and one `Seal`/`Admit` arm pair landing at the CONTRACT owner once Bim recomposes; a new restriction facet is one `ValueMatch` arm; a peer's new query dimension is one arm on ITS leaf family with zero edits here; a new result identity regime is one `Selection<TKey>` instantiation; never a parallel closure family, never a stored delegate filter, never a per-consumer verdict shape, and never an evaluation interpreter on this page.
- Boundary: this page is HOST-NEUTRAL vocabulary and projection — no graph walk, no store lowering, no UI compile lives here; `Predicate<TLeaf>.Holds` is a structural fold whose leaf and closure verdicts the CALLER supplies, so evaluation authority stays with the folder that owns the data it runs over. `Selection<TKey>` deliberately carries NO scope column: `ModelId` is Persistence's and a query's scope is the QUERY's input, not the result's — Persistence's wrapper carries `(Selection<SetKey>, SetScope, Receipt, Preimage)` and its canonical preimage framing stays at its own `Selections`; CloudEvents `FilterDialect` and CESQL grammars are wire-spec-fixed and stay at Persistence composing nothing here beyond the verdict. Name law: `ElementSet` resolves to ONE declaration corpus-wide (Bim renames `ElementQuery`, Persistence renames `KeySelection` — both W3); the NAME `PredicateWire` stays the Bim wire family's (contract-frozen), so the byte projection here is `PredicateKey`; `MatchVerdict` is named past the `Verdict` collisions (GH canvas, Rhino `SeatVerdict`) — `Rasm.Rhino` `Render/registry.md` declares a same-named host `RenderContent.MatchDataResult` row vocabulary on the barred host plane (no shared reach; the W3 ledger holds the rename question).
...
public abstract partial record ElementLeaf {
 ...
 // type-definition and group rows serve Bim's contracted ByTypeWire/ByZoneWire discriminators (the IDS partOf
 // Grouped lowering at Review/validation.md consumes the Group row), and a kind-suffixed sibling pair was the
 // rejected arity twin.
...
// seam content key a replayable selection and a memo share, which is the structural proof a stored
// Func<...,bool> filter can never join; the JSON wire crossing rides elsewhere — the contracted Rasm.Bim
// PredicateWire/PredicateCodec family (MANIFEST rasm.bim + the TS duplex mirror), whose W3 recompose re-targets
// its interior onto this algebra while its frozen wire face stands.
