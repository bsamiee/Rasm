...
- Law: failed notification or tray acquisition independently detaches and disposes each newly minted host widget without consuming input image, tray, or menu leases. `NoticeMount.LastFault` and `TrayMount.LastFault` retain release failures; a dependent image/menu cannot release before its host detaches. The obsolete `Notification.Icon` path is deleted, and notification/tray activation facts remain inside the owning lease windows.
