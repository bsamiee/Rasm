...
- Law: `BaseException.add_note(...)` annotates the caught exception with its call coordinate at the capture site, and the fault shape — not the note — decides whether the provider message survives inward: a category-only `Literal` keeps the recovery key while a message-carrying case (`[04]`'s `of_detail`) preserves the message, so the annotated coordinate enriches a defect that propagates or a `Try` payload carried to a single egress, never a second channel beside the typed case. A `@beartype` type-hint violation is one more provider raise this adapter catches, and the alternative that redirects the violation onto the carrier without a `try` is the surfaces page's guard weave, composed at definition time, not a second capture form taught here.
- Law: the runtime raises — a `TimeoutError` from a deadline scope, a `BaseExceptionGroup` from a task group, the cancellation class — cross this boundary as one more fault case each, but the `except*` group split and the scope that raised them are `concurrency.md`'s group edge; this card states only that the converted raise lands as a vocabulary case, never the group mechanics.
- Reject: `contextlib.suppress` hiding fallibility; a bare `try`/`except` wrapping interior carrier transforms; a bare `except Exception` silently carrying an unclassified defect; discarding the provider cause where the fault case carries it; `effect.try_` as a raw-exception trap — it composes `Try` values through `yield from` and never catches a provider raise, where `expression.extra.result.catch` is the substrate's actual single-exception trap.
...
- Law: a worker kernel returning `Result[T, E]` crossed through a runtime offload comes back as one result nested over the carrier, and the owner flattens it exactly once at the offload call — the outer boundary fault migrates onto one domain case, then `bind` threads the inner carrier — so the composed signature reads `Result[T, E | BoundaryFault]`; annotating the outer result as if the inner carrier did not exist mis-types every downstream `map`, and transporting the inner fault by a custom exception across the worker boundary is the rejected inversion.
- Reject: a round trip where an explicit projection carries the fault through; `Option.value` access without a prior `is_some` proof; an intermediate variable for a sequential transform `pipe` threads; a wrapper renaming `pipe`/`compose`; a collapse inside a pure projection that stays carried.
...
- Law: `Ok`/`Error`/`Some` are constructor functions, never classes, so carrier decomposition spells `case Result(tag="ok", ok=value)` / `case Result(tag="error", error=fault)` and `case Option(tag="some", some=value)` / `case Option(tag="none")` — a `case Ok(value)` class pattern raises `TypeError: class pattern must refer to a class` at the first match.
...
- Reject: a mid-pipeline collapse inside a pure projection; a `match` collapsing a carrier the next transform still needs carried; `raise`-and-log inside an interior `bind`; a bare value returned from a reusable transform where the carrier must survive to the edge.
...
type CaptureFault = Literal["<malformed>", "<missing>", "<host>", "<type-hint>", "<empty-key>"]
...
Traversal is carrier policy: the collection owner and the sequencing operator together decide how failures, strictness, and order compose. Operator choice is the sequencing decision, never a performance choice — fail-fast threads the carrier and aborts on the first `Error`, accumulating folds every element and combines the faults.
...
- Law: `Map` is an ordered tree — every key type carries a total order (`order=True` on a `msgspec.Struct` key, or the fold keys on a projected ordered token with the struct riding the value), a non-comparable or heterogeneous key set rides `frozendict`, and a second insert over an unordered key raises `TypeError` after every single-entry test passes; iteration is key-sorted, so a derivation that depends on insertion order — a first-wins inversion, a precedence election — rides `frozendict`, never `Map`.
...
- Reject: a mutable `list` appended in a `for` loop as domain flow; a bare `list`/`dict` carried as a carrier's success payload where `Block`/`Map` is the persistent owner the carrier should hold; re-enumerating a lazy `Seq` whose cost is paid per pass.

[SEQUENCING_OPERATOR]:
- Law: fail-fast traversal is `expression.extra.result.traverse(step, block)` — the substrate's own applicative threader that maps each element through a `Result`-returning step, short-circuits the whole `Block` on the first `Error`, and collects every `Ok` into `Result[Block[T], E]` — and `sequence(block_of_results)` is `traverse(identity, ...)` for an already-carried `Block`; the hand-rolled seed-`bind` fold over `Ok(Block.empty())` is the rejected reimplementation of a primitive the package already ships.
...
def resequenced(results: Block[Result[int, TraverseFault]], /) -> Result[Block[int], TraverseFault]:
...
- Law: every case in the family has a minting site in its owning surface — a fact the runtime already classifies (a worker death, a type-hint violation) weaves the runtime's own conversion instead of minting a local case no site reaches, and a case nothing mints is deleted, not diagrammed.
...
def reported(scores: Block[tuple[Option[int], int, int]], disposition: Disposition, /) -> Result[Block[int], Fault]:
    results = scores.starmap(guarded)
    match disposition:
        case Disposition.ABORT:
            return sequence(results)
        case Disposition.ACCUMULATE:
            faults = results.choose(lambda r: r.swap().to_option())
            return Ok(results.choose(lambda r: r.to_option())) if faults.is_empty() else Error(faults.reduce(Fault.combined))
