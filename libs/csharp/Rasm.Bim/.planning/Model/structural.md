...
public static class StructuralProjection {
    ...
    // this closes. The egress target database is SI by construction, so the bag's SI magnitudes land verbatim
    // (no inverse UnitScale fold exists to get wrong; ctors + settable columns decompile-verified). TOTAL and
    // void: a non-structural entity or a bag with no restraint/load facts re-stamps nothing — the ingest-landed
    // round-trip state is simply absent, never a fabricated condition or a zero-filled load.
    public static void Author(DatabaseIfc db, IfcObjectDefinition entity, Map<PropertyName, PropertyValue> attrs) {
        switch (entity) {
            case IfcStructuralConnection connection when attrs.ContainsKey(PropertyName.Create("TranslationX")):
                connection.AppliedCondition = new IfcBoundaryNodeCondition(db, "",
                ...
                break;
            case IfcStructuralActivity activity when attrs.ContainsKey(PropertyName.Create("ForceX")):
                activity.AppliedLoad = new IfcStructuralLoadSingleForce(db, Si(attrs, "ForceX"), Si(attrs, "ForceY"), Si(attrs, "ForceZ")) {
    ...
    static IfcTranslationalStiffnessSelect Translational(Map<PropertyName, PropertyValue> attrs, string fixity, string spring) =>
    ...
    static IfcRotationalStiffnessSelect Rotational(Map<PropertyName, PropertyValue> attrs, string fixity, string spring) =>
        Si(attrs, spring) is > 0d and var k ? new IfcRotationalStiffnessSelect(k) : new IfcRotationalStiffnessSelect(Fixity(attrs, fixity));

    static double Si(Map<PropertyName, PropertyValue> attrs, string name) =>
        attrs.Find(PropertyName.Create(name)).Bind(static v => v is PropertyValue.Measure m ? Some(m.Value.Si) : None).IfNone(0d);

    static bool Fixity(Map<PropertyName, PropertyValue> attrs, string name) =>
