...
- Packages: `Rasm.Numerics` (composed — `SpectralArena.Interleaved`/`Transform`/`SpectralSense.Forward`/`SpectralScaling.Symmetric` the kernel transform band over the bounded luminance staging, `Spectrum.Power` the per-bin read), `plane#TEXTURE_PLANE` (composed — `TexturePyramid.Levels` the bounded grading level, `TexturePlane.Grid` the arena's own lattice, `TexturePlane.Read` the streaming decoded rail both signals fold), CommunityToolkit.HighPerformance (`SpanOwner<T>` the row rentals), BCL inbox (`System.Numerics.Complex`), `Rasm` (project — `ValidityClaim`, `Evidence`/`Evidence.Of` the kernel probe evidence the `Learned` column and the `Tiled` fill ride).
