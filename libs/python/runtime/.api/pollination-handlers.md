...
- companion-gated: admitted `python_version < '3.15'` alongside the Ladybug Tools cluster; absent from the cp315 frozen env (`api resolve --frozen` cannot reach it), so every member below is confirmed by introspection against the installed companion distribution (`0.10.16`)
- depends: `lbt-dragonfly (>=0.11.682)` — honeybee/honeybee-energy/honeybee-radiance, ladybug/ladybug-comfort, dragonfly/dragonfly-energy (the domain classes the handlers read and write)
...
| :-----: | :------------------------------------------------------------------------------ | :------------- | :---------------------------------------- |
|  [01]   | `inputs.model.model_to_json(model_obj)` (+ `_room_check`/`_hvac_check`/`_grid_check`/`_grid_room_check`/`_view_check`) / `inputs.model.model_dragonfly_to_json` | model coercion | honeybee/dragonfly `Model` \| path -> HBJSON/DFJSON path with preconditions |
|  [02]   | `inputs.wea.wea_handler` (+ `_timestep_check`/`_timestep_annual_check`/`epw_or_wea_handler_timestep_annual_check`) / `inputs.ddy.ddy_handler` | weather coercion | ladybug `Wea`/`EPW`/`DDY` \| path -> weather artifact |
...
|  [06]   | `inputs.bool_options.*` (`use_multiplier_to_str`, `is_residential_to_str`, `cloudy_bool_to_str`, `sky_view_metric_to_str`, …) / `inputs.radiancepar.validate_rpict_params` / `validate_rtrace_params` / `validate_rcontrib_params` / `validate_rfluxmtx_params` | option/validate | bool/option -> acceptable string; Radiance-parameter validation |
...
|  [02]   | `outputs.daylight.sort_ill_from_folder` / `read_images_from_folder` / `read_hourly_continuous_collection_from_json` / `read_leed_datacollection_from_folder` | collection reader | sorted `.ill`, images, hourly `DataCollection` |
|  [03]   | `outputs.daylight.read_leed_summary_grid` / `read_leed_shade_transmittance_schedule` / `read_grid_metrics` / `ill_credit_json_from_path` / `read_json_dict` / `read_json_summary_list` / `read_csv_summary_formatted` | summary reader | LEED/credit summaries and grid-metric dicts |
...
|  [05]   | `outputs.helper.read_sensor_grid_result(result_folder, extension, key, ...)` / `outputs.helper.read_grid_results(...)` | reader primitive | the shared per-grid result-file readers the daylight handlers compose over |
