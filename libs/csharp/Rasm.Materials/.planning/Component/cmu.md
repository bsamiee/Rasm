...
- Owner: `CmuSeed` the ONE generator; `CmuPhysics` the ONE physical receipt; the six FORM-law SmartEnums (runtime key lookup + derivation columns, so they STAY per `SEED_ROW_LAW` tier 3); `CmuRow` the AUTHORED standards table (no admitted producer exists); `CmuSeed.Table` the `ComponentId`-keyed frozen axis join an axis consumer pairs with the M7-resolved `Component.Profile` (the ONE legal axis path — the `DetailLane.Realization` bag carries exactly the `DetailSchema.ProfileSubtype` wire row, never an axis column).
...
- Entry: `CmuSeed.Rows(Context) : Fin<Seq<ComponentRow>>` traverses the typed table and proves grade/form plus fill/reinforcement correspondence before constructing the lattice. Invalid counts or bar presence fault `ComponentFault.Dimension`; degenerate, out-of-bounds, or overlapping cells fault inside `CellularRectangle.Of`; every failure aborts the catalogue. Every row seeds `Sectioned: true`, so the M7 map carries the admitted as-built section, and every row carries the one-row `DetailSchema.Realization` bag whose `ProfileSubtype` token is `CmuPhysics.IfcSubtypeOf` over the admitted lattice.
...
// the deleted CmuSection carried, now a bag-free derivation (the physics axes ride the seed join, never bag rows): every quantity is
...
public static class CmuSeed {
    ...
    // The TYPED axis join for axis consumers (the realization bag carries only the ProfileSubtype wire row, never an axis), keyed by the SAME
    ...
    // grouted cells filled), the M7 map the capacity#SECTION_CAPACITY MasonryCompression check reads. Every row seeds
    // the one-row DetailSchema.Realization bag: the ProfileSubtype token off the admitted lattice, the Bim-egress wire datum.
