...
- Packages: `RhinoCommon` (`Rasm.Rhino/.api/api-rhinocommon-commands.md` — the `GetPoint`/`GetObject`/`GetTransform` custom-get family this rail brackets); `Thinktecture.Runtime.Extensions` (`libs/csharp/.api/api-thinktecture-runtime-extensions.md` — the `[SmartEnum]` rule/constraint rosters, `[ComplexValueObject]` carriers, `[ObjectFactory]` grammar admission); `Riok.Mapperly` (`libs/csharp/.api/api-mapperly.md` — the `[Mapper]` option projection); `Generator.Equals` (`libs/csharp/.api/api-generator-equals.md` — `[Equatable]` structural equality); `NodaTime` (`libs/csharp/.api/api-nodatime.md` — the acquisition instant); kernel `Domain/rails` + `Numerics/atoms`.
