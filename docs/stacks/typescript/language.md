...
TypeScript under the native TS7 `tsc` is the active language surface, and this page is the legality law: what a legal file, import, export, annotation, merge, and statement is before any type or value is designed. Emit erases the type plane, so every construct is either fully erased or plain JavaScript: emit-bearing syntax is banned outright, one terminal exports block reads without inference, absence and index trust are spelled exactly, deferred evaluation is boot-edge vocabulary, declaration merging asserts only what a machine discharges, and statements survive only at named sites — the marked kernel, the boot edge, an owning page's platform exemption.

Everything above legality is shed by kind: type-level derivation rides `derivation.md`, value primitives and identity `values.md`, the kernel's earn test and algorithmic forms `computation.md`, shape authority `shapes.md`, call-surface collapse `surfaces-and-dispatch.md`, the carrier `results-and-effects.md`, capability wiring `services-and-layers.md`, and edge admission `boundaries.md`.
...
- Import law: emit is verbatim — nothing elides by inference, so type-only traffic is spelled at every import and export; deferral is two distinct forms — `import defer` postpones evaluation at the boot edge, the boundary loader postpones loading at its boundary
- Merge law: declaration merging is one-directional evidence — a type-only `declare namespace` merged on a value asserts nothing about runtime and merges freely; a same-name `interface` merged onto a `class` asserts implanted members and is legal only at the implanting site
- Export law: declarations are authored unexported and the file ends at one `// --- [EXPORTS]` block — `export { ... }` and `export type { ... }` — so the tail is the whole public surface; every exported name carries its annotation, the interior is `_`-prefixed, and no default export, re-export, or barrel exists
- Absence law: `?:`, `| undefined`, and the unproven index are three distinct type-level facts with three distinct spellings; the read spelling carries key provenance — dot proves a declared key, bracket marks index trust; domain absence is `Option`
- Statement law: statements, `throw`, mutation, `as`, and `!` live only at `// BOUNDARY ADAPTER` sites, and load-order statements only at the boot edge; this page sanctions the in-process kernel. `Effect.gen` bindings are the carrier's do-notation, never statement residue, and a platform-forced statement body is legal only under its owning page's named exemption
...
|  [02]   | mixed specifier               | one statement, inline `type` specifiers                 | a value line and a type line per specifier  |
|  [03]   | package namespace             | one named root binding under its canonical name         | per-member cherry-picking, alias drift      |
|  [04]   | shadowed global               | `globalThis.<Name>` at the FFI boundary                 | renaming the binding to spare a global      |
|  [05]   | load-time effect              | side-effect import leading a boot-edge module           | registration import inside a domain module  |
|  [06]   | qualified type, erased import | `Shape.Field` type access through the erased binding    | importing the value side for one type name  |
|  [07]   | data-file import              | `with { type: "json" }` ingress feeding decode boundary | a JSON binding trusted as domain shape      |
|  [08]   | deferred module load          | boundary loader; the promise converts at its boundary   | bare `import()` or `require` in domain flow |
...
|  [10]   | machine-implanted members  | same-name `interface` by its class, implant site  | a merge promising members nothing implants        |
...
|  [04]   | foreign-module binding   | ESM `import` with interop pinned at the FFI boundary | `import <name> = require(...)`, `export =` |
...
|  [05]   | open-index read              | bracket read lifted to `Option` at the boundary  | `!` assertion on the unproven cell               |
...
Each rule fixes the placement detail the chooser row cannot state. Snippets compose settled doctrine as supporting material; the spotlight is the legality form itself, and each rule closes on the boundary that hands the value to its owning page.

[COMPILER_AUTHORITY_SITE]:
- Law: TS7 `tsc` is the conformance authority — its diagnostic codes are the ones doctrine cites, and a repair lands at the shape the diagnostic indicts.
- Law: checker blowups concentrate where inference runs deepest — unbounded conditional recursion, implicit-instantiation blowups, inference-dependent export types; value-anchored types and annotation-explicit exports present the checker with shallow, decidable obligations, which is why this corpus legislates them.
...
- Reject: `@ts-nocheck` and `@ts-ignore` in any file; a `@ts-expect-error` spanning a live finding.
...
- Reject: a value line and a type line splitting one specifier; a value import consumed only in type positions — verbatim emit keeps it as a live load; alias rebinding to spare a shadowed global; a side-effect import, top-level statement, top-level `await`, or `import defer` inside a domain module — the top level is declaration space, registration is Layer material, and deferral moves the deferred module's effects to an arbitrary first-touch site, an execution-order statement smuggled into declaration space; bare `require` and `import()` in domain flow — deferred loading is a boundary loader whose promise converts at its owning boundary; an inline `import("<specifier>").Shape` type annotation where an erased import binding owns the name; a data-file import consumed as its inferred shape — the attribute-carried binding is raw ingress and crosses the decode boundary before domain use; an unresolvable asset specifier trusted or suppressed — the module shape is declared at the FFI boundary so the import resolves; `declare global`, ambient declarations, and triple-slash references; module augmentation anywhere but its two owned sites — own-registry row contribution at the declaring module (`derivation.md`'s) and foreign quirk capture at the engine owner (`boundaries.md`'s) — a surface changes at its owner, never by remote merge.
- Law: a root namespace binding deliberately shadows its global namesake on the value plane only — `Array.map` is the package's, a bare type position still resolves the global type, so `ReadonlyArray<string>` and `Array<number>` keep their built-in meaning, and the global value is reachable as `globalThis.<Name>` at the FFI boundary; one file reads both planes with zero aliases.
...
const Band = {
    // public vocabulary: bare `as const` emits clean; the shape check homes below, not in a satisfies clause
...
declare namespace Band {
    ...
    type _Rows<T extends Record<Kind, { readonly ceiling: number; readonly weight: number }> = typeof Band> = T; // row guard: satisfies on the anchor is TS9010 under declaration emit; the merged companion carries the proof and the anchor keeps its literals
}
...
- Law: these forms live at the type level — option bags, platform payloads, interop cells; a domain shape's absence is `Option` admitted at the Schema owner, and a type-level bag never crosses into domain flow carrying `undefined`.
...
declare namespace overlay {
    ...
    type Patch = {
        // type-level bag: domain absence is Option at the Schema owner, never these spellings
...
const overlay = (ledger: overlay.Ledger, band: Grade, key: string, label: string | undefined): overlay.Patch => ({
    probe: label,
    floor: Option.getOrElse(Option.fromNullable(ledger[key]), () => ledger.floor + Grade[band]), // dot proves the declared key; the bracket read is index trust lifted at the boundary — dot on the band is TS4111
...
- Use when: a measured hot path is faster or a platform API forces statements — the `_`-prefixed kernel marked `// BOUNDARY ADAPTER` is the sanctioned in-process site for statements, `throw`, mutation, `as`, and `!`.
- Accept: loops and mutable accumulators confined to one kernel whose result detaches immutable; `!` where a loop bound or scan invariant is the evidence the checker cannot carry under unchecked-index semantics; `as unknown` pinning an `any`-producing platform API — the pinned value exists only to cross the decode boundary; a platform throw crossing the kernel signature only into the owning carrier conversion; a `catch` binding narrowed from `unknown` before anything leaves the kernel.
- Reject: a statement loop outside a marked kernel; an accumulator escaping as live state; `as` or `!` in expression-shaped flow — evidence belongs in the key type, the decode boundary, or the declaration; a kernel throw consumed by anything but the conversion boundary; an authored throw signaling an input condition — input rejection is a returned value, and a kernel throw is a platform's own or an unreachable-default defect; a `using`/`await using` declaration standing in for resource lifetime — brackets are carrier material at the owning boundary; the mark on ordinary logic a fold expresses without measurement pressure.
...
- Boundary: the conversion combinators that lift a kernel throw are `results-and-effects.md`'s; worker and marshal statement bodies are `boundaries.md`'s; this site owns the in-process compute kernel.
...
const admit = (
    ...
    Effect.try(() => ({ shape: _parse(text), spans: _spans(bytes, 0x1e) })); // the kernel throw converts at the owning boundary; above this line everything is expression-shaped
...
- Done when: the tail block is the entire surface — one owner with at most one operation family, each entry annotated and each meaning merged — and the interior is invisible.
...
- Collapse: the boot edge owns execution order and the deferral lever, a data import crosses the decode boundary, capability rides the Layer graph, and ownership moves to break the cycle.
- Done when: every domain top level is declaration space and the value-plane import graph is a DAG.

[ASSERTION_RESIDUE]:
- Smell: `!`, `as`, or a swallowed `catch` sits in expression-shaped flow, a directive spans a strictness finding, or a same-name `interface` merge promises members nothing implants.
- Collapse: move the proof into the key type, the decode boundary, or the declaration; author the member on the class or route the promise to its implanting machine; confine what survives to the marked kernel.
...
- Collapse: one statement per specifier with inline `type` specifiers, the canonical binding with `globalThis.<Name>` at the boundary, `import type` for the fully erased statement.
