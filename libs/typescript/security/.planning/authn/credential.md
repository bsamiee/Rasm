...
- Law: `verify` and `redeem` are keyed brute-force targets — each runs under its own subject-keyed `Curb` row (`otp`, `recovery`), an exhausted budget folds to `throttled` at the guard, a `Rejected` verdict lands `Reject.mark("credential", { surface })` with the `otp`/`recovery` surface facet, and each entrypoint composes `Reject.measured("credential", { surface })` so the same facet carries the admission and the ceremony span; a guessing campaign is bounded by the store-backed limiter and legible as a ratio rather than as a raw count that a traffic spike reproduces.
...
- Packages: `otplib` (`verify`/`generateSecret`/`generateURI`, `createGuardrails`, `OTPGuardrails`, `OTPHooks`, `afterTimeStep`); `crypt/verify` (`Reject`, `Curb`); `Digest` (recovery); `Crypto` (ports).
