...
- Law: verbs speak roles, never colors — a verb marks a value `path` or a verdict `fault` and the table decides what that means on a terminal; a theme change is a row edit with zero verb diffs, and `Print.themed(palette)` is the theming mapper — one `Doc.reAnnotate` mapping every role annotation through a caller-supplied palette record, so a second palette is a value, never a second render path.
...
- Law: rows return `AnsiDoc` values, never strings — composition stays open (a verb nests a `table` under a `banner` with `Doc.vsep`) and the fold to text happens once at the port; a string-returning row re-closes the algebra per call site and is the rejected form.
...
- Law: live redraw is a directive row over the same port — `Print.sweep(rows)` renders the published `AnsiDoc.eraseLines(rows)` document through `Print.text` before the next `out`, so a progress loop is erase-then-render with zero cursor arithmetic in verbs and zero raw escape strings; the directive annotates `Doc.empty`, so `plain` and `wire` strip it to nothing and piped output stays append-only without a mode branch at this port.
- Law: deeply nested structures compose through `Print.deep` — `Optimize.optimize(doc, Optimize.Deep)` fuses associativity while preserving `AnsiDoc`; the ambient mode row still performs the only render, so optimization never opens a second terminal port.
- Boundary: `@effect/cli`'s own `HelpDoc` lowers onto this same `AnsiDoc` algebra, so parse-error help and verb output share one render port; the `Terminal` binding is the runtime row's.
