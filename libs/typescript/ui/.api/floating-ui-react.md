...
[STACKING]:
- `@floating-ui/react-dom` (transitive dependency, no manifest row or catalogue of its own): the positioning substrate re-exported whole — `useFloating`, every middleware factory, and the geometry types — with interaction/focus/portal/tree layered on top; every `view/overlay` row imports `@floating-ui/react` alone, interaction-free anchoring included, since the umbrella carries the sibling's whole surface.
