...
- Growth: a new .NET package lands as one `PACKAGE_TAG` row and, when it carries a new tag, one `FAULT_CTOR` row, one `Data.TaggedEnum` case, and one `Match.tagsExhaustive` arm; a sixth fault rail is the named defect — every fault is one case on the one family and every wire package is one vocabulary row.
...
type FaultDetail = Data.TaggedEnum<{
  ...
  readonly HopFault: { readonly code: number; readonly evidence: Record<string, string> };
...
type FaultWire = { readonly package: string; readonly case: string; readonly code: number; readonly evidence: Record<string, string>; readonly correlation: string };
...
const FAULT_CTOR = {
  ComputeFault: (w: FaultWire) => FaultDetail.ComputeFault({ code: w.code, evidence: w.evidence, correlation: w.correlation }),
  StoreFault: (w: FaultWire) => FaultDetail.StoreFault({ code: w.code, evidence: w.evidence, correlation: w.correlation }),
  HopFault: (w: FaultWire) => FaultDetail.HopFault({ code: w.code, evidence: w.evidence }),
  ConfigError: (w: FaultWire) => FaultDetail.ConfigError({ code: w.code, evidence: w.evidence }),
  Quarantine: (w: FaultWire) => FaultDetail.Quarantine({ code: w.code, evidence: w.evidence }),
} as const satisfies Record<FaultTag, (w: FaultWire) => FaultDetail>;

const faultOf = (wire: FaultWire): FaultDetail => FAULT_CTOR[faultTagOf(wire)](wire);
...
- Owner: the `FaultDetailWire` trailer shape the rail decodes, transcribed verbatim from `csharp:Rasm.Compute/remote/remote#TS_PROJECTION` (the `FAULT_PROJECTION` cluster's one `FaultDetail` family carrying every typed fault across the wire).
- Entry: the trailer carries `package=1 string`, `code=2 int32`, `case=3 string`, `message=4 string`, `evidence=5 map`, `correlation=6 string`, `hlcPhysical=7 Timestamp`, and `hlcLogical=8 uint64`; `code` decodes as `Schema.Int` and `hlcLogical` as bigint against the `uint64` width; the package literal gates the `faultTagOf` projection.
- Packages: `effect` `Schema` for the trailer surface.
- Growth: a new trailer member lands as one field row; the branch authors no shape absent from the C# fence.
