...
class TableNode(Struct, frozen=True, tag=NodeKind.TABLE.value, tag_field="kind"):
    ...
    header_rows: int = 0                        # leading rows lowered as `TH` per PDF/UA + recovered from `Table.header`
...
_DOCUMENT_ENCODER: Final = msgspec.msgpack.Encoder(order="deterministic")
_DOCUMENT_DECODER: Final = msgspec.msgpack.Decoder(DocumentNode)
_CORPUS_ENCODER: Final = msgspec.msgpack.Encoder(order="deterministic")
...
_STRUCT_CATEGORY: Final[frozendict[StructEltKind, tuple[StructCategory, int]]] = frozendict({
    ...
    StructEltKind.QUOTE: (StructCategory.BLOCK, 0),
})
...
_STANDARD_FOR: Final[frozendict[StructCategory, StructEltKind]] = frozendict({
    StructCategory.GROUPING: StructEltKind.SECT,
    StructCategory.HEADING: StructEltKind.H1,
    StructCategory.BLOCK: StructEltKind.P,
    StructCategory.INLINE: StructEltKind.LINK,
    StructCategory.LIST: StructEltKind.L,
    StructCategory.TABLE: StructEltKind.TABLE,
    StructCategory.ILLUSTRATION: StructEltKind.FIGURE,
})
...
def to_corpus(node: DocumentNode, view: CorpusView = CorpusView.STRUCT, /) -> CorpusRow | bytes | dict[str, object]:
    ...
    match view:
        ...
        case CorpusView.BYTES:
            return _CORPUS_ENCODER.encode(row)
...
def to_typst_source(node: DocumentNode) -> str:
    match node:
        ...
        case ListNode(list_kind=kind, items=items):
            return f"#{_LIST_MARKUP[kind]}({', '.join(f'[{to_typst_source(item).strip()}]' for item in items)})\n"
        ...
        case TableNode(rows=rows, header_rows=header):
            cells = ", ".join("[" + to_typst_source(cell) + "]" for row in rows for cell in row)
            return f"#table(columns: {len(rows[0]) if rows else 0}, table.header.repeat: {header > 0}, {cells})\n"
...
def _styled(run: RunNode) -> str:
    body = _typst(run.text, TypstScope.MARKUP)
    wrapped = pipe(
        body,
