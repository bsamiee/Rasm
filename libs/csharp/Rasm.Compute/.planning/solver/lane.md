...
- [ASSEMBLY_KERNELS]: the open leaf is the `Rasm`/Vectors boundary-extraction member spelling for the `MeshSpace.Encloses` host-geometry inclusion test; the per-`PhysicsKind` element assembly (`SolveLane.Triplets`/`LocalStiffness` over the `ElementClass.ShapeGrad` delegate and the `QuadratureRule.Tet4`/`Hex27` Gauss tables, the `Bᵀ·D·B` stiffness against the `PhysicsKind.Material` `D`-matrix, and the COO handoff to `numeric#SPARSE_SOLVE`) is authored in the cluster fences.
- [GRADIENT_ADJOINT]: the open leaf is the gradient-adjoint route's correctness gate — the tensor-lane DDG-operator VJP bodies (`tensors#EQUIVALENCE_INTEROP`) and the design-variable-to-`TensorOpFamily`-row lowering for shape optimization and inverse design ground against the `Rasm`/Vectors operator member surface at cross-folder alignment; the NSGA-II, Bayesian-EI, SIMP, simulated-annealing, CMA-ES, PSO, and trust-region descent kernels are authored in-package folds.
