...
It owns the `GenerationPolicy` search/prompt policy with its `SearchKey`/`GenerationMode`/`RuntimeOption` axes, the `GenerationInput` payload family with its narrowed `MediaSet` media arm and staged `Width`, the `ModelClass` native-capability table over one frozen `ModelCapability` set, the `ChatRole`/`ChatTurn` message vocabulary under a source-generated wire context, the `DecoderPin`/`ModelData`/`StopOracle`/`ToolPolicy` carriers, the `GenerationEvent` `[Union]` with `GenerationTally`/`GenerationOutcome`, the `AdapterSet` LoRA registry, and the `GenerativeRun` boundary capsule whose process-global `OgaHandle`, fingerprint-keyed resident `Config`→`Model`→`AdapterSet` lease, per-call `GeneratorParams`→`Generator` chain, `Stage` fold, `Drain`, and `Collect` ride `Microsoft.ML.OnnxRuntimeGenAI`.

The `ExecutionProvider` from `Model/providers#EP_AXIS` and `ModelIdentity` from `Model/identity#MODEL_IDENTITY` ride `GenerationOutcome`; the AppHost `CancelScope`, `ContentHash`/`CanonicalWriter` (`Rasm/Domain/identity#CONTENT_KEY`), the kernel `MonotonicTimeline`, and `NodaTime` `Duration`/`Instant` arrive settled. `ModelSessions.Faulted` is the ONE native-fault classifier this page composes rather than re-deriving, the cold-build-outside-lock-publish-under-lock resident discipline arrives settled from `Model/sessions#SESSION_CAPSULE`, and a remote generative run crosses solely through the `Runtime/wire#PROTO_VOCABULARY` `Generate` rpc (`GenerateRequest` → `TokenChunk`).
...
- [03]-[TOKEN_DRAIN]: ONE `Drain` loop over `GenerateNextToken`/`GetNextTokens` that the one-shot run and the conversation turn both instantiate; the `ToolPhase` row machine and its `ToolStep` decisions; `Collect`'s total fold to `GenerationOutcome`.
...
- Result: `GenerationOutcome` carries the model identity, execution provider, model type, run mode, active adapter, generated-token count, constrained-token count, tool-call count, staged media tokens, token rate over the drain's measured span, guidance kind, and timestamp. The drain advances the injected `Runtime/progress#PROGRESS_CELL` cell with the running token count; no per-chunk metadata object is created.
...
<!-- SPIKE: the POSITIVE LoRA hot-swap path — `LoadAdapter` succeeding on a real `.onnx_adapter` payload and `SetActiveAdapter` measurably changing the drained tokens mid-run — is asset-gated and converges only on an operator-provisioned fine-tune. Its deterministic floor above ships whole: loaded-set guard, typed unload/activate refusals, and content-verified asset roster are all proven on the failure channels. -->
...
- Law: the span is MEASURED here. Tokens-per-second divided by a `Duration` the caller passed in, so the result's rate column was computed from a value this owner never observed; the drain captures its own monotone pair and `Completed` carries it, which is also what lets a caller that never timed anything still publish an honest rate.
...
- Result: `Collect` returns `GenerationOutcome` directly with token count, tokens per second over the drain's measured span, guidance dimension, constrained and tool counts, and optional staged media tokens.
...
- Result: each turn returns its own `GenerationOutcome`; the session's cumulative `Total` remains the conversation's running tally.
