...
- seam: the polars `DataFrame.style` accessor returns a real `GT` (`polars.DataFrame.style -> GT`, verified at the polars `frame.py` source and `isinstance(df.style, GT)`), so the `figures/table` ← `data/tabular` `[WIRE]` edge constructs the styled table in-process from a polars frame with no intermediate interchange. HTML (`as_raw_html(inline_css=True)`) and LaTeX (`as_latex`) are the host-free no-driver paths; PDF (`save`) rides a host-coupled Chrome/Selenium WebDriver, gated optional and never the default. The emitted HTML/SVG bytes are the flat egress the regrouped `figures/compose#COMPOSE` owner places beside its sibling graphics, the same outbound handoff `figures/chart#CHART` makes, so the table is a content-keyed source of the composition plane and never composites or re-renders downstream.
