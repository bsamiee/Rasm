...
`@rasm/ts-testkit` (`tests/typescript/testkit`) lifts a `Semigroup`/`Monoid` through any functor `F` and folds it over any `Foldable`, so one instance proves its laws inside `Option`, `Either`, `Array`, or a decoded op container without a bespoke harness.
