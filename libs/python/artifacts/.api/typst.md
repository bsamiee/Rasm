...
- `anyio`(`libs/python/.api/anyio.md`): the `Compiler` mints inside the offloaded arm (the font-discovery scan is GIL-releasing native), and each `CORE`-band render crosses the runtime `LanePolicy.offload` boundary as a `KernelTrait.RELEASING` kernel — in-process, never inline on the loop.
...
- `fonttools`(`fonttools.md`) / `uharfbuzz`(`uharfbuzz.md`): `Fonts(font_paths=[..]).fonts()` reads back the same OTF/TTF the shaping stages own and `typography/font` subset; the owner registers via `font_paths` and confirms `Fonts.families()`/`FontInfo.path` matches the shaped face before an `ignore_system_fonts=True` archival render.
