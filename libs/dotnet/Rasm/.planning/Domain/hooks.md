...
- Receipt: a point mints nothing — the fire IS the evidence event and the emitter's typed receipt already carries the fact; the shared `FaultCell` records veto refusals and shielded tap faults point-attributed.
...
- Receipt: `Faults` is the parked evidence, `Shed` its overflow count, and `Lost` its declined-park count; none is a rail outcome, so a consumer reads them and never branches a fire on them.
...
- Receipt: `Census` and `Riders` are the audit projection a composition reads; neither mutates.
