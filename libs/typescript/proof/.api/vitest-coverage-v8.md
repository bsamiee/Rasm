...
`Thresholds` is the gauge's core: coverage as a numeric pass gate, per-metric and optionally per-glob, with `autoUpdate` ratcheting the floor. This is what `gauge/mutation.ts` means by "coverage thresholds as data" — a config row, not a script.
...
[STACK: `@vitest/coverage-v8` `Thresholds` + `@stryker-mutator/core` `MutationScoreThresholds` (`gauge/mutation.ts`)] — the two-floor "thresholds as data" gate, the mirror of `stryker-mutator-core.md`'s reciprocal STACK. Coverage thresholds are the FIRST gate (did the specs execute the code); the Stryker mutation-score `break` is the SECOND (did they assert on it), and `@stryker-mutator/vitest-runner` runs BOTH floors over the same vitest spec set. `gauge/mutation.ts` holds the two floors as data — coverage `Thresholds` (`statements`/`functions`/`branches`/`lines`) here, `MutationScoreThresholds { high, low, break }` in `stryker-mutator-core.md`. A high coverage number with a low mutation score is the exact defect the two-gate stack catches.
