...
|  [03]   | `SKPath`                    | vector path with boolean SVG codec, transform   |
...
- `CreateLerp` SHORT-CIRCUITS at the CLOSED endpoints of its weight domain — `weight <= 0` returns `filter0` and `weight >= 1` returns `filter1`, each the caller's own instance — and returns NULL on a NaN weight, so only a strictly interior weight mints a node retaining both arms and a normalized phase carrying its endpoints reaches both traps. `CreateLerp` and `CreateCompose` throw `ArgumentNullException` on a null arm.
...
- arm custody: a nullable `input`/`background`/`foreground` slot admits null as the IMPLICIT source rather than refusing, and every non-null arm is retained under the `[TOPOLOGY]` regime — `CreateMerge` over two nulls or an empty span still yields a node. `CreateColorFilter`, `CreateCompose`, `CreateTile`, `CreateImage`, `CreateDisplacementMapEffect`, and `CreateBlendMode`'s `SKBlender` arm throw `ArgumentNullException` on their non-nullable slots instead.
- `CreateBlendMode` SHORT-CIRCUITS three modes into a value that is not a fresh node: `Src` returns `foreground` and `Dst` returns `background` — the caller's own instance, retained by nothing — while `Clear` mints a source-free node retaining NEITHER arm. `CreateArithmetic` returns NULL on any non-finite `k1`..`k4` and retains neither arm on that path, so the declared non-null return is fiction exactly where a coefficient arrives unadmitted.
...
- `SKRuntimeEffectUniforms.Add`/`SKRuntimeEffectChildren.Add` THROW `ArgumentOutOfRangeException` on a name the program never declared, and the uniform overload validates the value's own `SKRuntimeEffectUniform.DataType` against the declaration's type, count, and array flag and throws on a mismatch — the implicit conversions widen the CALL, never the check, so a typed result resolves the name and the declared shape ahead of the write rather than trapping the throw.
...
- `CreateShader`/`CreateColorFilter`/`CreateBlender` return a NULL effect with the diagnostic in `errors` on rejection and a non-null effect with `errors` normalized to null on success; `BuildShader`/`BuildColorFilter`/`BuildBlender` validate that identical pair and THROW, so a typed refusal takes the `out errors` form and wraps it with `new SKRuntimeShaderBuilder(effect)` itself.
...
- `sk_sp` custody is RETAIN, never consume. Every `SKImageFilter`/`SKColorFilter`/`SKShader`/`SKBlender` factory hands its arguments to the native constructor as bare handles under a call-scoped `GC.KeepAlive`, and that constructor takes its OWN reference: composing a node raises each argument's native reference count by one and disposing the node lowers it by one, so the caller keeps owning and disposing every input it minted, and the node holds no MANAGED reference to its arms — a dropped arm frees on the finalizer, never through the node. `SKPaint`'s `Shader`/`ColorFilter`/`ImageFilter`/`Blender` slots are the same regime: assignment retains, rebind or `Dispose` releases the paint's own reference alone.
- The returned object is caller-owned and disposes ONCE: the factory seats its handle in a process-wide handle-to-instance map owning one reference, `Dispose` is idempotent and releases exactly that one, and the finalizer covers a dropped wrapper. That map is the aliasing hazard — a factory whose native SHORT-CIRCUITS to one of its own arguments returns a handle already seated, the surplus reference drops, and the SAME managed instance comes back, so a fold releasing every arm beside a result that IS one of them disposes the value it just transferred. `ReferenceEquals` against each arm is the only probe the binding leaves, and an identical repeat mint yields a distinct handle, so aliasing arises from a named short-circuit and never from interning.
- Dispose-protected rows refuse public disposal outright: `SKBlender.CreateBlendMode` and `SKColorFilter.CreateSrgbToLinearGamma`/`CreateLinearToSrgbGamma` hand back shared instances seated without handle ownership whose public `Dispose` is inert, so `using` over any of them is a no-op and the handle outlives every scope. Handle registration and disposal both take one process-wide lock, so `SKObject` construction and release serialize across threads while the instance a factory returns stays shared.
...
- Capture path: `SKSurface.Create(SKImageInfo)` or a GPU surface from `GRRecordingContext` draws, `Snapshot()` returns an `SKImage`, and `Encode(SKEncodedImageFormat.Png, ...)` returns the diffable `SKData` buffer; `SKColorSpace.CreateSrgb`/`CreateIcc` + `SKImageInfo.WithColorSpace` make it color-managed, and animated evidence decodes through `SKCodec.FrameCount`/`GetFrameInfo` per frame.
...
- Paged export: `SKDocument.CreatePdf(stream, SKDocumentPdfMetadata)` -> per-sheet `BeginPage`/draw/`EndPage` -> `Close`, sharing the live pipeline's paint/path stack so on-screen and exported geometry are byte-identical.
