...
> =>
  Effect.gen(function* () {
    ...
    const floored = sql.onDialectOrElse({ orElse: () => false, pg: () => true })
...
- Law: `_BACKEND_DOCUMENT_CEILING` rejects transported ProtoJSON before generated decode and emitted ProtoJSON before publication. Its 512 KiB budget sits beneath the 1 MiB ConfigMap limit after base64 and object metadata; descriptor string, content, and repeated-field ceilings remain the constructed-message floor.
...
- Packages: `@bufbuild/protobuf` generated construction and semantic equality; `@rasm\/contracts` parity descriptors and enums; `effect` collections, schemas, and effects; core `Digest.Key`, `Digest`, `Fault.Class`, and `Format.proto`.
