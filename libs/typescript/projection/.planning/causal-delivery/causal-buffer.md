...
- Packages: `effect` for `Data.TaggedEnum`, `Match`, `HashMap`, `SortedMap`, `Option`, `Order`, `Stream`, `SubscriptionRef`, `Effect`, and `Scope`; the `dominates` predicate, the `VersionVectorWire`/`OriginCursor` shapes, and the `originFold` cursor advance arrive owned from `causality-graph/version-vector#ORIGIN_CURSOR` — the buffer composes them and mints no parallel ordering surface (charter law).
...
- Packages: `effect` for `Effect`, `Scope`, `Stream`, `SubscriptionRef`, and `HashMap`; the `stabilityFrontier` constructor arrives owned from `causal-delivery/stability-frontier#STABILITY_FRONTIER`, the `causalBuffer` from `[2]-[CAUSAL_BUFFER]`.
