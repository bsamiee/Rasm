...
Deployment inputs are `HOSTINGER_API_TOKEN`, the VM id, `SSH_USER@SSH_HOST`, the key path, and the remote app directory. SSH uses the universal key in `~/.ssh/config`, a configured host alias replaces explicit `-i` flags. Key provisioning for a new server is API work: register the public key, attach it to the VM, then verify with `ssh <host> "echo SSH_OK && hostname"`.

## [02]-[BASELINE]

First-time server setup, idempotent by construction:
