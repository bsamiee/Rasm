# [@radix-ui/react-visually-hidden] — the screen-reader-only primitive: clip content out of view, keep it in the accessibility tree

[PACKAGE_SURFACE]:
- package: `@radix-ui/react-visually-hidden` · version `1.2.7` · license `MIT`
- module: dual — `dist/index.mjs` (ESM) + `dist/index.js` (CJS); `sideEffects: false`; one `.` barrel, no subpaths.
- asset: `dist/index.d.ts` (`assay api resolve @radix-ui/react-visually-hidden` → `1.2.7`, `restore: restored`).
- runtime: React render-time; renders one `<span>` via `@radix-ui/react-primitive@2.1.7` (inheriting its `asChild` merge). Peer react/react-dom 19.
...
- Boundary: render-time only; composes `@radix-ui/react-primitive@2.1.7` (inherits `asChild`). The style object is frozen (`Readonly`). Peer react/react-dom 19.
