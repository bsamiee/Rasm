...
| [06] | `Deck.{pickObject,pickObjects,pickMultipleObjects}` | sync mirrors, `@deprecated` WebGL-only | retired sync picking; prefer the async pair under WebGPU |
...
| [14] | `_animate` / `_pickable` / `_framebuffer` / `_typedArrayManagerProps` | overlay | force-animate (trips), disable picking, offscreen target, attribute memory |
...
| [03] | `OrbitView` / `OrthographicView` / `FirstPersonView` / `_GlobeView` | non-geo + globe views | CAD-style orbit, 2D plan, walkthrough, 3D globe (`_` overlay) |
