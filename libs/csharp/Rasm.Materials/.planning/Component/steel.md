...
- Entry: `SteelSeed.Rows(Context)` traverses the unified `SteelRowSeed` table. `SteelDesign.Capacity` admits the rolled, cold-formed-stud, or steel-deck modality on the shape of its typed input and resolves the REGISTERED yield from the grade band at the element thickness — a `NationalAnnex` crosses, never a caller yield double. `SteelDesign.RetentionAt(double, Op) : Fin<(double Ky, double KE)>` rejects non-finite steel temperature before interpolating the EN 1993-1-2 table.
...
public static class SteelDesign {
    ...
    public static Fin<DesignCapacity> Capacity(SteelShape shape, ComputedSection s, NationalAnnex annex, double unbracedLengthMm, double effectiveLengthMm, Op key) =>
        from lengths in guard(double.IsFinite(unbracedLengthMm + effectiveLengthMm) && unbracedLengthMm >= 0.0 && effectiveLengthMm > 0.0,
            ComponentFault.Capacity(key, $"<steel-design-input-rejected:{unbracedLengthMm:R}:{effectiveLengthMm:R}>"))
        from yieldMpa in shape.Grade.YieldMpa(shape.Section.FlangeMm.Value, annex, key)
        select Rolled(shape, s, yieldMpa, unbracedLengthMm, effectiveLengthMm);

    // The projection body over the ADMITTED yield — the registered Table 3.1 value the grade band resolved at the
    // section's own flange thickness, never a caller double that could price an S355 shape at 235 MPa.
    static DesignCapacity Rolled(SteelShape shape, ComputedSection s, double yieldMpa, double unbracedLengthMm, double effectiveLengthMm) {
    ...
    public static Fin<DesignCapacity> Capacity(ColdFormedRow stud, ComputedSection s, NationalAnnex annex, double effectiveLengthMm, Op key) =>
        from length in guard(double.IsFinite(effectiveLengthMm) && effectiveLengthMm > 0.0,
            ComponentFault.Capacity(key, $"<cold-formed-design-input-rejected:{effectiveLengthMm:R}>"))
        from yieldMpa in stud.Grade.YieldMpa(stud.WallMm, annex, key)
        select ColdFormed(s, yieldMpa, stud.SeffRatio, effectiveLengthMm);

    // The STEEL-DECK modality of the SAME AISI projection, discriminating on the connector#CONNECTOR_FAMILY GaugeRow
    // input: a ribbed sheet is not a lipped C, so it carries no ColdFormedRow — but the F3.1/E3.1/G2 body is
    // identical over the deck's own solved Corrugated ComputedSection at the gauge band's PUBLISHED SS Grade 33/50
    // yield. The seffRatio is the deck's published effective-section proxy the panel row supplies. This overload is
    // what makes GaugeRow.AxialSectionCapacityKnPerMm a consumed datum rather than a declared one.
    public static Fin<DesignCapacity> Capacity(GaugeRow gauge, ComputedSection s, double seffRatio, double effectiveLengthMm, Op key) =>
        from admitted in guard(double.IsFinite(effectiveLengthMm + seffRatio) && effectiveLengthMm > 0.0 && seffRatio is > 0.0 and <= 1.0,
            ComponentFault.Capacity(key, $"<deck-design-input-rejected:{seffRatio:R}:{effectiveLengthMm:R}>"))
        select ColdFormed(s, gauge.YieldMpa, seffRatio, effectiveLengthMm);

    // The ONE AISI body both cold-formed ingresses drive — a published Seff/S < 1 IS the slender verdict, since
    // post-buckling effective width is the cold-formed design premise and never a B4.1 read.
    static DesignCapacity ColdFormed(ComputedSection s, double yieldMpa, double seffRatio, double effectiveLengthMm) {
        double λc = effectiveLengthMm / s.GoverningRadiusMm;
        double Fe = Math.PI * Math.PI * E / (λc * λc);
        double Fcr = Fe >= 0.44 * yieldMpa ? yieldMpa * Math.Pow(0.658, yieldMpa / Fe) : 0.877 * Fe;
        return new DesignCapacity(
            FlexuralNmm: φb * yieldMpa * s.SxMm3.Value * seffRatio,
            FlexuralMinorNmm: φb * yieldMpa * s.SyMm3.Value * seffRatio,
            CompressionN: φc * Fcr * s.AreaMm2.Value * seffRatio,
            ShearN: φv * 0.6 * yieldMpa * s.AvyMm2.Value,
            TorsionalNmm: 0.0,
            Classification: seffRatio < 1.0 ? CompactnessClass.Slender : CompactnessClass.Compact,
            Slenderness: λc);
    }
...
// Grade is RE-HOMED from the seed row: the AISI stud rolls the A653 SS Gr 50 cold-formed sheet band by construction,
// so the pairing SteelSeed.Augmented spelled beside the row belongs ON it — the capacity projection then reads its
// registered yield at the stud's own wall thickness with no caller double in the signature.
public readonly record struct ColdFormedRow(string Key, double DepthMm, double FlangeMm, double WallMm, double LipMm, double FilletMm, double EffectiveWebMm, double SeffRatio, SteelGrade Grade);

public static class ColdFormedSections {
    public static readonly ColdFormedRow S600S162_54 = new("600s162-54", 152.4, 41.3, 1.37, 12.7, 4.76, 119.0, 0.78, SteelGrade.A653);
...
public static class SteelSeed {
    ...
    // the 600S162-54 AISI stud on its OWN published ColdFormedRow at the A653 SS Gr 50 band (the prior C15x33_9
    // catalogue core published a 381 mm hot-rolled channel's twenty columns under the stud designation, the deleted
    // impersonation), and the two FABRICATED rows that make the open-thin-walled Forms algebra reachable from an
    ...
    static readonly Seq<SteelRowSeed> Augmented = Seq(
        ...
        new SteelRowSeed("steel.pg-1200x400-500", new SteelRowSource.Plated(static key => SectionProfile.AsymmetricIShape.Of(
            ...
            topFlangeThicknessMm: 25.0, bottomFlangeThicknessMm: 32.0, webThicknessMm: 12.0, key)), SteelGrade.S355, En),
        new SteelRowSeed("steel.zed-200x75x25", new SteelRowSource.Plated(static key => SectionProfile.Zed.Of(
            ...
            thicknessMm: 2.5, topLipMm: 20.0, bottomLipMm: 18.0, key)), SteelGrade.A653, Aisc));
    ...
    // IfcBinding.Supertype the IfcBuiltElement stamp (the leaf role is an occurrence refinement), Coring.None,
