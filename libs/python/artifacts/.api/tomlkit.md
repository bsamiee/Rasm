...
- egress axis (primary): `dumps(data, sort_keys=...)` accepts any `Mapping`; `tomlkit.dumps(msgspec.to_builtins(node), sort_keys=True)` lowers the model directly to deterministic TOML bytes for `EmitFact.data`.
