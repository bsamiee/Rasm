...
// Option<double> projects to the codec's own absence, never to a fabricated figure.
[MessagePackObject]
...
public sealed record EnvironmentLightWire(
    ...
    // Total by structure: Sh9.Of is the [08.2] 27-length admit gate, so every Irradiance this mirror reads has
    // already refused a wrong layout and the wire needs no second cardinality check.
...
public sealed record StageResultWire(
    ...
    public static Fin<StageResult> Admit(StageResultWire wire, ModelCard card, Op key) =>
        ...
        from admitted in StageResult.Admit(
            new StageResult(stage, echoed, outputs, provider, wire.PartitionCount, wire.ElapsedMs, wire.GoldenDelta, wire.TilesEmitted, key), card, key)
        select admitted;
...
- [MTLX_SCHEMA_VALIDATION]-[OPEN]: Which validator admits a projected `.mtlx` against the MaterialX 1.39 schema at the host import boundary?; verify the `MaterialX` Python/C++ runtime or a standalone schema validator at the app-root serialize boundary.
