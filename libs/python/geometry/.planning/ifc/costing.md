...
class DiffAxis(StrEnum):
    ...
    @staticmethod
    def of_markers(markers: "dict[str, object]") -> "RuntimeResult[frozenset[DiffAxis]]":
...
class LifecycleResult(Struct, frozen=True, gc=False):
    ...
    def fidelity_frame(self) -> "RuntimeResult[EvidenceFrame]":
...
def _reader(table: str) -> "RuntimeResult[TableFormat]":
...
class IfcLifecycle:
    ...
    async def run(
    ...
    ) -> "RuntimeResult[tuple[bytes, LifecycleResult]]":
        ...
        match held.bind(lambda inner: inner):
    ...
    ) -> "RuntimeResult[tuple[LifecycleResult, ifcopenshell.file]]":
    ...
    @staticmethod
    def _diff(model: "ifcopenshell.file", revision_path: str, spec: str) -> "RuntimeResult[LifecycleResult]":
    ...
    @staticmethod
    def _import(model: "ifcopenshell.file", table: str, fmt: TableFormat, spec: str) -> "RuntimeResult[LifecycleResult]":
        ...
        faults = Block.of_seq((applied, unwound, closed)).choose(lambda held: held.swap().to_option())
