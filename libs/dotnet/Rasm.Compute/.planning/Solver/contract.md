...
`SolveProblem.ContentKey` composes the kernel `ContentHash.Of<TState>` seed-zero path through `CanonicalWriter`, never a hand-framed byte walk over a formatted string. Every solver result is typed, and the page carries no TS_PROJECTION because solve interiors stay host-local behind the `Solve` rpc.
...
- Packages: MathNet.Numerics, CSparse, Rasm (project — kernel `ContentHash.Of<TState>`/`CanonicalWriter`, `Dimension`/`PositiveMagnitude`/`Band`), Rasm.Element (project — the contract `MaterialPropertySet.Mechanical` elasticity and density reads beside the `Thermal` `SpecificHeat` read the volumetric capacity composes), System.Numerics (`Vector3` — the contact normal), System.Numerics.Tensors, CommunityToolkit.HighPerformance, Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, BCL inbox
...
public abstract partial record SolveRoute {
    ...
    public sealed record Traced(RungeKuttaIntegrator Integrator, TrajectoryControl Control, QuadratureControl Accuracy, Seq<double> Stations) : SolveRoute;
    ...
    public string Key => Switch(
        ...
        transient: static row => row.Integrator.Key, traced: static row => row.Integrator.Method.Key,
