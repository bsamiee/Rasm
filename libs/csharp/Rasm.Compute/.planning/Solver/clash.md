...
public static class ClashScale {
    ...
    // The ray entry the Analysis/daylight shadow/obstruction fold composes: one Möller–Trumbore cast over
    // the SAME decoded node-link traversal Detect/Clearance walk — slab-test the node AABBs down the tree,
    // triangle-test only the reached leaves; maxDistance bounds a sun-ray at scene diameter. ONE ray engine
    // on the one acceleration owner — a daylight-local BVH walk beside it is the deleted form. `Admit` gates
    // the wire and the `% 9` triangle alignment; a zero-length `direction` rails `ModelRejected` before the
    // `Vector3.Normalize` NaN, so the cast and the normalize both run on admitted inputs.
    ...
    // Node-link ray walk: slab-test each AABB (six floats per node), descend on a hit, Möller–Trumbore the
    // leaf triangles; first hit within range answers true — occlusion needs no nearest-hit ordering.
...
- [CLASH_GOLDEN] (frozen two-sided fixture, both pages assert byte-identical): the canonical 8-primitive `BoundingBox[]` set built `Rasm/Spatial/index#CLASH_GOLDEN`-side through `SpatialIndex.Build(SpatialKind.Bvh, …)` with `BuildPolicy.Canonical` and projected through `ToAcceleration` serializes little-endian to the frozen 160-byte golden sequence (`NodeCount == 3`, root node 0 internal, two four-primitive leaves); this page's spec deserializes those same bytes into `AccelerationStructure.Bvh` and asserts `ClashScale.Detect` decodes them through `NodeLinkPairs` into the agreed clash-pair set `{(0,1),(2,3),(4,5),(6,7)}` (4 hard pairs, all intra-leaf, the X-cluster separation pruning every cross-leaf comparison). The octree projection of the same primitives rides the identical `NodeLinkPairs` descent, so the traversal is builder-agnostic and the seam is unchanged by the broad-phase kernel; the Rasm-emits → Compute-decodes round-trip is the seam-settled signal, PROVEN BY BYTE-LAYOUT DERIVATION from the two transcription-complete fences and RECORDED here: the kernel wire law pins 72 bounds bytes (3 nodes × 6 floats × 4 bytes, min/max interleaved little-endian under one-ULP outward rounding) + 88 node bytes (3 × 8-byte packed node-link words + the 64-byte leaf-primitive index block) = 160 bytes exactly; the pinned SAH topology (root internal at node 0, `FirstChild=1`, two 4-primitive leaves splitting the X-separated clusters) forces `NodeCount == 3`; decoding those bytes through `Decode(nodes[i])`'s `(leaf, first, count)` unpack yields leaf ranges `[0,4)`/`[4,8)`, and `NodeLinkPairs` over the frozen 8-cube overlap pattern (each cluster's cubes overlapping pairwise-adjacent at 0.5 offsets) yields exactly `{(0,1),(2,3),(4,5),(6,7)}` — the derivation, not an asserted harness, is the fixture's proof; no executable harness or test file exists in this campaign.
