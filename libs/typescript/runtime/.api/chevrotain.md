...
|  [02]   | `ITokenConfig`                                     | mint record    | the `createToken` input; `categories` builds the token lattice   |
...
| :-----: | :----------------------------------------------- | :-------- | :--------------------------------------------------------------- |
|  [01]   | `createToken(config)`                            | function  | one vocabulary row; `categories` and `longer_alt` build lattices |
...
- `categories` builds a token lattice, so an operator class is one category token every member declares and the grammar consumes the category rather than enumerating members.
...
- `core/value/fault`(`core/.planning/value/fault.md`): `ILexingError` and `IRecognitionException` both land as `Fault.Class` evidence at the one lowering seam; a parse failure refuses the SUBSCRIPTION at admission, never a delivery.
