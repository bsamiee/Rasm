...
- `SixLabors.ImageSharp`(`.api/api-imagesharp.md`): the container split — a `.ktx2` never routes through the `ImageSharp` `ImageFormatManager`, and a PNG16 or TIFF sidecar never routes here; a plane crosses between the two modules as raw texels in one pooled arena.
