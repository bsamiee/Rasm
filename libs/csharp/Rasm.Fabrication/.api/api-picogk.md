...
`PicoGK` COMPOSES — does NOT replace — the planar fabrication owners: it leaves the `Clipper2` 2D perimeter Boolean/offset substrate untouched, and the `geometry3Sharp` `MeshPlaneCut` planar mesh-section path intact. Its lane is the implicit/voxel/SDF concern those line/mesh owners cannot reach — TPMS infill, conformal lattices, distance-field morphology (true 3D shelling/filleting/over-offset), and the resin/powder grayscale-and-vector layer stack. The boundary is sharp: planar FFF perimeters and 2D toolpaths stay on `Clipper2`/`CavalierContours`; the `Voxels`/`IImplicit` SDF concern stays here. The extracted `Mesh` is the wire-meeting point — a `Voxels.mshAsMesh()` result re-enters the managed mesh world (`geometry3Sharp` `DMesh3`, the host `Mesh`) for downstream consumption.
...
| [INDEX] | [SURFACE]                                                       | [ENTRY_FAMILY] | [CAPABILITY]                                                                 |
| :-----: | :-------------------------------------------------------------- | :------------- | :--------------------------------------------------------------------------- |
|  [01]   | `Voxels.mshAsMesh()` / `new Mesh(in Voxels)`                   | extraction     | dual-contour the SDF to a watertight triangle `Mesh` — the wire back to the managed mesh world (`geometry3Sharp` `DMesh3`, host `Mesh`) |
...
- THE WIRE MEETS AT `Mesh`: `Voxels.mshAsMesh()` re-enters the managed mesh world — the extracted watertight mesh hands to `geometry3Sharp` `DMesh3` (the SOLE biarc/curve-fit + mesh owner) or the host `Mesh` for downstream planar sectioning, posting, or display. PicoGK does NOT own the planar mesh-section (`geometry3Sharp` `MeshPlaneCut`) — it extracts the mesh and hands off.
...
- Reject: any reference from a type that must load in the net48 in-Rhino plugin ALC (the native core firebreaks it — keep it sidecar/host-side); reaching into the planar mesh-section (`geometry3Sharp` `MeshPlaneCut`) or the 2D-perimeter Boolean/offset substrate (`Clipper2`/`CavalierContours` own those — PicoGK composes beside them, never replaces); minting a `Voxels`/field/mesh/library/VDB handle without disposal (native leak); mixing fields across libraries of different voxel sizes; and citing the native lib as a bare `picogk.dylib` (it is the versioned `picogk.26.2`)
