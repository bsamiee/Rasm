...
- [04]-[PROTOCOL_FACES]: Generated client projections of the SDK-owned MCP boundary.
- [05]-[APP_ROOT]: Service-host-root builder fold mounting the adopted primitives and the one ingress filter.
...
- Auto: a call needing more than one round trip rides the SDK's own multi-round-trip input protocol, never a host-held task cell — a tool body throws `InputRequiredException` carrying its `InputRequest` asks and an opaque `RequestState`, the client resolves each locally and RETRIES the same `tools/call` with the answers plus the echoed state, and `McpServer.IsMrtrSupported` is the guard a body checks before choosing that route. The retry is what makes the suspension durable without host state: a suspended call holds nothing on this side of the wire, so a host restart between rounds costs the client one re-send rather than a lost in-flight task. The consequence the brokered dispatch owns is IDEMPOTENCE — a body reached again for the same `RequestState` must not re-charge its grant or re-mint its result, so the pre-flight ask keys off that state and `CommandAlgebra.Run` stays the single commit the last round reaches.
...
- Output: the projection returns the brokered `AIFunction`/registered `McpServerTool` pairs; each intent crosses `AppHostPoint.Command` at the dispatch front door, while the served-method transition logs through `SpineLog`.
...
public static class ToolProjection {
    ...
    public static McpCatalog Project(McpRuntime runtime) =>
        ...
        && AIJsonUtilities.CreateJsonSchema(typeof(CommandResult), serializerOptions: runtime.Wire) is var resultSchema
            ? new McpCatalog(
                Level: level,
                Freshness: runtime.Freshness,
                Tools: rows.Map(row => Tool(row, resultSchema)),
...
- Entry: `Preview(McpRuntime runtime, string tool, CommandArguments arguments)` returns `IO<CostPreview>` — the dry-run cost preview prices the tool call through `GrantBroker.Admit(…, DrawMode.Priced)` and returns the estimated cost and whether the standing grant covers it, before any execution; `Call(McpRuntime runtime, string tool, CommandArguments arguments)` returns `IO<CommandResult>` — the brokered dispatch prices the call, surfaces an uncovered price through elicitation, and routes through `CommandAlgebra.Run`, yielding the result itself; `Project(string tool, CommandResult result)` returns `ToolResult` — THE one transport-edge projection of a result onto the structured result and its fault mapping; `ServerInitiated.Live` and `ServerInitiated.Retry` are the two one-level `AmbientSlot` values whose `Enter` the ingress filter and the call-tool request filter open; `ServerInitiated.Confirm(McpRuntime runtime, CostPreview preview, CommandArguments arguments)` returns `IO<Fin<CostApproval>>` — the input-required overage ask carrying the exact shortfall unit and price, suspending a first round on the MRTR path and refusing on the typed result where the client lacks it.
- Auto: the preview reuses the broker's admission fold, a retry round never re-asks, and `CommandAlgebra.Run` remains the single commit. `Call` yields the `CommandResult`; `Project` alone shapes it into `CallToolResult`.
- Output: `ToolResult` carries the structured content blocks, the SDK's `isError` flag, and the `CommandResult` correlation id.
...
- Law: `Project` dispatches over `CommandTxn` with the generated total `Switch`, so a new transaction case breaks the projection at compile time.
...
public static class McpDispatch {
    ...
    public static IO<CommandResult> Call(McpRuntime runtime, string tool, CommandArguments arguments) =>
        arguments.Round.IsSome
            ...
            : from preview in Preview(runtime, tool, arguments)
              ...
              from result in CommandAlgebra.Run(runtime.Command, tool, arguments)
              select result;

    public static ToolResult Project(string tool, CommandResult result) =>
        result.Txn.Switch(
            committed: c => new ToolResult(tool,
                [JsonSerializer.SerializeToNode(c.Dispatch, SuiteContracts.Host)!], IsError: false, result.Correlation),
            rolledBack: r => Failed(tool, r.Reason, result.Correlation),
            compensated: c => new ToolResult(tool,
                [JsonSerializer.SerializeToNode(c.Compensation, SuiteContracts.Host)!, Fault(c.Reason)],
                IsError: true, result.Correlation),
            refused: f => Failed(tool, f.Fault, result.Correlation));
...
- Owner: `McpToolWire` and `ToolResultWire` are the generated client projections of the SDK-owned MCP boundary. `ToolResultWire` projects `ToolResult` beside the shared result header.
- Entry: the tool catalog crosses as standard MCP `tools/list` JSON and the structured result as `CallToolResult.StructuredContent` under the declared output schema.
- Packages: BCL inbox
- Growth: one wire-member row per new tool annotation or notification field; zero new surface.
- Boundary: the tool input schema crosses as standard JSON Schema and effect annotations cross as MCP tool hints; measured progress uses the SDK's `IProgress<ProgressNotificationValue>` binding directly; `CostPreview` stops at the in-process elicitation ask; disconnected agents recover the terminal `ToolResultWire` by retrying under the echoed `requestState`; the AppHost projection carries it beside the shared result header.
