...
class Detail(Struct, frozen=True):
    ...
    def _key(self) -> ContentKey:
        ...
        # seed BEFORE the native fold runs — never a key over rendered layer bytes; the library and
        # palette are bytes-producing inputs (resolved blocks, ink), so identical callouts over distinct
        # libraries never share a key.
