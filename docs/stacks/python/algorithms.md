...
- Reject: collapsing count-exhaustion into the failure channel — it destroys the caller's seeded-direct retry; accepting the substitution iterate under a converged verdict without the independent residual witness.
...
The constant-coefficient periodic operators and the seeded sampler have no companion-band solver surface; they are composed from `numpy.fft`, the seeded `numpy.random.Generator`, and the carriers, with the route owning the policy the providers refuse.
