...
The host-neutral 5D cost-and-resource projection: one `CostItem` record joining an `IfcCostValue` applied rate to the `Semantics/properties#PROPERTY_SETS` `QuantitySet.Derive` takeoff quantity by element `GlobalId`, a closed `ConstructionResource` `[Union]` (`Labor`/`Material`/`Equipment`) over `IfcLaborResource`/`IfcConstructionMaterialResource`/`IfcConstructionEquipmentResource` bound to the `sequencing#ConstructionTask` it resources through `IfcRelAssignsToProcess`, and the `CostSchedule.Rollup` fold that folds the `(quantity x rate x resource)` join into one schedule total — one fold, never enumerated per-resource arms. The priced scalar is the `NodaMoney` `Money` `readonly struct` over a `decimal Amount` and an ISO 4217 `Currency`, never a `(double Amount, string Currency)` pair: a `CostItem` value is `Money`, a unit rate is `Money / decimal`, the quantity x rate join is `Money * (decimal)quantity.As(SiUnit)`, the `CostSchedule.Rollup` is a `Money` sum over the additive operator (`Money.AdditiveIdentity` the no-currency anchor), a cross-currency rollup composes `NodaMoney.Exchange.ExchangeRate.Convert`, and a lump-sum or contingency allocation across packages is the lossless `MoneyExtensions.Split` penny distribution rather than a remainder-losing `total / n` multiply — the whole estimate folds under one `NodaMoney.Context.MoneyContext` rounding/precision scope so a single banker's-rounding rule governs every line rather than a `MidpointRounding` argument threaded through every construction. The estimate is a VIEW of the federated `Model/elements#ELEMENT_MODEL` `BimModel`, never a second quantity or schedule source: each `CostItem` reads its quantity from the one `Semantics/properties#PROPERTY_SETS` `QuantitySet` takeoff the model already derives from the kernel `Rasm` geometry the element binds by reference, joins to the priced elements through the `IfcRelAssignsToControl.RelatedObjects` cost-control set the `IfcCostItem` controls, and the resource it consumes binds to the `sequencing#ConstructionTask` activity network through `IfcRelAssignsToProcess` rather than re-modeling a parallel schedule — so a wall's net-volume takeoff, its concrete unit rate, and the labor crew that places it carry one cost line while the physical element keeps its solid geometry and the activity keeps its calendar. The estimate is HOST-NEUTRAL — it joins elements, resources, and tasks by stable `GlobalId` and never carries a RhinoCommon type — and the `Model/query#ELEMENT_SET` `ByProperty(Qto_*)` arm selects the quantified element set the estimate prices, never a second selection surface. The `(QuantityKey, ResourceKey)` `UInt128` content-key pair the `CostSchedule.Identity` derives is the reference the cross-libs `csharp:TELEMETRY_LAKE_ANALYTICS` 5D cost catalog reads the schedule by at `Rasm.Compute/Runtime/codecs#CONTENT_ADDRESSING`, and the `csharp:Rasm.AppUi/Charts` estimate report renders the same rollup — Bim PRODUCES the cost source it previously had no owner for, never re-pricing it downstream; a foreign IFC amount enters through `new Money((decimal)amount, iso4217)` at the projection boundary trapping the `InvalidCurrencyException` onto `Model/faults#FAULT_BAND` `BimFault.CodecReject`, and every cost rejection lowers via `.ToError()`, never a new fault family or a thrown currency exception in domain logic.

## [01]-[INDEX]

- [01]-[ESTIMATE]: `CostItem` record (rate x `QuantitySet` quantity by `GlobalId`), the `ConstructionResource` `[Union]` (`Labor`/`Material`/`Equipment`), the `CostValue` rate carrier over the `NodaMoney` `Money`/`Currency`/`ExchangeRate` algebra, the `CostScheduleKind`/`ResourceKind` `[SmartEnum<string>]` partitions, the `CostSchedule` record with its `Money`-fold `Rollup` and `(QuantityKey, ResourceKey)` identity, the `CostMoney` boundary lift trapping `InvalidCurrencyException`, and the `CostProjection.Project` fold from the `IfcCostSchedule` surface.
...
- Packages: GeometryGymIFC_Core, NodaMoney, Thinktecture.Runtime.Extensions, System.IO.Hashing, LanguageExt.Core, Rasm
- Growth: a new cost-schedule kind is one `CostScheduleKind` row reading the next `IfcCostScheduleTypeEnum` member; a new construction-resource modality is one `ConstructionResource` union arm reading the next `IfcConstructionResource` subtype — the Thinktecture generated total `Switch` breaks every `BaseQuantity`/`TaskGlobalId` site at compile time until the arm is added, so a missing resource modality is a build error not a silent fallthrough; a new cost category is one `CostCategory` row; a new per-line binding is one column on `CostItem` projected from the existing `IfcCostItem` surface; a regional or custom currency is one `CurrencyRegistry.TryAdd(CurrencyInfo)` registration, never a parallel currency table; a new rounding rule (cash-denomination, exact) is one `NodaMoney.Context.MoneyContext` scope (`CashDenominationRounding`/`NoRounding`), never a `MidpointRounding` argument; a new cost schedule rides the existing `ProjectAll` fold on one row; never a per-resource-type cost record, never a parallel `LaborCost`/`MaterialCost`/`EquipmentCost` class family, never a `GetLaborCost`/`GetByCategory` operation family, never a hand-rolled `MonetaryAmount` `(double, string)` carrier beside `Money`, and never a second takeoff or schedule source.
- Boundary: `CostSchedule` is ONE record discriminated by the `CostScheduleKind` row and the `ConstructionResource` union — a `LaborResource`/`MaterialResource`/`EquipmentResource` class family or three sibling factory methods is the deleted form mirroring the no-per-element-class law at `Model/elements#ELEMENT_MODEL`; the priced scalar is the `NodaMoney` `Money` `readonly struct` and a hand-rolled `MonetaryAmount` `(double Amount, string Currency)` record with a manual `Add`/`Scale` fold, a `double` cost-arithmetic helper where the `Money` operators discriminate, a naive `total / n` allocation losing remainder pennies where `MoneyExtensions.Split` is lossless, a stringly-typed three-letter currency field validated by hand where the `Money(decimal, string code)` ctor resolves the ISO 4217 registry, a thrown `InvalidCurrencyException` in domain code instead of `.ToError()`, and a second rounding policy threaded as a `MidpointRounding` argument where one `MoneyContext` scope governs are the RETIRED forms; the estimate is a VIEW of the federated `BimModel` — the priced quantity is the `Semantics/properties#PROPERTY_SETS` `QuantitySet.Derive` takeoff joined by `GlobalId` and a re-derived parallel takeoff in this owner is the named seam violation, the resourced schedule is the `sequencing#ConstructionTask` activity network joined by `TaskGlobalId` and a re-modeled cost-side schedule is the named seam violation; the GeometryGym `IfcCostSchedule`/`IfcCostItem`/`IfcCostValue`/`IfcConstructionResource`/`IfcLaborResource`/`IfcConstructionMaterialResource`/`IfcConstructionEquipmentResource`/`IfcRelAssignsToControl`/`IfcRelAssignsToProcess` surface (`.api/api-geometrygym-ifc` scheduling-cost-resource family rows 9-16) is consumed as settled vocabulary through the `IfcConstructionResource` discrimination and a hand-rolled cost reader is the deleted form; the `NodaMoney` `Money`/`Currency` types cross the `Exchange/wire` boundary through `MoneyJsonConverter`/`CurrencyJsonConverter` or the integer `ToMinorUnits` form and never leak past the cost owner into unrelated domain code; the quantified-element selection is the `Model/query#ELEMENT_SET` `ByProperty(Qto_*)` predicate and a parallel cost-element selection arm is the no-second-selection-surface reject; the `(QuantityKey, ResourceKey)` content-key identity is derived through the `Review/diff#MODEL_DIFF` `XxHash128.HashToUInt128` idiom and the cross-libs `csharp:TELEMETRY_LAKE_ANALYTICS` cost catalog reads the schedule by that reference at `Rasm.Compute/Runtime/codecs#CONTENT_ADDRESSING` — minting a second identity scheme for the catalog join is the named cross-folder drift defect; the `CostSchedule.Rollup` schedule total is ONE `Money` fold over the `(quantity x rate x resource)` join, never enumerated per-resource arms; a cost rejection lowers onto `Model/faults#FAULT_BAND` `BimFault` through `.ToError()` and a bare `Fin.Fail` without that lowering is the named seam defect.
...
// --- [MODELS] -----------------------------------------------------------------------------
// The NodaMoney boundary lift: a foreign IFC (double measure, ISO 4217 code) folds into the typed
// `Money` once at the projection edge — the `Money(decimal, string code)` ctor resolves the embedded
// ISO 4217 registry, an unknown code throwing `InvalidCurrencyException` the rail traps onto BimFault.
// CodecReject so domain code never sees a thrown currency fault. An empty code yields `Money.Zero`-
// equivalent over `Currency.NoCurrency` (the additive identity the rollup seeds), never a sentinel pair.
public static class CostMoney {
    public static Money Of(decimal amount, string iso4217) =>
        iso4217.Trim() is { Length: > 0 } code ? new Money(amount, code) : new Money(0m, Currency.NoCurrency);

    public static Fin<Money> TryOf(decimal amount, string iso4217) =>
        Try.lift(() => Of(amount, iso4217)).Run()
            .MapFail(static error => new BimFault.CodecReject($"cost-currency:{error.Message}").ToError());
}

// The applied rate is a `Money` over a `decimal` amount + a resolved `Currency` — never a `double`
// amount beside a bare currency string. The per-basis rate is the native `Money / decimal` divide
// (UnitBasis the IfcMeasureWithUnit magnitude, 1m for a unit rate), so the line value arithmetic
// stays in the decimal-precision generic-math operator set, never a hand-written `amount / basis`.
...
public sealed record CostItem(
    ...
    // The line value is the takeoff quantity times the unit rate over every priced element, summed in
    // the decimal-precision Money algebra (Money * decimal -> Money, the additive operator folding the
    // element contributions). A no-currency rate (Currency.NoCurrency) contributes the additive identity
    // so an unpriced line never poisons the sum, and the cross-multiply is never a `double`.
    public Money ValueOf(BimModel federated) =>
        PricedGlobalIds
            .Choose(id => federated.Elements.Find(e => e.GlobalId == id))
            .Fold(Money.AdditiveIdentity, (total, element) => total + Value.Rate * (decimal)QuantityOf(element));
...
public sealed record CostSchedule(
    ...
    public (UInt128 QuantityKey, UInt128 ResourceKey) Identity => (
        XxHash128.HashToUInt128(Encoding.UTF8.GetBytes(string.Join("",
            Items.Map(static i => $"{string.Join(",", i.PricedGlobalIds)}={i.Value.Rate.Amount}{i.Value.Rate.Currency.Code}@{i.Value.Category.Key}")))),
        XxHash128.HashToUInt128(Encoding.UTF8.GetBytes(string.Join("",
            Resources.Map(static r => $"{r.GlobalId}={r.Kind.Key}:{r.BaseQuantity}")))));

    // The schedule total is ONE Money fold over the (quantity x rate x resource) join — every line value
    // (CostItem.ValueOf, the takeoff times the rate) sums through the Money additive operator (the
    // additive identity the empty seed), the by-category partition the same Money fold keyed on the
    // category, and the by-resource-kind partition a base-quantity sum — never enumerated per-resource arms
    // or a manual `double` accumulation. A cross-currency line is repriced through ExchangeRate.Convert into
    // the reporting currency before it joins the sum (Money + Money rejecting a currency mismatch otherwise).
    public CostRollup Rollup(BimModel federated, Option<ExchangeRate> fx = default) {
        var report = Items.HeadOrNone().Map(static i => i.Value.Applied.Currency).IfNone(Currency.NoCurrency);
        var lines = Items.Map(item => (item.Value.Category.Key, Amount: Reprice(item.ValueOf(federated), report, fx)));
    ...
    static Money Reprice(Money line, Currency report, Option<ExchangeRate> fx) =>
        line.Currency == report || line.Currency == Currency.NoCurrency
            ? line
            : fx.Filter(rate => rate.BaseCurrency == line.Currency).Match(Some: rate => rate.Convert(line), None: () => line);
}
...
public static class CostProjection {
    ...
    // The applied amount and the ISO 4217 currency lift TOGETHER into one Money through the CostMoney
    // boundary (the IfcMonetaryMeasure.Measure and the UnitBasis.UnitComponent IfcMonetaryUnit.Currency),
    // never a double amount beside a bare string; the per-basis denominator is the decimal UnitBasis.
    static CostValue ValueOf(IfcCostItem item) =>
        item.CostValues.AsIterable().HeadOrNone().Match(
            Some: value => new CostValue(
                CostMoney.Of((decimal)AmountOf(value), CurrencyOf(value)),
                BasisOf(value),
    ...
    static decimal BasisOf(IfcCostValue value) =>
...
// The contingency reserve is a Money over the same currency the schedule reports; a drawdown is the
// native Money subtraction floored at the additive identity (an over-draw clamps to zero through the
// IsNegative guard rather than going negative), never a hand-clamped `double` over a bare currency string.
...
public static class CostPerformance {
    public static Fin<EarnedValueReport> EarnedValue(this CostSchedule schedule, BimModel federated, ScheduleNetwork network, Instant statusDate) {
        ...
        return schedule.Items.TraverseM(item => Line(item, federated, taskByElement, taskById, statusDate)).As()
            ...
            .Map(t => EarnedValueReport.Of(t.Bac, t.Bcws, t.Bcwp, t.Acwp, schedule.Items.HeadOrNone().Map(static i => i.Value.Applied.Currency.Code).IfNone("")));
    }
    ...
    static Fin<(double Budget, double Bcws, double Bcwp, double Acwp)> Line(
        ...
        double budget = (double)item.ValueOf(federated).Amount;
...
- [COST_VALUE_MONETARY]: the `IfcCostValue` applied-rate members the `ValueOf` fold reads onto the `CostValue` carrier are verified against the live GeometryGym decompile — `IfcCostValue : IfcAppliedValue` inherits `AppliedValue` (`IfcAppliedValueSelect`), `UnitBasis` (`IfcMeasureWithUnit`), and `Category` (`String`); the `IfcMonetaryMeasure` direct-amount leg exposes `.Measure` (a `Double`, NOT `.Magnitude` — `IfcMonetaryMeasure : IfcDerivedMeasureValue`) and the `IfcMeasureWithUnit.UnitComponent` (`IfcUnit`, narrowed `is IfcMonetaryUnit` whose `.Currency` is the ISO 4217 code) lift TOGETHER into one `NodaMoney` `Money` through `CostMoney.Of((decimal)measure, currency)` — the `new Money(decimal amount, string code)` ctor resolving the embedded ISO 4217 `CurrencyRegistry` and an unknown code's `InvalidCurrencyException` trapping onto `BimFault.CodecReject` (verified: `Money` is a `readonly struct` over a `decimal Amount` + a `Currency`, the full generic-math operator set — `Money + Money`, `Money * decimal → Money`, `Money / decimal → Money`, `Money / Money → decimal` — native, and `MoneyExtensions.Split(int shares)`/`Split(int[] ratios)` returning `IEnumerable<Money>` is the lossless allocation; the `2.7.0` `lib/net10.0` asset binds with an empty net10 dependency group); the `IfcMeasureWithUnit.ValueComponent` (`IfcValue`, narrowed `is IfcMeasureValue` whose `.Measure` is the per-basis denominator) lifts onto the `decimal UnitBasis` so the line rate is the native `Applied / UnitBasis` (`Money / decimal`) divide — the `AmountOf`/`CurrencyOf`/`BasisOf` folds read the real `.Measure`-bearing measure shape and the hand-rolled `MonetaryAmount` `(double, string)` carrier with its manual `Add`/`Scale` is RETIRED for the `Money` algebra, the `IfcCostValue.Category` string lowering onto the `CostCategory` discriminant.
