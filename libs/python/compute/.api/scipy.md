...
| [21] | `scipy.sparse.linalg.ArpackNoConvergence` | ARPACK fault | raised by `eigsh`/`eigs`/`svds(solver='arpack')` on non-convergence within `maxiter`; carries partial `.eigenvalues`/`.eigenvectors` |
...
| [13] | `sparse.linalg.eigsh(A, k=6, M=None, sigma=None, which='LM', v0=None, ncv=None, maxiter=None, tol=0, OPinv=None, mode='normal')` | symmetric eigensolver | ARPACK symmetric eigenpairs; `sigma` triggers shift-invert about `sigma` (interior spectrum), `OPinv` supplies the matrix-free `[A - sigma·M]⁻¹` operator, `mode='normal'/'buckling'/'cayley'` |
| [14] | `sparse.linalg.eigs(A, k=6, M=None, sigma=None, which='LM', v0=None, ncv=None, maxiter=None, tol=0, OPinv=None, OPpart=None)` | general eigensolver | ARPACK nonsymmetric eigenpairs with the same `sigma`/`OPinv` shift-invert levers; `OPpart='r'/'i'` for real/imag shift-invert |
| [15] | `sparse.linalg.lobpcg(A, X, B=None, M=None, Y=None, tol=None, maxiter=None, largest=True)` | block eigensolver | preconditioned LOBPCG for the `k=X.shape[1]` largest/smallest symmetric-definite eigenpairs; `M` is the preconditioner, `B` the mass matrix, `Y` the constraint block — scales past ARPACK for large sparse SPD systems with a good preconditioner |
| [16] | `sparse.linalg.svds(A, k=6, ncv=None, tol=0, which='LM', v0=None, maxiter=None, return_singular_vectors=True, solver='arpack')` | truncated SVD | `k` singular triplets; `solver='arpack'` \| `'lobpcg'` \| `'propack'` selects the backend, `return_singular_vectors='u'/'vh'/True/False` restricts the returned factors |
| [17] | `sparse.linalg.aslinearoperator(A)` \| `LinearOperator(shape, matvec)` \| `expm_multiply(A, B)` \| `onenormest(A)` \| `matrix_power(A, p)` | operator algebra | matrix-free operator construction, action-only `exp(A)·B`, 1-norm estimate |
