...
|  [08]   | request carrying its own reply schemas               | `Schema.TaggedRequest` case owner      | per-channel       | `_tag`     |
...
|  [11]   | foreign class instance at the boundary               | `Schema.instanceOf` / `Schema.declare` | none (`FromSelf`) | reference  |
...
- Law: a case that carries its own reply schemas — payload, success, and failure schemas in one declaration — is `Schema.TaggedRequest`, and its structural `Equal` over the payload fields is the dedup identity the batching resolver consumes; the declaration form is this chooser's row, while the executing surfaces — worker protocol, resolver window — are `boundaries.md`'s and `streams.md`'s.
- Law: `Schema.Struct` survives only as row [02] — an anonymous field block embedded in one owner or a single-consumer shape; a passed-around `Struct` promoted to a `Class` later re-anchors every consumer, so a concept with a name is declared as a class first, never migrated to one.
...
- Law: `new Owner(...)` and `Owner.make(...)` run the filter set, so trusted interior construction proves the same invariants decode proves; raw material enters through decode at the admission boundary — placement is `boundaries.md`'s — and `{ disableValidation: true }` survives only inside a kernel that already proved the invariant it skips.
...
const _Key = Schema.NonEmptyString.pipe(
    // interior refinement: the brand reaches consumers only as Shape["key"], never a standalone export
...
class Group extends Schema.TaggedClass<Group>()("Group", {
    ...
    members: Schema.Array(
        Schema.Union(
            Leaf,
            Schema.suspend((): Schema.Schema<Group, GroupEncoded> => Group),
        ),
    ),
}) {
    get span(): number {
...
[FAULT_DECLARATION]:
- Use: `Schema.TaggedError<Self>()(tag, fields)` for a fault that crosses a wire, logs structurally, or joins a decoded union — one class is the fault value, the fault schema, and the `_tag` catch key, and the instance is yieldable on the carrier.
...
- Boundary: fault-family architecture — the family-per-surface partition, reason discriminants, policy folds, catch routing — is `results-and-effects.md`'s; this page owns the declaration form.
...
class Grade extends Schema.Class<Grade>("Grade")({
    ...
    static readonly FromPacked: Schema.transformOrFail<typeof Schema.NonEmptyString, typeof Grade> = Schema.transformOrFail(
...
- Reject: `Schema.Unknown` smuggling a foreign instance past the boundary; a standalone `is`-guard function beside a schema that owns it; an `as` cast where the declaration proves identity.
...
const HandleFromSelf = Schema.instanceOf(Handle, {
    // a first-class AST citizen: composes as a field, joins unions, feeds every derived surface
...
[DERIVED_INSTANCES]:
- Law: the derivation set is `Arbitrary.make(Owner)` yielding a `FastCheck.Arbitrary<Owner>`, `JSONSchema.make(Owner)` yielding a `JSONSchema.JsonSchema7Root`, `Pretty.make(Owner)`, `Schema.equivalence(Owner)` yielding an `Equivalence.Equivalence<Owner>`, `Schema.is(Owner)` as the derived guard, and `Schema.standardSchemaV1(Owner)` as the one view handed to foreign validation consumers — a hand-written generator, printer, comparator, guard, JSON schema, or parallel validator beside an owner restates what the AST already proves.
- Law: field refinements are what keep derived surfaces truthful — `between`, `maxLength`, `pattern`, and brands flow into generation and schema emission, so a refinement skipped at the field surfaces downstream as a lying arbitrary and an under-constrained schema.
...
[ANNOTATIONS]:
- Law: identity is positional on class owners — `Schema.Class<Self>("<identifier>")` — and `.annotations({ identifier })` on schema values; `JSONSchema.make` keys `$defs` by it and parse issues name it, so an owner without identity emits anonymous schemas and unattributable failures.
...
class Member extends Schema.Class<Member>("Member")(
    {
        ...
        score: Schema.Number.pipe(Schema.between(0, 1), Schema.annotations({ decodingFallback: () => Either.right(0) })),
    },
    {
        title: "<title-a>",
    ...
    static readonly Patch = Schema.Struct(Member.fields).pipe(Schema.omit("key"), Schema.partialWith({ exact: true }));
...
const _jsonSchema: JSONSchema.JsonSchema7Root = JSONSchema.make(Member);
