...
[STACKING]:
- `SharpGLTF.Core`(`Rasm.Bim/.api/api-sharpgltf.md`): SharpGLTF carries no meshopt or Draco encoder, so this surface owns the `EXT_meshopt_compression` encode path — `EncodeFilterOct`/`…Quat`/`…Exp` pre-quantize into the filter-coded layout the extension declares, then `EncodeVertexBufferLevel`/`EncodeIndexBuffer` produce the `byte[]` payload attached under the extension's bufferView metadata; `SharpGLTF.Toolkit` mesh building feeds raw positions and indices into the `Generate*`→`Optimize*` head.
...
- Bim consumer anchor: the canonical interleaved vertex struct threads the index-optimize/encode path as one `ReadOnlySpan<TVertex>` shared with Drako intake; every `Bounds`/`*Statistics` value return folds into typed receipt fields under the codec `Fin`/`Eff` rail, `ResolveLibrary`/`SetAllocator` native-load faults mapping to a typed boundary fault.
