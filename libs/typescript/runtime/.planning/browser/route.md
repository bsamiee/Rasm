...
- Law: the query codec is `nuqs/server` and only that — the `parseAs*` atoms, `createParser` for bespoke encodings, and `Router.param(shape)` for structured values: the fusion of `parseAsJson` over `Schema.standardSchemaV1`, so a core-Schema-refined value rides one URL param, decodes once into branded interior shape, and the kernel-Schema-into-URL mapper has one spelling; the React hook/adapter surface is the ui wave's tier and never imported here.
...
declare namespace Router {
  ...
  type StaticKey<Rows extends Router.Rows> = {
    readonly [K in keyof Rows & string]: _Names<Rows[K]["path"]> extends never ? K : never
  }[keyof Rows & string]
  type Travel =
    | { readonly kind: "push"; readonly href: string }
    | { readonly kind: "replace"; readonly href: string }
  ...
  type Shape<Rows extends Router.Rows> = {
    ...
    readonly go: (travel: Travel) => Effect.Effect<void>
  ...
  type Spec<Rows extends Router.Rows> = {
    ...
    readonly rows: Rows
    readonly fallback: StaticKey<Rows>
  }
}
...
const _routeFamily = Fault.Class.family(["unsupported"] as const, {
  unsupported: Fault.Class.row({
    ...
    render: () => "the host carries no Navigation API, so no intercept hook exists to mount",
  }),
})
  ...
  const layer = (
    ...
    Layer.scoped(
      ...
      Effect.gen(function* () {
        ...
        const _commit = (target: Router.Location<Rows>, url: URL): Effect.Effect<void> =>
        ...
        return {
          ...
          go: (travel) => Effect.sync(() => navigation.navigate(travel.href, { history: travel.kind })),
...
- Law: the pending flow is single-use and time-bounded — `land` drops the record before acting on it, so a replayed callback finds nothing and folds to `replay`; a record older than `spec.grace` folds to `lapsed`, while a minted instant ahead of the observed clock folds to `skew` instead of turning an absolute distance into a plausible age; the state echo, when the record carries one, equals the callback's or folds to `replay` — defense in depth beside the server-side single-use stash `security/authn/oauth` owns. The departure commit is the module's platform-forced statement boundary — `location.assign` unloads the document, nothing sequences after it, and the implementer carries the `// BOUNDARY ADAPTER` mark on `_departed`'s first line.
...
- Law: the command arm folds the core levels — `Evidence.Availability.admits(snapshot, command)` answers total over the level fallbacks, so a command absent from the snapshot's map still lands a verdict and the guard never re-derives posture the levels already decide.
- Law: the `beforeunload` arm and its synchronous registry read are the module's platform-forced statement boundary — the native handler decides within its dispatch, so it reads the dirty cell through the captured runtime's sync run (the sanctioned callback-boundary spelling) and prevents default only while work is held; the implementer carries the `// BOUNDARY ADAPTER` mark on the `beforeunload` bracket's first line.
...
- Boundary: what a flag verdict means is its serving surface's law; what availability means is `core/state/evidence`'s levels; the confirm ceremony renders behind the ui-satisfied `Confirm` Tag; this cluster owns only the fold order.
      ...
      const _settled: Effect.Effect<SessionStatus> = Effect.flatMap(vault.status.get, (held) =>
        SessionStatus.$is("Authenticating")(held)
          ? vault.status.changes.pipe(
              ...
              Stream.runHead,
              Effect.timeoutOption(boot.ceilings.settle),
