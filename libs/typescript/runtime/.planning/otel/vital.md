...
- Boundary: URL-bearing span enrichment (resource timing folded onto fetch/XHR spans) is the `sdk-trace-web` RUM toolkit riding `emit#LANES`' `web` row — this bridge reads the Performance Timeline for vitals only and is never a second span source.
...
- Packages: `effect` (`Metric`, `Layer`, `Stream`), `@rasm/ts/core` (`Convention`).
