...
- `MaximumStreamCapacity` caps a single stream and fires `StreamOverCapacity` — the bounded-payload guard the admission path reads; `ThrowExceptionOnToArray` upgrades an unintended `ToArray` edge copy to a fault; `GenerateCallStacks` captures allocation/dispose stacks for leak attribution under debug alone.
...
- `Runtime/tiles#TILE_PARTITION`: tileset-manifest emit writes a `Utf8JsonWriter` straight into a rented stream, commits each node past the writer's own `BytesPending` watermark (the bound a `TessellationPolicy` column carries beside the depth that forces it), and keys off `GetReadOnlySequence()` through the incremental identity above — so a policy-depth octree rides chained blocks rather than a growable writer's doubling ladder through the large-object heap.
- within-lib: the `Runtime/channels#ARTIFACT_FRAMES` `FrameEdge` fold sizes each frame via `IMessage.CalculateSize` and stages it here; the manager's `UsageReport`/`StreamDoubleDisposed`/`StreamOverCapacity`/`BufferDiscarded` events fold into `Tensor/memory#STREAM_POOL` `PoolLedger`; the `Model` inference and embedding lanes stage tensor and token buffers through the same manager.
