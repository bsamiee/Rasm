...
| :-----: | :-------------------------- | :---------------- | :---------------------------------------------- |
|  [01]   | `AbstractLinearOperator`    | operator base     | abstract base for all linear operators          |
|  [02]   | `MatrixLinearOperator`      | dense operator    | operator from a JAX array matrix                |
|  [03]   | `FunctionLinearOperator`    | function operator | operator defined by a linear callable `fn(v)`   |
|  [04]   | `JacobianLinearOperator`    | Jacobian operator | operator from `jax.jacobian` of a function      |
|  [05]   | `DiagonalLinearOperator`    | diagonal operator | operator from a diagonal vector                 |
|  [06]   | `IdentityLinearOperator`    | identity operator | identity of given shape                         |
|  [07]   | `PyTreeLinearOperator`      | pytree operator   | operator wrapping a pytree-structured matrix    |
|  [08]   | `TaggedLinearOperator`      | tagged operator   | operator with explicit structure tags           |
|  [09]   | `TangentLinearOperator`     | tangent operator  | forward-mode tangent of a linear operator       |
...
| :-----: | :----------------- | :--------------- | :--------------------------------------------------------------------------------------------- |
|  [01]   | `AutoLinearSolver` | dispatch solver  | `(well_posed: bool \| None)`                                                                   |
|  [02]   | `LU`               | direct solver    | `()`                                                                                           |
|  [03]   | `QR`               | direct solver    | `()`                                                                                           |
|  [04]   | `Cholesky`         | direct solver    | `()`; requires positive-definite operator                                                      |
|  [05]   | `SVD`              | direct solver    | `(rcond: float \| None = None)`; least-squares capable                                         |
|  [06]   | `Triangular`       | direct solver    | `()`; requires triangular tag                                                                  |
|  [07]   | `CG`               | iterative solver | `(rtol, atol, norm=max_norm, stabilise_every=10, max_steps=None)`; symmetric positive-definite |
|  [08]   | `GMRES`            | iterative solver | `(rtol, atol, norm=max_norm, max_steps=None, restart=20, stagnation_iters=20)`                 |
|  [09]   | `BiCGStab`         | iterative solver | `(rtol, atol, norm=max_norm, max_steps=None)`                                                  |
...
| [INDEX] | [SYMBOL]   | [TYPE_FAMILY]  | [CAPABILITY]                                                                                          |
| :-----: | :--------- | :------------- | :---------------------------------------------------------------------------------------------------- |
|  [01]   | `Solution` | result carrier | fields: `value`, `result` (`RESULTS`), `stats`, `state`                                               |
|  [02]   | `RESULTS`  | result enum    | `successful`, `max_steps_reached`, `singular`, `breakdown`, `stagnation`, `conlim`, `nonfinite_input` |
...
| :-----: | :------------------------------------------------------------------------------------------------------------------- | :------------- | :------------------------------------------- |
|  [01]   | `linear_solve(operator, vector, solver=AutoLinearSolver(True), *, options=None, state=..., throw=True)` → `Solution` | solve          | JIT-compatible solve                         |
|  [02]   | `invert(operator, solver=AutoLinearSolver(True), *, options=None, throw=True)` → `FunctionLinearOperator`            | invert         | operator inverse as `FunctionLinearOperator` |
...
| [INDEX] | [SURFACE]                                          | [ENTRY_FAMILY]   | [RAIL]                                   |
| :-----: | :------------------------------------------------- | :--------------- | :--------------------------------------- |
|  [01]   | `linearise(operator)` → `AbstractLinearOperator`   | linearise        | freeze operator into a linear form       |
|  [02]   | `materialise(operator)` → `AbstractLinearOperator` | materialise      | dense-matrix realization of any operator |
|  [03]   | `diagonal(operator)` → `Array`                     | diagonal extract | extract diagonal as shaped JAX array     |
|  [04]   | `conj(operator)` → `AbstractLinearOperator`        | conjugate        | complex-conjugate of operator            |
...
| [INDEX] | [SURFACE]                                                      | [ENTRY_FAMILY] | [RAIL]                                |
| :-----: | :------------------------------------------------------------- | :------------- | :------------------------------------ |
|  [01]   | `is_symmetric(operator)` / `symmetric_tag`                     | tag predicate  | symmetry check and tag constant       |
|  [02]   | `is_diagonal(operator)` / `diagonal_tag`                       | tag predicate  | diagonal structure                    |
|  [03]   | `is_positive_semidefinite(op)` / `positive_semidefinite_tag`   | tag            | PSD structure                         |
|  [04]   | `is_negative_semidefinite(op)` / `negative_semidefinite_tag`   | tag            | NSD structure                         |
|  [05]   | `is_lower_triangular(op)` / `lower_triangular_tag`             | tag            | lower-triangular structure            |
|  [06]   | `is_upper_triangular(op)` / `upper_triangular_tag`             | tag            | upper-triangular structure            |
|  [07]   | `is_tridiagonal(operator)` / `tridiagonal_tag`                 | tag            | tridiagonal structure                 |
|  [08]   | `has_unit_diagonal(operator)` / `unit_diagonal_tag`            | tag            | unit-diagonal structure               |
|  [09]   | `tridiagonal(lower, mid, upper)` → `TridiagonalLinearOperator` | construction   | tridiagonal from three arrays         |
