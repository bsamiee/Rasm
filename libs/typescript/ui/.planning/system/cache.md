...
- Law: the caller's own fault channel rides through untouched — `mint` fails on its own channel and this entry adds only `CacheFault`, so a cold-start warm that cannot fetch reports the fetch's reason rather than a cache reason wearing it.
