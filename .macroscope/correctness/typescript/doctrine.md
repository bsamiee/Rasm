...
- Every dependency is a Tag in the `R` channel satisfied through the Layer graph; the owner is one `Effect.Service` class. An `interface` + `Context.GenericTag` + `Layer.effect` triple restating one concept, module-level live singletons, parameter-drilled dependencies, and mock frameworks are findings. Do not flag the one sanctioned `Context.GenericTag`: a service polymorphic in a caller-supplied vocabulary keying a single `of<G>()` mint whose module owns the key string — the class forms would widen its vocabulary-typed members to bare `string` (`docs/stacks/typescript/services-and-layers.md` `[ONE_NAME_OWNER]`).
