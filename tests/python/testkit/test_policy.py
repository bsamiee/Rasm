...
from tests.python.testkit import laws as laws_mod
from tests.python.testkit.bench import _series_from_storage, pytest_benchmark_update_json
from tests.python.testkit.laws import assert_law_coverage, auto_exempt, consume_covers, LawRecord, MANIFEST, register_tree, spec, Sut, uncollected_laws
from tests.python.testkit.runtime import PROFILE_DEFAULT, PROFILE_STATEFUL, REPO_ROOT
from tests.python.testkit.spec import support_matrix
...
def test_law_coverage_gate() -> None:
    """Registered SUT public symbols must have a law or an explicit exemption; a partially collected package skips by name, every whole one gates."""
...
def test_law_census_detects_partial_collection_and_gate_self_skips(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest) -> None:
    """An uncollected on-disk law module suppresses the census by skip; once every law module is imported the gate still fails a real gap."""
...
def test_register_tree_registers_only_source_folders(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Disk shape drives registration: source folders register under the name their modules import by, with derived suites; sourceless folders and loose files never do."""
    source = tmp_path / "src"
...
def test_law_module_naming_matches_live_session_imports(pytestconfig: pytest.Config) -> None:
    """The census dotted-name derivation matches pytest's live import-mode naming, else every census would read silently partial."""
    law_files = {path for item in _collect_session_items(pytestconfig) if (path := Path(item.path)).is_relative_to(REPO_ROOT) and any(fnmatch.fnmatch(path.name, pattern) for pattern in laws_mod._LAW_GLOBS)}
...
def test_spec_follows_active_profile_pins_named_and_scales_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        @spec(int, profile="rasm-parity", law="probe-pinned")
    ...
    assert len(pinned_runs) == hyp_settings.get_profile("rasm-parity").max_examples, f"named pin lost: {len(pinned_runs)} examples"
...
def test_hypothesis_profile_registry_carries_lane_invariants() -> None:
    ...
    lanes = (PROFILE_DEFAULT, "rasm-ci", "rasm-stress", "rasm-debug", PROFILE_STATEFUL, "rasm-parity", "rasm-adversarial")
...
def test_covers_tuple_consumed_at_collection() -> None:
    ...
    assert any(rec.law == "covers" and rec.module == __name__ and rec.subject == "consume_covers" for rec in MANIFEST), "COVERS declared above was not consumed — pytest_collection_modifyitems is not folding module COVERS tuples"
...
def test_manifest_records_carry_named_identity() -> None:
    """Every manifest entry has non-empty subject, law, and module, and no subject is an anonymous ``<lambda>``."""
    if not MANIFEST:
        pytest.skip("MANIFEST is empty — no law files collected in this session")
    for rec in MANIFEST:
        assert rec.subject and rec.law and rec.module, f"empty identity field in {rec!r}"
        assert "<lambda>" not in rec.subject, f"anonymous-subject law record: {rec!r}"
...
def test_observability_gate_routes_hypothesis_observations_to_artifacts() -> None:
    ...
    law = "tests/python/testkit/test_strategies.py::test_literal_form_stays_inside_its_vocabulary"
