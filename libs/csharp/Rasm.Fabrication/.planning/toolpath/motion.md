...
| [INDEX] | [CLUSTER]  | [OWNS]                                                                                          |
| :-----: | :--------- | :--------------------------------------------------------------------------------------------- |
|   [1]   | CAM_MOTION | `ToolpathKind` contour/pocket/drill/trochoidal move generators over Geometry2D offset; one motion owner over the kind axis |

## [2]-[CAM_MOTION]

- Owner: `ToolpathKind` `[SmartEnum<string>]` the toolpath strategy axis (`contour`/`pocket`/`drill`/`trochoidal`) carrying the per-kind spiral/adaptive column; `Cam` the static motion fold over the `ToolpathKind` axis generating the cut moves, then driving each move target through the FK/IK chain and emitting the `Motion` joint stream.
...
public sealed partial class ToolpathKind {
    public static readonly ToolpathKind Contour = new("contour", spiral: false, adaptive: false);
    public static readonly ToolpathKind Pocket = new("pocket", spiral: true, adaptive: false);
    public static readonly ToolpathKind Drill = new("drill", spiral: false, adaptive: false);
    public static readonly ToolpathKind Trochoidal = new("trochoidal", spiral: true, adaptive: true);
...
public static class Cam {
    static readonly FrozenDictionary<ToolpathKind, Func<FrontierPolicy.Cam, Loop, Seq<Move>>> Generators =
        new (ToolpathKind Kind, Func<FrontierPolicy.Cam, Loop, Seq<Move>> Gen)[] {
            ...
            (ToolpathKind.Pocket, static (p, loop) => Pocket(loop, p.ToolRadius, p.StepOver)),
            (ToolpathKind.Drill, static (p, loop) => Peck(loop, p.ToolRadius)),
            (ToolpathKind.Trochoidal, static (p, loop) => Trochoidal(loop, p.ToolRadius, p.StepOver)),
