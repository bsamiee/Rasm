...
[StructLayout(LayoutKind.Auto)]
public readonly record struct Flow<T>(Pear<T> Pear, Option<Grasshopper2.Data.Path> Path) {
    public T Item => Pear.Item;
    public MetaData Meta => Pear.Meta;
    public Flow<TOut> Project<TOut>(TOut item) => new(Pear: Pear<TOut>.Create(item: item, meta: Meta), Path: Path);
}
...
public static class Bridge {
    ...
    private static Fin<Seq<Flow<TVal>>> MissingFlow<TVal>(IPort port) =>
        Missing<TVal>(port: port).Map(static pears => pears.Map(static pear => new Flow<TVal>(Pear: pear, Path: Option<Grasshopper2.Data.Path>.None)));
    private static Seq<Leaf<T>> Leaves<T>(Seq<Flow<T>> values) =>
        toSeq(values.GroupBy(static value => value.Path.IfNone(new Grasshopper2.Data.Path(0)))
        ...
        internal static readonly FrozenDictionary<Access, Func<IDataAccess, int, IPort, Fin<Seq<Flow<T>>>>> Readers =
            new Dictionary<Access, Func<IDataAccess, int, IPort, Fin<Seq<Flow<T>>>>> {
                [Access.Item] = static (access, slot, port) => access.GetPear<T>(index: slot, pear: out Pear<T> pear) switch {
                    true when pear is { Item: not null } => Fin.Succ(Seq(new Flow<T>(Pear: pear, Path: Option<Grasshopper2.Data.Path>.None))),
                ...
                [Access.Twig] = static (access, slot, port) => access.GetPears<T>(index: slot, pears: out Pear<T>[] pears) switch {
                    true when pears.Length > 0 => toSeq(pears.Select((pear, index) => (Pear: pear, Index: index))).TraverseM(item => item.Pear is { Item: not null }
                        ? Fin.Succ(new Flow<T>(Pear: item.Pear, Path: Option<Grasshopper2.Data.Path>.None))
                ...
                [Access.Tree] = static (access, slot, port) => access.GetTree<T>(index: slot, tree: out Tree<T> tree) switch {
                    true when tree.ItemCount > 0 => toSeq(tree.EnumerateLeaves().Select((leaf, index) => (Leaf: leaf, Index: index))).TraverseM(item => item.Leaf.Pear is { Item: not null }
                        ? Fin.Succ(new Flow<T>(Pear: item.Leaf.Pear, Path: Some(item.Leaf.Site.Path)))
        ...
        internal static readonly FrozenDictionary<Access, Func<IDataAccess, int, Seq<Flow<T>>, Unit>> Writers =
            new Dictionary<Access, Func<IDataAccess, int, Seq<Flow<T>>, Unit>> {
                ...
                [Access.Tree] = static (access, slot, values) => Effect(action: () => {
                    ITree tree = values.Exists(static value => value.Path.IsSome)
