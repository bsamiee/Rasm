...
Two zero-dependency cross-cutting tenants live here because faults is the one tier below every consumer. `latched[R, **P]` is the branch's one-shot install latch — a receipt-agnostic aspect over injected `read`/`write`/`reentrant` accessors that mints once and restamps the prior receipt on re-entry; `observability/telemetry#TELEMETRY` `Telemetry.install` and `observability/metrics#METRIC` `Metrics.install` compose it, each passing a `msgspec.structs.replace`-built `reentrant` closure. `SCOPES` is the one instrumentation-scope vocabulary: every tracer, meter, and service literal in the branch is a `Scope`-keyed row, each consumer minting its handle from its row (`trace.get_tracer(SCOPES[Scope.WIRE])`, `metrics.get_meter(SCOPES[Scope.METER])`), so no page re-mints a scope string and a scope rename is one row edit.
...
class BoundaryFault:
    ...
    def of(subject: str, cause: BaseException) -> "BoundaryFault":
        # catch-all keeps `str(cause)` — an unclassified raise's message is its discriminant,
        # class name only when message-less; CLASSIFY rows keep `type(cause).__name__`, the TYPE being their tag.
    ...
    def facts(self) -> dict[str, object]:
        # `budget: float`/`code: int` ride as native scalars — the receipts `EventDict` is
        # `dict[str, object]` and its `enc_hook=repr` renderer serializes them; pre-`str()` is the deleted form.
        match self:
            case BoundaryFault(tag="aggregate", aggregate=members):
...
# folds onto the rail, rather than each adapter re-catching the violation inline; the
# `boundary`/`async_boundary` decorations below are the in-page exemplar application.
FAULT_CONF: Final[BeartypeConf] = BeartypeConf(violation_type=BeartypeCallHintViolation)

# the one instrumentation-scope row table (Map rail): consumers mint handles from rows —
# trace.get_tracer(SCOPES[Scope.WIRE]), metrics.get_meter(SCOPES[Scope.METER]) — never a literal.
    ...
    match by:
        ...
        case Disposition.ACCUMULATE | Disposition.PARTITION:
            ...
            if by is Disposition.PARTITION:
                # PARTITION is total by construction — the split itself cannot fail, and the
                # uniform RuntimeRail return keeps the three dispositions one overloaded surface.
...
def latched[R, **P](
    ...
    # receipt-agnostic one-shot install latch: mint once, restamp the prior receipt on re-entry;
    # consumers inject a `msgspec.structs.replace`-built `reentrant` closure over their own receipt type.
