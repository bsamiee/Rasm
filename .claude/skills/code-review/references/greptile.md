...
Greptile reviews committed unmerged commits against a base ref — uncommitted edits never enter, recorded by a launch-time `warning: N uncommitted files not included` stderr line — and `-b <base>` aims the range at any committish: branch, remote-tracking ref, or SHA. Reviews are language-agnostic over the diff; `ignorePatterns` and `--include` are the only file gates.
...
- Size gating is a client-side preflight over `git diff -U3 <merge-base>...HEAD`, refusing in under a second before any network at three hard caps: over 500 changed files (`this review touches N files`), over 50 changed Greptile config files, and over 3,000,000 payload bytes (`this review is too large to send`) summing per-file patch and path bytes, range commit subjects and bodies, changed-config contents, and `--instructions` text.
...
- [FILES] admits a path only where the file defines correct for a diff the engine reviews and no sibling already carries that law: the tier registries (cross-libs, per branch, per test tree, per tool) earn rows because each settles a distinct scope, while a package-tier registry inherits its branch row's scope and earns none. Enumerating every instance of a growing class here re-arms the frozen-roster shape `.macroscope/correctness/general/live-table-law.md` legislates — a new tier earns a row, a new package never does.
