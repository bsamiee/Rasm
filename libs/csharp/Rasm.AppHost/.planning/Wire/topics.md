...
- Cases: topic rows are the declared event channels — `Command`, `Lifecycle`, `Health`, `Delivery`, and the two `Rasm.AppUi` `Collab/sync` channels, `Collab` carrying the durable document deltas and `Presence` the lossy presence and awareness frames — each binding its `DrainSpec` back-pressure row; `BusFault` = Text | TopicUnknown | SubscriptionFull | JoinUnmatched.
...
public sealed partial class Topic {
    ...
    public static readonly Topic Collab = new("collab", DrainSpec.ReceiptFanOut);
    public static readonly Topic Presence = new("presence", DrainSpec.ReceiptFanOut);
