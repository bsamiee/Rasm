...
using VividOrange.Standards.Eurocode;                // En1992/En1993/En1995/En1996 + NationalAnnex (the typed citations and the placement annex)
...
// consumer's DesignCode roster exactly for the member bases; the glazing and connection keys are
// section-and-load-path altitude only and cross to no member check.
...
public sealed partial class DesignBasis {
    ...
    public static readonly DesignBasis Tms402       = new("tms402",     ComponentAuthority.Astm, SafetyFormat.LimitState, 1.00, 1.00, NoCitation,   Linear,
    ...
    // partial factor. An LRFD project selects the same row and the hazard reduction reads the format off it.
    ...
    // equivalent-rectangular-stress-block coefficient 0.85 on f'c.
    ...
    // The EN citations, partitioned: Part 1-1 the general member rules every family reads, Part 1-8 the joint rules
    // the bolted-connection lifts cite (.api/api-vividorange-standards.md).
...
public abstract partial record CapacityReceipt {
    ...
    // owner bans. Lift folds Fastening.ShearCapacityKn × planes, BearingCapacityKn, and TensileLoadKn onto the ONE
    // Connection triple, so a bolt group reports through the same governing-action fold as a weld.
...
// The biaxial moment magnitude and the shear resultant are derived projections, never re-passed columns.
[ComplexValueObject]
public readonly partial struct Demand {
    ...
    static partial void ValidateFactoryArguments(
        ...
        validationError = double.IsFinite(axialKn) && double.IsFinite(momentYKnm) && double.IsFinite(momentZKnm)
            ...
            && double.IsFinite(unitShearKnPerM)
            ? null
            : new ValidationError($"<demand-nonfinite:n={axialKn:R}:my={momentYKnm:R}:mz={momentZKnm:R}:vy={shearYKn:R}:vz={shearZKn:R}:mt={torsionKnm:R}:rb={bearingKn:R}:q={unitShearKnPerM:R}>");
    ...
    public static Fin<Demand> Of(double axialKn, double momentYKnm, double momentZKnm, Op key,
        ...
        double unitShearKnPerM = 0.0) =>
        Validate(axialKn, momentYKnm, momentZKnm, shearYKn, shearZKn, torsionKnm, bearingKn, unitShearKnPerM, out Demand demand) is { } error
...
public abstract partial record SectionCapacity {
    ...
    public sealed record Connection(
        ...
        bool Combines = false) : SectionCapacity(Basis);   // aws-d1-1 · astm-d1002 · aisc360 · icc-es · en1993-1-8 · en1995
    ...
    // triple. Every arm is TOTAL over the seven Demand columns: an action the case's capacity surface does not resist
    // folds through GuardedRatio against 0 and governs loud — a hull shear, an RC torsion, a steel bearing, a masonry
    // torsion demand can never pass silent (the consumed-action discipline).
    ...
    static Utilisation ConnectionUtilisation(Connection c, Demand demand) =>
        Worst(
            (LateralRatio(c, demand), GoverningAction.Shear, None),
            (GuardedRatio(Math.Max(demand.AxialKn, 0.0), c.TensionKn.IfNone(0.0)), GoverningAction.Axial, None),
    ...
    public static SectionCapacity Lift(CapacityReceipt receipt) => receipt.Switch(
        ...
        reinforcedMasonry: static r => new MasonryReinforced(
            ...
            MasonryReduction.Of(r.Basis, r.HeightMm, r.Section.GoverningRadiusMm).Value),
        glass: static r => new GlassPane(r.Capacity.BendingKnmPerM, r.Capacity.ResistanceMpa, r.Capacity.EffectiveThicknessMm, r.Capacity.LoadShareFraction),
        ...
        weld: static r => new Connection(DesignBasis.AwsD11, r.Row.DirectionalShearKn(Angle.FromDegrees(r.LoadAngleDeg)), None, 0.0),
        ...
        // EN 1993-1-8 §3.6 bearing-type bolt: the shank/thread shear over the assembly's own admitted plane count, the
        // ply bearing the shank develops, and the tension the grade's banded fu over the stress area — every value the
        // sibling family's Fastening algebra already owns, read through the assembly rather than re-derived.
        bolt: static r => new Connection(DesignBasis.En1993Joints,
            r.Assembly.ShearPlanes.Value * Fastening.ShearCapacityKn(r.Assembly.Thread, r.Assembly.Grade, r.Plane),
            Some(Fastening.TensileLoadKn(r.Assembly.Thread, r.Assembly.Grade)),
            Fastening.BearingCapacityKn(r.Assembly.Thread, r.Bearing)),
