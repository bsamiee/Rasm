...
class ModelReceipt(Struct, frozen=True):
    ...
    def contribute(self) -> Iterable[Receipt]:
        yield Receipt.of("rasm.geometry.energy.model", self)
...
class BuildingModel(Struct, frozen=True):
    ...
    def wire(self) -> "RuntimeRail[tuple[bytes, ContentKey]]":
        # the cross-language seam: content-keyed canonical HBJSON document bytes — the C# Rasm.Bim
        # Energy exchange and Rasm.Compute simulation are peers at THIS payload, one XxHash128 seed.
        def fold() -> tuple[bytes, ContentKey]:
            document = _ENCODER.encode(self.model.to_dict(included_prop=("energy",)))
            return document, ContentIdentity.of("energy-model", document)
    ...
    def _gated(cls, model: "Model", modality: str) -> Self:
        ...
        return cls(model=model, content_key=ContentIdentity.of("energy-model", document))
