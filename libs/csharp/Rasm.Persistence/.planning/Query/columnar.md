...
// suite content addresses. `Partition` names the hive KEY the arm's generation directories carry, and the
// landed value fills it — the key is the arm's, the value the generation's.
[SmartEnum<string>]
[KeyMemberEqualityComparer<ComparerAccessors.StringOrdinal, string>]
...
// an alert lookback silently shortens the audit trail the moment an operator retunes the alert. `Root` names
...
// residence below the family is the defect this shape forecloses. `Fits`, `Ingest`, `Tenancy`, `Retain`,
...
// that limitation in the row's own words instead of handing an operator two keys and no reason.
[SmartEnum<string>]
[KeyMemberEqualityComparer<ComparerAccessors.StringOrdinal, string>]
[KeyMemberComparer<ComparerAccessors.StringOrdinal, string>]
public sealed partial class Residence {
    public static readonly Residence Series = new("series", ResidenceTenancy.SortKey,
        ...
        "`add_retention_policy` drops chunks in-database on the Timescale bgworker's own schedule",
    ...
    public static readonly Residence Fleet = new("fleet", ResidenceTenancy.SortKey,
        ...
        "`TTL … DELETE` with `ttl_only_drop_parts` on the engine's own merge schedule",
    ...
    public static readonly Residence Lake = new("lake", ResidenceTenancy.Prefix,
        ...
        "generation eviction on the object plane's own lifecycle; this residence creates no storage and expires none itself",
    ...
    public string Ingest { get; }
    public string Retain { get; }
    ...
    private Residence(string key, ResidenceTenancy tenancy, string fits, string ingest, string retain, string degrade,
        ...
        (Tenancy, Fits, Ingest, Retain, Degrade, Projections, Physical, ListOf, MapOf, DictOf, Quote, Literal, Stamp, Bucket, Quantile, Diagnose, Statements) =
        (tenancy, fits, ingest, retain, degrade, projections, physical, listOf, mapOf, dictOf, quote, literal, stamp, bucket, quantile, diagnose, statements);
    ...
    // mints rather than the verb that raised it: `Fits`, `Ingest`, `Retain`, and `Degrade` are the estate floor's
    // own column names, and a member group sharing a name with a property on one type does not compile.
...
// Every refusal names its RESIDENCE beside one engine-neutral diagnostic pair, so no case carries a column
// only one backend can fill — a per-engine case (the `int ErrorCode` a ClickHouse server exception supplies
// and no relational sibling has) makes the family answerable by exactly one row and forces every other
// backend through a case named for someone else's engine.
[Union]
...
public static class SeriesResidence {
    public static Seq<string> Statements(AnalyticsSchema schema, ResidencePolicy policy) {
        ...
        // bounded text keys alone — segmenting on a `KeyHex` content key mints one compressed batch per row and
        // deletes the compression the columnstore exists for. Both lead with the tenant, so a keyless dataset —
