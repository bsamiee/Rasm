...
|  [03]   | `DeploymentSchedule`          | `{ pulumiOperation: PulumiOperation, scheduleCron? XOR timestamp? }` — `update/preview/refresh/destroy` |
...
| :-----: | :----------- | :--------------------------------------------------------------------------------------------------------------- |
|  [01]   | environments | `Environment { organization, project?, name, yaml: Input<Asset \| Archive> }`, `EnvironmentVersionTag`           |
|  [02]   | team RBAC    | `Team`, `TeamStackPermission`, `TeamEnvironmentPermission`, `TeamRoleAssignment`                                 |
|  [03]   | org RBAC     | `OrganizationMember`, `OrganizationRole`, `ApprovalRule`                                                         |
|  [04]   | tokens       | `AccessToken`, `OrgAccessToken`, `TeamAccessToken`                                                               |
|  [05]   | token scopes | `buildStackScopedPermissions`/`buildEnvironmentScopedPermissions`/`buildAllowPermissions`                        |
|  [06]   | execution    | `AgentPool` (self-hosted runners), `OidcIssuer` (federated org trust), `TemplateSource` (review-stack templates) |
|  [07]   | governance   | `PolicyGroup`, `PolicyPack`, `Stack`, `StackTag`/`StackTags`, `InsightsAccount`                                  |
|  [08]   | reads        | `getCurrentUser`, `getEnvironment`, `getPolicyPack(s)`, `getOrganizationMember(s)`, `getInsightsAccount(s)`      |
