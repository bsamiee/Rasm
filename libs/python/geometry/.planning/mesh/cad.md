...
class StepBridge:
    @staticmethod
    def tessellate(
        contributor: ReceiptContributor, source_bytes: bytes, fmt: BridgeFormat, policy: IdentityPolicy = CANONICAL_POLICY
    ) -> "RuntimeRail[bytes]":
    ...
    def _run(contributor: ReceiptContributor, source_bytes: bytes, fmt: BridgeFormat, policy: IdentityPolicy) -> bytes:
        ...
        contributor.contribute("emitted", "step-bridge", glb_path, {"format": fmt, "shapes": str(labels.Length()), "mass": str(props.Mass())})
        return glb
```
