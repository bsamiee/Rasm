...
- members: every documented primitive, datastructure, free function, serializer, host probe, and subpackage resolves on the cp312 companion — no phantom. The construction/accessor classmethods (`Network.from_nodes_and_edges`, `Mesh.from_json`/`dual`/`subdivide`/`vertices_attributes`/`number_of_edges`, `VolMesh.dual`, `Assembly.graph`, `NurbsSurface.to_mesh`), the transform constructors (`Frame.worldXY`, `Transformation.from_frame_to_frame`, `Translation.from_vector`, `Scale.from_factors`, `Projection.from_plane`), the `centroid_points` free function, and the `rpc.Proxy.function` dotted-resolution + lifecycle resolve by introspection on the cp312 distribution under the verified 342-entry `__all__` and the `rpc` subpackage — gated on the companion interpreter, never asserted as a SciPy-core member on cp315. The `rpc.Proxy` lifecycle is introspection-confirmed against the cp312 companion `compas/rpc/proxy.py`: `__init__` is EAGER (`_try_reconnect` to the running localhost server on `url='http://127.0.0.1'`/`port=1753`, then `start_server()` only when none answers, recording `_implicitely_started_server`), `start_server()` is blocking (`Popen` + `time.sleep(0.1)` ping loop up to `max_conn_attempts=100`, raising `RPCServerError`), `Proxy` is a sync context manager (`__enter__`/`__exit__`) whose `__exit__` runs `stop_server()` only when `_implicitely_started_server` (ownership-aware teardown), `function(dotted_name)` is pure dispatch over an already-running server, and `restart_server()` = `stop_server()`+`start_server()` is recovery, not the bring-up; `RPCServerError`/`RPCClientError` export from `compas.rpc.__all__` — a settled contract, not an open gated question.
