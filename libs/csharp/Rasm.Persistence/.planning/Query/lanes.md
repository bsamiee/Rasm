...
|  [02]   | HNSW + IVFFlat + diskann     | postgres-server | HNSW/IVFFlat metadata on schema rows; diskann DDL on `Store/server#SEARCH_PROVISIONING`                                                                                    | approximate routes; exact scan stays the baseline; diskann scales disk-backed ANN beyond RAM-resident HNSW and complements never replaces                                                                                                                                                                                |
...
|  [08]   | binary-vector metrics        | postgres-server | `VectorMetric.Order` projects through the catalogued `HammingDistance` / `JaccardDistance` `DbFunctions` over a `bit` column                                               | `VectorMetric.Hamming` / `.Jaccard` bit rows project bit distance via the EF translator, exact scan baseline; the `VectorMetric.Op` column carries the catalogued server operators (`HammingDistance`→`<~>` / `JaccardDistance`→`<%>`, `.api/api-pgvector-ef.md` rows [5]/[6]) for the `HybridRetrieve.Fuse` raw-CTE leg |
|  [09]   | BM25 overlay                 | postgres-server | `@@@ pdb.parse(...)` query-builder match, `pdb.score(<key_col>)` / `pdb.snippet` via `FromSql` / `SqlQuery`; BM25 index DDL on `Store/server#SEARCH_PROVISIONING`          | high-power FTS overlay beside the always-present native baseline; `@@@` RHS is always a `pdb.*` builder, score/snippet anchor on the `key_field`; `FullTextQuery.Bm25` selects it; native FTS answers when pg_search is not preloaded                                                                                    |
...
| [INDEX] | [ROW]                   | [SURFACE]                                                                                                               | [LAW]                                                                                                                                                                                                   |
| :-----: | :---------------------- | :---------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
|  [11]   | lakehouse catalog mount | `INSTALL ducklake; LOAD ducklake; ATTACH 'ducklake:postgres:…' AS lake (DATA_PATH 's3://…')` raw SQL on `DuckDBCommand` | metadata catalog in the `Store/server#TIMESCALE_PROVISIONING` PG tier, data files in the `Store/remote#OBJECT_RESIDENCE` blob lane; `LakehouseKind` renders the `ATTACH` clause per backend             |
...
- [BIT_VECTOR_COLUMNS]: the `binary_quantize(emb)::bit(N)` expression-index materialization that derives the `bit` column from a dense embedding, and the `halfvec`/`sparsevec`/`bit` column-type round-trip, confirmed against a live PG18 + pgvector server before the bit-metric index DDL pins. The distance-operator literals are settled — `.api/api-pgvector-ef.md` rows [5]/[6] catalogue both the `HammingDistance`/`JaccardDistance` `DbFunctions` member and the `<~>`/`<%>` `[SQL_OPERATOR]` column the `VectorMetric.Op` rows carry, so the translated `Order` projection and the `HybridRetrieve.Fuse` raw-CTE leg both read settled values with no placeholder.
