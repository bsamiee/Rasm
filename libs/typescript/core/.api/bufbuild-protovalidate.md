...
- peer: `@bufbuild/protobuf` (`DescMessage`/`MessageShape`/`MessageValidType`/`Registry`; `.api/bufbuild-protobuf.md`), its `reflect` `Path` for violation coordinates
...
- [VERDICT_SHAPE]: `valid` narrows the message to `MessageValidType<Desc>` — identical to `MessageShape<Desc>` unless `protoc-gen-es` ran with `valid_types`, which this branch does not — so the narrowing buys the TYPE nothing today and the fold reads `kind` alone.
...
|  [02]   | `validator.validate(schema, message)`             | verdict        | behind `isMessage(value, schema)`; decode AND encode of every family     |
|  [03]   | `violationsToProto(violations)`                   | wire           | `[Violations, ViolationsSchema]` — the `buf.validate.Violations` message |
|  [04]   | `violationToProto(violation)`                     | wire           | one `Violation` message; unmined — violations cross as `FieldViolation`  |
|  [05]   | `pathFromViolationProto(schema, proto, registry?)` | path          | reverse of the wire form; unmined                                       |
|  [06]   | `createStandardSchema(schema, validator?)`        | bridge         | Standard Schema adapter; unmined — the branch's admission is `effect`'s  |
...
[STACKING]:
- `@bufbuild/protobuf`(`.api/bufbuild-protobuf.md`): descriptors come from the generated `_pb.ts` consts; `Violation.field` is a `reflect` `Path` rendered through `pathToString` into the filter issue's coordinate.
