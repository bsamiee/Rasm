...
- Owner: `InterchangeFormat` the `[SmartEnum<string>]` rows carrying media-type, extension set, `CanImport`/`CanExport` capability, codec-owner discriminant, `TessellationRequiresCompanion`, the `Frame` ingest basis-change column, the `StepProtocol` discriminant, and the `Serialization` `Option<FormatIfcSerialization>` column (`Some` exactly on the three GeometryGym rows — the ONE `InterchangeFormat`→IFC-serialization correspondence `export#EXPORT_RAIL` `ExportIfc` and `wire#WIRE_PROJECTION` `Seal` both read, retiring their duplicated `SerializationOf` ladders); `InterchangeCodec` the codec-owner vocabulary discriminating the managed package or companion that reads and writes the row; `KhrExtension` the glTF extension axis carrying each in-box `KHR_*`/`EXT_*`/vendor row on its `KhrSlot`/`KhrEncoder` discriminant; `BasisChange` the per-importer signed-permutation basis carrying positions and normals alike onto the canonical kernel frame; `FrameNormalization` the static reconciliation surface coercing every imported coordinate into the canonical kernel frame. The three `[SmartEnum<string>]` keys carry the case-insensitive `[KeyMemberComparer]`/`[KeyMemberEqualityComparer]` the codec/format/extension lookups dispatch on, so a `.glb`-vs-`.GLB` key match resolves one row.
...
- Boundary: format selection is row data resolved through `Detect`, never a call-site extension switch — a parallel `GltfImporter`/`IfcImporter`/`GltfExporter` family is the deleted form. `CanImport`/`CanExport` are capability columns the import and export folds read so a write-only or read-only direction faults at the boundary rather than mid-codec, and every `true` column NAMES ITS REALIZING ARM: mesh/scene artifact emit is the `export#EXPORT_RAIL` `BimExport.Export` total codec `Switch`, IFC emit is `ExportIfc`→`Projection/egress#IFC_EGRESS` `SemanticProjector.Emit`, geospatial-vector read/write is the `Semantics/geospatial#VECTOR_INGEST` `GeoVector.Read`/`GeoVector.Write` fold, raster read is `#RASTER_INGEST` — a capability flag with no realizing arm on any rail is the rejected PHANTOM, so `las` and `geotiff` carry `CanExport=false` (`Themis.Las` ships `LasWriter` and GDAL ships raster `CreateCopy`, but no Bim rail composes either yet — the flag flips WITH the arm, never before it) and `usdz` carries `CanExport=false` (the catalogued `UniversalSceneDescription` binding ships `UsdStage.Export` over `.usd`/`.usda`/`.usdc` and NO `.usdz` packaging member, so a `CreateNew("*.usdz")` author is the rejected phantom; `.usdz` READS through the package layer). Every IFC2x3-IFC4.x row carries `CanImport=true`/`CanExport=true` because GeometryGym is symmetric over STEP/XML/JSON and every glTF row symmetric over SharpGLTF, and the emergent `ifc5`/`.ifcx` ECS row is the live `CataloguePending` candidate (`CanImport=false`/`CanExport=false` on `InterchangeCodec.Ifc5Pending`) because no IFC5/IFCX toolkit is admitted and GeometryGym reads IFC2x3-IFC4.x only — the working exemplar of the candidate-row promotion discipline, never a fake `codec: GeometryGym` capability claim. The `TessellationRequiresCompanion` column is `true` exactly on the IFC/STEP/native rows because GeometryGym carries no tessellation kernel — a managed IFC geometry evaluation is the rejected form; the codec owner is the `InterchangeCodec` discriminant, not a delegate field, because the codec capsules carry no runtime state the row owns.
  ...
  - [DOTBIM_CODEC]: the `dotbim` codec names `dotbim` (pure-managed netstandard2.0, STJ wire, zero native payload) as the `.bim` mesh+metadata interchange — the ONLY admitted codec whose WIRE expresses instancing: `File` owns a shared `Mesh` pool (flat `Coordinates`/`Indices`) plus placed `Element` instances referencing a pool `MeshId` with a rigid `Vector`+quaternion `Rotation` placement, a validated `Guid` identity, a `Type` string, an RGBA `Color`, and a `string→string` `Info` bag — so N repeated objects serialize as N placements over ONE mesh, never N copied vertex buffers. The row is bidirectional (`CanImport=true`/`CanExport=true`): the `import#IMPORT_RAIL` arm resolves each `Element.MeshId` against the pool and bakes `Vector`/`Rotation` into the placement transform; the `export#EXPORT_RAIL` arm pools distinct geometry by content key, decomposes each placement onto `Vector`/`Rotation`, stamps the seam GlobalId onto `Element.Guid` and the classification onto `Type`, and round-trips IFC tags through the `Info` bag. Both `File.Read`/`File.Save` are PATH-BOUND (`.bim`-extension-enforced, no stream overload), so the byte arms cross a temp path exactly as the `usd-stage` codec does; the typed setters validate at assignment (`Color` 0..255, `MeshId >= 0`, malformed `Guid`), so a structurally-invalid model faults at build, before `Save`.
  - [POINT_CLOUD_AND_GEOSPATIAL]: the `las` row is the `point-cloud` codec naming `Themis.Las` (the managed ASPRS LAS reader the `reconstruct#RECONSTRUCTION` scan-to-BIM ingest front decodes through) with the `Unofficial.laszip.netstandard` pure-managed LASzip decompression front composing in for the compressed `.laz` extension, so the codec reads both uncompressed `.las` and arithmetic-coded `.laz` over one ingest path — the row is import-only (`CanExport=false`) because no rail composes the catalogued `Themis.Las` `LasWriter` yet (a point-cloud egress is `reconstruct#RECONSTRUCTION` growth over its point carrier, never a mesh-rail LAS dump); the `shp`/`gpkg`/`geojson`/`cityjson`/`fgb`/`geoparquet`/`kml`/`kmz`/`mvt` rows are the `geospatial-vector` codec the `Semantics/geospatial#VECTOR_INGEST` owner decodes and `GeoVector.Write` re-emits (managed `NetTopologySuite.IO.Esri.Shapefile`/`bertt.CityJSON`/`FlatGeobuf`/`GISBlox.IO.GeoParquet` plus the `MaxRev.Gdal.Core` OGR universal driver path whose LIBKML driver carries the `.kml`/`.kmz` pair today, `SharpKml.Core` the admitted managed upgrade — `KmlFile.Load`/`Save` over the typed `Dom` tree, `KmzFile.Open`/`Create`/`GetDefaultKmlFile` over the archive — and `NetTopologySuite.IO.VectorTiles.Mapbox` the MVT spec-2.1 protobuf pair `MapboxTileReader`/`MapboxTileWriter` over the `VectorTile`/`VectorTileTree` model), the `cityjson` row import-only (`CanExport=false`) because the planar `GeoFeature` egress cannot re-emit the 3D city hierarchy while the `fgb`/`geoparquet` managed codecs round-trip, and the `geotiff` row the `geospatial-raster` codec `Semantics/geospatial#RASTER_INGEST` reads through `MaxRev.Gdal.Core` — import-only (`CanExport=false`) until a raster egress arm composes the GDAL `CreateCopy` write on that owner.
  - [USD_PEER_AND_MEDIA_TYPES]: the `usd`/`usdz` rows carry the `usd-stage` OpenUSD scene-graph codec (`UniversalSceneDescription`, Core Spec 1.0) as a peer coexistence axis — USD carries the scene through `UsdStage` layer composition and the `UsdGeomMesh`/`UsdGeomXformable`/`UsdShadeMaterialBindingAPI` typed schemas, IFC carries the BIM semantics, so the USD codec is a scene-graph peer, never a BIM-semantic replacement; media types are the IANA `model/gltf-binary`, `model/gltf+json`, `application/step`, and the buildingSMART `application/x-step`/`application/ifc+xml`/`application/ifc+json` values, plus the `model/vnd.usd`/`model/vnd.usdz+zip` USD, the `application/vnd.autodesk.fbx`/`model/vnd.collada+xml` scene-exchange, the `application/vnd.dotbim+json` vendor-tree `.bim`, the `application/vnd.las` point-cloud, and the `application/vnd.shp`/`application/geopackage+sqlite3`/`application/geo+json`/`application/city+json`/`application/vnd.google-earth.kml+xml`/`application/vnd.google-earth.kmz`/`application/vnd.mapbox-vector-tile`/`image/tiff;application=geotiff` geospatial media types. The `usd`/`usdz` `Frame` column stays the Y-up DEFAULT a metadata-less stage falls to — USD's `upAxis` is PER-STAGE metadata (`UsdGeom.UsdGeomGetStageUpAxis`, decompile-verified), so the `import#IMPORT_RAIL` `Usd` arm selects the basis per stage and a Z-up CAD/BIM export lands canonical with NO row edit.
  - [ENERGY_MODEL_ROWS]: the `energy-model` codec names the three-package energy stack (`HoneybeeSchema` HBJSON, `DragonflySchema` DFJSON, `NREL.OpenStudio.macOS-arm64` OSM/gbXML/IDF) whose realizing arms live on `Exchange/energy` — `CanImport` is the `EnergyProjector` raise (`energy#ENERGY_PROJECTOR`), the hbjson/dfjson `CanExport` is the `EnergyDerive` BIM-to-BEM lower (`energy#MODEL_DERIVE`), and the osm/gbxml/idf emit rides the `EnergyTranslate` matrix over an admitted OSM-family SOURCE, never the graph, so those rows carry `CanExport=false` per the realizing-arm law; the rows never enter the `BimIo` mesh/scene fold (the geospatial-row precedent — a format row's arm may live on any rail), `TessellationRequiresCompanion=false` because an energy model carries no BRep, and the frames are Identity (ladybug geometry, OSM, and gbXML are Z-up right-handed).
...
public sealed partial class InterchangeFormat {
    ...
    // Energy-model rows (Exchange/energy the realizing owner): HBJSON/DFJSON raise+lower, OSM/gbXML/IDF raise-only
...
- [SCENE_EXCHANGE_AND_GEOSPATIAL]: the `scene-exchange` codec composes `AssimpNetter` (`.api/api-assimpnetter`, the `AssimpContext`/`PostProcessSteps`/`Scene`/`Mesh`/`Material` surface, an `osx-arm64` `libassimp.dylib`) owning FBX+Collada import/export and the standalone 3MF read leg that retires the BCL `ThreeMfReader` — the row keys `fbx`/`collada`/`3mf` are the verified `ExportFormatDescription.FormatId` values `ExportToBlob(scene, exportFormatId)` dispatches on, guarded by `IsExportFormatSupported`; the `dotbim` codec composes `dotbim` (`.api/api-dotbim`, `File.Read(path)`/`File.Save(path, format)` STJ round-trip over the `Mesh` pool + placed `Element` graph, setter-validated `Guid`/`Color`/`MeshId`) as the one instancing-expressing wire; the `ply-net` codec composes `Ply.Net` (`.api/api-ply-net`, `PlyParser.Parse`/`ParseHeader` over the `Header`/`Dataset`/`PropertyData` record graph) retiring the BCL `PlyReader`; the `point-cloud` codec composes `Themis.Las` (`.api/api-themis-las`, `LasReader`/`LasPoint` with `MathNet.Numerics.LinearAlgebra.Vector<double>` positions; the `LasWriter` egress catalogued but composed by NO rail yet, so the row stays import-only) plus the `Unofficial.laszip.netstandard` LASzip decompression front for the compressed `.laz` extension, the `reconstruct#RECONSTRUCTION` ingest front driving both; the `geospatial-vector` codec composes `NetTopologySuite.IO.Esri.Shapefile`/`bertt.CityJSON`/`FlatGeobuf`/`GISBlox.IO.GeoParquet`/`MaxRev.Gdal.Core` OGR plus the `.kml`/`.kmz` pair (the OGR LIBKML universal driver live today; `SharpKml.Core` `.api/api-sharpkml` `KmlFile.Load`/`Save` + `KmzFile.Open`/`Create`/`GetDefaultKmlFile` the admitted managed upgrade) and the `.mvt`/`.pbf` pair (`NetTopologySuite.IO.VectorTiles.Mapbox` `.api/api-nts-vectortiles-mapbox` `MapboxTileReader`/`MapboxTileWriter` over `VectorTile`/`VectorTileTree`), and the `geospatial-raster` codec composes `MaxRev.Gdal.Core` raster — both decode through the `Semantics/geospatial#VECTOR_INGEST`/`#RASTER_INGEST` owner producing `GeoFeature` rows over the `NetTopologySuite` planar algebra, so a codec admit reaching this axis is one `InterchangeCodec` row promotion and never a parallel importer family.
