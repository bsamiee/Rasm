...
- [E57_CONDITIONED_INTAKE]: the `e.read_scan(index, transform=True) -> dict` conditioned intake (`pye57.md` lifecycle ENTRYPOINT [04]) is the one polymorphic read the `e57` arm folds — it auto-detects the coordinate system via `ScanHeader.get_coordinate_system`, projects spherical input through `convert_spherical_to_cartesian`, masks the invalid-state column, and applies the per-scan pose so the dict always exits as global-frame `cartesianX`/`cartesianY`/`cartesianZ` (the `IMPLEMENTATION_LAW` read axis), never the unconditioned `read_scan_raw` escape (ENTRYPOINT [05]) that returns the fields as-stored with no mask and no transform and would force a hand-rolled `to_global(points, header.rotation, header.translation)` re-plumb the conditioned read already owns. The `e.scan_count` multi-scan count (query ENTRYPOINT [02]) and the `with E57(path, mode='r')` context-manager open (lifecycle ENTRYPOINT [01]) confirm against the `pye57` catalogue on the cp313 companion. The open item is whether a sensor-frame source omits the pose so `transform=True` is a no-op (the `ScanHeader.rotation`/`translation` degrade to identity/zero when `has_pose()` is false, lifecycle ENTRYPOINT [09] note), a per-source detail the conditioned read absorbs internally, not a catalogue dependency.

## [04]-[UPSTREAM]

- [COMPANION_BAND]: `pdal`, `pye57`, and `open3d` ride the `python_version<'3.15'` companion band the branch manifest owns — `pdal` is sdist-only (native `libpdal` C++ dependency, no cp315 wheel), `pye57` is the `libe57` wrapper with cp310-cp314 per-interpreter ABI wheels and no cp315 load path, and `open3d` caps at the cp312 compiled core — so all three import function-local under `# noqa: PLC0415` at boundary scope inside `_read_e57` and `_filter_graph`, never module-top, per the manifest import policy; the `import pdal` side-effect (`inject_pdal_drivers()`) is what binds the typed `Filter.<name>` factories, so the boundary import must complete before any `_FILTER` factory call. `laspy` (cp315-clean) is never imported here because the LAS decode rides the data-branch bridge. The `open3d` row carries the manifest-admission delta shared with `scan/reconstruction.md`/`registration.md`/`deviation.md` (verified absent from the manifest despite those pages already consuming it): one `open3d` row at `python_version<'3.15'` admits on the branch manifest, a pre-existing delta authored pages already need, not introduced by this page.
- [OFFLOAD_LANE]: the `_filter_graph` SMRF/voxel kernel is one of the heavy geometry CPU kernels the runtime `execution/lanes.md#LANES` `LanePolicy.offload` per-subinterpreter variant absorbs through `anyio.to_interpreter.run_sync` under one `CapacityLimiter` — the no-pickle PEP-734 hop the lanes owner confirms (`lanes.md` ENTRYPOINTS `LanePolicy.offload`, the async `offload[T](kernel, *args) -> RuntimeRail[T]`), degrading to `anyio.to_thread.run_sync` only where a cp315 build ships no runnable `concurrent.interpreters`, NEVER a `to_process` pickle round-trip the lanes owner rejects as the process-pool serialization tax. The optional `lane: LanePolicy | None` field on `ScanIngestion` is the imported seam, the hand-off a uniform one `offload(kernel, *args)` growth on the Growth bullet shared with `daemon.md#DAEMON`, `registration.md#REGISTRATION`, `reconstruction.md#RECONSTRUCTION`, and `repair.md#MESH`, sequence-after the runtime lane, never a second concurrency surface minted in geometry.
