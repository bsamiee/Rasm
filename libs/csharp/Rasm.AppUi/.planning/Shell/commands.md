...
- [04]-[EXECUTION_RECEIPTS]: Total outcome rail; receipts sealed through the sink message envelope.
...
- Boundary: Wire fields follow the suite camelCase law; duration, correlation, gesture, level, payload, and outcome keep owner spellings.
- Boundary: `many.ids` is a JSON string array and `fields.values` is an opaque JSON object owned by the addressed form schema.
- Boundary: The suite converter carries C# `Seq` and `HashMap` without changing their JSON shapes.
- Boundary: `targets` carries palette-kind keys and `arguments` carries the argument-schema key or null.
- Boundary: Palette frames and binding-editor rows stay host-local; remote callers cross through `CommandInvocationWire`.
- Boundary: `CommandPayloadWire` is the five-arm argument union and `CommandInvocationWire` is exactly `{key,payload}`.
- Boundary: TypeScript interchange decodes the same invocation and adds no command-side message envelope.
- Boundary: `CommandGateWire` carries the row gate; AppHost `CommandAvailabilityWire` remains a separate health snapshot.
