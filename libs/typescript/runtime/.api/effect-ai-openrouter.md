...
API key (or `effect/Config`), threads through the `net/client` `HttpClient`, and the whole 348-class OpenRouter
...
`model(id, config?)` is the row `ai/model.ts` selects; `layer`/`make` are the direct forms. `Config` is a
