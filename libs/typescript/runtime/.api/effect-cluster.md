...
- `@effect/workflow` (`work/.api/effect-workflow.md`): `ClusterWorkflowEngine.layer` satisfies the `WorkflowEngine` Tag over `Sharding` + `MessageStorage` — durable workflows run sharded on this runtime. `work/flow` defines the workflow; `work/entity` provides the engine.
