...
- transport/wire: `CrdtOpDecode` caches one `msgpack.Decoder` whose `ext_hook` reconstructs the binary lattice clock from a `msgspec.Ext` payload; a `Raw`-typed message envelope field defers the inner op decode until the tag routes it, and `Raw.copy()` detaches the buffer view so the outer input releases.
