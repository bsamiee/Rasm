...
[LOCAL_ADMISSION]:
- `CompressionPolicy.Lz4Fast`/`Lz4High` (`Version/snapshots#COMPRESSION_HASHING`) are `LZ4Pickler.Pickle(payload.Span, LZ4Level.L00_FAST)` / `Pickle(payload.Span, LZ4Level.L09_HC)` with `Unpickle(framed.Span)` — the self-describing frame carries its own decoded size so the snapshot header stores only the `CompressionId`, never a sidecar length.
