...
- offload axis: every `op`/`simplify`/`stroke`/`OpBuilder.resolve` is synchronous native CPU work, riding the `WORKER_BAND`-bounded `to_process` boundary the `resvg`/`svgelements` ops cross, never inline on the event loop.
...
- Each op faults its provider raise into the page's `RegionFault` `@tagged_union` and returns through the `expression` `Result[RegionResult, RegionFault]` the result speaks (`Ok`/`Error`, `bind`/`map`, `Block`-collected over a batch) — a typed `RegionResult.document`/`contours`/`extent` case carrying the result `d`/`bounds`/`area`, never an erased `bytes` a consumer re-parses.
