...
- [1]-[PHOTOMETRIC]: the `PhotometricQuantity` band, the `Photometric` admission fold composing Compute `QuantityFamily.Illuminance`, the `EmissionSpectrum` blackbody/SPD/constant family, the `PhotometricPolicy` light→emission map, and the `EmissionInput` payload.
