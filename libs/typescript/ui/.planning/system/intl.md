...
Intl localizes the folder with zero i18n package: one ambient locale spine over the kernel `Shape.Refined.Locale` brand, one per-locale native-`Intl` instance cache behind every formatter this page constructs itself, the `Format` option-row vocabulary every presentation reads, and the `Message` authority decoding catalogs as app data. Those owners hold every constructor, option object, clock read, and locale resolution, so a call site never touches `Intl.*`, domain flow never reads a wall clock, and no ICU engine or translation runtime enters the folder. Module: `ui/src/system/intl.ts`.
