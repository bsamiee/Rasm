...
- Events: a board mutation mints the `Exchange/events#EVENTS` `BimEvent.IssueMutated` row at the composing rail's edge — the topic `Guid`, one `BimIssueMutation` row (`topic-opened`/`topic-revised`/`comment-added`/`viewpoint-added`/`status-advanced`), the optional comment `Guid`, and the viewpoint-anchored `GlobalId` set — subject the topic `Guid`; envelope seal and transport are the events owner's, never a second emit path here.
...
// spell; the server-side events change-feed is the `csharp:Rasm.Persistence/Version/ledger` CDE-sync polling
...
// (client/retry/auth) stays the `csharp:Compute/Runtime/transport#TRANSPORT_AXIS` owner's — a transport minted here
