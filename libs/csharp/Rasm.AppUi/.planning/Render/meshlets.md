...
- [VIEWPORT_GPU]: the `SKMesh`/mesh-shader emit path against the leased `GRContext`, the per-backend bindless descriptor-table spelling (Metal argument buffers, Vulkan descriptor indexing), and the GPU upload of a resident tile to a bindless slot resolve under the shared-context lease — the meshlet cluster build, the screen-space-error cull, the residency plan, the prefetch fold, and the instance buffer are settled as the CPU/2D-fallback wireframe; the GPU mesh-shader draw and the bindless upload are the unverified surface gated on the live host-owned GPU context the `Render/pipeline` lease binds.
