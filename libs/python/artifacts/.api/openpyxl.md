...
|  [11]   | `cell.read_only.ReadOnlyCell`           | read-stream cell  | value-only lazy cell yielded under `read_only=True` (no style mutation) |
|  [12]   | `cell.cell.WriteOnlyCell`               | write-stream cell | styled cell appended to a `WriteOnlyWorksheet` (the only streamed-style carrier) |
|  [13]   | `packaging.core.DocumentProperties`     | core metadata     | `Workbook.properties` — title/creator/subject/keywords/category/created/modified/lastPrinted/revision/version descriptive-metadata seam |
|  [14]   | `workbook.protection.WorkbookProtection`| workbook lock     | `Workbook.security` — structure/window lock + workbook password hash |
|  [15]   | `worksheet.protection.SheetProtection`  | sheet lock        | `Worksheet.protection` — per-sheet lock with selection/format/insert allowances |
...
|  [02]   | `styles.differential.DifferentialStyle` | delta style  | the partial-style overlay a `formatting.rule.Rule` applies on match (font/fill/border subset) |
|  [03]   | `chart`                           | chart module       | `BarChart`/`LineChart`/`ScatterChart`/`PieChart`/`AreaChart`/`RadarChart`/`BubbleChart`/`SurfaceChart`/`StockChart` (+ `BarChart3D`/`LineChart3D`/`PieChart3D`/`AreaChart3D`/`SurfaceChart3D`), `Reference`, `Series` |
|  [04]   | `formatting.rule`                 | conditional module | `CellIsRule`/`ColorScaleRule`/`DataBarRule`/`IconSetRule`/`FormulaRule`/`Rule` (each lowers to a `DifferentialStyle`) |
|  [05]   | `formula.translate.Translator`    | formula engine     | shift cell references when copying a formula across a range; static `translate_range` for a whole range string |
|  [06]   | `formula.tokenizer.Tokenizer`     | formula lexer      | tokenize a formula string into `Token` operands/operators/functions for inspection |
|  [07]   | `worksheet.formula.ArrayFormula` / `DataTableFormula`  | array/table formula      | `ArrayFormula(ref, text=None)` dynamic/spilled array; `DataTableFormula(ref, ca, dt2D, dtr, r1, r2, del1, del2)` one/two-variable what-if data table bound to a `ref` |
|  [08]   | `worksheet.table.TableStyleInfo`  | table style        | banded-row/column table styling applied to a `Table`             |
|  [09]   | `worksheet.filters.AutoFilter`    | filter axis        | `Worksheet.auto_filter` — `add_filter_column`/`add_sort_condition` over a header `ref` |
|  [10]   | `worksheet.page.PrintPageSetup` / `PageMargins` | page setup | `Worksheet.page_setup`/`page_margins` — orientation/paper/fit-to-pages/scale + print-margin geometry for the pub/print + drawing-sheet plane |
|  [11]   | `utils`                           | coordinate utils   | `get_column_letter`/`column_index_from_string`/`range_boundaries`/`coordinate_to_tuple`/`absolute_coordinate`/`quote_sheetname`/`rows_from_range`/`cols_from_range` |
...
|  [11]   | `Workbook.defined_names`   | mapping property (`DefinedNameDict`)                                                                      | workbook-scoped name -> `DefinedName` resolution      |
|  [12]   | `Workbook.properties`      | property -> `DocumentProperties`                                                                          | core descriptive metadata (title/creator/subject/keywords/created/...) — the `exchange/metadata` seam |
|  [13]   | `Workbook.security`        | property -> `WorkbookProtection` (`Workbook.security = WorkbookProtection(workbookPassword=..., lockStructure=True)`) | workbook structure/window lock — the `egress#PROTECT` Office companion |
|  [14]   | `Workbook.sheetnames` / `Workbook[title]` | property -> `list[str]` / `__getitem__(title) -> Worksheet`                                | polymorphic sheet lookup by title (the single name-access surface, never a `get_sheet_by_name` verb) |
|  [15]   | `Workbook.iso_dates` / `epoch` / `template` | properties                                                                              | ISO-8601 date serialization toggle, `1900`/`1904` date epoch, template (`.xltx`) flag |
|  [16]   | `Workbook.save`            | `save(filename)`                                                                                          | serialize the workbook to a path or stream            |
|  [17]   | `Workbook.close`           | `close()`                                                                                                 | release `read_only`/`write_only` file handles         |
...
|  [05]   | `Worksheet.merge_cells` / `unmerge_cells` | `merge_cells(range_string=None, start_row=None, start_column=None, end_row=None, end_column=None)` / `unmerge_cells(...)` | merge / split a region    |
...
|  [08]   | `Worksheet.column_dimensions` / `row_dimensions` | `column_dimensions['A'].width = ...` / `row_dimensions[1].height = ...` (`DimensionHolder` mappings) | per-column width / per-row height + hidden/outline level |
|  [09]   | `Worksheet.add_chart`                  | `add_chart(chart, anchor=None)`                                                       | embed a chart at an anchor                |
|  [10]   | `Worksheet.add_image`                  | `add_image(img, anchor=None)`                                                         | embed an image                            |
|  [11]   | `Worksheet.add_table`                  | `add_table(table)`                                                                    | register a structured `Table` object      |
|  [12]   | `Worksheet.add_data_validation`        | `add_data_validation(data_validation)`                                                | attach a `DataValidation` rule            |
|  [13]   | `Worksheet.add_pivot`                  | `add_pivot(pivot)`                                                                    | attach a pivot table definition           |
|  [14]   | `Worksheet.freeze_panes`               | property (coordinate or cell, e.g. `"A2"`)                                            | freeze rows/columns above-left of a cell  |
|  [15]   | `Worksheet.auto_filter`                | property -> `AutoFilter`; `auto_filter.ref = "A1:D99"`, `.add_filter_column(col, vals)`, `.add_sort_condition(ref)` | filterable + sortable header range (an `AutoFilter` object, never an `.add()` method) |
|  [16]   | `Worksheet.conditional_formatting.add` | `add(range_string, cfRule)` (the `conditional_formatting` instance is a `ConditionalFormattingList`) | attach a conditional-format rule          |
|  [17]   | `Worksheet.protection`                 | property -> `SheetProtection`; `protection.sheet = True`, `protection.password = ...` | per-sheet lock the workbook `egress#PROTECT` seal complements |
...
|  [09]   | `utils.get_column_letter`       | `get_column_letter(idx)` / `column_index_from_string(letter)` / `range_boundaries(range_string)` / `coordinate_to_tuple(coord)` / `absolute_coordinate(coord)` / `quote_sheetname(name)` | A1 <-> tuple coordinate conversion at the boundary |
...
| [INDEX] | [SURFACE]                       | [CALL_SHAPE]                                                                            | [CAPABILITY]                                       |
| :-----: | :------------------------------ | :-------------------------------------------------------------------------------------- | :------------------------------------------------- |
|  [01]   | `Worksheet.page_setup`          | `page_setup.orientation='landscape'`, `.paperSize=PAPERSIZE_A4`, `.fitToWidth=1`, `.fitToHeight=0`, `.scale=100` (`sheet_properties.pageSetUpPr.fitToPage=True` enables fit) | orientation / paper / fit-to-pages / scale for plotted output |
|  [02]   | `Worksheet.page_margins`        | `page_margins = PageMargins(left=0.7, right=0.7, top=0.75, bottom=0.75, header=0.3, footer=0.3)` | print-margin geometry in inches                    |
|  [03]   | `Worksheet.print_area`          | `print_area = 'A1:D99'`                                                                  | bound the printable range                          |
|  [04]   | `Worksheet.print_title_rows` / `print_title_cols` | `print_title_rows = '1:1'` / `print_title_cols = 'A:A'`                | repeat title rows/cols on every printed page (ISO 5457 banner) |
|  [05]   | `Worksheet.sheet_properties.tabColor` | `sheet_properties.tabColor = 'FF0000'`                                             | sheet-tab color                                    |
|  [06]   | `Worksheet.oddHeader` / `oddFooter` | `oddHeader.center.text = '&P of &N'`, `.left`/`.right` `HeaderFooterItem`            | header/footer with `&P`/`&N`/`&D` field codes      |
|  [07]   | `Workbook.properties`           | `properties.title=`, `.creator=`, `.subject=`, `.keywords=`, `.category=`, `.created=`, `.lastPrinted=` | core OOXML descriptive metadata (the `exchange/metadata` seam) |
