...
[BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly record struct DiscreteCalculus(SparseMatrix D0, SparseMatrix D1, Arr<double> Star0, Arr<double> Star1, Arr<double> Star2) {
    public bool IsValid => D0.IsValid && D1.IsValid && Star0.Count == D0.Cols.Value && Star1.Count == D0.Rows.Value && Star2.Count == D1.Rows.Value && Star0.ForAll(static v => RhinoMath.IsValidDouble(x: v) && v > 0.0) && Star1.ForAll(static v => RhinoMath.IsValidDouble(x: v) && v >= 0.0) && Star2.ForAll(static v => RhinoMath.IsValidDouble(x: v) && v > 0.0);
}

[BoundaryAdapter, StructLayout(LayoutKind.Auto)]
public readonly record struct SpectralDescriptorReceipt(SpectralFilter Filter, int VertexCount, int EigenpairCount, int SourceCount, bool ComparisonReady, bool Pairwise, bool EnergyNormalized, bool BandwidthNormalized);
    ...
    internal Fin<SpectralDescriptor> ApplyDetailed(SpectralBasis basis, Option<Seq<int>> sources, Op key) =>
        Optional(this).ToFin(key.InvalidInput())
            .Bind(filter => guard(basis.IsValid, key.InvalidInput())
                .Bind(_ => SpectralCore.EvaluateFilteredDetailed(basis: basis, sources: sources, filter: filter, key: key)));
...
internal static class SpectralCore {
    ...
    internal static Fin<SpectralDescriptor> EvaluateFilteredDetailed(SpectralBasis basis, Option<Seq<int>> sources, SpectralFilter filter, Op key) {
        ...
        return result.All(RhinoMath.IsValidDouble) ? Fin.Succ(new SpectralDescriptor(Values: new Arr<double>(result), Receipt: new SpectralDescriptorReceipt(Filter: filter, VertexCount: n, EigenpairCount: basis.Eigenvalues.Count, SourceCount: sourceSet.Length, ComparisonReady: false, Pairwise: isPairwise, EnergyNormalized: false, BandwidthNormalized: false))) : Fin.Fail<SpectralDescriptor>(key.InvalidResult());
    }
    // --- [CROUZEIX_RAVIART] -----------------------------------------------------------------
    internal static Fin<SparseMatrix> BuildCrouzeixRaviartHeatSystem(MeshKernel.IntrinsicMesh mesh, double time, Op key) {
        ...
        foreach (int f in mesh.LiveFaceIndices()) {
            if (IntrinsicTriangleOf(imesh: mesh, faceIdx: f) is not { } face) continue;
    ...
    private static Fin<DiscreteCalculus> AssembleDecOperators(MeshKernel.IntrinsicMesh imesh, Arr<double> mass, Op key) {
        ...
        for (int row = 0; row < faceCount; row++) {
            if (IntrinsicTriangleOf(imesh: imesh, faceIdx: liveFaces[row]) is not { } face) continue;
    ...
    private static IntrinsicTriangle? IntrinsicTriangleOf(MeshKernel.IntrinsicMesh imesh, int faceIdx) {
        ...
        return slot is null || edges.Length != 3 || edges[0] < 0 || edges[1] < 0 || edges[2] < 0 || area < DegenerateTriangleArea
            ? null
    ...
    private static bool BoundaryCompositionIsZero(List<(int Row, int Col, double Value)> d0, List<(int Row, int Col, double Value)> d1) {
        ...
        return d1
            ...
            .All(static group => Math.Abs(value: group.Sum(static e => e.Value)) <= RhinoMath.SqrtEpsilon);
    }
}
