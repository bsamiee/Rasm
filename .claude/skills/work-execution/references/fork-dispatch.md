...
The style fits a plan with the steps numbered per system, the exact change per step written down, and the files of each system disjoint from the others. One file, one system that fits the main context whole, or steps that consume each other's results across systems run under the main agent directly, because the orchestrator layer then adds a relay and a review with nothing to relay or review.

## [02]-[DISPATCH]

The main agent dispatches every orchestrator in one message, as fresh Fable `general-purpose` agents, and the systems run at the same time from the same starting state. Before the first dispatch it reads the plan whole, maps the files in scope by listing them, reads none of them, and writes the memory of the plan's intent and the dispatch shape.

Each orchestrator brief holds, in order:
1. The edit rule in its first lines, read whole before the first write, one scoped change, read the result, repeated in every brief it sends
...
5. The standards every brief carries, and the definition of each file kind the work produces
...
Forks carry the orchestrator's context, and their briefs name the step and the entry and repeat nothing the context holds. When the fork type is unavailable, a fresh `general-purpose` agent takes the step with the entry text and the standards pasted in.
...
names a configuration value, and against the tool's documentation or help where it is about a tool. Correct only what
you can justify: a fact lost between the old file and the new, a fact that is false, a structure that hides a rule,
...
Orchestrators message `main` alone, and the main agent relays a finding about one system to the orchestrator that holds it. Findings that name a file outside every system (a repository manifest, a setting outside the plan) are the main agent's: it reads the file, makes the change, proves it by a run, and tells the orchestrator what changed, and the orchestrator's next brief carries the changed state. The main agent acts on each message in the turn it arrives and holds nothing for the return.

## [06]-[CLOSE]

When every orchestrator has returned, the main agent closes the run:
1. Fork one review agent that holds the plan and the intent, reads every touched file whole, and lists findings as file, line, finding, correction
...
The review lists a missed or partial step, a dropped or false fact, a paraphrased criterion, prose that assumes the session, and a pattern spread across files. The review runs as a fork because the intent it checks against sits in the main context, and the correction agents start fresh because a fork of the reviewer carries its reading along with its finding.
