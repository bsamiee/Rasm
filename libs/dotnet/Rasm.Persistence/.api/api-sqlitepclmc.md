...
[STACKING]:
- `Microsoft.Data.Sqlite`(`libs/dotnet/.api/api-sqlite.md`): `SqliteConnection.Handle` (`SQLitePCL.sqlite3`) is the one bridge every keying call crosses, and its `Password=` builder key opens an inspected foreign store for a single ephemeral read.
