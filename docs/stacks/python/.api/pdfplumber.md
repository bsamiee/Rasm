...
|  [21]   | `Page.to_dict` / `to_csv` / `to_json` | `to_csv(stream=None, object_types=None, precision=None, include_attrs=None, exclude_attrs=None)` -> `str` / `to_json(stream=None, object_types=None, include_attrs=None, exclude_attrs=None, precision=None, indent=None)` -> `str` / `to_dict(object_types=None)` -> `Dict`                                                                                                                                                | flatten parsed objects to tabular/JSON/dict export |
