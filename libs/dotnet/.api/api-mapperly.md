...
- `LanguageExt`(`.api/api-languageext.md`): a generated mapping method returns the bare target and throws or defaults per `ThrowOnMappingNullMismatch`, so `Try.lift` admits the call at the boundary and the carrier stays outside the generated body with the original exception preserved.
...
- `Rasm.Compute` (`Runtime/wire`, `Runtime/progress`, `Analysis/lifecycle`): `SplatMapper`, `FaultDetailMapper`, `ProgressWireMap`, and `Ec3Wire`+`EpdCodec` are the boundary-mapper catalog; no transaction mirror remains.
- `Rasm.AppUi` (`Shell/controls#TS_PROJECTION`, `Diagnostics/evidence`, `Render/viewpoint`): `ControlMap` projects the closed `ControlIntent` union through its generated total `Switch`; `EvidenceMap` projects `AppUiFact` cases; `ViewpointMap` transcribes the BCF camera and markup correspondences. `PlanMap`, `OpLogMap`, `ExportCardMap`, `GalleryIntake`/`DiffFolds`, `ProfileMap`/`EvidencePlan`, and `TableColumnMap` complete the branch mapper catalog.
- `Rasm.Grasshopper` (planned boundary-mapper catalog): `CanvasMap` (host canvas state, 9 `[MapProperty]` + 5 `[MapPropertyFromSource]`, pulse, pick result), `InputMap` (`ResponseMouseArgs`→kernel `PointerFact`, moment, drag facts), `SolutionMap` (`Solution`→`RunPulse`, `SolutionRecord`→`SolutionAudit`), `TrimMap` (existing-target pin-trim writes, one pair per parameter type), `ObjectMap` (existing-target native-object property writes with perceptual-colour source readers), `NativeMap` (`NSEvent`→`NativeInput` ABI projection), `CaptureMap` (ScreenCaptureKit survey facts) — one `[assembly: MapperDefaults(EnabledConversions = All & ~ExplicitCast)]`, `PrivateAssets` all, and every host demand that defeats generation (ordering-sensitive, method-call, Option-iter, reconciling writes) keeps a hand arm with the demand named.
- `Rasm.Element` (`Graph/wire` + its five family pages): the ONE `[Mapper]` `WireCodec` partial spread across six pages — `RequiredMappingStrategy.Both` with `EnabledConversions` excluding `ExplicitCast` (the RMG001 guard against LanguageExt's throwing `Option<T>` cast), `[UserMapping(Default = true)]` presence-shell wrappers over `[MapperIgnoreSource]`-named HAND-CROSSED columns, `[MappingTarget]` `MapField` fills for LanguageExt `Map` sources, per-type non-generic `Option<T> -> TWire?` carriers, and `[UseStaticMapper]` registering the public `BoundaryConverters` set beside the NodaTime protobuf extensions; the RMG001/007/012/013/020/036/060/093 severities ride `.editorconfig` as errors.
- within-library: one `[Mapper]` partial owns a whole boundary by composing per-concern projectors through `[UseMapper]` fields and `[UseStaticMapper]` types, so `[MapProperty(Use = …)]` resolves to a `[NamedMapping]` in any of them; `IncludeMappingConfigurationAttribute` then reuses one method's entire configuration on a sibling method.

[LOCAL_ADMISSION]:
- Mapperly owns boundary transcription alone — `ElementGraph`, `Node`, `Relationship`, and `PropertyValue` against protobuf DTOs, store row shapes, and import/export records. Identity and content hashing stay in the kernel, equality in `Generator.Equals`, the failure carrier in LanguageExt.
...
- An `extern alias`-gated boundary — the parity-spread geometry packages among them — takes hand-written copyists by construction, so a boundary crossing one is never carded as owed transcription.
