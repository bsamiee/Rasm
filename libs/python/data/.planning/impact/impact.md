...
- [02]-[IMPACT]: the `ImpactSource` provider axis into the normalized `MaterialImpact` carrier, the `ImpactRegime` policy rows, the one `_normalize` fold, and the eight-column egress frame.
...
class LcaSolve(Struct, frozen=True):
    demand: tuple[tuple[object, float], ...]
    method: tuple[str, ...]
    indicator: Indicator = Indicator.GWP
    label: LCIAMethod = LCIAMethod.EN_15978_2011
    iterations: int = 0
    contributions: int = 0
...
class PremiseSolve(Struct, frozen=True):
    solve: LcaSolve
    background: str
    scenario: tuple[str, str, int, str, str]
...
class ImpactSource:
    tag: SourceTag = tag()
    openepd: "tuple[Epd | IndustryEpd | GenericEstimate, Option[LCIAMethod]]" = case()
    ilcd_epd: "EPD" = case()
...
EN15804A2: Final[ImpactRegime] = ImpactRegime(
    ...
    methods=(LCIAMethod.EF_3_1, LCIAMethod.EF_3_0),
    units=_A2_UNIT,
)
...
def _doctyped(body: bytes) -> "Epd | IndustryEpd | GenericEstimate":
    from openepd.model.factory import RootDocumentFactory
