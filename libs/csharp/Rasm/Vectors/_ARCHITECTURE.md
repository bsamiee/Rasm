...
6. `[partial]` Field, kernels, and SDFs include derivative-aware kernels, anisotropic tensor metrics, RBF receipts, oriented MLS receipts, half-space/slab/capped-cone primitives, closed planar single-region profile-extrusion SDFs, Lipschitz bounds, mesh-backed SDF receipts, and native iso-surface receipts. `FieldNabla` owns shared kernel/periodic/reconstruction/profile/iso-surface/plane validation, extraction preserves successful `IsoSurfaceReceipt` facts, native iso-surface receipts carry bridge-proven parallel callback and fixed `0.001` tolerance labels, mesh-backed SDF receipts reuse topology facts, and bridge scenarios cover primitive, mesh-backed, boundary SignedHeat, support containment, evaluator failure, and native callback concurrency. Closed SignedHeat remains partial until a volume-backed grid/tet domain exists, and the next focused pass should stay in the existing `Field.cs` / `Mesh.cs` / `Matrix.cs` owners instead of adding new rails.
...
- `[partial]` `Field.cs`: iso-surface routes through intent/extraction, and successful extraction receipts preserve the native `IsoSurfaceReceipt`. `ScalarField.IsoSurfaceDetailed` still owns the Rhino callback, prewarms mesh-backed SDF/cache work before native marching, and rejects evaluator failures through `Fin` rather than returning a partial failure receipt. Static RhinoWIP decompile proof labels `Mesh.CreateFromIsosurface` as parallel-callback and fixed `0.001` root tolerance; bridge proof covers primitive and mesh-backed scalar paths plus support containment separately.
...
- `[partial]` SDF outputs: `ScalarField.LipschitzBound()` exists, mesh-backed signing routes through generalized winding / boundary-source signed heat, and SDF/iso-surface receipts expose approximate/native facts. Mesh SDF receipts reuse topology facts and no longer advertise an unused unsupported status; watertight preflight, closed SignedHeat, broader callback-failure receipts, and product-strength native extraction matrices remain incomplete.
