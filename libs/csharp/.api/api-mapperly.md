...
|  [05]   | `ThrowOnMappingNullMismatch`        | `bool`                       | `true`                     | throw `ArgumentNullException` when a non-nullable return would be null |
|  [06]   | `ThrowOnPropertyMappingNullMismatch`| `bool`                       | `false`                    | throw vs ignore when a non-nullable property would be null |
...
| [INDEX] | [DECLARED SIGNATURE]                                                          | [GENERATED CAPABILITY]                                  |
| :-----: | :--------------------------------------------------------------------------- | :----------------------------------------------------- |
...
- `[Union]` and `[SmartEnum]` owners map through their generated key/case surface: a `[Union]` lowers to its wire case via `[MapDerivedType]` (one attribute per case → a total type-switch), a `[ValueObject<T>]`/`[SmartEnum<TKey>]` maps via its key (`MappingConversionType.Constructor`/`ParseMethod`/`StaticConvertMethods`, or a `[UseStaticMapper]` over the Thinktecture key codec). Tighten `EnabledConversions` (e.g. `All & ~ToStringMethod`) when an implicit conversion would mask a real mismatch.
...
- Accept: boundary transcription between the canonical graph/value vocabulary and wire/DTO/row shapes; the `[Union]`→wire-case switch via `[MapDerivedType]`; the in-store projection via a `ProjectTo` partial; activation through the `IObjectFactory` mint via `[ObjectFactory]`.
- Reject: hand-rolled `switch`/assignment mappers Mapperly would generate; a runtime/reflection mapper; a forwarding wrapper around a generated mapper; identity, hashing, equality, or result-rail logic smuggled into a mapper (those belong to the kernel, `Generator.Equals`, and LanguageExt); the generator-internal `PreserveReferenceHandler` API treated as a stable hand-authored surface.
