...
Placeholders substitute into `command` and each `args` element and export as environment variables on the spawned process either way. Exec form (`args` present) passes placeholders unquoted as single arguments; shell form wraps them in double quotes. Everything else — `session_id`, `tool_name`, `tool_input`, `cwd` — arrives as JSON on stdin, never as environment variables. `SessionStart`, `Setup`, `CwdChanged`, and `FileChanged` can append `export` lines to `$CLAUDE_ENV_FILE` that persist into every later Bash call — the cache lane for an expensive one-time computation. The file is append-only and sourced in order, so a re-firing on resume supersedes a prior value rather than corrupting it. Every value written there passes `shlex.quote`: the file is sourced into bash, so an unquoted `$()` or backtick riding a branch name, a resolved path, or a fetched value executes on the next Bash call — the difference between caching a scalar and shipping a remote-code path, and the single most dangerous surface a `SessionStart` hook owns.
...
`asyncRewake` is never wired to `SessionStart`: the wake fires exit-2 stderr into the start of a fresh conversation, injecting a prior run's stale check output as if it were current context, so an `asyncRewake` body decodes `hook_event_name` and hard-returns 0 on `SessionStart` as defense-in-depth beside the wiring rule. The rewake wall is the 600s command-hook timeout; a scan that legitimately runs longer — a large-repo security sweep — does not stretch the hook, it forks: the firing spawns a detached process that writes its result to a session-keyed file and returns 0 immediately, and a later firing reads the completed file and wakes only on findings new since the last acknowledged set. This detached-harvest lane is also the dual-provider fallback, since Codex parses `async` and skips it, so a portable guardrail runs synchronously or splits its slow leg to the same detached process the body spawns itself.

## [05]-[SESSION_ROUTING]

No payload carries `tty` or `pid`, so a hook that routes a notification or a telemetry row back to a specific pane keys on `session_id` from stdin and derives terminal identity from its own runtime — the hook `command` is a child of the agent process and inherits its pane-env, while the tty answers only through the parent's process table. Read the tty from `ps -o tty= -p $PPID` and the pane from `$WEZTERM_PANE`/`$ZELLIJ_PANE_ID`/`$TMUX_PANE`/`$KITTY_WINDOW_ID`/`$TERM_SESSION_ID`. The durable pattern captures the `session_id` ↔ `{tty, pane}` map at `SessionStart` into a JSON registry under XDG state and looks the pane up by `session_id` on later `Notification`/`Stop`; the session-context template carries the capture lane, and the statusline JSON carries the same `session_id` as a second correlation surface. Any failure in a routing hook exits 0 so telemetry never blocks the harness. The same session-keyed discipline governs any cross-session state a hook writes — a counter, a receipt, a file claim — so parallel sessions on one shared worktree never clobber each other's file.
...
A transmitter hook chains after the policy hook on the same event array, so the gate decides and the transmitter forwards without ever touching the verdict — it is wired `async: true`, wraps its POST in a short timeout, and guarantees exit 0 unconditionally so a dead collector never gates the loop. Unconditional means the whole body sits under one broad `except Exception` or a `finally` that exits 0, never a two-type `(DecodeError, HTTPError)` catch: `httpx.InvalidURL`, `CookieConflict`, and `StreamError` are not `HTTPError` subclasses, so a narrow catch lets a malformed sink URL escape as a non-zero exit that blocks the loop the transmitter must never touch. The CloudEvents `time` is a true-UTC RFC3339 stamp (`datetime.now(UTC)` or `gmtime`), never a local `strftime` suffixed `Z`, which mislabels the offset by the machine's zone and corrupts the estate's ISO-8601 UTC ledger. The envelope is a policy choice of two tiers. A private dashboard or registry takes the lightweight tier: the raw stdin payload nested whole, with the hot query keys (`tool_name`, `session_id`, `hook_event_name`, `agent_id`) hoisted flat beside it so the store indexes without a custom parser. A shared or standard event bus takes a CloudEvents 1.0 envelope — `specversion`, a reverse-DNS `type`, a `source` derived from `HOOK_PROVIDER` so a dual-provider fleet's events self-identify their origin, an `id`, a `time`, and the raw payload in `data` with hook identity as flat lowercase extensions. Codex emits OTel natively through its `[otel]` exporter (`hooks.run`, `turn.token_usage`, `multi_agent.spawn`), so hook-level metrics duplicate it there — a Codex transmitter's value is the synchronous attention stream, not the metrics. The transmitter chain and both envelope tiers are the telemetry-transmitter example. Because a hook is invisible in the transcript unless it exits non-zero, a hook that silently rewrites or blocks surfaces its intent through `systemMessage` and logs every firing to a file or a transmitter, never stdout; observing and diagnosing a live hook is the verification reference.
