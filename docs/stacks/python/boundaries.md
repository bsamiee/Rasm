...
Absence taxonomy arrives settled from `shapes.md` — `None`-as-failure never crosses inward, computed non-failing absence is `Option`, a cause-bearing foreign state is a `@tagged_union` case — and the carrier-minting matrix is `rails-and-effects.md`'s. This page owns what neither settles: _where_ a foreign sentinel is admitted, _where_ a plugin set is discovered without importing it, and that every site that can mint absence or activate foreign code is a seam. A multi-channel provider verdict admits every channel — a diagnostic list's emptiness never proxies the verdict boolean, because a false verdict beside an empty list otherwise mints success-equivalent evidence.
