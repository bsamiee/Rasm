...
- companion: the concrete `Microsoft.Extensions.AI` `10.7.0` core package owns the middleware that is
  ...
  `Rasm.AppHost/.api/api-extensions-ai-middleware.md` (the model-governance composition edge). This catalog
  documents the contracts that pipeline builds on; the builder surface is out-of-package here by design.
- in-repo implementors: `OnnxRuntimeGenAIChatClient` (`api-onnxruntimegenai`) is the local generative
...
- this catalog documents the `Microsoft.Extensions.AI.Abstractions` `10.7.0` net10.0 `IChatClient`
  ...
  including the local `OnnxRuntimeGenAIChatClient` (`api-onnxruntimegenai`) — implements; the 10.7.0/net10.0
  central pin is the single M.E.AI floor across both catalogs, never the GenAI facade's transitive 9.8.0.
- the executable pipeline (`AsBuilder()`/`UseFunctionInvocation`/`UseOpenTelemetry`/`UseDistributedCache`)
  ...
  `Rasm.AppHost/.api/api-extensions-ai-middleware.md`, never re-declared here.
...
- the `Microsoft.Extensions.AI` core builder pipeline (`inner.AsBuilder().UseFunctionInvocation()
  ...
  `Rasm.AppHost/.api/api-extensions-ai-middleware.md`; `Microsoft.Extensions.Caching.Hybrid` is the
  ...
  these same contracts (`OnnxRuntimeGenAIChatClient`, `api-onnxruntimegenai`).
...
- Reject: provider SDK types, direct HTTP calls, and model-specific option objects inside compute
  ...
  (`Rasm.AppHost`-referenced, documented at `Rasm.AppHost/.api/api-extensions-ai-middleware.md`), never
