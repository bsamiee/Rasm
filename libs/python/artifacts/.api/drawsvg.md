...
- installed: `2.4.1` reflected via `importlib.metadata.version("drawsvg")` (the module exposes no `__version__`); pure-Python universal wheel, interpreter-agnostic, cp315-clean with zero install-time dependencies
- extras (NOT installed on the cp315 core): `raster`/`all` pull `cairoSVG~=2.3`, `numpy`, `imageio~=2.5`, `imageio-ffmpeg~=0.4`; `color`/`all` pull `pwkit~=1.0` + `numpy`; absent these, the `save_png`/`rasterize`/`as_gif`/`as_mp4`/`as_video`/`as_spritesheet`/`as_animation_frames` paths raise at call time — treat them as boundary-gated, not core capability
...
|  [12]   | `Drawing.as_gif/as_mp4/as_video/as_spritesheet/as_animation_frames(...)` / `save_gif/save_mp4/save_video/save_spritesheet(...)` | raster | [GATED] video/frame egress via optional `imageio`+ffmpeg; absent on core — run any ffmpeg assembly through the runtime gated-band `to_process.run_sync` seam |
...
- raster boundary: `save_png`/`rasterize`/`as_gif`/`as_mp4`/`as_video`/`as_spritesheet`/`as_animation_frames` and the module `render_svg_frames`/`save_video`/`frame_animate_*` depend on the optional `cairoSVG`+`numpy`+`imageio`+`imageio-ffmpeg` extras (absent on the cp315 core, raise at call time) — rasterization to PNG/PDF routes to `resvg-py`/`vl-convert`/`pyvips`, and any ffmpeg-backed video assembly runs through the runtime gated-band `to_process.run_sync` subprocess seam, never an in-page blocking shell-out.
...
- The authored SVG string feeds `resvg-py.svg_to_bytes` / `vl-convert` for PNG/PDF rasterization, so drawsvg is the vector-authoring owner and the raster owner is strictly downstream; the in-package `save_png` row stays unused on the cp315 core because its `cairoSVG` extra is absent.
...
- A synchronized animation built with `SyncedAnimationConfig` + `add_key_frame` serializes to standalone `as_html`; if a rasterized GIF/MP4 deliverable is required, the frame egress runs through the runtime gated-band `to_process.run_sync` seam (ffmpeg subprocess), keeping the blocking call off the async figure rail.
