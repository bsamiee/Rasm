...
- ownership: TRANSITIVE in `Rasm.Materials` via `Thinktecture.Runtime.Extensions.MessagePack`; the direct `MessagePack`/`MessagePackAnalyzer` pins own `Rasm.Persistence` + the bridge closure
...
|  [07]   | `WithOldSpec(bool?)` / `WithOmitAssemblyVersion(bool)` / `WithAllowAssemblyVersionMismatch(bool)` | builder | spec/typeless-metadata compatibility knobs |
...
- this bridge is the REASON MessagePack is on the Materials compile surface (transitive through it); a direct `MessagePack` reference is admitted in the folder csproj only if the design composes `MessagePackSerializer` directly beyond the bridge-resolved value-objects.
