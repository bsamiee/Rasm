...
public sealed partial class MeshSampleKind {
    ...
    public static readonly MeshSampleKind Manifold = new(key: 5, category: MeshSampleCategory.Validity, sample: static (m, _) => Fin.Succ(m.IsManifold(topologicalTest: true, isOriented: out bool _, hasBoundary: out bool _) ? 1 : 0)), BoundaryFree = new(key: 6, category: MeshSampleCategory.Validity, sample: static (m, _) => Fin.Succ((m.IsManifold(topologicalTest: true, isOriented: out bool _, hasBoundary: out bool boundary) && !boundary) ? 1 : 0));
    ...
    public static readonly MeshSampleKind Edges = new(key: 14, category: MeshSampleCategory.Count, sample: static (m, _) => Fin.Succ(m.TopologyEdges.Count)), Euler = new(key: 15, category: MeshSampleCategory.Count, sample: static (m, _) => Fin.Succ(m.TopologyVertices.Count - m.TopologyEdges.Count + m.Faces.Count));
