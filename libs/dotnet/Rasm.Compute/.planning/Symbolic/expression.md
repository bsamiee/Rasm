...
- Result: none of its own — an interior value; differentiate/compile/evaluate outcomes ride the `lowering` `CompiledExpr` cache result and the `optimizer` `Optimization` slot, and a parse/dimension/analytic fault rides the `ComputeFault` rail at the admitting edge.
...
- Result: none of its own — the differentiate output feeds `lowering` (compiled-Jacobian cache result), the optimizer adjoint (`Optimization` slot), the coefficient vector feeds the optimizer seed, the solve/integrate rows feed the AEC cost/QTO lane, and a transform fault rides the `ComputeFault` rail.
