...
public sealed partial class RebarGrade {
    ...
    public static readonly RebarGrade B500A = new("b500a", minimumYieldMpa: 500.0, standard: RebarStandard.En10080, appearanceId: "metal.steel", enGrade: Some(EnRebarGrade.B500A));
...
public readonly record struct RebarSection(
    ...
    // the structural-design seam reads — the bar's structural STEEL, sourced INDEPENDENTLY from the AppearanceId finish (the
    // component#COMPONENT_OWNER two-slot independence law: a coated bar's capacity steel and its coat finish are distinct
    // MaterialIds, neither derived from the other). The reinforcing steel is metal.steel for every grade.
...
public static class ComponentCatalogue {
    ...
    static Fin<Component> RebarOf(RebarRow r, Context context, Op key) =>
        ...
        from item in Component.Of(ComponentFamily.Reinforcement, r.Designation, new ComponentSection.Reinforcement(section), section.CapacityKey, section.AppearanceId, key)
        select item;
    ...
    public static FrozenDictionary<ComponentId, Component> BuildRebarRows(Context context) =>
        RebarRows
            ...
            .ToFrozenDictionary(static c => c.Designation, static c => c, ComparerAccessors.StringOrdinal.EqualityComparer);
}
```
...
public sealed record RcSection(IConcreteSection Section, EnConcreteMaterial Concrete, EnRebarMaterial Rebar, double GrossSteelAreaMm2, double CoverMm) {
    public Component ConcreteProfile { get; init; }   // the source concrete-outline Component (QTO key; capacity#SECTION_CAPACITY reads its Unit gross depth/width)
}
...
public static class RcSectionBuilder {
    ...
    public static Fin<RcSection> Of(Component concrete, EnConcreteGrade concreteGrade, RebarGrade barGrade, BarSize linkSize, Seq<RebarLayout> layout, double coverMm, NationalAnnex annex, Op key) =>
        ...
        from profile in ParametricSection.ProfileOf(concrete, key)   // component#COMPONENT_OWNER IProfile perimeter
