...
|   [9]   | BM25 overlay                 | postgres-server | `@@@ pdb.parse(...)` query-builder match, `pdb.score(<key_col>)` / `pdb.snippet` via `FromSql` / `SqlQuery`; BM25 index DDL on `server-tier#SEARCH_PROVISIONING` | high-power FTS overlay beside the always-present native baseline; `@@@` RHS is always a `pdb.*` builder, score/snippet anchor on the `key_field`; `FullTextQuery.Bm25` selects it; native FTS answers when pg_search is not preloaded |
