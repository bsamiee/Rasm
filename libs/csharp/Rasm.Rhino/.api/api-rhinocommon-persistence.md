...
- `ArchivableDictionary.Set(string key, T val) : bool` over carriers {nested `ArchivableDictionary`, `MeshingParameters`, `GeometryBase`, `ObjRef`} — carriers deep-copy into the item.
...
- `UserData.Copy(CommonObject source, CommonObject destination) : void` (static) / `MoveUserDataFrom(CommonObject objectWithUserData) : Guid` (static) / `MoveUserDataTo(CommonObject objectToGetUserData, Guid id, bool append) : void` (static) — cross-object custody transfer.
...
- `UserDataList : IEnumerable<UserData>` — `Add(UserData) : bool` throws `ArgumentException` unless the type is a public class with a parameterless constructor / `Remove(UserData) : bool` / `Find(Type userdataType) : UserData` / `Contains(Guid userdataId) : bool` / `Purge() : void` / `this[int index] : UserData` / `Count : int` / `GetEnumerator() : IEnumerator<UserData>` — the per-object user-data roster.
