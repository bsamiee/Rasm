...
- owner: `data`
- rail: terrain
- installed: `0.10.10` (latest PyPI release, tag `v0.10.10`); manifest-markered `python_version<'3.15'` because it pulls `numba`, which has no cp315 wheel — resolves on the companion interpreter only, so `assay api resolve` finds no source in the cp315 project venv; members are reflected from the `v0.10.10` source `__init__` export set
...
- io axis: `geotiff.open_geotiff`/`to_geotiff` are the GeoTIFF read/write rows, `rasterize`/`polygonize`/`clip_polygon` the vector<->raster bridge, and `reproject`/`resample`/`merge`/`sieve` the grid-alignment rows; raster I/O lives on this owner on the gated band, not a separate GDAL owner — the grid never leaves the `DataArray`/`Dataset` model.
...
- boundary: xarray-spatial owns Numba raster analytics AND `xarray`-native GeoTIFF I/O and rasterize/polygonize/reproject on the gated companion band; multi-format vector geometry editing and authoritative CRS database resolution route to their owners; the `DataArray`/`Dataset` result feeds the persistence and visuals owners directly.
