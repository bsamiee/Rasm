...
- Packages: Rasm (project — `FaultBand`/`[FaultCase]`/`Fault` the fault floor, `PerceptualColor`/`BlendPath`/`UnitInterval` the colour algebra), Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime, System.Numerics (inbox)
...
- Packages: Rasm (project — `PositiveMagnitude` the rate band), Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime
...
- Law: the tick cadence composes the kernel `PaceBand` the transport carries. The band bounds the request from both ends — a variable-refresh panel advertises a ceiling it will not hold and the slowest presentation is the other half of the clamp — so a 240 fps timeline on a 60 Hz display asks for a cadence the display can present instead of asking for one it refuses and reading the shortfall as drift. The band stays on the transport and the clock that reads it; `ReducedMotion.Granted` hands a kernel drive step only the `CapabilitySet<Accessibility>`, so no posture pairs pace with accessibility and no sample carries either.
...
- Entry: `public static IO<VisualArtifact> Render(VisualRuntime runtime, Timeline timeline, WalkthroughSpec spec, Func<Duration, TimelineSample, SKImageInfo, Fin<SKImage>> frame)` — the delegate carries the sampled instant beside the sample, so a frame renderer can draw anything time-varying beside the tracks (the tour's caption overlay is the first consumer) — renders every frame of the timeline to the encode path and seals one artifact for the sequence; the frame count is the timeline duration over the frame rate.
...
public static class Walkthrough {
    ...
    static IO<WalkthroughFold> Advance(
        ...
        Range(0L, timeline.Playhead().FrameCount)
            .Fold(IO.pure(WalkthroughFold.Empty), (acc, index) => acc.Bind(state =>
...
- Packages: Rasm (project — `CapabilitySet`/`ICapability` the lane flags, `PositiveMagnitude` the board metrics, `PaceBand` the cadence band), Thinktecture.Runtime.Extensions, LanguageExt.Core, NodaTime
