...
- Boundary — interpolation: ONE-dimensional interpolation is the kernel's whole. `Rasm/Numerics/transform#INTERPOLATE` owns `Interpolant.Fit(points, values, factory)`, with the caller selecting the exact MathNet `*Sorted` method group; its `Evaluate`, `Derivative`, `SecondDerivative`, and `Integrate` reads finite-gate MathNet results and use `IInterpolation.SupportsDifferentiation`/`SupportsIntegration` for typed refusal. The local capsule, phantom tiers, algorithm-named sibling factories, shared build surface, and absence-carrying read shell remain deleted. NAMED LOSS: the local `At`/`Slope`/`Curvature`/`Area` reads answered `Option<double>`, where the kernel answers `Fin<double>` — absence becomes a typed refusal, which is strictly more evidence and one more thing a caller must thread.
