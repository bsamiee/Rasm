...
- Packages: CloudNative.CloudEvents (+`.SystemTextJson` `JsonEventFormatter`, +`.Kafka` `ToKafkaMessage`, +`.Amqp` `AmqpExtensions.ToAmqpMessageWithUnderscorePrefix`/`ToCloudEvent` over the AMQPNetLite `Amqp.Message` carrier and its `SenderLink.Send` transport — the `AMQP 1.0` leg, protocol-disjoint from `RabbitMQ.Client`, +`.Mqtt` `MqttExtensions.ToMqttApplicationMessage` structured-mode `CloudEvent` → `MqttApplicationMessage` over the MQTTnet carrier — the MQTT v5 leg, payload-body-only; `Partitioning`/`Sequence` extension attributes), NATS.Net (`INatsJSContext.PublishAsync`/`TryPublishAsync`, `NatsHeaders`, `PubAckResponse.Duplicate`), Confluent.Kafka (`ProduceAsync`, `DeliveryResult.Status`, `InitTransactions`/`BeginTransaction`/`CommitTransaction`), Confluent.SchemaRegistry (`CachedSchemaRegistryClient`/`SchemaRegistryConfig`) + serdes (`AvroSerializer<T>`/`JsonSerializer<T>`/`ProtobufSerializer<T>` `SerializeAsync` — the composition-root payload framing), OpenTelemetry.Instrumentation.ConfluentKafka (`AsInstrumentedProducerBuilder` + `ConfluentKafkaInstrumentedProducerBuilderOptions` at the leg's builder seam; `AddKafkaProducerInstrumentation` registers on the tracer and meter builders at the AppHost root), RabbitMQ.Client (`CreateChannelOptions` confirms, `BasicPublishAsync`), MQTTnet (`MqttClientFactory.CreateMqttClient` per-instance mint, `IMqttClient.PublishAsync` → `MqttClientPublishResult`, `MqttApplicationMessageBuilder`, `MqttClientOptionsBuilder.WithProtocolVersion(V500)`, `MqttUserProperty.ValueBuffer` — the QoS-1 PUBACK reason-code leg and the v5 UserProperties tracing carrier), DotPulsar (`ISend.Send` → `MessageId`), StackExchange.Redis (`StreamAdd`/`StreamIdempotentId`/`StreamTrimMode.Acknowledged`), ClickHouse.Driver (`ClickHouseClient.InsertBinaryAsync` + `InsertOptions`/`QueryOptions` custom settings — the warehouse leg under the `insert_deduplication_token` producer-supplied dedup setting), pg_net (`net.http_post`/`net.request_status`/`net.http_response_result` over raw Npgsql), Google.Protobuf (`WriteLengthPrefixedTo` — the wire-native payload), NodaTime.Serialization.SystemTextJson (`ConfigureForNodaTime` on the one formatter options), Thinktecture.Runtime.Extensions.Json (`ThinktectureJsonConverterFactory` co-registered), BCL inbox.
...
public abstract partial record EgressSink {
    ...
    public sealed record Amqp(SinkBinding Bind, string Address) : EgressSink;
    ...
    // hand-off, StreamAdd idempotent XADD, the ClickHouse async insert under insert_deduplication_token, the
    // ToMqttApplicationMessage structured encode + PublishAsync PUBACK reason-code fold);
...
public static class Egress {
    ...
    // fired, and the trace slot continues through the AppHost `TraceContext` carrier adapter as the W3C
    // `traceparent`/`tracestate` composite — every sink leg (webhook, Pulsar, wire-native, AMQP 1.0 included)
    // therefore ships the same trace attributes, and the Kafka/NATS legs additionally mirror them onto their
    // native header carriers.
    public static CloudEvent Envelope(OpLogEntry row, Func<OpLogEntry, (ReadOnlyMemory<byte> Data, bool Redacted)> redact) {
        ...
        row.Trace.Continue().IfSome(ctx => {
            ce[TraceParent] = $"00-{ctx.TraceId}-{ctx.SpanId}-01";
            ctx.State.IfSome(state => ce[TraceState] = state);
        });
        return ce;
    }
}
...
|  [05]   | amqp         | settled `SenderLink.Send` disposition | receiver CloudEvents `id` dedup          |
|  [06]   | pulsar       | `ISend.Send` → `MessageId`            | receiver CloudEvents `id` dedup          |
|  [07]   | wire-native  | `OutboundHop` honesty verdict         | AppHost hop policy                       |
|  [08]   | redis-stream | awaited `StreamAdd` id                | producer `StreamIdempotentId`            |
|  [09]   | clickhouse   | awaited async insert                  | `insert_deduplication_token`             |
|  [10]   | mqtt         | awaited QoS-1 PUBACK reason code      | receiver CloudEvents `id` dedup          |
