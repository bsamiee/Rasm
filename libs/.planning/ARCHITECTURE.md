...
- Branch contributions meet at the composition root alone, merged by artifact key under the `[07]-[SCHEMA_STATE]` order.
...
- Domains widen at their owning branch under `[10]-[ADMISSION]`; the rows above carry no ceiling on what a branch admits next.
...
- `<domain>` is the capability subject `[06]-[OBSERVABILITY_CONFORMANCE]` fixes for metric names, so a board and a subscription join one vocabulary.
...
[HOOK_ORDER]: Message-envelope emitter is an `observe` subscription over fired hook facts, never an emit inside a domain fold; `[06]-[OBSERVABILITY_CONFORMANCE]` `[HOOK_PLANE]` owns the modality vocabulary, and this join fixes the emitter's rung on it.

[AXIS_REFUSAL]: Binding a deployment cannot serve refuses on the `providers` open axis as one `[08]-[CONSUMPTION_MODEL]` `[CONSUMPTION_DESCRIPTOR]` row, since a boolean knob re-mints the assumed consumer that roster forecloses.
