...
class Schedule(Struct, frozen=True):
    ...
    def _key(self) -> ContentKey:
        # key-over-INPUT: canonical (content ⊕ fmt ⊕ theme ⊕ standard) minted PRE-RUN — never a key over rendered bytes.
    ...
    async def _emit(self) -> RuntimeRail[ArtifactReceipt]:
        # the renamed private render thunk — the rendered bytes are content-addressed on the receipt FACTS;
        # the slot is the PRE-RUN input key. The fold crosses the runtime thread lane, never a folder-minted limiter.
...
def _lower(schedule: Schedule) -> tuple[pl.DataFrame, tuple[TableOp, ...], str, int, int]:
    ...
    # from the template, a legend authors its frame from the drawing/regime vocabulary; total by assert_never.
...
def _hatch_swatch(spec: PatternSpec) -> Swatch:
    ...
    # two-band box, or a bordered box crosshatched at the characteristic PatternSpec.angle; the legend SHOWS the real
