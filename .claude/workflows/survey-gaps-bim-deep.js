export const meta = {
  name: 'survey-gaps-bim-deep',
  description: 'ADDITIVE, broadened second-pass capability-gap survey-AND-APPLY for Rasm.Bim, weighted hard toward IFC, geospatial, and cost/scheduling/planning so the folder stops hand-rolling concerns a real ecosystem package owns. A deeper re-run than the generic per-folder twin: NINE opus xhigh research facets instead of six (IFC model graph + geometry/tessellation + validation + BCF + IDS depth, geospatial CRS/vector/raster/tiles/spatial-index, cost estimating/QTO/cost-models, scheduling/planning CPM/resource-leveling/4D, classification systems, clash/coordination, quantity-takeoff, and scan-to-BIM). One opus xhigh survey+synthesize agent emits the gaps and nine facets, nine opus xhigh research agents each hunt one facet and self-validate the six-condition gate, one opus xhigh execute agent consolidates and ADDS to the existing Bim admissions (read from the manifest/README/csproj, never duplicated) then restores and verifies at newest on osx-arm64, and one sonnet agent writes a one-line placeholder .api catalog per NEWLY admitted package. The full .api authoring and the design-page integration are deferred to a focused rebuild-api run and to plan-cs-folders.',
  whenToUse: 'Re-run Bim with broader, deeper coverage of IFC, geospatial, and cost/schedule/planning so nothing is hand-rolled, additive over the existing admissions.',
  phases: [
    { title: 'Survey', detail: 'one opus xhigh agent: domain, admitted packages, hand-rolls, gaps, AND nine IFC/geospatial/cost/schedule/planning research facets', model: 'opus' },
    { title: 'Research', detail: 'nine opus xhigh agents, one facet each: best-in-class modern candidate discovery with a self-validated six-condition gate', model: 'opus' },
    { title: 'Execute', detail: 'one opus xhigh agent: consolidate the nine findings, ADD to the existing admissions, update csproj + README, restore and verify the install at newest on osx-arm64, self-heal or revert', model: 'opus' },
