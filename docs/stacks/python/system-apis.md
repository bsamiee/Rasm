...
- Boundary: the transfer verb is a provider call, so its residual `OSError` — the permission, cross-device, or TOCTOU failure the `info` pre-checks cannot preclude — crosses through the single-exception `catch` trap `results-and-effects.md` owns, never an untrapped raise past the boundary.
...
- Gate: persisted text I/O states `encoding="utf-8"` (or `encoding="locale"` only at a genuine locale boundary), never an implicit default at a durable boundary.
...
- Boundary: the compile-time `re.PatternError` crosses through the single-exception `catch` trap `results-and-effects.md` owns and the parse-time `is None` miss becomes a guard arm in the same expression; the interior never sees the raise or a `None` match.
...
- Boundary: a wire timestamp admits to an aware `datetime` at the boundary and the interior carries the aware value; `date.strptime`/`time.strptime` return the component type directly where `datetime.strptime(...).date()`/`.time()` parses the whole stamp only to discard half, and the malformed-stamp `ValueError` crosses through the single-exception `catch` trap `results-and-effects.md` owns.
- Reject: `datetime.strptime` followed by `.date()`/`.time()` slicing; naive datetime arithmetic past the boundary.
...
- Exemption: `heapify_max`/`heapreplace_max`/`heappushpop_max` mutate the heap buffer in place — the one statement body this card forces — and the result-typed return copies nothing.
...
- Exemption: the `__reduce_ex__` zero-copy hook, the `os.readinto` caller-buffer fill, and the protocol-5 `buffer_callback` mutable sink are the platform-forced statement bodies no expression reaches; each still returns an `expression` `Result`.
