...
class Held extends Context.Tag('rasm-testkit-spec/Held')<
    Held,
...
layer(ObjectStores.memory)('object store double', (it) => {
    ...
    it.effect(
        'presign on the double refuses typed: the capability belongs to the real lane',
        () =>
            Effect.gen(function* () {
                const store = yield* ObjectStore;
...
layer(NodeContext.layer)('image pins', (it) => {
    ...
    it.effect(
        'an unpinned lane name is honest typed absence, never a vacuous default',
        () =>
            Effect.gen(function* () {
                const fault = yield* Effect.flip(
                    Containers.pin('never-pinned'),
                );
                expect(fault).toBeInstanceOf(HarnessFault);
                expect(fault.reason).toBe('unsupported');
            }),
    ...
    it.effect(
        'a malformed pin manifest is a typed engine fault, never a module-load throw',
        () =>
            Effect.scoped(
                Effect.gen(function* () {
...
describe.skipIf(!_LIVE)('pg container lane [RASM_TESTKIT_CONTAINERS]', () => {
    const _pgLive = Layer.provide(
        Layer.unwrapEffect(
            Effect.map(Containers.pin('pg'), (image) =>
                PgLanes.container({ image, seed: _DDL }),
            ),
        ),
        NodeContext.layer,
    );
    layer(_pgLive, { timeout: '150 seconds' })('real server', (it) => {
        it.effect(
            'the mapped-port driver serves the same lane algebra over the container-seeded schema',
            () =>
                Effect.gen(function* () {
                    const lane = yield* PgLane;
                    yield* lane.exec(
                        "INSERT INTO marks VALUES ('<key-a>', 3);",
                    );
                    const rows = yield* lane.decoded(_Mark)(
                        'SELECT key, rank FROM marks',
                    );
        ...
        it.effect(
            'LISTEN/NOTIFY delivers through the armed real-driver subscription',
            () =>
                Effect.scoped(
                    Effect.gen(function* () {
                        const lane = yield* PgLane;
        ...
        it.effect(
            'the pooled-driver sandbox joins one transaction connection and rolls back on exit',
            () =>
                Effect.gen(function* () {
                    const lane = yield* PgLane;
                    yield* lane.sandbox(
                        Effect.gen(function* () {
                            yield* lane.exec(
                                "INSERT INTO marks VALUES ('<key-pooled>', 9);",
                            );
                            expect(
                                yield* lane.rows(
                                    "SELECT key FROM marks WHERE key = '<key-pooled>'",
                                ),
                            ).toHaveLength(1);
                        }),
                    );
        ...
        it.effect(
            'a second concurrent listen arms without leaking its control frames into the first mailbox',
            () =>
                Effect.scoped(
                    Effect.gen(function* () {
                        const lane = yield* PgLane;
...
describe.skipIf(!_LIVE)('container substrate [RASM_TESTKIT_CONTAINERS]', () => {
    ...
    layer(_seeded, { timeout: '150 seconds' })('copy and exec rows', (it) => {
        it.effect(
            'a copy row lands its content in the image and exec reads it back',
            () =>
                Effect.gen(function* () {
                    const held = yield* Held;
                    const exec = yield* Containers.exec(held, [
                        'cat',
                        '/probe.sql',
                    ]);
                    expect(exec.exitCode).toBe(0);
                    expect(exec.output).toContain('SELECT 1;');
                }),
        );

        it.effect(
            'a failing in-container command reports its exit code as data, never a green pass',
            () =>
                Effect.gen(function* () {
                    const held = yield* Held;
                    const exec = yield* Containers.exec(held, [
    ...
    layer(_networked, { timeout: '150 seconds' })(
        'network and quota rows',
        (it) => {
            it.effect(
                'the net row registers its alias on the network-embedded DNS',
                () =>
                    Effect.gen(function* () {
                        const held = yield* Held;
                        const exec = yield* Containers.exec(held, [
                        ...
                        expect(exec.exitCode).toBe(0);
                        expect(exec.output).toContain('kit-pg');
                    }),
            );

            it.effect(
                'the resources row lands as a real cgroup memory ceiling',
                () =>
                    Effect.gen(function* () {
                        const held = yield* Held;
                        const exec = yield* Containers.exec(held, [
                        ...
                        expect(exec.exitCode).toBe(0);
                        expect(exec.output).toContain('536870912');
                    }),
            );
        },
    );
...
describe.skipIf(!_LIVE)(
    'object store container lane [RASM_TESTKIT_CONTAINERS]',
    () => {
        const _live = Layer.provide(
            Layer.unwrapScoped(
                Effect.gen(function* () {
                    const image = yield* Containers.pin('store');
                    const row = Containers.objectStore({
                        image,
                        rootUser: 'testing',
                        rootPassword: 'testing-secret',
                    });
        ...
        layer(_live, { timeout: '150 seconds' })('real store', (it) => {
            it.effect(
                'bytes round-trip and the presigned URL serves them',
                () =>
                    Effect.gen(function* () {
                        const store = yield* ObjectStore;
                        yield* store.put('band/one', _BYTES);
                        expect(yield* store.get('band/one')).toEqual(
                            Option.some(_BYTES),
                        );
                        const url = yield* store.url('band/one', 60);
                        const fetched = yield* Effect.promise(() =>
                            fetch(url).then((reply) => reply.arrayBuffer()),
                        );
            ...
            it.effect(
                'an absent key resolves typed None through the NoSuchKey code, never a thrown escape',
                () =>
                    Effect.gen(function* () {
                        const store = yield* ObjectStore;
                        expect(yield* store.get('band/never-written')).toEqual(
                            Option.none(),
                        );
                    }),
            );
