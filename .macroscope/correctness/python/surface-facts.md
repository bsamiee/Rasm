...
- Preview is the five-field shape with the trailing band; a DEGRADED roll double-buffers so capacity never gaps, and demanding NOT_SERVING before the roll advertises a phantom outage.
...
- `importlib.util.find_spec` on a dotted name imports the parent package — the standard library documents that behavior — so a probe-the-submodule demand against a parent-side no-import gate pays the exact native band the gate defers: the probe rides the top-level distribution name, and unconditional submodules of one distribution make distribution presence the capability unit.
