...
- Law: motion arrives as a WRAPPER, never a second renderer — the mounted shape is `motion.create(LinePath)` under the subtree `MotionConfig` `Chart.motion` supplies, whose `isValidProp` is the SOLE gate deciding which props the wrapper forwards to the DOM, so one predicate at the provider covers every wrapped visx shape beneath it. A hand-animated attribute, a per-mark reduced-motion read, or a `MotionValue` mirrored into React or the atom store is the engine's own named defect.
- Law: the accelerated set decides what a target may hold — `opacity`, `transform`, `filter`, `clipPath`, and `backgroundColor` hand off to WAAPI on SVG exactly as on HTML, and EVERY other property (the path `d` included) generates on rAF. A `Chart.Target` therefore carries the opacity and the inner-frame `clipPath` wipe alone; the geometry change rides `updateScale` re-domaining the held instances, and interpolating `d` is rejected because it pays a per-frame rAF cost to restate a path `LinePath` already regenerates from the re-domained scale.
...
declare namespace Chart {
  ...
  // the accelerated pair alone: `opacity` and `clipPath` hand off to WAAPI on SVG, so a reveal and a selection echo
...
// the engine drives the visx component itself, so the mounted mark IS the shape and no animated copy stands beside it
...
// the wipe closes over the INNER frame the margin already carved, so a reveal is one accelerated property and the
...
declare namespace Chart {
  // the package's own thunk source, narrowed to the deferred arm: a thunk is the ONLY form whose losing bitness
  ...
  // the `regular-layout` tree the workspace token carries; both arms are recursive, so the codec suspends
  type Layout =
  ...
  // the package types the callback `Function`, so the borrow's shape is stated here; the element class follows the
...
// the engine's config types take MUTABLE arrays, tuples, and maps, so every leaf of the persisted codec is minted
...
const _Window = Schema.Struct({
  ...
  // the frame trio: mutually exclusive by the engine and NOT encoded here, which is why the gate counts them
...
const _CONFIG = "rasm.ui.chart.workspace"
...
const _saved = (element: HTMLPerspectiveViewerElement, feed: string): Effect.Effect<Chart.Config, ChartFault> =>
  Effect.flatMap(
    ...
    (token) =>
      Effect.mapError(
        // the ONE grain asymmetry: `save` emits `layout: null` for an unlaid element while the restore field is
...
const _echo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      ...
      // the active-panel read both happen inside this turn — the patch has no other synchronous carrier
      const listen = (event: Event): void => {
        ...
        commit((config) =>
          Option.match(Record.get(config.panels, panel), {
            ...
            // the next save rather than inventing the table binding the panel is still missing
            onNone: () => config,
...
const _borrow = (
  ...
  // the snapshot lane IS the borrow: `with_typed_arrays` is one more `read`, so the lend inherits the one view
