...
- Entry: `public static Fin<RemnantLedger> Remnant.Reconcile(RemnantOp op, Map<UInt128, RemnantRow> ledger, ReusePolicy policy)` — the ONE lifecycle entry discriminating on the op case through the generated total `Switch`; `Fin<T>` routes `FabricationFault.RemnantStale` 2736 on an unresolvable or terminal claim/release key and succeeds with verdict rows everywhere else (a rejected stocking candidate is a RETIRED row, never a fault); `public static Seq<Stock> Remnant.Stockable(Map<UInt128, RemnantRow> ledger)` — the inventory projection minting each stocked row's usable interior as a lineage-stamped child (`Remnant.Of(usable, Some(identity))`) wrapped `Stock.FromRemnant`, feeding the next run's `FabricationInput.Inventory` so the landed nfp scheduler packs real offcuts before virgin sheets.
- Auto: `Stocking` gates each minted remnant — inset the boundary by `InsetMm` through `PolygonAlgebra.Offset` (negative delta, `OffsetEnds.Polygon`), measure the usable interior through `PolygonAlgebra.Area`, walk the `Parent` chain through the ledger for lineage depth (a broken link terminates the walk: depth gates REUSE, staleness gates CLAIM), and admit as a `stocked` row or retire as `scrapped` under the area/aspect/depth rows; `Claim` shifts a resolvable non-terminal row to `reserved` stamping the job, `Release` closes it to `consumed` or returns it to `stocked`; `Sweep` retires every live row whose identity is absent from the observed physical inventory and reports the stale keys; every arm returns the next ledger INSIDE the receipt — the caller carries it forward, the run-N→run-N+1 loop closing exactly as the owner's truth-carry fields do.
- Receipt: `RemnantLedger` — the next ledger map plus `Admitted`/`Retired`/`Stale` typed rows, `ReclaimedAreaMm2` (the usable area entering stock), and `WasteReductionMm2` (the recorded `NestWasteArea` wire reduction the Compute rollup decodes downstream); the rows ARE the audit trail, no parallel event log.
...
// The receipt carries the NEXT ledger (input-carried, run N feeds run N+1 — the owner's truth-carry
...
public sealed record RemnantLedger(Map<UInt128, RemnantRow> Rows, Seq<RemnantRow> Admitted, Seq<RemnantRow> Retired, Seq<ContentKey> Stale,
...
public sealed partial record Remnant {
    public static Fin<RemnantLedger> Reconcile(RemnantOp op, Map<UInt128, RemnantRow> ledger, ReusePolicy policy) =>
        op.Switch(
            state:    (Ledger: ledger, Policy: policy),
            stocking: static (s, o) => Fin.Succ(Stock(o.Minted, s.Ledger, s.Policy)),
            claim:    static (s, o) => s.Ledger.Find(o.Key.Digest)
                .Filter(row => row.State.Admits(RemnantState.Reserved))
                .Match(
                    Some: row => Fin.Succ(Shift(s.Ledger, row with { State = RemnantState.Reserved, ReservedJob = Some(o.JobId) })),
                    None: () => Fin.Fail<RemnantLedger>(FabricationFault.RemnantStale(o.Key).ToError())),
            release:  static (s, o) => s.Ledger.Find(o.Key.Digest)
                .Filter(static row => row.State == RemnantState.Reserved)
                .Match(
                    Some: row => Fin.Succ(Shift(s.Ledger, row with { State = o.Consumed ? RemnantState.Consumed : RemnantState.Stocked, ReservedJob = None })),
                    None: () => Fin.Fail<RemnantLedger>(FabricationFault.RemnantStale(o.Key).ToError())),
            sweep:    static (s, o) => Fin.Succ(Audit(s.Ledger, toSet(o.Observed.Map(static k => k.Digest)))));
    ...
    public static Seq<Stock> Stockable(Map<UInt128, RemnantRow> ledger) =>
        ledger.Values.ToSeq()
    ...
    static RemnantLedger Stock(Seq<Remnant> minted, Map<UInt128, RemnantRow> ledger, ReusePolicy policy) {
        Seq<(Remnant R, Option<RemnantRow> Row)> gated = minted.Map(r => (r, Gate(r, ledger, policy)));
        Seq<RemnantRow> admitted = gated.Bind(static g => g.Row.ToSeq());
        Seq<RemnantRow> retired = gated.Filter(static g => g.Row.IsNone)
            .Map(g => new RemnantRow(g.R, RemnantState.Scrapped, KeyOf(g.R), Depth(g.R, ledger), g.R.Boundary, 0.0, None));
        Map<UInt128, RemnantRow> next = admitted.Concat(retired).Fold(ledger, static (m, row) => m.AddOrUpdate(row.Remnant.Identity, row));
        double reclaimed = admitted.Sum(static a => a.UsableAreaMm2);
        return new RemnantLedger(next, admitted, retired, Seq<ContentKey>(), reclaimed, reclaimed);
    }
    ...
    static Option<RemnantRow> Gate(Remnant r, Map<UInt128, RemnantRow> ledger, ReusePolicy policy) =>
        PolygonAlgebra.Offset(Seq(r.Boundary), -policy.InsetMm, OffsetEnds.Polygon).ToOption()
            .Bind(static loops => loops.HeadOrNone())
            .Map(usable => (Usable: usable, Area: Math.Abs(PolygonAlgebra.Area(usable)), Depth: Depth(r, ledger)))
            .Filter(g => g.Area >= policy.MinUsableAreaMm2 && g.Depth <= policy.MaxLineageDepth && Aspect(g.Usable) >= policy.AspectFloor)
            .Map(g => new RemnantRow(r, RemnantState.Stocked, KeyOf(r), g.Depth, g.Usable, g.Area, None));

    // Lineage depth walks the Parent chain through the ledger; a broken link terminates the walk —
    // depth gates REUSE admission, staleness gates the CLAIM edge, never conflated.
    static int Depth(Remnant r, Map<UInt128, RemnantRow> ledger) =>
        r.Parent.Match(
            Some: p => ledger.Find(p).Match(Some: row => 1 + Depth(row.Remnant, ledger), None: static () => 1),
            None: static () => 0);

    static RemnantLedger Audit(Map<UInt128, RemnantRow> ledger, Set<UInt128> observed) {
        Seq<RemnantRow> stale = ledger.Values.ToSeq().Filter(row => !row.State.Terminal && !observed.Contains(row.Remnant.Identity));
        Map<UInt128, RemnantRow> next = stale.Fold(ledger, static (m, row) => m.AddOrUpdate(row.Remnant.Identity, row with { State = RemnantState.Scrapped }));
        return new RemnantLedger(next, Seq<RemnantRow>(), stale.Map(static row => row with { State = RemnantState.Scrapped }),
            stale.Map(static row => row.Key), 0.0, 0.0);
    }

    static RemnantLedger Shift(Map<UInt128, RemnantRow> ledger, RemnantRow row) =>
        new(ledger.AddOrUpdate(row.Remnant.Identity, row), Seq(row), Seq<RemnantRow>(), Seq<ContentKey>(), 0.0, 0.0);
