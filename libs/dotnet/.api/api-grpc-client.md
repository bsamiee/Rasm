...
- `LanguageExt.Core`(`.api/api-languageext.md`): `Try.lift` preserves the raised `RpcException` before the shared wire decoder admits a valid detail or classifies residual `StatusCode`; a fan-in over several calls accumulates through `Validation<Error, A>` where `Fin` short-circuits.
