...
[STACKING]:
- `TYoshimura.DoubleDouble`(`Rasm/.api/api-doubledouble.md`): `ddouble` is the fixed 106-bit refine tier of the direct ladder; the `EFloat` `Interval` bracket opens the constructed-point ladder instead, where no rounded-coordinate `double` filter exists, and both ladders end at the context-free `EFloat` arithmetic carried by `Dyadic`.
