...
`TinyEXR.NET` is the pure-managed OpenEXR module the flat scanline codecs cannot reach: a V3 block-level reader and writer over tiled, mip-levelled, deep, and multi-part files with the full compression roster including DWAA, DWAB, and HTJ2K, beside a V1 whole-image facade for one-call round trips. Its `ImageProcessing` and `Spectral` folds carry transfer, tone-map, colour-matrix, LUT, resize, and wavelength-sampled channel law on the same float planes.
...
|  [15]   | `Lut3D.Data -> ReadOnlySpan<float>` / `Lut3D.Size` / `DomainMinimum` / `DomainMaximum`    | property | the parsed grid       |
...
- Tiling and levels ride `Header.Tiles`: `TileLevelMode` selects one-level, mipmap, or ripmap, and `ExrReader.ReadTile(part, tileX, tileY, levelX, levelY)` addresses a tile within a level. This is the only mip-in-one-file EXR path in the repo.
...
- The `ImageProcessing` and `Lut3D` folds admit a destination aliasing their source at the SAME start and refuse a partial overlap, so one scratch span threads a whole row pipeline; the `PixelConversion` family refuses ANY overlap outright. Every refusal is an `ArgumentException`, never a silent miscompute.
...
- `ExrResult.WouldBlock` is a protocol state, never a fault row: it lowers to a resumed read, and only the remaining `ExrResult` rows and the typed limit exceptions reach the folder's fault channel.
