...
- `Rasm.Contracts`(`libs/contracts/.api/dotnet.md`): the pinned `buf.build/grpc/csharp` remote plugin emits the `<Service>Client` into that committed assembly, binding the `CallInvoker` from `CreateCallInvoker`, so every interceptor in the chain sits under the typed stub with no generated-code edit.
