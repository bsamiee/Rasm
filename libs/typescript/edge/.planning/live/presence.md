...
- Law: heartbeat cadence is edge policy — the socket row's 25-second `ping` cadence doubles as the client beat interval, and `_LEASE` defaults size the liveness windows against it (idle at one missed beat, gone at twelve) so the two policies cannot drift apart silently; a channel overrides both through its rule row.
