...
- A `STYLE` finding is corrected only in a file that the run already edits.
...
6. The file list for a folder scope: `fd -e props -e targets -e csproj -e nuspec -e rsp . <scope>`. A prompt that names files skips the listing.
...
A path outside the repo, or a scope with no MSBuild file, returns `result: not started` with the reason.
</context_gathering>
...
6. Before an `[AP-10]` row:
   ...
   2. Run `get_public_api_surface`, then `find_references` on each public type of that project. A reference from the candidate removes the row.
7. Classify each finding with the word the catalog uses: `ERROR` or `STYLE`. A finding without `file:line` and a catalog id or error code is not a finding.
...
<evidence_rules>
- The file read decides. A probe hit without a catalog match is not a finding.
...
- One row per `file:line`. A second row on the same line merges into the first.
...
A `clean` result reports the probe set and the `-check` build that earned it. A `not started` result carries the exact error text.
</output_contract>
