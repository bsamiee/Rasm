...
- Law: message envelopes ANNOUNCE an entry and gain no authority over it. `OpLogEntry` and its codec-encoded `Payload` stay the evidence truth the corpus `tests/contracts` `OPLOG_ENTRY` entry freezes — the thirteen-slot record in declaration order under its lane's own `SnapshotCodec`, whose `messagepack` and `JsonStj` rows the parity vectors are taken over — while the `CloudEvent` an appended crossing carries projects that entry: `subject` re-renders the payload content key, `id` re-renders the dot, and every other attribute is address, trace, or handling metadata the entry already holds. Envelope columns therefore never enter the corpus preimage and never re-key an entry; a divergence between the two convicts the projection, never the ledger, because a consumer routing on attributes and a consumer replaying the feed must reach one state and only one of them holds bytes anyone signed.
- Receipt: changefeed position and queue depth ride `SyncApplyReceipt`; the projected-segment evidence rides `ReceiptSinkPort`.
- Packages: Marten (`SubscriptionBase`/`ProcessEventsAsync`/`EventRange`/`ISubscriptionController`/`IDocumentOperations`/`IChangeListener`/`NullChangeListener`/`IEvent<T>`), Rasm.Element (`GraphDelta`/`Node`/`Node.Object`/`RepresentationContentHash` + `Graph/wire#EVENT_ENVELOPE` `GraphCrossing.Mint`/`GraphEventType`/`CrossingPorts`), Rasm (`Rasm.Domain` `ContentHash.Of` — the one federation hasher; a direct `XxHash128` call site is the deleted spelling), NodaTime, LanguageExt.Core, Thinktecture.Runtime.Extensions, System.Diagnostics.DiagnosticSource, BCL inbox.
...
public sealed partial class SyncOpKind {
    public static readonly SyncOpKind Upsert = new("upsert", "upserted", tombstone: false, wholeRelation: false);
    public static readonly SyncOpKind Delete = new("delete", "deleted", tombstone: true, wholeRelation: false);
    public static readonly SyncOpKind Truncate = new("truncate", "truncated", tombstone: true, wholeRelation: true);
    public static readonly SyncOpKind Presence = new("presence", "observed", tombstone: false, wholeRelation: false);
    public bool Tombstone { get; }
    public bool WholeRelation { get; }
    // The PAST-TENSE spelling an envelope `type` carries, kept as a column rather than derived from `Key`: the
    // estate grammar fixes `<fact>` past tense and no suffix rule turns every verb here into its own past form,
    // so a row states its announcement spelling once and the mint reads it.
    public string Fact { get; }
    private SyncOpKind(string key, string fact, bool tombstone, bool wholeRelation) : this(key) => (Fact, Tombstone, WholeRelation) = (fact, tombstone, wholeRelation);
}
...
public sealed partial class OperationId {
    ...
    // The dot's OWN wire spelling, and the one thing an envelope `id` carries: origin and counter are exactly what
    // makes the operation unique within its `source`, so a consumer reading `(source, id)` reconstructs the causal
    // coordinate without a second lookup. `Key` stays the INDEX projection — hashing the dot for a fixed-width
    // column — and the two never substitute, because a digest cannot answer which origin minted the operation.
    public string Wire => string.Create(CultureInfo.InvariantCulture, $"{Origin:N}.{Counter}");
...
public static class OpLog {
    ...
    // The envelope an appended structural crossing ANNOUNCES. The SEAM mints it (`Graph/wire#EVENT_ENVELOPE`) over
    // the SAME `GraphDelta` canonical bytes the `Payload` encodes, so `subject` IS the content-key identity this feed
    // and the broker lane agree on — a Persistence-minted spelling beside the seam's would fork that identity and
    // strand every partner's dedup. `id` takes a reserved DOT rather than the content key: two peers appending the
    // identical delta share one `subject` and carry two operations, and collapsing them onto one id drops the second
    // append while reading as successful dedup. Batch dedup keys on `subject` ALONE — a re-published crossing of
    // identical content IS one announcement — and keeps the first, because the changefeed already arrives
    // sequence-ordered. The tolerance is the frame `Header`'s, the one grid the delta's measure bytes were quantized
    // against, and the trace is the frame's captured slot rather than a second ambient read.
    public static Fin<Seq<CloudEvent>> Appended(Seq<GraphDelta> deltas, Header basis, DotSource dots,
        Func<ActivityContext, TraceCarrier> carrier, ProjectionContext frame) =>
        dots.Reserve(deltas.Count) is var reserved
            ? deltas.Zip(reserved).Traverse(pair => GraphCrossing.Mint(
                    GraphEventType.Delta,
                    ContentAddress.Of(pair.Item1.ToCanonicalBytes(basis.Tolerance).Span),
                    frame.Now(),
...
|  [09]   | crossing envelope   | seam `GraphCrossing.Mint`                    | `subject` is the content key; `id` the reserved dot                |
