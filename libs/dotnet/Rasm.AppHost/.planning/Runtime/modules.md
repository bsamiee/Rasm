...
Seat law:
- `kernel-rosters` FORCES the kernel's own boot proofs — `Buckets` advice, `AnalysisVerb` coverage, `NoiseKind` coverage — and seats nothing; `FaultBand.Proof` runs one step earlier still, inside `Arm`, because it is the only proof whose subject is the COMPOSED ASSEMBLY SET the module span names and no ledger row can see. A package's own rosters do not appear here: they ride its `TelemetryContributorPort.Rosters` column and force at `InstrumentMount.Mount`, which keeps this root's kernel-and-contracts-only dependency direction intact.
...
public static class CompositionRoot {
    ...
    [
        new RootBinding.Seated("kernel-rosters", static (services, _) =>
            Seq(Buckets.Proof(), AnalysisVerb.Proof(), NoiseKind.Proof())
