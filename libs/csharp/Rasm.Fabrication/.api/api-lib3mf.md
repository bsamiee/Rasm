...
- package: `lib3mf` (vendored — no NuGet; central manifest carries the RID-native asset rows, this folder's `.csproj` the `PackageReference`)
- version: `2.5.x` (ACT-generated binding, interface version `2.5.0`)
...
[NATIVE_TOPOLOGY]:
- every `C…` handle wraps an `IntPtr` over the native `lib3mf` core; the binding marshals through `Internal.Lib3MFWrapper` and lifts native error codes via `CheckError` — the boundary owner catches the binding exception ONCE and lowers it into the `Fin` rail
