...
from rasm.runtime.observe import DEFAULT_SCOPE, Ring, ScopeKey, logger
...
class Released(Struct, frozen=True):
    ...
    points: Block[HookId]
    installs: Block[str]
    faults: Ring[BoundaryFault]
    retained: Map[HookId, Ring[Struct]]


@tagged_union(frozen=True)
class TapRow:
    tag: Literal["log", "metrics"] = tag()
    log: str = case()
    metrics: tuple[Callable[[Struct], Mapping[str, float]], str, str] = case()

    def bound(self, scope: ScopeKey) -> Tap[Struct]:
        match self:
            case TapRow(tag="log", log=owner):

                def logged(point_id: HookId, payload: Struct) -> None:
                    logger(scope).info(point_id.value, owner=owner, **structs.asdict(payload))
...
class Hooks:
    ...
    _faults: ClassVar[Map[ScopeKey, Ring[BoundaryFault]]] = Map.empty()
    _gate = RLock()
    ...
    @classmethod
    def installed[R: Struct](cls, owner: str, install: R, *, scope: ScopeKey = DEFAULT_SCOPE) -> R:
        with cls._gate:
            cls._installs = cls._installs.add(scope, cls._scoped(cls._installs, scope).add(owner, install))
        return install
    ...
    @classmethod
    def faults(cls, *, scope: ScopeKey = DEFAULT_SCOPE) -> Ring[BoundaryFault]:
        with cls._gate:
    ...
    @classmethod
    def _isolated(cls, point_id: HookId, fault: BoundaryFault, scope: ScopeKey) -> BoundaryFault:
        written = boundary(HOOKS_ISOLATED, lambda: logger(scope).warning("hook.isolated", point=point_id.value, **fault.facts()), catch=Exception)
        with cls._gate:
            window = cls._faults.try_find(scope).default_value(Ring(cap=FAULT_WINDOW))
            parked = written.map(lambda _sunk: window.park(fault)).default_with(lambda _refused: window.refused(fault))
