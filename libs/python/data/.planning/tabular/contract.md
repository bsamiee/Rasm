...
- Cases: `CheckKind` is the one predicate axis — every case maps to a concrete `pandera.Check`, so the axis never carries a non-predicate. `cmp(op, v)` folds `ge`/`le`/`gt`/`lt`/`eq`/`ne` through one `_CMP` `frozendict` (`Check.ge`/`le`/`gt`/`lt`/`equal_to`/`not_equal_to`) so six comparison arms collapse to one table lookup · `in_range(lo, hi, inc)` reads the `(include_min, include_max)` pair off one `_INCLUSIVE` `frozendict` keyed by the closed `Inclusive` literal and threads it into `Check.in_range(min_value=, max_value=, include_min=, include_max=)`, so the four endpoint-closure combinations are one table row, never four inline `inc in (...)` membership tests · `member(present, values)` folds `isin`/`notin` through one `_SET` `frozendict` (`Check.isin`/`Check.notin`) so the membership polarity is a row, never two cases · `text(op, pattern)` folds the `str`-pattern family `matches`/`contains` through one `_TEXT` `frozendict` (`Check.str_matches`/`Check.str_contains`), both taking the `str | Pattern[str]` pattern · `length(lo, hi)` threads the `int | None` bounds straight into `Check.str_length(min_value=, max_value=)`, the distinct numeric-bound case `str_length` owns (never the `str`-pattern `_TEXT` table, whose values type-mismatch a length bound) · `monotonic(dim, increasing)` threads the required `dim` column name and the `increasing` direction into `Check.is_monotonic(dim, increasing=)`, matched by `match`/`case` closed by `assert_never` into the concrete `pandera.Check`, so the IDS-style rule vocabulary is one closed switch over three tables, never a per-check builder. `unique` is the `QualityRule.unique` column flag, not a `CheckKind` case — pandera routes uniqueness to `Column(unique=)`, never a `Check`, so the predicate axis stays total over real `Check`s and `to_check` never returns `None`.
- Entry: `DataQuality.of` folds a tuple of `QualityRule` into one `DataFrameSchema`; `DataQuality.validate` runs `schema.validate(frame, lazy=lazy, sample=sample, random_state=seed)` and returns a `RuntimeRail[ContractClaim]` — `lazy=True` collects all failure cases (the `SchemaErrors.failure_cases` frame), `lazy=False` short-circuits on the first `SchemaError`, and the optional `sample` row-count bound plus its `seed` (`random_state`) restrict validation to a deterministic row subset for a large frame, the pandera large-frame sampling knob the gate mines rather than scanning the whole scan unconditionally. Both outcomes land in one `ContractClaim`; the rail is `Ok` even on validation failure because the claim records but does not enforce. The `_schema()` fold is bound once per `_validate` call and reused by both the `ContentIdentity` key and the `validate` run, never reconstructed per branch.
- Auto: a passing validation yields `ContractClaim.of("data-quality", (columns,), (), key)` with `status=PASSED`; a failing lazy validation folds the `SchemaErrors.failure_cases` failing column/check pairs into `ContractClaim.breaches`, the `FAILED` status deriving from the non-empty breach tuple through `ContractClaim.of`; the polars backend keeps the frame lazy so validation pushes into the scan.
- Receipt: `ContractClaim.contribute` emits an emitted-phase `Receipt.of` row through `ReceiptContributor` keyed by `ContentIdentity` over the schema fingerprint — it is the data-contract evidence and never replaces the typed `QueryReceipt`.
- Packages: `pandera` (`pandera.polars.DataFrameSchema`/`Column(unique=)`/`Check.ge`/`le`/`gt`/`lt`/`equal_to`/`not_equal_to`/`in_range(min_value=, max_value=, include_min=, include_max=)`/`isin`/`notin`/`str_matches`/`str_contains`/`str_length(min_value=, max_value=)`/`is_monotonic(dim, increasing=)`/`errors.SchemaError`/`SchemaErrors`), `polars` (`LazyFrame`, `TYPE_CHECKING`-only since `polars` is on `banned-module-level-imports`; the runtime frame arrives pre-lowered through `narwhals`), runtime (`RuntimeRail`/`ContentIdentity`/`ReceiptContributor`).
...
_SET: Final[frozendict[bool, Callable[[Iterable[Any]], Check]]] = frozendict({True: Check.isin, False: Check.notin})
_TEXT: Final[frozendict[str, Callable[[str | Pattern[str]], Check]]] = frozendict({
...
class ContractClaim(Struct, frozen=True):
    ...
    def contribute(self) -> Receipt:
        ...
        return Receipt.of("emitted", self.subject, f"{self.subject}[{scope}]", {"status": self.status, "breaches": str(len(self.breaches))})


@tagged_union(frozen=True)
class CheckKind:
    tag: Literal["cmp", "in_range", "member", "text", "length", "monotonic"] = tag()
    cmp: tuple[Literal["ge", "le", "gt", "lt", "eq", "ne"], float] = case()
    in_range: tuple[float, float, Inclusive] = case()
    member: tuple[bool, tuple[Any, ...]] = case()
    text: tuple[Literal["matches", "contains"], str] = case()
...
class DataQuality(Struct, frozen=True):
    ...
    def validate(self, frame: pl.LazyFrame, *, lazy: bool = True, sample: int | None = None, seed: int | None = None) -> "RuntimeRail[ContractClaim]":
        return boundary("quality.validate", lambda: self._validate(frame, lazy, sample, seed))

    def _validate(self, frame: pl.LazyFrame, lazy: bool, sample: int | None, seed: int | None) -> ContractClaim:
        schema = self._schema()
        key = ContentIdentity.of("schema", repr(schema).encode())
        try:
            schema.validate(frame, lazy=lazy, sample=sample, random_state=seed)
...
class FieldShape(Struct, frozen=True):
    ...
    def resolve(self, live: "frozendict[str, FieldShape]") -> str:
...
class FrameAdmission(Struct, frozen=True):
    ...
    def enforce(self, admitted: AdmittedFrame) -> RuntimeRail[ContractClaim]:
        ...
        return self.interop.translate(admitted.frame, Backend.POLARS).bind(lambda lowered: self.quality.validate(lowered.lazy()))

    def _resolve(self, frame: Any, shapes: tuple[FieldShape, ...]) -> RuntimeRail[AdmittedFrame]:
        live = frozendict({s.field: s for s in shapes})
        breach = ", ".join(token for s in self.required if (token := s.resolve(live)))
        admitted = AdmittedFrame(frame=frame, backend=self.interop.source, shapes=shapes)
        return boundary("frame.admit", lambda: admitted) if not breach else Error(BoundaryFault(boundary=("frame.admit", breach)))
```
...
type CovenantOutcome = ContractClaim | bool | bytes
...
class FrameCovenant(Struct, frozen=True):
    ...
    schemas: "frozendict[str, type[dy.Schema]]"
    ...
    def of(cls, interop: FrameInterop, schemas: "Mapping[str, type[dy.Schema]]", *edges: RelationEdge, validation: Validation = "warn", cast: bool = False) -> "FrameCovenant":
        return cls(interop=interop, edges=edges, schemas=frozendict(schemas), validation=validation, cast=cast)

    def run(self, op: CovenantOp) -> "RuntimeRail[CovenantOutcome]":
        return self._lower(_payload(op)).bind(lambda data: boundary(f"covenant.{op.tag}", lambda: self._dispatch(op, data)))

    def _dispatch(self, op: CovenantOp, data: "dict[str, pl.LazyFrame]") -> CovenantOutcome:
        import dataframely as dy  # noqa: PLC0415

        with dy.Config(max_failure_examples=self.failure_examples) if self.failure_examples is not None else _passthrough():
            match op:
                case CovenantOp(tag="prove", prove=members):
                    return self._claim(members, data)
                case CovenantOp(tag="consistent", consistent=members):
                    return self._collection(members).is_valid(data, cast=self.cast)
                case CovenantOp(tag="restrict", restrict=(anchor, members)):
                    ...
                    return self._claim(members, _members(restricted))
                case CovenantOp(tag="extend", extend=runs):
                    return self._claim(
                        tuple(m for run in runs for m in run),
                        dy.concat_collection_members([self._collection(run).cast({m.name: data[m.name] for m in run}) for run in runs]),
                    )
                case CovenantOp(tag="persist", persist=(ContractIo.FRAMES, directory, members)):
                    collection = self._collection(members)
                    collection.validate(data, cast=self.cast).write_parquet(directory)
                    return self._claim(members, _members(collection.scan_parquet(directory, validation=self.validation)))
                case CovenantOp(tag="persist", persist=(ContractIo.CONTRACT, _, members)):
                    return self._collection(members).serialize().encode()
                case CovenantOp(tag="sample", sample=(rows, members)):
                    return self._claim(members, _members(self._collection(members).sample(rows, generator=self.generator)))
                case unreachable:
                    assert_never(unreachable)
    ...
    def _lower(self, members: "tuple[CovenantMember, ...]") -> "RuntimeRail[dict[str, pl.LazyFrame]]":
        return traversed(
            Block.of_seq(self.interop.translate(m.frame, Backend.POLARS).map(lambda lowered, _name=m.name: (_name, lowered.lazy())) for m in members),
            by=Disposition.ABORT,
        ).map(dict)

    def _claim(self, members: "tuple[CovenantMember, ...]", data: "dict[str, pl.LazyFrame]") -> ContractClaim:
        result = self._collection(members).filter(data, cast=self.cast, eager=True)
        breaches = tuple(
            row
            for name, failure in _failures(result).items()
            for row in (
                ...
                *((name, "detail", column, str(failure.details.filter(failure.details[column] == "invalid").height)) for column in failure.details.columns if column not in failure.invalid.columns),
                *(((name, "invalid", "rows", str(failure.invalid.height)),) if failure.invalid.height else ()),
            )
        )
        return ContractClaim.of("data-covenant", (len(self.schemas), len(self.edges)), breaches, ContentIdentity.of("covenant", tuple(m.content_key for m in members)))


def _payload(op: CovenantOp) -> "tuple[CovenantMember, ...]":
    match op:
        case CovenantOp(tag="prove", prove=members) | CovenantOp(tag="consistent", consistent=members):
            return members
        case CovenantOp(tag="restrict", restrict=(_, members)) | CovenantOp(tag="persist", persist=(_, _, members)) | CovenantOp(tag="sample", sample=(_, members)):
            return members
        case CovenantOp(tag="extend", extend=runs):
...
def _passthrough() -> "AbstractContextManager[None]":
    return nullcontext()


def _failures(result: "dy.CollectionFilterResult") -> "Mapping[str, dy.FailureInfo]":
...
- [DATAFRAMELY_COVENANT_SURFACE]: the `dataframely` `dy.Collection`/`dy.LazyFrame[S]` member annotation/`dy.CollectionMember(ignored_in_filters=, propagate_row_failures=, inline_for_sampling=)`/`dy.filter()`/`Collection.validate(data, cast=, eager=)`/`Collection.filter`/`Collection.is_valid`/`Collection.cast`/`Collection.join(primary_keys, how="semi", maintain_order=)`/`Collection.collect_all`/`Collection.member_schemas`/`Collection.common_primary_key()`/`Collection.sample(num_rows, generator=)`/`Collection.write_parquet(directory)`/`Collection.scan_parquet(directory, validation=)`/`Collection.serialize() -> str`/`deserialize_collection`/`concat_collection_members(collections)`/`require_relationship_one_to_one(lhs, rhs, on=, drop_duplicates=)`/`require_relationship_one_to_at_least_one`/`Config(max_failure_examples=)`/`FailureInfo.invalid`/`FailureInfo.counts()`/`FailureInfo.cooccurrence_counts()` members the `FrameCovenant` owner binds are catalogue-confirmed against the folder `dataframely` `.api` (`require_relationship_*`/`concat_collection_members` are catalogue-confirmed bare top-level `dataframely` exports, so the `getattr(dy, RelationCardinality.value)` builder resolution and `dy.concat_collection_members` resolve directly); `require_relationship_*` returns the keep-set `pl.LazyFrame`, the `Collection.{filter,validate,is_valid,cast}` family threads the one `self.cast` dtype-coercion policy as the catalogue-confirmed `cast=` call row, the `Collection.join` semi-restriction returns the filtered `Self`, `Collection.common_primary_key()` returns the `list[str]` the `Restrict` anchor projection selects (so the cross-member key derives off the covenant, never a smuggled `keys` parameter), `Collection.sample(num_rows, *, generator=None)` returns the synthetic collection `Self` the `Sample` arm projects back through `_members`, `Collection.serialize()` returns the contract `str` the `Persist`/`Contract` arm encodes and `deserialize_collection` restores, the `Validation` literal `"allow"`/`"warn"`/`"skip"` is the catalogue-confirmed `scan_parquet(validation=)` policy the `Persist`/`Frames` arm threads, `Config` is the catalogue-confirmed context manager whose `max_failure_examples` override bounds the captured `FailureInfo` examples over the whole dispatch (the `_passthrough` `nullcontext` is the plain-CPython no-op arm when `failure_examples is None`), `concat_collection_members` takes collection instances and returns the member→`pl.LazyFrame` map, `CollectionMember` carries the per-member behavior axes the `MemberPolicy.member()` projection folds, `FailureInfo.invalid` is the rejected-row frame whose `.height` totals the absolute breach count, `write_parquet`/`scan_parquet` embed and re-validate the serialized member contract, and the record-don't-enforce `filter`-over-`validate` split are settled fence code.
- [DATAFRAMELY_CONFIG_CAP]: `dy.Config` is the catalogue-confirmed sampling/failure-example cap context manager (folder `dataframely` `.api`, `Config(**options)` / `Config.set_max_failure_examples(n)` / `Config.set_max_sampling_iterations(n)` / `Config.restore_defaults()`); the `_dispatch` fold opens `dy.Config(max_failure_examples=self.failure_examples)` over the whole verb when `failure_examples` is set and `_passthrough` `nullcontext` otherwise. Whether the `max_failure_examples` cap binds through the `Config(**options)` constructor kwarg directly (versus requiring the explicit `Config.set_max_failure_examples(n)` setter inside the context) is unverified against the live distribution, so the `Config(max_failure_examples=)` constructor-kwarg spelling is the one marked seam and stays a RESEARCH item until the constructor signature is confirmed; the context-manager capability and the `nullcontext` no-op arm are settled.
- [DATAFRAMELY_COLLECTION_CONSTRUCTION]: the `FrameCovenant._collection` fold declares each member as `Annotated[dy.LazyFrame[Schema], MemberPolicy.member()]` — the documented member-field form fusing the `dy.CollectionMember` per-member policy onto the `dy.LazyFrame[S]` annotation, never a parallel class-attribute split off the annotation — and the `dy.filter()`-decorated edge methods through a runtime `__annotations__`/attribute namespace dict synthesized into `type("Covenant", (dy.Collection,), namespace)`; the `Annotated[dy.LazyFrame[S], dy.CollectionMember(...)]` member-field shape and the `dy.CollectionMember` keyword names (`ignored_in_filters`/`propagate_row_failures`/`inline_for_sampling`) are catalogue-confirmed against the folder `dataframely` `.api`. Whether `dataframely`'s `_native`-backed metaclass admits this `Annotated` member-field namespace plus the `dy.filter()` edge methods through the runtime-constructed `type(...)` namespace dict (versus requiring a literal `class` body the metaclass reads at lexical definition) is the one unverified mechanism and stays a marked RESEARCH item, never settled fence code, until confirmed against the live metaclass.
- [DATAFRAMELY_FILTER_RESULT]: `Collection.filter(data, *, cast=, eager=True)` returning a `CollectionFilterResult[Self]` carrying per-member `FailureInfo` is catalogue-confirmed against the folder `dataframely` `.api`; the exact per-member accessor the `_failures` seam reads (`result.failures` as a `Mapping[str, FailureInfo]` versus an `invalid()`/`failure_info()` method) is unverified against the live distribution, so the `_failures` function is the one marked seam carrying that unverified accessor and stays a RESEARCH item until the `CollectionFilterResult` member shape is confirmed.
- [DATAFRAMELY_COLLECTION_MEMBERS]: `Collection.member_schemas()` returning the member-name-to-schema map is catalogue-confirmed against the folder `dataframely` `.api`; the `_members` seam reads each restricted member off the `collect_all()` collection by `getattr(collection, name)` over those member names, but whether a collected `Collection` exposes each member as a `pl.LazyFrame` attribute under its declared name (versus a `to_dict()`/`members()` accessor) is unverified against the live distribution, so the `_members` function is the one marked seam carrying that unverified projection and stays a RESEARCH item until the collected-member accessor is confirmed. The `Restrict`, `Sample`, and `Persist`/`Frames` arms all read the collected, sampled, and re-scanned members through this one `_members` seam (`collect_all`/`sample`/`scan_parquet` each return the collection `Self`), so confirming the accessor settles all three.
- [DATAFRAMELY_FAILURE_DETAILS]: `FailureInfo.details` is a catalogue-confirmed property (folder `dataframely` `.api`) returning a `pl.DataFrame` of the invalid input rows extended by one per-rule column whose cell value is `"valid"`/`"invalid"`/`"unknown"`; the `_claim` `"detail"` breach row reads which column drove each rejection by `failure.details.filter(failure.details[column] == "invalid").height` over the per-rule columns isolated as `failure.details.columns` minus `failure.invalid.columns`. The cell-value vocabulary is catalogue-confirmed, but the exact `details`-frame layout the fold reads — the per-rule columns appended beside the verbatim invalid-row columns under their rule names, and the `"invalid"` string literal as the rejecting cell value (versus an enum/boolean encoding) — is unverified against the live distribution, so the `"detail"` breach arm is the one marked seam and stays a RESEARCH item until the `FailureInfo.details` frame shape is confirmed; the `"rule"`/`"co-occur"`/`"invalid"` breach arms over `counts()`/`cooccurrence_counts()`/`invalid.height` remain settled fence code.
