...
- Law: every refusal emits before it fails — `_refuse` is the one fail entry, incrementing `Convention.metric.admitRefused` tagged by the reason the fault already carries, so the refusal fan needs zero call-site wiring and a new reason joins the series the moment its row lands; `Gate.refuse` publishes that same entry to the serving route module, so a refusal minted at the entry counts itself through the one fold rather than a second `Effect.fail`.
...
[CURRENT_ROWS]:
- Owner: `Current` — ambient request `Context.Reference` rows for the mark, negotiated locale, and one admitted credential. The general edge provides header/cookie admission; a protocol-specific entry may replace the same seat.
...
- Law: `Current.Admitted` is the credential lift's one seat — the global edge resolves header and cookie credentials once, while the webhook's route-scoped query carriage lands the same `Admitted` value here after `Authn.webhook` resolves it. Scheme arms project this seat instead of re-verifying.
...
- Law: `Authn.webhook(identity, headers, url)` is the sole CloudEvents Webhook token owner. It admits either the `Authorization: Bearer` carriage already lifted by the global edge or exactly one non-empty `access_token` query carriage through the same `_verified` path. Both carriages together always refuse, even when their bytes match; repeated query tokens refuse; and an API-key or cookie cannot substitute for or accompany either required method. The query token never becomes a synthetic header, and its result returns the carriage so the route can stamp `private` on a successful response.
...
- Law: the admission plane measures itself off its own partitions — `admitPassed` counts each lift tagged by the `via` scheme so the refusal series has a denominator, `admitRefused` fans on the `GateFault` reason at the one `_refuse` entry, and `idempotencyOutcome` fans on the bracket's own three-way fold; every instrument mounts from its `Convention` row, so this page carries no bucket ladder and no constructor pick.
- Law: `RpcAuthn` is the same admission on the RPC arm — `RpcMiddleware.Tag` with `failure: GateFault`, `provides: Principal`, `requiredForClient: true`, and `wrap: true`, so one definition governs both ends: the wrap reads the frame headers, composes the same `Authn.admit` lift, provides the same `Principal` into `next`, and binds `TenantScope` around it exactly as the HTTP edge does. `requiredForClient` is what makes `Emit.caller` refuse to derive a credential-less client, and `RpcMiddleware.layerClient` is the client arm the app root supplies; a `Contribution.rpc` group scoping this Tag through `.middleware` cannot ship unauthenticated by omission.
