...
[HOOK_STATE_CHANNEL]:
- Declaring the factory hook with a non-void return type rewires the generated spine: the generator detects the return type (`GetValidateFactoryArgumentsReturnType`), captures the hook's return value into `factoryArgumentsValidationError`, and forwards it to `FactoryPostInit(<returnType> factoryArgumentsValidationError)`. The result is a typed state channel from admission computation to post-construction initialization — parse once in the hook, consume the parsed artifact after the instance exists, with zero re-computation and no static side channel:
...
- The non-void hook declaration changes to `private static partial` in the generated defining declaration, so the implementing half must also be private — the channel is invisible outside the owner by construction.
...
- The degradation map on instance miss is asymmetric and silent on the hot paths: the applicative apply or action fallback keeps only the first failure; the success-collecting combination keeps the first failure; the alternative-combination fallback keeps the last failure. Only the identity operations (alternative empty, monoid-unit contexts) fail loud, with an unsupported-operation throw naming the missing monoid. The only reliable contract is to give the fault family its trait instances or use the error base as the failure currency.
...
- The runtime gate therefore belongs to the seam that receives possibly-defaulted storage: infrastructure detecting `IDisallowDefaultValue` inspects the key member directly (null for reference keys, zero-pattern for value keys) and rejects with a typed fault before the ghost crosses inward. Reading the key beats calling equality against a default instance, because generated equality routes through key comparers whose null handling is comparer-specific.
- Deserializers and materializers that construct structs without factories are the dominant ghost source; routing them through the declared admission source type (factory-based wire conversion) closes the hole at definition time, which is the stronger fix — the runtime gate handles storage shapes that cannot be rerouted.
