...
- Law: every case in the family has a minting seam in its owning surface — a fact the runtime already classifies (a worker death, a contract violation) weaves the runtime's own conversion instead of minting a local case no seam reaches, and a case nothing mints is deleted, not diagrammed.
