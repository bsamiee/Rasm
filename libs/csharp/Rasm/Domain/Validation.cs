    ...
    private sealed partial class Check {
        ...
        internal Fin<Unit> Apply(Context context, GeometryBase geometry, CancellationToken cancel) =>
            cancel.IsCancellationRequested switch {
                ...
                false => applies(arg: geometry) ? run(arg1: this, arg2: context, arg3: geometry, arg4: cancel) : Demand(geometry: geometry, condition: true, log: string.Empty),
            };
    }
}
...
internal static class OpAcceptance {
    ...
    internal static Option<bool> ValidityOf(object? source) =>
        source switch {
            ...
            bool or int or Enum or SurfaceCurvature or MeshCheckParameters or RayQuery or ISmartEnum<int> or ISmartEnum<string> => Some(true),
            ...
            Stat s => Some(s is { Count: > 0, Minimum: double minimum, Maximum: double maximum, Mean: double mean, Variance: double variance, Rms: double rms } && RhinoMath.IsValidDouble(minimum) && RhinoMath.IsValidDouble(maximum) && RhinoMath.IsValidDouble(mean) && RhinoMath.IsValidDouble(variance) && RhinoMath.IsValidDouble(rms) && minimum <= maximum && variance >= 0.0 && rms >= 0.0 && (s.Context switch { StatContext.ToleranceCase t => RhinoMath.IsValidDouble(t.Value) && t.Value >= 0.0 && t.WithinTolerance == (Math.Max(val1: Math.Abs(value: minimum), val2: Math.Abs(value: maximum)) <= t.Value), _ => true })),
            Distribution d => Some(ValidityOf(source: d.Summary).IfNone(false) && RhinoMath.IsValidDouble(d.Median) && RhinoMath.IsValidDouble(d.Iqr) && d.Iqr >= 0.0 && d.Percentiles.ForAll(static p => RhinoMath.IsValidDouble(p.Percentile) && p.Percentile is >= 0.0 and <= 100.0 && RhinoMath.IsValidDouble(p.Value))),
