-- One transition on a finding at its current path and hash, :finding_id the row, :state and :by from the states table, :evidence its evidence or null, :verdict a bar_verdict row on the verifier's confirmed or null, run from the worktree
