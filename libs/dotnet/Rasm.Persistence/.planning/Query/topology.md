...
- Law: composing the boundary view REPLACES a per-model incremental patcher with a per-snapshot demand memo. NAMED LOSS: `Advance`'s `O(delta)` clone-and-patch of every already-materialized filter/orientation view, which re-used the previous snapshot's edges instead of re-scanning. WITNESS: `Advance` is now `Graph.Apply(delta, key).Map(next => Of(Model, next))`, and the next snapshot's first `View(filter, orientation)` pays one `O(V+E)` scan per DEMANDED row pair — a second incidence structure keyed to this lane is exactly what the boundary's own ban names, and a patcher obliged to stay bit-identical to the boundary builder across every new `EdgeFilter` row is the drift the memo deletes.
