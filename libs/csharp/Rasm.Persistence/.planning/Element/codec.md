...
- Entry: `Serialize(Type shape, object? value)` is the pure byte transform dispatching shape-discriminated through source-generated metadata; `Negotiate(WireSurface surface, Seq<string> accepted)` resolves the highest mutually-supported codec a consumer admits, railing `#SNAPSHOT_SPINE` `CodecFault.NoMutualCodec` (8310) rather than a bare `Error.New`.
