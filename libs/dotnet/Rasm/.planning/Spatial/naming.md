...
- [02]-[TOPO_NAMING]: one `Naming.Apply(NamingOp)` entry over the `TopoName` lineage algebra; the `NameTable` generation registry of `TopoName`-keyed lineage entries; overlap-fraction migration under `NamingPolicy`; the accumulating injectivity audit.
...
public static class Naming {
    public static Fin<NameTable> Apply(NamingOp op) {
        return op.Switch(
            state: minted,
            track: static (k, t) =>
                (Admit.Value(t.Prior).ToValidation(), Admit.Value(t.Rebuilt).ToValidation())
            ...
            resolve: static (k, r) => Admit.Value(r.Boundary)
                >> (boundary => Anchor(
