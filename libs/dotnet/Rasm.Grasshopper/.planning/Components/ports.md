...
- Boundary: a host `Add*` marked `[Obsolete]` is never suppressed — the row mints its parameter and attaches it through the public `InputAdder.Add(IParameter, Requirement)` entry, which is the same declaration with no diagnostic to silence.
...
- Owner: `Ports` is the one declaration fold — side selection is the adder argument's static type, every plan folds through the accumulating carrier so a malformed pin roster reports every violation at once, and each minted `IParameter` realizes its trim and persistent data on the same fault channel.
