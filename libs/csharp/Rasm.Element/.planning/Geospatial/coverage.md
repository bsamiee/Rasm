...
// seed-zero store. The OverviewLevel row shape, mirrored onto the time axis.
[Equatable]
...
public sealed partial record CoverageGrid {
 ...
 public GridDescriptor Grid { get; }
 [UnorderedEquality] public Seq<CoverageBand> Bands { get; }
 public GeoReference Crs { get; }
 [UnorderedEquality] public Seq<OverviewLevel> Overviews { get; }
 [UnorderedEquality] public Seq<TimeSlice> Slices { get; }
 public int BaseBlockX { get; }
 public int BaseBlockY { get; }

 // PRIVATE ctor + GET-ONLY members (the AssessmentPayload shape): Of is the ONLY public admission, so a degenerate
 // grid, a decode-degenerate band, a hollow palette, a shuffled pyramid, or a non-monotone timeline is
 // UNREPRESENTABLE — a positional public ctor beside the gates was the bypass that let unadmitted grid state reach
 // Sample/Window/LevelFor; a wire or persistence decoder re-admits through the SAME railed Of (the
 // ContentAddress.Verify distrust posture), and no init/set survives for a `with`/object-initializer to re-open an invariant.
 private CoverageGrid(
  CoverageKind kind, UInt128 rasterKey, GridDescriptor grid, Seq<CoverageBand> bands, GeoReference crs,
 ...
 // ACCUMULATE (VALIDATION_MONOID): each is one K-kinded K<Validation<Error>, Unit> slot (the [ADMISSION_SLOTS]
 // compile contract — the shipped tuple Apply binds only on (K<F,A>, …) receivers, so the fold state and every
 // slot carry the kind and .As() re-anchors after each join), the fold's Apply unions every
 ...
 // through and poison LevelFor); the slice gate enforces the stored timeline strictly increasing (the same
 // Zip-adjacent monotone law the pyramid holds), so SliceAt's latest-at-or-before fold is total over stored order.
 public static Fin<CoverageGrid> Of(
  ...
  Seq(Gate(!grid.IsDegenerate, key, $"<coverage-grid-degenerate:{grid.Width}x{grid.Height}/det:{grid.Determinant}>"),
   ...
   .Fold((K<Validation<Error>, Unit>)Success<Error, Unit>(unit), static (acc, gate) => (acc, gate).Apply(static (_, _) => unit).As())
   .As()
 ...
 // Validation lift behind the K upcast — the K-kinded return is what lets the fold's (K, K) tuple Apply bind).
 // Independence rides ACROSS slots and the fold's Apply owns the union; a dependent check would
 // bind INSIDE its slot — none of these gates needs to.
 ...
 public Fin<Option<OverviewLevel>> LevelFor(double targetCellSize, Op key) =>
  double.IsFinite(targetCellSize) && targetCellSize > 0.0
   ? Fin.Succ(Overviews
      .Filter(o => o.CellSize <= targetCellSize)
      .Fold(Option<OverviewLevel>.None, (best, o) =>
       best is { IsSome: true, Case: OverviewLevel current } && current.CellSize >= o.CellSize ? best : Some(o)))
   : ElementFault.ValueRejected(key, $"<coverage-target-cell-size-non-finite-or-non-positive:{targetCellSize:R}>");
 ...
 internal Option<(int Col, int Row, int SpanCol, int SpanRow)> WindowAdmitted(double x0, double y0, double x1, double y1, Option<OverviewLevel> level = default) {
  (int w, int h) = level is { IsSome: true, Case: OverviewLevel selected }
 ...
 public void CanonicalBytes(CanonicalWriter w) {
  ...
  foreach (OverviewLevel o in Overviews.OrderBy(static x => x.CellSize)) { o.CanonicalBytes(w); }   // the ONE admitted resolution key — Of gates strictly-increasing CellSize, so equality and canonical bytes share it
...
- [TEMPORAL_AXIS]: the environmental fields a site-context route consumes are time-VARYING — hourly/monthly solar irradiance, seasonal groundwater and flood levels, diurnal noise contours, climate-projection scenarios — delivered as time-sliced raster stacks (NetCDF, time-enabled COG) where every slice shares one grid and band schema and differs only by its instant; the `TimeSlice` run mirrors the `OverviewLevel` row shape onto the time axis (an `Instant` anchor + a per-slice content-keyed `RasterKey` in the same seed-zero store), `Of` enforces the stored timeline strictly increasing (the pyramid's `Zip`-adjacent monotone law), `SliceAt(instant)` is the step-function governing-slice read (the latest at-or-before — an hourly value holds until its successor), the railed `SliceWindow(from, to, key)` rejects reversed bounds before clipping the interval a seasonal fold composes with the per-slice fetch, and `CanonicalBytes` folds the count-prefixed instant-ordered run (each slice's `Instant` through the writer's fixed-width `I64` Unix-tick canon) so a re-sliced or re-anchored timeline forks the coverage identity; the pyramid and the timeline are ORTHOGONAL axes both metadata-selected before any byte moves — `LevelFor(targetCellSize, key)` picks the resolution over the base raster, `SliceAt` picks the slice whose full-resolution blob the fetch reads; a dataset shipping per-slice overviews lands them as one `TimeSlice` column (the growth law), never a parallel temporal-pyramid family.
