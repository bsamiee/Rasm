...
const APISTACK_SUBSTRATE = [
    ...
    'Cite ONLY members confirmed in the actual .api catalog file; a member you cannot verify is a PHANTOM to delete (verify novel members via ' +
        '`UV_CACHE_DIR=.cache/uv uv run python -m tools.assay api` — the cache prefix is load-bearing in a codex sandbox, where the default uv cache ' +
        'sits outside the workspace; assay blocked or unavailable, the package`s official surface via Context7/exa/tavily owns the fallback). An ' +
