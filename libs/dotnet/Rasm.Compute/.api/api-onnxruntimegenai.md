...
`Microsoft.ML.OnnxRuntimeGenAI` owns the Compute model pipeline's generative token-streaming runtime over a LIFO native-handle chain, and `Microsoft.Extensions.AI.Abstractions` supplies the `IChatClient` contract the built-in `OnnxRuntimeGenAIChatClient` composes over that same chain for the M.E.AI streaming consumer. Structured output constrains in the native layer through `SetGuidance` rather than managed-side, generated token buffers stage through the `api-recyclable-stream` pool, and both feed the model pipeline's `GENERATIVE_RUN` cluster. This page is HOST-LOCAL and carries no TS_PROJECTION.
...
|  [10]   | `OnnxRuntimeGenAIException` | class         | sole generative fault channel                           |
...
- `OnnxRuntimeGenAIException` is the sole fault channel at `Model`/`Config` construction and across the generation loop; a missing or malformed model directory faults it at construction.
...
- `OnnxRuntimeGenAIChatClient` is `sealed : IChatClient, IDisposable` over three ctors — path, `(Model, ownsModel)`, `(Config, ownsConfig)` — with no `(Model, options)` two-arg form, the `owns*` flag governing whether the client disposes the handle, and `OnnxRuntimeGenAIChatClientOptions.PromptFormatter` the override point for `Tokenizer.ApplyChatTemplate` when the M.E.AI consumer owns prompt assembly.
...
- `SetRuntimeOption` validates nothing managed-side: an unrecognized key is accepted SILENTLY, and `terminate_session` = `1` mid-drain ABORTS the process — an uncatchable native abort no managed channel observes — so a consumer admits runtime keys against a banned set rather than passing them through.
...
- `Generator.TokenCount()` is one batch-wide scalar, and `RewindTo` on a batch wider than one admits only `0` while a restart faults, so a multi-sequence run has no rewind or restart path at all.
