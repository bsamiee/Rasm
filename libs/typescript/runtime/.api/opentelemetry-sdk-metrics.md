...
- `MeterProvider` owns the entire view engine: `ViewOptions` selection, attribute processors, aggregation choice, and `aggregationCardinalityLimit` execute inside its shared state at instrument-storage time, and `MetricReaderOptions`' three selectors are consulted from the same path. Producers installed through `MetricReader.setMetricProducer` — or listed on `metricProducers` — bypass every one of them: `MetricReader.collect` concatenates producer `ScopeMetrics` onto the SDK's own result and hands the union straight to the exporter, so producer-supplied points keep whatever descriptor, scope, attributes, and `aggregationTemporality` the producer stamped. Compositions governing producer-sourced metrics project them themselves.
- `setMetricProducer` seats exactly one producer per reader and the interface REQUIRES implementations to throw on a second call, and `new MeterProvider({ readers })` spends that seat on every reader it takes. Running both planes — a registered producer beside a raw provider — therefore mints two reader instances from one construction spelling rather than sharing an instance, and the shared spelling is what keeps their cadence, exporter, temporality, and ceilings identical.
- `ViewOptions` selection narrows to what the consuming plane can see: `instrumentName` globs and `instrumentUnit` match a collected `MetricDescriptor`, which carries `{ name, description, unit, valueType }` alone, while `instrumentType`, `meterName`, `meterVersion`, and `meterSchemaUrl` resolve against instrument-storage state no collected snapshot carries. Spelling one of the latter against a producer-side projection selects nothing.
- Exemplars are unreachable at this pin: the `ExemplarFilter` family and both reservoir rows exist under `exemplar/` yet appear on no export of the package's `types` entry point and are referenced by no aggregator, storage, view, reader, or provider path in the shipped build; no option field accepts a filter and the package reads no `OTEL_METRICS_EXEMPLAR_FILTER` row. Metric-to-trace click-through therefore has no SDK-side producer here.

[STACKING]:
- `@effect/opentelemetry`(`.api/effect-opentelemetry.md`): `Metrics.layer`/`Metrics.registerProducer` call `MetricReader.setMetricProducer` with a producer draining Effect `Metric` values, and `NodeSdk`/`WebSdk` `Configuration.metricReader` takes exactly that `MetricReader` (or a readonly array). No `MeterProvider` exists on that path, so every view row, reader selector, and cardinality limit is inert for Effect-minted metrics and the facade's producer stamps its own scope, `CUMULATIVE` temporality, and explicit-bucket histograms; a composition needing governance over those series wraps the producer between `Metrics.makeProducer` and `Metrics.registerProducer`.
