...
from tests.python.testkit.spec import (
    absorbing,
    assert_close,
    assert_error,
    assert_error_status,
...
if TYPE_CHECKING:
    ...
    from tests.python.testkit.spec import RowCarrier
...
def test_projection_matrix_prefers_oracle_and_names_the_breach() -> None:
    ...
    cases = [ProjectionCase(label="derived", intent=4, supported_out=None, oracle=lambda n: n * 2, unsupported_out=None), ProjectionCase(label="static", intent=3, supported_out=6, oracle=None, unsupported_out=None)]
...
def test_matrix_folds_scope_rows_through_a_carrier_and_stop_without_one(subtests: RowCarrier) -> None:
    ...
    projection_matrix([ProjectionCase(label="off", intent=3, supported_out=7, oracle=None, unsupported_out=None)], project=lambda n: n * 2, subtests=recorder)
