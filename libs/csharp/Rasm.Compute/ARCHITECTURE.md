...
Solver            ⇄  csharp:Rasm/Spatial/index             # [SHAPE]: SpatialIndex.ToAcceleration BVH/octree node arrays into the GPU/parallel acceleration structure; Processing/solver consumes the Compute LM/constraint Solver rail
Solver/clash      ←  csharp:Rasm/Spatial/index             # [WIRE]: AccelerationStructure DECODED node-link wire (Bounds/Nodes) ToAcceleration emits via NodeLinkProjection; Compute decodes through ClashScale.NodeLinkPairs (the BvhPairs/OctreePairs collapse) and never Build/Refit/persists (the SpatialIndex seam owner); CLASH_GOLDEN 160-byte fixture byte-identical on both sides
