...
| [INDEX] | [SURFACE]              | [CALL_SHAPE]                                                                  | [CAPABILITY]                                                    |
| :-----: | :--------------------- | :--------------------------------------------------------------------------- | :-------------------------------------------------------------- |
|  [01]   | `Page` box read attrs  | `Page.mediabox` / `cropbox` / `trimbox` / `bleedbox` / `artbox -> Rect`       | read the five page boxes (unrotated, PDF coordinate space); `mediabox_size -> Point` and `cropbox_position -> Point` give sheet size and crop origin |
|  [02]   | `Page.set_mediabox`    | `set_mediabox(rect: Rect)`                                                    | set the physical sheet box (resets the other boxes to it — call FIRST) |
|  [03]   | `Page.set_cropbox`     | `set_cropbox(rect: Rect)`                                                     | set the visible-region box (clamped to MediaBox)               |
|  [04]   | `Page.set_trimbox`     | `set_trimbox(rect: Rect)`                                                     | set the finished-trim box — the sheet's cut line, inside the bleed |
|  [05]   | `Page.set_bleedbox`    | `set_bleedbox(rect: Rect)`                                                    | set the bleed-extent box — the printed-past-trim region the press fold reads |
|  [06]   | `Page.set_artbox`      | `set_artbox(rect: Rect)`                                                      | set the meaningful-content box                                 |
