...
- units: every strength/modulus is a `UnitsNet.Pressure`, every strain/factor a `UnitsNet.Ratio`, every dimension a
  ...
  parabola-rectangle `Exponent`, a dimensionless shape parameter). The `UnitsNet` family-gate (`libs/csharp/.api/api-unitsnet.md`)
...
- units seam: a computed section property (`api-vividorange-sections-sectionproperties.md`) and a material strength are
  the SAME `UnitsNet` quantity type (`Pressure`/`Area`/`Length`/`Ratio`, `libs/csharp/.api/api-unitsnet.md`) — a design check folds the
