...
|  [04]   | contract reshape     | one corpus emission + attach-time generation                        | per-seam compatibility checks  |
...
public sealed record ContractGeneration(string Family) {
    ...
    public string Subject => $"{Estate}{Family}";
...
- Law: no STJ schema export pins a contract — `JsonSchemaExporter` output is never compared, hashed, or advertised between peers; the contract generation compared at attach is the whole compatibility surface, so a schema-hash pin beside it is the foreclosed second authority.
