...
├── Rasm.Compute/        # APP-PLATFORM — measured execution: tensor/numeric/model lanes, the suite wire vocabulary, the field/geometry codecs + tessellation bridge, the solver/optimizer lane, the units boundary, the receipt union
├── Rasm.Persistence/    # APP-PLATFORM — durable stores: store profiles, object-store/server tiers, schema/query rails, snapshots, cache indexes, sync, version-control, federation, provenance, annotation, catalog, schedule
