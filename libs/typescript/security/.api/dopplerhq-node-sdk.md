...
[DOPPLER_TOPOLOGY]:
- `DopplerSDK` is a facade over ~23 services; the design admits `secrets` (read), `dynamicSecrets` (lease), and `auth` (probe) and treats the rest as out of scope — a runtime folder that called `projects`/`configs`/`environments`/`integrations` would be re-implementing Doppler administration, which belongs in provisioning, not custody.
