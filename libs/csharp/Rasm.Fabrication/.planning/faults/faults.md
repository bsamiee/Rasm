...
One cluster: `[2]-[FAULT_BAND]` owns the `FabricationFault` `[Union]` band-2500 (`NoFit`/`Unreachable`/`KerfCollision`/`OpenLoop`) with `Code`/`ToError`, composing the kernel band-2400 `GeometryFault` for the shared `DegenerateInput`.
