...
- [03]-[DISCIPLINE_AXIS]: the `Discipline` `[SmartEnum<string>]` analysis vocabulary keyed on a wire token with its `Title`/`Group`/`Physical` columns, the railed `Parse` admission, and the `In(group)` roster read — the one axis the typed property, the assessment, and the analysis route share.
...
- Receipt: a `Graph/element#ELEMENT_GRAPH` `Bake`-derived `Element` carries the primary `Classification` AND the `Classifications` set flat. The `Graph/element#NODE_MODEL` `Node.Object.AllClassifications` read closes that storage split at its own owner, and these reads translate across systems without choosing an order-dependent head and roll up only through projector-supplied authoritative relations.
