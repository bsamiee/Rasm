...
public abstract partial record Coercion {
    ...
    // Family is DECLARED beside the borrowed SI-base enum because the receipt's family column crosses the wire:
    // reading it off the enum's runtime type name spells `IrradianceUnit` where the quantity is a radiance, and a
    // package rename re-spells a wire column no consumer can re-derive.
    ...
    internal Fin<UnitEvidence> Admit(double value, Enum unit, Op key, Guid correlation) =>
        Switch(
            ...
            borrowed: static (s, b) => MaterialUnits.Coerce(s.Value, s.Unit, b.Canonical, s.Key)
                .Map(si => UnitEvidence.Raw(s.Value, s.Unit, si, b.Canonical, b.Family, s.Correlation)));
}
...
public sealed partial class PhotometricQuantity {
    ...
    // code names what CanonicalValue is measured in, which is exactly what the row's own Coercion already fixes, so
    // the two facts travel together and a row cannot exist without its wire spelling. The prior shape — a
    // side-table keyed by UnitsNet enum behind a throwing reader — inverted that: it made the code a lookup that
    // could MISS, so admitting a quantity and being able to name it on a wire were two separate successes and the
    // second failed by exception at the one site a wire needed it. It also covered only the units a caller had
    // happened to author, which is why the roster's per-steradian rows had no code at all. Owned HERE because
    // UnitsNet publishes no UCUM surface — its abbreviation cache yields DISPLAY renderings (`cd/m²`, Unicode
    // superscript) a reader consumes and a wire cannot — so the correspondence is declared, and the two Borrowed
    // rows carry the steradian their own SI-base enum is dimensionally silent about.
...
public readonly record struct UnitEvidence(string Family, string OriginalUnit, double OriginalValue, string CanonicalUnit, double CanonicalValue, double RadiometricSi, CorrelationId Correlation) {
    ...
    public static UnitEvidence Raw(double originalValue, Enum originalUnit, double canonicalValue, Enum canonical, string family, Guid correlation) =>
        new(family, originalUnit.ToString(), originalValue, canonical.ToString(), canonicalValue, canonicalValue, correlation);
}

public static class MaterialUnits {
    // The UCUM correspondence lives on the PhotometricQuantity row, so this owner publishes no unit-code table and no
    // reader that can throw. A consumer naming a light quantity reads that row's own Ucum column; a channel carrying
    // no light quantity is dimensionless and its code is the UCUM unity "1", which its own roster states rather than
    // asks a lookup for.
...
public readonly record struct PhotometricPolicy(EmissionSpectrum Spectrum, double Exposure, double EfficacyRatio, Observer Observer) {
    ...
    // the photopic-band fraction is a property of that curve rather than a number a caller states: the folder-root
    // SpectralCurve.LuminousEfficacy() folds the scale-invariant ∫V(λ)S(λ)/∫S(λ) over the curve's own sampling grid
    // — scale-invariant because the ratio divides the SPD out of itself, which is exactly why it survives where the
    // package's relative tristimulus readout cannot recover it. Every other arm has NO spectrum to integrate and
    // takes the DECLARED default of unity, the 555 nm monochromatic anchor at which the 683 lm/W divide is exact.
    // That unity is a stated idealization rather than a measurement — a broadband emitter admitted through it reads
    // high by its own out-of-band fraction — so the default is the fallback and the measured fold is the law, the
    // inversion of a page that once treated the ideal as the norm and the measurement as the exception.
...
public static class Photometric {
    ...
    // THE QUANTITY BAND GATES THE WRITE. EmissionLuminance is a RADIANCE column — the per-steradian, per-area
    // magnitude the integrator adds at a shading point — so only a row whose band answers CanonicalIsRadiance may
    // fill it. A flux, illuminance, irradiance, or power magnitude measures a whole emitter or a whole receiving
    // surface, and normalizing one into radiance needs the emitting AREA and the SOLID ANGLE no appearance row
    // carries; there is therefore no conversion to offer and the refusal names the band rather than inventing a
    // geometry. Gating HERE rather than at Resolve is deliberate: Resolve's UnitEvidence is a faithful receipt of
    // ANY admitted quantity and the sky and environment consumers read exactly that, so the band restriction
    // belongs at the one write that seats a magnitude in a radiance-shaped column.
    public static Fin<MaterialParameters> WithEmission(MaterialParameters row, EmissionInput emission, Op key) =>
        emission.Source.CanonicalIsRadiance
            ...
            : MaterialFault.Parameter(key, $"<emission-quantity-not-radiance:{emission.Source.Key}:{emission.Source.Ucum}>");
