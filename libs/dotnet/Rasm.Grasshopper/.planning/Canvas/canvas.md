...
- Law: `Canvas.ResolvePickMode(bool, bool)` is `public` on the PRIVATE nested `Canvas.ViewportMouseDragger`, so the host resolver is unreachable and `PickGrain.Resolve`'s column re-states its exact body — licensed by unreachability alone, re-proved on the `gh2` decompile pass whenever the host surface moves.
...
- Owner: `CanvasOp` `[Union]` — the closed command family over the host's public navigation, projection, dwell, sparkle, marquee, selection, policy, and inline-editing mutations, each case dispatching through the same total `Switch` that performs the mutation.
...
public sealed record InlinePrompt(
    ...
    private static readonly HookId Hook = HookId.Create(value: "rasm.grasshopper.canvas.prompt");
