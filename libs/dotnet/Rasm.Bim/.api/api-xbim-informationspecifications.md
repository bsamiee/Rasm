...
- within-lib: every match/parse takes an optional `Microsoft.Extensions.Logging.ILogger`; the validation fold threads its result logger so a parse diagnostic rides the structured log, `Try.lift` preserves thrown parser failures exactly, and an explicit empty IDS load mints `BimFault.Refused` under the review/rejected axes.
