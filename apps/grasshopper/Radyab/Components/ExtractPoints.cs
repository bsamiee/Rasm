...
public sealed class ExtractPoints : Component<ExtractPoints> {
    ...
    [Output] private IOutputGroup KindOut => Output.Query(input: Geometry, port: Port.List<GeometryKind>(kind: PortKind.Enum(initial: GeometryKind.Unknown), name: "Kind", code: "K", info: "Detected geometry kind, including primitive Brep and Surface families when Rhino can classify them."), operation: static (_, _) => Rasm.Analysis.Query.Kind<object, GeometryKind>());
}
