...
class HatchSpec(Struct, frozen=True):
    # the ISO 128-50 section-fill projection the section producer applies via
    # `hatch.set_pattern_fill(spec.pattern, scale=spec.scale, angle=spec.angle, definition=spec.definition())`.
    pattern: str        # the ACAD/ISO pattern name ezdxf.tools.pattern.load carries
    scale: float = 1.0
    angle: float = 0.0
...
class Standard(Struct, frozen=True):
    ...
    def graphics(self, layer: LayerName) -> GfxAttribs:
        # the ONE uniform attribute value object every downstream add_* entity carries as dxfattribs=.
        pen = layer.pen
        return GfxAttribs(
...
_HATCH: frozendict[HatchMaterial, HatchSpec] = frozendict({
    HatchMaterial.STEEL: HatchSpec("ANSI31", 1.0, 45.0),
    ...
    HatchMaterial.EARTH: HatchSpec("EARTH", 1.0, 45.0),
