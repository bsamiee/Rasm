...
# `ValueError`, an out-of-range `Proof` sheet index raises `IndexError` (the sole arm indexing the
# document by a caller-supplied page the per-sheet siblings never expose), the source stream raises
# `OSError`, and the `_GUARD` shape contract raises `BeartypeCallHintViolation` on a non-positive
# grid count or off-axis verso rotation. The boundary narrows on this real raise tuple so
# `async_boundary` discriminates each into its `BoundaryFault` case rather than the `Exception`
# catch-all the faults owner rejects; cancellation is excluded.
_FAULTS: tuple[type[BaseException], ...] = (RuntimeError, ValueError, KeyError, IndexError, OSError, BeartypeCallHintViolation)
...
def _admit(
    ...
    gutter: Span, head_trim: Span, spine: Span, creep: Span, bleed: Span, /,
) -> None:
...
class Geometry(Struct, frozen=True):
    ...
    def partition(self, shift: float = 0.0) -> tuple[Box, ...]:
        ...
        _admit(self.across, self.down, self.leaves, self.gutter, self.head_trim, self.spine, self.creep, self.bleed)
...
class ImposeOp:  # the closed request vocabulary; the fault rail is BoundaryFault at the boundary
    ...
    proof: tuple[bytes, float, int | tuple[int, ...]] = case()
    ...
    @staticmethod
    def Proof(source: bytes, dpi: float = 96.0, sheet: int | tuple[int, ...] = 0) -> "ImposeOp":  # an `int` proofs that one imposed sheet; a tuple proofs a contact strip of those sheets (empty = every sheet)
...
def _composed(op: ImposeOp) -> Composed:  # the one pure render fold both `of` and `contribute` read
    ...
    match op:
        ...
        case ImposeOp(tag="proof", proof=(source, dpi, sheet)):
            src = pymupdf.open(stream=source, filetype="pdf")
            pixmap = _contact(src, sheet, int(dpi)) if isinstance(sheet, tuple) else src[sheet].get_pixmap(dpi=int(dpi))
...
def _contact(src: "Document", sheets: tuple[int, ...], dpi: int, /) -> "Pixmap":
    ...
    # owns, then rasterize once, so a press operator proofs a whole signature set in one image instead of
    # one sheet at a time; the montage cell is the imposed-sheet rect and the caller's `dpi` sizes the strip.
    import pymupdf
    ...
    return montage[0].get_pixmap(dpi=dpi)
...
- [PROOF_AND_LAYERS_SETTLED]: the `Proof(source, dpi, sheet)` arm discriminates the `sheet` selector on input shape — an `int` `get_pixmap(dpi=)` rasterizes that one imposed sheet to a PNG, a tuple drives `_contact` to lay the selected sheets (every sheet when empty) row-major into one montage page through the same `show_pdf_page` cross-document draw and rasterize the contact strip once — both keyed by the same `ContentKey` with the `Pixmap.width`/`height` riding the `ArtifactReceipt.Preview` extent, so a press-check or contact-sheet consumer proofs ANY single imposed sheet — recto, verso, or an inner signature page — or a whole-signature contact strip, rather than re-imposing or only ever the first, the same raster-proof capability the sibling `composition/sheet#SHEET` `Preview` and `composition/compose#COMPOSE` arms expose; the `_contact` montage grid columns derive from `math.isqrt` over the selected-sheet count (never a layout knob), an imposition engine that cannot show any imposed sheet or a full contact strip being capability-thin. A separate `Contact` op or factory beside the one `Proof` arm, and a `columns` knob the sheet count already answers, are the rejected forms. The `Imposition.layers(names) -> tuple[Layer, ...]` projection hands the imposed layout outward to `export/layered#LAYERED` exactly as `composition/sheet#SHEET` `Sheet.layers` does: one `Layer(name, source, bbox)` row per OCG-bound signature group (the `_folded` `sig-N` names deduped through a `Map.add` fold whose value is the `_union` of that group's cell boxes), each filling the REQUIRED `bbox` positional the `export/layered` `Layer(name, source, bbox, visible, locked)` row declares (the imposed signature groups uniformly visible and unlocked, the `visible`/`locked` defaults the export row carries) — so the imposed press form hands outward as named rows the export owner binds into OCG/SVG layers, the named-layer authoring landing once in the export owner and this owner projecting the rows. A `Proof` `Pdf` receipt over PNG bytes (a PDF evidence shape over a raster), a per-placement duplicate `Layer` row, and a layered-export arm grafted onto `ImposeOp` are the rejected forms.
