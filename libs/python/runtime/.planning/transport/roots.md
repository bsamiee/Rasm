...
class StoreBackend(Struct, frozen=True, gc=False):
    ...
    # resolving to it, `kvstore` the native key-value driver name a chunk-store consumer opens under it, and `signs`
    # whether the backend mints presigned URLs. Every roster below is a comprehension over this one table, so a
    # seventh backend is ONE row and no consumer is edited — the deleted forms are a hand-listed scheme frozenset, a
    # re-declared backend literal, and a two-row driver map living beside a prose comment at a downstream consumer.
    backend: Backend
...
STORE_BACKENDS: Final[Block[StoreBackend]] = Block.of_seq([
    StoreBackend("s3", frozenset({"s3", "s3a"}), "s3", True),
    StoreBackend("gcs", frozenset({"gs", "gcs"}), "gcs", True),
    ...
    StoreBackend("azure", frozenset({"az", "abfs", "abfss", "azure"}), None, True),
    StoreBackend("http", frozenset({"http", "https"}), "http", False),
    StoreBackend("local", frozenset({"file", ""}), "file", False),
    StoreBackend("memory", frozenset({"memory"}), "memory", False),
])
...
# the remote object-store residence set, derived off the `signs` column because across these six backends the
# signable set and the remote-store set are the same set: `http`/`local`/`memory` are local or degenerate residences
# the filesystem read arm and the local chunk engine serve, so a scheme outside this set never opens a store handle.
OBJECT_STORE_SCHEMES: Final[frozenset[str]] = frozenset(alias for row in STORE_BACKENDS if row.signs for alias in row.aliases)
...
# response extensions, so these three keys ARE the state machine's observable projection — `Cache-Control`/`Age`
# header inference and a span-derived hit/miss counter are both deleted forms, the latter because the httpx
# instrumentation spans BENEATH the cache and a served entry opens no origin span to count.
...
class HttpEndpoint(Struct, frozen=True):
    ...
    # posture, and the composition-admitted scratch root the keyed store lands under — so cache location and cache
    # class are both destination facts rather than a global default or a hardcoded provider path.
    url: str
    cache_root: str
    ...
    @property
    def key(self) -> str:
        # grouping is `scheme://host:port` off `hostname`/`port`, NEVER the raw `netloc`: netloc carries `user:password@`
        # userinfo, leaking the credential into the pool key and pooling by secret rather than by destination. The
        # posture joins the key so two declarations over one host pool distinctly instead of colliding onto whichever landed first.
        parsed = urlsplit(self.url)
        host = f"{parsed.scheme}://{parsed.hostname}:{parsed.port}" if parsed.port else f"{parsed.scheme}://{parsed.hostname}"
        return f"{host}:{self.posture}"
    ...
    @property
    def pool(self) -> tuple[str, str, CachePosture]:
        # the client memo axes as ONE hashable tuple: destination grouping, keyed-store path, posture. The bearer
        # never joins — a pool is keyed by destination, never by secret.
        return (self.key, self.cache, self.posture)
...
class _StoreRow(Struct, frozen=True):
    ...
    # per-op mutation-class judgment: reads, `put`, `delete`, `sign`, and the lazy `open_reader`/`open_writer` handle
    # mints replay safely under OBJECT_STORE (a conditional-write collision stays the terminal `boundary` fault); `copy`
    # (a provider-side multi-step rewrite chain) and `rename` (copy-then-delete) override to None and cross unretried —
    # an ambiguously-succeeded first attempt makes a blind replay corrupt state or mis-report `NotFound`/`AlreadyExists`.
...
class ResourceRoot(Struct, frozen=True):
    ...
    async def read(self, ref: ResourceRef, delivery: Delivery = Delivery.WHOLE, known: Option[str] = Nothing) -> RuntimeRail[Acquired]:
        # `known` is the caller's PRIOR provider validator — the `e_tag` its last read reported — threaded as the
        # `if_none_match` precondition so an unchanged remote answers `NotModifiedError` the fence lifts instead of
        # re-downloading a payload the identity fold then discards. It is the origin's own opaque validator, never a
        # branch `ContentKey`: our digest names our bytes and no origin ever compares against it.
        ...
        plan = (
            ...
            if store is None
                ...
                ref.owner, RetryClass.OBJECT_STORE, lambda: _obj_body(store, ref.relative, options), lambda: _obj_chunks(store, ref.relative, options)
            )
        )
    ...
    async def acquire(self, relative: str, delivery: Delivery = Delivery.WHOLE) -> RuntimeRail[Acquired]:
        match self:
            case TransportResource(tag="http", http=(endpoint, retry_class)):
                client, auth = _transport_client(endpoint.pool), _auth(endpoint.bearer)
                ...
                planned = Ok(
                    TransferPlan(
                        "http",
                        retry_class,
                        lambda: _http_body(client, endpoint.url, auth, carry),
                        lambda: _http_chunks(client, endpoint.url, auth, carry),
                    )
                )
...
def _cached(response: httpx.Response) -> None:
    # the reached cache state stamped on the live acquisition span, read off the extensions the cache proxy publishes:
    # an absent key set means the request reached the origin, so "origin" is a measured reading rather than a filled zero.
...
@cache
def _transport_client(pool: tuple[str, str, CachePosture]) -> httpx.AsyncClient:
    # `_LIVE_CLIENTS` is the drainable handle because `functools.cache` surrenders no value enumeration to the `drain`
    # teardown. The cache rides the TRANSPORT seam, so this one long-lived client keeps its timeout, auth, and status
    # hook while `AsyncCacheTransport` wraps the pooled `AsyncHTTPTransport` beneath it.
    _, database, posture = pool
    client = httpx.AsyncClient(
        timeout=TRANSPORT_TIMEOUT,
        transport=AsyncCacheTransport(
            next_transport=httpx.AsyncHTTPTransport(retries=CONNECT_RETRIES, http2=True, limits=TRANSPORT_LIMITS),
            storage=AsyncSqliteStorage(database_path=database),
            policy=_CACHE_POLICY[posture](),
        ),
...
async def drain() -> None:
    ...
    # is the doubled teardown. The Rust `ObjectStore` has no async pool, so its cache only clears.
...
async def _obj_chunks(store: ObjectStore, relative: str, options: "GetOptions | None") -> AsyncIterator[Bytes]:
...
async def _http_body(client: httpx.AsyncClient, url: str, auth: httpx.Auth | None, extensions: dict[str, object]) -> bytes:
    response = await client.get(url, auth=auth, extensions=extensions)
    _cached(response)
    return response.content


async def _http_chunks(client: httpx.AsyncClient, url: str, auth: httpx.Auth | None, extensions: dict[str, object]) -> AsyncIterator[bytes]:
