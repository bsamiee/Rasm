...
- Boundary: `AccelerationStructure` decoding asserts the `Decode` and leaf-tail ROUND TRIP over the `Rasm/Spatial/index#SPATIAL_INDEX` producer's own pinned descriptors, so this page pins no bytes of its own; clash pairs stay outside that relation, since `NodeLinkPairs` classifies triangles under a `ClashPolicy` no round trip witnesses.
