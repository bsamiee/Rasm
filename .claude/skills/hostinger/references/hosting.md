...
WordPress installs onto an existing website. `installWordPressV1` requires `domain`, `site_title`, and a `credentials` block. `overwrite` defaults false, the async job fails when WordPress already occupies the path. Omitted `version` resolves to the latest core compatible with the vhost's PHP version.
...
Remote access opens per database (`POST .../databases/{name}/remote-connections` with an `ip`, or `%` for any host) and revokes by the same `ip` query, `getPhpMyAdminLinkV1` and `repairDatabaseV1` complete the set. Remote-connection listing is account-wide, create and delete are per-database.
