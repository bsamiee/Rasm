...
- `Rasm.Contracts`(`Rasm.Contracts/.api/rasm-contracts.md`): `fault.FaultDetail.violations` is `repeated google.rpc.BadRequest.FieldViolation`; `element`, `ui`, and `declaration` declare `google.type.Date`, `DateTime`, or `TimeOfDay` where owned.
