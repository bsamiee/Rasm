...
public sealed record TileSet(TileNode Root, TilesetCensus Census, Instant At) {
    public static Fin<TileSet> Build(ImportedGeometry geometry, Func<UInt128, Option<TileMetadata>> metadata, TilePolicy policy) {
    ...
    static Fin<TileNode> Partition(ImportedGeometry geometry, Func<UInt128, Option<TileMetadata>> metadata, TilePolicy policy, int depth) {
    ...
    static Fin<Seq<ImportedGeometry>> Split(ImportedGeometry geometry, float[] positions, float[] bounds) {
    ...
    static Fin<ImportedGeometry> Tessellate(ImportedGeometry geometry, Seq<int> triangles) {
        ...
        return Encode.Of(vertices, toSeq(lanes).Map(static lane => (lane.Channel, lane.Gathered)))
