...
from obstore.store import ObjectStoreMethods, from_url
...
# arm) or a `get_ranges` `tuple[Bytes, ...]` keyed `stream` (the `Iterable()` arm), `head` keys its
# remote `e_tag` evidence, and a control-plane op with no operation bytes (`list`/`delete`/`copy`/
# `rename`/`reader`/`writer`/`sign`) emits `None` so `_receipt` mints NO content key rather than
# digesting a path string — the identity owner's no-path law the egress would otherwise break. The
# `tuple[ContentKey, ...]` merkle arm is the identity owner's modality the egress mints no spine for.
...
def _listing(store: ObjectStoreMethods, prefix: str, *, offset: str | None, delimiter: bool) -> int:
    stream = obstore.list_with_delimiter(store, prefix, return_arrow=True)["objects"] if delimiter else obstore.list(store, prefix, offset=offset, return_arrow=True)
    return sum(batch.num_rows for batch in stream)


async def _listing_async(store: ObjectStoreMethods, prefix: str, *, offset: str | None, delimiter: bool) -> int:
    if delimiter:
        return sum(batch.num_rows for batch in (await obstore.list_with_delimiter_async(store, prefix, return_arrow=True))["objects"])
...
_ROUTE: Final[Map[str, _Row]] = Map.of_seq([
    ...
    ("head", _Row(obstore.head, obstore.head_async,
        lambda op, t: ((op.head,), {}),
        lambda op, t, meta: (meta["size"], meta, str(meta.get("e_tag") or "").encode(), None),
        lambda op, t: op.head)),
...
class EgressReceipt(Struct, frozen=True):
    ...
    e_tag: str
    version: str
    # `None` for a control-plane op carrying no operation bytes (`list`/`delete`/`copy`/`rename`/
    # `reader`/`writer`/`sign`): the receipt mints no content key rather than digesting a path string.
...
class ObjectEgress(Struct, frozen=True):
    ...
    def run(self, op: StoreOp, path: str = "") -> RuntimeRail[EgressReceipt]:
        with _TRACER.start_as_current_span(f"egress.{op.tag}", attributes={"rasm.egress.scheme": self.ref.scheme}):
            return boundary(f"egress.{op.tag}", lambda: self._apply(op, path or self.ref.relative)).bind(lambda rail: rail)
    ...
    def _receipt(self, op: StoreOp, target: str, returned: Any) -> RuntimeRail[EgressReceipt]:
        ...
        receipt = lambda key: EgressReceipt.of(op.tag, row.path(op, target), length, meta, key, payload)
        # a control-plane op (`source is None`) carries no operation bytes, so it mints NO content
        # key rather than digesting a path string — the identity owner's no-path law. A byte-bearing
        # op runs the one `ContentIdentity.of` call, which classifies the `Source` (a `Bytes` whole, a
        # `get_ranges` `tuple[Bytes, ...]` stream, or the `head` `e_tag` evidence) and rails the key,
        # scheme-scoped through `self.ref.scheme` so the key is the operation's real bytes.
        if source is None:
    ...
    async def _apply_async(self, op: StoreOp, target: str) -> RuntimeRail[EgressReceipt]:
        args, kwargs = _ROUTE[op.tag].plan(op, target)
        return self._receipt(op, target, await _ROUTE[op.tag].aio(self.store, *args, **kwargs))
```

## [03]-[RESEARCH]

- [OBSTORE_ROUTE_SURFACE]: every `_ROUTE` member — `store.from_url`/`put`/`put_async`/`get`/`get_async`/`get_range`/`get_range_async`/`get_ranges`/`get_ranges_async`/`list`/`list_with_delimiter`/`list_with_delimiter_async`/`head`/`head_async`/`delete`/`delete_async`/`copy`/`copy_async`/`rename`/`rename_async`/`open_reader`/`open_reader_async`/`open_writer`/`open_writer_async`/`sign`/`sign_async` — is catalogue-confirmed against the folder `data/.api/obstore.md` (get operations [01]-[08], write/mutation [01]-[08], list/streaming [01]-[07], presign [01]-[02], store construction [01]) and the substrate `runtime/.api/obstore.md`, reflection-confirmed on the cp315 abi3 `obstore` 0.10.1 distribution. The store handle types as the `store.ObjectStoreMethods` structural Protocol every store class satisfies, constructed once at `of` and frozen on the owner so no per-operation `from_url` re-mint exists; `from_url(ref.root, ...)` dispatches the backend off the URL scheme internally, so the egress calls no `parse_scheme` — `parse_scheme` is the top-level `obstore.parse_scheme(url) -> Literal["s3","gcs","http","local","memory","azure"]` classifier (reflection-confirmed live), not the `obstore.store.parse_scheme` member the prior draft imported, and it is unfit for keying a relative object path because it expects a full URL. The `Get` `read` reads `GetResult.bytes()` for the body, `GetResult.meta` for the `e_tag`/`version`/`size` evidence, and `GetResult.range`/`GetResult.attributes` for the served byte window and object attributes onto the receipt `payload` slot, while `GetResult.stream` `BytesStream` is the large-object streaming value the same `Get` row carries (result types [02] enumerates `.meta`/`.range`/`.attributes`/`.bytes()`/`.stream`/`.buffer`); the content key derives from the operation's real bytes through exactly one `ContentIdentity.of` in `_receipt` over the whole-buffer `Source` (`put`/`get`/`get_range`/`head` e-tag evidence) or the `tuple[Bytes, ...]` chunk-stream `Source` over the `get_ranges` `list[Bytes]` windows, while a control-plane op (`list`/`delete`/`copy`/`rename`/`reader`/`writer`/`sign`) emits a `None` `Source` so `_receipt` rails `content_key=None` rather than digesting a path, the `IdentitySource.lift` discriminator (`evidence/identity#IDENTITY` `Source = Buffer | Iterable[bytes] | tuple[ContentKey, ...]`) owning the whole-versus-stream-versus-merkle modality and `ContentIdentity.of` returning a `RuntimeRail[ContentKey]` the `_receipt` `.map` threads, the zero-copy `Bytes` buffer feeding the digest with no `bytes()` copy, never the path the identity owner forbids as a key source, never a hand-rolled per-window merkle fold. The catalogue `_async`-identity law (each operation has a matching `_async` variant with identical signature) settles every `_async`/`*_async` sibling — `put_async` carries `attributes`/`tags` identically to `put`, `get_ranges_async` carries `lengths` identically to `get_ranges`, `sign_async`/`open_reader_async`/`open_writer_async`/`list_with_delimiter_async` mirror their sync rows — so one kwargs plan serves both legs and no per-leg keyword allowlist exists: `put`/`put_async` carry `attributes`/`tags`/`mode`/`use_multipart`/`chunk_size`/`max_concurrency` (`chunk_size` 5-MiB default, `max_concurrency` 12 default); `get_range`/`get_range_async` carry `start`/`end`/`length` with `end` and `length` mutually exclusive window descriptors; `get_ranges`/`get_ranges_async` carry `starts`/`ends`/`lengths`/`coalesce` (1-MiB default) so the window batch addresses by `ends` or `lengths`; `copy`/`copy_async`/`rename`/`rename_async` carry `overwrite=True` as the conditional precondition; `delete`/`delete_async` accept `paths` as `str | Sequence[str]` so the bulk delete absorbs the singular and plural call; `open_reader`/`open_reader_async` carry `buffer_size`/`size` and `open_writer`/`open_writer_async` carry `attributes`/`tags`/`buffer_size`/`max_concurrency`; `sign`/`sign_async(store, method, paths, expires_in)` mints the presigned-URL batch — `method` the full `HTTP_METHOD` literal (`GET`/`PUT`/`POST`/`HEAD`/`PATCH`/`TRACE`/`DELETE`/`OPTIONS`/`CONNECT`, reflection-confirmed) and `expires_in` a `datetime.timedelta`, valid only on `SignCapableStore = S3Store | GCSStore | AzureStore`; `store.from_url(url, *, config, client_options, retry_config, credential_provider, **kwargs)` carries `credential_provider` (store construction [01]). The `list` member works under both `for` and `async for` over one `ListStream` (no `list_async`), so the row pairs `_listing`/`_listing_async` summing `arro3.core.RecordBatch.num_rows` over the `return_arrow=True` stream or the `list_with_delimiter` `ListResult` `objects` row-count, folding the listing case into the table rather than a `_list_rows` branch. `PutResult` `e_tag`/`version`, `ObjectMeta` `path`/`last_modified`/`size`/`e_tag`/`version`, and `GetResult.meta` are `TypedDict`/result subscripts the receipt reads directly (`meta["size"]`/`meta.get("e_tag")`/`result.meta`); `GetOptions` is the `TypedDict(total=False)` carrying `if_match`/`if_none_match`/`if_modified_since`/`if_unmodified_since`/`range`/`version`/`head` (folder catalogue result types [08], reflection-confirmed), so the `Get` card names the full conditional-get key set with no open RESEARCH tail. `PutResult`/`ObjectMeta`/`GetOptions`/`PutMode`/`GetResult`/`Attributes` are typing-only names the `obstore` stubs mark not importable at runtime (the runtime catalogue [02] note), so they import under `TYPE_CHECKING`. `obstore.store.ObjectStoreMethods` is the exception — reflection-confirmed a runtime-real `Protocol` (`runtime_attr=True`, absent from `obstore.store.__all__` yet present as a real attribute) — and because it is the bare annotation on the `ObjectEgress.store` msgspec `Struct` field it imports at module top, since a `TYPE_CHECKING`-only import there `NameError`s when msgspec resolves the field at class definition. `GetResult.bytes()` returns a `len`-able `Bytes` and `GetResult.stream(min_chunk_size=10*1024*1024)` returns a `BytesStream` sync/async chunk iterator; dispatch is the `op.tag` key into the `_ROUTE` `Map`, not a generated `Switch` nor an enumerated structural `match`, so the receipt path and the case projections fold onto the one row.
- [OBSTORE_FAULT_CONVERSION]: the `obstore.exceptions` taxonomy — `BaseError` root with `NotFoundError`/`AlreadyExistsError`/`PermissionDeniedError`/`UnauthenticatedError`/`PreconditionError`/`NotModifiedError`/`NotSupportedError`/`InvalidPathError`/`UnknownConfigurationKeyError`/`GenericError`/`JoinError` leaves — is catalogue-confirmed (`runtime/.api/obstore.md` exceptions [01]-[12]) and reflection-confirmed against the live 0.10.1 distribution (`dir(obstore.exceptions)` yields all eleven leaves plus the root). The async leg lifts the terminal raise through the `reliability/resilience#RESILIENCE` `guarded` envelope (its `reliability/faults#FAULT` `async_boundary`) and the sync leg through `boundary(subject, ...)`, both at the `Exception` catch-all the `CLASSIFY` fold owns — the `guarded` delegation is exactly the `transport/roots#RESOURCE` object-store-reader convention where the resilience owner's `async_boundary` catches `Exception` and `CLASSIFY` lands the obstore raise on its catch-all, so a narrowed `catch=obstore.exceptions.BaseError` is the deleted form because it would let a row-projection `KeyError`/`AttributeError` escape the egress rather than convert. The obstore taxonomy is NOT a `BoundaryFault.CLASSIFY` row (the `faults` owner's `CLASSIFY` `Block` rows only `TimeoutError`/`msgspec`/`beartype`/`ImportError`/`anyio`+`OSError`), so `BoundaryFault.of(subject, cause)` falls to the `try_head().default_with(...)` catch-all carrying the `boundary` case `(subject, str(cause) or type(cause).__name__)` — the obstore message first, the class name only on a message-less raise — and recovery keys on `fault.recoverable({"boundary"})`, never a reconstructed message, never a per-package `_classify` table, and never an invented HTTP-status integer since the EXCEPTIONS rows model no numeric status. `NotFoundError` (absent get/head) and `NotSupportedError` (sign on a non-`SignCapableStore` backend) ride the same catch-all, so the recovery vocabulary keys on `fault.recoverable({"boundary"})` like every other leaf. `SignCapableStore` and the full nine-member `HTTP_METHOD` literal (`GET`/`PUT`/`POST`/`HEAD`/`PATCH`/`TRACE`/`DELETE`/`OPTIONS`/`CONNECT`) are reflection-confirmed declared in `obstore._sign` and re-exported from the top-level `obstore` package (`__all__`), NOT from `obstore.store`, so the `Method` literal mirrors `HTTP_METHOD` exactly and a `SignCapableStore` annotation imports from `obstore`, never `obstore.store`.
- [OBSTORE_CREDENTIAL_CONFIG]: the `config` value `ObjectEgress.of` admits is the `S3Config | GCSConfig | AzureConfig` `TypedDict` union in `obstore.store` (or `None`) — `from_url(url, *, config: S3Config | GCSConfig | AzureConfig | None, ...)` reflection-confirmed, so the prior `dict[str, str]` arm is dropped as a wider-than-the-signature type — `client_options` the `ClientConfig` `TypedDict` and `retry_config` the `RetryConfig` `TypedDict` (`runtime/.api/obstore.md` config types); the `credential_provider` callable rides the runtime `TransportResource` for token refresh and threads through `from_url(url, *, config, client_options, retry_config, credential_provider, **kwargs)` to the per-scheme store, reflection-confirmed on the abi3 core.
- [OBSTORE_STREAM_AND_SIGN]: the buffered large-bundle streaming handles — `obstore.open_reader(store, path, *, buffer_size, size)`/`open_reader_async` returning `ReadableFile`/`AsyncReadableFile` and `obstore.open_writer(store, path, *, attributes, buffer_size, tags, max_concurrency)`/`open_writer_async` returning `WritableFile`/`AsyncWritableFile` (list/streaming [04]-[07]) — and the presigned-URL mint `obstore.sign(store, method, paths, expires_in)`/`sign_async` (presign [01]-[02]) fold into the one `StoreOp` axis as the `reader`/`writer`/`sign` cases, their `_ROUTE` `read`s carrying the obstore handle or URL batch on the `EgressReceipt.payload` slot rather than a parallel `StoreHandle` union or a second `handle`/`handle_async` surface; `open_reader`/`open_writer` own large-object streaming and `sign` requires a `SignCapableStore` (else `NotSupportedError`), both reflection-confirmed on the abi3 core.
- [EGRESS_OBSERVABILITY]: each `run`/`run_async` opens one `_TRACER.start_as_current_span("egress.{tag}", attributes={"rasm.egress.scheme": ...})` parent span over `opentelemetry.trace.get_tracer` (`branch/.api/opentelemetry-api.md` trace API [10]/[03]). The awaitable leg then calls `guarded(RetryClass.OBJECT_STORE, _apply_async, op, target, subject="egress.{tag}")`, whose `reliability/resilience#RESILIENCE` envelope opens its own `resilience.guarded` derivation span and a per-retry `resilience.retry` child span beneath the active egress parent and lifts the terminal raise through one `async_boundary` — so the egress span is the parent the retry spans attach to without the egress re-implementing the retry/span/lift triplet, the `transport/roots#RESOURCE` delegation rather than a bare `guard(cls)` caller in a hand-opened fence. The synchronous `run` fences `_apply` through `boundary` inside the same egress parent span with no `await`-only retry envelope, leaning on the `obstore` Rust-core `RetryConfig`. The `EgressReceipt.contribute` yields the emitted-phase `Receipt.of("object-egress", ("emitted", path, facts))` row the `observability/receipts#RECEIPT` `ReceiptContributor.contribute -> Iterable[Receipt]` Protocol drains, the facts dict carrying native scalars the `Encoder(enc_hook=repr)` serializes without a `str()` coerce.
