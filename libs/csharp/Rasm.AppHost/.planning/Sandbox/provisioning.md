...
- Receipt: `UpdateReceipt` — the outcome, its projected phase, channel key, target version, delta count, `Instant`, elapsed `Duration`, and correlation id; a rollback lowers its live `UpdateFault` once through `AppHostFaultMap`, so generated code, typed recovery, and bounded exact causes cross without serializing a LanguageExt error graph; the `Deltas >= 0` claim deletes because an array length never violates it.
...
public abstract partial record UpdateFault : Fault {
    ...
    [FaultCase(0)]
    public sealed partial record DownloadBroken(string Target, Error Cause)
        : UpdateFault($"{Target}: {Cause.Message}"), ICausedFault;
    [FaultCase(1)]
    public sealed partial record StagePending : UpdateFault { public StagePending(string detail) : base(detail) { } }
    [FaultCase(2)]
    public sealed partial record RolloverRejected(string Target, Error Cause)
        : UpdateFault($"{Target}: {Cause.Message}"), ICausedFault;
    [FaultCase(3)]
    public sealed partial record DowngradeBlocked : UpdateFault { public DowngradeBlocked(string detail) : base(detail) { } }
    [FaultCase(4)]
    public sealed partial record AdmissionRejected(string Target, Error Cause)
        : UpdateFault($"{Target}: {Cause.Message}"), ICausedFault;
    [FaultCase(5)]
...
public abstract partial record UpdateOutcome {
    ...
    public sealed record RolledBack(Option<string> Prior, FaultObservationWire Fault) : UpdateOutcome;
    ...
    public static UpdateOutcome Reverted(Option<string> prior, Error fault) =>
        new RolledBack(prior, AppHostFaultMap.Wire(fault));
}
...
public sealed class UpdateRail {
    ...
    public IO<UpdateReceipt> Rollover(VelopackAsset asset, DrainThread drain) =>
        from drained in host.Drain(DrainRows(), drain.Latency, drain.Checkpoint, drain.Instruments)
