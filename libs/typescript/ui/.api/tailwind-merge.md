...
| :-----: | :--------------------------------------------------------------------------------------- | :---------------- |
|  [01]   | `ClassNameValue` (string \| null \| undefined \| 0 \| 0n \| false \| `ClassNameValue[]`) | input union       |
|  [02]   | `ClassValidator` (`(classPart: string) => boolean`)                                      | class predicate   |
|  [03]   | `Config<ClassGroupIds, ThemeGroupIds>`                                                   | full config       |
|  [04]   | `ConfigExtension<ClassGroupIds, ThemeGroupIds>`                                          | config extension  |
|  [05]   | `DefaultClassGroupIds` / `DefaultThemeGroupIds`                                          | built-in id union |
|  [06]   | `ExperimentalParseClassNameParam` / `ExperimentalParsedClassName`                        | parse hook shape  |

- [01]-[INPUT_UNION]: the variadic argument of `twMerge`/`twJoin`; `clsx(...)` output flows straight in; falsy parts are dropped.
- [02]-[CLASS_PREDICATE]: a custom class-group member — matches an arbitrary value shape; the `validators.*` set are ready-made ones.
- [03]-[FULL_CONFIG]: `cacheSize`/`prefix`/`experimentalParseClassName` + `theme`/`classGroups`/`conflictingClassGroups`/`conflictingClassGroupModifiers`/`orderSensitiveModifiers`.
- [04]-[CONFIG_EXTENSION]: the `extendTailwindMerge` argument — `{ override?, extend?, cacheSize?, prefix?, ... }` where `override`/`extend` carry `PartialConfigGroupsPart`.
- [05]-[BUILTIN_ID_UNION]: the string-literal union of default group/theme-scale ids; extend it to add a custom group id type-safely.
- [06]-[PARSE_HOOK_SHAPE]: `experimentalParseClassName({ className, parseClassName })` — pre-parse transform for a nonstandard class syntax.
...
| :-----: | :-------------------------------------------------- | :------------- |
|  [01]   | `twMerge(...classLists: ClassNameValue[]) → string` | resolve merge  |
|  [02]   | `twJoin(...classLists: ClassNameValue[]) → string`  | fast concat    |

- [01]-[RESOLVE_MERGE]: THE merge half of `cn` — base + cva variants + tw-rac variants + override collapse to the last-wins set.
- [02]-[FAST_CONCAT]: conflict-free assembly (static token strings that cannot collide) — skips the parser; `clsx`-shaped inputs.
...
| [INDEX] | [SURFACE]                                                                            | [ENTRY_FAMILY]  |
| :-----: | :----------------------------------------------------------------------------------- | :-------------- |
|  [01]   | `extendTailwindMerge(extension \| createConfig, ...createConfig[]) → typeof twMerge` | extend default  |
|  [02]   | `createTailwindMerge(() => Config, ...ConfigExtension[]) → typeof twMerge`           | build from base |
|  [03]   | `mergeConfigs(baseConfig, configExtension) → Config`                                 | compose config  |
|  [04]   | `getDefaultConfig() → Config`                                                        | base config     |

- [01]-[EXTEND_DEFAULT]: the folder's ONE custom `cn` merger — `{ extend: { theme, classGroups } }` teaching it the `@theme` scales.
- [02]-[BUILD_FROM_BASE]: a fully custom instance when replacing (not extending) the default config wholesale.
- [03]-[COMPOSE_CONFIG]: fold a `ConfigExtension` into a `Config` inside a `createConfig` callback (`override` then `extend` semantics).
- [04]-[BASE_CONFIG]: the default catalog-bound-tuned config to inspect or spread when composing a custom one.
...
| [INDEX] | [SURFACE]                               | [ENTRY_FAMILY]  |
| :-----: | :-------------------------------------- | :-------------- |
|  [01]   | `fromTheme(themeGroupId) → ThemeGetter` | theme ref       |
|  [02]   | `validators` (class-part predicates)    | class predicate |

- [01]-[THEME_REF]: a class-group member that follows a theme scale — the bridge from `@theme` custom scales to a merge group.
- [02]-[CLASS_PREDICATE]: custom-group member matchers — reuse over a hand-rolled regex when defining a `classGroups` entry. The predicate object carries `isArbitraryValue`/`isArbitraryLength`/`isArbitraryNumber`/`isArbitrarySize`/`isArbitraryPosition`/`isArbitraryImage`/`isArbitraryShadow`/`isArbitraryWeight`/`isArbitraryVariable*`/`isNumber`/`isInteger`/`isPercent`/`isFraction`/`isTshirtSize`/`isNamedContainerQuery`/`isAny`/`isAnyNonArbitrary` and its siblings.
