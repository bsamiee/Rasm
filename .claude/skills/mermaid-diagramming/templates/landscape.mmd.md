...
Refill by renaming boundaries to the real ownership domains and elements to the real systems at ONE zoom — a container question re-declares as `C4Container` in its own fence. Every relation keeps its verb, kind color, and a label offset that clears the packed rows. Lavender boundary hooks, pink scaled markers, and retired sprites are fixed law — a refill renames the landscape, never strips the fidelity surface.
