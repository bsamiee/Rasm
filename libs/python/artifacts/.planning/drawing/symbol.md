...
def _grid_solve(marks: tuple[SymbolKind, ...]) -> tuple[SymbolKind, ...]:
    # a grid-bubble run aligns collinear and blended-even through one kiwisolver.Solver: each bubble's
    # position along the run is a `t` Variable, the endpoints pinned `required`, the given interior positions
    # honoured `weak`, an even gap preferred `medium`, and a monotone min-separation kept `weak`, so
    # `updateVariables()` writes each solved `value()` BACK into the mark's re-keyed anchor before `_element`.
    run = tuple(index for index, mark in enumerate(marks) if mark.tag == "grid")
    if len(run) < 3:  # two endpoints fully determine the line; the solve refines three or more
        return marks
    anchors = tuple(marks[index].grid[0] for index in run)
    start, stop = anchors[0], anchors[-1]
    span = math.dist(start, stop) or 1.0
    given = tuple(math.dist(start, anchor) / span for anchor in anchors)
    solver = kiwisolver.Solver()
    ts = tuple(kiwisolver.Variable(f"t{k}") for k in range(len(run)))
    ...
    def _placed(k: int, mark: SymbolKind) -> SymbolKind:
        t, (_anchor, radius, label, style) = ts[k].value(), mark.grid
        return SymbolKind.Grid((start[0] + t * (stop[0] - start[0]), start[1] + t * (stop[1] - start[1])), radius, label, style)

    solved = dict(zip(run, (_placed(k, marks[index]) for k, index in enumerate(run)), strict=True))
...
def _dxf_block(block: object, mark: SymbolKind) -> object:
    ...
    match mark:
        case SymbolKind(tag="north", north=(_, radius, _bearing, _style)):
            dxfpath.render_lines(block, [dxfpath.star(count=4, r1=radius, r2=radius * 0.4)])
            block.add_attdef("N", (0.0, radius * 1.2))
        case SymbolKind(tag="revision", revision=(_, size, _mark, _style)) | SymbolKind(tag="datum", datum=(_, size, _level, _style)):
