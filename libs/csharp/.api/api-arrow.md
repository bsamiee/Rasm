...
[LOCAL_ADMISSION]:
- Compute references core `Apache.Arrow` alone; the `Apache.Arrow.Adbc`, `Apache.Arrow.Flight`, `Apache.Arrow.Flight.AspNetCore`, `Apache.Arrow.Flight.Sql`, and `Apache.Arrow.Compression` egress packages are Persistence's and absent from the Compute closure. `CompanionRequest.GeoArrow.ArrowIpc` (`Runtime/tiles#TWO_HOP_TESSELLATION`) carries Arrow IPC bytes the Python geospatial branch already encoded; Compute relays them opaque and decodes nothing.
