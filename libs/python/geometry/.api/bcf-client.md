...
| :-----: | :--------------------- | :------------ | :------------------------------ |
|   [1]   | `bcf.bcfxml.BcfXml2`   | v2 alias      | schema alias resolved by `load` |
|   [2]   | `bcf.bcfxml.BcfXml3`   | v3 alias      | schema alias resolved by `load` |
|   [3]   | `bcf.v2.bcfxml.BcfXml` | v2 document   | full v2 BCF file lifecycle      |
|   [4]   | `bcf.v3.bcfxml.BcfXml` | v3 document   | full v3 BCF file lifecycle      |
...
| [INDEX] | [SYMBOL]                                  | [TYPE_FAMILY] | [CAPABILITY]                      |
| :-----: | :---------------------------------------- | :------------ | :-------------------------------- |
|   [1]   | `bcf.v2.topic.TopicHandler`               | issue handler | v2 topic comments plus viewpoints |
|   [2]   | `bcf.v3.topic.TopicHandler`               | issue handler | v3 topic comments plus viewpoints |
...
| [INDEX] | [SYMBOL]                                                    | [TYPE_FAMILY] | [CAPABILITY]              |
| :-----: | :---------------------------------------------------------- | :------------ | :------------------------ |
|   [1]   | `v2.model.markup.Topic` / `v3.model.markup.Topic`           | data class    | issue metadata            |
|   [2]   | `v2.model.markup.Comment` / `v3.model.markup.Comment`       | data class    | issue comment             |
|   [3]   | `v2.model.markup.Markup` / `v3.model.markup.Markup`         | data class    | topic container           |
|   [4]   | `v2.model.markup.ViewPoint` / `v3.model.markup.ViewPoint`   | data class    | viewpoint reference       |
|   [5]   | `v2.model.markup.Header` / `v3.model.markup.Header`         | data class    | IFC file header reference |
|   [6]   | `v2.model.markup.BimSnippet` / `v3.model.markup.BimSnippet` | data class    | linked BIM snippet        |
|   [7]   | `v3.model.markup.DocumentReference`                         | data class    | linked document reference |
...
| [INDEX] | [SYMBOL]           | [TYPE_FAMILY] | [CAPABILITY]                             |
| :-----: | :----------------- | :------------ | :--------------------------------------- |
|   [1]   | `BcfClient`        | REST client   | BCF API v3 topic/comment/viewpoint ops   |
|   [2]   | `FoundationClient` | REST client   | BCF API foundation auth/version services |
...
| :-----: | :------------------------------------------------------------------------------------------------ | :------------- | :------------------------------- |
|   [1]   | `bcf.bcfxml.load(filepath: Path, xml_handler=None) -> BcfXml \| None`                             | loader         | open BCF file, detect schema     |
|   [2]   | `BcfXml.load(filename: Path, xml_handler=None) -> BcfXml \| None`                                 | loader         | open BCF into a version instance |
|   [3]   | `bcf.v2.bcfxml.BcfXml.create_new(project_name=None, xml_handler=None) -> BcfXml`                  | factory        | new v2 BCF document              |
|   [4]   | `bcf.v3.bcfxml.BcfXml.create_new(project_name=None, extensions=None, xml_handler=None) -> BcfXml` | factory        | new v3 BCF document              |
|   [5]   | `BcfXml.save(filename: Path \| None=None, keep_open: bool=False) -> None`                         | serializer     | write BCF archive to disk        |
...
| [INDEX] | [SURFACE]                                                                                      | [ENTRY_FAMILY] | [CAPABILITY]                    |
| :-----: | :--------------------------------------------------------------------------------------------- | :------------- | :------------------------------ |
|   [1]   | `BcfXml.add_topic(title, description, author, topic_type='', topic_status='') -> TopicHandler` | mutator        | author a new topic              |
|   [2]   | `BcfXml.get_topics() -> dict[str, TopicHandler]`                                               | query          | all topics keyed by GUID        |
|   [3]   | `BcfXml.get_topic(guid: str) -> TopicHandler`                                                  | query          | one topic by GUID               |
|   [4]   | `BcfXml.add_comment(_topic, _comment=None) -> None`                                            | mutator        | append a comment to a topic     |
|   [5]   | `BcfXml.edit_comment() -> None`                                                                | mutator        | edit an existing comment        |
|   [6]   | `BcfXml.get_project()` / `BcfXml.edit_project(...)` / `BcfXml.save_project(...)`               | project        | project metadata read and write |
|   [7]   | `BcfXml.documents`                                                                             | property (v3)  | v3 `DocumentsHandler` access    |
...
| [INDEX] | [SURFACE]                                                                                                              | [ENTRY_FAMILY] | [CAPABILITY]                             |
| :-----: | :--------------------------------------------------------------------------------------------------------------------- | :------------- | :--------------------------------------- |
|   [1]   | `TopicHandler.add_viewpoint(element) -> VisualizationInfoHandler`                                                      | mutator        | attach an element viewpoint to the topic |
|   [2]   | `TopicHandler.add_viewpoint_from_point_and_guids(position, *guids) -> VisualizationInfoHandler`                        | mutator        | viewpoint from a camera point plus GUIDs |
|   [3]   | `TopicHandler.topic` / `comments` / `viewpoints` / `guid`                                                              | property       | topic, comments, viewpoints, GUID        |
|   [4]   | `TopicHandler.save(destination_zip) -> None`                                                                           | serializer     | persist the topic into the BCF zip       |
|   [5]   | `VisualizationInfoHandler.create_new(element, xml_handler=None) -> VisualizationInfoHandler`                           | factory        | viewpoint from an element                |
|   [6]   | `VisualizationInfoHandler.create_from_point_and_guids(position, *guids, xml_handler=None) -> VisualizationInfoHandler` | factory        | viewpoint from camera point plus GUIDs   |
|   [7]   | `VisualizationInfoHandler.set_selected_elements(elements) -> None`                                                     | mutator        | mark selected elements                   |
|   [8]   | `VisualizationInfoHandler.set_visible_elements(elements)` / `set_hidden_elements(elements) -> None`                    | mutator        | element visibility                       |
|   [9]   | `VisualizationInfoHandler.get_selected_guids() -> list[str] \| None`                                                   | query          | selected element GUIDs                   |
...
| [INDEX] | [SURFACE]                                                                                         | [ENTRY_FAMILY] | [CAPABILITY]                         |
| :-----: | :------------------------------------------------------------------------------------------------ | :------------- | :----------------------------------- |
|   [1]   | `BcfClient.get_projects()` / `get_project()`                                                      | query          | project listing and detail           |
|   [2]   | `BcfClient.get_topics()` / `get_topic()` / `create_topic()` / `update_topic()` / `delete_topic()` | topic          | topic CRUD                           |
|   [3]   | `BcfClient.get_comments()` / `create_comments()` / `update_comment()` / `delete_comment()`        | comment        | comment CRUD                         |
|   [4]   | `BcfClient.get_viewpoints()` / `create_viewpoints()` / `delete_viewpoint()`                       | viewpoint      | viewpoint CRUD                       |
|   [5]   | `BcfClient.get_snapshot()` / `get_bitmap()` / `get_selection()` / `get_visibility()`              | media          | viewpoint media and state            |
|   [6]   | `FoundationClient.login()` / `get_access_token()` / `get_new_access_token()`                      | auth           | OAuth2 token lifecycle               |
|   [7]   | `FoundationClient.get_auth_methods()` / `get_versions()`                                          | negotiate      | server auth methods and API versions |
