...
- [04]-[ORACLE_VECTOR]: the `OracleVector` fixture row per kernel, each expected value exactly computable from the kernel's own definition and quadrature, with the tolerance, the prefix-comparison law, and the parity read the benchmark corpus gates on.
...
- Owner: `PressDevice` the surfaceless bake device and pipeline cache; `DevicePolicy` the acquisition policy row over a `CapabilitySet<DeviceArm>` arm column; `DeviceArm` the acquisition-capability roster; `KernelUniform` the WGSL-aligned word writer; `KernelBuffer` `[Union]` the per-binding request; `KernelBinding` the dispatch request; `KernelReadback` the dispatch readback; `PressDevice.NativeSlot` the ONE completion slot every native callback writes.
...
public sealed record KernelReadback(WgslKernel Kernel, ReadOnlyMemory<float> Output, Option<ulong> GpuNanos, uint Dispatches)
    : IValidityEvidence {
...
public sealed unsafe class PressDevice : IDisposable {
    ...
    static Fin<PressDevice> Bring(DevicePolicy policy) {
        ...
        if (instance is null) { return Fin.Fail<PressDevice>(new RasterFault.Device($"<wgpu-instance-null:{Diagnostic()}>")); }
        ...
        if (adapter is null) {
            ...
            return Fin.Fail<PressDevice>(new RasterFault.Device($"<wgpu-adapter:{(RequestAdapterStatus)slot.Status}:{Detail(&slot)}>"));
        }
        ...
        if (!api.AdapterGetLimits(adapter, &supported)) {
            ...
            api.InstanceRelease(instance);
            return Fin.Fail<PressDevice>(new RasterFault.Device($"<adapter-limits-unread:{Diagnostic()}>"));
        }
        ...
        if (device is null) {
            api.AdapterRelease(adapter);
            api.InstanceRelease(instance);
            return Fin.Fail<PressDevice>(new RasterFault.Device($"<wgpu-device:{(RequestDeviceStatus)slot.Status}:{Detail(&slot)}>"));
        }
        ...
        if (!api.DeviceGetLimits(device, &negotiated)) {
            ...
            api.AdapterRelease(adapter);
            api.InstanceRelease(instance);
            return Fin.Fail<PressDevice>(new RasterFault.Device($"<device-limits-unread:{Diagnostic()}>"));
        }
        ...
        if (!api.TryGetDeviceExtension(device, out Wgpu vendor)) {
            ...
            api.AdapterRelease(adapter);
            api.InstanceRelease(instance);
            return Fin.Fail<PressDevice>(new RasterFault.Device("<wgpu-vendor-extension-absent>"));
        }
    ...
    Fin<Unit> Guard(WgslKernel kernel, KernelBinding binding) =>
        binding.Buffers.Count != kernel.Layout.Count || !binding.Buffers.Zip(kernel.Layout).ForAll(static pair => pair.Item1.Kind == pair.Item2)
            ? Fin.Fail<Unit>(new RasterFault.Device($"<kernel-layout-mismatch:{kernel.Key}>"))
            : Ceilings(kernel, binding).Find(static ceiling => ceiling.Reached > ceiling.Granted)
                .TraverseM(ceiling => Fin.Fail<Unit>(new RasterFault.Device($"<{ceiling.Name}-ceiling:{kernel.Key}:{ceiling.Reached}:{ceiling.Granted}>"))).As()
                .Map(static _ => unit);
    ...
    Fin<nint> Pipeline(WgslKernel kernel) {
        ...
        try {
            ...
            if (module is null) { return Fin.Fail<nint>(new RasterFault.Device($"<wgsl-compile:{kernel.Key}:{Diagnostic()}>")); }
            ...
            if (pipeline is null) { return Fin.Fail<nint>(new RasterFault.Device($"<compute-pipeline:{kernel.Key}:{Diagnostic()}>")); }
    ...
    Fin<KernelReadback> Run(WgslKernel kernel, nint pipeline, KernelBinding binding) {
        ...
        try {
            for (int slot = 0; slot < slots; slot++) {
                ...
                if (handle is null) { return Refuse($"<buffer-alloc:{kernel.Key}:{slot}:{bytes}>"); }
            ...
            if (writeSlot < 0 || writeBytes is 0) { return Refuse($"<kernel-no-write:{kernel.Key}>"); }
            ...
            if (stagingBuffer is null) { return Refuse($"<staging-alloc:{writeBytes}>"); }
            ...
            fixed (BindGroupEntry* seated = entries) {
                ...
                if (bound is null) { return Refuse($"<bind-group:{kernel.Key}>"); }
                group = (nint)bound;
            }
            ...
            if (api.BufferGetMapState(stagingBuffer) != BufferMapState.Mapped) {
                return Refuse($"<readback-unmapped:{kernel.Key}:{(BufferMapAsyncStatus)mapped.Status}:{Diagnostic()}>");
            }
            ...
            if (scoped) {
                ...
                if (scope.Fired is not 0 && scope.Status != (int)ErrorType.NoError) {
                    return Refuse($"<validation:{(ErrorType)scope.Status}:{Detail(&scope)}>");
                }
            }
            return Fin.Succ(new KernelReadback(kernel, kernel.Reduce.Fold(raw, kernel.Reduce.Stride), nanos, Dispatches: 1u));
        } finally {
            if (scoped) {
    ...
    static Fin<KernelReadback> Refuse(string detail) => Fin.Fail<KernelReadback>(new RasterFault.Device(detail));
...
- Entry: the table is the entry — `WgslKernel.Items` is the roster `[04]-[ORACLE_VECTOR]` iterates and `PressDevice.Dispatch` selects on; `Layout` declares the binding kinds a request answers, `Reduce` declares the host fold the dispatch applies to the raw readback, and `Groups(width, height, layers)` derives the full three-dimensional workgroup count from the row's own workgroup shape so no caller computes a dispatch dimension and a six-face cube dispatches in one call.
...
public sealed partial class WgslKernel {
    public static readonly WgslKernel NoiseField        = new("noiseField",        source: Wgsl.NoiseField,                      layout: Field,   x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.NoiseLatticeZero, Raster.Oracle.NoiseSolidLatticeZero));
    public static readonly WgslKernel CheckerField      = new("checkerField",      source: Wgsl.CheckerField,                    layout: Field,   x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.CheckerParity));
    public static readonly WgslKernel GradientField     = new("gradientField",     source: Wgsl.GradientField,                   layout: Sampled, x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.GradientMidpoint));
    public static readonly WgslKernel MathFold          = new("mathFold",          source: Wgsl.MathFold,                        layout: Binary,  x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.MathFloorMod, Raster.Oracle.MathSafeDiv, Raster.Oracle.MathSqrtClamp));
    public static readonly WgslKernel MixFold           = new("mixFold",           source: Wgsl.MixFold,                         layout: Binary,  x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.MixMultiply));
    public static readonly WgslKernel EquirectToCube    = new("equirectToCube",    source: Wgsl.FaceDir + Wgsl.EquirectToCube,   layout: Sampled, x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.CubeFaceCentre));
    public static readonly WgslKernel IrradianceSh      = new("irradianceSh",      source: Wgsl.IrradianceSh,                    layout: Sampled, x: 64u, y: 1u, reduce: KernelReduce.PartialSum, oracle: Seq(Raster.Oracle.ConstantIrradiance));
    public static readonly WgslKernel PrefilterSpecular = new("prefilterSpecular", source: Wgsl.PrefilterSpecular, layout: Sampled, x: 8u,  y: 8u, reduce: KernelReduce.None,       oracle: Seq(Raster.Oracle.ConstantPrefilter, Raster.Oracle.SplitDomePrefilter));
    ...
    public Seq<OracleVector> Oracle { get; }
...
internal static class Wgsl {
    ...
    private const string NoiseFieldBody = """
        ...
        // The ONE constant this module declares itself rather than deriving: the Fibonacci angle drives the two
        ...
        const FIBONACCI_ANGLE : f32 = 2.399963229728653;
        ...
        // Fibonacci-spiral unit vector as a DECLARED STAND-IN: same radius, same anchor, same hash index, a different
        ...
        fn randVec(h: i32) -> vec2<f32> {
            let a = f32((h >> 1u) & 255) * FIBONACCI_ANGLE;
...
- Owner: `OracleVector` the per-kernel fixture row; `Oracle` the fixture table.
...
- Entry: `public static Fin<Unit> Prove(PressDevice device, WgslKernel kernel)` runs the op-code totality gate, then dispatches EVERY fixture on the row's own `Oracle` sequence and compares each reduced output's leading `Expected.Length` elements against the fixture's tolerance; `Oracle.All` DERIVES from `WgslKernel.Items` — the `Projection/benchmarks` parity workload and the proof suite iterate one public projection, and no hand-maintained second roster exists to drift.
...
public static class Oracle {
    internal static readonly OracleVector NoiseLatticeZero = new("noise-lattice-node",
        ...
        Expected: new[] { 0f, 0f, 0f, 1f }, Width: One, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector NoiseSolidLatticeZero = new("noise-solid-lattice-node",
        KernelUniform.Empty.Extent(One, One).F32(2.0).F32(2.0)
            .F32(0.5).F32(0.0).F32(2.0).F32(1.0)
            .F32(0.0).F32(0.0).F32(1.0)
            .U32(1).I32(1337).Code(WgslOpCode.Of(NoiseBasis.Perlin)).Code(WgslOpCode.Of(FractalMode.FBm))
    ...
    internal static readonly OracleVector CheckerParity = new("checker-parity",
        KernelUniform.Empty.Extent(Two, Two).U32(2).Pad(1).Vec4(0.0, 0.0, 0.0, 1.0).Vec4(1.0, 1.0, 1.0, 1.0),
        Input: Seq<ReadOnlyMemory<float>>(),
        Expected: new[] { 0f, 0f, 0f, 1f, 1f, 1f, 1f, 1f }, Width: Two, Height: Two, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector GradientMidpoint = new("gradient-midpoint",
        KernelUniform.Empty.Extent(Two, One).U32(2).U32(0),
        Input: Seq<ReadOnlyMemory<float>>(new[] { 0f, 0f, 0f, 1f, 1f, 1f, 1f, 1f }),
        Expected: new[] { 0.25f, 0.25f, 0.25f, 1f, 0.75f, 0.75f, 0.75f, 1f }, Width: Two, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector MathFloorMod = new("math-floor-mod",
        KernelUniform.Empty.Extent(Two, One).U32(4).U32(0).U32(0).Pad(3),
        Input: Seq<ReadOnlyMemory<float>>(
            new[] { -1.5f, -1.5f, -1.5f, 1f, 1f, 1f, 1f, 1f },
            new[] { 1f, 1f, 1f, 1f, 0f, 0f, 0f, 1f }),
        Expected: new[] { 0.5f, 0.5f, 0.5f, 1f, 0f, 0f, 0f, 1f }, Width: Two, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector MathSafeDiv = new("math-safe-div",
        KernelUniform.Empty.Extent(Two, One).U32(3).U32(0).U32(0).Pad(3),
        Input: Seq<ReadOnlyMemory<float>>(
            new[] { 1f, 1f, 1f, 1f, 3f, 3f, 3f, 1f },
            new[] { 0f, 0f, 0f, 1f, 2f, 2f, 2f, 1f }),
        Expected: new[] { 0f, 0f, 0f, 1f, 1.5f, 1.5f, 1.5f, 1f }, Width: Two, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector MathSqrtClamp = new("math-sqrt-clamp",
        KernelUniform.Empty.Extent(Two, One).U32(7).U32(0).U32(0).Pad(3),
        Input: Seq<ReadOnlyMemory<float>>(
            new[] { -4f, -4f, -4f, 1f, 4f, 4f, 4f, 1f },
            new[] { 0f, 0f, 0f, 1f, 0f, 0f, 0f, 1f }),
        Expected: new[] { 0f, 0f, 0f, 1f, 2f, 2f, 2f, 1f }, Width: Two, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector MixMultiply = new("mix-multiply",
        ...
        Expected: new[] { 0.25f, 0.25f, 0.25f, 1f }, Width: One, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    internal static readonly OracleVector CubeFaceCentre = new("cube-face-centre",
        ...
        Width: One, Height: One, Layers: Six, Band: OracleBand.Abs(1e-6));
    ...
    internal static readonly OracleVector ConstantIrradiance = new("constant-irradiance",
        ...
        Expected: new[] {
            ...
            0f, 0f, 0f, 0f, 0f, 0f },
        Width: SixtyFour, Height: ThirtyTwo, Layers: One, Band: OracleBand.Abs(1e-4));

    internal static readonly OracleVector ConstantPrefilter = new("constant-prefilter",
        KernelUniform.Empty.Extent(Four, Two).U32(8).U32(4).F32(0.5).U32(64).Pad(2),
        Input: Seq<ReadOnlyMemory<float>>(Constant(8 * 4)),
        Expected: new[] { 1f, 1f, 1f, 1f }, Width: Four, Height: Two, Layers: One, Band: OracleBand.Rel(1e-5));

    internal static readonly OracleVector SplitDomePrefilter = new("split-dome-prefilter",
        ...
        Expected: new[] { 0.25f, 0.5f, 0.75f, 1f }, Width: One, Height: One, Layers: One, Band: OracleBand.Abs(1e-6));

    public static Seq<OracleVector> All => toSeq(WgslKernel.Items).Bind(static kernel => kernel.Oracle);

    public static Fin<Unit> Prove(PressDevice device, WgslKernel kernel) =>
    ...
    static Fin<Unit> Compare(WgslKernel kernel, OracleVector fixture, KernelReadback readback) {
        ReadOnlySpan<float> expected = fixture.Expected.Span;
        ReadOnlySpan<float> actual = readback.Output.Span;
        if (actual.Length < expected.Length) { return Fin.Fail<Unit>(new RasterFault.Device($"<oracle-output-short:{kernel.Key}:{actual.Length}<{expected.Length}>")); }
        for (int i = 0; i < expected.Length; i++) {
            ...
            if (Math.Abs(actual[i] - expected[i]) > bound) { return Fin.Fail<Unit>(new RasterFault.Device($"<oracle-divergence:{fixture.Name}:{i}:{actual[i]:R}!={expected[i]:R}>")); }
        }
...
- Entry: `ChainPlan.Of(Seq<ChainNode> nodes)` is the ONE planner — it gates the sequence structurally, scans the live ranges, and answers the assignment with its slot count and terminal slot; `ChainPlan.Admits(long texels)` is the ONE footprint gate a plan admission reads; `PressDevice.Dispatch(ChainPlan plan, Dimension width, Dimension height, Dimension layers)` executes it against a pool allocated once.
...
public sealed record ChainPlan(Seq<ChainStep> Steps, int Slots, int Terminal) {
    ...
    public static Fin<ChainPlan> Of(Seq<ChainNode> nodes) {
        if (nodes.IsEmpty) { return new RasterFault.Device("<chain-empty>"); }
        int count = nodes.Count;
        for (int at = 0; at < count; at++) {
            ChainNode node = nodes[at];
            if (node.Operands.Count != node.Kernel.Reads) {
                return new RasterFault.Device($"<chain-operand-arity:{at}:{node.Kernel.Key}:{node.Operands.Count}!={node.Kernel.Reads}>");
            }
            if (node.Kernel.Reads + 1 > StorageBindingsPerStage) {
                return new RasterFault.Device($"<chain-step-bindings:{node.Kernel.Key}:{node.Kernel.Reads + 1}:{StorageBindingsPerStage}>");
            }
            foreach (int operand in node.Operands) {
                if (operand < 0 || operand >= at) { return new RasterFault.Device($"<chain-operand-order:{at}:{operand}>"); }
            }
        }
    ...
    public Fin<Unit> Admits(long texels) =>
        texels * BytesPerTexel > BindingFloor
            ? Fin.Fail<Unit>(new RasterFault.Device($"<chain-slot-over-binding-floor:{texels * BytesPerTexel}:{BindingFloor}>"))
            : Bytes(texels) > Footprint
                ? Fin.Fail<Unit>(new RasterFault.Device($"<chain-footprint:{Slots}:{Bytes(texels)}:{Footprint}>"))
                : Fin.Succ(unit);
}
```
...
public sealed partial class PressDevice {
    ...
    Fin<KernelReadback> Pooled(
        ChainPlan plan, Dimension width, Dimension height, Dimension layers,
        Func<Seq<nint>, nint, Fin<int>> body);

    Fin<Unit> Record(
        Seq<nint> pool, nint encoder, nint pipeline, ChainStep step,
        Dimension width, Dimension height, Dimension layers);
}
```
