...
- [01]-[ENCODING]: `PackKind` six-row modality discriminant with per-kind active-channel columns; `EncodingChannel` `[SmartEnum<string>]` eight-row feature lattice; `ChannelDtype` `[SmartEnum<int>]` width/tolerance rows with bulk span `Pack`/`Unpack` arms over `TensorPrimitives`; `PackOp` `[Union]` over one dtype-strided `EncodedStore` byte arena; the `Apply` fold composing live `Vectors` channel readers and the `Reconciliation.Apply` `Encode` digest; `EncodedGeometry` the descriptor-dispatched typed-view carrier with its `RoundTripWitness`.
...
public sealed partial class ChannelDtype {
    ...
    public void Pack(ReadOnlySpan<float> raw, Span<byte> stored) {
        ...
        for (int i = 0; i < raw.Length; i++) stored[i] = (byte)MathF.Round(Math.Clamp(raw[i], 0f, 1f) * 255f);
    }
    ...
    public void Unpack(ReadOnlySpan<byte> stored, Span<float> restored) {
        ...
        for (int i = 0; i < stored.Length; i++) restored[i] = stored[i] / 255f;
    }
}
...
public static class Encode {
    ...
    static double ChannelError(float[] raw, ReadOnlySpan<byte> stored, ChannelDtype dtype) {
        ...
        return TensorPrimitives.MaxMagnitude<float>(restored);
    }
...
flowchart LR
    ...
    PackChannels -->|position / normal / curvature| Vectors["Vectors MeshSpace / VectorCloudMetric.OrientedNormals"]
