...
- Packages: `Rasm/Domain/results` (`Op`, `Ring<T>`, `Lease<T>`), `Domain/hooks` (`HookBinding`); `Rasm.Rhino/Document/events` (`RhinoPoint`, `PluginKey`, `MountRegistry`); `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-runtime.md` (`Skin` and its ten load-phase virtuals, `MainRhinoIcon`, `ApplicationName`).
