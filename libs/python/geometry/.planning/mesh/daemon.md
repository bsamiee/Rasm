...
- [REPRESENTATION_KEY]: the daemon's `TessellationResult.content_key` is the SOURCE-bytes-plus-`IdentityPolicy` re-tessellation CACHE key (the `seed=Nothing` policy-folded settings seed distinguishing a coarse from a fine pass), distinct from the GLB WIRE representation key the imported-geometry path aligns to the C# `csharp:Rasm.Element/Graph#NODE_MODEL` `RepresentationContentHash` keyed map: that wire key is the seed-zero (`seed=Some(0)`) `XxHash128` over the per-element GLB bytes — the bare C# `XxHash128.HashToUInt128(span)` `GeometryHash` discipline the authored-geometry key also rides — surfaced as the `evidence/identity#SEED_REPRODUCTION` `GLB_BY_KEY` corpus row [5] the C#/Python/TypeScript peers agree on byte-for-byte. The daemon mints neither key locally — it composes the shared `evidence/identity#IDENTITY` `ContentIdentity.of` owner, the `seed=Some(0)` seam selecting the seed-zero wire-key path that equals the Bim-projected `RepresentationContentHash` entry by reference (decoded, never re-minted) while the policy-folded cache key keeps the re-tessellation short-circuit; a policy-folded seed on the GLB wire key (which would diverge from the seed-zero authored-geometry key) is the named drift defect.
