...
- Packages: Rasm (`Rasm.Domain` `ContentHash.Of` + `CanonicalWriter` — the one federation hasher and the ONE preimage alphabet; `EpsilonPolicy.ZeroTolerance` the grid-free lane; the `MerkleRange.Of` peer anti-entropy digest folds the same entry and is never a content-key mint), NodaTime, LanguageExt.Core, Thinktecture.Runtime.Extensions, BCL inbox.
...
| :-----: | :------------ | :---------- | :-------------------------------------- | :----------------------------------------------- |
|  [01]   | `set`         | Ordered     | genuine conflict — one value loses      | total order on `(Hlc, Origin)`; loser recorded   |
...
public sealed partial class ParitySlot {
    public static readonly ParitySlot HlcCell = new("hlc-cell", "dotnet:Version/commits#CRDT_WIRE", mintedHere: true);
    public static readonly ParitySlot CommitKey = new("commit-key", "dotnet:Version/commits#COMMIT_DAG", mintedHere: true);
    public static readonly ParitySlot CrdtOp = new("crdt-op", "dotnet:Version/commits#CRDT_WIRE", mintedHere: true);
    public static readonly ParitySlot CrdtOpSet = new("crdt-op-set", "dotnet:Version/commits#CRDT_ALGEBRA", mintedHere: true);
    public static readonly ParitySlot ElementSet = new("elementset", "dotnet:Query/lane#ELEMENT_SET_ALGEBRA", mintedHere: false);
...
public static class ContentParityCorpus {
    ...
    public static Validation<Error, Unit> Reconcile(HashMap<ParitySlot, ParityVector> local, HashMap<ParitySlot, ParityVector> peer) =>
        toSeq(peer.AsIterable()).Traverse(slot => local.Find(slot.Key) is { IsSome: true, Case: ParityVector held } && held.Holds(slot.Value)
