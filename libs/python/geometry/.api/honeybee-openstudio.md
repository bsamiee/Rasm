...
- result axis: the translation result folds into the `expression` `Result` rail; `reset_properties=True` discards inbound OpenStudio `.properties.energy` and rebuilds defaults, `reset_properties=False` merges onto existing honeybee properties.
...
- Accept: in-process translation feeding the energy-modeling owner when the native `openstudio` SDK is importable; the OSW->OpenStudio bridge driving `dragonfly-energy`'s `run_urbanopt` per district feature; the CPU-bound `model_to_openstudio` offloaded through `anyio.to_thread.run_sync` under an `opentelemetry` span; the translation result folded into the `expression` `Result` rail
