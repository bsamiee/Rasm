...
// Min-layer-time cooling (Lookahead sibling): Off leaves the stream untouched; the fraction floor keeps a stretched layer's feeds usable.
public readonly record struct CoolingPolicy(double MinLayerSeconds, double MinFeedFraction) {
    public static readonly CoolingPolicy Off = new(MinLayerSeconds: 0.0, MinFeedFraction: 0.2);
}
...
public sealed record PostPolicy(
    double KerfWidth, LeadStyle Lead, double LeadRadius, double TabWidth, double TabSpacing, double SimplifyEpsilon, double PierceTime, double AssistPressure,
    double MeltTemp, double FeedCeiling, int MaxAxes, double RemovalRate, FeedPolicy Feed, BiarcPolicy Biarc, LookaheadPolicy Lookahead, CompPolicy Comp,
    CoolingPolicy Cooling, Arr<Loop> Profiles) {
    public static readonly PostPolicy Canonical = new(
        KerfWidth: 0.2, Lead: LeadStyle.Arc, LeadRadius: 3.0, TabWidth: 0.5, TabSpacing: 50.0, SimplifyEpsilon: 0.01, PierceTime: 0.0, AssistPressure: 0.0,
        MeltTemp: 0.0, FeedCeiling: 1200.0, MaxAxes: 3, RemovalRate: 0.0, Feed: FeedPolicy.Canonical, Biarc: BiarcPolicy.Canonical,
        Lookahead: LookaheadPolicy.Canonical, Comp: CompPolicy.Disabled, Cooling: CoolingPolicy.Off, Profiles: Arr<Loop>.Empty);
...
public static class Posting {
    public static Fin<CutProgram> Post(FabricationResult result, PostPolicy policy, FabricationInput input) =>
        Resolve(input.Dialect, input.Process) is var dialect && !dialect.Admits(input.Process.Modality)
            ...
            : result.Switch(
                ...
                motion:           static (s, m) => Fin.Succ(new CutProgram(Cooling(Lookahead(m.Moves.Map(ToBlock), s.policy), s.policy), s.dialect)),
                placement:        static (s, p) => Sequence(p.Parts, s.policy).Bind(ordered => Condition(ordered, s.policy, s.dialect)));

    public static PostDialect Resolve(Option<PostDialect> over, ProcessKind process) => over.IfNone(process.Dialect);
    ...
    static Fin<CutProgram> Condition(Seq<PartTransform> ordered, PostPolicy policy, PostDialect dialect) =>
        ordered.Map(t => (t.PartId, Loop: policy.Placed(t)))
            .Find(static x => !x.Loop.Closed).Match(
                Some: x => Fin.Fail<CutProgram>(new FabricationFault.OpenLoop(FabConcern.Program, x.PartId).ToError()),
                None: () => ordered.Map(t => Kerf(policy.Placed(t), policy).Bind(r => Simplify(r, policy)).Bind(s => Compensate(s, policy)))
                    .TraverseM(identity).As()
                    .Map(kerfed => new CutProgram(Cooling(Lookahead(Spindle(policy).Concat(kerfed.Bind(loop => CutPath(loop, policy))).Add(End()), policy), policy), dialect)));
    ...
    static Fin<Loop> Kerf(Loop contour, PostPolicy policy) {
        ...
        return PolygonAlgebra.Offset(Seq(contour.AsCcw()), hole ? -0.5 * policy.KerfWidth : 0.5 * policy.KerfWidth, OffsetEnds.Polygon)
            .Bind(rings => rings.HeadOrNone().Match(
                Some: r => Fin.Succ(r),
                None: () => Fin.Fail<Loop>(new FabricationFault.KerfCollision(contour.At(0), policy.KerfWidth).ToError())));
    }
    ...
    // Internal-visible: Posting/optimization re-runs THIS sweep after a feed rewrite — the ONE kinematic certifier, never re-rolled.
    internal static Seq<GWord> Lookahead(Seq<GWord> blocks, PostPolicy policy) {
    ...
    // Cooling min-layer-time redistribution: layers split at Z-word steps; a layer whose integrated
    // cutting time undercuts the floor scales its non-rapid feeds down uniformly (never up), so the
    // Lookahead certificate holds and the prior layer cools before the next lands.
    static Seq<GWord> Cooling(Seq<GWord> blocks, PostPolicy policy) =>
        policy.Cooling.MinLayerSeconds <= 0.0
            ? blocks
            : Layers(blocks).Bind(layer => {
                double t = LayerSeconds(layer, policy);
                if (t <= 0.0 || t >= policy.Cooling.MinLayerSeconds) return layer;
                double scale = Math.Min(1.0 / Math.Max(1e-9, policy.Cooling.MinFeedFraction), policy.Cooling.MinLayerSeconds / t);
                return layer.Map(w => w.F.IsSome && w.Command != GCommand.Rapid ? w with { F = w.F.Map(f => f / scale) } : w);
            });

    static Seq<Seq<GWord>> Layers(Seq<GWord> blocks) {
        var fold = blocks.Fold(
            (Done: Seq<Seq<GWord>>(), Open: Seq<GWord>(), Z: Option<double>.None),
            static (st, w) => w.Z.Match(
                Some: z => st.Z.Match(
                    Some: prev => Math.Abs(z - prev) > 1e-9
                        ? (st.Done.Add(st.Open), Seq(w), Some(z))
                        : (st.Done, st.Open.Add(w), st.Z),
                    None: () => (st.Done, st.Open.Add(w), Some(z))),
                None: () => (st.Done, st.Open.Add(w), st.Z)));
        return fold.Open.IsEmpty ? fold.Done : fold.Done.Add(fold.Open);
    }

    static double LayerSeconds(Seq<GWord> layer, PostPolicy policy) {
        GWord[] b = layer.ToArray();
        double t = 0.0;
        for (int k = 1; k < b.Length; k++)
            if (b[k].F.IsSome && b[k].Command != GCommand.Rapid) t += 60.0 * Span(b, k) / Math.Max(1e-9, b[k].F.IfNone(policy.FeedCeiling));
        return t;
    }
