...
- transitive: `Microsoft.CSharp` 4.7.0 + `System.Data.DataSetExtensions` 4.5.0 — both are net10 IN-BOX facades (the `dynamic` runtime binder and the `DataSet` LINQ extensions), so they pull NO `Xbim.Essentials` and stand up NO parallel IFC model; the central pins win resolution
...
- rail: `properties#PROPERTY_SETS` (the standard-Pset template source the typed property model resolves against)

## [02]-[PUBLIC_TYPES]

[PUBLIC_TYPE_SCOPE]: the definition catalogue (`Xbim.Properties`)
- rail: properties#PROPERTY_SETS
...
| [INDEX] | [SURFACE]                                                                       | [ENTRY_FAMILY] | [RAIL]                                            |
| :-----: | :------------------------------------------------------------------------------ | :------------- | :----------------------------------------------- |
|  [01]   | `propertyDef.PropertyType.PropertyValueType` → `IPropertyValueType` (`TypePropertySingleValue`/`…EnumeratedValue`/…) | read | the property's value-type kind |
|  [02]   | `propertyDef.ValueDef` → `ValueDef` / `propertyDef.ValueDef.DataType` → `DataType` (`DataTypeEnum`) | read | the property's IFC data type and allowed values |
...
- a `PropertyDef : QuantityPropertyDef` carries its `PropertyType` (the value-type kind — `TypePropertySingleValue`/`TypePropertyEnumeratedValue`/`TypePropertyBoundedValue`/`TypePropertyListValue`/`TypePropertyTableValue`/`TypePropertyReferenceValue`/`TypeComplexProperty`) and its `ValueDef` (the `DataType`/`DataTypeEnum` and allowed `Value`/`ValueRangeDef`/`EnumList`); a `QtoDef` carries its `QtoTypeEnum` quantity kind
...
[INTEGRATION_STACK]:
- properties owner: `Semantics/properties#PROPERTY_SETS` reads this catalogue as the OFFLINE authoritative template source — its `PropertyKey` `[SmartEnum<string>]` static anchors (`Pset_WallCommon`/`Pset_SlabCommon`/`Qto_*`) are the well-known set names, and the `Xbim.Properties` `PropertySetDef.PropertyDefinitions`/`QtoSetDef.QuantityDefinitions` supply each value's declared `DataType` and value-type kind so a `PropertyValue.Measure` carries its IFC data type rather than a stringly-typed text; the `properties` page resolves `propertyDef.ValueDef.DataType` (`DataTypeEnum`) into the `PropertyValue` union arm and `qtoDef.QuantityType` (`QtoTypeEnum`) into the `QuantityKind` axis
- bSDD union: the `properties` page's `PropertyKey.Resolve(element, BsddClass)` UNIONS the live `Semantics/classification#BSDD_RESOLUTION` `BsddClass.Properties` dictionary rows OVER this offline catalogue (dictionary-wins on a name collision) — `Xbim.Properties` is the OFFLINE/canonical buildingSMART template (deterministic, schema-versioned, network-free) and bSDD is the LIVE dictionary that enriches/overrides it; the two stack as static-template + live-dictionary, the offline catalogue the always-available floor when bSDD is unreachable, never a second property store
- quantity seam: a `QtoDef.QuantityType` (`QtoTypeEnum.Q_LENGTH`/`Q_AREA`/`Q_VOLUME`/`Q_WEIGHT`/`Q_COUNT`/`Q_TIME`) maps to the `properties` `QuantityKind` axis and thence to the `UnitsNet` typed quantity (`api-unitsnet`) — `Qto_SlabBaseQuantities.NetVolume` is a `QtoTypeEnum.Q_VOLUME`, derived as a `UnitsNet` `Volume` from the kernel geometry and SI-coerced; the template declares the KIND, `UnitsNet` carries the dimensioned value, the kernel geometry supplies the magnitude
- applicability seam: `PropertySetDef.ApplicableClasses` (`ApplicableClass { ClassName, PredefinedType }`) is the IFC entity/predefined-type the set applies to — the `properties` `PropertyKey` row's applicable `IfcClass`/`IfcDomain` is corroborated against this, so a `Pset_WallCommon` resolves only for a `Model/elements#ELEMENT_MODEL` `BimElement` whose class is in the applicable set
...
[LOCAL_ADMISSION]:
- the standard-Pset template source is `Definitions<PropertySetDef>(Version).LoadAllDefault()` (and the `QtoSetDef` twin) loaded ONCE at startup and held as the frozen template catalogue; the `properties` owner reads `definitions[psetName]` and `setDef[propertyName]` to resolve a property's declared `DataType`/value-type kind, never hand-coding a `Pset_*` property table
- a property's data type is `propertyDef.ValueDef.DataType` (`DataTypeEnum`) and its value-type kind is `propertyDef.PropertyType.PropertyValueType` — the typed `PropertyValue` union arm is chosen from these, never a stringly-typed guess
...
[RAIL_LAW]:
- Package: `Xbim.Properties` (6.0.11, CDDL-1.0, `requireLicenseAcceptance`, pure-managed `lib/netstandard2.1` AnyCPU IL; `Microsoft.CSharp` + `System.Data.DataSetExtensions` net10 in-box facades, NO `Xbim.Essentials`)
...
- Accept: `Definitions<PropertySetDef>(Version).LoadAllDefault()` (+ the `QtoSetDef` twin) as the frozen offline template source the `properties#PROPERTY_SETS` `PropertyKey` anchors read, the declared `DataTypeEnum`/`QtoTypeEnum` lowered into the typed `PropertyValue`/`QuantityKind` model, the `ApplicableClasses` corroborating the `PropertyKey` domain, and the catalogue unioned UNDER the bSDD live dictionary with dictionary-wins precedence
