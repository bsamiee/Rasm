...
sources into subscribable cells the TS planning state/view owners consume (`state/.planning` envelope/stream/availability
folds name `@effect-atom/atom` as the cell bridge). Grounded from installed `node_modules` `.d.ts` declarations;
exact spellings. The React binding `@effect-atom/atom-react` (0.5.0) is a separate package on the view/UI surface and
is not reflected here. The package re-exports seven modules as namespaces through `index.d.ts`; each section below is
one module. Cross-package owners (`Effect`, `Layer`, `Runtime`, `Stream`, `Scope`, `Schema`, `Option`, `Exit`, `Cause`,
`Context`, `Duration`, `SubscriptionRef`, `Subscribable`, `KeyValueStore`, `Reactivity`, and the `@effect/rpc`
`Rpc`/`RpcClient`/`RpcGroup` + `@effect/platform` `HttpApi*` families) resolve against `api-effect-core.md`,
`api-effect-rpc.md`, and `api-effect-platform.md`.
...
- tier: `neutral` (Nx `@nx/enforce-module-boundaries` — framework-agnostic; depends only on `effect`, `@effect/experimental` `Reactivity`, `@effect/platform` `KeyValueStore`/`HttpApi*`, `@effect/rpc`)
- rail: state-cell
...
// TypeId/WritableTypeId are NOT exported (module-internal); only their string values + the SerializableTypeId/ServerValueTypeId consts below are public
declare const TypeId: TypeId            // internal
type TypeId = "~effect-atom/atom/Atom"  // internal
type WritableTypeId = "~effect-atom/atom/Atom/Writable" // internal; declare const WritableTypeId also internal
...
const all: <const Arg extends Iterable<any> | Record<string, any>>(results: Arg) => Result</* mapped success / Result.Failure over the collection */ any, any>
...
providing it. The module re-exports the registry-bound `toStream`/`toStreamResult`/`getResult` in registry-first form.

```ts contract
// @effect-atom/atom/Registry
declare const TypeId: TypeId               // internal (not exported); only the [TypeId] key on Registry is observable
...
declare const TypeId: TypeId              // internal (not exported); only the [TypeId] key on ReadonlyRef is observable
...
procedures become atoms. `AtomRpcClient` is a tag carrying a flattened `RpcClient.Flat<Rpcs, RpcClientError>` whose
`query` produces a read `Atom`/`Writable<PullResult>` (streaming) and `mutation` produces an `AtomResultFn`; it also
exposes the `layer` and the `AtomRuntime`. `Tag` is the constructor binding a group + protocol layer to a client id.
...
interface AtomRpcClient<Self, Id extends string, Rpcs extends Rpc.Any, E> extends Context.Tag<Self, RpcClient.RpcClient.Flat<Rpcs, RpcClientError>> {
  ...
  readonly mutation: <Tag extends Rpc.Tag<Rpcs>>(arg: Tag) =>
    /* non-stream → */ Atom.AtomResultFn<{ readonly payload: Rpc.PayloadConstructor<…>; readonly reactivityKeys?: …; readonly headers?: Headers.Input | undefined }, _Success["Type"], _Error["Type"] | E | _Middleware["failure"]["Type"]>
  readonly query: <Tag extends Rpc.Tag<Rpcs>>(tag: Tag, payload: Rpc.PayloadConstructor<Rpc.ExtractTag<Rpcs, Tag>>, options?: { readonly headers?: Headers.Input | undefined; readonly reactivityKeys?: …; readonly timeToLive?: Duration.DurationInput | undefined }) =>
    /* stream → */ Atom.Writable<Atom.PullResult<_A["Type"], …>, void> /* | non-stream → */ Atom.Atom<Result.Result<_Success["Type"], _Error["Type"] | E | _Middleware["failure"]["Type"]>>
}
...
`AtomHttpApiClient` is a tag carrying a `HttpApiClient.Client<Groups, ApiE, never>` whose endpoints become atoms:
`query(group, endpoint, request)` returns a read `Atom<Result>` and `mutation(group, endpoint)` returns an
`AtomResultFn`, both keyed by `HttpApiGroup`/`HttpApiEndpoint` names with an optional `withResponse` flag.
`Tag` binds an `HttpApi` + `httpClient` layer to a client id.
...
`@effect-atom/atom/Hydration` — SSR transfer. `dehydrate` snapshots a registry's serializable atoms into
`DehydratedAtom[]` (with an `encodeInitialAs` policy for `Result.Initial`), `toValues` projects them to the
`DehydratedAtomValue` wire shape (`key`/`value`/`dehydratedAt`/optional `resultPromise`), and `hydrate` re-applies a
dehydrated state into a registry.
...
- `TypeId`/`WritableTypeId` (`Atom`, `Result`, `Registry`, `AtomRef`) are declared without `export` — module-internal brand keys, public only as the `[TypeId]` interface keys; only `SerializableTypeId`/`ServerValueTypeId` (`Atom`) are exported brand consts. Documented as internal above.
...
- `Result.all`, the `Result.Builder` conditional `on*` blocks, and the `AtomRpc`/`AtomHttpApi` `query`/`mutation` conditional-type result bodies are abbreviated to their resolved shapes; the full conditional inference is in `Result.d.ts:293-321`, `AtomRpc.d.ts:25-34`, `AtomHttpApi.d.ts:26-40`.
