...
`scikit-fem` and `interpax` carry no package, so the FEM eval/project fold and the 2-D/3-D resample ride the worker band; the numpy nodal-readout floor and the `np.interp` 1-D resample floor run unconditionally on runtime.
