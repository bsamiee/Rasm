...
- Auto: `AssetTransfer` is one `Schema.Literal` format axis — `csv`/`xlsx`/`pdf`/`archive` — selecting the export codec (`papaparse` for csv, `exceljs` for xlsx, `jspdf` for pdf, `jszip` for archive, `sharp` for image transform inside the asset pipeline), so the four export libs are one codec row each on the axis and a parallel exporter per format is the deleted form; a transfer failure rejects with the node-side `AssetTransferFault` keyed by format and `encode`/`object-store` stage, never the wire-side `interchange` `FaultDetail`, since this surface crosses no .NET wire.
...
class AssetTransferFault extends Schema.TaggedError<AssetTransferFault>()("AssetTransferFault", {
  format: AssetTransferFormat,
  stage: Schema.Literal("encode", "object-store"),
  cause: Schema.Unknown,
}) {}

interface AssetTransfer {
  readonly export: (format: AssetTransferFormat, rows: Stream.Stream<Record<string, unknown>>) => Effect.Effect<Uint8Array, AssetTransferFault>;
}
