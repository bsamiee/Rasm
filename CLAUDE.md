...
- Codex is a first-class worker, never a bent fallback: hand it ONE self-contained prompt (it inherits none of this conversation) carrying an explicit completion bar — the enumerated deliverables and the proof each is met; the bar bounds scope and layer, never depth — let it drive its own tools to completion, and take its final message as the result - relay a read leg's report, apply a write leg's edits as delivered. Verify load-bearing claims against source before acting; never silently rewrite, re-judge, or wrap its output in extra ceremony.
...
- Inside workflows the model parameter takes only Claude models, so a codex leg rides a thin sonnet wrapper making ONE blocking `codex` MCP call, labeled with the real worker (`terra:`/`sol:`/`luna:`/`gemini:`); the wrapper contract, batching economics, and receipt law are the workflow-creator codex-lanes reference.
...
[IMPORTANT]:
- [NEVER]: Aim `git sparse-checkout` or any working-tree-narrowing git mutation at this repo. External pulls clone into session scratch with explicit targets (`git clone <url> <abs-dir>`, `git -C <abs-dir> ...`), clean there, and copy only the cleaned folder into a destination the dispatching agent minted.
