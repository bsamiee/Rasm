...
```bash copy-safe
rg -n "AUTHORING_CONTRACT|SECTION_CARDINALITY|Accepted:|Rejected:|Near miss:|Command:|Expected|Proof gap:|Source of truth:|Availability:|Execution:|Profile:" docs/standards --glob '*.md' --glob '!_reports/**'
```
...
- `git diff --check -- docs/standards/_reports/standards-structure-notation-060626/track-adversarial-synthesis/03-information-structure-critic.md` passed after this report was created.
