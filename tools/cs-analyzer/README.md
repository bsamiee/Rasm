...
Every rule fires inside the doctrine scope it targets alone. `CompilationFacts.ScopeOf` resolves scope per analyzed symbol, in priority order: a `[CspScope]` type marker, then the `csp.scope` per-tree config, then the `build_property.CspScope` MSBuild value or an assembly-level `[CspScope]` marker; an undeclared scope is `Domain`. `Directory.Build.props` derives `CspScope` from project classification — `Test` for test, testkit, and scenario-kit projects, `Boundary` for plugin and bridge projects, `Tooling` for benchmark, analyzer, and analyzer-contract projects — and libraries and apps declare it explicitly.
