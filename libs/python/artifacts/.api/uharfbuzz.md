...
| [06] | `Buffer.set_script_from_ot_tag(tag)` / `set_language_from_ot_tag(tag)` / `set_message_func(fn)` / `reset()` / `clear_contents()` / `not_found_glyph` / `invisible_glyph` / `replacement_codepoint` | buffer control | set script/language from OT tag; install a shaping-trace message callback; reset/clear; override the .notdef/invisible/replacement glyph |
| [07] | `shape(font, buffer, features=None, shapers=None)` | shaping entry | run OpenType layout shaping; mutates buffer in-place |
| [08] | `Buffer.serialize(font, format=BufferSerializeFormat.TEXT, flags=BufferSerializeFlags.DEFAULT)` -> `str` | result text | human/JSON-readable shaped glyph sequence |
| [09] | `Buffer.glyph_infos` / `Buffer.glyph_positions` | shaped result | `list[GlyphInfo]` / `list[GlyphPosition]` read after `shape` |
