...
- `@bufbuild/protoc-gen-es`(same `catalog`): emits committed `_pb.ts` modules under `libs/typescript/contracts`; `Format.event.protobuf` binds the vendored CloudEvents single and batch schemas directly while excluding both from `Proto.registry`.
