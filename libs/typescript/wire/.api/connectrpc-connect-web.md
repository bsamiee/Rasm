...
- version: `2.1.2`
- license: `Apache-2.0`
- peer: `@connectrpc/connect 2.1.2` (exact — the `Transport`/`Interceptor` types; `.api/connectrpc-connect.md`), `@bufbuild/protobuf ^2.7.0` (the JSON/binary read-write option types; `.api/bufbuild-protobuf.md`)
- effect-peer: none direct — the returned `Transport` is consumed by `createClient`, wrapped in `effect` at `invoke/client` (`.api/effect.md`)
...
- runtime: browser-primary (fetch API); node ≥18 capable via global `fetch`; NO `grpc-web-text` (no base64/XHR streaming — that lane is `@effect/platform-browser` XHR)
...
| :-----: | :---------------------------------------------------------- | :-------------- | :------------------------------------------------------------ |
|  [01]   | `ConnectTransportOptions`                                   | transport policy| `invoke/client` connect arm — `useBinaryFormat` (default JSON), `useHttpGet` |
|  [02]   | `GrpcWebTransportOptions`                                   | transport policy| `invoke/client` grpc-web arm — `useBinaryFormat` (default binary) |
...
| :-----: | :------------------------------------------------------------------------------ | :------------- | :------------------------------------------------------- |
|  [01]   | `createConnectTransport(options: ConnectTransportOptions): Transport`            | connect arm    | `invoke/client` `protocol:"connect"` — JSON-default, `useHttpGet`-capable |
|  [02]   | `createGrpcWebTransport(options: GrpcWebTransportOptions): Transport`            | grpc-web arm   | `invoke/client` `protocol:"grpc-web"` — binary-default, gRPC gateway compat |
...
- Stack with `effect` (`.api/effect.md`) + `kernel/config`: `baseUrl`/`defaultTimeoutMs`/`useBinaryFormat`/`protocol` are `Config`-decoded policy values from `host/config`, never hardcoded; the transport is constructed once at the `invoke/client` composition root and the client wrapped in `Effect.tryPromise`/`Stream.fromAsyncIterable` there.
...
[RAIL_LAW]:
- Package: `@connectrpc/connect-web`
- Owns: the two browser `Transport` factories (`createConnectTransport`, `createGrpcWebTransport`) and their near-identical option records that together are the `invoke/client` protocol axis
