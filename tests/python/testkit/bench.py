...
def run_bench(benchmark: BenchmarkFixture, row: BenchCase, size: int) -> object:
    """Measure one benchmark case, enforce its dispersion and absolute-budget gates, and return the pedantic result."""
    ...
    iterations = min(_ITERATIONS_CAP, max(1, ceil(_CALIBRATION_FLOOR_NS / max(probe_dt, 1)))) if (row.iterations == 1 and not row.fresh_per_round and probe_dt < _CALIBRATION_FLOOR_NS) else row.iterations
    ...
    benchmark.extra_info.update(rss_delta_bytes=proc.memory_info().rss - rss_before, rss_after_bytes=proc.memory_info().rss, cpu_percent_delta=proc.cpu_percent(interval=None), budget_ms=row.budget_ms, observed_ms=observed_ms, rel_iqr=rel_iqr, iterations=iterations, size=size)
...
def run_registry(rows: Sequence[BenchCase]) -> Callable[..., None]:
    """Return a parametrized benchmark function stamped with the caller module for collection."""
...
def pytest_benchmark_update_json(config: pytest.Config, benchmarks: object, output_json: dict[str, object]) -> None:
    ...
    breaches = [(key, ratio) for key, series in series_by_key.items() if len(segments := _potts_segments(series)) >= 2 and (ratio := _regression(segments)) > _REGRESSION_TOLERANCE]
    (pytest.fail("sustained benchmark regression: " + "; ".join(f"{file}::{label}-{size}: +{ratio:.1%}" for (file, label, size), ratio in breaches), pytrace=False) if breaches else None)
