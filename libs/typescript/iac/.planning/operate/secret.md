...
class Secrets extends Tier {
  ...
  constructor(name: string, args: Secrets.Args, opts?: pulumi.ComponentResourceOptions) {
    ...
    this.config = new doppler.BranchConfig(name, {
      ...
      name: pulumi.interpolate`${environment.slug}_${name}`,
    }, this.child())
...
- Law: `Secrets.webhook(owner, name, { url, secret, payload }, child)` delivers signed secret-change events to the named endpoint; `secret` binds the Doppler-generated HMAC value the receiver verifies and `payload` shapes the provider-native delivery.
