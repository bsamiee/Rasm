...
- No WRITE metric reports added BYTES; the roster is files, rows, and partitions alone, so a per-commit byte volume exists only for `OPTIMIZE` through its `filesAdded` `totalSize` and a writer wanting written bytes measures them on its own sink.
- `get_add_actions` answers the WHOLE current snapshot's active file roster, never the commit's added files — two two-file appends answer four rows — so summing its `size_bytes` measures the table's TOTAL on-disk volume, a state fact; reading it as one commit's evidence prices every append at the whole table's size.
