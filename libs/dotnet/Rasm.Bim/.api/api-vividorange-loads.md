...
- `LanguageExt.Core`: a foreign load quantity enters through the shared `MeasureValue` gate; malformed value evidence remains `ElementFault.ValueRejected`, while provider throws stay exact exceptional errors through `Try.lift`, with no `.ToError()` remint
