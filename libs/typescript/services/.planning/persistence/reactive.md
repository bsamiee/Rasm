...
- [01]-[REACTIVE_QUERY]: owns the `ReactiveQuery` `SqlClient.reactive`/`reactiveMailbox` surface, the entity-keyed `InvalidationKey` row table, the `Reactivity.mutation` key-publish edge writers wrap, and the dependent-read re-run `Stream`.
