...
|  [03a]  | `CollectionFilterResult.failure`           | property `-> dict[str, FailureInfo]`                                                                 | per-member `FailureInfo` map (singular `failure`, keyed by member name; `result`/`collect_all`/`count`/`index` are the other members) |
