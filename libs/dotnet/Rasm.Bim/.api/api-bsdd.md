...
- `UnitsNet`(`libs/dotnet/.api/api-unitsnet.md`): the `dimension` string (`"L M T I Θ N J"`) and the seven `dimension*` exponents map to a `BaseDimensions` 7-vector, and `units[]`/`qudtCodes[]` select the source unit, so a measured property coerces to the SI-base `MeasureValue` through `Quantity.GetQuantitiesWithBaseDimensions(BaseDimensions)`.
