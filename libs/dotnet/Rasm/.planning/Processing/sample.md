...
- Entry: `SampleKind.PowerCcvt` mints `PowerCcvtPolicy.Preset()`; a preset `with { … }` re-admitted through `policy.Admit` is the whole override surface, and a throwing factory inside a static initializer is the deleted form.
