...
- Failure vocabulary is closed and owned — `Literal` set, `StrEnum`, or `@tagged_union` — never bare `str` for a multi-cause domain, never an `Exception` subclass in the carrier slot.
...
- Concurrent children run inside `anyio.create_task_group()`; `asyncio.gather`/`create_task` and closure-captured accumulation lists are findings. Deadlines are `CancelScope` (`fail_after`/`move_on_after`), never a `timeout=` parameter threaded inward.
