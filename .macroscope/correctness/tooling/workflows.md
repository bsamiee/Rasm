...
Operator and workflow code is deterministic orchestration judged as production source: schema-typed agent outputs, bounded loops, every stage writing its product to disk and returning its output path. Bare promises without await ownership, unbounded polling, wall-clock branching, subagents instructed to wait, and hand-authored exit semantics beside a typed envelope are findings.
...
- Per-run scratch directories mint deterministically from the normalized target set, never clock or randomness, so a resume rehydrates the same directory; a stage returns its output path with its full product on disk, never bulk product inline.
...
- A child process whose stdout feeds a parser launches with the runner's reporter silenced — `pnpm exec` rides `pnpm --silent exec`, because the reporter writes warnings to stdout ahead of child output and corrupts the parsed stream.
- A decoder folding failure to a success-shaped empty over non-empty input launders upstream corruption into a zero-results pass; the fold is legal only where the invocation record retains the raw stream as evidence, and a fold discarding it lands the undecodable input as a typed defect row.
