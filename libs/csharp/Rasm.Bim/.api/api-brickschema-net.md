...
- units leg: a Brick `Measureable` point value (`Temperature`/`ActivePower`/`Pressure`) reconciles to a canonical quantity through `UnitsNet` (`libs/csharp/.api/api-unitsnet.md`) at the boundary, so a BACnet engineering-unit reading and a model expectation agree before a `BrickBehavior` computes conformance.
...
- identity leg: a `SaveSchema` JSON-LD document's UTF-8 bytes feed `System.IO.Hashing` `XxHash3`/`XxHash128` (`libs/csharp/.api/api-hashing.md`) for the systems-graph snapshot content key, joining the IFC/energy-model exports on one content-identity rail; the `Deprecation*` shapes carry the schema-version migration the diff reads.
