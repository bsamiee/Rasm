...
- [1]-[SECRET_STORE]: owns the `SecretRef` provider axis, the `SecretStore` `Effect.Service` over the `ConfigProvider` boundary, the TTL-leased `SubscriptionRef`-backed rotation cache, and the tenancy-scoped `AuditReceipt`.
