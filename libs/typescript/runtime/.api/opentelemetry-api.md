...
- rail: observability/contract
- Rasm composition never calls the global singletons — the facade registers through its own Layer — so the admitted call surface is the guard/mint tier alone.

| [INDEX] | [SURFACE]                                                      | [ENTRY_FAMILY] | [CONSUMER_BOUNDARY]                                   |
| :-----: | :------------------------------------------------------------- | :------------- | :---------------------------------------------------- |
|  [01]   | `trace` / `metrics` / `context` / `propagation` / `diag`       | global entry   | SDK-internal registration; no Rasm module calls these |
|  [02]   | `isSpanContextValid(ctx)` / `isValidTraceId` / `isValidSpanId` | guard          | validate a decoded carrier before continuation        |
...
- Stack with `opentelemetry-core.md`: the propagator triad implements `TextMapPropagator`; `parseTraceParent` yields this package's `SpanContext`.
...
- Accept: type imports at the SDK-bridge and continuation seams; guard/mint calls (`isSpanContextValid`, `createTraceState`) at carrier admission; one deduped tree copy
