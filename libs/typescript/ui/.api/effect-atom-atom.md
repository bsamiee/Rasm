...
- `AtomRuntime` is the DI boundary: `Atom.runtime(layer)` memoizes one `Layer` build (`defaultMemoMap`), and every atom built through it resolves services from that shared graph — one app runtime shares `wire`/`edge` clients, config, and telemetry across all client state.
