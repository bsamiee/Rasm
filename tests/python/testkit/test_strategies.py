...
from tests.python.testkit.strategies import resolve
...
@_BUDGET
@given(resolve(Patch).filter(lambda patch: patch.note is msgspec.UNSET), resolve(Patch).filter(lambda patch: not isinstance(patch.note, msgspec.UnsetType)))
...
@_BUDGET
@given(resolve(Unbounded).filter(lambda value: value.count < 0 and value.ratio < 0.0), resolve(Unbounded).filter(lambda value: value.count > 0 and value.ratio > 0.0))
