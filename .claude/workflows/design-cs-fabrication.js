export const meta = {
  ...
  description: 'Ephemeral Phase-1 design pass for the Fabrication campaign: survey (4 plane agents re-verifying the register on disk + 1 FEASIBILITY PROBE: the OpenCAMLib C-shim/P-Invoke binding over the shipped C++-ABI archives, ruling the [V5] surface-engine siting at decision time) -> 4 complete folder/page-set blueprints under the brief lenses (namespace-true partition, kernel-seam-first, package-mining-first, output-first over the four flagship programs) -> 3 judge lenses with FIVE disqualifying gates (verdict disposition, folder=namespace 1:1, >=2 pages per folder, seam acyclicity under the two-node entry rule, both kernel-DECISION dispositions carried) -> synthesize winner+grafts -> red-team decide gate that WRITES RASM-CS-FABRICATION-DECISION.md in rebuild-engine-native action vocabulary with the surface-engine hinge ruled from probe evidence and the kernel-owned hinges CARRIED dual, never ruled -> salvage (4 draft miners + overturn auditor + 1 writing integrator). About 20 agents, peak concurrency 5. args = {brief, out, scratch} with defaults; a string arg overrides the brief path.',
  whenToUse: 'One-shot: turn RASM-CS-FABRICATION-BRIEF.md into RASM-CS-FABRICATION-DECISION.md — the binding folder/page-set/namespace/fault-cluster/seam-ledger/entry-contract/leg-partition blueprint the durable rebuild engine executes structure-first. Launch when the Fabrication campaign opens, after the cs track review has landed. Delete after the campaign lands.',
  phases: [
    ...
    { title: 'Judge', detail: '3 lenses; disqualifying gates: disposition completeness, folder=namespace 1:1 + >=2 pages, two-node acyclicity, dual kernel dispositions carried' },
    { title: 'Synthesize', detail: '1 agent merges the winning blueprint with every accepted graft' },
    { title: 'Decide', detail: '1 red-team gate: attack, rule the OpenCAMLib hinge on probe evidence, carry the kernel-owned hinges dual, then WRITE the DECISION (a-f contract)' },
...
const JUDGE_SCHEMA = { type: 'object', additionalProperties: false, required: ['lens', 'scores', 'winner'], properties: {
  ...
  scores: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['draft', 'gate_dispositions', 'gate_partition', 'gate_acyclic', 'gate_dual', 'capability', 'collapse'], properties: {
    draft: { type: 'string' }, gate_dispositions: { type: 'string', enum: ['pass', 'fail'] }, gate_partition: { type: 'string', enum: ['pass', 'fail'] },
    gate_acyclic: { type: 'string', enum: ['pass', 'fail'] }, gate_dual: { type: 'string', enum: ['pass', 'fail'] }, capability: { type: 'number' },
...
const PRE = 'Rasm monorepo. THE BRIEF ' + BRIEF + ' (repo root) is binding law — read it COMPLETELY first: [00]-[SHARED_LAW] (the 4-proof verdict, ' +
  ...
  'the one distance-field lane [V8], hull [V11], the [04] re-scopes — Fabrication CONSUMES, never re-implements), RASM-CS-PERSISTENCE-BRIEF.md ' +
  ...
  'assemblies. ROSTER STANCE: INTEGRATION-FIRST — the four zero-consumer admissions (PicoGK, OcctNet.Wrapper, DSTV.Net, SharpVoronoiLib) are ' +
  'integration MANDATES; a removal needs a per-package redundancy/abandonment/license proof. Your stance is HOSTILE: the interiors are craft ' +
  'but the production system is naive until disk proves otherwise; the register is an initial pointer, never a ceiling.'
...
const surveyPrompt = (lane, scope, rows) => [PRE, 'TASK: READ-ONLY SURVEY, lane = ' + lane + '. Scope: ' + scope + '. Deep-read fully. ' +
  ...
  'cross-package edge), both naivety axes, unwired pipeline claims, stub arms, phantom members (assay-verify where restored assemblies exist). ' +
...
const draftPrompt = (lens, thesis, dossiers, probes) => [PRE, ENGINE_VOCAB, ROW_SCHEMA, 'TASK: author ONE COMPLETE folder/page-set blueprint ' +
  ...
  'against an INFEASIBLE probe loses at judge). Honor every ruled default unless you carry disk proof a departure is stronger. The kernel-owned ' +
  'hinges (the [V2]d offset-rail siting, the medial radius carriage) are NOT yours to rule — carry BOTH dispositions explicitly on the affected ' +
  'rows, signature-locked at the seam, per the brief\'s dual-disposition law. The [V1] recommended 13-folder floor is a floor: exceed it only ' +
...
const judgePrompt = (lens, focus, drafts) => [PRE, 'TASK: ADVERSARIAL JUDGE, lens = ' + lens + '. ' + focus + ' Read the brief, the probe in ' +
  ...
  'data on the entry vocabulary never a page-compose edge), gate_dual (both kernel-DECISION dispositions carried on every affected row, ' +
  'signature-locked, neither ruled), capability 0-10, collapse 0-10, elegance 0-10. A gate fail DISQUALIFIES. Name the GRAFTS precisely. ' +
...
const synthPrompt = (drafts, judges) => [PRE, ENGINE_VOCAB, ROW_SCHEMA, 'TASK: SYNTHESIZE the blueprint of record. Judges: ' +
  ...
  'two-node rule, and dual-carried. WRITE ' + SCRATCH + '/synthesis.md in the full row schema. Return pointer, base, grafts, open items. NO ' +
...
const decidePrompt = (synthesis, probes) => [PRE, ENGINE_VOCAB, ROW_SCHEMA, 'TASK: RED-TEAM DECIDE GATE + EMIT. Attack the synthesis at ' +
  ...
  'landing failure modes and the leg\'s internal ordering); attack every departure from a ruled default. THE HINGE RULINGS: the OpenCAMLib ' +
  ...
  'fallback), recorded in probe_rulings and baked into the Toolpath/surface row; the kernel-owned hinges (offset-rail siting, medial radius ' +
  'carriage) are NOT yours — verify both dispositions stay carried and signature-locked, and record them as carried in probe_rulings. Then ' +
  ...
  'disposition table. Agent-facing declarative; every open item RULED or explicitly carried-dual. The DECISION stands alone without ' +
  SCRATCH + '/.'].join('\n')
...
const integratePrompt = (decision, findings) => [PRE, 'TASK: WRITING INTEGRATOR — the salvage close. Findings: ' + findings + '. Judge each on ' +
  ...
  'DECISION disposition-complete, partition-true, acyclic, and dual-carried after every edit.'].join('\n')
...
const LANES = [
  ...
  { lane: 'upstream-bindings', scope: 'the Fabrication-relevant clauses of all six upstream documents: RASM-CS-GEOMETRY-BRIEF.md ([V10] a-e, ' +
    '[V3], [V5], [V8], [V11], [04] re-scopes), RASM-CS-PERSISTENCE-BRIEF.md (the durable-rows card), RASM-CS-COMPUTE-BRIEF.md (waste rollup, ' +
...
const LENSES = [
  ...
  { lens: 'kernel-seam-first', thesis: 'work FORWARD from the five owner-to-consumer re-framings and the kernel planes ([V10] toolpath gate, ' +
    ...
    'the dual dispositions carried, and the structure follows the consumption map.' },
...
const JUDGES = [
  { lens: 'law-gates', focus: 'Own the disqualifying gates hardest: dispositions counted row-by-row, the partition gate (namespace 1:1, page ' +
    'floors, the six ratifications), two-node acyclicity walked edge-by-edge, dual-disposition carriage verified on every affected row, ' +
