...
- Receipt: the `Solve` `ComputeReceipt` case carries the constitutive-model key beside the physics key, the return-map iteration count, and the measured return-map residual, all of which `ConstitutiveResult` publishes as columns the assembly fold forwards. NAMED GAP: the integration-point count, the consistent-tangent condition, the contact active-set size, the penetration residual, and the multiplier-update count are MEASURED here and land on `ContactResult`/`ConstitutiveResult`, and `Runtime/receipts#RECEIPT_UNION` carries no column for any of them — a widening of the `Solve` case is owed at that owner and no claim of it is made here.
