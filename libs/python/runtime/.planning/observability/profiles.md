...
class Profiles:
    ...
    def phase(tags: Mapping[str, str]) -> AbstractContextManager[None]:
        ...
        # wrapper restores the prior thread tags on exit — never a hand-paired add/remove_thread_tag ladder.
...
- Owner: `BenchmarkReceipt` carries the macro-benchmark evidence — subject, rounds, warmup, the latency quartet, throughput — and `Bench.run` is the one runner: warmup rounds discarded, measured rounds folded into per-round wall samples, quantiles derived at read, never fold state.
- Cases: `BenchMode` splits the two measurement contracts — `LATENCY` times each round, `THROUGHPUT` counts completed rounds over the measured window — one runner, the mode a policy value.
...
class BenchmarkReceipt(Struct, frozen=True):
    ...
    @classmethod
    def of(cls, subject: str, mode: BenchMode, warmup: int, samples_ms: tuple[float, ...]) -> "BenchmarkReceipt":
...
class Bench:
    @staticmethod
    def run(subject: str, op: Callable[[], object], *, mode: BenchMode = BenchMode.LATENCY, rounds: int = 32, warmup: int = 4) -> BenchmarkReceipt:
        ...
        return BenchmarkReceipt.of(subject, mode, warmup, tuple(Block.range(rounds).map(timed)))
```

## [04]-[JOB]

- Owner: `JobRun.bounded` is the offline-job envelope — install with the hand-built job resource and the high-interval `JOB_SIGNAL_PROFILE`, run the body under the `boundary` fence, then drive the telemetry drain so every buffered signal exports before exit. Its drain is the settled telemetry flush-then-shutdown accumulate fold; a body fault outranks a drain fault, and a drain fault surfaces on a clean body.
...
- Boundary: the envelope composes `Telemetry.install`/`shutdown` and constructs no provider of its own; long-lived daemons keep the profile-keyed `SIGNAL_PROFILE` rows and never ride this envelope.
