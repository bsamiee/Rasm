...
public readonly partial struct Classification {
    ...
    static partial void ValidateFactoryArguments(
        ...
        ref string source, ref Option<LocalDate> editionDate, ref Option<string> title) {
    ...
    public static Fin<Classification> Of(
        ...
        string source = "", Option<LocalDate> editionDate = default, Option<string> title = default) =>
...
public static class ClassificationOps {
    extension(Seq<Classification> classifications) {
        ...
        // query `system` is normalized through the SAME Trim().ToLowerInvariant() the ValidateFactoryArguments admission
        // applies and compared Ordinal — the policy the (System) identity comparer (ComparerAccessors.StringOrdinal)
        // holds — so the read and the stored identity agree exactly, never a stored-Ordinal-vs-query-IgnoreCase drift.
        public Seq<Classification> ClassificationsIn(string system) =>
            classifications.Filter(c => string.Equals(c.System, system.Trim().ToLowerInvariant(), StringComparison.Ordinal));
