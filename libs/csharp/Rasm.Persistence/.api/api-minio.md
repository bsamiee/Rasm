...
|  [04]   | `PutObjectArgs`                | object write        | own: `WithStreamData`/`WithObjectSize`/`WithFileName`/`WithProgress`; inherited: `WithBucket`/`WithHeaders` (`BucketArgs`), `WithObject` (`ObjectArgs`), `WithContentType`/`WithTagging`/`WithServerSideEncryption`/`WithLegalHold` (`ObjectWriteArgs`/`EncryptionArgs`) — chunked multipart auto-managed |
