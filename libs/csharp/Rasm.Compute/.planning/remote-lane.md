...
- [COMPOSED_CREDENTIAL]: the `Grpc.Core.Api` `CallCredentials.Compose(CallCredentials, CallCredentials)` member stacking a node identity onto a hub identity for a farm node dialing through a hub stays uncatalogued in `api-grpc-client.md`; the `CredentialPolicy` composed-identity arm rides the catalogued `CallCredentials.FromInterceptor` plus `ChannelCredentials.Create(ChannelCredentials, CallCredentials)` until the `Compose` overload lands in the catalogue, composed call credentials transmitting only over a TLS channel row.
