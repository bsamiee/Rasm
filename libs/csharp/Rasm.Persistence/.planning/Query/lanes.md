...
- [1]-[LANE_AXIS]: seven-case lane union, profile capability fold, and the key-value shape.
- [2]-[DOCUMENT_LANE]: document mapping law, per-column JSON index policy, and index asymmetry.
- [3]-[SEARCH_LANES]: vector and full-text query shapes; embedding identity; provider rows.
- [4]-[GEO_LANES]: geometry residence rows, container window law, and driver-native concerns.
- [5]-[ANALYTICAL_LANE]: analytical read lane; live attach; parquet, tabular, shred, and DuckLake catalog.
...
|   [9]   | `jsonb_matches_schema` CHECK       | postgres-server | pg_jsonschema CHECK invariant | declared-shape documents validated server-side at write; extension declared on `Schema/ddl#EXTENSION_DDL`; falls to application-side when the compiled extension is unavailable |
...
|   [2]   | HNSW + IVFFlat + diskann     | postgres-server | HNSW/IVFFlat metadata on schema rows; diskann DDL on `Store/server#SEARCH_PROVISIONING` | approximate routes; exact scan stays the baseline; diskann scales disk-backed ANN beyond RAM-resident HNSW and complements never replaces |
...
|   [8]   | binary-vector metrics        | postgres-server | `VectorMetric.Order` projects through the catalogued `HammingDistance` / `JaccardDistance` `DbFunctions` over a `bit` column | `VectorMetric.Hamming` / `.Jaccard` bit rows project bit distance via the EF translator, exact scan baseline; the `VectorMetric.Op` column carries the catalogued server operators (`HammingDistance`→`<~>` / `JaccardDistance`→`<%>`, `.api/api-pgvector-ef.md` rows [5]/[6]) for the `HybridRetrieve.Fuse` raw-CTE leg |
|   [9]   | BM25 overlay                 | postgres-server | `@@@ pdb.parse(...)` query-builder match, `pdb.score(<key_col>)` / `pdb.snippet` via `FromSql` / `SqlQuery`; BM25 index DDL on `Store/server#SEARCH_PROVISIONING` | high-power FTS overlay beside the always-present native baseline; `@@@` RHS is always a `pdb.*` builder, score/snippet anchor on the `key_field`; `FullTextQuery.Bm25` selects it; native FTS answers when pg_search is not preloaded |
