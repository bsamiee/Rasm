...
[STACKING]:
- LIVE-session merge authority (not the durable spine): `SubscribeLocalUpdate` yields each local delta `byte[]`; the AppHost transport broadcasts it on a session-ephemeral topic and a live peer `Import`s it — `LoroCs` is the in-session merge authority, so the `Collab/` rail holds NO custom merge logic, retiring the bespoke `NotebookCrdt` LWW algebra. This Loro-native wire is IN-SESSION only (ephemeral, never persisted — the Persistence bit-parity law binds durable truth, not session state). `ExportMode.Updates(VersionVector)` is the catch-up delta a reconnecting live peer requests to sync an ACTIVE epoch; a cold start seeds its epoch from the ledger replay window, never from a Loro byte read.
- DURABLE truth is Persistence-owned, NOT a Loro byte (`[V2]` charter, `Collab/sync.md`): the durable stream is the typed EDIT-INTENT op stream — AppUi's own domain ops projected onto Persistence-owned `OpLogEntry`/`SyncOpKind` rows through the `Version/ledger` changefeed (`history.md`'s `RevertibleOp` -> `SyncOpKind` discipline generalized). The `LoroDoc.Export(Snapshot)` blob survives ONLY as a DERIVABLE content-keyed cold-start ACCELERATOR in the Persistence blob lane — its content key composes the kernel `ContentHash.Of` one-hasher entry (`Rasm/ARCHITECTURE.md:87`), reconstructible from the op-log alone, deletable, never system-of-record; a `System.IO.Hashing` `XxHash128` mint over the snapshot bytes is the deleted form. Cold-load = the replay window decoded from the ledger and replayed into a FRESH `LoroDoc` in log order; `ExportShallowSnapshot(Frontiers)` is the gc-trimmed accelerator variant for bounded history.
...
- tree/map into the graph canvas, inspector, and tables: `LoroTree` (movable hierarchy with per-node `GetMeta` map) is the co-edit data seam for `Editing/graph.md`'s NodeEditor canvas (`[V8]`c, `api-nodeeditor.md`) under ONE bidirectional projection with echo suppression — a subscriber diff discriminates `EventTriggerKind.Local`/`Import`/`Checkout` so a local UI mutation commits as tree ops without re-applying its own echo and a remote `Import` applies to the ReactiveUI graph model without re-emitting (the feedback loop is the deleted form); it also backs the inspector/outline surfaces. `LoroMovableList.Mov` is the stable-id row-reorder the `Avalonia.Controls.DataGrid` table edit needs (reorder without delete+insert losing identity); `LoroCounter` is the conflict-free aggregate tile.
