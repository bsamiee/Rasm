...
- Law: `ByClassification` carries the RESOLVED branch closure as payload (`Seq<Classification>` — a single classification is a one-element branch): classification hierarchy is bSDD-resolved at `Rasm.Bim`, the contract carries identity and never ancestry, so the resolver hands the closed branch IN and no contract-side crosswalk exists to be structurally blind.
- Law: `WalkDepth` resolves to ONE declaration — this contract's; Persistence `Query/lane#ELEMENT_SET_ALGEBRA` composes it directly (its former same-named `[ValueObject<int>]` is deleted, and the `SelectionFault.Depth` band re-keys at its `Selections.Depth` wrapper over the contract admission); Persistence's evaluator law binds every consumer's closure verdict — a `Closure` arm is the QUESTION and the answering fold must be a genuine bounded transitive walk, never an opaque-leaf pass-through.
- Output: `Selection<TKey>.StoreKey` is the STORE's certification key for a store-answered selection (Persistence's preimage-keyed `KeySelection` survives as its own wrapper columns); a set-algebra derivation answers `StoreKey: None` because the derived set was never store-certified — the consumer re-keys through `PredicateKey.Key` where it needs one.
- Packages: Thinktecture.Runtime.Extensions (`[Union]`/`[ValueObject<int>]`), LanguageExt.Core (`Seq`/`Option`/`Fin`/`Error`), Generator.Equals (`[Equatable]`/`[OrderedEquality]` on the stored value shapes), `Rasm` (`Try.lift` preserving the regex-construction cause, the kernel writer through the shared `ContentAddress`), `Properties/property#PROPERTY_VALUE` (`PropertyValue`/`PropertyName` the restriction operands), `Properties/quantity#MEASURE_ALGEBRA` (`MeasureValue` the range bound, `MeasureCanon.Measure` the dimensioned bound writer), `Relations/relation#EDGE_ALGEBRA` (`ComposeKind`/`ConnectKind`/`VoidKind`/`AssignKind`/`WireName`), `Classification/classification#CLASSIFICATION_AXIS` (`Classification`/`Discipline`), `Assessment/assessment#ASSESSMENT_NODE` (`AssessmentOutcome`), `Projection/address#CONTENT_ADDRESS` (the streamed key).
...
// --- [MODELS] --------------------------------------------------------------------------
[Equatable]
public readonly partial record struct Selection<TKey>([property: OrderedEquality] Seq<TKey> Keys, Option<UInt128> StoreKey) where TKey : notnull {
