...
- [02]-[EGRESS_PUMP]: `EgressPump` drains one fold past each sink cursor — advance law, dead-letter and replay rows, `EgressReceipt` floor, 8270 band.
- [03]-[EGRESS_SINK]: `Egress.Envelope` projects the CloudEvent every `EgressSink` row delivers, each outcome folding to `DeliveryAck` under the dedup table.
...
using Rasm.Persistence.Store;                     // OutboxCursor / SinkKey / OutboxAdvance (coordination#OUTBOX_CURSOR); RollingWindow (provisioning#SERVER_EXTENSIONS)
