...
[PUBLIC_TYPE_SCOPE]: controlled state slices and their atom-backed update path
...
| [INDEX] | [SYMBOL]                                       | [TYPE_FAMILY]  | [CAPABILITY]                       |
| :-----: | :--------------------------------------------- | :------------- | :--------------------------------- |
|  [01]   | `Updater<T>` / `OnChangeFn<T>`                 | update path    | the `on*Change` callback shape     |
...
- `@tanstack/store` (`.api/tanstack-store.md`): the atom vocabulary this path is typed in — `options.atoms`, `table.baseAtoms`, `table.atoms`, `table.store`, `table.optionsStore`, and `SubscribeSource` all speak it. It is admitted for that vocabulary alone; the fold stays with `@effect-atom` and the adapter atom is the only cell `ui` mints.
