...
|   [3]   | `gp_Vec`      | 3D vector         | direction plus magnitude             |
|   [4]   | `gp_Dir`      | 3D unit direction | normalized direction vector          |
|   [5]   | `gp_Ax1`      | axis              | point plus direction                 |
|   [6]   | `gp_Ax2`      | coordinate frame  | point plus normal plus X direction   |
|   [7]   | `gp_Ax3`      | right-hand frame  | full 3-axis coordinate system        |
|   [8]   | `gp_Trsf`     | affine transform  | rotation, translation, scale, mirror |
|   [9]   | `gp_GTrsf`    | general transform | non-orthogonal affine transform      |
...
|   [5]   | `Geom_Surface`        | abstract surface | UV-parameter 3D surface           |
|   [6]   | `Geom_BSplineSurface` | NURBS surface    | knot/pole/weight B-spline surface |
|   [7]   | `Geom_BezierSurface`  | Bezier surface   | rational Bezier surface           |
|   [8]   | `Geom_Plane`          | plane surface    | infinite plane geometry           |
|   [9]   | `Geom_CartesianPoint` | point geometry   | `gp_Pnt` in geometry hierarchy    |
...
| [INDEX] | [SYMBOL]                      | [TYPE_FAMILY]   | [CAPABILITY]                         |
| :-----: | :---------------------------- | :-------------- | :----------------------------------- |
|   [1]   | `BRepBuilderAPI_MakeVertex`   | vertex builder  | from `gp_Pnt`                        |
|   [2]   | `BRepBuilderAPI_MakeEdge`     | edge builder    | from line/circle/curve plus vertices |
|   [3]   | `BRepBuilderAPI_MakeWire`     | wire builder    | from edges or wires                  |
|   [4]   | `BRepBuilderAPI_MakePolygon`  | polygon builder | closed polyline wire from points     |
|   [5]   | `BRepBuilderAPI_MakeFace`     | face builder    | from surface/wire/plane              |
|   [6]   | `BRepBuilderAPI_MakeSolid`    | solid builder   | from shells                          |
|   [7]   | `BRepBuilderAPI_Sewing`       | sewing          | join open shells into closed solids  |
|   [8]   | `BRepBuilderAPI_Transform`    | transform       | apply `gp_Trsf` to a shape           |
|   [9]   | `BRepBuilderAPI_NurbsConvert` | conversion      | convert shape geometry to NURBS      |
...
| [INDEX] | [SYMBOL]                       | [TYPE_FAMILY]      | [CAPABILITY]                    |
| :-----: | :----------------------------- | :----------------- | :------------------------------ |
|   [1]   | `BRepPrimAPI_MakeBox`          | box                | aligned or oriented box         |
...
|   [6]   | `BRepPrimAPI_MakePrism`        | extrusion          | linear extrusion along vector   |
|   [7]   | `BRepPrimAPI_MakeRevol`        | revolution         | revolution around an axis       |
|   [8]   | `BRepAlgoAPI_Fuse`             | Boolean union      | unite two or more shapes        |
|   [9]   | `BRepAlgoAPI_Cut`              | Boolean difference | subtract tool from object       |
...
| [INDEX] | [SYMBOL]                    | [TYPE_FAMILY]  | [CAPABILITY]                                  |
| :-----: | :-------------------------- | :------------- | :-------------------------------------------- |
|   [1]   | `STEPControl_Reader`        | STEP reader    | shape-only STEP read into topology            |
|   [2]   | `STEPControl_Writer`        | STEP writer    | shape-only STEP write                         |
|   [3]   | `STEPControl_StepModelType` | enum           | `AsIs`/`ManifoldSolidBrep`/… write schema     |
|   [4]   | `IGESControl_Reader`        | IGES reader    | shape-only IGES read                          |
|   [5]   | `IGESControl_Writer`        | IGES writer    | shape-only IGES write                         |
|   [6]   | `STEPCAFControl_Reader`     | STEP CAF read  | assembly, color, name, layer, GD&T transfer   |
|   [7]   | `STEPCAFControl_Writer`     | STEP CAF write | assembly plus metadata write                  |
|   [8]   | `IGESCAFControl_Reader`     | IGES CAF read  | IGES read with color/name/layer modes         |
|   [9]   | `TDocStd_Document`          | XCAF document  | OCAF document root for the assembly tree      |
...
| [INDEX] | [SYMBOL]                   | [TYPE_FAMILY]  | [CAPABILITY]                               |
| :-----: | :------------------------- | :------------- | :----------------------------------------- |
|   [1]   | `BRepMesh_IncrementalMesh` | mesher         | deflection-bounded shape triangulation     |
|   [2]   | `StlAPI_Writer`            | STL writer     | write triangulated shape to STL            |
|   [3]   | `StlAPI_Reader`            | STL reader     | read STL into a shape                      |
|   [4]   | `RWGltf_CafWriter`         | glTF writer    | write XCAF document to glTF/GLB            |
|   [5]   | `RWGltf_CafReader`         | glTF reader    | read glTF/GLB into an XCAF document        |
|   [6]   | `BRepGProp`                | property tool  | `VolumeProperties_s`/`SurfaceProperties_s` |
|   [7]   | `GProp_GProps`             | property accum | mass, centroid, and moments accumulator    |
...
| :-----: | :------------------------------------------------------------------ | :------------- | :------------------------------------ |
|   [1]   | `STEPControl_Reader.ReadFile(path) -> IFSelect_ReturnStatus`        | reader         | load STEP file into the model         |
|   [2]   | `STEPControl_Reader.TransferRoots() -> int`                         | transfer       | transfer all roots to topology        |
|   [3]   | `STEPControl_Reader.TransferRoot(n) -> bool`                        | transfer       | transfer the nth root                 |
|   [4]   | `STEPControl_Reader.NbShapes() -> int`                              | query          | transferred shape count               |
|   [5]   | `STEPControl_Reader.OneShape() -> TopoDS_Shape`                     | result         | all transferred roots as one compound |
|   [6]   | `STEPControl_Reader.Shape(n) -> TopoDS_Shape`                       | result         | the nth transferred shape             |
|   [7]   | `STEPControl_Writer.Transfer(shape, mode) -> IFSelect_ReturnStatus` | writer         | convert shape under a `StepModelType` |
|   [8]   | `STEPControl_Writer.Write(path) -> IFSelect_ReturnStatus`           | writer         | flush STEP file to disk               |
...
| [INDEX] | [SURFACE]                                                                               | [ENTRY_FAMILY] | [CAPABILITY]                      |
| :-----: | :-------------------------------------------------------------------------------------- | :------------- | :-------------------------------- |
|   [1]   | `STEPCAFControl_Reader.ReadFile(path) -> IFSelect_ReturnStatus`                         | reader         | load STEP into the CAF reader     |
|   [2]   | `STEPCAFControl_Reader.SetColorMode(bool)` / `SetNameMode(bool)` / `SetLayerMode(bool)` | config         | select transferred metadata       |
|   [3]   | `STEPCAFControl_Reader.Transfer(doc) -> bool`                                           | transfer       | populate the XCAF document        |
|   [4]   | `STEPCAFControl_Reader.Perform(path, doc) -> bool`                                      | transfer       | read plus transfer in one call    |
|   [5]   | `XCAFDoc_DocumentTool.ShapeTool_s(label) -> XCAFDoc_ShapeTool`                          | accessor       | assembly/instance label tree      |
|   [6]   | `XCAFDoc_DocumentTool.ColorTool_s(label) -> XCAFDoc_ColorTool`                          | accessor       | per-shape color labels            |
|   [7]   | `STEPCAFControl_Writer.Transfer(doc, mode) -> bool`                                     | writer         | stage XCAF document for write     |
|   [8]   | `STEPCAFControl_Writer.Write(path) -> IFSelect_ReturnStatus`                            | writer         | write STEP with assembly metadata |
...
| [INDEX] | [SURFACE]                                 | [ENTRY_FAMILY] | [CAPABILITY]               |
| :-----: | :---------------------------------------- | :------------- | :------------------------- |
|   [1]   | `BRepPrimAPI_MakeBox(dx, dy, dz).Shape()` | box factory    | axis-aligned box solid     |
|   [2]   | `BRepPrimAPI_MakeBox(P1, P2).Shape()`     | box factory    | corner-point box solid     |
|   [3]   | `BRepPrimAPI_MakeSphere(R).Shape()`       | sphere factory | full sphere solid          |
|   [4]   | `BRepPrimAPI_MakeCylinder(R, H).Shape()`  | cylinder       | full cylinder solid        |
|   [5]   | `BRepBuilderAPI_MakeEdge(curve).Edge()`   | edge factory   | edge from a `Geom_Curve`   |
|   [6]   | `BRepBuilderAPI_MakeFace(wire).Face()`    | face factory   | face from a closed wire    |
|   [7]   | `BRepAlgoAPI_Fuse(S1, S2).Shape()`        | Boolean union  | union of two shapes        |
|   [8]   | `BRepAlgoAPI_Cut(S1, S2).Shape()`         | Boolean diff   | difference of two shapes   |
|   [9]   | `BRepAlgoAPI_Common(S1, S2).Shape()`      | Boolean isect  | intersection of two shapes |
...
| [INDEX] | [SURFACE]                                                                     | [ENTRY_FAMILY] | [CAPABILITY]                       |
| :-----: | :---------------------------------------------------------------------------- | :------------- | :--------------------------------- |
|   [1]   | `TopExp_Explorer(shape, TopAbs_ShapeEnum)` + `.More()`/`.Current()`/`.Next()` | explorer       | iterate sub-shapes by kind         |
|   [2]   | `TopExp.MapShapes_s(shape, type, map)`                                        | map builder    | index all sub-shapes of a type     |
|   [3]   | `TopoDS.Edge_s(shape) -> TopoDS_Edge`                                         | downcast       | safe static downcast to edge       |
|   [4]   | `BRepMesh_IncrementalMesh(shape, deflection)`                                 | mesher         | build the shape triangulation      |
|   [5]   | `StlAPI_Writer().Write(shape, path)`                                          | export         | write triangulated shape to STL    |
|   [6]   | `RWGltf_CafWriter(path, binary).Perform(doc, progress)`                       | export         | write XCAF document to glTF/GLB    |
|   [7]   | `BRepGProp.VolumeProperties_s(shape, gprops)`                                 | property       | accumulate mass, centroid, moments |
