...
|  [03]   | precomputed wrap    | upstream `ContentAddress.Of(UInt128)`          | snapshot/chunk digest wrapped, never raw `Create`             |
...
public static class Snapshots {
    ...
    public static IO<SnapshotCatalogRow> Write<T>(ProjectionContext frame, SnapshotRoute route, T value, Func<SnapshotCatalogRow, IO<Unit>> persist) =>
        !route.Format.Admits(typeof(T))
            ...
            .Map(stamped => new SnapshotCatalogRow(stamped.File.Id, route.Kind, route.Format.Codec, route.Format.Compression, ContentAddress.Of(stamped.File.Header.ContentHash), stamped.File.Header.PlainLength, stamped.File.Header.StoredLength, route.SchemaFingerprint, route.Epoch, route.Lineage, route.RetentionClass, route.Classification, stamped.Stamp.Physical, stamped.Stamp.Logical))
