...
Corpus floor is py3.15 — its surface is never flagged as invalid syntax, a missing import, or a hallucinated API, at any severity, because doctrine fences themselves spell these forms. Pinned-newest provider surfaces — anyio, loky, pebble among them — are corpus law, so a released-library semantics claim verifies against the owning doctrine page before flagging, and the page read precedes severity assignment. Every entry below is a verified truth a generic reviewer misfires on; flagging one is a false positive.

## [01]-[LANGUAGE_SURFACE]

- `from builtins import frozendict` is the legislated spelling, and "frozendict is not importable from builtins" is a refuted standing claim at every severity (`docs/stacks/python/language.md` [FROZENDICT_TABLE_SITE]): `frozendict` is a builtin, not a `dict` subclass — insertion-ordered for iteration, order-free for equality and hash, shallowly immutable.
...
- Paren-less chained `except A, B:` is py3.14+ grammar and the ruff-format canonical form; an invalid-syntax finding on it is a false positive at every severity, and re-adding grouping parens fails the format gate.
...
- `tg.start_soon(...)` returns `TaskHandle[T]` and a child result reads `handle.return_value` after group close — the pinned-anyio CHILD_CARRIER law `docs/stacks/python/concurrency.md` rules; "start_soon returns None" is a refuted standing claim at every severity, released-library semantics applied to the pinned surface.
...
- Preview is the five-field contract with the trailing band; a DEGRADED roll double-buffers so capacity never gaps, and demanding NOT_SERVING before the roll advertises a phantom outage.
...
- `expression.collections.Block.map` eagerly materializes and executes its mapper (`Block((*builtins.map(f, self),))`) — an "unexecuted mapper" claim against it is false; the real defect class there is a discarded mapped result with no explicit emission policy.
...
- `importlib.util.find_spec` on a dotted name imports the parent package — the stdlib contract states the parent is automatically imported — so a probe-the-submodule demand against a parent-side no-import gate pays the exact native band the gate defers: the probe rides the top-level distribution name, and unconditional submodules of one distribution make distribution presence the capability unit.
- Single-door jax pages arm process-global configuration on the door's first line (`jax_enable_x64` first, or solves silently yield float32) — the ruled arm-first form; a move-to-application-startup or compile-time-rejection demand against it re-litigates the ruling, and every door writes the same monotone value, so concurrent compiles cannot race to divergent states.
- `segno.helpers` ships `make_make_email_data(to, cc, bcc, subject, body)` and no `make_email_data` — the doubled spelling IS the installed surface (host-api-misspelling-is-truth), so a corrected-spelling demand breaks the binding.
- `ifcopenshell.util.geolocation.get_true_north` is total — `0` for a model declaring no TrueNorth, never `None`, so a rejects-on-None claim is void; the real axis is declaredness read off the provider's own `IfcGeometricRepresentationContext` roster, since the total-`0` collapse fabricates a due-north declaration.
- A laundering claim against a ruled builder-`ValueError` tail arm proves the named interior paths actually raise that class: provider exception kinds inheriting builtin `ValueError` route to the preceding provider-family `except` arm by MRO order, so the sanctioned tail traps caller data alone and a dedicated-admission-exception demand re-litigates the ruled arm.
