...
|  [07]   | `BoundingBox.extmin`/`extmax`/`has_data`   | extents      | corner points and emptiness for `DxfComposed.extent`                    |
...
- layout axis: the `Modelspace`/`Paperspace`/`BlockLayout` IS the `GraphicsFactory` — every drawable is an `add_*` builder on the layout, never a constructed entity class. `doc.blocks.new(name)` mints a reusable symbol as one block `add_blockref` places, never a per-placement geometry copy.
...
- evidence: `DxfComposed` carries the DXF version, per-type entity counts, `BoundingBox` extent, layer/block counts, `audit()` error/fix counts, units, and bytes.
- boundary: IFC semantic modeling stays `dotnet:Rasm.Bim`; SVG geometry meets `svgelements`/`graphic/vector` at the `Path`/`d`-string wire; raster beyond the bundled backends routes to `pyvips`/`pillow`, PDF assembly to `pymupdf`/`pikepdf`, font shaping to `fonttools`/`uharfbuzz`; the AEC standards vocabularies lower from the `drawing/*` owners onto ezdxf tables, and sheet placement/scale stays `composition/sheet`.
