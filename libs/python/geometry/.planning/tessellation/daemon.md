...
- Packages: `ifcopenshell` (`open`/`guess_format`/`by_type`/`schema_identifier`/`geom.settings`/`geom.iterator`/`geom.serializers.gltf`/`geom.serializers.buffer`); the CAD arms compose `step-bridge/cad#BRIDGE` `StepBridge.tessellate` rather than the IFC iterator; runtime (`ServerHost`/`ContentIdentity`/`LanePolicy`/`ReceiptContributor`/`RuntimeRail`).
...
class TessellationDaemon:
    ...
    def _run_ifc(self, request: TessellationRequest) -> MeshResult:
        ...
        with NamedTemporaryFile(suffix=".ifc", delete=False) as src:
            src.write(request.source_bytes)
            model = ifcopenshell.open(src.name, format=ifcopenshell.guess_format(src.name))
