...
[LOCAL_ADMISSION]:
- egress composes `ce.ToAmqpMessageWithUnderscorePrefix(ContentMode.Binary, EventFormat.Json.Formatter)`; every extension attribute is an `EventRoster.Declared` row the branch owner spells ONCE, read back on ingress through the `ToCloudEvent` overload with that identical enumerable — a per-catalog `CreateExtension` call beside it is the hand-spelled twin the owner's roster forecloses.
