...
[STACKING]:
- `api-avalonia.md`: `FluentTheme` extends `Styles`/`IResourceNode` and `Palettes` keys on `Avalonia.Styling.ThemeVariant`; the floor carries the `ThemeVariant.Light` and `.Dark` entries alone, each written from its own resolved theme, while the high-contrast rows bind the SHIPPED `SemiTheme.Desert`/`NightSky` keys — a locally minted `new ThemeVariant("high-contrast", …)` inherits nothing the shipped dictionary defines — and `host-matched` is a preference fold that resolves to a concrete row before any partition is keyed.
...
- `Theme/tokens.md`: `ThemeCatalog.Resolve(ThemeVariantRow, DensityRow, AppearanceSeed, FontChain, PreferenceCell)` generates the paint ladder from the seed through `PaintRole`/`PaintDerivation`, and `ThemeGate.Floor(light, dark)` seats the two resolved themes' `ColorPaletteResources` on `Palettes` while `ThemeGate.ApplyTo` assigns `DensityStyle` from the resolved row.
