...
- Output: `MachinePrincipal` on grant, rotate, and claim, the introspection evidence on introspect, the raw `Response` on call (the caller decodes at its own seam), `void` on retire and handoff — never a raw `TokenEndpointResponse`.
