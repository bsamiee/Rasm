...
- `HttpClientFactory` is the one hook a persistent-queue or custom-certificate handler installs through, and the SHIPPED factory a set value displaces is the sole application point for both `HttpClient.Timeout` and the mutual-auth client — a replacement carries both or loses both.
