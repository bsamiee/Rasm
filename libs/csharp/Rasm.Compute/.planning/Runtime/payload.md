...
// The cluster-LOD chain columns (Level/Error/ParentError/Parent) ride the descriptor beside the cone set:
// Error is the cluster's object-space simplification error (level 0 = 0), ParentError the linked parent's
// error raised to at least the maximum of its children's — MONOTONIC BY CONSTRUCTION (ParentError >= Error),
// so a consumer's screen-space threshold cut (draw the finest level whose Error <= t < ParentError) is
// well-defined, crack-free, and double-draw-free; AppUi reads these encoded columns for its hysteresis LOD
// cut and NEVER re-clusters (it is the sole cluster consumer).
...
public static class Residency {
    ...
    // The cluster-LOD chain (AppUi the demanding consumer, its viewport hysteresis cut the reader): each
    // coarser level simplifies the previous level's index buffer through the Meshopt.Simplify ladder the
    // SimplifyTarget policy drives — result_error scaled to object space by SimplifyScale — re-clusters it,
    // and links each fine cluster to the coarse PARENT whose sphere CONTAINS it (fine sphere within the coarse
    // sphere), falling back to nearest center only when no covering parent exists. The MONOTONIC cut
    // guarantee is enforced at link time: a parent's Error is raised to at least the maximum of its
    // children's before the children stamp it as ParentError (parent error >= child error), so a
    // screen-space threshold cut — draw the finest cluster whose Error <= t < ParentError — is crack-free
    // and double-draw-free BY CONSTRUCTION. The ladder terminates when a level stops shrinking or one
    // meshlet remains; roots carry Parent = -1, ParentError = +inf (never culled by coarseness).
    ...
    // Parent linkage + the monotonic raise: each fine cluster binds the nearest-center coarse cluster whose
    // sphere CONTAINS the fine sphere (d + fineRadius <= coarseRadius), else the nearest center, so a child is
    // never bound outside its parent's coverage; the parent's Error raises to max(parent, children), and the
    // children re-stamp ParentError from the raised value — the invariant structural.
...
- [SPLAT_COMPANION_WIRE]: the `GaussianSplatScan` scan-buffer frame is MINTED on `Runtime/wire#PROTO_VOCABULARY` — the SPZ v4 / SOG v2 stable published specifications ground the packed position/scale/rotation/spherical-harmonic buffer order, the harmonic-degree band, and the per-splat count, byte-mirroring this page's `SplatScan` member set (`FormatKey`/`Positions`/`Scales`/`Rotations`/`Harmonics`/`HarmonicDegree`/`SplatCount`) — so the wire vocabulary is live and the scan crosses the `Runtime/transport#ARTIFACT_FRAMES` `ArtifactFrame` byte seam as a STANDALONE artifact (the `GeometryPayload` oneof carries point-cloud/mesh/voxel only), reassembled and admitted to a `SplatScan` at that wire seam, not in this page. This page's `gaussian-splat` arm is fully implemented over whatever `SplatScan` it is handed — exponent-filtering positions/scales/harmonics and quaternion-filtering rotations; the Python `realitycapture` companion's SPZ/SOG DECODE and the `xxhash` cp315/abi3 wheel stay named sibling-branch facts, the render-side consumer leaf riding `csharp:Rasm.AppUi/Render/reality#SPLAT_SOURCE`.
