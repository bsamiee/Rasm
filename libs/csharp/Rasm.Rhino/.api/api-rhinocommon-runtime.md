...
- `NamedParametersEventArgs()` / `Dispose() : void`; the typed `Set(string name, <T> value)` / `TryGet<T>(string name, out <T> value) : bool` pairs span `string`/`string[]`/`bool`/`int`/`uint`/`double`/`Guid`/`Guid[]`/`Color`/`Point3d`/`Vector3d`/`Line`/`Arc`/`Plane`/`Point3d[]`/`ObjRef[]`/`ViewportInfo`/`GeometryBase`/`MeshingParameters`, with `SetWindowHandle`/`TryGetWindowHandle`/`TryGetWindowImageHandle` for native handles
