...
└── Analysis/              # C#-first discipline-assessment rail over the Rasm.Element ElementGraph
    ...
    ├── Energy.cs          # OpenStudio in-process model + EnergyPlus subprocess via parameterized EnergyToolchain discovery
...
Analysis          ←  csharp:Rasm.Element/Graph             # [READ]: ElementGraph concrete above the seam (no IElementProjection); seam-owned Find/ObjectNodes/Material/MaterialsOf/CompositionOf/MechanicalOf/SectionOf reads + Compute-owned AxisOf/SpacesOf/BoundingSurfacesOf discipline extensions composing them
...
Analysis/aggregator ←  csharp:Rasm.Element/Composition     # [SHAPE]: MaterialComposition/MaterialPropertySet + the seam StcContourFit (single-material acoustic folds seam-owned)
...
Analysis/energy   ⇄  csharp:Rasm.Bim/Exchange              # [SHAPE]: OpenStudio admitted for SIMULATION (Compute) distinct from IFC↔OSM semantic exchange (Bim); api-openstudio needed in Compute/.api
...
Analysis/energy   ←  csharp:Rasm.Bim/Projection           # [PROJECTION]: SemanticProjector bakes the IfcRelSpaceBoundary neutral Generic edges (BoundaryLevel "1st"/"2nd" payload), the Node.Object.BoundaryPolygon analytical surface, the Qto_SpaceBaseQuantities NetFloorArea, and Pset_SpaceCommon.IsExternal that the Compute-owned EnergyGraphReads SpacesOf/BoundingSurfacesOf/ConditionedFloorArea/IsConditioned read (2nd-level preferred when both levels exist) — the wire-name/Qto-name/Pset-name contract is load-bearing
...
Solver/clash      ←  csharp:Rasm/Geometry/Spatial/index    # [WIRE]: AccelerationStructure DECODED node-link wire (Bounds/Nodes) ToAcceleration emits via NodeLinkProjection; Compute decodes through ClashScale.NodeLinkPairs (the BvhPairs/OctreePairs collapse) and never Build/Refit/persists (the SpatialIndex seam owner); CLASH_GOLDEN 160-byte fixture byte-identical on both sides
...
Tensor/dispatch   ⇄  csharp:Rasm/Vectors                   # [SHAPE]: TensorOpKind.Geometry rows mirror the kernel OperatorRow vocabulary; Sensitivity.Operator routes OperatorRow.Apply (forward JVP) / OperatorRow.Adjoint (reverse VJP) over the recorded MeshAdjointSnapshot — kernel owns IntrinsicMesh/DiscreteCalculus, Compute names only MeshAdjointSnapshot/OperatorRow, never a re-assembled DEC matrix
Tensor/residency  ←  csharp:Rasm/Geometry/Drawing          # [CONTENT_KEY]: EncodedGeometry (Descriptors/Payload/Count/RoundTripWitness) wrapped as EncodedTensor — residency, never a re-pack; the witness content hash is the kernel CANONICAL_BYTE_IDENTITY seed-zero XxHash128 composed (Spatial/reconciliation#CANONICAL_BYTE_IDENTITY), no second key minted, no host geometry type named
...
| [08] | energy toolchain | The EnergyPlus subprocess binary is the PARAMETERIZED `EnergyToolchain` discovery (env-var → configured-path → bundled-fallback) version-locked to the OpenStudio SWIG version (3.11.0 → EnergyPlus 25.2.0); dev/CI points `OPENSTUDIO_ENERGYPLUSDIR` at the bundled 25.2.0, a shipped app owns its own provisioning, and `Parametric_Forge` is a dev/CI probe only. | A hardcoded EnergyPlus path; selecting the mismatched standalone 26.1.0; bundling `Parametric_Forge` or the Forge standalone as a shipped dependency; OpenStudio running the solver itself (it builds the model and reads the SQL only). |
| [09] | closed-form + aggregator ownership | Closed-form ISO/EN physics (the thermal/acoustic/fire folds) and the multi-ply `AssemblyAggregator` (ISO 6946 series-U / ISO 12354 layered-STC / rule-of-mixtures / GWP / cost) live in `Analysis`, never the seam; the single-material PURE acoustic folds (`Nrc`/`Saa`/`StcWeighted`) and the `StcContourFit` kernel stay seam-owned, and `Analysis` composes them. | A closed-form ISO/EN fold in `Rasm.Element`; a second `AssemblyAggregator` retained in `Rasm.Materials`; a second `StcContourFit` beside the seam kernel; design-code rules as imperative arms instead of the `DesignCode`×`LimitState` capacity table. |
