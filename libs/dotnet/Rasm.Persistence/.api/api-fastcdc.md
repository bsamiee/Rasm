...
- `Store/blobstore#MULTIPART_TRANSFER`: upload windows span a whole number of content-defined chunks rather than a fixed `PartSize` slice, and the manifest keys dedup against `Query/cache#ARTIFACT_BLOB_INDEX`, so a re-uploaded artifact skips the chunks the index already holds.
