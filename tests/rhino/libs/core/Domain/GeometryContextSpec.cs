...
public sealed class GeometryContextSpec {
    ...
    [Test]
    public void ValidatesOperationInput() {
        GeometryContext context = Context();
        using Curve curve = new Circle(plane: Plane.WorldXY, radius: 1.0).ToNurbsCurve();
        Operation<Curve, double> operation = Operation.Create<Curve, double>(
                operation: static (Curve input) => Fin.Succ(Seq(input.GetLength())))
            .Match(
                Succ: static (Operation<Curve, double> candidate) => candidate,
                Fail: static (Error fault) => throw new AssertionException(message: fault.Message));
        Operation<Curve, double> validated = Operation.WithValidation<Curve, double>(
                program: operation,
                context: context)
            .Match(
                Succ: static (Operation<Curve, double> candidate) => candidate,
                Fail: static (Error fault) => throw new AssertionException(message: fault.Message));
        double[] output = validated.Execute(
                executionMode: Operation.Mode.FailFast,
                input: [curve])
            .ToFin()
            .Match(
                Succ: static (Seq<double> values) => values.ToArray(),
                Fail: static (Error fault) => throw new AssertionException(message: fault.Message));

        Assert.That(actual: output, expression: Has.Length.EqualTo(expected: 1));
        Assert.That(actual: output[0], expression: Is.EqualTo(expected: curve.GetLength()).Within(1e-12));
    }

    [Test]
    public void RejectsOperationInput() {
        GeometryContext context = Context();
        using Curve curve = new Circle(plane: Plane.WorldXY, radius: 0.0001).ToNurbsCurve();
        Operation<Curve, double> operation = Operation.Create<Curve, double>(
                operation: static (Curve _) => Fin.Fail<Seq<double>>(
                    Error.New(code: 9001, message: "validated operation reached invalid geometry")))
            .Match(
                Succ: static (Operation<Curve, double> candidate) => candidate,
                Fail: static (Error fault) => throw new AssertionException(message: fault.Message));
        Operation<Curve, double> validated = Operation.WithValidation<Curve, double>(
        ...
        Assert.That(
            actual: validated.Execute(
                    executionMode: Operation.Mode.FailFast,
                    input: [curve])
                .ToFin()
                .Match(
                    Succ: static (Seq<double> _) => false,
                    Fail: static (Error fault) => !fault.HasCode(code: 9001)),
            expression: Is.True);
    }

    [Test]
    public void PreservesValidatedOperationFailure() {
        GeometryContext context = Context();
        using Curve curve = new Circle(plane: Plane.WorldXY, radius: 1.0).ToNurbsCurve();
        Operation<Curve, double> operation = Operation.Create<Curve, double>(
                operation: static (Curve _) => Fin.Fail<Seq<double>>(
                    Error.New(code: 9002, message: "downstream failure")))
            .Match(
                Succ: static (Operation<Curve, double> candidate) => candidate,
                Fail: static (Error fault) => throw new AssertionException(message: fault.Message));
        Operation<Curve, double> validated = Operation.WithValidation<Curve, double>(
        ...
        Assert.That(
            actual: validated.Execute(
                ...
                .Match(
                    Succ: static (Seq<double> _) => false,
                    Fail: static (Error fault) => fault.HasCode(code: 9002)),
            expression: Is.True);
    }
