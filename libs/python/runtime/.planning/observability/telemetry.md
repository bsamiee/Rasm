...
lazy import grpc  # the OTLP gRPC exporter's transitive (runtime RULINGS): reached for `grpc.Compression`, the exporter's own `compression=` type, and only a selected GRPC row reifies it
...
# OTLP/gRPC re-drive set, PINNED on every gRPC exporter row. That exporter owns a whole retry schedule of its own —
# a jittered exponential curve inside one `timeout`-derived deadline, a peer's `google.rpc.retryinfo-bin` window
# overriding each computed wait, and a shutdown event preempting a pending backoff — and `retryable_error_codes` is
# its CONSTRUCTOR slot deciding which codes enter that schedule. Left empty, that slot reads the deployment's own
# `OTEL_PYTHON_EXPORTER_OTLP_GRPC_RETRYABLE_ERROR_CODES` and only then the package default, so an operator moves the
# re-drive policy of a running pipeline and no receipt records it — exactly what the exemplar filter's constructor
# slot avoids by not riding `SDK_ENV`. Rows carry NAMES because `grpc` reifies lazily here, so `_retryable` resolves
# them at the row's own call, the same deferral `grpc.Compression[...]` already takes. The roster HOLDS the OTLP
# specification set rather than narrowing it: the pin's whole job is denying the environment a vote, and a narrower
# set would fork a client behavior every conforming peer agrees on.
...
# does not. `WIRE_RETRYABLE` binds the same way and for the same reason on the gRPC exporter rows.
...
def _retryable() -> tuple[grpc.StatusCode, ...]:
    # ONE resolution of the pinned roster, called from the gRPC rows alone so the `lazy import grpc` stays cold on
    # every HTTP install. A member spelling `grpc` no longer carries breaks HERE at the first gRPC install rather
    # than degrading to the environment fallback an empty slot would have taken.
...
def _joined(values: Iterable[str]) -> str:
    ...
    # local receipt reproduces from the same rows only by luck.
...
def _square(*bounds: int) -> str:
    ...
    # locale-sensitive counterpart has no place here and no row spells its own separator.
...
CONFORMANCE: Final[Map[ConformanceKey, RowCell]] = Map.of_seq([
    ...
    # Only the gRPC exporter carries a re-drive slot, so the HTTP arm answers `absent` rather than a fabricated
    # roster: an unpinned slot and a transport with no slot at all are the two states an auditor must read apart.
    # This cell reads the NAME roster, never `_retryable`, so minting a document leaves the cold gRPC tier cold.
    (
...
# Import-time totality beside the projection spine, one member serving both duties. `CONFORMANCE[key]` is the TOTAL read
# — the `observability/metrics#METRIC` `MEASURES` idiom — so a legislated key whose cell never landed refuses the
# module rather than a mint, and the sorted roster is the one row order every emitted document reads. The reverse
# direction needs no gate: a cell filed under a key outside this vocabulary cannot be spelled at all.
