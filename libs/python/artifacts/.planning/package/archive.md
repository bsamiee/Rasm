...
- [02]-[ARCHIVE]: the `Archive` multi-file container concern over the `package/codec#CODEC` `SEVEN_Z`/`ZIP_STREAM` `CompressionAlgo` rows, the `SevenZKnobs`/`ZipStreamKnobs` `CodecProfile` cases, the `SevenZFilter`/`SevenZPreset`/`ZipMethod`/`ZipArity`/`ZipMechanism` closed `Literal` vocabularies resolved through the frozen `_ZIP_METHOD`/`_ZIP_MECHANISM`/`ident` dispatch rows, the `_Xxh3Factory`/`_Xxh3Sink` streamed `xxh3_128` `WriterFactory` sink replacing the built-in sha256 `HashIOFactory`, the `_zip_members` bounded-memory `MemberFile` fold, the `_EPOCH`/`zlib_ng`-at-`level` byte-reproducible container seam, the `_ARCHIVE_CEILING` `max_extract_size` bomb bound, and the `_archive_in_process`/`_archive_unpack` worker arms the codec `_in_process`/`_unpack_in_process` dispatch delegates the `seven_z`/`zip_stream` tags to; `py7zr` `SevenZipFile`(`writef`/`list`/`extractall(factory=)`/`test`/`max_extract_size`)/`FILTER_*`/`PRESET_*`/`FILTER_CRYPTO_AES256_SHA256`/`DecompressionBombError`/`AbsolutePathError`, `stream-zip` `stream_zip`/`ZIP_AUTO`/`ZIP_64`/`ZIP_32`/`NO_COMPRESSION_32`/`NO_COMPRESSION_64`/`get_compressobj`, and `stream-unzip` `stream_unzip`/`AE_1`/`AE_2`/`AES_128`/`AES_192`/`AES_256`/`NO_ENCRYPTION`/`ZIP_CRYPTO`/`allowed_encryption_mechanisms` settled against the both-tier `.api`, contributing the shared `core/receipt#RECEIPT` `ArtifactReceipt.Bundle` `entries`/`verified` container slots.
...
- Modality: both arms are multi-file — the `*payloads` spread the codec `pack` recovers folds into ONE container. `SEVEN_Z` writes each payload as a `names[i]`-or-`payload-{i}` entry through `SevenZipFile.writef(BytesIO(payload), name)` then re-reads the written bytes through `SevenZipFile.test()` for the CRC-verified entry count; `ZIP_STREAM` folds the spread into one `stream_zip` member sequence so a multi-payload bundle is a single archive, never N archives. The `unpack` inverse recovers the full member row set with a real per-member streamed digest on the runtime XXH3_128 content-identity family, bomb-bounded — `SEVEN_Z` reads the `(filename, uncompressed)` rows from `SevenZipFile.list()` then streams each entry through the `_Xxh3Factory` `WriterFactory` sink under `max_extract_size=_ARCHIVE_CEILING`, so the decode folds into the sink's `xxh3_128` without buffering the whole payload and a high-ratio container raises `DecompressionBombError`; `ZIP_STREAM` drains each member's chunks through a rolling `xxhash.xxh3_128` fold — so neither arm buffers a payload, only its size and its 16-byte `xxh3_128` content digest, uniform with the codec `_walked` frame walk and the delta arm on the ONE identity family `ContentIdentity.of` uses, and the recovered triples feed the codec `BundleManifest.of` content-key fold.
    ...
    match profile:
        ...
        case CodecProfile(tag="zip_stream", zip_stream=ZipStreamKnobs() as k):
            ...
            return tuple(
                ...
                for folded in (reduce(lambda acc, chunk: (acc[0] + len(chunk), acc[1].update(chunk) or acc[1]), chunks, (0, xxhash.xxh3_128())),)
            )
        case _:
            raise ValueError(f"<non-archive-profile:{profile.tag}>")  # the codec dispatch routes only seven_z/zip_stream here; assert_never is invalid on the non-exhausted CodecProfile family
```
