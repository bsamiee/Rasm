...
- Boundary: `SceneSun` is a BAND, not a descriptor — `Objects/lights.md`'s `Lights.Capture` is the declared whole-descriptor emitter and takes this value beside its photometric rows, and its `[Mapper] SceneMap` is the ONE transcription onto `rasm.contracts.scene.v1`; a second mapper here forks the wire.
