...
[LOCAL_ADMISSION]:
- Drive every canvas from `nodes`/`edges` + the change folds; `defaultNodes`/`defaultEdges` and `useNodesState`/`useEdgesState` serve throwaway examples alone, since an uncontrolled canvas holds graph truth the package cannot read.
