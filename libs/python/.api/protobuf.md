...
`protobuf` supplies the `Message` base class, the descriptor schema model, the descriptor-pool/symbol-database registries, and the binary/JSON/text serialization surfaces for Protocol Buffer messages generated by `protoc`. All generated `_pb2.py` message classes derive from `google.protobuf.message.Message`, registering into the default `DescriptorPool`. The runtime selects a backend at import (`upb` native C, `cpp`, or pure `python`) via `api_implementation`; the wheel ships the `_message` C extension and the `google._upb._message` module, so the fast path is native by default. The dense design composes generated messages through `proto`-module functional encode/decode and `json_format`/`text_format` at boundaries, plus the well-known types (`Any`, `Timestamp`, `Duration`, `Struct`, `FieldMask`) as first-class value carriers.
...
- wheel: `cp39-abi3-macosx_10_9_universal2` — NATIVE abi3 wheel, `Root-Is-Purelib: false`; ships `google/protobuf/_message.abi3.so` + `google/_upb/_message.abi3.so`. ABI3 (`cp39-abi3`) means one binary covers CPython >=3.9 including cp315.
...
[BACKEND_TOPOLOGY]:
- `api_implementation.Type()` returns the active backend chosen at import: `'upb'` if `google._upb._message` imports (the wheel default), else `'cpp'` if the legacy `_message` extension is present, else `'python'`. `PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION` env or the `--cpp_implementation` flag overrides; any other value raises at import.
- the abi3 native wheel means message ops (`SerializeToString`, parse, field access) execute in C; the pure-`python` backend is the slow fallback only when the extension cannot load.
...
- Boundary intake/emit: `json_format.ParseDict` at intake from JSON config/payloads into a typed message; `MessageToDict(preserving_proto_field_name=True)` at emit to keep snake_case keys. The well-known types are the canonical carriers — `Timestamp.FromDatetime` at the time boundary, `Struct.update` for dynamic JSON-ish bodies, `Any.Pack`/`Unpack` for heterogeneous typed envelopes.
