# [@tanstack/react-virtual] — virtualized collection rows for large lists and tables
...
- package: `@tanstack/react-virtual` (3.14.5, MIT, © Tanner Linsley) — React adapter re-exporting `@tanstack/virtual-core` (3.17.x), which owns the `Virtualizer` and every option/type below
...
- peer: `react@^16.8||^17||^18||^19`, `react-dom@^16.8||^17||^18||^19` — satisfied by the folder React 19 spine
- asset: pure-TypeScript library, fully TSDECL-reflectable; `useVirtualizer` verified present via `assay api query --key @tanstack/react-virtual`
