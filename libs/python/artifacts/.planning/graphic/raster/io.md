...
flowchart LR
    ...
    Norm --> Cross["per-op crossed: _WORKER_RETRY(to_process.run_sync, _worker_raster, WORKER_BAND)"]
    ...
    Worker --> Det["detect -> _detect (magic.from_buffer -> MediaClass/Container)"]
    Worker --> Eng["probe / thumbnail / convert / crop -> _ENGINE[engine] (FitMode contain/cover/stretch/pad)"]
    Worker --> Mont["montage -> _montage (pillow grid)"]
    Worker --> Comp["composite -> _composite (libvips composite2 BlendMode)"]
    Worker --> Tx["transform -> _transformed (reference check + composed table)"]
