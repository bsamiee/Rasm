...
[PROTOCOL_EDGE]:
- Use: payload structs, message envelopes, serializer contracts, persisted packets, and remote frames — split by WHICH wire: a seam a peer runtime decodes is a protobuf message and crosses as its generated class; a document no peer decodes — a branch-local JSON record, a registry-fronted Avro body, a MessagePack op-log under a ruled discriminant — is a `msgspec.Struct` or a pydantic ingress model.
