...
- universal rail (`libs/python/.api/`): only picklable `IdmlFact` crosses back, never a live `IDMLPackage`/`_Element`; `Idml.of` admits the template through `IndesignPayload` and `TypeAdapter`; each process worker binds `IdmlFact` counts to its span, while startup observation binds `simple_idml.VERSION` once.
