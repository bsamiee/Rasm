...
public sealed class ExtractPoints : Component {
    ...
    private static readonly IOutputGroup Vertices = Output.Single(input: Geometry, port: Port.Tree<Point3d>(name: "Vertices", code: "V", info: "Native vertices, point-cloud locations, polyline corners, curve endpoints, points, or bbox corners."), aspect: new PointSampling.Vertices());
    private static readonly IOutputGroup ControlPoints = Output.Single(input: Geometry, port: Port.Tree<Point3d>(name: "Control Points", code: "CP", info: "NURBS control polygon points for curves and surfaces, converting through RhinoCommon when required."), aspect: new PointSampling.ControlPoints());
    private static readonly IOutputGroup EdgeMidpoints = Output.Single(input: Geometry, port: Port.Tree<Point3d>(name: "Edge Midpoints", code: "EM", info: "Length midpoints of curves and edges; per-segment for polylines and box-like geometry."), aspect: new PointSampling.EdgeMidpoints());
    private static readonly IOutputGroup Quadrants = Output.Single(input: Geometry, port: Port.Tree<Point3d>(name: "Quadrants", code: "Q", info: "World-cardinal extrema (top/bottom/left/right + Z) of a curve. Curve-only output."), aspect: new PointSampling.Quadrants());
    ...
    private static readonly IOutputGroup Kind = Output.Details<Rasm.Domain.Kind>(
        input: Geometry,
        aspect: static _ => Fin.Succ<Func<Shape, Eff<Env, Seq<Rasm.Domain.Kind>>>>(shape => Rasm.Analysis.Query.Kind<object, Rasm.Domain.Kind>().Apply(geometry: shape.Inner)),
