...
- [03]-[INSTRUMENTATION]: composition-root instrumentor train over the contrib packages, generic PEP-249 `DbapiDriver` wrap arm, and the system-metrics slice ruling.
...
DOMAINS: Final[Map[str, str]] = Map.of_seq([
    ...
    ("link", "the graduation-handoff subject crossing the geometry boundary"),
...
- Growth: a new instrumentor is one `lazy from` line and one `TRAIN` row naming the driver it wraps; a new system-metrics family is one `_SYSTEM_SLICE` key; a new dedicated-instrumentor-less driver is one `DbapiDriver` value the composition root threads through `Instrumentation.dbapi`.
- Boundary: the Connect server legs stay the serve page's `OpenTelemetryMiddleware` and `Admission` interceptor — its context authority forbids a second server-leg patch — and no sibling package activates an instrumentor. DBAPI spans complement the data profile plane, never replace it: the data owner's query profile stays its truth, `capture_parameters` stays `False` as the export posture, and a driver carrying its own contrib instrumentor never routes through the generic boundary.
...
class DbapiDriver(Struct, frozen=True):
    name: str
...
class Instrumentation:
    ...
    @overload
    @classmethod
    def dbapi(cls, boundary: DbapiDriver) -> None: ...
    @overload
    @classmethod
    def dbapi[C](cls, boundary: DbapiDriver, connection: C) -> C: ...
    @classmethod
    def dbapi[C](cls, boundary: DbapiDriver, connection: C | None = None) -> C | None:
        with cls._gate:
            match connection:
                case None:
                    wrap_connect(boundary.name, boundary.connect_module, boundary.connect_method_name, boundary.database_system, **_DBAPI_POSTURE)
                    return None
                case live:
                    return instrument_connection(boundary.name, live, boundary.database_system, **_DBAPI_POSTURE)
```
