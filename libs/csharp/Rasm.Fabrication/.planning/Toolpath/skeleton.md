...
- Packages: `Rhino.Geometry` (`Point3d`/`Vector3d` — composed), `Rasm.Numerics` (`Predicate.Orient2D` — settled, the bisector/side verdict), Clipper2 (via `Polygon/clipper#POLYGON_ALGEBRA` — the convex-inset offset cross-check), Thinktecture.Runtime.Extensions, LanguageExt.Core, BCL inbox.
