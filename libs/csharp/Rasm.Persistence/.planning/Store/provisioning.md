...
- Entry: `Verify` folds one catalog batch into a typed verdict; `Register` and `Admit` consume its exact snapshot, and `BackendObservation.Of` projects a `Provisioned` verdict into the observation runtime admission joins against the expected generation — a probed verdict is the ONE capability evidence the backend contract admits, so a desired roster or an availability read never reaches it.
...
public abstract partial record ProvisionVerdict {
    ...
    // between the two — the probe IS the observation, and only a `Provisioned` verdict is admissible evidence.
...
public static class ClusterProvision {
    ...
    public static ReconcileManifest Manifest(ClusterDemand demand, EmbeddedRitual ritual) => new(
        demand.Required.Map(static row => new ReconcileRow("relational-sor", row.Key, row.CreateSql, row.Restart.Key))
        + toSeq(ClusterSetting.Items).Map(static row => new ReconcileRow("relational-sor", row.Key, row.Expected, row.Restart.Key))
        + toSeq(MaintenanceJob.Items).Map(static row => new ReconcileRow("maintenance", row.Key, row.RegisterSql, row.Owner.Restart.Key))
        + toSeq(RollingWindow.Items).Map(static row => new ReconcileRow("maintenance", row.Key, $"{row.Period}:+{row.Ahead}/-{row.Aged}", RestartClass.Session.Key))
        + Seq(new ReconcileRow("embedded-relational", "<cipher-provider>", "SQLitePCLRaw.bundle_e_sqlite3mc", RestartClass.Restart.Key))
        + ritual.ConnectionRows.Map(static row => new ReconcileRow("embedded-relational", row.Row, row.Sql, RestartClass.Session.Key))
        + ritual.DbConfig.Map(static row => new ReconcileRow("embedded-relational", row.Row, row.Value.ToString(CultureInfo.InvariantCulture), RestartClass.Session.Key)),
        demand.Epoch);
