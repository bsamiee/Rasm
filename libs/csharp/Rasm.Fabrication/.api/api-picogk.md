...
- THE WIRE MEETS AT `Mesh`: `Voxels.mshAsMesh()` extracts a watertight mesh that crosses to the kernel `MeshSpace` vocabulary for downstream slicing, posting, or display; any biarc fitting of resulting line-sourced section chains is the owned Bolton biarc fold (`Posting/program`), never a package mesh owner. PicoGK does NOT own the planar mesh-section (the kernel `Meshing/slice` slice-stack owns it) — it extracts the mesh and hands off.
