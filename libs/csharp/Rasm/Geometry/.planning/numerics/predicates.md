...
- [1]-[ROBUST_PREDICATES]: Closed `Predicate` family — `Orient2D`/`Orient3D`/`InCircle`/`InSphere` direct plus `OrientLPI`/`OrientTPI`/`InCircleLPI`/`InSphereTPI` implicit-point — filter-then-exact two-stage adaptive evaluation returning an exact `Sign`.
