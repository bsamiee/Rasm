...
| [INDEX] | [TIER / BASE]              | [DISTINGUISHING FIELDS over `type`/`name`/`description`]                          | [RAIL]                                          |
| :-----: | :------------------------- | :------------------------------------------------------------------------------- | :---------------------------------------------- |
...
- artifact + transfer: a `Job` `S3`/`HTTP` artifact source's `(bucket, key, endpoint)` / `url` feeds an `obstore`/`s3fs` `get`/`put` or an `httpx` stream; a `ProjectFolder` source feeds the `universal-pathlib`/`fsspec` local root. The artifact-content key for the runtime cache is the XxHash128 content-identity owner (`xxhash.xxh128_hexdigest`) over `recipe.to_json` bytes — distinct from queenbee's own `BakedRecipe.digest`/`PackageVersion.digest` lock identity, which keys repository resolution.
