...
- A task-matched partial-loading demand against a mandatory whole-set reference load in a bundle whose every invocation consumes the set: void — the flat-bundle design is ruled, and the demand re-litigates the bundle law.
- allowed-tools grants judge by contract and consumption: the documented project-directory substitution is the sanctioned spelling (the skill-directory one binds skill content and bash injection alone), and a tool a bundle's mandatory reference reads or its ruled rails consume is load-bearing — portability-swap and minimization demands against either are void.
- A resilience-machinery demand a file's own stated charter rules out — a streaming drain on a timeout-bounded one-shot, retry rails on a cheap probe — contradicts the settled contract, and a template header stating its fill-and-replace contract in the bundle's sibling-consistent schema is schema, never narration to delete.
