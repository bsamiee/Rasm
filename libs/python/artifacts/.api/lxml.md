...
|  [03]   | `etree._Attrib`                                          | attribute map      | live ordered attribute mapping (`get`/`__setitem__`/`__delitem__`/`pop`/`update`/`clear`/`has_key`/`items`/`iteritems`) on `_Element.attrib`; the element-level `_Element.set(name, value)` is the convenience setter |
...
|  [08]   | `etree.XSLT`                                             | compiled transform | reusable XSLT stylesheet (`__call__`, `strparam`, `tostring`, `set_global_max_depth`, `error_log`) |
...
|  [03]   | `etree.XSLT`                       | `XSLT(stylesheet, *, extensions=, access_control=)` then `__call__(tree, profile_run=False, **params)` -> `_XSLTResultTree` | compile/apply a stylesheet  |
...
- streaming axis: `iterparse`/`iterwalk` (in, `.clear()`-as-you-go) and `xmlfile`/`htmlfile` (out, incremental) own the bounded-memory path for large documents; the pull parsers (`XMLPullParser`) own feed-driven event streaming over a non-seekable source — never a full-tree read where the document exceeds memory.
...
- rail-and-effects: parse/validate/transform compose under the `expression` `Result[T, E]` rail (`libs/python/.api/expression.md`) so an `XMLSyntaxError`/`DocumentInvalid`/`XSLTApplyError` lifts to a typed `Error` arm; `@beartype` (`libs/python/.api/beartype.md`) guards the boundary signatures; a structured-documents receipt records the `error_log` row count, the element/namespace tally, and the output byte length; `etree.PyErrorLog` routes the libxml2 error stream into the `structlog` chain (`libs/python/.api/structlog.md`) so a parse diagnostic is structured, not a flattened string; a network-touching parse (DTD/XInclude over a URL, never the untrusted default) wraps `stamina.retry` (`libs/python/.api/stamina.md`).
