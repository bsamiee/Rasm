...
- Packages: NREL.OpenStudio.macOS-arm64, LanguageExt.Core, Rasm.Element (project — `ElementGraph`, `Node.Object`, `MaterialComposition`, `MaterialPropertySet.Thermal`/`.Mechanical` read through the `MaterialPropertyAccess` accessors, `MaterialLayer`, `NodeId`, `RepresentationContentHash`, `FootprintPolygon`, `GeometrySource` the analytical-surface resolution port), Rasm (project — the kernel `Vector3` the `FootprintPolygon` ring carries), BCL inbox.
