...
|  [05]   | `MathTransform`                   | geometry | numeric transform pipeline; abstract `Transform(ref x,ref y,ref z)` + per-coord, array, list, and `Span<double>`/`Span<XY>`/`Span<XYZ>` batch overloads; `Inverse`/`Invert`, `DimSource`/`DimTarget`; `Derivative`/`GetDomainFlags`/`GetCodomainConvexHull` are decompile-refuted phantoms (`[PHANTOM_SURFACE]`) |
...
|  [10]   | `MathTransform.Inverse` / `Invert`        | `()` → `MathTransform` / in-place               | the reverse transform (target → source); every concrete 2.1.0 transform overrides `Inverse` |
...
[PHANTOM_SURFACE]: decompile-refuted members — never cite, never compose
- `MathTransform.Derivative(double[])`, `GetDomainFlags(List<double>)`, and `GetCodomainConvexHull(List<double>)` are base-only `virtual` members throwing `NotImplementedException` with ZERO concrete overrides across the 2.1.0 assembly — no transform computes a Jacobian, a domain verdict, or a codomain hull.
