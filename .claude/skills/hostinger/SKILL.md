...
- [SNAPSHOT_FIRST]: Snapshots precede every destructive operation (recreate, backup restore, migration), one per VM, overwritten by the next
- [FIREWALL_SYNC]: Firewall rule changes take effect only after an explicit sync to the VM, unsynced, the VM runs stale rules
...
`POST .../recreate` reinstalls the OS and destroys all data: snapshot first, confirm with the operator, and meet the password policy of 12+ characters, mixed case, numbers, screened against leaked-password databases.
...
# Container stats, logs (last 300), lifecycle endpoints
...
Troubleshooting: a restart-looping container reads its `logs` first (missing env, wrong image, port conflict), one service binds a host port at a time, disk pressure shows in `GET .../metrics`, a GitHub URL failure means the compose file is absent from the master-branch root, use the raw file URL for other branches.

## [04]-[SSH_KEYS]

Keys register at account level and attach per VM, an unattached key does not authenticate. When SSH refuses a key, check attachment, then firewall sync.
...
Firewalls are account-level resources activated per VM. Default policy drops all inbound traffic, one firewall binds per VM, every rule change requires an explicit sync to take effect, an unsynced firewall is the prime suspect when SSH or a service is unreachable.
...
|  [03]   | `POST`                | `.../{id}/setup`, `start`, `stop`, `restart`, `recreate`  | Lifecycle (recreate destroys data)       |
