...
The research folders were extracted from instructional sources (2 textbooks, an article series, and package documentation) and then corrected and compacted over many sessions. They are complete in facts and wrong in framing: a textbook narrates, motivates, and repeats, and a skill instructs. The work is a restructuring of that content, one section at a time, into instructions, and it is never a rewrite from memory.
...
- The intro names each sibling skill in one line with what it owns, and the body never repeats that pointer
...
- The skill carries no mechanics of a sibling: no attribute settings, no generated-API listing, no conversion catalog
...
- Comment out research content in place with `<!-- -->` once a skill or reference holds it, never delete it, so a later session sees what is done and what remains
- Move content removed from a skill to its research file or to a stub reference, so nothing is lost or forgotten
- Choose the strongest research section for a fact when several files cover it at different depths, integrate that one, and comment out the weaker ones where they duplicate it
...
- Defer to a reference from the section that owns the topic, as a one-line pointer, not as a summary that repeats the reference

A skill drives behavior, it is instructions to an agent and not a textbook. Framing decides what the agent writes:
