...
- `tools/assay/composition/catalog.py` registers `shellcheck -f json1`, `shfmt -d`, and `shfmt -w` as Bash static tools when available. That proves the assay catalog can invoke the tools; it does not add a docs-standard proof gate for `.reports/` research reports.
...
- [x] `git diff --check -- .reports/code-documentation-050626/track-bash/02-shellcheck-shfmt.md` passed.
