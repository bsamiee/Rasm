...
- We are in a long-term planning state, working exclusively on `.md` design/spec sheets within planning folders, NOT implementing code. Aggressively and iteratively refine, rebuild, and push existing design docs to the pinnacle of possibility — never by spamming code, but through cycles of adding functionality then aggressively rebuilding/collapsing: reduce LOC, surface, and object/shape/type/constant count while maintaining functionality, and ULTRA-stack all external libs/packages per planning folder, using them as sources of new functionality and capability.
