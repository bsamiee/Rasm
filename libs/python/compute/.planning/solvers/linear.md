...
- Engine carrier: `LinearEngine` is the ONE frozen value object folding the gated `jax`/`jnp`/`lx` modules so the operator build, the solver cell, and the per-leaf value read are carrier methods reading `self.lx`/`self.jnp`/`self.jax` rather than each helper re-importing `lineax`/`jax.numpy` — the same `SolveEngine.gated()`/`DiffEngine.gated()` discipline `solvers/nonlinear.md#NONLINEAR` and `solvers/sensitivity.md#SENSITIVITY` run. `LinearEngine.gated()` imports `jax`/`jax.numpy`/`lineax` once behind the band and runs `jax.config.update("jax_enable_x64", True)`, the load-bearing float64 promotion: the `1e-10` tolerance is below float32 eps so on the JAX default float32 the `lineax` termination criterion is unsatisfiable and the implicit-function-theorem adjoint degrades below double precision. `LinearEngine.operator(m)` is the SINGLE owner of the `LinearMap`→`lineax`-operator projection (the diagonal/tridiagonal/dense/free/sparse arms), deriving each `FunctionLinearOperator` `input_structure` as a `jax.ShapeDtypeStruct` over the operand's column count (the operator DOMAIN) rather than the RHS vector (the codomain, which mis-sizes a non-square least-squares/min-norm operand); `LinearEngine.solver(shape, structure, *, spd_free, tol, maxiter)` is the SINGLE owner of the `SolveShape`/structure→solver cell threading the policy tolerance and iteration cap, and `LinearEngine.residual(operator, x, b)` contracts the lineax-rail residual `‖operator.mv(x) - b‖` over the operator's own `.mv` so the differentiable route scores its solve on the JAX rail it solved on — never `LinearMap.scipy_op @ x` re-entering scipy off a JAX solution — and the batched sweep folds the per-row scalar through `jnp.max` inside `filter_vmap` rather than a host-side `max(...)` over a `zip`. The gated import, the float64 promotion, and the operator/solver/residual build are one rail rather than inline imports across helpers. This moves every gated import OFF `LinearMap`, which stays cp315-clean carrying only its `scipy_op`/`structure`/`residual` projections — the `LinearMap.residual` scipy-rail projection scoring the scipy dense/sparse/eigen routes, the `LinearEngine.residual` lineax-rail one scoring the operator route.
