-- Confirms ast-grep hits as sites, text and bytes from the checker, :worktree the scanned tree, :out the JSON `ast-grep scan --json=compact` printed over paths relative to it, run from :worktree
