...
- version: `1.6.4`
- license: `MIT`
- effect-peer: none — decode output crosses into `effect` `Schema.decodeUnknown` at the `codec/snapshot` seam (`.api/effect.md`); no bundled peer
...
- surface drift (1.6.4): runtime `index.js` exports `setSizeLimits`/`decodeIter`/`encodeIter`/`REUSE_BUFFER_MODE`; `index.d.ts` instead declares `setMaxLimits` (runtime-absent) and phantoms `MAX_LIMITS_OPTIONS` — the `codec/snapshot` boundary owner reconciles with a local `declare module 'cbor-x'` augmentation and calls `setSizeLimits`
...
| :-----: | :---------------------------------------------------- | :-------------- | :---------------------------------------------------------------- |
|  [01]   | `Decoder` (`.decode`, `.decodeMultiple`)              | decoder         | `codec/snapshot` `SnapshotHeader` decode; one instance per configured policy |
...
|  [06]   | `SizeLimitOptions`                                    | DoS bound       | `fault/quarantine` — the `maxArraySize`/`maxMapSize`/`maxObjectSize` ceiling record passed to `setSizeLimits` |
...
| :-----: | :------------------------------------------------------------------------------------ | :-------------- | :------------------------------------------------------- |
|  [01]   | `new Decoder(options?).decode(bytes: Buffer \| Uint8Array): any`                       | decode          | `codec/snapshot` one-frame decode → `Schema.decodeUnknown(SnapshotHeader)` |
...
|  [06]   | `setSizeLimits({ maxArraySize, maxMapSize, maxObjectSize })`                           | DoS gate        | `fault/quarantine` ceilings before untrusted decode; runtime name (`index.d.ts` mislabels `setMaxLimits`) |
...
- the DoS gate precedes decode: untrusted frames pass `setSizeLimits` array/map/object ceilings; an overrun or malformed frame throws, caught at the `Effect.try` boundary and routed to the `fault/quarantine` poison buffer, never a raw throw into domain code.
- the boundary owner internalizes the shipped surface drift: in 1.6.4 the runtime exports `setSizeLimits`/`decodeIter`/`encodeIter`/`REUSE_BUFFER_MODE` while `index.d.ts` declares the runtime-absent `setMaxLimits` and phantoms `MAX_LIMITS_OPTIONS`; a local `declare module 'cbor-x'` augmentation at the `codec/snapshot` owner reconciles the type surface so downstream code composes `setSizeLimits` typed, never re-discovering the mismatch — the exact provider-quirk-into-owner pattern.

[INTEGRATION_LAW]:
- Stack with the codec siblings (`@bufbuild/protobuf` / `@msgpack/msgpack` / `rfc6902`, `wire/.api/`): the wire folder is multi-codec and `codec/snapshot` is the CBOR arm — `@bufbuild/protobuf` owns the descriptor-typed proto families (`ElementGraph`, `FaultDetail`, `GeometryPayload`, the RPC suite) and never mints a `SnapshotHeader`, `@msgpack/msgpack` owns the `CrdtOpWire`/`OpLog` MessagePack union, `rfc6902` owns the `JsonPatchDocument` egress. A `codec` page picks ONE codec by the C# mint format; a proto page never reaches for CBOR and `codec/snapshot` never reaches for proto — the descriptor reflection and the `contract/descriptor` drift gate are `@bufbuild/protobuf`'s altitude, not this one's.
...
- Stack with `kernel/identity`: the header's content-key field (a `Uint8Array`, preserved via `tagUint8Array`/`copyBuffers:false`) is re-verified against the `XxHash128` seed-zero mint `frame/artifact` delegates to; `wire` composes the mint, never re-implements it.
...
- Stack with `fault/quarantine`: a `DecodeError`/`RangeError` (frame incomplete, limit exceeded) caught at `Effect.try` becomes a quarantine row on an `effect` `Queue`, replayable, never a process throw.
...
- call the runtime-true `setSizeLimits`, never the runtime-absent `setMaxLimits`, and never cite `MAX_LIMITS_OPTIONS`; the `codec/snapshot` owner carries the local `declare module` augmentation that makes `setSizeLimits`/`decodeIter`/`encodeIter` type-visible.
...
- Accept: a configured-once `Decoder` with `useRecords:false`/`mapsAsObjects:true`, decode output crossing `Schema.decodeUnknown` into owned vocabulary, content-key re-verify through the one `kernel/identity` mint, `setSizeLimits` before untrusted decode, `Match`/`addExtension` tag dispatch
