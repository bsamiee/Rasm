...
|  [03]   | `Metadata.SetRpcStatus(Google.Rpc.Status)`                                 | static  | replace every trailer entry with one serialized status |
...
| [INDEX] | [SURFACE]                                                                   | [SHAPE] | [CAPABILITY]                                          |
| :-----: | :-------------------------------------------------------------------------- | :------ | :---------------------------------------------------- |
|  [01]   | `Google.Rpc.Status.ToRpcException() -> RpcException`                        | static  | `Code` → `StatusCode`, `Message` → detail, status trailer set |
|  [02]   | `Google.Rpc.Status.ToRpcException(StatusCode, string) -> RpcException`      | static  | caller-chosen wire code and detail over the same trailer |
|  [03]   | `Exception.ToRpcDebugInfo(int innerDepth = 0) -> Google.Rpc.DebugInfo`      | static  | type-qualified message plus stack lines, inner chain to depth |
...
- `Rasm.AppHost` (`Runtime/ports#WIRE_LAW`): `FaultWire.Raise` folds a `Fault` into `Google.Rpc.Status{Code, Message, Details = {Any.Pack(FaultDetail)[, RetryInfo]}}.ToRpcException()` at every failing handler arm; `FaultWire.Decode` reads `RpcException.GetRpcStatus()` under `Op.Catch`, so a malformed trailer lands typed on `WireBoundary.RemoteStatus` and an absent one answers `None`.
