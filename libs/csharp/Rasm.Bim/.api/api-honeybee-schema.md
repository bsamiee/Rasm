...
[STACKING]:
- `OpenStudio`(`libs/csharp/.api/api-openstudio.md`): the parallel EnergyPlus runtime binding; the two energy DTOs meet at the canonical Bim energy model over gbXML (`OpenStudio.GbXMLReverseTranslator`), the end-to-end `honeybee-openstudio` Python translation reaching through `Helper.PythonCommand`.
...
- `System.IO.Hashing`(`libs/csharp/.api/api-hashing.md`): a `Model.ToJson()` UTF-8 string feeds `XxHash3` for the fast fingerprint and `XxHash128` for the persisted content key, the same identity rail the IFC/glTF exports ride; an HBJSON document rides a `Speckle`(`libs/csharp/.api/api-speckle.md`) commit or lands in the `Rasm.Persistence` artifact index keyed by that hash.
