...
- difference through `PlineBoolean.PolylineBoolean<Polyline<double>, double>(p1, p2, options)` keyed on `BooleanOp { Or, And, Not, Xor }`; `Not` is the kerf-inflated arc-space remnant the `Nesting/nfp` `Remnant` producer consumes.
