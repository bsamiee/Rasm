...
| :-----: | ------------------------------------------------ | ------------------------- |
|  [01]   | **Constrained scalar (Email, Amount, Id)**       | `[ValueObject<T>]`        |
|  [02]   | **Closed behavioral set (status/type/strategy)** | `[SmartEnum<T>]`          |
|  [03]   | **Closed variant payload space**                 | `[Union]`                 |
|  [04]   | **Multi-field invariant bundle**                 | `[ComplexValueObject]`    |
|  [05]   | **Identity-bearing lifecycle object**            | `sealed record` aggregate |
|  [06]   | **Stack-confined parser/workspace**              | `readonly ref struct`     |
...
| :-----: | :-------------------------------------------- | --------------------------------------------------------- | :-------- |
|  [01]   | Primitive obsession in signatures             | Value object canonicalization + DomainBridge              | [2], [3]  |
|  [02]   | Enum/switch sprawl                            | SmartEnum + exhaustive generated behavior                 | [04]      |
|  [03]   | Variant ambiguity via nullable fields         | Union + exhaustive `Switch`/`Map`                         | [05]      |
|  [04]   | Multi-field VO without scalar key             | `[ComplexValueObject]` + `[ValidationError<T>]`           | [2.1]     |
|  [05]   | Union SelfOp analyzer policy                  | `[SkipUnionOps]` / `[GenerateUnionOps]` on project unions | [5.1]     |
|  [06]   | Mutable aggregate drift                       | Sealed record + `with`-expression transitions             | [06]      |
|  [07]   | Stack-only type leaking into domain contracts | `ref struct` isolation + conversion bridge                | [07]      |
|  [08]   | Dual object representations per concept       | Collapse to one canonical shape                           | [1], [8]  |
