...
- rail: mail egress (folder-tier; internalized once at `work/deliver/mail.ts`)
...
- `jspdf` + `exceljs` + `papaparse` + `jszip` (`./jspdf.md`, `./exceljs.md`, `./papaparse.md`, `./jszip.md`): the `deliver/report` byte producers — a PDF/XLSX/CSV `Uint8Array` (one output-format policy row over the same decoded rows) or a `jszip` bundle attaches as `Mail.Attachment` `{ content: bytes, contentType }`; nodemailer transports what the report format arms and the archive container produce, never rendering a document itself.
...
- `@effect/workflow` + `@effect/cluster` (`./effect-workflow.md`, `./effect-cluster.md`): a send is a durable `Activity` with a compensation arm (record a suppression on hard bounce); the `deliver/mail` entity fences per-tenant send quota, and the outbox relay drains queued messages under the `isIdle` pool signal.
