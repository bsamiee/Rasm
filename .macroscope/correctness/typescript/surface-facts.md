...
- `Schema.suspend` with a mandatory return annotation, and the one sanctioned interior `interface OwnerEncoded` beside a recursive owner, are required — inference cannot self-close; getters and `static` `Order`/algebra members on a `Schema.Class` are the canonical owner, not logic-in-models.
- `as const satisfies Record<string, Row>` (satisfies without a widening annotation) is the vocabulary anchor form; merged registry `interface` with `declare module` row contribution is the intended cross-module vocabulary.
...
- `Array.partition` returns `[excluded, satisfying]` — the predicate-false half leads — so an inverted-halves claim against a binding that spells that order is false, and a binding spelling it the other way inverts both consumers.
...
- Emit-shaping options (`declaration`, `isolatedDeclarations`) bind only declaration-emitting packages in their own configs — the shared root `tsconfig.json` carrying them taxes every non-emitting project, and a per-project waiver re-spelling the base's absent emit gate is redundant residue.
