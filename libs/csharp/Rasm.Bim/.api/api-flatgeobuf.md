...
- dependency: `NetTopologySuite` >=, `NetTopologySuite.Features` >=, `NetTopologySuite.IO.GeoJSON` >=, `Nito.AsyncEx` >= (the `netstandard2.1` group; the central `NetTopologySuite` pin wins resolution); the `netstandard2.0` group additionally pulls `Microsoft.Bcl.AsyncInterfaces` — NOT pulled by the bound ns2.1 asset
...
| :-----: | :-------------------------------------- | :--------------------- | :-------------------------------------------------------------------- |
|  [01]   | `FeatureCollectionConversions`          | codec entrypoint       | `Serialize`/`Deserialize` (+ async) — NTS features ↔ FGB bytes/stream |
|  [02]   | `FeatureConversions`                    | per-feature codec      | one `IFeature` ↔ FlatBuffers `Feature`; the `FromByteBuffer` leg      |
|  [03]   | `GeometryConversions`                   | geometry codec         | NTS `Geometry` ↔ FlatBuffers (`ToGeometryType`/`FromFlatbuf`)         |
|  [04]   | `AsyncFeatureEnumerator`                | async streaming reader | `IAsyncEnumerator<IFeature>` over a stream with `rect` bbox filter    |
|  [05]   | `GeoJsonConversions`                    | GeoJSON bridge         | `Serialize(string)`/`Deserialize(byte[])` — GeoJSON ↔ FGB direct      |
|  [06]   | `LayerMeta`                             | layer schema           | `Name`/`GeometryType`/`Dimensions`/`Columns` — FGB layer header       |
|  [07]   | `ColumnMeta`                            | column schema          | attribute-column descriptor (name + `ColumnType`)                     |
|  [08]   | `FlatGeobufCoordinateSequence(Factory)` | packed sequence        | NTS `CoordinateSequence` over the FlatBuffers ordinate buffer         |
...
|  [02]   | `PackedRTree.StreamSearch` | range query    | bbox query over random-access fetch → `(Offset, Index)` hits                 |
|  [03]   | `PackedRTree.ReadNode`     | fetch delegate | `delegate Stream ReadNode(ulong offset, ulong length)` — the byte-range read |
|  [04]   | `PackedRTree.CalcSize`     | sizing         | index byte size to skip to the feature body                                  |
...
|  [03]   | `Geometry` / `GeometryT` | geometry struct    | the encoded ordinate/part buffer `GeometryConversions` decodes                        |
|  [04]   | `Column` / `ColumnT`     | column struct      | one attribute-column definition                                                       |
|  [05]   | `Crs` / `CrsT`           | CRS struct         | the coordinate reference system (`Code` → NTS SRID)                                   |
|  [06]   | `GeometryType`           | geometry-kind enum | `enum: byte` — `Unknown`(=0)/Point/LineString/Polygon/Multi*/GeometryCollection       |
|  [07]   | `ColumnType`             | column-type enum   | `enum: byte` — attribute scalar type (Byte/Int/Long/Double/String/…)                  |
|  [08]   | `Helpers`                | header codec       | static `ReadHeader`/`GetEnvelope`/`GetCrsCode` — the header decode leg                |
...
The `Serialize`/`SerializeAsync` overloads take `(target, features, GeometryType geometryType, byte dimensions = 2, IList<ColumnMeta> columns = null)`; `target` is a `FeatureCollection`/`IEnumerable<IFeature>` or a `Stream`.

| [INDEX] | [SURFACE]                                                            | [CALL_SHAPE]                | [RAIL]                              |
| :-----: | :------------------------------------------------------------------- | :-------------------------- | :---------------------------------- |
|  [01]   | `FeatureCollectionConversions.Serialize(FeatureCollection fc, …)`    | → `byte[]`                  | whole-collection FGB emit           |
|  [02]   | `FeatureCollectionConversions.Serialize(Stream output, …)`           | → `void`                    | streaming emit, no whole-set buffer |
|  [03]   | `FeatureCollectionConversions.SerializeAsync(Stream output, …)`      | → `Task`                    | async streaming emit                |
|  [04]   | `GeometryConversions.ToGeometryType(Geometry geometry)`              | → `GeometryType`            | resolve the layer geometry kind     |
|  [05]   | `GeoJsonConversions.Serialize(string geojson)` / `SerializeAsync(…)` | → `byte[]` / `Task<byte[]>` | GeoJSON-text → FGB direct           |
...
Surfaces elide their owner: rows [01]-[02] are `FeatureCollectionConversions`, [03]-[05] are `AsyncFeatureEnumerator`. `Create` takes `(Stream stream, PrecisionModel? pm = null, Envelope? rect = null, CancellationToken? token = null)`.

| [INDEX] | [SURFACE]                                           | [CALL_SHAPE]                             | [RAIL]                            |
| :-----: | :-------------------------------------------------- | :--------------------------------------- | :-------------------------------- |
|  [01]   | `Deserialize(byte[] bytes)`                         | → `FeatureCollection`                    | whole-set decode from a buffer    |
|  [02]   | `Deserialize(Stream stream, Envelope? rect = null)` | → `IEnumerable<IFeature>`                | streaming decode, bbox filter     |
|  [03]   | `Create(Stream stream, …)`                          | → `Task<AsyncFeatureEnumerator>`         | open the async reader (bbox + pm) |
|  [04]   | `MoveNextAsync()` / `.Current`                      | → `ValueTask<bool>` / `IFeature`         | the forward async cursor          |
|  [05]   | `Extent`/`SRID`/`Crs`/`NumFeatures`/`Title`         | → `Envelope`/`int`/`CrsT`/`int`/`string` | header metadata, before iterating |
...
`StreamSearch` takes `(ulong numItems, ushort nodeSize, Envelope rect, ReadNode readNode)` and `FromByteBuffer` takes `(GeometryFactory factory, FlatGeobufCoordinateSequenceFactory seqFactory, ByteBuffer bb, HeaderT header)`; `StreamSearch` offsets are index-relative, re-based onto `12 + headerSize`, and each body record is size-prefixed then handed prefix-stripped.

| [INDEX] | [SURFACE]                                        | [CALL_SHAPE]                                 | [RAIL]                                |
| :-----: | :----------------------------------------------- | :------------------------------------------- | :------------------------------------ |
|  [01]   | `PackedRTree.StreamSearch(…)`                    | → `IEnumerable<(ulong Offset, ulong Index)>` | bbox query over random-access fetch   |
|  [02]   | `PackedRTree.CalcSize(ulong, ushort)`            | → `ulong`                                    | index byte size to seek to the body   |
|  [03]   | `PackedRTree.ReadNode` delegate                  | `Stream (ulong offset, ulong length)`        | consumer byte-range / HTTP-range read |
|  [04]   | `Helpers.ReadHeader(Stream, out int headerSize)` | → `Header`                                   | decode the header off one head fetch  |
|  [05]   | `FeatureConversions.FromByteBuffer(…)`           | → `IFeature`                                 | lower one prefix-stripped body record |
...
- the wire layout is `8-byte magic → 4-byte size prefix → header blob → Packed R-tree → size-prefixed feature records`: `Helpers.ReadHeader(stream, out headerSize)` consumes magic + prefix + blob so the index seats at `12 + headerSize` (the package's own `Deserialize` seeks exactly that), `CalcSize` seats the body, and each body hit reads its 4-byte record length then hands the PREFIX-STRIPPED record to `FeatureConversions.FromByteBuffer` — the same per-record leg the whole-file `Deserialize` and `AsyncFeatureEnumerator` drive after their own `ReadInt32`
...
- cloud-streaming seam: the `csharp:Rasm.Persistence/Store` `ObjectStore.Fetch` transport supplies the `.fgb` bytes the whole-file `Deserialize(byte[])` decodes; the `PackedRTree.StreamSearch` range-read leg is the escalation for a remote `.fgb` too large to fetch whole — `Helpers.ReadHeader` decodes the head fetch, the `ReadNode` delegate backed by the object-store random read fetches only the index nodes and matching runs, and `FeatureConversions.FromByteBuffer` lowers each prefix-stripped record, the streaming counterpart of the GDAL `/vsicurl/` remote read
...
[RAIL_LAW]:
- Package: `FlatGeobuf` (, BSD-2-Clause, pure-managed `lib/netstandard2.1` AnyCPU IL binding under net10; deps `NetTopologySuite`/`NetTopologySuite.Features`/`NetTopologySuite.IO.GeoJSON`/`Nito.AsyncEx`, the central NTS pin winning; `Microsoft.Bcl.AsyncInterfaces` NOT pulled by the ns2.1 bind)
- Owns: the cloud-optimized streaming FGB vector codec over NTS features — header-bbox + Packed-Hilbert-R-tree spatial index, streaming/async feature read with bbox push-down, the `FlatGeobufCoordinateSequence` packed layout, random-access range query with the `Helpers.ReadHeader`/`FeatureConversions.FromByteBuffer` header-and-record decode, and the GeoJSON-text↔FGB bridge
