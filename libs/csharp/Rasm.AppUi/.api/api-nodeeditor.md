...
- Accept: `Editor.Drawing` binds an `IDrawingNode` and `Editor.Templates` a palette; pan/zoom is `NodeZoomBorder` (the package's distinct `Avalonia.Controls.PanAndZoom` asset — NOT the separately admitted `PanAndZoom.dll`/`ZoomBorder` from `api-panandzoom.md` — so both restore without collision); connector routing is `ConnectorRoutingAlgorithm`/`ConnectorStyle` over `OrthogonalRouter`, never a hand-rolled path layout.
