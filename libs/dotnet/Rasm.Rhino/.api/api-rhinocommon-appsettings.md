...
Analysis owners share the round-trip shape and feed the `VisualAnalysisMode` overlays of `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-display.md`; each pairs a `{Owner}State` snapshot.
...
- analysis owners are one family: each preset repeats the round-trip and differs only in its `*State` payload, so an analysis mode is a `VisualAnalysisMode` row of `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-display.md` reading its settings owner.

[STACKING]:
- `LanguageExt`(`libs/dotnet/.api/api-languageext.md`): `FindFile`/`FindAlias`/`GetMacro` misses fold to `Option<A>`, `SetMacro`/`AddSearchPath`/`DeleteSearchPath` outcomes fold to `Fin<A>`, and a `*State` edit accumulates through an immutable `with` projection rather than in-place static mutation.
- `Thinktecture.Runtime.Extensions`(`libs/dotnet/.api/api-thinktecture-runtime-extensions.md`): keyed policy enums map to `SmartEnum` owners, `OsnapModes` wraps as a flag `SmartEnum` policy set, and each `*State` snapshot wraps as a `ComplexValueObject` crossing domain code as one validated record.
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-persistence.md`: `PersistentSettings` under every owner is the custody spine, and its typed writer/reader surface composes there rather than being re-minted per settings family.
- `libs/dotnet/Rasm.Rhino/.api/api-rhinocommon-display.md`: `*AnalysisSettings` owners feed the `VisualAnalysisMode` false-color overlays, and `ViewSettings` restore-scope flags compose the defined-view restore there.
