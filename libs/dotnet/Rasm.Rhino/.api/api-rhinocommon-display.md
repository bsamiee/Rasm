...
- `libs/dotnet/.api/api-languageext.md`: every host call that can fail or return null crosses through `Try.lift`; nullable results bind `Optional(...).ToFin(Fail: new KernelFault.InvalidResult())`, so a captured bitmap, resolved mode, or bound conduit crosses into domain code as `Fin<A>`, never as a nullable host handle.
