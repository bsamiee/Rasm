...
- package: `Confluent.SchemaRegistry.Serdes.Protobuf` (Apache-2.0)
...
- `CloudNative.CloudEvents.Kafka`(`.api/api-cloudevents-kafka.md`): `SchemaIdSerializerStrategy.Header` moves the id into `Message<TKey,TValue>.Headers` beside the CloudEvents attributes, leaving the value a clean Protobuf message for a non-Confluent consumer.
