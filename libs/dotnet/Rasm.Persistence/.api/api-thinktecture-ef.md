...
- `Thinktecture.Runtime.Extensions.Json`(`libs/dotnet/.api/api-thinktecture-json.md`) and `.MessagePack`(`libs/dotnet/.api/api-thinktecture-messagepack.md`): the JSON converter factory and the MessagePack formatter resolver close over the same generated `ToValue` projection and static `Validate` admission this `ValueConverter` binds, so a stored key column, a JSON payload, and a binary snapshot carry one owner model.
