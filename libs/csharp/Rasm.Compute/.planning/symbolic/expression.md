...
- Receipt: the operation fold lands no receipt of its own; the differentiate output feeds `lowering` (which caches the compiled Jacobian delegate and stamps the `CompiledExpr` cache receipt) and the optimizer adjoint (which stamps the `Optimization` slot), the coefficient vector seeds `numeric/algebra#OWNED_BUILDS` integrator builds, and a transform fault rides the `ComputeFault` rail.
