...
- Packages: GeometryGymIFC_Core, NodaTime, VividOrange.Stages, Thinktecture.Runtime.Extensions, LanguageExt.Core, Rasm.Element, Rasm
...
// The free-label admission beside ProjectStage.Of: IfcProject.Phase is authored IfcLabel text with NO declared
// grammar (the egress restamps it VERBATIM off the Projection/semantic ProjectAttributeSet bag, and the COBie
// CobiePhase is a free named header entity), so the label-to-stage correspondence is a COMPOSITION VALUE — the
// caller declares which national scales its projects author and what an unmatched label means. Resolve runs
// rung-major — the composite Governance:Id spelling, then the scale-local Id, then the display Name, each trimmed
// ordinal-ignore-case, roster order the intra-rung precedence — each hit lowering through the settled two-tier
// ProjectStage.Of; an unmatched or blank label lands the roster's own declared Fallback, None absent — never a
// fabricated construction phase.
...
// back to the network default. Authored carries the IFC-declared duration under its own basis; the CPM ELECTS
// between it and the working content of the scheduled window, never a silent single source.
public sealed record ConstructionTask(
    ...
    // The task duration ELECTION: the authored ScheduleDuration read under its own DurationType row, else the
    // working content of the scheduled window. An authored duration is the planner's statement of work content and
    // the window is only its placement — reading the window alone prices every task whose bounds carry float.
    public Duration WorkContent(WorkCalendar calendar) =>
        Authored.Match(
            Some: authored => authored.Content(calendar, Scheduled.Start),
...
public sealed record ScheduleNetwork(
    ...
    // Identity mints through the ONE kernel seed-zero ContentHash over the seam CanonicalWriter fold — the hasher
    // every federation, solver, cache, and diff edge in this package shares, and which the Review/diff owner
    // already rules a hand-rolled XxHash128/Encoding.UTF8 string-join preimage the deleted form. The writer is not
    // a formatting convenience: it LENGTH-PREFIXES every string under a per-run Ordinal count, so no delimiter or
    // section marker can forge an equality between two different decompositions of one concatenation, and it
    // writes each bound as an I64 tick and each ratio at its own canon rather than through an `:R` render whose
    // text a culture or a runtime revision can move. Ordering stays ordinal: the assigned ids directly, the task
    // set by its unique GlobalId, and the edge set by its (predecessor, successor, kind, lag) chain so a real
    // parallel SS+FF pair between one task pair keys deterministically — both keys invariant to the unstable
    // IfcSet iteration order a re-parse yields.
    ...
    static UInt128 GeometryKeyOf(Seq<string> assigned) =>
        ContentHash.Of(assigned
            .Fold(new CanonicalWriter(0.0).Ordinal(assigned.Count), static (writer, id) => writer.String(id))
            .ToBytes().Span);

    static UInt128 ScheduleKeyOf(Seq<ConstructionTask> tasks, Seq<SequenceRel> edges) =>
        ContentHash.Of(edges
            .Fold(tasks
                .Fold(new CanonicalWriter(0.0).Ordinal(tasks.Count), static (writer, task) => writer
                    ...
                    .String(task.Stage.Map(static s => s.Key).IfNone(""))
                    ...
                    .Bool(task.IsMilestone))
                .Ordinal(edges.Count),
                static (writer, edge) => writer
                    .String(edge.PredecessorGlobalId)
                    .String(edge.SuccessorGlobalId)
                    .String(edge.Kind.Key)
                    .String(PeriodPattern.Roundtrip.Format(edge.Lag)))
            .ToBytes().Span);
    ...
    public Fin<ScheduleNetwork> BindAssignments(ElementGraph graph, Op key) {
        ...
        return Assignments
            ...
            .Match(
                Some: id => Fin.Fail<ScheduleNetwork>(new BimFault.DanglingReference(key, $"task-assigns-absent-element:{id}")),
                None: () => Fin.Succ(this));
    }
}
...
public static class ScheduleProjection {
    ...
    static Seq<SequenceRel> SequencesOf(Seq<IfcTask> tasks) =>
        toSeq(tasks
            .SelectMany(static task => task.IsPredecessorTo.OfType<IfcRelSequence>())
            .Select(rel => SequenceRel.Of(
                rel.SequenceType,
    ...
    static Fin<Interval> IntervalOf(DateTime? start, DateTime? finish, DateTimeZone zone, Op key) =>
        (InstantOf(start, zone), InstantOf(finish, zone)) switch {
            ...
            ({ } from, { } to)                 => Fin.Fail<Interval>(new BimFault.ModelRejected(key, $"task-finish-before-start:{to}<{from}")),
            ({ } from, null)                   => Fin.Succ(new Interval(from, from)),
            _                                  => Fin.Fail<Interval>(new BimFault.ModelRejected(key, "task-missing-schedule-start")),
        };
...
public sealed record WorkCalendar(
    ...
    // empty work-week or a barren-exhausted calendar RAILS: returning the seed fabricates a working instant no
    // calendar admits, and every float, early start, and late finish downstream then reads as a plausible schedule.
    public Fin<Instant> Advance(Instant from, Duration work, Op key) {
        if (work <= Duration.Zero) { return Fin.Succ(from); }
        if (WorkWeek.Count == 0) { return Fin.Fail<Instant>(new BimFault.ModelRejected(key, "work-calendar-empty-work-week")); }
        var cursor = from;
        var remaining = work;
        var barren = 0;
        ...
        return Fin.Fail<Instant>(new BimFault.ModelRejected(key, $"work-calendar-barren:{from}+{work}"));
    }
    ...
    public Fin<Instant> Recede(Instant to, Duration work, Op key) {
        ...
        return Fin.Fail<Instant>(new BimFault.ModelRejected(key, $"work-calendar-barren:{to}-{work}"));
    }
    ...
    // The COMPLEMENT of the same walk: the spans inside [from, to] carrying no working content, merged where they
    // abut. A working day contributes its pre-shift head and post-shift tail, a non-working day its whole extent,
    // and consecutive pieces that touch fold into one span — so a weekend reads as ONE band rather than as two days
    // and four partial-shift slivers. This is the one place non-working time is enumerable rather than measured:
    // WorkingBetween answers how much content a span carries, which cannot say WHERE the gaps are, and a consumer
    // that needs to draw or reason over those gaps would otherwise re-derive the work-week, the exception spans, and
    // the annual rows against this calendar and drift the moment a project declares an exception window.
    ...
    // The zone rebase every consumer holding a network reaches for. A sealed record's copy constructor is PRIVATE, so
    // a `calendar with { Zone }` outside this body does not compile — and without the rebase the calendar-LESS plan
    // silently grades in two zones at once: Default carries Utc while the network carries the model's work-calendar
    // zone, so a Berlin project reads its 08:00-16:00 shift as 08:00-16:00 UTC and every early start, working-content
    // measure, and float lands an offset out. The network's zone is the model authority; a calendar folded from that
    // same model already agrees and rebases to itself.
...
public static class ScheduleCpm {
    // The calendar is PER TASK — network.CalendarFor elects the task's own assigned IfcWorkCalendar and falls back
    // to the network default — so a six-day concrete crew and a five-day commissioning crew price their float on
    // their own shift content. A single network-wide calendar parameter is the deleted form: it silently grades one
    // crew's work on another's week, and every early start, float, and late finish downstream reads plausible.
    // Every calendar in the map is already rebased onto the network zone at projection.
    public static Fin<Map<string, CriticalPath>> Schedule(this ScheduleNetwork network, Op key) =>
        network.Tasks.IsEmpty
            ? Fin.Succ(Map<string, CriticalPath>())
            : Graph(network, key).Bind(graph => {
                var calendars = network.Tasks.Map(t => (t.GlobalId, network.CalendarFor(t))).ToMap();
                // The duration ELECTION runs per task on its OWN elected calendar: the authored ScheduleDuration
                // under its DurationType row, else the working content of the scheduled window.
                ...
                return Forward(graph, calendars, duration, projectStart, key).Bind(forward => {
                    var projectFinish = forward.Values.Map(static p => p.Ef).Max(projectStart);
                    return Backward(graph, calendars, duration, projectFinish, key)
                        .Bind(backward => Paths(graph, network, calendars, forward, backward, duration, key));
                });
            });
    ...
    static Fin<Map<string, (Instant Es, Instant Ef)>> Forward(
        ...
        toSeq(graph.SourceFirstBidirectionalTopologicalSort())
            .Fold(Fin.Succ(Map<string, (Instant Es, Instant Ef)>()), (rail, id) => rail.Bind(acc =>
    ...
    static Fin<Map<string, (Instant Ls, Instant Lf)>> Backward(
        ...
        toSeq(graph.SourceFirstBidirectionalTopologicalSort(TopologicalSortDirection.Backward))
            .Fold(Fin.Succ(Map<string, (Instant Ls, Instant Lf)>()), (rail, id) => rail.Bind(acc =>
    ...
    static Fin<Map<string, CriticalPath>> Paths(
        ...
        network.Tasks.Fold(Fin.Succ(Map<string, CriticalPath>()), (rail, task) => rail.Bind(acc =>
    ...
    // modality (ToFinish) RECEDES the successor working content off the lagged finish through the SUCCESSOR's own
    // calendar so the early start skips that crew's non-working days — the FF/SF backoff is calendar-consistent,
    // never a raw Duration subtraction. The lag anchors in the successor calendar's zone, which every calendar on
    // the network shares by the projection-time rebase.
    ...
    // predecessor START) ADVANCES the predecessor working content on the PREDECESSOR's own calendar so the bound
    // lands on its finish. A backward gate reading every out-edge as finish-to-start (dropping the columns and the
    // lag) is the deleted form — it misprices the float of every lagged or SS/FF/SF dependency.
            ...
            .Match(
                ...
                None: () => {
                    ...
                    return graph.IsDirectedAcyclicGraph()
                        ? Fin.Succ(graph)
                        : Fin.Fail<BidirectionalGraph<string, STaggedEdge<string, SequenceRel>>>(new BimFault.ModelRejected(key, "schedule-cyclic-dependency"));
                });
    }
}
```
