...
Exact commands from clone to running checks. Section contains one H2 heading (`## Development Setup`) followed by a single `sh` fenced code block with six sequential commands: `git clone <repo-url>`, `cd <repo>`, `pnpm install`, `pnpm check:ts`, `pnpm check:py`, `pnpm check:cs`.
...
- **Run language checks:** `pnpm check:ts`, `pnpm check:py`, `pnpm check:cs`
