...
|  [04]   | FAULT_DETAIL              | `fault-detail`              | domain         | `wire-bytes` + `canonical-json` | DESIGN-PIN |
...
- Consumers: `typescript:core/interchange/codec#FAULT_RAIL` admits the generated message and `typescript:core/interchange/invoke#TRANSPORT_FAULT` routes its recovery arm; `python:runtime/transport/shapes#VOCABULARY` decodes the producer schema, with `python:runtime/evidence/reproduction#SEED_REPRODUCTION` carrying the unpinned corpus row.
- Payload: `wire-bytes` + `canonical-json`
- Pin: DESIGN-PIN
- Blocker: the producer has not frozen one concrete envelope for each recovery arm.
- Shape: the compact `FaultDetail` carries `code`, `message`, `correlation`, HLC physical/logical coordinates, `tenant`, and the `FaultRecovery` oneof `terminal | transient | retry_after`. `code` is the sole transported identity; package, family, band, offset, and case stay local, and no peer rehydrates the producer union or mirrors `FaultBand`.
- Shape: admission requires one positive code and exactly one valid recovery arm; a malformed recognized detail refuses typed, an unknown detail coexists, and an unknown code remains opaque remote evidence. Remote domain codes never elect topology; transport outcomes alone drive failover.
- Shape: this spec-stage schema is compacted at every producer and consumer when a field is deleted; no reserved coordinate, alias, legacy field, or dual decoder preserves an unshipped shape.
- Regenerate when: the compact `FaultDetail` field roster or the `FaultRecovery` arm roster changes.
...
- Shape: `(serialization, release)` legality states admit and seal per cell — `ifcx`/`IFC5` decodes at the peers and refuses at the producer by name.
...
- Pin: DESIGN-PIN
- Blocker: every source this entry rosters carries a landed `.proto` beside a frozen `FileDescriptorSet` snapshot minted at one parity setting — single file in the set, no `include_imports`, no `source_code_info`: each source stamping the `option csharp_namespace` its own package derives — `rasm.<family>.v1` spells `Rasm.<Family>` — so a family owns its generated identity and a later source stamps its own without re-deciding, where one namespace shared across packages demands globally-unique message names estate-wide. Reachability closes with it: every peer decode of `rasm.element.v1` now resolves in a pool `typescript:core/interchange/format#PROTO_ENGINE`, `codec`, and `python:runtime/transport/shapes#REGISTRY_AND_DRIFT` each compile the same source into. EXECUTION is the residual gap at two of the three peers: `buf` mints every snapshot at the corpus, and `typescript:core/interchange/format#PROTO_ENGINE` now carries a runnable generation row — the root `buf.gen.yaml` `protoc-gen-es` row under `nx run workspace-foundation:proto`, proved emitting one `_pb.ts` per source — while no C# or Python source tree exists yet to run `Grpc.Tools` or `grpc_tools.protoc` over those sources and compare its own emission byte-for-byte, so the three-minter parity this pin graduates on holds by construction and stays unrun at those two peers.
...
- Class: infrastructure
- Minters: `csharp:Rasm.AppHost/Observability/telemetry#CONFORMANCE_PROJECTION`; `python:runtime/observability/telemetry#CONFORMANCE`; `typescript:core/observe/convention#IDENTITY_PROJECTION`
...
- Payload: `canonical-json` + `digest`
- Pin: DESIGN-PIN
- Blocker: two of three minters now project their rows as a document — `Conformance.Of(TelemetryComposition)` at `csharp:Rasm.AppHost/Observability/telemetry#CONFORMANCE_PROJECTION` and `ConformanceDocument.of(receipt)` at `python:runtime/observability/telemetry#CONFORMANCE` each return the role-stamped, schema-pinned `ConformanceRow` set, every row a case whose disposition and payload move together and whose owner column names the member the value projects off, the python leg resolving one cell per legislated key against a closed key vocabulary at import and framing through its branch's one deterministic encoder — while the typescript convention rows still hold their values as composition code with no canonical projection member, so one of three branches carries no byte-deriving input and the parity this entry proves has nothing to compare that leg against.
- Shape: the closed conformance row set `libs/.planning/ARCHITECTURE.md` `[08]-[OBSERVABILITY_CONFORMANCE]` legislates, projected as one document per minter — the resource triple and its detector-merge precedence, the scope coordinate (emitting package id, version stamp, semconv schema url) shared by tracer, meter, and logger, the metric-name grammar with its UCUM unit vocabulary and the minter's own domain roster of segment beside admitted subject, wire temporality and default histogram aggregation, exemplar posture, the propagation dialect set with its one global composite registration and the inbound parent-adoption posture, the tenant key with its promotion allowlist and view cap, the absent-entry posture every sometimes-absent dimension shares — tenant, level-family, and substrate keys read as the untagged whole when absent, stay rostered on the allow-list while sometimes absent, and a key that is its cell's own identity never constructs the absent state, egress protocol and compression beside the metrics-receiver translation strategy, buffer and loss posture, and lifecycle drain bounds. Role-aware absence is part of the shape rather than an exception to it: each row carries the disposition its minter's role admits over `process` | `browser` | `deploy`, so a browser minter marks the process profiler, the process health probe, and the process support archive `absent` while carrying logs, metrics, traces, vitals, and crash evidence, and a reader distinguishes a role that cannot carry a signal from a branch that dropped one. Capability absence rides the same column under its own disposition and never under role absence: a row whose plane exposes no seat at the minter's pinned distribution — the provider-wide histogram aggregation, the exemplar filter, the sender encoding knob, the per-view cardinality ceiling, and the log-record limits constructor are the seats the three planes reach unevenly — projects the value the plane carries beside the pin that withholds the seat, so the digest reads a stated ceiling rather than a dropped row and a pin bump that opens the seat fails here until the row re-values. Which seats a minter withholds is itself parity evidence: the C# plane withholds the aggregation seat the python plane carries on its metric exporter, and the python plane withholds the cardinality and log-record seats the SDK train exposes on no member. Every branch mints from its own composition inputs and parity across the three mints IS the conformance; transcription remains how each branch spells the rows, and this entry is what proves the three spellings agree. Domain rows carry that proof furthest: two minters claiming one segment project one subject spelling byte-identical or fail the digest here, so a vocabulary each branch declares locally still resolves to one estate namespace.
...
- Shape: `HopReceiptWire` and `DeliveryReceiptWire` each carry `fault` as a `FaultObservationWire` — the same compact observation `[02.4]` rules — and the column is COMPLEMENTARY to the outcome: a `delivered` leg carries none, while `refused` and `faulted` retain the exact observation the hop lowered, so a dashboard reads typed recovery beside the attempt count rather than re-deriving a disposition from a message string.
- Shape: `OutboxRowWire.fault` and `DeadLetterRowWire.fault` are `FaultObservationWire`, never a fault STRING, and they differ in obligation: the outbox row's is optional because a pending row has not failed, while the dead-letter row's is REQUIRED because a letter reaches that table only by failing — a dead letter carrying no observation names a producer that lost its own evidence.
- Shape: the AppHost runtime-evidence family set as ONE registration — `ReceiptEnvelopeWire`, `TenantContextWire`, `CommandAvailabilityWire`, `CredentialPemWire`, `BindingStatusWire`, `CoercedValueWire`, `WriteReceiptWire`, `FlagVerdictWire`, `SupportCaptureWire`, `HopReceiptWire`, `DeliveryReceiptWire`, `DropReceiptWire`, `OutboxRowWire`, `DeadLetterRowWire`, `ReplayTallyWire`, `OutboxLaneWire`, `OutboxSweepWire` — each family one `[JsonSerializable]` row on the producer roster and one census row at the consumer, field names the mechanical camelCase projection of the producer's own members, whose one author is the explicit `PropertyNamingPolicy = JsonNamingPolicy.CamelCase` the `Strict`-based merge sets: `Strict` carries no naming policy of its own, so the camelCase field-name law is that assignment rather than a preset default. Membership in this roster IS the cross-language discriminant: a family here holds a decoder row on the consumer census, while a C#-only receipt (`SandboxReceipt`, `UpdateReceipt`, `FenceReceipt<LeaseKey>`, the companion `VerbReceipt`/`CascadeReceipt`/`BindReceipt`/`Delivery`) rides `ReceiptEnvelopeWire` through the same merged context as a `[JsonSerializable]` row alone and carries no family row here. Two family names FORK across the boundary: `ReceiptEnvelopeWire` and `TenantContextWire` name the TS faces of the C# records `ReceiptEnvelope` and `TenantContext` that `csharp:Rasm.AppHost/Runtime/ports#WIRE_LAW` registers, the `Wire` suffix being minted at the TS projection, so a census join resolves the pair as one family under two spellings and neither end renames. Eight message-plane families project through generated Mapperly seams (`csharp:Rasm.AppHost/Wire/outbound#TS_PROJECTION` `OutboundMap`, `Wire/outbox#TS_PROJECTION` `OutboxMap`, `Wire/topics#TS_PROJECTION` `TopicsMap`) rather than hand transcriptions, so a producer column rename breaks the projection at build rather than at a peer decode. Two crossings are DECLARED narrowings the census must honor: the HLC ordinal crosses as a decimal STRING on `OutboxRowWire`, `DeadLetterRowWire`, `OutboxSweepWire`, and `DeliveryReceiptWire.watermark` because a `ulong` past 2^53 loses precision in a JSON number and exact comparison is the ordinal's whole purpose — the delivery receipt's watermark and the sweep receipt's are the SAME fenced advance, so a number on one face and text on the other would spell one cursor two ways across two receipts of one relay, and the hop and delivery measurement pair (`attempts`, `elapsedSeconds`) crosses ABSENT together on any leg that reached no pipeline, so a decoder reading either as zero reports a dial that never happened. Both `LiveWireContext` union lowerings carry no row of their own, and they carry none for OPPOSITE reasons: `WriteBackWire` crosses INSIDE `WriteReceiptWire.disposition` as a `kind`-discriminated literal union, so `WriteReceiptWire`'s census row decodes both, while `QualityWire` crosses INSIDE `MachineObservationWire.quality` and is therefore withdrawn with the carrier below — a census row for a quality union no decoded family reaches strands a decoder against nothing. `MachineObservationWire` is WITHDRAWN from this seam — its producer (`Wire/livewire#MACHINE_LANE`), its decoder (`Observability/instruments#RECEIPT_PROJECTION` `ReceiptKind.Observation`), and its readers (the `Rasm.Fabrication` wear, fleet, and engagement consumers) are all C#, so it is an in-process receipt payload and a registration asserting a peer decoder no branch declares is the stranded state this manifest exists to refuse. Carriage is JSON by the producer's landed wire law, so the consumer census carries these rows under its `json` arm, never `proto` — no descriptor source exists or is owed under `[02.9]`. Families a sibling entry already owns register THERE and not here: `HlcStampWire` byte layout is `[02.7]`'s shape (this producer carries it as a message-envelope field), `CapabilityDescriptorWire` is `[02.12]`, `HostFingerprintWire` is `[02.15]`, and `BenchmarkClaimWire` is `[02.14]` with `csharp:Rasm.Compute` the minter — a census row sourcing it to AppHost mis-names the producer. Host runtime evidence is the producer's domain capability, so peers decode the families and re-author none.
- Shape: `WriteReceiptWire.disposition` closes FOUR arms — `acknowledged{echo}` over the four-row echo-class roster, `rejected{fault}`, `rolled-back{priorValue}`, and `indeterminate{attempt,rollback}` — each fault slot a `FaultObservationWire`. The three failure arms stay distinct because their repairs differ: a rejection leaves the prior external value standing, a rollback restored a value the receipt names, and an indeterminate write means the attempt AND its rollback both failed, so the external value is unknown to this process. A boolean or a success flag over the three shows one badge for three unlike states.
- Shape: `FaultObservationWire` is the COMPACT fault shape this seam carries — `{recovery, causes, truncated, code?}` where `recovery` closes `terminal | transient | throttled{retryAfter}` — so no package, family, band, offset, or case column crosses, exactly as `[02.4]` rules for the remote detail. `truncated` says the bounded cause chain was cut, so a reader knows whether it holds the whole list.
- Shape: live-wire receipt kinds are distinct tokens a consumer routes on — `wire-status` carries `BindingStatusWire` and `write-back` carries `WriteReceiptWire`, so a committed transition and a committed write route apart; a reader keying both on one token cannot tell them apart.
- Shape: `CoercedValueWire` is `{bindingId,canonical,canonicalUnit,sourceUnit,sourceAt}` and the binding id is the KEY the value was coerced for, projected off `BindingSpec` at the producer's mapper rather than stored on the coercion record, which stays a pure two-unit transform. The push envelope's capability descriptor addresses the invocation and is a routing coordinate, not a column the decoded value carries, so an unkeyed face reached a consumer as a bare number pair no diff or board fold could attribute; the seam's conversion receipt still stays at the producer, so `MeasureEvidence` crosses nowhere.
- Shape: `WireRejectionWire` is WITHDRAWN from this seam — its producers (the transport, streaming, machine, and write-back lanes at `Wire/livewire`), its decoder (`Observability/instruments#RECEIPT_PROJECTION` `ReceiptKind.WireRejection`), and its readers are all C#, so it is an in-process refusal payload, the `wire-rejection` kind stays host-local, and a registration asserting a peer decoder no branch declares is the stranded state this manifest exists to refuse.
- Shape: every optional slot on this seam crosses ABSENT rather than null — the merge's `WhenWritingNull` posture and the resolver's `OmitAbsent` modifier omit an unset `Option<T>` outright — so a peer face spells it `field?: T` and a `| null` union declares a token that never appears.
...
- Shape: `CommandOutcomeWire` closes FIVE arms — `completed | cancelled | rejected{fault} | rolled-back{reason} | compensated{reason}` — where `rejected` carries a `FaultObservationWire` rather than a detail string beside a numeric code: a code is telemetry identity and a sentence is prose, and neither answers whether the refusal is worth re-offering where the observation's own recovery arm does. `rolled-back` and `compensated` stay two arms because an undone command and a compensated one owe the user different words.
- Shape: `DeckReceiptWire` nests `{key,surface,elapsed,outcome,payloadDigest,correlation}` as a command-family member, and the `EvidenceTimelineWire` command arm carries it typed rather than as an untyped payload.
...
- Shape: The `EvidenceTimelineWire` media and layout arms carry `fault` as a `FaultObservationWire` — the same compact observation `[02.21]` rules — replacing the media arm's numeric `code` and the layout arm's fault STRING; on the media arm `outcome` and `fault` are complementary by the producer's own projection, so a `ready` outcome lowers to no observation and a `failed` one always carries its error.
- Shape: The `EvidenceTimelineWire` native-asset arm carries `NativeAssetFactWire` `{library,version,path,rid}` typed, so no timeline slot decodes as an untyped payload.
...
- Shape: Command row, intent, invocation, outcome, and receipt wires are nested family members.
- Shape: `CommandRowWire` is `{key,scope,requires,gesture,targets,arguments}` — the deck's STATIC columns only; the availability delegate never crosses, so a head reads its verdict from `CommandGateWire` rather than re-running the predicate against a capability set it does not hold.
...
- Shape: the descriptor snapshot re-mints in the same change as any message, field, or reservation edit — a source ahead of its snapshot is the drift defect, never a pending regeneration.
...
- Shape: one product declaration keyed to a material identity — issuer + registration the duplicate-check pair, declared unit admitted at its own functional unit AND at quantity exactly one — a declaration whose amount reads as anything else refuses at ingest, so the wire carries no `declared_qty` column and no consumer rescales a cell — issue and expiry dates, and the per-indicator per-module impact map at declaration granularity (the frozen 13-indicator EN 15804+A2 roster over the 15-module EN 15978 roster) where KEY PRESENCE is the coverage census — an absent cell is undeclared absence, never a zero. Discriminating laws: a declared cell is a measured value with negative biogenic/avoided-burden carbon valid; the two resource fractions are optional scenario data, absent when undeclared; consumers band modules as their own seam requires (the C# leg sums declared cells onto its six-band `LifecycleStage` axis and constructs the full matrix only when every core indicator covers every band). Declaration semantics are the python data branch's host-free ingestion capability; peers decode and re-author none.
