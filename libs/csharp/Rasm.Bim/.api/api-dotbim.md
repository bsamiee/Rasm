...
[STACKING]:
- `SharpGLTF`(`libs/csharp/.api/api-sharpgltf.md`), `AssimpNetter`(`.api/api-assimpnetter.md`), `Alimer.MeshOptimizer`(`libs/csharp/.api/api-alimer-meshoptimizer.md`), `Openize.Drako`(`.api/api-openize-drako.md`): every mesh sibling meets `dotbim` at the canonical triangle mesh, so a `.bim` model re-exports as glTF through the shared carrier and its flat `Coordinates`/`Indices` soup feeds the meshopt and Draco compressors before a heavier export.
