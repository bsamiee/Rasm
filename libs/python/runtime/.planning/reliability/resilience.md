...
- Cases: `RetryClass` members `OBJECT_STORE` · `HTTP` · `SSH` · `WIRE` · `SCAN` · `SECRET` · `OCCT` · `RPC` · `LAKE_COMMIT` · `REMOTE_DB` · `STREAMING`, each one row in `POLICY` carrying its own attempts, timeout budget, `target`, and (where it diverges from the `stamina` default) its own backoff geometry as behavior columns. `REMOTE_DB` is the remote-database transport transient the `data:tabular/query#QUERY` `QueryEngine.run` `Remote` leg rides through `guarded(RetryClass.REMOTE_DB, ...)` — its `_adbc_transient(ConnectionError, TimeoutError)` `BackoffHook` discriminating an ADBC `dbapi.OperationalError` by `status_code in {AdbcStatusCode.TIMEOUT, AdbcStatusCode.IO}` (the shared DBAPI `Error` base both `adbc-driver-manager` and the Flight SQL driver raise, retried only on a genuine transport-transient status, a non-transient `INVALID_ARGUMENT`/`NOT_FOUND` left to fail) plus the stdlib `ConnectionError`/`TimeoutError` a re-dial clears, the one row whose status-code predicate forces a hook over a bare tuple; both ADBC dists are cp315-clean so the type imports module-top rather than `_named`. `STREAMING` is the out-of-core daft transient the same owner's `Streaming` leg rides through `guarded(RetryClass.STREAMING, ...)` — its bare `(DaftTransientError, TimeoutError, ConnectionError)` tuple over `daft.exceptions.DaftTransientError`, the single Rust-backed base for the `ConnectTimeoutError`/`ReadTimeoutError`/`ByteStreamError`/`SocketError`/`ThrottleError`/`MiscTransientError` object-store-and-network transients a re-scan clears (`daft` cp315-clean, so the base imports as a type), `wait_max` widened for a long distributed scan. Neither transient is caught by the `WIRE` `(ConnectionError,)` intra-mesh row or the `RPC` `_named` COMPAS XML-RPC qualnames the prior remote/streaming arms wrongly rode — an ADBC `OperationalError` subclasses `DatabaseError`/`Error`/`Exception` (never `ConnectionError`/`TimeoutError`) and a daft fault is a Rust-backed `DaftCoreException`, so under the old targets the remote/streaming legs retried nothing. `LAKE_COMMIT` is the transactional-commit-conflict transient the synchronous `data:tabular/lakehouse#LAKEHOUSE` `Lakehouse.run` mutating arms ride through the sync `guarded_sync` envelope — its `_named("CommitFailedError", "CommitFailedException", "CommitStateUnknownException")` structural `BackoffHook` naming the `deltalake.exceptions.CommitFailedError` and pyiceberg `CommitFailedException`/`CommitStateUnknownException` concurrent-writer conflicts by `__qualname__` (the third use of the `_named` arm beside `RPC`, since `pyiceberg` is the gated `python_version<'3.15'` companion dark on the cp315 runtime core), a re-read-and-retry clearing the conflict. `OCCT` is the subinterpreter-offload transient class the `execution/lanes#LANE` `offload` retry leg rides — its `target=(anyio.BrokenWorkerInterpreter, anyio.BrokenWorkerProcess)` the PEP-734 worker cold-start crash surface a transient re-spawn clears, the one `anyio` worker-break transient the table admits so the geometry/scan offload consumers (`mesh/daemon.md#DAEMON`, the scan kernels) retry a transient hop by passing `RetryClass.OCCT` rather than hand-opening a retry loop around the offload. `RPC` is the cross-process `compas.rpc.Proxy` bring-up transient class the `geometry/graph/algebra.md#ALGEBRA` `solver_proxy` scope-entry offload rides — the XML-RPC analog of `OCCT`'s subinterpreter cold-start: a fresh `Proxy(...)` whose `_try_reconnect` finds no running localhost server spawns one through a blocking `start_server()` (synchronous `Popen` plus a `time.sleep(0.1)` ping loop up to `max_conn_attempts=100`) that raises `compas.rpc.RPCServerError` ("The server is not available.") on a cold-start timeout and `RPCClientError` on a remote-dispatch fault, so a transient bring-up retries under one stamina row rather than the geometry owner hand-rolling a re-spawn loop. The `target` column is the full `stamina` discriminator: `HTTP` binds a `BackoffHook` that honours a server-directed `Retry-After` delay (an exception satisfying the `RetryAfter` structural protocol the transport boundary populates from the `429`/`503` response header) before falling through to the `stamina` exponential schedule on its transient exception set, while `OBJECT_STORE`/`SSH`/`WIRE`/`SCAN`/`SECRET` bind a bare retryable-exception tuple — `OBJECT_STORE` is the deliberate outer envelope over `obstore`'s Rust-core `RetryConfig`, so the stamina row catches only the `BaseError` transients that survive the store's internal retry and never re-introspects a provider header the store owns. `SECRET` is the `execution/admission#SETTINGS` `SecretBoundary` tier-probe envelope, its `target=(keyring.errors.KeyringLocked, OSError)` tuple the keystore/file transients a reused `SSH`/`OBJECT_STORE` row would never catch — `keyring.errors.KeyringLocked` is one of the four provider-specific transient roots the table type-imports beyond stdlib/structural — the others being the `OBJECT_STORE` row's `obstore.exceptions.BaseError`, the `REMOTE_DB` row's ADBC `dbapi.OperationalError`/`AdbcStatusCode` (a status-code predicate over the typed root, not a response-shape introspection), and the `STREAMING` row's `daft.exceptions.DaftTransientError` base — each a typed transient class or its status enum, never a provider response shape, so resilience reads a typed root or a documented status code and never grows a header-introspection surface. The `RPC` row is the one transient whose provider package (`compas`, the gated `python_version<'3.15'` companion band) is dark on the cp315 runtime core, so it cannot import `RPCServerError`/`RPCClientError` as types the way `SECRET`/`OBJECT_STORE` import their roots without crashing the runtime load on cp315; it instead binds the `_named("RPCServerError", "RPCClientError")` `BackoffHook` (the second use of the `BackoffHook` arm beside `HTTP`), a structural by-`__qualname__` discriminator that names the compas transient without a hard cp315-breaking `compas` import — the proxy only ever runs on the cp312 companion where the real exception is raised, while the policy table loads everywhere. Backoff geometry is a per-row override, not a global knob: `WIRE` keeps the tight `stamina` default for fast intra-mesh reconnection (every `wait_*` column `UNSET`), `SCAN` sets `wait_max=30.0` so a long-poll filesystem scan backs off far past the default cap without inflating the attempt count, and `RPC` opens `wait_initial` wide (the spawned worker needs time to bind its port before the next attempt pings) under a long `timeout` covering the worst-case ping-loop bring-up — a class needing distinct geometry adds the column on its own row, never a tuning parameter threaded through `guarded`/`retrying`. The class is the key, the policy is the value, and the row carries its full behavior including whether a server-rate-limit hook overrides the schedule and how wide the schedule opens, never a `streaming=True`-style flag the caller re-derives.
...
- Packages: `stamina` (`AsyncRetryingCaller`, `BoundAsyncRetryingCaller`, `RetryingCaller`, `BoundRetryingCaller` the sync caller pair `guard_sync`/`guarded_sync` build on, `retry_context`, `Attempt`, `ExcOrBackoffHook`, `BackoffHook`, `set_testing`, `instrumentation.set_on_retry_hooks`, `instrumentation.StructlogOnRetryHook`, `instrumentation.RetryHook`, `instrumentation.RetryHookFactory`, `instrumentation.RetryDetails`), `anyio` (`BrokenWorkerInterpreter`/`BrokenWorkerProcess` the `OCCT` row's subinterpreter/worker-break transient set the `execution/lanes#LANE` `offload` retry leg retries a cold-start crash under, the one `anyio` worker-break transient the table admits beyond the stdlib/structural exception types), `obstore` (`exceptions.BaseError` the `OBJECT_STORE` row's transient base — the store's typed root the stamina envelope catches after the Rust-core `RetryConfig`, never `OSError` which would miss every obstore fault), `keyring` (`errors.KeyringLocked` the `SECRET` row's transient the `execution/admission#SETTINGS` tier probe retries under, the one type-imported provider-specific `target` beyond `obstore`), `adbc-driver-manager` (`AdbcStatusCode` the transport status enum the `REMOTE_DB` `_adbc_transient` hook reads `TIMEOUT`/`IO` off and `dbapi.OperationalError` the shared DBAPI `Error` base it discriminates by `status_code` — both cp315-clean, type-imported module-top, the `data:tabular/query#QUERY` `Remote` leg's transient root, the Flight SQL driver raising the same DBAPI base), `daft` (`exceptions.DaftTransientError` the single Rust-backed transient base the `STREAMING` row's tuple catches, cp315-clean, the `data:tabular/query#QUERY` `Streaming` leg's transient root), `compas` (the `RPC` row's `rpc.RPCServerError`/`rpc.RPCClientError` cross-process proxy bring-up transient surface from the `geometry/graph/algebra.md#ALGEBRA` `solver_proxy` scope — named structurally by `__qualname__` through the `_named` `BackoffHook` rather than imported as a type, since `compas` is the gated `python_version<'3.15'` companion dark on the cp315 runtime core; the same structural/provider-root-only justification as the `obstore`/`keyring` roots, never a provider response shape), `expression` (`Map.of_seq`, `Map.empty`, `Map.__getitem__`, `Map.add`), `opentelemetry-api` (`trace.get_tracer` ENTRYPOINTS [03], `Tracer.start_as_current_span` ENTRYPOINTS [02], `Tracer.start_span` ENTRYPOINTS [01] with `attributes=` at creation, `trace.use_span` ENTRYPOINTS [08], `Span.set_status`/`Status`/`StatusCode` ENTRYPOINTS [06]/PUBLIC_TYPES [11]), `reliability/faults#FAULT` (`RuntimeRail`, `async_boundary` the async envelope, `boundary` the synchronous `guarded_sync` envelope), `deltalake`/`pyiceberg` (the `LAKE_COMMIT` row's `CommitFailedError`/`CommitFailedException`/`CommitStateUnknownException` commit-conflict surface from the `data:tabular/lakehouse#LAKEHOUSE` mutating arms — named structurally by `__qualname__` through the `_named` `BackoffHook` rather than imported as types, since `pyiceberg` is the gated `python_version<'3.15'` companion dark on the cp315 runtime core, the same structural justification as the `compas` `RPC` row), `observability/receipts#RECEIPT` (`Receipt.of(owner, evidence)`, `Redaction`, `Signals.emit`), `observability/metrics#METRIC` (`Metrics.retry_hook()` the metrics-owned `RetryHook`-returning classmethod composed into the one `RETRY_HOOKS` registration so the `retry.attempts` counter rides the same on-retry signal as the receipt and the structlog warning), `msgspec` (`Struct` the frozen `Policy` carrier, `UNSET`/`UnsetType` the absent-column sentinel distinguishing "use the `stamina` default" from a column the row sets to `None`, dropped from the `Policy.schedule` projection so an unset axis never reaches the `stamina` keyword), `functools` (`cache`), `typing` (`Protocol`, `runtime_checkable`, `assert_never`).
- Growth: a new retryable class is one `RetryClass` member plus one `POLICY.add` row; the `guard` bound-caller cache, the inline `retrying` context, the fused `guarded` envelope, and the receipt hook all derive from that row with zero new surface — the `SECRET` member the `execution/admission#SETTINGS` tier ladder requires is exactly that one member plus its `POLICY.add("secret", ...)` row, and the `RPC` member the `geometry/graph/algebra.md#ALGEBRA` `solver_proxy` scope requires is exactly one member plus its `("rpc", Policy(target=_named("RPCServerError", "RPCClientError"), ...))` row in the `POLICY` `of_seq` seed, never a knob threaded through `guarded`. A presign-vs-payload split, a distinct circuit envelope, or a `Retry-After`-honouring variant of an existing class is one new member with its own `target` column — never a `guarded`/`retrying` parameter or a parallel method. A transient whose provider package is gated-dark on the runtime core binds a `_named(*qualnames)` `BackoffHook` rather than importing the exception type, so the row loads on cp315 while still matching the real raise on the companion; a cp315-clean provider whose transient carries a sub-class-level status discriminant (the `REMOTE_DB` ADBC `OperationalError` `status_code`) binds a typed predicate `BackoffHook` (`_adbc_transient`) gating on that status, while one whose transients share a single base (the `STREAMING` `DaftTransientError`) binds a bare tuple over the base — the row's `target` shape follows the provider's own discriminant, never a flag the caller re-derives. `POLICY` carries exactly one row per `RetryClass` member, so `guarded`, `retrying`, and `guard` are total over the closed `StrEnum` by construction — a `Map.__getitem__` miss can only arise from adding a member without its `POLICY.add`, a build/wiring error rather than a domain fault, which is the single way to break totality.
- Boundary: product outbound resilience, circuit breaking, and hop policy stay AppHost-owned. Deleted forms: a manual retry loop with hand-coded `sleep` backoff; a parallel `Retry-After` sleep path outside the `on=` hook; resilience introspecting an httpx/provider response shape to read `Retry-After` (the transport owner maps that header into the host-neutral `RetryAfter` slot resilience reads); a second stamina envelope duplicating `obstore`'s Rust-core `RetryConfig`; blanket exception retrying; a fetch-shaped leg composing the bare `guard(cls)` caller inside its own hand-opened span and `async_boundary` fence where `guarded` fuses the retry/span/terminal-lift triplet once — the doubled-span/doubled-lift form the transport/secret legs name a deleted shape (distinct from the `execution/lanes#LANE` `retried` admission row that legitimately binds bare `guard(cls)` as a per-unit retry aspect inside its own already-railed drain); re-binding a caller per call or a `functools.cached_property` on the enum; caching the one-shot `retry_context`; a per-caller wait knob threaded through `guarded`/`retrying` where a class needing distinct geometry sets its own `wait_*` column on its `Policy` row; a thin `guard` forwarder wrapping a private `_caller` where the `functools.cache` rides the public entry directly; the schedule keyword set re-spelled independently in the bound-caller build and `retrying` where `Policy.schedule` projects it once; a provider-introspection surface on any `target` beyond the `OBJECT_STORE` `obstore.exceptions.BaseError` base, the `SECRET` `keyring.errors.KeyringLocked` transient, the `OCCT` `anyio.BrokenWorkerInterpreter`/`BrokenWorkerProcess` structural worker-break set, the `RPC` `_named("RPCServerError", "RPCClientError")` structural by-`__qualname__` discriminator, the `REMOTE_DB` `_adbc_transient` ADBC `dbapi.OperationalError` `status_code in {TIMEOUT, IO}` predicate over the typed root, and the `STREAMING` `daft.exceptions.DaftTransientError` typed base where every other class stays on stdlib/structural exception types (the `anyio` worker-break types being structural concurrency transients, the compas RPC bring-up errors structural cross-process transients, the ADBC status code a documented transport-status enum and not a response shape, and the daft transient a typed Rust-backed base, never a provider response shape) — and a bare `(adbc OperationalError, ...)` tuple on the `REMOTE_DB` row that over-retries a non-transient ADBC fault the status code marks permanent where the `_adbc_transient` hook gates on `TIMEOUT`/`IO`; a hard module-level `from compas.rpc import ...` type import on the `RPC` row where the gated companion is dark on the cp315 runtime core and the `_named` `BackoffHook` names the transient structurally without crashing the runtime load; `str()`-pre-formatted numeric receipt facts where the receipts `dict[str, object]` map and its `enc_hook=repr` renderer coerce the native scalar; a `TEST` mode that calls `set_testing(True)` but leaves a prior install's hooks in the process-global table where it must also register `()`; retrying a domain `Error(BoundaryFault)` (not an exception — retry triggers only on a raised transient or the row's `target` hook); a duplicate recoverability classification minted here rather than the faults owner's `BoundaryFault.recoverable` at the caller; and a second `set_on_retry_hooks` anywhere — the metrics owner re-registering or a per-leg hook install — because the stamina hook table is process-global and the last write wins, so the composed `RETRY_HOOKS` is the one source of the on-retry signal.
...
def _named(*qualnames: str) -> stamina.BackoffHook:
    ...
    # gated-dark on the cp315 runtime core (`compas`, the `python_version<'3.15'` companion): the
    # `RPC` row names `RPCServerError`/`RPCClientError` here rather than importing the type, so the
    # `POLICY` table loads on cp315 while the proxy raise (only ever on the cp312 companion) still
...
def _adbc_transient(*transient: type[Exception]) -> stamina.BackoffHook:
    ...
    # both cp315-clean — Cython source-build / native wheel — so the type imports module-top, never
...
POLICY: Final[Map[str, Policy]] = Map.of_seq([
    ...
    # cp312 companion dark on the cp315 runtime core; `wait_initial` opens wide so a re-spawned worker binds
    ...
    # gated `python_version<'3.15'` companion dark on the cp315 runtime core, so the row names the
    ...
    # needs its own row; `adbc-driver-manager`/`adbc-driver-flightsql` are cp315-clean, so the type
    ...
    # rather than `_named`, since `daft` is cp315-clean (the `WIRE` `(ConnectionError,)` row caught none
