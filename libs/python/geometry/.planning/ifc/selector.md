...
The validated element-selection grammar admitting a structured query before `ifcopenshell.util.selector.filter_elements`: `IfcSelector` compiles one `lark` EBNF faithful to the upstream `filter_elements_grammar`, folds the parse `Tree` into a frozen `SelectorQuery` of `Facet` cases, and lifts an `UnexpectedInput` parse failure into the `RuntimeRail` at admission, so a malformed selector is a typed `BoundaryFault` whose `subject` names the offending query at the fence, never a silent empty match three arms deep. `ifcopenshell` runs the filter; `lark` owns the closed query vocabulary the string parses against — the grammar admits and re-serializes selection without a second engine.

The `geometry:ifc/analysis.md#ANALYSIS` quantity/pset arms and the `geometry:ifc/costing.md#LIFECYCLE` take-off arm thread their free-form `query` through this boundary and drive elements off `IfcSelector.filter`, the only `filter_elements` caller. `SelectorQuery.filter_string` re-serializes the validated query to the exact `filter_elements` grammar and round-trips — the upstream engine re-accepts every string this owner emits, the frozen wire name the siblings pass back. The parse admits through the `reliability/faults#FAULT` `boundary`/`traversed` rail, and `SelectorQuery` is itself the `observability/receipts#RECEIPT` contributor the `@receipted` egress aspect harvests, so the parse-once gate the two siblings share streams its admission/rejection fact without an inline emit.

## [01]-[INDEX]

- [01]-[SELECTOR]: the one `lark`-grammar selector surface — the upstream-faithful EBNF, the `Facet` row algebra whose case renders back to the `filter_elements` string, the `parse` boundary lifting `UnexpectedInput` into the `RuntimeRail`, and the `filter` leg driving `filter_elements`.

## [02]-[SELECTOR]

- Owner: `IfcSelector` — the `@staticmethod` boundary capsule whose `@cache`-memoized `_engine` builds the `Lark` parser and `SelectorTransformer` once, exposing the polymorphic `parse`, the `filter` leg, and the private `@receipted` `_emit` point. `Facet` the `@tagged_union(frozen=True)` collapsing the eleven upstream facets onto four cases by shared shape — `identified` a negatable `instance` GlobalId or `entity` IfcClass, `attribute` a capital-initial name plus a comparison, `keyed` the six `keyword comparison value` facets, `qualified` a `property`/`query` dotted-path predicate — never eleven parallel cases or a flat `axis`-tagged bag. `SelectorComparison` the frozen value object owning the operator/negate/value triple and its `render`, one carrier every comparing facet shares rather than three fold-positional children re-discriminated per case. `SelectorQuery` the frozen fold product holding the facet groups, owning the `filter_string`/`axes`/`span_facts` projections and implementing the receipt `ReceiptContributor` itself — no parallel `SelectorReceipt`. `SelectorOperator`/`IdentifyAxis`/`QualifyAxis`/`SelectorKeyword` the closed `StrEnum` vocabularies; `SelectorTransformer` the `Transformer_NonRecursive` folding the wide `+`/`,` spine iteratively, no Python recursion limit.
- Cases: the grammar `start` is one `filter_group` — a `+`-union of `,`-chained `facet_list`s, upstream's two operators: `+` unions groups (`|=` across the appended lists), `,` chains additive/subtractive facets against a running set. Contains is `*=`, negation the `!` prefix on an identifier or comparison. Each `facet` folds to one `Facet` case matched by `match`/`assert_never` on both the fold and the `render` re-serialization, mirroring the `geometry:ifc/analysis.md#ANALYSIS` `AnalysisRow.facts` self-projecting row.
- Entry: `IfcSelector.parse` is polymorphic — a `str` parses one query, an `Iterable[str]` folds through `traversed(..., by=Disposition.ABORT)` into one `RuntimeRail[Block[SelectorQuery]]` so a batch validates under one rail short-circuiting on the first malformed member, never a per-arm loop. The single-string arm runs `parser.parse` then `transformer.transform` under `boundary(f"selector.parse:{text}", ...)`, so the offending query rides the fence `subject` the rejected receipt's `BoundaryFault.facts()["subject"]` carries — a `CLASSIFY` `(subject, cause)` builder never sees the source `text`.
- Auto: the parser is `Lark(SelectorGrammar, start="start", parser="earley")` — Earley for the ambiguous `+`/`,`/predicate grammar, the algorithm upstream itself builds. `cache=` is NOT set: `lark` raises `ConfigurationError` on parser-cache serialization for any parser but `lalr`, so the `@cache`-memoized `_engine` compiling the EBNF once on first parse is the build-once mechanism. `@receipted(_REDACTION)` decorates the private `_emit`; `filter` emits transitively because it composes `parse`, never a second decorated leg.
- Packages: `lark` (`Lark(..., parser="earley")`, `Transformer_NonRecursive().transform`, `v_args(inline=True)`, `UnexpectedInput` — `cache=` excluded, `lalr`-only), `ifcopenshell` (`util.selector.filter_elements` consuming `filter_string`, the only selection engine), `reliability/faults#FAULT` (`RuntimeRail`/`boundary`/`traversed`/`Disposition`/`FAULT_CONF` — no dedicated `lark` `CLASSIFY` row, since the universal faults owner never imports a geometry-domain grammar and a parse failure is exactly the message-carrying catch-all case), `observability/receipts#RECEIPT`, `expression` (`tagged_union` the `Facet` algebra, `Block` the batch carrier), `msgspec` (`Struct` the frozen `SelectorComparison`/`SelectorQuery`), `beartype` (`@beartype(conf=FAULT_CONF)` on `parse`).
- Growth: a new upstream facet is one EBNF alternative, one `Facet` case (or one `SelectorKeyword`/`IdentifyAxis`/`QualifyAxis` row when it folds onto an existing shape), one transformer method, and one `render` arm; a new operator is one `SelectorOperator` row plus one `OP` alternative — no second parser, no per-facet sibling class, no receipt edit. Threading the lexer's raw `pos_in_stream` into the rejected receipt is the deferred faults enhancement: it needs the source `text` at the classifier a `(subject, cause)` builder cannot reach, a faults-owner edit, never a runtime→`lark` coupling.
- Boundary: no privately re-invented dialect — `SelectorGrammar` mirrors `filter_elements_grammar` rule-by-rule, so fabricated operators, prefixes, and qualifiers upstream rejects never enter; no hand-rolled regex/split parser; no second selection engine past the `filter_string` round-trip; no stringly passthrough of the raw query past admission; no `cache=True` on an Earley parser, and no `SelectorOperator(str(token))` or `raise UnexpectedInput` in a fold body where the grammar terminal already bounds the children.
...
# delimiter re-quotes as ESCAPED_STRING to round-trip. Delimiter set is upstream's `[^,.=><*!\s]` plus `"` — `.` delimits
