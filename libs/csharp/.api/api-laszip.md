...
- `int` status returns and `get_error()`/`get_warning()` messages trap onto the consuming rail's typed fault at the boundary — `Rasm.Bim/Model/faults#FAULT_BAND` `BimFault.CodecReject` on the reconstruct leg, `ScanFault.CodecReject` on the residence leg
