...
[STACKING]:
- `api-cloudevents-mqtt`(`.api/api-cloudevents-mqtt.md`): `MqttExtensions.ToMqttApplicationMessage(ContentMode.Structured, formatter, topic)` returns exactly the `MqttApplicationMessage` `PublishAsync` consumes, so a publishing leg appends its trace rows to `UserProperties` and nothing else before the call.
