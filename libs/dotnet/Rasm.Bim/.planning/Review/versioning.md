...
public sealed record BimCommit(
    ...
    public static Fin<BimCommit> Of(ElementGraph graph, Seq<ContentAddress> parents, string author, string message, Instant at, Option<FidelityLog> fidelity = default) {
        ...
        return collided.IsEmpty
            ...
            : Fin.Fail<BimCommit>(new BimFault.Refused(BimScope.Review, BimReason.Rejected, string.Join(':', new object?[] { "duplicate-globalid", "commit", string.Join(',', collided) })));
    }
...
public sealed record BimRepository(Map<ContentAddress, BimCommit> Commits, Map<string, BimBranch> Branches) {
    ...
    public static Fin<BimRepository> Of(Seq<BimCommit> commits, Seq<BimBranch> branches) {
        ...
        return (duplicated.IsEmpty, dangling.IsEmpty) switch {
            (false, _) => Fin.Fail<BimRepository>(new BimFault.Refused(BimScope.Review, BimReason.Rejected, string.Join(':', new object?[] { "repository-branch-duplicate", string.Join(',', duplicated) }))),
            (_, false) => Fin.Fail<BimRepository>(new BimFault.Refused(BimScope.Review, BimReason.DanglingReference, string.Join(':', new object?[] { "repository-branch-absent", string.Join(',', dangling) }))),
    ...
    public Fin<MergeOutcome> Merge(ContentAddress ours, ContentAddress theirs, Func<string, ImmutableArray<AspectDelta>> deltas) =>
        from lineage in Fin.Succ(Lineage())
        from _ in guard(lineage.IsDirectedAcyclicGraph(), () => (Error)new BimFault.Refused(BimScope.Review, BimReason.Rejected, string.Join(':', new object?[] { "merge-lineage-cyclic" })))
        from o in Commits.Find(ours).ToFin(new BimFault.Refused(BimScope.Review, BimReason.DanglingReference, string.Join(':', new object?[] { "merge-commit-absent", "ours", ours.ToValue().ToString("X32", CultureInfo.InvariantCulture) })))
        from t in Commits.Find(theirs).ToFin(new BimFault.Refused(BimScope.Review, BimReason.DanglingReference, string.Join(':', new object?[] { "merge-commit-absent", "theirs", theirs.ToValue().ToString("X32", CultureInfo.InvariantCulture) })))
    ...
    public Fin<(BimRepository Repository, BimCommit Commit)> CommitMerge(
        MergeOutcome resolved, Seq<ContentAddress> parents, string branch, string author, string message, Instant at,
    ...
    (BimRepository Repository, BimCommit Commit) Seal(BimCommit commit, string branch, Option<BimHooks> hooks) {
        ...
        ignore(hooks.Map(live => live.Fire(BimPoint.Committed, new BimFact.Committed(CommitKey: commit.CommitKey.ToValue(),
