...
- Shape: branch README `[02]` rows and `libs/csharp/.api/` catalogs for `ACadSharp`, `Alimer.Bindings.MeshOptimizer`, `Apache.Arrow`, the three `CloudNative.CloudEvents` rows, `MessagePack`, `Microsoft.Data.Sqlite`, `NATS.Net`, `NREL.OpenStudio.macOS-arm64`, the two `NetTopologySuite.IO` rows, `PeterO.Numbers`, `PollinationSDK`, the two `Speckle` rows, and `geometry3Sharp`; each folder-tier catalog re-cuts to a genuine domain overlay or a registration. `pocketken.H3` and the `SharpGLTF` rows are LANDED at the branch tier (`api-h3.md`, `api-sharpgltf.md`, `api-sharpgltf-3dtiles.md` with registry rows and folder-README `[03]` moves) — the executed pattern the remaining roster follows.
