...
public sealed partial class ProvKind {
    ...
    // A neural acquisition run: the activity whose recorded model card, licence class, and execution provider ARE
    // the evidence the Blob retention class of an acquired texture rests on — a retired card or a drifted provider
    // makes the bytes unreproducible, and this row is where that fact durably lands.
...
// A completed neural-acquisition run as VALUES the composing seam projects off the producer's own provenance
// receipt (the Materials wire carries ModelCard/License/ModelArtefact as its Key(12)/(13)/(16) columns) — the
// model card + licence + artefact digest triple is the hadPlan Plan, the execution provider names the software
// agent, and no Materials type crosses the S2 seam. This fact is the durable ground under the Blob retention
// class of an acquired texture: the catalog's "unreproducible bytes" claim now cites a landed activity.
...
public static class CausalDag {
    ...
    // and artefact digest key the hadPlan Plan entity, so "which card produced these bytes" is a lineage read;
    // the execution provider is the software agent, delegating to the human submitter — the evidence fold the
