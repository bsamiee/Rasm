...
- Entry: `MaterialsHooks.Live(gates, taps, cell)` mints the hook set once at composition, seating one kernel point per `MaterialsPoint` row from `Items` alone; `hooks.Fire(fact.At, fact)` is the emitter entry and `hooks.Points` the census a `HookRegistry` freezes at the app root; `HookMounts<MaterialsPoint, TelemetrySource>` carries any rider custody a host composition claims over these seats.
