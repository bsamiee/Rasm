...
[LOCAL_ADMISSION]:
- MiniExcel is the spreadsheet + CSV codec for tabular interchange profiles (`Sync/schedule`, `Catalog/cost` import-export, `Ingest/tabular` element ingress) where the source is `.xlsx` or `.csv`; profile options live on a typed `OpenXmlConfiguration`/`CsvConfiguration`, never inlined.
