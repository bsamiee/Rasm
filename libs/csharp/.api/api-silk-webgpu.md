...
- within-lib: three composing folders hold three device dispositions over one binding — `Rasm.AppUi` mints the presented device and owns its lifecycle capsule, `Rasm.Compute` binds that same device for GPGPU dispatch and mints none, and `Rasm.Materials` acquires its own SURFACELESS bake device because a bake runs with no viewport open. Each folder folds every native-handle create-and-release pair in a scoped fold and projects validation faults and resolved timestamp durations onto its own telemetry spine.
