...
- tailwind target: v3 default config is tuned for Tailwind CSS v4 (the folder's `tailwindcss@4`); v3's own default groups match v4's default theme
...
|  [04]   | `getDefaultConfig() → Config`                                                           | base config    | the default v4-tuned config to inspect or spread when composing a custom one |
...
- Set `prefix`/`cacheSize` on the shared config, not per call; treat `experimentalParseClassName` as an experimental escape hatch, pinned by version, only for a nonstandard class syntax.
