...
public sealed record MaterialWire(
    ...
    public static MaterialWire Project(MaterialId id, MaterialParameters parameters, Provenance provenance, SurfaceShade preview) {
        ...
        return new MaterialWire(
            ...
            Emission: parameters.EmissionProvenance.Match(Some: static e => WireMap.ToWire(e), None: static () => (WireEmission?)null));
    }
...
// --- [OPERATIONS] --------------------------------------------------------------------------
// Mapperly GENERATES the wire transcription: it emits the ~29-column OpenPbrSurface→wire and the Provenance→wire member assignments at compile
// time, and the RMG unmapped-member diagnostics (RequiredMappingStrategy.Both) are the wire-completeness proof — a new source column with no
// wire slot, or a wire column with no source, FAILS the build (the named-argument guard the deleted hand projections carried, now
// compiler-owned). MappingConversionType.None: every cross-type hop is an explicit user mapping (WireColor.Of via [UseStaticMapper]), never an
// implicit cast masking a mismatch. Union-case dispatch is NOT mapped here — generated Switch is compile-time total where [MapDerivedType]'s
// unregistered-case arm throws at runtime.
...
public static partial class WireMap {
    ...
    static double? CalibrationDeltaEOf(Provenance p) => p.CalibrationDeltaE.Match(Some: static v => v, None: () => (double?)null);
...
public static class Mtlx {
    ...
    // Row resolves per-node category + concrete output type as ONE step: an Input probes its pulled PortValue polarity, a Texture reads its bound
    // source's category row (image floor when unbound), math/mix read their OP rows, and an op with no MaterialX 1.39 stdlib category (Fresnel, the
    // six HSL/soft blends) rails loud, never a silent multiply. Lowered mix rows register their OUTER wrapper (the `mix` node downstream edges
    // reference and type against).
    ...
    // MixRows carries the BlendMode→MaterialX disposition as DATA keyed by the MixOp row's Mode datum. mix/screen/overlay/dodge/burn/difference
    // are native factor-bearing blend nodes; Multiply/Darken/Lighten ride the factorless math multiply/min/max and HardLight the operand-swapped
    // overlay, each Lowered under the `mix` wrapper; SoftLight/Exclusion/Hue/Saturation/Colour/Luminosity have NO MaterialX 1.39 stdlib node and
    // rail loud per the Fresnel law — never a silently re-labeled composite, never a dropped factor.
    ...
    // lerp survives, downstream edges keep referencing node{id}, and the DAG topology stays recoverable. Row gated membership, so the MixRows
    // index cannot miss here.
...
public sealed record EnvironmentLightWire(
    ...
    [property: Key(16)] double IntensityValue) : IAppearanceWire<EnvironmentLightWire> {
...
public sealed record StageResultWire(
    ...
    [property: Key(11)] float Coverage) : IAppearanceWire<StageResultWire> {
    ...
    public static Fin<StageResult> Admit(StageResultWire wire, ModelCard card, Op key) =>
        ...
        from admitted in StageResult.Admit(
            new StageResult(stage, echoed, artefact, outputs, provider, wire.PartitionCount, wire.ElapsedMs, wire.GoldenDelta, wire.ParityFresh, wire.Coverage, wire.TilesEmitted, key), card, key)
        select admitted;
