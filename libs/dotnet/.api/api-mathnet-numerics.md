...
- Infinite-limit substitution lives on these three public facades alone; the direct double-exponential kernel below them feeds infinity into abscissa evaluation and yields `NaN` weights, so infinite bounds never bypass the facade.
...
[COMBINATORIC]: `Factorial` `FactorialLn` `Binomial(int, int) -> double` `BinomialLn` `Multinomial` `RisingFactorial` `FallingFactorial` `Harmonic` `GeneralHarmonic`
...
- The 19 members above (15 tapers plus the four `*Periodic` twins) are the DECOMPILE-VERIFIED complete `Window` surface — `Rasm.Compute` binds the four periodic factories directly, `Rasm` `Numerics/transform#WINDOW` owns only the absent Kaiser and Bohman algorithms, and `Rasm.Materials` `Raster/plane#TEXTURE_PYRAMID` owns its distinct polyphase windowed-sinc tap table — never a phantom `Window.Kaiser` spelling.
...
[STACKING]:
- The row-column 2D fold — 1D `Fourier.Forward`/`Inverse` per axis, the managed-total form the multidim provider gap forces — is OWNED at `Rasm/Numerics/transform#SPECTRAL` as `Spectral.Transform(Complex[], CellLattice, TransformDirection, FourierOptions)`; consumers project power, frequency axes, and pointwise products directly from the arrays and MathNet entrypoints they own.
