...
- Motion has two paths: canvas paint loop (`Animated<T>` + optional `Pacer`/`MotionClock`) vs CoreAnimation cosmetics (`CosmeticIntent` on `NSView.Layer`). Both honour `MotionAccessibility.ShouldReduceMotion`; spring integrator prefers `Pacer.LastFrameTimestamp` delta with `TimeProvider` fallback on first frame / MessageLoop. Cosmetics map through content→control locus before CALayer attach; decorative motion skips under reduce-motion or differentiate-without-color.
