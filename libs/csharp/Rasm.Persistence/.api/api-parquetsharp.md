...
[STACKING]:
- `Apache.Arrow`(`libs/csharp/.api/api-arrow.md`): Parquet ↔ `RecordBatch` is the load-bearing stack — the analytical lane reads a Parquet file as an `IArrowArrayStream`, feeds it to a DuckDB query or an ADBC consumer (`Apache.Arrow.Adbc`), and writes the result back through `Arrow.FileWriter`, one Arrow batch type crossing all three codecs.
