...
public sealed partial class SupportProjection {
    ...
    public static readonly SupportProjection Direction = Hit(accepts: DirectionOrVector,
        projectRaw: static s => DirectionOf(vector: s.Hit.Point - s.Sample, context: s.Context, key: s.Key));
    ...
    public static readonly SupportProjection Normal = Hit(accepts: DirectionOrVector,
        ...
        projectRaw: static s => s.Hit.Normal.ToFin(Fail: s.Key.InvalidResult()).Bind(n => DirectionOf(vector: n, context: s.Context, key: s.Key)));
    ...
    public static readonly SupportProjection SignedDistance = Hit(
        ...
        projectRaw: static s => guard(s.Space.SignedReach(hit: s.Hit), s.Key.Unsupported(s.Space.SourceType, typeof(double))).ToFin()
    ...
    public static readonly SupportProjection Tangent = Hit(accepts: DirectionOrVector,
        ...
        projectRaw: static s => s.Hit.Tangent.ToFin(Fail: s.Key.InvalidResult()).Bind(t => DirectionOf(vector: t, context: s.Context, key: s.Key)));
    ...
    [UseDelegateFromConstructor] private partial Fin<object> ProjectRaw((SupportSpace Space, ClosestHit Hit, Point3d Sample, Context Context, Op Key, Type Output) state);

    private bool Admits(SupportSpace space) => space.Capabilities.AdmitsAll(required: Requires);

    private static SupportProjection Hit(Func<Type, bool> accepts, Func<(SupportSpace Space, ClosestHit Hit, Point3d Sample, Context Context, Op Key, Type Output), Fin<object>> projectRaw,
    ...
    internal bool CanProject<TOut>(SupportSpace space) => Accepts(output: typeof(TOut)) && Admits(space: space);
    ...
    internal Fin<TOut> Project<TOut>(SupportSpace space, ClosestHit hit, Point3d sample, Context context, Op key) =>
        (hit.IsValid, Admits(space: space), Accepts(output: typeof(TOut))) switch {
            ...
            _ => ProjectRaw(state: (Space: space, Hit: hit, Sample: sample, Context: context, Key: key, Output: typeof(TOut)))
    ...
    private static Fin<object> DirectionOf(Vector3d vector, Context context, Op key) =>
        Numerics.Direction.Of(value: vector, context: context, key: key).Map(static direction => (object)direction);
}
```
