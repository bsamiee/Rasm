# [tailwindcss-react-aria-components] — react-aria-components interaction state as declarative Tailwind variants: one build-time plugin that turns every RAC render-prop state (`isSelected`→`data-selected`) into a `selected:` variant, so `className="selected:bg-accent pressed:bg-accent-strong"` replaces the render-prop `className={({isSelected})=>…}` branch

`tailwindcss-react-aria-components` is a build-time Tailwind CSS v4 plugin (zero runtime, no component import) that registers a variant for each state a react-aria-components primitive exposes. RAC renders its interaction state as DOM `data-*` attributes — `data-hovered`, `data-pressed`, `data-selected`, `data-focus-visible`, `data-disabled`, `data-open`, `data-entering`/`data-exiting`, and the enum attributes `data-placement`, `data-orientation`, etc. — and this plugin maps each to a Tailwind variant selecting that attribute, through one `addVariant` fold over a data table of `(attribute → selector)` pairs. The payoff is a collapse: RAC's render-prop `className`/`style` functions (`className={({isSelected, isPressed}) => isSelected ? "…" : "…"}`) become static, composable utility strings (`className="selected:… pressed:…"`), so state-conditional styling lives in the `token`/`view` class vocabulary and flows through the same `cn` merge rail as everything else. It is registered CSS-first in v4 via `@plugin "tailwindcss-react-aria-components"`, with an optional `prefix` to namespace the variants; unprefixed, the native-overlapping variants (`hover`/`focus`/`disabled`/…) are emitted as a unified `:is()` selector so they also apply to non-RAC native elements.
...
- peer: `tailwindcss@^4.0.0` (the folder's `tailwindcss@4`); v4 registers it CSS-first via `@plugin`, not a JS `plugins: []` array
...
- rail: types
- Each row is a *category* of the closed variant set the plugin registers, grouped as its source groups them; a variant name is what you write in `className`, and it selects the matching `data-*` attribute RAC emits. This is a data table fed to one `addVariant` loop, not a hand-written selector per state — a new RAC state is a new row upstream, not a new mechanism.

| [INDEX] | [VARIANT FAMILY]                                                                       | [STATE_KIND]   | [CONSUMER / BOUNDARY — the RAC render-prop it mirrors]           |
| :-----: | :------------------------------------------------------------------------------------- | :------------- | :--------------------------------------------------------------- |
...
| [INDEX] | [VARIANT FAMILY]                                                                       | [ENUM_KIND]    | [CONSUMER / BOUNDARY]                                     |
| :-----: | :------------------------------------------------------------------------------------- | :------------- | :-------------------------------------------------------- |
...
- The whole surface is a v4 `@plugin` directive in the folder's Tailwind entry CSS; there is no JS import. The `prefix` option namespaces every variant (`rac-selected:`) when a name would collide with a project convention.

| [INDEX] | [SURFACE]                                                                              | [ENTRY_FAMILY] | [CONSUMER / BOUNDARY]                                     |
| :-----: | :------------------------------------------------------------------------------------- | :------------- | :-------------------------------------------------------- |
|  [01]   | `@plugin "tailwindcss-react-aria-components";`                                          | registration   | the `token`/CSS entry — registers all boolean + enum variants; v4 CSS-first, not a JS config array |
...
- build-time and runtime-free: it is a Tailwind v4 plugin evaluated during CSS compilation. A component never imports it; it emits no JS. Registration is the CSS `@plugin` directive, and the `prefix` option is the only knob.
...
- Stack with `tailwindcss@4` (`.api/tailwindcss.md`): registered via the v4 `@plugin` directive in the `token` entry CSS; the variants compose with Tailwind's own (`selected:hover:…`, `md:pressed:…`) and with `group-*`/`peer-*` since RAC exposes group/peer data-attributes on parents.
...
[LOCAL_ADMISSION]:
- Register once via `@plugin` in the `token` entry CSS; never import the plugin from a component and never add it to a JS `plugins: []` array (v3 style) under `tailwindcss@4`.
...
[RAIL_LAW]:
- Package: `tailwindcss-react-aria-components`
- Owns: the build-time mapping of react-aria-components state to Tailwind variants — the boolean state variants (interactive/selection/overlay-motion/field/drag/structural), the `name-value` enum variants (`placement`/`orientation`/`selection`/`sort`/`resizable`/`type`/`layout`), the v4 `@plugin` registration + `prefix` option, and the unprefixed native-collapse selector emission
...
- Reject: importing the plugin from a component, a JS `plugins: []` registration under v4, render-prop `className`/`style` functions where a variant exists, hand-written enter/exit keyframe machines, and a needless `prefix` that breaks the native-collapse of `hover`/`focus`/`disabled`
