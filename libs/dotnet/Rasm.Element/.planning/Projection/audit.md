...
`ModelAudit` holds ZERO authority: it mutates nothing, mints no node, and returns no fault of its own — every verdict is data on the `ModelAudit`, so a model too broken to assemble still grades.
...
- [04]-[AUDIT_FOLD]: `AuditThresholds` the delivery-policy row, `ModelAudit` the graded value with its `Blocking`/`Tallies`/`Clears` derived reads, and `ModelAudit.Of` the entry that composes the census and every sweep into one graded value.
...
- Entry: the census is folded, never constructed — `[04]` `ModelAudit.Of` runs the one pass and `ModelAudit` carries the result.
- Auto: the population is the OCCURRENCE roster (`ObjectKind.Occurrence`), because a Type is a catalogue definition and grading it beside its instances double-counts the shared data it exists to share. Every `Discipline` row seeds at zero population, so a discipline no occurrence touches reads a vacuous full share rather than an absent key a gate must special-case. `Bake` refusing on an element contributes its verdict to the finding stream and no counter — an element that cannot be read cannot be graded.
- Output: `CoverageCensus` is the completeness half of the `ModelAudit`; a report reads the counts, a gate reads the shares, and neither re-walks the graph.
...
- Packages: LanguageExt.Core (`Seq`/`HashMap`/`Option`/`Fin` + the `Fold` state thread and the `ManyErrors` unpack), `Projection/projection#PROJECTION_CONTRACT` (`ConstraintSeverity` the shared grade), `Projection/address#CONTENT_ADDRESS` (`ContentAddress.OfGraph` the snapshot identity, `Verify` the drift census), `Graph/element#ELEMENT_GRAPH` (the accessor family and the memoized `Bake`).
...
- Boundary: `Clears` reads the `ModelAudit` alone, which is what makes it persistable beside the model; re-folding the graph to answer a question the audit already carries is the deleted form.
