...
The responder acts during an active operational condition, so the page supplies the exact response path, not background, severity routing, normal-task instruction, or a postmortem template. Local incident process carries severity names, response clocks, mutation permission, communication cadence, escalation thresholds, and profile tie-breakers. This standard uses SRE and incident-response sources as external sources for response shape, not as a universal severity taxonomy.
...
- Section cardinality: required response sections appear once; rollback/abort, communication, follow-up cleanup, and checklist appear only when they change responder action.
...
Use external incident-response sources for durable response principles, and use local operational truth for the values a responder invokes during an incident. Google SRE incident-response guidance supports clear response paths, communication, working records, impact assessment before or alongside mitigation, and mitigation before full root cause is known. PagerDuty incident-response guidance separates incidents, alerts, priorities, urgency, and provider notification behavior; a local incident process must map any provider terms into local response profiles before a runbook can use them. Evidence: [Google SRE incident management guide](https://sre.google/resources/practices-and-processes/incident-management-guide/) and [PagerDuty incidents](https://support.pagerduty.com/main/docs/incidents).
...
- List only prerequisites consumed during response: access, mutation permission, permissions, break-glass path, dashboards, diagnostic tools, known-good backups, evidence-preservation constraints, and a safe execution context.
...
|   [2]   | no                     |              yes               | Apply local mitigation; verify recovery signal          |
...
Standards and incident-process sources:
