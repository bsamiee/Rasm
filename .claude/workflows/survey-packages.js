...
const LAW = [
  ...
  'PYTHON RUNTIME GATE (binding when language=python): the workspace floor is CPython 3.15 (pyproject.toml requires-python >=3.15) on osx-arm64, and the heavy native substrates (GDAL/GEOS/PROJ/Arrow/HDF5/netCDF/tesseract/libvips and peers) are provisioned by Parametric_Forge, NOT pip-built. An ADD or REPLACE is admissible ONLY if it is (a) pure-Python or a cp315-clean wheel (abi3 cp3x<=15 or py3-none-any), OR (b) a source/native build the Forge scientific toolchain already owns, OR (c) correctly companion-banded with a python_version<3.15 environment marker exactly like the existing gated rows (matplotlib/pillow/lxml/lz4/vtk/scikit-image carry that marker precisely because no cp315 wheel ships yet). A package with NO cp315 wheel AND no Forge native substrate added WITHOUT the python_version<3.15 marker is REJECTED — it breaks the uv install that the ruff/ty verify gate does NOT exercise. Mirror the existing pyproject.toml marker + inline-comment convention for every add, and reject any legacy/abandoned distribution.',
