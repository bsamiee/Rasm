...
- target: MULTI-TARGET `lib/netstandard1.0` AND `lib/netstandard2.0` — the `net10.0` consumer binds the `netstandard2.0` asset (verified surface member from `netstandard2.0`); the two TFMs expose the SAME public surface (the floor difference is BCL, not API), so there is no consumer-TFM signature drift here
...
| [12] | `ConvexHullGenerationException` | the exceptional outcome | carries `Error` (`…Outcome`) + `ErrorMessage` — thrown only by the exception path; prefer the `Result.Outcome` fold |
...
- typed-result rail: `ConvexHull.Create*` returns a `ConvexHullCreationResult` carrying `Outcome` (`ConvexHullCreationResultOutcome` = `Success`/`DimensionSmallerTwo`/`DimensionTwoWrongMethod`/`NotEnoughVerticesForDimension`/`NonUniformDimension`/`DegenerateData`/`UnknownError`) + `ErrorMessage` — a TYPED ERROR RAIL, NOT an exception. The kernel reads `.Outcome` and folds to `Fin`/`Validation` at the boundary; `ConvexHullGenerationException` is the exception path, avoided in favour of the result fold.
...
[STACKING_LAW]:
- vs the vendored NURBS engine (`Parametric/nurbs`): the vendored algorithm set carries NO hull concern — MIConvexHull's typed-result `Create` (consumer `IVertex` + `Outcome` rail) is the kernel hull owner, and no parametric-tier page mints a second hull.
...
- Reject: calling the ND `Create` on 2D data (it errors `DimensionTwoWrongMethod` — use `Create2D`); ignoring `Result.Outcome` and reading a possibly-null `Result`, or taking the throwing `ConvexHullGenerationException` path instead of the outcome fold; minting any second hull beside this typed-result `Create` (the ONE kernel hull owner); using MIConvexHull for a CONSTRAINED 2D mesh (that is the kernel `Meshing/delaunay`) or a 2D edge-clipped Voronoi (that is `api-sharpvoronoilib`); touching the package's public-but-internal `FaceConnector`/`ObjectManager`/`IndexBuffer`/`ConvexFaceInternal` plumbing; expecting robust/exact arithmetic (it is `double`-precision Quickhull — near-degenerate input is `DegenerateData`, escalate to the kernel's exact-predicate ladder where robustness is required).
