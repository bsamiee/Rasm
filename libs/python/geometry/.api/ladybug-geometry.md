...
- marker: `python_version < '3.15'` — pure Python but pinned below cp315 by the honeybee/ladybug family floor; resolves only in the cp<3.15 companion environment, so it is absent from the default cp315 `assay api` distribution set
...
|  [04]   | `Polyline2D`   | 1d chain        | open/closed chain; `offset`, `smooth`, `to_polygon`, `join_segments` |
...
|  [05]   | `Polyline3D`   | 1d chain        | 3D chain; `from_polygon`, `to_polyline`, `remove_colinear_vertices`  |
...
|  [09]   | `Sphere` / `Cone` / `Cylinder` | analytic solid | parametric solids; `intersect_line_ray`/`intersect_plane`, `area`/`volume` |
...
|  [04]   | `Face3D.split_with_line(line, tol)` / `split_with_lines(...)` / `split_with_polyline(...)` / `split_with_plane(...)` / `split_through_holes(...)` | cutter geometry | carve a face into sub-faces                         |
...
|  [07]   | `bounding.bounding_box(geometries)` / `bounding_rectangle(...)` / `bounding_domain_x/y/z(...)` / `overlapping_bounding_boxes(geos, tol)` | geometry sequence | AABB extents + pairwise overlap broad-phase          |
|  [08]   | `triangulation.earcut(data, hole_indices=None, dim=2)` | flat coord list + holes   | ear-clipping triangulation (mapbox earcut port)      |
|  [09]   | `projection.project_geometry(plane, geometries)` / `project_geometry_2d(plane, geometries)` | plane + geometries | orthographic projection onto a plane                 |
...
|  [05]   | `util.coordinates_hash(point, tolerance)` / `coordinates_hash_3d(point, tolerance)` | point + tol | tolerance-bucketed coordinate hash for dedup/identity |
...
- `util.coordinates_hash(point, tolerance)` buckets coordinates to a tolerance before hashing, giving a geometry-stable key for vertex dedup and the runtime `ContentIdentity` seed (the same content-key pattern the C# `XxHash128` rail uses): tolerance-equal geometry shares a cache key so a memoized boolean/triangulation/projection result is reused without re-running the op.

## [05]-[IMPLEMENTATION_LAW]

[ENERGY_GEOMETRY_SUBSTRATE]:
- import: `import ladybug_geometry` (and the `geometry2d`/`geometry3d` submodules) at boundary scope only; module-level import is banned by the manifest import policy, and the companion-env marker means the import resolves only under cp<3.15.
...
- kernel axis: 2D boolean (`boolean.*`, `Polygon2D.boolean_*`, n-ary `_all` folds), intersection (`intersection2d`/`intersection3d` free functions), bounding broad-phase (`bounding.*`, `overlapping_bounding_boxes`), and `triangulation.earcut` are the package's own pure-Python kernels feeding the `triangulated_mesh*` properties; the owner uses them in place and routes only the heavy watertight-CSG/repair concerns to `manifold3d`/`trimesh`.
...
- boundary: ladybug-geometry owns the pure-Python planar/solid primitive algebra, its own 2D boolean/intersection/bounding/triangulation/projection kernels, the directed-graph network, OBJ/STL interop, and coordinate hashing. Robust watertight mesh CSG and repair route to `manifold3d`/`trimesh`; industrial planar operations to `shapely`; NURBS/BREP to `cadquery-ocp`/`rhino3dm`; mesh-file encode to the data `MeshPayload` codec; the honeybee/dragonfly/ladybug model semantics to those packages, which consume this substrate.
...
[CAPTURE_GAP]:
- members: verified by live introspection (`inspect`/`importlib`, the same machinery the `assay api` PYDIST arm uses) against an installed `ladybug-geometry==1.35.1` companion distribution, because the `python_version < '3.15'` marker excludes the package from the default cp315 `assay api` resolution set (an `api resolve --key ladybug-geometry` returns `unsupported` under cp315). Every documented type, classmethod, property, and module function resolves with the signatures shown — no phantom. Confirmed shapes: `geometry_dict_to_object(ladybug_geom_dict, raise_exception=True)`; `triangulation.earcut(data, hole_indices=None, dim=2)`; `boolean.union(poly1, poly2, tolerance)` (free functions take/return raw point-tuple lists, the `Polygon2D.boolean_*` mirrors take/return `Polygon2D`); `Mesh3D.to_obj(folder, name, include_colors=True, include_normals=False, triangulate_quads=False, include_mtl=False)`; `OBJ(vertices, faces, vertex_texture_map=None, vertex_normals=None, vertex_colors=None, material_structure=None)` with `from_mesh3ds`; `Face3D.sub_faces_by_ratio(ratio)`; `Sun.sun_vector` confirmed to return `ladybug_geometry.geometry3d.pointvector.Vector3D`. License confirmed AGPL-3.0 from distribution metadata.
