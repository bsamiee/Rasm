...
- Cases: `VectorMetric` l2 | cosine | inner-product | l1 | hamming | jaccard; `FullTextMode` match | prefix | phrase | websearch
...
- Boundary: one query shape per search concern projects to every provider — provider-twin query shapes are the deleted pattern; a `VectorMetric.Bit` false row carries the catalogued distance operator the server projects through `L2Distance`, `CosineDistance`, `MaxInnerProduct`, and `L1Distance`, while a `Bit` true row projects the binary-vector distance through the catalogued `HammingDistance` and `JaccardDistance` `DbFunctions` over a `bit`-typed column so the metric axis stays one homogeneous fold and a parallel bit-query shape is the deleted form — its raw `Op` operator spelling stays a placeholder traceable to `[BIT_VECTOR_COLUMNS]` because the bit-operator strings are not catalogued beside the confirmed `DbFunctions` projection; the embedded vector route stays gate-resolved with the brute-force scan as the always-present correctness baseline; fuzzy admission normalizes through unaccent and prefilters through trigram similarity before rank; the embedded `Regex.IsMatch` predicate translates through `SqliteRegexMethodTranslator` so a LINQ regex filter projects to the sqlite `REGEXP` operator instead of client evaluation.
...
|   [7]   | mapped receipt staging    | `CreateAppender<T,TMap>` over `DuckDBMappedAppender` / `DuckDBAppenderMap`; `AppendRecords` then `Close` | derived rollup staging only; record-table drift fails at `DuckDBAppenderMap` construction; `AppendValue`/`AppendNullValue` write a typed cell or a null and an engine-default cell rides the `AppendDefault` member held on `[DUCKDB_FAULT_KEY]`; `Clear` discards a pending batch before a mid-run abort; the batch flushes at the vector quantum and `Close` seals the run; a row-at-a-time appender open is rejected |
|   [8]   | in-process sequences      | `RegisterScalarFunction` / `RegisterTableFunction` with `CardinalityHint` and `ScalarFunctionOptions`    | a managed sequence participates as a relation with planner cardinality instead of a single-query staging table; `ScalarFunctionOptions` declares purity and null-handling; a bound `DuckDBParameter` pins its `DbType`                                                                                                                                                       |
...
- [REGISTER_TABLE_FUNCTION]: managed `RegisterTableFunction` chunk-fill contract under streaming-mode consumption — vector-fill arity and `CardinalityHint` accuracy effect on join-plan selection; `ScalarFunctionOptions` purity-flag effect on engine constant-folding for a registered deterministic scalar.
- [DUCKDB_FAULT_KEY]: the `DuckDBException` discriminating member (the `ErrorType`-keyed native fault class) the `Fault` arm folds beyond the catalogued exception type, the `Clear` abort-state contract on a partially-appended `DuckDBMappedAppender`, and the `AppendDefault` member spelling on the appender row that writes a column's engine default beyond the catalogued `AppendValue`/`AppendNullValue` surface, resolved against the installed DuckDB.NET assembly before the staging row consumes the default-write form.
- [SEP_OPTION_BINDING]: the `SepReaderOptions`/`SepWriterOptions` property spelling that binds the `SepColNotSetOption` unset policy and the exact `Cols.Select` range-parse and zero-row `SepWriter` header call shapes, beyond the catalogued `SepColNotSetOption` type and `Cols.Select` surface.
...
- [BIT_VECTOR_COLUMNS]: the `Pgvector` `halfvec`, `sparsevec`, and `bit` column-type mappings and the `binary_quantize` expression-index form that feed the `Hamming`/`Jaccard` `VectorMetric` rows, and the `<~>` / `<%>` server distance-operator spellings the `Bit` rows project, beyond the catalogued `HammingDistance` / `JaccardDistance` `DbFunctions` extensions.
