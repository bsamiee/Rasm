...
A skill loads in two stages: the description enters every session at startup, and the body enters only after selection — so the description is an activation boundary, and every body line competes with the task's own context once loaded. The register, fact law, and defect catalog bind skill prose in full; the classes here are the failures only a skill bundle can commit, and findings cite class and line.

## [01]-[ALTITUDE]

A capability lands at the lowest bundle tier that owns it: material every activation needs rides the root, material one branch needs rides one hop down, and deterministic mechanics ride a script the root invokes.

- [DESCRIPTION]: The owned deliverable, the concrete objects and verbs that select the skill, and one negative boundary — third person, nothing else.
- [ROOT]: `SKILL.md` routes the common workflow and states hard law; depth lives one hop down, never inline.
- [REFERENCES]: Deep doctrine one hop from the root, each route labeled by the task that opens it.
- [EXAMPLES]: Pressure cases at real composed scale, symptom-indexed with the fix demonstrated.
- [SCRIPTS]: Deterministic checks, conversions, and extractions; the root carries only the invocation contract and the gate it satisfies.
- [TEMPLATES]: File-kind instances copied verbatim with slot fills.
- [ASSETS]: Binary and data material scripts consume, never read into context.

Platform loading mechanics, field limits, and install surfaces belong to the platform spec; a skill restates none of them.
...
The root is a router under budget; everything an activation pays for earns its cost on the common path.
...
- Each skill owns one deliverable kind and names in its description the adjacent deliverable it refuses; two skills matching one prompt is a boundary defect on both.
- Shared doctrine has one owning skill and every sibling composes it silently; a policy sentence or marker vocabulary spelled in two bundles is `TWIN_TRUTH`.
- A mixed task routes by deliverable, never by topic: prose to the prose owner, fences to the fence owner, artifacts to the artifact owner, in sequence.
