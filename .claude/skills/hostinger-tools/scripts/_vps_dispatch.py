...
VPS_HANDLERS: Final[dict[str, Handler]] = {
    ...
    "vps-start": (lambda args: ("POST", f"/api/vps/v1/virtual-machines/{args['id']}/start", None), _action_fmt("started")),
    "vps-stop": (lambda args: ("POST", f"/api/vps/v1/virtual-machines/{args['id']}/stop", None), _action_fmt("stopped")),
    "vps-restart": (lambda args: ("POST", f"/api/vps/v1/virtual-machines/{args['id']}/restart", None), _action_fmt("restarted")),
    "vps-metrics": (
        lambda args: ("GET", f"/api/vps/v1/virtual-machines/{args['id']}/metrics?date_from={args['from_date']}&date_to={args['to_date']}", None),
        _item_fmt("metrics"),
    ),
    "vps-actions": (lambda args: ("GET", f"/api/vps/v1/virtual-machines/{args['id']}/actions", None), _list_fmt("actions")),
    "vps-action-view": (
    ...
    "vps-hostname-reset": (lambda args: ("DELETE", f"/api/vps/v1/virtual-machines/{args['id']}/hostname", None), _action_fmt("reset")),
    "vps-nameservers-set": (
        lambda args: (
            "PUT",
    ...
    "vps-recovery-stop": (lambda args: ("DELETE", f"/api/vps/v1/virtual-machines/{args['id']}/recovery", None), _action_fmt("stopped")),
    "vps-recreate": (
        lambda args: (
            "POST",
            f"/api/vps/v1/virtual-machines/{args['id']}/recreate",
            {"template_id": int(args["template_id"]), **({"password": args["password"]} if args.get("password") else {})},
        ),
    ...
    "firewall-rule-create": (
        lambda args: (
            ...
            {"protocol": args["protocol"], "port": args["port"], "source": args["source"], "source_detail": args["source_detail"]},
        ),
    ...
    "ssh-key-attached": (lambda args: ("GET", f"/api/vps/v1/virtual-machines/{args['vps_id']}/public-keys", None), _list_fmt("keys")),
    # --- SCRIPTS ---
    ...
    "script-delete": (lambda args: ("DELETE", f"/api/vps/v1/post-install-scripts/{args['id']}", None), _action_fmt("deleted")),
    # --- REFERENCE ---
