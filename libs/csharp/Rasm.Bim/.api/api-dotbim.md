...
|  [02]   | `Mesh`     | interchange | shared geometry: `MeshId` (`"mesh_id"`, the pool key), `Coordinates` (`List<double>`, flat XYZ triples), `Indices` (`List<int>`, triangle vertex indices) |
...
[COLOR_LAW]:
- `Color` channels are `int` validated to `0..255` at the property setter (out-of-range throws). Element-level `Color` is the whole-object color; `Element.FaceColors` is the optional per-face override stream (RGBA quadruples aligned to the mesh faces) and wins over `Color` where present.
