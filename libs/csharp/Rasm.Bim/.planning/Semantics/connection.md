...
public static class ConnectionProjection {
    ...
    public static PropertyBag Detail(IfcElement realizing) {
        ...
        return detail with { Values = detail.Values.Filter(static v => v switch {
            PropertyValue.Measure m => double.IsFinite(m.Value.Si),
            PropertyValue.Text t    => t.Value.Length > 0,
            _                       => true,
        }) };
    }
    ...
    static PropertyBag AccessoryDetail(IfcDiscreteAccessory accessory) {
        ...
        PropertyBag detail = Rows(Joint("Bolted"), Token(DetailSchema.AccessoryType, accessory.PredefinedType.ToString()));
        return fasteners switch {
            { IsEmpty: true } => detail,
