...
- Auto: `Classify` matches the trimmed code against the row's `CodeShape` regex (the actual shape enforcement the migration source's decorative `ClassificationCode` never applied) and, on a match, mints the seam `Classification.Create(Key, code, None)` (the author path carries no resolved title; the seam `[ComplexValueObject]` 3-member factory normalizing the system token and trimming the code), so a `"Ss_25_10_30"` lowers onto a `Classification("uniclass2015", "Ss_25_10_30")` whose seam `Parent`/`Within` projections derive the containment hierarchy; a code the shape rejects faults rather than lowering a malformed value. `Ingest` reverses the egress through `SystemOf`/`CodeOf`: it reads the `Name` off the root `IfcClassification` (walking any nested `IfcClassificationReference` chain) plus the `Location`, matches a roster row by title equality or dictionary-URI prefix, and `URI`-unescapes the `Location` trailing segment when the `Identification` is absent; `TitleOf` reads the reference's OWN `IfcClassificationReference.Name` (distinct from the root dictionary `Name` `SystemOf` matches on) as the resolved concept `Title` the seam `Classification.Title` carries, so the resolved-name read is live at the only path that can populate it rather than perpetually `None` at IFC ingest. `Author` resolves the row through the generated `TryGet` and authors the reference with the URI-escaped `Location` (`ClassUri`) so an OmniClass/MasterFormat code carrying spaces round-trips; the bSDD class-to-property mapping resolves separately through the `[03]-[BSDD_RESOLUTION]` owner.
- Packages: GeometryGymIFC_Core, Rasm.Element, Thinktecture.Runtime.Extensions, LanguageExt.Core, Rasm
...
public sealed partial class ClassificationSystem {
    ...
    // node carries — the actual shape enforcement, then the seam factory. The typed UnmappedClass case lifts BARE off the
    ...
    public Fin<Classification> Classify(string code, Op key) =>
        CodeShape.IsMatch(code.Trim())
            ? Fin.Succ(Classification.Create(Key, code, None))
    ...
    // Object node value (the inverse of Author, the leg the migration source never had). The system resolves off the
    // reference's ReferencedSource dictionary Name OR its Location identifier-URI prefix against the roster; None for an
    // unrostered source so a foreign system rides the Generic passthrough rather than a wrong lowering, and a blank code
    ...
    public static Option<Classification> Ingest(IfcClassificationReference reference) =>
        SystemOf(reference).Bind(system =>
            CodeOf(reference) is { Length: > 0 } code
                ? Some(Classification.Create(system.Key, code, TitleOf(reference)))
                : None);

    // The dictionary system resolves off the ROOT IfcClassification title OR the Location identifier-URI prefix. A nested
    // IfcClassificationReference (the IFC hierarchical-classification pattern) points its ReferencedSource at a PARENT
    // reference, not the dictionary, so RootSource walks the IfcClassificationReferenceSelect chain to the root before
    // reading its Name — a flat `as IfcClassification` would read "" on a nested ref and silently miss the dictionary.
    static Option<ClassificationSystem> SystemOf(IfcClassificationReference reference) {
        string title = RootSource(reference)?.Name ?? "";
        string location = reference.Location ?? "";
        return Optional(Items.FirstOrDefault(row =>
            string.Equals(row.Title, title, StringComparison.OrdinalIgnoreCase)
    ...
    // hierarchy to the root dictionary; the depth bound makes a malformed cyclic chain terminate rather than spin.
...
// the IFC DataType + Pset placement the Semantics/properties#PROPERTY_SETS PropertyKey threads into the seam PropertyValue,
